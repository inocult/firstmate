---
name: overwatch
description: Enable, pause or inspect automatic pickup of ready-for-agent Plane tickets in a configured Firstmate home, and handle its registered watcher events.
metadata:
  internal: true
---
...
`/overwatch on` enables automatic implementation only for the project/repository/executor in this home's confirmed `config/plane.json`.
If no scope has been configured, discover it through the Plane setup procedure and resolve the intended project before enabling.
Treat `/overwatch` without an action as status/help, not an instruction to start new work.
Use `doctor` to verify the readiness label, pickup states and lifecycle mappings before `on`.
...
On `overwatch:` wakes, heartbeats or task completion, reconcile existing Plane executions and worker/PR state first.
...
If configuration changed, the budget is exhausted, or MCP/registry state is uncertain, run `off` and report the specific blocker once.

Count all this home's nonterminal Plane executions toward the slot limit, including reservations, held work and PRs waiting for review.
...
Breach must win the adapter claim before dispatch; a competing claim means skip that ticket, not an error permitting duplicate work.
