...
4. Provision the vocabulary, then complete the configuration.
   ...
   Compare all five canonical names against every label `doctor` returned, and settle that whole comparison before writing anything.
   A label is a conflict when it normalizes onto a canonical name without being exactly it, comparing with case, separators and surrounding whitespace ignored: `Ready for agent`, `ready_for_agent` and `Ready-For-Agent` are each a conflict against `ready-for-agent`. Adoption stays exact - only the exact canonical name is ever the role.
   ...
   Only once the comparison is clean, create the roles the project does not carry, using the wording above, then re-run `doctor` to pick up the new identifiers.
...
If any of the five canonical roles is unresolved for any reason - a conflict the captain has not settled, a failed connector precondition, an identifier that did not verify - the home half does not complete and no ticket can be claimed.
