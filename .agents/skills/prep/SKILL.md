...
4. Provision the vocabulary, then complete the configuration.
   Compare all five canonical names against every label `doctor` returned, and settle that whole comparison before writing anything. The comparison is read-only and needs no write surface.
   ...
   Without one - none available, one that resolves elsewhere, or one that cannot be compared - create nothing, and neither guess nor silently skip: halt the home half and report just the roles that are missing, with the meanings fixed above, for the captain to create in Plane by hand. A home on any harness then re-enters at this step, where the comparison finds them present and prepping continues.
...
5. Verify, then report exactly how far that verification reaches.
   First confirm all five canonical names appear exactly in the `labels` array of the most recent `doctor` run - the re-run after a create, or the step 3 run when nothing needed creating. The completion gate covers the whole set while only `ready_label_id` is stored, so verifying stored identifiers alone would leave four roles unchecked: a create that failed, was rate-limited, or returned something read as "already exists" would reach the project's committed docs as a role the tracker does not carry.
