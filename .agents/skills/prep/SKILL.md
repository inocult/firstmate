...
4. Provision the vocabulary, then complete the configuration.
   Provisioning requires a confirmed Plane write surface resolving to the same workspace and the same project as `FM_HOME/config/plane.json`. That is a precondition, not a caution.
   ...
   If no write surface is confirmed - none available, one that resolves elsewhere, or one that cannot be compared - create nothing, and neither guess nor silently skip. Halt the home half and report the exact canonical labels the captain must create in Plane, with the meanings fixed above, so a home on any harness can still be brought to a prepped state by hand and re-enter at this step.
