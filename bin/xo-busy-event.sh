#!/usr/bin/env bash
# xo-busy-event.sh - the ONLY writer of the semantic busy-state contract
# owned by bin/xo-busy-lib.sh (record format, gen binding, and classification
...
#       the record at seq=1 (default: busy, source xo-spawn, event
...
#       Claude xo-send --key Escape path (xo-interrupt) and xo recovery
#       paths (xo-recovery) may pass --current-gen to bind to the incarnation
#       armed right now.
#
...
usage() {
  cat >&2 <<'EOF'
usage:
  xo-busy-event.sh arm <state-dir> <id> [--state busy|idle|unknown] [--source S] [--event E]
  xo-busy-event.sh apply <state-dir> <id> <busy|idle|unknown> (--gen G | --current-gen) --source S --event E
  xo-busy-event.sh progress <state-dir> <id> --gen G
  xo-busy-event.sh retire <state-dir> <id> (--gen G | --current-gen)
...
if [ "$CMD" = apply ] || [ "$CMD" = arm ]; then
  ...
  xo_busy_token_valid "$SOURCE" || { echo "error: invalid --source" >&2; exit 1; }
  xo_busy_token_valid "$EVENT" || { echo "error: invalid --event" >&2; exit 1; }
fi

[ "$CMD" != progress ] || [ "$USE_CURRENT_GEN" = 0 ] || usage

REC=$(xo_busy_record_path "$STATE" "$ID")
GEN_FILE=$(xo_busy_gen_path "$STATE" "$ID")
LOCK="$REC.lock"
...
# exactly as bin/xo-watch.sh does.
...
# pair; a holder that died mid-write is broken after XO_BUSY_LOCK_STALE_SECS.
lock_acquire() {
  ...
  while ! mkdir "$LOCK" 2>/dev/null; do
    ...
    if [ "$tries" -ge 40 ]; then
      ...
      # aborting the writer - and its caller, xo-teardown.sh - under `set -u`.
      case "$mtime" in ''|*[!0-9]*) mtime=$now ;; esac
      age=$((now - mtime))
      if [ "$age" -ge "${XO_BUSY_LOCK_STALE_SECS:-5}" ]; then
...
if [ "$USE_CURRENT_GEN" = 1 ] && [ "$CMD" != retire ]; then
  GEN=$(xo_busy_current_gen "$STATE" "$ID") || {
    umask "$old_umask"
...
if [ "$USE_CURRENT_GEN" != 1 ] || [ "$CMD" != retire ]; then
  xo_busy_token_valid "$GEN" || { umask "$old_umask"; echo "error: invalid --gen" >&2; exit 1; }
fi

lock_acquire || { umask "$old_umask"; exit 1; }
CURRENT=$(xo_busy_current_gen "$STATE" "$ID") || {
