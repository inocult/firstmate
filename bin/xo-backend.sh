#!/usr/bin/env bash
# xo-backend.sh - runtime-backend selection, meta helpers, selector resolution,
# and dispatch for xo's session-provider abstraction.
#
# Design: data/xo-backend-design-d7/report.md ("Backend Interface") and
# data/xo-backend-design-d7/herdr-addendum.md ("Events as the core
# abstraction"). P1 extracted the tmux command sequences that xo-send.sh,
# xo-peek.sh, xo-watch.sh, xo-spawn.sh, and xo-teardown.sh already ran inline
...
# EXPERIMENTAL spawn-capable backend behind `--backend herdr`/`XO_BACKEND=herdr`/
# `config/backend`, and behind runtime auto-detection when xo itself is
# running inside herdr with no explicit backend setting; see herdr-addendum.md and
# data/xo-backend-design-d7/herdr-verification-p2.md for its empirical basis.
# P3 adds bin/backends/zellij.sh, also EXPERIMENTAL and spawn-capable, behind
# `--backend zellij`/`XO_BACKEND=zellij`/`config/backend` - NOT behind runtime
...
# EXPERIMENTAL and spawn-capable, behind `--backend cmux`/`XO_BACKEND=cmux`/
# `config/backend`, and behind runtime auto-detection when xo itself is
...
# treats that as `tmux` (xo_backend_of_meta), and xo-spawn.sh does not write
...
# task events (status-changed, went-stale, exited) that map onto xo's
# existing signal/stale/check/heartbeat wake vocabulary. The tmux adapter has
# no native event push, so xo-watch.sh's poll loop over the pull primitives
...
# on their own for on-demand reads (xo-peek.sh, xo-crew-state.sh).

XO_BACKEND_SCRIPT=${BASH_SOURCE[0]:-$0}
XO_BACKEND_LIB_DIR="$(cd "$(dirname "$XO_BACKEND_SCRIPT")" && pwd)"
unset XO_BACKEND_SCRIPT
XO_BACKEND_DEFAULT_ROOT="$(cd "$XO_BACKEND_LIB_DIR/.." && pwd)"
XO_ROOT="${XO_ROOT_OVERRIDE:-${XO_ROOT:-$XO_BACKEND_DEFAULT_ROOT}}"
XO_HOME="${XO_HOME:-${XO_ROOT_OVERRIDE:-$XO_ROOT}}"
XO_BACKEND_CONFIG_DIR="${XO_CONFIG_OVERRIDE:-$XO_HOME/config}"
...
# data/xo-backend-design-d7/herdr-addendum.md) - verified against the real
# v0.7.1/protocol-14 binary (data/xo-backend-design-d7/herdr-verification-p2.md)
# but newer than tmux's long-proven default path. zellij is EXPERIMENTAL (P3;
# data/xo-backend-design-d7/report.md "Zellij Backend") - verified against the
...
XO_BACKEND_KNOWN="tmux herdr zellij orca cmux"
XO_BACKEND_SPAWN="tmux herdr zellij orca cmux"

# xo_backend_list_contains: whitespace-delimited membership without relying on
# shell word splitting. xo-backend.sh is normally sourced by bash scripts, but
# zsh diagnostics can source it too, so backend-name matching must stay portable.
xo_backend_list_contains() {  # <list> <name>
  local list=$1 name=$2
  case "$name" in
    *[[:space:]]*) return 1 ;;
  esac
...
xo_backend_is_known() {  # <name>
  xo_backend_list_contains "$XO_BACKEND_KNOWN" "$1"
}

# xo_backend_detect: detect the runtime xo itself is CURRENTLY executing
# inside, from verified environment markers (mirrors bin/xo-harness.sh's
...
# before exec'ing the real binary - so a claude-harness xo launched in
...
# Callers needing the winning signal read XO_BACKEND_DETECT_SIGNAL (set to
# TMUX, HERDR_ENV, CMUX_WORKSPACE_ID, bundle-id, or ancestry) and
# XO_BACKEND_DETECTED after a direct (non-command-substitution) call.
XO_BACKEND_CMUX_BUNDLE_ID="com.cmuxterm.app"

xo_backend_detect() {
  XO_BACKEND_DETECTED=""
  XO_BACKEND_DETECT_SIGNAL=""
  if [ -n "${TMUX:-}" ]; then
    XO_BACKEND_DETECTED=tmux
    XO_BACKEND_DETECT_SIGNAL=TMUX
    printf 'tmux'
    return 0
  fi
  if [ "${HERDR_ENV:-}" = "1" ]; then
    XO_BACKEND_DETECTED=herdr
    XO_BACKEND_DETECT_SIGNAL=HERDR_ENV
    printf 'herdr'
    return 0
  fi
  if [ -n "${CMUX_WORKSPACE_ID:-}" ]; then
    XO_BACKEND_DETECTED=cmux
    XO_BACKEND_DETECT_SIGNAL=CMUX_WORKSPACE_ID
    printf 'cmux'
    return 0
  fi
  if xo_backend_detect_cmux_fallback; then
    XO_BACKEND_DETECTED=cmux
    printf 'cmux'
    return 0
  fi
...
# xo_backend_detect_cmux_fallback: the two macOS-only cmux fallback signals
# (see xo_backend_detect's header comment). Sets XO_BACKEND_DETECT_SIGNAL to
...
xo_backend_detect_cmux_fallback() {
  [ "$(uname 2>/dev/null)" = Darwin ] || return 1
  if [ "${__CFBundleIdentifier:-}" = "$XO_BACKEND_CMUX_BUNDLE_ID" ]; then
    XO_BACKEND_DETECT_SIGNAL=bundle-id
    return 0
  fi
  if xo_backend_detect_cmux_app_is_ancestor; then
    XO_BACKEND_DETECT_SIGNAL=ancestry
    return 0
  fi
  return 1
}

# xo_backend_detect_cmux_app_pid: the running cmux app's pid, resolved by
...
xo_backend_detect_cmux_app_pid() {
  command -v lsappinfo >/dev/null 2>&1 || return 1
  local out pid
  out=$(lsappinfo info -only pid -app "$XO_BACKEND_CMUX_BUNDLE_ID" 2>/dev/null) || return 1
  pid=${out##*=}
...
# xo_backend_detect_cmux_app_is_ancestor: walk this process's parent chain and
...
xo_backend_detect_cmux_app_is_ancestor() {
  local cmux_pid pid ppid comm hops=0
  cmux_pid=$(xo_backend_detect_cmux_app_pid) || cmux_pid=""
  pid=$$
  while [ "$hops" -lt 32 ]; do
...
# xo_backend_name: resolve the ACTIVE backend for a NEW spawn, absent an
# explicit per-task override. Precedence: XO_BACKEND env, then config/backend
# (a single word on its first non-empty line, mirroring config/crew-harness),
# then runtime auto-detection (xo_backend_detect), then default tmux. A
# per-task `--backend` flag is parsed by the caller (xo-spawn.sh) and takes
...
xo_backend_name() {
  ...
  if [ -n "${XO_BACKEND:-}" ]; then
    printf '%s' "$XO_BACKEND"
    return 0
  fi
  if [ -f "$XO_BACKEND_CONFIG_DIR/backend" ]; then
  ...
  if xo_backend_detect >/dev/null; then
    ...
    if [ "$detected" = cmux ]; then
      case "$XO_BACKEND_DETECT_SIGNAL" in
        bundle-id) marker="FALLBACK signal __CFBundleIdentifier=$XO_BACKEND_CMUX_BUNDLE_ID; CMUX_WORKSPACE_ID absent, stripped by cmux's bundled claude wrapper" ;;
...
# xo_backend_validate: refuse an unknown backend LOUDLY. Silent on success.
xo_backend_validate() {  # <name>
  local name=$1
  if ! xo_backend_is_known "$name"; then
    echo "error: unknown backend '$name' (known: $XO_BACKEND_KNOWN)" >&2
    return 1
  fi
  return 0
}

xo_backend_validate_spawn() {  # <name>
  local name=$1
  xo_backend_validate "$name" || return 1
  xo_backend_list_contains "$XO_BACKEND_SPAWN" "$name" && return 0
  echo "error: backend '$name' does not support task spawning yet (spawn-supported: $XO_BACKEND_SPAWN)" >&2
  return 1
}

# xo_backend_required_tools: the backend-SPECIFIC CLI tools an xo home on
# <backend> genuinely requires, beyond xo's universal toolchain (owned by
...
#     tool check, e.g. xo_backend_herdr_tool_check);
...
xo_backend_required_tools() {  # <backend>
  case "$1" in
...
xo_backend_required_tool_available() {  # <backend> <tool>
  local backend=$1 tool=$2 required
  required=$(xo_backend_required_tools "$backend") || return 1
  xo_backend_list_contains "$required" "$tool" || return 1
  case "$backend:$tool" in
    cmux:cmux)
      xo_backend_source cmux >/dev/null 2>&1 || return 1
      xo_backend_cmux_bin >/dev/null 2>&1
      ;;
    *) command -v "$tool" >/dev/null 2>&1 ;;
  esac
}

# xo_meta_get: the LAST value of `key=` in <meta-file>, or empty (never
# errors) if the file or key is absent. Mirrors the ad hoc `grep '^key=' |
# tail -1 | cut -d= -f2-` snippet every xo-*.sh script used to repeat inline.
xo_meta_get() {  # <meta-file> <key>
...
# xo_backend_of_meta: the backend recorded in <meta-file>, defaulting to
# `tmux` when the field is absent - the P1 compatibility contract.
xo_backend_of_meta() {  # <meta-file>
  local v
  v=$(xo_meta_get "$1" backend)
  printf '%s' "${v:-tmux}"
}

xo_backend_target_of_meta() {  # <meta-file>
...
# xo_backend_validate_task_endpoint: validate a task cleanup record entirely
...
# valid only when their window name itself is exactly xo-<task-id>.
# On success, sets XO_BACKEND_VALIDATED_BACKEND and
# XO_BACKEND_VALIDATED_TARGET. On failure, prints one refusal and returns 1.
xo_backend_meta_exact_value() {  # <meta-file> <key>
...
xo_backend_endpoint_atom_valid() {  # <value>
  case "$1" in
    ''|*[!A-Za-z0-9._@%+-]*) return 1 ;;
  esac
}

xo_backend_validate_task_endpoint() {  # <meta-file> <task-id>
  local meta=$1 id=$2 backend_count backend window worktree project binding_count binding
  local session pane recorded_session workspace tab terminal worktree_id surface
  XO_BACKEND_VALIDATED_BACKEND=
  XO_BACKEND_VALIDATED_TARGET=
  [ -f "$meta" ] && [ ! -L "$meta" ] || {
  ...
  case "$backend_count" in
    0) backend=tmux ;;
    1) backend=$(xo_backend_meta_exact_value "$meta" backend) || backend= ;;
    *) backend= ;;
  esac
  if [ -z "$backend" ] || ! xo_backend_is_known "$backend"; then
  ...
  case "$binding_count" in
    0) binding= ;;
    1)
      binding=$(xo_backend_meta_exact_value "$meta" endpoint_task_id) || {
  ...
  case "$backend" in
    ...
    herdr)
      ...
      recorded_session=$(xo_backend_meta_exact_value "$meta" herdr_session) || recorded_session=
      workspace=$(xo_backend_meta_exact_value "$meta" herdr_workspace_id) || workspace=
      tab=$(xo_backend_meta_exact_value "$meta" herdr_tab_id) || tab=
      pane=$(xo_backend_meta_exact_value "$meta" herdr_pane_id) || pane=
      if [ -z "$recorded_session" ] || [ -z "$workspace" ] || [ -z "$tab" ] || [ -z "$pane" ] \
        || [ "$window" != "$recorded_session:$pane" ] \
        || ! xo_backend_endpoint_atom_valid "$recorded_session" \
        || ! xo_backend_endpoint_atom_valid "$workspace" \
        || ! xo_backend_endpoint_atom_valid "${tab//:/_}" \
        || ! xo_backend_endpoint_atom_valid "${pane//:/_}"; then
    ...
    zellij)
      ...
      recorded_session=$(xo_backend_meta_exact_value "$meta" zellij_session) || recorded_session=
      tab=$(xo_backend_meta_exact_value "$meta" zellij_tab_id) || tab=
      pane=$(xo_backend_meta_exact_value "$meta" zellij_pane_id) || pane=
      case "$tab:$pane" in *[!0-9:]*) tab= ;; esac
      if [ -z "$recorded_session" ] || [ -z "$tab" ] || [ -z "$pane" ] \
        || [ "$window" != "$recorded_session:$pane" ] \
        || ! xo_backend_endpoint_atom_valid "$recorded_session"; then
    ...
    orca)
      ...
      terminal=$(xo_backend_meta_exact_value "$meta" terminal) || terminal=
      worktree_id=$(xo_backend_meta_exact_value "$meta" orca_worktree_id) || worktree_id=
      [ -n "$terminal" ] || {
      ...
      if [ "$window" != "xo-$id" ] \
        || ! xo_backend_endpoint_atom_valid "$terminal" \
        || ! xo_backend_endpoint_atom_valid "$worktree_id"; then
    ...
    cmux)
      ...
      workspace=$(xo_backend_meta_exact_value "$meta" cmux_workspace_id) || workspace=
      surface=$(xo_backend_meta_exact_value "$meta" cmux_surface_id) || surface=
      if [ -z "$workspace" ] || [ -z "$surface" ] || [ "$window" != "$workspace:$surface" ] \
        || ! xo_backend_endpoint_atom_valid "$workspace" \
        || ! xo_backend_endpoint_atom_valid "$surface"; then
  ...
  XO_BACKEND_VALIDATED_BACKEND=$backend
  # shellcheck disable=SC2034 # Output globals are consumed by sourcing callers.
  XO_BACKEND_VALIDATED_TARGET=$window
  return 0
}

xo_backend_meta_for_window() {  # <target> <state-dir>
...
xo_backend_task_id_for_selector() {  # <raw-target> <state-dir>
...
xo_backend_meta_for_selector() {  # <raw-target> <state-dir>
  local raw=$1 state=$2 id
  id=$(xo_backend_task_id_for_selector "$raw" "$state") || return 1
  printf '%s/%s.meta' "$state" "$id"
}

xo_backend_of_selector() {  # <raw-target> <resolved-target> <state-dir>
  local raw=$1 resolved=$2 state=$3 meta
  meta=$(xo_backend_meta_for_selector "$raw" "$state" 2>/dev/null || true)
  [ -n "$meta" ] && { xo_backend_of_meta "$meta"; return 0; }
  if [ -n "$resolved" ]; then
    meta=$(xo_backend_meta_for_window "$resolved" "$state" 2>/dev/null || true)
    [ -n "$meta" ] && { xo_backend_of_meta "$meta"; return 0; }
  fi
  printf 'tmux'
}

xo_backend_expected_label_of_selector() {  # <raw-target> <state-dir>
  local raw=$1 state=$2 id
  id=$(xo_backend_task_id_for_selector "$raw" "$state" 2>/dev/null || true)
  [ -n "$id" ] && printf 'xo-%s' "$id"
  return 0
}

# xo_backend_source: source the named backend's adapter file, once per shell.
...
xo_backend_source() {  # <name>
  local name=$1
  xo_backend_validate "$name" || return 1
  case "$name" in
    tmux)
      if [ -z "${_XO_BACKEND_TMUX_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$XO_BACKEND_LIB_DIR/backends/tmux.sh" || return 1
        _XO_BACKEND_TMUX_SOURCED=1
      fi
      ;;
    herdr)
      if [ -z "${_XO_BACKEND_HERDR_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$XO_BACKEND_LIB_DIR/backends/herdr.sh" || return 1
        _XO_BACKEND_HERDR_SOURCED=1
      fi
      ;;
    zellij)
      if [ -z "${_XO_BACKEND_ZELLIJ_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$XO_BACKEND_LIB_DIR/backends/zellij.sh" || return 1
        _XO_BACKEND_ZELLIJ_SOURCED=1
      fi
      ;;
    orca)
      if [ -z "${_XO_BACKEND_ORCA_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$XO_BACKEND_LIB_DIR/backends/orca.sh" || return 1
        _XO_BACKEND_ORCA_SOURCED=1
      fi
      ;;
    cmux)
      if [ -z "${_XO_BACKEND_CMUX_SOURCED:-}" ]; then
        # shellcheck source=/dev/null
        . "$XO_BACKEND_LIB_DIR/backends/cmux.sh" || return 1
        _XO_BACKEND_CMUX_SOURCED=1
      fi
      ;;
  esac
}

# xo_backend_resolve_selector: resolve a raw xo-send.sh/xo-peek.sh style
...
#                      this xo home) - backend-independent, a literal string.
...
#   "xo-<id>"          legacy task window label fallback routed through
#                      <state-dir>/<id>.meta when no exact
#                      <state-dir>/xo-<id>.meta exists.
...
xo_backend_resolve_selector() {  # <raw-target> <state-dir>
  ...
  meta=$(xo_backend_meta_for_selector "$raw" "$state" 2>/dev/null || true)
  if [ -n "$meta" ]; then
    window=$(xo_backend_target_of_meta "$meta")
  ...
  case "$raw" in
    xo-*)
      echo "error: no metadata for $raw in $state; pass session:window to target a window outside this xo home" >&2
      return 1
      ;;
    *)
      meta=$(xo_backend_meta_for_window "$raw" "$state" 2>/dev/null || true)
      if [ -n "$meta" ]; then
        window=$(xo_backend_target_of_meta "$meta")
        [ -n "$window" ] || { echo "error: no backend target recorded in $meta" >&2; return 1; }
        printf '%s' "$window"
      ...
      xo_backend_source tmux || return 1
      xo_backend_tmux_resolve_bare_selector "$raw"
      ;;
  esac
}
...
# rather than hand-writing `case "$backend" in tmux) xo_backend_tmux_x ;; esac`
# at every call site. Each verified backend adds its own arm here, without
# changing call sites.

# xo_backend_capture: bounded plain-text session capture.
xo_backend_capture() {  # <backend> <target> <lines> [expected-label]
  local backend=$1
  shift
  xo_backend_source "$backend" || return 1
  case "$backend" in
    tmux) xo_backend_tmux_capture "$@" ;;
    herdr) xo_backend_herdr_capture "$@" ;;
    zellij) xo_backend_zellij_capture "$@" ;;
    orca) xo_backend_orca_capture "$@" ;;
    cmux) xo_backend_cmux_capture "$@" ;;
    *) echo "error: no capture implementation for backend '$backend'" >&2; return 1 ;;
  esac
}

# xo_backend_send_key: one backend-supported named special key.
xo_backend_send_key() {  # <backend> <target> <key> [expected-label]
  local backend=$1
  shift
  xo_backend_source "$backend" || return 1
  case "$backend" in
    tmux) xo_backend_tmux_send_key "$@" ;;
    herdr) xo_backend_herdr_send_key "$@" ;;
    zellij) xo_backend_zellij_send_key "$@" ;;
    orca) xo_backend_orca_send_key "$@" ;;
    cmux) xo_backend_cmux_send_key "$@" ;;
    *) echo "error: no send-key implementation for backend '$backend'" >&2; return 1 ;;
  esac
}

# xo_backend_send_text_submit: type text once, then submit and verify,
# retrying only the submission (never retyping). Echoes the backend's
# proof-carrying verdict; callers require exact empty for confirmed delivery.
xo_backend_send_text_submit() {  # <backend> <target> <text> <retries> <enter-sleep> <settle> [expected-label]
  local backend=$1
  shift
  xo_backend_source "$backend" || return 1
  case "$backend" in
    tmux) xo_backend_tmux_send_text_submit "$@" ;;
    herdr) xo_backend_herdr_send_text_submit "$@" ;;
    zellij) xo_backend_zellij_send_text_submit "$@" ;;
    orca) xo_backend_orca_send_text_submit "$@" ;;
    cmux) xo_backend_cmux_send_text_submit "$@" ;;
    *) echo "error: no send-text implementation for backend '$backend'" >&2; return 1 ;;
  esac
}

# xo_backend_kill: remove the task's session endpoint (best-effort; a
...
xo_backend_kill() {  # <backend> <target>
  local backend=$1
  shift
  [ -n "${1:-}" ] || { echo "error: refusing empty backend kill target" >&2; return 1; }
  xo_backend_source "$backend" || return 1
  case "$backend" in
    tmux) xo_backend_tmux_kill "$@" ;;
    herdr) xo_backend_herdr_kill "$@" ;;
    zellij) xo_backend_zellij_kill "$@" ;;
    orca) xo_backend_orca_kill "$@" ;;
    cmux) xo_backend_cmux_kill "$@" ;;
    *) echo "error: no kill implementation for backend '$backend'" >&2; return 1 ;;
  esac
}

xo_backend_remove_worktree() {  # <backend> <worktree-id>
  local backend=$1
  shift
  xo_backend_source "$backend" || return 1
  case "$backend" in
    orca) xo_backend_orca_remove_worktree "$@" ;;
    *) echo "error: backend '$backend' does not own task worktrees" >&2; return 1 ;;
  esac
}

xo_backend_worktree_path() {  # <backend> <worktree-id>
  local backend=$1
  shift
  xo_backend_source "$backend" || return 1
  case "$backend" in
    orca) xo_backend_orca_worktree_path "$@" ;;
    *) echo "error: backend '$backend' does not own task worktrees" >&2; return 1 ;;
  esac
}

# xo_backend_busy_state: semantic busy/idle/unknown for backends that expose
...
# primitive (tmux) report unknown. Callers own the fallback policy: xo-watch.sh
# uses unknown as the cue for harness-scoped pane-tail detection, while
# xo-crew-state.sh also corroborates native idle verdicts with the recorded
# harness's signature before treating a no-run crew as not busy.
xo_backend_busy_state() {  # <backend> <target>
  local backend=$1
  shift
  xo_backend_source "$backend" || { printf 'unknown'; return 0; }
  case "$backend" in
    herdr) xo_backend_herdr_busy_state "$@" ;;
    *) printf 'unknown' ;;
  esac
}

# xo_backend_composer_state: classify the composer/input area of <target> as
...
# supervisor-pane pending-input guard in bin/xo-supervise-daemon.sh, and
# xo-spawn.sh's kimi readiness/delivery checks) can ask the same question
...
# the one shared shape owner (bin/xo-composer-lib.sh,
# xo_composer_classify_screen) - so no backend can hold a private shape
# assumption; zellij's classifier reads `dump-screen --ansi`, which replaced
# its old no-classifier content-diff reporting.
xo_backend_composer_state() {  # <backend> <target> [expected-label] -> empty|pending|pending-unproven|unknown
  local backend=$1
  shift
  xo_backend_source "$backend" || { printf 'unknown'; return 0; }
  case "$backend" in
    tmux) xo_tmux_composer_state "$@" ;;
    herdr) xo_backend_herdr_composer_state "$@" ;;
    orca) xo_backend_orca_composer_state "$@" ;;
    cmux) xo_backend_cmux_composer_state "$@" ;;
    zellij) xo_backend_zellij_composer_state "$@" ;;
    *) printf 'unknown' ;;
  esac
}

# xo_backend_target_exists: cheap, READ-ONLY existence check - does the
...
# going through xo_backend_herdr_target_ready (which auto-starts the herdr
# server as a side effect via xo_backend_herdr_server_ensure - fine for an
...
# Mirrors xo-crew-state.sh's pane_readable check; exists here as one shared
# primitive so callers that only need a fast alive/dead read (recovery
# digests, the session-start fleet digest) do not re-derive it inline.
xo_backend_target_exists() {  # <backend> <target> [expected-label]
  ...
  case "$backend" in
    ...
    herdr)
      ...
      # xo_backend_herdr_cli (not a raw HERDR_SESSION-only call): verified
      ...
      # running instead. xo_backend_herdr_cli appends the required --session
      # flag on top, so this check is correctly scoped even when the caller's
      # own ambient session (e.g. the primary xo's default session) is
      # a DIFFERENT one than the target's.
      xo_backend_herdr_cli "$session" pane get "$pane" >/dev/null 2>&1
      ;;
    zellij)
      xo_backend_source zellij || return 1
      xo_backend_zellij_target_ready "$target" "$expected_label"
      ;;
    orca)
      xo_backend_source orca || return 1
      xo_backend_orca_capture "$target" 1 >/dev/null 2>&1
      ;;
    cmux)
      xo_backend_source cmux || return 1
      xo_backend_cmux_target_ready "$target" "$expected_label"
      ;;
    *)
      return 1
      ;;
...
# xo_backend_agent_state: the single recovery-grade agent/endpoint state
# contract. It is deliberately richer than xo_backend_target_exists's cheap
...
# process level through the shared classifier in bin/xo-agent-process-lib.sh,
...
xo_backend_agent_state() {  # <backend> <target>
  local backend=$1 target=$2
  xo_backend_source "$backend" || { printf 'unverified'; return 0; }
  case "$backend" in
    tmux) xo_backend_tmux_agent_state "$target" ;;
    herdr) xo_backend_herdr_agent_state "$target" ;;
    *) printf 'unverified' ;;
  esac
}
...
xo_backend_agent_alive() {  # <backend> <target>
  case "$(xo_backend_agent_state "$1" "$2")" in
    alive) printf 'alive' ;;
...
#
# The watcher's event-wait splice (bin/xo-watch.sh) is backend-agnostic: it asks
# xo_backend_has_push whether a window's backend can push semantic state changes,
# and for those backends replaces its blind `sleep POLL` with a bounded wait on
# xo_backend_wait_transition. Every push-capable backend reuses the shared
# normalized-transition shape and policy table (bin/xo-transition-lib.sh); today
...
# xo_backend_has_push: 0 if <backend> exposes a native transition push stream.
xo_backend_has_push() {  # <backend>
  case "$1" in
    herdr) return 0 ;;
    *) return 1 ;;
  esac
...
# xo_backend_events_capable: 0 if <backend>'s push path is usable for <session>
...
xo_backend_events_capable() {  # <backend> <session>
  local backend=$1
  shift
  xo_backend_has_push "$backend" || return 1
  xo_backend_source "$backend" || return 1
  case "$backend" in
    herdr) xo_backend_herdr_events_capable "$@" ;;
    *) return 1 ;;
  esac
}

# xo_backend_wait_transition: bounded wait for a fresh actionable (blocked)
...
xo_backend_wait_transition() {  # <backend> <session> <timeout_secs> <state_dir> <pane_window...>
  local backend=$1
  shift
  xo_backend_has_push "$backend" || return 2
  xo_backend_source "$backend" || return 2
  case "$backend" in
    herdr) xo_backend_herdr_wait_transition "$@" ;;
    *) return 2 ;;
  esac
}

xo_backend_commit_transition() {  # <backend> <state_dir> <session> <record>
  local backend=$1
  shift
  xo_backend_has_push "$backend" || return 1
  xo_backend_source "$backend" || return 1
  case "$backend" in
    herdr) xo_backend_herdr_commit_transition "$@" ;;
    *) return 1 ;;
  esac
}

xo_backend_clear_transition() {  # <backend> <state_dir> <window>
  local backend=$1
  shift
  xo_backend_has_push "$backend" || return 0
  xo_backend_source "$backend" || return 1
  case "$backend" in
    herdr) xo_backend_herdr_clear_transition "$@" ;;
    *) return 0 ;;
  esac
}
