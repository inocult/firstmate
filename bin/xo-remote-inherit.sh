...
#
# Usage:
#   xo-remote-inherit.sh put <allowlisted-relative-path> <bytes> <sha256> <generation> < stdin
#   xo-remote-inherit.sh absent <allowlisted-relative-path> 0 <empty-sha256> <generation>
#
...
# (XO_INHERITABLE_CONFIG in bin/xo-config-inherit-lib.sh), so this code root's
...
HOME_REAL=$(CDPATH='' cd -- "$XO_HOME" 2>/dev/null && pwd -P) || die "XO_HOME is unavailable"
PARENT="$HOME_REAL/$(dirname "$REL")"
# The captain accepts this config/data parent TOCTOU within XO's single-user trust boundary.
...
case "$PARENT_REAL" in "$HOME_REAL/config"|"$HOME_REAL/data") ;; *) die "inherited destination escapes XO_HOME" ;; esac
...
BASE=$(basename "$REL")
LOCK="$PARENT_REAL/.xo-inherit-$BASE.lock"
GENERATION_FILE="$PARENT_REAL/.xo-inherit-$BASE.generation"
xo_lock_acquire_wait "$LOCK" || die "cannot lock inherited destination"
TMP=
GENERATION_TMP=
cleanup() {
  [ -z "$TMP" ] || rm -f -- "$TMP"
  [ -z "$GENERATION_TMP" ] || rm -f -- "$GENERATION_TMP"
  xo_lock_release "$LOCK" || true
}
trap cleanup EXIT
