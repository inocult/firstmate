...
# names below, and tests/xo-tmux-agent-liveness.test.sh plus
# tests/xo-harness-liveness-drift-live-e2e.test.sh keep them honest.

# shellcheck source=bin/xo-session-lock-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/xo-session-lock-lib.sh"
# shellcheck source=bin/xo-gemini-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/xo-gemini-lib.sh"

# xo_agent_process_classify_name: the single owner of the process-name
...
xo_agent_process_classify_name() {  # <path> [argv0] -> agent|shell|other
  ...
  case "$base" in
    ...
    # COMPONENT, so the xo_harness_path_name fallback below never fires for it.
    ...
    *)
      if xo_harness_path_name "$path" >/dev/null || xo_harness_path_name "$argv0" >/dev/null; then
        printf 'agent'
      ...
      # comes from the narrowed structural rule in bin/xo-cursor-lib.sh, which
      ...
      elif xo_cursor_process_matches "${path:-$argv0}" '' "$argv0"; then
        printf 'agent'
      else
        printf 'other'
      fi
...
# xo_agent_process_classify: one process, from every identity surface a
...
#            harnesses whose identity sits in argv[1] (bin/xo-gemini-lib.sh).
#   [pid]    when given, lets the Gemini rule read argv boundaries from the
#            live process instead of the flattened line.
xo_agent_process_classify() {  # <name> <argv0> <args> [pid] -> agent|shell|other
  local name=${1:-} argv0=${2:-} args=${3:-} pid=${4:-} by_name by_argv0
  by_name=$(xo_agent_process_classify_name "$name" "$argv0")
  ...
  if [ -n "$argv0" ]; then
    ...
    by_argv0=$(xo_agent_process_classify_name "$argv0" "$argv0")
    [ "$by_argv0" != agent ] || { printf 'agent'; return 0; }
  else
    by_argv0=$by_name
  fi
  if [ -n "$pid" ] && xo_gemini_pid_is_gemini "$pid"; then
    printf 'agent'
    return 0
  fi
  if [ -n "$args" ] && xo_gemini_args_are_gemini "$args"; then
    printf 'agent'
    return 0
  fi
