...
if ! command -v xo_backlog_atomic_transition >/dev/null 2>&1; then
  ...
  . "$_XO_X_LIB_DIR/xo-tasks-axi-lib.sh"
  # shellcheck source=bin/xo-backlog-transition-lib.sh
  . "$_XO_X_LIB_DIR/xo-backlog-transition-lib.sh"
fi
...
xox_poll_shim_content() {
  ...
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    '# Auto-generated by xo-bootstrap.sh - X mode connector poll shim.' \
...
xox_request_relay_context() {
  ...
  body_file=$(mktemp "${TMPDIR:-/tmp}/xo-x-reqctx-body.XXXXXX") || { rm -f "$payload_file"; printf '%s\n' "$empty"; return 1; }
...
# xo-x-followup.sh posts against that link (within the window, up to the cap),
...
xox_meta_link_set() {
  ...
  printf 'x_request=%s\n' "$rid" >> "$tmp" || { rm -f "$tmp"; xo_lock_release "$lock"; return 1; }
  printf 'x_request_ts=%s\n' "$ts" >> "$tmp" || { rm -f "$tmp"; xo_lock_release "$lock"; return 1; }
  printf 'x_followups=%s\n' "$followups" >> "$tmp" || { rm -f "$tmp"; xo_lock_release "$lock"; return 1; }
  if [ -n "$platform" ]; then
    printf 'x_platform=%s\n' "$platform" >> "$tmp" || { rm -f "$tmp"; xo_lock_release "$lock"; return 1; }
  fi
  case "$reply_max" in
    ''|*[!0-9]*) ;;
    *) printf 'x_reply_max_chars=%s\n' "$reply_max" >> "$tmp" || { rm -f "$tmp"; xo_lock_release "$lock"; return 1; } ;;
  esac
...
xox_meta_followups_set() {
  ...
  printf 'x_followups=%s\n' "$n" >> "$tmp" || { rm -f "$tmp"; xo_lock_release "$lock"; return 1; }
  # shellcheck disable=SC2153
  if ! xo_backlog_atomic_transition publish "$tmp" "$meta" "task record" "$STATE"; then
    rm -f "$tmp"; xo_lock_release "$lock"; return 1
  fi
...
xox_meta_link_clear() {
  ...
  if [ "$#" -ge 2 ]; then
    ...
    [ -d "$parent" ] && [ ! -L "$parent" ] && [ -r "$parent" ] \
      && [ -x "$parent" ] || return 1
    xo_backlog_record_parent_authorized "$meta" "task record" "$STATE" || return 1
  fi
  [ ! -L "$meta" ] || return 1
  [ -f "$meta" ] || return 0
  ...
  [ ! -L "$meta" ] || { xo_lock_release "$lock"; return 1; }
  [ -f "$meta" ] || { xo_lock_release "$lock"; return 0; }
  if [ "$expected_set" -eq 1 ]; then
    link_present=0
    rid=
    while IFS= read -r line || [ -n "$line" ]; do
    ...
    done < "$meta" || { xo_lock_release "$lock"; return 1; }
    [ "$link_present" -eq 0 ] || {
