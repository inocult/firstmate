#!/usr/bin/env bash
# xo-bearings-snapshot.sh - compact, bounded, TOON-by-default bearings projection.
#
# A thin wrapper OVER the canonical bin/xo-fleet-snapshot.sh. It does not parse
# fleet state itself: it shells out to `xo-fleet-snapshot.sh --json`, projects that
...
# secondmate_landed roll-up (xo-fleet-snapshot.sh), so merges a secondmate managed -
...
# order. Which closed rows either side contributes is bin/xo-landed-lib.sh's rule.
#
# Flags:
...
#   -h,--help        usage
#
# Output contract: `xo-bearings.v1`. No locks or reports; the underlying snapshot's
# parent-side remote-ledger cache refresh is the only default fleet-state mutation.
set -u

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
FLEET="$SCRIPT_DIR/xo-fleet-snapshot.sh"
...
XO_BEARINGS_LANDED=${XO_BEARINGS_LANDED:-6}
XO_BEARINGS_LANDED_PER_HOME=${XO_BEARINGS_LANDED_PER_HOME:-$XO_BEARINGS_LANDED}
XO_BEARINGS_IN_FLIGHT=${XO_BEARINGS_IN_FLIGHT:-20}
XO_BEARINGS_DECISIONS=${XO_BEARINGS_DECISIONS:-20}
XO_BEARINGS_SECONDMATES=${XO_BEARINGS_SECONDMATES:-20}
XO_BEARINGS_GATES=${XO_BEARINGS_GATES:-20}
XO_BEARINGS_REPORTS=${XO_BEARINGS_REPORTS:-20}
XO_BEARINGS_RECORDED_PRS=${XO_BEARINGS_RECORDED_PRS:-20}
XO_BEARINGS_UNHEALTHY=${XO_BEARINGS_UNHEALTHY:-20}
XO_BEARINGS_PR_REPOS=${XO_BEARINGS_PR_REPOS:-10}
XO_BEARINGS_PR_LIMIT=${XO_BEARINGS_PR_LIMIT:-20}
XO_BEARINGS_PR_TIMEOUT=${XO_BEARINGS_PR_TIMEOUT:-20}
case "$XO_BEARINGS_PR_TIMEOUT" in ''|*[!0-9]*|0) XO_BEARINGS_PR_TIMEOUT=20 ;; esac
validate_bound() {  # <name> <value>
  case "$2" in ''|*[!0-9]*|0) echo "xo-bearings-snapshot: $1 must be a positive integer" >&2; exit 2 ;; esac
}
validate_bound XO_BEARINGS_LANDED "$XO_BEARINGS_LANDED"
validate_bound XO_BEARINGS_LANDED_PER_HOME "$XO_BEARINGS_LANDED_PER_HOME"
validate_bound XO_BEARINGS_IN_FLIGHT "$XO_BEARINGS_IN_FLIGHT"
validate_bound XO_BEARINGS_DECISIONS "$XO_BEARINGS_DECISIONS"
validate_bound XO_BEARINGS_SECONDMATES "$XO_BEARINGS_SECONDMATES"
validate_bound XO_BEARINGS_GATES "$XO_BEARINGS_GATES"
validate_bound XO_BEARINGS_REPORTS "$XO_BEARINGS_REPORTS"
validate_bound XO_BEARINGS_RECORDED_PRS "$XO_BEARINGS_RECORDED_PRS"
validate_bound XO_BEARINGS_UNHEALTHY "$XO_BEARINGS_UNHEALTHY"
validate_bound XO_BEARINGS_PR_REPOS "$XO_BEARINGS_PR_REPOS"
validate_bound XO_BEARINGS_PR_LIMIT "$XO_BEARINGS_PR_LIMIT"

usage() {
  cat <<'EOF'
usage: xo-bearings-snapshot.sh [--json] [--include-prs] [--fields <list>]
...
Compact bearings projection over xo-fleet-snapshot.sh. TOON by default.
...
landed merges this home's Done with registered secondmate homes' Done, bounded by
  a per-home cap (XO_BEARINGS_LANDED_PER_HOME) and an overall cap (XO_BEARINGS_LANDED),
...
Raise XO_BEARINGS_PR_LIMIT to expand per-repository open-PR results.
EOF
}
...
command -v jq >/dev/null 2>&1 || { echo "xo-bearings-snapshot: jq not found" >&2; exit 1; }
...
"$SCRIPT_DIR/xo-afk-return.sh" guard || exit $?

NOW=${XO_BEARINGS_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
if [ "$ALL_LANDED" = 1 ] || [ "$ALL_SECONDMATES" = 1 ]; then
  if [ "$ALL_LANDED" = 1 ]; then
    SNAP=$(XO_SNAPSHOT_NOW="$NOW" XO_SNAPSHOT_SECONDMATES=0 XO_SNAPSHOT_SECONDMATE_LANDED_PER_HOME=0 "$FLEET" --json) || exit $?
  else
    SNAP=$(XO_SNAPSHOT_NOW="$NOW" XO_SNAPSHOT_SECONDMATES=0 "$FLEET" --json) || exit $?
  fi
else
  SNAP=$(XO_SNAPSHOT_NOW="$NOW" "$FLEET" --json) || exit $?
fi
HOME_LABEL=$(printf '%s' "$SNAP" | jq -er '.xo_home | strings | split("/") | (.[-2:] | join("/"))') \
  || { echo "xo-bearings-snapshot: invalid canonical snapshot" >&2; exit 1; }
...
# bin/xo-timeout-lib.sh owns the bound itself.
gh_bounded() {  # <args...>
  xo_run_timed "$XO_BEARINGS_PR_TIMEOUT" \
...
EOF
    ...
    pr_fetch_limit=$((XO_BEARINGS_PR_LIMIT + 1))
    for repo in $repos; do
      if [ "$ALL_PR_REPOS" != 1 ] && [ "$nrepos" -ge "$XO_BEARINGS_PR_REPOS" ]; then break; fi
      nrepos=$((nrepos + 1))
      ...
      [ -n "$out" ] || out='[]'
      repo_result=$(printf '%s' "$out" | jq --arg repo "$repo" --argjson limit "$XO_BEARINGS_PR_LIMIT" '
        [ .[] | {
          num:(.number|tostring),
          repo:$repo,
          task:(if (.headRefName // "" | startswith("xo/")) then (.headRefName | ltrimstr("xo/")) else "-" end),
          url:(.url // "-"),
      ...
      [ "$returned" -gt "$XO_BEARINGS_PR_LIMIT" ] && ncapped=$((ncapped + 1))
      npr=$((npr + cnt))
...
# --- projection: canonical snapshot -> xo-bearings.v1 model (JSON) ----------
...
MODEL=$(printf '%s' "$SNAP" | jq \
  ...
  --arg prs "$PR_STATUS" \
  --arg fields "$FIELDS" \
  --argjson landed_n "$XO_BEARINGS_LANDED" \
  --argjson landed_per_home_n "$XO_BEARINGS_LANDED_PER_HOME" \
  --argjson in_flight_n "$XO_BEARINGS_IN_FLIGHT" \
  --argjson decisions_n "$XO_BEARINGS_DECISIONS" \
  --argjson secondmates_n "$XO_BEARINGS_SECONDMATES" \
  --argjson gates_n "$XO_BEARINGS_GATES" \
  --argjson reports_n "$XO_BEARINGS_REPORTS" \
  --argjson recorded_prs_n "$XO_BEARINGS_RECORDED_PRS" \
  --argjson unhealthy_n "$XO_BEARINGS_UNHEALTHY" \
  ...
  --argjson candidate_prs "$CANDIDATE_PRS" "$XO_LANDED_JQ_DEFS"'
  ...
  | . + {omitted: (
      [ (if $f_bodies then empty else {surface:"backlog item bodies", reveal:"--fields bodies"} end),
        ...
        (($snap.secondmate_current.records // [])[] as $m
         ...
         | if $n > 0 then {surface:("secondmate " + $m.id + " active children omitted by snapshot bound: \($n)"), reveal:"raise XO_SNAPSHOT_SECONDMATE_CHILDREN"} else empty end),
        ...
        (if (($snap.secondmate_current.truncated // 0) > 0) then {surface:("registered secondmates omitted by snapshot bound: \($snap.secondmate_current.truncated)"), reveal:"raise XO_SNAPSHOT_SECONDMATES"} else empty end),
        (if $snap.secondmate_current.registry.input_truncated == true then {surface:"secondmate registry input truncated by bounded read", reveal:"raise XO_SNAPSHOT_REGISTRY_LINES or XO_SNAPSHOT_REGISTRY_BYTES"} else empty end),
        (if $snap.secondmate_current.registry.records_truncated == true then {surface:"secondmate registry records omitted by bounded read", reveal:"raise XO_SNAPSHOT_REGISTRY_RECORDS"} else empty end),
        ...
        (([($snap.secondmate_current.records // [])[] | select(.parent_event.activity_scan.input_truncated == true or .parent_event.activity_scan.retained_truncated == true)] | length) as $n | if $n > 0 then {surface:("secondmate parent activity evidence truncated for \($n) record(s)"), reveal:"raise XO_SNAPSHOT_PARENT_ACTIVITY_LINES, XO_SNAPSHOT_PARENT_ACTIVITY_BYTES, or XO_SNAPSHOT_PARENT_ACTIVITIES"} else empty end),
        ...
        (if $include_prs == 1 and $pr_rows_capped > 0 then {surface:("candidate_prs showing \($candidate_prs | length) of at least \($pr_rows_min_total); capped in \($pr_rows_capped) repo(s)"), reveal:"raise XO_BEARINGS_PR_LIMIT"} else empty end),
        (if $include_prs == 1 then empty else {surface:"live PR discovery + checks", reveal:"--include-prs"} end) ]) }
') || { echo "xo-bearings-snapshot: projection failed" >&2; exit 1; }
...
') || { echo "xo-bearings-snapshot: TOON rendering failed" >&2; exit 1; }
printf '%s\n' "$TOON"
