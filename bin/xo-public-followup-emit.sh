#!/usr/bin/env bash
# xo-public-followup-emit.sh - emit ONE structured terminal work result for work
...
# TYPED result. XO must never recover the source home, work id, outcome,
...
# home (bin/xo-public-followup.sh deliver).
#
# Usage:
#   xo-public-followup-emit.sh (--home <owning-home> | --stage-in <work-home>) \
...
#                          `xo-public-followup.sh register`. Use this whenever
...
#                          transport (bin/xo-public-followup-collect.sh). Exactly
#                          one of --home and --stage-in is required;
#                          `xo-public-followup.sh brief` prints whichever the
...
usage() {
  cat >&2 <<'EOF'
usage: xo-public-followup-emit.sh (--home <owning-home> | --stage-in <work-home>)
...
die() { printf 'xo-public-followup-emit: %s\n' "$1" >&2; exit "${2:-2}"; }
...
xo_pf_slug_valid "$OUTCOME"    || die "unsafe outcome type: $OUTCOME"
xo_pf_home_id_valid "$SOURCE_HOME" \
...
# home's state tree, so a path that is not an xo home would swallow the
# result silently. Refuse it here instead.
if [ "$HOME_MODE" = staging ]; then
  case "$SOURCE_HOME" in
    secondmate:*) STAGING_HOME_ID=${SOURCE_HOME#secondmate:} ;;
    *) die "--stage-in must name the secondmate xo home identified by --source-home" ;;
  esac
  [ -d "$HOME_DIR/state" ] && [ ! -L "$HOME_DIR/state" ] \
    && [ -f "$HOME_DIR/.xo-secondmate-home" ] && [ ! -L "$HOME_DIR/.xo-secondmate-home" ] \
    || die "--stage-in must name the secondmate xo home identified by --source-home"
  STAGING_HOME_MARKER=$(sed -n '1p' "$HOME_DIR/.xo-secondmate-home" 2>/dev/null) || STAGING_HOME_MARKER=
...
if [ "$HOME_MODE" = owning ]; then
  xo_pf_relay_active "$HOME_DIR" || exit 0
  command -v jq >/dev/null 2>&1 || die "jq is required to build a typed terminal event" 1

  REGISTRY="$(xo_pf_registry_dir "$STATE")/$OBLIGATION"
  ...
  reg_mismatch relation   "$(xo_pf_registry_get "$STATE" "$OBLIGATION" relation_id)" "$RELATION"
  reg_mismatch source-home "$(xo_pf_registry_get "$STATE" "$OBLIGATION" work_home)"  "$SOURCE_HOME"
  reg_mismatch work-id    "$(xo_pf_registry_get "$STATE" "$OBLIGATION" work_id)"     "$WORK_ID"
  reg_mismatch generation "$(xo_pf_registry_get "$STATE" "$OBLIGATION" generation)"  "$GENERATION"
else
...
case "$TEXT_MODE" in
  inline) OUTCOME_TEXT=$(printf '%s' "$TEXT_SOURCE" | xo_pf_clean_outcome_text) ;;
  file)
    if [ "$TEXT_SOURCE" = '-' ]; then
      OUTCOME_TEXT=$(xo_pf_clean_outcome_text)
    else
      [ -f "$TEXT_SOURCE" ] || die "outcome text file not found: $TEXT_SOURCE"
      OUTCOME_TEXT=$(xo_pf_clean_outcome_text < "$TEXT_SOURCE")
    fi
    ;;
esac
...
EVENT_JSON=$(jq -Sc -n \
  --argjson schema_version "$XO_PF_EVENT_SCHEMA_VERSION" \
  ...
  --argjson outcome_max "$XO_PF_OUTCOME_TEXT_MAX" \
...
[ "$EVENT_BYTES" -le "$XO_PF_EVENT_BYTES_MAX" ] \
  || die "typed terminal event exceeds $XO_PF_EVENT_BYTES_MAX bytes" 2

if [ "$HOME_MODE" = owning ]; then
  DESTINATION=$(xo_pf_events_dir "$STATE")
else
