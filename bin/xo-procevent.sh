...
#
# Usage:
#   xo-procevent.sh register <adapter> <source-id> -- <argv>...
#   xo-procevent.sh register-extension <adapter> <source-id> --config-ref <reference>
#   xo-procevent.sh start <source-id>
#   xo-procevent.sh reconcile
#   xo-procevent.sh classify <result-file>
#   xo-procevent.sh handled <source-id> <sequence>
#   xo-procevent.sh retire <source-id> [--if-absent|--if-matches <adapter> -- <argv>...|--if-owner <registration-token>]
#   xo-procevent.sh sweep-home [--preflight]
#   xo-procevent.sh binding-retirement-preflight <binding-digest>
#   xo-procevent.sh extension-retirement <binding|transfer> <retirement-arguments...>
#   xo-procevent.sh extension-bind <bind|receive-transfer-bind> <binding-arguments...>
#   xo-procevent.sh extension-process-event <process-event-arguments...>
#   xo-procevent.sh list
#
...
#            (XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS; docs/configuration.md).
...
#            claim remains. Used by supported XO home retirement.
...
# `bin/xo-procevent-<adapter>.sh terminal <result-file>` path, while an external
...
# `bin/xo-procevent-<adapter>.sh autohandle <source-id> <sequence> <result-file>`
...
# adapter that answers exit 0 to `bin/xo-procevent-<adapter>.sh self-announcing`
...
# once, by bin/xo-captain-hold.sh's keyed-answer intake, and reaching it must not
# depend on an agent remembering. So after capture, a bound source
# has its result passed to
# `bin/xo-procevent-<adapter>.sh answers <result-file>`, and whatever that prints
...
# while ACTING on it is xo's judgement, so the capture stays unacknowledged
...
# home. A runner exports the inherited XO_PROCEVENT_IN_RUNNER marker and every
...
# bin/xo-lease-lib.sh documents: a source that DELIBERATELY strips the marker
...
# another home is untouched. See bin/xo-procevent-lib.sh for the lease itself.
#
# Ownership is machine-wide per canonical source, because separate XO
...
#
# Durability boundary: see bin/xo-procevent-lib.sh. This runner proves capture
...
MAX_OUTPUT_BYTES=${XO_PROCEVENT_MAX_OUTPUT_BYTES:-1048576}
EXTENSION_HOST="$SCRIPT_DIR/xo-extension.mjs"
...
run_extension_invocation_cleanup() {  # [cleanup selector...]
  ...
  if [ -n "${XO_STATE_OVERRIDE:-}" ]; then
    ...
    XO_HOME="$XO_HOME" "$EXTENSION_HOST" cleanup-invocations "$@" >/dev/null 2>&1
  fi
}
...
cleanup_extension_registration_invocations_locked() {  # <source-id>
  local owner_state
  xo_procevent_extension_registration_load_locked "$STATE" "$1"
  owner_state=$?
  case "$owner_state" in
    0) cleanup_extension_binding_invocations "$XO_PROCEVENT_EXTENSION_BINDING_DIGEST" ;;
    1) return 0 ;;
    *) return 1 ;;
  esac
}
...
extension_result_command() {  # <adapter> <operation> <result-file>
  ...
  xo_procevent_result_extension_load "$result"
  owner_state=$?
  [ "$owner_state" -eq 0 ] || return 1
  [ -x "$EXTENSION_HOST" ] && [ ! -L "$EXTENSION_HOST" ] || return 1
  case "$operation" in
    result.terminal) reservation=${XO_PROCEVENT_CAPTURE_RESERVATION_TERMINAL:-} ;;
    result.silent) reservation=${XO_PROCEVENT_CAPTURE_RESERVATION_SILENT:-} ;;
  esac
  local -a command=("$EXTENSION_HOST" process-event "$adapter" "$operation"
    --result-file "$result"
    --expect-extension "$XO_PROCEVENT_RESULT_EXTENSION_ID"
    --expect-version "$XO_PROCEVENT_RESULT_EXTENSION_VERSION"
    --expect-capability-version "$XO_PROCEVENT_RESULT_EXTENSION_CAPABILITY_VERSION"
    --expect-package-digest "$XO_PROCEVENT_RESULT_EXTENSION_PACKAGE_DIGEST"
    --expect-binding-digest "$XO_PROCEVENT_RESULT_EXTENSION_BINDING_DIGEST")
  if [ -n "$reservation" ]; then
    extension_lifecycle_lock_acquire || return 1
    owner=${XO_LOCK_OWNER_DIR:-}
    [ -n "$owner" ] || { extension_lifecycle_lock_release; return 1; }
    claim_path=$(xo_procevent_claim_path "$CLAIM_ID") || { extension_lifecycle_lock_release; return 1; }
    XO_EXTENSION_RETIREMENT_MODE=process-event \
      XO_EXTENSION_LIFECYCLE_LOCK="$EXTENSION_LIFECYCLE_LOCK" \
      XO_EXTENSION_LIFECYCLE_OWNER="$owner" \
      perl "$SCRIPT_DIR/xo-procevent-extension-capture.pl" handoff \
        8 6 "$claim_path" "$CLAIM_HOME" "$CLAIM_ID" "$CLAIM_TOKEN" "$CLAIM_PID" \
        "$(xo_pid_identity "$CLAIM_PID")" "$XO_PROCEVENT_RESULT_EXTENSION_BINDING_DIGEST" "$reservation" \
...
adapter_result_is_terminal() {  # <adapter> <result-file>
  ...
  xo_procevent_result_extension_load "$2"
  owner_state=$?
  case "$owner_state" in
...
adapter_autohandle() {  # <adapter> <source-id> <result-file>
  ...
  [ -f "$script" ] && [ ! -L "$script" ] || return 1
  seq=$(xo_procevent_result_sequence "$result") || return 1
  case "$seq" in ''|*[!0-9]*) return 1 ;; esac
  xo_procevent_is_handled "$STATE" "$id" "$seq" && return 0
...
feed_keyed_answers() {  # <adapter> <source-id> <result-file>
  ...
  script=$(adapter_script "$adapter")
  [ -f "$script" ] && [ ! -L "$script" ] || return 1
  origin=$("$SCRIPT_DIR/xo-captain-hold.sh" binding "$id" 2>/dev/null) || return 1
  [ -n "$origin" ] || return 1
  seq=$(xo_procevent_result_sequence "$result") || return 1
  "$script" answers "$result" 2>/dev/null \
    | "$SCRIPT_DIR/xo-captain-hold.sh" answers "$origin" \
...
feed_reconcile_requests() {  # <adapter> <source-id> <result-file>
  ...
  printf '%s\n' "$rows" \
    | "$SCRIPT_DIR/xo-captain-hold.sh" reconcile-requests \
...
extension_registration_replacement_safe_locked() {  # <source-id>
  ...
  xo_procevent_extension_registration_load_locked "$STATE" "$id"
  owner_state=$?
...
cmd_register() {
  ...
  xo_procevent_adapter_valid "$adapter" || die "adapter name must be lowercase alphanumeric or dash: $adapter"
  xo_procevent_source_id_valid "$id" || die "source id must be path-safe and at most 64 characters: $id"
  [ "$sep" = -- ] || usage
  ...
  xo_procevent_source_lock_acquire "$id" || die "cannot lock the source"
  ...
  if ! xo_procevent_registration_publish_locked "$STATE" "$adapter" "$id" "$@"; then
    xo_procevent_source_lock_release "$id"
...
extension_source_request_id() {  # <adapter> <source-id> <next-sequence> <registration-token> <package-digest>
  ...
  if command -v shasum >/dev/null 2>&1; then
    digest=$(printf 'xo-process-event-request-v1\n%s\n%s\n%s\n%s\n%s\n' "$@" \
...
cmd_register_extension() {
  ...
  xo_procevent_extension_config_ref_valid "$config_ref" \
  ...
  if [ "$schema" != xo-extension-process-event-resolution.v1 ] || [ -n "$extra" ]; then
    extension_lifecycle_lock_release
    die "extension adapter resolution was malformed: $adapter"
  fi
  if ! xo_procevent_extension_id_valid "$extension_id" \
    || ! xo_procevent_extension_version_valid "$extension_version" \
    || [ "$capability_version" != 1 ] \
    || ! xo_procevent_digest_valid "$package_digest" \
    || ! xo_procevent_digest_valid "$binding_digest"; then
    extension_lifecycle_lock_release
  ...
  if ! xo_procevent_source_lock_acquire "$id"; then
    extension_lifecycle_lock_release
  ...
  if ! xo_procevent_extension_registration_publish_locked "$STATE" "$adapter" "$id" \
  ...
  printf 'retire: bin/xo-procevent.sh retire %s --if-owner %s\n' "$id" "$registration_token"
}
...
# and drains until `xo_procevent_mark_handled` records it.
publish_result() {  # <result-file>
  local result=$1 id seq adapter line status=1
  id=$(xo_procevent_result_source_id "$result")
  seq=$(xo_procevent_result_sequence "$result")
  xo_procevent_source_id_valid "$id" || return 1
  adapter=$(xo_procevent_result_adapter "$result" 2>/dev/null || true)
  [ -n "$adapter" ] || return 1
  line=$(xo_procevent_event_line "$adapter" "$id" "$seq") || return 1
  xo_procevent_source_lock_acquire "$id" || return 1
  if ! xo_procevent_is_handled "$STATE" "$id" "$seq"; then
    ...
    export XO_PROCEVENT_CAPTURE_SOURCE_LOCK_HELD=1
    if adapter_result_is_silent "$adapter" "$result"; then
      unset XO_PROCEVENT_CAPTURE_SOURCE_LOCK_HELD
      xo_procevent_mark_handled "$STATE" "$id" "$seq"
      case "$?" in
        0|1)
          xo_procevent_source_lock_release "$id"
          return 1
    ...
    unset XO_PROCEVENT_CAPTURE_SOURCE_LOCK_HELD
    if xo_wake_append check "procevent:$id:$seq" "check: $line"; then
      status=0
    fi
  fi
  xo_procevent_source_lock_release "$id"
...
publish_pending() {  # [result-file-to-skip]
  ...
  done < <(xo_procevent_pending "$STATE")
  printf '%s\n' "$published"
}
...
isolate_runner() {  # <wait|detach> <source-id>
  isolate_process "$1" "$SCRIPT_DIR/xo-procevent.sh" _start "$2"
}

require_isolated_group() {  # <role>
  local role=$1 pgid
  [ "${XO_PROCEVENT_RUNNER_GROUP:-}" = "$$" ] \
  ...
  unset XO_PROCEVENT_RUNNER_GROUP
}
...
# XO_PROCEVENT_IN_RUNNER marker, so a runner and its ordinary children do not
...
owner_lease_refresh() {
  [ "${XO_PROCEVENT_IN_RUNNER:-0}" = 1 ] && return 0
  xo_procevent_owner_lease_touch "$STATE" 2>/dev/null || true
}
...
owner_lease_keepalive() {  # <parent-pid> <parent-identity>
  ...
  while :; do
    sleep 1
    xo_procevent_pid_state "$parent" "$identity"
    state=$?
    case "$state" in
...
cmd_start_public() {
  ...
  [ "$#" -eq 1 ] || usage
  xo_procevent_source_id_valid "$id" || die "source id must be path-safe: $id"
  owner_lease_refresh
  identity=$(xo_pid_identity "$$" 2>/dev/null) || die "cannot identify the attached owner"
...
cmd_start() {
  ...
  xo_procevent_source_lock_acquire "$id" || die "cannot lock source: $id"
  ...
  if ! xo_procevent_adapter_valid "$adapter"; then
    xo_procevent_source_lock_release "$id"
  ...
  case "$extension_load_state" in
    0)
      extension_owner=1
      [ "$XO_PROCEVENT_EXTENSION_ADAPTER" = "$adapter" ] || {
        xo_procevent_source_lock_release "$id"
      ...
      extension_sequence=$(next_result_sequence "$id") \
        || { xo_procevent_source_lock_release "$id"; die "cannot derive extension request sequence: $id"; }
      extension_request_id=$(extension_source_request_id "$adapter" "$id" "$extension_sequence" \
        "$XO_PROCEVENT_EXTENSION_REGISTRATION_TOKEN" "$XO_PROCEVENT_EXTENSION_PACKAGE_DIGEST") \
        || { xo_procevent_source_lock_release "$id"; die "cannot derive extension request identity: $id"; }
      ARGV=("$EXTENSION_HOST" process-event "$adapter" source.poll \
        --source-id "$id" --config-ref "$XO_PROCEVENT_EXTENSION_CONFIG_REF" \
        --request-id "$extension_request_id" \
        --expect-extension "$XO_PROCEVENT_EXTENSION_ID" \
        --expect-version "$XO_PROCEVENT_EXTENSION_VERSION" \
        --expect-capability-version "$XO_PROCEVENT_EXTENSION_CAPABILITY_VERSION" \
        --expect-package-digest "$XO_PROCEVENT_EXTENSION_PACKAGE_DIGEST" \
        --expect-binding-digest "$XO_PROCEVENT_EXTENSION_BINDING_DIGEST")
      ;;
    1)
  ...
  xo_procevent_claim_acquire_locked "$id" "$XO_HOME" "$$" "$(source_file "$id")" "$STATE"
  claimed=$?
  xo_procevent_source_lock_release "$id"
  case "$claimed" in
    0) ;;
  ...
  CLAIM_HOME=$XO_HOME
  CLAIM_PID=$$
  CLAIM_TOKEN=$XO_PROCEVENT_CLAIM_TOKEN
  CLAIM_REG_IDENTITY=$XO_PROCEVENT_CLAIM_REG_IDENTITY
  CLAIM_STATE_DEVICE=$XO_PROCEVENT_CLAIM_STATE_DEVICE
  CLAIM_STATE_INODE=$XO_PROCEVENT_CLAIM_STATE_INODE
  STAGED_OUTPUT=
  ...
  release_start_claim() {
    ...
    xo_procevent_source_lock_try_acquire "$CLAIM_ID" 2>/dev/null || return 0
    if xo_procevent_claim_load_locked "$CLAIM_ID" 2>/dev/null \
      && [ "$XO_PROCEVENT_CLAIM_HOME" = "$CLAIM_HOME" ] \
      && [ "$XO_PROCEVENT_CLAIM_PID" = "$CLAIM_PID" ] \
      && [ "$XO_PROCEVENT_CLAIM_TOKEN" = "$CLAIM_TOKEN" ] \
      && [ "$XO_PROCEVENT_CLAIM_TERMINAL" = terminal ]; then
      xo_procevent_source_lock_release "$CLAIM_ID" 2>/dev/null || true
      return 0
    fi
    xo_procevent_claim_release_locked "$CLAIM_ID" "$CLAIM_HOME" "$CLAIM_PID" "$CLAIM_TOKEN" 2>/dev/null || true
    xo_procevent_source_lock_release "$CLAIM_ID" 2>/dev/null || true
  }
  ...
  export XO_PROCEVENT_IN_RUNNER=1
  ...
  launch_floor=$(xo_procevent_launch_floor_seconds) \
    || die "XO_PROCEVENT_LAUNCH_FLOOR_SECONDS must be whole seconds from $XO_PROCEVENT_LAUNCH_FLOOR_MIN_SECONDS to $XO_PROCEVENT_LAUNCH_FLOOR_MAX_SECONDS"
  if [ "$extension_owner" -eq 1 ]; then
    staging=$(xo_procevent_extension_staging_prepare "$STATE") \
      || die "cannot safely prepare the external registry staging boundary"
    inbox=$(xo_procevent_capture_inbox_prepare "$STATE") \
    ...
    reservation_dir=$(xo_procevent_capture_reservation_prepare "$STATE") \
    ...
    export XO_PROCEVENT_CAPTURE_INBOX_FD=8
    runner="$id.runner"
  else
    runner=$(runner_file "$id")
  fi

  case "$MAX_OUTPUT_BYTES" in ''|*[!0-9]*) die "XO_PROCEVENT_MAX_OUTPUT_BYTES must be a nonnegative integer" ;; esac
  if [ "$extension_owner" -eq 1 ]; then
  ...
  xo_procevent_launch_floor_wait "$STATE" "$id" "$CLAIM_REG_IDENTITY" "$launch_floor"
  case "$?" in
    0) ;;
  ...
  if [ "$extension_owner" -eq 1 ]; then
    ...
    perl "$SCRIPT_DIR/xo-procevent-extension-capture.pl" \
      9 8 6 "$id" "$adapter" "$XO_PROCEVENT_EXTENSION_ID" \
      "$XO_PROCEVENT_EXTENSION_VERSION" "$XO_PROCEVENT_EXTENSION_CAPABILITY_VERSION" \
      "$XO_PROCEVENT_EXTENSION_PACKAGE_DIGEST" "$XO_PROCEVENT_EXTENSION_BINDING_DIGEST" \
      "$CLAIM_TOKEN" "$runner" "$out" "$$" "$(xo_pid_identity "$$")" "$MAX_OUTPUT_BYTES" \
...
EOF
    ...
    if [ "$capture_state" = captured ]; then
      XO_PROCEVENT_CAPTURE_RESERVATION_TERMINAL=$reservation_terminal
      XO_PROCEVENT_CAPTURE_RESERVATION_SILENT=$reservation_silent
    fi
  else
    [ ! -e "$out" ] && [ ! -L "$out" ] || {
      xo_procevent_source_lock_release "$id"
  ...
  if [ "$extension_owner" -eq 1 ]; then
    ...
    durable=$(xo_procevent_capture "$STATE" "$id" "$adapter" "$out") \
  ...
  if [ "$extension_owner" -eq 0 ] && adapter_self_announcing "$adapter"; then
    ...
    if publish_result "$durable"; then
      published_capture=1
    elif xo_procevent_is_handled "$STATE" "$id" "$(xo_procevent_result_sequence "$durable")"; then
      handled_capture=1
    fi
...
retire_owned_terminal_source() {  # <source-id>
  ...
  xo_procevent_source_lock_acquire "$id" || return 1
  if xo_procevent_claim_load_locked "$id" 2>/dev/null \
    && [ "$XO_PROCEVENT_CLAIM_HOME" = "$CLAIM_HOME" ] \
    && [ "$XO_PROCEVENT_CLAIM_PID" = "$CLAIM_PID" ] \
    && [ "$XO_PROCEVENT_CLAIM_TOKEN" = "$CLAIM_TOKEN" ] \
    && [ "$XO_PROCEVENT_CLAIM_REG_IDENTITY" = "$CLAIM_REG_IDENTITY" ] \
    && current_identity=$(xo_pr_file_identity "$registration" 2>/dev/null) \
    && [ "$current_identity" = "$CLAIM_REG_IDENTITY" ] \
    && xo_procevent_claim_mark_terminal_locked "$id" "$CLAIM_HOME" "$CLAIM_PID" "$CLAIM_TOKEN"; then
    if rm -f -- "$registration" && [ ! -e "$registration" ] && [ ! -L "$registration" ]; then
      xo_procevent_claim_release_terminal_self_locked "$id" "$CLAIM_HOME" "$CLAIM_PID" "$CLAIM_TOKEN" || status=1
    else
      status=1
    fi
  else
...
start_owner_guard() {  # <source-id>
  ...
  identity=$(xo_pid_identity "$$" 2>/dev/null) || return 1
  ready=$(umask 077; mktemp "$REG/.owner-guard-ready.XXXXXX") || return 1
  if ! isolate_process detach "$SCRIPT_DIR/xo-procevent.sh" _owner-watchdog \
...
cmd_owner_watchdog() {  # <source-id> <runner-pid> <runner-identity> <ready-file> <state-device> <state-inode>
  ...
  lease=$(xo_procevent_owner_lease_seconds) \
    || die "XO_PROCEVENT_OWNER_LEASE_SECONDS must be whole seconds from $XO_PROCEVENT_OWNER_LEASE_MIN_SECONDS to $XO_PROCEVENT_OWNER_LEASE_MAX_SECONDS"
  tick=$(xo_procevent_owner_check_seconds) \
    || die "XO_PROCEVENT_OWNER_CHECK_SECONDS must be whole seconds from $XO_PROCEVENT_OWNER_CHECK_MIN_SECONDS to $XO_PROCEVENT_OWNER_CHECK_MAX_SECONDS"
  ...
  state_identity=$(xo_procevent_claim_state_root_identity "$STATE") \
  ...
  xo_procevent_owner_alive "$STATE" "$lease" \
  ...
  while :; do
    ...
    state_identity=$(xo_procevent_claim_state_root_identity "$STATE" 2>/dev/null || true)
    current_device=
    current_inode=
    [ -z "$state_identity" ] \
      || IFS=$'\t' read -r _ current_device current_inode _ _ <<< "$state_identity"
    if [ "$current_device" = "$state_device" ] \
      ...
      && xo_procevent_owner_alive "$STATE" "$lease"; then
      misses=0
      continue
    fi
...
# reach xo through the ordinary queue, and it carries what clears the
...
report_launch_failure() {  # <source-id> <registration-identity>
  ...
  announce_source_once "$(launch_failed_file "$id")" "$episode" \
    ...
    "check: process-event source $id is registered but its launch did not prove it took the source's claim within XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS, so nothing is confirmed to be collecting from it; reconcile reports that as failed= and keeps launching it every supervision cycle. If it stays that way, check the source command and the adapter binary the registration names, and run an attached bin/xo-procevent.sh start $id to reproduce a refusal on its stderr - the detached launch discards it, and a hand-run reconcile only counts it as failed=. A later cycle that finds the source owned ends this episode on its own, so a runner that was merely slow to claim needs nothing from you." \
    "$episode $nonce"
}
...
announce_source_once() {  # <marker> <generation> <key> <payload> [marker-record]
  ...
  if ! xo_wake_append check "$key" "$payload"; then
    rm -f -- "$marker"
    return 1
  fi
  return 0
...
stranded_reused_pid_detail() {  # <source-id>
  printf '%s' "its claim names a dead runner whose process group still has members, so reconcile preserves that claim and starts no replacement. Check that nothing is still polling the source, then reclaim it with: bin/xo-procevent.sh start $1 - that reclaims it provided the dead generation's reservation records can still be tidied, and otherwise refuses with: cannot claim source"
}
...
cmd_reconcile() {
  ...
  xo_procevent_launch_confirm_seconds >/dev/null \
    || die "XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS must be whole seconds from $XO_PROCEVENT_LAUNCH_CONFIRM_MIN_SECONDS to $XO_PROCEVENT_LAUNCH_CONFIRM_MAX_SECONDS"
  owner_lease_refresh
  ...
  for claim in "$(xo_procevent_claim_root)"/*.claim; do
    [ -e "$claim" ] || continue
    id=${claim##*/}; id=${id%.claim}
    xo_procevent_source_id_valid "$id" || continue
    xo_procevent_source_lock_acquire "$id" || continue
    if [ -f "$(source_file "$id")" ] && [ ! -L "$(source_file "$id")" ]; then
      xo_procevent_source_lock_release "$id"
      continue
    fi
    if ! xo_procevent_claim_load_locked "$id" 2>/dev/null; then
      uncertain=$((uncertain + 1))
      xo_procevent_source_lock_release "$id"
      continue
    fi
    owner=$XO_PROCEVENT_CLAIM_HOME
    pid=$XO_PROCEVENT_CLAIM_PID
    token=$XO_PROCEVENT_CLAIM_TOKEN
    identity=$XO_PROCEVENT_CLAIM_IDENTITY
    if ! xo_procevent_claim_owned_by_state "$STATE" "$XO_HOME"; then
      xo_procevent_source_lock_release "$id"
      continue
    fi
    ...
    case "$stop_state" in
      0|1)
        if xo_procevent_claim_reclaim_locked "$id" "$owner" "$pid" "$token" 2>/dev/null; then
  ...
  if [ -d "$REG" ]; then
    for rec in "$REG"/*.source; do
      ...
      if [ -f "$(source_file "$id")" ] && [ ! -L "$(source_file "$id")" ]; then
        xo_procevent_claim_state_locked "$id"
        claim_state=$?
        if [ "$claim_state" -eq 1 ] && xo_procevent_claim_undisplaceable_locked "$id"; then
          ...
          # (xo_procevent_claim_undisplaceable_locked owns that reasoning).
          ...
          uncertain=$((uncertain + 1))
          report_stranded_source "$id" "$XO_PROCEVENT_CLAIM_TOKEN" \
          ...
          launch_identity=$(xo_pr_file_identity "$(source_file "$id")" 2>/dev/null) || launch_identity=
          launch_mark=
          if [ -n "$launch_identity" ] \
            && launch_stamp=$(xo_procevent_launch_floor_stamp_path "$STATE" "$id" "$launch_identity"); then
          ...
          pid=$XO_PROCEVENT_CLAIM_PID
          token=$XO_PROCEVENT_CLAIM_TOKEN
          if xo_procevent_claim_owned_by_state "$STATE" "$XO_HOME" \
            && rm -f -- "$(source_file "$id")" \
            && [ ! -e "$(source_file "$id")" ] \
            && [ ! -L "$(source_file "$id")" ] \
            && xo_procevent_claim_reclaim_locked "$id" "$owner" "$pid" "$token" 2>/dev/null; then
            stopped=$((stopped + 1))
          else
            uncertain=$((uncertain + 1))
          fi
...
confirm_launched_runners() {  # <source-id><TAB><registration-identity><TAB><launch-stamp-before>...
  ...
  window=$(xo_procevent_launch_confirm_seconds) || return 1
  ...
  while :; do
    ...
    for entry in "${pending[@]+"${pending[@]}"}"; do
      ...
      before=${rest#*$'\t'}
      state=1
      if xo_procevent_source_lock_try_acquire "$id"; then
        xo_procevent_claim_state_locked "$id"
        state=$?
        xo_procevent_source_lock_release "$id"
      fi
      ...
      mark=
      if [ -n "$identity" ] \
        && stamp=$(xo_procevent_launch_floor_stamp_path "$STATE" "$id" "$identity"); then
...
runner_group_signal() {  # <signal> <pid> <identity> [proved]
  ...
  if [ -n "$proved" ]; then
    ...
    xo_procevent_group_alive "$pid" || return 1
  else
    ...
    case "$state" in
      0) ;;
      1) xo_procevent_group_alive "$pid" && return 2; return 1 ;;
      *) return 2 ;;
    esac
...
cmd_classify() {
  ...
  [ "$#" -eq 1 ] || usage
  adapter=$(xo_procevent_result_adapter "$result" 2>/dev/null) \
...
cmd_retire() {
  ...
  case "$condition" in
    ...
    --if-owner)
      [ "$#" -eq 3 ] || usage
      expected_owner=${3-}
      xo_procevent_extension_registration_token_valid "$expected_owner" \
    ...
    --if-matches)
      ...
      xo_procevent_adapter_valid "$adapter" \
  ...
  if [ -e "$(source_file "$id")" ] || [ -L "$(source_file "$id")" ]; then
    ...
    case "$condition" in
      --if-absent)
        ...
        ;;
      --if-matches)
        if ! xo_procevent_registration_matches_locked "$STATE" "$adapter" "$id" "$@"; then
          xo_procevent_source_lock_release "$id"
          die "source registration does not match the expected owner: $id"
        fi
        ;;
      --if-owner)
        ...
        owner_state=$?
        if [ "$owner_state" -ne 0 ] \
          || [ "$XO_PROCEVENT_EXTENSION_REGISTRATION_TOKEN" != "$expected_owner" ]; then
          xo_procevent_source_lock_release "$id"
          die "source registration does not match the expected owner: $id"
        fi
        extension_binding_digest=$XO_PROCEVENT_EXTENSION_BINDING_DIGEST
        ;;
    esac
  elif [ "$condition" = --if-owner ] \
    && { [ -e "$(xo_procevent_claim_path "$id")" ] || [ -L "$(xo_procevent_claim_path "$id")" ]; }; then
    xo_procevent_source_lock_release "$id"
    die "source owner cannot be proved after its registration disappeared: $id"
  fi
  if [ -e "$(xo_procevent_claim_path "$id")" ]; then
    if ! xo_procevent_claim_load_locked "$id" 2>/dev/null; then
      xo_procevent_source_lock_release "$id"
    ...
    if xo_procevent_claim_owned_by_state "$STATE" "$XO_HOME"; then
      ...
      if ! xo_procevent_claim_reclaim_locked "$id" "$owner" "$pid" "$token"; then
        xo_procevent_source_lock_release "$id"
  ...
  "$SCRIPT_DIR/xo-captain-hold.sh" unbind "$id" >/dev/null 2>&1 || true
  printf 'retired: %s\n' "$id"
}
...
sweep_relevant_state() {
  ...
  for path in "$(xo_procevent_claim_root)"/*.claim; do
    [ -f "$path" ] && [ ! -L "$path" ] || continue
    owner=${path##*/}; owner=${owner%.claim}
    xo_procevent_source_id_valid "$owner" || return 0
    xo_procevent_source_lock_acquire "$owner" || return 0
    if ! xo_procevent_claim_load_locked "$owner" 2>/dev/null; then
      xo_procevent_source_lock_release "$owner"
      return 0
    fi
    if xo_procevent_claim_owned_by_state "$STATE" "$XO_HOME"; then
      xo_procevent_source_lock_release "$owner"
...
sweep_source_preflight() {
  local id=$1 state
  xo_procevent_source_lock_acquire "$id" || return 1
  if [ -e "$(xo_procevent_claim_path "$id")" ] || [ -L "$(xo_procevent_claim_path "$id")" ]; then
    if ! xo_procevent_claim_load_locked "$id" 2>/dev/null; then
      xo_procevent_source_lock_release "$id"
      return 1
    fi
...
sweep_retire_source() {  # <source-id>
  ...
  if [ -e "$(source_file "$id")" ] || [ -L "$(source_file "$id")" ]; then
    ...
    owner_state=$?
    case "$owner_state" in
      0) expected_owner=$XO_PROCEVENT_EXTENSION_REGISTRATION_TOKEN ;;
      1) ;;
  ...
  if [ -n "$expected_owner" ]; then
    XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
      "$SCRIPT_DIR/xo-procevent.sh" retire "$id" --if-owner "$expected_owner"
  else
    XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
      "$SCRIPT_DIR/xo-procevent.sh" retire "$id"
  fi
}
...
cmd_sweep_home() {
  ...
  for path in "$REG"/*.source; do
    if [ -e "$path" ] || [ -L "$path" ]; then
      ...
      if xo_procevent_source_id_valid "$id"; then
        sweep_add_id "$id"
      else
        failed=$((failed + 1))
      fi
...
cmd_list() {
  ...
  owner_lease_refresh
  if ! xo_procevent_any_registered "$STATE"; then
  ...
  for rec in "$REG"/*.source; do
    ...
    case "$claim_state" in
      ...
      1)
        owner=none
        if xo_procevent_claim_undisplaceable_locked "$id"; then
          owner=orphaned
        fi
        ;;
      3) owner=orphaned ;;
    ...
    xo_procevent_source_lock_release "$id"
    pending=$(xo_procevent_pending "$STATE" | grep -c "/$id\." || true)
...
cmd_binding_retirement_preflight() {
  ...
  if [ "$#" -ne 1 ] || ! xo_procevent_digest_valid "$digest"; then
  ...
  for rec in "$REG"/*.source; do
    ...
    id=${rec##*/}; id=${id%.source}
    xo_procevent_source_id_valid "$id" || die "binding retirement found malformed registration state"
    xo_lock_try_acquire "$(xo_procevent_source_lock_path "$id")" \
      || die "binding still owns process-event registration: $id"
    xo_procevent_extension_registration_load_locked "$STATE" "$id"
    owner_state=$?
    xo_lock_release "$(xo_procevent_source_lock_path "$id")"
    case "$owner_state" in
      0) [ "$XO_PROCEVENT_EXTENSION_BINDING_DIGEST" != "$digest" ] \
        || die "binding still owns process-event registration: $id" ;;
      1) ;;
      *) die "binding retirement found malformed extension registration: $id" ;;
    esac
  done
  for result in "$(xo_procevent_inbox_dir "$STATE")"/*.result; do
    [ -e "$result" ] || continue
    ...
    owner_state=$?
    case "$owner_state" in
      0) [ "$XO_PROCEVENT_RESULT_EXTENSION_BINDING_DIGEST" != "$digest" ] \
...
cmd_extension_retirement() {
  ...
  export XO_EXTENSION_RETIREMENT_MODE="$mode"
  export XO_EXTENSION_LIFECYCLE_LOCK="$EXTENSION_LIFECYCLE_LOCK"
  export XO_EXTENSION_LIFECYCLE_OWNER="$owner"
  exec "$EXTENSION_HOST" "$@"
}
...
cmd_extension_bind() {
  ...
  owner=${XO_LOCK_OWNER_DIR:-}
  [ -n "$owner" ] || die "extension lifecycle lock has no owner identity"
  export XO_EXTENSION_RETIREMENT_MODE=bind
  export XO_EXTENSION_LIFECYCLE_LOCK="$EXTENSION_LIFECYCLE_LOCK"
  export XO_EXTENSION_LIFECYCLE_OWNER="$owner"
  exec "$EXTENSION_HOST" "$@"
}
...
cmd_extension_process_event() {
  ...
  owner=${XO_LOCK_OWNER_DIR:-}
  [ -n "$owner" ] || die "extension lifecycle lock has no owner identity"
  export XO_EXTENSION_RETIREMENT_MODE=process-event
  export XO_EXTENSION_LIFECYCLE_LOCK="$EXTENSION_LIFECYCLE_LOCK"
  export XO_EXTENSION_LIFECYCLE_OWNER="$owner"
...
unset XO_PROCEVENT_CAPTURE_PINNED_INBOX XO_PROCEVENT_CAPTURE_ABSOLUTE_INBOX \
  XO_PROCEVENT_CAPTURE_RESERVATION_TERMINAL \
  XO_PROCEVENT_CAPTURE_RESERVATION_SILENT
{ exec 7<&-; } 2>/dev/null || true
{ exec 6<&-; } 2>/dev/null || true
{ exec 8<&-; } 2>/dev/null || true
{ exec 9<&-; } 2>/dev/null || true
