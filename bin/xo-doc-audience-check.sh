#!/usr/bin/env bash
# xo-doc-audience-check.sh - validate the tracked documentation audience inventory.
#
# Usage:
#   bin/xo-doc-audience-check.sh
#   bin/xo-doc-audience-check.sh --root <repo> [--inventory <path>]
#
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate XO documentation audiences and local links.")
    ...
    try:
        ...
        print(f"xo-doc-audience-check: {exc}", file=sys.stderr)
        return 1
    print(f"xo-doc-audience-check: ok surfaces={surfaces} local_links={links}")
    return 0
