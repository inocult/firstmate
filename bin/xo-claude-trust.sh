...
#
# Usage: xo-claude-trust.sh <worktree> <project>
...
# with the cursor on "No, exit" and xo's steering plane carries only
# Enter, Escape and C-c with no arrow navigation, so xo cannot answer it
...
# spawn fails loudly here rather than wedging on the dialog later. xo-spawn.sh's
...
# opt-in guard family (XO_*_LIVE_E2E=1) and record the result in
...
# preserved, and the replacement is atomic. xo-spawn.sh forwards CLAUDE_CONFIG_DIR
...
[ "$#" -eq 2 ] || { echo "usage: xo-claude-trust.sh <worktree> <project>" >&2; exit 2; }
WT_ARG=$1
PROJ_ARG=$2
...
# worker's own cwd once xo-spawn.sh forwards it verbatim onto the launch, so the
...
# xo-spawn forwards a set CLAUDE_CONFIG_DIR onto the launch without requiring it
...
# bin/xo-bootstrap.sh lists node in COMMON_TOOLS and reports it at setup, which is
...
# Read-modify-write, then read back and confirm. xo-spawn runs from a live
# xo Claude Code session that writes this same file, so the store can move
...
# the dialog this registration removes, which reaches xo as an ordinary
...
const attempt = () => {
  ...
  const tmp = path.join(path.dirname(store), `.claude.json.xo-trust.${unique}`);
