...
# repository (the xo checkout) and be synced under that directory's label.
# Anything else is reported as "skipped: not a clone root" naming the repository
# that would have been touched.
# Pruning never deletes the checked-out branch or a branch that still has a
# worktree, so it cannot discard unlanded work; set XO_FLEET_PRUNE=0 to disable it.
...
# fetch_with_packed_refs_lock_guard and the XO_FLEET_SYNC_PACKED_REFS_LOCK_* knobs.
# Usage: xo-fleet-sync.sh [<project-dir-or-name>]
# The single-project form accepts either a path (absolute, or relative to the
# caller's cwd) or a bare "<name>"/"projects/<name>" form, resolved against
# this home's projects dir ($XO_HOME/projects, or $XO_PROJECTS_OVERRIDE).
...
# `xo-fleet-sync.sh dotfiles-private` syncs just that one clone, same as
...
# Inert unless XO_TIMING_LOG names a file; only the deferred network stage sets it.
# shellcheck source=bin/xo-timing-lib.sh
. "$SCRIPT_DIR/xo-timing-lib.sh"
XO_LOCK_LOG_PREFIX=fleet-sync
...
FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=${XO_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES:-3}
FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=${XO_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS:-1}
FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS=${XO_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS:-30}
...
usage() {
  echo "usage: xo-fleet-sync.sh [<project-dir-or-name>]" >&2
}
...
# is provably stale per xo-lock-lib.sh (still present, mtime age past the
...
fetch_with_packed_refs_lock_guard() {
  ...
  if [ -n "$lock" ] && [ -e "$lock" ]; then
    if xo_lock_is_provably_stale "$lock" "$PROJ" "$FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS"; then
      if ! rm -f "$lock"; then
...
prune_gone_branches() {
  ...
  # net. Set XO_FLEET_PRUNE=0 to skip pruning entirely.
  [ "${XO_FLEET_PRUNE:-1}" != "0" ] || return 0
...
sync_project() {
  ...
  # resolves to the ENCLOSING repository, which in an xo home is the
  # xo checkout itself. Every later `git -C "$PROJ"` would then read, prune
  ...
  mode_line=$("$XO_ROOT/bin/xo-project-mode.sh" "$label" 2>/dev/null || echo "no-mistakes off")
  mode=${mode_line%% *}
...
for proj in "$PROJECTS"/*; do
  ...
  sync_project "$proj"
  xo_timing_record clone sync "$__xo_timing_stamp" "$(basename "$proj")"
done
