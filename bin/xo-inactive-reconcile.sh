#!/usr/bin/env bash
# xo-inactive-reconcile.sh - bounded reconciliation of suspicious inactive terminal outcomes.
#
# Usage:
#   xo-inactive-reconcile.sh scan [--startup]
#   xo-inactive-reconcile.sh report <task-id>
#   xo-inactive-reconcile.sh acknowledge <fingerprint>
#
...
# bin/xo-parent-channel-lib.sh as
...
# report pointer, without consulting xo-crew-state.sh and without waiting for
...
# caller that already holds the child's meta lock, which bin/xo-teardown.sh
...
# XO_INACTIVE_RECONCILE_SECS (default 900, valid 60..1800) per home, except
# that --startup performs the same scan immediately in the locked session
# start's deferred worker. Each scan uses an aggregate
# XO_INACTIVE_RECONCILE_BUDGET_SECS deadline (default 10, valid 1..30) and
...
# xo-crew-state.sh as the sole current-state source.
...
# through bin/xo-parent-channel-lib.sh; the ledger-first path and the inactive
# path share the same receipt store.
# In a main home, a presentation-stage record is acknowledged by xo-wake-drain
...
#
# New xo-terminal-outcome.v1 receipts contain schema, fingerprint, task_id,
...
#
# The scan reads only durable local state and xo-crew-state.sh; it never invokes
# gh, gh-axi, curl, xo-pr-check.sh, xo-pr-poll.sh, or a state *.check.sh.
set -u
export LC_ALL=C
...
CREW_STATE_BIN="${XO_INACTIVE_CREW_STATE_BIN:-$SCRIPT_DIR/xo-crew-state.sh}"
...
XO_INACTIVE_RECONCILE_SECS=${XO_INACTIVE_RECONCILE_SECS:-900}
case "$XO_INACTIVE_RECONCILE_SECS" in
  ''|*[!0-9]*|0)
    printf 'xo-inactive-reconcile: XO_INACTIVE_RECONCILE_SECS must be a whole number from 60 to 1800\n' >&2
    exit 2
    ;;
esac
if [ "$XO_INACTIVE_RECONCILE_SECS" -lt 60 ] || [ "$XO_INACTIVE_RECONCILE_SECS" -gt 1800 ]; then
  printf 'xo-inactive-reconcile: XO_INACTIVE_RECONCILE_SECS must be a whole number from 60 to 1800\n' >&2
  exit 2
fi
XO_INACTIVE_RECONCILE_BUDGET_SECS=${XO_INACTIVE_RECONCILE_BUDGET_SECS:-10}
case "$XO_INACTIVE_RECONCILE_BUDGET_SECS" in
  ''|*[!0-9]*|0)
    printf 'xo-inactive-reconcile: XO_INACTIVE_RECONCILE_BUDGET_SECS must be a whole number from 1 to 30\n' >&2
    exit 2
    ;;
esac
if [ "$XO_INACTIVE_RECONCILE_BUDGET_SECS" -gt 30 ]; then
  printf 'xo-inactive-reconcile: XO_INACTIVE_RECONCILE_BUDGET_SECS must be a whole number from 1 to 30\n' >&2
  exit 2
fi
...
reconcile_now() {
  case "${XO_INACTIVE_RECONCILE_NOW:-}" in
    ''|*[!0-9]*) date +%s ;;
    *) printf '%s\n' "$XO_INACTIVE_RECONCILE_NOW" ;;
  esac
}
...
ensure_record() { # <fingerprint> <task> <incarnation> <state> <outcome-key> <origin> <phase> <pr> [status-head]
  ...
  {
    printf 'schema=xo-terminal-outcome.v1\n'
...
queue_key_exists() { # <key>
  local key=$1 queued
  queued=$(xo_wake_queued_keys check 2>/dev/null || true)
...
publish_actionable() { # <key> <payload>
  ...
  xo_wake_append check "$key" "$payload" || return 2
  printf 'actionable: %s\n' "$payload"
}
...
home_secondmate_id() {
  xo_parent_channel_home_id "$XO_HOME"
}
...
report_to_parent() { # <task> <state> <outcome-key> <fingerprint> <pr>
  ...
  xo_parent_channel_report "$XO_HOME" "$STATE" "$line"
}
...
notice_parent_report_failed() { # <record> <fingerprint> <payload>
  ...
  if ! xo_secondmate_parent_record_parse "$XO_HOME/.xo-secondmate-parent"; then
    payload="$payload (missing or unreadable parent binding .xo-secondmate-parent)"
  fi
...
child_terminal_ledger_line() { # <status>
  local status=$1 snapshot last marker='__XO_LEDGER_SNAPSHOT_END__'
...
report_child_ledger_locked() { # <id> <meta>
  ...
  if xo_parent_channel_report "$XO_HOME" "$STATE" "$line"; then
    mark_reported "$RECORD_PENDING" || return 1
    return 0
  fi
...
ledger_pass() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    lock=$(xo_meta_lock_path "$meta") || continue
    xo_lock_try_acquire "$lock" || continue
    if [ ! -f "$meta" ] || [ -L "$meta" ] \
...
reconcile_direct_child_locked() { # <id> <meta> <secondmate-id-or-empty> <timeout>
  ...
  [ "$age" -ge "$XO_INACTIVE_RECONCILE_SECS" ] || return 0
  state_line=$(xo_run_timed "$timeout" env XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
...
scan() {
  ...
  if [ "$startup" != 1 ] && [ "$(scan_marker_age)" -lt "$XO_INACTIVE_RECONCILE_SECS" ]; then
    return 0
  fi
  ...
  if [ -z "$self" ] && [ "$marker_rc" -ne 1 ]; then
    publish_actionable "inactive-reconcile-diagnostic:invalid-secondmate-home" \
      "inactive terminal outcomes remain unreconciled: invalid .xo-secondmate-home marker" || true
    return 0
  fi
  deadline=$(( $(date +%s) + XO_INACTIVE_RECONCILE_BUDGET_SECS ))
...
case "$mode" in
  scan)
    ...
    case "${2:-}" in
      '') ;;
      --startup) startup=1 ;;
      *) printf 'usage: xo-inactive-reconcile.sh scan [--startup]\n' >&2; exit 2 ;;
    esac
    ...
    if xo_run_timed $((XO_INACTIVE_RECONCILE_BUDGET_SECS + 1)) "$0" _scan-locked "$startup"; then
      :
    elif [ "$?" -ne 124 ]; then
      exit 1
    fi
  ...
  _scan-locked)
    [ "$#" -eq 2 ] || exit 2
    xo_lock_acquire_wait "$SCAN_LOCK" || exit 1
    trap 'xo_lock_release "$SCAN_LOCK"' EXIT
    scan "$2"
    ;;
  report)
    if [ "$#" -ne 2 ] || ! valid_id "$2"; then
      printf 'usage: xo-inactive-reconcile.sh report <task-id>\n' >&2
      exit 2
    fi
    report_child "$2"
    ;;
  acknowledge)
    [ "$#" -eq 2 ] || { printf 'usage: xo-inactive-reconcile.sh acknowledge <fingerprint>\n' >&2; exit 2; }
    xo_lock_acquire_wait "$SCAN_LOCK" || exit 1
    trap 'xo_lock_release "$SCAN_LOCK"' EXIT
    acknowledge "$2"
    ;;
  ...
  *)
    printf 'usage: xo-inactive-reconcile.sh scan [--startup]\n' >&2
    printf '       xo-inactive-reconcile.sh acknowledge <fingerprint>\n' >&2
    exit 2
    ;;
esac
