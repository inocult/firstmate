...
# in a genuine xo primary home.
# This file is sourced by hook entrypoints and has no side effects on source.

# Return 0 when $1 carries a genuine secondmate-home marker.
xo_root_is_secondmate_home() {
  local marker="$1/.xo-secondmate-home" id LC_ALL=C
  [ -L "$marker" ] && return 1
  [ -f "$marker" ] || return 1
...
xo_primary_scope_matches() {
  local root=$1 state=$2 git_dir git_common_dir
  if ! xo_root_is_secondmate_home "$root"; then
