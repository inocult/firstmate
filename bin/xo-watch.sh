...
# beyond XO_PAUSE_RESURFACE_SECS cannot extend the ordinary recheck cadence, and
...
#                          xo hands it to a no-mistakes validation. A declared
...
#                          count in the reason; at XO_WEDGE_DEMAND_INSPECT_COUNT
...
#   stale: <window> (unread xo instruction: ...)
...
#                          successful attempts never wake xo
#                          (bin/xo-task-inbox-lib.sh owns the ladder policy)
...
#                          for it (bin/xo-procevent.sh reconcile queues it
...
#                          (bin/xo-procevent.sh reconcile queues it once per
...
#                          between observations for XO_SECONDMATE_WAKE_STALL_SECS
...
. "$SCRIPT_DIR/xo-push-transition-lib.sh"
# shellcheck source=bin/xo-pr-lib.sh
. "$SCRIPT_DIR/xo-pr-lib.sh"
# Only for the arm-time check on XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS below;
# the per-cycle reconcile itself runs as a separate process.
# shellcheck source=bin/xo-procevent-lib.sh
. "$SCRIPT_DIR/xo-procevent-lib.sh"
# Single owner of durable merge-outcome publication, shared with
# bin/xo-pr-merge.sh so self and poll origins use the same role-routed outcome.
...
# shellcheck source=/dev/null
. "$SCRIPT_DIR/xo-merge-outcome-lib.sh"
# shellcheck source=bin/xo-x-lib.sh
. "$SCRIPT_DIR/xo-x-lib.sh"
...
# expectations created by xo-send on marked secondmate requests. The tick is
...
# Steering-inbox loss detection: bin/xo-task-inbox-lib.sh owns the record,
...
# shellcheck source=bin/xo-task-inbox-lib.sh
. "$SCRIPT_DIR/xo-task-inbox-lib.sh"
# The away-posture record (state/.afk-contract) is the posture in both the
# attended and the afk session; bin/xo-afk-contract.sh owns its schema and this
# watcher reads only its presence (afk_record_present below).
# shellcheck source=bin/xo-afk-contract.sh
. "$SCRIPT_DIR/xo-afk-contract.sh"

WATCH_LOCK="$STATE/.watch.lock"
WATCH_PATH="$SCRIPT_DIR/xo-watch.sh"
...
# bin/xo-classify-lib.sh owns status reported-state signatures and presentation
# markers, while bin/xo-wake-lib.sh owns their wake-facing routing, the legacy
# turn-ended signature, annotation staleness checks, and guarded bookkeeping writes.

POLL=${XO_POLL:-15}                   # seconds between cycles
...
# between touches. xo_poll_derived_grace (bin/xo-wake-lib.sh, already sourced
...
# POLL is known.
WATCHER_STALE_GRACE=${XO_WATCHER_STALE_GRACE:-${XO_GUARD_GRACE:-$(xo_poll_derived_grace "$POLL")}}
HEARTBEAT=${XO_HEARTBEAT:-600}        # base seconds between heartbeat scans
HEARTBEAT_MAX=${XO_HEARTBEAT_MAX:-7200}  # heartbeat backoff cap
CHECK_INTERVAL=${XO_CHECK_INTERVAL:-300}  # seconds between *.check.sh sweeps
CHECK_TIMEOUT=${XO_CHECK_TIMEOUT:-30}     # seconds allowed per *.check.sh
HOME_SUMMARY_INTERVAL=${XO_HOME_SUMMARY_INTERVAL:-300}
case "$HOME_SUMMARY_INTERVAL" in
  ''|*[!0-9]*|0) HOME_SUMMARY_INTERVAL=300 ;;
esac
SIGNAL_GRACE=${XO_SIGNAL_GRACE:-30}   # seconds to linger after a signal so trailing
                                      # signals (a status write, then the same turn's
                                      # turn-end hook) coalesce into one wake
TURNEND_CHURN_ABSORB_SECS=${XO_TURNEND_CHURN_ABSORB_SECS:-900}  # longest a task's
                                      # bare turn-ends may be deferred on pane-churn
                                      # evidence alone (signal_turnend_panes_churned)
# Busy state is decided by the semantic contract in bin/xo-busy-lib.sh, which
...
# than wake xo's LLM for each, this watcher classifies every wake in bash
...
# xo-crew-state.sh; where config/turnend-churn-absorb opts in, a bare turn-end alone
...
# (xo-classify-lib.sh) backs the away-mode daemon; while state/.afk exists the
# daemon owns triage, so this watcher reverts to one-shot (enqueue + exit on every
# wake) and never double-triages - and never runs the costly provably-working read.
STALE_ESCALATE_SECS=${XO_STALE_ESCALATE_SECS:-240}  # idle secs before a provably-working stale escalates as a possible wedge
...
BUSY_TURN_MAX_SECS=${XO_BUSY_TURN_MAX_SECS:-3600}
...
SECONDMATE_WAKE_STALL_SECS=${XO_SECONDMATE_WAKE_STALL_SECS:-}
...
PAUSE_RESURFACE_SECS=${XO_PAUSE_RESURFACE_SECS:-$XO_PAUSE_RESURFACE_SECS_DEFAULT}
# A declared wait that names WHEN it clears (`paused: ... until <UTC ISO 8601>`,
# status_paused_until in xo-classify-lib.sh) is condition-aware: it is not
...
# Consecutive event-path failures (xo_backend_wait_transition returning 2 -
...
EVENT_CAP_FAIL_MAX=${XO_EVENT_CAP_FAIL_MAX:-3}
# Per-process memo for the push-capability probe (xo_backend_events_capable runs
...
afk_record_present() { xo_afk_contract_present "$STATE"; }
...
# the semantic busy-state contract (bin/xo-busy-lib.sh). Only an exact busy
...
window_is_busy() {  # <window> <tail40>
  ...
  if [ -n "$task" ] && [ -f "$meta" ]; then
    verdict=$(xo_busy_classify_meta "$meta" "$task" "$STATE" "$tail40")
  else
    verdict=$(xo_busy_classify "$(window_backend "$w")" "$w" "$(window_harness "$w")" \
...
window_kind() {
  local w=$1 meta kind
  meta=$(xo_backend_meta_for_window "$w" "$STATE" 2>/dev/null || true)
  if [ -n "$meta" ]; then
...
inbox_steer_escalate_unavailable() {  # <window> <task> <record>
  ...
  reason="stale: $w (unread xo instruction: $rec is unhandled and the worker's agent has exited or its endpoint is missing, so the doorbell was not typed; recover the worker)"
  if [ ! -d "${rec%/*}" ] || [ ! -f "$rec" ]; then
    xo_task_inbox_due_action "$STATE" "$task" >/dev/null || true
    return 0
  fi
  xo_wake_append stale "$w" "$reason" || exit 1
  if ! xo_task_inbox_record_escalated "$STATE" "$task" "$rec"; then
...
# glob and produces nothing. When the ladder (xo_task_inbox_due_action, the
...
inbox_steer_check() {  # <window> <task>
  ...
  action=$(xo_task_inbox_due_action "$STATE" "$task") || return 0
  verb=${action%% *}
  [ "$verb" != quiet ] || return 0
  rec=${action#* }
  count=
  ...
  agent_state=$(xo_backend_agent_state "$backend" "$w" 2>/dev/null || true)
  case "$agent_state" in
    dead|missing)
  ...
  tail40=$(xo_backend_capture "$backend" "$w" 40 "$(window_label "$w")" 2>/dev/null) || tail40=
  ...
  case "$verb" in
    ring)
      ring_rc=0
      xo_task_inbox_ring "$backend" "$w" "$rec" "$(window_label "$w")" || ring_rc=$?
      if [ "$ring_rc" -eq 3 ]; then
        inbox_steer_escalate_unavailable "$w" "$task" "$rec"
        return 0
      fi
      if ! xo_task_inbox_record_ring "$STATE" "$task" "$rec"; then
        if [ ! -f "$rec" ]; then
          xo_task_inbox_due_action "$STATE" "$task" >/dev/null || true
          return 0
        fi
      ...
      ;;
    escalate)
      reason="stale: $w (unread xo instruction: $rec still unhandled after $count doorbell delivery attempts with an idle pane; inspect the worker)"
      if [ ! -d "${rec%/*}" ] || [ ! -f "$rec" ]; then
        xo_task_inbox_due_action "$STATE" "$task" >/dev/null || true
        return 0
      fi
...
# busy state has no verified source: bin/xo-crew-state.sh can only answer unknown
...
# ride churn evidence for at most XO_TURNEND_CHURN_ABSORB_SECS, tracked per window
...
signal_turnend_panes_churned() {  # <file> ...
  ...
  for meta in "$STATE"/*.meta; do
    ...
    if [ "$backend" = orca ]; then
      terminal=$(xo_meta_get "$meta" terminal)
      w=${terminal:-$(xo_meta_get "$meta" window)}
    else
      w=$(xo_meta_get "$meta" window)
    fi
    key=
  ...
  for task_index in "${churn_indexes[@]}"; do
    ...
    now=$(xo_backend_capture "$backend" "$w" 40 "$label" 2>/dev/null) || return 1
    [ -n "$now" ] || return 1
...
recorded_windows() {
  ...
  for meta in "$STATE"/*.meta; do
    [ -e "$meta" ] || continue
    w=$(xo_backend_target_of_meta "$meta")
    [ -n "$w" ] || continue
    case "$seen" in
      *"|$w|"*) continue ;;
    esac
...
secondmate_in_active_turn() {  # <task> <window>
  ...
  tail40=$(xo_backend_capture "$(window_backend "$w")" "$w" 40 "$(window_label "$w")" 2>/dev/null) || return 1
...
secondmate_wake_stall_tick() {
  ...
  for meta in "$STATE"/*.meta; do
    ...
    home=$(xo_meta_get "$meta" home)
    [ -n "$home" ] || continue
    [ -f "$home/.xo-secondmate-home" ] && [ ! -L "$home/.xo-secondmate-home" ] || continue
    [ "$(cat "$home/.xo-secondmate-home" 2>/dev/null || true)" = "$task" ] || continue
...
EOF
    ...
    if [ -z "$observed_at" ] || [ -z "$observed_key" ] \
      ...
      xo_wake_secondmate_progress_marker_write "$task" "$now" "$row_key" || return 1
...
# Consecutive wedge-escalation count for a window past XO_WEDGE_DEMAND_INSPECT_COUNT
...
# below).
XO_WEDGE_DEMAND_INSPECT_COUNT=${XO_WEDGE_DEMAND_INSPECT_COUNT:-3}
...
resurface_absorbed() {  # <window> <throttle-marker> <age> <reason> [scope] [min-age]
  ...
  xo_wake_append stale "$win" "$reason" || exit 1
...
# xo-classify-lib.sh). The pane and the run step both say nothing is happening;
  ...
  case "$since" in
    ...
    *)
      ...
      if [ "$age" -ge "$STALE_ESCALATE_SECS" ]; then
        ...
        if [ "$n" -ge "$XO_WEDGE_DEMAND_INSPECT_COUNT" ]; then
...
# xo-classify-lib.sh owns that record test), and the backend confidently reports
...
# XO_STALE_ESCALATE_SECS until the captain merged, because the absent agent
...
stale_held_for_merge() {  # <window> <task> <hash>
  ...
  [ "$(xo_backend_agent_alive "$(window_backend "$win")" "$win" 2>/dev/null)" = dead ] || return 1
  key=$(window_key "$win")
...
handle_paused_stale() {  # <window> <task> <hash>
  ...
  declaration="declared:$(xo_wake_signal_sig "$statusf" || true)"
...
busy_turn_bound_check() {  # <window> <task> <hash> <since-file> <escalation-file>
  ...
  if status_is_paused_or_captain_held "$(last_status_line "$statusf")"; then
    if afk_present; then
      ...
      # wait itself once per XO_STALE_ESCALATE_SECS for as long as the wait lasts.
      ...
      clear_write_tracking "$key"
      declared="declared:$(xo_wake_signal_sig "$statusf" || true)"
      ...
      if [ "$(cat "$STATE/.stale-$key" 2>/dev/null || true)" != "$declared" ]; then
        xo_wake_append stale "$win" "stale: $win" || exit 1
        printf '%s' "$declared" > "$STATE/.stale-$key"
        wake "stale: $win"
      fi
      return 0
...
# After xo-crew-state has fallen back to stopped or unknown, paused classification is
...
pause_state_class() {  # <window> <task>
  ...
  if [ -e "$STATE/.paused-$key" ] && [ "$(age_of "$recheck_file")" -lt "$STALE_ESCALATE_SECS" ]; then
    if [ "$kind" != secondmate ]; then
      agent_alive=$(xo_backend_agent_alive "$(window_backend "$win")" "$win" 2>/dev/null) || agent_alive=unknown
...
# there is: once xo hands work to the captain, the wait is written into the
# BACKLOG by bin/xo-captain-hold.sh, and the worker's last line stays whatever it
...
task_captain_call_open() {  # <task>
  ...
  CAPTAIN_CALL_IDENTITY=
  [ -n "$task" ] || return 1
  CAPTAIN_CALL_IDENTITY=$(XO_HOME="$XO_HOME" "$SCRIPT_DIR/xo-captain-hold.sh" \
...
stale_wait_declaration() {  # <task>
  printf 'declared:%s' "$(xo_wake_signal_sig "$STATE/$1.status" || true)"
}
...
captain_call_declaration() {  # <task> <call-identity>
  printf 'captain-hold:%s:%s' "$2" "$(xo_wake_signal_sig "$STATE/$1.status" || true)"
}
...
# Surface a stale pane no classifier could resolve, so xo inspects it: it
...
# without that bound one wait re-alarms xo for its whole duration.
...
# above): the status line the worker declared, and the backlog hold xo
...
scan_signals() {
  ...
  for f in "$STATE"/*.status "$STATE"/*.turn-ended; do
    ...
    sig=$(xo_wake_signal_sig "$f") || continue
    [ -n "$sig" ] || continue
    sf=$(xo_wake_signal_seen_path "$STATE" "$f")
    case "$f" in
      *.status) xo_wake_signal_seen_current "$STATE" "$f" && continue ;;
...
# Deliver a durably queued process-event result to xo. Publication is
# owned by bin/xo-procevent.sh - by the runner at capture time and by reconcile's
...
# happens to wake xo, which is exactly the missed delivery this repairs.
...
procevent_surface_queued() {
  ...
  done < <(xo_wake_queued_keys_locked check)
  ...
  XO_WAKE_POST_OUTPUT_ACTION=procevent_surface_after_output
  wake "$reason"
}
...
run_check_process() {
  local c=$1
  shift
  if [ "${XO_CHECK_FORCE_FALLBACK:-0}" != 1 ] && command -v timeout >/dev/null 2>&1; then
    exec timeout "$CHECK_TIMEOUT" bash "$c" "$@"
  elif [ "${XO_CHECK_FORCE_FALLBACK:-0}" != 1 ] && command -v gtimeout >/dev/null 2>&1; then
    exec gtimeout "$CHECK_TIMEOUT" bash "$c" "$@"
  else
    # shellcheck disable=SC2016  # single quotes are deliberate: Perl expands its own variables.
    exec perl -e 'my $t = shift; my $owned = shift; my $pid = fork; die "fork failed" unless defined $pid; if (!$pid) { setpgrp(0, 0) unless $owned; exec @ARGV } my $group = $owned ? getpgrp(0) : $pid; my $stop = sub { $SIG{HUP} = $SIG{INT} = $SIG{TERM} = "IGNORE"; kill "TERM", -$group; select undef, undef, undef, 0.2; kill "KILL", -$group; waitpid $pid, 0; exit 124 }; local $SIG{ALRM} = $stop; local $SIG{HUP} = $stop; local $SIG{INT} = $stop; local $SIG{TERM} = $stop; alarm $t; waitpid $pid, 0; exit($? >> 8)' "$CHECK_TIMEOUT" "${XO_CHECK_OWNED_GROUP:-0}" bash "$c" "$@"
  fi
}
...
XO_ACTIVE_CHECK_PID=
XO_ACTIVE_CHECK_PGID=
XO_CHECK_OUTPUT=
XO_CHECK_RESULT=
XO_CHECK_SIGNAL_PENDING=

xo_check_output_cleanup() {
  [ -z "$XO_CHECK_OUTPUT" ] || rm -f -- "$XO_CHECK_OUTPUT"
  XO_CHECK_OUTPUT=
}

xo_active_check_stop() {
  local pid=${XO_ACTIVE_CHECK_PID:-} pgid=${XO_ACTIVE_CHECK_PGID:-} i
...
run_check_capture() {
  ...
  xo_check_output_cleanup
  XO_CHECK_RESULT=
  XO_CHECK_OUTPUT=$(mktemp "$STATE/.xo-check-output.XXXXXX") || return 1
  chmod 0600 "$XO_CHECK_OUTPUT" || { xo_check_output_cleanup; return 1; }
  XO_CHECK_SIGNAL_PENDING=
  trap 'XO_CHECK_SIGNAL_PENDING=1' HUP INT TERM
  set -m
  ( XO_CHECK_OWNED_GROUP=1 run_check_process "$@" ) > "$XO_CHECK_OUTPUT" 2>/dev/null &
  XO_ACTIVE_CHECK_PID=$!
  XO_ACTIVE_CHECK_PGID=$XO_ACTIVE_CHECK_PID
  set +m
  pgid=$(ps -o pgid= -p "$XO_ACTIVE_CHECK_PID" 2>/dev/null | tr -d '[:space:]')
  trap 'exit 1' HUP INT TERM
  if [ -n "$pgid" ] && [ "$pgid" != "$XO_ACTIVE_CHECK_PGID" ]; then
    xo_active_check_stop || true
    xo_check_output_cleanup
    return 1
  fi
  [ -z "$XO_CHECK_SIGNAL_PENDING" ] || exit 1
  wait "$XO_ACTIVE_CHECK_PID" 2>/dev/null || true
  XO_ACTIVE_CHECK_PID=
  xo_active_check_stop || return 1
  XO_CHECK_RESULT=$(cat "$XO_CHECK_OUTPUT" 2>/dev/null || true)
  xo_check_output_cleanup
}
...
# classified-position field in that file's .seen-* marker, and xo-classify-lib.sh's
...
# Also populates XO_SIGNAL_NEEDS_DECISION_FILES (space-separated status-file
...
signal_files_actionable() {  # <status-file> ...
  ...
  XO_SIGNAL_SURFACE_ENDPOINTS=''
  XO_SIGNAL_NEEDS_DECISION_FILES=''
  for f in "$@"; do
    ...
    status_span_first_actionable_record "$f" \
      "$(xo_wake_signal_seen_size "$STATE" "$f")" record needs_decision
    rc=$?
    ...
    XO_SIGNAL_SURFACE_ENDPOINTS="${XO_SIGNAL_SURFACE_ENDPOINTS}${f}"$'\t'"${endpoint}"$'\t'"${ident}"$'\n'
    if [ "$needs_decision" -eq 1 ]; then
      XO_SIGNAL_NEEDS_DECISION_FILES="${XO_SIGNAL_NEEDS_DECISION_FILES} ${f}"
    fi
...
# xo-push-transition-lib.sh because push and poll paths must write one format.
...
$XO_HEARTBEAT_SURFACE_ENDPOINTS
EOF
  return "$rc"
}
...
# surfaced to xo (.hb-surfaced-<task>). It walks every log rather than only
...
# surfaced when it wakes xo, this normally finds nothing and the heartbeat
...
heartbeat_scan_finds_actionable() {
  ...
  for f in "$STATE"/*.status; do
    ...
    if [ "$rc" -eq 2 ]; then
      ...
      XO_HEARTBEAT_SURFACE_ENDPOINTS="${XO_HEARTBEAT_SURFACE_ENDPOINTS}${f}"$'\t'"ERROR"$'\t'"${sig}"$'\n'
      found=0
      continue
    fi
    endpoint=${record%%$'\t'*}; rest=${record#*$'\t'}; ident=${rest%%$'\t'*}
    XO_HEARTBEAT_SURFACE_ENDPOINTS="${XO_HEARTBEAT_SURFACE_ENDPOINTS}${f}"$'\t'"${endpoint}"$'\t'"${ident}"$'\n'
    [ "$rc" -eq 0 ] && found=0
  done
  return "$found"
}
...
event_wait_or_sleep() {
  ...
  while IFS= read -r w; do
    b=$(window_backend "$w")
    xo_backend_has_push "$b" || continue
  ...
  # Memoized capability probe (xo_backend_events_capable runs a heavy schema
  ...
  if [ "$_event_cap_key" != "$first_backend:$first_session" ]; then
    ...
    if xo_backend_events_capable "$first_backend" "$first_session"; then
      _event_cap_ok=1
    else
      _event_cap_ok=0
    fi
  ...
  rec=$(XO_BACKEND_EVENTS_CAPABILITY_CONFIRMED=1 xo_backend_wait_transition "$first_backend" "$first_session" "$POLL" "$STATE" "${windows[@]}")
  rc=$?
  case "$rc" in
    0)
      _event_cap_fails=0
...
# XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS is validated here, at arm time, and an
...
# reason is specific: every supervision cycle runs `xo-procevent.sh reconcile`
...
# the liveness guard's WATCHER DOWN banner in xo's own session. The
# message shape is reconcile's own, so the operator reads one refusal in both
# places. The refusal goes to stdout because bin/xo-watch-arm.sh relays the
# child's stdout and recognises `watcher: FAILED` as the typed failure line.
if ! xo_procevent_launch_confirm_seconds >/dev/null; then
  echo "watcher: FAILED - XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS must be whole seconds from $XO_PROCEVENT_LAUNCH_CONFIRM_MIN_SECONDS to $XO_PROCEVENT_LAUNCH_CONFIRM_MAX_SECONDS"
  exit 1
fi

if ! xo_lock_try_acquire "$WATCH_LOCK"; then
  BEAT="$STATE/.last-watcher-beat"
  if [ -n "${XO_LOCK_HELD_PID:-}" ]; then
    if [ -e "$BEAT" ]; then
      beat_age=$(xo_path_age "$BEAT")
      if [ "$beat_age" -ge "$WATCHER_STALE_GRACE" ]; then
        echo "watcher: lock held by live pid $XO_LOCK_HELD_PID but heartbeat is stale for ${beat_age}s (>${WATCHER_STALE_GRACE}s); inspect or stop that watcher before re-arming." >&2
        exit 1
      fi
    elif [ "$(xo_path_age "$WATCH_LOCK")" -ge "$WATCHER_STALE_GRACE" ]; then
      echo "watcher: lock held by live pid $XO_LOCK_HELD_PID but no heartbeat exists; inspect or stop that watcher before re-arming." >&2
      exit 1
    fi
    echo "watcher: already running pid $XO_LOCK_HELD_PID"
  else
...
if [ -n "${XO_LOCK_RECOVERED_PID:-}" ]; then
  WATCHER_RECOVERY_PENDING=1
fi
if [ "${XO_WATCH_HANDLING_SUCCESSOR:-0}" != 1 ]; then
  if ! xo_recovery_marker_reopen_announced "$WATCHER_DOWNTIME_MARKER"; then
    echo "watcher: recovery state could not be reopened safely; retaining stale lock evidence" >&2
    exit 1
  fi
fi
if ! xo_recovery_marker_arm_check "$WATCHER_DOWNTIME_MARKER"; then
  echo "watcher: recovery state could not be consumed safely; retaining stale lock evidence" >&2
  exit 1
fi
if [ "${XO_WATCH_HANDLING_SUCCESSOR:-0}" = 1 ]; then
  WATCHER_RECOVERY_PENDING=0
elif [ "$XO_RECOVERY_MARKER_ACTION" = recover ]; then
  WATCHER_RECOVERY_PENDING=1
fi
# Side-band ledger publication, detached from the poll loop.
#
# The poll loop owns the liveness beacon below, and xo-guard.sh reads that
# beacon's freshness as proof that supervision is alive. Publication is a
# side-band nicety bounded by XO_HOME_SUMMARY_TIMEOUT, but that bound is far
...
home_summary_refresh_detached() {
  ...
  XO_HOME_SUMMARY_IF_IDLE=1 \
    "$SCRIPT_DIR/xo-home-summary-refresh.sh" --best-effort </dev/null >/dev/null 2>&1 &
  HOME_SUMMARY_PID=$!
}
...
reconcile_requests_detached() {
  ...
  XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/xo-secondmate-reconcile.sh" process-requests </dev/null >/dev/null 2>&1 &
  RECONCILE_REQUEST_PID=$!
}
...
watcher_cleanup() {
  ...
  if [ "$(cat "$WATCH_LOCK/pid" 2>/dev/null || true)" = "${WATCHER_PID:-}" ]; then
    ...
    if [ "${WATCHER_RECOVERY_PENDING:-0}" -eq 1 ] \
      && [ "${XO_WATCH_DELIVERED_REASON:-}" = "check: rearm-resurface" ]; then
      transition=release-lock-existing
    fi
  fi
  xo_active_check_stop || cleanup_status=1
  xo_check_output_cleanup
  xo_custom_check_snapshot_cleanup
  if [ "$owns_lock" -eq 1 ] \
    && ! xo_recovery_transition "$WATCHER_DOWNTIME_MARKER" "$transition" "$WATCH_LOCK" downtime; then
...
trap 'exit 1' HUP INT TERM
# This watcher's own pid, as recorded in the lock by xo_lock_claim (which writes
# ${BASHPID:-$$} from this same main shell). Read directly, never via a command
# substitution, so it matches the stored holder pid for the self-eviction check.
WATCHER_PID=${BASHPID:-$$}
printf '%s\n' "$XO_HOME" > "$WATCH_LOCK/xo-home" || true
printf '%s\n' "$WATCH_PATH" > "$WATCH_LOCK/watcher-path" || true
# shellcheck disable=SC2034 # Consumed by wake() in the separately linted transition owner.
XO_WATCH_DELIVERY_PID=$WATCHER_PID
XO_WATCH_DELIVERY_IDENTITY=$(xo_pid_identity "$WATCHER_PID" 2>/dev/null || true)
printf '%s\n' "$XO_WATCH_DELIVERY_IDENTITY" > "$WATCH_LOCK/pid-identity" 2>/dev/null || true
...
if ! xo_pr_poll_retirement_recover_all "$STATE" "$SCRIPT_DIR/xo-pr-poll.sh"; then
  reason="check: rejected unauthenticated PR poll retirement receipts:$XO_PR_POLL_RETIREMENT_REJECTED"
  xo_wake_append check pr-poll-retirement "$reason" || exit 1
  touch "$STATE/.last-check"
  wake "$reason"
fi

# Shared by both the first-notification and already-notified paths below so
# the retirement sequence (bin/xo-pr-lib.sh) is stated once.
retire_merged_pr_poll() {  # <id>
  local id=$1
  if xo_pr_poll_retirement_publish "$STATE" "$id" "$SCRIPT_DIR/xo-pr-poll.sh" merged; then
...
resurface_after_downtime() {
  ...
  if [ "$WATCHER_RECOVERY_PENDING" -ne 1 ]; then
    ...
    [ "$XO_RECOVERY_MARKER_ACTION" = recover ] || return 0
  fi
...
while :; do
  ...
  # Liveness beacon for xo-guard.sh: a fresh mtime here means a watcher is
  ...
  xo_pending_reply_tick "$STATE" || true
  ...
  if [ -d "$STATE/procevent" ]; then
    XO_HOME="$XO_HOME" "$SCRIPT_DIR/xo-procevent.sh" reconcile >/dev/null 2>&1 || true
  fi
  ...
  # was created, so quiet cycles never wake xo or consume model tokens.
  inactive_out=
  if inactive_out=$(XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/xo-inactive-reconcile.sh" scan 2>/dev/null); then
  ...
  # Slow per-task checks (xo writes these, e.g. a merged-PR poll).
  ...
  if [ "$(age_of "$STATE/.last-check")" -ge "$CHECK_INTERVAL" ]; then
    ...
    for c in "$STATE"/*.check.sh; do
      ...
      if [ "$(basename "$c")" = x-watch.check.sh ]; then
        ...
        if xo_pr_poll_snapshot_capture "$STATE" "$id" "$SCRIPT_DIR/xo-pr-poll.sh"; then
          is_pr_poll=1
          provider=$XO_PR_POLL_SNAPSHOT_PROVIDER
          url=$XO_PR_POLL_SNAPSHOT_URL
          host=$XO_PR_POLL_SNAPSHOT_HOST
          path=$XO_PR_POLL_SNAPSHOT_PATH
          number=$XO_PR_POLL_SNAPSHOT_NUMBER
          run_check_capture "$SCRIPT_DIR/xo-pr-poll.sh" --validated \
            "$provider" "$url" "$host" "$path" "$number" || exit 1
          out=$XO_CHECK_RESULT
        elif xo_custom_check_snapshot_prepare "$STATE" "$id"; then
          custom_snapshot=$XO_CUSTOM_CHECK_SNAPSHOT
      ...
      if [ -n "$out" ]; then
        ...
        if [ "$is_pr_poll" -eq 1 ] && [ "$out" = merged ]; then
          merge_outcome_rc=0
          xo_merge_outcome_report "$XO_HOME" "$STATE" "$id" "$url" poll \
            || merge_outcome_rc=$?
          ...
          retire_merged_pr_poll "$id"
          touch "$STATE/.last-check"
          if [ "$XO_MERGE_OUTCOME_ALREADY_RECORDED" = true ]; then
            triage_log "absorbed duplicate merged PR poll result for $id"
            continue
          fi
          wake "$reason"
        fi
        xo_wake_append check "$c" "$reason" || exit 1
        touch "$STATE/.last-check"
        wake "$reason"
      fi
    done
    if [ -n "$rejected_checks" ]; then
      ...
      xo_wake_append check unauthenticated-state-checks "$reason" || exit 1
      touch "$STATE/.last-check"
      wake "$reason"
    fi
    touch "$STATE/.last-check"
  ...
  # costs a full xo turn each. The re-scan also picks up a newer
...
EOF
    ...
    # decision-owned row off the supervision branch (xo-branch-dispatch.ts,
    # xo-primary-pi-watch.ts), and the away daemon, whose handle_durable_wakes
    # passes it to handle_wake (see the comment above handle_wake in
    # bin/xo-supervise-daemon.sh).
    ...
    if afk_present || [ "$signal_actionable" -eq 0 ] \
      ...
      while IFS=$(printf '\t') read -r sf sig f; do
        [ -n "$sf" ] || continue
        file_reason="$reason"
        case " $XO_SIGNAL_NEEDS_DECISION_FILES " in *" $f "*) file_reason="needs-decision:$files" ;; esac
        xo_wake_append signal "$(basename "$f")" "$file_reason" || exit 1
      done <<EOF
$pending
EOF
      ...
      while IFS=$(printf '\t') read -r sf sig f; do
        ...
        case "$f" in
          *.status)
            xo_wake_status_reported_commit "$STATE" "$f" "$sig" || true
...
EOF
      while IFS=$(printf '\t') read -r f surface_end surface_ident; do
        [ -n "$f" ] || continue
        xo_wake_status_seen_commit "$STATE" "$f" "$surface_end" "$surface_ident" || true
...
EOF
      ...
      while IFS=$(printf '\t') read -r f surface_end surface_ident; do
        [ -n "$f" ] || continue
        xo_wake_status_seen_commit "$STATE" "$f" "$surface_end" "$surface_ident" \
...
EOF
      if [ "$signal_commit_error" -ne 0 ]; then
        while IFS=$(printf '\t') read -r sf sig f; do
          [ -n "$sf" ] || continue
          xo_wake_append signal "$(basename "$f")" "$reason" || exit 1
        done <<EOF
$pending
EOF
        wake "$reason"
  ...
  while IFS= read -r w; do
    ...
    tail40=$(xo_backend_capture "$(window_backend "$w")" "$w" 40 "$(window_label "$w")" 2>/dev/null) || continue
    ...
    if [ "$h" = "$prev" ]; then
      ...
      if [ "$n" -ge 2 ] && [ "$busy_now" -ne 0 ]; then
        ...
        # xo. Detection itself is unchanged from above.
        if [ "$kind" = secondmate ]; then
          ...
          if captain_held_silenced "$last"; then
            ...
            xo_wake_append stale "$w" "stale: $w" || exit 1
            printf '%s' "$h" > "$sf"
            wake "stale: $w"
          fi
          ...
          # new entry once xo hands it to a no-mistakes validation
          ...
          # authoritative source xo-crew-state.sh itself already prioritizes
          ...
          #     Surface immediately so xo inspects the inconclusive state
  ...
  if [ "$(age_of "$STATE/.last-heartbeat")" -ge "$hb" ]; then
    ...
    # every heartbeat.
    if afk_present; then
      xo_wake_append heartbeat heartbeat heartbeat || exit 1
      ...
      # this wake sends xo to the whole fleet, so every log is read.
      xo_wake_append heartbeat heartbeat heartbeat || exit 1
      touch "$STATE/.last-heartbeat"
