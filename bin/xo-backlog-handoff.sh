#!/usr/bin/env bash
# Hand already-identified, in-scope backlog items off from the main xo
...
#
# Scope-matching is xo's JUDGMENT: you pass the task-id keys you have
...
#     (.xo-secondmate-home marker, AGENTS.md + bin/), never a project clone, the
#     active home, or the xo repo;
...
# everywhere; the `config/backlog-backend=manual` knob only governs xo's
...
# then an idempotent confined transfer and xo-backlog-receive.sh deliver it.
...
# Usage: xo-backlog-handoff.sh <secondmate-id> <item-key>...
#        xo-backlog-handoff.sh --resume-pending
set -eu
...
RECEIVER_WAKE_MESSAGE='New routed work is in your backlog. Run bin/xo-session-start.sh now, then act on the routed task.'
...
release_remote_locks() {
  if [ -n "$ACTIVE_HANDOFF_LOCK" ]; then
    xo_lock_release "$ACTIVE_HANDOFF_LOCK"
    ACTIVE_HANDOFF_LOCK=
  fi
  if [ -n "$ACTIVE_REGISTRY_LOCK" ]; then
    xo_lock_release "$ACTIVE_REGISTRY_LOCK"
    ACTIVE_REGISTRY_LOCK=
  fi
}
...
if [ "${1:-}" = --resume-pending ]; then
  [ "$#" -eq 1 ] || { echo "usage: xo-backlog-handoff.sh --resume-pending" >&2; exit 1; }
  RESUME_PENDING=1
  ID=
  shift
else
  [ "$#" -ge 2 ] || { echo "usage: xo-backlog-handoff.sh <secondmate-id> <item-key>..." >&2; exit 1; }
  ID=$1
...
validate_secondmate_home() {
  ...
  if [ ! -f "$abs_home/.xo-secondmate-home" ]; then
    echo "error: xo home $home is not a seeded secondmate home" >&2
    return 1
  fi
  marker_id=$(cat "$abs_home/.xo-secondmate-home" 2>/dev/null || true)
...
warn_stale_public_commitments() { # <secondmate-id> <moved-key>...
  ...
  for key in "$@"; do
    rc=0
    out=$("$SCRIPT_DIR/xo-public-followup.sh" guard-work main "$key" 2>/dev/null) || rc=$?
    [ "$rc" -ne 0 ] || continue
    [ -z "$out" ] || printf '%s\n' "$out" >&2
    printf 'warning: %s still owes a public reply bound to main/%s; rebind it to secondmate:%s (tasks-axi public-followup bind-work, then bin/xo-public-followup.sh register <obligation-id> --relation <relation-id> --work-home secondmate:%s --work-id %s --generation <n>) or the promised reply will be reconciled against work this home no longer owns.\n' \
      "$key" "$key" "$id" "$id" "$key" >&2
  done
  if xo_pf_relay_active "$XO_HOME" && xo_pf_has_delivered_open_loops "$STATE"; then
    printf 'warning: this home has an open public loop with nothing owed; routing work to secondmate:%s does not close it. Hand it on with bin/xo-public-followup.sh rechain or close it with retire --reason.\n' \
      "$id" >&2
  fi
...
receiver_wake_mark() { # <secondmate-id> <prepared|pending> [batch-id]
  ...
  corr=$(xo_pending_reply_create "$XO_HOME" "$STATE" "$id" "$RECEIVER_WAKE_MESSAGE") || return 1
  ...
  if ! receiver_wake_state_write "$id" "$wake_state"; then
    xo_pending_reply_discard_undelivered "$STATE" "$corr" || true
    return 1
  fi
}
...
receiver_wake_discard_prepared() { # <secondmate-id>
  ...
  xo_pending_reply_discard_undelivered "$STATE" "$corr" || return 1
  rm -f -- "$marker"
}
...
receiver_wake_pending_valid() { # <secondmate-id>
  ...
  rec=$(xo_pending_reply_path "$STATE" "$corr")
  [ -f "$rec" ] && [ ! -L "$rec" ] \
    && [ "$(xo_pending_reply_get "$rec" task_id)" = "$id" ] || return 1
  delivered=$(xo_pending_reply_get "$rec" delivered_epoch)
  [ -z "$delivered" ] || return 1
  xo_pending_reply_corr_reusable "$STATE" "$corr" "$id"
}
...
wake_secondmate_receiver() { # <secondmate-id> <correlation-id>
  ...
  out=$(XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" XO_ROOT_OVERRIDE="$XO_ROOT" \
    XO_PENDING_REPLY_EXISTING_CORR="$corr" \
    "$SCRIPT_DIR/xo-send.sh" "$id" "$RECEIVER_WAKE_MESSAGE" 2>&1) || rc=$?
  if [ "$rc" -ne 0 ]; then
    [ -z "$out" ] || printf '%s\n' "$out" >&2
...
wake_pending_secondmate_receiver() { # <secondmate-id> [retain-confirmed]
  ...
  [ -f "$rec" ] && [ ! -L "$rec" ] \
    && [ "$(xo_pending_reply_get "$rec" task_id)" = "$id" ] || return 1
  xo_pending_reply_reconcile_delivery "$STATE" "$corr" >/dev/null 2>&1 || true
  delivered=$(xo_pending_reply_get "$rec" delivered_epoch)
  if [ -z "$delivered" ]; then
    xo_pending_reply_corr_reusable "$STATE" "$corr" "$id" || {
...
remote_deliver_outbox() { # <secondmate-id> <outbox-path>
  ...
  snapshot=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-handoff-payload.XXXXXX") || return 1
  ...
  if ! "$SCRIPT_DIR/xo-on.sh" --stdin "$id" xo-remote-file.sh put "$remote_rel" 1048576 \
  ...
  rm -f -- "$snapshot"
  if ! receive_out=$("$SCRIPT_DIR/xo-on.sh" "$id" xo-backlog-receive.sh \
...
remote_handoff() { # <secondmate-id> <keys...>
  ...
  xo_tasks_axi_compatible || {
    echo "error: a compatible tasks-axi with atomic multi-ID mv support is required to stage remote handoffs; run bin/xo-bootstrap.sh for the required version" >&2
    return 1
  }
  to_move=()
  already=()
...
with_remote_route_locks() { # <secondmate-id> <function> <args...>
  ...
  xo_lock_acquire_wait "$ACTIVE_REGISTRY_LOCK"
  ...
  xo_lock_acquire_wait "$ACTIVE_HANDOFF_LOCK"
...
if ! xo_tasks_axi_compatible; then
  echo "error: a compatible tasks-axi with atomic multi-ID mv support is required to move backlog items; run bin/xo-bootstrap.sh for the required version" >&2
  exit 1
fi
...
# Seed the destination with xo's standard three-section scaffold when it
...
# which is not xo's home-backlog convention.)
