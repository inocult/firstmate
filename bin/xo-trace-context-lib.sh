...
# Native W3C trace-context propagation for xo spawns (default-off).
#
# When enabled, xo resolves one W3C `traceparent` carrier for a task,
# injects it into the agent's pane shell as the TRACEPARENT environment variable
# before launch (bin/xo-spawn.sh, alongside GOTMPDIR, so it reaches every spawn
...
#
# TRACEPARENT here is an xo CONVENTION that carries a W3C-formatted
...
#
# Usage: . bin/xo-trace-context-lib.sh
#
# Public entry points:
#   xo_trace_context_session_start <config-dir> <effective-state-file>
#     Resolves config/trace-context plus XO_TRACE_CONTEXT once and atomically
#     writes the normalized on/off decision bound to the locked home session.
#   xo_trace_context_session_effective <effective-state-file>
...
#   xo_trace_context_resolve <config-dir> <meta-file>
...
#   XO_TRACE_CONTEXT        env override: 1/on/true/yes enables, any other
...
#   home (XO_INHERITABLE_CONFIG in bin/xo-config-inherit-lib.sh) and passes its
#   frozen on/off decision into the new process as a non-empty XO_TRACE_CONTEXT
#   value in the launch prefix (bin/xo-spawn.sh). The Secondmate freezes that
...
#   that task's meta: xo-spawn's spawn_remote_secondmate resolves the carrier,
#   hands it to the configured host through xo-spawn's --traceparent, and records
...
# guarantee any collector stores a span, and xo emits no spans itself.
#
...
# cleared, xo-spawn refuses to append the launch command. Every carrier this lib
# yields is either an xo-MINTED random root that reads no prompt, path,
...
xo_trace_context_hex() {  # <byte-count>
  local bytes=$1 hex
...
xo_trace_context_enabled() {  # <config-dir>
  ...
  if [ -n "${XO_TRACE_CONTEXT:-}" ]; then
    v=$(printf '%s' "$XO_TRACE_CONTEXT" | tr '[:upper:]' '[:lower:]')
    case "$v" in
...
xo_trace_context_session_lock() {  # <effective-state-file>
...
xo_trace_context_session_start() {  # <config-dir> <effective-state-file>
  ...
  lock_pid=$(xo_trace_context_session_lock "$effective_file") || {
    rm -f "$effective_file" 2>/dev/null || true
    return 0
  }
  xo_trace_context_enabled "$config_dir" && value=on
...
xo_trace_context_session_effective() {  # <effective-state-file>
  ...
  current_lock=$(xo_trace_context_session_lock "$effective_file") || {
    printf '%s' off
    return 0
  }
...
xo_trace_context_recorded() {  # <meta-file>
  local meta=$1 line
  [ -f "$meta" ] || return 0
...
xo_trace_context_mint() {
  local trace span tp
  trace=$(xo_trace_context_hex 16) || return 1
  span=$(xo_trace_context_hex 8) || return 1
  tp="00-$trace-$span-01"
  xo_trace_context_valid "$tp" || return 1
  printf '%s' "$tp"
}
...
xo_trace_context_resolve() {  # <config-dir> <meta-file>
  ...
  xo_trace_context_enabled "$config_dir" || return 0
  existing=$(xo_trace_context_recorded "$meta")
  if xo_trace_context_valid "$existing"; then
    printf '%s' "$existing"
    return 0
  fi
  xo_trace_context_mint || return 0
}
