#!/usr/bin/env node
// Trusted external XO extension binding host.
//
// Usage:
//   xo-extension.mjs bind <package-root> --adapter <name> [--adapter <name> ...]
//     --trust-same-user-code [--consent <fact> ...] [--timeout-ms <milliseconds>]
//   xo-extension.sh remote-bind <secondmate-id> <package-root> [bind options]
//   xo-extension.mjs retire-binding <extension-id>
//     --if-binding-digest <sha256:digest>
//   xo-extension.mjs retire-transfer <extension-id>
//     --if-transfer-digest <sha256:digest> --if-binding-digest <sha256:digest>
//   xo-extension.mjs list
//   xo-extension.mjs inspect <extension-id>
//   xo-extension.mjs verify [extension-id]
//   xo-extension.mjs resolve-process-event <adapter>
//   xo-extension.mjs process-event <adapter> <operation> [internal options]
//   xo-extension.mjs cleanup-invocations [--source-id <id> | --binding-digest <sha256:digest>]
//
...
// process-event
//           Internal invocation boundary used by bin/xo-procevent.sh. It
...
//
// Discovery is only $XO_HOME/config/extensions.d/*.json. Current directories,
...
const LAUNCH_BARRIER = path.join(CODE_ROOT, "bin", "xo-extension-launch-barrier.mjs");
const MANIFEST_NAME = "xo-extension.json";
const HOST_PROTOCOLS = [1];
const PROCESS_EVENT_CAPABILITY = "process-event-adapter";
const PROCESS_EVENT_VERSIONS = [1];
const MANIFEST_SCHEMA = "xo.extension-manifest.v1";
const BINDING_SCHEMA = "xo.extension-binding.v1";
const HANDSHAKE_REQUEST_SCHEMA = "xo.extension-handshake-request.v1";
const HANDSHAKE_RESPONSE_SCHEMA = "xo.extension-handshake-response.v1";
const REQUEST_SCHEMA = "xo.extension-request.v1";
const RESPONSE_SCHEMA = "xo.extension-response.v1";
const RESOLUTION_SCHEMA = "xo-extension-process-event-resolution.v1";
const ERROR_EVIDENCE_SCHEMA = "xo.process-event-extension-error.v1";
const INVOCATION_OWNER_SCHEMA = "xo.extension-invocation-owner.v1";
const INVOCATION_READY_SCHEMA = "xo.extension-invocation-ready.v1";
const INVOCATION_RELEASE_SCHEMA = "xo.extension-invocation-release.v1";
const CAPTURE_RESERVATION_SCHEMA = "xo-procevent-capture-reservation.v1";
const MAX_JSON_BYTES = 65536;
const MAX_RESULT_BYTES = 32768;
const MAX_STDERR_BYTES = 8192;
const MAX_TREE_ENTRIES = 4096;
const MAX_TREE_BYTES = 64 * 1024 * 1024;
const TRANSFER_SCHEMA = "xo.extension-package-transfer.v1";
const TRANSFER_MANIFEST_SCHEMA = "xo.extension-package-transfer-manifest.v1";
...
function makeRequestId(seed = randomBytes(32)) {
  ...
  return digestBytes(Buffer.concat([Buffer.from("xo-extension-request-v1\0"), bytes]));
}
...
async function activeHome() {
  const configured = process.env.XO_HOME || process.env.XO_ROOT_OVERRIDE || CODE_ROOT;
  const absolute = path.resolve(configured);
  const info = await maybeLstat(absolute);
  if (!info || !info.isDirectory()) fail("home-invalid", `XO home is not a directory: ${absolute}`);
...
async function scanTree(root, { installed = false } = {}) {
  ...
  hash.update("xo-package-tree-v1\0");
...
async function validateSourceRoot(home, input) {
  ...
  if (isInside(home, canonical)) fail("path-unsafe", "package source must be outside the active XO home");
...
function effectiveStateRoot(home) {
  return path.resolve(process.env.XO_STATE_OVERRIDE || path.join(home, "state"));
}

async function ensureExtensionState(home, binding) {
  let root;
  if (process.env.XO_STATE_OVERRIDE) {
...
function childEnvironment(binding, statePath = "") {
  const env = {
    ...
    LANG: "C",
    LC_ALL: "C",
    XO_EXTENSION_ID: binding.extension_id,
    XO_EXTENSION_VERSION: binding.extension_version,
  };
  if (statePath) env.XO_EXTENSION_STATE = statePath;
...
async function selfIdentity() {
  if (!cachedSelfIdentity) {
    ...
    process.title = `xo-extension-host ${cachedSelfIdentity}`;
  }
...
async function inheritedCaptureCapability(home) {
  ...
  if (capability.schema !== "xo-procevent-capture-capability.v1" || !/^[a-f0-9]{64}$/.test(capability.token)
...
async function cmdBindFrom(args, stagedRoot) {
  ...
  for (const adapter of selected) {
    ...
    if (await maybeLstat(path.join(CODE_ROOT, "bin", `xo-procevent-${adapter}.sh`))) {
...
async function claimInheritedLifecycleLock(home) {
  const mode = process.env.XO_EXTENSION_RETIREMENT_MODE;
  ...
  const lockPath = path.resolve(process.env.XO_EXTENSION_LIFECYCLE_LOCK || "");
  const ownerPath = path.resolve(process.env.XO_EXTENSION_LIFECYCLE_OWNER || "");
...
async function runLifecycleRetirement(mode, args) {
  const command = path.join(CODE_ROOT, "bin", "xo-procevent.sh");
  const home = await activeHome();
  const env = { PATH: sanitizedPath(), LANG: "C", LC_ALL: "C", HOME: process.env.HOME || home, XO_HOME: home, XO_ROOT_OVERRIDE: CODE_ROOT };
  if (process.env.XO_STATE_OVERRIDE) env.XO_STATE_OVERRIDE = process.env.XO_STATE_OVERRIDE;
  if (process.env.XDG_STATE_HOME) env.XDG_STATE_HOME = process.env.XDG_STATE_HOME;
  if (process.env.XO_PROCEVENT_CLAIM_ROOT) env.XO_PROCEVENT_CLAIM_ROOT = process.env.XO_PROCEVENT_CLAIM_ROOT;
...
async function runLifecycleProcessEvent(args) {
  ...
  if (process.env.XO_STATE_OVERRIDE) env.XO_STATE_OVERRIDE = process.env.XO_STATE_OVERRIDE;
  if (process.env.XDG_STATE_HOME) env.XDG_STATE_HOME = process.env.XDG_STATE_HOME;
  if (process.env.XO_PROCEVENT_CAPTURE_SOURCE_LOCK_HELD === "1") env.XO_PROCEVENT_CAPTURE_SOURCE_LOCK_HELD = "1";
...
async function cmdProcessEvent(args) {
  if (process.env.XO_EXTENSION_RETIREMENT_MODE === "process-event") {
...
function usage() {
  process.stderr.write(`Trusted external XO extension binding host.

Usage:
  bin/xo-extension.mjs bind <package-root> --adapter <name> [--adapter <name> ...] --trust-same-user-code [--consent <fact> ...] [--timeout-ms <milliseconds>]
  bin/xo-extension.sh remote-bind <secondmate-id> <package-root> --adapter <name> --trust-same-user-code [bind options]
  bin/xo-extension.mjs retire-binding <extension-id> --if-binding-digest <sha256:digest>
  bin/xo-extension.mjs retire-transfer <extension-id> --if-transfer-digest <sha256:digest> --if-binding-digest <sha256:digest>
  bin/xo-extension.mjs list
  bin/xo-extension.mjs inspect <extension-id>
  bin/xo-extension.mjs verify [extension-id]

The manifest file is xo-extension.json. Supported consent facts are network, credential-store, task-metadata, and artifact-references. The host supports only process-event-adapter/1; see docs/extension-bindings.md for its manifest, binding, handshake, and invocation contracts.
`);
  process.exitCode = 2;
}

async function main() {
  if (process.env.XO_EXTENSION_RETIREMENT_MODE) {
