...
# Usage: xo-harness.sh                  print own harness: claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|unknown
#        xo-harness.sh crew             print the effective CREWMATE harness
#                                        (config/crew-harness; "default" resolves to own)
#        xo-harness.sh secondmate       print the harness the PRIMARY uses to launch
...
#        xo-harness.sh secondmate-model    print the optional MODEL token from
#                                        config/secondmate-harness, or empty when absent.
#        xo-harness.sh secondmate-effort   print the optional EFFORT token from
#                                        config/secondmate-harness, or empty when absent.
#        xo-harness.sh validate-native-effort <harness> <model> <effort>
...
detect_own() {
  ...
  # the verdict correct; bin/xo-spawn.sh additionally clears the foreign markers
  # at the launch boundary. Both are kept: the launch sanitization only covers
  # sessions xo-spawn started, while this ordering also covers a cursor session
  ...
  # correct; bin/xo-spawn.sh additionally clears the foreign markers at the
  ...
  # the same ordering hazard cursor documents above (see issue #3517). bin/xo-spawn.sh
  ...
  # process environment. XO_OMP_HARNESS=omp is therefore an XO-OWNED
  # launch marker, established by bin/xo-spawn.sh at the omp launch boundary
  ...
  # ancestry, so `XO_OMP_HARNESS=omp omp` started from a Claude pane identifies
  ...
  if [ "${XO_OMP_HARNESS:-}" = omp ] && ancestry_names_omp; then
    echo omp
    return
  fi
  [ "${CLAUDECODE:-}" = "1" ] && { echo claude; return; }
  if [ "${PI_CODING_AGENT:-}" = "true" ]; then
    if [ "${XO_PI_HARNESS:-}" = pi-signed ]; then echo pi-signed; else echo pi; fi
    return
  fi
  ...
  for _ in 1 2 3 4 5 6 7 8; do
    ...
    argv0=$(xo_cursor_argv0_for_pid "$pid" "$comm" 2>/dev/null || true)
    if xo_cursor_process_matches "$comm" '' "$argv0"; then
      echo cursor
      return
    fi
    if xo_gemini_path_is_gemini "$comm"; then
      echo gemini
      return
    fi
    case "$(basename -- "$comm")" in
      ...
      *grok*) echo grok; return ;;
      ...
      node*|python*)
        ...
        if xo_gemini_args_are_gemini "$args"; then
          echo gemini
          return
        fi
        case "$args" in
...
# name) wins; absent or "default" mirrors xo's own harness.
resolve_crew() {
  local crew=
