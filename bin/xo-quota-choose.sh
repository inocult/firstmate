...
#
# Usage:
#   xo-quota-choose.sh [--snapshot <path>] [--candidate <harness:model>]...
#
...
printf '%s\n' "$QUOTA_JSON" | xo_quota_json_valid || die "invalid quota-axi provider data"

# provider_for_harness <harness> [<model>]
# Map an xo harness name to its primary quota-axi provider family.
...
for c in "${CANDIDATES[@]}"; do
  ...
  xo_control_harness_supported "$harness" || die "unknown harness: $harness"
