#!/usr/bin/env bash
# xo-decision-hold.sh - transitional compatibility shim over bin/xo-captain-hold.sh.
#
# The separate "decision" concept collapsed into the one primitive the captain
# cares about: a task held for the captain. bin/xo-captain-hold.sh owns every
...
CAPTAIN_HOLD="$SCRIPT_DIR/xo-captain-hold.sh"
...
task_show() {
  (cd "$XO_HOME" && tasks-axi show "$1" --full) 2>/dev/null
}
...
command_resolve() {
  ...
  case "$hold_body" in
    *"Resolution recorded by xo-decision-hold."*"Routed identities: "*)
  ...
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-decision-hold-resolve.XXXXXX") \
  ...
  for dep in $routed; do
    ...
    if list_has_key "$(normalized_blocked_by "$show")" "$id"; then
      (cd "$XO_HOME" && tasks-axi unblock "$dep" --by "$id" >/dev/null) \
