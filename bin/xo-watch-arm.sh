#!/usr/bin/env bash
# Safe, home-scoped (re-)arm of the xo watcher, with honest verification.
#
# The watcher (bin/xo-watch.sh) blocks until it has an actionable wake to
...
# call and NOTIFIES on exit, so xo must run this script as the harness's
...
# (bin/xo-claude-stop-autoarm.sh), where the harness owns the process group and
...
# On a harness with a PreToolUse-equivalent hook, bin/xo-arm-pretool-check.sh
...
# liveness beacon (state/.last-watcher-beat) is fresh within XO_GUARD_GRACE (the
# single source of truth, shared with xo-watch.sh and xo-guard.sh), and prints
...
#
# --restart: stop ONLY this XO_HOME's watcher (the pid recorded in THIS home's
...
# bin/xo-watch.sh`: that pattern matches every xo home's watcher
...
CONFIRM_TIMEOUT=${XO_ARM_CONFIRM_TIMEOUT:-$ARM_CONFIRM_DEFAULT}
# Poll interval while attached to an existing healthy watcher.
ATTACH_POLL=${XO_ARM_ATTACH_POLL:-0.5}
CYCLE_LOG="$STATE/.watch-cycle-exits.log"
CYCLE_LOG_LOCK="$STATE/.watch-cycle-exits.lock"
CYCLE_LOG_MAX_BYTES=${XO_WATCH_CYCLE_LOG_MAX_BYTES:-262144}
CYCLE_LOG_KEEP_LINES=${XO_WATCH_CYCLE_LOG_KEEP_LINES:-1000}
ARM_PID=${BASHPID:-$$}
...
cycle_log_append() {
  ...
  [ "$cycle_active" -eq 1 ] || return 0
  ended_at=$(date +%s)
  beacon_age=$(xo_path_age "$BEAT")
  lock_after=$(lock_snapshot)

  i=0
  while ! xo_lock_try_acquire "$CYCLE_LOG_LOCK"; do
    [ "$i" -lt 20 ] || return 0
    sleep 0.02
    i=$((i + 1))
  done
  ...
  xo_lock_release "$CYCLE_LOG_LOCK"
  cycle_active=0
}
...
cycle_mark_predecessor_successor() {
  local successor=$1 predecessor=${XO_WATCH_PREDECESSOR_ARM_PID:-} i tmp
  case "$predecessor" in
    ''|*[!0-9]*) return 0 ;;
  esac
...
clear_stale_recorded_watcher_lock() {
  ...
  lock_home=$(cat "$WATCH_LOCK/xo-home" 2>/dev/null || true)
  lock_path=$(cat "$WATCH_LOCK/watcher-path" 2>/dev/null || true)
  lock_identity=$(cat "$WATCH_LOCK/pid-identity" 2>/dev/null || true)
  [ "$lock_home" = "$XO_HOME" ] || return 0
  [ "$lock_path" = "$WATCH" ] || return 0
  [ -n "$lock_identity" ] || return 0
  xo_recovery_transition "$STATE/.watcher-down" clear-stale-lock "$WATCH_LOCK" downtime
}
...
healthy_watcher() {
  HEALTHY_PID=
  HEALTHY_IDENTITY=
  xo_watcher_healthy "$STATE" "$WATCH" "$GRACE" "$XO_HOME" || return 1
  HEALTHY_PID=$XO_WATCHER_HEALTHY_PID
  HEALTHY_IDENTITY=$XO_WATCHER_HEALTHY_IDENTITY
}
...
handling_successor_generation() {
  [ -n "${XO_WATCH_PREDECESSOR_ARM_PID:-}" ] || return 0
  xo_recovery_marker_snapshot "$STATE/.watcher-down" || return 1
  case "$XO_RECOVERY_MARKER_TOKEN" in
    pending:downtime:*|pending:handling:*|announced:downtime:*|announced:handling:*) printf '%s' "${XO_RECOVERY_MARKER_TOKEN##*:}" ;;
    acked:*|'') ;;
    *) return 1 ;;
  esac
}
...
if [ "$mode" = handling-delivered ]; then
  xo_pid_alive "$handling_watcher_pid" \
    && xo_watcher_lock_matches_pid "$STATE" "$WATCH" "$handling_watcher_pid" "$XO_HOME" \
    && xo_recovery_marker_begin_handling "$STATE/.watcher-down" "$handling_generation"
  exit $?
fi
...
if [ "$mode" = restart ]; then
  ...
  if xo_pid_alive "$lock_pid"; then
    if xo_watcher_lock_matches_pid "$STATE" "$WATCH" "$lock_pid" "$XO_HOME"; then
      kill -TERM "$lock_pid" 2>/dev/null || true
      # Wait for it to actually exit before relaunching, so the fresh watcher
      # either takes a released lock or reclaims a now-dead-pid stale lock instead
      # of seeing the dying one as a live holder and no-opping.
      i=0
      while [ "$i" -lt 50 ] && xo_pid_alive "$lock_pid"; do
        sleep 0.1
        i=$((i + 1))
      done
    else
...
# wake exit propagates out so the harness re-notifies xo.
child=
child_out=
cleanup_child() {
  if [ -n "$child" ] && xo_pid_alive "$child"; then
...
if [ -n "${XO_WATCH_PREDECESSOR_ARM_PID:-}" ]; then
  XO_WATCH_HANDLING_SUCCESSOR=1 "$WATCH" >"$child_out" &
else
  "$WATCH" >"$child_out" &
fi
child=$!
cycle_begin "$child" started "$(xo_pid_identity "$child" 2>/dev/null || true)"
child_done=0
...
while :; do
  ...
  if [ "$child_done" -eq 0 ] && ! xo_pid_alive "$child"; then
    wait "$child"
    rc=$?
    child_done=1
    owned_child_finished "$rc"
