...
# is the same hazard bin/xo-cursor-lib.sh exists to close for cursor-agent, and
...
# must not happen.
#
# Detection of xo's OWN harness uses these structural rules for the
# ancestry fallback. The GEMINI_CLI=1 environment marker in bin/xo-harness.sh
...
xo_gemini_pid_is_gemini() {  # <pid>
  local pid=$1 token argv0='' index=0
  [ -r "/proc/$pid/cmdline" ] || return 1
  while IFS= read -r -d '' token; do
    if [ "$index" -eq 0 ]; then
      argv0=$token
      xo_gemini_path_is_gemini "$argv0" && return 0
      case "${argv0##*/}" in
      ...
      case "$token" in
        -*) ;;
        *) xo_gemini_path_is_gemini "$token" && return 0; return 1 ;;
      esac
    fi
    index=$((index + 1))
...
xo_gemini_args_are_gemini() {  # <args>
  ...
  while [ -n "$rest" ]; do
    ...
    xo_gemini_path_is_gemini "$token" && return 0
    return 1
  done
  return 1
}
