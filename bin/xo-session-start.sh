#!/usr/bin/env bash
# xo-session-start.sh - one command for the whole session start.
#
...
# xo-bootstrap.sh, then separately read data/projects.md, data/secondmates.md,
# data/captain.md, data/captain-shared.md, data/learnings.md, then run
# xo-lock.sh, xo-wake-drain.sh, then read data/backlog.md, every state/*.meta,
...
#
# COMPOSITION, NOT DUPLICATION: this script calls xo-lock.sh, xo-bootstrap.sh,
# xo-wake-drain.sh, and xo-startup-network.sh as real subprocesses and prints
...
# standalone with unchanged default behavior - other flows (xo-bootstrap.sh
# install <tools> after consent, /updatexo, the afk daemon, existing
# tests) still call them directly. The one seam this script needed -
# bootstrap running its detect-only diagnostics without its six mutating
# sweeps - is an opt-in XO_BOOTSTRAP_DETECT_ONLY=1 flag on xo-bootstrap.sh
...
# entire XO_SESSION_START_TIMEOUT and truncate the digest, so a slow network
...
# be slow. bin/xo-startup-network.sh owns that stage and its safety argument;
# bin/xo-bootstrap.sh and bin/xo-inactive-reconcile.sh remain the owners of the
...
# go dark. So on refusal, bootstrap still runs (in XO_BOOTSTRAP_DETECT_ONLY=1
...
#     reporting surfaces (bin/xo-bearings-snapshot.sh, /ahoy), and at startup it
...
#     XO_SESSION_START_QUEUED_LIMIT, default 20. Anything it omits is disclosed
#     with an exact remainder count and the command that shows the rest, so a
#     deep queue costs a counter rather than kilobytes.
#     (This replaces XO_SESSION_START_BACKLOG_LIMIT, which bounded the whole
...
# truly needed.
#
# STATUS TAILS: XO_SESSION_START_STATUS_TAIL bounds how many lines each task's
# tail prints, and bin/xo-line-cap-lib.sh bounds how long each of those lines
...
# rather than current state - bin/xo-crew-state.sh owns current state.
#
# RUNTIME BOUND: the digest is now executed through a native session-open
# adapter (see bin/xo-sessionstart-run.sh), which blocks either hook-driven
...
# (XO_SESSION_START_TIMEOUT, default 120s). The deferred network stage
...
# records its progress in XO_SESSION_START_STAGE_FILE, which is also the flag
...
#
# Usage: xo-session-start.sh [--reemit] [--source <source>]
...
#             ownership must be re-verified rather than assumed: xo-lock.sh already treats a lock
...
#             xo-sessionstart-run.sh. A genuine `startup` that owns the active
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --source=*)
      ...
      ;;
    -h|--help)
      sed -n '2,/^set -u$/p' "$SCRIPT_DIR/xo-session-start.sh" | sed 's/^# \{0,1\}//; $d'
      exit 0
      ;;
    *)
      printf 'xo-session-start: unknown argument: %s\n' "$1" >&2
      printf 'usage: xo-session-start.sh [--reemit] [--source <source>]\n' >&2
      exit 2
      ;;
  esac
done
...
stage() {  # <stage-name>: breadcrumb for the parent's truncation banner
  [ -n "${XO_SESSION_START_STAGE_FILE:-}" ] || return 0
  printf '%s\n' "$1" > "$XO_SESSION_START_STAGE_FILE" 2>/dev/null || true
}
...
if [ -z "${XO_SESSION_START_STAGE_FILE:-}" ]; then
  SESSION_START_BUDGET=${XO_SESSION_START_TIMEOUT:-120}
  ...
  SESSION_START_STAGE_FILE=$(mktemp "${TMPDIR:-/tmp}/xo-session-start-stage.XXXXXX" 2>/dev/null) || SESSION_START_STAGE_FILE=
  ...
  if [ "$REEMIT" -eq 1 ]; then
    if [ -n "$SESSION_SOURCE" ]; then
      xo_run_timed "$SESSION_START_BUDGET" \
        env XO_SESSION_START_STAGE_FILE="$SESSION_START_STAGE_FILE" \
        "$SCRIPT_DIR/xo-session-start.sh" --reemit --source "$SESSION_SOURCE"
    else
      xo_run_timed "$SESSION_START_BUDGET" \
        env XO_SESSION_START_STAGE_FILE="$SESSION_START_STAGE_FILE" \
        "$SCRIPT_DIR/xo-session-start.sh" --reemit
    fi
  elif [ -n "$SESSION_SOURCE" ]; then
    xo_run_timed "$SESSION_START_BUDGET" \
      env XO_SESSION_START_STAGE_FILE="$SESSION_START_STAGE_FILE" \
      "$SCRIPT_DIR/xo-session-start.sh" --source "$SESSION_SOURCE"
  else
    xo_run_timed "$SESSION_START_BUDGET" \
      env XO_SESSION_START_STAGE_FILE="$SESSION_START_STAGE_FILE" \
      "$SCRIPT_DIR/xo-session-start.sh"
  fi
  SESSION_START_RC=$?
  if [ "$SESSION_START_RC" -eq 124 ]; then
    ...
    printf '●    %s\n' "${SESSION_START_PENDING% }"
    printf '●  Rerun bin/xo-session-start.sh now to finish taking the helm. If it truncates\n'
    printf '●  again, raise XO_SESSION_START_TIMEOUT and report the slow stage - a stage that\n'
...
PRIMARY_HARNESS=$("$SCRIPT_DIR/xo-harness.sh" 2>/dev/null || printf unknown)
...
# the backlog listing and again inside the xo-bootstrap.sh child, which reports
# an incompatible build as MISSING. Computing it once and handing it to that
# child collapses six subprocesses to three. xo-tasks-axi-lib.sh owns both reuse
# layers and the one-hop consumption rule that keeps the verdict out of any
# agent's environment.
if xo_tasks_axi_compatible; then TASKS_AXI_COMPATIBLE=1; else TASKS_AXI_COMPATIBLE=0; fi

STATUS_TAIL=${XO_SESSION_START_STATUS_TAIL:-5}
case "$STATUS_TAIL" in ''|*[!0-9]*) STATUS_TAIL=5 ;; esac
QUEUED_LIMIT=${XO_SESSION_START_QUEUED_LIMIT:-20}
...
# meaningful for every one of these files (captain.md absent = xo
    ...
    END {
      ...
      if (in_flight + queued_total + done_total == 0) {
        ...
        if (plain_total > plain_shown) {
          printf "(%d more queued - raise XO_SESSION_START_QUEUED_LIMIT or read data/backlog.md for the rest)\n", plain_total - plain_shown
        }
      }
    }
  ' "$path"
...
print_backlog_compact() {
  ...
  if [ -f "$path" ]; then
    if [ -s "$path" ]; then
      if xo_tasks_axi_backend_available "$CONFIG"; then
        print_backlog_tasks_axi_compact "$path"
      elif xo_backlog_backend_manual "$CONFIG"; then
...
print_status_tail() {
  ...
  printf 'status tail (last %s line(s), each capped at %s characters, wake-EVENT history, not current state; full log: %s):\n' \
    "$STATUS_TAIL" "$XO_LINE_CAP_DEFAULT" "$status"
...
agents_baseline_drifted() {  # <rebuilding-session-pid>
  ...
  current_hash=$(hash_file_sha256 "$XO_ROOT/AGENTS.md" 2>/dev/null || true)
...
# working XO delivery path arrive here. Claude fresh-reads on reset, and
...
print_agents_refresh_if_required() {  # <rebuilding-session-pid>
  ...
  if [ -f "$XO_ROOT/AGENTS.md" ]; then
    cat <<'EOF'
The complete on-disk AGENTS.md below supersedes the instruction copy this session
started with. Apply it as the current XO instruction contract.

EOF
    cat "$XO_ROOT/AGENTS.md"
  else
...
if [ "$REEMIT" -eq 0 ] && [ "$SESSION_SOURCE" = startup ]; then
  AGENTS_START_HASH=$(hash_file_sha256 "$XO_ROOT/AGENTS.md" 2>/dev/null || true)
fi

if [ "$REEMIT" -eq 1 ]; then
  section "SESSION START (CONTEXT RE-EMIT) - $XO_HOME"
  ...
  section "SESSION START - $XO_HOME"
fi
# --- 1. lock -----------------------------------------------------------
stage lock
subsection "LOCK"
LOCK_OUT=$("$SCRIPT_DIR/xo-lock.sh" 2>&1)
LOCK_RC=$?
printf '%s\n' "$LOCK_OUT"
READ_ONLY=0
...
REBUILDING_SESSION_PID=$(xo_harness_ancestry_pid 2>/dev/null || true)
...
if [ "$READ_ONLY" -eq 0 ]; then
  ...
  xo_trace_context_session_start "$CONFIG" "$STATE/.trace-context-effective"
  ...
  "$SCRIPT_DIR/xo-startup-network.sh" start \
    --locked "$NETWORK_STAGE_LOCKED" --harvest-pid $$ >/dev/null 2>&1 || true
fi

# --- 2. bootstrap --------------------------------------------------------
# XO_BOOTSTRAP_NETWORK=skip on every path: bootstrap's own network half is what
...
if [ "$READ_ONLY" -eq 1 ]; then
  BOOT_OUT=$(XO_BOOTSTRAP_DETECT_ONLY=1 XO_BOOTSTRAP_NETWORK=skip \
    XO_TASKS_AXI_COMPATIBLE="$TASKS_AXI_COMPATIBLE" "$SCRIPT_DIR/xo-bootstrap.sh" 2>&1)
elif [ "$REEMIT" -eq 1 ]; then
  BOOT_OUT=$(XO_BOOTSTRAP_DETECT_ONLY=1 XO_BOOTSTRAP_LOCKED=1 XO_BOOTSTRAP_NETWORK=skip \
    XO_TASKS_AXI_COMPATIBLE="$TASKS_AXI_COMPATIBLE" "$SCRIPT_DIR/xo-bootstrap.sh" 2>&1)
else
  BOOT_OUT=$(
    "$SCRIPT_DIR/xo-herdr-session-cleanup.sh" 2>&1 || true
    XO_BOOTSTRAP_NETWORK=skip XO_TASKS_AXI_COMPATIBLE="$TASKS_AXI_COMPATIBLE" \
      "$SCRIPT_DIR/xo-bootstrap.sh" 2>&1
  )
fi
if [ -n "$BOOT_OUT" ]; then
  printf '%s\n' "$BOOT_OUT"
...
# The drain also runs xo-guard.sh internally on the locked path, so the
...
# xo-guard.sh directly with non-mutating advisory text, so the same alarms
...
if [ "$READ_ONLY" -eq 1 ]; then
  ...
  GUARD_OUT=$(XO_GUARD_READ_ONLY=1 "$SCRIPT_DIR/xo-guard.sh" 2>&1)
  ...
  if [ "$PRIMARY_HARNESS" = pi ] || [ "$PRIMARY_HARNESS" = pi-signed ]; then
    XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" "$SCRIPT_DIR/xo-lease.sh" sweep 2>/dev/null || true
    BRANCH_REPLAY_OUT=$(XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
      "$SCRIPT_DIR/xo-branch-outcome.sh" startup-replay 2>&1) || BRANCH_REPLAY_OUT=
    if [ -n "$BRANCH_REPLAY_OUT" ]; then
      printf '%s\n' "$BRANCH_REPLAY_OUT"
    fi
  fi
  DRAIN_OUT=$("$SCRIPT_DIR/xo-wake-drain.sh" 2>&1)
  if [ -n "$DRAIN_OUT" ]; then
    printf '%s\n' "$DRAIN_OUT"
  else
...
if [ "$PRIMARY_HARNESS" = pi ] || [ "$PRIMARY_HARNESS" = pi-signed ]; then
  PI_EXT="$XO_ROOT/.pi/extensions/xo-primary-pi-watch.ts"
  PI_TURNEND_EXT="$XO_ROOT/.pi/extensions/xo-primary-turnend-guard.ts"
  ...
  PI_WATCH_VERSION=$(xo_pi_extension_version "$PI_EXT" || printf '')
  PI_TURNEND_VERSION=$(xo_pi_extension_version "$PI_TURNEND_EXT" || printf '')
  if ! xo_pi_extension_loaded "$PI_WATCH_MARKER" "$PI_WATCH_VERSION" "$PI_LOCK" \
    || ! xo_pi_extension_loaded "$PI_TURNEND_MARKER" "$PI_TURNEND_VERSION" "$PI_LOCK"; then
...
# build plus the loading pid, exactly as the Pi ones do (bin/xo-wake-lib.sh).
if [ "$PRIMARY_HARNESS" = omp ]; then
  OMP_EXT="$XO_ROOT/.omp/extensions/xo-primary-omp-watch.ts"
  OMP_TURNEND_EXT="$XO_ROOT/.omp/extensions/xo-primary-turnend-guard.ts"
  OMP_WATCH_MARKER="$STATE/.omp-watch-extension-loaded"
  OMP_TURNEND_MARKER="$STATE/.omp-turnend-extension-loaded"
  OMP_LOCK="$STATE/.lock"
  OMP_WATCH_VERSION=$(xo_pi_extension_version "$OMP_EXT" || printf '')
  OMP_TURNEND_VERSION=$(xo_pi_extension_version "$OMP_TURNEND_EXT" || printf '')
  if ! xo_pi_extension_loaded "$OMP_WATCH_MARKER" "$OMP_WATCH_VERSION" "$OMP_LOCK" \
    || ! xo_pi_extension_loaded "$OMP_TURNEND_MARKER" "$OMP_TURNEND_VERSION" "$OMP_LOCK"; then
...
"$SCRIPT_DIR/xo-supervision-instructions.sh" \
...
Go to a source directly only when:
  ...
  - the NETWORK CHECKS section reported its checks still IN PROGRESS and this
    turn needs their verdict (bin/xo-startup-network.sh report),
...
for meta in "$STATE"/*.meta; do
  ...
  if [ -n "$window" ]; then
    ...
    if xo_backend_target_exists "$backend" "${target:-$window}" "xo-$id"; then
...
subsection "AFK"
# The away posture is the record (bin/xo-afk-contract.sh); the legacy flag
# still marks a running daemon on the harnesses that launch one.
if [ -f "$STATE/.afk-contract" ]; then
  printf 'present - away posture recorded at %s (hold-for-return only; bin/xo-afk-contract.sh readback for the mandate)' \
    "$("$SCRIPT_DIR/xo-afk-contract.sh" field entered 2>/dev/null || printf unknown)"
  if [ -e "$STATE/.afk" ]; then
...
# subsection, and never reaches xo-public-followup.sh.
if xo_pf_relay_active "$XO_HOME" \
  && { xo_pf_has_registrations "$STATE" || xo_pf_has_events "$STATE"; }; then
  PUBLIC_FOLLOWUP=$("$SCRIPT_DIR/xo-public-followup.sh" pending 2>/dev/null) || PUBLIC_FOLLOWUP=
...
if [ "$READ_ONLY" -eq 1 ]; then
  ...
  "$SCRIPT_DIR/xo-startup-network.sh" harvest --pid $$ 2>&1 || true
fi
