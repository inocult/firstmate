#!/usr/bin/env bash
# xo-inbox.sh - the captain's out-of-band capture surface.
#
# Solves three DIFFERENT problems with three different mechanisms, because they
# are not the same problem:
#
#   note    Queue an idea for xo while xo is mid-turn and cannot
...
#           xo, the backlog, or the wake queue. A side question is not
...
#
#   config/inbox-region     XO_INBOX_REGION     AWS region.            required
#   config/inbox-stt-model  XO_INBOX_STT_MODEL  speech-to-text model.  required by say
#   config/inbox-ask-model  XO_INBOX_ASK_MODEL  side-question model.   required by ask
#   config/inbox-profile    XO_INBOX_PROFILE    AWS profile.           optional
#
# An absent profile means the call uses whatever credentials are already in the
# environment, which is also what XO_INBOX_PROFILE= (empty) forces.
#
...
#
# Environment:
#   XO_HOME              operational home whose state/ and data/ are used.
#
...
# in bin/xo-voice-relay.py hands real work over to xo, it runs this
...
# bin/xo_voice_records.py owns the scope-controlled machine view the voice agent
# reads, because the voice agent must be able to answer without record free text
# ever reaching a model.
set -euo pipefail

# A non-interactive `ssh host xo-inbox.sh ...` does NOT get a login shell, so it
...
XO_ROOT="$(cd "$SELF_DIR/.." && pwd)"
XO_HOME="${XO_HOME:-$XO_ROOT}"
...
REGION="${XO_INBOX_REGION:-}"
STT_MODEL="${XO_INBOX_STT_MODEL:-}"
ASK_MODEL="${XO_INBOX_ASK_MODEL:-}"
# Unset falls through to config; explicitly empty means "use ambient credentials".
PROFILE="${XO_INBOX_PROFILE-$(read_setting inbox-profile)}"
...
need_region() {
  [ -n "$REGION" ] || REGION=$(require_setting inbox-region XO_INBOX_REGION "AWS region")
}

need_stt_model() {
  need_region
  [ -n "$STT_MODEL" ] || STT_MODEL=$(require_setting inbox-stt-model \
    XO_INBOX_STT_MODEL "speech-to-text model")
}

need_ask_model() {
  need_region
  [ -n "$ASK_MODEL" ] || ASK_MODEL=$(require_setting inbox-ask-model \
    XO_INBOX_ASK_MODEL "side-question model")
}
...
# ambient ones win. Set XO_INBOX_PROFILE= (empty) to force that even without env
# credentials present.
aws_call() {
...
# Append exactly one wake so xo picks the note up at its next drain.
...
wake_for() {
  local id=$1 summary=$2 lib="$XO_ROOT/bin/xo-wake-lib.sh"
  if [ ! -r "$lib" ]; then
    printf 'xo-inbox: note saved but NOT announced (missing %s)\n' "$lib" >&2
    return 1
  fi
  # shellcheck source=/dev/null
  XO_ROOT_OVERRIDE="$XO_ROOT" XO_HOME="$XO_HOME" STATE="$STATE" . "$lib"
  xo_wake_append check "inbox:$id" "check: captain inbox note $id - $summary"
}
...
queue_note() {
  ...
  if wake_for "$id" "$summary"; then
    printf '  xo will pick this up at its next check.\n'
  else
    die "note $id is saved at $INBOX/$id.note but xo was NOT woken"
  fi
}
...
cmd_say() {
  ...
  raw=$(mktemp /tmp/xo-inbox-audio-XXXXXX)
  wav=$(mktemp /tmp/xo-inbox-wav-XXXXXX.wav)
...
if data[:4] == b'RIFF':
    ...
    sys.stderr.write("xo-inbox: input is WAV, passing through\n")
elif data[:4] in (b'OggS', b'fLaC') or data[:3] == b'ID3':
    sys.exit("xo-inbox: got Ogg/FLAC/MP3; re-encode to WAV first")
else:
    if len(data) % 2:
        data = data[:-1]
    w = wave.open(dst, 'wb')
    ...
    sys.stderr.write("xo-inbox: input looked like raw PCM, wrapped as 16kHz mono WAV\n")
PY
...
w=wave.open('$wav'); print(round(w.getnframes()/w.getframerate(),2))")
  printf 'xo-inbox: %ss of audio, transcribing with %s in %s\n' "$secs" "$STT_MODEL" "$REGION" >&2
...
cmd_status() {
  ...
  printf '=== xo status (read-only, no wake sent) ===\n'
  printf 'home     %s\n' "$XO_HOME"
  printf 'time     %s\n' "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
  printf 'inbox    %s note(s) waiting for xo\n' "$pending"
...
cmd_ask() {
  [ "$#" -gt 0 ] || die "usage: xo-inbox.sh ask <question>..."
  need_ask_model
  need aws
  need python3
  local q="$*" msg
  msg=$(mktemp /tmp/xo-inbox-ask-XXXXXX.json)
  # shellcheck disable=SC2064
  trap "rm -f '$msg'" EXIT
...
cmd_drain() {
  if [ "${1:-}" = "--ack" ]; then
    shift
    [ "$#" -gt 0 ] || die "usage: xo-inbox.sh drain --ack <id>..."
  ...
  cmd_list
  printf '\nAck with: xo-inbox.sh drain --ack <id>...\n'
}
