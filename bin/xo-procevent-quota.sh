...
#
# Usage:
#   xo-procevent-quota.sh arm [--interval <secs>] [--threshold <percent>] [--provider <provider>]
#   xo-procevent-quota.sh poll [--interval <secs>] [--threshold <percent>] [--provider <provider>] [--timeout <secs>]
#   xo-procevent-quota.sh classify <result-file>
#   xo-procevent-quota.sh terminal <result-file>
#   xo-procevent-quota.sh source-id
#   xo-procevent-quota.sh retire [--provider <provider>]
#
# arm        Register a recurring quota-axi --json poll that wakes xo
...
#            registered through `bin/xo-procevent.sh register`.
...
# shellcheck source=bin/xo-quota-axi-lib.sh
...
resolve_provider() {
  ...
  xo_procevent_source_id_valid "$CANONICAL_SOURCE_ID" || die "source id is not path-safe: $CANONICAL_SOURCE_ID"
}
...
quota_json() {
  ...
  if [ -n "$timeout" ]; then
    xo_quota_axi_compatible "$timeout" >/dev/null 2>&1 || return 2
    output=$(xo_run_timed "$timeout" quota-axi --json 2>/dev/null </dev/null) || return 2
  else
    xo_quota_axi_compatible >/dev/null 2>&1 || return 2
...
condition_status() {
  ...
  printf '%s\n' "$json" | xo_quota_json_valid || { printf 'error\n'; return; }
...
cmd_arm() {
  ...
  xo_quota_axi_compatible 5 >/dev/null 2>&1 || die "quota-axi is missing or below the compatibility floor"
  local timeout
  timeout=$(perl -e 'print int($ARGV[0] * 0.8 + 0.5)' "$interval") || timeout=30
  [ "$timeout" -ge 5 ] || timeout=5
  "$SCRIPT_DIR/xo-procevent.sh" register quota "$CANONICAL_SOURCE_ID" \
    -- "$SCRIPT_DIR/xo-procevent-quota.sh" poll --interval "$interval" --threshold "$threshold" --provider "$PROVIDER" --timeout "$timeout" || exit 1
  printf 'armed: %s\n' "$CANONICAL_SOURCE_ID"
  printf 'provider: %s\n' "${PROVIDER:-(aggregate)}"
  printf 'threshold: %s%%\n' "$threshold"
  printf 'interval: %ss\n' "$interval"
