...
# bin/xo-pr-check.sh, so teardown can verify landed work after squash merges.
# The full canonical URL is parsed by bin/xo-pr-lib.sh. A GitHub pull request is
...
#
# Usage: xo-pr-merge.sh <task-id> <pr-url> [--attended-override] [--allow-red <check-name>] [-- <extra forge merge args>]
#
# On GitLab, this script confirms the MR is actually merged before reporting it;
# an auto-merge-queued or unconfirmed request leaves the poll armed and records
# no landed outcome. bin/xo-merge-outcome-lib.sh owns a confirmed merge's
...
# shellcheck source=bin/xo-merge-outcome-lib.sh
...
PR_OWNER=$XO_PR_OWNER
PR_REPO=$XO_PR_REPO
PR_NUMBER=$XO_PR_NUMBER
...
PROJECT_URL="https://$XO_PR_HOST/$XO_PR_PATH"
shift 2
...
xo_backlog_directory_present "$STATE" "state directory" || {
  echo "error: PR merge refused: $XO_BACKLOG_TRANSITION_ERROR" >&2
  exit 1
}
META="$STATE/$ID.meta"
...
# green PR and never merges (contract: bin/xo-lease-lib.sh; no-op in homes
...
xo_lease_forbid_branch "PR merge (xo-pr-merge)"

if [ ! -f "$META" ] || [ -L "$META" ]; then
  echo "error: task metadata is unavailable" >&2
  exit 1
fi
if ! xo_backlog_meta_spawn_gen_optional "$META" "$STATE"; then
  echo "error: PR merge refused: $XO_BACKLOG_TRANSITION_ERROR" >&2
  exit 1
fi
MERGE_EXPECTED_SPAWN_GEN=$XO_BACKLOG_META_SPAWN_GEN

MERGE_CONTROL_LOCK=
merge_control_cleanup() {
  [ -z "$MERGE_CONTROL_LOCK" ] || xo_lock_release "$MERGE_CONTROL_LOCK" || true
}
trap merge_control_cleanup EXIT
MERGE_CONTROL_LOCK="$STATE/.control-$ID.lock"
xo_lock_acquire_wait "$MERGE_CONTROL_LOCK"
if ! xo_backlog_meta_spawn_gen_optional "$META" "$STATE"; then
  echo "error: task $ID changed while waiting to merge; refusing: $XO_BACKLOG_TRANSITION_ERROR" >&2
  exit 1
fi
if [ "$XO_BACKLOG_META_SPAWN_GEN" != "$MERGE_EXPECTED_SPAWN_GEN" ]; then
...
# The recorded head is read before bin/xo-pr-check.sh rewrites the metadata,
...
# the merge request. Sets XO_PR_MERGE_HEAD to the verified head on success and
...
gitlab_verify_mergeable() {
  ...
  if ! json=$(GITLAB_HOST="$XO_PR_HOST" glab mr view "$PR_NUMBER" -R "$PROJECT_URL" -F json 2>/dev/null) \
    || [ -z "$json" ]; then
...
FIELDS
  ...
  if ! xo_pr_head_valid "$live_head"; then
...
# Sets XO_PR_MERGE_HEAD to the verified head on success.
...
XO_PR_GITHUB_QUEUED=
XO_PR_GITHUB_BASE=
XO_PR_GITHUB_QUEUE_OBSERVED=false
...
FIELDS
  ...
  XO_PR_GITHUB_STATE=$state
  XO_PR_GITHUB_MERGED=$merged
  XO_PR_GITHUB_QUEUED=$queued
  XO_PR_GITHUB_BASE=$base
  XO_PR_GITHUB_QUEUE_OBSERVED=true
}
...
github_read_outcome_with_gh_axi() {
  ...
  case "$state" in
    merged)
      XO_PR_GITHUB_STATE=MERGED
      XO_PR_GITHUB_MERGED=true
      XO_PR_GITHUB_QUEUED=false
      ;;
    *)
      XO_PR_GITHUB_STATE=$state
      XO_PR_GITHUB_MERGED=false
      XO_PR_GITHUB_QUEUED=unknown
      ;;
  esac
  XO_PR_GITHUB_BASE=
  XO_PR_GITHUB_QUEUE_OBSERVED=false
...
github_read_outcome() {
  if ! command -v gh >/dev/null 2>&1; then
    if github_read_outcome_with_gh_axi && [ "$XO_PR_GITHUB_MERGED" = true ]; then
      return 0
    fi
...
XO_PR_GITHUB_QUEUE_METHOD=
XO_PR_GITHUB_QUEUE_METHODS=
XO_PR_GITHUB_QUEUE_STATUS=unreadable
github_read_queue_method() {
  ...
  [ -n "$XO_PR_GITHUB_BASE" ] || return 0
  branch_path=$(github_urlencode_path_segment "$XO_PR_GITHUB_BASE")
  if ! methods=$(gh api \
  ...
  while IFS= read -r line; do
    ...
    if [ -z "$XO_PR_GITHUB_QUEUE_METHODS" ] && [ "$count" -eq 1 ]; then
      XO_PR_GITHUB_QUEUE_METHODS=$candidate
    else
      case ",$XO_PR_GITHUB_QUEUE_METHODS," in
        *",$candidate,"*) ;;
        *)
          XO_PR_GITHUB_QUEUE_METHODS="$XO_PR_GITHUB_QUEUE_METHODS,$candidate"
          conflicting=true
          ;;
      esac
    fi
...
METHODS
  if [ "$count" -eq 0 ]; then
    XO_PR_GITHUB_QUEUE_STATUS=none
  elif [ "$conflicting" = true ]; then
    XO_PR_GITHUB_QUEUE_STATUS=conflicting
  elif [ "$unrecognised" = true ]; then
    XO_PR_GITHUB_QUEUE_STATUS=unrecognised
  else
    XO_PR_GITHUB_QUEUE_STATUS=single
    XO_PR_GITHUB_QUEUE_METHOD=$method
  fi
}

record_pr_metadata() {
  if ! "$SCRIPT_DIR/xo-pr-check.sh" "$ID" "$URL"; then
    return 1
  fi
...
require_released_captain_hold() {
  ...
  XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/xo-captain-hold.sh" open "$ID" --distinguish-absent || hold_status=$?
  case "$hold_status" in
    0)
...
XO_PR_MERGE_AUTHORITY=
require_away_merge_grant() {
  local yolo grants grant
  XO_PR_MERGE_AUTHORITY=
  xo_afk_contract_present "$STATE" || return 0
  if ! XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/xo-afk-contract.sh" validate >/dev/null 2>&1; then
  ...
  if [ "$yolo" = on ]; then
    XO_PR_MERGE_AUTHORITY=yolo
    return 0
  fi
  grants=$(XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/xo-afk-contract.sh" grants 2>/dev/null) || {
    echo "error: PR merge refused - the away-posture record's grants could not be read; nothing was merged" >&2
    return 1
  }
  while IFS= read -r grant; do
    [ "$grant" = "$ID" ] || continue
    XO_PR_MERGE_AUTHORITY=away-grant
    return 0
  done <<EOF
$grants
EOF
...
require_current_away_authority() {
  ...
  if xo_afk_contract_present "$STATE" && [ "${#ALLOW_RED[@]}" -gt 0 ]; then
...
XO_PR_GITHUB_AUTO_REQUESTED=false
XO_PR_GITHUB_MERGE_ACCEPTED=false
XO_PR_GITHUB_CALLER_METHOD=
...
github_merge_command_succeeded() {
  [ "$XO_PR_GITHUB_MERGE_ACCEPTED" = true ]
}
...
github_caller_method_is() {
  case "$XO_PR_GITHUB_CALLER_METHOD" in
  ...
  case "$XO_PR_GITHUB_QUEUE_STATUS" in
    ...
    unrecognised)
      ...
      printf 'error: base branch %s requires the merge queue, but its configured merge method (%s) is not one this script recognises, so exact retry flags cannot be named\n' \
        "$XO_PR_GITHUB_BASE" "$methods_display" >&2
      ;;
    unreadable)
...
github_report_unmerged_outcome() {
  printf 'error: GitHub merge outcome was not successful: state=%s, merged=%s, isInMergeQueue=%s\n' \
    "$XO_PR_GITHUB_STATE" "$XO_PR_GITHUB_MERGED" "$XO_PR_GITHUB_QUEUED" >&2
  if ! github_state_is_open || [ "$XO_PR_GITHUB_MERGED" != false ] \
    || [ "$XO_PR_GITHUB_QUEUED" = true ]; then
    return 0
  fi
  if [ "$XO_PR_GITHUB_AUTO_REQUESTED" = true ]; then
  ...
  if [ "$XO_PR_GITHUB_QUEUE_OBSERVED" != true ]; then
...
gitlab_confirm_merged() {
  local json state
  if ! json=$(GITLAB_HOST="$XO_PR_HOST" glab mr view "$PR_NUMBER" \
...
case "$PROVIDER" in
  github)
    ...
    XO_PR_GITHUB_CALLER_METHOD=$(caller_merge_method "$@")
    ...
    merge_output=$(gh pr merge "$PR_NUMBER" --repo "$PR_OWNER/$PR_REPO" \
      --match-head-commit "$XO_PR_MERGE_HEAD" \
      "${merge_args[@]+"${merge_args[@]}"}" "$@" 2>&1) || merge_status=$?
    xo_lock_release "$MERGE_CONTROL_LOCK" || true
    MERGE_CONTROL_LOCK=
    if [ "$merge_status" -eq 0 ]; then
      ...
      if github_read_outcome; then
        if [ "$XO_PR_GITHUB_MERGED" != true ] && [ "$XO_PR_GITHUB_QUEUED" != true ]; then
          github_report_unmerged_outcome
        else
          printf 'actionable: the merge command for %s failed, but the pull request reads back as state=%s, merged=%s, isInMergeQueue=%s\n' \
            "$URL" "$XO_PR_GITHUB_STATE" "$XO_PR_GITHUB_MERGED" "$XO_PR_GITHUB_QUEUED" >&2
        fi
      fi
      exit "$merge_status"
    fi
    ...
    if [ "$XO_PR_GITHUB_MERGED" = true ]; then
      printf 'verified: %s is merged (state=%s, merged=%s, isInMergeQueue=%s)\n' \
        "$URL" "$XO_PR_GITHUB_STATE" "$XO_PR_GITHUB_MERGED" "$XO_PR_GITHUB_QUEUED"
    elif [ "$XO_PR_GITHUB_QUEUED" = true ]; then
  ...
  gitlab)
    ...
    [ "$away_status" -eq 0 ] || exit "$away_status"
    merge_status=0
    GITLAB_HOST="$XO_PR_HOST" glab mr merge "$PR_NUMBER" -R "$PROJECT_URL" \
      --sha "$XO_PR_MERGE_HEAD" --yes "$@" || merge_status=$?
    xo_lock_release "$MERGE_CONTROL_LOCK" || true
    MERGE_CONTROL_LOCK=
...
outcome_rc=0
xo_merge_outcome_report "$XO_HOME" "$STATE" "$ID" "$URL" self \
  "${XO_PR_MERGE_AUTHORITY:-}" || outcome_rc=$?
case "$outcome_rc" in
  0) ;;
  3)
    printf 'actionable: merged %s but could not report it upward: this home has no readable secondmate identity or parent binding (.xo-secondmate-home, .xo-secondmate-parent)\n' \
      "$URL" >&2
    ;;
  *)
