...
# xo-wake-drain.sh after it empties queued wakes, and by xo-session-start.sh in
# read-only advisory mode whenever session-lock ownership was not verified.
# First, always warn if the xo primary checkout (XO_ROOT) is on a named
# non-default branch, because that means xo-on-itself work landed in the
# primary instead of an isolated worktree.
# Then, if the home needs supervision (bin/xo-supervision-lib.sh owns that
...
# has. Supervision health is MODEL-AWARE (xo_watcher_supervision_verdict in
# bin/xo-wake-lib.sh): under the Claude Stop auto-arm model the watcher runs only
# between turns, so mid-turn a fresh beacon with no live watcher is healthy, and
# a stale beacon is still healthy while xo_autoarm_midturn_healthy proves a
...
# per distinct down-episode in this XO_HOME (keyed to the failing condition, not
...
# itself present or retire (xo_wake_actor_pending_count), so it is never an
...
# actor (XO_SUPERVISION_ACTOR=branch), because that actor runs guarded commands
...
READ_ONLY=${XO_GUARD_READ_ONLY:-0}
case "$READ_ONLY" in 1|true|TRUE|yes|YES) READ_ONLY=1 ;; *) READ_ONLY=0 ;; esac
CONTINUE_LINE=${XO_GUARD_CONTINUE_LINE:-This is a supervision warning only; the guarded operation WILL still run.}
...
# shellcheck source=bin/xo-tangle-lib.sh
...
# The current actor (xo_lease_actor is the one owner of that identity); a
# malformed value is a wiring bug elsewhere, so the guard just warns as main.
GUARD_ACTOR=$(xo_lease_actor 2>/dev/null) || GUARD_ACTOR=main
...
tangle_branch=$(xo_primary_tangle_branch "$XO_ROOT" || true)
...
# grace-based predicate (bin/xo-supervision-lib.sh), which owns what needs
# supervision.
xo_supervision_status "$STATE" "$GRACE"
in_flight=$XO_SUP_IN_FLIGHT
sources=$XO_SUP_SOURCES
checks=$XO_SUP_CHECKS
needed=$XO_SUP_NEEDED
beacon_desc=$XO_SUP_BEACON_DESC
xo_watcher_supervision_verdict "$STATE" "$WATCH" "$GRACE" "$XO_HOME" "$XO_ROOT"
watcher_healthy=$XO_WATCHER_VERDICT_OK
watcher_down_reason=$XO_WATCHER_VERDICT_REASON
if [ "$needed" = false ]; then
  ...
  [ "$READ_ONLY" -eq 1 ] || xo_guard_clear_stale_banner
  exit 0
fi
...
# xo-wake-lib.sh owns that per-actor classification. A non-empty queue with
...
if [ -s "$XO_WAKE_QUEUE" ]; then
  if [ "$(xo_wake_actor_pending_count "$GUARD_ACTOR")" -gt 0 ]; then
    queue_pending=true
  elif [ "$GUARD_ACTOR" != branch ] && [ "$(xo_wake_actor_pending_count branch)" -gt 0 ]; then
...
if [ "$watcher_healthy" = false ]; then
  episode_key=$(xo_guard_stale_episode_key "$watcher_down_reason")
  episode_key=${episode_key%$'\n'}
  print_full_banner=0
  if [ "$READ_ONLY" -eq 1 ]; then
    xo_guard_stale_banner_seen "$STATE" "$episode_key" || print_full_banner=1
  elif xo_guard_claim_stale_banner "$STATE" "$episode_key"; then
    print_full_banner=1
  fi
  if [ "$print_full_banner" -eq 1 ]; then
    ...
    fix=$("$SCRIPT_DIR/xo-supervision-instructions.sh" \
      --read-only "$READ_ONLY" \
      --afk "$afk" \
      --x-mode "$x_mode" \
...
# The supervision branch is the exception: it runs guarded commands (xo-peek,
# xo-crew-state) in the middle of handling the very rows that are queued, and
...
if "$queue_pending"; then
  if [ "$READ_ONLY" -eq 1 ]; then
    ...
    echo "WARNING: queued wakes pending - drain them with bin/xo-wake-drain.sh before anything else." >&2
  fi
