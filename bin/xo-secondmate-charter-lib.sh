...
# Source only. XO_SECONDMATE_CHARTER and XO_SECONDMATE_SCOPE remain explicit
...
registry_summary_for_brief() {
  local brief=$1
  if [ -n "${XO_SECONDMATE_CHARTER:-}" ]; then
    printf '%s\n' "$XO_SECONDMATE_CHARTER" | normalize_registry_text
  else
...
registry_scope_for_brief() {
  local brief=$1
  if [ -n "${XO_SECONDMATE_SCOPE:-}" ]; then
    printf '%s\n' "$XO_SECONDMATE_SCOPE" | normalize_registry_text
  else
