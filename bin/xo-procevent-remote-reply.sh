...
#
# Usage:
#   xo-procevent-remote-reply.sh arm <secondmate-id>
#   xo-procevent-remote-reply.sh handle <secondmate-id> <sequence> <result-file>
#   xo-procevent-remote-reply.sh autohandle <source-id> <sequence> <result-file>
#   xo-procevent-remote-reply.sh classify <result-file>
#   xo-procevent-remote-reply.sh terminal <result-file>
#   xo-procevent-remote-reply.sh self-announcing
#   xo-procevent-remote-reply.sh source-id <secondmate-id>
#   xo-procevent-remote-reply.sh retire <secondmate-id>
#
...
# note therefore produces exactly one xo wake, through the same signal
...
# a per-line property that xo-pending-reply-lib.sh consumes; it is never a gate
...
#     bin/xo-pending-reply-lib.sh, because a report that exists remotely but has
#     not been mirrored yet must not be mistaken for a report the mate never
#     wrote (see WINDOW_CLOSED_EMPTY below)
# Line framing and size bounding belong to bin/xo-remote-delta-read.sh, which
...
WAIT_SECONDS=${XO_REMOTE_REPLY_WAIT_SECONDS:-55}
MAX_DOC_BYTES=${XO_REMOTE_REPLY_MAX_DOC_BYTES:-262144}
# xo-on.sh returns ssh's status unchanged, so 255 alone means unavailable
...
empty_hash() {
  local tmp
  tmp=$(mktemp "${TMPDIR:-/tmp}/xo-empty-hash.XXXXXX") || return 1
  : > "$tmp"
  sha256_file "$tmp"
  rm -f -- "$tmp"
}
...
read_cursor() { # <id>; sets CURSOR_OFFSET and CURSOR_HASH
  ...
  [ "$schema" = xo-remote-reply-cursor.v1 ] || die "reply cursor has an incompatible schema: $path"
...
write_cursor() { # <id> <offset> <hash>
  ...
  {
    printf 'schema=xo-remote-reply-cursor.v1\n'
    printf 'offset=%s\n' "$offset"
...
classify_result() {
  ...
  [ "$schema" = xo-remote-delta.v1 ] || { printf 'malformed\n'; return 0; }
  case "$status" in
    delta) printf 'delta\n' ;;
...
cmd_arm_locked() {
  ...
  read_cursor "$id"
  sid=$(source_id "$id")
  "$SCRIPT_DIR/xo-procevent.sh" register remote-reply "$sid" -- \
    "$SCRIPT_DIR/xo-procevent-remote-reply.sh" source "$id" || return 1
...
cmd_arm() {
  ...
  (
    xo_lock_acquire_wait "$lock" || die "cannot lock remote reply lifecycle for $id"
...
# honest watermark, and bin/xo-pending-reply-lib.sh consumes it so a missing
...
cmd_source() {
  ...
  validate_id "$id"
  read_cursor "$id"
  started=$(xo_pending_reply_now)
  "$SCRIPT_DIR/xo-on.sh" "$id" xo-remote-delta-read.sh \
    "$REMOTE_LOG" "$CURSOR_OFFSET" "$CURSOR_HASH" "$WAIT_SECONDS" < /dev/null || rc=$?
  if [ "$rc" -eq "$WINDOW_CLOSED_EMPTY" ]; then
    xo_pending_reply_note_remote_channel_caught_up "$STATE" "$id" "$started" || true
  fi
  return "$rc"
}
...
fetch_document() { # <id> <remote-relative> <result-var>
  ...
  "$SCRIPT_DIR/xo-on.sh" "$id" xo-remote-file.sh get "$rel" "$MAX_DOC_BYTES" < /dev/null > "$tmp" || rc=$?
  if [ "$rc" -ne 0 ]; then
    rm -f -- "$tmp"
...
cmd_ingest() {
  ...
  [ "$schema" = xo-remote-delta.v1 ] && [ "$path" = "$REMOTE_LOG" ] || die "result identifies the wrong source"
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-remote-reply-ingest.XXXXXX") || die "cannot create ingest staging directory"
  trap 'rm -rf -- "$tmp"' EXIT
  payload="$tmp/payload"
  ...
  xo_lock_acquire_wait "$lock" || die "cannot lock remote reply ingest for $id"
  read_cursor "$id"
  ...
  if [ "$class" = continuity-broken ]; then
    ...
    [ "$append_rc" -ne 2 ] || { xo_lock_release "$lock"; die "cannot append continuity escalation"; }
    xo_lock_release "$lock"
    printf 'continuity-broken: %s (%s)\n' "$id" "$reason"
    return 3
  fi
  [ "$status" = delta ] && [ "$payload_bytes" -gt 0 ] || { xo_lock_release "$lock"; die "delta result has no payload"; }
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    while IFS= read -r doc; do
      ...
      [ "$fetch_rc" -ne "$SSH_UNAVAILABLE" ] \
        || { xo_lock_release "$lock"; die "remote transport was unavailable while fetching $doc"; }
      [ "$fetch_rc" -eq 0 ] \
        || { xo_lock_release "$lock"; die "could not store referenced remote document: $doc"; }
    ...
    [ "$append_rc" -ne 2 ] || { xo_lock_release "$lock"; die "cannot append remote reply"; }
  ...
  if [ -n "$undelivered" ]; then
    ...
    append_rc=0
    append_status_once "$status_file" "$line" || append_rc=$?
    [ "$append_rc" -ne 2 ] || { xo_lock_release "$lock"; die "cannot append document escalation"; }
    [ "$append_rc" -ne 0 ] || appended=$((appended + 1))
  fi
  while IFS= read -r corr; do
    [ -n "$corr" ] || continue
    xo_pending_reply_try_resolve "$STATE" "$corr" "$status_file" >/dev/null 2>&1 || true
  done < <(grep -Eo 'corr=[A-Fa-f0-9]{16}' "$normalized_payload" | cut -d= -f2- | tr 'A-F' 'a-f' | awk '!seen[$0]++')
  if [ -n "$seq" ]; then
    write_ingest_receipt "$id" "$seq" "$result" \
      || { xo_lock_release "$lock"; die "cannot commit remote reply ingestion receipt"; }
  fi
  if [ "$cursor_already" -eq 0 ]; then
    write_cursor "$id" "$to" "$to_hash" || { xo_lock_release "$lock"; die "cannot commit remote reply cursor"; }
  fi
  xo_lock_release "$lock"
  trap - EXIT
  rm -rf -- "$tmp"
...
cmd_handle_locked() {
  ...
  "$SCRIPT_DIR/xo-procevent.sh" handled "$sid" "$seq" || return 1
  return "$rc"
}
...
cmd_retire_quiesce_locked() {
  ...
  sid=$(source_id "$id")
  "$SCRIPT_DIR/xo-procevent.sh" retire "$sid" || return 1
...
cmd_retire_finalize_locked() {
  ...
  rm -f -- "$(xo_pending_reply_remote_channel_watermark_path "$STATE" "$id")"
}
...
require_parent_lifecycle_lock() {
  ...
  lock=$(secondmate_reply_lifecycle_lock_path "$STATE" "$id")
  if [ -L "$lock" ]; then
    owner=$(xo_lock_link_owner "$lock" 2>/dev/null || true)
