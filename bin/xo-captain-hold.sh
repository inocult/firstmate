#!/usr/bin/env bash
# xo-captain-hold.sh - deterministic mechanics for tasks held for the captain.
#
...
# `<origin>-decision-<key>` identities through bin/xo-decision-hold.sh; those
...
# directory the way bin/xo-backlog-transition-lib.sh does, which keeps main-home
# and secondmate-home ownership aligned with the work that discovered the call.
#
# Usage:
#   xo-captain-hold.sh hold <task-id> --reason <reason> \
#     [--title <title>] [--repo <repo>] [--origin <origin-id>] [--until YYYY-MM-DD]
#   xo-captain-hold.sh answer <task-id> --decision-file <path> [--release]
#   xo-captain-hold.sh answers [<legacy-origin> | --any-origin] --source <provenance>   (keyed answers on stdin)
#   xo-captain-hold.sh reconcile-requests --source-id <source-id> --source <provenance>   (task ids on stdin)
#   xo-captain-hold.sh bind <source-id> [<legacy-origin> | --any-origin]
#   xo-captain-hold.sh unbind <source-id>
#   xo-captain-hold.sh binding <source-id>
#   xo-captain-hold.sh complete <origin-id> (--none | <task-id>...)
#   xo-captain-hold.sh verify <origin-id>
#   xo-captain-hold.sh open <task-id> [--identity] [--distinguish-absent]
#   xo-captain-hold.sh diverged
#   xo-captain-hold.sh reconcile list
#   xo-captain-hold.sh reconcile close <task-id> --evidence-file <path>
#   xo-captain-hold.sh reconcile note <task-id> --note-file <path>
#
...
# identity, so pre-collapse metadata written by xo-decision-hold.sh verifies
...
# accepted through the migrated row xo-hold-migration produced, found by the
...
# bin/xo-teardown.sh asks it before its automatic
# backlog close and, on 0, returns the row to Queued with its deliverable
# recorded instead (bin/xo-backlog-transition-lib.sh owns that transition), so
...
# bin/xo-watch.sh asks it when an ordinary
...
# reconciled. Records written by the retired xo-decision-hold.sh (routed,
...
# bin/xo-parent-channel-lib.sh, whether or not the mate model appends anything.
...
# the record; bin/xo-inactive-reconcile.sh's diagnostics name a broken binding.
set -eu
...
publish_parent_hold() {  # <task-id> <occurrence> <verb> <note>
  ...
  PARENT_HOLD_PUBLISHED=0
  xo_parent_channel_report "$XO_HOME" "$STATE" \
    "$verb [key=captain-hold-$id-$occurrence]: captain hold $id: $(xo_parent_channel_clean_note "$note")" || rc=$?
  case "$rc" in
...
captain_hold_cleanup() {
  if [ "$CAPTAIN_META_LOCK_HELD" = 1 ]; then
    xo_lock_release "$CAPTAIN_META_LOCK" || true
    CAPTAIN_META_LOCK_HELD=0
  fi
  if [ "$CAPTAIN_CONTROL_LOCK_HELD" = 1 ]; then
    xo_lock_release "$CAPTAIN_CONTROL_LOCK" || true
    CAPTAIN_CONTROL_LOCK_HELD=0
  fi
}
...
# way bin/xo-backlog-transition-lib.sh addresses every transition, so a home
...
tasks_axi() {
  ...
  data=$(xo_backlog_data_absolute "$DATA") || fail "data directory cannot be resolved: $DATA"
  root=$(xo_backlog_root "$data") || fail "$XO_BACKLOG_TRANSITION_ERROR"
  backend=$(xo_tasks_axi_backend "$root") || return 2
  if [ "$backend" = markdown ]; then
    file=$(xo_backlog_file "$data") || fail "$XO_BACKLOG_TRANSITION_ERROR"
...
require_tasks_axi() {
  xo_tasks_axi_compatible || fail "compatible tasks-axi is required"
...
task_show() {  # <id>
  local data
  data=$(xo_backlog_data_absolute "$DATA") || fail "data directory cannot be resolved: $DATA"
  xo_backlog_row_show "$data" "$1" --full 2>/dev/null
}
...
# xo-decision-hold.sh. Both carry the same leader-then-captain-decision shape.
body_has_resolution_record() {  # <task-body>
  case "$1" in
    *"Resolution recorded by xo-captain-hold."*"Captain decision:"*) return 0 ;;
    *"Resolution recorded by xo-decision-hold."*"Captain decision:"*) return 0 ;;
    *"Resolution recorded by xo-captain-hold."*"Reconciliation evidence:"*) return 0 ;;
  esac
  return 1
}
...
resolution_record_count() {  # <task-body>
  ...
  printf '%s\n' "$body" \
    | grep -Ec '^Resolution recorded by xo-(captain|decision)-hold\.$' || true
}
...
resolution_block() {  # <mode>
  ...
  printf 'Resolution recorded by xo-captain-hold.\nDecision digest: %s\nResolution mode: %s\n\n%s\n%s\n' \
...
# row under a prefixed xo- id and recorded its markdown identity in the row's
# notes as "migrated from data/backlog.md id <legacy id>", alone or followed by
# " on <date>" (xo-hold-migration wrote the dated form on 2026-09-04). When an
...
captain_migration_scan_load() {  # <resolved-data-dir>
  ...
  root=$(xo_backlog_root "$data") || {
    printf 'xo-captain-hold: the configured data directory cannot be resolved for a migration scan: %s\n' "$XO_BACKLOG_TRANSITION_ERROR" >&2
    return 2
  }
  backend=$(xo_tasks_axi_backend "$root") || return 2
  ...
  bd_bin=${bd_bin:-bd}
  if [ -z "$bd_path" ]; then
    printf 'xo-captain-hold: the beads backend carries no graph path in %s, so a migrated hold cannot be found\n' "$root/.tasks.toml" >&2
    return 2
  fi
  ...
  command -v "$bd_bin" >/dev/null 2>&1 || {
    printf 'xo-captain-hold: the beads binary %s is not on PATH, so a migrated hold cannot be found\n' "$bd_bin" >&2
    return 2
  }
  command -v jq >/dev/null 2>&1 || {
    printf 'xo-captain-hold: jq is required to scan the beads graph for a migrated hold\n' >&2
    return 2
  }
  local bd_err
  bd_err=$(mktemp "${TMPDIR:-/tmp}/xo-captain-hold-bd.XXXXXX") || {
    printf 'xo-captain-hold: cannot stage the beads graph read diagnostics\n' >&2
    return 2
  }
  if ! CAPTAIN_MIGRATION_SCAN_JSON=$(BEADS_DIR="$bd_path" "$bd_bin" list --all --json 2>"$bd_err"); then
    printf 'xo-captain-hold: reading the beads graph at %s failed (%s), so a migrated hold cannot be found\n' \
...
resolve_migrated_entry() {  # <origin-or-empty> <entry>
  ...
  data=$(xo_backlog_data_absolute "$DATA") || {
    printf 'xo-captain-hold: the migrated hold of %s cannot be resolved: %s\n' \
      "$entry" "${XO_BACKLOG_TRANSITION_ERROR:-the configured data directory $DATA cannot be resolved}" >&2
    return 2
  }
  root=$(xo_backlog_root "$data") || {
    printf 'xo-captain-hold: the migrated hold of %s cannot be resolved: %s\n' \
  ...
  # origin would have minted, because xo-hold-migration recorded the DERIVED
  ...
  if [ -n "$CAPTAIN_MIGRATION_SCAN_JSON" ]; then
    for candidate in $CAPTAIN_MIGRATION_IDENTITIES; do
      candidate_matches=$(printf '%s\n' "$CAPTAIN_MIGRATION_SCAN_JSON" | jq -r \
        ...
        printf 'xo-captain-hold: the beads graph scan for the migrated hold of %s could not be parsed\n' "$candidate" >&2
        return 2
      }
    ...
    case "$count" in
      ...
      *)
        printf 'xo-captain-hold: the migrated hold of %s is ambiguous: %s rows carry its marker line (identities tried: %s)\n' \
  ...
  printf 'xo-captain-hold: the migrated hold of %s is ambiguous: %s captain-held rows carry the configured prefix (identities tried: %s)\n' \
...
write_hold_set_stamp() {  # <task-id> <shown-body> <timestamp> <preserve-existing-0-or-1>
  ...
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-captain-hold-stamp.XXXXXX") \
...
command_hold() {
  ...
  hold_set=${XO_CAPTAIN_HOLD_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
  case "$hold_set" in
    [0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]T[0-9][0-9]:[0-9][0-9]:[0-9][0-9]Z) : ;;
    *) fail "XO_CAPTAIN_HOLD_NOW must be a UTC YYYY-MM-DDTHH:MM:SSZ timestamp" ;;
  esac
...
write_resolution_record() {  # <task-id> <mode> <shown-body>
  ...
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-captain-hold-body.XXXXXX") \
...
report_retained_artifact_failure() {  # <task-id> <marker-path>
  printf 'xo-captain-hold: cannot apply the artifact recorded for %s in %s: %s\n' \
    "$1" "$2" "${XO_BACKLOG_TRANSITION_ERROR:-no reason reported}" >&2
}
...
apply_pending_retained_artifact() {  # <task-id>
  ...
  xo_backlog_close_marker_validate "$marker" "$DATA" "$id" "$STATE" \
    || { report_retained_artifact_failure "$id" "$marker"; return 1; }
  [ "$XO_BACKLOG_CLOSE_VALIDATED_MODE" = retain ] || return 0
  args=("${XO_BACKLOG_CLOSE_VALIDATED_ARGS[@]+"${XO_BACKLOG_CLOSE_VALIDATED_ARGS[@]}"}")
  case "${args[0]-}" in
    --pr|--report)
      xo_backlog_row_artifact_supported "$id" "${args[@]}" || return 0
      xo_backlog_mutate "$DATA" update "$id" "${args[@]}" \
        || { report_retained_artifact_failure "$id" "$marker"; return 1; }
      ;;
  esac
}
...
remove_interrupted_answer_stamp() {  # <task-id>
  ...
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-captain-hold-normalize.XXXXXX") \
...
BINDING_SCHEMA=xo-decision-binding.v1
...
command_answers() {
  ...
  require_tasks_axi
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-keyed-decision.XXXXXX") || fail "cannot stage the captain decision"
  err=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-keyed-decision-err.XXXXXX") \
  ...
  while IFS= read -r row; do
    ...
    if body_has_resolution_record "$body" \
      && { [ "$recorded_digest" = "$digest" ] \
        || { case "$body" in *"Resolution recorded by xo-decision-hold."*) true ;; *) false ;; esac \
...
RECONCILE_SCHEMA=xo-reconcile-request.v1
...
reconcile_request_record() {  # <task-id> <provenance>
  ...
  if {
    printf 'schema=%s\n' "$RECONCILE_SCHEMA"
    printf 'task=%s\n' "$id"
    printf 'requested=%s\n' "${XO_CAPTAIN_HOLD_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}"
...
reconcile_note() {
  ...
  stamp=${XO_CAPTAIN_HOLD_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
  tmp=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-captain-hold-note.XXXXXX") \
...
command_complete() {
  ...
  [ -f "$meta" ] && has_meta=1
  if [ "$has_meta" = 1 ]; then
    CAPTAIN_META_LOCK=$(xo_meta_lock_path "$meta") || fail "could not resolve task metadata lock"
    xo_lock_acquire_wait "$CAPTAIN_META_LOCK"
    CAPTAIN_META_LOCK_HELD=1
    [ -f "$meta" ] || fail "task metadata disappeared while recording completion"
  fi
  require_tasks_axi
  origin_exists_here "$origin" || fail "origin $origin is not owned by the active home $XO_HOME"
  if [ "$#" -eq 1 ] && [ "$1" = --none ]; then
    supplied=''
  else
    while [ "$#" -gt 0 ]; do
...
EOF
  ...
  if [ "$has_meta" = 1 ]; then
    ...
    xo_lock_release "$CAPTAIN_META_LOCK"
    CAPTAIN_META_LOCK_HELD=0
    ...
    # guarded self-announced append (bin/xo-wake-lib.sh) and does not wake this
    # same session; an append failure still fails this command loudly.
    if [ -n "$keys" ]; then
      while IFS=$'\t' read -r key _verb _summary; do
        [ -n "$key" ] || continue
        transfer_rc=0
        xo_wake_status_append_self_announced "$STATE" "$status_file" \
...
open_task_ids() {
  local data
  data=$(xo_backlog_data_absolute "$DATA") || return 1
  xo_backlog_row_list "$data" 2>/dev/null | awk -F, '
    /^  [A-Za-z0-9._-]+,/ {
      id = $1
      sub(/^ +/, "", id)
      if ($2 != "done") print id
...
command_diverged() {
  ...
  # tasks-axi reads $XO_HOME's backlog, so a state dir pointed somewhere else
...
command_open() {  # <task-id> [--identity] [--distinguish-absent]
  ...
  case "$id" in
    ''|*[!A-Za-z0-9._-]*)
      printf 'xo-captain-hold: task id must be a non-empty privacy-safe slug: %s\n' "$id" >&2
      exit 2
      ;;
  esac
  data=$(xo_backlog_data_absolute "$DATA") \
    || { printf 'xo-captain-hold: data directory cannot be resolved: %s\n' "$DATA" >&2; exit 2; }
  root=$(xo_backlog_root "$data") \
    || { printf 'xo-captain-hold: %s\n' "$XO_BACKLOG_TRANSITION_ERROR" >&2; exit 2; }
  if ! backend=$(xo_tasks_axi_backend_resolve "$root"); then
    exit 2
  fi
  if [ "$backend" = markdown ]; then
    file=$(xo_backlog_file "$data") \
      || { printf 'xo-captain-hold: %s\n' "$XO_BACKLOG_TRANSITION_ERROR" >&2; exit 2; }
  ...
  xo_tasks_axi_compatible || { printf 'xo-captain-hold: compatible tasks-axi is required\n' >&2; exit 2; }
  if xo_backlog_row_probe "$data" "$id"; then
    state=${XO_BACKLOG_ROW_STATE%% *}
    if [ "$state" != "done" ] && [ "$XO_BACKLOG_ROW_HOLD_KIND" = captain ]; then
      if [ "$identity" -eq 1 ]; then
        show=$(task_show "$id") || {
          printf 'xo-captain-hold: captain call %s is open but its record could not be read\n' "$id" >&2
          exit 2
        }
