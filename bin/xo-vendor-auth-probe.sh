#!/usr/bin/env bash
# xo-vendor-auth-probe.sh - one hard-bounded, non-destructive authentication
...
# eligible. The dispatching XO owns that judgment from `quota-axi`'s data
...
# captain's 2026-07-30 `xo-grok-auth-preflight` decision approved exactly
...
#
# Usage:
#   xo-vendor-auth-probe.sh <probe>
#
# Environment:
#   XO_VENDOR_AUTH_PROBE_TIMEOUT   hard per-command bound in seconds; must be a
...
usage() {
  cat <<'EOF'
xo-vendor-auth-probe.sh - one hard-bounded, non-destructive authentication probe
...
eligibility. The dispatching XO owns that judgment.
...
Environment:
  XO_VENDOR_AUTH_PROBE_TIMEOUT   hard per-command bound in seconds (default 20);
...
die_usage() {
  printf 'xo-vendor-auth-probe: %s\n' "$1" >&2
  printf 'usage: xo-vendor-auth-probe.sh <probe>   (registered probes: grok)\n' >&2
  exit 2
}
...
TIMEOUT=${XO_VENDOR_AUTH_PROBE_TIMEOUT:-20}
case "$TIMEOUT" in
  ''|*[!0-9]*|0*) TIMEOUT=20 ;;
esac

# Bounded execution is owned by bin/xo-timeout-lib.sh, so a macOS host without
...
grok_version() {
  local output
  output=$(xo_run_timed "$TIMEOUT" grok --version 2>/dev/null </dev/null) || { printf 'none\n'; return 0; }
  printf '%s\n' "$output" | sed -nE 's/.*[^0-9]([0-9]+\.[0-9]+\.[0-9]+).*/\1/p' | head -n 1 | grep . || printf 'none\n'
}

probe_grok() {
  local output first rc=0
  output=$(xo_run_timed "$TIMEOUT" grok models 2>/dev/null </dev/null) || rc=$?
  if [ "$rc" -eq 124 ]; then
    printf 'timeout\n'
    return 0
  fi
