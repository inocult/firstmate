...
#
# Design: data/xo-backend-design-d7/report.md ("Zellij Backend" section - the
...
# bin/xo-backend.sh's xo_backend_source in normal operation; the unit tests
# source it directly.
#
# Session shape (report "Zellij implementation choices" #1, unchanged by
# empirical verification): ONE zellij session (default name "xo",
# overridable via XO_ZELLIJ_SESSION for test isolation - mirrors herdr's
# HERDR_SESSION), ONE tab per task, with caller-facing label "xo-<id>" and a
...
# task in every xo home - primary or secondmate - shares this ONE
# session's tab bar with no per-home split, and zellij enforces no tab-name
# uniqueness at all, two xo homes whose task ids happen to collide
...
# xo_backend_cmux_scoped_title, now shared via bin/xo-backend-hometag-lib.sh)
# is ported here for the identical reason. Every NEW tab is created with a
# title tagged with this installation's home label (xo_backend_zellij_scoped_title,
# "xo-<hometag>-<id>"); every list/find/recover/kill path is scoped to this
# home's own tag. A tab created before this change carries the old untagged
# "xo-<id>" title; target_ready, kill, and ad hoc selector fallback still
# match it, but ONLY when that bare title is unambiguous (exactly one live tab
# in the session carries it) - see xo_backend_zellij_tab_matches_label and
# docs/zellij-backend.md "Home-scoped tab titles" for the full migration
# posture. Moving/relocating an xo installation changes its tag
...
#      xo_backend_zellij_current_path below and docs/zellij-backend.md
...
#      Mitigated (xo_backend_zellij_create_task): capture the previously
...
#     (xo_backend_zellij_session_exists, a passive list-sessions query, never
...
# xo_backend_required_tools only when zellij is the resolved backend; this
# adapter also gates them again before spawning.

# XO_HOME fallback: every real caller already sets XO_HOME as a global before
# sourcing xo-backend.sh (which sources this file); this exists only so this
...
# XO_HOME/XO_ROOT now also feed xo_backend_zellij_home_label's tab-title tag
# below.
XO_BACKEND_ZELLIJ_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
XO_ROOT="${XO_ROOT_OVERRIDE:-${XO_ROOT:-$XO_BACKEND_ZELLIJ_ROOT}}"
XO_HOME="${XO_HOME:-${XO_ROOT_OVERRIDE:-$XO_ROOT}}"

# shellcheck source=bin/xo-backend-hometag-lib.sh
. "$XO_BACKEND_ZELLIJ_ROOT/bin/xo-backend-hometag-lib.sh"
...
. "$XO_BACKEND_ZELLIJ_ROOT/bin/xo-composer-lib.sh"
...
XO_BACKEND_ZELLIJ_MIN_MAJOR=0
XO_BACKEND_ZELLIJ_MIN_MINOR=44

# xo_backend_zellij_session: the session name this spawn/op uses.
# XO_ZELLIJ_SESSION mirrors herdr's HERDR_SESSION ambient-selection knob: an
# operator (or xo's own isolated test harness) sets it explicitly;
# absent means the shared "xo" session. Do not use this alone for
# destructive test cleanup; tests/zellij-test-safety.sh documents and guards
# that path (mirrors tests/herdr-test-safety.sh).
xo_backend_zellij_session() {
  printf '%s' "${XO_ZELLIJ_SESSION:-xo}"
}

# xo_backend_zellij_home_label: readable home prefix plus a short hash of the
# resolved XO_ROOT path (bin/xo-backend-hometag-lib.sh). Zellij has one
# session-global tab namespace shared by every xo home, so the path
...
xo_backend_zellij_home_label() {
  xo_backend_hometag
}

# xo_backend_zellij_scoped_title: the actual tab title a NEW task's tab is
# created with - the caller-facing "xo-<id>" label, home-tagged as
# "xo-<hometag>-<id>" (mirrors bin/backends/cmux.sh's identical
# xo_backend_cmux_scoped_title). Every list/find/recover/kill path below
# scopes its own-home matches through this.
xo_backend_zellij_scoped_title() {  # <xo-task-label>
  local label=$1 rest home
  home=$(xo_backend_zellij_home_label)
  case "$label" in
    xo-*) rest=${label#xo-} ;;
    *) rest=$label ;;
  esac
  printf 'xo-%s-%s' "$home" "$rest"
}

# xo_backend_zellij_tool_check: refuse loudly if zellij or jq is missing.
xo_backend_zellij_tool_check() {
...
# xo_backend_zellij_version_check: refuse loudly on a missing/incompatible
...
xo_backend_zellij_version_check() {
  xo_backend_zellij_tool_check || return 1
  ...
  if [ "$major" -lt "$XO_BACKEND_ZELLIJ_MIN_MAJOR" ] || { [ "$major" -eq "$XO_BACKEND_ZELLIJ_MIN_MAJOR" ] && [ "$minor" -lt "$XO_BACKEND_ZELLIJ_MIN_MINOR" ]; }; then
    echo "error: zellij $ver is older than the verified minimum $XO_BACKEND_ZELLIJ_MIN_MAJOR.$XO_BACKEND_ZELLIJ_MIN_MINOR; update zellij before using backend=zellij" >&2
    return 1
  fi
  return 0
}

# xo_backend_zellij_cli: run `zellij --session <session> action <args...>`,
...
# for defense in depth, mirroring bin/backends/herdr.sh's xo_backend_herdr_cli
# rationale even though no equivalent env-var-unreliable incident has been
# observed for zellij).
xo_backend_zellij_cli() {  # <session> <action-subcommand-and-args...>
  local session=$1
  shift
  ZELLIJ_SESSION_NAME="$session" zellij --session "$session" "$@"
}

# xo_backend_zellij_session_exists: passive, READ-ONLY liveness check - never
...
xo_backend_zellij_session_exists() {  # <session>
  zellij list-sessions --short --no-formatting 2>/dev/null | grep -qxF "$1"
}

# xo_backend_zellij_server_ensure: create the named session in the background,
...
xo_backend_zellij_server_ensure() {  # <session>
  local session=$1 i
  xo_backend_zellij_session_exists "$session" && return 0
...
# xo_backend_zellij_container_ensure: the full spawn-time container-ensure
...
xo_backend_zellij_container_ensure() {
  local session
  xo_backend_zellij_version_check || return 1
  session=$(xo_backend_zellij_session)
  xo_backend_zellij_server_ensure "$session" || return 1
  printf '%s' "$session"
}

# xo_backend_zellij_pane_for_tab: the terminal (non-plugin) pane id for
...
xo_backend_zellij_pane_for_tab() {  # <session> <tab_id>
  local session=$1 tab_id=$2
  xo_backend_zellij_cli "$session" action list-panes --json 2>/dev/null \
    | jq -r --argjson t "$tab_id" '.[]? | select(.tab_id == $t and .is_plugin == false) | .id' 2>/dev/null | head -1
}

# xo_backend_zellij_tab_for_pane: the owning tab id for <pane_id> in
...
xo_backend_zellij_tab_for_pane() {  # <session> <pane_id>
  local session=$1 pane_id=$2
  xo_backend_zellij_cli "$session" action list-panes --json 2>/dev/null \
    | jq -r --argjson p "$pane_id" '.[]? | select(.id == $p and .is_plugin == false) | .tab_id' 2>/dev/null | head -1
}

xo_backend_zellij_pane_exists() {  # <session> <pane_id>
  local session=$1 pane_id=$2
  xo_backend_zellij_cli "$session" action list-panes --json 2>/dev/null \
    | jq -e --argjson p "$pane_id" '[.[]? | select(.id == $p and .is_plugin == false)] | length > 0' >/dev/null 2>&1
}

# xo_backend_zellij_tab_matches_label: does <tab_id> in <session> carry the
# tab name xo expects for the caller-facing task label <label>?
# Checks the home-scoped, tagged title first (xo_backend_zellij_scoped_title
# - what every NEW tab is created with), then falls back to the legacy
# untagged bare title (the plain <label>, e.g. "xo-<id>") for a tab created
...
# same-named tab from a different xo home sharing this one zellij
...
xo_backend_zellij_tab_matches_label() {  # <session> <tab_id> <label>
  local session=$1 tab_id=$2 label=$3 scoped tabs count
  scoped=$(xo_backend_zellij_scoped_title "$label")
  tabs=$(xo_backend_zellij_cli "$session" action list-tabs --json 2>/dev/null)
...
# xo_backend_zellij_create_task: create the task's tab (one terminal pane) in
...
# (xo_backend_zellij_scoped_title), never the bare caller-facing label - see
...
# no-op when no client is attached (the common case: an unattended xo
# spawn). Best-effort: a failure to restore never fails the spawn.
#
# Echoes "<tab_id> <pane_id>" on success.
xo_backend_zellij_create_task() {  # <session> <label> <cwd>
  local session=$1 label=$2 cwd=$3 title tabs dup prev_active tab_id pane_id
  xo_backend_zellij_session_exists "$session" || { echo "error: zellij session '$session' does not exist; run container_ensure first" >&2; return 1; }
  title=$(xo_backend_zellij_scoped_title "$label")
  tabs=$(xo_backend_zellij_cli "$session" action list-tabs --json 2>/dev/null)
  ...
  tab_id=$(xo_backend_zellij_cli "$session" action new-tab --cwd "$cwd" --name "$title" 2>/dev/null | tr -d '[:space:]')
  case "$tab_id" in
    ''|*[!0-9]*)
  ...
  pane_id=$(xo_backend_zellij_pane_for_tab "$session" "$tab_id")
  if [ -z "$pane_id" ]; then
  ...
  if [ -n "$prev_active" ] && [ "$prev_active" != "$tab_id" ]; then
    xo_backend_zellij_cli "$session" action go-to-tab-by-id "$prev_active" >/dev/null 2>&1 || true
  fi
  printf '%s %s' "$tab_id" "$pane_id"
}

# xo_backend_zellij_parse_target: split "<session>:<pane_id>" on the FIRST
...
# XO_BACKEND_ZELLIJ_SESSION and XO_BACKEND_ZELLIJ_PANE for the caller.
xo_backend_zellij_parse_target() {  # <target>
  local target=$1
  XO_BACKEND_ZELLIJ_SESSION=${target%%:*}
  XO_BACKEND_ZELLIJ_PANE=${target#*:}
  [ -n "$XO_BACKEND_ZELLIJ_SESSION" ] && [ -n "$XO_BACKEND_ZELLIJ_PANE" ] && [ "$XO_BACKEND_ZELLIJ_PANE" != "$target" ]
}

# xo_backend_zellij_target_ready: parse the target and verify its session and
# pane are alive. When the caller knows the owning xo task label, verify
# the pane belongs to that named tab before trusting the numeric pane id.
xo_backend_zellij_target_ready() {  # <target> [expected-label]
  local expected_label=${2:-} tab_id
  xo_backend_zellij_parse_target "$1" || return 1
  xo_backend_zellij_session_exists "$XO_BACKEND_ZELLIJ_SESSION" || return 1
  if [ -n "$expected_label" ]; then
    tab_id=$(xo_backend_zellij_tab_for_pane "$XO_BACKEND_ZELLIJ_SESSION" "$XO_BACKEND_ZELLIJ_PANE" 2>/dev/null)
    [ -n "$tab_id" ] || return 1
    xo_backend_zellij_tab_matches_label "$XO_BACKEND_ZELLIJ_SESSION" "$tab_id" "$expected_label"
    return $?
  fi
  xo_backend_zellij_pane_exists "$XO_BACKEND_ZELLIJ_SESSION" "$XO_BACKEND_ZELLIJ_PANE"
}

# xo_backend_zellij_current_path: the live pane's cwd, or empty on any error.
...
# and read only that marker line. Scoped to xo-spawn.sh's own worktree-discovery
...
# answer.
xo_backend_zellij_current_path() {  # <target> [expected-label]
  local target=$1 expected_label=${2:-} out line marker_begin="__XO_ZELLIJ_CWD_BEGIN__" marker_end="__XO_ZELLIJ_CWD_END__" in_block=0 chunk="" last=""
  xo_backend_zellij_target_ready "$target" "$expected_label" || return 0
  xo_backend_zellij_send_text_line "$target" "printf '%s\n' '$marker_begin'; pwd; printf '%s\n' '$marker_end'" "$expected_label" || return 0
  sleep 0.3
  out=$(xo_backend_zellij_capture "$target" 200 "$expected_label") || return 0
...
# xo_backend_zellij_send_literal: send TEXT as literal, UNSUBMITTED input via
...
xo_backend_zellij_send_literal() {  # <target> <text> [expected-label]
  xo_backend_zellij_target_ready "$1" "${3:-}" || return 1
  xo_backend_zellij_cli "$XO_BACKEND_ZELLIJ_SESSION" action paste --pane-id "$XO_BACKEND_ZELLIJ_PANE" -- "$2" >/dev/null 2>&1
}

# xo_backend_zellij_normalize_key: map xo's key vocabulary (Enter,
...
# words all fail.
xo_backend_zellij_normalize_key() {  # <key>
  case "$1" in
...
# xo_backend_zellij_send_key: one named special key, targeted at the pane by
# its EXPLICIT --pane-id (never the ambient "focused pane" default - verified
# unreliable, see file header). Mirrors xo-send.sh's --key path.
xo_backend_zellij_send_key() {  # <target> <key> [expected-label]
  xo_backend_zellij_target_ready "$1" "${3:-}" || return 1
  local key
  key=$(xo_backend_zellij_normalize_key "$2")
  xo_backend_zellij_cli "$XO_BACKEND_ZELLIJ_SESSION" action send-keys --pane-id "$XO_BACKEND_ZELLIJ_PANE" "$key" >/dev/null 2>&1
}

# xo_backend_zellij_send_text_line: send one line of TEXT then submit.
xo_backend_zellij_send_text_line() {  # <target> <text> [expected-label]
  xo_backend_zellij_send_literal "$1" "$2" "${3:-}" || return 1
  xo_backend_zellij_send_key "$1" Enter "${3:-}" && return 0
  xo_backend_zellij_send_key "$1" C-c "${3:-}" >/dev/null 2>&1 && return 1
  return 2
}

# xo_backend_zellij_capture: bounded plain-text pane capture. Mirrors
# xo-peek.sh's/xo-watch.sh's `tmux capture-pane -p -t T -S -N`. `dump-screen`
...
xo_backend_zellij_capture() {  # <target> <lines> [expected-label]
  xo_backend_zellij_target_ready "$1" "${3:-}" || return 1
  local lines=${2:-40} out
  case "$lines" in ''|*[!0-9]*) lines=40 ;; esac
  if [ "$lines" -le 40 ]; then
    out=$(xo_backend_zellij_cli "$XO_BACKEND_ZELLIJ_SESSION" action dump-screen --pane-id "$XO_BACKEND_ZELLIJ_PANE" 2>/dev/null) || return 1
  else
    out=$(xo_backend_zellij_cli "$XO_BACKEND_ZELLIJ_SESSION" action dump-screen --pane-id "$XO_BACKEND_ZELLIJ_PANE" --full 2>/dev/null) || return 1
  fi
...
# the shared owner (bin/xo-composer-lib.sh, xo_composer_classify_screen);
# this adapter contributes only the capture and its capability facts.

# xo_backend_zellij_composer_capture: bounded styled tail of the pane. When
# --ansi is unsupported (an older zellij), the caller falls back to the plain
# dump and a styled=0 descriptor - see xo_backend_zellij_composer_state.
xo_backend_zellij_composer_capture() {  # <target> [expected-label]
  xo_backend_zellij_target_ready "$1" "${2:-}" || return 1
  local out
  out=$(xo_backend_zellij_cli "$XO_BACKEND_ZELLIJ_SESSION" action dump-screen --pane-id "$XO_BACKEND_ZELLIJ_PANE" --ansi 2>/dev/null) || return 1
  [ -n "$out" ] || return 1
  printf '%s' "$out" | tail -n "$XO_COMPOSER_CAPTURE_LINES"
}

# xo_backend_zellij_composer_state: thin adapter - capture plus capabilities
...
xo_backend_zellij_composer_state() {  # <target> [expected-label] -> empty|pending|pending-unproven|unknown
  local target=$1 expected_label=${2:-} cap caps verdict
  if cap=$(xo_backend_zellij_composer_capture "$target" "$expected_label"); then
    caps=$(printf 'styled=1\ncursor=0\nidentity=0\nrows=%s' "$XO_COMPOSER_CAPTURE_LINES")
  elif cap=$(xo_backend_zellij_capture "$target" "$XO_COMPOSER_CAPTURE_LINES" "$expected_label") && [ -n "$cap" ]; then
...
xo_backend_zellij_composer_content() {  # <target> [expected-label]
  local target=$1 expected_label=${2:-} cap caps
  cap=$(xo_backend_zellij_composer_capture "$target" "$expected_label") || return 1
  caps=$(printf 'styled=1\ncursor=0\nidentity=0\nrows=%s' "$XO_COMPOSER_CAPTURE_LINES")
  xo_composer_extract_selected_content "$caps" "$cap"
}

xo_backend_zellij_composer_observed_append() {  # <target> <before> <text> [expected-label]
  local target=$1 before=$2 text=$3 expected_label=${4:-} cap caps after expected
  [ -n "$text" ] || return 1
  cap=$(xo_backend_zellij_composer_capture "$target" "$expected_label") || return 1
  caps=$(printf 'styled=1\ncursor=0\nidentity=0\nrows=%s' "$XO_COMPOSER_CAPTURE_LINES")
  after=$(xo_composer_extract_selected_content "$caps" "$cap") || return 1
  xo_composer_normalize_spaces_var before
  xo_composer_normalize_spaces_var text
  xo_composer_normalize_spaces_var after
  before=${before//[$' \t\r\n\v\f']/}
  text=${text//[$' \t\r\n\v\f']/}
  after=${after//[$' \t\r\n\v\f']/}
  [ -n "$text" ] || return 1
...
# xo_backend_zellij_send_text_submit: type <text> into <target> once (raw,
# unsubmitted, via send_literal), then drive the shared verify-and-retry-Enter
# loop (bin/xo-composer-lib.sh: xo_composer_submit_retry_core) against the
...
xo_backend_zellij_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle> [expected-label]
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5 expected_label=${6:-} before
  before=$(xo_backend_zellij_composer_content "$target" "$expected_label") \
    || { printf 'send-failed'; return 0; }
  xo_backend_zellij_send_literal "$target" "$text" "$expected_label" || { printf 'send-failed'; return 0; }
  sleep "$settle"
  xo_backend_zellij_composer_observed_append "$target" "$before" "$text" "$expected_label" \
    || { printf 'send-failed'; return 0; }
  xo_composer_submit_retry_core xo_backend_zellij_send_key xo_backend_zellij_composer_state \
    "$target" "$retries" "$sleep_s" "$expected_label"
}

# xo_backend_zellij_kill: remove the task's tab, best-effort (mirrors
...
# pane id when possible via xo_backend_zellij_tab_for_pane; teardown also
...
xo_backend_zellij_kill() {  # <target> [tab_id] [expected_label]
  xo_backend_zellij_parse_target "$1" || return 0
  xo_backend_zellij_session_exists "$XO_BACKEND_ZELLIJ_SESSION" || return 0
  local tab_id fallback_tab_id=${2:-} expected_label=${3:-}
  tab_id=$(xo_backend_zellij_tab_for_pane "$XO_BACKEND_ZELLIJ_SESSION" "$XO_BACKEND_ZELLIJ_PANE" 2>/dev/null)
  if [ -n "$tab_id" ] && [ -n "$expected_label" ] && ! xo_backend_zellij_tab_matches_label "$XO_BACKEND_ZELLIJ_SESSION" "$tab_id" "$expected_label"; then
    tab_id=
  fi
  case "$fallback_tab_id" in
    ...
    *)
      if [ -z "$tab_id" ]; then
        if [ -z "$expected_label" ] || xo_backend_zellij_tab_matches_label "$XO_BACKEND_ZELLIJ_SESSION" "$fallback_tab_id" "$expected_label"; then
  ...
  if [ -n "$tab_id" ]; then
    xo_backend_zellij_cli "$XO_BACKEND_ZELLIJ_SESSION" action close-tab-by-id "$tab_id" >/dev/null 2>&1 || true
  elif [ -z "$expected_label" ]; then
    xo_backend_zellij_cli "$XO_BACKEND_ZELLIJ_SESSION" action close-pane --pane-id "$XO_BACKEND_ZELLIJ_PANE" >/dev/null 2>&1 || true
  fi
}

# xo_backend_zellij_list_live: recovery/orphan discovery. Lists every tab in
# <session> whose title carries THIS xo home's own tag
# (xo-<hometag>-, xo_backend_zellij_home_label) - never any other home's
# tagged tabs, and never a bare untagged "xo-<id>" tab either (this sweep
# deliberately does NOT attempt the legacy-bare-title fallback
# xo_backend_zellij_tab_matches_label uses for a single already-known tab:
...
# fallback. One "<session>:<pane_id>\t<plain xo-<id> label>" line per live,
...
xo_backend_zellij_list_live() {  # <session>
  local session=$1 home prefix tabs tab_id name pane_id plain
  xo_backend_zellij_session_exists "$session" || return 0
  home=$(xo_backend_zellij_home_label)
  prefix="xo-$home-"
  tabs=$(xo_backend_zellij_cli "$session" action list-tabs --json 2>/dev/null) || return 0
  while IFS=$'\t' read -r tab_id name; do
    [ -n "$tab_id" ] || continue
    plain=${name#"$prefix"}
    [ -n "$plain" ] || continue
    pane_id=$(xo_backend_zellij_pane_for_tab "$session" "$tab_id") || continue
    [ -n "$pane_id" ] || continue
    printf '%s:%s\txo-%s\n' "$session" "$pane_id" "$plain"
  done < <(printf '%s' "$tabs" | jq -r --arg prefix "$prefix" '.[]? | select(.name | startswith($prefix)) | "\(.tab_id)\t\(.name)"' 2>/dev/null)
}

# xo_backend_zellij_resolve_bare_selector: the live-tab-listing fallback for
...
# (xo_backend_zellij_scoped_title), then falls back to an exact bare <name>
# match ONLY when unambiguous - exactly one tab across every active session
# carries it - mirroring xo_backend_zellij_tab_matches_label's migration
# posture. Rare path in practice (zellij tasks normally carry meta);
# best-effort. Not wired into xo_backend_resolve_selector's dispatcher
# (bin/xo-backend.sh), mirroring herdr: that bare-selector fallback stays
# tmux-only by design, and zellij/herdr tasks are targeted via task-selector
# meta or an explicit recorded target.
xo_backend_zellij_resolve_bare_selector() {  # <name>
  local name=$1 scoped sessions session tabs tab_id count=0 pane_id bare_session='' bare_tab_id=''
  scoped=$(xo_backend_zellij_scoped_title "$name")
...
EOF
  if [ "$count" -eq 1 ]; then
    pane_id=$(xo_backend_zellij_pane_for_tab "$bare_session" "$bare_tab_id") || pane_id=
    if [ -n "$pane_id" ]; then
