...
xo_backend_orca_json_get() {  # <field> ; fields: worktree-id worktree-path terminal-handle worktree-terminal-handle repo-id
...
xo_backend_orca_run_json() {
  local out
  out=$("$@") || return 1
  printf '%s' "$out" | xo_backend_orca_json_ok
}

xo_backend_orca_repo_ensure() {  # <project-path>
  local project=$1 out repo_id
  xo_backend_orca_tool_check || return 1
  out=$(orca repo show --repo "path:$project" --json 2>/dev/null || true)
  if repo_id=$(printf '%s' "$out" | xo_backend_orca_json_get repo-id 2>/dev/null); then
    printf '%s' "$repo_id"
    return 0
  fi
  out=$(orca repo add --path "$project" --json) || return 1
  repo_id=$(printf '%s' "$out" | xo_backend_orca_json_get repo-id) || {
...
xo_backend_orca_worktree_create() {  # <project-path> <name>
  local project=$1 name=$2 repo_id out wt_id wt_path terminal
  repo_id=$(xo_backend_orca_repo_ensure "$project") || return 1
  out=$(orca worktree create --repo "id:$repo_id" --name "$name" --no-parent --setup skip --json) || return 1
  wt_id=$(printf '%s' "$out" | xo_backend_orca_json_get worktree-id) || {
    echo "error: orca worktree create did not return a worktree id for $name" >&2
    return 1
  }
  terminal=$(printf '%s' "$out" | xo_backend_orca_json_get worktree-terminal-handle 2>/dev/null || true)
  wt_path=$(printf '%s' "$out" | xo_backend_orca_json_get worktree-path) || {
    echo "error: orca worktree create did not return a path for $name" >&2
    [ -z "$terminal" ] || xo_backend_orca_kill "$terminal" >/dev/null 2>&1 || true
    if xo_backend_orca_remove_worktree "$wt_id" >/dev/null; then
      return 1
    fi
    if [ -n "$terminal" ]; then
...
xo_backend_orca_send_text_line() {  # <terminal-id> <text>
  local terminal=$1 text=$2
  xo_backend_orca_tool_check || return 1
  xo_backend_orca_run_json orca terminal send --terminal "$terminal" --text "$text" --enter --json
}

xo_backend_orca_send_literal() {  # <terminal-id> <text>
  local terminal=$1 text=$2
  xo_backend_orca_tool_check || return 1
  xo_backend_orca_run_json orca terminal send --terminal "$terminal" --text "$text" --json
}

xo_backend_orca_remove_worktree() {  # <worktree-id>
  local worktree_id=${1:-}
  [ -n "$worktree_id" ] || { echo "error: missing Orca worktree id; cannot remove worktree" >&2; return 1; }
  xo_backend_orca_tool_check || return 1
  xo_backend_orca_run_json orca worktree rm --worktree "id:$worktree_id" --force --json
}

xo_backend_orca_worktree_path() {
  ...
  path=$(printf '%s' "$out" | xo_backend_orca_json_get worktree-path) || {
...
xo_backend_orca_capture() {  # <terminal-id> <lines>
  local terminal=$1 lines=${2:-40} out
  xo_backend_orca_tool_check || return 1
  out=$(orca terminal read --terminal "$terminal" --limit "$lines" --json) || return 1
  xo_backend_orca_json_text "$out"
}

xo_backend_orca_json_text() {  # <json>
  printf '%s' "$1" | node -e '
const fs = require("fs");
const data = JSON.parse(fs.readFileSync(0, "utf8"));
if (data.ok === false) {
...
# xo_backend_orca_composer_capture: the orca composer screen - one bounded
...
xo_backend_orca_composer_capture() {  # <terminal-id> [expected-label]
  xo_backend_orca_capture "$1" "$XO_COMPOSER_CAPTURE_LINES"
}

# xo_backend_orca_composer_caps: static capability facts, not logic (see the
# capability model in bin/xo-composer-lib.sh). Orca's `terminal read` returns
...
xo_backend_orca_composer_caps() {
  printf 'styled=0\ncursor=0\nidentity=0\nrows=%s\n' "$XO_COMPOSER_CAPTURE_LINES"
}

# xo_backend_orca_composer_state: thin adapter - capture plus capabilities in,
...
# unconfirmed) lives in bin/xo-composer-lib.sh.
xo_backend_orca_composer_state() {  # <terminal-id> [expected-label] -> empty|pending|pending-unproven|unknown
  local cap verdict
  cap=$(xo_backend_orca_composer_capture "$1") || { printf 'unknown'; return 0; }
  verdict=$(xo_composer_classify_screen "$(xo_backend_orca_composer_caps)" "$cap")
  [ "$verdict" != need-identity ] || verdict=unknown
  printf '%s' "$verdict"
}

xo_backend_orca_send_key() {  # <terminal-id> <key>
  local terminal=$1 key=$2
  xo_backend_orca_tool_check || return 1
  case "$key" in
    C-c|ctrl+c|Ctrl-c|Ctrl-C)
      xo_backend_orca_run_json orca terminal send --terminal "$terminal" --interrupt --json
      ;;
    Enter|enter)
      xo_backend_orca_run_json orca terminal send --terminal "$terminal" --text "" --enter --json
      ;;
    *)
...
# xo_backend_orca_send_text_submit: type <text> once, then drive the shared
# verify-and-retry-Enter loop (bin/xo-composer-lib.sh:
# xo_composer_submit_retry_core) against the shared composer verdict, so a
# slash-command popup placeholder fill gets the required second Enter without
# duplicating text.
xo_backend_orca_send_text_submit() {  # <terminal-id> <text> <retries> <enter-sleep> <settle>
  local terminal=$1 text=$2 retries=$3 sleep_s=$4 settle=$5
  xo_backend_orca_tool_check || { printf 'send-failed'; return 0; }
  xo_backend_orca_send_literal "$terminal" "$text" || { printf 'send-failed'; return 0; }
  sleep "$settle"
  xo_composer_submit_retry_core xo_backend_orca_send_key xo_backend_orca_composer_state \
    "$terminal" "$retries" "$sleep_s"
}

xo_backend_orca_kill() {  # <terminal-id>
  xo_backend_orca_tool_check || return 0
