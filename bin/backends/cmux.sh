...
# stays treehouse. Sourced only through bin/xo-backend.sh's xo_backend_source
...
# layer, so workspace titles are scoped by xo home and installation
...
# runtime auto-detection when xo itself is already running inside a
...
#      spirit (xo_backend_cmux_current_path below).
...
#      as xo_backend_cmux_target_ready's liveness probe (the design sketch's
...
#      gap and is used instead (xo_backend_cmux_surface_exists), mirroring
...
#      workspace in its window. `xo_backend_cmux_kill` handles the documented
...
#      therefore uses scoped-title matching from the caller-facing xo-<id>
...
#      shared cmux app cannot cross-match another xo home's task.
#
...
#   inside cmux can connect"). Since xo always drives cmux from an
...
# see xo_backend_cmux_bin), jq (JSON parsing). Bootstrap detects these through
# xo_backend_required_tools only when cmux is the resolved backend; this adapter
...
XO_BACKEND_CMUX_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
XO_ROOT="${XO_ROOT_OVERRIDE:-${XO_ROOT:-$XO_BACKEND_CMUX_ROOT}}"
XO_HOME="${XO_HOME:-${XO_ROOT_OVERRIDE:-$XO_ROOT}}"

# shellcheck source=bin/xo-backend-hometag-lib.sh
. "$XO_BACKEND_CMUX_ROOT/bin/xo-backend-hometag-lib.sh"
...
. "$XO_BACKEND_CMUX_ROOT/bin/xo-composer-lib.sh"

# Verified minimum: the version the live pass ran against (docs/cmux-backend.md).
XO_BACKEND_CMUX_MIN_MAJOR=0
XO_BACKEND_CMUX_MIN_MINOR=64

# xo_backend_cmux_bin: resolve the cmux CLI binary. cmux does not reliably
...
XO_BACKEND_CMUX_BUNDLE_BIN="${XO_BACKEND_CMUX_BUNDLE_BIN:-/Applications/cmux.app/Contents/Resources/bin/cmux}"
xo_backend_cmux_bin() {
  if command -v cmux >/dev/null 2>&1; then
    printf 'cmux'
    return 0
  fi
  if [ -x "$XO_BACKEND_CMUX_BUNDLE_BIN" ]; then
    printf '%s' "$XO_BACKEND_CMUX_BUNDLE_BIN"
    return 0
  fi
  return 1
}

xo_backend_cmux_tool_check() {
  xo_backend_cmux_bin >/dev/null 2>&1 || { echo "error: backend=cmux selected but the 'cmux' CLI was not found on PATH or at $XO_BACKEND_CMUX_BUNDLE_BIN (https://cmux.com)" >&2; return 1; }
...
# xo_backend_cmux_password: the optional socket password from
...
# is absent - xo_backend_cmux_cli only exports this when it resolves non-empty.
xo_backend_cmux_password() {
  local config_dir="${XO_CONFIG_OVERRIDE:-$XO_HOME/config}" f line
...
# xo_backend_cmux_cli: run `cmux <args...>`, quieted (suppresses legacy-alias
...
xo_backend_cmux_cli() {  # <cmux-subcommand-and-args...>
  local bin pw
  bin=$(xo_backend_cmux_bin) || return 1
  pw=$(xo_backend_cmux_password)
  if [ -n "$pw" ]; then
...
# xo_backend_cmux_version_check: refuse loudly on a missing/incompatible cmux
...
# separate from reachability/auth (xo_backend_cmux_ping_state below).
xo_backend_cmux_version_check() {
  xo_backend_cmux_tool_check || return 1
  local raw ver major rest minor
  raw=$(xo_backend_cmux_cli version 2>/dev/null) || { echo "error: 'cmux version' failed; is cmux installed correctly?" >&2; return 1; }
  ...
  if [ "$major" -lt "$XO_BACKEND_CMUX_MIN_MAJOR" ] || { [ "$major" -eq "$XO_BACKEND_CMUX_MIN_MAJOR" ] && [ "$minor" -lt "$XO_BACKEND_CMUX_MIN_MINOR" ]; }; then
    echo "error: cmux $ver is older than the verified minimum $XO_BACKEND_CMUX_MIN_MAJOR.$XO_BACKEND_CMUX_MIN_MINOR; update cmux before using backend=cmux" >&2
    return 1
  fi
  return 0
}

# xo_backend_cmux_ping_state: classify socket reachability/auth from `cmux
...
# relaunching the app.
xo_backend_cmux_ping_state() {
  local out
  out=$(xo_backend_cmux_cli ping 2>&1)
  if [ "$out" = "PONG" ]; then
    printf 'ok'
    return 0
  fi
...
# xo_backend_cmux_refuse_denied / xo_backend_cmux_refuse_unauth: the two
...
# xo_backend_cmux_ensure_running: launch cmux (mirrors the CLI's own
...
# launch on the wire.
xo_backend_cmux_ensure_running() {
  local state i
  state=$(xo_backend_cmux_ping_state)
  case "$state" in
    ok) return 0 ;;
    denied)
      xo_backend_cmux_refuse_denied
...
# xo_backend_cmux_container_ensure: the full spawn-time container-ensure
...
# straight to xo_backend_cmux_create_task.
xo_backend_cmux_container_ensure() {
  xo_backend_cmux_version_check || return 1
  xo_backend_cmux_ensure_running || return 1
  return 0
}

# xo_backend_cmux_home_label: readable home prefix plus a short hash of the
# resolved XO_ROOT path. cmux has one app-global workspace namespace, so the
# path hash distinguishes every xo installation, including multiple
...
# itself lives in bin/xo-backend-hometag-lib.sh, shared with zellij's
# identical shared-namespace collision fix (docs/zellij-backend.md
# "Home-scoped tab titles").
xo_backend_cmux_home_label() {
  xo_backend_hometag
}

xo_backend_cmux_scoped_title() {  # <xo-task-label>
  local label=$1 rest home
  home=$(xo_backend_cmux_home_label)
  case "$label" in
    xo-*) rest=${label#xo-} ;;
    *) rest=$label ;;
  esac
...
# xo_backend_cmux_workspace_id_for_label: the live workspace id whose title
...
xo_backend_cmux_workspace_id_for_label() {  # <label>
  local label=$1
  xo_backend_cmux_cli workspace list --json --id-format uuids 2>/dev/null \
    | jq -r --arg want "$label" '.workspaces[]? | select(.title == $want) | .id' 2>/dev/null | head -1
}

xo_backend_cmux_surface_id_for_workspace() {  # <workspace_id>
  local wsid=$1
  xo_backend_cmux_cli list-panes --workspace "$wsid" --json --id-format uuids 2>/dev/null \
    | jq -r '.panes[0] // {} | .selected_surface_id // (.surface_ids[0] // empty)' 2>/dev/null
}

# xo_backend_cmux_create_task: create the task's workspace (one surface),
...
xo_backend_cmux_create_task() {  # <label> <cwd>
  local label=$1 cwd=$2 title dup out wsid sfid
  title=$(xo_backend_cmux_scoped_title "$label")
  dup=$(xo_backend_cmux_workspace_id_for_label "$title")
  if [ -n "$dup" ]; then
    echo "error: cmux workspace '$title' already exists" >&2
    return 1
  fi
  out=$(xo_backend_cmux_cli new-workspace --name "$title" --cwd "$cwd" --focus false --id-format uuids 2>&1) || {
    echo "error: cmux new-workspace failed for '$title': $out" >&2
    return 1
  }
  wsid=$(xo_backend_cmux_workspace_id_for_label "$title")
  [ -n "$wsid" ] || { echo "error: could not resolve a cmux workspace id for '$title' after creation" >&2; return 1; }
  sfid=$(xo_backend_cmux_surface_id_for_workspace "$wsid")
  [ -n "$sfid" ] || { echo "error: could not resolve the default surface for cmux workspace '$title' ($wsid)" >&2; return 1; }
  printf '%s %s' "$wsid" "$sfid"
}

# xo_backend_cmux_parse_target: split "<workspace_uuid>:<surface_uuid>" on the
# FIRST colon (neither UUID contains a colon, so this is unambiguous). Sets
# XO_BACKEND_CMUX_WORKSPACE and XO_BACKEND_CMUX_SURFACE for the caller.
xo_backend_cmux_parse_target() {  # <target>
  local target=$1
  XO_BACKEND_CMUX_WORKSPACE=${target%%:*}
  XO_BACKEND_CMUX_SURFACE=${target#*:}
  [ -n "$XO_BACKEND_CMUX_WORKSPACE" ] && [ -n "$XO_BACKEND_CMUX_SURFACE" ] && [ "$XO_BACKEND_CMUX_SURFACE" != "$target" ]
}

# xo_backend_cmux_surface_exists: does <surface_id> currently appear as one of
...
# would make read-screen unusable as xo_backend_cmux_target_ready's liveness
...
# (xo_backend_zellij_pane_exists) rather than the design sketch's original
# read-screen-based suggestion.
xo_backend_cmux_surface_exists() {  # <workspace_id> <surface_id>
  local wsid=$1 sfid=$2
  xo_backend_cmux_cli list-panes --workspace "$wsid" --json --id-format uuids 2>/dev/null \
    | jq -e --arg s "$sfid" '[.panes[]? | select(.surface_ids // [] | index($s))] | length > 0' >/dev/null 2>&1
}

# xo_backend_cmux_target_ready: parse the target and verify it is live via
# xo_backend_cmux_surface_exists (never read-screen - see that function's
# header for the fresh-surface pitfall this avoids). When the caller knows
# the owning xo task label, refresh stale workspace/surface ids by label.
xo_backend_cmux_target_ready() {  # <target> [expected-label]
  local expected_label=${2:-} expected_title title wsid sfid
  xo_backend_cmux_parse_target "$1" || return 1
  if [ -n "$expected_label" ]; then
    expected_title=$(xo_backend_cmux_scoped_title "$expected_label")
    title=$(xo_backend_cmux_cli workspace list --json --id-format uuids 2>/dev/null | jq -r --arg id "$XO_BACKEND_CMUX_WORKSPACE" '.workspaces[]? | select(.id == $id) | .title' 2>/dev/null)
    if [ "$title" = "$expected_title" ]; then
      xo_backend_cmux_surface_exists "$XO_BACKEND_CMUX_WORKSPACE" "$XO_BACKEND_CMUX_SURFACE" && return 0
      wsid=$XO_BACKEND_CMUX_WORKSPACE
    elif [ -n "$title" ]; then
      return 1
    else
      wsid=$(xo_backend_cmux_workspace_id_for_label "$expected_title")
      [ -n "$wsid" ] || return 1
    fi
    sfid=$(xo_backend_cmux_surface_id_for_workspace "$wsid")
    [ -n "$sfid" ] || return 1
    XO_BACKEND_CMUX_WORKSPACE=$wsid
    XO_BACKEND_CMUX_SURFACE=$sfid
    return 0
  fi
  xo_backend_cmux_surface_exists "$XO_BACKEND_CMUX_WORKSPACE" "$XO_BACKEND_CMUX_SURFACE"
}

# xo_backend_cmux_current_path: the live foreground process's cwd, or empty on
# any error. Mirrors xo_backend_zellij_current_path's active pwd-marker-probe
...
# that marker line. Scoped to xo-spawn.sh's own worktree-discovery poll loop.
xo_backend_cmux_current_path() {  # <target> [expected-label]
  local target=$1 expected_label=${2:-} out line marker_begin="__XO_CMUX_CWD_BEGIN__" marker_end="__XO_CMUX_CWD_END__" in_block=0 chunk="" last=""
  xo_backend_cmux_target_ready "$target" "$expected_label" || return 0
  xo_backend_cmux_send_text_line "$target" "printf '%s\n' '$marker_begin'; pwd; printf '%s\n' '$marker_end'" "$expected_label" || return 0
  sleep 0.3
  out=$(xo_backend_cmux_capture "$target" 200 "$expected_label") || return 0
...
# xo_backend_cmux_send_literal: send TEXT as literal, UNSUBMITTED input - the
...
xo_backend_cmux_send_literal() {  # <target> <text> [expected-label]
  xo_backend_cmux_target_ready "$1" "${3:-}" || return 1
  xo_backend_cmux_cli send --workspace "$XO_BACKEND_CMUX_WORKSPACE" --surface "$XO_BACKEND_CMUX_SURFACE" -- "$2" >/dev/null 2>&1
}

# xo_backend_cmux_normalize_key: map xo's key vocabulary (Enter,
...
# sigint/sigtstp/sigquit - `TerminalSurface+Input.swift`), but xo's
# shared vocabulary across backends only needs these three today.
xo_backend_cmux_normalize_key() {  # <key>
  case "$1" in
...
# xo_backend_cmux_send_key: one named special key. Escape IS natively
# supported here (unlike Orca, docs/orca-backend.md), so it is wired directly.
xo_backend_cmux_send_key() {  # <target> <key> [expected-label]
  xo_backend_cmux_target_ready "$1" "${3:-}" || return 1
  local key
  key=$(xo_backend_cmux_normalize_key "$2")
  xo_backend_cmux_cli send-key --workspace "$XO_BACKEND_CMUX_WORKSPACE" --surface "$XO_BACKEND_CMUX_SURFACE" "$key" >/dev/null 2>&1
}

# xo_backend_cmux_send_text_line: send one line of TEXT then submit.
xo_backend_cmux_send_text_line() {  # <target> <text> [expected-label]
  xo_backend_cmux_send_literal "$1" "$2" "${3:-}" || return 1
  xo_backend_cmux_send_key "$1" Enter "${3:-}" && return 0
  xo_backend_cmux_send_key "$1" C-c "${3:-}" >/dev/null 2>&1 && return 1
  return 2
}

# xo_backend_cmux_capture: bounded plain-text surface capture. No herdr-style
...
xo_backend_cmux_capture() {  # <target> <lines> [expected-label]
  xo_backend_cmux_target_ready "$1" "${3:-}" || return 1
  local lines=${2:-200} fetch raw out
  case "$lines" in ''|*[!0-9]*) lines=200 ;; esac
  fetch=$lines
  case "$fetch" in ''|*[!0-9]*) fetch=200 ;; *) [ "$fetch" -ge 200 ] || fetch=200 ;; esac
  raw=$(xo_backend_cmux_cli read-screen --workspace "$XO_BACKEND_CMUX_WORKSPACE" --surface "$XO_BACKEND_CMUX_SURFACE" --scrollback --lines "$fetch" --json 2>/dev/null) || return 1
  out=$(printf '%s' "$raw" | jq -r '.text // empty' 2>/dev/null) || return 1
  printf '%s' "$out" | tail -n "$lines"
}

# xo_backend_cmux_composer_capture: the cmux composer screen - a bounded
...
xo_backend_cmux_composer_capture() {  # <target> [expected-label]
  xo_backend_cmux_capture "$1" "$XO_COMPOSER_CAPTURE_LINES" "${2:-}"
}

# xo_backend_cmux_composer_caps: static capability facts, not logic (see the
# capability model in bin/xo-composer-lib.sh).
xo_backend_cmux_composer_caps() {
  printf 'styled=0\ncursor=0\nidentity=0\nrows=%s\n' "$XO_COMPOSER_CAPTURE_LINES"
}

# xo_backend_cmux_composer_state: thin adapter - capture plus capabilities in,
...
# bin/xo-composer-lib.sh, so a new harness shape is taught there once and
# never here. cmux has no identity probe, so the classifier's identity
# sentinel resolves to unknown.
xo_backend_cmux_composer_state() {  # <target> [expected-label] -> empty|pending|pending-unproven|unknown
  local cap verdict
  cap=$(xo_backend_cmux_composer_capture "$1" "${2:-}") || { printf 'unknown'; return 0; }
  verdict=$(xo_composer_classify_screen "$(xo_backend_cmux_composer_caps)" "$cap")
  [ "$verdict" != need-identity ] || verdict=unknown
  printf '%s' "$verdict"
}

# xo_backend_cmux_send_text_submit: type <text> into <target> once (raw,
...
xo_backend_cmux_send_text_submit() {  # <target> <text> <retries> <enter-sleep> <settle> [expected-label]
  local target=$1 text=$2 retries=$3 sleep_s=$4 settle=$5 expected_label=${6:-}
  xo_backend_cmux_parse_target "$target" || { printf 'unknown'; return 0; }
  xo_backend_cmux_send_literal "$target" "$text" "$expected_label" || { printf 'send-failed'; return 0; }
  sleep "$settle"
  xo_composer_submit_retry_core xo_backend_cmux_send_key xo_backend_cmux_composer_state \
    "$target" "$retries" "$sleep_s" "$expected_label"
}

# xo_backend_cmux_window_of_workspace: echo "<window_id> <workspace_count>" for
...
xo_backend_cmux_window_of_workspace() {  # <workspace_id> -> "<window_id> <count>"
  local wsid=$1 wins wid wss count
  wins=$(xo_backend_cmux_cli list-windows --json --id-format uuids 2>/dev/null) || return 0
  while IFS= read -r wid; do
    [ -n "$wid" ] || continue
    wss=$(xo_backend_cmux_cli workspace list --json --id-format uuids --window "$wid" 2>/dev/null) || continue
...
# xo_backend_cmux_kill: remove the task's whole workspace, best-effort (mirrors
...
# leaving that window a fresh default workspace (never an xo-<home>- title, so
# recovery/list_live ignore it) - cmux's own "closed the last tab" outcome.
xo_backend_cmux_kill() {  # <target> [unused] [expected-label]
  local expected_label=${3:-} wsid wininfo win count
  if [ -n "$expected_label" ]; then
    xo_backend_cmux_target_ready "$1" "$expected_label" || return 0
  else
    xo_backend_cmux_parse_target "$1" || return 0
  fi
  wsid=$XO_BACKEND_CMUX_WORKSPACE
  wininfo=$(xo_backend_cmux_window_of_workspace "$wsid")
  win=${wininfo%% *}
  count=${wininfo##* }
  if [ -n "$win" ] && [ "$count" = 1 ]; then
    xo_backend_cmux_cli new-workspace --window "$win" --focus false --id-format uuids >/dev/null 2>&1 || true
  fi
  xo_backend_cmux_cli close-workspace --workspace "$wsid" >/dev/null 2>&1 || true
}

# xo_backend_cmux_list_live: recovery/orphan discovery. Lists every workspace
# whose title is scoped to this xo home, by TITLE - never by trusting a
# stored uuid, since workspace ids do NOT survive an app relaunch (finding #5).
# One "<workspace_id>:<surface_id>\t<xo-id>" line per live task workspace.
# Read-only: an unreachable cmux simply lists nothing.
xo_backend_cmux_list_live() {
  local wss wsid title sfid home prefix plain
  home=$(xo_backend_cmux_home_label)
  prefix="xo-$home-"
  wss=$(xo_backend_cmux_cli workspace list --json --id-format uuids 2>/dev/null) || return 0
