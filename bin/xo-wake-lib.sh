...
XO_WAKE_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XO_WAKE_DEFAULT_ROOT="$(cd "$XO_WAKE_LIB_DIR/.." && pwd)"
XO_ROOT="${XO_ROOT_OVERRIDE:-${XO_ROOT:-$XO_WAKE_DEFAULT_ROOT}}"
XO_HOME="${XO_HOME:-${XO_ROOT_OVERRIDE:-$XO_ROOT}}"
STATE="${XO_STATE_OVERRIDE:-${STATE:-$XO_HOME/state}}"
XO_WAKE_QUEUE="${XO_WAKE_QUEUE:-$STATE/.wake-queue}"
XO_WAKE_QUEUE_LOCK="${XO_WAKE_QUEUE_LOCK:-$STATE/.wake-queue.lock}"
XO_LOCK_STALE_AFTER="${XO_LOCK_STALE_AFTER:-2}"
# Resolved once at source time: xo_pid_identity and xo_path_mtime run inside 0.2s
# confirm and 0.5s attach polls, and forking uname per call is a measurable cost on
# the platform (Git Bash/MSYS) that already pays the highest fork price.
_XO_UNAME=$(uname 2>/dev/null || echo unknown)
mkdir -p "$STATE"
...
_xo_wake_require_timeout() {
  command -v xo_run_timed >/dev/null 2>&1 && return 0
  # shellcheck source=bin/xo-timeout-lib.sh
  . "$XO_WAKE_LIB_DIR/xo-timeout-lib.sh"
}
...
xo_current_pid() {  # [output-variable]
  local xo_pid
  xo_pid=${BASHPID:-$(exec sh -c 'printf "%s\n" "$PPID"')} || return 1
  case "$xo_pid" in ''|*[!0-9]*|0) return 1 ;; esac
  if [ "$#" -gt 0 ]; then
    printf -v "$1" '%s' "$xo_pid"
  else
    printf '%s\n' "$xo_pid"
...
xo_pid_identity() {
  ...
  if [ -r "$proc_root/$pid/stat" ] && [ -r "$proc_root/$pid/cmdline" ]; then
    ...
    [ "$_XO_UNAME" != Linux ] || identity_key=linux-starttime
...
xo_path_age() {
  local path=$1 m
  m=$(xo_path_mtime "$path") || { echo 999999; return; }
  echo $(( $(date +%s) - m ))
}

# xo_poll_derived_grace [poll-seconds]
...
# Defaults to $XO_POLL (xo-watch.sh's own poll env var) when no argument is
# given, so a caller with no independent notion of the poll cadence still
# derives the same default xo-watch.sh itself would use.
# docs/turnend-guard.md "Guard grace and the poll cadence" is the single owner
# of the rationale; every XO_GUARD_GRACE default should derive from this.
xo_poll_derived_grace() {
  local poll=${1:-${XO_POLL:-15}} margin=60 derived
...
XO_WATCHER_MATCHED_IDENTITY=
xo_watcher_lock_matches_pid() {
  local state=$1 watch_path=$2 pid=$3 home=${4:-$XO_HOME} lockdir lock_home lock_path lock_identity current_identity
  XO_WATCHER_MATCHED_IDENTITY=
  lockdir="$state/.watch.lock"
  lock_home=$(cat "$lockdir/xo-home" 2>/dev/null || true)
  lock_path=$(cat "$lockdir/watcher-path" 2>/dev/null || true)
  lock_identity=$(cat "$lockdir/pid-identity" 2>/dev/null || true)
  [ "$lock_home" = "$home" ] || return 1
  [ "$lock_path" = "$watch_path" ] || return 1
  [ -n "$lock_identity" ] || return 1
  current_identity=$(xo_pid_identity "$pid") || return 1
  [ "$current_identity" = "$lock_identity" ] || return 1
  XO_WATCHER_MATCHED_IDENTITY=$lock_identity
}

XO_WATCHER_HEALTHY_PID=
XO_WATCHER_HEALTHY_IDENTITY=
xo_watcher_healthy() {
  local state=$1 watch_path=$2 grace=${3:-${XO_GUARD_GRACE:-300}} home=${4:-$XO_HOME} lockdir beat pid identity age
  XO_WATCHER_HEALTHY_PID=
  XO_WATCHER_HEALTHY_IDENTITY=
  lockdir="$state/.watch.lock"
  ...
  xo_watcher_lock_matches_pid "$state" "$watch_path" "$pid" "$home" || return 1
  identity=$XO_WATCHER_MATCHED_IDENTITY
  age=$(xo_path_age "$beat")
  [ "$age" -lt "$grace" ] || return 1
  # shellcheck disable=SC2034 # Read by callers after xo_watcher_healthy returns.
  XO_WATCHER_HEALTHY_PID=$pid
  # shellcheck disable=SC2034 # Read by callers after xo_watcher_healthy returns.
  XO_WATCHER_HEALTHY_IDENTITY=$identity
  return 0
}

# xo_watcher_healthy above is the PID-STRICT primitive: true only when a live,
# identity-matched watcher PROCESS holds this home's lock with a fresh beacon. The
# arm layer (bin/xo-watch-arm.sh, bin/xo-claude-stop-autoarm.sh) needs exactly
# that - it decides whether to start, attach to, or replace a real watcher
# process, so a leftover beacon must never satisfy it. bin/xo-turnend-guard.sh
# also keeps this strict check because it fires at the turn boundary where the
# auto-arm brings a fresh watcher up. The pull warning (bin/xo-guard.sh) fires
# mid-turn, where the auto-arm model runs no watcher at all, so it wants a
# different, model-aware question:

# xo_supervision_model
...
#               (xo_autoarm_midturn_healthy).
#   extension   Pi (and pi-signed): .pi/extensions/xo-primary-pi-watch.ts owns
...
# XO_SUPERVISION_MODEL overrides detection (tests, and callers that already know
# the harness). Otherwise bin/xo-harness.sh is the single detection owner, so this
# stays consistent with the harness-specific repair line the guards already emit.
xo_supervision_model() {
  local harness
  case "${XO_SUPERVISION_MODEL:-}" in
    autoarm|extension|persistent) printf '%s\n' "$XO_SUPERVISION_MODEL"; return 0 ;;
  esac
  harness=$("$XO_WAKE_LIB_DIR/xo-harness.sh" 2>/dev/null || printf unknown)
  case "$harness" in
...
#
# xo_pi_extension_version <file>
# Print the marker version string the Pi extensions record for <file>. Must stay
# byte-identical to the "sha256:<hex>" digest .pi/extensions/xo-primary-pi-watch.ts
# and .pi/extensions/xo-primary-turnend-guard.ts compute for themselves; a host
...
# xo_pi_extension_loaded <marker> <expected-version> <session-lock>
...
# xo_pi_extension_owns_supervision <state> <root>
...
xo_pi_extension_owns_supervision() {
  xo_extension_pair_owns_supervision "$1" "$2/.pi/extensions" \
    "xo-primary-pi-watch.ts:.pi-watch-extension-loaded" \
    "xo-primary-turnend-guard.ts:.pi-turnend-extension-loaded"
}

# xo_omp_extension_owns_supervision <state> <root>
...
xo_omp_extension_owns_supervision() {
  xo_extension_pair_owns_supervision "$1" "$2/.omp/extensions" \
    "xo-primary-omp-watch.ts:.omp-watch-extension-loaded" \
    "xo-primary-turnend-guard.ts:.omp-turnend-extension-loaded"
}

# xo_extension_owns_supervision <state> <root>
...
xo_extension_owns_supervision() {
  xo_pi_extension_owns_supervision "$1" "$2" || xo_omp_extension_owns_supervision "$1" "$2"
}

xo_extension_pair_owns_supervision() {  # <state> <extension-dir> <source:marker>...
  ...
  for pair in "$@"; do
    source=${pair%%:*}
    marker=${pair#*:}
    version=$(xo_pi_extension_version "$dir/$source") || return 1
    xo_pi_extension_loaded "$state/$marker" "$version" "$lock" || return 1
  done
  session_pid=$(sed -n '1p' "$lock" 2>/dev/null)
  xo_pid_alive "$session_pid"
}

# Away-mode supervision evidence. While state/.afk exists the away-mode daemon
# (bin/xo-supervise-daemon.sh) owns supervision: it runs bin/xo-watch.sh
...
#
# xo_afk_daemon_owns_supervision <state>
...
# watcher lock uses (xo_watcher_lock_matches_pid): a recycled pid, a lock left
...
# beacon passes grace.
xo_afk_daemon_owns_supervision() {
  ...
  [ -n "$recorded" ] || return 1
  current=$(xo_pid_identity "$pid" 2>/dev/null) || return 1
  [ -n "$current" ] || return 1
  [ "$current" = "$recorded" ]
}

# xo_watcher_supervision_verdict <state> <watch-path> [grace] [home] [root]
# Model-aware "is supervision healthy right now" verdict for the pull warning
# guard (bin/xo-guard.sh), NOT the arm layer or the turn-end guard. Sets:
#   XO_WATCHER_VERDICT_OK      true when supervision is healthy for this model
#   XO_WATCHER_VERDICT_REASON  when not ok, the true failing condition:
...
# while xo_autoarm_midturn_healthy proves a Claude auto-arm generation
...
# session provably owns continuity (xo_extension_owns_supervision: the Pi or the
...
# (xo_watcher_healthy); a fresh leftover beacon with no live watcher is still down.
# shellcheck disable=SC2034 # Read by callers after the function returns.
XO_WATCHER_VERDICT_OK=false
# shellcheck disable=SC2034 # Read by callers after the function returns.
XO_WATCHER_VERDICT_REASON=stale-beacon
xo_watcher_supervision_verdict() {
  local state=$1 watch=$2 grace=${3:-${XO_GUARD_GRACE:-300}} home=${4:-$XO_HOME}
  local root=${5:-$XO_ROOT}
  ...
  model=$(xo_supervision_model)
  if [ "$model" = autoarm ]; then
    if [ "$fresh" = true ] || xo_autoarm_midturn_healthy "$state" "$grace"; then
      XO_WATCHER_VERDICT_OK=true
    fi
    return 0
  fi
  if xo_watcher_healthy "$state" "$watch" "$grace" "$home"; then
    # shellcheck disable=SC2034 # Read by callers after the function returns.
    XO_WATCHER_VERDICT_OK=true
  elif [ "$fresh" = true ]; then
    if [ "$model" = extension ] && xo_watcher_lock_unheld "$state" \
      && xo_extension_owns_supervision "$state" "$root"; then
      # shellcheck disable=SC2034 # Read by callers after the function returns.
      XO_WATCHER_VERDICT_OK=true
    else
      # shellcheck disable=SC2034 # Read by callers after the function returns.
      XO_WATCHER_VERDICT_REASON=no-watcher
    fi
  fi
  return 0
}

xo_lock_clean_known_files() {
  local lockdir=$1
  rm -f \
    "$lockdir/pid" \
    "$lockdir/xo-home" \
...
xo_lock_set_role() {
  ...
  xo_current_pid current || return 1
  pid=$(cat "$lockdir/pid" 2>/dev/null || true)
...
xo_lock_owner_dir() {
  ...
  lock_abs=$(xo_lock_abs_path "$lockdir") || return 1
  mktemp -d "${lock_abs}.owner.XXXXXX" 2>/dev/null
}

xo_lock_prepare_owner() {
  local ownerdir=$1 mypid back
  xo_current_pid mypid || return 1
...
xo_lock_points_to_owner() {
  local lockdir=$1 ownerdir=$2 actual
  actual=$(readlink "$lockdir" 2>/dev/null) || return 1
  [ "$actual" = "$ownerdir" ]
}

xo_lock_discard_owner() {
  local ownerdir=$1
  [ -n "$ownerdir" ] || return 0
  xo_lock_clean_known_files "$ownerdir"
  rmdir "$ownerdir" 2>/dev/null || true
}

xo_lock_remove_stray_owner_link() {
...
xo_lock_claim_blocked_by_steal() {
  local lockdir=$1 allowed_steal_owner=${2:-} steal
  steal="$lockdir.steal"
  [ -e "$steal" ] || [ -L "$steal" ] || return 1
  if [ -n "$allowed_steal_owner" ] && xo_lock_points_to_owner "$steal" "$allowed_steal_owner"; then
    return 1
  fi
  return 0
}
...
xo_lock_claim() {
  ...
  if ! { printf '%s\n' "$mypid" > "$ownerdir/pid"; } 2>/dev/null; then
    xo_lock_discard_owner "$ownerdir"
    return 1
  fi
  back=$(cat "$ownerdir/pid" 2>/dev/null || true)
  if [ "$back" != "$mypid" ]; then
  ...
  if ! xo_lock_points_to_owner "$lockdir" "$ownerdir"; then
    xo_lock_discard_owner "$ownerdir"
    return 1
  fi
  if xo_lock_claim_blocked_by_steal "$lockdir" "$allowed_steal_owner"; then
    if xo_lock_points_to_owner "$lockdir" "$ownerdir"; then
...
xo_lock_try_create() {
  ...
  XO_LOCK_OWNER_DIR=
  ownerdir=$(xo_lock_owner_dir "$lockdir") || return 1
  if [ -e "$lockdir" ] || [ -L "$lockdir" ]; then
    xo_lock_discard_owner "$ownerdir"
    return 1
  fi
  if ! xo_lock_prepare_owner "$ownerdir"; then
    xo_lock_discard_owner "$ownerdir"
    return 1
  fi
  if ln -s "$ownerdir" "$lockdir" 2>/dev/null && xo_lock_points_to_owner "$lockdir" "$ownerdir"; then
    if xo_lock_claim "$lockdir" "$ownerdir" "$allowed_steal_owner"; then
      XO_LOCK_OWNER_DIR=$ownerdir
      return 0
    fi
    if xo_lock_points_to_owner "$lockdir" "$ownerdir"; then
      rm -f "$lockdir" 2>/dev/null || true
    ...
    xo_lock_remove_stray_owner_link "$lockdir" "$ownerdir"
  fi
  xo_lock_discard_owner "$ownerdir"
  return 1
}
...
xo_lock_remove_path() {
  ...
  if [ -L "$lockdir" ]; then
    ownerdir=$(xo_lock_link_owner "$lockdir" 2>/dev/null || true)
    rm -f "$lockdir" 2>/dev/null || return 1
    [ -n "$ownerdir" ] && xo_lock_discard_owner "$ownerdir"
    return 0
  fi
  xo_lock_clean_known_files "$lockdir"
  rmdir "$lockdir" 2>/dev/null
}

xo_lock_mid_acquire_is_fresh() {
  local lockdir=$1 pid=$2 mid_acquire_stale
  case "$pid" in
    ''|*[!0-9]*)
      mid_acquire_stale=$XO_LOCK_STALE_AFTER
      [ "$mid_acquire_stale" -lt 2 ] && mid_acquire_stale=2
      [ "$(xo_path_age "$lockdir")" -lt "$mid_acquire_stale" ]
      return
      ;;
  esac
  return 1
...
xo_lock_recheck_stale_owner() {
  local lockdir=$1 expected_owner=$2 expected_pid=$3 actual_pid
  if [ -n "$expected_owner" ]; then
    xo_lock_points_to_owner "$lockdir" "$expected_owner" || return 1
  elif [ -e "$lockdir" ] || [ -L "$lockdir" ]; then
    [ -d "$lockdir" ] && [ ! -L "$lockdir" ] || return 1
  fi
  actual_pid=$(cat "$lockdir/pid" 2>/dev/null || true)
  [ "$actual_pid" = "$expected_pid" ] || return 1
  if xo_pid_alive "$actual_pid"; then
    return 1
  fi
  if xo_lock_mid_acquire_is_fresh "$lockdir" "$actual_pid"; then
    return 1
  fi
  return 0
}

XO_RECOVERY_MARKER_TOKEN=
XO_RECOVERY_MARKER_ACTION='none'
...
_xo_recovery_marker_write_locked() {
  ...
  [ -n "$generation" ] || generation="$(xo_current_pid).$(date +%s).${tmp##*.}"
  if ! printf '%s:%s:%s\n' "$status" "$kind" "$generation" > "$tmp" \
    || ! chmod 0600 "$tmp" \
    || ! _xo_atomic_replace "$tmp" "$marker"; then
    rm -f -- "$tmp"
    return 1
  fi
}
...
_xo_recovery_marker_publish() {
  ...
  if [ "$kind" = downtime ]; then
    ...
    saved_token=$XO_RECOVERY_MARKER_TOKEN
    if xo_recovery_marker_read "$marker"; then
      case "$XO_RECOVERY_MARKER_TOKEN" in
        pending:handling:*|pending:downtime:*)
          generation=${XO_RECOVERY_MARKER_TOKEN##*:}
          status=pending
          ;;
    ...
    XO_RECOVERY_MARKER_TOKEN=$saved_token
  fi
  if ! _xo_recovery_marker_write_locked "$marker" "$kind" "$generation" "$status"; then
    xo_lock_release "$lock"
    return 1
  fi
  xo_lock_release "$lock"
...
_xo_recovery_marker_begin_handling() {
  ...
  case "$line" in
    ...
    pending:downtime:*)
      if ! _xo_recovery_marker_write_locked "$marker" handling "$generation"; then
        xo_lock_release "$lock"
        return 1
      fi
      XO_RECOVERY_MARKER_TOKEN="pending:handling:$generation"
      ;;
    announced:downtime:*)
      if ! _xo_recovery_marker_write_locked "$marker" handling "$generation" announced; then
        xo_lock_release "$lock"
        return 1
      fi
      XO_RECOVERY_MARKER_TOKEN="announced:handling:$generation"
      ;;
    *) xo_lock_release "$lock"; return 1 ;;
  esac
  xo_lock_release "$lock"
}

xo_recovery_marker_snapshot() {
  local marker=$1 lock
  XO_RECOVERY_MARKER_TOKEN=
  lock="${marker}.lock"
  xo_lock_acquire_wait "$lock" || return 1
  xo_recovery_marker_read "$marker" || true
  xo_lock_release "$lock"
}

_xo_recovery_marker_ack() {
  local marker=$1 expected_generation=$2 lock tmp line
  [ -n "$expected_generation" ] || return 2
  lock="${marker}.lock"
  xo_lock_acquire_wait "$lock" || return 1
  if ! xo_recovery_marker_read "$marker" \
    || [ "${XO_RECOVERY_MARKER_TOKEN##*:}" != "$expected_generation" ]; then
    xo_lock_release "$lock"
    return 3
  fi
  line=$XO_RECOVERY_MARKER_TOKEN
  case "$line" in
    ...
    acked:*) xo_lock_release "$lock"; return 0 ;;
    *) xo_lock_release "$lock"; return 1 ;;
  esac
  tmp=$(mktemp "${marker}.tmp.XXXXXX") || { xo_lock_release "$lock"; return 1; }
  if ! printf '%s\n' "$line" > "$tmp" \
    || ! chmod 0600 "$tmp" \
...
_xo_recovery_marker_arm_check() {
  local marker=$1 lock line quarantine
  XO_RECOVERY_MARKER_ACTION='none'
  lock="${marker}.lock"
  xo_lock_acquire_wait "$XO_WAKE_QUEUE_LOCK" || return 1
  if ! xo_lock_acquire_wait "$lock"; then
    xo_lock_release "$XO_WAKE_QUEUE_LOCK"
    return 1
  fi
  if [ ! -e "$marker" ] && [ ! -L "$marker" ]; then
    if [ -s "$XO_WAKE_QUEUE" ]; then
      if ! _xo_recovery_marker_write_locked "$marker" downtime "" announced; then
        xo_lock_release "$lock"
        xo_lock_release "$XO_WAKE_QUEUE_LOCK"
        return 1
      fi
      XO_RECOVERY_MARKER_ACTION='recover'
    fi
    xo_lock_release "$lock"
    xo_lock_release "$XO_WAKE_QUEUE_LOCK"
    return 0
  ...
  if ! xo_recovery_marker_read "$marker"; then
    ...
    if ! mv -- "$marker" "$quarantine/marker" \
      || ! _xo_recovery_marker_write_locked "$marker" downtime "" announced; then
  ...
  case "$line" in
    pending:handling:*|announced:handling:*|announced:downtime:*)
      XO_RECOVERY_MARKER_ACTION='wait'
      xo_lock_release "$lock"
      xo_lock_release "$XO_WAKE_QUEUE_LOCK"
      return 0
      ;;
    pending:downtime:*)
      if ! _xo_recovery_marker_write_locked "$marker" downtime "${line##*:}" announced; then
        xo_lock_release "$lock"
        xo_lock_release "$XO_WAKE_QUEUE_LOCK"
        return 1
      fi
      XO_RECOVERY_MARKER_TOKEN="announced:downtime:${line##*:}"
      XO_RECOVERY_MARKER_ACTION='recover'
      ;;
    acked:*)
      if [ -s "$XO_WAKE_QUEUE" ]; then
...
_xo_recovery_marker_reopen_announced() {
  local marker=$1 lock
  lock="${marker}.lock"
  xo_lock_acquire_wait "$lock" || return 1
  if ! xo_recovery_marker_read "$marker"; then
  ...
  case "$XO_RECOVERY_MARKER_TOKEN" in
    announced:*)
      if ! _xo_recovery_marker_write_locked "$marker" downtime ""; then
        xo_lock_release "$lock"
        return 1
      fi
      ;;
...
xo_recovery_transition() {
  local marker=$1 action=$2 target=${3:-} value=${4:-}
  case "$action" in
    publish)
      _xo_recovery_marker_publish "$marker" "${target:-downtime}"
      ;;
    acknowledge)
      _xo_recovery_marker_ack "$marker" "$target"
      ;;
    arm-check)
      _xo_recovery_marker_arm_check "$marker"
      ;;
    reopen-announced)
      _xo_recovery_marker_reopen_announced "$marker"
      ;;
    release-lock)
      [ -n "$target" ] || return 1
      _xo_recovery_marker_publish "$marker" "${value:-downtime}" || return 1
      xo_lock_release "$target"
      ;;
    ...
    clear-stale-lock)
      [ -n "$target" ] || return 1
      _xo_recovery_marker_publish "$marker" "${value:-downtime}" || return 1
      xo_lock_remove_path "$target"
      ;;
    *) return 2 ;;
  esac
}

xo_recovery_marker_publish() {
  xo_recovery_transition "$1" publish "${2:-downtime}"
}

xo_recovery_marker_ack() {
  xo_recovery_transition "$1" acknowledge "$2"
}

xo_recovery_marker_begin_handling() {
  _xo_recovery_marker_begin_handling "$1" "${2:-}"
}

xo_recovery_marker_arm_check() {
  xo_recovery_transition "$1" arm-check
}

xo_recovery_marker_reopen_announced() {
  xo_recovery_transition "$1" reopen-announced
}
...
xo_lock_try_acquire() {
  ...
  XO_LOCK_HELD_PID=
  XO_LOCK_OWNER_DIR=
  XO_LOCK_RECOVERED_PID=

  if xo_lock_try_create "$lockdir"; then
    return 0
  fi
  ...
  if [ -n "$pid" ] && [ "$pid" = "$current" ]; then
    ...
    # tests/xo-wake-queue.test.sh - so reclaim the abandoned hold instead.
    xo_lock_remove_path "$lockdir" || true
    if xo_lock_try_create "$lockdir"; then
      return 0
    fi
    XO_LOCK_HELD_PID=$(cat "$lockdir/pid" 2>/dev/null || true)
    return 1
  fi
  if xo_pid_alive "$pid"; then
    XO_LOCK_HELD_PID=$pid
    return 1
  fi
  if xo_lock_mid_acquire_is_fresh "$lockdir" "$pid"; then
    XO_LOCK_HELD_PID=$pid
    return 1
  fi

  steal="$lockdir.steal"
  if ! xo_lock_try_acquire "$steal"; then
    XO_LOCK_HELD_PID=$(cat "$lockdir/pid" 2>/dev/null || true)
    XO_LOCK_OWNER_DIR=
    return 1
  fi
  steal_owner=${XO_LOCK_OWNER_DIR:-}

  cur=$(cat "$lockdir/pid" 2>/dev/null || true)
  if xo_pid_alive "$cur"; then
    xo_lock_release "$steal"
    XO_LOCK_HELD_PID=$cur
    XO_LOCK_OWNER_DIR=
    return 1
  fi
  if xo_lock_mid_acquire_is_fresh "$lockdir" "$cur"; then
    xo_lock_release "$steal"
    XO_LOCK_HELD_PID=$cur
    XO_LOCK_OWNER_DIR=
    return 1
  fi
  if ! xo_lock_points_to_owner "$steal" "$steal_owner"; then
    xo_lock_release "$steal"
    XO_LOCK_HELD_PID=$(cat "$lockdir/pid" 2>/dev/null || true)
    XO_LOCK_OWNER_DIR=
    return 1
  ...
  primary_owner=
  if [ -L "$lockdir" ]; then
    primary_owner=$(xo_lock_link_owner "$lockdir" 2>/dev/null || true)
  fi
  cur=$(cat "$lockdir/pid" 2>/dev/null || true)
  if ! xo_lock_recheck_stale_owner "$lockdir" "$primary_owner" "$cur"; then
    xo_lock_release "$steal"
    XO_LOCK_HELD_PID=$(cat "$lockdir/pid" 2>/dev/null || true)
    XO_LOCK_OWNER_DIR=
    return 1
  ...
  if [ "$lockdir" = "$STATE/.watch.lock" ] \
    && ! _xo_recovery_marker_publish "$STATE/.watcher-down" downtime; then
    xo_lock_release "$steal"
    XO_LOCK_HELD_PID=$cur
    XO_LOCK_OWNER_DIR=
    return 1
  ...
  xo_lock_remove_path "$lockdir" || true
  rc=1
  if xo_lock_try_create "$lockdir" "$steal_owner"; then
    rc=0
    # shellcheck disable=SC2034 # Read by sourcing callers after lock acquisition.
    XO_LOCK_RECOVERED_PID=$cur
  fi
  if [ "$rc" -ne 0 ]; then
    # shellcheck disable=SC2034 # Read by callers after xo_lock_try_acquire returns.
    XO_LOCK_HELD_PID=$(cat "$lockdir/pid" 2>/dev/null || true)
    XO_LOCK_OWNER_DIR=
  fi
  xo_lock_release "$steal"
...
xo_lock_acquire_wait() {
  local lockdir=$1
  while ! xo_lock_try_acquire "$lockdir"; do
    sleep 0.1
  done
}
...
_xo_lock_acquire_wait_handoff() {  # <lockdir> <caller-pid>
  local lockdir=$1 caller_pid=$2 ownerdir current back
  case "$caller_pid" in ''|*[!0-9]*) return 1 ;; esac
  xo_pid_alive "$caller_pid" || return 1
  trap 'xo_lock_release "$lockdir"; exit 143' TERM INT
  xo_lock_acquire_wait "$lockdir" || return 1
  if [ -L "$lockdir" ]; then
    ownerdir=$(xo_lock_link_owner "$lockdir" 2>/dev/null) || {
      xo_lock_release "$lockdir"
      return 1
    }
  else
    ownerdir=$lockdir
  fi
  xo_current_pid current || { xo_lock_release "$lockdir"; return 1; }
  back=$(cat "$ownerdir/pid" 2>/dev/null || true)
  if [ "$back" != "$current" ] \
...
# xo_lock_acquire_wait_bounded <lockdir> <positive-seconds>
#
# Bounded acquire variant. It preserves the ordinary wait/reclaim behavior
# until xo-timeout-lib.sh's hard deadline, returns 124 when a live holder still
# owns the lock, and leaves XO_LOCK_HELD_PID naming that holder.
...
# Mutation-critical callers that can safely block keep xo_lock_acquire_wait.
xo_lock_acquire_wait_bounded() {
  local lockdir=$1 seconds=$2 caller_pid rc owner_pid
  case "$seconds" in ''|*[!0-9]*|0) return 2 ;; esac
  _xo_wake_require_timeout || return 1
  if xo_lock_try_acquire "$lockdir"; then
    return 0
  fi

  xo_current_pid caller_pid || return 1
  # shellcheck disable=SC2016 # Positional parameters expand in the child shell.
  if xo_run_timed "$seconds" env \
    "XO_STATE_OVERRIDE=$STATE" \
    "XO_ROOT_OVERRIDE=$XO_ROOT" \
    "XO_LOCK_STALE_AFTER=$XO_LOCK_STALE_AFTER" \
    bash -c '. "$1"; _xo_lock_acquire_wait_handoff "$2" "$3"' \
      _ "$XO_WAKE_LIB_DIR/xo-wake-lib.sh" "$lockdir" "$caller_pid" \
  ...
  if [ "$rc" -eq 124 ]; then
    ...
    case "$owner_pid" in
      ''|*[!0-9]*|0) ;;
      *)
        if [ "$owner_pid" -gt 0 ] 2>/dev/null && xo_pid_alive "$owner_pid"; then
          XO_LOCK_HELD_PID=$owner_pid
          return 124
        fi
        ;;
    esac
...
xo_lock_release() {
  ...
  if [ -L "$lockdir" ]; then
    ...
    xo_lock_points_to_owner "$lockdir" "$ownerdir" || return 0
...
# xo_task_set_lock_path: the per-home lock guarding WHICH tasks exist in a home,
# as opposed to xo_meta_lock_path, which guards one task's record.
#
...
# fail closed.
xo_task_set_lock_path() {  # <state-dir>
  local state=$1
  [ -n "$state" ] || return 1
  case "$state" in *[$'\n\r\t']*) return 1 ;; esac
...
# The top-most xo home reachable from this one on THIS machine, used as
...
# top of the local tree that bin/xo-teardown.sh's collect_local_xo_states
...
xo_xo_root_home() {
  local home=${1:-$XO_HOME} marker parent seen="|" depth=0
  home=$(CDPATH='' cd -- "$home" 2>/dev/null && pwd -P) || return 1
  while [ -e "$home/.xo-secondmate-parent" ] || [ -L "$home/.xo-secondmate-parent" ]; do
    marker="$home/.xo-secondmate-parent"
    if ! command -v xo_secondmate_parent_record_parse >/dev/null 2>&1; then
      # shellcheck source=bin/xo-secondmate-parent-lib.sh
      . "$XO_WAKE_LIB_DIR/xo-secondmate-parent-lib.sh"
    fi
    xo_secondmate_parent_record_parse "$marker" || return 1
    case "$XO_SECONDMATE_PARENT_ROUTE" in
      local) ;;
      remote) break ;;
      *) return 1 ;;
    esac
    parent=$(CDPATH='' cd -- "$XO_SECONDMATE_PARENT_HOME" 2>/dev/null && pwd -P) || return 1
...
xo_treehouse_project_lock_path() {  # <project-dir>
  local project=$1 root origin identity hash top
  [ -d "$project" ] || return 1
  root=$(xo_xo_root_home "$XO_HOME") || return 1
...
xo_treehouse_pool_slot() {  # <project-dir> <worktree>
...
# releases it, and XO uses exactly that for secondmate homes
# (bin/xo-home-seed.sh). Crewmate spawns do not take that path: they acquire
...
# this task's or has since been handed to another one. XO therefore keeps
# its own claim on top: one file naming the task that took the slot, written by
# bin/xo-spawn.sh under the same project lock that allocates the slot and
# released by bin/xo-teardown.sh when the slot goes back to the pool. Moving
# crewmate spawns onto the durable lease is separate follow-up work.
#
# The claim lives at <pool>/<slot>/.xo-slot-owner - a sibling of the repo
...
xo_treehouse_slot_owner_marker() {  # <worktree>
  local worktree=$1 slot
  slot=$(CDPATH='' cd -- "$worktree" 2>/dev/null && pwd -P) || return 1
  printf '%s/.xo-slot-owner\n' "$(dirname "$slot")"
}
...
xo_treehouse_slot_owner_claim() {  # <worktree> <task-id> <home>
  local worktree=$1 id=$2 home=$3 marker tmp
  [ -n "$id" ] || return 1
  marker=$(xo_treehouse_slot_owner_marker "$worktree") || return 1
...
# Sets XO_TREEHOUSE_SLOT_OWNER to one of:
...
# XO_TREEHOUSE_SLOT_OWNER_ID and XO_TREEHOUSE_SLOT_OWNER_HOME carry the recorded
# claimant as evidence. The home is reported, never matched: a home that moved
# must not turn a task's own slot into a refusal.
xo_treehouse_slot_owner_state() {  # <worktree> <task-id>
  local worktree=$1 id=$2 marker line owner_id='' owner_home=''
  XO_TREEHOUSE_SLOT_OWNER=unsafe
  XO_TREEHOUSE_SLOT_OWNER_ID=
  XO_TREEHOUSE_SLOT_OWNER_HOME=
  marker=$(xo_treehouse_slot_owner_marker "$worktree") || return 0
  if [ ! -e "$marker" ] && [ ! -L "$marker" ]; then
    XO_TREEHOUSE_SLOT_OWNER=absent
    return 0
  fi
  ...
  XO_TREEHOUSE_SLOT_OWNER_ID=$owner_id
  # shellcheck disable=SC2034 # Output globals, read by the sourcing caller.
  XO_TREEHOUSE_SLOT_OWNER_HOME=$owner_home
  if [ "$owner_id" = "$id" ]; then
    XO_TREEHOUSE_SLOT_OWNER=mine
  else
    XO_TREEHOUSE_SLOT_OWNER=other
  fi
}
...
xo_treehouse_slot_owner_release() {  # <worktree> <task-id>
  local worktree=$1 id=$2 marker
  xo_treehouse_slot_owner_state "$worktree" "$id"
  [ "$XO_TREEHOUSE_SLOT_OWNER" = mine ] || return 0
  marker=$(xo_treehouse_slot_owner_marker "$worktree") || return 0
  rm -f "$marker" 2>/dev/null || true
}

xo_failure_episode_reset() {
  local state=$1 mode=${2:-acquire} lock current pid acquired=0 path
  lock="$state/.turnend-claude-blocks.lock"
  case "$mode" in
    acquire)
      xo_lock_try_acquire "$lock" || return 1
      acquired=1
      ;;
    held)
      xo_current_pid current || return 1
...
# Both Stop-event participants (bin/xo-claude-stop-autoarm.sh and
# bin/xo-turnend-guard.sh --claude) coordinate through the epoch ledger
...
#     supervision lock in this repo records (xo_pid_identity above). The
...
#   - A claim is OPEN (xo_autoarm_claim_open) while its outcome is "arming",
...
#     generation N+1 (xo_autoarm_claim_next). Taking a newer generation IS the
...
#     xo_lock_try_acquire's ordinary dead-owner steal.
...
# goes silent.
#
# xo_autoarm_claim_abandoned / xo_autoarm_release_abandoned below survive as
...
_xo_autoarm_epoch_field() {  # <epoch-file> <field>
...
# Parse the current ledger claim. Sets XO_AUTOARM_GEN, XO_AUTOARM_OWNER,
# XO_AUTOARM_OUTCOME, XO_AUTOARM_SESSION, XO_AUTOARM_RECOVERY, and
# XO_AUTOARM_IDENTITY (line 2 of the entry, and ONLY
...
xo_autoarm_ledger_read() {  # <state-dir>
  local state=$1 epoch
  ...
  XO_AUTOARM_RECOVERY=
  XO_AUTOARM_IDENTITY=
  XO_AUTOARM_GEN=$(_xo_autoarm_epoch_field "$epoch" epoch) || return 1
  XO_AUTOARM_OWNER=$(_xo_autoarm_epoch_field "$epoch" owner_pid) || return 1
  XO_AUTOARM_OUTCOME=$(_xo_autoarm_epoch_field "$epoch" outcome) || return 1
  XO_AUTOARM_SESSION=$(_xo_autoarm_epoch_field "$epoch" session_pid 2>/dev/null || true)
  XO_AUTOARM_RECOVERY=$(_xo_autoarm_epoch_field "$epoch" recovery_generation 2>/dev/null || true)
  case "$XO_AUTOARM_GEN" in
    ''|*[!0-9]*) return 1 ;;
  esac
  XO_AUTOARM_IDENTITY=$(sed -n '2p' "$epoch" 2>/dev/null || true)
  return 0
}
...
# (the contract comment above owns the stuck proof). xo_path_age reports an
...
xo_autoarm_claim_open() {  # <state-dir> [grace]
  local state=$1 grace=${2:-${XO_GUARD_GRACE:-300}} epoch current
  epoch="$state/.claude-autoarm-epoch"
  case "$grace" in
    ''|*[!0-9]*|0) grace=300 ;;
  esac
  xo_autoarm_ledger_read "$state" || return 1
  [ "$XO_AUTOARM_OUTCOME" = arming ] || return 1
  xo_pid_alive "$XO_AUTOARM_OWNER" || return 1
  [ -n "$XO_AUTOARM_IDENTITY" ] || return 1
  current=$(xo_pid_identity "$XO_AUTOARM_OWNER" 2>/dev/null) || return 1
  [ -n "$current" ] || return 1
  [ "$current" = "$XO_AUTOARM_IDENTITY" ] || return 1
  if [ "$(xo_path_age "$epoch")" -ge "$grace" ] \
    && [ "$(xo_path_age "$state/.last-watcher-beat")" -ge "$grace" ]; then
    return 1
  fi
  return 0
}
...
# exists to stop.
xo_autoarm_midturn_healthy() {  # <state-dir> [grace]
  ...
  xo_autoarm_ledger_read "$state" || return 1
  [ "$XO_AUTOARM_OUTCOME" = rewake ] || return 1
  lock_pid=$(sed -n '1p' "$state/.lock" 2>/dev/null || true)
  [ -n "$XO_AUTOARM_SESSION" ] && [ "$XO_AUTOARM_SESSION" = "$lock_pid" ] || return 1
  xo_pid_alive "$lock_pid" || return 1
  xo_recovery_marker_read "$state/.watcher-down" || return 1
  recovery=${XO_RECOVERY_MARKER_TOKEN##*:}
  [ -n "$XO_AUTOARM_RECOVERY" ] && [ "$XO_AUTOARM_RECOVERY" = "$recovery" ] || return 1
  epoch_mtime=$(xo_path_mtime "$state/.claude-autoarm-epoch") || return 1
  beacon_mtime=$(xo_path_mtime "$state/.last-watcher-beat") || return 1
  [ "$epoch_mtime" -ge "$beacon_mtime" ]
}

# Atomically publish this process as the owner of generation N+1, under one
# short micro-mutex hold. Returns 0 with XO_AUTOARM_MY_GEN set on success, 2
...
xo_autoarm_claim_next() {  # <state-dir> [grace]
  local state=$1 grace=${2:-${XO_GUARD_GRACE:-300}} lock epoch pid gen identity tmp
  ...
  pid=${BASHPID:-$$}
  identity=$(xo_pid_identity "$pid" 2>/dev/null) || return 1
  [ -n "$identity" ] || return 1
  xo_lock_try_acquire "$lock" || return 1
  ...
  gen=$(_xo_autoarm_epoch_field "$epoch" epoch 2>/dev/null || true)
  case "$gen" in
    ''|*[!0-9]*) gen=0 ;;
  esac
  gen=$((gen + 1))
...
xo_autoarm_write_owned() {  # <state-dir> <gen> <outcome> [marker-file] [session-pid] [recovery-generation]
  local state=$1 gen=$2 outcome=$3 marker=${4:-} session=${5:-} recovery=${6:-} lock epoch pid identity tmp i
  lock="$state/.claude-autoarm.lock"
  epoch="$state/.claude-autoarm-epoch"
  pid=${BASHPID:-$$}
  i=0
  while ! xo_lock_try_acquire "$lock"; do
    [ "$i" -lt 20 ] || return 1
    sleep 0.02
    i=$((i + 1))
  done
  if ! xo_autoarm_ledger_read "$state" \
    || [ "$XO_AUTOARM_GEN" != "$gen" ] || [ "$XO_AUTOARM_OWNER" != "$pid" ]; then
    xo_lock_release "$lock"
    return 2
  fi
  identity=$XO_AUTOARM_IDENTITY
  tmp="$epoch.tmp.$pid"
  if ! {
...
xo_autoarm_still_owner() {  # <state-dir> <gen>
  local state=$1 gen=$2 pid
  pid=${BASHPID:-$$}
  xo_autoarm_ledger_read "$state" || return 1
  [ "$XO_AUTOARM_GEN" = "$gen" ] && [ "$XO_AUTOARM_OWNER" = "$pid" ]
}

xo_autoarm_reset_owned() {  # <state-dir> <gen>
  local state=$1 gen=$2 lock pid
  lock="$state/.claude-autoarm.lock"
  pid=${BASHPID:-$$}
  xo_lock_try_acquire "$lock" || return 2
  if ! xo_autoarm_ledger_read "$state" \
    || [ "$XO_AUTOARM_GEN" != "$gen" ] || [ "$XO_AUTOARM_OWNER" != "$pid" ]; then
    xo_lock_release "$lock"
    return 2
  fi
  if ! xo_failure_episode_reset "$state"; then
    xo_lock_release "$lock"
    return 1
  fi
  xo_lock_release "$lock"
...
#      proof as xo_autoarm_claim_open).
xo_autoarm_claim_abandoned() {  # <state-dir> [grace]
  local state=$1 grace=${2:-${XO_GUARD_GRACE:-300}} epoch lock role pid owner outcome recorded current
  lock="$state/.claude-autoarm.lock"
  epoch="$state/.claude-autoarm-epoch"
  case "$grace" in
    ''|*[!0-9]*|0) grace=300 ;;
  ...
  if [ -n "$recorded" ] && current=$(xo_pid_identity "$pid" 2>/dev/null) \
    && [ -n "$current" ] && [ "$current" != "$recorded" ]; then
    return 0
  fi
  owner=$(_xo_autoarm_epoch_field "$epoch" owner_pid) || return 1
  [ "$owner" = "$pid" ] || return 1
  outcome=$(_xo_autoarm_epoch_field "$epoch" outcome) || return 1
  case "$outcome" in
    '') return 1 ;;
    arming)
      [ "$(xo_path_age "$epoch")" -ge "$grace" ] || return 1
      [ "$(xo_path_age "$state/.last-watcher-beat")" -ge "$grace" ] || return 1
      return 0
      ;;
  esac
  return 0
...
# serialization xo_lock_try_acquire uses for stale-owner reclaim: while it is
...
xo_autoarm_release_abandoned() {  # <state-dir> [grace]
  local state=$1 grace=${2:-${XO_GUARD_GRACE:-300}} lock steal epoch lock_pid recorded current owner line1 tmp i
  lock="$state/.claude-autoarm.lock"
  steal="$lock.steal"
  epoch="$state/.claude-autoarm-epoch"
  xo_autoarm_claim_abandoned "$state" "$grace" || return 1
  xo_lock_try_acquire "$steal" || return 1
  if ! xo_autoarm_claim_abandoned "$state" "$grace"; then
    xo_lock_release "$steal"
    return 1
  fi
  lock_pid=$(cat "$lock/pid" 2>/dev/null || true)
  recorded=$(cat "$lock/pid-identity" 2>/dev/null || true)
  if [ -n "$recorded" ] && xo_pid_alive "$lock_pid" \
    && current=$(xo_pid_identity "$lock_pid" 2>/dev/null) \
    && [ -n "$current" ] && [ "$current" = "$recorded" ]; then
    ...
    i=0
    while [ "$i" -lt 20 ] && xo_pid_alive "$lock_pid"; do
      sleep 0.05
      i=$((i + 1))
    done
  fi
  ...
  if [ -n "$recorded" ] && [ -n "$lock_pid" ] \
    && owner=$(_xo_autoarm_epoch_field "$epoch" owner_pid 2>/dev/null) \
    && [ "$owner" = "$lock_pid" ] \
...
xo_wake_append() {
  ...
  xo_wake_append_locked "$@" || status=$?
  xo_lock_release "$XO_WAKE_QUEUE_LOCK"
  return "$status"
}

# xo_wake_append_locked <kind> <key> <payload>
# Locked core of xo_wake_append: appends the wake row under an already-held
# XO_WAKE_QUEUE_LOCK. Callers that must commit another durable record atomically
...
xo_wake_append_locked() {
  local kind=$1 key=$2 payload=$3 clean_key clean_payload epoch seq seq_file status
  local recovery_marker
  case "$kind" in
    signal|stale|check|heartbeat) ;;
    *) printf 'xo_wake_append: invalid wake kind: %s\n' "$kind" >&2; return 2 ;;
  esac

  clean_key=$(printf '%s' "$key" | xo_wake_clean_field)
  clean_payload=$(printf '%s' "$payload" | xo_wake_clean_field)
  epoch=$(date +%s)
  seq_file="$STATE/.wake-queue.seq"
  recovery_marker="$STATE/.watcher-down"
  status=0

  _xo_recovery_marker_publish "$recovery_marker" downtime || status=$?
  if [ "$status" -eq 0 ]; then
  ...
  if [ "$status" -eq 0 ]; then
    printf '%s\t%s\t%s\t%s\t%s\n' "$epoch" "$seq" "$kind" "$clean_key" "$clean_payload" >> "$XO_WAKE_QUEUE" || status=$?
  fi
  return "$status"
}

# xo_wake_queued_keys <kind>
...
xo_wake_queued_keys() {
  ...
  case "$kind" in
    signal|stale|check|heartbeat) ;;
    *) printf 'xo_wake_queued_keys: invalid wake kind: %s\n' "$kind" >&2; return 2 ;;
  esac
  xo_lock_acquire_wait "$XO_WAKE_QUEUE_LOCK"
  xo_wake_queued_keys_locked "$kind"
  xo_lock_release "$XO_WAKE_QUEUE_LOCK"
}

xo_wake_queued_keys_locked() {
  local kind=$1
  awk -F '\t' -v kind="$kind" 'NF >= 5 && $3 == kind && !seen[$4]++ { print $4 }' \
    "$XO_WAKE_QUEUE" 2>/dev/null || true
}

xo_wake_secondmate_progress_marker_write() { # <task> <observed-at> <oldest-row-key>
...
xo_wake_secondmate_stall_marker_write() { # <task> <row-key>
...
xo_wake_secondmate_stall_receipt_write() { # <task> <row-key>
  ...
  if ! printf '%s\n' "$row_key" > "$tmp" || ! chmod 0600 "$tmp" \
    || ! _xo_atomic_replace "$tmp" "$receipt"; then
    rm -f -- "$tmp"
    return 1
  fi
}

xo_wake_commit_secondmate_stall_receipts_through() { # <cutoff> [<rows-file>]
  ...
  while IFS= read -r key; do
    ...
    case "$task" in ''|*[!A-Za-z0-9._-]*) return 1 ;; esac
    row_key="$epoch-$seq"
    xo_wake_secondmate_stall_receipt_write "$task" "$row_key" || return 1
  ...
  ' "$XO_WAKE_QUEUE" 2>/dev/null)
}

xo_wake_restore_queue() {
  local drained=$1 restore
  restore="$STATE/.wake-queue.restore.$(xo_current_pid)"
  if [ -e "$XO_WAKE_QUEUE" ]; then
    cat "$drained" "$XO_WAKE_QUEUE" > "$restore" && mv "$restore" "$XO_WAKE_QUEUE"
  else
    mv "$drained" "$XO_WAKE_QUEUE"
  fi
}

xo_wake_print_deduped() {
  local file=$1
  awk -F '\t' '
    NF >= 5 {
      dedupe = $3 SUBSEP $4
...
xo_wake_grant_rows_valid() {  # <rows-file>
...
xo_wake_branch_owner_matches() {  # <owner-file> [<pid>] [<generation>]
  ...
  exec 8<&-
  [ "$version" = xo-branch-eligible-owner-v1 ] || return 1
...
xo_wake_branch_grant_live() {  # <rows-file> <owner-file>
  xo_wake_grant_rows_valid "$1" && xo_wake_branch_owner_matches "$2"
}
...
xo_wake_actor_pending_count() {  # <actor> [<rows-file> <owner-file>]
  local actor=${1:-main} rows=${2:-$STATE/.branch-eligible-rows}
  local owner=${3:-$STATE/.branch-eligible-owner} grant='' count=''
  [ -f "$XO_WAKE_QUEUE" ] || { printf '0\n'; return 0; }
  if xo_wake_branch_grant_live "$rows" "$owner"; then
    grant=$rows
  fi
...
#
# The watcher's per-file signal scan (bin/xo-watch.sh scan_signals) detects a
# status or turn-ended change by comparing a file signature against a persisted
# state/.seen-* marker.
# xo-classify-lib.sh's header owns the status marker contract, including its
...
xo_wake_signal_sig() {  # <file> -> reported-state signature
  case "$1" in
    *.status)
      _xo_wake_require_classify || return 1
      status_observed_signature "$1"
      ;;
    *)
      if [ "$_XO_UNAME" = Darwin ]; then /usr/bin/stat -f '%z:%Fm' "$1" 2>/dev/null; else stat -c '%s:%Y' "$1" 2>/dev/null; fi
      ;;
  esac
}

xo_wake_signal_seen_path() {  # <state> <file>
  local task
  case "$2" in
    *.status)
...
xo_wake_signal_seen_size() {  # <state> <file>
  local marker sig size
  marker=$(xo_wake_signal_seen_path "$1" "$2")
  case "$2" in
    *.status)
      _xo_wake_require_classify || { printf '0'; return 0; }
...
xo_wake_signal_seen_current() {  # <state> <file>
  local sig marker
  sig=$(xo_wake_signal_sig "$2") || return 1
  [ -n "$sig" ] || return 1
  marker=$(xo_wake_signal_seen_path "$1" "$2")
  case "$2" in
    *.status)
...
xo_wake_status_reported_commit() {  # <state> <status-file> <reported-signature>
  _xo_wake_require_classify || return 1
  status_presentation_marker_report "$(xo_wake_signal_seen_path "$1" "$2")" "$3"
}

xo_wake_status_seen_commit() {  # <state> <status-file> <captured-end> <captured-identity>
  _xo_wake_require_classify || return 1
  status_presentation_marker_commit "$(xo_wake_signal_seen_path "$1" "$2")" "$2" "$3" "$4"
}
...
xo_wake_status_mark_current() {  # <state> <status-file>
  local size ident
  _xo_wake_require_classify || return 1
  size=$(_xo_status_file_size "$2") || return 1
  ident=$(_xo_open_decisions_file_ident "$2") || return 1
  xo_wake_status_seen_commit "$1" "$2" "$size" "$ident"
}
...
xo_wake_status_append_self_announced() {  # <state> <status-file> <line>
  local state=$1 file=$2 line=$3 marker pre_sig='' pre_size='' pre_ident='' post_size post_ident
  local LC_ALL=C
  _xo_wake_require_classify || return 1
  marker=$(xo_wake_signal_seen_path "$state" "$file")
  if [ -e "$file" ]; then
    pre_sig=$(xo_wake_signal_sig "$file") || pre_sig=''
    pre_size=$(_xo_status_file_size "$file") || pre_size=''
    pre_ident=$(_xo_open_decisions_file_ident "$file") || pre_ident=''
  fi
  printf '%s\n' "$line" >> "$file" || return 2
  [ -n "$pre_sig" ] || return 1
  ...
  post_size=$(_xo_status_file_size "$file") || return 1
  post_ident=$(_xo_open_decisions_file_ident "$file") || return 1
  ...
  xo_wake_status_seen_commit "$state" "$file" "$post_size" "$post_ident" || return 1
  return 0
}
...
XO_WAKE_STATUS_KEY=
XO_WAKE_STATUS_HISTORICAL=false
xo_wake_status_key_map() {  # <queue-key>
  local key=$1 id
  XO_WAKE_STATUS_KEY=
  XO_WAKE_STATUS_HISTORICAL=false
  case "$key" in
    ...
    *.turn-ended)
      id=${key%.turn-ended}
      XO_WAKE_STATUS_HISTORICAL=true
      ;;
    *)
      return 1
      ;;
  ...
  [ "${#id}" -le 64 ] || return 1
  XO_WAKE_STATUS_KEY="$id.status"
}

xo_wake_annotation_manifest() {  # <deduped-raw-rows>
  local rows=$1 epoch seq kind key payload
  while IFS=$(printf '\t') read -r epoch seq kind key payload; do
    [ "$kind" = signal ] || continue
    xo_wake_status_key_map "$key" || continue
    if [ "$XO_WAKE_STATUS_HISTORICAL" = true ]; then
      printf '%s\thistorical\n' "$XO_WAKE_STATUS_KEY"
    else
      printf '%s\tdirect\n' "$XO_WAKE_STATUS_KEY"
    fi
...
XO_WAKE_EVENT_LINE=
XO_WAKE_UNREAD_LINES=
xo_wake_status_cursor_offset() {  # <validated-status-path> -> already-presented byte offset
  local path=$1 offset
...
xo_wake_unread_events() {  # <validated-status-path> <unused-tail-byte-cap> <min-offset> [<end-offset>]
  ...
  XO_WAKE_UNREAD_LINES=$(printf '%s' "$chunk" | LC_ALL=C awk -v start="$chunk_start" -v min="$min_offset" '
    BEGIN { pos = start + 0 }
    {
      line_start = pos
      pos += length($0) + 1
  ...
  ') || return 1
  [ -n "$XO_WAKE_UNREAD_LINES" ] || return 1
  XO_WAKE_EVENT_LINE=$(printf '%s\n' "$XO_WAKE_UNREAD_LINES" | tail -1)
  XO_WAKE_EVENT_LINE=$(printf '%s' "$XO_WAKE_EVENT_LINE" | LC_ALL=C tr '\t\r' '  ')
}

xo_wake_latest_event() {  # <validated-status-path> <tail-byte-cap>
  xo_wake_unread_events "$1" "$2" 0
}
...
xo_wake_print_annotations() {  # <deduped-raw-rows> [<presentation-snapshot>]
  ...
  manifest=$(xo_wake_annotation_manifest "$rows" | awk -F '\t' '
    {
      key = $1
      if (!(key in seen)) {
        order[++count] = key
  ...
  case "${XO_WAKE_ENRICH_TEST_DELAY:-0}" in
    0) ;;
    ''|*[!0-9]*) ;;
    *) sleep "$XO_WAKE_ENRICH_TEST_DELAY" ;;
  esac
  ...
  while IFS=$(printf '\t') read -r status_key mode; do
    ...
    # surfaced to xo or deliberately absorbed by the signal triage.
    ...
    if [ "$mode" = historical ] && xo_wake_signal_seen_current "$STATE" "$path"; then
      continue
    fi
    offset=$(xo_wake_status_cursor_offset "$path") || return 1
    endpoint=
    if [ -n "$snapshot" ]; then
...
EOF
    ...
    if ! xo_wake_unread_events "$path" 0 "$offset" "$endpoint"; then
