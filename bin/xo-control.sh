#!/usr/bin/env bash
# xo-control.sh - the CONTROL PLANE for an xo-owned agent: allowlisted
# lifecycle verbs addressed to an exact task id.
#
# Usage: xo-control.sh <task-id> interrupt
#        xo-control.sh <task-id> exit
#        xo-control.sh <task-id> relaunch [--harness <name>] [--model <name>]
#                                         [--effort <level>]
#                                         (--note <text> | --note-file <path>)
#
# Why this exists, and how it differs from xo-send.sh. bin/xo-send.sh is the
...
# (bin/xo-control-lib.sh) rather than improvised in agent prose, and a verified
# postcondition for every action. There is deliberately NO arbitrary-text and
# NO generic raw-key entry point here; xo-send remains the only way to send an
...
#              bin/xo-spawn.sh --relaunch. A failure before publication keeps
...
# endpoint, or discarding work stays with bin/xo-teardown.sh, which owns the
# landed-work test.
#
# `resume` is not a verb: it is not deterministic across the verified adapters
# (bin/xo-control-lib.sh's header owns that reasoning). `relaunch` covers the
...
# validation (bin/xo-backend.sh's xo_backend_validate_task_endpoint). A legacy
# xo-<id> label, an explicit session:window endpoint, and a bare window name
...
#   XO_CONTROL_POLL              poll interval for postcondition waits (0.5)
#   XO_CONTROL_SETTLE_WAIT       adapter acknowledgement wait after interrupt (5)
#   XO_CONTROL_EXIT_WAIT         alive->dead wait after the exit command (30)
#   XO_CONTROL_LAUNCH_WAIT       dead->alive wait after a relaunch (90)
#   XO_CONTROL_EXIT_RETRIES      Enter retries for the exit command (3)
set -eu
...
# drive a crewmate's lifecycle (see bin/xo-gate-refuse-lib.sh).
xo_refuse_if_gate_agent

if [ -z "${XO_HOME+x}" ] || [ -z "${XO_HOME:-}" ]; then
  echo "error: XO_HOME is not set; xo-control refuses to resolve a task without an explicit xo home" >&2
  exit 1
fi
[ -d "$XO_HOME" ] || {
  echo "error: XO_HOME '$XO_HOME' is not a directory" >&2
  exit 1
}
STATE="${XO_STATE_OVERRIDE:-$XO_HOME/state}"
DATA="${XO_DATA_OVERRIDE:-$XO_HOME/data}"
[ -d "$STATE" ] || {
  echo "error: state dir '$STATE' is missing; xo-control cannot resolve tasks for XO_HOME '$XO_HOME'" >&2
  exit 1
}
...
POLL=${XO_CONTROL_POLL:-0.5}
SETTLE_WAIT=${XO_CONTROL_SETTLE_WAIT:-5}
EXIT_WAIT=${XO_CONTROL_EXIT_WAIT:-30}
LAUNCH_WAIT=${XO_CONTROL_LAUNCH_WAIT:-90}
EXIT_RETRIES=${XO_CONTROL_EXIT_RETRIES:-3}
...
control_cleanup() {
  ...
  if declare -F xo_lease_guard_release >/dev/null 2>&1; then
...
if ! xo_control_verb_allowed "$VERB"; then
  {
    if [ "$VERB" = resume ]; then
...
case "$RAW_ID" in
  *:*) die "'$RAW_ID' is an explicit backend endpoint; xo-control accepts an exact task id only, so a lifecycle command can never land on an endpoint this home does not own" ;;
esac
if ! xo_task_id_creation_valid "$RAW_ID"; then
...
# live lease (contract: bin/xo-lease-lib.sh; no-op in homes without leases).
# shellcheck source=bin/xo-lease-lib.sh
. "$SCRIPT_DIR/xo-lease-lib.sh"
xo_lease_guard "$ID" "lifecycle control (xo-control)"
...
if [ ! -f "$META" ]; then
  case "$RAW_ID" in
    xo-*)
      if [ -f "$STATE/${RAW_ID#xo-}.meta" ]; then
        die "'$RAW_ID' is a window label, not a task id; pass the exact task id '${RAW_ID#xo-}'"
      fi
      ;;
  esac
  die "no task '$ID' in $STATE (xo-control resolves an exact task id only)"
fi
...
# instead, using the same `remote_host` signal bin/xo-send.sh routes on.
if [ -n "$(xo_meta_get "$META" remote_host)" ]; then
  die "task $ID is a remotely placed secondmate on $(xo_meta_get "$META" remote_host); its agent runs outside this home, so no lifecycle action here could verify that it interrupted, stopped, or came back. Drive its lifecycle on that host, and reconcile it through the secondmate recovery path rather than this plane"
fi
...
LABEL="xo-$ID"
RECORDED_HARNESS=$(xo_meta_get "$META" harness)
KIND=$(xo_meta_get "$META" kind)
WT=$(xo_meta_get "$META" worktree)
[ -n "$KIND" ] || KIND=ship

HARNESS=$(xo_control_harness_family "$RECORDED_HARNESS") \
  || die "task $ID records harness '${RECORDED_HARNESS:-none}', which has no verified control mechanics; xo-control refuses to guess an interrupt key or exit command"
xo_control_harness_supported "$HARNESS" \
  || die "task $ID records harness '${RECORDED_HARNESS:-none}', which has no verified control mechanics; xo-control refuses to guess an interrupt key or exit command"

xo_backend_validate "$BACKEND" || exit 1

# --- shared helpers ---------------------------------------------------------

agent_state() {
  xo_backend_agent_state "$BACKEND" "$T"
}

busy_verdict() {
  xo_busy_classify_meta "$META" "$ID" "$STATE"
}
...
require_state_verified_backend() {  # <verb>
  xo_control_backend_state_verified "$BACKEND" && return 0
...
send_interrupt_keys() {
  ...
  key=$(xo_control_interrupt_key "$HARNESS")
  repeat=$(xo_control_interrupt_repeat "$HARNESS")
  clear=$(xo_control_interrupt_clear_key "$HARNESS")
  ...
  [ -z "$clear" ] || xo_control_backend_supports_key "$BACKEND" "$clear" \
  ...
  while [ "$i" -lt "$repeat" ]; do
    xo_backend_send_key "$BACKEND" "$T" "$key" "$LABEL" \
      || die "interrupt key $key was not delivered to task $ID on $BACKEND"
    i=$((i + 1))
    [ "$i" -ge "$repeat" ] || sleep 0.2
  done
  [ -z "$clear" ] || xo_backend_send_key "$BACKEND" "$T" "$clear" "$LABEL" \
...
prepare_interrupt_ack() {
  INTERRUPT_ACK_SOURCE=$(xo_control_interrupt_ack_source "$HARNESS")
  INTERRUPT_ACK_LOG=
  INTERRUPT_ACK_RUN=
  case "$INTERRUPT_ACK_SOURCE" in
    muse-session-terminal)
      INTERRUPT_ACK_LOG=$(xo_busy_muse_session_log "$STATE" "$ID" 2>/dev/null || true)
      [ -n "$INTERRUPT_ACK_LOG" ] || return 0
      INTERRUPT_ACK_RUN=$(xo_busy_muse_active_run_id "$INTERRUPT_ACK_LOG" 2>/dev/null || true)
      ;;
  esac
}
...
interrupt_cancel_claim() {
  ...
  while :; do
    terminal=$(xo_busy_muse_run_terminal "$INTERRUPT_ACK_LOG" "$INTERRUPT_ACK_RUN" 2>/dev/null || true)
    case "$terminal" in
...
verify_interrupt_running() {
  local proof after
  xo_backend_target_exists "$BACKEND" "$T" "$LABEL" \
...
retire_busy_incarnation() {
  if [ -f "$STATE/$ID.busy-gen" ]; then
    "$SCRIPT_DIR/xo-busy-event.sh" retire "$STATE" "$ID" --current-gen >/dev/null 2>&1 || true
  fi
}
...
do_exit() {
  ...
  cmd=$(xo_control_exit_command "$HARNESS")
  ...
  verdict=$(xo_backend_send_text_submit "$BACKEND" "$T" "$cmd" "$EXIT_RETRIES" "$POLL" 1.2 "$LABEL") \
    ...
    exited|launching)
      if [ "$RELAUNCH_AGENT_CONFIRMED" = 1 ]; then
        ...
        echo "error: $ID's replacement is running on $TARGET_HARNESS, but transaction completion could not be persisted; its published record was retained for reconciliation" >&2
         ...
         || { [ -n "$RELAUNCH_TX" ] \
              && [ "$(xo_meta_get "$META" control_relaunch_tx)" = "$RELAUNCH_TX" ]; }; then
...
resolve_relaunch_profile() {
  PRIOR_HARNESS=$HARNESS
  PRIOR_RECORDED_HARNESS=$RECORDED_HARNESS
  PRIOR_MODEL=$(xo_meta_get "$META" model)
  PRIOR_EFFORT=$(xo_meta_get "$META" effort)
  ...
  if [ "$KIND" = secondmate ]; then
    ...
    # from xo's own dispatch-profile judgment at intake, and silently
    # re-resolving it would bypass that consultation.
    CONFIG_HARNESS=$("$SCRIPT_DIR/xo-harness.sh" secondmate 2>/dev/null || true)
    CONFIG_MODEL=$("$SCRIPT_DIR/xo-harness.sh" secondmate-model 2>/dev/null || true)
    CONFIG_EFFORT=$("$SCRIPT_DIR/xo-harness.sh" secondmate-effort 2>/dev/null || true)
  ...
  if [ "$HARNESS_SET" = 1 ]; then
    xo_control_harness_supported "$NEW_HARNESS" \
      || die "'$NEW_HARNESS' is not a verified harness; xo-control refuses to relaunch onto an adapter with no verified control or launch mechanics"
    TARGET_HARNESS=$NEW_HARNESS
  elif [ "$HARNESS_SET" = 0 ] && [ -n "$CONFIG_HARNESS" ]; then
    xo_control_harness_supported "$CONFIG_HARNESS" \
      || die "the configured secondmate harness '$CONFIG_HARNESS' is not verified; xo-control refuses to relaunch onto an adapter with no verified control or launch mechanics"
  ...
  xo_control_harness_supports_kind "$TARGET_HARNESS" "$KIND" \
  ...
  if [ "$TARGET_EFFORT" = ultra ]; then
    "$SCRIPT_DIR/xo-harness.sh" validate-native-effort "$TARGET_HARNESS" "$TARGET_MODEL" "$TARGET_EFFORT" || return 1
  fi
}
...
safe_checkpoint() {
  ...
  if [ "$KIND" = secondmate ]; then
    ...
    marker=$(cat "$WT/.xo-secondmate-home" 2>/dev/null || true)
    [ "$marker" = "$ID" ] \
...
do_relaunch() {
  ...
  # The launch owner (xo-spawn --relaunch) clears the previous incarnation's
  ...
  if XO_CONTROL_RELAUNCH_TX="$RELAUNCH_TX" \
      "$SCRIPT_DIR/xo-spawn.sh" "${spawn_args[@]}" >/dev/null; then
    RELAUNCH_META_PUBLISHED=1
  else
    [ "$(xo_meta_get "$META" control_relaunch_tx)" != "$RELAUNCH_TX" ] \
      || RELAUNCH_META_PUBLISHED=1
