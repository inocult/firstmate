#!/usr/bin/env bash
# Link a spawned task to the X-mode mention that triggered it, so xo can
# post up to THREE completion follow-ups when the task lands (within a 7-day window).
#
# Usage: xo-x-link.sh <task-id> <request_id> [--carry-count <n> --carry-ts <epoch> [--carry-platform <x|discord>] [--carry-max <n>]]
#
...
# no local meta, this refuses with the promised-final path (bin/xo-public-followup.sh
...
#
# Both ids are relay/xo slugs that compose a filename, so they are guarded
...
usage() {
  echo "usage: xo-x-link.sh <task-id> <request_id> [--carry-count <n> --carry-ts <epoch> [--carry-platform <x|discord>] [--carry-max <n>]]" >&2
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --carry-count)
      ...
      CARRY_COUNT=${1:-}
      case "$CARRY_COUNT" in
        ''|*[!0-9]*) echo "xo-x-link: --carry-count needs a non-negative integer" >&2; exit 2 ;;
      esac
      ;;
    --carry-ts)
      shift
      CARRY_TS=${1:-}
      case "$CARRY_TS" in
        ''|*[!0-9]*) echo "xo-x-link: --carry-ts needs a non-negative epoch integer" >&2; exit 2 ;;
      esac
      ;;
    --carry-platform)
      shift
      ...
      case "$CARRY_PLATFORM" in
        ...
        *) echo "xo-x-link: --carry-platform needs x or discord" >&2; exit 2 ;;
      esac
      ;;
    --carry-max)
      shift
      CARRY_MAX=${1:-}
      case "$CARRY_MAX" in
        ''|*[!0-9]*) echo "xo-x-link: --carry-max needs an integer of at least 50" >&2; exit 2 ;;
        *) [ "$CARRY_MAX" -ge 50 ] 2>/dev/null || { echo "xo-x-link: --carry-max needs an integer of at least 50" >&2; exit 2; } ;;
      esac
      ;;
    *) usage; exit 2 ;;
  esac
...
if [ -n "$CARRY_COUNT" ] && [ -z "$CARRY_TS" ]; then
  echo "xo-x-link: --carry-count requires --carry-ts to preserve the original follow-up window" >&2
  exit 2
fi
if [ -n "$CARRY_TS" ] && [ -z "$CARRY_COUNT" ]; then
  echo "xo-x-link: --carry-ts requires --carry-count to preserve the consumed follow-up count" >&2
  exit 2
fi
if { [ -n "$CARRY_PLATFORM" ] || [ -n "$CARRY_MAX" ]; } && { [ -z "$CARRY_COUNT" ] || [ -z "$CARRY_TS" ]; }; then
  echo "xo-x-link: --carry-platform and --carry-max require --carry-count and --carry-ts" >&2
  exit 2
fi
...
xo_pr_task_id_valid "$ID" || { echo "xo-x-link: unsafe task id: $ID" >&2; exit 2; }
case "$RID" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "xo-x-link: unsafe request_id: $RID" >&2; exit 2 ;;
esac
...
META="$STATE/$ID.meta"
if [ ! -f "$META" ]; then
  echo "xo-x-link: no such task: state/$ID.meta" >&2
  scan_secondmate_routes "$ID"
  if [ -n "$ROUTE_MATCHES" ]; then
    printf 'xo-x-link: %s is a second mate task (found in: %s), so this home cannot link it - a link only binds work whose record lives here.\n' \
      "$ID" "$ROUTE_MATCHES" >&2
  elif [ "$ROUTE_REGISTERED" -eq 1 ]; then
    printf 'xo-x-link: this home has registered second mates and no record of %s, so the work may be routed to one - a link only binds work whose record lives here.\n' \
      "$ID" >&2
  fi
  if [ -n "$ROUTE_MATCHES" ] || [ "$ROUTE_REGISTERED" -eq 1 ]; then
    ...
    printf 'xo-x-link: bind the public promise through the promised-final path instead: tasks-axi public-followup add + bind-work, then bin/xo-public-followup.sh register <obligation-id> --relation <relation-id> --work-home %s --work-id %s --generation <n>, and put the bin/xo-public-followup.sh brief <obligation-id> command into the routed worker instructions.\n' \
      "$ROUTE_HOME_ARG" "$ID" >&2
  fi
  exit 1
fi
...
if [ -n "$CARRY_TS" ] && { [ -z "$REQ_PLATFORM" ] || [ -z "$REQ_REPLY_MAX" ]; }; then
  echo "xo-x-link: relink requires carried reply context; pass --carry-platform and --carry-max from the prior task" >&2
  exit 2
fi

if [ -z "$CARRY_TS" ] && { [ -z "$REQ_PLATFORM" ] || [ -z "$REQ_REPLY_MAX" ]; }; then
  echo "xo-x-link: WARNING: incomplete authoritative reply context for request $RID; every completion follow-up will be HELD until both platform and explicit budget can be resolved. Ensure the relay request-context lookup supplies both values." >&2
fi
