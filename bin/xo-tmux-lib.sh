#!/usr/bin/env bash
# xo-tmux-lib.sh — shared tmux pane primitives for xo.
#
# ONE tmux source for delivery-busy detection, composer capture primitives,
# and verified submit.
# Both the away-mode daemon and bin/xo-send.sh reach these primitives through
# backend dispatch, while bin/xo-composer-lib.sh owns the shared verdict.
#
# Composer shapes and verdicts are owned by bin/xo-composer-lib.sh.
...
# Styled captures remain internal; xo-peek and every human-facing capture stay
# plain.
#
...
# become busy.
# The queued-Enter policy itself lives in xo_composer_queued_enter_verdict
# (bin/xo-composer-lib.sh); this file supplies tmux's pane-busy primitive.
#
# XO_COMPOSER_IDLE_RE is interpreted by the shared classifier with its structural
# and styling safety gates.
# XO_BUSY_REGEX overrides the rendered delivery-busy matching used here.
#
# NOT a task-state source: task busy state is owned by bin/xo-busy-lib.sh's
...
# bin/xo-composer-lib.sh (xo_composer_classify_screen), sourced below and
...
. "$(dirname -- "${BASH_SOURCE[0]}")/xo-composer-lib.sh"
# shellcheck source=bin/xo-cursor-lib.sh
. "$(dirname -- "${BASH_SOURCE[0]}")/xo-cursor-lib.sh"


# xo_tmux_strip_ghost: thin adapter over the shared, fleet-wide ghost extractor
# xo_composer_strip_ghost (bin/xo-composer-lib.sh). It drops de-emphasised
...
xo_tmux_strip_ghost() { xo_composer_strip_ghost; }
...
# (bin/xo-composer-lib.sh, xo_composer_classify_screen), so a new harness
# shape is taught there once and never here.

# xo_tmux_composer_capture: the visible pane WITH ANSI styling. The styled
# capture is consumed internally by the classifier and is NEVER surfaced
# (xo-peek and every human/LLM-facing path stay plain).
xo_tmux_composer_capture() {  # <target>
  tmux capture-pane -e -p -t "$1" -S 0 -E - 2>/dev/null
}

# xo_tmux_composer_cursor_row: the pane's cursor row, zero-based, relative to
# the visible pane - tmux's genuine primitive that no other backend has.
xo_tmux_composer_cursor_row() {  # <target>
  tmux display-message -p -t "$1" '#{cursor_y}' 2>/dev/null
}

# xo_tmux_composer_caps: the tmux capability descriptor - static data, not
# logic (see the capability model in bin/xo-composer-lib.sh).
xo_tmux_composer_caps() {
  printf 'styled=1\ncursor=1\nidentity=1\nrows=0\n'
}

# xo_tmux_composer_identity: the tmux agent-identity probe backing the
...
#     same scoping as xo_backend_tmux_foreground_comms) contains a pi-family
...
#   - status: pi's verified busy footer via xo_pane_is_busy, mapped onto the
...
# live pi.
xo_tmux_composer_identity() {  # <target>
...
EOF
  ...
  [ "$found" -eq 1 ] || return 1
  status=$(xo_pane_busy_state "$target" pi)
  case "$status" in
...
# xo_tmux_composer_state: the tmux composer verdict - a thin adapter over the
...
# future verdicts failing safe) is owned by bin/xo-composer-lib.sh. Identity
...
xo_tmux_composer_state() {  # <target> -> empty|pending|pending-unproven|unknown
  local target=$1 cy pane verdict identity
  cy=$(xo_tmux_composer_cursor_row "$target") || { printf 'unknown'; return 0; }
  case "$cy" in ''|*[!0-9]*) printf 'unknown'; return 0 ;; esac
  pane=$(xo_tmux_composer_capture "$target") || { printf 'unknown'; return 0; }
  verdict=$(xo_composer_classify_screen "$(xo_tmux_composer_caps)" "$pane" "$cy")
  if [ "$verdict" = need-identity ]; then
    if ! identity=$(xo_tmux_composer_identity "$target") || [ -z "$identity" ]; then
      identity=probe-absent
    fi
    verdict=$(xo_composer_classify_screen "$(xo_tmux_composer_caps)" "$pane" "$cy" "$identity")
  ...
  if [ "$verdict" = unknown ] && xo_tmux_pane_is_cursor "$target"; then
    verdict=$(xo_composer_classify_screen "$(xo_tmux_composer_caps)" "$pane" '')
  fi
  printf '%s' "$verdict"
}

# xo_tmux_pane_is_cursor: true when the pane's FOREGROUND process group contains
...
# single owner is bin/xo-cursor-lib.sh. The foreground scoping (pgid = tpgid)
# matches xo_tmux_composer_identity, so a pane whose agent exited to a shell has
# no Cursor foreground process and gets no reclassification.
xo_tmux_pane_is_cursor() {  # <target>
  ...
  while read -r pid pgid tpgid comm; do
    ...
    argv0=${args%%[[:space:]]*}
    xo_cursor_process_matches "$comm" '' "$argv0" && return 0
  done <<EOF
$(LC_ALL=C ps -t "${tty#/dev/}" -o pid=,pgid=,tpgid=,comm= 2>/dev/null)
EOF
  return 1
...
# xo_pane_input_pending: 0 when the composer is not proven empty, so pending
# text, ambiguous structure, unreadable state, and future verdicts all defer.
xo_pane_input_pending() {  # <target>
  [ "$(xo_tmux_composer_state "$1")" != empty ]
}

# xo_pane_is_busy: 0 if the pane's last few non-blank lines show a busy footer
# (an agent mid-turn). Scans a 40-line tail like xo-watch.sh.
xo_pane_busy_state() {  # <target> [harness] -> busy|idle|unknown
...
xo_pane_is_busy() {  # <target> [harness]
  [ "$(xo_pane_busy_state "$1" "${2:-}")" = busy ]
}

# xo_tmux_submit_core: type <text> into <target> ONCE, then submit with Enter,
...
# `xo_pane_is_busy`: a busy pane means the Enter was accepted and queued (report
...
# xo_tmux_submit_enter_core caller, or a pane already busy before typing) an
...
xo_tmux_submit_enter_core() {  # <target> <retries> <enter-sleep> [baseline-idle]
  local target=$1 retries=$2 sleep_s=$3 baseline_idle=${4:-} i=0 j state busy_state
  while :; do
    tmux send-keys -t "$target" Enter 2>/dev/null || true
    sleep "$sleep_s"
    state=$(xo_tmux_composer_state "$target")
    case "$state" in
      ...
      unknown)
        if [ "$baseline_idle" = 1 ]; then
          ...
          while [ "$j" -lt "$retries" ]; do
            if xo_pane_is_busy "$target"; then
              printf 'empty'
              return 0
            fi
            j=$((j + 1))
  ...
  # Busy conversion is owned by xo_composer_queued_enter_verdict.
  busy_state=idle
  xo_pane_is_busy "$target" && busy_state=busy
  xo_composer_queued_enter_verdict "$state" "$busy_state"
}

xo_tmux_submit_core() {  # <target> <text> <retries> <enter-sleep> <settle>
  ...
  baseline_state=$(xo_pane_busy_state "$target")
  [ "$baseline_state" = idle ] && baseline_idle=1
  tmux send-keys -t "$target" -l "$text" 2>/dev/null || { printf 'send-failed'; return 0; }
  sleep "$settle"
  xo_tmux_submit_enter_core "$target" "$retries" "$sleep_s" "$baseline_idle"
}
