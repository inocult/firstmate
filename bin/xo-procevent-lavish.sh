...
#
# Usage:
#   xo-procevent-lavish.sh arm <artifact.html>
#   xo-procevent-lavish.sh classify <result-file>
#   xo-procevent-lavish.sh terminal <result-file>
#   xo-procevent-lavish.sh silent <result-file>
#   xo-procevent-lavish.sh answers <result-file>
#   xo-procevent-lavish.sh reconciles <result-file>
#   xo-procevent-lavish.sh read <result-file>
#   xo-procevent-lavish.sh source-id <artifact.html>
#   xo-procevent-lavish.sh retire <artifact.html>
#   xo-procevent-lavish.sh poll <artifact.html>
#
...
#            keeps it armed. This is the generic adapter contract bin/xo-procevent.sh
...
#            is the generic no-op contract bin/xo-procevent.sh calls, and the
...
# and restart recovery all belong to bin/xo-procevent.sh.
#
# `answers` is this adapter's half of the generic keyed-answer contract in
# bin/xo-procevent.sh. It reports what the captain actually chose, as
...
# intake in bin/xo-captain-hold.sh, which the runner feeds. A Lavish review is
...
# Lavish fact, so the generic runner in bin/xo-procevent.sh stays
...
# runner reads the process output is unrecoverable, and no XO wrapper can
...
cmd_arm() {
  ...
  "$SCRIPT_DIR/xo-procevent.sh" register lavish "$id" \
    -- "$SCRIPT_DIR/xo-procevent-lavish.sh" poll "$real" || exit 1
  printf 'armed: %s\n' "$id"
  printf 'artifact: %s\n' "$real"
}
...
# Minimum seconds between retry attempt starts. XO_LAVISH_POLL_RETRY_DELAY is a
...
# meaning anything.
poll_retry_delay() {
  local delay=${XO_LAVISH_POLL_RETRY_DELAY-}
  if [ -z "$delay" ]; then
    printf '%s\n' "$POLL_RETRY_DELAY_DEFAULT"
    return 0
  fi
  case "$delay" in
    *[!0-9]*) die "XO_LAVISH_POLL_RETRY_DELAY must be whole seconds from $POLL_RETRY_DELAY_MIN to $POLL_RETRY_DELAY_MAX: $delay" ;;
  esac
  [ "$delay" -ge "$POLL_RETRY_DELAY_MIN" ] && [ "$delay" -le "$POLL_RETRY_DELAY_MAX" ] \
    || die "XO_LAVISH_POLL_RETRY_DELAY must be whole seconds from $POLL_RETRY_DELAY_MIN to $POLL_RETRY_DELAY_MAX: $delay"
  printf '%s\n' "$delay"
}
...
cmd_poll() {
  ...
  response=$(mktemp "${TMPDIR:-/tmp}/xo-lavish-poll.XXXXXX") || die "cannot stage the poll response"
  ...
  perl -MJSON::PP -e '
    ...
    for my $row (@rows) {
      ...
      if (defined($data->{schema}) && !ref($data->{schema})
          && $data->{schema} eq "xo-bearings-answer.v1") {
        $key = $data->{question};
