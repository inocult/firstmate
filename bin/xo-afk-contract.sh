#!/usr/bin/env bash
# xo-afk-contract.sh - the one owner of the away-posture record: its schema, the
...
# in bin/xo-afk-return.sh calls `archive` through bin/xo-afk-launch.sh stop).
...
# deferred to phase 4 (xo-afk-clauses-execute-r1).
#
...
#
# Usage:
#   xo-afk-contract.sh propose [--words-file <path> | --words <text>]
#       [--action <verb> --object <text> --when <text> [--stop <text>]]...
...
#   xo-afk-contract.sh readback [--proposal]
#   xo-afk-contract.sh field <name> [--proposal]
#   xo-afk-contract.sh words [--proposal | --path <record>]
#   xo-afk-contract.sh clauses [--proposal | --path <record>]   TSV: id action object when stop
#   xo-afk-contract.sh flags [--proposal | --path <record>]     TSV: id concept (flagged clauses only)
#   xo-afk-contract.sh validate [--proposal | --path <record>]  exit 0 when the record is readable and, for a record, confirmed
...
#   xo-afk-contract.sh refused [--proposal | --path <record>]   TSV: id text missing
#   xo-afk-contract.sh grants [--proposal | --path <record>]    one task id per line
#   xo-afk-contract.sh archive              move the record aside; print its path
#   xo-afk-contract.sh archived <entered_epoch>   print that archived record's path
#
# Sourceable: with the BASH_SOURCE guard, other scripts get the path and
# presence helpers (xo_afk_contract_path, xo_afk_contract_present,
# xo_afk_contract_proposal_path, xo_afk_contract_archive_dir) without running main.
set -u

XO_AFK_CONTRACT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XO_ROOT="${XO_ROOT_OVERRIDE:-$(cd "$XO_AFK_CONTRACT_DIR/.." && pwd)}"
XO_HOME="${XO_HOME:-${XO_ROOT_OVERRIDE:-$XO_ROOT}}"
XO_AFK_CONTRACT_STATE="${XO_STATE_OVERRIDE:-$XO_HOME/state}"

# shellcheck source=bin/xo-classify-lib.sh
. "$XO_AFK_CONTRACT_DIR/xo-classify-lib.sh"

XO_AFK_CONTRACT_VERSION=1
XO_AFK_CONTRACT_VERBS="merge land prerelease install rerun dispatch abort-run answer discard wake-me"
XO_AFK_CONTRACT_REACH_ANNOUNCED='No phone channel is configured; anything that needs you waits for your return.'
XO_AFK_CONTRACT_SPEND_DEFAULT=4

xo_afk_contract_path() {  # [state-dir]
  printf '%s/.afk-contract' "${1:-$XO_AFK_CONTRACT_STATE}"
}

xo_afk_contract_proposal_path() {  # [state-dir]
  printf '%s/.afk-contract.proposed' "${1:-$XO_AFK_CONTRACT_STATE}"
}

xo_afk_contract_archive_dir() {  # [state-dir]
  printf '%s/afk-contracts' "${1:-$XO_AFK_CONTRACT_STATE}"
}

xo_afk_contract_present() {  # [state-dir]
  [ -f "$(xo_afk_contract_path "${1:-$XO_AFK_CONTRACT_STATE}")" ]
}

xo_afk_contract_log() { printf 'xo-afk-contract: %s\n' "$*" >&2; }

xo_afk_contract_usage() {
  sed -n '/^# Usage:/,/^# Sourceable:/p' "${BASH_SOURCE[0]}" | sed '$d' | sed 's/^# \{0,1\}//'
}

xo_afk_contract_now_iso() {
  date -u +%Y-%m-%dT%H:%M:%SZ
}

xo_afk_contract_lower() {  # <text>
  printf '%s' "$1" | tr '[:upper:]' '[:lower:]'
}

xo_afk_contract_action() {  # <text>
  xo_afk_contract_lower "$1" | tr '\t\r\n' '   ' | sed 's/^ *//; s/ *$//; s/  */ /g'
}

xo_afk_contract_blank() {  # <text>
  [ -z "$(printf '%s' "$1" | tr -d '[:space:]')" ]
}

# Same alphabet as xo_pr_task_id_valid / xo_task_id_path_safe in bin/xo-pr-lib.sh.
# Kept local so sourcing this file cannot reset that library's parse globals.
xo_afk_contract_grant_id_valid() {  # <id>
  local LC_ALL=C id=${1-}
  case "$id" in
...
xo_afk_contract_unescape() {  # <escaped-text>
  printf '%b' "$1"
}

# --- clause structural check and never-set scan ------------------------------

# xo_afk_contract_never_set_hit <text...>: prints the protected concept the
...
xo_afk_contract_never_set_hit() {  # <text...>
...
xo_afk_contract_clause_check() {  # <action> <object> <when> <stop> <stop-given 0|1>
  C_ACTION=$(xo_afk_contract_action "$1")
  C_OBJECT=$2
  C_WHEN=$3
  C_STOP=$4
  C_MISSING=
  C_FLAG=$(xo_afk_contract_never_set_hit "$C_ACTION" "$C_OBJECT" "$C_WHEN" "$C_STOP") || C_FLAG=
  if [ -z "$C_ACTION" ]; then
    C_MISSING='action - the clause names no action'
    return 1
  fi
  case " $XO_AFK_CONTRACT_VERBS " in
    *" $C_ACTION "*) ;;
    *)
      C_MISSING="action - '$C_ACTION' is not a mandate verb (one of: ${XO_AFK_CONTRACT_VERBS// /, })"
      return 1 ;;
  esac
  if xo_afk_contract_blank "$C_OBJECT"; then
    C_MISSING='object - the clause names no thing to act on'
    return 1
  fi
  if xo_afk_contract_blank "$C_WHEN"; then
    C_MISSING='when - the clause states no precondition'
    return 1
  fi
  if [ "$5" -eq 1 ] && xo_afk_contract_blank "$C_STOP"; then
...
xo_afk_contract_clause_as_given() {  # <action> <object> <when> <stop> <stop-given 0|1>
  local text
...
xo_afk_contract_validate_iso() {  # <ts>
  xo_utc_iso_to_epoch "$1" >/dev/null 2>&1
}
...
# SPEND, MERGE_GRANTS.
xo_afk_contract_render_body() {  # <entered-iso> <entered-epoch>
  local entered=$1 entered_epoch=$2 ordinal=0 i as_given grant
  local accepted_block="" refused_block=""
  i=0
  while [ "$i" -lt "${#CLAUSE_ACTIONS[@]}" ]; do
    ordinal=$((ordinal + 1))
    if xo_afk_contract_clause_check "${CLAUSE_ACTIONS[$i]}" "${CLAUSE_OBJECTS[$i]}" "${CLAUSE_WHENS[$i]}" "${CLAUSE_STOPS[$i]}" "${CLAUSE_STOP_GIVENS[$i]}"; then
      accepted_block="$accepted_block$(printf '  - id: %s\n    action: %s\n    object: e:%s\n    when: e:%s\n' \
        "$ordinal" "$C_ACTION" "$(xo_afk_contract_escape "$C_OBJECT")" "$(xo_afk_contract_escape "$C_WHEN")"
      if [ -n "$C_STOP" ]; then
        printf '    stop: e:%s\n' "$(xo_afk_contract_escape "$C_STOP")"
      else
        printf '    stop: -\n'
      fi
      if [ -n "$C_FLAG" ]; then
...
"
      ...
      as_given=$(xo_afk_contract_clause_as_given "${CLAUSE_ACTIONS[$i]}" "${CLAUSE_OBJECTS[$i]}" "${CLAUSE_WHENS[$i]}" "${CLAUSE_STOPS[$i]}" "${CLAUSE_STOP_GIVENS[$i]}"; printf x)
      as_given=${as_given%x}
      refused_block="$refused_block$(printf '  - id: %s\n    text: e:%s\n    missing: %s' \
        "$ordinal" "$(xo_afk_contract_escape "$as_given")" "$C_MISSING")
"
    fi
    i=$((i + 1))
  done
  printf 'version: %s\n' "$XO_AFK_CONTRACT_VERSION"
  printf 'entered: %s\n' "$entered"
  printf 'entered_epoch: %s\n' "$entered_epoch"
  printf 'expected_return: %s\n' "${EXPECTED_RETURN:--}"
  printf 'reach_channels: none\n'
  printf 'reach_announced: %s\n' "$XO_AFK_CONTRACT_REACH_ANNOUNCED"
  printf 'spend_max_concurrent_workers: %s\n' "${SPEND:-$XO_AFK_CONTRACT_SPEND_DEFAULT}"
...
xo_afk_contract_write_atomic() {  # <path> (content on stdin)
...
xo_afk_contract_read_field() {  # <path> <name>
  local path=$1 name=$2
  [ -f "$path" ] || return 1
  sed -n "s/^${name}: //p" "$path" | head -1
}

xo_afk_contract_read_words() {  # <path>
  local path=$1
  [ -f "$path" ] || return 1
  awk -v record="$path" '
    function die(reason) {
      printf "xo-afk-contract: record %s has an invalid words block: %s\n", record, reason > "/dev/stderr"
      bad = 1
      exit 2
    }
...
xo_afk_contract_read_grants() {  # <path>
  local path=$1
  [ -f "$path" ] || return 1
  awk -v record="$path" '
    function die(reason) {
      printf "xo-afk-contract: record %s has an invalid merge_grants field: %s\n", record, reason > "/dev/stderr"
      bad = 1
      exit 2
    }
...
xo_afk_contract_read_list() {  # <path> <section>
  local path=$1 section=$2
  [ -f "$path" ] || return 1
  awk -v want="$section" -v verbs="$XO_AFK_CONTRACT_VERBS" -v record="$path" '
    function row_name() { return (id != "" ? id : ordinal + 1) }
    function die(part) {
      printf "xo-afk-contract: record %s has malformed %s row %s: missing or invalid %s\n", record, section, row_name(), part > "/dev/stderr"
      bad = 1
      exit 2
    }
    ...
    END {
      ...
      if (!done) flush()
      if (!found) {
        printf "xo-afk-contract: record %s lacks its %s section\n", record, section > "/dev/stderr"
        exit 2
      }
    }
  ' "$path"
...
# schema.
xo_afk_contract_validate() {  # <path> <require-confirmed 0|1>
  ...
  [ -f "$path" ] || return 1
  version=$(xo_afk_contract_read_field "$path" version)
  [ "$version" = "$XO_AFK_CONTRACT_VERSION" ] || {
    xo_afk_contract_log "record $path carries version '${version:-none}', expected $XO_AFK_CONTRACT_VERSION; refusing to read it"
    return 1
  }
  entered=$(xo_afk_contract_read_field "$path" entered)
  xo_afk_contract_validate_iso "$entered" || { xo_afk_contract_log "record $path has no valid entered time"; return 1; }
  entered_epoch=$(xo_afk_contract_read_field "$path" entered_epoch)
  case "$entered_epoch" in ''|*[!0-9]*) xo_afk_contract_log "record $path has no entered_epoch"; return 1 ;; esac
  expected=$(xo_afk_contract_read_field "$path" expected_return)
  [ "$expected" = - ] || xo_afk_contract_validate_iso "$expected" || { xo_afk_contract_log "record $path has no valid expected_return"; return 1; }
  reach=$(xo_afk_contract_read_field "$path" reach_channels)
  [ "$reach" = none ] || { xo_afk_contract_log "record $path has no valid reach_channels"; return 1; }
  announced=$(xo_afk_contract_read_field "$path" reach_announced)
  [ -n "$announced" ] || { xo_afk_contract_log "record $path has no reach announcement"; return 1; }
  spend=$(xo_afk_contract_read_field "$path" spend_max_concurrent_workers)
  case "$spend" in ''|*[!0-9]*|0) xo_afk_contract_log "record $path has no valid spend cap"; return 1 ;; esac
  words_header=$(sed -n '/^words: /{p;q;}' "$path")
  case "$words_header" in 'words: -'|'words: |'|'words: |-') ;; *) xo_afk_contract_log "record $path has no valid words field"; return 1 ;; esac
  xo_afk_contract_read_words "$path" >/dev/null || return 1
  xo_afk_contract_read_grants "$path" >/dev/null || {
    xo_afk_contract_log "record $path has no valid merge_grants field"
    return 1
  }
  if [ "$require_confirmed" -eq 1 ]; then
    confirmed=$(xo_afk_contract_read_field "$path" confirmed)
    xo_afk_contract_validate_iso "$confirmed" || { xo_afk_contract_log "record $path has no valid confirmed time"; return 1; }
    case "$(xo_afk_contract_read_field "$path" confirmed_epoch)" in
      ''|*[!0-9]*) xo_afk_contract_log "record $path was never confirmed"; return 1 ;;
    esac
  fi
  if ! clause_rows=$(xo_afk_contract_read_list "$path" clauses); then
    return 1
  fi
  while IFS= read -r clause; do
    ...
    decoded=$(xo_afk_contract_unescape "$object"; printf x)
    decoded=${decoded%x}
    if xo_afk_contract_blank "$decoded"; then
      xo_afk_contract_log "record $path has malformed clauses row $id: missing or invalid object"
      return 1
    fi
    decoded=$(xo_afk_contract_unescape "$when"; printf x)
    decoded=${decoded%x}
    if xo_afk_contract_blank "$decoded"; then
      xo_afk_contract_log "record $path has malformed clauses row $id: missing or invalid when"
      return 1
    fi
    if [ "$stop" != - ]; then
      decoded=$(xo_afk_contract_unescape "$stop"; printf x)
      decoded=${decoded%x}
      if xo_afk_contract_blank "$decoded"; then
        xo_afk_contract_log "record $path has malformed clauses row $id: missing or invalid stop"
        return 1
      fi
    fi
  done <<EOF
$clause_rows
EOF
  if ! refused_rows=$(xo_afk_contract_read_list "$path" refused); then
    return 1
  fi
  while IFS= read -r refused; do
    ...
    decoded=$(xo_afk_contract_unescape "$text"; printf x)
    decoded=${decoded%x}
    if xo_afk_contract_blank "$decoded"; then
      xo_afk_contract_log "record $path has malformed refused row $id: missing or invalid text"
      return 1
    fi
  done <<EOF
$refused_rows
...
EOF
  printf '%s\n' "$title"
  printf '  entered: %s\n' "$(xo_afk_contract_read_field "$path" entered)"
  ...
  printf '  reach: hold-for-return only. %s\n' "$(xo_afk_contract_read_field "$path" reach_announced)"
  words=$(xo_afk_contract_read_words "$path"; printf x)
  words=${words%x}
  if [ -n "$words" ]; then
  ...
  while IFS="$(printf '\t')" read -r id action object when stop; do
    ...
    xo_afk_contract_unescape "$object"
    printf ' when '
    xo_afk_contract_unescape "$when"
    if [ "$stop" != - ]; then
      printf ' stop '
      xo_afk_contract_unescape "$stop"
    fi
    flag=$(xo_afk_contract_read_list "$path" flags | awk -F '\t' -v id="$id" '$1 == id { print $2 }')
    [ -z "$flag" ] || printf " - flagged: names '%s', a never-set concept that is never pre-authorizable; recorded, judged at execution" "$flag"
    printf '\n'
  done <<EOF
$(xo_afk_contract_read_list "$path" clauses)
EOF
  ...
  while IFS="$(printf '\t')" read -r id text missing; do
    ...
    count=$((count + 1))
    printf '    %s. "' "$id"
    xo_afk_contract_unescape "$text"
    printf '" - refused: missing %s\n' "$missing"
  done <<EOF
$(xo_afk_contract_read_list "$path" refused)
EOF
  [ "$count" -gt 0 ] || printf '    (none)\n'
...
xo_afk_contract_render_announcement() {  # <path>
  local path=$1 accepted refused flagged expected clause_text
  accepted=$(xo_afk_contract_read_list "$path" clauses | grep -c . || true)
  refused=$(xo_afk_contract_read_list "$path" refused | grep -c . || true)
  flagged=$(xo_afk_contract_read_list "$path" flags | grep -c . || true)
  expected=$(xo_afk_contract_read_field "$path" expected_return)
  ...
  printf 'Away posture confirmed at %s: hold-for-return only. %s %s Expected return: %s. Spend cap: %s concurrent workers.\n' \
    "$(xo_afk_contract_read_field "$path" confirmed)" \
    "$(xo_afk_contract_read_field "$path" reach_announced)" \
    "$clause_text" \
    "$( [ "$expected" = - ] && printf 'not given' || printf '%s' "$expected")" \
    "$(xo_afk_contract_read_field "$path" spend_max_concurrent_workers)"
}

# --- subcommands ------------------------------------------------------------

xo_afk_contract_parse_inputs() {  # <args...>; sets WORDS, the CLAUSE_* arrays, EXPECTED_RETURN, SPEND, MERGE_GRANTS
  local words_file='' open=-1 grant
  WORDS=; EXPECTED_RETURN=-; SPEND=$XO_AFK_CONTRACT_SPEND_DEFAULT
  CLAUSE_ACTIONS=(); CLAUSE_OBJECTS=(); CLAUSE_WHENS=(); CLAUSE_STOPS=(); CLAUSE_STOP_GIVENS=()
  MERGE_GRANTS=()
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --words-file)
        [ "$#" -gt 1 ] || { xo_afk_contract_log '--words-file requires a path'; return 2; }
        words_file=$2
        shift 2 ;;
      --words)
        [ "$#" -gt 1 ] || { xo_afk_contract_log '--words requires text'; return 2; }
        WORDS=$2
        shift 2 ;;
      --action)
        [ "$#" -gt 1 ] || { xo_afk_contract_log '--action requires a verb; it opens a clause for the --object, --when, and --stop that follow it'; return 2; }
        CLAUSE_ACTIONS+=("$2"); CLAUSE_OBJECTS+=(''); CLAUSE_WHENS+=(''); CLAUSE_STOPS+=(''); CLAUSE_STOP_GIVENS+=(0)
        open=$(( ${#CLAUSE_ACTIONS[@]} - 1 ))
        shift 2 ;;
      --object|--when|--stop)
        [ "$#" -gt 1 ] || { xo_afk_contract_log "$1 requires text"; return 2; }
        [ "$open" -ge 0 ] || { xo_afk_contract_log "$1 must follow the --action that opens its clause"; return 2; }
        case "$1" in
        ...
        shift 2 ;;
      --expected-return)
        [ "$#" -gt 1 ] || { xo_afk_contract_log '--expected-return requires a UTC ISO 8601 time'; return 2; }
        if ! xo_afk_contract_validate_iso "$2"; then
          xo_afk_contract_log "--expected-return must be UTC ISO 8601 (YYYY-MM-DDTHH:MM[:SS]Z), got '$2'"
          return 2
        fi
        EXPECTED_RETURN=$2
        shift 2 ;;
      --spend)
        [ "$#" -gt 1 ] || { xo_afk_contract_log '--spend requires a positive integer'; return 2; }
        case "$2" in ''|*[!0-9]*|0) xo_afk_contract_log "--spend must be a positive integer, got '$2'"; return 2 ;; esac
        SPEND=$2
        shift 2 ;;
      --grant)
        [ "$#" -gt 1 ] || { xo_afk_contract_log '--grant requires a task id'; return 2; }
        xo_afk_contract_grant_id_valid "$2" || {
          xo_afk_contract_log "--grant must be a valid task id, got '$2'"
          return 2
        }
        for grant in "${MERGE_GRANTS[@]+"${MERGE_GRANTS[@]}"}"; do
          [ "$grant" != "$2" ] || {
            xo_afk_contract_log "--grant lists '$2' more than once"
            return 2
          }
        done
        MERGE_GRANTS+=("$2")
        shift 2 ;;
      --grant=*)
        xo_afk_contract_log '--grant takes a separate task-id argument'
        return 2 ;;
      *)
        xo_afk_contract_log "unknown option '$1'"
        return 2 ;;
    esac
  done
  if [ -n "$words_file" ]; then
    [ -f "$words_file" ] || { xo_afk_contract_log "words file not found: $words_file"; return 2; }
...
xo_afk_contract_cmd_propose() {
  local entered entered_epoch proposal rc=0 refused
  xo_afk_contract_parse_inputs "$@" || return 2
  entered=$(xo_afk_contract_now_iso)
  entered_epoch=$(date +%s)
  proposal=$(xo_afk_contract_proposal_path)
  xo_afk_contract_render_body "$entered" "$entered_epoch" | xo_afk_contract_write_atomic "$proposal" || {
    xo_afk_contract_log "failed to write the proposal at $proposal"
    return 1
  }
  refused=$(xo_afk_contract_read_list "$proposal" refused | grep -c . || true)
  [ "$refused" -eq 0 ] || rc=3
  xo_afk_contract_render_readback "$proposal" 'Away posture read-back (proposed, not yet confirmed):'
  printf 'Say go to confirm; restate any refused clause first if you want it recorded.\n'
  return "$rc"
}

xo_afk_contract_archive_target() {  # <record> [superseded-stamp]
  local record=$1 stamp=${2:-} dir entered_epoch target
  dir=$(xo_afk_contract_archive_dir)
  mkdir -p "$dir" || return 1
  entered_epoch=$(xo_afk_contract_read_field "$record" entered_epoch)
...
xo_afk_contract_cmd_confirm() {
  local record proposal body confirmed confirmed_epoch archived archived_tmp staged session_entered session_entered_epoch
  record=$(xo_afk_contract_path)
  proposal=$(xo_afk_contract_proposal_path)
  confirmed=$(xo_afk_contract_now_iso)
  confirmed_epoch=$(date +%s)
  if [ -f "$proposal" ]; then
    xo_afk_contract_validate "$proposal" 0 || return 1
    body=$(cat "$proposal")
  elif [ -f "$record" ]; then
    xo_afk_contract_validate "$record" 1 || return 1
    xo_afk_contract_log "away posture already recorded at $(xo_afk_contract_read_field "$record" entered); nothing to confirm"
    xo_afk_contract_render_announcement "$record"
    return 0
  else
    xo_afk_contract_log "no away-posture proposal exists; run propose before confirm"
    return 1
  fi
  session_entered=$confirmed
  session_entered_epoch=$confirmed_epoch
  if [ -f "$record" ]; then
    session_entered=$(xo_afk_contract_read_field "$record" entered)
    session_entered_epoch=$(xo_afk_contract_read_field "$record" entered_epoch)
  fi
  ...
  xo_afk_contract_validate "$staged" 1 || { rm -f "$staged"; return 1; }
  if [ -f "$record" ]; then
    archived=$(xo_afk_contract_archive_target "$record" "$confirmed_epoch") || { rm -f "$staged"; return 1; }
  ...
  if [ -n "${archived:-}" ]; then
    xo_afk_contract_log "replaced the earlier away posture; its record is archived at $archived"
  fi
  rm -f "$proposal"
  xo_afk_contract_render_announcement "$record"
}

xo_afk_contract_cmd_archive() {
  local record target
  record=$(xo_afk_contract_path)
  [ -f "$record" ] || return 0
  if ! xo_afk_contract_validate "$record" 1; then
    xo_afk_contract_log "confirmed away-posture record at $record is invalid; refusing to archive"
    return 1
  fi
  target=$(xo_afk_contract_archive_target "$record") || return 1
  mv "$record" "$target" || return 1
  printf '%s\n' "$target"
}

xo_afk_contract_select_path() {  # <args...> -> prints the record path chosen by --proposal/--path
  local path
  path=$(xo_afk_contract_path)
  while [ "$#" -gt 0 ]; do
    case "$1" in
      --proposal) path=$(xo_afk_contract_proposal_path); shift ;;
...
xo_afk_contract_main() {
  local cmd=${1:-} path
  [ -n "$cmd" ] || { xo_afk_contract_usage >&2; return 2; }
  shift
  case "$cmd" in
    propose) xo_afk_contract_cmd_propose "$@" ;;
    confirm) [ "$#" -eq 0 ] || { xo_afk_contract_usage >&2; return 2; }; xo_afk_contract_cmd_confirm ;;
    readback)
      path=$(xo_afk_contract_select_path "$@") || { xo_afk_contract_usage >&2; return 2; }
      [ -f "$path" ] || { xo_afk_contract_log "no record at $path"; return 1; }
      if [ "$path" = "$(xo_afk_contract_proposal_path)" ]; then
        xo_afk_contract_render_readback "$path" 'Away posture read-back (proposed, not yet confirmed):'
      else
        xo_afk_contract_render_readback "$path" 'Away posture (confirmed):'
      fi ;;
    field)
      [ "$#" -ge 1 ] || { xo_afk_contract_usage >&2; return 2; }
      local name=$1; shift
      path=$(xo_afk_contract_select_path "$@") || { xo_afk_contract_usage >&2; return 2; }
      xo_afk_contract_read_field "$path" "$name" ;;
    words)
    ...
    flags)
      path=$(xo_afk_contract_select_path "$@") || { xo_afk_contract_usage >&2; return 2; }
      xo_afk_contract_read_list "$path" flags ;;
    validate)
      path=$(xo_afk_contract_select_path "$@") || { xo_afk_contract_usage >&2; return 2; }
      if [ "$path" = "$(xo_afk_contract_proposal_path)" ]; then
        xo_afk_contract_validate "$path" 0
      else
        xo_afk_contract_validate "$path" 1
      fi ;;
    refused)
      path=$(xo_afk_contract_select_path "$@") || { xo_afk_contract_usage >&2; return 2; }
      xo_afk_contract_read_list "$path" refused ;;
    grants)
      ...
      [ -f "$path" ] || { xo_afk_contract_log "no record at $path"; return 1; }
      xo_afk_contract_read_grants "$path" ;;
    archive) xo_afk_contract_cmd_archive ;;
    archived)
      [ "$#" -eq 1 ] || { xo_afk_contract_usage >&2; return 2; }
      path="$(xo_afk_contract_archive_dir)/$1.afk-contract"
      [ -f "$path" ] || { xo_afk_contract_log "no archived record for entered_epoch $1"; return 1; }
      printf '%s\n' "$path" ;;
    -h|--help|help) xo_afk_contract_usage ;;
    *) xo_afk_contract_usage >&2; return 2 ;;
  esac
}
