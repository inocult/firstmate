...
# It resolves the parent channel through xo_parent_channel_destination
# (bin/xo-parent-channel-lib.sh): a local mate writes the parent home's
# state/<id>.status, and a remote mate writes this home's
# state/parent-replies.status. Call it from the secondmate home with XO_HOME
# set to that home.
#
# Usage:
#   xo-secondmate-report.sh <verb> <corr_id> <note...>
#   xo-secondmate-report.sh --doc <verb> <corr_id> <doc-path> <note...>
#
# Examples:
#   xo-secondmate-report.sh done abcdef0123456789 "audit clean"
#   xo-secondmate-report.sh --doc done abcdef0123456789 data/x/report.md "see report"
set -eu

CALLER_XO_HOME=${XO_HOME:-}
...
HOME_DIR=$CALLER_XO_HOME
case "$HOME_DIR" in
  '')
    echo "error: XO_HOME is required so the helper can resolve the parent channel" >&2
    exit 1
    ;;
esac
STATE_DIR="${XO_STATE_OVERRIDE:-$HOME_DIR/state}"

DESTINATION=
DEST_RC=0
DESTINATION=$(xo_parent_channel_destination "$HOME_DIR" "$STATE_DIR") || DEST_RC=$?
