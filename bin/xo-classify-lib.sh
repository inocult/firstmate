...
# (bin/xo-watch.sh) and the away-mode daemon (bin/xo-supervise-daemon.sh) so the
# overlapping triage policy lives in one place instead of two copies that can
# drift apart.
#
# Most functions are pure, side-effect-free reads of status files: each takes
# what it needs as arguments and touches no globals beyond the optional
# XO_CAPTAIN_RE override. Consumers layer their own dedup/marker state on top (the
...
# read: it reuses bin/xo-crew-state.sh, which may make a bounded no-mistakes call,
...
# Directory of this library, used to locate the sibling xo-crew-state.sh reader.
# Resolved at source time from BASH_SOURCE so it works whether sourced by a
# bin/ script (which sets its own SCRIPT_DIR) or directly by a test.
_XO_CLASSIFY_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd 2>/dev/null)" || _XO_CLASSIFY_LIB_DIR="."

# The crew current-state reader used for the "provably working" decision.
# Overridable so tests can stub the run-step/pane verdict without a real worktree
# or no-mistakes install; absent, it points at the real sibling script.
XO_CREW_STATE_BIN="${XO_CREW_STATE_BIN:-$_XO_CLASSIFY_LIB_DIR/xo-crew-state.sh}"

# xo_run_timed, the shared hard bound the worktree write probe below puts around
# its one filesystem walk. bin/xo-timeout-lib.sh owns bounded execution for this
...
case $- in *u*) _xo_classify_nounset=on ;; *) _xo_classify_nounset=off ;; esac
# shellcheck source=bin/xo-timeout-lib.sh
# shellcheck disable=SC1091
. "$_XO_CLASSIFY_LIB_DIR/xo-timeout-lib.sh"
[ "$_xo_classify_nounset" = on ] || set +u
unset _xo_classify_nounset

# Captain-relevant status verbs. A status line carrying any of these is work
# xo must see. Lines without these verbs are no-verb signals: the watcher
# absorbs them only with positive provably-working evidence, while the daemon uses
# its away-mode classification. XO_CAPTAIN_RE overrides the whole set when a home
...
XO_CLASSIFY_CAPTAIN_RE_DEFAULT='done:|needs-decision:|blocked:|failed:|PR ready|checks green|ready in branch|merged'

# The deliberate-external-wait verb. A crew (or xo steering it) appends
#   paused: <reason>
...
# `blocked:` (stuck, xo must help) an idle `paused:` pane is EXPECTED, so
...
# drift between the two consumers. XO_CLASSIFY_PAUSED_VERB overrides it.
XO_CLASSIFY_PAUSED_VERB_DEFAULT='paused'

# Bounded re-surface cadence for a declared external-wait pause.
# Far longer than the wedge threshold (XO_STALE_ESCALATE_SECS, default 240s), it
...
# XO_PAUSE_RESURFACE_SECS with this default so
# the cadence has one owner. An item held for the captain is not rechecked at all
# while the away-posture record exists (bin/xo-watch.sh owns that rule).
# shellcheck disable=SC2034 # Read by the watcher and daemon (xo-watch.sh, xo-supervise-daemon.sh), not this lib.
XO_PAUSE_RESURFACE_SECS_DEFAULT=14400

# xo_utc_iso_to_epoch <YYYY-MM-DDTHH:MM[:SS]Z>: the one portable UTC ISO 8601
# reader shared by the declared-wait vocabulary and the away-posture record
# (bin/xo-afk-contract.sh). Prints epoch seconds; returns 1 on any other shape
# so a malformed time is refused rather than read as "now".
xo_utc_iso_to_epoch() {  # <timestamp>
  local ts=$1
  case "$ts" in
...
# xo-captain-hold.sh has verified the corresponding captain-held backlog item.
XO_CLASSIFY_RESOLVE_VERB_DEFAULT='resolved'
XO_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT='captain-held'
...
status_is_captain_relevant() {
  ...
  verb=$(status_line_verb "$line")
  case "$verb" in
    working|resolved|captain-held|"${XO_CLASSIFY_PAUSED_VERB:-$XO_CLASSIFY_PAUSED_VERB_DEFAULT}")
      return 1
      ;;
  esac
  if [ -z "${XO_CAPTAIN_RE+x}" ]; then
    case "$verb" in
      done|needs-decision|blocked|failed) return 0 ;;
    esac
  fi
  printf '%s' "$line" | grep -qiE "${XO_CAPTAIN_RE:-$XO_CLASSIFY_CAPTAIN_RE_DEFAULT}"
}

# 0 if a status line's leading verb is the pause verb (paused: <reason>). A pure
# read of the line itself, so the daemon's classify_stale can reuse the last line
# it already read without an xo-crew-state.sh call. Matches only the verb before the
# first colon, so a reason mentioning "paused" elsewhere does not false-match.
status_is_paused() {  # <status-line>
  local line=$1 verb
  [ -n "$line" ] || return 1
  verb=$(status_line_verb "$line")
  [ "$verb" = "${XO_CLASSIFY_PAUSED_VERB:-$XO_CLASSIFY_PAUSED_VERB_DEFAULT}" ]
}
...
status_is_captain_held() {  # <status-line>
  ...
  [ -n "$line" ] || return 1
  verb=$(status_line_verb "$line")
  [ "$verb" = "${XO_CLASSIFY_CAPTAIN_HELD_VERB:-$XO_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT}" ]
}
...
# once pause_state_class has admitted the wait (xo-watch.sh owns which liveness
...
# Who WRITES the closing line is owned elsewhere: the answering xo closes
# at answer time through xo-send's --resolve-key (bin/xo-send.sh header), and a
# worker self-closes only a blocker that cleared without an answer (bin/xo-brief.sh
...
# including the "[corr=<16 hex>]" form bin/xo-secondmate-report.sh writes. It
# does not cover the UNBRACKETED token that bin/xo-pending-reply-lib.sh writes
# (xo_pending_reply_corr_token), which a secondmate answering a marked request
# echoes on its parent status line ahead of the key tag (bin/xo-brief.sh), so a
...
# glued on matches no arm of _xo_decision_fold_line, so the opener never opens
...
#
# The token grammar is OWNED by bin/xo-pending-reply-lib.sh
# (xo_pending_reply_corr_token, XO_PENDING_REPLY_CORR_RE). That library sources
# this one, so it cannot be sourced back here; the pattern below is a deliberate
# second statement of the SHAPE alone, and tests/xo-classify-corr-token.test.sh
...
# parse and _xo_decision_key_transition_allowed exist to prevent. Recognising
# only what an xo library actually writes costs one more line here each
...
# variable, the same way bin/xo-secondmate-report.sh validates the id it is
...
_xo_classify_is_corr_token() {  # <word>
  case "$1" in
...
status_line_verb() {  # <status-line> -> leading verb word
  ...
  while [ -n "$v" ]; do
    ...
    v=${v#"$word"}
    v=${v#"${v%%[![:space:]]*}"}
    _xo_classify_is_corr_token "$word" && continue
    out="$out $word"
  done
  printf '%s' "$out"
}
...
_xo_key_before_colon() {  # <status-line>
  case "${1%%:*}" in
    *\[key=*\]*) return 0 ;;
    *) return 1 ;;
  esac
...
# the caller's check via _xo_decision_slug_ok, exactly as for the before-colon
# position.
_xo_key_at_note_head() {  # <status-line> -> raw slug
  local rest
  case "$1" in
    *:*) rest=${1#*:} ;;
    *) return 1 ;;
...
status_line_note() {  # <status-line> -> text after the first colon, trimmed
  ...
  # yield the same note.
  if ! _xo_key_before_colon "$1" && k=$(_xo_key_at_note_head "$1") \
    && _xo_decision_slug_ok "$k"; then
    n=${n#"[key=$k]"}
    n=${n#"${n%%[![:space:]]*}"}
  fi
  printf '%s' "$n"
}
_xo_decision_key() {  # <status-line> -> key slug, or "default" when no token
  local k
  if _xo_key_before_colon "$1"; then
    k=${1%%:*}
    k=${k#*\[key=}
    k=${k%%\]*}
  else
    k=$(_xo_key_at_note_head "$1") || { printf 'default'; return 0; }
  fi
  _xo_decision_slug_ok "$k" || return 1
  printf '%s' "$k"
}
# Drop the record for <key> from a newline-terminated "<key>\t<verb>\t<note>" set.
# Portable (no associative arrays) so the fold runs on bash 3.2 as well as 4+.
_xo_decision_drop() {  # <open-set> <key>
...
XO_CLASSIFY_RESERVED_KEY_PREFIXES_DEFAULT='pending-reply-'

# 0 when <key> is not reserved, or is reserved and <note> speaks its vocabulary.
_xo_decision_key_transition_allowed() {  # <key> <note>
  local key=$1 note=$2 prefix
  for prefix in ${XO_CLASSIFY_RESERVED_KEY_PREFIXES:-$XO_CLASSIFY_RESERVED_KEY_PREFIXES_DEFAULT}; do
    case "$key" in
      "$prefix"*)
        case "$note" in
          "$prefix"*:*) return 0 ;;
...
_xo_is_pending_reply_escalation() {  # <key> <note>
...
_xo_decision_fold_line() {  # <open-set> <status-line> <resolve-verb> <held-verb>
  ...
  verb=$(status_line_verb "$line")
  key=$(_xo_decision_key "$line") || { printf '%s' "$open"; return 0; }
  _xo_decision_key_transition_allowed "$key" "$(status_line_note "$line")" \
    || { printf '%s' "$open"; return 0; }
  case "$verb" in
    needs-decision|blocked)
      note=$(status_line_note "$line")
      open=$(_xo_decision_drop "$open" "$key")
      [ -n "$open" ] && open="${open}"$'\n'
...
# the file, no globals beyond the optional XO_CLASSIFY_RESOLVE_VERB override. This
...
# before any read - a cheap builtin, unlike xo_wake_latest_event's O_NOFOLLOW
...
status_open_decisions() {  # <status-file>
  ...
  resolve=${XO_CLASSIFY_RESOLVE_VERB:-$XO_CLASSIFY_RESOLVE_VERB_DEFAULT}
  held=${XO_CLASSIFY_CAPTAIN_HELD_VERB:-$XO_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT}
  while IFS= read -r line || [ -n "$line" ]; do
    open=$(_xo_decision_fold_line "$open" "$line" "$resolve" "$held")
  done < "$f"
  printf '%s' "$open"
}

# 0 when <key> has a record in a folded "<key>\t<verb>\t<note>" open set.
_xo_open_set_has() {  # <open-set> <key>
  case "$1" in
    "$2"$'\t'*|*$'\n'"$2"$'\t'*) return 0 ;;
    *) return 1 ;;
  esac
...
_xo_open_set_verb() {  # <open-set> <key>
  local line
  while IFS= read -r line; do
    case "$line" in
...
# (xo-captain-hold.sh complete writes it only after verifying that task), so the
...
# contradiction between the two records - see xo-captain-hold.sh's `diverged`.
#
# Semantics are not re-derived here: every line goes through the same
# _xo_decision_fold_line rule the two folds use, and the reported verb is read
...
status_key_closing_verb() {  # <status-file> <key>
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    was=0
    _xo_open_set_has "$open" "$want" && was=1
    open=$(_xo_decision_fold_line "$open" "$line" "$resolve" "$held")
    if [ "$was" = 1 ] && ! _xo_open_set_has "$open" "$want"; then
      verb=$(status_line_verb "$line")
    fi
  done <<EOF
$stream
EOF
  if _xo_open_set_has "$open" "$want"; then
    _xo_open_set_verb "$open" "$want"
    return 0
  fi
  printf '%s' "$verb"
}
...
# persisted running open-set, via the exact same _xo_decision_fold_line rule
...
# XO_OPEN_DECISIONS_FOLD_VERSION must be bumped whenever
# _xo_decision_fold_line semantics change, so persisted state from an older
...
# prior cursor or the new one, never a partial one. bin/xo-wake-drain.sh calls
...
_xo_open_decisions_cursor_path() {  # <status-file>
  local f=$1 dir base
  dir=$(dirname "$f")
  base=$(basename "$f")
...
XO_OPEN_DECISIONS_FOLD_VERSION=5

# Portable device:inode identity for the rotation/recreation check below.
_xo_open_decisions_file_ident() {  # <file> -> strongest available identity
  local f=$1 epoch birth ident
  if [ -n "${XO_STATUS_IDENTITY_READER:-}" ]; then
    "$XO_STATUS_IDENTITY_READER" "$f"
    return
  fi
...
_xo_status_file_size() {  # <status-file>
  local f=$1
  if [ -n "${XO_STATUS_SIZE_READER:-}" ]; then
    "$XO_STATUS_SIZE_READER" "$f"
    return
  fi
  if [ "$(uname -s 2>/dev/null)" = Darwin ]; then
...
_xo_status_file_mtime() {  # <status-file>
  local f=$1
  if [ "$(uname -s 2>/dev/null)" = Darwin ]; then
...
_xo_status_span_scratch() {  # <status-file>
  printf '%s.span.%s' "$(_xo_open_decisions_cursor_path "$1")" "$$"
}

_xo_status_read_span() {  # <status-file> <start-offset> <byte-length>
  local f=$1 start=$2 length=$3
  if [ -n "${XO_STATUS_SPAN_READER:-}" ]; then
    "$XO_STATUS_SPAN_READER" "$f" "$start" "$length"
    return
  fi
...
status_open_decisions_incremental() {  # <status-file> [<captured-end-offset>]
  ...
  local target_cursor
  [ -f "$f" ] && [ -r "$f" ] && [ ! -L "$f" ] || return 0
  cf=$(_xo_open_decisions_cursor_path "$f")
  offset=0
  ident=''
  ...
  if [ -n "${cursor_data:-}" ]; then
      ...
      case "$first" in
        version=*)
          ...
          [ "$version" = "$XO_OPEN_DECISIONS_FOLD_VERSION" ] || version=''
          rest=${cursor_data#*$'\n'}
          offset_line=${rest%%$'\n'*}
          case "$offset_line" in
  ...
  cur_ident=$(_xo_open_decisions_file_ident "$f") || { printf '%s' "$trusted_open"; return 0; }
  [ -n "$cur_ident" ] || { printf '%s' "$trusted_open"; return 0; }
  actual_size=$(_xo_status_file_size "$f") \
  ...
  if [ "$offset" -lt "$size" ]; then
    chunk_file="$cf.read.$$"
    _xo_status_read_span "$f" "$offset" "$((size - offset))" > "$chunk_file" 2>/dev/null \
    ...
    [ -n "${XO_OPEN_DECISIONS_READ_PROBE:-}" ] \
      && printf '%s\t%s\n' "$f" "$chunk_size" >> "$XO_OPEN_DECISIONS_READ_PROBE"
    resolve=${XO_CLASSIFY_RESOLVE_VERB:-$XO_CLASSIFY_RESOLVE_VERB_DEFAULT}
    held=${XO_CLASSIFY_CAPTAIN_HELD_VERB:-$XO_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT}
    while IFS= read -r line || [ -n "$line" ]; do
      open=$(_xo_decision_fold_line "$open" "$line" "$resolve" "$held")
  ...
  if [ "$cursor_dirty" -eq 1 ]; then
    ...
    {
      printf 'version=%s\n' "$XO_OPEN_DECISIONS_FOLD_VERSION"
      printf 'offset=%s\n' "$offset"
      printf 'ident=%s\n' "$cur_ident"
...
status_presentation_snapshot() {  # <state>
  ...
  for f in "$state"/*.status; do
    ...
    task=$(basename "$f"); task="${task%.status}"
    size=$(_xo_status_file_size "$f") || return 1
    size=${size//[[:space:]]/}
    ident=$(_xo_open_decisions_file_ident "$f") || return 1
...
XO_STATUS_SNAPSHOT_EVENT_LINE=
XO_STATUS_SNAPSHOT_EVENT_MTIME=
XO_STATUS_SNAPSHOT_EVENT_ENDPOINT=
...
status_snapshot_latest_event() {  # <status-file> <captured-endpoint> <captured-identity>
  ...
  before_mtime=$(_xo_status_file_mtime "$f") || return 1
  before_size=$(_xo_status_file_size "$f") || return 1
  before_size=${before_size//[[:space:]]/}
  before_ident=$(_xo_open_decisions_file_ident "$f") || return 1
  ...
  scratch="$(_xo_status_span_scratch "$f").latest"
  _xo_status_read_span "$f" "$start" "$length" > "$scratch" 2>/dev/null \
  ...
  after_mtime=$(_xo_status_file_mtime "$f") || return 1
  after_size=$(_xo_status_file_size "$f") || return 1
  after_size=${after_size//[[:space:]]/}
  after_ident=$(_xo_open_decisions_file_ident "$f") || return 1
  ...
  XO_STATUS_SNAPSHOT_EVENT_LINE=$line
  XO_STATUS_SNAPSHOT_EVENT_MTIME=$before_mtime
  XO_STATUS_SNAPSHOT_EVENT_ENDPOINT=$event_endpoint
}
...
EOF
    ...
    legacy=$(_xo_open_decisions_cursor_path "$f")
  ...
  cur_ident=$(_xo_open_decisions_file_ident "$f") || return 1
  size=$(_xo_status_file_size "$f") || return 1
  size=${size//[[:space:]]/}
...
status_outcome_backstop_cursor_offset() {  # <status-file>
  ...
  while IFS=$(printf '\t') read -r row_task ident presented row_backstop extra; do
    ...
    if [ "$row_task" = "$task" ]; then
      current=$(_xo_open_decisions_file_ident "$f") || return 1
      size=$(_xo_status_file_size "$f") || return 1
      size=${size//[[:space:]]/}
      case "$size" in ''|*[!0-9]*) return 1 ;; esac
...
status_observed_signature() {
  ...
  if [ -z "$size" ]; then
    size=$(_xo_status_file_size "$f") || size='size-error'
    size=${size//[[:space:]]/}
    case "$size" in ''|*[!0-9]*) size='size-error' ;; esac
  fi
  if [ -z "$ident" ]; then
    ident=$(_xo_open_decisions_file_ident "$f") || ident=identity-error
...
status_presentation_marker_offset() {
  ...
  current=$(_xo_open_decisions_file_ident "$2") || { printf '0'; return 0; }
  [ "$ident" = "$current" ] || { printf '0'; return 0; }
  printf '%s' "$offset"
}
...
status_presentation_marker_commit() {
  ...
  current=$(_xo_open_decisions_file_ident "$file") || return 1
...
status_commit_presentation_snapshot() {  # <state> <snapshot>
  ...
  while IFS=$(printf '\t') read -r task endpoint ident; do
    ...
    cur_ident=$(_xo_open_decisions_file_ident "$f") || { rm -f "$tmp"; return 1; }
    size=$(_xo_status_file_size "$f") || { rm -f "$tmp"; return 1; }
    size=${size//[[:space:]]/}
...
status_open_decisions_cursor_offset() {  # <status-file>
  ...
  if [ -n "${XO_STATUS_CURSOR_SNAPSHOT_FILE:-}" ]; then
    {
      printf 'version=%s\n' "$XO_OPEN_DECISIONS_FOLD_VERSION"
      printf 'offset=%s\n' "$offset"
      printf 'ident=%s\n' "$cur_ident"
      if [ -n "$open" ]; then printf '%s' "$open"; fi
    } > "$XO_STATUS_CURSOR_SNAPSHOT_FILE" || return 1
  fi
  printf '%s' "$offset"
}
...
status_new_lines_since_cursor() {  # <status-file> [<captured-end-offset>]
  ...
  actual_size=$(_xo_status_file_size "$f") || return 1
  actual_size=${actual_size//[[:space:]]/}
...
status_line_is_unread_surface() {  # <status-line>
  ...
  key=$(_xo_decision_key "$line") || return 1
  note=$(status_line_note "$line")
  for prefix in ${XO_CLASSIFY_RESERVED_KEY_PREFIXES:-$XO_CLASSIFY_RESERVED_KEY_PREFIXES_DEFAULT}; do
    case "$key" in
      "$prefix"*)
...
# phase outrank a structured home snapshot or xo-crew-state result.
_xo_status_open_activities_stream() {
  local line verb key note resolve held open='' pause
  resolve=${XO_CLASSIFY_RESOLVE_VERB:-$XO_CLASSIFY_RESOLVE_VERB_DEFAULT}
  held=${XO_CLASSIFY_CAPTAIN_HELD_VERB:-$XO_CLASSIFY_CAPTAIN_HELD_VERB_DEFAULT}
  pause=${XO_CLASSIFY_PAUSED_VERB:-$XO_CLASSIFY_PAUSED_VERB_DEFAULT}
  while IFS= read -r line || [ -n "$line" ]; do
    # Blank-line guard; see _xo_decision_fold_line for why this is a glob.
    case "$line" in
      *[![:space:]]*) ;;
      *) continue ;;
    esac
    verb=$(status_line_verb "$line")
    key=$(_xo_decision_key "$line") || continue
    case "$verb" in
      working|"$pause")
        note=$(status_line_note "$line")
        open=$(_xo_decision_drop "$open" "$key")
...
status_open_activities() {  # <status-file-or-dash>
  ...
  [ -f "$f" ] || return 0
  _xo_status_open_activities_stream < "$f"
}

# task id from a recorded window target, falling back to the tmux-shaped
# "<session>:xo-<id>" form when no metadata state is available.
window_to_task() {
  local w=$1 state=${2:-${STATE:-${XO_STATE_OVERRIDE:-}}} meta mw mt t
  if [ -n "$state" ]; then
...
_xo_decision_origin_drop() {  # <origins> <key>
  local origin
...
_xo_status_open_decision_origins() {  # <status-file>
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    number=$((number + 1))
    after=$(_xo_decision_fold_line "$open" "$line" "$resolve" "$held")
    key=$(_xo_decision_key "$line") || { open=$after; continue; }
    verb=$(status_line_verb "$line")
    note=$(status_line_note "$line")
    case "$verb" in
      needs-decision|blocked)
        if _xo_open_set_has "$after" "$key" \
          && [ "$(_xo_open_set_verb "$after" "$key")" = "$verb" ]; then
          case "$after" in
            "$key"$'\t'"$verb"$'\t'"$note"|*$'\n'"$key"$'\t'"$verb"$'\t'"$note")
              origins=$(_xo_decision_origin_drop "$origins" "$key")
        ...
        ;;
      "$resolve"|"$held")
        _xo_open_set_has "$after" "$key" || origins=$(_xo_decision_origin_drop "$origins" "$key")
        ;;
    esac
    open=$after
  done < "$f"
...
status_span_first_actionable_record() {  # <status-file> <start-offset> [record-var] [needs-decision-var]
  ...
  local line verb key origins='' folded=0 rc=1 failed=0 prefix_lines=0 line_number=0 live_line='' events='' _line _key _xo_span_needs_decision=0
  [ -e "$f" ] || { [ -L "$f" ] && return 2; return 1; }
  [ -f "$f" ] && [ -r "$f" ] && [ ! -L "$f" ] || return 2
  ident=$(_xo_open_decisions_file_ident "$f") || return 2
  size=$(_xo_status_file_size "$f") || return 2
  size=${size//[[:space:]]/}
  ...
  scratch=$(_xo_status_span_scratch "$f") || return 2
  chunk_file="${scratch}.span"; full_file="${scratch}.full"; prefix_file="${scratch}.prefix"
  _xo_status_read_span "$f" "$start" "$((size - start))" > "$chunk_file" 2>/dev/null \
    || { rm -f "$chunk_file" "$full_file" "$prefix_file"; return 2; }
  cur_ident=$(_xo_open_decisions_file_ident "$f") || {
    rm -f "$chunk_file" "$full_file" "$prefix_file"; return 2;
  }
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    case "$verb" in
      needs-decision|blocked)
        key=$(_xo_decision_key "$line") || {
          [ -n "$events" ] && events="${events} ; "
          events="${events}${line}"
          [ "$verb" = needs-decision ] && _xo_span_needs_decision=1
          rc=0
          continue
        }
        _xo_decision_key_transition_allowed "$key" "$(status_line_note "$line")" || {
        ...
        if [ "$folded" -eq 0 ]; then
          _xo_status_read_span "$f" 0 "$size" > "$full_file" 2>/dev/null \
            || { failed=1; break; }
          if [ "$start" -gt 0 ]; then
            _xo_status_read_span "$full_file" 0 "$start" > "$prefix_file" 2>/dev/null \
              || { failed=1; break; }
            while IFS= read -r _line || [ -n "$_line" ]; do prefix_lines=$((prefix_lines + 1)); done < "$prefix_file"
          fi
          origins=$(_xo_status_open_decision_origins "$full_file") || { failed=1; break; }
          folded=1
        fi
...
EOF
        ...
        if [ "$verb" = needs-decision ] || { [ "$verb" = blocked ] &&
          _xo_is_pending_reply_escalation "$key" "$(status_line_note "$line")"; }; then
          _xo_span_needs_decision=1
        fi
        rc=0
        ;;
  ...
  if [ -n "$output_var" ]; then
    printf -v "$output_var" '%s' "$result"
    [ -z "$needs_var" ] || printf -v "$needs_var" '%s' "$_xo_span_needs_decision"
  else
    printf '%s' "$result"
  fi
  return "$rc"
...
# from bin/xo-crew-state.sh's one authoritative current-state line
...
# One xo-crew-state.sh read serves BOTH absorb reasons at once. Reading the state
# authoritatively (not the status log) is what keeps run-step precedence: a crew
# that appended paused: but then STARTED a run reports working, never paused.
# NOT a pure read: xo-crew-state.sh may make a bounded no-mistakes call, so callers
# run it only on no-verb signal and first-sighting stale paths, never every wake.
# XO_CREW_STATE_BIN lets tests stub the verdict.
crew_absorb_class() {  # <id>
  local id=$1 line state src
  [ -n "$id" ] || { printf 'none'; return; }
  line=$("$XO_CREW_STATE_BIN" "$id" 2>/dev/null) || true
...
# shared authoritative proof for no-verb signals. Where a home opts in, xo-watch.sh
...
# without the crew doing any work - .git first, so xo's own read-only git
...
XO_WORKTREE_WRITE_PRUNE=${XO_WORKTREE_WRITE_PRUNE-'.git node_modules .venv venv __pycache__ .mypy_cache .pytest_cache .ruff_cache .tox target dist build .next .cache vendor'}
XO_WORKTREE_WRITE_MAXDEPTH=${XO_WORKTREE_WRITE_MAXDEPTH:-6}
...
# gets it too.
XO_WORKTREE_WRITE_TIMEOUT=${XO_WORKTREE_WRITE_TIMEOUT:-10}
...
#
# A kind=secondmate task records a provisioned xo home, not a code tree, and
...
# that outlives XO_WORKTREE_WRITE_TIMEOUT is killed and reported as no evidence, so
...
crew_worktree_written_since() {  # <id> <state> <anchor-file>
  ...
  if [ -e "$wt/.xo-secondmate-home" ] || [ -L "$wt/.xo-secondmate-home" ]; then
    return 1
  fi
  read -r -a names <<< "$XO_WORKTREE_WRITE_PRUNE"
  for name in ${names[@]+"${names[@]}"}; do
    [ "${#prune[@]}" -eq 0 ] || prune+=( -o )
    prune+=( -name "$name" )
  done
  bound=$XO_WORKTREE_WRITE_TIMEOUT
  case "$bound" in ''|*[!0-9]*|0) bound=10 ;; esac
  if [ "${#prune[@]}" -gt 0 ]; then
    hit=$(xo_run_timed "$bound" find "$wt" -xdev -maxdepth "$XO_WORKTREE_WRITE_MAXDEPTH" \
...
# records pr= (written only by bin/xo-pr-check.sh, which validated the URL) and
# its merge poll is armed (state/<id>.check.sh exists; bin/xo-pr-lib.sh owns the
...
# is its expected shape rather than a wedge: bin/xo-crew-state.sh reports that
# gone endpoint as done, held for merge, and bin/xo-watch.sh keeps no stale or
