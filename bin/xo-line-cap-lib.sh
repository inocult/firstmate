...
# Usage: . bin/xo-line-cap-lib.sh; xo_cap_line "<line>" [<max>]
#
# ONE OWNER for the bounded-line shape both digests use. The wake digest's
# OPEN DECISIONS section (bin/xo-wake-drain.sh) and the session-start digest's
# per-task status tails (bin/xo-session-start.sh) render the same kind of
...
#
# Callers keep their own composite policy: xo-wake-drain.sh still owns the
# OPEN DECISIONS global byte cap and its "N more omitted" disclosure, and
# xo-session-start.sh still owns how many tail lines it prints per task. This
...
XO_LINE_CAP_DEFAULT=220
XO_LINE_CAP_SUFFIX=' [truncated]'

# xo_cap_line_var <line> [<max>]: put <line> in XO_LINE_CAP_LINE, cut to <max>
# characters with XO_LINE_CAP_SUFFIX in place of the tail when it is longer. A
...
xo_cap_line_var() {
  local line=$1 max=${2:-$XO_LINE_CAP_DEFAULT} keep
  if [ "${#line}" -le "$max" ]; then
    XO_LINE_CAP_LINE=$line
    return 0
  fi
  keep=$((max - ${#XO_LINE_CAP_SUFFIX}))
  [ "$keep" -ge 0 ] || keep=0
  XO_LINE_CAP_LINE="${line:0:$keep}$XO_LINE_CAP_SUFFIX"
}

# xo_cap_line <line> [<max>]: the same cut, printed on stdout, for a caller that
