#!/usr/bin/env bash
# Provision the XO_HOME selected by the fixed remote entrypoint.
#
# Usage:
#   xo-remote-home-provision.sh < manifest
#
# Manifest schema xo-remote-home-provision.v1 carries a base64 charter, the
...
# from it and re-validates it through bin/xo-project-origin-lib.sh instead of
...
# published, the durable .xo-secondmate-parent record names this home's route to its parent as
# "remote" - read by bin/xo-teardown.sh's cleanup gate so a delegated public
...
# .xo-secondmate-home marker commits the complete seed last.
...
XO_HOME=${XO_HOME:?XO_HOME is required}
MAX_MANIFEST_BYTES=1048576

# shellcheck source=bin/xo-project-origin-lib.sh
. "$SCRIPT_DIR/xo-project-origin-lib.sh"
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/xo-remote-provision.XXXXXX") || die "cannot create provisioning state"
CREATED_HOME=0
CREATED_BACKLOG=0
EXISTING_HOME=0
PUBLISHED=0
...
release_provision_lock() {
  if [ "$PROVISION_LOCK_HELD" -eq 1 ]; then
    xo_lock_release "$PROVISION_LOCK"
    PROVISION_LOCK_HELD=0
  fi
}
restore_owned_file() { # <relative-path>
  local rel=$1 dest="$XO_HOME/$1" backup="$TMP/before/$1"
  ...
  if [ "$status" -ne 0 ] && [ "$PUBLISHED" -eq 0 ]; then
    if [ "$CREATED_HOME" -eq 1 ]; then
      ...
      while IFS= read -r project; do
        [ -n "$project" ] && rm -rf -- "$XO_HOME/projects/$project"
      done < "$CREATED_PROJECTS"
      restore_owned_file data/charter.md || true
      restore_owned_file data/projects.md || true
      restore_owned_file .xo-secondmate-home || true
      restore_owned_file .xo-secondmate-parent || true
      [ "$CREATED_BACKLOG" -eq 0 ] || rm -f -- "$XO_HOME/data/backlog.md"
    fi
  fi
  release_provision_lock
  rm -rf -- "$TMP"
...
[ "$SCHEMA" = xo-remote-home-provision.v1 ] || die "incompatible provisioning manifest"
...
PROVISION_LOCK_STATE="$HOME_PARENT/.xo-provision-locks"
...
if command -v shasum >/dev/null 2>&1; then
  HOME_LOCK_KEY=$(printf '%s' "$XO_HOME" | shasum -a 256 | awk '{print $1}')
elif command -v sha256sum >/dev/null 2>&1; then
  HOME_LOCK_KEY=$(printf '%s' "$XO_HOME" | sha256sum | awk '{print $1}')
else
  die "no SHA-256 tool is available for provisioning serialization"
fi
XO_STATE_OVERRIDE="$PROVISION_LOCK_STATE"
...
xo_lock_acquire_wait "$PROVISION_LOCK"
PROVISION_LOCK_HELD=1

if [ -e "$XO_HOME" ] || [ -L "$XO_HOME" ]; then
  [ -d "$XO_HOME" ] && [ ! -L "$XO_HOME" ] || die "remote home exists but is not a safe directory"
  [ -f "$XO_HOME/AGENTS.md" ] && [ ! -L "$XO_HOME/AGENTS.md" ] \
    && [ -d "$XO_HOME/bin" ] && [ ! -L "$XO_HOME/bin" ] || die "existing remote home is not a safe XO checkout"
  for operational_dir in data state config projects; do
    operational_path="$XO_HOME/$operational_dir"
  ...
  for rel in data/charter.md data/projects.md .xo-secondmate-home .xo-secondmate-parent; do
    existing="$XO_HOME/$rel"
  ...
  if [ -f "$XO_HOME/.xo-secondmate-home" ]; then
    [ "$(cat "$XO_HOME/.xo-secondmate-home")" = "$ID" ] || die "existing remote home belongs to another secondmate"
  elif find "$XO_HOME/data" "$XO_HOME/state" "$XO_HOME/projects" -mindepth 1 -maxdepth 1 -print -quit 2>/dev/null | grep -q .; then
    die "unmarked existing remote home contains operational data"
  fi
else
  CREATED_HOME=1
  git clone --quiet -- "$XO_ROOT" "$XO_HOME" || die "could not clone the remote XO home"
fi
for operational_dir in data state config projects; do
  operational_path="$XO_HOME/$operational_dir"
  if [ -e "$operational_path" ] || [ -L "$operational_path" ]; then
...
if [ -e "$XO_HOME/data/backlog.md" ] || [ -L "$XO_HOME/data/backlog.md" ]; then
  [ -f "$XO_HOME/data/backlog.md" ] && [ ! -L "$XO_HOME/data/backlog.md" ] \
    || die "remote backlog is not a safe regular file"
else
  printf '## In flight\n\n## Queued\n\n## Done\n' > "$XO_HOME/data/backlog.md"
  CREATED_BACKLOG=1
fi
...
EOF
  ...
  xo_project_origin_safe "$ORIGIN" || die "project $NAME origin is not an accepted clone URL: $ORIGIN"
...
cp "$TMP/charter" "$XO_HOME/data/charter.md.tmp.$$"
chmod 600 "$XO_HOME/data/charter.md.tmp.$$"
mv -f -- "$XO_HOME/data/charter.md.tmp.$$" "$XO_HOME/data/charter.md"
cp "$PROJECT_REG" "$XO_HOME/data/projects.md.tmp.$$"
mv -f -- "$XO_HOME/data/projects.md.tmp.$$" "$XO_HOME/data/projects.md"
{
  printf 'schema=xo-secondmate-parent.v1\n'
  printf 'route=remote\n'
  [ -z "$PARENT_HOST" ] || printf 'parent_host=%s\n' "$PARENT_HOST"
} > "$XO_HOME/.xo-secondmate-parent.tmp.$$"
mv -f -- "$XO_HOME/.xo-secondmate-parent.tmp.$$" "$XO_HOME/.xo-secondmate-parent"
printf '%s\n' "$ID" > "$XO_HOME/.xo-secondmate-home.tmp.$$"
mv -f -- "$XO_HOME/.xo-secondmate-home.tmp.$$" "$XO_HOME/.xo-secondmate-home"
PUBLISHED=1
release_provision_lock
trap - EXIT
rm -rf -- "$TMP"
