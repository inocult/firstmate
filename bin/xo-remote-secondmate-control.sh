#!/usr/bin/env bash
# Host-local lifecycle control for the remote secondmate home selected by xo-on.
#
# Usage:
#   xo-remote-secondmate-control.sh launch <id> <harness> <model|-> <effort|-> herdr [traceparent]
#   xo-remote-secondmate-control.sh relaunch <id> <harness> <model|default|-> <effort|default|->
#   xo-remote-secondmate-control.sh state <id>
#   xo-remote-secondmate-control.sh route <id>
#   xo-remote-secondmate-control.sh send <id> <message> [fire-and-forget]
#   xo-remote-secondmate-control.sh key <id> <key>
#   xo-remote-secondmate-control.sh capture <id> [lines]
#   xo-remote-secondmate-control.sh observe <id>
#   xo-remote-secondmate-control.sh sync <id> [<parent-commit>]
#   xo-remote-secondmate-control.sh update <id>
#   xo-remote-secondmate-control.sh retire <id> [--force]
#
# Remote placement ends here, but the second-mate agent always runs on the
# Herdr backend in the dedicated xo-remote session, so launch refuses any other
# selection rather than reading this home's config/backend. The interactive
# default session remains for the user's work.
# xo-spawn/xo-send/xo-teardown keep owning the local endpoint mechanics.
# The home's own workers keep their ordinary backend selection.
# bin/xo-remote-doctor.sh owns that host's readiness for Herdr.
...
# this host's XO copy happens to hold. Omitting <parent-commit> targets
# this host's own code-root HEAD instead, which is what /updatexo wants
...
# bin/xo-ff-lib.sh, so the clean, ancestry, and branch guards have a single owner.
...
# stops xo-remote or removes a sibling secondmate's workspace or panes.
#
...
# pane, and xo-spawn validates it (bin/xo-trace-context-lib.sh). Omitting it is
...
TARGET_HOME=${XO_HOME:?XO_HOME is required}
...
validate_home() { # <id> [allow-absent]
  ...
  [ -f "$TARGET_HOME/.xo-secondmate-home" ] && [ ! -L "$TARGET_HOME/.xo-secondmate-home" ] \
    || die "remote home is not a seeded secondmate home"
  marker=$(cat "$TARGET_HOME/.xo-secondmate-home")
  [ "$marker" = "$id" ] || die "remote home belongs to $marker, not $id"
  [ -f "$TARGET_HOME/AGENTS.md" ] && [ -d "$TARGET_HOME/bin" ] || die "remote home is not an XO checkout"
}
...
remote_endpoint_load() {
  ...
  if ! xo_backend_validate_task_endpoint "$REMOTE_ENDPOINT_META" "$id" 2>/dev/null; then
    REMOTE_ENDPOINT_ERROR="remote secondmate $id endpoint metadata is invalid; refusing access until it is explicitly migrated"
    return 1
  fi
  REMOTE_ENDPOINT_BACKEND=$XO_BACKEND_VALIDATED_BACKEND
  REMOTE_ENDPOINT_TARGET=$XO_BACKEND_VALIDATED_TARGET
  ...
  herdr_session=$(xo_backend_meta_exact_value "$REMOTE_ENDPOINT_META" herdr_session 2>/dev/null || true)
...
state_value() { # <id>; prints recovery-grade state
  ...
  xo_backend_agent_state "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" 2>/dev/null || printf 'unreadable\n'
}

print_route() { # <id>
  local id=$1 harness traceparent
  remote_endpoint_require "$id"
  harness=$(xo_meta_get "$REMOTE_ENDPOINT_META" harness)
  traceparent=$(xo_meta_get "$REMOTE_ENDPOINT_META" traceparent)
...
cmd_launch() {
  ...
  # a remote route depends on. bin/xo-remote-doctor.sh is the readiness owner.
  ...
  if [ -f "$meta" ]; then
    remote_endpoint_require "$id"
    current=$(xo_backend_agent_state "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" 2>/dev/null || printf 'unreadable\n')
    case "$current" in
      alive)
        print_route "$id"
        return 0
        ;;
      dead)
        xo_backend_kill "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" 2>/dev/null \
  ...
  # XO copy, which would target the wrong checkout.
  ...
  if ! out=$(HERDR_SESSION="$REMOTE_HERDR_SESSION" XO_HOME="$XO_ROOT" XO_ROOT_OVERRIDE="$XO_ROOT" \
    XO_STATE_OVERRIDE="$CONTROL_STATE" XO_DATA_OVERRIDE="$CONTROL_DATA" \
    XO_CONFIG_OVERRIDE="$TARGET_HOME/config" XO_SKIP_SECONDMATE_INHERIT=1 \
    XO_SKIP_SECONDMATE_SYNC=1 \
    "$SCRIPT_DIR/xo-spawn.sh" "${ARGS[@]}" 2>&1); then
    [ -z "$out" ] || printf '%s\n' "$out" >&2
    die "remote host-local secondmate launch failed"
  fi
  [ -f "$meta" ] || die "remote launch returned without endpoint metadata"
  herdr_session=$(xo_meta_get "$meta" herdr_session)
...
# was written by a host-local xo-spawn and carries no remote_host= field, so
# bin/xo-control.sh's remote refusal never fires, and every checkpoint, journal,
...
cmd_relaunch() {
  ...
  # the dedicated xo-remote session, and the parent already owns both convergence
  # legs, so the host-local spawn must not re-sync or re-inherit against this
  # host's own XO copy.
  HERDR_SESSION="$REMOTE_HERDR_SESSION" XO_HOME="$XO_ROOT" XO_ROOT_OVERRIDE="$XO_ROOT" \
    XO_STATE_OVERRIDE="$CONTROL_STATE" XO_DATA_OVERRIDE="$CONTROL_DATA" \
    XO_CONFIG_OVERRIDE="$TARGET_HOME/config" XO_SKIP_SECONDMATE_INHERIT=1 \
    XO_SKIP_SECONDMATE_SYNC=1 \
    "$SCRIPT_DIR/xo-control.sh" "${control_args[@]}"
}
...
cmd_send() {
  ...
  validate_home "$id"
  meta=$(meta_path "$id")
  meta_lock=$(xo_meta_lock_path "$meta") || die "remote secondmate metadata lock path is invalid"
  xo_task_inbox_lock_acquire "$meta_lock" \
  ...
  # best-effort (bin/xo-task-inbox-lib.sh owns the record and doorbell). The
  ...
  if ! rec=$(xo_task_inbox_write_idempotent "$CONTROL_STATE" "$id" "$message" "$delivery_mode"); then
    xo_lock_release "$meta_lock"
  ...
  xo_task_inbox_ring "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" "$rec" "xo-$id" || ring_rc=$?
  case "$ring_rc" in
...
cmd_key() {
  ...
  validate_home "$id"
  remote_endpoint_require "$id"
  XO_HOME="$TARGET_HOME" XO_ROOT_OVERRIDE="$XO_ROOT" XO_STATE_OVERRIDE="$TARGET_HOME/state" \
    "$SCRIPT_DIR/xo-send.sh" "$REMOTE_ENDPOINT_TARGET" --key "$key"
}
...
cmd_capture() {
  ...
  remote_endpoint_require "$id"
  xo_backend_capture "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" "$lines" "xo-$id" | head -c 65536
}
...
cmd_observe() {
  ...
  remote_endpoint_require "$id"
  harness=$(xo_meta_get "$REMOTE_ENDPOINT_META" harness)
  xo_pending_reply_backend_observation "$REMOTE_ENDPOINT_BACKEND" "$REMOTE_ENDPOINT_TARGET" "xo-$id" "$harness"
  printf '\n'
}

# Make <commit> readable in this home's own object store without moving any other
# checkout. Ordered by cost: already present, then this host's XO copy (a
...
import_home_commit() { # <home> <commit>
  ...
  if git -C "$home" fetch --quiet --no-tags -- "$XO_ROOT" "$commit" 2>/dev/null \
...
cmd_sync() {
  ...
  if [ "$#" -ge 2 ]; then
    ...
    commit=$(git -C "$XO_ROOT" rev-parse HEAD 2>/dev/null) || die "remote code root HEAD is unreadable"
  fi
  import_home_commit "$TARGET_HOME" "$commit" \
    || die "remote home could not import $commit from this host's XO copy or the home's origin; run /updatexo to refresh this host's copy, or push that commit first"
  # ff_target publishes its verdict in FF_STATUS, so it must run in THIS shell.
  report=$(mktemp "${TMPDIR:-/tmp}/xo-remote-sync.XXXXXX") || die "cannot stage the sync report"
...
cmd_update() {
  ...
  validate_id "$id"
  validate_home "$id"
  if ! update_out=$(XO_HOME="$XO_ROOT" XO_ROOT_OVERRIDE="$XO_ROOT" \
    "$SCRIPT_DIR/xo-update.sh" 2>&1); then
    [ -z "$update_out" ] || printf '%s\n' "$update_out" >&2
    die "remote code root update failed"
  fi
  root_status=$(printf '%s\n' "$update_out" | grep '^xo:' | tail -1)
  case "$root_status" in
    'xo: updated '*|'xo: already current'*) ;;
    *)
      [ -z "$update_out" ] || printf '%s\n' "$update_out" >&2
...
cmd_retire() {
  ...
  remote_endpoint_require "$id"
  XO_HOME="$TARGET_HOME" XO_ROOT_OVERRIDE="$XO_ROOT" XO_STATE_OVERRIDE="$TARGET_HOME/state" \
    XO_CONFIG_OVERRIDE="$TARGET_HOME/config" "$SCRIPT_DIR/xo-guard.sh" || true
  if [ -n "$force" ]; then
    XO_HOME="$XO_ROOT" XO_ROOT_OVERRIDE="$XO_ROOT" \
      XO_STATE_OVERRIDE="$CONTROL_STATE" XO_DATA_OVERRIDE="$CONTROL_DATA" \
      XO_CONFIG_OVERRIDE="$TARGET_HOME/config" XO_TEARDOWN_GUARD_DONE=1 \
      "$SCRIPT_DIR/xo-teardown.sh" "$id" --force
  else
    XO_HOME="$XO_ROOT" XO_ROOT_OVERRIDE="$XO_ROOT" \
      XO_STATE_OVERRIDE="$CONTROL_STATE" XO_DATA_OVERRIDE="$CONTROL_DATA" \
      XO_CONFIG_OVERRIDE="$TARGET_HOME/config" XO_TEARDOWN_GUARD_DONE=1 \
      "$SCRIPT_DIR/xo-teardown.sh" "$id"
  fi
}
