...
#
# A stray persistent top-level `cd projects/<clone>` in the PRIMARY xo
# shell silently relocates the shell, so a later xo-owned command (a
# backlog write, an xo-* lifecycle call, tasks-axi) runs inside a project clone
# instead of the home. This seatbelt denies such a command before it runs.
# bin/xo-cd-command-policy.mjs is the sole owner of the block/allow decision; it
# reuses the shell classifier owned by bin/xo-arm-command-policy.mjs. This
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/xo-cd-pretool-check.sh
#   bin/xo-cd-pretool-check.sh --command '<cmd>'
#
...
#
# Exit/output contract (identical shape to bin/xo-arm-pretool-check.sh):
...
usage() {
  cat <<'EOF'
Usage: xo-cd-pretool-check.sh [--command <cmd>] [--claude|--cursor]

With no --command, reads a PreToolUse-style JSON payload on stdin (Grok
toolInput.command, or Claude/Codex tool_input.command).
Fires only in the real primary xo checkout; it is a silent no-op in a
crewmate/scout task worktree or any non-xo repo.
...
if [ "$CMD_SET" -eq 0 ]; then
  ...
  . "$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)/xo-hook-host-lib.sh"
...
# decoder set in bin/xo-arm-command-policy.mjs: adding any new quote/expansion
...
# Scope to a plain, non-worktree xo checkout, where git-dir equals
# git-common-dir. A crewmate/scout task worktree - the shape bin/xo-spawn.sh
# always hands out - is a linked git worktree where the two differ. This guard
# does not inspect .xo-secondmate-home, so it applies in a git-cloned secondmate
...
[ -f "$XO_ROOT/AGENTS.md" ] || exit 0
[ -d "$XO_ROOT/bin" ] || exit 0
command -v git >/dev/null 2>&1 || exit 0
GIT_DIR=$(git -C "$XO_ROOT" rev-parse --git-dir 2>/dev/null) || exit 0
GIT_COMMON_DIR=$(git -C "$XO_ROOT" rev-parse --git-common-dir 2>/dev/null) || exit 0
[ "$GIT_DIR" = "$GIT_COMMON_DIR" ] || exit 0

POLICY="$XO_ROOT/bin/xo-cd-command-policy.mjs"
