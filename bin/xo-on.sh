#!/usr/bin/env bash
# Execute one tracked XO command in a configured remote secondmate home.
#
# Usage:
#   xo-on.sh [--stdin] <secondmate-id|unambiguous-ssh-alias> <xo-command> [args...]
#
# Routes come only from remote records in data/secondmates.md. A record names an
# SSH config alias, remote XO code root, and remote XO_HOME. A host alias
...
# bin/xo-*.sh namespace. No per-command table exists.
#
# argv is encoded as one NUL-delimited stream and passed through the fixed
# xo-remote-entrypoint.sh. The remote command's stdin is /dev/null by default,
...
# killed. XO_SSH_ALIVE_INTERVAL and XO_SSH_ALIVE_COUNT_MAX override the
...
case "$COMMAND" in
  xo-*.sh) ;;
  *) die "remote command must be a basename in the xo-*.sh namespace: $COMMAND" ;;
esac
case "$COMMAND" in */*|*..*) die "remote command must not contain a path or traversal: $COMMAND" ;; esac
LOCAL_COMMAND="$XO_ROOT/bin/$COMMAND"
[ -f "$LOCAL_COMMAND" ] && [ ! -L "$LOCAL_COMMAND" ] && [ -x "$LOCAL_COMMAND" ] \
  || die "remote command is not a genuine tracked executable in this XO checkout: $COMMAND"
git -C "$XO_ROOT" ls-files --error-unmatch "bin/$COMMAND" >/dev/null 2>&1 \
  || die "remote command is not tracked by this XO checkout: $COMMAND"
...
SSH_BIN=${XO_SSH_BIN:-ssh}
ALIVE_INTERVAL=${XO_SSH_ALIVE_INTERVAL:-15}
ALIVE_COUNT_MAX=${XO_SSH_ALIVE_COUNT_MAX:-3}
case "$ALIVE_INTERVAL" in ''|*[!0-9]*) die "XO_SSH_ALIVE_INTERVAL must be a positive integer: $ALIVE_INTERVAL" ;; esac
case "$ALIVE_COUNT_MAX" in ''|*[!0-9]*) die "XO_SSH_ALIVE_COUNT_MAX must be a positive integer: $ALIVE_COUNT_MAX" ;; esac
[ "$ALIVE_INTERVAL" -gt 0 ] || die "XO_SSH_ALIVE_INTERVAL must be a positive integer: $ALIVE_INTERVAL"
[ "$ALIVE_COUNT_MAX" -gt 0 ] || die "XO_SSH_ALIVE_COUNT_MAX must be a positive integer: $ALIVE_COUNT_MAX"
...
SSH_ARGS=(
  ...
  -- "$HOST" xo-remote-entrypoint.sh "$PROTOCOL" "$ROOT_B64" "$HOME_B64" "$ARGV_B64"
)
