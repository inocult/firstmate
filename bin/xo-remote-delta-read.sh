...
#
# Usage:
#   xo-remote-delta-read.sh <relative-log> <offset> <prefix-sha256> [wait-seconds]
#
...
# that preemption as distinct exit 76. The bin/xo-remote-job-lib.sh header owns
# that contract.
set -eu

XO_HOME=${XO_HOME:?XO_HOME is required}
MAX_BYTES=${XO_REMOTE_DELTA_MAX_BYTES:-65536}
POLL_SECONDS=${XO_REMOTE_DELTA_POLL_SECONDS:-0.2}
...
resolve_log() { # <relative-path>
  ...
  parent=$(dirname "$rel")
  base=$(basename "$rel")
  parent_real=$(CDPATH='' cd -- "$XO_HOME/$parent" 2>/dev/null && pwd -P) || die "log parent is unavailable: $rel"
  case "$parent_real" in "$home_real"|"$home_real"/*) ;; *) die "log escapes XO_HOME: $rel" ;; esac
...
case "$MAX_BYTES" in ''|*[!0-9]*|0) die "XO_REMOTE_DELTA_MAX_BYTES must be a positive integer" ;; esac
[ "$MAX_BYTES" -le 1048576 ] || die "XO_REMOTE_DELTA_MAX_BYTES exceeds the safety bound"

LOG=$(resolve_log "$REL")
TMP=$(mktemp -d "${TMPDIR:-/tmp}/xo-remote-delta.XXXXXX") || die "cannot create delta staging directory"
trap 'rm -rf -- "$TMP"' EXIT
trap 'exit 75' TERM
: > "$TMP/empty"
