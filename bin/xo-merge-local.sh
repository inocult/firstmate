...
# project's default branch to the crewmate's xo/<id> branch.
#
# This is xo's merge gate-action (the captain's merge authority applied
...
# Usage: xo-merge-local.sh <task-id>
set -eu
...
xo_backlog_directory_present "$STATE" "state directory" || {
  echo "error: local merge refused: $XO_BACKLOG_TRANSITION_ERROR" >&2
  exit 1
}
META="$STATE/$ID.meta"
...
# branch reports readiness and never lands (contract: bin/xo-lease-lib.sh;
...
xo_lease_forbid_branch "local-only landing (xo-merge-local)"
...
[ "$MODE" = local-only ] || { echo "error: task $ID is mode=$MODE, not local-only; merge PR tasks with bin/xo-pr-merge.sh <id> <PR url> after approval" >&2; exit 1; }
...
# fast-forward lands predictably (xo never writes here otherwise).
