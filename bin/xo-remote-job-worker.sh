#!/bin/bash
# Long-lived per-account worker for remote xo-on jobs.
#
# This process is launched by the XO-owned dev.xo.remote-job
# LaunchAgent on macOS and by a detached restart supervisor on Linux. It claims
# only complete 0700 records staged by xo-remote-job-lib.sh under the fixed
# account queue, refuses symlinks and malformed records, and executes only a
# tracked non-symlink xo-*.sh under this worker's configured XO_ROOT/bin.
#
# Each child runs under env -i with the shared filesystem-composed PATH, HOME,
# XO_HOME, XO_ROOT_OVERRIDE, and XO_REMOTE_JOB_ACTIVE=1. Commands receive their
...
#
# The worker is abandoned when its configured XO_ROOT stops being a genuine
# XO checkout - the state a pruned no-mistakes gate worktree, a returned
...
# reparented to init. XO_REMOTE_JOB_ORPHAN_GRACE_SECONDS is how long the root
...
# XO_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS and gives up after
# XO_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS failed children in total, since a
# restart loop only burns CPU and grows its log without bound. A child that
# stays up for XO_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS clears the
...
# either. xo-on's ensure path restarts a worker that gave up.
set -u
...
XO_REMOTE_JOB_ORPHAN_GRACE_SECONDS=$(worker_bounded_setting "${XO_REMOTE_JOB_ORPHAN_GRACE_SECONDS:-}" 5)
XO_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS=$(worker_bounded_setting "${XO_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS:-}" 20)
XO_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS=$(worker_bounded_setting "${XO_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS:-}" 5)
XO_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS=$(worker_bounded_setting "${XO_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS:-}" 10)

SCRIPT_DIR=$(CDPATH='' cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)
XO_ROOT=${XO_ROOT_OVERRIDE:-$(CDPATH='' cd "$SCRIPT_DIR/.." && pwd -P)}

# shellcheck source=bin/xo-remote-job-lib.sh
. "$SCRIPT_DIR/xo-remote-job-lib.sh"
...
worker_account_home() {
  local home=${HOME:-}
  if [ -n "$home" ]; then
    xo_remote_job_canonical_existing_dir "$home" && return 0
  fi
  unset HOME
...
worker_write_heartbeat() {
  ...
  tmp=$(umask 077; mktemp "$XO_REMOTE_JOB_STATE/.ready.XXXXXX") || return 1
...
worker_publish_pid() {
  ...
  tmp=$(umask 077; mktemp "$XO_REMOTE_JOB_STATE/.pid.XXXXXX") || return 1
  printf '%s\n' "${BASHPID:-$$}" > "$tmp" || { rm -f -- "$tmp"; return 1; }
  chmod 600 "$tmp" || { rm -f -- "$tmp"; return 1; }
  mv -f -- "$tmp" "$pid_file"
}
...
worker_publish_identity() {
  ...
  identity=$(xo_remote_job_code_identity "$XO_ROOT" "$account_home") || return 1
  identity_file=$(xo_remote_job_worker_identity_path)
  tmp=$(umask 077; mktemp "$XO_REMOTE_JOB_STATE/.identity.XXXXXX") || return 1
...
worker_publish_lock_owner() {
  ...
  pid=${BASHPID:-$$}
  start=$(xo_remote_job_process_start "$pid") || return 1
  command=$(xo_remote_job_process_command "$pid") || return 1
...
worker_lock_recent() {
  local mtime now
  mtime=$(xo_remote_job_path_mtime "$WORKER_LOCK" 2>/dev/null || true)
...
worker_quarantined_execution_stopped() { # <account-home>
  ...
  xo_remote_job_regular_bounded "$WORKER_LOCK/quarantine" 256 || return 1
  xo_remote_job_lock_owner_matches_process "$account_home" && return 1
...
worker_acquire_lock() {
  ...
  while [ "$attempt" -lt 150 ]; do
    ...
    if xo_remote_job_lock_owner_matches_process "$account_home"; then return 2; fi
    if xo_remote_job_probe "$account_home" || worker_lock_recent; then
      attempt=$((attempt + 1))
      sleep 0.1
      continue
    fi
...
worker_cleanup() {
  ...
  owner_pid=$(xo_remote_job_read_single_line "$WORKER_LOCK/pid" 64 2>/dev/null || true)
  if [ -z "$owner_pid" ]; then
  ...
  pid_file=$(xo_remote_job_worker_pid_path)
  ready=$(xo_remote_job_worker_ready_path)
  identity=$(xo_remote_job_worker_identity_path)
...
worker_code_root_abandoned() {
  local waited=0
  xo_remote_job_root_is_live "$XO_ROOT" && return 1
  while [ "$waited" -lt "$XO_REMOTE_JOB_ORPHAN_GRACE_SECONDS" ]; do
    sleep 1
    waited=$((waited + 1))
    xo_remote_job_root_is_live "$XO_ROOT" && return 1
  done
...
worker_read_process_id() { # <file>
  local file=$1 pid
  xo_remote_job_regular_bounded "$file" 64 || return 1
  pid=$(tr -d '\n' < "$file")
...
worker_supervisor_identity_status() { # <job-dir> <pid>
  ...
  recorded_start=$(xo_remote_job_read_single_line "$job/.claim/supervisor_start" 256 2>/dev/null) || return 2
  actual_start=$(xo_remote_job_process_start "$pid" 2>/dev/null) || {
...
worker_group_identity_status() { # <job-dir> <pid>
  ...
  recorded_start=$(xo_remote_job_read_single_line "$file" 256 2>/dev/null) || return 2
  actual_start=$(xo_remote_job_process_start "$pid" 2>/dev/null) || {
...
worker_claim() { # <job-dir>
  ...
  pid=${BASHPID:-$$}
  start=$(xo_remote_job_process_start "$pid") || { rmdir "$claim" 2>/dev/null || true; return 1; }
...
worker_claim_owner_alive() { # <job-dir>
  ...
  owner="$claim/owner"
  xo_remote_job_regular_bounded "$owner" 64 || return 1
  pid=$(tr -d '\n' < "$owner")
  case "$pid" in ''|*[!0-9]*) return 1 ;; esac
  if [ -e "$claim/owner_start" ] || [ -L "$claim/owner_start" ]; then
    recorded_start=$(xo_remote_job_read_single_line "$claim/owner_start" 256 2>/dev/null) || return 1
    actual_start=$(xo_remote_job_process_start "$pid" 2>/dev/null) || return 1
...
worker_clear_dead_claim() { # <job-dir>
  ...
  xo_remote_job_remove_claim_records "$claim" || return 1
  rmdir "$claim"
}
...
worker_reclaim_running_job() { # <job-dir>
  ...
  state=$(xo_remote_job_read_state "$job" 2>/dev/null) || return 1
...
worker_read_text() { # <job-dir> <field> <max>
  ...
  xo_remote_job_regular_bounded "$job/$field" "$max" || return 1
...
worker_publish_result() { # <job-dir> <exit>
  ...
  for tmp in stdout stderr; do
    xo_remote_job_regular_bounded "$job/$tmp" "$XO_REMOTE_JOB_MAX_BYTES" || return 1
  done
  tmp=$(umask 077; mktemp "$job/.exit.XXXXXX") || return 1
  printf '%s\n' "$exit_status" > "$tmp" || { rm -f -- "$tmp"; return 1; }
  chmod 600 "$tmp" || { rm -f -- "$tmp"; return 1; }
  mv -f -- "$tmp" "$job/exit" || { rm -f -- "$tmp"; return 1; }
  xo_remote_job_write_state "$job" 'done' || return 1
  if xo_remote_job_cancelled "$job"; then
    account_home=$(worker_account_home 2>/dev/null || true)
    if [ -n "$account_home" ]; then
      xo_remote_job_reap "$account_home" "${job##*/}" 2>/dev/null || true
    fi
  fi
}
...
worker_run_with_timeout() { # <job-dir> <seconds> <command> [args...]
  ...
  group_pid=$!
  set +m
  group_start=$(xo_remote_job_process_start "$group_pid") || {
  ...
  [ "$WORKER_PREEMPTED" -eq 0 ] || return "$XO_REMOTE_JOB_PREEMPTED_EXIT"
  return "$rc"
}

worker_job_command() { # <job-dir>; the first argv element of a staged record
  local job=$1 first=
  xo_remote_job_regular_bounded "$job/argv" "$XO_REMOTE_JOB_MAX_BYTES" || return 1
...
worker_preempting_waiter_exists() { # <lane-home>
  ...
  for job in "$XO_REMOTE_JOB_JOBS"/job-*; do
    ...
    xo_remote_job_cancelled "$job" && continue
    ...
    xo_remote_job_command_preemptible "$command" || return 0
  done
  return 1
}
...
worker_capture_output() { # <fifo> <destination>
  ...
  {
    head -c "$XO_REMOTE_JOB_MAX_BYTES"
    cat >/dev/null
  } < "$fifo" > "$destination"
}
...
worker_run_job() { # <account-home> <job-dir>
  ...
  root=$(xo_remote_job_canonical_existing_dir "$root") || { worker_publish_result "$job" 126; return; }
  home=$(xo_remote_job_canonical_home "$home") || { worker_publish_result "$job" 126; return; }
  ...
  [ "$root" = "$XO_ROOT" ] || { worker_publish_result "$job" 126; return; }
  [ -f "$root/AGENTS.md" ] && [ ! -L "$root/AGENTS.md" ] &&
    [ -d "$root/bin" ] && [ ! -L "$root/bin" ] || { worker_publish_result "$job" 126; return; }
  xo_remote_job_regular_bounded "$job/argv" "$XO_REMOTE_JOB_MAX_BYTES" || { worker_publish_result "$job" 126; return; }
  xo_remote_job_regular_bounded "$job/stdin" "$XO_REMOTE_JOB_MAX_BYTES" || { worker_publish_result "$job" 126; return; }
  deadline=$(xo_remote_job_read_deadline "$job") || { worker_publish_result "$job" 126; return; }
  ...
  command=${argv[0]}
  case "$command" in xo-*.sh) ;; *) worker_publish_result "$job" 126; return ;; esac
  case "$command" in */*|*..*) worker_publish_result "$job" 126; return ;; esac
  if xo_remote_job_command_preemptible "$command"; then preemptible=1; fi
  ...
  xo_remote_job_build_child_path "$root" >/dev/null
  ...
  child_env=(
    /usr/bin/env -i
    "PATH=$XO_REMOTE_JOB_CHILD_PATH"
    "HOME=$account_home"
    "XO_HOME=$home"
    "XO_ROOT_OVERRIDE=$root"
    XO_REMOTE_JOB_ACTIVE=1
  )
  if [ -n "${XO_REMOTE_JOB_PLATFORM_OVERRIDE:-}" ]; then
    child_env+=("XO_REMOTE_JOB_PLATFORM_OVERRIDE=$XO_REMOTE_JOB_PLATFORM_OVERRIDE")
  fi
  remaining=$((deadline - $(date +%s)))
  [ "$remaining" -gt 0 ] || {
...
worker_finalize_cancelled() { # <account-home> <job-dir>
  ...
  xo_remote_job_reap "$account_home" "${job##*/}" || true
}
...
worker_lane_execute() { # <account-home> <job-dir>
  ...
  supervisor_start=$(xo_remote_job_process_start "$supervisor_pid") || {
  ...
  queue_deadline=$(xo_remote_job_read_number "$job" queue_deadline 2>/dev/null || true)
  ...
  timeout=$(xo_remote_job_read_number "$job" timeout 2>/dev/null || true)
  ...
  xo_remote_job_write_number "$job" deadline "$deadline" || {
    worker_publish_result "$job" 125 || true
    return 0
  }
  xo_remote_job_write_state "$job" running || {
    worker_publish_result "$job" 125 || true
    return 0
  }
...
worker_start_lane() { # <job-dir> <home>
  ...
  "$SCRIPT_DIR/xo-remote-job-worker.sh" --lane "${job##*/}" &
  lane_pid=$!
  lane_start=$(xo_remote_job_process_start "$lane_pid" 2>/dev/null || true)
...
worker_lane_main() { # <job-id>
  ...
  xo_remote_job_safe_id "$1" || { worker_error "invalid lane job id"; exit 2; }
  account_home=$(worker_account_home) || { worker_error "cannot resolve account home"; exit 1; }
  XO_ROOT=$(xo_remote_job_canonical_existing_dir "$XO_ROOT") || { worker_error "configured XO_ROOT is unsafe"; exit 1; }
  xo_remote_job_prepare_state "$account_home" || { worker_error "$XO_REMOTE_JOB_ERROR"; exit 1; }
  job=$(xo_remote_job_job_dir "$1" 2>/dev/null) || exit 0
...
worker_process_once() { # <account-home>
  ...
  for job in "$XO_REMOTE_JOB_JOBS"/job-*; do
    ...
    job=$(xo_remote_job_job_dir "$id" 2>/dev/null || true)
    [ -n "$job" ] || continue
    state=$(xo_remote_job_read_state "$job" 2>/dev/null || true)
    case "$state" in
      queued)
        ...
        seq=$(xo_remote_job_read_number "$job" seq 2>/dev/null || true)
  ...
  while IFS=$'\t' read -r seq id home; do
    ...
    job=$(xo_remote_job_job_dir "$id" 2>/dev/null || true)
    [ -n "$job" ] || continue
    [ "$(xo_remote_job_read_state "$job" 2>/dev/null || true)" = queued ] || continue
...
main() {
  ...
  account_home=$(worker_account_home) || { worker_error "cannot resolve account home"; exit 1; }
  XO_ROOT=$(xo_remote_job_canonical_existing_dir "$XO_ROOT") || { worker_error "configured XO_ROOT is unsafe"; exit 1; }
  [ -f "$XO_ROOT/AGENTS.md" ] && [ ! -L "$XO_ROOT/AGENTS.md" ] || { worker_error "XO_ROOT is not an XO checkout"; exit 1; }
  xo_remote_job_prepare_state "$account_home" || { worker_error "$XO_REMOTE_JOB_ERROR"; exit 1; }
  WORKER_LOCK=$(xo_remote_job_worker_lock_path)
  ...
  while :; do
    ...
    if worker_code_root_abandoned; then
      worker_error "configured XO_ROOT $XO_ROOT no longer exists; stopping the abandoned worker"
      exit 0
    fi
    worker_reap=0
    if [ "$worker_reap" -eq 0 ]; then
      xo_remote_job_reap_stale "$account_home" || true
      worker_reap=1
    fi
...
worker_supervisor_cleanup_dead_child() { # <account-home> <pid>
  ...
  recorded=$(xo_remote_job_read_single_line "$lock/pid" 64) || return 1
...
worker_supervise_linux() {
  ...
  XO_ROOT=$(xo_remote_job_canonical_existing_dir "$XO_ROOT") || { worker_error "configured XO_ROOT is unsafe"; return 1; }
  [ -f "$XO_ROOT/AGENTS.md" ] && [ ! -L "$XO_ROOT/AGENTS.md" ] || { worker_error "XO_ROOT is not an XO checkout"; return 1; }
  xo_remote_job_prepare_state "$account_home" || { worker_error "$XO_REMOTE_JOB_ERROR"; return 1; }
  trap worker_supervisor_shutdown HUP INT TERM
  while :; do
    if worker_code_root_abandoned; then
      worker_error "configured XO_ROOT $XO_ROOT no longer exists; stopping the abandoned worker supervisor"
      return 0
    fi
    started=$SECONDS
    "$SCRIPT_DIR/xo-remote-job-worker.sh" --serve &
    WORKER_SUPERVISED_PID=$!
    ...
    WORKER_SUPERVISED_PID=
    restarts=$((restarts + 1))
    if [ "$restarts" -ge "$XO_REMOTE_JOB_SUPERVISOR_MAX_RESTARTS" ]; then
      worker_error "remote job worker exited $restarts times; stopping the supervisor"
      return 1
    fi
    if [ $((SECONDS - started)) -ge "$XO_REMOTE_JOB_SUPERVISOR_HEALTHY_SECONDS" ]; then
      failures=0
      sleep 0.1
      continue
    fi
    failures=$((failures + 1))
    backoff=$failures
    [ "$backoff" -le "$XO_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS" ] ||
      backoff=$XO_REMOTE_JOB_SUPERVISOR_MAX_BACKOFF_SECONDS
    sleep "$backoff"
  done
}
...
case "${1:-}" in
  ...
  --lane)
    ...
    ;;
  '')
    if [ "$(xo_remote_job_platform)" = linux ]; then worker_supervise_linux; else main; fi
    ;;
