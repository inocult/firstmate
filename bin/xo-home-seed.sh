...
#
# Usage:
#   xo-home-seed.sh <id> <home|-> {<project>...|--no-projects}
#       Provision <home> as an isolated xo home. If <home> is "-", acquire
#       a fresh xo worktree via "treehouse get --lease", which durably
...
#       subject is the xo repo itself; it is mutually exclusive with a
...
#       initialized, an ignored .xo-secondmate-parent binding is published before
#       the .xo-secondmate-home identity marker, and data/secondmates.md is updated.
...
#       Set XO_SECONDMATE_CHARTER='<charter>' to seed from inline charter text
#       when no filled charter brief exists. Set XO_SECONDMATE_SCOPE='<scope>'
#       to override the registry routing scope. Otherwise the registry summary
#       and scope are derived from the filled charter brief.
#   xo-home-seed.sh validate
...
# shellcheck source=bin/xo-secondmate-charter-lib.sh
. "$SCRIPT_DIR/xo-secondmate-charter-lib.sh"
# shellcheck source=bin/xo-wake-lib.sh
. "$SCRIPT_DIR/xo-wake-lib.sh"

usage() {
  echo "usage: xo-home-seed.sh <id> <home|-> {<project>...|--no-projects}" >&2
  echo "       xo-home-seed.sh validate" >&2
}
...
refuse_active_home_path() {
  ...
  abs_active_home=$(resolved_path "$XO_HOME")
  abs_root=$(resolved_path "$XO_ROOT")
  if [ "$abs_home" = "/" ]; then
...
validate_existing_parent_binding() {
  ...
  xo_secondmate_parent_record_parse "$record" || return 0
  [ "$XO_SECONDMATE_PARENT_ROUTE" = local ] || return 0

  recorded_parent=$(resolved_path "$XO_SECONDMATE_PARENT_HOME")
  requested_parent=$(resolved_path "$XO_HOME")
...
validate_project_destination() {
  ...
  if [ "$abs_dst" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the active xo home: $dst" >&2
    return 1
  fi
  if [ "$abs_dst" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dst"; then
    echo "error: seeded project $project destination cannot be inside the xo repo: $dst" >&2
    return 1
  fi
  printf '%s\n' "$abs_dst"
}
...
acquire_treehouse_home() {
  local id=$1 home
  # Durably lease an xo worktree from the pool. The lease persists with no
  ...
  home=$(cd "$XO_ROOT" && treehouse get --lease --lease-holder "$id") || {
    echo "error: treehouse get --lease failed to lease an xo home" >&2
    return 1
  }
  [ -n "$home" ] || { echo "error: treehouse get --lease did not report an xo home" >&2; return 1; }
  printf '%s\n' "$home"
}
...
verify_xo_home() {
  local home=$1
  refuse_active_home_path "$home" || return 1
  [ -f "$home/AGENTS.md" ] || { echo "error: $home is not an xo home (missing AGENTS.md)" >&2; return 1; }
  [ -d "$home/bin" ] || { echo "error: $home is not an xo home (missing bin/)" >&2; return 1; }
...
clone_project() {
  ...
  read -r mode _ <<EOF
$(XO_HOME="$XO_HOME" XO_DATA_OVERRIDE="$DATA" "$XO_ROOT/bin/xo-project-mode.sh" "$project")
EOF
...
seed_registry_lock_release() {
  if [ "$SEED_REGISTRY_LOCK_HELD" -eq 1 ]; then
    xo_lock_release "$SEED_REGISTRY_LOCK"
    SEED_REGISTRY_LOCK_HELD=0
  fi
}
...
seed_rollback_target() {
  ...
  abs_home=$(resolved_path "$XO_HOME")
  abs_root=$(resolved_path "$XO_ROOT")
  if [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the active xo home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label rollback target $target is the xo repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the active xo home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label rollback target $target is an ancestor of the xo repo" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the active xo home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label rollback target $target is inside the xo repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
seed_return_treehouse_home() {
  ...
  ( cd "$XO_ROOT" && treehouse return --force "$abs_home" >/dev/null ) || {
...
project_mode_in_home() {
  ...
  read -r mode _ <<EOF
$(XO_ROOT_OVERRIDE='' XO_STATE_OVERRIDE='' XO_DATA_OVERRIDE='' XO_PROJECTS_OVERRIDE='' XO_CONFIG_OVERRIDE='' XO_HOME="$home" "$XO_ROOT/bin/xo-project-mode.sh" "$project")
EOF
  printf '%s\n' "$mode"
}
...
refuse_projectful_projectless_charter() {
  ...
  printf 'error: re-scaffold it with xo-brief.sh %s --secondmate --no-projects or remove the stale brief before seeding\n' "$id" >&2
  return 1
}
...
seed_home() {
  ...
  xo_lock_acquire_wait "$SEED_REGISTRY_LOCK" || return 1
  ...
  SEED_HOME_ACQUIRED=0
  SEED_HOME_BACKED_UP=0
  SEED_BACKUP_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xo-home-seed.XXXXXX")
  ...
  if [ ! -f "$SEED_PARENT_BRIEF" ]; then
    [ -n "${XO_SECONDMATE_CHARTER:-}" ] || {
      echo "error: no filled secondmate charter brief at $SEED_PARENT_BRIEF; set XO_SECONDMATE_CHARTER or scaffold one and replace {TASK}" >&2
      return 1
    }
    [ -d "$DATA/$id" ] || SEED_PARENT_BRIEF_DIR_CREATED=1
    if [ "$no_projects" -eq 1 ]; then
      "$XO_ROOT/bin/xo-brief.sh" "$id" --secondmate --no-projects
    else
      "$XO_ROOT/bin/xo-brief.sh" "$id" --secondmate "$@"
    fi
  ...
  # to the identity marker: the cleanup check in xo-teardown.sh reads it so a
  # restart that drops the launch-time XO_PUBLIC_FOLLOWUP_PRIMARY_HOME prefix
  # can still resolve the real parent instead of silently treating its relay
  # as inactive.
  {
    printf 'schema=xo-secondmate-parent.v1\n'
    printf 'route=local\n'
    printf 'parent_home=%s\n' "$(resolved_path "$XO_HOME")"
