...
#
# Usage: xo-remote-job-reap-orphans.sh [--dry-run]
#   --dry-run reports what would be reaped and signals nothing.
#
# A remote job worker (bin/xo-remote-job-worker.sh) is launched from a specific
# XO code root: the account's own checkout under the LaunchAgent, a
...
# gone (bin/xo-remote-job-worker.sh); this sweep is the belt-and-suspenders pass
# that clears workers already orphaned that way, including ones started before
# self-termination shipped.
#
# The reap condition is exactly xo_remote_job_root_is_live failing for the root
...
# the shared xo_remote_job_stop_worker_tree, so the whole worker tree goes at
...
DRY_RUN=0
REAP_SUFFIX=/bin/xo-remote-job-worker.sh

reap_die() { printf 'xo-remote-job-reap-orphans: %s\n' "$1" >&2; exit 2; }

reap_usage() {
  cat <<'TXT'
Usage: xo-remote-job-reap-orphans.sh [--dry-run]

Stop every remote job worker whose XO code root has been pruned. A
...
reap_orphans() {
  ...
  own_pgid=$(xo_remote_job_process_pgid "$$" 2>/dev/null || true)
  ...
  while read -r pid command; do
    ...
    xo_remote_job_root_is_live "$root" && continue
    [ "$pid" != "$$" ] || continue
    reap_is_self_or_ancestor "$pid" && continue
    if [ -n "$own_pgid" ]; then
      pgid=$(xo_remote_job_process_pgid "$pid" 2>/dev/null || true)
    ...
    live=$(xo_remote_job_process_command "$pid" 2>/dev/null || true)
    read -r live <<< "$live"
    ...
    if xo_remote_job_stop_worker_tree "$pid"; then
