...
#   state/x-context/            the private full request context (xo-x-lib.sh).
#   bin/xo-x-reply.sh           posting to the relay, thread splitting, dry run.
#   bin/xo-public-followup-lib.sh  the activation gate and private transport.
#   bin/xo-on.sh                the SSH route to a REMOTE secondmate home, whose
#                               state no local path can reach.
#   bin/xo-public-followup-collect.sh  reading and retiring the typed terminal
...
#
#   xo-public-followup.sh register <obligation-id> --relation <relation-id>
...
#
#   xo-public-followup.sh brief <obligation-id>
#       Print the exact xo-public-followup-emit.sh command line the bound worker
...
#
#   xo-public-followup.sh deliver <obligation-id> [--text-file <path>]
...
#
#   xo-public-followup.sh record-posted <obligation-id> --attempt <n> --chunks <n>
...
#
#   xo-public-followup.sh guard-work <work-home-id> <work-id>
...
#
#   xo-public-followup.sh rechain <new-obligation-id> --from <delivered-id>
...
#
#   xo-public-followup.sh retire <obligation-id> --reason "<why the loop is done>" [--force]
...
# XO_PF_RETRY_BACKOFF_SECS (default 900) sets the next-attempt time recorded with
...
RETRY_BACKOFF=${XO_PF_RETRY_BACKOFF_SECS:-900}
case "$RETRY_BACKOFF" in ''|*[!0-9]*) RETRY_BACKOFF=900 ;; esac

usage() {
  echo "usage: xo-public-followup.sh <active|register|brief|consume|pending|deliver|record-posted|guard-work|rechain|retire> [args]" >&2
}

# The header comment IS the help text, so the two can never drift apart.
help() { sed -n '2,/^set -u$/p' "$0" | sed '$d; s/^# \{0,1\}//'; }

die() { printf 'xo-public-followup: %s\n' "$1" >&2; exit "${2:-2}"; }
...
pf_registry_lock_acquire() {
  ...
  xo_pf_registry_lock_acquire "$STATE" "$id" || return 1
...
pf_registry_lock_release() {
  ...
  local -a remaining=()
  pf_registry_lock_held "$id" || return 0
  xo_pf_registry_lock_release "$STATE" "$id"
  for held in ${PF_REGISTRY_LOCK_IDS[@]+"${PF_REGISTRY_LOCK_IDS[@]}"}; do
...
pf_cleanup() {
  ...
  for ((i=${#PF_REGISTRY_LOCK_IDS[@]}-1; i>=0; i--)); do
    xo_pf_registry_lock_release "$STATE" "${PF_REGISTRY_LOCK_IDS[$i]}" 2>/dev/null || true
  done
  [ "${#PF_TEMP_FILES[@]}" -eq 0 ] || rm -f -- "${PF_TEMP_FILES[@]}"
}
trap pf_cleanup EXIT

now_rfc3339() { xo_pf_now_rfc3339; }
...
# same convention bin/xo-captain-hold.sh uses for typed backlog state.
tx() { (cd "$XO_HOME" && tasks-axi "$@"); }
...
gate_or_exit() {
  xo_pf_relay_active "$XO_HOME" || exit 0
  xo_pf_has_registrations "$STATE" || xo_pf_has_events "$STATE" || exit 0
}

# --- subcommand: active -----------------------------------------------------

cmd_active() {
  xo_pf_relay_active "$XO_HOME" || exit 1
  xo_pf_has_registrations "$STATE" || xo_pf_has_events "$STATE" || exit 1
  exit 0
}
...
cmd_register() {
  ...
  xo_pf_slug_valid "$relation" || die "unsafe relation id: $relation"
  xo_pf_slug_valid "$work_id"  || die "unsafe work id: $work_id"
  xo_pf_home_id_valid "$work_home" \
  ...
  [ -z "$request" ] || xo_pf_slug_valid "$request" || die "unsafe request id: $request"
  ...
  if [ -n "$request_json" ]; then
    request_context_b64=$(printf '%s' "$request_json" | xo_pf_b64_encode)
  fi
  work_home_path=
  case "$work_home" in
    secondmate:*)
...
brief_emit_target() {
  ...
  case "$work_home" in
    ...
    *) printf '%s\n--home %s\n' "$XO_ROOT/bin/xo-public-followup-emit.sh" "$XO_HOME"; return 0 ;;
  esac
  kind=$(public_followup_route_kind "$sid" "$recorded_home") || return 1
  if [ "$kind" = local ]; then
    printf '%s\n--home %s\n' "$XO_ROOT/bin/xo-public-followup-emit.sh" "$XO_HOME"
    return 0
  fi
  ...
  printf '%s\n--stage-in %s\n' "$root/bin/xo-public-followup-emit.sh" "$home"
}
...
cmd_brief() {
  ...
  [ -n "$id" ] || { usage; exit 2; }
  xo_pf_slug_valid "$id" || die "unsafe obligation id: $id"
  xo_pf_relay_active "$XO_HOME" || die "the relay is not active for this home" 1
  [ -f "$(xo_pf_registry_dir "$STATE")/$id" ] \
    || die "no registration for '$id' in this home" 1

  relation=$(xo_pf_registry_get "$STATE" "$id" relation_id)
  work_home=$(xo_pf_registry_get "$STATE" "$id" work_home)
  work_home_path=$(xo_pf_registry_get "$STATE" "$id" work_home_path)
  work_id=$(xo_pf_registry_get "$STATE" "$id" work_id)
  generation=$(xo_pf_registry_get "$STATE" "$id" generation)
...
collect_remote_staged_events() {
  ...
  dir=$(xo_pf_registry_dir "$STATE")
  [ -d "$dir" ] && [ ! -L "$dir" ] || return 0
  for file in "$dir"/*; do
    id=$(basename "$file")
    xo_pf_slug_valid "$id" || continue
    ...
    loop_state=$(xo_pf_registry_loop_state "$STATE" "$id")
    ...
    collect_rc=0
    payload=$("$XO_ROOT/bin/xo-on.sh" "$sid" xo-public-followup-collect.sh drain "$id") \
      || collect_rc=$?
    # xo-on.sh returns ssh's status unchanged, so 255 is the established
    ...
    while IFS= read -r line; do
      ...
      if [ "${#line}" -gt "$XO_PF_EVENT_BYTES_MAX" ] \
        || [ -z "$event_id" ] || ! xo_pf_slug_valid "$event_id"; then
      ...
      dropped=0
      "$XO_ROOT/bin/xo-on.sh" "$sid" xo-public-followup-collect.sh drop "$id" "$event_id" \
...
cmd_consume() {
  ...
  if ! xo_pf_has_events "$STATE"; then
  ...
  for file in "$events_dir"/*.json; do
    ...
    if ! xo_pf_slug_valid "$event_id"; then
    ...
    if [ "$(wc -c < "$file" 2>/dev/null || echo 0)" -gt "$XO_PF_EVENT_BYTES_MAX" ]; then
      reject_event "$file" "$event_id" "event exceeds $XO_PF_EVENT_BYTES_MAX bytes" || consume_rc=1
      continue
    fi
    ...
    # xo-public-followup-emit.sh, so it is refused before tasks-axi sees it.
    ...
    if ! xo_pf_slug_valid "$obligation"; then
    ...
    if [ "$rc" -ne 0 ]; then
      reason=$( { cat "$stderr_file" 2>/dev/null; printf '%s\n' "$out"; } \
        | grep -v '^[[:space:]]*$' | head -1 | xo_pf_clean_outcome_text | xo_pf_bound_bytes 400)
  ...
  # A fresh event must be able to wake xo again, so drop the surfaced
  # signature once the inbox has been worked.
  rm -f -- "$(xo_pf_root "$STATE")/$XO_PF_SURFACED_BASENAME" 2>/dev/null || true
  return "$consume_rc"
}
...
print_window_escalation() {
  ...
  window=$(xo_pf_followup_window_class "$expires")
  case "$window" in
    expired)
...
print_open_loop() {
  ...
  request=$(xo_pf_registry_get "$STATE" "$id" request_id)
  [ -n "$request" ] || request=$(pf_field "$payload" '.public_followup.request.request_id')
  platform=$(xo_pf_registry_get "$STATE" "$id" platform)
  [ -n "$platform" ] || platform=$(pf_field "$payload" '.public_followup.request.platform')
  delivered=$(xo_pf_registry_get "$STATE" "$id" delivered_at)
  expires=$(xo_pf_registry_get "$STATE" "$id" followup_expires_at)
  [ -n "$expires" ] || expires=$(pf_field "$payload" '.public_followup.request.followup_expires_at')
  summary=$(pf_field "$payload" '.public_followup.request.public_safe_summary' | xo_pf_clean_outcome_text)
  if [ -z "$summary" ]; then
    ctx=$(xo_pf_registry_get "$STATE" "$id" request_context_b64)
    if [ -n "$ctx" ]; then
      summary=$(printf '%s' "$ctx" | xo_pf_b64_decode | jq -r '.public_safe_summary // empty' 2>/dev/null | xo_pf_clean_outcome_text)
    fi
  fi
  ...
  printf '  summary=%s\n' "$summary"
  if ! xo_pf_registry_rechainable "$STATE" "$id"; then
...
cmd_pending() {
  ...
  if ! command -v jq >/dev/null 2>&1 || ! command -v tasks-axi >/dev/null 2>&1 \
      ...
      ' >/dev/null 2>&1; then
    if xo_pf_has_registrations "$STATE"; then
      printf 'cannot read this home'\''s public commitments through tasks-axi; %s registration(s) are still recorded under state/%s/registry\n' \
        "$(xo_pf_registry_ids "$STATE" | grep -c . || true)" "$XO_PF_DIRNAME"
      printed=1
    fi
    if xo_pf_has_events "$STATE"; then
      printf 'unconsumed terminal results are waiting; run %s/bin/xo-public-followup.sh consume\n' "$XO_ROOT"
      printed=1
    fi
    [ "$printed" -eq 1 ] || exit 0
    return 0
  ...
  while IFS= read -r id; do
    ...
    if [ "$settled" -eq 1 ]; then
      if [ "$loop_state" != delivered ]; then
        stamp_rc=0
        xo_pf_registry_stamp_delivered "$STATE" "$id" "$(now_rfc3339)" || stamp_rc=$?
        if [ "$stamp_rc" -eq 3 ] && xo_pf_retirement_receipt_exists "$STATE" "$id"; then
          continue
        fi
        [ "$stamp_rc" -eq 0 ] \
...
public_followup_registration_valid() {
  ...
  file="$(xo_pf_registry_dir "$STATE")/$id"
  ...
  xo_pf_home_id_valid "$work_home" || return 1
  xo_pf_slug_valid "$work_id" || return 1
...
public_followup_secondmate_home() {
  ...
  [ -f "$home/.xo-secondmate-home" ] && [ ! -L "$home/.xo-secondmate-home" ] || return 2
  marker=$(sed -n '1p' "$home/.xo-secondmate-home" 2>/dev/null)
  [ "$marker" = "$id" ] || return 2
  printf '%s\n' "$home"
}
...
# here for the same reason xo-on.sh and xo-send.sh treat it as one: a remote home
...
# xo-on.sh returns ssh's status unchanged, so 255 is the established "delivered
# but completion unknown" status this codebase already reconciles rather than
# reads as done or refused (bin/xo-on.sh, bin/xo-remote-readiness-lib.sh,
# bin/xo-teardown.sh). It is passed through so a caller can say the remote never
...
clear_public_followup_link_remote() {
  ...
  "$XO_ROOT/bin/xo-on.sh" "$id" xo-x-followup.sh --clear "$work_id" \
...
clear_public_followup_link() {
  ...
  work_home=$(xo_pf_registry_get "$STATE" "$id" work_home)
  work_id=$(xo_pf_registry_get "$STATE" "$id" work_id)
  request_id=$(xo_pf_registry_get "$STATE" "$id" request_id)
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" XO_ROOT_OVERRIDE="$XO_ROOT" \
    "$XO_ROOT/bin/xo-x-followup.sh" --clear "$work_id" >/dev/null
}
...
# messages are unchanged. A remote clear returns xo-on.sh's pass-through ssh
...
record_error() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/xo-pf-error.XXXXXX") || return 1
  if [ -n "$next" ]; then
...
record_posted() {
  ...
  tmp=$(mktemp "${TMPDIR:-/tmp}/xo-pf-receipt.XXXXXX") || return 1
...
mark_loop_delivered() {
  local id=$1 rc=0
  xo_pf_registry_stamp_delivered "$STATE" "$id" "$(now_rfc3339)" || rc=$?
  case "$rc" in
    0) return 0 ;;
    3) return 3 ;;
...
cmd_deliver() {
  ...
  tmp_text=$(mktemp "${TMPDIR:-/tmp}/xo-pf-text.XXXXXX") || die "could not stage the reply text" 1
  PF_TEMP_FILES+=("$tmp_text")
  receipt=$(mktemp "${TMPDIR:-/tmp}/xo-pf-postreceipt.XXXXXX") || die "could not stage the post receipt" 1
  PF_TEMP_FILES+=("$receipt")
  printf '%s' "$text" > "$tmp_text"

  hash=$(xo_pf_sha256 < "$tmp_text") || die "sha256 (shasum or sha256sum) is required" 1
...
cmd_guard_work() {
  ...
  xo_pf_relay_active "$XO_HOME" || exit 0
  xo_pf_has_registrations "$STATE" || exit 0
  ...
  bound=$(xo_pf_registry_ids_for_work "$STATE" "$work_home" "$work_id")
  [ -n "$bound" ] || exit 0
...
cmd_rechain() {
  ...
  expires=$(xo_pf_registry_get "$STATE" "$from" followup_expires_at)
  ...
  rechain_to=$(xo_pf_registry_get "$STATE" "$from" rechain_to)
  ...
  if [ -z "$rechain_to" ]; then
    ...
    [ ! -e "$(xo_pf_registry_dir "$STATE")/$new_id" ] \
      && [ ! -L "$(xo_pf_registry_dir "$STATE")/$new_id" ] \
      && [ ! -e "$(xo_pf_retired_dir "$STATE")/$new_id" ] \
      && [ ! -L "$(xo_pf_retired_dir "$STATE")/$new_id" ] \
  ...
  ctx=$(xo_pf_registry_get "$STATE" "$from" request_context_b64)
  ctx_file=$(mktemp "${TMPDIR:-/tmp}/xo-pf-rechain-ctx.XXXXXX") \
    || die "could not stage the retained request context" 1
  expected_file=$(mktemp "${TMPDIR:-/tmp}/xo-pf-rechain-exp.XXXXXX") \
    || die "could not stage the expected-final document" 1
  relation_file=$(mktemp "${TMPDIR:-/tmp}/xo-pf-rechain-rel.XXXXXX") \
    || die "could not stage the relation document" 1
  PF_TEMP_FILES+=("$ctx_file" "$expected_file" "$relation_file")
  printf '%s' "$ctx" | xo_pf_b64_decode > "$ctx_file" \
  ...
  new_registry="$(xo_pf_registry_dir "$STATE")/$new_id"
  if [ -f "$new_registry" ] && [ ! -L "$new_registry" ]; then
    [ "$(xo_pf_registry_get "$STATE" "$new_id" relation_id)" = rel-1 ] \
      && [ "$(xo_pf_registry_get "$STATE" "$new_id" work_home)" = "$work_home" ] \
      && [ "$(xo_pf_registry_get "$STATE" "$new_id" work_id)" = "$work_id" ] \
      && [ "$(xo_pf_registry_get "$STATE" "$new_id" generation)" = 1 ] \
...
cmd_retire() {
  ...
  reason=$(printf '%s' "$reason" | xo_pf_clean_outcome_text)
  [ -n "$reason" ] || die "retire requires --reason \"<why the public loop is done>\"" 2
  require_tools
  pf_registry_lock_acquire "$id" \
  ...
  retired_dir=$(xo_pf_retired_dir "$STATE")
