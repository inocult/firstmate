...
#
# An xo primary that delegates through a harness's own delegation,
# scheduling, or background-work tool creates work with no `state/<id>.meta` and
# no `data/<id>/brief.md`. Only `bin/xo-spawn.sh` writes that metadata, and
# untracked project work contributes nothing to the in-flight branch of
# bin/xo-supervision-lib.sh or bin/xo-turnend-guard.sh. So such work is not
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/xo-subagent-pretool-check.sh
#   bin/xo-subagent-pretool-check.sh --tool '<tool-name>'
#
...
#
# Exit/output contract (identical shape to bin/xo-cd-pretool-check.sh):
#   ALLOW - exit 0 and no output.
#   DENY - exit 2, a Claude-shaped deny object on stderr, and a Grok-shaped
#          deny object on stdout unless --claude was supplied.
#   INERT - not a genuine primary home (a crewmate/scout task worktree or a
#           non-xo repo): exit 0 with no output, exactly like ALLOW.
#   ESCAPE - XO_ALLOW_SUBAGENT=1 in the environment allows deliberately.
...
# work, an agent, a schedule, or an isolated workspace that xo would not
...
# and starts nothing that could outlive the session or escape an xo
# guard. Denying them stops the primary tracking its own plan while granting no
# delegation power, and the deny text would tell it to run bin/xo-brief.sh for a
...
usage() {
  cat <<'EOF'
Usage: xo-subagent-pretool-check.sh [--tool <tool-name>] [--claude]
...
Fires only in a genuine xo primary home; it is a silent no-op in a
crewmate/scout task worktree or any non-xo repo, where a worker using
delegation tools is legitimate.
Exits 0 to allow and 2 to deny, naming the real crewmate dispatch path instead.
Set XO_ALLOW_SUBAGENT=1 in the session environment to allow deliberately.
...
[ "${XO_ALLOW_SUBAGENT:-}" != "1" ] || exit 0

SCRIPT_DIR=$(CDPATH='' cd -- "$(dirname -- "${BASH_SOURCE[0]}")" 2>/dev/null && pwd -P) || exit 0
XO_ROOT=${XO_ROOT_OVERRIDE:-$(CDPATH='' cd -- "$SCRIPT_DIR/.." 2>/dev/null && pwd -P)} || exit 0
...
# turn-end guard do. xo_primary_scope_matches accepts a plain checkout or a
# marked secondmate home - both operate a fleet and must dispatch through it -
# and rejects a linked task worktree, which is the shape bin/xo-spawn.sh always
...
if [ -f "$XO_ROOT/bin/xo-scout.sh" ]; then
  ROUTE='first classify the work under the AGENTS.md intake contract: work already classified as a scout goes to bin/xo-scout.sh "<question>" [project], while authorized ship work and its bounded research go to bin/xo-brief.sh then bin/xo-spawn.sh'
else
  ROUTE='first classify the work under the AGENTS.md intake contract, then use bin/xo-brief.sh followed by bin/xo-spawn.sh for dispatched work'
fi

REASON="[subagent-dispatch] the xo primary dispatches through the fleet, not the harness's own delegation tools: work started that way has no durable fleet record, leaves every xo guard inert, and dies with this session. Instead, $ROUTE (blocked tool: $TOOL, delegation-shaped on \"$MATCHED\"). Launch the session with XO_ALLOW_SUBAGENT=1 for a deliberate exception."
