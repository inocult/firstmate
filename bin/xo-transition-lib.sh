...
#
#   2. The STATUS -> ACTION POLICY TABLE (xo_transition_policy) - the SINGLE
...
# (xo_backend_herdr_wait_transition) for the herdr producer and bin/xo-watch.sh
...
XO_TRANSITION_FIELD_SEP=$'\t'

# xo_transition_record: THE constructor for a normalized transition record.
...
xo_transition_record() {  # <pane_id> <workspace_id> <from_status> <to_status> <agent>
  local pane_id ws from to agent
  pane_id=$(xo_transition_clean_field "${1:-}")
  ws=$(xo_transition_clean_field "${2:-}")
  from=$(xo_transition_clean_field "${3:-}")
  to=$(xo_transition_clean_field "${4:-}")
  agent=$(xo_transition_clean_field "${5:-}")
  printf '%s\t%s\t%s\t%s\t%s' "$pane_id" "$ws" "$from" "$to" "$agent"
}

# xo_transition_clean_field: collapse any TAB/CR/LF in a field value to spaces
# so a stray control char can never desync the fixed five-field record.
xo_transition_clean_field() {  # <value>
  printf '%s' "${1:-}" | LC_ALL=C tr '\t\r\n' '   '
}

# Field accessors (1-based), so consumers never hardcode the column layout.
xo_transition_field() {  # <record> <n>
  printf '%s' "$1" | cut -d"$XO_TRANSITION_FIELD_SEP" -f"$2"
}

xo_transition_pane_id()      { xo_transition_field "$1" 1; }
xo_transition_workspace_id() { xo_transition_field "$1" 2; }
xo_transition_from_status()  { xo_transition_field "$1" 3; }
xo_transition_to_status()    { xo_transition_field "$1" 4; }
xo_transition_agent()        { xo_transition_field "$1" 5; }

# xo_transition_policy: THE single-owner status -> supervision-action table.
...
# immediate action.
xo_transition_policy() {  # <to_status> -> actionable|absorb|defer|fallback
  case "$1" in
