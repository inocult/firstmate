...
XO_SECOND_MATE_NUDGE_MESSAGE='xo was updated to the latest - please re-read your AGENTS.md to pick up the new instructions.'
XO_REMOTE_SECOND_MATE_NUDGE_MESSAGE='XO instructions or inherited config changed on this host. Re-read AGENTS.md and the inherited config files before further work.'

xo_secondmate_nudge_marker_path() { # <state-dir> <id>
  local state=$1 id=$2
  case "$id" in *[!/A-Za-z0-9._-]*|''|*/*) return 1 ;; esac
  printf '%s/.secondmate-nudge-pending/%s.pending\n' "$state" "$id"
}

xo_remote_inherit_transaction_lock_path() { # <state-dir> <id>
  local state=$1 id=$2
  case "$id" in *[!/A-Za-z0-9._-]*|''|*/*) return 1 ;; esac
  printf '%s/.remote-inherit-%s.lock\n' "$state" "$id"
}

xo_remote_inherit_generation_next() { # <state-dir> <id>
...
xo_secondmate_nudge_write() { # <state> <id> <home> <commit> <instructions> <message> <remote:0|1>
  ...
  marker=$(xo_secondmate_nudge_marker_path "$state" "$id") || return 1
  parent=${marker%/*}
