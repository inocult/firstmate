...
# sourced by both the sending parent (bin/xo-remote-home-seed.sh) and the
# receiving host (bin/xo-remote-home-provision.sh), so an unsafe value is
...
# XO is a shared template, so any host must be able to serve a project:
...
xo_project_origin_safe() { # <url>; 0 when the URL is an accepted clone URL
