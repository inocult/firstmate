#!/usr/bin/env bash
# xo-install-shellcheck.sh - install CI's pinned, verified ShellCheck build.
#
...
#
# Usage:
#   xo-install-shellcheck.sh <destination-directory>
set -eu

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VERSION="$("$ROOT/bin/xo-lint.sh" --required-version)"

die() {
  printf 'xo-install-shellcheck.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: xo-install-shellcheck.sh <destination-directory>}
...
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/xo-shellcheck.XXXXXX")
trap 'rm -rf "$TMP"' EXIT
...
while ! curl -fsSL "$URL" -o "$TMP/$ARCHIVE"; do
  [ "$download_attempt" -lt "$DOWNLOAD_ATTEMPTS" ] || {
    printf 'xo-install-shellcheck.sh: download failed after %s attempts\n' "$DOWNLOAD_ATTEMPTS" >&2
    exit 1
  }
  printf 'xo-install-shellcheck.sh: download attempt %s failed; retrying\n' "$download_attempt" >&2
...
[ "$ACTUAL_SHA256" = "$SHA256" ] || {
  printf 'xo-install-shellcheck.sh: checksum mismatch for %s (expected %s, got %s)\n' \
