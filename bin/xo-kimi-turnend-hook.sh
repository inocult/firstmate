#!/usr/bin/env bash
# Install or remove XO's guarded Kimi crew turn-end hook.
#
...
# serializes it: install adds or replaces one marker-delimited XO region,
...
# hook payload, checks for a .xo-kimi-turnend pointer before registry work, and
# touches a task turn-end marker only when the pointer names an XO-created
# token in $HOME/.kimi-code/xo-turn-end.d/.
#
# Usage:
#   xo-kimi-turnend-hook.sh install
#   xo-kimi-turnend-hook.sh remove
set -u
...
if [ -z "${HOME:-}" ]; then
  printf 'xo-kimi-turnend-hook: refused: HOME is unset.\n' >&2
  exit 1
fi
if ! command -v python3 >/dev/null 2>&1; then
  printf 'xo-kimi-turnend-hook: refused: python3 with tomllib is required to validate config.toml.\n' >&2
  exit 1
fi
if [ "$ACTION" = install ] && ! command -v jq >/dev/null 2>&1; then
  printf 'xo-kimi-turnend-hook: refused: jq is required by the installed Kimi turn-end hook.\n' >&2
  exit 1
fi
...
try:
    ...
    print(
        "xo-kimi-turnend-hook: refused: python3 with tomllib is required to validate config.toml.",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
HOOK = os.path.join(CONFIG_DIR, "xo-turn-end.sh")
REGISTRY = os.path.join(CONFIG_DIR, "xo-turn-end.d")
BEGIN = b"# BEGIN XO KIMI TURN-END HOOK"
BEGIN_OWNS_NEWLINE = BEGIN + b" (OWNS PRECEDING NEWLINE)"
END = b"# END XO KIMI TURN-END HOOK"
IDENTIFIER = b"XO KIMI TURN-END HOOK"
HOOK_NAME = b"xo-turn-end.sh"
TOKEN_NAME = re.compile(r"xo\.[A-Za-z0-9]{12}\Z")

HOOK_BYTES = b'''#!/usr/bin/env bash
# XO Kimi turn-end hook. Managed by xo-kimi-turnend-hook.sh.
...
pointer="$workspace/.xo-kimi-turnend"
[ -f "$pointer" ] || exit 0
first=
...
auth_dir=${HOME:-}/.kimi-code/xo-turn-end.d
[ -n "${HOME:-}" ] || exit 0
...
def refuse(reason: str) -> None:
    print(f"xo-kimi-turnend-hook: refused: {reason}", file=sys.stderr)
    raise SystemExit(1)
...
def locate_region(data: bytes):
    ...
    if normal_count + owned_count != 1 or end_count != 1 or identifier_count != 2:
        refuse("config.toml has partial, duplicated, or altered XO region markers.")
    ...
    if marker_at != 0 and data[marker_at - 1 : marker_at] != b"\n":
        refuse("the XO begin marker is not at a line boundary.")
    start = marker_at
    if owned_count:
        if marker_at == 0 or data[marker_at - 1 : marker_at] != b"\n":
            refuse("the XO region claims a preceding newline that is absent.")
        start -= 1
    end_at = data.find(END, marker_at + len(marker))
    if end_at < 0:
        refuse("the XO end marker is missing.")
    after = end_at + len(END)
    if after < len(data):
        if data[after : after + 1] != b"\n":
            refuse("the XO end marker is not a complete line.")
        after += 1
...
def block(marker: bytes) -> bytes:
    return b"\n".join(
        (
            ...
            b'event = "Stop"',
            b'matcher = "^$"',
            b'command = "bash \\"$HOME/.kimi-code/xo-turn-end.sh\\" >/dev/null 2>&1 || true"',
            b"timeout = 1",
            END,
            b"",
        )
...
def validate_xo_files_for_remove() -> None:
    if os.path.lexists(HOOK):
        info = regular_not_symlink(HOOK, "XO hook script")
        with open(HOOK, "rb") as stream:
            if stream.read() != HOOK_BYTES:
                refuse(f"XO hook script has unexpected content at {HOOK}.")
        if stat.S_IMODE(info.st_mode) & 0o077:
            refuse(f"XO hook script has unexpectedly broad permissions at {HOOK}.")
    if os.path.lexists(REGISTRY):
        info = os.lstat(REGISTRY)
        if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
            refuse(f"XO registry is not a regular directory at {REGISTRY}.")
        for name in os.listdir(REGISTRY):
            ...
            if not TOKEN_NAME.fullmatch(name) or stat.S_ISLNK(child.st_mode) or not stat.S_ISREG(child.st_mode):
                refuse(f"XO registry contains an unexpected entry at {path}.")
...
try:
    ...
    if HOOK_NAME in outside:
        refuse("config.toml references xo-turn-end.sh outside the XO-owned region.")
    ...
    if ACTION == "install":
        if os.path.lexists(REGISTRY):
            ...
            if stat.S_ISLNK(info.st_mode) or not stat.S_ISDIR(info.st_mode):
                refuse(f"XO registry is not a regular directory at {REGISTRY}.")
        if os.path.lexists(HOOK):
            regular_not_symlink(HOOK, "XO hook script")
            with open(HOOK, "rb") as stream:
                existing_hook = stream.read()
            if existing_hook != HOOK_BYTES and not existing_hook.startswith(
                b"#!/usr/bin/env bash\n# XO Kimi turn-end hook."
            ):
                refuse(f"XO hook path has unexpected content at {HOOK}.")
        if region is None:
        ...
        validate_xo_files_for_remove()
        candidate = outside
        parse_toml(candidate, "config.toml after XO region removal")
        if candidate != original:
            atomic_write(CONFIG, candidate, stat.S_IMODE(config_info.st_mode))
        if os.path.lexists(HOOK):
            os.unlink(HOOK)
