#!/usr/bin/env bash
# xo-fleet-view.sh - human renderer over xo-fleet-snapshot.sh.
#
# This command intentionally does not parse fleet state itself.
# It shells out to xo-fleet-snapshot.sh --json and renders that stable
...
usage() {
  cat <<'EOF'
usage: xo-fleet-view.sh [--json]

Render a human fleet view from xo-fleet-snapshot.sh.
...
case "${1:-}" in
  -h|--help) usage; exit 0 ;;
  --json) "$SCRIPT_DIR/xo-fleet-snapshot.sh" --json; exit $? ;;
  "") ;;
  *) usage >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "xo-fleet-view: jq not found" >&2; exit 1; }

SNAPSHOT=$("$SCRIPT_DIR/xo-fleet-snapshot.sh" --json) || exit $?
