...
# Shared fast-forward machinery for xo self-sync.
# Usage: . bin/xo-ff-lib.sh   (after XO_ROOT and XO_HOME are set)
#
# This is the one implementation of "advance an xo checkout to a base by a
# clean fast-forward, never forcing, merging, or stashing" used by every sync
# path:
#   - /updatexo (bin/xo-update.sh) pulls from origin: base_mode "origin".
#   - the local-HEAD secondmate sync (bin/xo-spawn.sh on launch, bin/xo-bootstrap.sh
...
# this object store, so bin/xo-spawn.sh and bin/xo-bootstrap.sh hand the commit to
# bin/xo-remote-secondmate-control.sh, which imports it on that host and then runs
...
# The seeded .xo-secondmate-home identity marker is gitignored too; the local
...
SUB_HOME_MARKER="${SUB_HOME_MARKER:-.xo-secondmate-home}"
# shellcheck source=bin/xo-secondmate-registry-lib.sh
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/xo-secondmate-registry-lib.sh"
...
validate_operational_dirs() {
  ...
  for name in data state config projects; do
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      VALIDATION_ERROR="secondmate $name directory cannot be inside the active xo home"
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      VALIDATION_ERROR="secondmate $name directory cannot be inside the xo repo"
      return 1
    fi
  done
}
...
validate_secondmate_home() {
  ...
  abs_active_home=$(resolved_existing_dir "$XO_HOME") || {
    VALIDATION_ERROR="active xo home is not a directory"
    return 1
  }
  abs_root=$(resolved_existing_dir "$XO_ROOT") || {
    VALIDATION_ERROR="xo repo is not a directory"
    return 1
  }
  if [ "$abs_home" = "/" ]; then
  ...
  if [ "$abs_home" = "$abs_active_home" ]; then
    VALIDATION_ERROR="secondmate home cannot be the active xo home"
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    VALIDATION_ERROR="secondmate home cannot be the xo repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    VALIDATION_ERROR="secondmate home cannot be inside the active xo home"
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    VALIDATION_ERROR="secondmate home cannot be inside the xo repo"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    VALIDATION_ERROR="secondmate home cannot be an ancestor of the active xo home"
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    VALIDATION_ERROR="secondmate home cannot be an ancestor of the xo repo"
    return 1
  fi
  ...
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    VALIDATION_ERROR="not an xo home (missing AGENTS.md)"
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    VALIDATION_ERROR="not an xo home (missing bin/)"
    return 1
  fi
...
# (AGENTS.md, which CLAUDE.md imports via @AGENTS.md), its skills (the canonical
# skills/ tree plus the .agents/skills/ activation links that select which of
# them this home loads; docs/configuration.md owns that layout), and its
# tooling (bin/). A diff scoped to .agents/skills sees only the links, so the
# tree itself is watched by name.
changed_instr() {
  local dir=$1 base=$2 p out=""
  for p in AGENTS.md bin .agents/skills skills; do
...
# status, which on this leg can only mean that host's XO copy predates the
...
remote_sync_failure_reason() { # <exit-status> <output>
  if [ "$1" = "$REMOTE_SYNC_UNSUPPORTED_STATUS" ]; then
    printf '%s\n' "the XO copy on that host is too old to sync to this primary's commit; run /updatexo"
    return 0
  fi
  first_line "$2"
}
...
#   origin       - fetch origin and advance to origin/<default> (the /updatexo
...
# xo-<id> task selector into FF_NUDGE_WINDOWS when it should be live-converged.
# Args:
...
#     forces, stashes, or discards its work. /updatexo uses this hook to
...
process_secondmate() {
  local id=$1 home=$2 window=${3:-} base_mode=$4 nudge_requires_instr=${5:-no} home_real xo_root_real
  [ -n "$id" ] || return 0
  [ -n "$home" ] || return 0
  xo_root_real=$(resolve_path "$XO_ROOT")
  home_real=$(resolve_path "$home")
  [ "$home_real" != "$xo_root_real" ] || return 0
  ...
  if [ -n "$window" ] && { [ "$FF_STATUS" = "updated" ] || [ "$FF_STATUS" = "current" ]; } \
    && type xo_ff_after_secondmate_settled >/dev/null 2>&1; then
    xo_ff_after_secondmate_settled "$id" "$home_real" "$window" "$FF_STATUS" "$FF_INSTR"
  fi
  if [ "$FF_STATUS" = "updated" ] && [ -n "$window" ]; then
    if [ "$nudge_requires_instr" = yes ] && [ -z "$FF_INSTR" ]; then
      return 0
    fi
    FF_NUDGE_WINDOWS="$FF_NUDGE_WINDOWS xo-$id"
    if [ "$nudge_requires_instr" = yes ] && [ -n "$FF_INSTR" ] \
      && type xo_ff_after_instruction_update >/dev/null 2>&1; then
      xo_ff_after_instruction_update "$id" "$home_real" "$window" "$FF_INSTR"
    fi
  fi
}
...
sweep_live_secondmate_metas() {
  local state=$1 base_mode=$2 nudge_requires_instr=${3:-no} registry=${4:-$XO_HOME/data/secondmates.md} id home window meta
  [ -d "$state" ] || return 0
