...
# XO_HOME-relative mixup, not acknowledgement of an arbitrary mate-home file.
#
# Record location (parent XO_HOME):
#   state/pending-replies/<corr_id>
# One more durable input, owned by bin/xo-procevent-remote-reply.sh and read
...
#                           xo_pending_reply_reset_known_undelivered returns it to
...
# keyed decision in the parent status log, and bin/xo-classify-lib.sh's fold is
# the one owner of what closes it. So this library owns both ends of that
# decision: xo_pending_reply_maybe_escalate opens it under a per-request key, and
# xo_pending_reply_close_escalation closes it once the record resolves. Resolving
...
# reserved-key rule in bin/xo-classify-lib.sh.
# The operator-facing close of that same keyed decision is still
# xo-send --resolve-key (bin/xo-send.sh header): it must speak the close note
# owned below (xo_pending_reply_resolved_note), because a bare answered: note is
...
# to resend under the same correlation (xo-send's XO_PENDING_REPLY_EXISTING_CORR
...
# (bin/xo-backlog-handoff.sh's receiver wake) stayed refused forever once the
# watcher escalated between the lost transport and the next resume.
#
# Sourced by bin/xo-send.sh, bin/xo-watch.sh, bin/xo-secondmate-report.sh, and
# tests. No side effects on source. set -u / set -e safe.
#
# Tunables (env):
#   XO_PENDING_REPLY_GRACE_SECS   default 120
#   XO_PENDING_REPLY_DIR_OVERRIDE override the pending-replies directory (tests)
#   XO_PENDING_REPLY_SEND_HOOK    optional command template for recovery delivery
#                                 (tests); receives task_id and full message as args
#   XO_PENDING_REPLY_NOW          optional fixed epoch for deterministic tests

# shellcheck source=bin/xo-marker-lib.sh
_XO_PENDING_REPLY_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd 2>/dev/null)" || _XO_PENDING_REPLY_LIB_DIR="."
# shellcheck source=bin/xo-marker-lib.sh
. "$_XO_PENDING_REPLY_LIB_DIR/xo-marker-lib.sh"
# shellcheck source=bin/xo-backend.sh
. "$_XO_PENDING_REPLY_LIB_DIR/xo-backend.sh"
# shellcheck source=bin/xo-tmux-lib.sh
. "$_XO_PENDING_REPLY_LIB_DIR/xo-tmux-lib.sh"
# shellcheck source=bin/xo-classify-lib.sh
. "$_XO_PENDING_REPLY_LIB_DIR/xo-classify-lib.sh"

XO_PENDING_REPLY_SCHEMA='xo-pending-reply.v1'
XO_PENDING_REPLY_CORR_RE='corr=[A-Fa-f0-9]{16}'
XO_PENDING_REPLY_GRACE_DEFAULT=120

xo_pending_reply_now() {
  if [ -n "${XO_PENDING_REPLY_NOW:-}" ]; then
    printf '%s' "$XO_PENDING_REPLY_NOW"
    return 0
  fi
  date +%s
}

xo_pending_reply_grace_secs() {
  local g=${XO_PENDING_REPLY_GRACE_SECS:-$XO_PENDING_REPLY_GRACE_DEFAULT}
  case "$g" in
    ''|*[!0-9]*) g=$XO_PENDING_REPLY_GRACE_DEFAULT ;;
  esac
  printf '%s' "$g"
}

# Directory holding durable pending-reply records for <state-dir>.
xo_pending_reply_dir() {  # <state-dir>
  local state=$1
  if [ -n "${XO_PENDING_REPLY_DIR_OVERRIDE:-}" ]; then
    printf '%s' "$XO_PENDING_REPLY_DIR_OVERRIDE"
    return 0
  fi
  printf '%s/pending-replies' "$state"
}

xo_pending_reply_path() {  # <state-dir> <corr_id>
  printf '%s/%s' "$(xo_pending_reply_dir "$1")" "$2"
}

# Privacy-safe correlation id: 16 lowercase hex chars (64 bits of entropy).
xo_pending_reply_new_id() {
  local raw hex
...
xo_pending_reply_corr_token() {  # <corr_id>
  printf 'corr=%s' "$1"
}

# Extract the first corr=<16hex> token from free text, or empty.
xo_pending_reply_extract_corr() {  # <text>
  local text=$1
  printf '%s' "$text" | grep -oE "$XO_PENDING_REPLY_CORR_RE" 2>/dev/null | head -1 | cut -d= -f2- | tr 'A-F' 'a-f' || true
}

# 0 if <text> carries the exact correlation token for <corr_id>.
xo_pending_reply_text_has_corr() {  # <text> <corr_id>
...
xo_pending_reply_summarize() {  # <text>
...
xo_pending_reply_get() {  # <record-path> <key>
  local rec=$1 key=$2
  [ -f "$rec" ] || return 0
  grep "^${key}=" "$rec" 2>/dev/null | tail -1 | cut -d= -f2- || true
}

xo_pending_reply_sighting_encode() {  # <path> <line-number>
...
xo_pending_reply_sighting_display() {  # <encoded-sighting>
...
xo_pending_reply_corr_reusable() {  # <state-dir> <corr_id> <task_id>
  ...
  [ -f "$rec" ] || return 1
  [ "$(xo_pending_reply_get "$rec" task_id)" = "$task_id" ] || return 1
  phase=$(xo_pending_reply_get "$rec" phase)
  case "$phase" in
    ...
    delivery_unknown|escalated)
      ...
      delivered=$(xo_pending_reply_get "$rec" delivered_epoch)
      [ -z "$delivered" ]
      return $?
      ;;
  esac
...
xo_pending_reply_set() {  # <record-path> <key> <value>
...
xo_pending_reply_embed_corr() {  # <message> <corr_id> <result-var>
...
xo_pending_reply_create() {  # <parent-home> <state-dir> <task_id> <request-text>
  ...
  dir=$(xo_pending_reply_dir "$state")
  mkdir -p "$dir" || return 1
  chmod 700 "$dir" 2>/dev/null || true
  corr=$(xo_pending_reply_new_id)
  [ "${#corr}" -eq 16 ] || return 1
  rec=$(xo_pending_reply_path "$state" "$corr")
  ...
  now=$(xo_pending_reply_now)
  summary=$(xo_pending_reply_summarize "$request_text")
  ...
  tmp="$dir/.${corr}.tmp.$$"
  cat > "$tmp" <<EOF
schema=$XO_PENDING_REPLY_SCHEMA
corr_id=$corr
task_id=$task_id
...
wrong_home_scan_signature=
grace_secs=$(xo_pending_reply_grace_secs)
EOF
...
xo_pending_reply_mark_delivered() {  # <state-dir> <corr_id> [confirmed-epoch]
  ...
  delivered=$(xo_pending_reply_get "$rec" delivered_epoch)
  if [ -z "$delivered" ]; then
    now=${confirmed_epoch:-$(xo_pending_reply_now)}
    xo_pending_reply_set "$rec" delivered_epoch "$now" || return 1
  fi
  if [ "$phase" = delivery_unknown ]; then
    xo_pending_reply_set "$rec" phase awaiting_report || return 1
  fi
  return 0
}

xo_pending_reply_delivery_confirmation_path() {  # <state-dir> <corr_id>
  printf '%s/.delivery-confirmed-%s' "$(xo_pending_reply_dir "$1")" "$2"
}

xo_pending_reply_write_delivery_confirmation() {  # <state-dir> <corr_id> <state> <value>
  local pending_state=$1 corr=$2 delivery_state=$3 value=$4 marker dir tmp
  marker=$(xo_pending_reply_delivery_confirmation_path "$pending_state" "$corr")
  dir=$(dirname "$marker")
  mkdir -p "$dir" || return 1
  tmp="$marker.tmp.$$"
...
xo_pending_reply_prepare_delivery() {  # <state-dir> <corr_id>
  ...
  [ -f "$marker" ] && return 0
  now=$(xo_pending_reply_now)
  xo_pending_reply_write_delivery_confirmation "$state" "$corr" attempted "$now"
}

xo_pending_reply_confirm_delivery() {  # <state-dir> <corr_id>
  local state=$1 corr=$2 lock rc=0
  local STATE XO_WAKE_QUEUE XO_WAKE_QUEUE_LOCK
  STATE=$state
  lock="$state/.pending-reply-$corr.lock"
  . "$_XO_PENDING_REPLY_LIB_DIR/xo-wake-lib.sh"
  xo_lock_acquire_wait "$lock" || return 1
  _xo_pending_reply_confirm_delivery_locked "$@" || rc=$?
  xo_lock_release "$lock"
  return "$rc"
}

_xo_pending_reply_confirm_delivery_locked() {  # <state-dir> <corr_id>
  local state=$1 corr=$2 now marker
  marker=$(xo_pending_reply_delivery_confirmation_path "$state" "$corr")
  if ! xo_pending_reply_prepare_delivery "$state" "$corr"; then
    return 1
  fi
  now=$(xo_pending_reply_now)
  xo_pending_reply_write_delivery_confirmation "$state" "$corr" confirmed "$now" || return 1
  if xo_pending_reply_mark_delivered "$state" "$corr" "$now"; then
...
xo_pending_reply_mark_delivery_unknown() {  # <state-dir> <corr_id>
  ...
  xo_pending_reply_set "$rec" phase delivery_unknown
}

_xo_pending_reply_reconcile_delivery_locked() {  # <state-dir> <corr_id>
  ...
  case "$delivery_state" in
    confirmed)
      ...
      xo_pending_reply_mark_delivered "$state" "$corr" "$epoch" || return 1
      rm -f "$marker" 2>/dev/null || true
      return 0
      ;;
    attempted)
      epoch=$value
      case "$epoch" in ''|*[!0-9]*) return 1 ;; esac
      grace=$(xo_pending_reply_get "$rec" grace_secs)
      case "$grace" in ''|*[!0-9]*) grace=$(xo_pending_reply_grace_secs) ;; esac
      now=$(xo_pending_reply_now)
      age=$((now - epoch))
      [ "$age" -ge "$grace" ] || return 1
      phase=$(xo_pending_reply_get "$rec" phase)
      [ "$phase" = awaiting_report ] || return 1
      xo_pending_reply_set "$rec" phase delivery_unknown || return 1
      return 0
      ;;
  esac
  return 1
...
xo_pending_reply_reconcile_delivery() {  # <state-dir> <corr_id>
  local state=$1 corr=$2 lock rc=0
  local STATE XO_WAKE_QUEUE XO_WAKE_QUEUE_LOCK
  STATE=$state
  lock="$state/.pending-reply-$corr.lock"
  . "$_XO_PENDING_REPLY_LIB_DIR/xo-wake-lib.sh"
  xo_lock_acquire_wait "$lock" || return 1
  _xo_pending_reply_reconcile_delivery_locked "$@" || rc=$?
  xo_lock_release "$lock"
  return "$rc"
}

xo_pending_reply_delivery_attempt_unresolved() {  # <state-dir> <corr_id>
...
xo_pending_reply_reset_known_undelivered() {  # <state-dir> <corr_id>
  local state=$1 corr=$2 lock rc=0
  local STATE XO_WAKE_QUEUE XO_WAKE_QUEUE_LOCK
  STATE=$state
  lock="$state/.pending-reply-$corr.lock"
  . "$_XO_PENDING_REPLY_LIB_DIR/xo-wake-lib.sh"
  xo_lock_acquire_wait "$lock" || return 1
  _xo_pending_reply_reset_known_undelivered_locked "$@" || rc=$?
  xo_lock_release "$lock"
  return "$rc"
}

_xo_pending_reply_reset_known_undelivered_locked() {  # <state-dir> <corr_id>
...
xo_pending_reply_discard_undelivered() {  # <state-dir> <corr_id>
...
xo_pending_reply_line_resolves() {  # <line> <corr_id>
  local line=$1 corr=$2
  [ -n "$line" ] && [ -n "$corr" ] || return 1
  case "$line" in
    *pending-reply-missed*) return 1 ;;
  esac
  xo_pending_reply_text_has_corr "$line" "$corr"
}

# Scan a status file for a correlated resolve. Prints the matching line or empty.
xo_pending_reply_find_resolve_line() {  # <status-file> <corr_id>
  local status_file=$1 corr=$2 line
  [ -f "$status_file" ] || return 0
  while IFS= read -r line || [ -n "$line" ]; do
    [ -n "$line" ] || continue
    if xo_pending_reply_line_resolves "$line" "$corr"; then
      printf '%s' "$line"
      return 0
    fi
  done < "$status_file"
...
xo_pending_reply_file_signature() {  # <path>
  local path=$1
...
xo_pending_reply_status_set_signature() {  # <status-dir>
  local status_dir=$1 status_file signature
  {
    for status_file in "$status_dir"/*.status; do
      [ -f "$status_file" ] || continue
      signature=$(xo_pending_reply_file_signature "$status_file")
...
xo_pending_reply_resolve_via_of_line() {  # <line>
  local line=$1
  case "$line" in
    *data/*report*|*report.md*|*document*|*pointer*)
      printf 'document'
      ;;
    *via-helper*|*xo-secondmate-report*)
      printf 'helper'
      ;;
    *)
      printf 'status'
...
xo_pending_reply_try_resolve() {  # <state-dir> <corr_id> [status-file-override]
  # Serialized per correlation so a resolution and an escalation cannot interleave.
  # bin/xo-wake-lib.sh owns the lock primitives but assigns its own globals when
  ...
  . "$_XO_PENDING_REPLY_LIB_DIR/xo-wake-lib.sh"
  xo_lock_acquire_wait "$lock" || return 1
  _xo_pending_reply_try_resolve_locked "$@" || rc=$?
  xo_lock_release "$lock"
  return "$rc"
}

_xo_pending_reply_try_resolve_locked() {  # <state-dir> <corr_id> [status-file-override]
  ...
  if [ "$phase" = resolved ]; then
    _xo_pending_reply_close_escalation_locked "$state" "$corr" || true
    return 0
  fi
  delivered=$(xo_pending_reply_get "$rec" delivered_epoch)
  if [ -z "$delivered" ]; then
  ...
  status_file=${status_override:-$(xo_pending_reply_get "$rec" parent_status)}
  if [ -z "$status_override" ] && [ "$unconfirmed" = 0 ]; then
    signature=$(xo_pending_reply_file_signature "$status_file")
    previous=$(xo_pending_reply_get "$rec" parent_status_scan_signature)
    [ "$signature" != "$previous" ] || return 1
  fi
  line=$(xo_pending_reply_find_resolve_line "$status_file" "$corr")
  if [ -z "$line" ]; then
    if [ -z "$status_override" ] && [ "$unconfirmed" = 0 ]; then
      xo_pending_reply_set "$rec" parent_status_scan_signature "$signature" || return 1
    fi
    return 1
  fi
  via=$(xo_pending_reply_resolve_via_of_line "$line")
  now=$(xo_pending_reply_now)
  xo_pending_reply_set "$rec" phase resolved || return 1
  if [ -z "$delivered" ]; then
    xo_pending_reply_mark_delivered "$state" "$corr" "$now" || return 1
    rm -f "$marker" 2>/dev/null || true
  fi
  xo_pending_reply_set "$rec" resolved_epoch "$now" || return 1
  xo_pending_reply_set "$rec" resolved_via "$via" || return 1
...
xo_pending_reply_observe_busy() {  # <state-dir> <corr_id> <busy_state>
  ...
  seen=$(xo_pending_reply_get "$rec" "$field_seen")
  completed=$(xo_pending_reply_get "$rec" "$field_completed")
  case "$busy_state" in
    busy)
      if [ "$seen" != 1 ]; then
        xo_pending_reply_set "$rec" "$field_seen" 1 || return 1
      fi
      ;;
    idle)
      if [ -z "$completed" ]; then
        ...
        if [ "$seen" = 1 ] || [ "$seen" = 0 ]; then
          now=$(xo_pending_reply_now)
          xo_pending_reply_set "$rec" "$field_completed" "$now" || return 1
        fi
      fi
      ;;
    unknown)
...
xo_pending_reply_fallback_idle_eligible() {  # <record-path>
  local rec=$1 phase start seen grace now age
  phase=$(xo_pending_reply_get "$rec" phase)
  case "$phase" in
    awaiting_report)
      start=$(xo_pending_reply_get "$rec" delivered_epoch)
      seen=$(xo_pending_reply_get "$rec" turn_seen_busy)
      ;;
    recovery_sent)
      start=$(xo_pending_reply_get "$rec" recovery_sent_epoch)
      seen=$(xo_pending_reply_get "$rec" recovery_turn_seen_busy)
      ;;
    *) return 1 ;;
  esac
  [ "$seen" = 1 ] && return 0
...
# xo_pending_reply_backend_observation: one busy/idle observation of a
# SECONDMATE endpoint, without ever reading its conversation.
#
# Deliberately NOT the semantic busy-state contract (bin/xo-busy-lib.sh).
# That contract covers ordinary task workers, whose turn lifecycle xo
...
# category as the submit acknowledgement matcher in bin/xo-composer-lib.sh - never task
# state, and never a source consumers can confuse with semantic state.
#
# It stays harness-scoped (xo_busy_lines_match with the recorded harness, no
...
xo_pending_reply_backend_observation() {  # <backend> <target> [expected-label] [harness]
  local backend=$1 target=$2 expected_label=${3-} harness=${4-} native tail40
  native=$(xo_backend_busy_state "$backend" "$target" 2>/dev/null || printf 'unknown')
  case "$native" in
    busy|idle) printf '%s' "$native"; return 0 ;;
  esac
  tail40=$(xo_backend_capture "$backend" "$target" 40 "$expected_label" 2>/dev/null) \
    || { printf 'unknown'; return 0; }
  if printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
    | xo_busy_lines_match "$harness"; then
    printf 'busy'
  else
...
xo_pending_reply_busy_state_from_observation() {  # <record-path> <observation>
  local rec=$1 observation=$2
  case "$observation" in
    busy|idle|unknown) printf '%s' "$observation" ;;
    fallback-idle)
      if xo_pending_reply_fallback_idle_eligible "$rec"; then
        printf 'idle'
      else
        printf 'unknown'
      fi
...
xo_pending_reply_mark_turn_completed() {  # <state-dir> <corr_id> [which: request|recovery]
  ...
  now=$(xo_pending_reply_now)
  xo_pending_reply_set "$rec" "$field" "$now" || return 1
...
# through the asynchronous mirror in bin/xo-procevent-remote-reply.sh, so the
...
xo_pending_reply_remote_channel_watermark_path() {  # <state-dir> <task_id>
...
xo_pending_reply_note_remote_channel_caught_up() {  # <state-dir> <task_id> [epoch]
  local state=$1 task_id=$2 epoch=${3-} path dir tmp
  [ -n "$state" ] && [ -n "$task_id" ] || return 2
  case "$epoch" in ''|*[!0-9]*) epoch=$(xo_pending_reply_now) ;; esac
  path=$(xo_pending_reply_remote_channel_watermark_path "$state" "$task_id")
  dir=$(dirname "$path")
  mkdir -p "$dir" || return 1
  chmod 700 "$dir" 2>/dev/null || true
  [ ! -L "$path" ] || return 1
...
xo_pending_reply_remote_channel_epoch() {  # <state-dir> <task_id>
  local path epoch
  path=$(xo_pending_reply_remote_channel_watermark_path "$1" "$2")
...
xo_pending_reply_target_is_remote() {  # <state-dir> <task_id>
  local meta="$1/$2.meta"
  [ -f "$meta" ] || return 1
  [ -n "$(xo_meta_get "$meta" remote_host)" ]
}
...
xo_pending_reply_missing_report_is_evidence() {  # <state-dir> <task_id> <since-epoch>
  local state=$1 task_id=$2 since=$3 caught
  xo_pending_reply_target_is_remote "$state" "$task_id" || return 0
  case "$since" in ''|*[!0-9]*) return 1 ;; esac
  caught=$(xo_pending_reply_remote_channel_epoch "$state" "$task_id")
  [ -n "$caught" ] || return 1
  [ "$caught" -ge "$since" ]
}

# Build the one automatic recovery message for a pending record.
xo_pending_reply_recovery_message() {  # <record-path>
  local rec=$1 corr summary token msg
  corr=$(xo_pending_reply_get "$rec" corr_id)
  summary=$(xo_pending_reply_get "$rec" request_summary)
  token=$(xo_pending_reply_corr_token "$corr")
  ...
  xo_pending_reply_embed_corr "$msg" "$corr" msg
  printf '%s' "$msg"
}

# Deliver the recovery message once. Caller must hold phase awaiting_report with
# turn completed and grace elapsed. Uses XO_PENDING_REPLY_SEND_HOOK when set
# (tests), otherwise invokes xo-send with XO_PENDING_REPLY_EXISTING_CORR so a
# second expectation is not created.
xo_pending_reply_send_recovery() {  # <state-dir> <corr_id>
  ...
  phase=$(xo_pending_reply_get "$rec" phase)
  [ "$phase" = awaiting_report ] || return 1
  attempted=$(xo_pending_reply_get "$rec" recovery_attempted_epoch)
  if [ -n "$attempted" ]; then
    xo_pending_reply_reconcile_recovery "$state" "$corr" || true
    return 1
  fi
  completed=$(xo_pending_reply_get "$rec" request_turn_completed_epoch)
  ...
  age=$((now - delivered))
  [ "$age" -ge "$grace" ] || return 1
  task_id=$(xo_pending_reply_get "$rec" task_id)
  # A remote mate's report may exist and simply not have been mirrored yet.
  xo_pending_reply_missing_report_is_evidence "$state" "$task_id" "$completed" || return 1
  parent_home=$(xo_pending_reply_get "$rec" parent_home)
  msg=$(xo_pending_reply_recovery_message "$rec")
  sender_pid=${BASHPID:-$$}
  sender_identity=$(xo_pending_reply_pid_identity "$sender_pid") || return 1
  xo_pending_reply_set "$rec" recovery_sender_pid "$sender_pid" || return 1
  xo_pending_reply_set "$rec" recovery_sender_identity "$sender_identity" || return 1
  xo_pending_reply_set "$rec" recovery_attempted_epoch "$now" || return 1
  xo_pending_reply_set "$rec" phase recovery_sending || return 1
  if [ -n "${XO_PENDING_REPLY_SEND_HOOK:-}" ]; then
    # Hook receives: task_id message
    # shellcheck disable=SC2086
    if ! eval "$XO_PENDING_REPLY_SEND_HOOK" "$(printf '%q' "$task_id")" "$(printf '%q' "$msg")"; then
      send_status=1
    fi
  else
    if [ -z "$parent_home" ] || [ ! -d "$parent_home" ]; then
      send_status=1
    elif ! env XO_HOME="$parent_home" XO_PENDING_REPLY_EXISTING_CORR="$corr" \
      "$_XO_PENDING_REPLY_LIB_DIR/xo-send.sh" "$task_id" "$msg"; then
      send_status=1
    fi
  fi
  if [ "$send_status" = 0 ]; then
    xo_pending_reply_finish_recovery "$state" "$corr" confirmed
    return $?
  fi
  xo_pending_reply_finish_recovery "$state" "$corr" failed || return 1
  return 1
}

xo_pending_reply_pid_identity() {  # <pid>
...
xo_pending_reply_sender_alive() {  # <record-path>
  local rec=$1 pid expected actual
  pid=$(xo_pending_reply_get "$rec" recovery_sender_pid)
  expected=$(xo_pending_reply_get "$rec" recovery_sender_identity)
  [ -n "$expected" ] || return 1
  actual=$(xo_pending_reply_pid_identity "$pid") || return 1
  [ "$actual" = "$expected" ]
}

xo_pending_reply_finish_recovery() {  # <state-dir> <corr_id> <confirmed|failed>
  local state=$1 corr=$2 outcome=$3 rec phase now sent
  rec=$(xo_pending_reply_path "$state" "$corr")
  [ -f "$rec" ] || return 1
  phase=$(xo_pending_reply_get "$rec" phase)
  [ "$phase" = recovery_sending ] || return 1
  xo_pending_reply_set "$rec" recovery_delivery_outcome "$outcome" || return 1
  if [ "$outcome" = confirmed ]; then
    sent=$(xo_pending_reply_get "$rec" recovery_sent_epoch)
    if [ -z "$sent" ]; then
      now=$(xo_pending_reply_now)
      xo_pending_reply_set "$rec" recovery_sent_epoch "$now" || return 1
    fi
    xo_pending_reply_set "$rec" recovery_turn_seen_busy 0 || return 1
    xo_pending_reply_set "$rec" recovery_turn_completed_epoch "" || return 1
    xo_pending_reply_set "$rec" phase recovery_sent || return 1
  else
    [ "$outcome" = failed ] || return 1
    xo_pending_reply_set "$rec" phase recovery_failed || return 1
  fi
}

xo_pending_reply_reconcile_recovery() {  # <state-dir> <corr_id>
  ...
  outcome=$(xo_pending_reply_get "$rec" recovery_delivery_outcome)
  case "$outcome" in
    confirmed) xo_pending_reply_finish_recovery "$state" "$corr" confirmed; return $? ;;
    failed) xo_pending_reply_finish_recovery "$state" "$corr" failed; return $? ;;
    unknown)
      xo_pending_reply_set "$rec" phase recovery_unknown || return 1
      return 0
      ;;
  esac
  xo_pending_reply_sender_alive "$rec" && return 1
  xo_pending_reply_set "$rec" recovery_delivery_outcome unknown || return 1
  xo_pending_reply_set "$rec" phase recovery_unknown || return 1
}
...
xo_pending_reply_escalation_key() {  # <corr_id>
  printf 'pending-reply-%s' "$1"
}

# Close-note body the reserved-key fold accepts as this library's resolution.
# The fold's guard (bin/xo-classify-lib.sh _xo_decision_key_transition_allowed)
...
xo_pending_reply_resolved_note() {  # <task-id> <corr_id> <via> [extra]
...
# namespace (pending-reply-<corr>). xo-send --resolve-key uses this so an
# operator close speaks the same vocabulary as xo_pending_reply_close_escalation
# instead of writing a silent no-op answered: note.
xo_pending_reply_close_note_for_key() {  # <key> <task-id> <via> [extra]
  case "$1" in
    pending-reply-*)
      xo_pending_reply_resolved_note "$2" "${1#pending-reply-}" "$3" "${4:-}"
      ;;
    *) return 1 ;;
  esac
}

xo_pending_reply_escalation_payload() {  # <record-path> <kind>
...
xo_pending_reply_escalation_line() {  # <status-file> <record-path> <corr_id>
  local status_file=$1 rec=$2 corr=$3 line found='' kind payload own_key
  [ -f "$status_file" ] || return 0
  [ "$(xo_pending_reply_get "$rec" corr_id)" = "$corr" ] || return 0
  own_key=$(xo_pending_reply_escalation_key "$corr")
  while IFS= read -r line || [ -n "$line" ]; do
    [ "$(status_line_verb "$line")" = blocked ] || continue
    for kind in missed delivery-unknown recovery-delivery; do
      payload=$(xo_pending_reply_escalation_payload "$rec" "$kind") || continue
      case "$line" in
...
# bin/xo-classify-lib.sh's fold. Records that never escalated are left untouched.
xo_pending_reply_close_escalation() {  # <state-dir> <corr_id>
  # Serialized per correlation so a resolution and an escalation cannot interleave.
  # bin/xo-wake-lib.sh owns the lock primitives but assigns its own globals when
  # sourced, so they are declared local here: that contains them to this call
  # instead of leaking into every script that sources this library, without the
  ...
  . "$_XO_PENDING_REPLY_LIB_DIR/xo-wake-lib.sh"
  xo_lock_acquire_wait "$lock" || return 1
  _xo_pending_reply_close_escalation_locked "$@" || rc=$?
  xo_lock_release "$lock"
  return "$rc"
}

_xo_pending_reply_close_escalation_locked() {  # <state-dir> <corr_id>
  ...
  rec=$(xo_pending_reply_path "$state" "$corr")
  [ -f "$rec" ] || return 1
  [ "$(xo_pending_reply_get "$rec" phase)" = resolved ] || return 0
  escalated=$(xo_pending_reply_get "$rec" escalated_epoch)
  [ -n "$escalated" ] || return 0
  closed=$(xo_pending_reply_get "$rec" escalation_closed_epoch)
  [ -z "$closed" ] || return 0
  parent_status=$(xo_pending_reply_get "$rec" parent_status)
  [ -n "$parent_status" ] || return 1
  escalation=$(xo_pending_reply_escalation_line "$parent_status" "$rec" "$corr")
  if [ -n "$escalation" ]; then
    key=$(_xo_decision_key "$escalation") || key=''
    ...
    while IFS= read -r open_line; do
      ...
      # self-announced append (bin/xo-wake-lib.sh, sourced by this function's
      ...
      _task=$(xo_pending_reply_get "$rec" task_id)
      _via=$(xo_pending_reply_get "$rec" resolved_via)
      close_line="resolved [key=${key}]: $(xo_pending_reply_resolved_note "$_task" "$corr" "$_via")"
      close_rc=0
      xo_wake_status_append_self_announced "${parent_status%/*}" "$parent_status" "$close_line" \
        2>/dev/null || close_rc=$?
...
xo_pending_reply_maybe_escalate() {  # <state-dir> <corr_id>
  ...
  . "$_XO_PENDING_REPLY_LIB_DIR/xo-wake-lib.sh"
  xo_lock_acquire_wait "$lock" || return 1
  _xo_pending_reply_maybe_escalate_locked "$@" || rc=$?
  xo_lock_release "$lock"
  return "$rc"
}

_xo_pending_reply_maybe_escalate_locked() {  # <state-dir> <corr_id>
  local state=$1 corr=$2
  ...
  phase=$(xo_pending_reply_get "$rec" phase)
  if [ "$phase" = delivery_unknown ]; then
    _xo_pending_reply_reconcile_delivery_locked "$state" "$corr" || true
    phase=$(xo_pending_reply_get "$rec" phase)
    [ "$phase" = delivery_unknown ] || return 0
  fi
  case "$phase" in
    recovery_sent)
      completed=$(xo_pending_reply_get "$rec" recovery_turn_completed_epoch)
      [ -n "$completed" ] || return 1
      ...
      xo_pending_reply_missing_report_is_evidence "$state" \
        "$(xo_pending_reply_get "$rec" task_id)" "$completed" || return 1
      ;;
  ...
  if [ -n "$delivered" ] && [ -f "$meta" ]; then
    ...
    sm_home=$(xo_meta_get "$meta" home)
    if [ -z "$remote_host" ] && [ -n "$sm_home" ]; then
      xo_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" || true
      xo_pending_reply_restatement_copy_same_basename "$state" "$corr" "$sm_home" || true
    fi
  fi
  # Resolve wins if a late report arrived between completion and this call.
  if _xo_pending_reply_try_resolve_locked "$state" "$corr"; then
    return 0
  fi
  parent_status=$(xo_pending_reply_get "$rec" parent_status)
  case "$phase" in
  ...
  payload=$(xo_pending_reply_escalation_payload "$rec" "$kind") || return 1
  if [ "$kind" = missed ]; then
    first=$(xo_pending_reply_get "$rec" wrong_home_first_sighting)
    if display=$(xo_pending_reply_sighting_display "$first"); then
      payload="$payload token seen in $display; parent channel has no corr="
    fi
  fi
  [ -n "$parent_status" ] || return 1
  mkdir -p "$(dirname "$parent_status")" 2>/dev/null || return 1
  line="blocked [key=$(xo_pending_reply_escalation_key "$corr")]: $payload"
  if ! grep -Fqx "$line" "$parent_status" 2>/dev/null; then
    printf '%s\n' "$line" >> "$parent_status" 2>/dev/null || return 1
  fi
  now=$(xo_pending_reply_now)
  xo_pending_reply_set "$rec" escalated_epoch "$now" || return 1
  xo_pending_reply_set "$rec" phase escalated || return 1
  return 0
}
...
xo_pending_reply_detect_wrong_home() {  # <state-dir> <corr_id> <secondmate-home>
  ...
  delivered=$(xo_pending_reply_get "$rec" delivered_epoch)
  [ -n "$delivered" ] || return 0
  snapshot=$(xo_pending_reply_status_set_signature "$sm_home/state")
  previous=$(xo_pending_reply_get "$rec" wrong_home_scan_signature)
  ...
  sightings=$(xo_pending_reply_get "$rec" wrong_home_sightings)
  # shellcheck source=bin/xo-parent-channel-lib.sh
  . "$_XO_PENDING_REPLY_LIB_DIR/xo-parent-channel-lib.sh"
  if xo_parent_channel_destination "$sm_home" "$sm_home/state" >/dev/null 2>&1 \
    && [ "$XO_PARENT_CHANNEL_ROUTE" = remote ]; then
  ...
  for status_file in "$sm_home"/state/*.status; do
    ...
    sighting_base=$(xo_pending_reply_sighting_encode "$status_file" 0) || continue
  ...
  if [ -n "$first" ] && [ -z "$(xo_pending_reply_get "$rec" wrong_home_first_sighting)" ]; then
    xo_pending_reply_set "$rec" wrong_home_first_sighting "$first" || return 1
  fi
  if [ "$changed" = 1 ]; then
    xo_pending_reply_set "$rec" wrong_home_sightings "$sightings" || return 1
    xo_pending_reply_set "$rec" wrong_home_hits "$hits" || return 1
  fi
  xo_pending_reply_set "$rec" wrong_home_scan_signature "$snapshot" || return 1
  return 0
}
...
xo_pending_reply_restatement_copy_same_basename() {  # <state-dir> <corr_id> <secondmate-home>
  local state=$1 corr=$2 sm_home=$3
  ...
  line=$(xo_pending_reply_find_resolve_line "$stranded" "$corr")
  [ -n "$line" ] || return 1
  # shellcheck source=bin/xo-parent-channel-lib.sh
  . "$_XO_PENDING_REPLY_LIB_DIR/xo-parent-channel-lib.sh"
  xo_parent_channel_append_once "$parent_status" "$line"
}
...
xo_pending_reply_tick_one() {  # <state-dir> <corr_id> <busy_state> [secondmate-home]
  local state=$1 corr=$2 busy_state=$3 sm_home=${4-}
  local rec phase delivered
  rec=$(xo_pending_reply_path "$state" "$corr")
  [ -f "$rec" ] || return 1
  xo_pending_reply_reconcile_delivery "$state" "$corr" || true
  phase=$(xo_pending_reply_get "$rec" phase)
  delivered=$(xo_pending_reply_get "$rec" delivered_epoch)
  if [ -z "$delivered" ]; then
    case "$phase" in
      delivery_unknown) xo_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null || true ;;
      escalated) xo_pending_reply_try_resolve "$state" "$corr" >/dev/null 2>&1 || true ;;
    esac
    return 0
  fi
  ...
  case "$phase" in
    awaiting_report|recovery_sending)
      if [ -n "$(xo_pending_reply_get "$rec" recovery_attempted_epoch)" ]; then
        xo_pending_reply_reconcile_recovery "$state" "$corr" || true
        phase=$(xo_pending_reply_get "$rec" phase)
      fi
      ;;
  ...
  case "$phase" in
    ...
    escalated)
      ...
      if [ -n "$sm_home" ]; then
        xo_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" || true
        if xo_pending_reply_restatement_copy_same_basename "$state" "$corr" "$sm_home"; then
          if xo_pending_reply_try_resolve "$state" "$corr"; then
            return 0
          fi
        fi
    ...
    recovery_failed|recovery_unknown)
      xo_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null || true
      return 0
      ;;
  esac
  if [ -n "$sm_home" ]; then
  ...
  xo_pending_reply_observe_busy "$state" "$corr" "$busy_state" || true
  ...
  phase=$(xo_pending_reply_get "$rec" phase)
  if [ "$phase" = awaiting_report ]; then
    xo_pending_reply_send_recovery "$state" "$corr" 2>/dev/null || true
  fi
  phase=$(xo_pending_reply_get "$rec" phase)
  case "$phase" in
    recovery_sent|recovery_failed|recovery_unknown)
...
xo_pending_reply_tick() {  # <state-dir>
  ...
  for rec in "$dir"/*; do
    ...
    if [ "$phase" = resolved ]; then
      ...
      xo_pending_reply_close_escalation "$state" "$corr" || true
      continue
    fi
    xo_pending_reply_reconcile_delivery "$state" "$corr" || true
    phase=$(xo_pending_reply_get "$rec" phase)
    ...
    if [ -z "$delivered" ]; then
      case "$phase" in
        delivery_unknown|escalated)
          xo_pending_reply_tick_one "$state" "$corr" unknown "" || true
          ;;
      esac
      continue
    fi
    ...
    if [ -f "$meta" ]; then
      ...
      target=$(xo_backend_target_of_meta "$meta")
      sm_home=$(xo_meta_get "$meta" home)
      ...
      if [ -n "$target" ]; then
        ...
        if [ "$found" = 0 ]; then
          if [ -n "$remote_host" ]; then
            observation=$("$_XO_PENDING_REPLY_LIB_DIR/xo-on.sh" "$task_id" \
              xo-remote-secondmate-control.sh observe "$task_id" < /dev/null 2>/dev/null || printf 'unknown')
            case "$observation" in busy|idle|fallback-idle|unknown) ;; *) observation=unknown ;; esac
          else
            observation=$(xo_pending_reply_backend_observation "$backend" "$target" "$label" "$harness")
          fi
          observation_tasks+=("$task_id")
          observation_values+=("$observation")
        fi
        busy=$(xo_pending_reply_busy_state_from_observation "$rec" "$observation")
      fi
    fi
    xo_pending_reply_tick_one "$state" "$corr" "$busy" "$sm_home" || true
  done
  return 0
}

# True when any open (non-resolved) pending reply exists for a task.
xo_pending_reply_task_has_open() {  # <state-dir> <task_id>
  local state=$1 task_id=$2 dir rec phase tid
  dir=$(xo_pending_reply_dir "$state")
  [ -d "$dir" ] || return 1
  for rec in "$dir"/*; do
    [ -f "$rec" ] || continue
    tid=$(xo_pending_reply_get "$rec" task_id)
