...
# xo-config-inherit-lib.sh. Remote routes receive one durable marked reread nudge
...
Push the primary xo home's declared inherited local material into each
...
Environment overrides follow the rest of xo:
  XO_HOME            active xo home
  XO_ROOT_OVERRIDE  xo repo root
  XO_STATE_OVERRIDE state dir
  XO_DATA_OVERRIDE  data dir
  XO_CONFIG_OVERRIDE config dir
EOF
}
...
case "${1:-}" in
  ...
  "")
    ;;
  *)
    echo "usage: xo-config-push.sh [--help]" >&2
    exit 2
    ;;
esac
...
records=$(mktemp "${TMPDIR:-/tmp}/xo-config-push-records.XXXXXX" 2>/dev/null) || exit 1
reports=""
...
echo "config-push: $XO_HOME -> live secondmate homes"
...
while IFS='|' read -r id home _window meta; do
  ...
  if [ -n "$remote_host" ]; then
    ...
    remote_marker=$(xo_secondmate_nudge_marker_path "$STATE" "$id" 2>/dev/null || true)
    remote_pending=0
    if [ -f "$remote_marker" ] && [ "$(xo_meta_get "$remote_marker" remote)" = 1 ]; then remote_pending=1; fi
    if ! xo_secondmate_nudge_write "$STATE" "$id" "$home" "" remote \
      "$XO_REMOTE_SECOND_MATE_NUDGE_MESSAGE" 1; then
      echo "  config-reread: retry marker failed"
      errors=1
      xo_lock_release "$remote_lock" || true
      continue
    fi
    if remote_out=$(XO_CONFIG_INHERIT_LIVE=1 \
      ...
      if [ "$remote_nudge" -eq 1 ]; then
        if XO_HOME="$XO_HOME" XO_ROOT_OVERRIDE="$XO_ROOT" XO_STATE_OVERRIDE="$STATE" \
          "$SCRIPT_DIR/xo-send.sh" "xo-$id" "$XO_REMOTE_SECOND_MATE_NUDGE_MESSAGE" >/dev/null 2>&1; then
          rm -f -- "$remote_marker"
  ...
  report=$(mktemp "${TMPDIR:-/tmp}/xo-config-push-report.XXXXXX" 2>/dev/null) || {
  ...
  reread_pending=0
  if xo_config_reread_has_pending "$home_real" || xo_config_reread_has_staged "$XO_HOME" "$id"; then
    reread_pending=1
  fi
  if reread_out=$(XO_HOME="$XO_HOME" XO_ROOT_OVERRIDE="$XO_ROOT" \
    ...
    if [ -n "$(xo_config_reread_changed_items "$report")" ] || [ "$reread_pending" -eq 1 ]; then
