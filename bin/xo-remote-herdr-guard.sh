#!/usr/bin/env bash
# launchd exec target for the XO-owned dev.xo.herdr.xo-remote
# launch agent: make the Aqua login session own the xo-remote Herdr server.
#
# Usage:
#   xo-remote-herdr-guard.sh <herdr-path> <session>
#
# bin/xo-remote-doctor.sh renders the launch agent as the account's login
# shell running `exec <this script> <herdr> xo-remote` with
...
# docs/verification/runtime-backends.md under "xo-remote server birth and login-keychain access", so the final exec provides the complete supervision lifecycle.
#
...
# A takeover closes every pane in that session; the parent xo's
# secondmate liveness sweep relaunches its mates into the Aqua-born server.
# bin/xo-remote-herdr-owner-lib.sh owns the owner discovery and the birth
# markers; XO_REMOTE_HERDR_GUARD_STOP_WAIT_TENTHS (default 50) bounds the
...
HERDR_BIN=$1
SESSION=$2
[ -n "$HERDR_BIN" ] && [ -x "$HERDR_BIN" ] || { printf 'xo-remote-herdr-guard: herdr is not executable: %s\n' "$HERDR_BIN" >&2; exit 1; }
[ -n "$SESSION" ] || usage
command -v jq >/dev/null 2>&1 || { printf 'xo-remote-herdr-guard: jq does not resolve on the launch agent PATH\n' >&2; exit 1; }
STOP_WAIT_TENTHS=${XO_REMOTE_HERDR_GUARD_STOP_WAIT_TENTHS:-50}

log() { printf 'xo-remote-herdr-guard: %s\n' "$*"; }
...
OWNER=$(xo_remote_herdr_socket_owner "$SOCKET"); OWNER_RC=$?
...
if xo_remote_herdr_birth_is_aqua "$BIRTH"; then
