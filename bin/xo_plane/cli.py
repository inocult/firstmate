...
Config is private at XO_HOME/config/plane.json. A confirmed shared claim is
...
def task_receipt(task):
    ...
    return Path(os.environ.get("XO_DATA_OVERRIDE", str(home() / "data"))).resolve() / task / "plane.json"
...
def bind_task(service, ticket, execution, task, mode):
    ...
    result = subprocess.run([str(ROOT / "bin/xo-brief.sh"), task, service.config["repository_url"],
                             "--mode", mode], capture_output=True, text=True, timeout=30)
    if result.returncode:
        raise AdapterError("XO brief scaffold failed; shared claim remains bound for recovery")
...
def merged_pr(pr):
    ...
    headers = {"Accept": "application/vnd.github+json", "User-Agent": "xo-plane-adapter"}
...
def main():
    ...
    parser.add_argument("--config", help="private config path; default XO_HOME/config/plane.json")
