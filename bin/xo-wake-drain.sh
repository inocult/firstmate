...
# XO_STATUS_PRESENTATION_LOCK_TIMEOUT sets the positive whole-second wait for
...
PRESENTATION_LOCK_TIMEOUT=${XO_STATUS_PRESENTATION_LOCK_TIMEOUT:-10}
...
# main (XO_SUPERVISION_ACTOR unset or "main", via xo-lease-lib.sh's xo_lease_actor
# - the same actor identity xo-send.sh/xo-control.sh/xo-teardown.sh already use)
# claims every row not already granted to branch, then drains and acks only
# that claimed set. branch (XO_SUPERVISION_ACTOR=branch, injected
...
# .pi/extensions/lib/xo-branch-dispatch.ts is the single owner of that
...
rows_file_valid() { xo_wake_grant_rows_valid "$1"; }

reclaim_stale_branch_grant_locked() {
  [ -e "$ELIGIBLE_ROWS_FILE" ] || [ -L "$ELIGIBLE_ROWS_FILE" ] || return 0
  if ! xo_wake_branch_grant_live "$ELIGIBLE_ROWS_FILE" "$ELIGIBLE_OWNER_FILE"; then
...
retire_unconsumable_rows_locked() {
  ...
  if DRAIN_TMP=$(mktemp "$STATE/.wake-queue.retire.XXXXXX") \
    ...
    ' "$XO_WAKE_QUEUE"); then
    queued=$(awk 'END { print NR }' "$XO_WAKE_QUEUE")
    ...
    if _xo_atomic_replace "$DRAIN_TMP" "$XO_WAKE_QUEUE"; then
      DRAIN_TMP=
...
print_branch_held_notice() {
  local held seqs
  held=$(xo_wake_actor_pending_count branch "$ELIGIBLE_ROWS_FILE" "$ELIGIBLE_OWNER_FILE") || return 0
  [ "$held" -gt 0 ] || return 0
  seqs=$(xo_wake_grant_rows_valid "$ELIGIBLE_ROWS_FILE" \
...
write_rows_file_locked() { # <target> <source>
  ...
  _xo_atomic_replace "$source" "$target"
}
...
claim_main_rows_locked() {
  ...
  ' "$XO_WAKE_QUEUE" | LC_ALL=C sort -n > "$DRAIN_TMP" || return 1
...
case "${1:-}" in
  ...
  --ack-through)
    ...
    ;;
  *) echo "usage: xo-wake-drain.sh [--ack-through SEQUENCE --recovery-generation GENERATION]" >&2; exit 2 ;;
esac
...
# only when a guarded supervision script (xo-peek/xo-send/...) happens to run.
# Reuse xo-guard.sh's model-aware alarm and XO_GUARD_GRACE instead of duplicating
...
acknowledge_inactive_outcomes() { # <mode> <newline-separated-fingerprints>
  ...
  while IFS= read -r fingerprint; do
    ...
    "$SCRIPT_DIR/xo-inactive-reconcile.sh" "$mode" "$fingerprint" || return 1
  done <<< "$fingerprints"
}

BRANCH_OUTCOME_INDEX_VERSION=xo-branch-outcome-index-v1
...
load_branch_outcome_index() { # <task>
  ...
  size=$(_xo_status_file_size "$path") || { BRANCH_OUTCOME_INDEX_STATE=invalid; return 0; }
  size=${size//[[:space:]]/}
...
print_status_outcome_backstop_section() {  # <task-and-endpoint-snapshot>
  ...
  if [ -e "$store" ] || [ -L "$store" ]; then
    ...
    if ! xo_lock_acquire_wait_bounded "$lock" "$PRESENTATION_LOCK_TIMEOUT"; then
    ...
    if ! outcome_index_ready_ok "$ready"; then
      if ! "$SCRIPT_DIR/xo-branch-outcome.sh" processed-init --held-lock >/dev/null 2>&1 \
  ...
  while IFS=$(printf '\t') read -r task endpoint ident; do
    ...
    event=$XO_STATUS_SNAPSHOT_EVENT_LINE
    event_endpoint=$XO_STATUS_SNAPSHOT_EVENT_ENDPOINT
    [ "$receipt" -lt "$event_endpoint" ] || continue
    status_is_captain_relevant "$event" || continue
    verb=$(status_line_verb "$event")
    case "$verb" in
      needs-decision|blocked)
        key=$(_xo_decision_key "$event") || key=
    ...
    line="$task $event"
    xo_cap_line_var "$line" $((item_bytes - 1))
    line=$XO_LINE_CAP_LINE
    bytes=$(( ${#line} + 1 ))
...
EOF
  ...
  if [ -e "$store" ] || [ -L "$store" ]; then xo_lock_release "$lock"; fi
...
# xo-classify-lib.sh's status_open_decisions fold (via its cursor-backed
...
# xo-classify-lib.sh's "incremental (cursor-backed) open-decisions fold").
...
EOF
  ...
  # bin/xo-send.sh header).
  printf "OPEN DECISIONS: close one by answering it: bin/xo-send.sh <task> --resolve-key <key> '<answer>'\n" || return 1
}
...
# with no warning at all. bin/xo-captain-hold.sh's `diverged` owns which pairs
...
print_record_divergence_section() {
  ...
  # A non-positive bound is not a bound (bin/xo-timeout-lib.sh), so a bad
  # override falls back to the default rather than disabling the deadline.
  bound=${XO_DIVERGENCE_TIMEOUT:-20}
  ...
  diverged=$(xo_run_timed "$bound" "$SCRIPT_DIR/xo-captain-hold.sh" diverged 2>/dev/null) || return 0
  [ -n "$diverged" ] || return 0
...
EOF
  ...
  printf 'RECORD DIVERGENCE: reconcile each one - record the captain'"'"'s own words with bin/xo-captain-hold.sh answer <task> --decision-file <path>, or re-open the status decision when that resolution was not the captain'"'"'s word.\n' || return 1
}
...
print_status_presentation() {  # [<deduped-raw-rows>]
  ...
  if xo_lock_acquire_wait_bounded "$lock" "$PRESENTATION_LOCK_TIMEOUT"; then
    ...
    if [ "$lock_rc" -eq 124 ]; then
      holder_pid=${XO_LOCK_HELD_PID:-unknown}
  ...
  if [ "$rc" -eq 0 ] && [ -n "$rows" ]; then
    xo_wake_print_annotations "$rows" "$snapshot" || rc=1
    if [ "$rc" -eq 0 ]; then
      annotation_manifest=$(xo_wake_annotation_manifest "$rows") || rc=1
...
elif xo_lock_acquire_wait_bounded "$XO_WAKE_QUEUE_LOCK" "$PRESENTATION_LOCK_TIMEOUT"; then
  :
else
  lock_rc=$?
  if [ "$lock_rc" -eq 124 ]; then
    printf 'WAKE DRAIN SKIPPED: queue lock remains held by live pid %s after %ss; retry on the next drain.\n' \
      "${XO_LOCK_HELD_PID:-unknown}" "$PRESENTATION_LOCK_TIMEOUT"
    exit 0
  fi
...
if [ -n "$ACK_THROUGH" ]; then
  ...
  if [ "$ACTOR" = branch ]; then
    ...
    ' "$XO_WAKE_QUEUE" > "$DRAIN_TMP" || exit 1
  else
    ...
    ' "$XO_WAKE_QUEUE" > "$DRAIN_TMP" || exit 1
    xo_wake_commit_secondmate_stall_receipts_through "$ACK_THROUGH" "$MAIN_ROWS_FILE" || {
      echo "wake drain: secondmate stall receipt could not be recorded safely" >&2
      exit 1
    }
  fi
  ACK_REMOVED=$(( $(awk 'END { print NR }' "$XO_WAKE_QUEUE") - $(awk 'END { print NR }' "$DRAIN_TMP") ))
  if [ ! -s "$DRAIN_TMP" ]; then
    xo_recovery_marker_ack "$RECOVERY_MARKER" "$ACK_GENERATION"
    RECOVERY_ACK_STATUS=$?
    case "$RECOVERY_ACK_STATUS" in
      0) ;;
      3) RECOVERY_ACK_MOVED=true ;;
      *)
        echo "wake drain: recovery episode could not be retired safely; re-run bin/xo-wake-drain.sh and use the new WAKE_ACK_REQUIRED command" >&2
        exit 1
        ;;
    esac
  else
    xo_recovery_marker_snapshot "$RECOVERY_MARKER" || exit 1
    RECOVERY_MARKER_TOKEN=$XO_RECOVERY_MARKER_TOKEN
  ...
  if [ "$ACK_REMOVED" -eq 0 ] && [ "$PRESENTED_MAX" -gt "$ACK_THROUGH" ]; then
    ...
    case "$RECOVERY_MARKER_TOKEN" in
      pending:*|announced:*)
        printf 'wake drain: nothing was acknowledged through %s (none of your presented wake rows is at or below it); the current wake is row %s: run bin/xo-wake-drain.sh --ack-through %s --recovery-generation %s after handling it\n' \
          "$ACK_THROUGH" "$PRESENTED_MAX" "$PRESENTED_MAX" "${RECOVERY_MARKER_TOKEN##*:}" >&2
        ;;
      *)
        printf 'wake drain: nothing was acknowledged through %s (none of your presented wake rows is at or below it); the current wake is row %s: re-run bin/xo-wake-drain.sh and use the WAKE_ACK_REQUIRED command it prints\n' \
          "$ACK_THROUGH" "$PRESENTED_MAX" >&2
        ;;
    esac
  elif [ "$RECOVERY_ACK_MOVED" = true ]; then
    printf 'wake drain: acknowledged wakes through %s (%s row(s) consumed), but a newer recovery episode is pending; re-run bin/xo-wake-drain.sh and use the new WAKE_ACK_REQUIRED command\n' \
...
if [ ! -s "$XO_WAKE_QUEUE" ]; then
  : > "$XO_WAKE_QUEUE"
  xo_recovery_marker_snapshot "$RECOVERY_MARKER" || true
  RECOVERY_MARKER_TOKEN=$XO_RECOVERY_MARKER_TOKEN
  case "$RECOVERY_MARKER_TOKEN" in
    pending:downtime:*|announced:downtime:*)
      xo_recovery_marker_begin_handling "$RECOVERY_MARKER" || {
  ...
  if [ "$RECOVERY_ACK_REQUIRED" = true ]; then
    printf 'WAKE_ACK_REQUIRED: after handling completes run bin/xo-wake-drain.sh --ack-through 0 --recovery-generation %s\n' "${RECOVERY_MARKER_TOKEN##*:}" >&2
  fi
  assert_watcher_liveness
  exit 0
fi
...
if [ -z "$RECOVERY_MARKER_TOKEN" ]; then
  ...
  xo_recovery_marker_publish "$RECOVERY_MARKER" downtime || {
...
awk -F '\t' -v seqs="$ACTOR_ROWS_FILE" '
  ...
  NF >= 5 && ($2 in keep)
' "$XO_WAKE_QUEUE" > "$DRAIN_VIEW_TMP" || exit 1
RAW_ROWS=$(xo_wake_print_deduped "$DRAIN_VIEW_TMP") || exit "$?"
rm -f -- "$DRAIN_VIEW_TMP" || exit 1
DRAIN_VIEW_TMP=
ACK_THROUGH=$(printf '%s\n' "$RAW_ROWS" | awk -F '\t' '$2 ~ /^[0-9]+$/ && $2 > max { max=$2 } END { print max + 0 }') || exit 1
case "${XO_WAKE_DRAIN_TEST_DELAY_BEFORE_COMMIT:-0}" in
  0) ;;
  ''|*[!0-9]*) ;;
  *) sleep "$XO_WAKE_DRAIN_TEST_DELAY_BEFORE_COMMIT" ;;
esac
if [ -n "$RAW_ROWS" ]; then
  printf '%s\n' "$RAW_ROWS" || exit "$?"
fi
...
xo_lock_release "$XO_WAKE_QUEUE_LOCK"
DRAIN_LOCK_HELD=false
printf 'WAKE_ACK_REQUIRED: after handling completes run bin/xo-wake-drain.sh --ack-through %s --recovery-generation %s\n' \
