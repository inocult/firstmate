...
#     exact xo-turnend-guard.sh scope. Child crew/scout worktrees stay inert.
...
#     the hook delegates guarded recovery to bin/xo-lock.sh and then re-verifies
...
#     bin/xo-supervision-lib.sh defines it; an idle home exits 0.
...
#     continuation (xo_autoarm_claim_open/xo_autoarm_claim_next in
#     bin/xo-wake-lib.sh own the contract, including the legacy shim for a
#     pre-generation lock).
#   - Foreground arm: the owner runs bin/xo-watch-arm.sh in the FOREGROUND of
...
# (bin/xo-turnend-guard.sh --claude) can allow a stop whose recovery this hook
...
AUTOARM_ATTEMPTS=${XO_CLAUDE_AUTOARM_ATTEMPTS:-2}
...
# xo-watch.sh touches the liveness beacon once per cycle, immediately before
# its terminal wait, so a healthy watcher's beacon can legitimately age up to
# XO_POLL seconds between touches (docs/turnend-guard.md "Guard grace and the
# poll cadence"). xo_poll_derived_grace (bin/xo-wake-lib.sh) is the single
# owner of that max(300, poll+60) derivation.
GRACE=${XO_GUARD_GRACE:-$(xo_poll_derived_grace)}
...
xo_hook_payload_is_foreign_host "$PAYLOAD" && exit 0
...
if ! xo_session_lock_owned_by_self "$STATE"; then
  LOCK_PID=$(cat "$STATE/.lock" 2>/dev/null || true)
  case "$LOCK_PID" in
    ''|*[!0-9]*) exit 0 ;;
  esac
  xo_harness_pid_alive "$LOCK_PID" && exit 0
...
# --- need: whatever bin/xo-supervision-lib.sh counts as supervision need ------
need_supervision() {
  xo_supervision_needed "$STATE" "$GRACE"
}
need_supervision || exit 0

# --- stale session-lock recovery ---------------------------------------------
# Delegate the claim to xo-lock.sh so its live-owner refusal and write semantics
# remain the single acquisition owner, then re-verify current-session identity
# before touching any auto-arm state.
if [ "$RECOVER_SESSION_LOCK" -eq 1 ]; then
  "$SCRIPT_DIR/xo-lock.sh" >/dev/null 2>&1 || exit 0
  xo_session_lock_owned_by_self "$STATE" || exit 0
fi
...
# is superseded by taking the next generation (xo_autoarm_claim_open and
# xo_autoarm_claim_next in bin/xo-wake-lib.sh own the contract). No mutex is
...
# abandoned.
xo_autoarm_claim_open "$STATE" "$GRACE" && exit 0
xo_autoarm_claim_next "$STATE" "$GRACE"
CLAIM_RC=$?
if [ "$CLAIM_RC" -ne 0 ]; then
  [ "$CLAIM_RC" -eq 2 ] && exit 0
  ROLE=$(xo_lock_role "$OWNER_LOCK" 2>/dev/null || true)
  [ -n "$ROLE" ] || exit 0
  xo_autoarm_release_abandoned "$STATE" "$GRACE" || exit 0
  xo_autoarm_claim_next "$STATE" "$GRACE" || exit 0
fi
MY_GEN=$XO_AUTOARM_MY_GEN
[ -n "$MY_GEN" ] || exit 0
...
autoarm_commit() {  # <outcome> [marker-file]
  ...
  if [ "$outcome" = rewake ]; then
    xo_session_lock_owned_by_self "$STATE" || return 2
    session_pid=$(sed -n '1p' "$STATE/.lock" 2>/dev/null || true)
    xo_recovery_marker_snapshot "$STATE/.watcher-down" || return 2
    case "$XO_RECOVERY_MARKER_TOKEN" in
      pending:downtime:*|announced:downtime:*) recovery=${XO_RECOVERY_MARKER_TOKEN##*:} ;;
      *) return 2 ;;
    esac
    xo_autoarm_write_owned "$STATE" "$MY_GEN" "$outcome" "$marker" "$session_pid" "$recovery"
  elif [ -n "$marker" ]; then
    xo_autoarm_write_owned "$STATE" "$MY_GEN" "$outcome" "$marker"
  else
    xo_autoarm_write_owned "$STATE" "$MY_GEN" "$outcome"
  fi
}
...
autoarm_record() {  # <outcome>
  xo_autoarm_write_owned "$STATE" "$MY_GEN" "$1" >/dev/null 2>&1 || true
}

# X mode cadence: source the generated config so an X instance polls at its
# 30s cadence (xo-bootstrap.sh x_mode_setup contract).
...
while [ "$attempt" -lt "$AUTOARM_ATTEMPTS" ]; do
  ...
  if ! xo_autoarm_still_owner "$STATE" "$MY_GEN"; then
  ...
  if [ -n "$OUT" ]; then
    XO_GUARD_GRACE="$GRACE" "$SCRIPT_DIR/xo-watch-arm.sh" >"$OUT" 2>&1 || true
  else
    XO_GUARD_GRACE="$GRACE" "$SCRIPT_DIR/xo-watch-arm.sh" >/dev/null 2>&1 || true
  fi
  ...
  if xo_watcher_healthy "$STATE" "$SCRIPT_DIR/xo-watch.sh" "$GRACE" "$XO_HOME"; then
    HEALTHY=1
    break
  fi
...
if [ "$HEALTHY" -eq 1 ]; then
  xo_autoarm_reset_owned "$STATE" "$MY_GEN"
  RESET_RC=$?
...
if [ "$ACTIONABLE" -eq 1 ]; then
  ...
  {
    printf 'xo watcher wake - one supervision event needs a handling turn now.\n'
    [ -n "$OUT" ] && grep -E '^(signal:|stale:|check:|heartbeat)' "$OUT" 2>/dev/null | head -8
    printf 'Run bin/xo-wake-drain.sh first, handle the wake, then run its exact WAKE_ACK_REQUIRED --ack-through command. Until that post-handling acknowledgement, interruption leaves the wake durable for idempotent re-handling. This Stop hook owns watcher continuity: when the handling turn ends, the next needed cycle arms automatically - do NOT run bin/xo-watch-arm.sh after an ordinary wake.\n'
  } >&2
...
if [ ! -e "$FAILURE_NOTICE" ]; then
  ...
  {
    printf 'xo watcher auto-arm FAILED - the Stop-owned automatic supervision mechanism is broken after %s bounded attempts, and no live watcher with a fresh beacon was verified.\n' "$attempt"
