#!/usr/bin/env bash
# Post xo's composed answer back to the relay for a pending X-mode mention.
#
# Usage: xo-x-reply.sh <request_id> [--image <path>] <text>
#        xo-x-reply.sh <request_id> [--image <path>] --text-file <path>
#        xo-x-reply.sh <request_id> [--image <path>] -
#        xo-x-reply.sh <request_id> --followup [--image <path>] ...
#        xo-x-reply.sh <request_id> ... --receipt-file <path>
#
...
# posted (bin/xo-public-followup.sh, building a typed delivery receipt) does not
...
# xo-x-followup.sh can tell "exhausted binding" apart from a transient post
...
# call can instead see a benign no-op 200, so xo-x-followup.sh's local
...
# 9) rather than posting with a locally defaulted budget - xo holds and
# retries it.
#
...
reply_make_tmp_file() {
  ...
  file=$(mktemp "${TMPDIR:-/tmp}/xo-x-reply.XXXXXX") || return 1
  TMP_FILES+=("$file")
...
write_reply_receipt() {
  ...
  if ! (umask 077; jq -n --arg r "$REQ" --arg e "$ENDPOINT" --argjson c "$1" --argjson d "$2" \
    ...
    echo "xo-x-reply: warning: posted but could not write the receipt to $RECEIPT_FILE" >&2
  fi
}

usage() {
  echo "usage: xo-x-reply.sh <request_id> [--followup] [--image <path>] [--receipt-file <path>] <text> | ... --text-file <path> | ... -" >&2
}

help() {
  cat <<'EOF'
usage: xo-x-reply.sh <request_id> [--followup] [--image <path>] [--receipt-file <path>] <text>
       xo-x-reply.sh <request_id> [--followup] [--image <path>] [--receipt-file <path>] --text-file <path>
       xo-x-reply.sh <request_id> [--followup] [--image <path>] [--receipt-file <path>] -
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --image)
      shift
      if [ "$#" -lt 1 ] || [ -z "$1" ]; then
        echo "xo-x-reply: missing --image path" >&2
        usage
        exit 2
      fi
      IMAGE_PATH=$1
    ...
    --receipt-file)
      shift
      if [ "$#" -lt 1 ] || [ -z "$1" ]; then
        echo "xo-x-reply: missing --receipt-file path" >&2
        usage
        exit 2
      fi
      RECEIPT_FILE=$1
...
case "$1" in
  --text-file)
    if [ "$#" -lt 2 ]; then
      echo "usage: xo-x-reply.sh <request_id> [--followup] [--image <path>] --text-file <path>" >&2
      exit 2
    fi
    TEXT=$(cat -- "$2") || { echo "xo-x-reply: cannot read text file: $2" >&2; exit 1; }
    ;;
  -)
    TEXT=$(cat)
    ;;
...
if [ -z "$TEXT" ]; then
  echo "xo-x-reply: empty reply text" >&2
  exit 2
fi
...
case "$REQ" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "xo-x-reply: unsafe request_id: $REQ" >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "xo-x-reply: jq not found" >&2; exit 1; }

# Resolve the reply platform + split budget. An explicit env override wins per
# axis (xo-x-followup passes recorded task-link context this way); otherwise
...
if [ "$FOLLOWUP" = 1 ] && [ "$CONTEXT_RESOLVED" = 0 ]; then
  ...
  printf 'xo-x-reply: refusing follow-up for %s: could not authoritatively determine both the reply platform and explicit budget (local per-request context was incomplete%s). Hold and retry once both values are recoverable.\n' \
    "$REQ" "$relay_note" >&2
  exit 8
fi
...
case "$N" in ''|*[!0-9]*) echo "xo-x-reply: failed to split reply into a thread" >&2; exit 1 ;; esac
[ "$N" -gt 0 ] || { echo "xo-x-reply: empty reply text" >&2; exit 2; }
  ...
  409)
    if [ "$FOLLOWUP" = 1 ]; then
      if [ -s "$RESPONSE_BODY_FILE" ] && {
        ...
        echo "xo-x-reply: relay rejected the follow-up (confirmed followup_unavailable marker): HTTP 409" >&2
      else
        echo "xo-x-reply: relay rejected the follow-up (HTTP 409 cap/window exhaustion; marker absent)" >&2
      fi
      exit 9
    fi
    echo "xo-x-reply: relay returned HTTP $code" >&2
    exit 1
    ;;
  *) echo "xo-x-reply: relay returned HTTP $code" >&2; exit 1 ;;
esac
