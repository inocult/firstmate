#!/usr/bin/env bash
# Path-confined remote file transfer for xo-on.sh.
#
# Usage:
#   xo-remote-file.sh get <relative-path> [max-bytes]
#   xo-remote-file.sh put state/handoff/<id>.outbox.md <max-bytes> <bytes> <sha256> <generation>
#
# A get path is relative to XO_HOME, must resolve through ordinary directories
...
resolve_file() { # <relative-path>
  ...
  parent=$(dirname "$rel")
  base=$(basename "$rel")
  parent_real=$(CDPATH='' cd -- "$XO_HOME/$parent" 2>/dev/null && pwd -P) || die "file parent is unavailable: $rel"
  case "$parent_real" in "$home_real"|"$home_real"/*) ;; *) die "file escapes XO_HOME: $rel" ;; esac
...
put_handoff_file() { # <home-real> <name> <max-bytes> <relative-path> <bytes> <sha256> <generation>
  ...
  (
    ...
    [ ! -L "$name" ] || exit 3
    lock="./.$ID.upload.lock"
    xo_lock_acquire_wait "$lock" || exit 5
    tmp=
    generation_tmp=
    cleanup_put() {
      [ -z "$tmp" ] || rm -f -- "$tmp"
...
case "$COMMAND" in
  get)
    ...
    TMP=$(mktemp -d "${TMPDIR:-/tmp}/xo-remote-file.XXXXXX") \
