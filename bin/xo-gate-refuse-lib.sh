#!/usr/bin/env bash
# xo-gate-refuse-lib.sh - fail-closed refusal that keeps a no-mistakes GATE agent
# out of xo's fleet lifecycle.
#
# The hazard (data/nm-gate-ambient-authority-containment-c3/report.md): a
# no-mistakes gate agent runs inside an xo checkout with a free shell, so
# it auto-loads xo's AGENTS.md, adopts the captain identity, and - seeing
# crew "in flight" - reaches for xo-spawn.sh/xo-send.sh/xo-teardown.sh to
...
# THIS is the xo capability-removal half: an enforceable script refusal,
...
#
# A NORMAL xo session - a real primary checkout, a real treehouse/Orca
...
#
# This mirrors the unspoofable-marker precedent in bin/xo-marker-lib.sh: a signal
# the agent cannot forge, keyed on at a chokepoint, keeping the pattern familiar
# to xo maintainers. It layers ABOVE no-mistakes' separately-shipping
# HEAD-continuity guard, which remains the adversarial/residual backstop.
#
# TEST-HARNESS ESCAPE HATCH (XO_GATE_REFUSE_BYPASS=1): xo's own test suite
# must exercise the REAL xo-spawn/xo-send/xo-teardown, but the no-mistakes gate
...
# environment this guard refuses. So both signals would fire during xo's
# own validation and break unrelated tests. XO_GATE_REFUSE_BYPASS=1 makes the
# guard a no-op; xo's shared test helpers (tests/lib.sh and the backend
...
# bin/xo-spawn.sh directly after adopting xo's identity - it never sources
# xo's test helpers, so it never carries the bypass; and the adversarial
...
# tests/xo-gate-refuse.test.sh strips the bypass so it still verifies real refusal.
#
# Sourced by bin/xo-spawn.sh, bin/xo-send.sh, bin/xo-teardown.sh,
# bin/xo-sessionstart-nudge.sh, and the tests.
...
XO_GATE_REFUSE_EXIT=3

# xo_is_gate_agent: return 0 without output when this process looks like a
...
xo_is_gate_agent() {
  ...
  if [ "${XO_GATE_REFUSE_BYPASS:-}" = 1 ]; then
    return 1
  fi
  if [ "${NO_MISTAKES_GATE+x}" = x ]; then
    XO_GATE_REFUSE_REASON='env'
    return 0
  fi
  ...
  case "$common" in
    */.no-mistakes/repos/*.git)
      XO_GATE_REFUSE_REASON='path'
      XO_GATE_REFUSE_COMMON=$common
      return 0 ;;
  esac
  return 1
}

# xo_refuse_if_gate_agent: exit XO_GATE_REFUSE_EXIT with a clear stderr message if
# this process looks like a no-mistakes gate agent. Call before any fleet
# mutation. No-ops (returns 0) for a normal xo session, or when xo's
# own test harness sets XO_GATE_REFUSE_BYPASS=1 (see the header).
xo_refuse_if_gate_agent() {
  xo_is_gate_agent "${1:-.}" || return 0
  if [ "$XO_GATE_REFUSE_REASON" = env ]; then
    echo "error: no-mistakes gate agent must not drive the fleet (NO_MISTAKES_GATE set)" >&2
  else
    echo "error: refusing fleet lifecycle from inside a no-mistakes gate worktree ($XO_GATE_REFUSE_COMMON)" >&2
  fi
  exit "$XO_GATE_REFUSE_EXIT"
}
