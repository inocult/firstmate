#!/usr/bin/env bash
# xo-startup-network.sh - the deferred startup stage of a session start.
#
...
# whole XO_SESSION_START_TIMEOUT budget and truncate the digest outright, turning
...
#
# WHAT IS PRESERVED. Nothing is dropped. bin/xo-bootstrap.sh remains the single
# owner of every network sweep and still runs all of them, unchanged, via its
# XO_BOOTSTRAP_NETWORK=only phase. bin/xo-inactive-reconcile.sh remains the
...
#
# Usage: xo-startup-network.sh start --locked <0|1> --harvest-pid <pid>
...
#        xo-startup-network.sh run --locked <0|1>
...
#        xo-startup-network.sh harvest --pid <pid>
#          Print the digest's NETWORK CHECKS section and release the inline-print
#          claim. Called by bin/xo-session-start.sh, not by hand.
#        xo-startup-network.sh report
...
#        xo-startup-network.sh wait [<seconds>]
...
#                             bin/xo-bootstrap.sh produced it, plus a
...
#                             bin/xo-timing-lib.sh's tab-separated format: the
...
#
# The whole stage is bounded by XO_STARTUP_NETWORK_TIMEOUT (default 120s), one
...
# silence. bin/xo-timeout-lib.sh remains the single owner of bounded execution.
set -u
...
# xo-timing-lib.sh owns the per-step elapsed record this stage publishes beside
# its report. Recording is opt-in per run: it stays inert until cmd_run points
# XO_TIMING_LOG at a file, so nothing else that sources these scripts pays for it.
# shellcheck source=bin/xo-timing-lib.sh
. "$SCRIPT_DIR/xo-timing-lib.sh"
# xo-wake-lib.sh owns both the portable lock helpers used below and the durable
...
usage() {
  sed -n '2,/^set -u$/p' "$SCRIPT_DIR/xo-startup-network.sh" | sed 's/^# \{0,1\}//; $d'
}
...
stage_budget() {
  local budget=${XO_STARTUP_NETWORK_TIMEOUT:-120}
  case "$budget" in ''|*[!0-9]*|0) budget=120 ;; esac
  printf '%s' "$budget"
}

delivery_budget() {
  local budget=${XO_SESSION_START_TIMEOUT:-120}
  case "$budget" in ''|*[!0-9]*|0) budget=120 ;; esac
  printf '%s' "$budget"
}
...
cmd_start() {  # <locked> <harvest-pid>
  ...
  if [ "$locked" = 1 ] && ! xo_session_lock_owned_by_self "$STATE"; then
    return 1
  fi

  xo_lock_acquire_wait "$PUBLISH_LOCK"
...
EOF
  ...
  nohup "$SCRIPT_DIR/xo-startup-network.sh" run --locked "$locked" --lock-pid "$lock_pid" \
    --generation "$generation" \
...
# rewrites this value - bin/xo-lock.sh overwrites a dead holder's pid with its
...
EOF
    ...
    if [ "$claim_live" -eq 0 ]; then
      if report_requires_wake "$state"; then
        xo_wake_append check startup-network \
          "check: startup-network: deferred startup network checks finished ($state); read them with $XO_ROOT/bin/xo-startup-network.sh report" \
          || true
      fi
      xo_lock_release "$PUBLISH_LOCK"
      return 0
...
cmd_run() {  # <locked> <lock-pid> <generation>
  ...
  elif [ "$locked" = 1 ] && ! xo_session_lock_owned_by_self "$STATE"; then
    downgraded=1
    locked=0
  fi
  if [ "$locked" = 1 ]; then
...
EOF
  ...
  out=$(mktemp "${TMPDIR:-/tmp}/xo-startup-network.XXXXXX" 2>/dev/null) || return 1
  ...
  # Sweeps run in child processes (bin/xo-bootstrap.sh, and bin/xo-fleet-sync.sh
  # below it), so XO_TIMING_LOG is exported and appended to by all of them.
  timings=$(mktemp "${TMPDIR:-/tmp}/xo-startup-network-timings.XXXXXX" 2>/dev/null) || timings=
  [ -z "$timings" ] || xo_timing_start "$timings"
  stage_started=$(xo_timing_now_ms)
  rc=0
  if [ "$sweep_locked" -eq 1 ]; then
    xo_lock_acquire_wait "$STATE/.lock.acquire"
    lease_held=1
    if ! lock_unchanged "$lock_pid"; then
      sweep_locked=0
      phases=probe
  ...
  # owners only so xo_run_timed can govern them as one process group.
  if [ "$sweep_locked" -eq 1 ]; then
    # shellcheck disable=SC2016  # Child-shell variables expand inside the bound.
    xo_run_timed "$budget" env XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
      XO_BOOTSTRAP_NETWORK=only XO_BOOTSTRAP_NETWORK_LOCK_PID="$lock_pid" \
      bash -c '
        script_dir=$1
        "$script_dir/xo-inactive-reconcile.sh" scan --startup >/dev/null 2>&1 || true
        exec "$script_dir/xo-bootstrap.sh"
      ' _ "$SCRIPT_DIR" >"$out" 2>&1 || rc=$?
  else
    xo_run_timed "$budget" env XO_BOOTSTRAP_NETWORK=only XO_BOOTSTRAP_DETECT_ONLY=1 \
      "$SCRIPT_DIR/xo-bootstrap.sh" >"$out" 2>&1 || rc=$?
  fi
  [ "$lease_held" -eq 0 ] || xo_lock_release "$STATE/.lock.acquire"
  ...
  xo_timing_record stage network-checks "$stage_started" "$phases"
  ...
  case "$rc" in
    ...
    124)
      printf 'NETWORK_CHECKS: hit the %ss bound before finishing, so %s may be incomplete; rerun %s/bin/xo-startup-network.sh run --locked %s\n' \
        "$budget" "$(phase_label "$phases")" "$XO_ROOT" "$sweep_locked" >> "$out"
      publish "$generation" timeout "$phases" "$sweep_locked" "$started" "$rc" "$out" "$timings"
      ;;
    *)
      printf 'NETWORK_CHECKS: the deferred check worker exited %s, so %s may be incomplete; rerun %s/bin/xo-startup-network.sh run --locked %s\n' \
        "$rc" "$(phase_label "$phases")" "$XO_ROOT" "$sweep_locked" >> "$out"
...
print_finished() {  # <state>
  ...
  if [ "$report_published" = 0 ]; then
    printf 'NETWORK_CHECKS: could not publish the deferred check report, so %s results are unavailable; rerun %s/bin/xo-startup-network.sh run --locked %s\n' \
      "$(phase_label "$phases")" "$XO_ROOT" "$(status_get locked)"
...
print_pending() {
  ...
  printf 'The durable result is readable on demand with %s/bin/xo-startup-network.sh report; until it finishes, treat none of it as confirmed.\n' "$XO_ROOT"
}
  ...
  case "$(status_get state)" in
    ...
    running)
      if worker_alive; then
        ...
        printf 'NETWORK_CHECKS: the deferred check worker stopped before publishing, so %s did not complete; rerun %s/bin/xo-startup-network.sh run --locked %s\n' \
          "$(phase_label "$(status_get phases)")" "$XO_ROOT" "$(status_get locked)"
      fi
      ;;
...
case "$MODE" in
  ...
  -h|--help) usage ;;
  *)
    printf 'xo-startup-network: unknown mode: %s\n' "${MODE:-<none>}" >&2
    printf 'usage: xo-startup-network.sh start|run|harvest|report|wait\n' >&2
    exit 2
    ;;
esac
exit 0
