#!/usr/bin/env bash
# xo-mail-check.sh - recurring received-mail poll as a standing watcher check.
#
# Usage:
#   xo-mail-check.sh [check]
#   xo-mail-check.sh arm
#   xo-mail-check.sh disarm
#   xo-mail-check.sh --help
#
# `check` runs the mail poll from this home (sourcing the same .env and using
# the same inbox state as xo-mail.sh itself). It composes with the existing
# watcher state-check contract instead of needing a schedule of its own: a
# printed line becomes a `check:` wake so xo can drain durable
...
# XO_MAIL_USER, XO_MAIL_PASS, XO_IMAP_HOST, and XO_SMTP_HOST is reported once
...
# python3, missing xo-mail.sh, heal could not record a uid) that cannot
...
# (XO_CHECK_TIMEOUT, default 30, read from this check's own environment
# because the watcher runs it as a direct child). The internal budget
# XO_MAIL_CHECK_BUDGET (default 15, valid 5..25) is cut down to whatever fits
...
MAIL_BIN="$SCRIPT_DIR/xo-mail.sh"
REGISTER_BIN="$SCRIPT_DIR/xo-check-register.sh"
RECORD_SCHEMA=xo-mail-check-v1
MAX_LINE=240
...
usage() {
  cat <<'EOF'
Usage:
  xo-mail-check.sh [check]   run the received-mail poll; wake line unless the poll is a proven no-op
  xo-mail-check.sh arm       write and register state/mail.check.sh
  xo-mail-check.sh disarm    remove the check shim, its trust binding, and the record
  xo-mail-check.sh --help    print this help

Mail configuration (XO_MAIL_USER, XO_MAIL_PASS, XO_IMAP_HOST, XO_SMTP_HOST,
XO_IMAP_PORT, XO_SMTP_PORT) is read from <XO_HOME>/.env by xo-mail.sh.
See docs/configuration.md "Mail plane" for the schema.
EOF
}

die_usage() {
  printf 'xo-mail-check: %s\n' "$1" >&2
  usage >&2
  exit 2
}

record_epoch_now() {
  case "${XO_MAIL_CHECK_NOW:-}" in
    ''|*[!0-9]*) date +%s ;;
    *) printf '%s\n' "$XO_MAIL_CHECK_NOW" ;;
  esac
}
...
BUDGET_SECS=${XO_MAIL_CHECK_BUDGET:-15}
case "$BUDGET_SECS" in
  ''|*[!0-9]*|0)
    printf 'xo-mail-check: XO_MAIL_CHECK_BUDGET must be a whole number from 5 to 25\n' >&2
    exit 2
    ;;
esac
...
# poll's own "xo-mail: ..." diagnostics name the missing setup value, the
...
poll_summary() {
  ...
  line=$(printf '%s\n' "$out" | sed -n '/^xo-mail: woke for /d; s/^xo-mail: //p' | head -n 1)
  if [ -z "$line" ]; then
    line=$(printf '%s\n' "$out" | sed -n '/^xo-mail: woke for /d; /^$/d; p' | head -n 1)
  fi
  if [ -z "$line" ]; then
...
poll_has_publication_evidence() {
  ...
  [ "$rc" -eq 124 ] && return 0
  if [ -n "$out" ] && printf '%s\n' "$out" | grep -qE \
    '^xo-mail: woke for |the wake stays queued|could not clear retry for recovered'
  then
    return 0
  fi
...
action_check() {
  ...
  if [ ! -x "$MAIL_BIN" ]; then
    line="xo-mail.sh is missing next to this check ($MAIL_BIN)"
  else
    out=$(xo_run_timed "$BUDGET_SECS" "$MAIL_BIN" poll 2>&1) || rc=$?
    if [ "${rc:-0}" -eq 124 ]; then
      line="poll did not finish within the ${BUDGET_SECS}s budget"
    elif [ "${rc:-0}" -ne 0 ]; then
      line=$(poll_summary "$rc" "$out")
    elif printf '%s\n' "$out" | grep -q '^xo-mail: woke for '; then
      ...
      line=$(printf '%s\n' "$out" | grep '^xo-mail: woke for ' | tail -n 1 | sed 's/^xo-mail: /new mail: /')
    else
      line=
    fi
  fi
  ...
  if [ -n "$line" ] && { [ "$line" != "$RECORD_REPORTED" ] || [ "$queued" -eq 1 ]; }; then
    xo_cap_line_var "mail: $line" "$MAX_LINE"
    printf '%s\n' "$XO_LINE_CAP_LINE"
  fi
...
shim_content() {
  ...
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    '# Auto-generated by xo-mail-check.sh - received-mail poll shim.' \
    '# The watcher validates these bytes, then dispatches the trusted check script.' \
    "export XO_HOME=$(printf '%q' "$home")" \
    "exec $(printf '%q' "$SCRIPT_DIR/xo-mail-check.sh") check"
}
...
shim_write() {
  ...
  tmp=$(umask 077; mktemp "$STATE/.xo-mail-check.XXXXXX" 2>/dev/null) || return 1
  SHIM_WRITE_TMP=$tmp
  if ! printf '%s\n' "$want" > "$tmp" \
    || ! chmod 0700 "$tmp" \
...
arm_interrupted() {
  arm_rollback
  printf 'xo-mail-check: arming was interrupted, so state/%s.check.sh is not armed\n' "$CHECK_ID" >&2
  exit 1
}
...
action_arm() {
  local want home
  if [ ! -x "$MAIL_BIN" ]; then
    printf 'xo-mail-check: the mail plane is missing at %s; cannot arm\n' "$MAIL_BIN" >&2
    return 1
  fi
  mkdir -p "$STATE" || return 1
  case "$XO_HOME" in
    ...
    *)
      home=$(CDPATH='' cd -- "$XO_HOME" 2>/dev/null && pwd -P) || {
        printf 'xo-mail-check: cannot resolve XO_HOME %s\n' "$XO_HOME" >&2
        return 1
      }
      ;;
  esac
  ...
  if [ -f "$CHECK_SHIM" ] && [ ! -L "$CHECK_SHIM" ]; then
    ARM_BACKUP=$(shim_backup) || {
      printf 'xo-mail-check: could not save the existing %s\n' "$CHECK_SHIM" >&2
      return 1
    }
  fi
  ...
  if ! shim_write "$want"; then
    trap - HUP INT TERM
    arm_rollback
    printf 'xo-mail-check: could not write %s\n' "$CHECK_SHIM" >&2
    return 1
  fi
  if ! XO_HOME="$home" "$REGISTER_BIN" "$CHECK_ID" >/dev/null; then
    trap - HUP INT TERM
    arm_rollback
    printf 'xo-mail-check: could not register %s\n' "$CHECK_SHIM" >&2
    return 1
  fi
  trap - HUP INT TERM
