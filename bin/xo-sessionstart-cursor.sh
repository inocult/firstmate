...
# It is a thin transport around bin/xo-sessionstart-run.sh, which remains the
...
#
# Usage: xo-sessionstart-cursor.sh --source <source>
...
DIGEST=$("$SCRIPT_DIR/xo-sessionstart-run.sh" --source "$SOURCE" </dev/null 2>/dev/null || true)
[ -n "$DIGEST" ] || exit 0
