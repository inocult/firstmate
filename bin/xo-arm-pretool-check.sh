...
#
# An xo primary must arm the watcher or run a Codex checkpoint as a
# standalone verified harness call.
# bin/xo-arm-command-policy.mjs is the sole owner of shell classification,
...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/xo-arm-pretool-check.sh
#   bin/xo-arm-pretool-check.sh --command '<cmd>' [--background true|false]
#
...
usage() {
  cat <<'EOF'
Usage: xo-arm-pretool-check.sh [--command <cmd>] [--background true|false] [--claude|--cursor]
...
# xo-watch byte sequence AFTER the classifier's byte normalization, so a command
# that cannot contain xo-watch even after that normalization can never be a
...
# protected paths such as xo-watc\<newline>h-arm.sh or xo-"watch"-arm.sh still
# delegate. Stripping only these non-alphanumeric bytes can never destroy an
# existing xo-watch run.
#
# The fast path may allow ONLY when BOTH hold: (a) the stripped/normalized text
# lacks the xo-watch watcher substring, AND (b) the raw command carries no
...
# decodes and can therefore reconstruct xo-watch from bytes this cheap byte
# strip cannot. This marker set is COUPLED to the classifier's decoder set in
# bin/xo-arm-command-policy.mjs: adding any new quote/expansion form the
