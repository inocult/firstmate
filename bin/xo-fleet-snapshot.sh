#!/usr/bin/env bash
# xo-fleet-snapshot.sh - structured fleet snapshot with observational caching.
#
# Output contract: `--json` prints one object with schema
# `xo-fleet-snapshot.v1`.
...
#   xo_home: resolved operational home.
...
#     is at least XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS old (default 14; legacy
...
#     Local current_state is parsed from bin/xo-crew-state.sh <id> and preserves
...
#     xo-classify-lib.sh's authoritative status_open_decisions fold and reconciled
...
#     Which closed rows a home contributes is bin/xo-landed-lib.sh's rule, shared
...
SNAPSHOT_NOW=${XO_SNAPSHOT_NOW:-$(date -u +%Y-%m-%dT%H:%M:%SZ)}
if [ -n "${XO_SNAPSHOT_NOW_EPOCH:-}" ]; then
  SNAPSHOT_EPOCH=$XO_SNAPSHOT_NOW_EPOCH
else
...
XO_SNAPSHOT_SECONDMATES=${XO_SNAPSHOT_SECONDMATES:-20}
XO_SNAPSHOT_CREW_STATE_TIMEOUT=${XO_SNAPSHOT_CREW_STATE_TIMEOUT:-10}
XO_SNAPSHOT_LOCAL_READ_CONCURRENCY=${XO_SNAPSHOT_LOCAL_READ_CONCURRENCY:-8}
XO_SNAPSHOT_BUDGET=${XO_SNAPSHOT_BUDGET:-5}
XO_SNAPSHOT_CACHE_DIR=${XO_SNAPSHOT_CACHE_DIR:-$STATE/secondmate-summary-cache}
XO_SNAPSHOT_SECONDMATE_MAX_BYTES=${XO_SNAPSHOT_SECONDMATE_MAX_BYTES:-262144}
XO_SNAPSHOT_SECONDMATE_CHILDREN=${XO_SNAPSHOT_SECONDMATE_CHILDREN:-20}
XO_SNAPSHOT_SECONDMATE_QUEUED=${XO_SNAPSHOT_SECONDMATE_QUEUED:-20}
XO_SNAPSHOT_SECONDMATE_DECISIONS=${XO_SNAPSHOT_SECONDMATE_DECISIONS:-20}
XO_SNAPSHOT_TERMINAL_LINES=${XO_SNAPSHOT_TERMINAL_LINES:-8}
XO_SNAPSHOT_TERMINAL_BYTES=${XO_SNAPSHOT_TERMINAL_BYTES:-4096}
XO_SNAPSHOT_TERMINAL_TIMEOUT=${XO_SNAPSHOT_TERMINAL_TIMEOUT:-2}
XO_SNAPSHOT_PARENT_ACTIVITY_LINES=${XO_SNAPSHOT_PARENT_ACTIVITY_LINES:-256}
XO_SNAPSHOT_PARENT_ACTIVITY_BYTES=${XO_SNAPSHOT_PARENT_ACTIVITY_BYTES:-65536}
XO_SNAPSHOT_PARENT_ACTIVITIES=${XO_SNAPSHOT_PARENT_ACTIVITIES:-20}
XO_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT=${XO_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT:-2}
XO_SNAPSHOT_REGISTRY_LINES=${XO_SNAPSHOT_REGISTRY_LINES:-256}
XO_SNAPSHOT_REGISTRY_BYTES=${XO_SNAPSHOT_REGISTRY_BYTES:-65536}
XO_SNAPSHOT_REGISTRY_RECORDS=${XO_SNAPSHOT_REGISTRY_RECORDS:-40}
XO_SNAPSHOT_REGISTRY_TIMEOUT=${XO_SNAPSHOT_REGISTRY_TIMEOUT:-2}
validate_positive_bound() {  # <name> <value>
  case "$2" in
    ''|*[!0-9]*|0)
      printf 'xo-fleet-snapshot: %s must be a positive integer\n' "$1" >&2
      exit 2
      ;;
  esac
}
case "$XO_SNAPSHOT_SECONDMATES" in
  ''|*[!0-9]*)
    echo "xo-fleet-snapshot: XO_SNAPSHOT_SECONDMATES must be a non-negative integer" >&2
    exit 2
    ;;
esac
validate_positive_bound XO_SNAPSHOT_CREW_STATE_TIMEOUT "$XO_SNAPSHOT_CREW_STATE_TIMEOUT"
validate_positive_bound XO_SNAPSHOT_LOCAL_READ_CONCURRENCY "$XO_SNAPSHOT_LOCAL_READ_CONCURRENCY"
validate_positive_bound XO_SNAPSHOT_BUDGET "$XO_SNAPSHOT_BUDGET"
validate_positive_bound XO_SNAPSHOT_SECONDMATE_MAX_BYTES "$XO_SNAPSHOT_SECONDMATE_MAX_BYTES"
validate_positive_bound XO_SNAPSHOT_SECONDMATE_CHILDREN "$XO_SNAPSHOT_SECONDMATE_CHILDREN"
validate_positive_bound XO_SNAPSHOT_SECONDMATE_QUEUED "$XO_SNAPSHOT_SECONDMATE_QUEUED"
validate_positive_bound XO_SNAPSHOT_SECONDMATE_DECISIONS "$XO_SNAPSHOT_SECONDMATE_DECISIONS"
validate_positive_bound XO_SNAPSHOT_TERMINAL_LINES "$XO_SNAPSHOT_TERMINAL_LINES"
validate_positive_bound XO_SNAPSHOT_TERMINAL_BYTES "$XO_SNAPSHOT_TERMINAL_BYTES"
validate_positive_bound XO_SNAPSHOT_TERMINAL_TIMEOUT "$XO_SNAPSHOT_TERMINAL_TIMEOUT"
validate_positive_bound XO_SNAPSHOT_PARENT_ACTIVITY_LINES "$XO_SNAPSHOT_PARENT_ACTIVITY_LINES"
validate_positive_bound XO_SNAPSHOT_PARENT_ACTIVITY_BYTES "$XO_SNAPSHOT_PARENT_ACTIVITY_BYTES"
validate_positive_bound XO_SNAPSHOT_PARENT_ACTIVITIES "$XO_SNAPSHOT_PARENT_ACTIVITIES"
validate_positive_bound XO_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT "$XO_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT"
validate_positive_bound XO_SNAPSHOT_REGISTRY_LINES "$XO_SNAPSHOT_REGISTRY_LINES"
validate_positive_bound XO_SNAPSHOT_REGISTRY_BYTES "$XO_SNAPSHOT_REGISTRY_BYTES"
validate_positive_bound XO_SNAPSHOT_REGISTRY_RECORDS "$XO_SNAPSHOT_REGISTRY_RECORDS"
validate_positive_bound XO_SNAPSHOT_REGISTRY_TIMEOUT "$XO_SNAPSHOT_REGISTRY_TIMEOUT"
XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS=${XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS:-14}
case "$XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS" in
  ''|*[!0-9]*)
    echo "xo-fleet-snapshot: XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS must be a non-negative integer" >&2
    exit 2
    ;;
esac
...
# shellcheck disable=SC1091
. "$SCRIPT_DIR/xo-ff-lib.sh"  # validate_secondmate_home: shared seeded-home boundary checks
# shellcheck source=bin/xo-timeout-lib.sh
# shellcheck disable=SC1091
. "$SCRIPT_DIR/xo-timeout-lib.sh"  # xo_run_timed: the shared hard bound
# shellcheck source=bin/xo-landed-lib.sh
# shellcheck disable=SC1091
. "$SCRIPT_DIR/xo-landed-lib.sh"  # XO_LANDED_JQ_DEFS: the shared landed selector

usage() {
  cat <<'EOF'
usage: xo-fleet-snapshot.sh --json
       xo-fleet-snapshot.sh --secondmate-home-summary

Print a structured snapshot of the xo fleet.
...
Cross-home collection uses XO_SNAPSHOT_SECONDMATES (default 20, 0 lifts the
count bound) and XO_SNAPSHOT_SECONDMATE_MAX_BYTES.
Every sampled remote home's state/home-summary.json is fetched concurrently
under one XO_SNAPSHOT_BUDGET (default 5 seconds), with a valid prior copy under
XO_SNAPSHOT_CACHE_DIR used when the live read fails, is invalid, or consumes the
...
that home.
Each local per-task current-state read is bounded by XO_SNAPSHOT_CREW_STATE_TIMEOUT
(default 10 seconds); a read that hits the bound reports state unknown. Local task
observations run concurrently, up to XO_SNAPSHOT_LOCAL_READ_CONCURRENCY (default 8).
Remote secondmate endpoint liveness is not probed by this command.
Terminal contradiction evidence uses
XO_SNAPSHOT_TERMINAL_LINES, XO_SNAPSHOT_TERMINAL_BYTES, and
XO_SNAPSHOT_TERMINAL_TIMEOUT and never becomes canonical current state.
Parent activity evidence uses XO_SNAPSHOT_PARENT_ACTIVITY_LINES,
XO_SNAPSHOT_PARENT_ACTIVITY_BYTES, XO_SNAPSHOT_PARENT_ACTIVITIES, and
XO_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT, with truncation disclosed in the result.
The registered secondmate table uses XO_SNAPSHOT_REGISTRY_LINES,
XO_SNAPSHOT_REGISTRY_BYTES, XO_SNAPSHOT_REGISTRY_RECORDS, and
XO_SNAPSHOT_REGISTRY_TIMEOUT, with unavailability and truncation disclosed.
...
XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS old (default 14; 0 ages every hold with a
...
command -v jq >/dev/null 2>&1 || { echo "xo-fleet-snapshot: jq not found" >&2; exit 1; }
...
crew_state_json() {  # <id> [<captured-meta>] [<captured-status>]
  ...
  raw=$(
    xo_run_timed "$XO_SNAPSHOT_CREW_STATE_TIMEOUT" \
      env XO_ROOT_OVERRIDE="$XO_ROOT" \
      XO_HOME="$XO_HOME" \
      XO_STATE_OVERRIDE="$STATE" \
      XO_CREW_STATE_META_OVERRIDE="$captured_meta" \
      XO_CREW_STATE_STATUS_OVERRIDE="$captured_status" \
      XO_DATA_OVERRIDE="$DATA" \
      XO_PROJECTS_OVERRIDE="$PROJECTS" \
      XO_CONFIG_OVERRIDE="$CONFIG" \
      "$SCRIPT_DIR/xo-crew-state.sh" "$id" 2>/dev/null || true
  )
  raw=$(printf '%s\n' "$raw" | head -1)
  sep=' · '
  state=unknown
...
backlog_json() {  # [<backlog-path>] - defaults to this home's $BACKLOG
  ...
  jq -Rn --arg path "$backlog" --arg today "$SNAPSHOT_TODAY" --arg now "$SNAPSHOT_NOW" \
    --argjson age_days "$XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS" '
    ...
    # the producer word-boundary regression in tests/xo-bearings-snapshot.test.sh.
    def kind_of($rest):
    ...
    | .records |= map(
        if (.body_lines | length) > 0 then
          ...
          | .local_note = (.local_note
              // (if any(.body_lines[];
                    test("^Resolution recorded by xo-(captain|decision)-hold\\.$"))
                  then null
...
prefetch_task_observations() {  # <meta> <id>
  ...
  if [ -n "$remote_host" ]; then
    ...
    if [ -n "$target" ]; then
      if xo_backend_target_exists "$backend" "$target" "xo-$id" >/dev/null 2>&1; then
        endpoint_exists=true
      else
        endpoint_exists=false
      fi
      if [ "$kind" = secondmate ]; then
        agent_alive=$(xo_backend_agent_alive "$backend" "$target" 2>/dev/null || printf unknown)
      fi
    fi
  else
...
prefetch_task_current_states() {
  ...
  local -a pids=()
  snapshot_task_cleanup
  SNAPSHOT_TASK_DIR=$(umask 077; mktemp -d "${TMPDIR:-/tmp}/xo-fleet-tasks.XXXXXX") || return 1
  ...
  while [ "$index" -lt "$SNAPSHOT_TASK_META_COUNT" ]; do
    ...
    active=$((active + 1))
    index=$((index + 1))
    if [ "$active" -ge "$XO_SNAPSHOT_LOCAL_READ_CONCURRENCY" ]; then
...
task_json_lines() {
  ...
  while [ "$index" -lt "$SNAPSHOT_TASK_META_COUNT" ]; do
    ...
    # (xo-classify-lib.sh's status_open_decisions) so a later unrelated event can
    ...
    jq -n \
      ...
      '{
        ...
        actions:(
          if $kind == "secondmate" then
            {send:"bin/xo-send.sh xo-\($id) \u0027<request>\u0027",
             watch:"read status/doc return channel; do not routinely xo-peek a secondmate for answers",
             return_channel_note:"Secondmate answers come back through status/doc paths after a marked xo-send request."}
          else
            {watch:"bin/xo-peek.sh xo-\($id)",
             steer:"bin/xo-send.sh xo-\($id) \u0027<instruction>\u0027",
...
secondmate_home_summary_json() {  # <backlog-json-file> <tasks-json-file>
  jq -n \
    ...
    --arg home "$XO_HOME" \
    --argjson child_n "$XO_SNAPSHOT_SECONDMATE_CHILDREN" \
    --argjson queued_n "$XO_SNAPSHOT_SECONDMATE_QUEUED" \
    --argjson decisions_n "$XO_SNAPSHOT_SECONDMATE_DECISIONS" \
    --argjson landed_n "$XO_SNAPSHOT_SECONDMATE_LANDED_PER_HOME" \
    --slurpfile backlog "$1" \
    --slurpfile tasks "$2" "$XO_LANDED_JQ_DEFS"'
    ($backlog[0]) as $backlog
    | ($tasks[0]) as $tasks
    | def trunc($n):
      tostring | gsub("\\s+"; " ")
...
XO_SNAPSHOT_SECONDMATE_LANDED_PER_HOME=${XO_SNAPSHOT_SECONDMATE_LANDED_PER_HOME:-10}
case "$XO_SNAPSHOT_SECONDMATE_LANDED_PER_HOME" in ''|*[!0-9]*) XO_SNAPSHOT_SECONDMATE_LANDED_PER_HOME=10 ;; esac
...
JQ
  ...
  out=$(xo_run_timed "$XO_SNAPSHOT_REGISTRY_TIMEOUT" bash -c "$script" \
    xo-secondmate-registry "$reg" "$XO_SNAPSHOT_REGISTRY_LINES" \
    "$XO_SNAPSHOT_REGISTRY_BYTES" "$XO_SNAPSHOT_REGISTRY_RECORDS" "$reg" "$SNAPSHOT_NOW" \
...
# xo-timeout-lib's single fleet-wide deadline. A timed-out child therefore cannot
...
summary_file_read() {  # <file> <expected-home> <output-file>
  ...
  if ! LC_ALL=C head -c "$((XO_SNAPSHOT_SECONDMATE_MAX_BYTES + 1))" "$file" > "$captured"; then
    rm -f -- "$captured"
    return 1
  fi
  ...
  if [ "$bytes" -gt "$XO_SNAPSHOT_SECONDMATE_MAX_BYTES" ] \
...
summary_file_oversized() {  # <file>
  ...
  [ "$bytes" -gt "$XO_SNAPSHOT_SECONDMATE_MAX_BYTES" ]
}

snapshot_cache_prepare() {
  local mode
  SNAPSHOT_CACHE_AVAILABLE=0
  if [ -e "$XO_SNAPSHOT_CACHE_DIR" ] || [ -L "$XO_SNAPSHOT_CACHE_DIR" ]; then
    [ -d "$XO_SNAPSHOT_CACHE_DIR" ] && [ ! -L "$XO_SNAPSHOT_CACHE_DIR" ] || return 1
    mode=$(file_mode_octal "$XO_SNAPSHOT_CACHE_DIR")
    case "$mode" in ''|*[!0-7]*) return 1 ;; esac
    [ $((8#$mode & 077)) -eq 0 ] || return 1
  else
    [ -d "$(dirname "$XO_SNAPSHOT_CACHE_DIR")" ] || return 1
    (umask 077; mkdir "$XO_SNAPSHOT_CACHE_DIR") 2>/dev/null || return 1
  fi
...
snapshot_route_cache_path() {  # <id> <host> <home>
  ...
  [ "${#key}" -eq 64 ] || return 1
  printf '%s/%s.json\n' "$XO_SNAPSHOT_CACHE_DIR" "$key"
}

snapshot_cache_store() {  # <summary-json-file> <destination>
  local summary_file=$1 destination=$2 tmp
  [ "$SNAPSHOT_CACHE_AVAILABLE" -eq 1 ] || return 1
  case "$destination" in "$XO_SNAPSHOT_CACHE_DIR"/*) ;; *) return 1 ;; esac
  [ ! -L "$destination" ] || return 1
  tmp=$(umask 077; mktemp "$XO_SNAPSHOT_CACHE_DIR/.summary.XXXXXX") || return 1
...
prepare_remote_summary_collection() {  # <sampled-row-json-lines>
  ...
  SNAPSHOT_COLLECT_DIR=$(umask 077; mktemp -d "${TMPDIR:-/tmp}/xo-fleet-ledgers.XXXXXX") || return 1
  SNAPSHOT_SUMMARY_FILTER="$SNAPSHOT_COLLECT_DIR/summary-filter.jq"
  cat > "$SNAPSHOT_SUMMARY_FILTER" <<'JQ'
length == 1 and (.[0] |
  .schema == "xo-secondmate-home-summary.v1"
  and .hold_classifier_schema == "xo-captain-hold-buckets.v1"
  and .home == $home
...
collect_one() {  # <manifest-row>
  ...
  if bounded_collect "$fetch" "$out_dir/$slot.fetch.err" \
      "$script_dir/xo-on.sh" "$id" xo-remote-file.sh get state/home-summary.json "$max_bytes" \
...
BASH
  chmod 700 "$collector"
  SNAPSHOT_COLLECTION_TIMED_OUT=0
  if xo_run_timed "$XO_SNAPSHOT_BUDGET" bash "$collector" \
      "$SCRIPT_DIR" "$manifest" "$SNAPSHOT_COLLECT_DIR" "$SNAPSHOT_SUMMARY_FILTER" \
      "$XO_SNAPSHOT_SECONDMATE_MAX_BYTES"; then
    :
  else
    rc=$?
...
BASH
  ...
  out=$(xo_run_timed "$XO_SNAPSHOT_PARENT_ACTIVITY_TIMEOUT" bash -c "$script" \
    xo-parent-activities "$SCRIPT_DIR/xo-classify-lib.sh" "$f" \
    "$XO_SNAPSHOT_PARENT_ACTIVITY_LINES" "$XO_SNAPSHOT_PARENT_ACTIVITY_BYTES" \
    "$XO_SNAPSHOT_PARENT_ACTIVITIES" "$SNAPSHOT_STAT_STYLE" 2>/dev/null)
  rc=$?
  if [ "$rc" -eq 0 ] && printf '%s' "$out" | jq -e '
...
terminal_evidence_json() {  # <parent-task-json> <event-note> <evidence-contradicts>
  ...
  out=$(xo_run_timed "$XO_SNAPSHOT_TERMINAL_TIMEOUT" bash -c \
    '. "$1"; xo_backend_capture "$2" "$3" "$4" "$5" | LC_ALL=C head -c "$6"; rc=${PIPESTATUS[0]}; [ "$rc" -eq 141 ] && rc=0; exit "$rc"' \
    xo-terminal-capture "$SCRIPT_DIR/xo-backend.sh" "$backend" "$target" "$XO_SNAPSHOT_TERMINAL_LINES" "$expected" "$XO_SNAPSHOT_TERMINAL_BYTES" 2>/dev/null)
  rc=$?
  if [ "$rc" -ne 0 ]; then
  ...
  clean=$(printf '%s' "$out" | tail -n "$XO_SNAPSHOT_TERMINAL_LINES" | LC_ALL=C head -c "$XO_SNAPSHOT_TERMINAL_BYTES")
...
secondmate_current_json() {  # <parent-tasks-json-file> <output-file>
  ...
  rows=$(jq -c --argjson cap "$XO_SNAPSHOT_SECONDMATES" '(if $cap == 0 then .records else .records[:$cap] end)[]' "$union_file")
  shown=$(printf '%s\n' "$rows" | grep -c . || true)
...
BACKLOG_JSON=$(backlog_json) || { echo "xo-fleet-snapshot: backlog read failed" >&2; exit 1; }
prefetch_task_current_states || { echo "xo-fleet-snapshot: task observation failed" >&2; exit 1; }
TASKS_JSON=$(task_json_lines) || { echo "xo-fleet-snapshot: task snapshot failed" >&2; exit 1; }

JSON_TRANSPORT_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xo-fleet-snapshot.XXXXXX") \
  || { echo "xo-fleet-snapshot: temporary transport directory creation failed" >&2; exit 1; }
...
printf '%s\n' "$BACKLOG_JSON" > "$BACKLOG_JSON_FILE" \
  || { echo "xo-fleet-snapshot: temporary backlog file write failed" >&2; exit 1; }
printf '%s\n' "$TASKS_JSON" > "$TASKS_JSON_FILE" \
  || { echo "xo-fleet-snapshot: temporary task file write failed" >&2; exit 1; }

if [ "$OUTPUT_MODE" = secondmate-home-summary ]; then
  secondmate_home_summary_json "$BACKLOG_JSON_FILE" "$TASKS_JSON_FILE" \
    || { echo "xo-fleet-snapshot: secondmate home summary failed" >&2; exit 1; }
  exit 0
fi

scout_report_lines > "$SCOUT_REPORTS_JSON_FILE" \
  || { echo "xo-fleet-snapshot: scout report snapshot failed" >&2; exit 1; }
main_inventory_json "$BACKLOG_JSON_FILE" "$TASKS_JSON_FILE" > "$MAIN_INVENTORY_JSON_FILE" \
  || { echo "xo-fleet-snapshot: main inventory summary failed" >&2; exit 1; }
secondmate_current_json "$TASKS_JSON_FILE" "$SECONDMATE_CURRENT_JSON_FILE" \
  || { echo "xo-fleet-snapshot: registered secondmate aggregation failed" >&2; exit 1; }
secondmate_landed_from_current_json "$SECONDMATE_CURRENT_JSON_FILE" "$SECONDMATE_LANDED_JSON_FILE" \
  || { echo "xo-fleet-snapshot: secondmate landed projection failed" >&2; exit 1; }
...
jq -n \
  ...
  '($backlog[0]) as $backlog
   ...
   {
     schema:"xo-fleet-snapshot.v1",
     generated:$generated,
     xo_home:$xo_home,
     roots:{xo_root:$xo_root,state:$state,data:$data,config:$config,projects:$projects},
     backlog:$backlog,
