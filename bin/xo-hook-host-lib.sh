...
# 2026.08.11-e8db854). A Cursor primary running in an XO checkout therefore
...
# seatbelt twice. XO's Cursor registration owns those events, so the
...
# XO registration already covers this event.
xo_hook_payload_is_foreign_host() {  # <payload>
  local payload=${1-}
  [ -n "$payload" ] || return 1
