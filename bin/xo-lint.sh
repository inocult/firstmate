#!/usr/bin/env bash
# xo-lint.sh - the single owner of xo's lint definition.
#
...
# The default (no explicit-path) path also runs bin/xo-lint-workflows.sh so a
...
# deterministic shard and root order after every worker finishes. XO_LINT_JOBS=1
...
#
# Usage:
#   xo-lint.sh                         lint the context-selected file set (see above)
#   xo-lint.sh --fast [path]...       local lint with extended analysis disabled
#   xo-lint.sh <path>...               lint explicit roots with the same config
#   xo-lint.sh --jobs <1|2> [path]...  override bounded worker count
#   xo-lint.sh --telemetry <path> ...  write a quiet metrics snapshot
#   xo-lint.sh --required-version      print the ShellCheck pin
#   xo-lint.sh --list-files            print the file set that would be linted
#   xo-lint.sh --help                  print this usage
set -u
...
SELF="$SELF_DIR/xo-lint.sh"
ROOT="$(cd "$SELF_DIR/.." && pwd -P)"
cd "$ROOT" || exit 1

XO_LINT_WORKER_SHELLCHECK_PID=
# shellcheck disable=SC2329 # Registered by the private worker's signal traps.
xo_lint_worker_stop() {
  [ -n "$XO_LINT_WORKER_SHELLCHECK_PID" ] || return 0
  kill "$XO_LINT_WORKER_SHELLCHECK_PID" 2>/dev/null || true
  wait "$XO_LINT_WORKER_SHELLCHECK_PID" 2>/dev/null || true
  XO_LINT_WORKER_SHELLCHECK_PID=
}

xo_lint_worker() {  # <manifest> <output-dir> <shard-index>
  ...
  if [ "${#roots[@]}" -gt 0 ]; then
    trap 'xo_lint_worker_stop; exit 129' HUP
    trap 'xo_lint_worker_stop; exit 130' INT
    trap 'xo_lint_worker_stop; exit 143' TERM
    shellcheck_args=(--norc)
    if [ "${XO_LINT_INTERNAL_FOLLOW_SOURCES:-1}" -eq 1 ]; then
      shellcheck_args+=(--external-sources)
    fi
    if [ -n "${XO_LINT_INTERNAL_EXCLUDE:-}" ]; then
      shellcheck_args+=(--exclude="$XO_LINT_INTERNAL_EXCLUDE")
    fi
    if [ "${XO_LINT_INTERNAL_FAST:-0}" -eq 1 ]; then
      shellcheck_args+=(--extended-analysis=false)
    fi
    : > "$output.out"
    if [ "${XO_LINT_INTERNAL_FOLLOW_SOURCES:-1}" -eq 1 ]; then
      "$XO_LINT_SHELLCHECK" "${shellcheck_args[@]}" -- "${roots[@]}" >> "$output.out" 2>&1 &
      XO_LINT_WORKER_SHELLCHECK_PID=$!
      wait "$XO_LINT_WORKER_SHELLCHECK_PID" || rc=$?
      XO_LINT_WORKER_SHELLCHECK_PID=
    else
      for path in "${roots[@]}"; do
        invocation_rc=0
        "$XO_LINT_SHELLCHECK" "${shellcheck_args[@]}" -- "$path" >> "$output.out" 2>&1 &
        XO_LINT_WORKER_SHELLCHECK_PID=$!
        wait "$XO_LINT_WORKER_SHELLCHECK_PID" || invocation_rc=$?
        XO_LINT_WORKER_SHELLCHECK_PID=
...
if [ "${1:-}" = "--internal-worker" ]; then
  [ "${XO_LINT_INTERNAL:-}" = 1 ] || {
    printf 'xo-lint.sh: --internal-worker is private to the lint owner.\n' >&2
    exit 2
  }
  [ "$#" -eq 4 ] && [ -n "${XO_LINT_SHELLCHECK:-}" ] || exit 2
  xo_lint_worker "$2" "$3" "$4"
  exit $?
fi
...
xo_lint_run_workflows() {
  [ "$EXPLICIT_PATHS" -eq 0 ] || return 0
  "$SELF_DIR/xo-lint-workflows.sh"
}

# Backend adapters belong behind tasks-axi. Keep direct Beads CLI invocations
# out of xo's core scripts so every configured backend follows the same
# lifecycle path.
xo_lint_run_backend_purity() {
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --jobs)
      [ "$#" -ge 2 ] || { printf 'xo-lint.sh: --jobs requires 1 or 2.\n' >&2; exit 2; }
      JOBS=$2
      shift 2
      ;;
    --jobs=*)
      ...
      ;;
    --telemetry)
      [ "$#" -ge 2 ] || { printf 'xo-lint.sh: --telemetry requires a path.\n' >&2; exit 2; }
      TELEMETRY=$2
      shift 2
      ;;
    --telemetry=*)
...
case "$JOBS" in
  1|2) ;;
  *) printf 'xo-lint.sh: jobs must be 1 or 2, got %s.\n' "$JOBS" >&2; exit 2 ;;
esac

if [ "$FAST" -eq 1 ] && { [ "${GITHUB_ACTIONS:-}" = true ] || [ "${CI:-}" = true ]; }; then
  printf 'xo-lint.sh: --fast is local-only; CI uses full ShellCheck analysis.\n' >&2
  exit 2
fi

# xo_lint_changed_base_ref prints the ref to diff the working branch against:
...
# xo_lint_is_canonical_root tests membership in the canonical set (a direct
...
if [ "$#" -gt 0 ]; then
  ...
  if [ "${GITHUB_ACTIONS:-}" != true ] && [ "${CI:-}" != true ] \
    ...
    base_ref=$(xo_lint_changed_base_ref) || base_ref=
    merge_base=
  ...
  if [ "$full_lint" -eq 1 ]; then
    ...
    while IFS= read -r -d '' changed_path; do
      xo_lint_is_canonical_root "$changed_path" || continue
...
if [ "$LIST_FILES" -eq 1 ]; then
  [ "$#" -eq 0 ] || {
    printf 'xo-lint.sh: --list-files does not accept explicit paths.\n' >&2
    exit 2
  }
...
if ! command -v shellcheck >/dev/null 2>&1; then
  printf 'xo-lint.sh: ShellCheck not found; install ShellCheck %s with bin/xo-install-shellcheck.sh <destination-directory> and put that directory on PATH.\n' \
...
if ! PERL_BIN=$(command -v perl); then
  printf 'xo-lint.sh: perl is required for bounded worker cleanup.\n' >&2
  exit 127
fi
resolved=$("$SHELLCHECK_BIN" --version | awk '/^version:/ {print $2; exit}')
printf 'xo-lint.sh: ShellCheck %s (pinned %s)\n' "$resolved" "$REQUIRED_SHELLCHECK" >&2
if [ "$resolved" != "$REQUIRED_SHELLCHECK" ]; then
  printf 'xo-lint.sh: ShellCheck %s required for CI parity, found %s. Install %s with bin/xo-install-shellcheck.sh <destination-directory>.\n' \
    "$REQUIRED_SHELLCHECK" "$resolved" "$REQUIRED_SHELLCHECK" >&2
  exit 1
fi
if [ "$FAST" -eq 1 ]; then
  printf 'xo-lint.sh: fast local mode; ShellCheck extended analysis disabled\n' >&2
elif [ "$FOLLOW_SOURCES" -eq 0 ]; then
  printf 'xo-lint.sh: local changed-file mode; ShellCheck source following disabled\n' >&2
else
  printf 'xo-lint.sh: full ShellCheck extended analysis enabled\n' >&2
fi

if [ "$CHANGED_MODE" -eq 1 ] && [ "$ROOT_COUNT" -eq 0 ]; then
  printf 'xo-lint.sh: no changed lint targets\n'
  overall_rc=0
  xo_lint_run_backend_purity || overall_rc=$?
  xo_lint_run_workflows || overall_rc=$?
  exit "$overall_rc"
fi
...
if [ -n "$TELEMETRY" ]; then
  ...
  [ -d "$telemetry_parent" ] || {
    printf 'xo-lint.sh: telemetry directory does not exist: %s\n' "$telemetry_parent" >&2
    exit 2
  }
fi

TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xo-lint.XXXXXX") || exit 1
ACTIVE_PIDS=()
...
for path in "${ROOTS[@]}"; do
  case "$path" in
    *"$TAB"*|*$'\n'*)
      printf 'xo-lint.sh: paths containing tabs or newlines are not supported: %s\n' "$path" >&2
      exit 2
      ;;
  esac
  if [ -f "$path" ]; then
...
if [ -n "$TELEMETRY" ]; then
  ...
  TELEMETRY_SHELLCHECK_START=$(xo_lint_shellcheck_count)
  TELEMETRY_LOAD_START=$(xo_lint_load_average)
  TELEMETRY_CPU_START=$(xo_lint_aggregate_cpu)
fi

xo_lint_run_worker() {  # <worker-index>
  ...
  if [ -n "$TELEMETRY" ] && [ -x /usr/bin/time ]; then
    if [ "$(uname)" = Darwin ]; then
      exec "$PERL_BIN" -e 'setpgrp(0, 0) or die "setpgrp: $!"; exec @ARGV or die "exec: $!"' \
        ...
        env XO_LINT_INTERNAL=1 XO_LINT_INTERNAL_FAST="$FAST" \
        XO_LINT_INTERNAL_FOLLOW_SOURCES="$FOLLOW_SOURCES" XO_LINT_INTERNAL_EXCLUDE="$EXCLUDE_CODES" \
        XO_LINT_SHELLCHECK="$SHELLCHECK_BIN" \
...
xo_lint_start_worker() {
  xo_lint_run_worker "$1" &
  ACTIVE_PIDS+=("$!")
}

xo_lint_wait_workers() {
  local pid
...
while [ "$worker" -lt "$SHARD_COUNT" ]; do
  ...
  if [ -f "$output.rc" ]; then
    ...
    printf 'xo-lint.sh: worker produced no result for shard %s.\n' "$worker" >&2
    rc=2
  fi
...
if [ -n "$TELEMETRY" ]; then
  ...
  TELEMETRY_SHELLCHECK_END=$(xo_lint_shellcheck_count)
  TELEMETRY_LOAD_END=$(xo_lint_load_average)
  TELEMETRY_CPU_END=$(xo_lint_aggregate_cpu)
...
EOF
  ...
  {
    printf 'format\txo-lint-telemetry-v1\n'
    printf 'git_head\t%s\n' "$git_head"
  ...
  if ! mv -f "$telemetry_tmp" "$TELEMETRY"; then
    printf 'xo-lint.sh: could not write telemetry to %s.\n' "$TELEMETRY" >&2
    [ "$overall_rc" -ne 0 ] || overall_rc=2
  fi
fi

purity_rc=0
xo_lint_run_backend_purity || purity_rc=$?
