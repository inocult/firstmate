#!/usr/bin/env bash
# xo-install-herdr.sh - install CI's pinned, verified Herdr build.
#
...
#
# Usage:
#   xo-install-herdr.sh <destination-directory>
#
...
XO_HERDR_CI_VERSION=0.7.4
XO_HERDR_CI_TAG="v${XO_HERDR_CI_VERSION}"
XO_HERDR_CI_MIN_PROTOCOL=16
# Bounded download ceiling (bytes). The largest official 0.7.4 asset is under 20 MiB.
XO_HERDR_CI_MAX_BYTES=25000000
XO_HERDR_CI_REPO=ogulcancelik/herdr

die() {
  printf 'xo-install-herdr.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: xo-install-herdr.sh <destination-directory>}
...
URL="https://github.com/${XO_HERDR_CI_REPO}/releases/download/${XO_HERDR_CI_TAG}/${ASSET}"
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/xo-herdr.XXXXXX")
trap 'rm -rf "$TMP"' EXIT

printf 'xo-install-herdr.sh: downloading %s from %s\n' "$ASSET" "$URL" >&2
# --fail: HTTP errors; --location: follow redirects; --max-filesize: bound.
curl -fsSL --max-filesize "$XO_HERDR_CI_MAX_BYTES" "$URL" -o "$TMP/$ASSET" \
  || die "download failed for $URL (bounded at $XO_HERDR_CI_MAX_BYTES bytes)"
...
[ "$installed_version" = "$XO_HERDR_CI_VERSION" ] \
  || die "installed herdr version is '${installed_version:-<empty>}', expected exact pin $XO_HERDR_CI_VERSION"
...
[ "$protocol" -ge "$XO_HERDR_CI_MIN_PROTOCOL" ] \
  || die "herdr protocol $protocol is below the required floor $XO_HERDR_CI_MIN_PROTOCOL"

printf 'xo-install-herdr.sh: installed herdr %s (protocol %s) to %s\n' \
