...
# xo-crew-state.sh (read-only current-state reporting) and xo-teardown.sh
...
# pipeline-owned continuation through xo_nm_runs_status_for_worktree below:
...
# stderr, while xo_nm_run keeps the fail-open query contract for read-only callers.
xo_nm_run_bounded() {  # <dir> <timeout_secs> <args...>
...
xo_nm_run_checked() {  # <dir> <timeout_secs> <args...>
  xo_nm_run_bounded "$@" 2>/dev/null
}

xo_nm_run() {  # <dir> <timeout_secs> <args...>
  xo_nm_run_checked "$@" || true
}
...
xo_nm_resolve_commit() {  # <worktree> <sha-ish>
...
# rejected; xo_nm_runs_status_for_worktree below owns the one ledger-anchored
# recognition for that case, and xo_nm_run_is_pipeline_owned_active below
...
# unchanged - for the runs ledger, xo_nm_runs_status_for_worktree's
# newest-row-decides rule below.
# xo_nm_run_status_class next classifies a recorded status word for that
...
xo_nm_head_matches_worktree() {  # <worktree> <run_head>
  local wt=$1 run_head=$2 local_full run_full
  [ -n "$run_head" ] || return 1
  local_full=$(git -C "$wt" rev-parse HEAD 2>/dev/null) || return 1
  run_full=$(xo_nm_resolve_commit "$wt" "$run_head")
  [ -n "$run_full" ] || return 1
...
# field as well, which xo_nm_run_is_active below checks directly.
xo_nm_run_status_class() {  # <status_word>
  case "${1:-}" in
...
xo_nm_branch_sync_state() {  # <toon-output>
  local s
  s=$(printf '%s\n' "$1" \
    | sed -n '/^[[:space:]]*branch_sync:[[:space:]]*$/,/^[^[:space:]][^:]*:/s/^[[:space:]]\{1,\}state:[[:space:]]*\(.*\)/\1/p' \
    | head -1)
  xo_nm_strip_quotes "$s"
}

# 0 if the run in captured `axi status` TOON $1 is still in flight: no
# terminal outcome and no terminal status.
xo_nm_run_is_active() {  # <toon-output>
  local status outcome
  status=$(xo_nm_strip_quotes "$(xo_nm_field "$1" status)")
  outcome=$(xo_nm_strip_quotes "$(xo_nm_field "$1" outcome)")
  [ -z "$outcome" ] || return 1
...
xo_nm_run_is_pipeline_owned_active() {  # <toon-output>
  [ "$(xo_nm_branch_sync_state "$1")" = pipeline_owned ] || return 1
  xo_nm_run_is_active "$1"
}
...
#   - newest row's head resolves and matches the worktree (xo_nm_head_matches_worktree):
#     its status word
...
# with xo_nm_head_matches_worktree above, and it only ever replaces a TERMINAL
...
# copy binds by xo_nm_head_matches_worktree. A live row whose head does NOT
...
xo_nm_runs_status_for_worktree() {  # <worktree> <branch> <runs-list-output> [expected-head]
  ...
  while IFS= read -r row; do
    ...
    if [ -n "$decided" ]; then
      ...
      [ "$(xo_nm_run_status_class "$st")" = live ] || continue
      if [ -n "$(xo_nm_resolve_commit "$wt" "$sha")" ]; then
        xo_nm_head_matches_worktree "$wt" "$sha" || continue
      else
    ...
    if [ -n "$pending_st" ]; then
      ...
      if [ "$(xo_nm_resolve_commit "$wt" "$sha")" = "$local_full" ]; then
        decided=$pending_st
      fi
      break
    fi
    ...
    row_full=$(xo_nm_resolve_commit "$wt" "$sha")
    if [ -n "$row_full" ]; then
      if xo_nm_head_matches_worktree "$wt" "$sha"; then
        decided=$st
        ...
        if [ "$(xo_nm_run_status_class "$st")" = terminal ]; then
