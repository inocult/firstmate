#!/usr/bin/env bash
# xo-remote-readiness-lib.sh - the remote second-mate readiness gate sequence.
#
# Source this file and call:
#   xo_remote_readiness_ensure <bin-dir> <secondmate-id>
#
# It runs bin/xo-remote-doctor.sh on that route's configured host, and when the
...
# never trusted on its own word. bin/xo-remote-doctor.sh remains the single
...
# XO_REMOTE_READINESS_OUT always holds the output of the last run, which carries
...
xo_remote_readiness_ensure() { # <bin-dir> <secondmate-id>
  ...
  out=$("$bin_dir/xo-on.sh" "$id" xo-remote-doctor.sh < /dev/null 2>&1)
  rc=$?
  XO_REMOTE_READINESS_OUT=$out
  [ "$rc" -ne 0 ] || return 0
  [ "$rc" -ne 255 ] || return 255

  out=$("$bin_dir/xo-on.sh" "$id" xo-remote-doctor.sh --fix < /dev/null 2>&1)
  rc=$?
  XO_REMOTE_READINESS_OUT=$out
  [ "$rc" -ne 255 ] || return 255
