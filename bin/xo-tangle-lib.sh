...
# Shared worktree-tangle guard for the xo-on-itself case.
# Usage: . bin/xo-tangle-lib.sh
#
# XO is a treehouse-pooled git repo of itself: crewmate worktrees and
# secondmate homes are all linked `git worktree`s of the same repo, while the
# PRIMARY checkout (the repo root xo operates from) is a normal checkout
# on a real branch - normally the default branch, main. The "worktree tangle"
# failure mode is a crewmate spawned to work on xo ITSELF branching and
# committing in the primary checkout instead of its own disposable worktree,
# stranding the primary on a feature branch (e.g. xo/readme-restructure-d3).
#
# xo_primary_tangle_branch detects exactly that and nothing else: a NAMED,
