#!/usr/bin/env bash
# xo-mail.sh - general-purpose mail plane for reading and sending mail.
#
...
# message is a notification xo reads before deciding, and xo still
...
#                        `check` wake so xo answers it concisely. IMAP
...
#                        bin/xo-mail-check.sh (docs/configuration.md
#                        "Mail plane").
#   status               Print configuration and the last poll cursor. No
#                        network, no wake.
#
# Volume: poll surfaces at most XO_MAIL_POLL_MAX_WAKES messages per run
...
#   XO_IMAP_HOST=<imap host>
#   XO_IMAP_PORT=<imap port>     (default 993, implicit TLS)
#   XO_SMTP_HOST=<smtp host>
#   XO_SMTP_PORT=<smtp port>     (default 465, implicit TLS)
#   XO_MAIL_TIMEOUT=<seconds>    (default 20; IMAP/SMTP socket timeout)
# XO_HOME falls back to the repo root when unset. This script carries no secret
# and no default endpoint that could resolve against a wrong home; XO_MAIL_USER,
# XO_MAIL_PASS, XO_IMAP_HOST, and XO_SMTP_HOST are always required, and
# XO_MAIL_PASS is never logged. The wake library is sourced from next to this
# script, not from $XO_HOME/bin; cursor, journal, retry set, and queue stay
# under $XO_HOME/state.
#
# IMAP/SMTP work is delegated to bin/xo-mail.py (imaplib/smtplib, implicit TLS
# on 993/465). STARTTLS and port 587 are not supported. BODY.PEEK is used on
# read/poll so mail is never marked seen before xo actually answers it.
...
for r in XO_MAIL_USER XO_MAIL_PASS XO_IMAP_HOST XO_SMTP_HOST; do
  if [ -z "${!r:-}" ]; then
    echo "xo-mail: missing required \$XO_HOME/.env value: $r" >&2
    echo "xo-mail: add $r (and the other three XO_MAIL_* values) to $ENV_FILE" >&2
    exit 1
  fi
done
IMAP_HOST="$XO_IMAP_HOST"
IMAP_PORT="${XO_IMAP_PORT:-993}"
SMTP_HOST="$XO_SMTP_HOST"
SMTP_PORT="${XO_SMTP_PORT:-465}"
case "$IMAP_PORT" in
  ''|*[!0-9]*|0)
    echo "xo-mail: XO_IMAP_PORT must be a positive integer, got: ${XO_IMAP_PORT:-}" >&2
    exit 1
    ;;
esac
case "$SMTP_PORT" in
  ''|*[!0-9]*|0)
    echo "xo-mail: XO_SMTP_PORT must be a positive integer, got: ${XO_SMTP_PORT:-}" >&2
    exit 1
    ;;
esac
MAIL_MAX_WAKES="${XO_MAIL_POLL_MAX_WAKES:-20}"
...
if [ -z "$PY" ]; then
  echo "xo-mail: python3 required" >&2
  exit 1
fi
PY_BIN="$SCRIPT_DIR/xo-mail.py"
if [ ! -f "$PY_BIN" ]; then
  echo "xo-mail: $PY_BIN missing" >&2
  exit 1
fi

STATE_DIR="$XO_HOME/state"
mkdir -p "$STATE_DIR"
...
# xo-mail's own record of emission and survives any drain ack, which makes
...
run_py() {
  XO_MAIL_USER="$XO_MAIL_USER" XO_MAIL_PASS="$XO_MAIL_PASS" \
  XO_IMAP_HOST="$IMAP_HOST" XO_IMAP_PORT="$IMAP_PORT" \
  XO_SMTP_HOST="$SMTP_HOST" XO_SMTP_PORT="$SMTP_PORT" \
  XO_MAIL_CURSOR="$CURSOR" XO_MAIL_RETRY="$RETRY" \
  XO_MAIL_RETRY_POS="$RETRY_POS" XO_MAIL_TURN="$TURN" \
  XO_MAIL_POLL_MAX_WAKES="$MAIL_MAX_WAKES" \
    "$PY" "$PY_BIN" "$@"
}
...
usage() {
  cat <<'EOF'
xo-mail.sh read
xo-mail.sh send <to> <subject> <body | ->
xo-mail.sh poll
xo-mail.sh status
EOF
}
...
mail_rollback_wake_locked() {
  ...
  # Runs under the held XO_WAKE_QUEUE_LOCK, so the rewrite cannot race an
  ...
  clean_key=$(printf '%s' "$wake_key" | xo_wake_clean_field)
  ...
  # durable record.
  tmp=$(mktemp "$XO_WAKE_QUEUE.rollback.XXXXXX") || return 1
  if ! awk -F '\t' -v key="$clean_key" '
    NF >= 5 && $3 == "check" && $4 == key { next }
    { print }
  ' "$XO_WAKE_QUEUE" > "$tmp"; then
    rm -f -- "$tmp"
    return 1
  fi
  chmod 0600 "$tmp" 2>/dev/null || true
  if ! mv -f -- "$tmp" "$XO_WAKE_QUEUE"; then
    rm -f -- "$tmp"
    return 1
  fi
...
wake_for() {
  ...
  # XO_WAKE_QUEUE_LOCK. The key is generation-aware when the mailbox reports a
  ...
  local generation=$1 id=$2 summary=$3 retry_id=${4:-} lib="$SCRIPT_DIR/xo-wake-lib.sh" status=0 tag=""
  ...
  if [ ! -f "$lib" ]; then
    echo "xo-mail: $lib missing; cannot wake" >&2
    return 1
  fi
  ...
  if xo_wake_append_locked check "$wake_key" "check: mail $id - $summary"; then
    if mail_record_evidence "$generation" "$id" "$tag"; then
      :
    elif mail_rollback_wake_locked "$wake_key" "$generation" "$id"; then
      echo "xo-mail: wake for $id rolled back (journal and cursor writes failed); retried on next poll" >&2
      status=1
    elif mail_record_evidence "$generation" "$id" "$tag"; then
      echo "xo-mail: wake for $id durably recorded after the queue rewrite failed" >&2
    else
      echo "xo-mail: wake for $id could not be rolled back or durably recorded; the wake stays queued and the next poll heals it - a possible duplicate, never a lost mail" >&2
      status=2
    fi
  else
    echo "xo-mail: wake append failed for $id; retried on next poll" >&2
    status=3
  fi
  ...
  if { [ "$status" -eq 0 ] || [ "$status" -eq 2 ]; } && [ -n "$retry_id" ]; then
    if ! mail_retry_remove "$retry_id"; then
      echo "xo-mail: could not clear retry for recovered $retry_id after publish; retried on next poll" >&2
      status=4
    fi
  fi
...
mail_heal() {
  ...
  done < <(xo_wake_queued_keys check 2>/dev/null || true)
  return "$heal_ok"
}
...
mail_poll() {
  ...
  if [ ! -f "$SCRIPT_DIR/xo-wake-lib.sh" ]; then
    echo "xo-mail: $SCRIPT_DIR/xo-wake-lib.sh missing; cannot poll" >&2
    return 1
  fi
  # shellcheck source=bin/xo-wake-lib.sh
  # shellcheck disable=SC1091
  . "$SCRIPT_DIR/xo-wake-lib.sh"
  xo_lock_acquire_wait "$STATE_DIR/.mail-seen.lock"
  if ! list="$(run_py poll_list)"; then
    ...
    # lock still held.
    xo_lock_release "$STATE_DIR/.mail-seen.lock"
    return 1
  fi
  ...
  if ! mail_heal "$generation"; then
    echo "xo-mail: heal could not record a uid; journal kept; retried on next poll" >&2
    xo_lock_release "$STATE_DIR/.mail-seen.lock"
    return 1
  fi
  ...
  while IFS= read -r line || [ -n "$line" ]; do
      ...
      retry)
        ...
        if mail_retry_published "$generation" "$uid"; then
          if ! mail_retry_remove "$uid"; then
            echo "xo-mail: could not clear retry for recovered $uid after publish; retried on next poll" >&2
            xo_lock_release "$STATE_DIR/.mail-seen.lock"
            return 1
          fi
        else
    ...
    if [ "$need_wake" -eq 1 ]; then
      ...
      # bounds the durable wake queue instead of flooding xo.
      if [ "$woke" -ge "$MAIL_MAX_WAKES" ]; then
        echo "xo-mail: per-poll wake cap ($MAIL_MAX_WAKES) reached; remaining mail surfaces on the next poll" >&2
        break
      fi
      if [ "$status" = degraded ]; then
        ...
        if ! mail_retry_add "$uid"; then
          echo "xo-mail: could not record retry for $uid; retried on next poll" >&2
          xo_lock_release "$STATE_DIR/.mail-seen.lock"
          return 1
        fi
      fi
      ...
      if [ "$wake_rc" -eq 0 ]; then
        echo "xo-mail: woke for $uid"
        woke=$((woke + 1))
      else
        echo "xo-mail: wake failed for $uid; retried on next poll" >&2
        xo_lock_release "$STATE_DIR/.mail-seen.lock"
        return 1
      fi
    fi
...
case "${1:-}" in
  ...
  send)
    ...
    ;;
  status)
    echo "mail account: $XO_MAIL_USER"
