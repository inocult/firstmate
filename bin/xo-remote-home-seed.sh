...
#
# Usage:
#   xo-remote-home-seed.sh <id> <ssh-alias> <remote-root> <remote-home> {<project>[=<origin-url>]...|--no-projects}
#
# The SSH alias must already reach a host whose non-interactive PATH exposes the
# fixed xo-remote-entrypoint.sh from <remote-root>. The command records the
# remote host dimension in data/secondmates.md, gates the host on
# xo-remote-doctor.sh readiness before touching it, sends a bounded provisioning
# manifest through xo-on.sh, and lets the remote host clone its own XO
# home and project origins. No project tree or secret environment is copied.
#
# Each project needs an origin the remote account can clone. XO resolves
...
# bin/xo-project-origin-lib.sh owns which URLs are accepted, and this home's
...
xo_lock_acquire_wait "$REGISTRY_LOCK" || die "cannot lock the secondmate registry"
...
BRIEF_CREATED=0
if [ ! -f "$BRIEF" ]; then
  [ -n "${XO_SECONDMATE_CHARTER:-}" ] || die "no filled charter at $BRIEF; set XO_SECONDMATE_CHARTER or scaffold one first"
  if [ "$NO_PROJECTS" -eq 1 ]; then
    "$SCRIPT_DIR/xo-brief.sh" "$ID" --secondmate --no-projects >/dev/null
  else
    "$SCRIPT_DIR/xo-brief.sh" "$ID" --secondmate "${PROJECT_NAMES[@]}" >/dev/null
  fi
  BRIEF_CREATED=1
fi
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/xo-remote-home-seed.XXXXXX") || die "cannot create seed staging directory"
REG_EXISTED=0
...
for project in "${PROJECT_NAMES[@]+"${PROJECT_NAMES[@]}"}"; do
  ...
  MODE_LINE=$(XO_HOME="$XO_HOME" XO_DATA_OVERRIDE="$DATA" "$SCRIPT_DIR/xo-project-mode.sh" "$project")
  read -r MODE _ <<EOF
$MODE_LINE
EOF
  case "$MODE" in
...

{
  ...
  # lives on another machine (bin/xo-teardown.sh's cleanup gate). It is
...
set +e
xo_remote_readiness_ensure "$SCRIPT_DIR" "$ID"
PREFLIGHT_RC=$?
set -e
if [ "$PREFLIGHT_RC" -ne 0 ]; then
  ...
  die "remote runtime preflight failed; nothing was provisioned. Close the gaps listed above, or update the remote code root if it predates the current xo-remote-doctor.sh"
fi

set +e
PROVISION_OUT=$("$SCRIPT_DIR/xo-on.sh" --stdin "$ID" xo-remote-home-provision.sh < "$TMP/manifest" 2>&1)
PROVISION_RC=$?
set -e
