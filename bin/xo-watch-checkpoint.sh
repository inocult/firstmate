...
SECONDS_ARG=${XO_CODEX_WATCH_CHECKPOINT:-180}

usage() {
  cat <<'EOF'
Usage: xo-watch-checkpoint.sh [--seconds <n>]

Run bin/xo-watch.sh in the foreground for a bounded checkpoint.
...
OUT=$(mktemp "${TMPDIR:-/tmp}/xo-watch-checkpoint.out.XXXXXX") || exit 1
ERR=$(mktemp "${TMPDIR:-/tmp}/xo-watch-checkpoint.err.XXXXXX") || {
  rm -f "$OUT"
  exit 1
}
...
run_with_perl_timeout() {
  perl -e '
    ...
    local $SIG{ALRM} = sub {
      kill "TERM", -$pid;
      my $grace = $ENV{XO_SIGNAL_GRACE} || 5;
      local $SIG{ALRM} = sub {
        kill "KILL", -$pid;
        waitpid $pid, 0;
        exit 124;
    ...
    alarm 0;
    exit($? >> 8);
  ' "$SECONDS_ARG" "$SCRIPT_DIR/xo-watch.sh"
}

set +e
if command -v timeout >/dev/null 2>&1; then
  timeout "$SECONDS_ARG" "$SCRIPT_DIR/xo-watch.sh" >"$OUT" 2>"$ERR"
  RC=$?
elif command -v gtimeout >/dev/null 2>&1; then
  gtimeout "$SECONDS_ARG" "$SCRIPT_DIR/xo-watch.sh" >"$OUT" 2>"$ERR"
  RC=$?
else
