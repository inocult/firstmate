...
# Historical payload literals are intentionally isolated below this line. They
# exist only for persisted pre-protocol transcripts, whose bytes predate both
# the typed envelope and the firstmate-to-xo rename, so the ones that named the
...
xo_legacy_operational_input_kind() {  # <message> <result-var>
  ...
  case "$message" in
    "$XO_OPERATIONAL_PREFIX"?*|"$XO_LEGACY_OPERATIONAL_PREFIX"?*)
