#!/usr/bin/env bash
# xo-bearings-board.sh - build and arm the /bearings lavish fleet board.
#
...
# injected xo-bearings-board.v1 JSON payload. This script owns the mechanics so
# the invoking agent's per-run work stays "compose the JSON, run build" - the
# agent never authors board UI at invocation time.
#
# Usage:
#   xo-bearings-board.sh build <data.json>
#   xo-bearings-board.sh path
#
...
#            Bind to the keyed-answer intake (bin/xo-captain-hold.sh) ALWAYS
...
# rows, or when `bin/xo-captain-hold.sh open` reports the task is no longer an
...
# schema=xo-bearings-board.v1 and every renderer-consumed field must satisfy
# the xo-bearings-board.v1 types and item invariants below. Every fleet row and
...
#
# The board path is stable - $XO_HOME/.lavish/bearings-board.html - so a
...
#
# XO_BEARINGS_BOARD_TEMPLATE overrides the shipped template path (tests only).
set -eu
...
TEMPLATE="${XO_BEARINGS_BOARD_TEMPLATE:-$SCRIPT_DIR/../.agents/skills/bearings/assets/board-template.html}"
PLACEHOLDER='__XO_BEARINGS_BOARD_DATA__'
BOARD_SCHEMA=xo-bearings-board.v1
...
decision_card_is_stale() {  # <task-id> <landed-0-or-1>
  ...
  "$SCRIPT_DIR/xo-captain-hold.sh" open "$task" --distinguish-absent >/dev/null 2>&1 || rc=$?
...
# The OWNER column bin/xo-procevent.sh already publishes: live, none,
# orphaned, or uncertain. Empty means the source is not registered at all.
source_owner() {  # <source-id>
  "$SCRIPT_DIR/xo-procevent.sh" list 2>/dev/null \
...
command_build() {
  ...
  effective=$(umask 077; mktemp "${TMPDIR:-/tmp}/xo-bearings-payload.XXXXXX") \
  ...
  sid=$("$SCRIPT_DIR/xo-procevent-lavish.sh" source-id "$board") \
    || fail "cannot derive the board source id"
  pre_reopen_owner=$(source_owner "$sid")
  establish_board_session "$board"
  if [ "$BOARD_SESSION_REOPENED" = 1 ]; then
    "$SCRIPT_DIR/xo-procevent-lavish.sh" retire "$board" >/dev/null \
  ...
  "$SCRIPT_DIR/xo-captain-hold.sh" bind "$sid" >/dev/null \
    || fail "cannot bind the board source to the keyed-answer intake"
  printf 'bound: %s\n' "$sid"

  owner=$(source_owner "$sid")
  if [ "$BOARD_SESSION_REOPENED" = 1 ]; then
    "$SCRIPT_DIR/xo-procevent-lavish.sh" arm "$board" >/dev/null \
  ...
  if [ "$owner" != live ]; then
    "$SCRIPT_DIR/xo-procevent.sh" reconcile >/dev/null 2>&1 || true
