#!/usr/bin/env bash
# xo-test-isolation-proof.sh - bounded concurrent isolation proofs for portable
...
# bounded local xo-test-run.sh --jobs admission, and family worker caps are owned
# by bin/xo-test-run.sh (docs/xo-test-portable-shards.md).
#
# It does NOT compose production CI shard membership; xo-test-run.sh owns that
...
#
# Usage:
#   xo-test-isolation-proof.sh [--pool <name>] [--jobs N] [--json path] [--list]
#   xo-test-isolation-proof.sh --list-exclusions
#   xo-test-isolation-proof.sh -h | --help
#
# Options:
#   --pool NAME  candidate pool: "portable" (default, this harness's own curated
#                set) or a bin/xo-test-run.sh family name, to prove a stateful
#                family that stays serial on CI but may earn bounded local
#                concurrency. bin/xo-test-run.sh's list_concurrent_safe_families
...
#                run; xo_test_run_jobs_enabled is true only for a successful
...
#   - TMPDIR/TMP point only at that root so mktemp/xo_test_tmproot stay private
#   - ambient XO_HOME / XO_*_OVERRIDE cleared so no shared home is reused
#   - no global git config mutation (snapshot before/after)
#   - no production sharding and no retry-until-green
#
# Markers (stdout):
#   XO_ISOLATION_BEGIN <iso8601> concurrency=<n> candidates=<n>
#   XO_ISOLATION_CANDIDATE_BEGIN <iso8601> <script> worker=<i>
#   XO_ISOLATION_CANDIDATE_END <iso8601> <script> exit=<code> duration_ms=<n> worker=<i>
#   XO_ISOLATION_SUMMARY total=<n> failed=<n> concurrency=<n> duration_ms=<n>
#
...
die() {
  printf 'xo-test-isolation-proof: %s\n' "$*" >&2
  exit 2
}
...
exclusion_reason() {
  case "$1" in
    ...
    xo-backend.test.sh)
      ...
      ;;
    xo-spawn-dispatch-profile.test.sh|xo-spawn-worktree-settle.test.sh|xo-trace-context-spawn.test.sh)
      printf '%s\n' 'real isolated git worktrees plus spawn settle loops; gray zone until dedicated proof'
      ;;
    xo-pr-check-security.test.sh)
    ...
    xo-teardown.test.sh)
      ...
      ;;
    xo-herdr-session-cleanup.test.sh)
    ...
    xo-wake-queue.test.sh|xo-watch-checkpoint.test.sh|xo-watch-triage.test.sh|\
    xo-watcher-lock.test.sh)
      printf '%s\n' 'watcher/wake/lock family; intentional process locks and daemon races'
      ;;
    xo-afk-inject-e2e.test.sh|xo-afk-return.test.sh|xo-afk-inject-herdr-e2e.test.sh|\
    xo-afk-launch.test.sh)
    ...
    xo-opencode-primary-live-e2e.test.sh|xo-pi-primary-live-e2e.test.sh|\
    xo-quota-array-dispatch-live-e2e.test.sh|xo-send-secondmate-marker-herdr-e2e.test.sh|\
    xo-sessionstart-instruction-refresh-live-e2e.test.sh)
      printf '%s\n' 'live harness opt-in; never default parallel CI'
      ;;
    xo-backend-autodetect-smoke.test.sh|xo-backend-herdr-eventwait-smoke.test.sh|\
    xo-backend-herdr-presentation-e2e.test.sh|xo-backend-herdr-prune-safety-e2e.test.sh|\
    xo-backend-herdr-respawn-idem-e2e.test.sh|xo-backend-herdr-smoke.test.sh|\
    xo-backend-herdr-agent-exit-shell-e2e.test.sh|\
    xo-backend-herdr-workspace-per-home-e2e.test.sh|xo-herdr-session-cleanup-e2e.test.sh)
...
xo-backend-tmux-smoke.test.sh
xo-backend.test.sh
xo-spawn-dispatch-profile.test.sh
xo-spawn-worktree-settle.test.sh
xo-trace-context-spawn.test.sh
xo-pr-check-security.test.sh
xo-teardown.test.sh
xo-watcher-lock.test.sh
xo-wake-queue.test.sh
xo-afk-inject-e2e.test.sh
xo-backend-herdr-smoke.test.sh
xo-backend-cmux-smoke.test.sh
xo-pi-primary-live-e2e.test.sh
xo-quota-array-dispatch-live-e2e.test.sh
EOF
}
...
doc = {
    ...
    "xo_test_run_jobs_enabled": jobs_enabled == "1",
}
...
# concurrency; bin/xo-test-run.sh's list_concurrent_safe_families records which
...
pool_candidates() {
  case "$POOL:$LIST_ONLY" in
    portable:1)
      ...
      ;;
    portable:0)
      "$ROOT/bin/xo-test-run.sh" --list-scheduled --proven-isolated
      ;;
    *:1)
      "$ROOT/bin/xo-test-run.sh" --list --family "$POOL" \
        || die "--pool $POOL is not a known family (see bin/xo-test-run.sh --list-families)"
      ;;
    *)
      "$ROOT/bin/xo-test-run.sh" --list-scheduled --family "$POOL" \
        || die "--pool $POOL is not a known family (see bin/xo-test-run.sh --list-families)"
      ;;
  esac
}
...
PROOF_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xo-isolation-proof.XXXXXX")
...
RUN_ID="xo-isolation-${RUN_STARTED_MS}-$$"
TOTAL=${#CANDIDATES[@]}
FAILED=0
AGG_RC=0

printf 'XO_ISOLATION_BEGIN %s concurrency=%s candidates=%s\n' \
...
wait_one_slot() {
  ...
  printf 'XO_ISOLATION_CANDIDATE_END %s %s exit=%s duration_ms=%s worker=%s\n' \
...
for script in "${CANDIDATES[@]}"; do
  ...
  printf 'XO_ISOLATION_CANDIDATE_BEGIN %s %s worker=%s\n' \
    "$(now_iso)" "$script" "$idx"
...
if find "$PROOF_ROOT" -type f -name 'xo-arm-pretool-check-claude-stderr.*' 2>/dev/null | grep -q .; then
...
printf 'XO_ISOLATION_SUMMARY total=%s failed=%s concurrency=%s duration_ms=%s\n' \
...
if [ -n "$JSON_PATH" ]; then
  ...
  jobs_enabled=0
  jobs_max=0
  if "$ROOT/bin/xo-test-run.sh" --list-concurrent-safe-families | grep -Fxq "$POOL"; then
    jobs_max=$("$ROOT/bin/xo-test-run.sh" --concurrent-safe-family-jobs-max "$POOL")
  fi
