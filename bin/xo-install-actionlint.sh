#!/usr/bin/env bash
# xo-install-actionlint.sh - install CI's pinned, verified actionlint build.
#
...
#
# Usage:
#   xo-install-actionlint.sh <destination-directory>
set -eu

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
VERSION="$("$ROOT/bin/xo-lint-workflows.sh" --required-version)"

die() {
  printf 'xo-install-actionlint.sh: %s\n' "$*" >&2
  exit 1
}

DESTINATION=${1:?usage: xo-install-actionlint.sh <destination-directory>}
...
TMP=$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/xo-actionlint.XXXXXX")
trap 'rm -rf "$TMP"' EXIT
...
while ! curl -fsSL "$URL" -o "$TMP/$ARCHIVE"; do
  [ "$download_attempt" -lt "$DOWNLOAD_ATTEMPTS" ] || {
    printf 'xo-install-actionlint.sh: download failed after %s attempts\n' "$DOWNLOAD_ATTEMPTS" >&2
    exit 1
  }
  printf 'xo-install-actionlint.sh: download attempt %s failed; retrying\n' "$download_attempt" >&2
...
[ "$ACTUAL_SHA256" = "$SHA256" ] || {
  printf 'xo-install-actionlint.sh: checksum mismatch for %s (expected %s, got %s)\n' \
