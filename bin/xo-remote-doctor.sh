...
#
# Run it through xo-on.sh so the fixed entrypoint invokes this readiness owner
...
# xo-remote session. Its account therefore needs the XO-owned Aqua Herdr
# agent plus the sibling dev.xo.remote-job worker that runs normal xo-on
# commands through the Aqua or Linux job-worker path. On darwin, that Herdr
# agent runs bin/xo-remote-herdr-guard.sh through the remote account's login
...
# bin/xo-remote-herdr-owner-lib.sh owns that birth test. Doctor remains
...
# both XO-owned Aqua agents, starts the Linux workers where no Aqua agent
...
# changes FileVault, stores an account password, or replaces a non-XO
...
# shellcheck source=bin/xo-remote-herdr-owner-lib.sh
. "$SCRIPT_DIR/xo-remote-herdr-owner-lib.sh"
...
LAUNCH_AGENT_LABEL=dev.xo.herdr.xo-remote
...
case "${1:-}" in
  ...
  --fix) MODE=fix; shift ;;
  --worker-tool-probe)
    [ "${XO_REMOTE_JOB_ACTIVE:-}" = 1 ] || { printf 'error: worker tool probe requires the remote job worker\n' >&2; exit 64; }
...
herdr_adapter_load() {
  [ -z "${XO_REMOTE_DOCTOR_HERDR_LOADED:-}" ] || return 0
  herdr_cli_available || return 1
  [ -f "$SCRIPT_DIR/xo-backend.sh" ] && [ -f "$SCRIPT_DIR/backends/herdr.sh" ] || return 1
  # shellcheck source=bin/xo-backend.sh
  . "$SCRIPT_DIR/xo-backend.sh" || return 1
  xo_backend_source herdr || return 1
  XO_REMOTE_DOCTOR_HERDR_LOADED=1
}

herdr_server_status_json() {
  herdr_adapter_load || return 1
  xo_backend_herdr_cli "$HERDR_SESSION_NAME" status --json 2>/dev/null
}
...
# not resolve. bin/xo-remote-herdr-owner-lib.sh owns the markers.
herdr_server_birth() {
  local socket owner rc birth
  socket=$(herdr_server_status_json | jq -r '.server.socket // empty' 2>/dev/null) || socket=
  owner=$(xo_remote_herdr_socket_owner "$socket"); rc=$?
  if [ "$rc" -eq 2 ]; then
    printf 'nolsof\n'
    return 0
  fi
  ...
  birth=$(xo_remote_herdr_owner_birth "$owner")
  printf '%s %s\n' "$birth" "$owner"
}
...
herdr_server_aqua_owned() {
  ...
  xo_remote_herdr_birth_is_aqua "${birth%% *}"
}
...
resolve_launch_agent_shell() {
  ...
  if [ -n "${XO_LAUNCH_AGENT_SHELL:-}" ] && [ -x "$XO_LAUNCH_AGENT_SHELL" ]; then
    printf '%s' "$XO_LAUNCH_AGENT_SHELL"
    return 0
  fi
...
# Login-shell command that execs the XO-owned guard, which in turn execs
...
launch_agent_guard_path() {
  printf '%s/bin/xo-remote-herdr-guard.sh' "$XO_ROOT"
}
...
remote_job_existing_state() {
  local root
  root=${XO_REMOTE_JOB_STATE_ROOT:-${HOME:-}/.xo/remote-job}
  root=$(xo_remote_job_canonical_existing_dir "$root") || return 1
  xo_remote_job_canonical_existing_dir "$root/jobs" >/dev/null || return 1
...
remote_job_probe_ok() {
  ...
  ready="$XO_REMOTE_JOB_STATE/worker.ready"
...
remote_job_identity_ok() {
  ...
  xo_remote_job_worker_identity_matches "$XO_ROOT" "${HOME:-}"
}

check_remote_job_worker() {
  local worker
  worker="$XO_ROOT/bin/xo-remote-job-worker.sh"
  if [ ! -f "$worker" ] || [ -L "$worker" ] || [ ! -x "$worker" ]; then
    record remote-job-worker "human: the configured XO code root has no safe remote job worker" \
      "update the remote XO checkout, then rerun this command with --fix"
  ...
  if [ "$PLATFORM" = darwin ]; then
    xo_remote_job_launchagent_paths "${HOME:-}"
    if xo_remote_job_launchagent_contract_matches "$XO_ROOT" "${HOME:-}"; then
      record remote-job-worker "ok: $XO_REMOTE_JOB_LAUNCH_AGENT_PLIST matches the XO-owned Aqua worker contract"
    else
      record remote-job-worker "fixable: $XO_REMOTE_JOB_LAUNCH_AGENT_PLIST does not match the XO-owned Aqua worker contract" \
        "rerun this command with --fix to write dev.xo.remote-job"
    fi
    ...
    elif xo_remote_job_launchagent_loaded "$XO_ROOT" "${HOME:-}" "$UID_NUM"; then
      record remote-job-worker-loaded "ok: $XO_REMOTE_JOB_LABEL is loaded in gui/$UID_NUM"
    elif check_is_ok gui-session; then
      record remote-job-worker-loaded "fixable: $XO_REMOTE_JOB_LABEL is not loaded in gui/$UID_NUM" \
        "rerun this command with --fix to bootstrap the worker"
    else
      record remote-job-worker-loaded "human: $XO_REMOTE_JOB_LABEL cannot be loaded because gui/$UID_NUM has no login session" \
        "close the login-session gap first; SSH cannot create an Aqua session"
    fi
  else
    local pid
    pid=$(cat "${XO_REMOTE_JOB_STATE_ROOT:-${HOME:-}/.xo/remote-job}/worker.pid" 2>/dev/null || true)
    if [ "${XO_REMOTE_JOB_ACTIVE:-}" = 1 ] ||
  ...
  if ! remote_job_probe_ok; then
    record remote-job-probe "fixable: the remote job worker has not reported a fresh probe" \
      "rerun this command with --fix to restart the worker, then rerun through xo-on.sh"
  elif ! remote_job_identity_ok; then
    set_check remote-job-worker "fixable: the running remote job worker does not match the current XO code" \
...
report_required_tools() {
  ...
  for tool in "${REQUIRED_TOOLS[@]}"; do
    ...
    if [ -n "$resolved" ] && [ -x "$resolved" ]; then
      if [ "$tool" = tasks-axi ] && ! xo_tasks_axi_compatible; then
...
report_required_tools_from_worker() {
  ...
  if ! job_id=$(xo_remote_job_stage "${HOME:-}" "$XO_ROOT" "${XO_HOME:-}" \
    xo-remote-doctor.sh --worker-tool-probe </dev/null); then
  ...
  if ! xo_remote_job_wait "${HOME:-}" "$job_id"; then
    xo_remote_job_reap "${HOME:-}" "$job_id" 2>/dev/null || true
    set_check remote-job-probe "fixable: the remote job worker did not complete the required-tool probe" \
      "rerun this command with --fix to restart the worker"
    report_required_tools
    return 0
  fi
  probe_stdout=$XO_REMOTE_JOB_STDOUT
  probe_stderr=$XO_REMOTE_JOB_STDERR
  probe_exit=$XO_REMOTE_JOB_EXIT
  MISSING=()
...
repair_tool_wrapper() { # <tool>
  ...
  target=$(xo_remote_job_manager_tool "${HOME:-}" "$tool" 2>/dev/null || true)
  [ -n "$target" ] || return 1
  wrapper="${HOME:-}/.local/bin/$tool"
  if [ -e "$wrapper" ] || [ -L "$wrapper" ]; then
    if ! wrapper_is_xo_owned "$wrapper"; then
      fix_report "required-$tool" failed "$wrapper exists and is not XO-owned"
      return 1
    fi
  else
...
repair_required_wrappers() {
  ...
  for tool in "${HARNESS_TOOLS[@]}"; do
    xo_remote_job_manager_tool "${HOME:-}" "$tool" >/dev/null 2>&1 || continue
    repair_tool_wrapper "$tool" && return 0
  done
}

fix_remote_job_worker() {
  if xo_remote_job_ensure_worker "$XO_ROOT" "${HOME:-}"; then
    [ "$XO_REMOTE_JOB_REPAIRED" -eq 0 ] || fix_report remote-job-worker applied "installed or reloaded $XO_REMOTE_JOB_LABEL"
    return 0
  fi
  fix_report remote-job-worker failed "${XO_REMOTE_JOB_ERROR:-the remote job worker could not start}"
  return 1
}
...
check_herdr() {
  ...
  if resolved=$(command -v herdr 2>/dev/null) && [ -x "$resolved" ]; then
    if herdr_adapter_load; then
      xo_backend_herdr_client_select "$HERDR_SESSION_NAME"
      selected=$(xo_backend_herdr_bin)
...
check_gui_session() {
  ...
  if [ -z "$UID_NUM" ]; then
    record gui-session "human: the account uid could not be read, so its login session cannot be inspected" \
      "run 'id -u' on that account and report the failure; XO cannot address gui/<uid> without it"
    return 0
  fi
  ...
  record gui-session "human: no Aqua login session exists for uid $UID_NUM" \
    "log that account in once at the console, and enable automatic login in System Settings > Users & Groups if the machine runs headless; SSH cannot create a GUI session, and XO never writes an auto-login password or changes FileVault"
}
...
check_launch_agent() { # <resolved-login-shell>
  ...
  if [ -f "$LAUNCH_AGENT_PLIST" ] && [ ! -L "$LAUNCH_AGENT_PLIST" ]; then
    if launch_agent_contract_matches "$shell"; then
      record launchagent "ok: $LAUNCH_AGENT_PLIST matches the XO-owned contract"
    else
      record launchagent "fixable: $LAUNCH_AGENT_PLIST does not match the current XO-owned contract" \
    ...
    record launchagent "fixable: no XO herdr launch agent at $LAUNCH_AGENT_PLIST" \
...
check_launch_agent_loaded() { # <resolved-login-shell>
  ...
  if launchctl print "gui/$UID_NUM/$LAUNCH_AGENT_LABEL" >/dev/null 2>&1; then
    if launch_agent_loaded_contract_matches "$shell"; then
      ...
      record launchagent-loaded "fixable: gui/$UID_NUM/$LAUNCH_AGENT_LABEL does not match the effective XO-owned contract" \
    ...
    case "$birth" in
      ...
      unproven)
        record herdr-server "fixable: session $HERDR_SESSION_NAME is running but no herdr process can be shown to own its socket, so its birth cannot be proven" \
          "rerun this command with --fix so the launch agent takes the session over (its current panes close and the parent xo relaunches its mates)"
        ;;
      *)
...
check_entrypoint_link() {
  ...
  if [ -e "$ENTRYPOINT_LINK" ] || [ -L "$ENTRYPOINT_LINK" ]; then
    record entrypoint-link "human: $ENTRYPOINT_LINK exists but is not the symlink to $want" \
      "inspect that path yourself and replace it with 'ln -sfn $want $ENTRYPOINT_LINK' if it is stale; XO never overwrites a file it did not create there"
    return 0
  fi
...
start_herdr_server() {
  ...
  if xo_backend_herdr_server_ensure "$HERDR_SESSION_NAME" >/dev/null 2>&1; then
...
link_entrypoint() {
  local want="${XO_ROOT_OVERRIDE:-}/bin/xo-remote-entrypoint.sh"
...
printf 'mode=%s\n' "$MODE"
printf 'path=%s\n' "${PATH:-}"
if [ -n "${XO_ROOT_OVERRIDE:-}" ] && [ "${PATH%%:*}" = "$XO_ROOT_OVERRIDE/bin" ]; then
...
if [ "${XO_REMOTE_JOB_ACTIVE:-}" = 1 ] || ! remote_job_identity_ok; then
  report_required_tools
else
  report_required_tools_from_worker
fi
