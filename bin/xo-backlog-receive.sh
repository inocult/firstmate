...
#
# Usage:
#   xo-backlog-receive.sh state/handoff/<secondmate-id>.outbox.md <bytes> <sha256> <generation>
#
# The delivered file must be a non-symlink backlog-format scratch file confined
# to XO_HOME/state/handoff. Every item must be Queued. Keys already present in
...
[ -f "$XO_HOME/.xo-secondmate-home" ] && [ ! -L "$XO_HOME/.xo-secondmate-home" ] \
  || die "XO_HOME is not a seeded secondmate home"
[ -f "$XO_HOME/AGENTS.md" ] && [ -d "$XO_HOME/bin" ] || die "XO_HOME is not an XO home"
HOME_REAL=$(CDPATH='' cd -- "$XO_HOME" 2>/dev/null && pwd -P) || die "XO_HOME cannot be resolved"
PARENT=$(dirname "$XO_HOME/$REL")
...
xo_lock_acquire_wait "$TRANSFER_LOCK" || die "cannot lock delivered outbox"
trap 'xo_lock_release "$TRANSFER_LOCK" || true' EXIT
...
if [ "${#TO_MOVE[@]}" -gt 0 ]; then
  xo_tasks_axi_compatible || die "a compatible tasks-axi is required for atomic backlog receipt; run bin/xo-bootstrap.sh for the required version"
...
xo_lock_release "$TRANSFER_LOCK" || die "receipt succeeded but transfer lock cleanup failed"
trap - EXIT
