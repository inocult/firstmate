#!/usr/bin/env bash
# xo-afk-return.sh - deterministic away-mode return: the return brief and the
# catch-up gate.
#
# Usage:
#   xo-afk-return.sh          Stop away mode, render the return brief, and open/check the gate.
#   xo-afk-return.sh begin    Same as the default command.
#   xo-afk-return.sh check    Re-render the brief and close the gate only after blockers resolve.
#   xo-afk-return.sh guard    Read-only refusal while away or catch-up is pending.
#
...
# (bin/xo-afk-contract.sh), the supervision outcome store
# (bin/xo-branch-outcome.sh), the held set in the backlog (tasks-axi), and the
...
#
# THE GATE. `blocked:` is the crewmate protocol's xo-actionable verb. A
...
# (xo-afk-clauses-execute-r1). Away-window attribution uses second-resolution
...
window_start_epoch() {
  ...
  if [ -z "$epoch" ] && xo_afk_contract_present "$STATE"; then
...
store_rows_load() {  # <since-epoch>
  ...
  raw=$("$SCRIPT_DIR/xo-branch-outcome.sh" list --recent 1000000 2>/dev/null) \
    || return 1
...
print_blockers() {  # <file>
  ...
  while IFS="$(printf '\t')" read -r tag id key summary; do
    ...
    printf 'xo-actionable blocker: %s [key=%s] %s\n' "$id" "$key" "$summary"
  done < "$file"
}
...
return_guard() {
  if [ -e "$STATE/.afk" ] || xo_afk_contract_present "$STATE"; then
    printf 'xo-afk-return: away mode is still active; run bin/xo-afk-return.sh before ordinary captain work\n' >&2
    return 3
  fi
  if [ -e "$GATE" ]; then
    printf 'xo-afk-return: return catch-up is pending; remediate or durably reclassify every listed blocker, then run bin/xo-afk-return.sh check\n' >&2
...
render_return_brief() {  # <evidence-file> <blockers-file> <since-epoch>
  ...
  if [ -n "$record" ]; then
    archive_dir=$(xo_afk_contract_archive_dir "$STATE")
  ...
  count=0
  HELD_READ_FAILED=0
  HELD_READ_PATH=$(xo_backlog_file "$DATA" 2>/dev/null || printf '%s/backlog.md' "$DATA")
  if held=$(xo_backlog_row_list "$DATA" --state held --fields hold_kind,hold_reason,hold_until 2>&1); then
...
EOF
  ...
  while IFS="$(printf '\t')" read -r tag task key summary; do
    [ "$tag" = blocker ] || continue
    count=$((count + 1))
    printf '  - %s [key=%s] still blocked, xo remediates before ordinary work: %s\n' "$task" "$key" "$summary"
  done < "$blockers"
  for meta in "$STATE"/*.meta; do
    [ -f "$meta" ] || continue
    task=$(basename "$meta"); task=${task%.meta}
...
return_reconcile() {
  ...
  while IFS="$(printf '\t')" read -r tag kind text; do
    ...
    if [ ! -f "$retained_live" ]; then
      ...
      lifecycle_ok=0
    elif ! xo_afk_contract_validate "$retained_live" 1; then
...
EOF
  ...
  if [ -e "$STATE/.afk" ] || [ -e "$STATE/.afk-daemon-terminal" ] || xo_afk_contract_present "$STATE"; then
    if ! "$SCRIPT_DIR/xo-afk-launch.sh" stop; then
      lifecycle_ok=0
      append_evidence lifecycle 'away-mode shutdown failed; lifecycle state preserved for retry' "$evidence"
    fi
  fi

  drained=$("$SCRIPT_DIR/xo-wake-drain.sh" 2> "$drain_err") || {
  ...
  if xo_afk_contract_present "$STATE"; then
    if ! xo_afk_contract_validate "$(xo_afk_contract_path "$STATE")" 1; then
      append_evidence lifecycle "away-posture record unreadable: $(xo_afk_contract_path "$STATE"); catch-up stays gated" "$evidence"
      lifecycle_ok=0
    else
    ...
    if [ -z "$archived_contract" ]; then
      ...
      lifecycle_ok=0
    elif ! xo_afk_contract_validate "$archived_contract" 1; then
    ...
    while IFS="$(printf '\t')" read -r tag retained_record; do
      ...
      if [ ! -f "$retained_record" ]; then
        ...
        lifecycle_ok=0
      elif ! xo_afk_contract_validate "$retained_record" 1; then
...
EOF
    ...
    for superseded_record in "$(xo_afk_contract_archive_dir "$STATE")/$contract_since-superseded-"*.afk-contract; do
      [ -f "$superseded_record" ] || continue
      if ! xo_afk_contract_validate "$superseded_record" 1; then
  ...
  if [ "$lifecycle_ok" -ne 1 ] || grep -q "^blocker$(printf '\t')" "$blockers"; then
    ...
    printf 'xo-afk-return: catch-up must finish before the captain request\n' >&2
    print_evidence "$GATE" >&2
    print_blockers "$GATE" >&2
    printf 'xo-afk-return: handle each blocker now, or close it with resolved [key=...] and append a durable reclassification reason, then run bin/xo-afk-return.sh check\n' >&2
  ...
  if ! print_evidence "$evidence"; then
    ...
    write_gate "$evidence" "$blockers" || { rm -f "$evidence" "$blockers" "$drain_err"; return 1; }
    printf 'xo-afk-return: recovery evidence could not be published; catch-up remains pending\n' >&2
    rm -f "$evidence" "$blockers" "$drain_err"
    return 3
  fi
  ...
  clear_delivery_artifacts
  rm -f "$evidence" "$blockers" "$drain_err"
  printf 'xo-afk-return: catch-up clear; ordinary captain work may proceed\n'
  return 0
}
...
main() {
  ...
  # backlog reader. `guard` returned above without sourcing xo-wake-lib.sh,
  ...
  contract_epoch=$(gate_contract_epoch)
  if [ -z "$contract_epoch" ] && xo_afk_contract_present "$STATE"; then
    contract_epoch=$("$CONTRACT" field entered_epoch 2>/dev/null || true)
    case "$contract_epoch" in ''|*[!0-9]*) contract_epoch= ;; esac
  fi
  write_pending_seed "$window_epoch" "$contract_epoch" || { xo_lock_release "$LOCK"; trap - EXIT; return 1; }
  return_reconcile
  rc=$?
