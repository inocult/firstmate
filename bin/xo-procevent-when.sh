...
#
# Usage:
#   xo-procevent-when.sh arm <name> [options] --condition <argv>... --action <argv>...
#   xo-procevent-when.sh classify <result-file>
#   xo-procevent-when.sh terminal <result-file>
#   xo-procevent-when.sh source-id <name>
#   xo-procevent-when.sh retire <name>
#   xo-procevent-when.sh run <source-id>
#
# arm        Bind a (condition, action) pair as process-event source
#            "when-<name>". The spec is written privately under state/when/ and
#            hash-bound by a trust record the same way xo-check-register.sh
...
#              --deadline <secs>           give up and wake xo if the condition
...
#            retrying silently, so xo is always woken with the evidence.
#
...
# acknowledgement all belong to bin/xo-procevent.sh; this adapter owns only the
...
WHEN_DIR="$STATE/when"
OUTPUT_TAIL_BYTES=${XO_WHEN_OUTPUT_TAIL_BYTES:-8192}
...
when_name_valid() {
  local name=${1-}
  xo_task_id_path_safe "$name" || return 1
  xo_procevent_source_id_valid "when-$name"
}
...
cmd_arm() {
  ...
  xo_procevent_source_lock_acquire "$sid" || die "cannot lock the watch source"
  trap 'xo_procevent_source_lock_release "$sid"' EXIT
  local leftover
  for leftover in "$(spec_file "$sid")" "$(trust_file "$sid")" "$(fired_file "$sid")" \
    "$(xo_procevent_registry_dir "$STATE")/$sid.source"; do
    if [ -e "$leftover" ] || [ -L "$leftover" ]; then
      die "watch already exists or left state behind: $leftover (retire it first)"
    fi
  done
  local pending
  pending=$(xo_procevent_pending "$STATE" | grep -c "/$sid\." || true)
  ...
  action_hash=$(xo_pr_sha256 "$action_path") || die "cannot hash the action executable"
  act[0]=$action_path
  device=$(xo_pr_file_device "$WHEN_DIR") || die "cannot inspect the watch directory"
  tmp=$(umask 077; mktemp "$WHEN_DIR/.spec.XXXXXX") || die "cannot stage the spec"
  {
    printf 'xo-when-spec-v1\n'
    printf 'armed=%s\n' "$(date +%s)"
    printf 'interval=%s\n' "$interval"
    printf 'stable=%s\n' "$stable"
    printf 'deadline=%s\n' "$deadline"
  ...
  if ! xo_pr_private_file_valid "$(spec_file "$sid")" 600 "$device" \
    || ! xo_pr_private_file_valid "$(trust_file "$sid")" 600 "$device"; then
    rm -f -- "$(spec_file "$sid")" "$(trust_file "$sid")"
    die "published spec failed validation"
  fi

  if ! xo_procevent_registration_publish_locked "$STATE" when "$sid" \
    "$SCRIPT_DIR/xo-procevent-when.sh" run "$sid"; then
    rm -f -- "$(spec_file "$sid")" "$(trust_file "$sid")"
    die "cannot register the watch source"
  fi
  xo_procevent_source_lock_release "$sid"
  trap - EXIT
  printf 'armed: %s\n' "$sid"
  printf 'starts on the watcher'"'"'s next cycle; or run: bin/xo-procevent.sh reconcile\n'
...
spec_load() {
  ...
  device=$(xo_pr_file_device "$WHEN_DIR") || { SPEC_ERROR="cannot inspect the watch directory"; return 1; }
  xo_pr_private_file_valid "$spec" 600 "$device" || { SPEC_ERROR="spec is missing or not private"; return 1; }
  xo_pr_private_file_valid "$trust" 600 "$device" || { SPEC_ERROR="trust record is missing or not private"; return 1; }
  {
    IFS= read -r version && IFS= read -r want && ! IFS= read -r extra
  } < "$trust" || { SPEC_ERROR="trust record is malformed"; return 1; }
  [ "$version" = xo-when-trust-v1 ] || { SPEC_ERROR="trust record has an unknown version"; return 1; }
  local LC_ALL=C
  [[ "$want" =~ ^[0-9a-f]{64}$ ]] || { SPEC_ERROR="trust record hash is malformed"; return 1; }
  hash=$(xo_pr_sha256 "$spec") || { SPEC_ERROR="cannot hash the spec"; return 1; }
  ...
  local cond_argc='' act_argc='' in_argv=0 read_cond=0 read_act=0
  {
    ...
    [ "$version" = xo-when-spec-v1 ] || { SPEC_ERROR="spec has an unknown version"; return 1; }
...
bounded_run() {
  ...
  shift 2
  xo_run_timed "$secs" "$@" 2>&1 | tail -c "$OUTPUT_TAIL_BYTES" > "$out"
  rc=${PIPESTATUS[0]}
  return "$rc"
}
...
cmd_run() {
  ...
  xo_procevent_source_id_valid "$sid" || die "source id must be path-safe: $sid"
  fired=$(fired_file "$sid")

  if ! positive_int "$OUTPUT_TAIL_BYTES"; then
    emit_doc "$sid" rejected "XO_WHEN_OUTPUT_TAIL_BYTES must be a positive integer; nothing was executed" 0 '' ''
    exit 0
  fi
  ...
  current_action_hash=$(xo_pr_sha256 "${ACT_ARGV[0]}") || current_action_hash=
...
cmd_retire() {
  ...
  if [ -e "$(fired_file "$sid")" ]; then
    for result in "$(xo_procevent_inbox_dir "$STATE")/$sid".*.result; do
      [ -e "$result" ] && captured=1
    done
  ...
  "$SCRIPT_DIR/xo-procevent.sh" retire "$sid" || die "cannot retire the watch source: $sid"
