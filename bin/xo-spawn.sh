...
# secondmate in its isolated xo home.
# Usage: xo-spawn.sh <task-id> <project-dir> --mode <no-mistakes|direct-PR|local-only> --yolo <on|off> [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>]
#        xo-spawn.sh <task-id> <project-dir> --scout [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>]
#        xo-spawn.sh <task-id> [<xo-home>] [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>] --secondmate
#   --mode and --yolo are this task's delivery contract, REQUIRED for every ship
#   spawn and refused on --scout and --secondmate spawns. XO resolves both
...
#   ship or scout spawn also refuses leftover `{TASK}` / `{XO_SPEC}`
...
#   intent. A legacy mixed Task is accepted there only under bin/xo-dod-lib.sh's
...
#   Ship/scout launches always supply xo-dod-lib.sh's current worker role scope
#   using the same private launch-brief overlay. This never rewrites a project's
#   instruction files or a secondmate's charter.
#        xo-spawn.sh <task-id> --relaunch [--harness <name>] [--model <name>] [--effort <level>]
#   --relaunch launches a replacement agent for an EXISTING task into that
#   task's own recorded endpoint and worktree instead of creating either. It is
#   the launch half of the control plane (bin/xo-control.sh relaunch), which
#   owns the checkpoint, the progress note, stopping the previous agent, and the
#   transaction; call xo-control rather than this flag directly unless you are
...
#   axes chosen by xo at intake. They are only threaded into harnesses whose
#   installed CLIs were verified to support that axis; unsupported axes are omitted
#   from that harness's launch rather than guessed. Ultra is the explicit
#   exception: bin/xo-harness.sh validate-native-effort owns its model scope;
#   supported Pi launches receive --codex-effort ultra, never --thinking ultra.
#   --backend <name> is the explicit runtime session-provider backend for this
#   exact task only (docs/configuration.md "Runtime backend" owns when that flag
#   is authorized). Without it, the script resolves XO_BACKEND, then
#   config/backend, then runtime auto-detection from the runtime xo's
#   environment: $TMUX, HERDR_ENV=1, or cmux runtime signals (via
#   bin/xo-backend.sh's xo_backend_detect, with cmux fallback details in
...
#   A herdr crewmate or scout is placed in the exact workspace of the xo
#   or secondmate process launching it, resolved from that process's own herdr
#   pane rather than from a workspace label (herdr enforces no label uniqueness,
#   so a label cannot tell two "xo" workspaces apart). A claimed parent
...
#   immediately after its owning parent (xo or 2ndmate-<id>) contiguous
...
#   root XO home's state directory before slot allocation and holds it through
...
#   what lets teardown leave a slot reassigned since untouched; bin/xo-wake-lib.sh
#   owns the claim and bin/xo-teardown.sh owns what it protects. A slot that
...
#   The local root is whatever bin/xo-wake-lib.sh's
#   xo_xo_root_home resolves, so a home seeded from another machine anchors
#   that lock itself rather than failing to resolve one;
#   contention refuses rather than waits.
#   With no harness arg, a crewmate/scout spawn resolves the CREW harness only when
#   config/crew-dispatch.json is absent. When that file exists, crewmate/scout
#   spawns require an explicit harness so xo cannot silently skip dispatch
#   profile consultation. A --secondmate spawn is exempt and resolves the SECONDMATE
#   harness (config/secondmate-harness -> config/crew-harness -> own), so the
#   secondmate-vs-crewmate split is DURABLE across every respawn (recovery,
#   /updatexo, restart). A bare adapter name (claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp)
#   overrides it for this spawn (either kind). A non-flag string containing
#   whitespace is treated as a RAW launch command - the escape hatch for verifying
#   new adapters. For pi and pi-signed, xo-spawn resolves the selected executable
#   name from PATH once, probes that concrete path with --help, and launches the
#   same path. It adds --tui-mode regular only when that help advertises the flag;
#   a failed or inconclusive probe omits it so older Pi versions remain launchable.
#   A missing selected executable refuses before endpoint creation, and pi-signed
#   never falls back to pi.
#   For omp (Oh My Pi), xo-spawn resolves the `omp` executable from PATH once and
#   refuses when it is absent. Every omp launch clears the foreign harness
#   markers (omp publishes none of its own), sets the XO-owned
#   XO_OMP_HARNESS=omp detection marker, suppresses the first-run provider
...
#   .omp/xo-worker-overlay.yml through --config. That overlay pins composer
...
#   (xo-config-inherit-lib.sh). A successful launch clears pending inherited
#   config reread generations because the new agent reads the converged files.
#   --scout records kind=scout in the task's meta (report deliverable, scratch worktree;
#   see AGENTS.md task lifecycle); --secondmate records kind=secondmate and launches in a
#   provisioned xo home; the default is kind=ship.
...
#   also receives `export XO_TASK_ID=<task-id>` before the launch command, on
#   the same channel as GOTMPDIR, and bin/xo-test-run.sh refuses to execute the
...
#     xo-spawn.sh fix-a-k3=projects/foo add-b-q7=projects/bar [--scout]
...
#   ZELLIJ ZELLIJ_SESSION_NAME ZELLIJ_PANE_ID XO_ZELLIJ_SESSION, plus the task
#   marker XO_TASK_ID that ship and scout panes receive above.
#   An enabled task trace also retains TRACEPARENT. Explicit XO launch
...
#   restart, and it is inherited into secondmate homes (bin/xo-config-inherit-lib.sh).
...
#     __PITURNEND__ absolute path to .pi/extensions/xo-primary-turnend-guard.ts in a pi secondmate home
#     __PIWATCH__   absolute path to .pi/extensions/xo-primary-pi-watch.ts in a pi secondmate home
...
#     __OMPWORKERCFG__ absolute path to the tracked .omp/xo-worker-overlay.yml posture overlay
#     __OPINPUT__   absolute path to the canonical operational-input encoder
#     __WORKTREE__  absolute path to the task worktree
#     __CURSORBIN__ resolved, cursor-verified executable for a cursor launch
#     __GEMINISETTINGS__ xo-owned per-task gemini settings file (busy-state hooks)
#     __ROVOBIN__   resolved, rovo-verified executable for a rovo launch
# Verified per-harness turn-end hooks are installed automatically where enabled; some live outside the worktree.
# Kimi uses one surgically installed XO region in $HOME/.kimi-code/config.toml,
# an xo-owned global hook and registry, and a gitignored per-task pointer.
# grok uses an xo-owned global hook under ${GROK_HOME:-$HOME/.grok}/hooks
# plus a gitignored .xo-grok-turnend worktree pointer and a state token.
...
# the launching user's own Claude trust store through bin/xo-claude-trust.sh,
# because Claude's interactive workspace-trust dialog gates a fresh worktree and
# xo cannot answer it. That helper's header owns the structural scope test
...
# step, not two: bin/xo-backlog-transition-lib.sh owns that invariant, and this
...
# docs/configuration.md and bin/xo-trace-context-lib.sh), the meta also records
...
#   pane export happens on the remote host (bin/xo-remote-secondmate-control.sh).
...
resolve_directory_input() {
  ...
  raw_bytes=$(xo_backlog_bytes_of_string "$path") || return 1
...
XO_HOME=$(resolve_directory_input XO_HOME "$XO_HOME") || exit 1
if [ -n "${XO_STATE_OVERRIDE:-}" ]; then
  XO_STATE_OVERRIDE=$(resolve_directory_input XO_STATE_OVERRIDE "$XO_STATE_OVERRIDE") || exit 1
fi
if [ -n "${XO_DATA_OVERRIDE:-}" ]; then
  XO_DATA_OVERRIDE=$(resolve_directory_input XO_DATA_OVERRIDE "$XO_DATA_OVERRIDE") || exit 1
fi
STATE="${XO_STATE_OVERRIDE:-$XO_HOME/state}"
DATA="${XO_DATA_OVERRIDE:-$XO_HOME/data}"
PROJECTS="${XO_PROJECTS_OVERRIDE:-$XO_HOME/projects}"
CONFIG="${XO_CONFIG_OVERRIDE:-$XO_HOME/config}"
# shellcheck source=bin/xo-config-inherit-lib.sh
. "$SCRIPT_DIR/xo-config-inherit-lib.sh"
if ! LAUNCH_ENV_ENABLED=$(xo_config_source_present "$CONFIG/launch-env-allowlist"); then
  exit 1
fi
LAUNCH_ENV_NAMES=
...
if ! CLAUDE_PERM_PRESENT=$(xo_config_source_present "$CONFIG/claude-permission-mode"); then
  exit 1
fi
...
SUB_HOME_MARKER=".xo-secondmate-home"
if [ -e "$STATE" ] || [ -L "$STATE" ]; then
  xo_backlog_directory_present "$STATE" "state directory" || {
    echo "error: spawn refused: $XO_BACKLOG_TRANSITION_ERROR" >&2
    exit 1
  }
fi
# shellcheck source=bin/xo-ff-lib.sh
. "$SCRIPT_DIR/xo-ff-lib.sh"
...
# shellcheck source=bin/xo-secondmate-nudge-lib.sh
. "$SCRIPT_DIR/xo-secondmate-nudge-lib.sh"
# shellcheck source=bin/xo-backend.sh
. "$SCRIPT_DIR/xo-backend.sh"
# shellcheck source=bin/xo-control-lib.sh
. "$SCRIPT_DIR/xo-control-lib.sh"
# shellcheck source=bin/xo-gate-refuse-lib.sh
. "$SCRIPT_DIR/xo-gate-refuse-lib.sh"
# shellcheck source=bin/xo-busy-lib.sh
. "$SCRIPT_DIR/xo-busy-lib.sh"
# shellcheck source=bin/xo-cursor-lib.sh
. "$SCRIPT_DIR/xo-cursor-lib.sh"
# shellcheck source=bin/xo-pr-lib.sh
. "$SCRIPT_DIR/xo-pr-lib.sh"
# shellcheck source=bin/xo-dod-lib.sh
. "$SCRIPT_DIR/xo-dod-lib.sh"
# shellcheck source=bin/xo-trace-context-lib.sh
. "$SCRIPT_DIR/xo-trace-context-lib.sh"
# shellcheck source=bin/xo-remote-readiness-lib.sh
. "$SCRIPT_DIR/xo-remote-readiness-lib.sh"
# Fail closed before any fleet mutation: a no-mistakes gate agent must never spawn
# a direct report (see bin/xo-gate-refuse-lib.sh).
xo_refuse_if_gate_agent
# Skip the watcher guard when re-exec'd for one pair of a batch (XO_SPAWN_NO_GUARD is
# set by the batch loop below), so the guard runs once for the batch, not once per pair.
[ -n "${XO_SPAWN_NO_GUARD:-}" ] || "$XO_ROOT/bin/xo-guard.sh" || true
KIND=ship
KIND_SET=0
HARNESS_ARG=
MODEL=
...
if [ "$TRACEPARENT_SET" -eq 1 ]; then
  ...
  xo_trace_context_valid "$TRACEPARENT_ARG" || {
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # xo's per-task decision, so they are required and closed-set validated
...
spawn_remote_secondmate() {
  ...
  local -a launch_args
  id=${POS[0]:-}
  xo_task_id_creation_valid "$id" || { echo "error: invalid task id" >&2; return 2; }
  mkdir -p "$STATE" || { echo "error: could not create parent state directory" >&2; return 1; }
  SPAWN_TASK_LOCK="$STATE/.spawn-$id.lock"
  if ! xo_lock_try_acquire "$SPAWN_TASK_LOCK"; then
    echo "error: another spawn is already creating task $id" >&2
    return 1
  fi
  registry_lock=$(secondmate_registry_lock_path "$STATE")
  if ! xo_lock_acquire_wait "$registry_lock"; then
    xo_lock_release "$SPAWN_TASK_LOCK" || true
  ...
  if [ "$remote" != 1 ]; then
    xo_lock_release "$registry_lock" || true
    xo_lock_release "$SPAWN_TASK_LOCK" || true
    return 3
  fi
  ...
  if [ -n "$HARNESS_ARG" ]; then
    ...
    harness=$("$XO_ROOT/bin/xo-harness.sh" secondmate)
  fi
  case "$harness" in
  ...
  if [ -z "$HARNESS_ARG" ] && [ -z "$positional" ]; then
    if [ "$MODEL_SET" -eq 0 ]; then
      model=$("$SCRIPT_DIR/xo-harness.sh" secondmate-model)
      [ -n "$model" ] || model=-
    fi
    if [ "$EFFORT_SET" -eq 0 ]; then
      effort=$("$SCRIPT_DIR/xo-harness.sh" secondmate-effort)
      [ -n "$effort" ] || effort=-
    fi
  fi
  ...
  # supervises it. bin/xo-remote-doctor.sh gates that host on the same
  ...
  if [ "$effort" = ultra ] && ! "$SCRIPT_DIR/xo-harness.sh" validate-native-effort "$harness" "$model" "$effort"; then
    xo_lock_release "$registry_lock" || true
    xo_lock_release "$SPAWN_TASK_LOCK" || true
    return 1
  fi
  ...
  if [ -e "$meta" ] || [ -L "$meta" ]; then
    if ! xo_backlog_record_present "$meta" "task record" "$STATE" \
      || [ "$(xo_meta_get "$meta" kind)" != secondmate ] \
      || [ "$(xo_meta_get "$meta" remote_host)" != "$host" ] \
      || [ "$(xo_meta_get "$meta" remote_root)" != "$root" ] \
      || [ "$(xo_meta_get "$meta" home)" != "$home" ]; then
      xo_lock_release "$registry_lock" || true
      xo_lock_release "$SPAWN_TASK_LOCK" || true
  ...
  rc=0
  xo_remote_readiness_ensure "$SCRIPT_DIR" "$id" || rc=$?
  if [ "$rc" -ne 0 ]; then
    xo_lock_release "$registry_lock" || true
    xo_lock_release "$SPAWN_TASK_LOCK" || true
    ...
    [ -z "$XO_REMOTE_READINESS_OUT" ] || printf '%s\n' "$XO_REMOTE_READINESS_OUT" >&2
    [ "$rc" -ne 255 ] || return 255
    return 1
  fi
  # Pre-launch sync, the remote twin of the local-HEAD sync below: this home
  # follows THIS primary's default-branch commit, not the XO copy on that
  ...
  if sm_primary_head=$(primary_head_commit "$XO_ROOT"); then
    if sync_out=$("$SCRIPT_DIR/xo-on.sh" "$id" xo-remote-secondmate-control.sh sync "$id" \
  ...
  remote_lock=$(xo_remote_inherit_transaction_lock_path "$STATE" "$id")
  if ! xo_lock_acquire_wait "$remote_lock"; then
    xo_lock_release "$registry_lock" || true
    xo_lock_release "$SPAWN_TASK_LOCK" || true
    echo "error: remote secondmate $id inheritance transaction could not be locked" >&2
    return 1
  fi
  remote_generation=$(xo_remote_inherit_generation_next "$STATE" "$id" 2>/dev/null || true)
  if [ -z "$remote_generation" ]; then
    xo_lock_release "$remote_lock" || true
    xo_lock_release "$registry_lock" || true
    xo_lock_release "$SPAWN_TASK_LOCK" || true
    echo "error: remote secondmate $id inheritance generation could not be published" >&2
    return 1
  fi
  if "$SCRIPT_DIR/xo-remote-inherit-push.sh" "$id" "$remote_generation" >/dev/null; then
    :
  else
    rc=$?
    xo_lock_release "$remote_lock" || true
  ...
  remote_traceparent=
  if [ "$(xo_trace_context_session_effective "$STATE/.trace-context-effective")" = on ]; then
    remote_traceparent=$(XO_TRACE_CONTEXT=on xo_trace_context_resolve "$CONFIG" "$meta" || true)
  fi
  launch_args=("$id" "$harness" "$model" "$effort" "$backend")
  [ -z "$remote_traceparent" ] || launch_args+=("$remote_traceparent")
  if out=$("$SCRIPT_DIR/xo-on.sh" "$id" xo-remote-secondmate-control.sh launch \
  ...
  if [ "$remote_herdr_session" != xo-remote ] || [ "${remote_target%%:*}" != "$remote_herdr_session" ]; then
    xo_lock_release "$remote_lock" || true
    xo_lock_release "$registry_lock" || true
    xo_lock_release "$SPAWN_TASK_LOCK" || true
    echo "error: remote launch returned Herdr session '${remote_herdr_session:-missing}', expected 'xo-remote'; preserving the remote route for reconciliation" >&2
    return 1
  fi
  ...
  xo_trace_context_valid "$remote_recorded_traceparent" || remote_recorded_traceparent=
  tmp="$meta.tmp.$$"
  {
  ...
  if ! xo_backlog_atomic_transition publish "$tmp" "$meta" "task record" "$STATE"; then
    if [ "$SPAWN_TASK_SET_LOCK_HELD" = 1 ]; then
      ...
      xo_lock_release "$SPAWN_TASK_SET_LOCK" || true
    fi
    xo_lock_release "$remote_lock" || true
    xo_lock_release "$registry_lock" || true
    xo_lock_release "$SPAWN_TASK_LOCK" || true
    echo "error: remote secondmate $id launched, but its task record could not be published ($XO_BACKLOG_TRANSITION_ERROR)" >&2
    return 1
  fi
  if [ "$SPAWN_TASK_SET_LOCK_HELD" = 1 ]; then
    SPAWN_TASK_SET_LOCK_HELD=0
    xo_lock_release "$SPAWN_TASK_SET_LOCK"
  fi
  xo_lock_release "$remote_lock" || true
  xo_lock_release "$registry_lock" || true
  xo_lock_release "$SPAWN_TASK_LOCK" || true
  "$SCRIPT_DIR/xo-home-summary-refresh.sh" --best-effort || true
  if ! "$SCRIPT_DIR/xo-procevent-remote-reply.sh" arm "$id" >/dev/null; then
...
spawn_fresh_commit_rollback() {
  if xo_backlog_atomic_transition rollback "$STATE/$ID.meta" \
      "$XO_ROOT/bin/xo-busy-event.sh" "$STATE" "$ID" "${BUSY_GEN:-}"; then
    SPAWN_FRESH_COMMIT_PENDING=0
    return 0
  fi
  echo "error: $XO_BACKLOG_TRANSITION_ERROR" >&2
  return 1
}
...
spawn_abort_cleanup() {
  ...
  if [ "$RELAUNCH_REPLACEMENT_PENDING" = 1 ]; then
    ...
    if [ -n "$RELAUNCH_REPLACEMENT_BUSY_GEN" ]; then
      if ! "$XO_ROOT/bin/xo-busy-event.sh" retire \
  ...
  if [ "$HERDR_PROJECTION_ABORT_CLEANUP" = 1 ]; then
    HERDR_PROJECTION_ABORT_CLEANUP=0
    xo_backend_herdr_projection_cleanup_exact \
      "$HERDR_PROJECTION_ABORT_SESSION" \
      "$HERDR_PROJECTION_ABORT_TASK_PANE" \
  ...
  if [ "$HERDR_PRESENTATION_ORDER_LOCK_HELD" = 1 ]; then
    HERDR_PRESENTATION_ORDER_LOCK_HELD=0
    xo_lock_release "$HERDR_PRESENTATION_ORDER_LOCK" || true
  fi
  if [ "$ORCA_ABORT_CLEANUP" = 1 ]; then
    ORCA_ABORT_CLEANUP=0
    if [ -n "${ORCA_TERMINAL:-}" ]; then
      xo_backend_kill orca "$ORCA_TERMINAL" 2>/dev/null || true
    fi
    if [ -n "${ORCA_WORKTREE_ID:-}" ]; then
      if ! xo_backend_remove_worktree orca "$ORCA_WORKTREE_ID" 2>/dev/null; then
        ...
        if [ -d "$STATE" ]; then
          SPAWN_META_TMP="$STATE/.$ID.meta.orca-recovery.${BASHPID:-$$}"
          {
            ...
            && xo_backlog_atomic_transition publish "$SPAWN_META_TMP" "$STATE/$ID.meta" "task record" "$STATE" \
            || true
        fi
      fi
    fi
  ...
  if [ "$SPAWN_META_LOCK_HELD" = 1 ]; then
    SPAWN_META_LOCK_HELD=0
    xo_lock_release "$SPAWN_META_LOCK" || true
  fi
  ...
  if [ "$SPAWN_SLOT_CLAIMED" = 1 ] && [ -n "${WT:-}" ] \
     ...
     && xo_treehouse_pool_slot "$PROJ_ABS" "$WT"; then
    SPAWN_SLOT_CLAIMED=0
    if [ "$SPAWN_TREEHOUSE_PROJECT_LOCK_HELD" = 1 ]; then
      xo_treehouse_slot_owner_release "$WT" "$ID" || true
    else
  ...
  if [ "$SPAWN_TREEHOUSE_PROJECT_LOCK_HELD" = 1 ]; then
    SPAWN_TREEHOUSE_PROJECT_LOCK_HELD=0
    xo_lock_release "$SPAWN_TREEHOUSE_PROJECT_LOCK" || true
  fi
  if [ "$SPAWN_TASK_SET_LOCK_HELD" = 1 ]; then
    SPAWN_TASK_SET_LOCK_HELD=0
    xo_lock_release "$SPAWN_TASK_SET_LOCK" || true
  ...
  if [ "$SPAWN_CONTROL_LOCK_HELD" = 1 ]; then
    SPAWN_CONTROL_LOCK_HELD=0
    xo_lock_release "$SPAWN_CONTROL_LOCK" || true
  fi
  [ -z "$SPAWN_META_TMP" ] || rm -f "$SPAWN_META_TMP" 2>/dev/null || true
  if [ "$CONFIG_INHERIT_LOCK_HELD" = 1 ]; then
    CONFIG_INHERIT_LOCK_HELD=0
    xo_lock_release "$CONFIG_INHERIT_LOCK" || true
  fi
  return "$status"
}
...
spawn_herdr_presentation_order_lock_acquire() {
  ...
  lock_path=$(xo_backend_herdr_presentation_session_lock_path "$session") || return 1
  HERDR_PRESENTATION_ORDER_LOCK="$lock_path"
  attempt=0
  while [ "$attempt" -lt 50 ]; do
    if xo_lock_try_acquire "$HERDR_PRESENTATION_ORDER_LOCK"; then
...
clear_relaunch_harness_wiring() {
  ...
  harness=$(xo_control_harness_family "$harness") || harness=
  token_path=$(xo_control_harness_turnend_token_path "$harness" "$state" "$id") || return 1
  token=
  if [ -n "$token_path" ] && [ -f "$token_path" ]; then
    IFS= read -r token < "$token_path" || [ -n "$token" ] || return 1
  fi
  auth_path=$(xo_control_harness_turnend_auth_path "$harness" "$token") || return 1
  if [ -n "$auth_path" ]; then
...
$(xo_control_harness_wiring_paths "$harness" "$wt" "$state" "$id")
EOF
}
...
# the XO_ROOT path (not $0) so it works whatever cwd or relative path invoked us, and reuse
...
if [ "${#POS[@]}" -gt 0 ] && [ "${POS[0]}" != "$idpart" ] && case "$idpart" in */*) false ;; *) true ;; esac; then
  ...
  for pair in "${POS[@]}"; do
    ...
    if [ "$KIND" = secondmate ]; then
      ...
      if XO_SPAWN_NO_GUARD=1 "$XO_ROOT/bin/xo-spawn.sh" "${pair%%=*}" "${pair#*=}" "${shared_args[@]+"${shared_args[@]}"}" --scout; then :; else echo "batch: FAILED to spawn ${pair%%=*} (${pair#*=})" >&2; rc=1; fi
    else
      if XO_SPAWN_NO_GUARD=1 "$XO_ROOT/bin/xo-spawn.sh" "${pair%%=*}" "${pair#*=}" "${shared_args[@]+"${shared_args[@]}"}"; then :; else echo "batch: FAILED to spawn ${pair%%=*} (${pair#*=})" >&2; rc=1; fi
    fi
  done
  exit "$rc"
fi
...
# task is legitimate branch recovery (xo-control drives it through this same
# entrypoint), so only a fresh spawn refuses the branch actor (contract:
# bin/xo-lease-lib.sh; no-op in homes without a branch actor).
# shellcheck source=bin/xo-lease-lib.sh
. "$SCRIPT_DIR/xo-lease-lib.sh"
if [ "$RELAUNCH" -ne 1 ]; then
  xo_lease_forbid_branch "new-task spawn (xo-spawn)"
fi
if [ "$RELAUNCH" -eq 1 ]; then
  SPAWN_CONTROL_LOCK="$STATE/.control-$ID.lock"
  control_owner=$(cat "$SPAWN_CONTROL_LOCK/pid" 2>/dev/null || true)
  if [ "$control_owner" = "$PPID" ] && xo_pid_alive "$control_owner"; then
    SPAWN_CONTROL_PARENT=1
  elif [ "$(xo_lease_actor)" = branch ]; then
    # Role partition refinement: branch recovery relaunches only through the
    # xo-control transaction that owns the control lock, never by invoking
    # this entrypoint directly (contract: bin/xo-lease-lib.sh).
    echo "error: relaunch (xo-spawn) refused - the supervision branch must relaunch through xo-control" >&2
    exit "$XO_LEASE_REFUSE_EXIT"
  elif xo_lock_try_acquire "$SPAWN_CONTROL_LOCK"; then
...
if [ "$RELAUNCH" -eq 0 ]; then
  ...
  # while never lifecycle-locked (bin/xo-wake-lib.sh's xo_task_set_lock_path
  # owns the evidence; bin/xo-teardown.sh holds the same lock from enumeration
  ...
  SPAWN_TASK_SET_LOCK=$(xo_task_set_lock_path "$STATE") || {
    echo "error: could not resolve the task-set lock for $STATE" >&2
    exit 1
  }
  if ! xo_lock_try_acquire "$SPAWN_TASK_SET_LOCK"; then
...
# Backend selection (data/xo-backend-design-d7): explicit --backend, else
# XO_BACKEND env, else config/backend, else runtime auto-detection, else
# default tmux (xo_backend_name). xo_backend_validate_spawn refuses unknown or
# non-spawn-capable backends. The resolved value is
# recorded in meta only when it is NOT tmux (xo-teardown.sh and xo-watch.sh's
# window_backend/xo_backend_of_meta already treat an absent backend= as tmux),
# so the default path's meta stays byte-identical.
if [ "$RELAUNCH" -eq 0 ]; then
  if [ "$BACKEND_SET" -eq 1 ]; then
    BACKEND=$BACKEND_ARG
  else
    BACKEND=$(xo_backend_name)
  fi
  xo_backend_validate_spawn "$BACKEND" || exit 1
  xo_backend_source "$BACKEND" || exit 1
  ...
  if [ "$BACKEND" = orca ]; then
    xo_backend_orca_runtime_check || exit 1
  fi
fi
SPAWN_TASK_LOCK="$STATE/.spawn-$ID.lock"
if ! xo_lock_try_acquire "$SPAWN_TASK_LOCK"; then
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  xo_backlog_record_present "$RELAUNCH_META" "task record" "$STATE" || {
    echo "error: --relaunch refused: $XO_BACKLOG_TRANSITION_ERROR" >&2
    exit 1
  }
  SPAWN_META_LOCK=$(xo_meta_lock_path "$RELAUNCH_META") || exit 1
  xo_lock_acquire_wait "$SPAWN_META_LOCK"
  SPAWN_META_LOCK_HELD=1
  xo_backlog_record_present "$RELAUNCH_META" "task record" "$STATE" || {
    echo "error: --relaunch refused after locking: $XO_BACKLOG_TRANSITION_ERROR" >&2
    exit 1
  }
  xo_backend_validate_task_endpoint "$RELAUNCH_META" "$ID" || exit 1
  BACKEND=$XO_BACKEND_VALIDATED_BACKEND
  RELAUNCH_TARGET=$XO_BACKEND_VALIDATED_TARGET
  xo_backend_validate_spawn "$BACKEND" || exit 1
  xo_backend_source "$BACKEND" || exit 1
  # A relaunch must PROVE the previous agent is gone before it launches another
  # one into the same endpoint, and only tmux and herdr have a recovery-grade
  # classifier that can (bin/xo-control-lib.sh owns that capability table).
  xo_control_backend_state_verified "$BACKEND" || {
    echo "error: backend '$BACKEND' has no recovery-grade agent-state classifier, so a relaunch cannot prove the previous agent exited; refusing rather than risking two agents in one endpoint" >&2
    exit 1
  }
  RELAUNCH_STATE=$(xo_backend_agent_state "$BACKEND" "$RELAUNCH_TARGET")
  [ "$RELAUNCH_STATE" = dead ] || {
    echo "error: task $ID's endpoint reads '$RELAUNCH_STATE'; a relaunch requires a positively agent-free endpoint (stop the agent first with bin/xo-control.sh $ID exit)" >&2
    exit 1
  }
  RELAUNCH_PRIOR_HARNESS=$(xo_meta_get "$RELAUNCH_META" harness)
  KIND=$(xo_meta_get "$RELAUNCH_META" kind)
  [ -n "$KIND" ] || KIND=ship
  MODE=$(xo_meta_get "$RELAUNCH_META" mode)
  YOLO=$(xo_meta_get "$RELAUNCH_META" yolo)
  RELAUNCH_WT=$(xo_meta_get "$RELAUNCH_META" worktree)
  [ -n "$RELAUNCH_WT" ] && [ -d "$RELAUNCH_WT" ] || {
    echo "error: task $ID's recorded worktree '${RELAUNCH_WT:-none}' is missing; refusing to relaunch without the local copy its work lives in" >&2
    exit 1
  }
  if [ "$KIND" = secondmate ]; then
    SECONDMATE_HOME=$(xo_meta_get "$RELAUNCH_META" home)
    [ -n "$SECONDMATE_HOME" ] || SECONDMATE_HOME=$RELAUNCH_WT
  else
    PROJ=$(xo_meta_get "$RELAUNCH_META" project)
    [ -n "$PROJ" ] || {
  ...
  if [ "$BACKEND" = herdr ]; then
    HERDR_SES=$(xo_meta_get "$RELAUNCH_META" herdr_session)
    HERDR_WORKSPACE_ID=$(xo_meta_get "$RELAUNCH_META" herdr_workspace_id)
    HERDR_TAB_ID=$(xo_meta_get "$RELAUNCH_META" herdr_tab_id)
    HERDR_PANE_ID=$(xo_meta_get "$RELAUNCH_META" herdr_pane_id)
  fi
  ...
  # the caller's explicit decision, made with --harness (bin/xo-control.sh
...
launch_template() {
  ...
  case "$harness" in
    ...
    # xo captures the pane (see the harness-adapters skill). It is a per-launch env
    # prefix scoped to this xo-launched agent; it never touches the captain's
    # global config. The CLI's --prompt-suggestions flag is print/SDK-mode only and
    # does NOT suppress the interactive ghost text (verified empirically), so the env
    # var is the correct control. The dim-aware composer reader in xo-tmux-lib.sh is
    ...
    # XO_OMP_HARNESS=omp established for bin/xo-harness.sh, OMP_SKIP_SETUP=1
    ...
    omp)
      printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u XO_PI_HARNESS -u GEMINI_CLI -u CURSOR_AGENT -u CURSOR_INVOKED_AS XO_OMP_HARNESS=omp OMP_SKIP_SETUP=1 __OMPBIN__ --config __OMPWORKERCFG__ --auto-approve --cwd __WORKTREE__'
      if [ "$kind" = secondmate ]; then
    ...
    # SECOND worktree under ~/.cursor/worktrees and would break xo's
    ...
    cursor) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u XO_PI_HARNESS -u GEMINI_CLI -u CURSOR_INVOKED_AS __CURSORBIN__ --trust --yolo __MODELFLAG__--workspace __WORKTREE__ "$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
    ...
    # discovered. An xo-repo task needs exactly those, so the workspace
    # is trusted.
    # GEMINI_CLI_SYSTEM_SETTINGS_PATH points gemini at the xo-owned
    ...
    # project's own hooks still run alongside xo's.
    ...
    # bin/xo-harness.sh must not read a gemini worker as its launcher.
    ...
    gemini) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u XO_PI_HARNESS GEMINI_CLI_TRUST_WORKSPACE=true GEMINI_CLI_SYSTEM_SETTINGS_PATH=__GEMINISETTINGS__ gemini -y __MODELFLAG__"$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
    ...
    # plugin engine is off in the default build, so xo folds muse's own
    # session event log instead (bin/xo-busy-lib.sh), bound by the sidecar
    # written below. Nothing to place in the template for it.
    # codex, opencode, and kimi are also markerless and share this inherited-marker hazard; changing their verified launch boundaries belongs in follow-up work.
    muse) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u XO_PI_HARNESS XDG_CONFIG_HOME=__MUSECONFIG__ XDG_DATA_HOME=__MUSEDATA__ MUSE_EXPERIMENTAL_FOREIGN_PERSONAL_CONTEXT_KILL=on __MUSEBIN__ --yolo __MODELFLAG____EFFORTFLAG__"$(__OPINPUT__ encode launch-brief < __BRIEF__)"' ;;
    ...
    # defense in depth alongside the marker-ordering fix in bin/xo-harness.sh
    ...
    rovo) printf '%s' 'env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u XO_PI_HARNESS __ROVOBIN__ run --yolo __MODELFLAG____ROVOCONFIGOVERRIDE__' ;;
    *) return 1 ;;
  esac
}
...
case "$ARG3" in
  *' '*)  # raw launch command (unverified-adapter escape hatch)
  ...
  '')
    ...
    # respawn (recovery, /updatexo, restart) re-resolves, so
    ...
    if [ "$KIND" = secondmate ]; then
      ...
      HARNESS=$("$XO_ROOT/bin/xo-harness.sh" crew)
...
# an xo instance, so it needs a primary supervision protocol.
...
# asyncRewake handlers that xo's primary turn-end supervision is built on
...
# verified primary integration, so a secondmate (an xo instance that must
...
case "$HARNESS" in
  pi|pi-signed)
    ...
    LAUNCH="XO_PI_HARNESS=$HARNESS $LAUNCH"
    ;;
  cursor)
    ...
    CURSOR_BIN=$(xo_cursor_resolve_binary) || exit 1
    if [ -n "$MODEL" ] && [ "$MODEL" != default ]; then
      if CURSOR_MODELS=$(xo_cursor_list_models "$CURSOR_BIN"); then
        if ! printf '%s\n' "$CURSOR_MODELS" | xo_cursor_catalog_has_model "$MODEL"; then
  ...
  omp)
    ...
    OMP_WORKER_CFG="$XO_ROOT/.omp/xo-worker-overlay.yml"
    [ -f "$OMP_WORKER_CFG" ] || {
...
if [ "$KIND" = secondmate ] && [ -z "$ARG3" ]; then
  if [ "$MODEL_SET" -eq 0 ]; then
    SM_MODEL=$("$SCRIPT_DIR/xo-harness.sh" secondmate-model)
    [ -z "$SM_MODEL" ] || MODEL=$SM_MODEL
  fi
  if [ "$EFFORT_SET" -eq 0 ]; then
    SM_EFFORT=$("$SCRIPT_DIR/xo-harness.sh" secondmate-effort)
    if [ -n "$SM_EFFORT" ]; then
      case "$SM_EFFORT" in
...
if [ "$EFFORT" = ultra ]; then
  "$SCRIPT_DIR/xo-harness.sh" validate-native-effort "$HARNESS" "$MODEL" "$EFFORT" || exit 1
  [ "$RAW_LAUNCH" = 0 ] || {
...
muse_worker_meta_api_key_present() {
  ...
  if [ -n "${TMUX:-}" ]; then
    ...
    tmux has-session -t xo 2>/dev/null || return 1
    session=xo
  fi
...
effort_flag_for_harness() {
  ...
  case "$harness" in
    ...
    codex)
      ...
      ;;
    grok)
      # grok exposes both --effort and --reasoning-effort; xo's profile
    ...
    pi|pi-signed)
      ...
      case "$effort" in
        ultra)
          "$SCRIPT_DIR/xo-harness.sh" validate-native-effort "$harness" "$model" "$effort" || return 1
    ...
    muse)
      ...
      # ultra is muse's max-CLASS level, so xo's max maps onto it - but
      ...
      # none/minimal levels sit below xo's shared vocabulary and are
    ...
    # to a different, non-interactive launch mode, so xo-spawn does not pass it.
  ...
  *__MUSEBIN__*)
    ...
    if ! muse_credential_present "$MUSE_AUTH_FILE"; then
      if [ -n "${META_API_KEY:-}" ]; then
        echo "error: muse has no worker-reachable credential; META_API_KEY is set for xo-spawn but cannot be proven present in the $BACKEND worker environment. Store the fleet credential at '$MUSE_AUTH_FILE' with 'muse login' or 'muse auth set --api-key-stdin'. The secret will not be copied into the launch command." >&2
      else
...
case "$LAUNCH" in
  *__KIMIBIN__*)
    ...
    if [ "$KIND" != secondmate ]; then
      "$XO_ROOT/bin/xo-kimi-turnend-hook.sh" install || {
...
resolved_existing_dir() {
  local path=$1
  [ -d "$path" ] || { echo "error: xo home does not exist or is not a directory: $path" >&2; return 1; }
  cd "$path" && pwd -P
}
...
validate_xo_home_for_spawn() {
  local id=$1 home=$2 abs_home abs_active_home abs_root marker_id
  abs_home=$(resolved_existing_dir "$home") || return 1
  abs_active_home=$(resolved_existing_dir "$XO_HOME")
  abs_root=$(resolved_existing_dir "$XO_ROOT")
  if [ "$abs_home" = "/" ]; then
    echo "error: secondmate home cannot be the filesystem root: $home" >&2
    return 1
  fi
  if [ "$abs_home" = "$abs_active_home" ]; then
    echo "error: secondmate home cannot be the active xo home: $home" >&2
    return 1
  fi
  if [ "$abs_home" = "$abs_root" ]; then
    echo "error: secondmate home cannot be the xo repo: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_active_home" "$abs_home"; then
    echo "error: secondmate home cannot be inside the active xo home: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_home"; then
    echo "error: secondmate home cannot be inside the xo repo: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_active_home"; then
    echo "error: secondmate home cannot be an ancestor of the active xo home: $home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_home" "$abs_root"; then
    echo "error: secondmate home cannot be an ancestor of the xo repo: $home" >&2
    return 1
  fi
  validate_xo_operational_dirs "$abs_home" "$abs_active_home" "$abs_root" || return 1
  if [ ! -f "$abs_home/$SUB_HOME_MARKER" ]; then
    echo "error: xo home $home is not a seeded secondmate home" >&2
    return 1
  fi
  marker_id=$(cat "$abs_home/$SUB_HOME_MARKER" 2>/dev/null || true)
  if [ "$marker_id" != "$id" ]; then
    echo "error: xo home $home is marked for secondmate ${marker_id:-unknown}, expected $id" >&2
    return 1
  fi
  if [ ! -f "$abs_home/AGENTS.md" ]; then
    echo "error: $home is not an xo home (missing AGENTS.md)" >&2
    return 1
  fi
  if [ ! -d "$abs_home/bin" ]; then
    echo "error: $home is not an xo home (missing bin/)" >&2
    return 1
  fi
  printf '%s\n' "$abs_home"
}

validate_xo_operational_dirs() {
  ...
  for name in data state config projects; do
    ...
    if [ "$abs_dir" = "$abs_active_home" ] || path_is_ancestor_of "$abs_active_home" "$abs_dir"; then
      echo "error: secondmate $name directory cannot be inside the active xo home: $dir" >&2
      return 1
    fi
    if [ "$abs_dir" = "$abs_root" ] || path_is_ancestor_of "$abs_root" "$abs_dir"; then
      echo "error: secondmate $name directory cannot be inside the xo repo: $dir" >&2
      return 1
    fi
  done
}
...
if [ "$KIND" = secondmate ]; then
  if [ -z "$SECONDMATE_HOME" ] && { [ -e "$STATE/$ID.meta" ] || [ -L "$STATE/$ID.meta" ]; }; then
    xo_backlog_record_present "$STATE/$ID.meta" "task record" "$STATE" || {
      echo "error: secondmate task record is unsafe: $XO_BACKLOG_TRANSITION_ERROR" >&2
      exit 1
    }
    SECONDMATE_HOME=$(grep '^home=' "$STATE/$ID.meta" | cut -d= -f2- || true)
  fi
  if [ -z "$SECONDMATE_HOME" ]; then
    SECONDMATE_HOME=$(secondmate_registry_value "$ID" home || true)
  fi
fi

if [ "$KIND" = secondmate ]; then
  [ -n "$SECONDMATE_HOME" ] || { echo "error: no xo home supplied or registered for $ID" >&2; exit 1; }
  PROJ_ABS=$(validate_xo_home_for_spawn "$ID" "$SECONDMATE_HOME")
  if [ -e "$DATA/secondmates.md" ] || [ -L "$DATA/secondmates.md" ]; then
    if ! secondmate_registry_validate_bindings "$DATA/secondmates.md" resolve_path "$ID" "$SECONDMATE_HOME"; then
  ...
  # already synced the home to ITS primary commit, and $XO_ROOT here is only that
  # host's own XO copy; syncing again would target the wrong checkout, so
  # the caller turns this step off (bin/xo-remote-secondmate-control.sh).
  if [ "${XO_SKIP_SECONDMATE_SYNC:-0}" = 1 ]; then
    :
  elif sm_primary_head=$(primary_head_commit "$XO_ROOT"); then
  ...
  if [ "${XO_SKIP_SECONDMATE_INHERIT:-0}" != 1 ]; then
    CONFIG_INHERIT_LOCK=$(xo_config_inherit_lock_path "$PROJ_ABS") || {
      echo "error: could not resolve secondmate inheritance lock for $PROJ_ABS" >&2
      exit 1
    }
    if ! xo_lock_acquire_wait "$CONFIG_INHERIT_LOCK"; then
    ...
    # surface into this secondmate home (xo-config-inherit-lib.sh).
    XO_CONFIG_INHERIT_LIVE=1 \
      propagate_secondmate_inheritance "$XO_HOME" "$PROJ_ABS" "$CONFIG" "$DATA" \
...
if [ "$RELAUNCH" -eq 0 ] && [ "$KIND" != secondmate ] && [ "$BACKEND" != orca ]; then
  SPAWN_TREEHOUSE_PROJECT_LOCK=$(xo_treehouse_project_lock_path "$PROJ_ABS") || {
    echo "error: could not resolve the shared Treehouse project lock for $PROJ_ABS" >&2
    exit 1
  }
  if ! xo_lock_try_acquire "$SPAWN_TREEHOUSE_PROJECT_LOCK"; then
...
if [ "$KIND" != secondmate ] && [ -f "$DATA/$ID/plane.json" ]; then
  "${XO_PLANE_PYTHON:-python3}" "$SCRIPT_DIR/xo-plane.py" check --task "$ID" >/dev/null || {
    echo "error: Plane execution claim is not valid for this task; reconcile before dispatch" >&2
    exit 1
  }
fi
if [ "$KIND" = ship ] || [ "$KIND" = scout ]; then
  if xo_brief_task_placeholders_present "$BRIEF"; then
    echo "error: $BRIEF still contains {TASK} or {XO_SPEC}; fill ## Captain's intent and ## XO spec before spawn" >&2
    exit 1
  fi
  if ! xo_brief_task_content_valid "$BRIEF"; then
    echo "error: $BRIEF must contain nonempty ## Captain's intent and ## XO spec subsections (or a nonempty legacy # Task body) before spawn" >&2
    exit 1
  fi
  if [ "$KIND" = ship ] && [ "$MODE" = no-mistakes ]; then
    if xo_brief_task_heading_present "$BRIEF" "## Captain's intent"; then
      CAPTAIN_INTENT=$(xo_brief_task_heading_body "$BRIEF" "## Captain's intent")
    else
      LEGACY_TASK_BODY=$(xo_brief_heading_body "$BRIEF" "# Task")
      CAPTAIN_INTENT=$(xo_brief_marked_captain_words "$LEGACY_TASK_BODY")
      if [ -z "$(printf '%s' "$CAPTAIN_INTENT" | tr -d '[:space:]')" ]; then
        echo "error: legacy mixed # Task brief has no provenance-marked captain words for no-mistakes --intent; add Captain: lines or migrate to ## Captain's intent and ## XO spec" >&2
        exit 1
      fi
    fi
  fi
  ...
  BRIEF_TMP="$DATA/$ID/.launch-brief.md.${BASHPID:-$$}"
  {
    cat "$SOURCE_BRIEF" &&
      ...
      if [ "$KIND" = ship ] && [ "$MODE" = no-mistakes ]; then
        xo_brief_intent_overlay "$CAPTAIN_INTENT"
      fi
...
# xo-brief.sh records a ship brief's mode as a fixed "Delivery contract: mode=<mode>"
...
if [ "$KIND" = ship ]; then
  ...
  STANDING_MODE=$("$XO_ROOT/bin/xo-project-mode.sh" --raw "$PROJ_NAME" 2>/dev/null | cut -d' ' -f1) || STANDING_MODE=
...
# per-backend routing (xo_backend_resolve_selector).
...
spawn_worktree_isolated() {  # <path>
  ...
  # it would resolve to xo-spawn's OWN cwd and report the path as a subdirectory
  # of whatever checkout xo happens to be running from.
  wt_top_real=
...
herdr_projection_existing_meta_allows_flat() {  # <meta>
  ...
  old_backend=$(xo_backend_of_meta "$meta")
  old_target=$(xo_backend_target_of_meta "$meta")
  [ -n "$old_target" ] || {
  ...
  if [ "$old_backend" = herdr ]; then
    xo_backend_herdr_parse_target "$old_target" || {
      echo "error: existing herdr endpoint for $ID is malformed; refusing duplicate launch" >&2
      return 1
    }
    target_session=$XO_BACKEND_HERDR_SESSION
    target_pane=$XO_BACKEND_HERDR_PANE
    ...
    xo_backend_herdr_server_ensure "$old_session" || {
      echo "error: existing herdr endpoint for $ID could not be inspected; refusing duplicate launch" >&2
      return 1
    }
    old_state=$(xo_backend_herdr_pane_agent_state "$old_session" "$old_pane")
    case "$old_state" in
  ...
  old_state=$(xo_backend_agent_alive "$old_backend" "$old_target")
  case "$old_state" in
    dead) return 0 ;;
    alive|unknown)
...
# Backlog preflight (bin/xo-backlog-transition-lib.sh). This spawn is about to
...
BACKLOG_ROW_STATE=
if xo_backlog_transition_applies "$CONFIG" "$DATA" "$KIND"; then
  BACKLOG_TRANSITION=1
  if xo_backlog_row_probe "$DATA" "$ID"; then
    BACKLOG_ROW_STATE=$XO_BACKLOG_ROW_STATE
  elif [ "$XO_BACKLOG_ROW_RESULT" = not_found ]; then
    ...
    echo "error: task $ID's backlog item could not be read before dispatch ($XO_BACKLOG_ROW_ERROR)" >&2
    exit 1
  fi
  if ! xo_backlog_row_dispatchable "$BACKLOG_ROW_STATE"; then
  ...
  if [ "$BACKLOG_GATE_STATUS" -eq 2 ]; then
    echo "error: task $ID cannot be dispatched because its backlog data directory is inaccessible: $DATA ($XO_BACKLOG_TRANSITION_ERROR)" >&2
    exit 1
  fi
fi

if [ "$SPAWN_META_LOCK_HELD" != 1 ]; then
  SPAWN_META_LOCK=$(xo_meta_lock_path "$STATE/$ID.meta") || exit 1
  xo_lock_acquire_wait "$SPAWN_META_LOCK"
  SPAWN_META_LOCK_HELD=1
fi
...
case "$BACKEND" in
  tmux)
    SES=$(xo_backend_tmux_container_ensure)
    T="$SES:$W"
    # #134 robustness (tmux): xo_backend_tmux_create_task captures a stable window
    # id and pins the window name (automatic-rename/allow-rename off) so a captain's
    # non-default tmux config cannot rename the window away from xo-<id> once
    # treehouse cd's into the worktree. WT_TARGET carries that stable id for the
    # rename-critical worktree-detection steps below; the persisted window= handle
    # stays $T (the name form), which is safe now that rename is disabled.
    WID=$(xo_backend_tmux_create_task "$SES" "$W" "$PROJ_ABS") || exit 1
    WT_TARGET="$WID"
    ;;
  herdr)
    # xo_backend_herdr_workspace_label resolves the target workspace from
    # XO_HOME. For every KIND except secondmate, this process's own XO_HOME is
    ...
    # XO_HOME - the latter needs no glue at all). A --secondmate spawn is the
    # one case that does: it is the PRIMARY's own xo-spawn.sh process
    # launching a DIFFERENT home (PROJ_ABS, already validated above as the
    # secondmate's home), so XO_HOME here still names the primary. Shadow it
    # to PROJ_ABS for just these two calls (bash restores it automatically
    # after each prefixed simple-command call) so the secondmate's tab lands
    # in the secondmate's own workspace, not the primary's "xo" one.
    #
    ...
    HERDR_LABEL_HOME=$XO_HOME
    HERDR_LAUNCHER_RELATIONSHIP=launcher-home
    if [ "$KIND" = secondmate ]; then
      HERDR_LABEL_HOME=$PROJ_ABS
      HERDR_LAUNCHER_RELATIONSHIP=other-home
    fi
    HERDR_PRESENTATION_JOURNAL=$(xo_backend_herdr_projection_journal_path "$STATE" "$ID")
    HERDR_PROJECTED=0
    if [ "$KIND" != secondmate ] && xo_backend_herdr_presentation_enabled "$CONFIG" "$STATE"; then
      HERDR_SES=$(xo_backend_herdr_session)
      HERDR_PARENT_LABEL=$(XO_HOME="$HERDR_LABEL_HOME" xo_backend_herdr_workspace_label)
      if [ -e "$HERDR_PRESENTATION_JOURNAL" ] || [ -L "$HERDR_PRESENTATION_JOURNAL" ]; then
        xo_backend_herdr_server_ensure "$HERDR_SES" || {
        ...
        xo_backend_herdr_projection_recovery_allows_flat \
          "$HERDR_SES" "$HERDR_PRESENTATION_JOURNAL" "$ID" || exit 1
        if [ "${HERDR_RECOVERY_BACKEND:-}" = herdr ]; then
          set +e
          XO_HOME="$HERDR_LABEL_HOME" xo_backend_herdr_projection_reclaim_task \
          ...
          case "$HERDR_RECLAIM_STATUS" in
            0)
              ...
              HERDR_TAB_ID=$XO_BACKEND_HERDR_PROJECTION_TAB_ID
              HERDR_PANE_ID=$XO_BACKEND_HERDR_PROJECTION_PANE_ID
        ...
        if ! xo_backend_herdr_server_ensure "$HERDR_SES"; then
          echo "warning: herdr presentation could not ensure its session server; using the ordinary flat layout without projection" >&2
        elif [ "${XO_BACKEND_HERDR_PRESENTATION_PREFERENCE:-default}" = default ] \
          && ! xo_backend_herdr_presentation_default_supported "$STATE" "$HERDR_SES"; then
          :
          ...
          set +e
          xo_backend_herdr_launcher_identity "$HERDR_SES"
          HERDR_LAUNCHER_STATUS=$?
          set -e
          case "$HERDR_LAUNCHER_STATUS" in
            0) HERDR_PARENT_WORKSPACE_ID=$XO_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID ;;
            2) HERDR_PARENT_WORKSPACE_ID=$(xo_backend_herdr_projection_parent_workspace_exact \
          ...
          if [ -z "$HERDR_PARENT_WORKSPACE_ID" ]; then
            ...
            HERDR_PROJECTION_ID=$(xo_backend_herdr_projection_journal_create "$STATE" "$ID") || exit 1
            HERDR_PROJECTION_LABEL=$(xo_backend_herdr_projection_workspace_label "$ID" "$HERDR_PROJECTION_ID")
            if ! XO_HOME="$HERDR_LABEL_HOME" xo_backend_herdr_projection_create_task \
              "$PROJ_ABS" "$HERDR_PROJECTION_LABEL" "$W"; then
              if [ "${XO_BACKEND_HERDR_PROJECTION_CLEANUP_SAFE:-0}" = 1 ]; then
                HERDR_PROJECTION_ABORT_CLEANUP=1
                HERDR_PROJECTION_ABORT_SESSION=$XO_BACKEND_HERDR_PROJECTION_SESSION
                HERDR_PROJECTION_ABORT_TASK_PANE=$XO_BACKEND_HERDR_PROJECTION_PANE_ID
                HERDR_PROJECTION_ABORT_SEEDED_PANE=$XO_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID
              fi
              exit 1
            fi
            HERDR_PROJECTED=1
            HERDR_SES=$XO_BACKEND_HERDR_PROJECTION_SESSION
            HERDR_WORKSPACE_ID=$XO_BACKEND_HERDR_PROJECTION_WORKSPACE_ID
            HERDR_SEEDED_DEFAULT_TAB_ID=$XO_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID
            HERDR_TAB_ID=$XO_BACKEND_HERDR_PROJECTION_TAB_ID
            HERDR_PANE_ID=$XO_BACKEND_HERDR_PROJECTION_PANE_ID
            HERDR_PROJECTION_ABORT_CLEANUP=1
            HERDR_PROJECTION_ABORT_SESSION=$HERDR_SES
            HERDR_PROJECTION_ABORT_TASK_PANE=$HERDR_PANE_ID
            HERDR_PROJECTION_ABORT_SEEDED_PANE=$XO_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID
            xo_backend_herdr_projection_order_best_effort \
              "$HERDR_SES" "$HERDR_WORKSPACE_ID" "$HERDR_PARENT_LABEL" "$HERDR_PARENT_WORKSPACE_ID"
            HERDR_HOME_ID=$(xo_backend_herdr_projection_home_identity "$HERDR_LABEL_HOME" 2>/dev/null || true)
            if [ -n "$HERDR_HOME_ID" ] \
    ...
    if [ "$HERDR_PROJECTED" -ne 1 ]; then
      HERDR_CONTAINER_RAW=$(XO_HOME="$HERDR_LABEL_HOME" xo_backend_herdr_container_ensure "$PROJ_ABS" "$HERDR_LAUNCHER_RELATIONSHIP") || exit 1
      # xo_backend_herdr_container_ensure echoes "<session>:<workspace_id>\t<seeded_default_tab_id>"
      ...
      HERDR_TASK_IDS=$(XO_HOME="$HERDR_LABEL_HOME" xo_backend_herdr_create_task "$CONTAINER" "$W" "$PROJ_ABS" "$HERDR_SEEDED_DEFAULT_TAB_ID") || exit 1
...
EOF
    ...
    ;;
  zellij)
    ZELLIJ_SES=$(xo_backend_zellij_container_ensure) || exit 1
    ZELLIJ_TASK_IDS=$(xo_backend_zellij_create_task "$ZELLIJ_SES" "$W" "$PROJ_ABS") || exit 1
...
EOF
    ...
    ;;
  cmux)
    xo_backend_cmux_container_ensure || exit 1
    CMUX_TASK_IDS=$(xo_backend_cmux_create_task "$W" "$PROJ_ABS") || exit 1
...
EOF
  ...
  orca)
    set +e
    ORCA_WT_RAW=$(xo_backend_orca_worktree_create "$PROJ_ABS" "$W")
    ORCA_WT_STATUS=$?
    set -e
    ...
    if [ -z "$ORCA_TERMINAL" ]; then
      ORCA_TERMINAL=$(xo_backend_orca_terminal_create "$ORCA_WORKTREE_ID" "$W") || exit 1
    fi
    T="$ORCA_TERMINAL"
    ;;
esac
...
if [ "$KIND" = secondmate ]; then
  XO_INHERITABLE_CONFIG=trace-context \
...
spawn_send_text_line() {  # <target> <text>
  case "$BACKEND" in
    tmux) xo_backend_tmux_send_text_line "$1" "$2" ;;
    herdr) xo_backend_herdr_send_text_line "$1" "$2" ;;
    zellij) xo_backend_zellij_send_text_line "$1" "$2" "$W" ;;
    orca) xo_backend_orca_send_text_line "$1" "$2" ;;
    cmux) xo_backend_cmux_send_text_line "$1" "$2" "$W" ;;
  esac
}
spawn_current_path() {  # <target>
  case "$BACKEND" in
    tmux) xo_backend_tmux_current_path "$1" ;;
    herdr) xo_backend_herdr_current_path "$1" ;;
    zellij) xo_backend_zellij_current_path "$1" "$W" ;;
    cmux) xo_backend_cmux_current_path "$1" "$W" ;;
  esac
}
spawn_send_literal() {  # <target> <text>
  case "$BACKEND" in
    tmux) xo_backend_tmux_send_literal "$1" "$2" ;;
    herdr) xo_backend_herdr_send_literal "$1" "$2" ;;
    zellij) xo_backend_zellij_send_literal "$1" "$2" "$W" ;;
    orca) xo_backend_orca_send_literal "$1" "$2" ;;
    cmux) xo_backend_cmux_send_literal "$1" "$2" "$W" ;;
  esac
}
spawn_send_key() {  # <target> <key>
  case "$BACKEND" in
    tmux) xo_backend_tmux_send_key "$1" "$2" ;;
    herdr) xo_backend_herdr_send_key "$1" "$2" ;;
    zellij) xo_backend_zellij_send_key "$1" "$2" "$W" ;;
    orca) xo_backend_orca_send_key "$1" "$2" ;;
    cmux) xo_backend_cmux_send_key "$1" "$2" "$W" ;;
  esac
}

kimi_capture() {
  xo_backend_capture "$BACKEND" "$T" 120 "$W" 2>/dev/null || true
}

# Kimi launch-readiness and delivery route their composer-emptiness half
# through the shared classifier (bin/xo-composer-lib.sh via
# xo_backend_composer_state), the same owner every steer and injection guard
...
kimi_composer_is_empty() {
  [ "$(xo_backend_composer_state "$BACKEND" "$T" "$W" 2>/dev/null)" = empty ]
}

kimi_wait_for_ready() {
  local pane i=0 max=${XO_KIMI_READY_POLLS:-60} interval=${XO_KIMI_POLL_INTERVAL:-0.5}
  while [ "$i" -lt "$max" ]; do
    pane=$(kimi_capture)
...
kimi_wait_for_delivery() {
  local pane i=0 max=${XO_KIMI_DELIVERY_POLLS:-40} interval=${XO_KIMI_POLL_INTERVAL:-0.5}
  while [ "$i" -lt "$max" ]; do
    pane=$(kimi_capture)
...
# composer-emptiness half through the shared classifier (xo_backend_composer_state)
...
rovo_wait_for_ready() {
  local pane i=0 max=${XO_ROVO_READY_POLLS:-60} interval=${XO_ROVO_POLL_INTERVAL:-0.5}
  while [ "$i" -lt "$max" ]; do
    pane=$(rovo_capture)
    ...
    # ghost-strip threshold) that bin/xo-composer-lib.sh does not currently strip
...
rovo_wait_for_delivery() {
  local pane i=0 max=${XO_ROVO_DELIVERY_POLLS:-40} interval=${XO_ROVO_POLL_INTERVAL:-0.5}
  while [ "$i" -lt "$max" ]; do
    pane=$(rovo_capture)
...
# orphaned autonomous agent outside task control. Mirrors xo-teardown.sh's own
...
rovo_endpoint_cleanup() {
  ...
  xo_backend_kill "$BACKEND" "$T" "$tab_id" "xo-$ID" 2>/dev/null || true
}
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # active client's window, which would misread xo's OWN pane path as the
  ...
  # `get --lease --lease-holder`, which bin/xo-home-seed.sh uses for secondmate
  ...
  # bin/xo-teardown.sh leave a slot that has since been reassigned untouched, so
  ...
  if xo_treehouse_pool_slot "$PROJ_ABS" "$WT"; then
    if ! xo_treehouse_slot_owner_claim "$WT" "$ID" "$XO_HOME"; then
...
# without it. bin/xo-claude-trust.sh owns the structural scope test and refuses
# any path that is not this project's own isolated worktree; a refusal blocks the
# spawn rather than launching a worker that would wedge on a dialog xo
...
if [ "$KIND" != secondmate ]; then
  case "$HARNESS" in
    claude*)
      if ! "$XO_ROOT/bin/xo-claude-trust.sh" "$WT" "$PROJ_ABS" >/dev/null; then
...
# Per-task temp root: /tmp/xo-<id>/ with Go's build temp nested at gotmp/. Go won't
# create GOTMPDIR, so mkdir before it is used; xo-teardown removes the whole root.
# Nested (not a bare /tmp/xo-<id>/gotmp) so other per-task temp can live alongside
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  # (bin/xo-control-lib.sh owns where those artifacts live).
...
if [ "$KIND" != secondmate ]; then
  # Arm the semantic busy-state contract (bin/xo-busy-lib.sh) for every
  # adapter with a verified semantic source. The launch brief sent below IS a
  # submitted turn, so the seed record is busy/xo-spawn. The minted gen is
  ...
  # xo_busy_kimi_verified opens, so none of the three is armed here. Gemini IS
  # armed: its BeforeAgent / AfterAgent / SessionEnd hooks are a verified
  # open-close pair.
  BUSY_GEN=
  case "$HARNESS" in
    codex*)
      if xo_busy_codex_semantic_source; then
  ...
  case "$HARNESS" in
    claude*|opencode*|pi|pi-signed|omp)
      BUSY_GEN=$("$XO_ROOT/bin/xo-busy-event.sh" arm "$STATE_REAL" "$ID") || {
    ...
    gemini)
      ...
      ;;
    kimi*)
      # Standalone Kimi stays unknown until xo_busy_kimi_verified opens on a
      # live-verified installed version (bin/xo-busy-lib.sh owns the gate and
  ...
  case "$HARNESS" in
    claude*)
      # Semantic busy-state hooks (bin/xo-busy-lib.sh): UserPromptSubmit opens
      ...
      # interrupt: xo-control preserves the adapter-owned state, while the
      # legacy xo-send --key Escape path records idle/xo-interrupt. Stop keeps
      # the turn-ended NOTIFICATION touch for the watcher. Every
      # hook command tolerates a refused event (|| true) so a stale-gen writer
      # can never break Claude's own lifecycle.
      mkdir -p "$WT/.claude"
      busy_cmd_prefix="$(shell_quote "$XO_ROOT/bin/xo-busy-event.sh") apply $(shell_quote "$STATE_REAL") $(shell_quote "$ID")"
...
EOF
    ...
    gemini)
      if [ "$RAW_LAUNCH" -eq 0 ]; then
      # Semantic busy-state hooks (bin/xo-busy-lib.sh): BeforeAgent opens a
      ...
      # These are written into an XO-OWNED settings file under state/,
...
EOF
    ...
    opencode*)
      ...
      cat > "$WT/.opencode/plugins/xo-busy-state.js" <<EOF
// XO semantic busy-state events + turn-end notification; written by
// xo-spawn under the contract owned by bin/xo-busy-lib.sh.
...
const busyEvent = (state, event) =>
  new Promise((resolve) => {
    execFile("$XO_ROOT/bin/xo-busy-event.sh", [
      "apply", "$STATE_REAL", "$ID", state,
      "--gen", "$BUSY_GEN", "--source", "opencode-plugin", "--event", event,
    ], () => resolve());
  });
export const XoBusyState = async () => {
...
EOF
      exclude_path '.opencode/plugins/xo-busy-state.js'
      ;;
    pi|pi-signed)
...
// XO semantic busy-state events + turn-end notification for omp (Oh My
// Pi); written by xo-spawn under the contract owned by bin/xo-busy-lib.sh.
// Semantic state: "agent_start" -> busy when a low-level agent run begins;
...
EOF
      ;;
    codex*)
      # Semantic busy-state source negotiation (bin/xo-busy-lib.sh owns the
      ...
      # xo-launched worker. Codex therefore classifies unknown with
    ...
    grok*)
      ...
      # xo cannot establish at launch without editing grok's own managed
      ...
      # lives OUTSIDE the worktree as a single xo-owned global hook that is a
      # guarded no-op for every non-xo grok session: it fires only when the
      # current workspace holds a .xo-grok-turnend token pointer that matches the
      # xo-owned hook registry. xo then drops that per-task pointer
      ...
      # touches grok's managed config - only xo-owned files.
      GROK_HOOKS_DIR="${GROK_HOME:-$HOME/.grok}/hooks"
      GROK_AUTH_DIR="$GROK_HOOKS_DIR/xo-turn-end.d"
      mkdir -p "$GROK_AUTH_DIR"
      old_umask=$(umask)
      umask 077
      auth_file=$(mktemp "$GROK_AUTH_DIR/xo.XXXXXXXXXXXX")
      umask "$old_umask"
      printf '%s\n' "$TURNEND" > "$auth_file"
      printf '%s\n' "${auth_file##*/}" > "$STATE/$ID.grok-turnend-token"
      sq_grok_auth_dir=$(shell_quote "$GROK_AUTH_DIR")
      cat > "$GROK_HOOKS_DIR/xo-turn-end.sh" <<EOF
#!/usr/bin/env bash
set -u
auth_dir=$sq_grok_auth_dir
workspace=\${GROK_WORKSPACE_ROOT:-}
[ -n "\$workspace" ] || exit 0
p="\$workspace/.xo-grok-turnend"
[ -f "\$p" ] || exit 0
first=
IFS= read -r -n 256 first < "\$p" 2>/dev/null || [ -n "\$first" ] || exit 0
case "\$first" in token=*) token=\${first#token=} ;; *) exit 0 ;; esac
case "\$token" in xo.????????????) : ;; *) exit 0 ;; esac
...
exit 0
EOF
      chmod +x "$GROK_HOOKS_DIR/xo-turn-end.sh"
      hook_command=$(json_escape "bash $(shell_quote "$GROK_HOOKS_DIR/xo-turn-end.sh")")
      printf '{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"%s"}]}]}}\n' "$hook_command" > "$GROK_HOOKS_DIR/xo-turn-end.json"
      printf 'token=%s\n' "${auth_file##*/}" > "$WT/.xo-grok-turnend"
      exclude_path '.xo-grok-turnend'
      ;;
    muse*)
      ...
      # xo reads muse's own durable session event log instead
      # (bin/xo-busy-lib.sh owns the fold). That is a PULL
...
$(xo_busy_muse_matching_logs "$MUSE_SESSIONS_ROOT" "$WT" || true)
EOF
    ...
    cursor*)
      ...
      # there (bin/xo-busy-lib.sh owns the fold). Like muse that is a PULL
      # source with no writer, so nothing is armed and no record is seeded.
      ...
      CURSOR_PROJECTS_ROOT="${CURSOR_PROJECTS_ROOT_OVERRIDE:-$HOME/.cursor/projects}"
      {
        ...
        printf 'workspace_root=%s\n' "$WT"
        if CURSOR_PRIOR_PROJECT=$(xo_busy_cursor_project_dir "$CURSOR_PROJECTS_ROOT" "$WT" 2>/dev/null); then
    ...
    kimi*)
      ...
      # task's token pointer and the token resolves through XO's private
      # registry. The installer above owns the format-preserving config edit and
      # the always-zero, silent hook script.
      KIMI_AUTH_DIR="$HOME/.kimi-code/xo-turn-end.d"
      old_umask=$(umask)
      umask 077
      auth_file=$(mktemp "$KIMI_AUTH_DIR/xo.XXXXXXXXXXXX")
      umask "$old_umask"
      printf '%s\n' "$TURNEND" > "$auth_file"
      printf '%s\n' "${auth_file##*/}" > "$STATE/$ID.kimi-turnend-token"
      printf 'token=%s\n' "${auth_file##*/}" > "$WT/.xo-kimi-turnend"
      exclude_path '.xo-kimi-turnend'
      ;;
  esac
fi

# Delivery posture recorded in meta so xo-teardown's safety check and the
...
# (xo-teardown.sh defaults an absent mode to no-mistakes, and xo-promote.sh
...
# Resolve the optional default-off W3C trace context (bin/xo-trace-context-lib.sh,
...
if [ "$TRACEPARENT_SET" -eq 1 ]; then
  ...
  SPAWN_TRACE_EFFECTIVE=$(xo_trace_context_session_effective "$STATE/.trace-context-effective")
  if [ "$SPAWN_TRACE_EFFECTIVE" = on ]; then
    SPAWN_TRACEPARENT=$(XO_TRACE_CONTEXT=on xo_trace_context_resolve "$CONFIG" "$STATE/$ID.meta" || true)
  else
    SPAWN_TRACEPARENT=
  fi
fi
...
}
{
  ...
  # data/xo-backend-design-d7's P1 compatibility contract).
  ...
  if [ "$SPAWN_CONTROL_PARENT" = 1 ] && [ -n "${XO_CONTROL_RELAUNCH_TX:-}" ]; then
    echo "control_relaunch_tx=$XO_CONTROL_RELAUNCH_TX"
  fi
} > "$SPAWN_META_PATH" || {
...
if [ "$RELAUNCH" -eq 0 ]; then
  if ! xo_backlog_atomic_transition publish "$SPAWN_META_TMP" "$STATE/$ID.meta" "task record" "$STATE"; then
...
# the record (bin/xo-backlog-transition-lib.sh owns the invariant). It runs under
...
spawn_commit_backlog_transition() {
  ...
  xo_backlog_atomic_transition dispatch "$STATE/$ID.meta" "$DATA" "$ID" "$STATE"
}
...
# actually move (xo-yi4j evidence, 2026-09-05). This re-reads the paired record
...
spawn_report_preserved_state() {
  local repair_error=
  if ! xo_backlog_record_present "$STATE/$ID.meta" "task record" "$STATE"; then
  ...
  if ! xo_backlog_row_probe "$DATA" "$ID"; then
    if [ "$XO_BACKLOG_ROW_RESULT" = not_found ]; then
      ...
      SPAWN_PRESERVED_CLAIM="preservation could not be verified: its backlog item state is unreadable (${XO_BACKLOG_ROW_ERROR:-no error recorded}); close out its paired task record and backlog item by hand"
    fi
    return 1
  fi
  if [ "$XO_BACKLOG_ROW_STATE" = "in_flight no no" ]; then
  ...
  xo_backlog_start "$DATA" "$ID" || repair_error=$XO_BACKLOG_TRANSITION_ERROR
  if [ -z "$repair_error" ] \
     && xo_backlog_row_probe "$DATA" "$ID" \
     && [ "$XO_BACKLOG_ROW_STATE" = "in_flight no no" ]; then
  ...
  SPAWN_PRESERVED_CLAIM="preservation could not be verified: its backlog item reads ${XO_BACKLOG_ROW_STATE:-unreadable}${repair_error:+, and moving it to In flight failed ($repair_error)}; close out its paired task record and backlog item by hand"
  return 1
}
...
if [ "$RELAUNCH" -eq 1 ]; then
  SPAWN_META_PUBLISH_STARTED=1
  if ! xo_backlog_atomic_transition publish "$SPAWN_META_TMP" "$STATE/$ID.meta" "task record" "$STATE"; then
    echo "error: replacement task record for $ID could not be published ($XO_BACKLOG_TRANSITION_ERROR)" >&2
    exit 1
  fi
  RELAUNCH_REPLACEMENT_PENDING=0
  SPAWN_META_PUBLISH_STARTED=0
...
if [ "$SPAWN_TREEHOUSE_PROJECT_LOCK_HELD" = 1 ]; then
  SPAWN_TREEHOUSE_PROJECT_LOCK_HELD=0
  xo_lock_release "$SPAWN_TREEHOUSE_PROJECT_LOCK"
fi
if [ "$SPAWN_TASK_SET_LOCK_HELD" = 1 ]; then
...
sq_piturnend=$(shell_quote "$PROJ_ABS/.pi/extensions/xo-primary-turnend-guard.ts")
sq_piwatch=$(shell_quote "$PROJ_ABS/.pi/extensions/xo-primary-pi-watch.ts")
sq_ompext=$(shell_quote "$STATE/$ID.omp-ext.ts")
sq_ompcfg=$(shell_quote "${OMP_WORKER_CFG:-$XO_ROOT/.omp/xo-worker-overlay.yml}")
sq_opinput=$(shell_quote "$XO_ROOT/bin/xo-operational-input.sh")
...
# inherit xo's current environment, so a bare `claude` in the pane falls
# back to the default ~/.claude store even when xo itself runs under a
# different CLAUDE_CONFIG_DIR (for example a work-vs-personal subscription split).
# Forward xo's own resolved store onto the claude launch so the crewmate
# uses the same credential/config xo is authenticated with. Only when set;
...
if [ "$KIND" = secondmate ]; then
  ...
  sq_primary_home=$(shell_quote "$XO_HOME")
  # Keep this in step with xo_supervision_model (bin/xo-wake-lib.sh): Claude's
  ...
  # receive extension to match xo_supervision_model's own table, so their pull
  ...
  # literal (never the raw XO_TRACE_CONTEXT string) so a XO_TRACE_CONTEXT override
  ...
  # not enable them across the launch boundary (bin/xo-trace-context-lib.sh header).
  # Reuse the single frozen decision from the carrier resolution above so the
  # injected carrier and this on/off snapshot are guaranteed to agree.
  LAUNCH="XO_ROOT_OVERRIDE= XO_STATE_OVERRIDE= XO_DATA_OVERRIDE= XO_PROJECTS_OVERRIDE= XO_CONFIG_OVERRIDE= XO_PUBLIC_FOLLOWUP_PRIMARY_HOME=$sq_primary_home XO_HOME=$sq_home XO_TRACE_CONTEXT=$SPAWN_TRACE_EFFECTIVE XO_SUPERVISION_MODEL=$supervision_model $LAUNCH"
fi
...
spawn_record_traceparent() {
  ...
  if [ "$SPAWN_META_LOCK_HELD" != 1 ]; then
    SPAWN_META_LOCK=$(xo_meta_lock_path "$meta") || return 1
    xo_lock_acquire_wait "$SPAWN_META_LOCK"
    SPAWN_META_LOCK_HELD=1
    acquired=1
  fi
  ...
  if [ ! -f "$meta" ] || [ ! -w "$meta" ] \
     ...
     || ! xo_backlog_atomic_transition publish "$SPAWN_META_TMP" "$meta" "task record" "$STATE"; then
    status=1
    rm -f "$SPAWN_META_TMP" 2>/dev/null || true
  fi
  SPAWN_META_TMP=
  if [ "$acquired" = 1 ]; then
    xo_lock_release "$SPAWN_META_LOCK" || status=1
    SPAWN_META_LOCK_HELD=0
  fi
  return "$status"
}
...
# Mark the pane as a task worker so bin/xo-test-run.sh can refuse to run the
...
# syntax of its own.
if [ "$KIND" = ship ] || [ "$KIND" = scout ]; then
  spawn_send_text_line "$T" "export XO_TASK_ID=$ID"
fi
...
if [ "$LAUNCH_ENV_ENABLED" = 1 ]; then
  ...
  for env_name in HOME PATH USER LOGNAME SHELL TERM COLORTERM LANG LC_ALL LC_CTYPE \
    ...
    CMUX_SOCKET_PATH ZELLIJ ZELLIJ_SESSION_NAME ZELLIJ_PANE_ID XO_ZELLIJ_SESSION \
    XO_TASK_ID \
    $LAUNCH_ENV_NAMES; do
...
if [ "$HARNESS" = kimi ]; then
  ...
  KIMI_SUBMIT_RETRIES=${XO_KIMI_SUBMIT_RETRIES:-3}
  KIMI_SUBMIT_SLEEP=${XO_KIMI_SUBMIT_SLEEP:-${XO_KIMI_POLL_INTERVAL:-0.5}}
  KIMI_SUBMIT_SETTLE=${XO_KIMI_SUBMIT_SETTLE:-0}
  if ! KIMI_SUBMIT_VERDICT=$(xo_backend_send_text_submit \
...
if [ "$HARNESS" = rovo ]; then
  ...
  ROVO_SUBMIT_RETRIES=${XO_ROVO_SUBMIT_RETRIES:-3}
  ROVO_SUBMIT_SLEEP=${XO_ROVO_SUBMIT_SLEEP:-${XO_ROVO_POLL_INTERVAL:-0.5}}
  ROVO_SUBMIT_SETTLE=${XO_ROVO_SUBMIT_SETTLE:-0}
  if ! ROVO_SUBMIT_VERDICT=$(xo_backend_send_text_submit \
...
if [ "$KIND" = secondmate ] && [ "${XO_SKIP_SECONDMATE_INHERIT:-0}" != 1 ]; then
  if ! xo_config_reread_discard_pending "$PROJ_ABS" "$ID" "$XO_HOME"; then
    if xo_config_reread_quarantine_pending "$PROJ_ABS" "$ID" "$XO_HOME"; then
      echo "CONFIG_REREAD: secondmate $ID: quarantined pre-relaunch generations after cleanup failure (destination=$PROJ_ABS/state/.xo-inherited-config-reread-quarantine source=$XO_HOME/state/.xo-inherited-config-reread-quarantine)" >&2
    else
      echo "CONFIG_REREAD: secondmate $ID: cleanup failed; pre-relaunch generations were force-cleared where possible (destination=$PROJ_ABS source=$XO_HOME)" >&2
    fi
  fi
fi
...
# (bin/xo-backlog-transition-lib.sh's xo_tasks_axi): a timed-out call
...
XO_TASKS_AXI_TIMEOUT=${XO_TASKS_AXI_TIMEOUT:-30}
...
if [ "$SPAWN_BACKLOG_COMMIT_STATUS" -ne 0 ]; then
  if [ "$RELAUNCH" -eq 0 ]; then
    if spawn_fresh_commit_rollback; then
      echo "error: task $ID's backlog item could not be moved to In flight ($XO_BACKLOG_TRANSITION_ERROR); its record was removed so no worker is left that the backlog does not own - close out endpoint $T and local copy $WT by hand, then re-run the spawn" >&2
    else
      echo "error: task $ID's backlog item could not be moved to In flight ($XO_BACKLOG_TRANSITION_ERROR), and failed-dispatch cleanup is incomplete; the provisional record may remain at $STATE/$ID.meta - close out endpoint $T and local copy $WT by hand, then remove the record and busy state before retrying" >&2
    fi
  else
    echo "error: task $ID was republished but its backlog item could not be moved to In flight ($XO_BACKLOG_TRANSITION_ERROR); fix the backlog and re-run the relaunch" >&2
  fi
fi
trap - HUP INT TERM
if [ "$SPAWN_BACKLOG_COMMIT_STATUS" -ne 0 ]; then
...
xo_lock_release "$SPAWN_META_LOCK"
SPAWN_META_LOCK_HELD=0
