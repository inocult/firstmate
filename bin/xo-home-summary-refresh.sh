#!/usr/bin/env bash
# xo-home-summary-refresh.sh - publish this home's structured summary ledger.
#
# Usage: xo-home-summary-refresh.sh [--best-effort]
#
# The published state/home-summary.json is the exact
# `xo-fleet-snapshot.sh --secondmate-home-summary` document for this XO_HOME.
# Its schema remains `xo-secondmate-home-summary.v1`, declares the current hold
...
# timeout owner bounds the complete refresh with XO_HOME_SUMMARY_TIMEOUT
...
ERROR_LOG_MAX_BYTES=${XO_HOME_SUMMARY_ERROR_LOG_MAX_BYTES:-65536}
HOME_SUMMARY_TIMEOUT=${XO_HOME_SUMMARY_TIMEOUT:-60}
HOME_SUMMARY_IF_IDLE=${XO_HOME_SUMMARY_IF_IDLE:-0}
BEST_EFFORT=0
...
case "${1:-}" in
  ...
  --_worker)
    ...
    BEST_EFFORT=${XO_HOME_SUMMARY_WORKER_BEST_EFFORT:-0}
    ;;
...
home_summary_cleanup() {
  ...
  if [ "$HOME_SUMMARY_LOCK_HELD" -eq 1 ]; then
    xo_lock_release "$REFRESH_LOCK" || true
    HOME_SUMMARY_LOCK_HELD=0
  fi
}
...
home_summary_refresh_once() {
  ...
  if [ "$HOME_SUMMARY_IF_IDLE" -eq 1 ]; then
    xo_lock_try_acquire "$REFRESH_LOCK" || return 0
  else
    xo_lock_acquire_wait "$REFRESH_LOCK"
  fi
  ...
  if env \
    ...
    "$SCRIPT_DIR/xo-fleet-snapshot.sh" --secondmate-home-summary \
...
home_summary_log_failure() {
  ...
  if ! printf '[%s] %s\n' "$stamp" "$HOME_SUMMARY_ERROR" >> "$ERROR_LOG" 2>/dev/null; then
    printf 'xo-home-summary-refresh: %s\n' "$HOME_SUMMARY_ERROR" >&2
    return 0
  fi
...
if [ "$HOME_SUMMARY_MODE" = log-failure ]; then
  HOME_SUMMARY_ERROR=${XO_HOME_SUMMARY_PARENT_ERROR:-"refresh worker failed"}
  HOME_SUMMARY_FAILURE_STAMP=${XO_HOME_SUMMARY_PARENT_STAMP:-}
  home_summary_log_failure
  exit 0
fi
...
if [ "$HOME_SUMMARY_MODE" = parent ]; then
  ...
  if xo_run_timed "$HOME_SUMMARY_TIMEOUT" env \
    XO_HOME_SUMMARY_WORKER_BEST_EFFORT="$BEST_EFFORT" \
    XO_HOME_SUMMARY_IF_IDLE="$HOME_SUMMARY_IF_IDLE" \
    "$SCRIPT_DIR/xo-home-summary-refresh.sh" --_worker; then
    exit 0
  else
    refresh_rc=$?
  fi
  if [ "$BEST_EFFORT" -eq 1 ]; then
    ...
    xo_run_timed 2 env \
      XO_HOME_SUMMARY_PARENT_ERROR="$parent_error" \
      XO_HOME_SUMMARY_PARENT_STAMP="$attempt_stamp" \
      "$SCRIPT_DIR/xo-home-summary-refresh.sh" --_log-failure >/dev/null || true
    exit 0
  fi
  if [ "$refresh_rc" -eq 124 ]; then
    printf 'xo-home-summary-refresh: refresh exceeded its %s-second deadline\n' \
