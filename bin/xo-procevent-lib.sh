# shellcheck shell=bash
# Shared identity, ownership, capture, and publication rules for the generic
# process-to-event runner.
# Usage: . bin/xo-procevent-lib.sh   (requires xo-pr-lib.sh and xo-wake-lib.sh)
#
# The runner lets xo learn that a registered long-polling source produced
# a result without holding that blocking process in its conversational turn. It
# is domain-neutral: a thin adapter supplies source identity, the argv to run,
# and how to classify a completed result. Everything else - ownership, durable
# capture, publication, and restart recovery - lives here.
#
# It adds no second notification control plane: a completed result is published
# as an ordinary `check` wake through the existing durable wake queue, which is
# the same mechanism merge polls and X mode already use.
#
# DURABILITY BOUNDARY, stated precisely. This runner proves exactly one thing:
# once a child process has exited and its output has been read, that output is
# stored atomically at mode 0600 BEFORE any event referencing it is published,
# and a captured result with no durable handled acknowledgement remains eligible
# for bounded re-announcement - including across a restart between publication
# and handling - until `xo-procevent.sh handled` records it. It proves nothing
# about the source side of the handoff. In particular the currently published
# `lavish-axi poll` destructively clears feedback before returning it, so a
# result lost between that clearing and this runner reading the process output
# is unrecoverable. An XO wrapper cannot close that window, and marking a
# result handled says nothing about whether a paired external effect performed
# before that call actually completed: a crash between the effect and the
# acknowledgement can still repeat the effect on replay. Never describe this
# runner as at-least-once, no-loss, or lossless, and never claim generic
# exactly-once effects from the handled acknowledgement alone.

# Machine-wide claim root. Homes can share one underlying source store, so the
# "one owner per canonical source" rule cannot live inside a single home.
xo_procevent_claim_root() {
  printf '%s\n' "${XO_PROCEVENT_CLAIM_ROOT:-${XDG_STATE_HOME:-$HOME/.local/state}/xo/procevent-claims}"
}

xo_procevent_registry_dir() { printf '%s\n' "$1/procevent"; }
xo_procevent_inbox_dir()    { printf '%s\n' "$1/procevent-inbox"; }
xo_procevent_capture_reservation_dir() { printf '%s\n' "$1/procevent-capture-reservations"; }

# A source id names a private file and a bounded wake slug, so it is held to the
# same path-safe shape as a task id. Adapters derive it from canonical source
# identity, never from a caller-supplied display string.
xo_procevent_source_id_valid() {
  local id=${1-}
  xo_task_id_path_safe "$id" || return 1
  [ "${#id}" -le 64 ]
}

xo_procevent_adapter_valid() {
  local a=${1-}
  case "$a" in
    ''|*[!a-z0-9-]*) return 1 ;;
  esac
  [ "${#a}" -le 32 ]
}

xo_procevent_extension_id_valid() {
  local id=${1-}
  case "$id" in
    ''|[!a-z0-9]*|*[-.]|*[!a-z0-9.-]*|*..*|*.-*|*-.*|*--*) return 1 ;;
  esac
  [ "${#id}" -le 128 ]
}

xo_procevent_extension_version_valid() {
  local version=${1-}
  case "$version" in
    ''|*[!A-Za-z0-9.+-]*) return 1 ;;
  esac
  [ "${#version}" -le 128 ]
}

xo_procevent_digest_valid() {
  local digest=${1-} hex
  case "$digest" in sha256:*) ;; *) return 1 ;; esac
  hex=${digest#sha256:}
  [ "${#hex}" -eq 64 ] || return 1
  case "$hex" in *[!0-9a-f]*) return 1 ;; esac
}

xo_procevent_extension_config_ref_valid() {
  local ref=${1-}
  local LC_ALL=C
  [ -n "$ref" ] && [ "${#ref}" -le 512 ] || return 1
  ! printf '%s' "$ref" | grep -q '[[:cntrl:]]'
}

xo_procevent_extension_registration_token_valid() {
  xo_procevent_digest_valid "${1-}"
}

# xo_procevent_any_registered <state>
xo_procevent_any_registered() {
  local reg rec
  reg=$(xo_procevent_registry_dir "$1")
  [ -d "$reg" ] || return 1
  for rec in "$reg"/*.source; do
    [ -e "$rec" ] || continue
    return 0
  done
  return 1
}

# --- owning-session lease ---------------------------------------------------
# A runner is detached into its own process group so it survives the turn that
# started it. That is what makes a persistent source work, and on its own it is
# also what lets a runner outlive its whole home: once reparented to init,
# nothing bounds its lifetime, so its blocking child - and everything that child
# spawns - can keep running indefinitely.
#
# The bound is a lease on the OWNING STATE ROOT. Owner-presence operations
# refresh it, an attached public start keeps it fresh while its caller remains
# attached, and the watcher's reconcile cycle keeps it fresh in a live home.
# A guard proves the runner's owner is still there by reading that lease from
# the physical state root recorded in the claim. After two consecutive checks
# cannot prove both the root identity and a fresh lease, it stops the runner's
# process group. The lease is keyed by state root, so another home's live runner
# is untouched: that home refreshes its own lease. Nothing here keys on a script
# name, a command line, or a process name, all of which are shared across homes.

xo_procevent_owner_lease_path() {  # <state-root>
  printf '%s/.owner-lease\n' "$(xo_procevent_registry_dir "$1")"
}

# Record owner-presence activity in this home's process-event state. Best
# effort by design: a home with no registry directory yet owns no runner.
xo_procevent_owner_lease_touch() {  # <state-root>
  local reg lease tmp now
  reg=$(xo_procevent_registry_dir "$1")
  [ -d "$reg" ] && [ ! -L "$reg" ] || return 1
  lease=$(xo_procevent_owner_lease_path "$1")
  now=$(perl -MTime::HiRes=clock_gettime,CLOCK_MONOTONIC -e \
    'printf "%.6f\n", clock_gettime(CLOCK_MONOTONIC)') || return 1
  tmp=$(umask 077; mktemp "$reg/.owner-lease.XXXXXX") || return 1
  if ! printf '%s\n' "$now" > "$tmp" || ! mv -f -- "$tmp" "$lease"; then
    rm -f -- "$tmp"
    return 1
  fi
}

# Seconds since the last refresh. Fails when the lease is absent or unreadable,
# which is what a removed home looks like from inside a surviving runner.
xo_procevent_owner_lease_age() {  # <state-root>
  local lease value
  lease=$(xo_procevent_owner_lease_path "$1")
  [ -f "$lease" ] && [ ! -L "$lease" ] || return 1
  IFS= read -r value < "$lease" || return 1
  perl -MTime::HiRes=clock_gettime,CLOCK_MONOTONIC -e '
    use strict;
    use warnings;
    my $value = shift;
    $value =~ /\A[0-9]+(?:\.[0-9]+)?\z/ or exit 1;
    my $now = clock_gettime(CLOCK_MONOTONIC);
    $now >= $value or exit 1;
    printf "%d\n", int($now - $value);
  ' "$value"
}

# How long a runner keeps going with no activity in its owning home. The default
# is forty watcher cycles at the default poll interval, so an ordinary busy or
# briefly wedged home never trips it, while a home that is simply gone stops
# owning processes within the hour rather than within a day.
XO_PROCEVENT_OWNER_LEASE_DEFAULT_SECONDS=600
XO_PROCEVENT_OWNER_LEASE_MIN_SECONDS=1
XO_PROCEVENT_OWNER_LEASE_MAX_SECONDS=86400

xo_procevent_owner_lease_seconds() {
  local value=${XO_PROCEVENT_OWNER_LEASE_SECONDS-}
  if [ -z "$value" ]; then
    printf '%s\n' "$XO_PROCEVENT_OWNER_LEASE_DEFAULT_SECONDS"
    return 0
  fi
  case "$value" in ''|*[!0-9]*) return 1 ;; esac
  [ "$value" -ge "$XO_PROCEVENT_OWNER_LEASE_MIN_SECONDS" ] || return 1
  [ "$value" -le "$XO_PROCEVENT_OWNER_LEASE_MAX_SECONDS" ] || return 1
  printf '%s\n' "$value"
}

# Detection-interval semantics: docs/configuration.md, Process-to-event sources.
XO_PROCEVENT_OWNER_CHECK_DEFAULT_SECONDS=15
XO_PROCEVENT_OWNER_CHECK_MIN_SECONDS=1
XO_PROCEVENT_OWNER_CHECK_MAX_SECONDS=3600

xo_procevent_owner_check_seconds() {
  local value=${XO_PROCEVENT_OWNER_CHECK_SECONDS-}
  if [ -z "$value" ]; then
    printf '%s\n' "$XO_PROCEVENT_OWNER_CHECK_DEFAULT_SECONDS"
    return 0
  fi
  case "$value" in ''|*[!0-9]*) return 1 ;; esac
  [ "$value" -ge "$XO_PROCEVENT_OWNER_CHECK_MIN_SECONDS" ] || return 1
  [ "$value" -le "$XO_PROCEVENT_OWNER_CHECK_MAX_SECONDS" ] || return 1
  printf '%s\n' "$value"
}

XO_PROCEVENT_LAUNCH_FLOOR_DEFAULT_SECONDS=1
XO_PROCEVENT_LAUNCH_FLOOR_MIN_SECONDS=1
XO_PROCEVENT_LAUNCH_FLOOR_MAX_SECONDS=3600

xo_procevent_launch_floor_seconds() {
  local value=${XO_PROCEVENT_LAUNCH_FLOOR_SECONDS-}
  if [ -z "$value" ]; then
    printf '%s\n' "$XO_PROCEVENT_LAUNCH_FLOOR_DEFAULT_SECONDS"
    return 0
  fi
  case "$value" in ''|*[!0-9]*) return 1 ;; esac
  [ "$value" -ge "$XO_PROCEVENT_LAUNCH_FLOOR_MIN_SECONDS" ] || return 1
  [ "$value" -le "$XO_PROCEVENT_LAUNCH_FLOOR_MAX_SECONDS" ] || return 1
  printf '%s\n' "$value"
}

# How long reconcile waits for a runner it just detached to prove it took the
# source's claim. Confirmation reads durable evidence, so a healthy launch
# settles on the first poll and only a launch not yet proved spends the
# window. The default stays well below XO_POLL because bin/xo-watch.sh runs
# reconcile once per supervision cycle, and every launch of a cycle shares ONE
# window rather than taking a window each.
XO_PROCEVENT_LAUNCH_CONFIRM_DEFAULT_SECONDS=3
XO_PROCEVENT_LAUNCH_CONFIRM_MIN_SECONDS=1
XO_PROCEVENT_LAUNCH_CONFIRM_MAX_SECONDS=600

xo_procevent_launch_confirm_seconds() {
  local value=${XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS-}
  if [ -z "$value" ]; then
    printf '%s\n' "$XO_PROCEVENT_LAUNCH_CONFIRM_DEFAULT_SECONDS"
    return 0
  fi
  case "$value" in ''|*[!0-9]*) return 1 ;; esac
  [ "$value" -ge "$XO_PROCEVENT_LAUNCH_CONFIRM_MIN_SECONDS" ] || return 1
  [ "$value" -le "$XO_PROCEVENT_LAUNCH_CONFIRM_MAX_SECONDS" ] || return 1
  printf '%s\n' "$value"
}

# The one place the launch-pacing stamp's name is constructed. Every writer,
# pruner and reader goes through here so the naming rule is stated once.
xo_procevent_launch_floor_stamp_path() {  # <state-root> <source-id> <registration-identity>
  local reg identity
  case "$3" in *:*) ;; *) return 1 ;; esac
  case "$3" in ''|*[!0-9:]*) return 1 ;; esac
  xo_procevent_source_id_valid "$2" || return 1
  reg=$(xo_procevent_registry_dir "$1") || return 1
  identity=${3//:/-}
  printf '%s\n' "$reg/$2.$identity.last-launch"
}

xo_procevent_launch_floor_reset_locked() {  # <state-root> <source-id> <registration-identity>
  local stamp
  stamp=$(xo_procevent_launch_floor_stamp_path "$1" "$2" "$3") || return 1
  rm -f -- "$stamp"
}

xo_procevent_launch_floor_prune_locked() {  # <state-root> <source-id> <registration-identity>
  local reg keep stamp
  keep=$(xo_procevent_launch_floor_stamp_path "$1" "$2" "$3") || return 1
  reg=$(xo_procevent_registry_dir "$1") || return 1
  for stamp in "$reg/$2".*.last-launch "$reg/$2.last-launch"; do
    [ "$stamp" = "$keep" ] && continue
    [ -e "$stamp" ] || [ -L "$stamp" ] || continue
    rm -f -- "$stamp" || return 1
  done
}

xo_procevent_launch_floor_wait() {  # <state-root> <source-id> <registration-identity> <seconds>
  local state=$1 id=$2 expected=$3 floor=$4 reg stamp registration current_identity status=0
  stamp=$(xo_procevent_launch_floor_stamp_path "$state" "$id" "$expected") || return 1
  reg=$(xo_procevent_registry_dir "$state") || return 1
  [ ! -L "$stamp" ] || return 1
  [ ! -e "$stamp" ] || [ -f "$stamp" ] || return 1
  perl -MTime::HiRes=clock_gettime,sleep,CLOCK_MONOTONIC -e '
    use strict;
    use warnings;
    my ($path, $floor) = @ARGV;
    my $previous;
    if (-e $path) {
      open my $in, "<", $path or exit 1;
      my $value = <$in>;
      close $in or exit 1;
      defined($value) && $value =~ /\A([0-9]+(?:\.[0-9]+)?)\n?\z/ or exit 1;
      $previous = 0 + $1;
    }
    my $now = clock_gettime(CLOCK_MONOTONIC);
    my $elapsed = defined($previous) && $now >= $previous ? $now - $previous : undef;
    sleep($floor - $elapsed) if defined($elapsed) && $elapsed < $floor;
  ' "$stamp" "$floor" || return 1

  # Registration publication holds this same source lock while replacing and
  # pruning pacing state, so a superseded sleeper cannot recreate its stamp.
  xo_procevent_source_lock_acquire "$id" || return 1
  registration="$reg/$id.source"
  current_identity=$(xo_pr_file_identity "$registration" 2>/dev/null) || current_identity=
  if [ "$current_identity" != "$expected" ]; then
    xo_procevent_source_lock_release "$id" || return 1
    return 2
  fi
  [ ! -L "$stamp" ] && { [ ! -e "$stamp" ] || [ -f "$stamp" ]; } || status=1
  if [ "$status" -eq 0 ]; then
    perl -MTime::HiRes=clock_gettime,CLOCK_MONOTONIC -MFcntl=:DEFAULT -e '
      use strict;
      use warnings;
      my $path = shift;
      my $now = clock_gettime(CLOCK_MONOTONIC);
      my $tmp = "$path.$$";
      sysopen(my $out, $tmp, O_WRONLY | O_CREAT | O_EXCL, 0600) or exit 1;
      print {$out} "$now\n" or exit 1;
      close $out or exit 1;
      rename $tmp, $path or exit 1;
    ' "$stamp" || status=1
  fi
  if [ "$status" -ne 0 ]; then
    xo_procevent_source_lock_release "$id" || :
    return "$status"
  fi
  return 0
}

# True while the owning home is provably still active.
xo_procevent_owner_alive() {  # <state-root> <lease-seconds>
  local age
  age=$(xo_procevent_owner_lease_age "$1") || return 1
  [ "$age" -le "$2" ]
}

# --- ownership --------------------------------------------------------------
# A claim is a private file recording the home, runner pid, claim generation,
# and process identity. Registration and every ownership transition are
# serialized at one source boundary.

xo_procevent_claim_path() {
  printf '%s/%s.claim\n' "$(xo_procevent_claim_root)" "$1"
}

xo_procevent_source_lock_path() {
  printf '%s/%s.lock\n' "$(xo_procevent_claim_root)" "$1"
}

xo_procevent_source_lock_acquire() {
  local id=$1 root
  xo_procevent_source_id_valid "$id" || return 1
  root=$(xo_procevent_claim_root)
  (umask 077; mkdir -p "$root") || return 1
  [ -d "$root" ] && [ ! -L "$root" ] || return 1
  xo_lock_acquire_wait "$(xo_procevent_source_lock_path "$id")"
}

# xo_procevent_source_lock_try_acquire <source-id>
# Non-blocking acquisition for release_start_claim in bin/xo-procevent.sh;
# that caller owns the exit-cleanup lock-order invariant.
xo_procevent_source_lock_try_acquire() {
  local id=$1 root
  xo_procevent_source_id_valid "$id" || return 1
  root=$(xo_procevent_claim_root)
  (umask 077; mkdir -p "$root") || return 1
  [ -d "$root" ] && [ ! -L "$root" ] || return 1
  xo_lock_try_acquire "$(xo_procevent_source_lock_path "$id")"
}

xo_procevent_source_lock_release() {
  xo_lock_release "$(xo_procevent_source_lock_path "$1")"
}

xo_procevent_registration_publish_locked() {  # <state> <adapter> <source-id> <argv...>
  local state=$1 adapter=$2 id=$3 reg dest tmp arg identity
  shift 3
  xo_procevent_adapter_valid "$adapter" || return 1
  xo_procevent_source_id_valid "$id" || return 1
  [ "$#" -ge 1 ] || return 1
  for arg in "$@"; do
    case "$arg" in *$'\n'*) return 1 ;; esac
  done
  reg=$(xo_procevent_registry_dir "$state")
  (umask 077; mkdir -p "$reg") || return 1
  [ -d "$reg" ] && [ ! -L "$reg" ] || return 1
  dest="$reg/$id.source"
  tmp=$(umask 077; mktemp "$reg/.source.XXXXXX") || return 1
  if {
    printf 'adapter=%s\n' "$adapter"
    printf 'argc=%s\n' "$#"
    printf 'argv:\n'
    printf '%s\n' "$@"
  } > "$tmp" && chmod 0600 "$tmp" \
    && identity=$(xo_pr_file_identity "$tmp") \
    && xo_procevent_launch_floor_reset_locked "$state" "$id" "$identity" \
    && mv -f -- "$tmp" "$dest"; then
    xo_procevent_launch_floor_prune_locked "$state" "$id" "$identity" 2>/dev/null || :
    return 0
  fi
  rm -f -- "$tmp"
  return 1
}

# Publish one extension-owned registration. Its identity fields and random
# registration token are immutable owner evidence; the executable argv is never
# stored because the tracked host constructs that command at run time.
xo_procevent_extension_registration_publish_locked() {  # <state> <adapter> <source-id> <extension-id> <extension-version> <capability-version> <package-digest> <binding-digest> <config-ref> <registration-token>
  local state=$1 adapter=$2 id=$3 extension_id=$4 extension_version=$5 capability_version=$6
  local package_digest=$7 binding_digest=$8 config_ref=$9 registration_token=${10} reg dest tmp identity
  xo_procevent_adapter_valid "$adapter" || return 1
  xo_procevent_source_id_valid "$id" || return 1
  xo_procevent_extension_id_valid "$extension_id" || return 1
  xo_procevent_extension_version_valid "$extension_version" || return 1
  [ "$capability_version" = 1 ] || return 1
  xo_procevent_digest_valid "$package_digest" || return 1
  xo_procevent_digest_valid "$binding_digest" || return 1
  xo_procevent_extension_config_ref_valid "$config_ref" || return 1
  xo_procevent_extension_registration_token_valid "$registration_token" || return 1
  reg=$(xo_procevent_registry_dir "$state")
  (umask 077; mkdir -p "$reg") || return 1
  [ -d "$reg" ] && [ ! -L "$reg" ] || return 1
  dest="$reg/$id.source"
  tmp=$(umask 077; mktemp "$reg/.source.XXXXXX") || return 1
  if {
    printf 'adapter=%s\n' "$adapter"
    printf 'owner=extension\n'
    printf 'extension_schema=xo-procevent-extension-owner.v1\n'
    printf 'extension_id=%s\n' "$extension_id"
    printf 'extension_version=%s\n' "$extension_version"
    printf 'capability_version=%s\n' "$capability_version"
    printf 'package_digest=%s\n' "$package_digest"
    printf 'binding_digest=%s\n' "$binding_digest"
    printf 'config_ref=%s\n' "$config_ref"
    printf 'registration_token=%s\n' "$registration_token"
    printf 'argc=0\n'
    printf 'argv:\n'
  } > "$tmp" && chmod 0600 "$tmp" \
    && identity=$(xo_pr_file_identity "$tmp") \
    && xo_procevent_launch_floor_reset_locked "$state" "$id" "$identity" \
    && mv -f -- "$tmp" "$dest"; then
    xo_procevent_launch_floor_prune_locked "$state" "$id" "$identity" 2>/dev/null || :
    return 0
  fi
  rm -f -- "$tmp"
  return 1
}

# Load an extension-owned registration under the caller's source lock.
# 0 = valid extension owner, 1 = ordinary built-in registration, 2 = malformed
# extension owner. Sets XO_PROCEVENT_EXTENSION_* on success.
xo_procevent_extension_registration_load_locked() {  # <state> <source-id>
  local state=$1 id=$2 file adapter_line owner_line schema_line id_line version_line capability_line
  local package_line binding_line config_line token_line argc_line argv_line extra
  file="$(xo_procevent_registry_dir "$state")/$id.source"
  [ -f "$file" ] && [ ! -L "$file" ] || return 2
  owner_line=$(sed -n '2p' "$file") || return 2
  [ "$owner_line" = owner=extension ] || return 1
  [ "$(xo_pr_file_mode "$file")" = 600 ] \
    && [ "$(xo_pr_file_link_count "$file")" = 1 ] || return 2
  {
    IFS= read -r adapter_line \
      && IFS= read -r owner_line \
      && IFS= read -r schema_line \
      && IFS= read -r id_line \
      && IFS= read -r version_line \
      && IFS= read -r capability_line \
      && IFS= read -r package_line \
      && IFS= read -r binding_line \
      && IFS= read -r config_line \
      && IFS= read -r token_line \
      && IFS= read -r argc_line \
      && IFS= read -r argv_line \
      && ! IFS= read -r extra
  } < "$file" || return 2
  [ "$owner_line" = owner=extension ] || return 2
  [ "$schema_line" = extension_schema=xo-procevent-extension-owner.v1 ] || return 2
  [ "$capability_line" = capability_version=1 ] || return 2
  [ "$argc_line" = argc=0 ] && [ "$argv_line" = argv: ] || return 2
  XO_PROCEVENT_EXTENSION_ADAPTER=${adapter_line#adapter=}
  XO_PROCEVENT_EXTENSION_ID=${id_line#extension_id=}
  XO_PROCEVENT_EXTENSION_VERSION=${version_line#extension_version=}
  # shellcheck disable=SC2034 # Public loader output consumed by xo-procevent.sh.
  XO_PROCEVENT_EXTENSION_CAPABILITY_VERSION=${capability_line#capability_version=}
  XO_PROCEVENT_EXTENSION_PACKAGE_DIGEST=${package_line#package_digest=}
  XO_PROCEVENT_EXTENSION_BINDING_DIGEST=${binding_line#binding_digest=}
  XO_PROCEVENT_EXTENSION_CONFIG_REF=${config_line#config_ref=}
  XO_PROCEVENT_EXTENSION_REGISTRATION_TOKEN=${token_line#registration_token=}
  [ "$adapter_line" = "adapter=$XO_PROCEVENT_EXTENSION_ADAPTER" ] || return 2
  [ "$id_line" = "extension_id=$XO_PROCEVENT_EXTENSION_ID" ] || return 2
  [ "$version_line" = "extension_version=$XO_PROCEVENT_EXTENSION_VERSION" ] || return 2
  [ "$package_line" = "package_digest=$XO_PROCEVENT_EXTENSION_PACKAGE_DIGEST" ] || return 2
  [ "$binding_line" = "binding_digest=$XO_PROCEVENT_EXTENSION_BINDING_DIGEST" ] || return 2
  [ "$config_line" = "config_ref=$XO_PROCEVENT_EXTENSION_CONFIG_REF" ] || return 2
  [ "$token_line" = "registration_token=$XO_PROCEVENT_EXTENSION_REGISTRATION_TOKEN" ] || return 2
  xo_procevent_adapter_valid "$XO_PROCEVENT_EXTENSION_ADAPTER" || return 2
  xo_procevent_extension_id_valid "$XO_PROCEVENT_EXTENSION_ID" || return 2
  xo_procevent_extension_version_valid "$XO_PROCEVENT_EXTENSION_VERSION" || return 2
  xo_procevent_digest_valid "$XO_PROCEVENT_EXTENSION_PACKAGE_DIGEST" || return 2
  xo_procevent_digest_valid "$XO_PROCEVENT_EXTENSION_BINDING_DIGEST" || return 2
  xo_procevent_extension_config_ref_valid "$XO_PROCEVENT_EXTENSION_CONFIG_REF" || return 2
  xo_procevent_extension_registration_token_valid "$XO_PROCEVENT_EXTENSION_REGISTRATION_TOKEN" || return 2
}

# Exact legacy registration comparison used by conditional built-in retirement.
xo_procevent_registration_matches_locked() {  # <state> <adapter> <source-id> <argv...>
  local state=$1 adapter=$2 id=$3 reg dest tmp arg status=1
  shift 3
  xo_procevent_adapter_valid "$adapter" || return 1
  xo_procevent_source_id_valid "$id" || return 1
  [ "$#" -ge 1 ] || return 1
  for arg in "$@"; do
    case "$arg" in *$'\n'*) return 1 ;; esac
  done
  reg=$(xo_procevent_registry_dir "$state")
  [ -d "$reg" ] && [ ! -L "$reg" ] || return 1
  dest="$reg/$id.source"
  [ -f "$dest" ] && [ ! -L "$dest" ] || return 1
  tmp=$(umask 077; mktemp "$reg/.source-match.XXXXXX") || return 1
  if {
    printf 'adapter=%s\n' "$adapter"
    printf 'argc=%s\n' "$#"
    printf 'argv:\n'
    printf '%s\n' "$@"
  } > "$tmp" && cmp -s -- "$tmp" "$dest"; then
    status=0
  fi
  rm -f -- "$tmp"
  return "$status"
}

xo_procevent_claim_load_locked() {  # <source-id>
  local claim home pid token identity reg_dir reg_identity terminal state_root state_device state_inode state_owner state_mode extra
  claim=$(xo_procevent_claim_path "$1")
  [ -f "$claim" ] && [ ! -L "$claim" ] || return 1
  {
    IFS= read -r home \
      && IFS= read -r pid \
      && IFS= read -r token \
      && IFS= read -r identity \
      && { IFS= read -r reg_dir || reg_dir=; } \
      && { IFS= read -r reg_identity || reg_identity=; } \
      && { IFS= read -r terminal || terminal=active; }
    if IFS= read -r state_root; then
      IFS= read -r state_device \
        && IFS= read -r state_inode \
        && IFS= read -r state_owner \
        && IFS= read -r state_mode \
        && ! IFS= read -r extra
    else
      state_root=
      state_device=
      state_inode=
      state_owner=
      state_mode=
    fi
  } < "$claim" || return 1
  [ -n "$home" ] || return 1
  case "$pid" in ''|*[!0-9]*) return 1 ;; esac
  case "$token" in ''|*[!A-Za-z0-9._-]*) return 1 ;; esac
  [ -n "$identity" ] || return 1
  case "$reg_dir" in ''|/*) ;; *) return 1 ;; esac
  case "$reg_identity" in ''|*:* ) ;; *) return 1 ;; esac
  case "$terminal" in active|terminal) ;; *) return 1 ;; esac
  if [ -n "$state_root" ]; then
    case "$state_root" in /*) ;; *) return 1 ;; esac
    xo_procevent_claim_state_root_field_valid "$state_root" || return 1
    case "$state_device" in ''|*[!0-9]*) return 1 ;; esac
    case "$state_inode" in ''|*[!0-9]*) return 1 ;; esac
    case "$state_owner" in ''|*[!0-9]*) return 1 ;; esac
    case "$state_mode" in ''|*[!0-7]*) return 1 ;; esac
    [ $((8#$state_mode & 8#022)) -eq 0 ] || return 1
  elif [ -n "$state_device$state_inode$state_owner$state_mode" ]; then
    return 1
  fi
  XO_PROCEVENT_CLAIM_HOME=$home
  XO_PROCEVENT_CLAIM_PID=$pid
  XO_PROCEVENT_CLAIM_TOKEN=$token
  XO_PROCEVENT_CLAIM_IDENTITY=$identity
  XO_PROCEVENT_CLAIM_REG_DIR=$reg_dir
  XO_PROCEVENT_CLAIM_REG_IDENTITY=$reg_identity
  XO_PROCEVENT_CLAIM_TERMINAL=$terminal
  XO_PROCEVENT_CLAIM_STATE_ROOT=$state_root
  XO_PROCEVENT_CLAIM_STATE_DEVICE=$state_device
  XO_PROCEVENT_CLAIM_STATE_INODE=$state_inode
  XO_PROCEVENT_CLAIM_STATE_OWNER=$state_owner
  XO_PROCEVENT_CLAIM_STATE_MODE=$state_mode
}

xo_procevent_claim_state_root_field_valid() {  # <canonical-state-root>
  local value=$1 LC_ALL=C
  case "$value" in *[[:cntrl:]]*) return 1 ;; esac
  return 0
}

xo_procevent_claim_state_root_identity() {  # <state-root>
  local state=$1 canonical device inode owner mode
  canonical=$(xo_procevent_state_root_resolve "$state") || return 1
  xo_procevent_claim_state_root_field_valid "$canonical" || return 1
  device=$(xo_pr_file_device "$canonical") || return 1
  inode=$(xo_pr_file_inode "$canonical") || return 1
  owner=$(id -u) || return 1
  mode=$(xo_pr_file_mode "$canonical") || return 1
  printf '%s\t%s\t%s\t%s\t%s\n' "$canonical" "$device" "$inode" "$owner" "$mode"
}

xo_procevent_claim_owned_by_state() {  # <state-root> <legacy-home>
  if [ -n "${XO_PROCEVENT_CLAIM_STATE_ROOT:-}" ]; then
    [ "$XO_PROCEVENT_CLAIM_STATE_ROOT" = "$1" ]
  else
    [ "$XO_PROCEVENT_CLAIM_HOME" = "$2" ]
  fi
}

xo_procevent_claim_recorded_state_root_valid() {
  local identity state_root state_device state_inode state_owner state_mode
  state_root=${XO_PROCEVENT_CLAIM_STATE_ROOT:-}
  [ -n "$state_root" ] || return 0
  identity=$(xo_procevent_claim_state_root_identity "$state_root") || return 1
  IFS=$'\t' read -r state_root state_device state_inode state_owner state_mode <<< "$identity"
  [ "$state_root" = "$XO_PROCEVENT_CLAIM_STATE_ROOT" ] \
    && [ "$state_device" = "$XO_PROCEVENT_CLAIM_STATE_DEVICE" ] \
    && [ "$state_inode" = "$XO_PROCEVENT_CLAIM_STATE_INODE" ] \
    && [ "$state_owner" = "$XO_PROCEVENT_CLAIM_STATE_OWNER" ] \
    && [ "$state_mode" = "$XO_PROCEVENT_CLAIM_STATE_MODE" ]
}

xo_procevent_claim_capture_reservation_remove_locked() {
  [ -n "${XO_PROCEVENT_CLAIM_STATE_ROOT:-}" ] || return 0
  xo_procevent_claim_recorded_state_root_valid || return 1
  xo_procevent_capture_reservation_remove_claim "$XO_PROCEVENT_CLAIM_STATE_ROOT" "$XO_PROCEVENT_CLAIM_TOKEN"
}

# xo_procevent_claim_generation_gone_locked
# True only when the loaded claim's owner is stale and the process group it led
# independently has no members left. The separate group check also covers a
# reused live pid whose identity differs while the old generation survives.
# A live matched owner (state 0), an unreadable identity (state 2), and a
# crashed leader with a still-live ambiguous group (state 3) all return false.
xo_procevent_claim_generation_gone_locked() {
  local state=0
  xo_procevent_pid_state "${XO_PROCEVENT_CLAIM_PID:-}" "${XO_PROCEVENT_CLAIM_IDENTITY:-}" || state=$?
  [ "$state" -eq 1 ] \
    && ! xo_procevent_group_alive "${XO_PROCEVENT_CLAIM_PID:-}"
}

# xo_procevent_claim_undisplaceable_locked <source-id>
# The single owner of "this stale claim is one no unattended caller may
# displace". True when a claim record is still present for the source and its
# generation is NOT provably gone. Call it only where
# xo_procevent_claim_state_locked has just returned 1, so the XO_PROCEVENT_CLAIM_*
# globals below describe this source: that same return also covers a source with
# no claim record at all, which leaves those globals holding whatever the
# previous load put there, so the record check has to travel with the generation
# check rather than being left to each caller.
#
# What the surviving process group means is why this refuses rather than
# relaunches. xo_procevent_group_alive probes the runner's OWN process group,
# and the runner leads that group with its polling source child inside it, so
# "the group still has members" can mean that child is still attached to the
# session the source collects from. Starting a replacement there puts a second
# destructive poller on one session, which drains and loses what the source was
# collecting. A source that needs a human beats a source that silently eats what
# it was supposed to deliver.
xo_procevent_claim_undisplaceable_locked() {  # <source-id>
  [ -e "$(xo_procevent_claim_path "$1")" ] || return 1
  ! xo_procevent_claim_generation_gone_locked
}

# Capture-reservation cleanup for a claim being reclaimed.
#
# Reservation records are keyed by CLAIM TOKEN, and every replacement claims a
# fresh token, so a dead generation's leftovers can never collide with the
# generation that replaces it. They are hygiene, not an ownership invariant -
# the runner's own successful-capture path already tidies them best-effort.
# The cleanup is still attempted and remains authoritative for a generation
# that is not provably gone; it stops being a veto only after the stale owner
# and independent group check prove the whole generation gone.
xo_procevent_claim_capture_reservation_reclaim_locked() {
  xo_procevent_claim_capture_reservation_remove_locked && return 0
  xo_procevent_claim_generation_gone_locked
}

# xo_procevent_group_alive <pid>
# True while any process remains in the runner's numeric process group. A runner
# starts as its own group leader, but after that leader exits a same-numbered
# group may be reused, so group presence prevents proving the generation gone.
xo_procevent_group_alive() {
  case "$1" in ''|*[!0-9]*) return 1 ;; esac
  kill -0 -"$1" 2>/dev/null
}

# xo_procevent_pid_state <pid> <identity>
# 0 live match, 1 stale, 2 uncertain, 3 ambiguous leaderless group.
#
# State 3 is the crash cut: the runner leader is gone, but a process group with
# its numeric id still has members. That group may be the old generation or a
# leaderless group created after PID/PGID reuse, so cleanup preserves the claim
# without signalling the group or starting a replacement.
xo_procevent_pid_state() {
  local pid=$1 expected=$2 actual
  if ! xo_pid_alive "$pid"; then
    xo_procevent_group_alive "$pid" && return 3
    return 1
  fi
  if actual=$(xo_pid_identity "$pid" 2>/dev/null); then
    [ "$actual" = "$expected" ] && return 0
    return 1
  fi
  xo_pid_alive "$pid" || { xo_procevent_group_alive "$pid" && return 3; return 1; }
  return 2
}

# <source-id>: 0 live, 1 stale/absent, 2 uncertain, 3 leader gone with its owned
# process group still alive, 4 terminal retirement pending.
xo_procevent_claim_state_locked() {
  local claim registration current_identity
  claim=$(xo_procevent_claim_path "$1")
  [ -e "$claim" ] || return 1
  xo_procevent_claim_load_locked "$1" || return 2
  if [ "$XO_PROCEVENT_CLAIM_TERMINAL" = terminal ] && [ -n "$XO_PROCEVENT_CLAIM_REG_IDENTITY" ]; then
    registration="$XO_PROCEVENT_CLAIM_REG_DIR/$1.source"
    current_identity=$(xo_pr_file_identity "$registration" 2>/dev/null || true)
    [ "$current_identity" = "$XO_PROCEVENT_CLAIM_REG_IDENTITY" ] && return 4
  fi
  xo_procevent_pid_state "$XO_PROCEVENT_CLAIM_PID" "$XO_PROCEVENT_CLAIM_IDENTITY"
}

# xo_procevent_claim_acquire_locked <source-id> <home> <pid> <registration> <state-root>
# 0 acquired, 1 error, 2 held by a live owner (possibly another home).
xo_procevent_claim_acquire_locked() {
  local id=$1 home=$2 pid=$3 registration=$4 state=$5 root claim tmp identity token status claim_state old_home old_token old_reg_dir reg_dir reg_identity stage state_root state_device state_inode state_owner state_mode
  xo_procevent_source_id_valid "$id" || return 1
  [ -f "$registration" ] && [ ! -L "$registration" ] || return 1
  reg_dir=${registration%/*}
  case "$reg_dir" in /*) ;; *) return 1 ;; esac
  reg_identity=$(xo_pr_file_identity "$registration" 2>/dev/null) || return 1
  identity=$(xo_pid_identity "$pid" 2>/dev/null) || return 1
  root=$(xo_procevent_claim_root)
  claim=$(xo_procevent_claim_path "$id")
  status=0
  if [ -e "$claim" ] || [ -L "$claim" ]; then
    xo_procevent_claim_state_locked "$id"
    claim_state=$?
    case "$claim_state" in
      0|2|3|4) status=2 ;;
      1)
        if [ -f "$claim" ] && [ ! -L "$claim" ]; then
          old_home=$XO_PROCEVENT_CLAIM_HOME
          old_token=$XO_PROCEVENT_CLAIM_TOKEN
          old_reg_dir=$XO_PROCEVENT_CLAIM_REG_DIR
          if [ -z "$old_reg_dir" ]; then
            if [ "$old_home" = "$home" ]; then
              old_reg_dir=$reg_dir
            else
              old_reg_dir="$old_home/state/procevent"
            fi
          fi
          if [ -L "$old_reg_dir" ] || { [ -e "$old_reg_dir" ] && [ ! -d "$old_reg_dir" ]; }; then
            status=1
          else
            stage="$old_reg_dir/.$id.$old_token.output"
            if { [ -e "$stage" ] || [ -L "$stage" ]; } && ! rm -f -- "$stage"; then
              status=1
            fi
          fi
          if [ "$status" -eq 0 ]; then
            xo_procevent_claim_capture_reservation_reclaim_locked || status=1
          fi
          # Every cleanup above tidies leftovers that belong to the DEAD
          # generation - its staging file and its capture reservation, both keyed
          # by ITS claim token - and a replacement always claims a fresh token,
          # so nothing a failed tidy-up leaves behind can collide with the
          # generation that replaces it.
          # xo_procevent_claim_capture_reservation_reclaim_locked already states
          # that rule for the reservation record; the staging file takes the same
          # rule here, and so does the shape check on the registry directory
          # recorded to hold it, which only decides whether that removal is safe
          # to attempt. Once the stale owner and the
          # independently absent process group prove the whole generation gone,
          # the documented ownership promise is already granted, so a failed
          # tidy-up may leave litter and nothing more. Vetoing the claim instead
          # is what leaves a provably dead runner owning the source permanently,
          # where no reconcile, no retire and no fresh arm can displace it.
          if [ "$status" -ne 0 ] && xo_procevent_claim_generation_gone_locked; then
            status=0
          fi
          # Two owners is the one outcome worse than none: never proceed on a
          # claim record that is still there.
          [ "$status" -ne 0 ] || rm -f -- "$claim" || status=1
        else
          status=1
        fi
        ;;
      *) status=1 ;;
    esac
    if [ "$status" -eq 0 ] && { [ ! -f "$registration" ] || [ -L "$registration" ]; }; then
      status=1
    fi
  fi
  if [ "$status" -eq 0 ]; then
    tmp=$(umask 077; mktemp "$root/.claim.XXXXXX") || status=1
  fi
  if [ "$status" -eq 0 ]; then
    IFS=$'\t' read -r state_root state_device state_inode state_owner state_mode \
      < <(xo_procevent_claim_state_root_identity "$state") || status=1
  fi
  if [ "$status" -eq 0 ]; then
    token=${tmp##*/}-$pid
    printf '%s\n%s\n%s\n%s\n%s\n%s\nactive\n%s\n%s\n%s\n%s\n%s\n' \
      "$home" "$pid" "$token" "$identity" "$reg_dir" "$reg_identity" \
      "$state_root" "$state_device" "$state_inode" "$state_owner" "$state_mode" > "$tmp" || status=1
    [ "$status" -ne 0 ] || chmod 0600 "$tmp" || status=1
    [ "$status" -ne 0 ] || mv -f -- "$tmp" "$claim" || status=1
    if [ "$status" -eq 0 ]; then
      XO_PROCEVENT_CLAIM_TOKEN=$token
      XO_PROCEVENT_CLAIM_REG_IDENTITY=$reg_identity
      XO_PROCEVENT_CLAIM_STATE_ROOT=$state_root
      XO_PROCEVENT_CLAIM_STATE_DEVICE=$state_device
      XO_PROCEVENT_CLAIM_STATE_INODE=$state_inode
      XO_PROCEVENT_CLAIM_STATE_OWNER=$state_owner
      XO_PROCEVENT_CLAIM_STATE_MODE=$state_mode
    fi
  fi
  [ "$status" -eq 0 ] || { [ -z "${tmp:-}" ] || rm -f -- "$tmp"; }
  return "$status"
}

xo_procevent_claim_mark_terminal_locked() {
  local id=$1 home=$2 pid=$3 token=$4 claim root tmp
  claim=$(xo_procevent_claim_path "$id")
  xo_procevent_claim_load_locked "$id" \
    && [ "$XO_PROCEVENT_CLAIM_HOME" = "$home" ] \
    && [ "$XO_PROCEVENT_CLAIM_PID" = "$pid" ] \
    && [ "$XO_PROCEVENT_CLAIM_TOKEN" = "$token" ] \
    && [ -n "$XO_PROCEVENT_CLAIM_REG_IDENTITY" ] || return 1
  root=$(xo_procevent_claim_root)
  tmp=$(umask 077; mktemp "$root/.claim.XXXXXX") || return 1
  if [ -n "$XO_PROCEVENT_CLAIM_STATE_ROOT" ]; then
    if printf '%s\n%s\n%s\n%s\n%s\n%s\nterminal\n%s\n%s\n%s\n%s\n%s\n' \
      "$XO_PROCEVENT_CLAIM_HOME" "$XO_PROCEVENT_CLAIM_PID" "$XO_PROCEVENT_CLAIM_TOKEN" \
      "$XO_PROCEVENT_CLAIM_IDENTITY" "$XO_PROCEVENT_CLAIM_REG_DIR" \
      "$XO_PROCEVENT_CLAIM_REG_IDENTITY" "$XO_PROCEVENT_CLAIM_STATE_ROOT" \
      "$XO_PROCEVENT_CLAIM_STATE_DEVICE" "$XO_PROCEVENT_CLAIM_STATE_INODE" \
      "$XO_PROCEVENT_CLAIM_STATE_OWNER" "$XO_PROCEVENT_CLAIM_STATE_MODE" > "$tmp" \
      && chmod 0600 "$tmp" \
      && mv -f -- "$tmp" "$claim"; then
      return 0
    else
      rm -f -- "$tmp"
      return 1
    fi
  fi
  if printf '%s\n%s\n%s\n%s\n%s\n%s\nterminal\n' \
    "$XO_PROCEVENT_CLAIM_HOME" "$XO_PROCEVENT_CLAIM_PID" "$XO_PROCEVENT_CLAIM_TOKEN" \
    "$XO_PROCEVENT_CLAIM_IDENTITY" "$XO_PROCEVENT_CLAIM_REG_DIR" \
    "$XO_PROCEVENT_CLAIM_REG_IDENTITY" > "$tmp" \
    && chmod 0600 "$tmp" \
    && mv -f -- "$tmp" "$claim"; then
    return 0
  else
    rm -f -- "$tmp"
    return 1
  fi
}

# xo_procevent_claim_release_locked <source-id> <home> <pid> <token>
# The live owner uses this path for its own release. Reservation cleanup must
# succeed normally; stale-generation relaxation is never consulted.
xo_procevent_claim_release_locked() {
  xo_procevent_claim_release_mode_locked release "$@"
}

# xo_procevent_claim_release_terminal_self_locked <source-id> <home> <pid> <token>
# A live runner uses this only while retiring its own terminal source mid-capture.
# Its in-flight reservation is transient, so attempt cleanup without making that
# cleanup a veto; exact ownership still must match before releasing the claim.
xo_procevent_claim_release_terminal_self_locked() {
  xo_procevent_claim_release_mode_locked terminal-self "$@"
}

# xo_procevent_claim_reclaim_locked <source-id> <home> <pid> <token>
# Lifecycle commands use this only after proving or stopping a dead generation.
xo_procevent_claim_reclaim_locked() {
  xo_procevent_claim_release_mode_locked reclaim "$@"
}

xo_procevent_claim_release_mode_locked() {
  local mode=$1 id=$2 home=$3 pid=$4 token=$5 claim
  xo_procevent_source_id_valid "$id" || return 1
  claim=$(xo_procevent_claim_path "$id")
  [ -e "$claim" ] || return 0
  if xo_procevent_claim_load_locked "$id" \
    && [ "$XO_PROCEVENT_CLAIM_HOME" = "$home" ] \
    && [ "$XO_PROCEVENT_CLAIM_PID" = "$pid" ] \
    && [ "$XO_PROCEVENT_CLAIM_TOKEN" = "$token" ]; then
    case "$mode" in
      reclaim)
        xo_procevent_claim_capture_reservation_reclaim_locked || return 1
        ;;
      terminal-self)
        xo_procevent_claim_capture_reservation_remove_locked || true
        ;;
      release)
        xo_procevent_claim_capture_reservation_remove_locked || return 1
        ;;
      *) return 1 ;;
    esac
    rm -f -- "$claim"
    return $?
  fi
  return 1
}

# --- durable capture and publication ----------------------------------------

xo_procevent_path_normalize() {
  local path=${1-} part
  local -a parts normalized=()
  [ -n "$path" ] || return 1
  case "$path" in
    /*) ;;
    *) path="$(pwd -P)/$path" ;;
  esac
  IFS=/ read -r -a parts <<< "$path"
  for part in "${parts[@]}"; do
    case "$part" in
      ''|.) ;;
      ..) [ "${#normalized[@]}" -gt 0 ] && unset 'normalized[${#normalized[@]}-1]' ;;
      *) normalized+=("$part") ;;
    esac
  done
  printf '/%s\n' "$(IFS=/; printf '%s' "${normalized[*]}")"
}

xo_procevent_directory_owned_by_current_user() {
  local owner
  if [ "$(uname)" = Darwin ]; then
    owner=$(/usr/bin/stat -f %u "$1" 2>/dev/null)
  else
    owner=$(stat -c %u "$1" 2>/dev/null)
  fi
  [ "$owner" = "$(id -u)" ]
}

# xo_procevent_state_root_resolve <state-root>
# Print the physical private directory this module operates on, or fail. A home
# is legitimately spelled through a symlinked ancestor - /tmp and $TMPDIR are
# symlinks on macOS - so the caller's spelling is resolved exactly once here and
# every derived path, recorded claim identity, and later confinement check uses
# the physical root instead. Resolving before validating is what makes the
# private-directory contract hold for the directory actually operated on, rather
# than only for callers that already spelled it physically.
xo_procevent_state_root_resolve() {  # <state-root>
  local state=$1 canonical
  canonical=$(CDPATH='' cd -P -- "$state" 2>/dev/null && pwd -P) || return 1
  xo_procevent_private_directory_valid "$canonical" 0 || return 1
  printf '%s\n' "$canonical"
}

xo_procevent_private_directory_valid() {
  local directory=$1 exact_mode=$2 canonical normalized mode
  [ -d "$directory" ] && [ ! -L "$directory" ] || return 1
  xo_procevent_directory_owned_by_current_user "$directory" || return 1
  mode=$(xo_pr_file_mode "$directory") || return 1
  case "$mode" in ''|*[!0-7]*) return 1 ;; esac
  if [ "$exact_mode" = 1 ]; then
    [ "$mode" = 700 ] || return 1
  elif [ $((8#$mode & 8#022)) -ne 0 ]; then
    return 1
  fi
  canonical=$(cd -P -- "$directory" && pwd -P) || return 1
  normalized=$(xo_procevent_path_normalize "$directory") || return 1
  [ "$canonical" = "$normalized" ]
}

xo_procevent_capture_inbox_prepare() {
  local state=$1 inbox
  state=$(xo_procevent_state_root_resolve "$state") || return 1
  inbox=$(xo_procevent_inbox_dir "$state")
  if [ ! -e "$inbox" ] && [ ! -L "$inbox" ]; then
    (umask 077; mkdir "$inbox") || return 1
  fi
  xo_procevent_private_directory_valid "$inbox" 1 || return 1
  printf '%s\n' "$inbox"
}

# Print the validated physical registry directory, like the inbox and
# reservation preparers beside it, so a caller that pins the boundary with
# `pwd -P` compares against the same physical path this validated.
xo_procevent_extension_staging_prepare() {
  local state=$1 registry
  state=$(xo_procevent_state_root_resolve "$state") || return 1
  registry=$(xo_procevent_registry_dir "$state")
  xo_procevent_private_directory_valid "$registry" 1 || return 1
  printf '%s\n' "$registry"
}

xo_procevent_capture_reservation_prepare() {
  local state=$1 reservation
  state=$(xo_procevent_state_root_resolve "$state") || return 1
  reservation=$(xo_procevent_capture_reservation_dir "$state")
  if [ ! -e "$reservation" ] && [ ! -L "$reservation" ]; then
    (umask 077; mkdir "$reservation") || return 1
  fi
  xo_procevent_private_directory_valid "$reservation" 1 || return 1
  printf '%s\n' "$reservation"
}

xo_procevent_capture_reservation_remove_claim() {  # <state> <claim-token>
  local state=$1 token=$2 reservation record
  case "$token" in ''|*[!A-Za-z0-9._-]*) return 1 ;; esac
  reservation=$(xo_procevent_capture_reservation_dir "$state")
  [ -d "$reservation" ] || return 0
  xo_procevent_private_directory_valid "$reservation" 1 || return 1
  for record in "$reservation"/.extension-capture-"$token".*.json \
    "$reservation"/.extension-capture-"$token".*.consumed-*; do
    [ -e "$record" ] || continue
    [ -f "$record" ] && [ ! -L "$record" ] || return 1
    rm -f -- "$record" || return 1
  done
}

# xo_procevent_capture <state> <source-id> <adapter> <output-file>
#   [<extension-id> <extension-version> <capability-version> <package-digest> <binding-digest>]
# Atomically store the completed output at 0600 and print its durable path. The
# rename is the commit point; nothing referencing this result may be published
# before it returns successfully. Extension captures retain immutable package
# identity beside the legacy adapter sidecar, so later classification cannot
# silently move to a replacement binding.
xo_procevent_capture() {
  local state=$1 id=$2 adapter=$3 src=$4 extension_id=${5-} extension_version=${6-}
  local capability_version=${7-} package_digest=${8-} binding_digest=${9-}
  local inbox seq dest tmp adapter_dest adapter_tmp extension_dest='' extension_tmp=''
  [ "$#" -eq 4 ] || [ "$#" -eq 9 ] || return 1
  xo_procevent_source_id_valid "$id" || return 1
  xo_procevent_adapter_valid "$adapter" || return 1
  if [ "$#" -eq 9 ]; then
    xo_procevent_extension_id_valid "$extension_id" || return 1
    xo_procevent_extension_version_valid "$extension_version" || return 1
    [ "$capability_version" = 1 ] || return 1
    xo_procevent_digest_valid "$package_digest" || return 1
    xo_procevent_digest_valid "$binding_digest" || return 1
  fi
  if [ "$#" -eq 9 ]; then
    if [ "${XO_PROCEVENT_CAPTURE_PINNED_INBOX:-}" != 1 ]; then
      inbox=$(xo_procevent_capture_inbox_prepare "$state") || return 1
      (
        CDPATH='' cd -- "$inbox" 2>/dev/null || exit 1
        [ "$(pwd -P)" = "$inbox" ] || exit 1
        XO_PROCEVENT_CAPTURE_PINNED_INBOX=1 \
          XO_PROCEVENT_CAPTURE_ABSOLUTE_INBOX="$inbox" \
          xo_procevent_capture "$@"
      )
      return $?
    fi
    inbox=.
  else
    inbox=$(xo_procevent_inbox_dir "$state")
    (umask 077; mkdir -p "$inbox") || return 1
  fi
  seq=1
  while [ -e "$inbox/$id.$seq.result" ]; do seq=$((seq + 1)); done
  dest="$inbox/$id.$seq.result"
  adapter_dest="$inbox/$id.$seq.adapter"
  if [ "$#" -eq 9 ]; then
    [ ! -e "$dest" ] && [ ! -L "$dest" ] \
      && [ ! -e "$adapter_dest" ] && [ ! -L "$adapter_dest" ] || return 1
  fi
  tmp=$(umask 077; mktemp "$inbox/.capture.XXXXXX") || return 1
  adapter_tmp=$(umask 077; mktemp "$inbox/.adapter.XXXXXX") || { rm -f -- "$tmp"; return 1; }
  if [ "$#" -eq 9 ]; then
    extension_dest="$inbox/$id.$seq.extension"
    [ ! -e "$extension_dest" ] && [ ! -L "$extension_dest" ] || {
      rm -f -- "$tmp" "$adapter_tmp"
      return 1
    }
    extension_tmp=$(umask 077; mktemp "$inbox/.extension.XXXXXX") \
      || { rm -f -- "$tmp" "$adapter_tmp"; return 1; }
  fi
  if ! cat "$src" > "$tmp"; then rm -f -- "$tmp" "$adapter_tmp" "$extension_tmp"; return 1; fi
  if ! printf '%s\n' "$adapter" > "$adapter_tmp"; then rm -f -- "$tmp" "$adapter_tmp" "$extension_tmp"; return 1; fi
  if [ "$#" -eq 9 ] && ! {
    printf 'schema=xo-procevent-extension-owner.v1\n'
    printf 'extension_id=%s\n' "$extension_id"
    printf 'extension_version=%s\n' "$extension_version"
    printf 'capability_version=%s\n' "$capability_version"
    printf 'package_digest=%s\n' "$package_digest"
    printf 'binding_digest=%s\n' "$binding_digest"
  } > "$extension_tmp"; then
    rm -f -- "$tmp" "$adapter_tmp" "$extension_tmp"
    return 1
  fi
  if ! chmod 0600 "$tmp" "$adapter_tmp"; then
    rm -f -- "$tmp" "$adapter_tmp" "$extension_tmp"
    return 1
  fi
  if [ "$#" -eq 9 ] && ! chmod 0600 "$extension_tmp"; then
    rm -f -- "$tmp" "$adapter_tmp" "$extension_tmp"
    return 1
  fi
  if ! mv -f -- "$adapter_tmp" "$adapter_dest"; then rm -f -- "$tmp" "$adapter_tmp" "$extension_tmp"; return 1; fi
  if [ "$#" -eq 9 ] && ! mv -f -- "$extension_tmp" "$extension_dest"; then
    rm -f -- "$tmp" "$adapter_dest" "$extension_tmp"
    return 1
  fi
  if ! mv -f -- "$tmp" "$dest"; then
    rm -f -- "$tmp" "$adapter_dest"
    [ -z "$extension_dest" ] || rm -f -- "$extension_dest"
    return 1
  fi
  if [ "$#" -eq 9 ]; then
    printf '%s\n' "$XO_PROCEVENT_CAPTURE_ABSOLUTE_INBOX/$id.$seq.result"
  else
    printf '%s\n' "$dest"
  fi
}

# xo_procevent_pending <state>
# Print every durably captured result that has no durable handled
# acknowledgement yet, oldest first. A result stays here - and so remains
# eligible for repeat publication on the existing durable wake queue - across
# any number of restarts and drains until `xo_procevent_mark_handled` records
# it; this is what makes a restart between publication and handling recover
# instead of silently losing the result.
xo_procevent_pending() {
  local state=$1 inbox result base seq
  inbox=$(xo_procevent_inbox_dir "$state")
  [ -d "$inbox" ] || return 0
  for result in "$inbox"/*.result; do
    [ -f "$result" ] && [ ! -L "$result" ] || continue
    [ -e "${result%.result}.handled" ] && continue
    base=${result%.result}
    seq=${base##*.}
    case "$seq" in ''|*[!0-9]*) continue ;; esac
    printf '%s\t%s\n' "$seq" "$result"
  done | sort -n -k1,1 -k2,2 | cut -f2-
}

# xo_procevent_event_line <adapter> <source-id> <sequence>
# The complete normalized event. Bounded by construction: a fixed verb, a
# validated adapter name, and a validated id. No source output, path, or
# caller-supplied text can appear here.
xo_procevent_event_line() {
  local adapter=$1 id=$2 seq=$3
  xo_procevent_adapter_valid "$adapter" || return 1
  xo_procevent_source_id_valid "$id" || return 1
  case "$seq" in ''|*[!0-9]*) return 1 ;; esac
  printf 'procevent %s %s %s\n' "$adapter" "$id" "$seq"
}

# xo_procevent_handled_marker <state> <source-id> <sequence>
xo_procevent_handled_marker() {
  if [ "${XO_PROCEVENT_CAPTURE_PINNED_INBOX:-}" = 1 ]; then
    printf './%s.%s.handled\n' "$2" "$3"
    return
  fi
  printf '%s/%s.%s.handled\n' "$(xo_procevent_inbox_dir "$1")" "$2" "$3"
}

# xo_procevent_is_handled <state> <source-id> <sequence>
xo_procevent_is_handled() {
  local marker; marker=$(xo_procevent_handled_marker "$1" "$2" "$3")
  [ -f "$marker" ] && [ ! -L "$marker" ]
}

# xo_procevent_mark_handled <state> <source-id> <sequence>
# The one durable handled acknowledgement per captured generation: keyed by the
# exact source id and sequence, private at mode 0600, and path-safe through the
# same validation as every other source-id use. Atomically check-and-set - the
# create uses O_EXCL so two concurrent callers can never both win - so a caller
# pairing this with an external effect can trust the return code to authorize
# that effect at most once per generation. This is the only terminal state:
# announcing a result never blocks it from being re-announced, only this does.
# 0 = newly recorded (first-ever handling for this generation, safe to perform
# a paired effect that has not yet run), 1 = already recorded (repeat call; do
# not repeat a paired effect), 2 = error.
xo_procevent_mark_handled() {
  local state=$1 id=$2 seq=$3 inbox result adapter_file marker tmp
  xo_procevent_source_id_valid "$id" || return 2
  case "$seq" in ''|*[!0-9]*) return 2 ;; esac
  if [ "${XO_PROCEVENT_CAPTURE_PINNED_INBOX:-}" = 1 ]; then
    inbox=.
  else
    inbox=$(xo_procevent_inbox_dir "$state")
  fi
  result="$inbox/$id.$seq.result"
  adapter_file="$inbox/$id.$seq.adapter"
  [ -f "$result" ] && [ ! -L "$result" ] || return 2
  [ -f "$adapter_file" ] && [ ! -L "$adapter_file" ] || return 2
  marker=$(xo_procevent_handled_marker "$state" "$id" "$seq")
  [ ! -L "$marker" ] || return 2
  tmp=$(umask 077; mktemp "$inbox/.handled.XXXXXX") || return 2
  if ! chmod 0600 "$tmp"; then
    rm -f -- "$tmp"
    return 2
  fi
  if ln "$tmp" "$marker" 2>/dev/null; then
    rm -f -- "$tmp"
    return 0
  fi
  rm -f -- "$tmp"
  [ -f "$marker" ] && [ ! -L "$marker" ] && return 1
  return 2
}

# xo_procevent_result_source_id <result-path>
xo_procevent_result_source_id() {
  local base=${1##*/}
  base=${base%.result}
  printf '%s\n' "${base%.*}"
}

xo_procevent_result_sequence() {
  local base=${1##*/}
  base=${base%.result}
  printf '%s\n' "${base##*.}"
}

xo_procevent_result_adapter() {
  local result=$1 adapter_file="${1%.result}.adapter" adapter extra
  [ -f "$result" ] && [ ! -L "$result" ] || return 1
  [ -f "$adapter_file" ] && [ ! -L "$adapter_file" ] || return 1
  {
    IFS= read -r adapter \
      && ! IFS= read -r extra
  } < "$adapter_file" || return 1
  [ -z "$extra" ] || return 1
  xo_procevent_adapter_valid "$adapter" || return 1
  printf '%s\n' "$adapter"
}

# Load immutable extension identity for one captured result.
# 0 = valid extension sidecar, 1 = built-in result (sidecar absent),
# 2 = malformed or unsafe extension sidecar.
xo_procevent_result_extension_load() {  # <result-path>
  local result=$1 file="${1%.result}.extension" schema_line id_line version_line capability_line
  local package_line binding_line extra
  [ -e "$file" ] || return 1
  [ -f "$file" ] && [ ! -L "$file" ] || return 2
  [ "$(xo_pr_file_mode "$file")" = 600 ] \
    && [ "$(xo_pr_file_link_count "$file")" = 1 ] || return 2
  {
    IFS= read -r schema_line \
      && IFS= read -r id_line \
      && IFS= read -r version_line \
      && IFS= read -r capability_line \
      && IFS= read -r package_line \
      && IFS= read -r binding_line \
      && ! IFS= read -r extra
  } < "$file" || return 2
  [ "$schema_line" = schema=xo-procevent-extension-owner.v1 ] || return 2
  [ "$capability_line" = capability_version=1 ] || return 2
  XO_PROCEVENT_RESULT_EXTENSION_ID=${id_line#extension_id=}
  XO_PROCEVENT_RESULT_EXTENSION_VERSION=${version_line#extension_version=}
  # shellcheck disable=SC2034 # Public loader output consumed by xo-procevent.sh.
  XO_PROCEVENT_RESULT_EXTENSION_CAPABILITY_VERSION=${capability_line#capability_version=}
  XO_PROCEVENT_RESULT_EXTENSION_PACKAGE_DIGEST=${package_line#package_digest=}
  XO_PROCEVENT_RESULT_EXTENSION_BINDING_DIGEST=${binding_line#binding_digest=}
  [ "$id_line" = "extension_id=$XO_PROCEVENT_RESULT_EXTENSION_ID" ] || return 2
  [ "$version_line" = "extension_version=$XO_PROCEVENT_RESULT_EXTENSION_VERSION" ] || return 2
  [ "$package_line" = "package_digest=$XO_PROCEVENT_RESULT_EXTENSION_PACKAGE_DIGEST" ] || return 2
  [ "$binding_line" = "binding_digest=$XO_PROCEVENT_RESULT_EXTENSION_BINDING_DIGEST" ] || return 2
  xo_procevent_extension_id_valid "$XO_PROCEVENT_RESULT_EXTENSION_ID" || return 2
  xo_procevent_extension_version_valid "$XO_PROCEVENT_RESULT_EXTENSION_VERSION" || return 2
  xo_procevent_digest_valid "$XO_PROCEVENT_RESULT_EXTENSION_PACKAGE_DIGEST" || return 2
  xo_procevent_digest_valid "$XO_PROCEVENT_RESULT_EXTENSION_BINDING_DIGEST" || return 2
}
