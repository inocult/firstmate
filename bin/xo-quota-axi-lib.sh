...
# Usage: . bin/xo-quota-axi-lib.sh
#
# XO_QUOTA_AXI_MIN follows the axi-family floor policy owned beside the floor
# constants in bin/xo-bootstrap.sh.
#
# This file is the single owner of that version number. bin/xo-bootstrap.sh
...
XO_QUOTA_AXI_MIN=0.1.29

xo_quota_axi_compatible() {
  ...
  if [ -n "$timeout" ]; then
    ...
    [ "$(type -t xo_run_timed)" = function ] || return 1
    output=$(xo_run_timed "$timeout" quota-axi --version 2>/dev/null </dev/null) || return 1
  else
  ...
  # The floor is compared from XO_QUOTA_AXI_MIN so bumping it needs one edit.
  IFS='.' read -r min_major min_minor min_patch min_extra <<< "$XO_QUOTA_AXI_MIN"
