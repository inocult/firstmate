...
# two: bin/xo-backlog-transition-lib.sh owns that invariant, and both halves run
...
# (bin/xo-captain-hold.sh `open` owns that predicate), because the policy holds
...
# 0/1/2 exit-code contract. The optional `--identity` output that bin/xo-watch.sh
...
# captain's question), and bin/xo-captain-hold.sh answer stays the only act
...
# where the usual "checks green" xo-pr-check.sh trigger never fires) - by looking
...
# branch (xo performs that merge after configured approval) as a fallback
...
# this home or any locally registered XO home may name the same live path
...
# reads the slot's own owner claim, written by bin/xo-spawn.sh at the moment the
# slot is taken and dropped here once it is genuinely returned; bin/xo-wake-lib.sh
...
# bin/xo-backend.sh's endpoint validation refuses an empty or missing worktree=
...
# claim file sits on top of it, is owned by bin/xo-wake-lib.sh's slot-owner
# claim comment.
...
# XO home's state directory, as resolved by bin/xo-wake-lib.sh's
# xo_xo_root_home; a home seeded from another machine is its own local
...
# every local XO home hold the same lock from before slot allocation
...
# Usage: xo-teardown.sh <task-id> [--force] [--legacy-record]
...
#   agent-less (bin/xo-backend.sh's recovery-grade classifier), and without
...
#   1. Retries up to XO_TREEHOUSE_RETURN_LOCK_RETRIES times (default 3), waiting
#      XO_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS (default 1s; falls back to the older
#      XO_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS name when the new one is unset) between
...
#      bin/xo-lock-lib.sh's xo_lock_is_provably_stale, passing the worktree dir as the
#      companion directory and XO_STALE_WORKTREE_LOCK_AGE_SECS (default 30s) as the age
...
#     THIS task only when its branch AND code identity (bin/xo-nm-run-lib.sh's
#     strict xo_nm_head_matches_worktree rule) both match this worktree, then
...
#     xo_nm_runs_status_for_worktree ledger rule, whose anchored continuation
...
#     log in a pruned no-mistakes gate worktree). bin/xo-remote-job-reap-orphans.sh
...
SUB_HOME_PARENT_MARKER=".xo-secondmate-parent"
...
# shellcheck source=bin/xo-lock-lib.sh
. "$SCRIPT_DIR/xo-lock-lib.sh"
# shellcheck source=bin/xo-classify-lib.sh
. "$SCRIPT_DIR/xo-classify-lib.sh"
...
# shellcheck source=bin/xo-pending-reply-lib.sh
. "$SCRIPT_DIR/xo-pending-reply-lib.sh"
# shellcheck source=bin/xo-nm-run-lib.sh
. "$SCRIPT_DIR/xo-nm-run-lib.sh"
if [ "$#" -lt 1 ] || ! xo_task_id_path_safe "$1"; then
...
xo_backlog_directory_present "$STATE" "state directory" || {
  echo "error: teardown refused: $XO_BACKLOG_TRANSITION_ERROR" >&2
  exit 1
}
...
# task's live lease (contract: bin/xo-lease-lib.sh; no-op in homes without
# leases).
...
# territory (contract: bin/xo-lease-lib.sh).
if [ "$FORCE" = --force ] && [ "$(xo_lease_actor)" = branch ]; then
  echo "error: forced teardown refused - the supervision branch cannot discard work" >&2
  exit "$XO_LEASE_REFUSE_EXIT"
fi
xo_lease_guard "$ID" "teardown (xo-teardown)"
...
if [ -f "$META" ] && [ ! -L "$META" ]; then
  TEARDOWN_LOCK_KIND=$(xo_meta_get "$META" kind)
  [ -n "$TEARDOWN_LOCK_KIND" ] || TEARDOWN_LOCK_KIND=ship
  TEARDOWN_LOCK_BACKEND=$(xo_meta_get "$META" backend)
  [ -n "$TEARDOWN_LOCK_BACKEND" ] || TEARDOWN_LOCK_BACKEND=tmux
  TEARDOWN_LOCK_WT=$(xo_meta_get "$META" worktree)
  TEARDOWN_LOCK_PROJECT=$(xo_meta_get "$META" project)
  if [ "$TEARDOWN_LOCK_KIND" != secondmate ] \
     && [ "$TEARDOWN_LOCK_BACKEND" != orca ] \
     && xo_treehouse_pool_slot "$TEARDOWN_LOCK_PROJECT" "$TEARDOWN_LOCK_WT"; then
    TREEHOUSE_SLOT_LOCK_REQUIRED=1
    TREEHOUSE_PROJECT_LOCK=$(xo_treehouse_project_lock_path "$TEARDOWN_LOCK_PROJECT") || {
      echo "REFUSED: cannot resolve the shared Treehouse project lock for ${TEARDOWN_LOCK_PROJECT:-<missing>}; nothing was changed" >&2
      exit 1
    }
    xo_lock_try_acquire "$TREEHOUSE_PROJECT_LOCK" || {
...
teardown_release_locks() {
  ...
  for ((i=${#DESCENDANT_LOCK_PATHS[@]} - 1; i >= 0; i--)); do
    xo_lock_release "${DESCENDANT_LOCK_PATHS[$i]}" || true
  done
  DESCENDANT_LOCK_PATHS=()
  if [ -n "${HANDOFF_WAKE_RETIRE_LOCK:-}" ]; then
    xo_lock_release "$HANDOFF_WAKE_RETIRE_LOCK" || true
    HANDOFF_WAKE_RETIRE_LOCK=
  fi
  if [ -n "${LOCAL_HANDOFF_LOCK:-}" ]; then
    xo_lock_release "$LOCAL_HANDOFF_LOCK" || true
    LOCAL_HANDOFF_LOCK=
  fi
  if [ -n "${LOCAL_REGISTRY_LOCK:-}" ]; then
    xo_lock_release "$LOCAL_REGISTRY_LOCK" || true
    LOCAL_REGISTRY_LOCK=
  fi
  ...
  if [ "$TREEHOUSE_PROJECT_LOCK_HELD" = 1 ]; then
    xo_lock_release "$TREEHOUSE_PROJECT_LOCK" || true
    TREEHOUSE_PROJECT_LOCK_HELD=0
  fi
  xo_lease_guard_release || true
  return "$status"
}
...
# down a worktree (see bin/xo-gate-refuse-lib.sh).
xo_refuse_if_gate_agent
XO_LOCK_LOG_PREFIX=teardown

xo_backlog_record_present "$META" "task record" "$STATE" || {
  echo "error: teardown refused: $XO_BACKLOG_TRANSITION_ERROR" >&2
  exit 1
}
...
META_LOCK_HELD=1
xo_backlog_record_present "$META" "task record" "$STATE" || {
  echo "error: teardown refused after locking: $XO_BACKLOG_TRANSITION_ERROR" >&2
  exit 1
}
TEARDOWN_META_KIND=$(xo_meta_get "$META" kind)
[ -n "$TEARDOWN_META_KIND" ] || TEARDOWN_META_KIND=ship
TEARDOWN_CLEANUP_RECOVERY=$(xo_meta_get "$META" cleanup_recovery)
...
if [ "$TEARDOWN_CLEANUP_RECOVERY" != orca ]; then
  if xo_backlog_transition_applies "$CONFIG" "$DATA" "$TEARDOWN_META_KIND"; then
    TEARDOWN_BACKLOG_APPLIES=1
  else
    TEARDOWN_BACKLOG_GATE_STATUS=$?
    if [ "$TEARDOWN_BACKLOG_GATE_STATUS" -eq 2 ]; then
      echo "error: task $ID cannot be torn down because its backlog data directory is inaccessible: $DATA ($XO_BACKLOG_TRANSITION_ERROR)" >&2
      exit 1
    fi
    TEARDOWN_BACKLOG_SKIP_REASON=$XO_BACKLOG_TRANSITION_SKIP
  fi
fi
if [ "$TEARDOWN_BACKLOG_APPLIES" = 1 ]; then
  if ! xo_backlog_meta_spawn_gen "$META" "$STATE"; then
    ...
    if [ "$TEARDOWN_LEGACY_GEN_COUNT" = 0 ] && [ "$LEGACY_RECORD_GIVEN" = 1 ]; then
      ...
      echo "error: task $ID's record has no spawn_gen that identifies one exact incarnation ($XO_BACKLOG_TRANSITION_ERROR); refusing automatic teardown - relaunch the task to publish an unambiguous incarnation, then retry teardown, or pass --legacy-record once its recorded endpoint is confirmed dead or agent-less" >&2
      exit 1
    else
      echo "error: task $ID's record has an unreadable spawn_gen that identifies one exact incarnation ($XO_BACKLOG_TRANSITION_ERROR); refusing automatic teardown - fix the record, then retry teardown" >&2
      exit 1
    fi
  else
    case "$XO_BACKLOG_META_SPAWN_GEN" in
      legacy-*)
        ...
        if [ "$LEGACY_RECORD_GIVEN" != 1 ]; then
          echo "error: task $ID's record carries the legacy incarnation stamp $XO_BACKLOG_META_SPAWN_GEN left by an abandoned --legacy-record teardown, not an incarnation published by a spawn; refusing automatic teardown - relaunch the task to publish an unambiguous incarnation, then retry teardown, or pass --legacy-record once its recorded endpoint is confirmed dead or agent-less" >&2
          exit 1
        fi
        TEARDOWN_LEGACY_PENDING=1
        TEARDOWN_LEGACY_RETAINED_STAMP=$XO_BACKLOG_META_SPAWN_GEN
        ;;
    esac
  fi
  [ "$TEARDOWN_LEGACY_PENDING" = 1 ] || TEARDOWN_META_SPAWN_GEN=$XO_BACKLOG_META_SPAWN_GEN
fi
...
if [ "$TEARDOWN_BACKLOG_APPLIES" = 1 ]; then
  ...
  TEARDOWN_CAPTAIN_OPEN_OUT=$(XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
    XO_DATA_OVERRIDE="$DATA" XO_CONFIG_OVERRIDE="$CONFIG" \
    "$SCRIPT_DIR/xo-captain-hold.sh" open "$ID" 2>&1) || TEARDOWN_CAPTAIN_OPEN_STATUS=$?
...
handoff_wake_retire_validate() {
  ...
  if [ -n "$corr" ]; then
    rec=$(xo_pending_reply_path "$STATE" "$corr")
    if [ -e "$rec" ] || [ -L "$rec" ]; then
      [ -f "$rec" ] && [ ! -L "$rec" ] \
        && [ "$(xo_pending_reply_get "$rec" task_id)" = "$ID" ] || {
        echo "REFUSED: receiver wake correlation for secondmate $ID is unsafe or belongs to another task" >&2
        return 1
      }
    fi
    confirmation=$(xo_pending_reply_delivery_confirmation_path "$STATE" "$corr")
...
handoff_wake_retire() {
  ...
  if [ -n "$corr" ]; then
    ...
    if { [ ! -e "$rec" ] && [ ! -L "$rec" ]; } \
      || { [ -f "$rec" ] && [ ! -L "$rec" ] \
        && [ "$(xo_pending_reply_get "$rec" task_id)" = "$ID" ]; }; then
...
handoff_wake_retire_stage_restore() {
  ...
  if [ -n "$HANDOFF_WAKE_RETIRE_CORR" ]; then
    rec=$(xo_pending_reply_path "$STATE" "$HANDOFF_WAKE_RETIRE_CORR")
    confirmation=$(xo_pending_reply_delivery_confirmation_path "$STATE" "$HANDOFF_WAKE_RETIRE_CORR")
  fi
  ...
  if [ -n "$HANDOFF_WAKE_RETIRE_LOCK" ]; then
    xo_lock_release "$HANDOFF_WAKE_RETIRE_LOCK" || return 1
    HANDOFF_WAKE_RETIRE_LOCK=
  fi
  HANDOFF_WAKE_RETIRE_STAGE=
}
...
handoff_wake_retire_stage_recover() {
  ...
  if [ -n "$corr" ]; then
    ...
    xo_lock_acquire_wait "$HANDOFF_WAKE_RETIRE_LOCK" || return 1
  fi
...
handoff_wake_retire_stage() {
  ...
  if [ -n "$corr" ]; then
    HANDOFF_WAKE_RETIRE_LOCK="$STATE/.pending-reply-$corr.lock"
    xo_lock_acquire_wait "$HANDOFF_WAKE_RETIRE_LOCK" || {
      HANDOFF_WAKE_RETIRE_LOCK=
...
remote_teardown_locks_release() {
  if [ -n "$REMOTE_REPLY_LIFECYCLE_LOCK" ]; then
    xo_lock_release "$REMOTE_REPLY_LIFECYCLE_LOCK"
    REMOTE_REPLY_LIFECYCLE_LOCK=
  fi
  if [ -n "$REMOTE_HANDOFF_LOCK" ]; then
    xo_lock_release "$REMOTE_HANDOFF_LOCK"
    REMOTE_HANDOFF_LOCK=
  fi
  if [ -n "$REMOTE_REGISTRY_LOCK" ]; then
    xo_lock_release "$REMOTE_REGISTRY_LOCK"
    REMOTE_REGISTRY_LOCK=
  fi
}
...
remote_pending_replies_cleanup() {
  ...
  (
    ...
    for rec in ./*; do
      [ -e "$rec" ] || [ -L "$rec" ] || continue
      [ -f "$rec" ] && [ ! -L "$rec" ] || exit 1
      [ "$(xo_meta_get "$rec" task_id)" = "$ID" ] && rm -f -- "$rec"
    done
  )
}
...
remote_secondmate_teardown() {
  ...
  remote_root=$(xo_meta_get "$META" remote_root)
  remote_home=$(xo_meta_get "$META" home)
  ...
  if [ "$FORCE" != --force ] && [ -d "$STATE/pending-replies" ]; then
    for rec in "$STATE/pending-replies"/*; do
      [ -f "$rec" ] || continue
      task_id=$(xo_meta_get "$rec" task_id)
      [ "$task_id" = "$ID" ] || continue
      phase=$(xo_meta_get "$rec" phase)
      [ "$phase" = resolved ] || {
  ...
  "$SCRIPT_DIR/xo-procevent-remote-reply.sh" retire-quiesce-locked "$ID" "$FORCE" >/dev/null 2>&1 || {
    echo "REFUSED: remote secondmate $ID still has an unhandled captured reply" >&2
    return 1
  }
  "$XO_ROOT/bin/xo-guard.sh" || true
  if [ "$FORCE" = --force ]; then
    if out=$("$SCRIPT_DIR/xo-on.sh" "$ID" xo-remote-secondmate-control.sh retire "$ID" --force < /dev/null 2>&1); then rc=0; else rc=$?; fi
  else
    if out=$("$SCRIPT_DIR/xo-on.sh" "$ID" xo-remote-secondmate-control.sh retire "$ID" < /dev/null 2>&1); then rc=0; else rc=$?; fi
  fi
  if [ "$rc" -ne 0 ]; then
    [ -z "$out" ] || printf '%s\n' "$out" >&2
    if [ "$rc" -eq 255 ]; then
      echo "error: remote retirement completion is unknown; preserving the route and local records for same-host reconciliation" >&2
    elif ! "$SCRIPT_DIR/xo-procevent-remote-reply.sh" arm-locked "$ID" >/dev/null 2>&1; then
  ...
  "$SCRIPT_DIR/xo-procevent-remote-reply.sh" retire-finalize-locked "$ID" "$FORCE" >/dev/null 2>&1 || {
  ...
  xo_backlog_atomic_transition remove "$STATE/$ID.meta" "task record" "$STATE" || return 1
...
# complete before xo-guard, a backend command, file removal, branch deletion,
# worktree return, registry change, or process termination can run.
xo_backend_validate_task_endpoint "$META" "$ID" || exit 1
BACKEND=$XO_BACKEND_VALIDATED_BACKEND
T=$XO_BACKEND_VALIDATED_TARGET
WT=$(xo_meta_get "$META" worktree)
PROJ=$(xo_meta_get "$META" project)
T_ORCA=
[ "$BACKEND" != orca ] || T_ORCA=$T
if [ "${XO_TEARDOWN_GUARD_DONE:-0}" != 1 ]; then
  "$XO_ROOT/bin/xo-guard.sh" || true
fi
HOME_PATH=$(grep '^home=' "$META" | cut -d= -f2- || true)
PR_URL=$(grep '^pr=' "$META" | tail -1 | cut -d= -f2- || true)
# tasktmp is recorded by xo-spawn for tasks that set up a per-task temp root
# (/tmp/xo-<id>/); absent for tasks spawned before that change, so tolerate empty.
TASK_TMP=$(grep '^tasktmp=' "$META" | cut -d= -f2- || true)
BUSY_GEN=$(xo_meta_get "$META" busy_gen)
if [ -z "$BUSY_GEN" ]; then
  BUSY_GEN=$(cat "$STATE/$ID.busy-gen" 2>/dev/null || true)
fi
ORCA_WORKTREE_ID=$(xo_meta_get "$META" orca_worktree_id)
...
if [ "$KIND" != secondmate ] && [ "$BACKEND" != orca ] \
   && xo_treehouse_pool_slot "$PROJ" "$WT"; then
  EXPECTED_TREEHOUSE_PROJECT_LOCK=$(xo_treehouse_project_lock_path "$PROJ") || {
...
if [ "$TEARDOWN_LEGACY_PENDING" = 1 ]; then
  TEARDOWN_LEGACY_ENDPOINT=$(xo_backend_agent_state "$BACKEND" "$T")
  case "$TEARDOWN_LEGACY_ENDPOINT" in
    ...
    *)
      ...
      echo "Reconcile the endpoint first (bin/xo-crew-state.sh $ID), or relaunch the task to publish an unambiguous incarnation, then retry teardown." >&2
      exit 1
      ;;
  esac
...
public_followup_resolve_primary_home() {
  ...
  [ "$(xo_meta_get "$parent_meta" kind)" = secondmate ] || return 1
  meta_home=$(xo_meta_get "$parent_meta" home)
...
if [ -f "$XO_HOME/$SUB_HOME_MARKER" ]; then
  SECOND_MATE_ID=$(sed -n '1p' "$XO_HOME/$SUB_HOME_MARKER")
  ...
  PARENT_ROUTE_FILE="$XO_HOME/$SUB_HOME_PARENT_MARKER"
  PARENT_ROUTE_RECORD=absent
  PARENT_ROUTE=
  PARENT_ROUTE_HOME=
  if [ -e "$PARENT_ROUTE_FILE" ] || [ -L "$PARENT_ROUTE_FILE" ]; then
    PARENT_ROUTE_RECORD=invalid
    if xo_secondmate_parent_record_parse "$PARENT_ROUTE_FILE"; then
      PARENT_ROUTE=$XO_SECONDMATE_PARENT_ROUTE
      PARENT_ROUTE_HOME=$XO_SECONDMATE_PARENT_HOME
  ...
  if [ "$PARENT_ROUTE_RECORD" = invalid ]; then
    ...
    # construction (bin/xo-public-followup-emit.sh header): a parent recorded
    ...
    PUBLIC_FOLLOWUP_PARENT_UNRESOLVED=1
    PRIMARY_HOME_CANDIDATE=${XO_PUBLIC_FOLLOWUP_PRIMARY_HOME:-$PARENT_ROUTE_HOME}
    PARENT_BINDINGS_MATCH=1
    if [ -n "${XO_PUBLIC_FOLLOWUP_PRIMARY_HOME:-}" ]; then
      LIVE_PARENT_HOME=$(public_followup_canonical_home \
        "$XO_PUBLIC_FOLLOWUP_PRIMARY_HOME") || PARENT_BINDINGS_MATCH=0
    ...
    PRIMARY_HOME_CANDIDATE=${XO_PUBLIC_FOLLOWUP_PRIMARY_HOME:-}
    if [ -n "$PRIMARY_HOME_CANDIDATE" ]; then
      if xo_pf_relay_active "$PRIMARY_HOME_CANDIDATE"; then
    ...
    if [ "$PUBLIC_FOLLOWUP_PARENT_RELAY_ACTIVE" = 1 ]; then
      PUBLIC_FOLLOWUP_PARENT_UNRESOLVED=1
      if xo_pf_home_id_valid "secondmate:$SECOND_MATE_ID"; then
        PUBLIC_FOLLOWUP_WORK_HOME="secondmate:$SECOND_MATE_ID"
        if PUBLIC_FOLLOWUP_HOME=$(public_followup_resolve_primary_home \
            "$PRIMARY_HOME_CANDIDATE" "$XO_HOME" "$SECOND_MATE_ID"); then
          PUBLIC_FOLLOWUP_STATE="$PUBLIC_FOLLOWUP_HOME/state"
  ...
  if [ "$FORCE" != "--force" ] && xo_pf_relay_active "$XO_HOME"; then
    PUBLIC_FOLLOWUP_RELAY_ACTIVE=1
  fi
elif [ "$FORCE" != "--force" ] && xo_pf_relay_active "$XO_HOME"; then
  PUBLIC_FOLLOWUP_RELAY_ACTIVE=1
fi
...
# Where a harness's xo-owned global turn-end registry entry lives is
# owned by bin/xo-control-lib.sh, so teardown and the control plane's relaunch
...
remove_grok_turnend_auth() {
  ...
  token_path=$(xo_control_harness_turnend_token_path grok "$state_dir" "$id") || return 1
  if [ -n "$token_path" ] && [ -f "$token_path" ]; then
    IFS= read -r token < "$token_path" || [ -n "$token" ] || return 1
  fi
  path=$(xo_control_harness_turnend_auth_path grok "$token") || return 1
  [ -n "$path" ] || return 0
  rm -f -- "$path"
}
...
remove_kimi_turnend_auth() {
  local state_dir=$1 id=$2 token_path token='' path
  token_path=$(xo_control_harness_turnend_token_path kimi "$state_dir" "$id") || return 1
  if [ -n "$token_path" ] && [ -f "$token_path" ]; then
    IFS= read -r token < "$token_path" || [ -n "$token" ] || return 1
  fi
  path=$(xo_control_harness_turnend_auth_path kimi "$token") || return 1
  [ -n "$path" ] || return 0
  rm -f -- "$path"
}
...
retire_busy_state() {
  ...
  if [ -n "$gen" ]; then
    "$SCRIPT_DIR/xo-busy-event.sh" retire "$state_dir" "$id" --gen "$gen"
  elif [ -f "$state_dir/$id.busy-gen" ]; then
    "$SCRIPT_DIR/xo-busy-event.sh" retire "$state_dir" "$id" --current-gen
  fi
}

validate_pr_poll_cleanup() {
  local state_dir=$1 id=$2 state_device artifact has_artifact=0
  xo_task_id_path_safe "$id" || return 0
  ...
  state_device=$(xo_pr_file_device "$state_dir") || return 1
  for artifact in "$state_dir/$id.check.sh" "$state_dir/$id.pr-poll" \
    "$state_dir/$id.pr-poll-registration" "$state_dir/$id.pr-poll-retirement" \
    "$state_dir/$id.check-trust"; do
    [ -e "$artifact" ] || [ -L "$artifact" ] || continue
  ...
  if [ -e "$state_dir/$id.pr-poll-retirement" ] \
    ...
    xo_pr_poll_retirement_state_valid "$state_dir" "$id" || {
...
remove_pr_poll_artifacts() {
  ...
  xo_pr_poll_retirement_recover_one "$state_dir" "$id" "$SCRIPT_DIR/xo-pr-poll.sh" || return 1
  xo_pr_poll_merge_notified_remove "$state_dir" "$id" || return 1
...
backlog_done_args() {
  ...
  case "$KIND" in
    scout)
      data_relative=$(xo_backlog_data_relative "$DATA") || return 1
...
# printed instruction for a later turn (bin/xo-backlog-transition-lib.sh owns the
...
backlog_refresh_reminder() {
  ...
  if root=$(xo_backlog_root "$DATA"); then
    backend=$(xo_tasks_axi_backend "$root") || return 2
  fi
  if [ "$backend" != markdown ]; then
    backlog_display="this home's configured tasks-axi backend (data directory $DATA)"
  elif backlog_display=$(xo_backlog_file "$DATA"); then
    :
  else
    backlog_display="${DATA%/}/backlog.md"
  fi
  if [ "$BACKLOG_CLOSED" = 1 ] && [ "$BACKLOG_TRANSITION" = retain ]; then
    printf '%s\n' "Backlog: $ID stays open in $backlog_display, still held for the captain with its deliverable recorded. Relay the question and close it only with bin/xo-captain-hold.sh answer."
...
STALE_WORKTREE_LOCK_AGE_SECS=${XO_STALE_WORKTREE_LOCK_AGE_SECS:-30}
# Bounded patience window for transient index.lock after killing a crew process.
# New knobs are preferred; XO_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS remains an alias
# for the per-attempt wait so existing tests and operators keep working.
TREEHOUSE_RETURN_LOCK_RETRIES=${XO_TREEHOUSE_RETURN_LOCK_RETRIES:-3}
TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS=${XO_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS:-${XO_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS:-1}}
...
# is owned by bin/xo-lock-lib.sh's xo_lock_is_provably_stale, sourced above.
...
cleanup_stale_lock_for_safety_check() {
  ...
  if xo_lock_is_provably_stale "$lock" "$dir" "$STALE_WORKTREE_LOCK_AGE_SECS"; then
    rm -f "$lock"
...
validate_worktree_teardown_safety() {
  ...
  dirty=$(printf '%s\n' "$dirty_raw" | grep -vE '^\?\? (\.claude/|\.xo-(grok|kimi)-turnend$)' | head -1 || true)
  ...
  if [ -n "$unpushed" ] && [ "$MODE" = local-only ]; then
    ...
    if [ -n "$dirty" ] || [ -n "$unmerged" ]; then
      ...
      echo "Merge the branch into local $DEFAULT first (bin/xo-merge-local.sh after the captain approves), or push to a fork/remote, or get the captain's explicit OK to discard, then --force." >&2
      return 1
    fi
  elif [ -n "$dirty" ]; then
...
# binds through the strict object-local head rule, with bin/xo-nm-run-lib.sh's
# shared ledger-anchored continuation rule as the only recognition for a head
# this copy cannot resolve at all.
NM_TEARDOWN_TIMEOUT=${XO_TEARDOWN_NM_TIMEOUT:-10}
...
# continuation proof may scan, mirroring bin/xo-crew-state.sh's limit posture
# (generous: rows of other branches interleave freely in the real ledger).
NM_TEARDOWN_RUNS_LIMIT=${XO_TEARDOWN_NM_RUNS_LIMIT:-200}
...
task_status_is_own_parked_run() {  # <worktree> <axi-status-output>
  ...
  [ -n "$branch" ] || return 1
  [ -n "$out" ] || return 1
  run_id=$(xo_nm_strip_quotes "$(xo_nm_field "$out" id)")
  [ -n "$run_id" ] || return 1
  run_branch=$(xo_nm_strip_quotes "$(xo_nm_field "$out" branch)")
  [ -n "$run_branch" ] && [ "$run_branch" = "$branch" ] || return 1
  run_head=$(xo_nm_strip_quotes "$(xo_nm_field "$out" head)")
  outcome=$(xo_nm_strip_quotes "$(xo_nm_field "$out" outcome)")
  [ -z "$outcome" ] || return 1
  status=$(xo_nm_strip_quotes "$(xo_nm_field "$out" status)")
  [ -n "$status" ] || return 1
  case "$status" in
    completed|failed|cancelled|passed|checks-passed|running|fixing|ci) return 1 ;;
  esac
  if ! xo_nm_head_matches_worktree "$wt" "$run_head"; then
    ...
    # bin/xo-nm-run-lib.sh owns the only remaining recognition, and it prints
    ...
    [ -n "$run_head" ] || return 1
    [ -z "$(xo_nm_resolve_commit "$wt" "$run_head")" ] || return 1
    ledger=$(xo_nm_run "$wt" "$NM_TEARDOWN_TIMEOUT" runs --limit "$NM_TEARDOWN_RUNS_LIMIT")
    [ "$(xo_nm_runs_status_for_worktree "$wt" "$branch" "$ledger" "$run_head")" = running ] || return 1
  fi
...
task_run_is_own_parked_run() {  # <worktree>
  ...
  out=$(xo_nm_run "$wt" "$NM_TEARDOWN_TIMEOUT" axi status)
...
# worktree (scouts and secondmates never do, mirroring bin/xo-crew-state.sh);
...
conclude_task_no_mistakes_run() {  # <worktree>
  ...
  xo_nm_run_checked "$wt" "$NM_TEARDOWN_TIMEOUT" axi abort --run "$run_id" >/dev/null 2>&1 || true
  if out=$(xo_nm_run_bounded "$wt" "$NM_TEARDOWN_TIMEOUT" axi status --run "$run_id" 2>&1); then
...
task_process_identity() {  # <pid>
  ...
  local -a stat_fields
  proc_root=${XO_PROC_ROOT_OVERRIDE:-/proc}
...
require_orca_worktree_path_match() {
  ...
  resolved=$(xo_backend_worktree_path orca "$worktree_id") || {
...
teardown_live_slot_path() {
  ...
  xo_treehouse_pool_slot "$PROJ" "$WT" || return 1
  canonical_existing_dir "$WT"
}

collect_local_xo_states() {
  local record_state=$1 root home reg line child known existing i=0
  local -a homes
  TREEHOUSE_OWNER_STATES=("$record_state")
  root=$(xo_xo_root_home "$XO_HOME") || {
    echo "REFUSED: cannot resolve the root XO home; nothing was changed" >&2
    return 1
  }
  homes=("$root")
  while [ "$i" -lt "${#homes[@]}" ]; do
    ...
    [ -f "$reg" ] && [ ! -L "$reg" ] || {
      echo "REFUSED: local XO registry is unsafe at $reg; nothing was changed" >&2
      return 1
    }
      ...
      case "$line" in
        "- "*)
          secondmate_registry_parse_line "$line" || {
            echo "REFUSED: malformed local XO registry entry in $reg; nothing was changed" >&2
            return 1
          }
          [ "$SECONDMATE_REGISTRY_REMOTE" -eq 0 ] || continue
          child=$(canonical_existing_dir "$SECONDMATE_REGISTRY_HOME") || {
            echo "REFUSED: registered local XO home is unavailable: $SECONDMATE_REGISTRY_HOME; nothing was changed" >&2
            return 1
          }
          known=0
...
require_exclusive_worktree_slot_record() {
  ...
  collect_local_xo_states "$record_state" || return 1
  for state_dir in "${TREEHOUSE_OWNER_STATES[@]}"; do
    for other in "$state_dir"/*.meta; do
      ...
      for field in worktree home; do
        other_path=$(xo_meta_get "$other" "$field")
        [ -n "$other_path" ] || continue
        other_slot=$(canonical_existing_dir "$other_path") || continue
        [ "$other_slot" = "$slot" ] || continue
        echo "REFUSED: task $record_id's recorded worktree $slot is also task $other_id's recorded $field." >&2
        echo "Returning that pool slot would kill $other_id's processes and reset its copy, so nothing was changed - not even with --force." >&2
        echo "Reconcile whichever record is wrong (bin/xo-crew-state.sh $record_id; bin/xo-crew-state.sh $other_id), then re-run teardown." >&2
        return 1
      done
    done
  done
...
# into the slot itself (bin/xo-wake-lib.sh owns the claim and its states).
#
...
# stays in XO_TREEHOUSE_SLOT_OWNER_ID and XO_TREEHOUSE_SLOT_OWNER_HOME.
#
...
require_owned_worktree_slot_record() {  # <task-id> <worktree>
  ...
  xo_treehouse_slot_owner_state "$worktree" "$record_id"
  case "$XO_TREEHOUSE_SLOT_OWNER" in
    mine|absent) return 0 ;;
    other)
      echo "warning: task $record_id's recorded worktree $worktree was reassigned to task $XO_TREEHOUSE_SLOT_OWNER_ID${XO_TREEHOUSE_SLOT_OWNER_HOME:+ (home $XO_TREEHOUSE_SLOT_OWNER_HOME)}, which claimed that pool slot after this record was written; that slot is no longer $record_id's, so its processes, copy, and claim are left untouched and only $record_id's own cleanup runs." >&2
      return "$TEARDOWN_SLOT_REASSIGNED_RC"
      ;;
  esac
  marker=$(xo_treehouse_slot_owner_marker "$worktree" 2>/dev/null) || marker="beside $worktree"
...
require_owned_task_worktree_slot() {
  ...
  case "$rc" in
    ...
    "$TEARDOWN_SLOT_REASSIGNED_RC")
      ...
      TEARDOWN_SLOT_REASSIGNED_TO=$XO_TREEHOUSE_SLOT_OWNER_ID
      TEARDOWN_SLOT_REASSIGNED_HOME=$XO_TREEHOUSE_SLOT_OWNER_HOME
      return 0
      ;;
  esac
  return 1
...
xo_home_has_treehouse_slot() {
  local home=$1
  worktree_registered_for_project "$XO_ROOT" "$home"
}
...
validate_removal_target() {
  ...
  [ -e "$target" ] || return 0
  abs_target=$(removal_target_abs_path "$target")
  if abs_home=$(cd "$XO_HOME" 2>/dev/null && pwd -P); then
    :
  else
    abs_home=
  fi
  abs_root=$(cd "$XO_ROOT" && pwd -P)
  case "$abs_target" in
    ''|/) echo "REFUSED: unsafe $label removal target $target" >&2; return 1 ;;
  esac
  if [ -n "$abs_home" ] && [ "$abs_target" = "$abs_home" ]; then
    echo "REFUSED: unsafe $label removal target $target is the active xo home" >&2
    return 1
  fi
  if [ "$abs_target" = "$abs_root" ]; then
    echo "REFUSED: unsafe $label removal target $target is the xo repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_target" "$abs_home"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the active xo home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_target" "$abs_root"; then
    echo "REFUSED: unsafe $label removal target $target is an ancestor of the xo repo" >&2
    return 1
  fi
  if [ -n "$abs_home" ] && path_is_ancestor_of "$abs_home" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the active xo home" >&2
    return 1
  fi
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe $label removal target $target is inside the xo repo" >&2
    return 1
  fi
  printf '%s\n' "$abs_target"
}
...
validate_xo_operational_dirs_for_removal() {
...
validate_child_worktree_for_removal() {
  ...
  if abs_home=$(cd "$XO_HOME" 2>/dev/null && pwd -P); then
    if path_is_ancestor_of "$abs_home" "$abs_target"; then
      echo "REFUSED: unsafe child worktree removal target $target is inside the active xo home" >&2
      return 1
    fi
  fi
  abs_root=$(cd "$XO_ROOT" && pwd -P)
  if path_is_ancestor_of "$abs_root" "$abs_target"; then
    echo "REFUSED: unsafe child worktree removal target $target is inside the xo repo" >&2
    return 1
  fi
...
validate_xo_home_for_removal() {
  ...
  validate_xo_operational_dirs_for_removal "$abs_home_path" "$label" || return 1
  conflict=
...
remove_xo_home() {
  ...
  [ -n "$home" ] || return 0
  [ -e "$home" ] || return 0
  abs_home_path=$(validate_xo_home_for_removal "$home" "$label" "$expected_id") || return 1
  [ -n "$abs_home_path" ] || return 0
  process_event_backup=$(snapshot_xo_home_process_events "$abs_home_path" "$label") || return 1
  if ! cleanup_xo_home_process_events "$abs_home_path" "$label"; then
    restore_xo_home_process_events "$abs_home_path" "$label" "$process_event_backup" || return $?
    return 1
  fi
  if xo_home_has_treehouse_slot "$abs_home_path"; then
    command -v treehouse >/dev/null 2>&1 || {
      echo "error: treehouse command not found; cannot return $label $abs_home_path" >&2
      restore_xo_home_process_events "$abs_home_path" "$label" "$process_event_backup" || return $?
      return 1
    }
    teardown_treehouse_return "$abs_home_path" "$XO_ROOT" "$label" || {
...
xo_home_has_process_events() {
  ...
  claim_root=${XO_PROCEVENT_CLAIM_ROOT:-${XDG_STATE_HOME:-$HOME/.local/state}/xo/procevent-claims}
...
snapshot_xo_home_process_events() {
  local home=$1 label=$2 backup path
  if ! xo_home_has_process_events "$home"; then
    printf '\n'
    return 0
  fi
  backup=$(umask 077; mktemp -d "${home%/*}/.xo-procevent-restore.XXXXXX") || {
...
restore_xo_home_process_events() {
  ...
  runner="$home/bin/xo-procevent.sh"
  if [ ! -f "$runner" ] || [ -L "$runner" ] || [ ! -x "$runner" ]; then
    runner="$SCRIPT_DIR/xo-procevent.sh"
  fi
  if ! XO_HOME="$home" XO_ROOT_OVERRIDE="$XO_ROOT" "$runner" reconcile >/dev/null; then
...
cleanup_xo_home_process_events() {
  local home=$1 label=$2 runner="$1/bin/xo-procevent.sh"
  xo_home_has_process_events "$home" || return 0
  if [ ! -f "$runner" ] || [ -L "$runner" ] || [ ! -x "$runner" ]; then
    echo "REFUSED: $label $home has process-event state but no sweep-capable bin/xo-procevent.sh; restore the home script and rerun teardown" >&2
    return 1
  fi
  if ! XO_HOME="$home" XO_ROOT_OVERRIDE="$home" "$runner" sweep-home; then
...
preflight_xo_home_process_events() {
  local home=$1 label=$2 runner="$1/bin/xo-procevent.sh"
  xo_home_has_process_events "$home" || return 0
  if [ ! -f "$runner" ] || [ -L "$runner" ] || [ ! -x "$runner" ]; then
    echo "REFUSED: $label $home has process-event state but no sweep-capable bin/xo-procevent.sh; restore the home script and rerun teardown" >&2
  ...
  if ! XO_HOME="$home" XO_ROOT_OVERRIDE="$home" "$runner" sweep-home --preflight >/dev/null; then
...
preflight_xo_home_process_event_tree() {
  ...
  if [ -d "$sub_state" ]; then
    for child_meta in "$sub_state"/*.meta; do
      ...
      preflight_xo_home_process_event_tree "$child_home" "child xo home for $child_id" || return 1
    done
  fi
  preflight_xo_home_process_events "$home" "$label"
}
...
collect_descendant_task_locks() {
  ...
  # without ever having been lifecycle-locked (bin/xo-wake-lib.sh's
  # xo_task_set_lock_path owns why). Taken per home, parent before child, and
  # held until this teardown exits.
  task_set_lock=$(xo_task_set_lock_path "$sub_state") || {
    echo "REFUSED: secondmate home $home has an invalid task-set lock path; forced teardown changed nothing" >&2
    return 1
  }
  if ! xo_lock_try_acquire "$task_set_lock"; then
...
preflight_descendant_task_locks() {
  ...
  # back for a parent lock. bin/xo-spawn.sh takes the same task-set lock before
  ...
  for ((i=0; i < ${#DESCENDANT_TASK_IDS[@]}; i++)); do
    ...
    if ! xo_lock_try_acquire "$meta_lock"; then
...
preflight_descendant_treehouse_slots() {
  ...
  for ((i=0; i < ${#DESCENDANT_TASK_IDS[@]}; i++)); do
    ...
    if ! xo_treehouse_pool_slot "$project" "$worktree"; then
      continue
    fi
    lock_path=$(xo_treehouse_project_lock_path "$project") || {
  ...
  for ((i=0; i < ${#DESCENDANT_TASK_IDS[@]}; i++)); do
    ...
    xo_backend_validate_task_endpoint "$meta" "$task_id" || return 1
...
validate_xo_home_children_removal() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    [ -e "$child_meta" ] || continue
    child_id=$(basename "$child_meta" .meta)
    xo_backend_validate_task_endpoint "$child_meta" "$child_id" || return 1
    validate_pr_poll_cleanup "$sub_state" "$child_id" || return 1
    child_wt=$(meta_value "$child_meta" worktree)
    child_kind=$(meta_value "$child_meta" kind)
    [ -n "$child_kind" ] || child_kind=ship
    child_backend=$(xo_backend_of_meta "$child_meta")
    if [ "$child_kind" = secondmate ]; then
      child_home=$(meta_value "$child_meta" home)
      [ -n "$child_home" ] || child_home=$child_wt
      validate_xo_home_for_removal "$child_home" "child xo home" "$child_id" >/dev/null || return 1
      validate_xo_home_children_removal "$child_home" || return 1
...
teardown_herdr_require_prerequisites() {  # <task-id>
  ...
  if ! xo_backend_source herdr; then
  ...
  for prerequisite in \
    xo_backend_herdr_parse_target \
    xo_backend_herdr_pane_presence_state \
    xo_backend_herdr_workspace_presence_state \
    xo_backend_herdr_endpoint_confirmed_gone \
    xo_backend_herdr_explicit_close_pane_confirmed \
    xo_backend_herdr_presentation_session_lock_path; do
  ...
  if ! declare -F xo_lock_try_acquire >/dev/null 2>&1 \
    || ! declare -F xo_lock_release >/dev/null 2>&1; then
...
teardown_herdr_preflight_target() {  # <target> <task-id>
  ...
  if ! xo_backend_herdr_parse_target "$target"; then
  ...
  pane=$XO_BACKEND_HERDR_PANE
  presence=$(xo_backend_herdr_pane_presence_state "$session" "$pane")
  case "$presence" in
    dead|present) ;;
    *)
...
FMEOF
  ...
  while [ "$attempt" -lt 50 ]; do
    if xo_lock_try_acquire "$lock_path"; then
      if ! verified_lock_path=$(xo_backend_herdr_presentation_session_lock_path "$session") \
...
preflight_xo_home_herdr_children() {  # <home>
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    child_id=$(basename "$child_meta" .meta)
    xo_backend_validate_task_endpoint "$child_meta" "$child_id" || return 1
    child_backend=$XO_BACKEND_VALIDATED_BACKEND
    child_target=$XO_BACKEND_VALIDATED_TARGET
    ...
    if [ "$child_kind" = secondmate ]; then
      ...
      child_home=$(meta_value "$child_meta" home)
      [ -n "$child_home" ] || child_home=$child_wt
      preflight_xo_home_herdr_children "$child_home" || return 1
    fi
  done
}

cleanup_xo_home_children() {
  ...
  for child_meta in "$sub_state"/*.meta; do
    ...
    if [ "$child_backend" = orca ]; then
      ...
      child_t=$(xo_backend_target_of_meta "$child_meta")
    fi
    ...
    if [ -n "$child_t" ]; then
      if [ "$child_backend" = herdr ]; then
        xo_backend_herdr_parse_target "$child_t" || return 1
        if ! teardown_herdr_session_lock_held "$XO_BACKEND_HERDR_SESSION"; then
        ...
        xo_backend_herdr_kill_serialized "$XO_BACKEND_HERDR_SESSION" "$XO_BACKEND_HERDR_PANE" 2>/dev/null || true
        if ! xo_backend_herdr_endpoint_confirmed_gone "$child_t"; then
        ...
        ( unset XO_ROOT_OVERRIDE; XO_HOME=$home XO_ROOT=$home xo_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "xo-$child_id" ) 2>/dev/null || true
      else
        xo_backend_kill "$child_backend" "$child_t" "$(meta_value "$child_meta" zellij_tab_id)" "xo-$child_id" 2>/dev/null || true
      fi
    fi
    if [ "$child_kind" = secondmate ]; then
      child_home=$(meta_value "$child_meta" home)
      ...
      if [ -n "$child_home" ] && [ -d "$child_home" ]; then
        cleanup_xo_home_children "$child_home" || return $?
        remove_xo_home "$child_home" "child xo home" "$child_id" || return $?
      fi
    elif [ "$child_backend" = orca ]; then
      if [ -n "$child_wt" ] && [ -d "$child_wt" ]; then
        validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
        rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/xo-turn-end.js" \
          "$child_wt/.xo-grok-turnend" "$child_wt/.xo-kimi-turnend"
      fi
      xo_backend_remove_worktree "$child_backend" "$child_orca_worktree_id" || return 1
      ...
      child_owner_rc=0
      if xo_treehouse_pool_slot "$child_proj" "$child_wt"; then
      ...
      if [ "$child_owner_rc" -eq "$TEARDOWN_SLOT_REASSIGNED_RC" ]; then
        ...
        validate_child_worktree_for_removal "$child_wt" "$child_proj" >/dev/null || return 1
        rm -f "$child_wt/.claude/settings.local.json" "$child_wt/.opencode/plugins/xo-turn-end.js" \
          "$child_wt/.opencode/plugins/xo-busy-state.js" \
          "$child_wt/.xo-grok-turnend" "$child_wt/.xo-kimi-turnend"
        if [ -n "$child_proj" ] && [ -d "$child_proj" ] && command -v treehouse >/dev/null 2>&1; then
          if teardown_treehouse_return "$child_wt" "$child_proj" "child worktree"; then
            xo_treehouse_slot_owner_release "$child_wt" "$child_id"
          else
            child_return_rc=$?
    ...
    xo_backlog_atomic_transition remove "$sub_state/$child_id.meta" "task record" "$sub_state" || return 1
...
remove_secondmate_registry_entry() {
  ...
  [ "$acquired" -eq 0 ] || xo_lock_release "$lock"
  return "$rc"
}
...
if [ "$KIND" = secondmate ]; then
  ...
  xo_lock_acquire_wait "$LOCAL_REGISTRY_LOCK" || exit 1
  LOCAL_HANDOFF_LOCK="$STATE/.backlog-handoff-$ID.lock"
  xo_lock_acquire_wait "$LOCAL_HANDOFF_LOCK" || exit 1
  [ -n "$HOME_PATH" ] || HOME_PATH=$WT
  handoff_wake_retire_stage_recover "$HOME_PATH" || exit 1
  handoff_wake_retire_validate || exit 1
  validate_xo_home_for_removal "$HOME_PATH" "secondmate home" "$ID" >/dev/null || exit 1
  if [ "$FORCE" = "--force" ]; then
    validate_xo_home_children_removal "$HOME_PATH" || exit 1
    ...
    preflight_xo_home_herdr_children "$HOME_PATH" || exit 1
  fi
fi
...
if [ "$KIND" = secondmate ]; then
  preflight_xo_home_process_event_tree "$HOME_PATH" "secondmate home" || exit 1
fi

if [ "$KIND" = secondmate ] && [ "$FORCE" = "--force" ]; then
  cleanup_xo_home_children "$HOME_PATH" || exit $?
fi
...
if [ "$KIND" = scout ] && [ "$FORCE" != "--force" ]; then
  ...
  if ! XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" XO_DATA_OVERRIDE="$DATA" \
      XO_CONFIG_OVERRIDE="$CONFIG" "$SCRIPT_DIR/xo-captain-hold.sh" verify "$ID" >/dev/null; then
    echo "REFUSED: scout task $ID has not passed the captain-call completion gate." >&2
    echo "Inventory its report and any visual review through bin/xo-captain-hold.sh before teardown." >&2
    exit 1
  fi
fi
...
if [ "$KIND" = ship ] && [ -n "$PR_URL" ] \
    ...
    && xo_pf_has_delivered_open_loops "$PUBLIC_FOLLOWUP_STATE"; then
  echo "warning: an open public loop with nothing owed is still recorded in the consent-holding home while cleaning up ship task $ID. Hand it on with bin/xo-public-followup.sh rechain or close it with retire --reason." >&2
fi
...
if [ "$BACKEND" = herdr ]; then
  teardown_herdr_preflight_target "$T" "$ID" || exit 1
  xo_backend_herdr_parse_target "$T" || exit 1
  TEARDOWN_HERDR_SESSION=$XO_BACKEND_HERDR_SESSION
  TEARDOWN_HERDR_PANE=$XO_BACKEND_HERDR_PANE
fi
...
teardown_legacy_stamp_rollback() {
  ...
  if [ "$TEARDOWN_LEGACY_ACCEPTED" = 1 ] && [ -z "$TEARDOWN_LEGACY_RETAINED_STAMP" ]; then
    ...
    if [ -z "$TEARDOWN_LEGACY_STAMP_FAILED" ] \
       && ! xo_backlog_meta_spawn_gen "$META" "$STATE"; then
    ...
    if [ -n "$TEARDOWN_LEGACY_STAMP_FAILED" ]; then
      ...
      if [ "$TEARDOWN_LEGACY_STAMP_FAILED" = validate ]; then
        echo "error: the stamped legacy incarnation does not validate for $ID ($XO_BACKLOG_TRANSITION_ERROR); refusing destructive teardown" >&2
      else
  ...
  if ! xo_backlog_close_marker_write "$STATE" "$ID" "$DATA" "$META_SPAWN_GEN" \
    ...
    if [ "$TEARDOWN_LEGACY_ACCEPTED" = 1 ] && [ -z "$TEARDOWN_LEGACY_RETAINED_STAMP" ] \
      ...
      echo "error: the pending backlog $BACKLOG_TRANSITION for $ID could not be recorded ($XO_BACKLOG_TRANSITION_ERROR); the accepted legacy incarnation was rolled back, retaining every durable task record" >&2
    else
      echo "error: the pending backlog $BACKLOG_TRANSITION for $ID could not be recorded ($XO_BACKLOG_TRANSITION_ERROR); retaining every durable task record" >&2
      if [ "$TEARDOWN_LEGACY_ACCEPTED" = 1 ] && [ -z "$TEARDOWN_LEGACY_RETAINED_STAMP" ]; then
        echo "error: the legacy incarnation stamp on $ID's record could not be rolled back; re-run teardown with --legacy-record after reconciling its endpoint" >&2
      fi
    fi
...
# dedicated process-event and xo-home removal machinery further below,
...
"$SCRIPT_DIR/xo-remote-job-reap-orphans.sh" >&2 || true
...
if [ "$BACKEND" = orca ] && [ "$KIND" != secondmate ]; then
  ...
  if [ -d "$WT" ]; then
    ...
    rm -f "$WT/.claude/settings.local.json" "$WT/.opencode/plugins/xo-turn-end.js" \
      "$WT/.opencode/plugins/xo-busy-state.js" \
      "$WT/.xo-grok-turnend" "$WT/.xo-kimi-turnend"
  fi
  [ -z "$T_ORCA" ] || xo_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "xo-$ID" 2>/dev/null || true
  xo_backend_remove_worktree "$BACKEND" "$ORCA_WORKTREE_ID"
  ...
  xo_treehouse_slot_owner_release "$WT" "$ID"
fi
...
if [ "$BACKEND" = herdr ] \
  ...
  if [ -n "$HERDR_PRESENTATION_SESSION" ] \
     ...
     && xo_backend_herdr_projection_endpoint_matches_journal \
...
if [ "$HERDR_PRESENTATION_RETIRE_CANDIDATE" = 1 ]; then
  ...
  if teardown_herdr_session_lock_held "$TEARDOWN_HERDR_SESSION"; then
    xo_backend_herdr_kill_serialized "$TEARDOWN_HERDR_SESSION" "$TEARDOWN_HERDR_PANE" 2>/dev/null || true
  else
    echo "warning: herdr session presentation lock path is unavailable; skipping the pane close rather than closing unlocked" >&2
  fi
elif [ "$BACKEND" != orca ]; then
  xo_backend_kill "$BACKEND" "$T" "$(meta_value "$META" zellij_tab_id)" "xo-$ID" 2>/dev/null || true
fi
if [ "$HERDR_PRESENTATION_RETIRE_CANDIDATE" = 1 ]; then
  if [ "$(xo_backend_herdr_pane_agent_state "$HERDR_PRESENTATION_SESSION" "$HERDR_PRESENTATION_PANE")" = dead ]; then
...
if [ "$BACKEND" = herdr ]; then
  xo_backend_source herdr || true
  if ! declare -F xo_backend_herdr_endpoint_confirmed_gone >/dev/null 2>&1; then
    echo "error: herdr endpoint confirmation is unavailable for $ID; retaining every durable task record" >&2
    exit 1
  fi
  if ! xo_backend_herdr_endpoint_confirmed_gone "$T"; then
...
if [ "$KIND" != secondmate ]; then
  if ! XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" XO_DATA_OVERRIDE="$DATA" \
      "$SCRIPT_DIR/xo-inactive-reconcile.sh" report "$ID"; then
...
if [ "$KIND" = secondmate ]; then
  ...
  if remove_xo_home "$HOME_PATH" "secondmate home" "$ID"; then
    :
  else
    rc=$?
    handoff_wake_retire_stage_restore \
...
xo_backend_clear_transition "$BACKEND" "$STATE" "$T" || true
# Remove the per-task temp root (/tmp/xo-<id>/, incl. its gotmp/) recorded by spawn.
...
# The steering inbox (bin/xo-task-inbox-lib.sh) is runtime state for the
...
if [ "$BACKLOG_CLOSED" = 1 ]; then
  BACKLOG_CLOSE_MARKER=$(xo_backlog_close_marker_path "$STATE" "$ID") || exit 1
  if ! xo_backlog_atomic_transition "$BACKLOG_TRANSITION" "$STATE/$ID.meta" "$BACKLOG_CLOSE_MARKER" \
      "$DATA" "$ID" "$STATE" "${BACKLOG_DONE_ARGS[@]+"${BACKLOG_DONE_ARGS[@]}"}"; then
    xo_lock_release "$META_LOCK"
    META_LOCK_HELD=0
    if [ "$BACKLOG_TRANSITION" = retain ]; then
      echo "error: $ID's endpoint and local copy are cleaned up, but its captain-held backlog item could not be returned to Queued atomically ($XO_BACKLOG_TRANSITION_ERROR); the pending retention is recorded and the next session start retries it" >&2
    else
      echo "error: $ID's endpoint and local copy are cleaned up, but its backlog item could not be closed atomically ($XO_BACKLOG_TRANSITION_ERROR); the pending close is recorded and the next session start retries it" >&2
    fi
    exit 1
  fi
elif [ "$KIND" = secondmate ] && [ ! -e "$STATE" ] && [ ! -L "$STATE" ]; then
  # A nested remote retirement can keep its route record inside the home being
  # removed. remove_xo_home above already performed that physical
  # deletion; do not turn its confirmed absence into a false cleanup failure.
  :
else
  if ! xo_backlog_atomic_transition remove "$STATE/$ID.meta" "task record" "$STATE"; then
    xo_lock_release "$META_LOCK"
    META_LOCK_HELD=0
    echo "error: $ID's endpoint and local copy are cleaned up, but its task record could not be removed ($XO_BACKLOG_TRANSITION_ERROR)" >&2
    exit 1
  fi
fi
xo_lock_release "$META_LOCK"
...
if [ "$KIND" != scout ] && [ "$KIND" != secondmate ] && [ "$MODE" != local-only ]; then
  "$XO_ROOT/bin/xo-fleet-sync.sh" "$PROJ" || true
fi
