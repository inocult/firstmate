#!/usr/bin/env bash
# xo-timeout-lib.sh - the single owner of bounded command execution.
#
...
#
#   xo_timeout_mechanism
#       Prints the mechanism xo_run_timed will use on this host: "timeout",
#       "gtimeout", "perl", or "bash". Set XO_TIMEOUT_MECHANISM_OVERRIDE=bash
#       to force the dependency-free fallback.
#
#   xo_run_timed <seconds> <command> [args...]
...
xo_timeout_mechanism() {
  if [ "${XO_TIMEOUT_MECHANISM_OVERRIDE:-}" = bash ]; then
    printf 'bash\n'
...
xo_run_bash_timeout() {
  ...
  shift
  command_status=$(mktemp "${TMPDIR:-/tmp}/xo-bash-timeout-command.XXXXXX" 2>/dev/null) || return 124
...
xo_run_external_timeout() {
  local runner=$1 seconds=$2 status_file runner_pid runner_rc command_rc
  shift 2
  status_file=$(mktemp "${TMPDIR:-/tmp}/xo-timeout-status.XXXXXX" 2>/dev/null) || return 124
...
xo_run_timed() {  # <seconds> <command...>
  local seconds=$1
  shift
  case "$(xo_timeout_mechanism)" in
    timeout) xo_run_external_timeout timeout "$seconds" "$@" ;;
    gtimeout) xo_run_external_timeout gtimeout "$seconds" "$@" ;;
    perl)
      perl -e 'my $t = shift; my $pid = fork; die "fork failed" unless defined $pid; if (!$pid) { setpgrp(0, 0); exec @ARGV } local $SIG{ALRM} = sub { kill "TERM", -$pid; select undef, undef, undef, 0.2; kill "KILL", -$pid; exit 124 }; alarm $t; waitpid $pid, 0; exit($? >> 8)' \
        "$seconds" "$@"
      ;;
    bash) xo_run_bash_timeout "$seconds" "$@" ;;
    *) return 124 ;;
  esac
}
