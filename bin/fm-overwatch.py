#!/usr/bin/env python3
"""Persist scoped Plane auto-pickup and register its timer with Firstmate's watcher.

Usage: fm-overwatch.py on [--slots 2] [--interval 300] [--max-pickups 10]
       fm-overwatch.py status|check|off
       fm-overwatch.py defer --outcome picked|empty|busy|error

Requires FM_HOME and its configured Plane adapter. No Plane/GitHub writes or
...
from fm_plane.cli import load_config
from fm_plane.registry import AdapterError
...
def main():
    ...
    if not os.environ.get('FM_HOME'):
        raise AdapterError('FM_HOME must explicitly identify the Control home')
    ...
    with open(lock_path, 'a') as lock:
        ...
        now = int(time.time())
        env = dict(os.environ, FM_HOME=str(home), FM_STATE_OVERRIDE=str(state))
        if args.command == 'on':
            ...
            content = '#!/bin/sh\nexec env ' + shlex.join(['FM_HOME=' + str(home), 'FM_STATE_OVERRIDE=' + str(state)]) + ' ' + command + '\n'
            fd = os.open(shim, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o700)
            with os.fdopen(fd, 'w') as out:
                out.write(content)
            result = subprocess.run([str(ROOT / 'bin/fm-check-register.sh'), 'overwatch'], env=env, capture_output=True)
            ...
            save(policy_path, policy)
            result = subprocess.run([str(ROOT / 'bin/fm-check-unregister.sh'), 'overwatch'], env=env, capture_output=True)
            if result.returncode:
