...
XO_ROOT="${XO_ROOT_OVERRIDE:-$REPO_ROOT}"
...
usage() {
  cat <<'EOF'
Usage: xo-supervision-instructions.sh [--harness <name>] [--read-only 0|1] [--afk 0|1] [--x-mode 0|1] [--repair-line] [--queue-pending 0|1]
...
if [ -z "$HARNESS" ]; then
  HARNESS=$("$SCRIPT_DIR/xo-harness.sh" 2>/dev/null || printf unknown)
fi
...
checkpoint_seconds=${XO_CODEX_WATCH_CHECKPOINT:-180}
...
render_snippet() {
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    line=${line//__XO_PI_EXT__/$pi_ext}
    line=${line//__XO_PI_TURNEND_EXT__/$pi_turnend_ext}
    line=${line//__XO_OMP_EXT__/$omp_ext}
    line=${line//__XO_OMP_TURNEND_EXT__/$omp_turnend_ext}
    line=${line//__XO_X_MODE_ENV_SH__/$x_mode_env_sh}
    line=${line//__XO_X_MODE_ENV__/$x_mode_env}
    printf '%s\n' "$line"
  done < "$SNIPPET"
}
...
repair_line() {
  ...
  case "$HARNESS" in
    claude)
      ...
      ;;
    codex)
      printf '%s%s%s%s\n' "$prefix" 'repair missing watcher supervision with a foreground checkpoint: bin/xo-watch-checkpoint.sh --seconds ' "$checkpoint_seconds" '.'
      ;;
    pi|pi-signed)
      printf '%s%s%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the Pi tool xo_watch_arm_pi, or restart Pi with -e ' "$pi_turnend_ext" ' -e ' "$pi_ext" ' if the extensions are not loaded.'
      ;;
    omp)
      printf '%s%s%s%s%s%s\n' "$prefix" 'repair a missing or failed watcher cycle with the omp tool xo_watch_arm_omp, or restart omp inside this home so ' "$omp_turnend_ext" ' and ' "$omp_ext" ' auto-load from .omp/extensions/ (use -e with both paths only when starting omp from another directory).'
      ;;
    opencode)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision by letting the OpenCode TUI plugin arm after idle; use bin/xo-watch-arm.sh only as a manual recovery probe if the plugin reports failure.'
      ;;
    grok)
      printf '%s%s\n' "$prefix" 'repair missing watcher supervision with bin/xo-watch-arm.sh as its own Grok tracked background task, never shell &.'
      ;;
    cursor)
...
ordinary_wake_line() {
  case "$HARNESS" in
    claude)
      printf '%s\n' '- Ordinary wake: the Stop-owned auto-arm (bin/xo-claude-stop-autoarm.sh) already owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    codex)
      printf '%s\n' '- Ordinary wake: take the next foreground bin/xo-watch-checkpoint.sh checkpoint as directed below.'
      ;;
    pi|pi-signed)
    ...
    opencode)
      ...
      ;;
    grok)
      printf '%s\n' '- Ordinary wake: re-arm exactly one bin/xo-watch-arm.sh Grok tracked background task as directed below.'
      ;;
    cursor)
      printf '%s\n' '- Ordinary wake: the stop-hook park (bin/xo-turnend-guard-cursor.sh) already owns watcher continuity; drain and handle the wake, and do not arm another cycle yourself.'
      ;;
    *)
