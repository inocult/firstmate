...
#
# Usage: xo-secondmate-restart.sh <secondmate-id>... [--help]
#
# This is the executable half of /updatexo's reload step. A running agent
...
# with bin/xo-control.sh <id> relaunch; a remote mate is restarted by running THAT
# SAME command on its host over bin/xo-on.sh, through the host-local
# xo-remote-secondmate-control.sh relaunch verb. The restart decision, the
...
#
# Nothing here forces, stashes, or discards anything. bin/xo-control.sh owns the
...
#
# Restart candidacy itself belongs to bin/xo-update.sh, which knows which homes
...
#
# Environment knobs:
#   XO_SECONDMATE_PERSIST_WAIT  seconds to wait for one mate's persist answer (900)
#   XO_SECONDMATE_PERSIST_POLL  seconds between checks of that answer (5)
#
...
if [ -z "${XO_HOME:-}" ]; then
  echo "error: XO_HOME is not set; xo-secondmate-restart refuses to resolve second mates without an explicit xo home" >&2
  exit 1
fi
[ -d "$XO_HOME" ] || { echo "error: XO_HOME '$XO_HOME' is not a directory" >&2; exit 1; }
STATE="${XO_STATE_OVERRIDE:-$XO_HOME/state}"
[ -d "$STATE" ] || { echo "error: state dir '$STATE' is missing; xo-secondmate-restart cannot resolve second mates for XO_HOME '$XO_HOME'" >&2; exit 1; }

# shellcheck source=bin/xo-secondmate-restart-lib.sh
. "$SCRIPT_DIR/xo-secondmate-restart-lib.sh"
...
PERSIST_WAIT=${XO_SECONDMATE_PERSIST_WAIT:-900}
PERSIST_POLL=${XO_SECONDMATE_PERSIST_POLL:-5}
case "$PERSIST_WAIT" in ''|*[!0-9]*) echo "error: XO_SECONDMATE_PERSIST_WAIT must be a non-negative integer: $PERSIST_WAIT" >&2; exit 2 ;; esac
case "$PERSIST_POLL" in ''|*[!0-9]*|0) echo "error: XO_SECONDMATE_PERSIST_POLL must be a positive integer: $PERSIST_POLL" >&2; exit 2 ;; esac
...
for arg in "$@"; do
  ...
  # /updatexo's action line names each mate by its xo-<id> selector; the
...
fall_back_to_nudge() {  # <id> <reason>
  ...
  if out=$(XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
    "$SCRIPT_DIR/xo-send.sh" "$id" "$XO_SECOND_MATE_NUDGE_MESSAGE" 2>&1); then
...
restart_mate() {  # <array-index>
  ...
  if [ "${PLACEMENT[i]}" = remote ]; then
    restart_out=$(XO_HOME="$XO_HOME" "$SCRIPT_DIR/xo-on.sh" "$id" \
      xo-remote-secondmate-control.sh relaunch \
      "$id" "${HARNESS[i]}" "${MODEL[i]:-default}" "${EFFORT[i]:-default}" < /dev/null 2>&1)
    restart_rc=$?
  else
    restart_out=$(XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
      "$SCRIPT_DIR/xo-control.sh" "$id" relaunch 2>&1)
    restart_rc=$?
  fi
...
while [ "$i" -lt "${#IDS[@]}" ]; do
  ...
  MODEL[i]=""
  EFFORT[i]=""
  if ! xo_secondmate_restart_capable "$STATE/$id.meta"; then
    REASON[i]=$XO_SECONDMATE_RESTART_REASON
    i=$((i + 1))
    continue
  fi
  PLACEMENT[i]=$XO_SECONDMATE_RESTART_PLACEMENT
  HOST[i]=$XO_SECONDMATE_RESTART_HOST
  HARNESS[i]=$XO_SECONDMATE_RESTART_HARNESS
  if [ "${PLACEMENT[i]}" = remote ]; then
    ...
    HARNESS[i]=$("$SCRIPT_DIR/xo-harness.sh" secondmate 2>/dev/null || true)
    [ -n "${HARNESS[i]}" ] || HARNESS[i]=$XO_SECONDMATE_RESTART_HARNESS
    MODEL[i]=$("$SCRIPT_DIR/xo-harness.sh" secondmate-model 2>/dev/null || true)
    EFFORT[i]=$("$SCRIPT_DIR/xo-harness.sh" secondmate-effort 2>/dev/null || true)
    case "${EFFORT[i]}" in
      ''|low|medium|high|xhigh|max|ultra) ;;
      *) EFFORT[i]="" ;;
    esac
    if [ "${EFFORT[i]}" = ultra ] && ! "$SCRIPT_DIR/xo-harness.sh" validate-native-effort "${HARNESS[i]}" "${MODEL[i]}" "${EFFORT[i]}"; then
  ...
  if ! corr=$(xo_pending_reply_create "$XO_HOME" "$STATE" "$id" \
    "$XO_SECONDMATE_PERSIST_REQUEST"); then
    REASON[i]="its answer about the open work cannot be tracked, so a clean reload could not be proven"
    i=$((i + 1))
    continue
  fi
  if ! send_out=$(XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
    XO_PENDING_REPLY_EXISTING_CORR="$corr" \
    "$SCRIPT_DIR/xo-send.sh" "$id" "$XO_SECONDMATE_PERSIST_REQUEST" 2>&1); then
    xo_pending_reply_discard_undelivered "$STATE" "$corr" >/dev/null 2>&1 || true
...
while [ "$((pending_count + restart_active_count))" -gt 0 ]; do
  ...
  while [ "$i" -lt "${#IDS[@]}" ]; do
    if [ "${PLAN[i]}" = persisted-pending ] \
      && xo_pending_reply_try_resolve "$STATE" "${CORR[i]}"; then
      pending_count=$((pending_count - 1))
      launch_restart "$i"
    fi
    i=$((i + 1))
  ...
  while [ "$i" -lt "${#IDS[@]}" ]; do
    ...
    if [ "$now" -ge "${DEADLINE[i]}" ]; then
      ...
      if xo_pending_reply_try_resolve "$STATE" "${CORR[i]}"; then
        pending_count=$((pending_count - 1))
        launch_restart "$i"
      else
