...
#               bin/xo-send.sh so it sweeps its own uncaptured session
...
#               generic remote write path (bin/xo-remote-file.sh put reaches
...
# one hard bound (XO_STOW_CASCADE_TIMEOUT seconds, default 60), so a slow or
...
BUDGET_CMD=xo-startup-memory-budget.sh
...
BOUND=${XO_STOW_CASCADE_TIMEOUT:-60}
case "$BOUND" in
  ''|*[!0-9]*) printf 'error: XO_STOW_CASCADE_TIMEOUT must be a positive integer: %s\n' "$BOUND" >&2; exit 2 ;;
esac
[ "$BOUND" -gt 0 ] \
  || { printf 'error: XO_STOW_CASCADE_TIMEOUT must be a positive integer: %s\n' "$BOUND" >&2; exit 2; }

die() { printf 'error: %s\n' "$1" >&2; exit 1; }
emit() { printf '%s\n' "$1"; }

TMP=$(mktemp -d "${TMPDIR:-/tmp}/xo-stow-cascade.XXXXXX") || exit 1
...
run_step() {
  local rc=0
  xo_run_timed "$BOUND" "$@" < /dev/null > "$STEP_OUT" 2>/dev/null || rc=$?
  return "$rc"
}
...
resolve_local_transport() { # <id> <resolved-home>
  ...
  [ -n "$target" ] || target=$(xo_meta_get "$meta" window)
  if [ -z "$target" ]; then
    set_transport direct 'recorded endpoint has no target'
    return 0
  fi
  case "$(xo_backend_agent_state "$backend" "$target" 2>/dev/null || printf 'unreadable')" in
...
resolve_remote_transport() { # <id>
  ...
  run_step "$SCRIPT_DIR/xo-on.sh" "$id" xo-remote-secondmate-control.sh state "$id" || rc=$?
  if [ "$rc" -eq 124 ]; then
...
if [ -e "$XO_HOME/$SUB_HOME_MARKER" ] || [ -L "$XO_HOME/$SUB_HOME_MARKER" ]; then
...
while IFS= read -r line || [ -n "$line" ]; do
  ...
  if [ "$remote" -eq 1 ]; then
    ...
    emit "host=$host"
    emit "home=$home"
    run_step "$SCRIPT_DIR/xo-on.sh" "$id" "$BUDGET_CMD" report || rc=$?
  else
    ...
    emit "home=$resolved"
    # Every XO_*_OVERRIDE is restated so a caller's own override cannot leak
    # this home's memory files into the accounting of another home.
    run_step env \
      XO_ROOT_OVERRIDE="$XO_ROOT" \
      XO_HOME="$resolved" \
      XO_STATE_OVERRIDE="$resolved/state" \
      XO_DATA_OVERRIDE="$resolved/data" \
      XO_CONFIG_OVERRIDE="$resolved/config" \
