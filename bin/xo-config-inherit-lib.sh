...
# Inheritance propagation: the PRIMARY xo pushes a declared, extensible
...
#
# Usage: . bin/xo-config-inherit-lib.sh   (no XO_* setup required)
#
# Why this is separate from the tracked-files fast-forward (xo-ff-lib.sh): config/
...
# secondmate spawn (bin/xo-spawn.sh), the bootstrap secondmate sweep
# (bin/xo-bootstrap.sh), and the focused mid-session config push
# (bin/xo-config-push.sh). It is PRIMARY-AUTHORITATIVE: the primary's value wins
...
# invoke xo_config_send_reread_nudge so the live agent re-reads exact post-write
# bytes (spawn/respawn already re-reads at launch and needs no redundant nudge).
#
# Extensible by design: XO_INHERITABLE_CONFIG is the single declared list of
...
# allowlist for remote routes: bin/xo-remote-inherit-push.sh (sender) and
# bin/xo-remote-inherit.sh (receiver, executing inside the remote home) both
# derive their item set from xo_config_inherit_items rather than restating it,
...
. "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/xo-startup-memory-budget-lib.sh"

# The one shared data file in this inheritance contract. There is deliberately
# no shared learnings file.
XO_SHARED_CAPTAIN_FILE="captain-shared.md"
XO_SHARED_CAPTAIN_REL="data/$XO_SHARED_CAPTAIN_FILE"
XO_SHARED_CAPTAIN_MODE="444"

# The declared inheritable set (space-separated, config-dir-relative item paths).
# Extend here to inherit more of the primary's local config; override via the
# environment only in tests. Items must not contain whitespace.
XO_INHERITABLE_CONFIG="${XO_INHERITABLE_CONFIG:-crew-dispatch.json crew-harness backlog-backend backend herdr-presentation-spaces startup-memory-budget trace-context launch-env-allowlist claude-permission-mode}"
...
# already frozen for its current session (bin/xo-trace-context-lib.sh).
XO_SESSION_SCOPED_INHERITABLE_CONFIG="trace-context"

# True when <item> is session-scoped in the sense above.
xo_config_inherit_item_session_scoped() {  # <item>
  local item=$1 candidate
  for candidate in $XO_SESSION_SCOPED_INHERITABLE_CONFIG; do
...
# line, in propagation order: every XO_INHERITABLE_CONFIG item under config/,
...
xo_config_inherit_items() {
  local item
  for item in $XO_INHERITABLE_CONFIG; do
    printf 'config/%s\n' "$item"
  done
  printf '%s\n' "$XO_SHARED_CAPTAIN_REL"
}

xo_config_source_present() {
...
copy_inheritable_file() {
  ...
  tmp=$(mktemp "$dest_parent/.xo-inherit.XXXXXX" 2>/dev/null) || return 1
...
# absence on both sides is a no-op. When XO_CONFIG_INHERIT_REPORT points at a writable
...
record_inheritable_config_result() {
  ...
  [ -n "${XO_CONFIG_INHERIT_REPORT:-}" ] || return 0
  printf '%s\t%s\t%s\n' "$item" "$status" "$reason" >> "$XO_CONFIG_INHERIT_REPORT" 2>/dev/null || true
}
...
warn_inheritable_config_skip() {
  ...
  echo "xo-config-inherit: warning: skipped $item for $dest_config: $reason" >&2
}

warn_inheritable_config_error() {
  local item=$1 dest=$2 reason=$3
  echo "xo-config-inherit: error: $reason $item at $dest" >&2
}
...
shared_captain_header_valid() {
  ...
  case "$head" in *"main xo"*) ;; *) return 1 ;; esac
...
shared_captain_file_safe_existing() {
  ...
  [ "$(xo_inherit_file_link_count "$path")" = 1 ]
}
...
restore_shared_captain_readonly() {
  ...
  chmod "$XO_SHARED_CAPTAIN_MODE" "$dest" 2>/dev/null || return 1
}

shared_captain_quarantine_existing_for_hash() {
  local parent=$1 hash=$2 artifact artifact_hash
  for artifact in "$parent"/."$XO_SHARED_CAPTAIN_FILE".quarantine.*."$hash" "$parent"/."$XO_SHARED_CAPTAIN_FILE".quarantine.*."$hash".[0-9]*; do
    [ -e "$artifact" ] || [ -L "$artifact" ] || continue
    shared_captain_file_safe_existing "$artifact" || return 1
    artifact_hash=$(xo_inherit_sha256 "$artifact") || return 1
...
shared_captain_quarantine_name() {
  ...
  base="$parent/.$XO_SHARED_CAPTAIN_FILE.quarantine.$stamp.$hash"
  candidate=$base
  n=0
...
quarantine_shared_captain_dest() {
  ...
  shared_captain_file_safe_existing "$dest" || return 1
  dest_hash=$(xo_inherit_sha256 "$dest") || return 1
...
copy_shared_captain_file() {
  ...
  tmp=$(mktemp "$dest_parent/.xo-captain-shared.XXXXXX" 2>/dev/null) || return 1
  if ! cp "$src" "$tmp" 2>/dev/null; then
    rm -f "$tmp" 2>/dev/null || true
    return 1
  fi
...
propagate_shared_captain_preferences() {
  ...
  src="$src_data/$XO_SHARED_CAPTAIN_FILE"
  dest="$dest_data/$XO_SHARED_CAPTAIN_FILE"
  dest_parent=${dest%/*}
  ...
  if [ -e "$src" ] || [ -L "$src" ]; then
    if ! shared_captain_file_safe_existing "$src"; then
      ...
      warn_inheritable_config_error "$XO_SHARED_CAPTAIN_REL" "$src" "$reason"
      record_inheritable_config_result "$XO_SHARED_CAPTAIN_REL" error "$reason"
      return 1
    fi
    ...
    src_hash=$(xo_inherit_sha256 "$src") || {
    ...
    if [ -e "$dest" ] || [ -L "$dest" ]; then
      if ! shared_captain_file_safe_existing "$dest"; then
        ...
        warn_inheritable_config_error "$XO_SHARED_CAPTAIN_REL" "$dest" "$reason"
        record_inheritable_config_result "$XO_SHARED_CAPTAIN_REL" error "$reason"
        return 1
      fi
      dest_hash=$(xo_inherit_sha256 "$dest") || {
      ...
      if [ "$src_hash" = "$dest_hash" ]; then
        if restore_shared_captain_readonly "$dest"; then
          record_inheritable_config_result "$XO_SHARED_CAPTAIN_REL" unchanged ""
          return 0
        fi
      ...
      if ! shared_captain_dir_safe "$dest_parent"; then
        ...
        warn_inheritable_config_error "$XO_SHARED_CAPTAIN_REL" "$dest_parent" "$reason"
        record_inheritable_config_result "$XO_SHARED_CAPTAIN_REL" error "$reason"
        restore_shared_captain_readonly "$dest" || true
        return 1
      fi
    ...
    if copy_shared_captain_file "$src" "$dest"; then
      if [ -n "${quarantine:-}" ]; then
        record_inheritable_config_result "$XO_SHARED_CAPTAIN_REL" pushed "quarantined local drift at $quarantine"
      else
        record_inheritable_config_result "$XO_SHARED_CAPTAIN_REL" pushed ""
      fi
    else
    ...
    if quarantine=$(quarantine_shared_captain_dest "$dest" "$dest_parent"); then
      printf 'SECONDMATE_SYNC: secondmate home %s: quarantined %s drift at %s\n' "$dest_home" "$XO_SHARED_CAPTAIN_REL" "$quarantine"
      record_inheritable_config_result "$XO_SHARED_CAPTAIN_REL" pushed "mirrored primary absence after quarantining local copy at $quarantine"
    else
...
propagate_inheritable_config() {
  ...
  for item in $XO_INHERITABLE_CONFIG; do
    ...
    if [ "${XO_CONFIG_INHERIT_LIVE:-0}" = 1 ] && xo_config_inherit_item_session_scoped "$item"; then
      record_inheritable_config_result "$item" unchanged "session-scoped"
      continue
    fi
    src="$src_config/$item"
    dest="$dest_config/$item"
    if ! source_present=$(xo_config_source_present "$src"); then
    ...
    if [ "$item" = "$XO_STARTUP_MEMORY_BUDGET_FILE" ]; then
      if [ -e "$src_config" ] || [ -L "$src_config" ]; then
        if ! xo_startup_memory_budget_config_dir_safe "$src_config"; then
          reason="unsafe primary config directory: $XO_STARTUP_MEMORY_BUDGET_ERROR"
      ...
      if [ -e "$dest_config" ] || [ -L "$dest_config" ]; then
        if ! xo_startup_memory_budget_config_dir_safe "$dest_config"; then
          reason="unsafe destination config directory: $XO_STARTUP_MEMORY_BUDGET_ERROR"
      ...
      if [ -e "$src" ] || [ -L "$src" ]; then
        if ! xo_startup_memory_budget_file_valid "$src"; then
          reason="unsafe or invalid primary source: $XO_STARTUP_MEMORY_BUDGET_ERROR"
          warn_inheritable_config_error "$item" "$src" "$reason"
          record_inheritable_config_result "$item" error "$reason"
          rc=1
          continue
      ...
      if [ -e "$dest" ] || [ -L "$dest" ]; then
        if ! xo_startup_memory_budget_file_valid "$dest"; then
          reason="unsafe or invalid destination: $XO_STARTUP_MEMORY_BUDGET_ERROR"
...
XO_CONFIG_REREAD_INSTRUCTION_PREFIX_REL="state/.xo-inherited-config-reread"
XO_CONFIG_REREAD_MAX_SENT=16
XO_CONFIG_REREAD_RETRY_ROOT_REL="state/.xo-inherited-config-reread-retry"
XO_CONFIG_REREAD_MAX_PENDING=16
XO_CONFIG_REREAD_MAX_QUARANTINE=16
XO_CONFIG_INHERIT_LOCK_REL="state/.xo-inherited-config.lock"
...
XO_CONFIG_REREAD_FRAMING='These inherited config files changed. Re-read and apply their exact contents at every future intake. They are defaults/rules and do not remove your judgment to choose differently when warranted.'

# xo_config_reread_is_allowlisted_item <item>
# True only for the declared inheritable config allowlist (bare item name as
# recorded in XO_CONFIG_INHERIT_REPORT). data/captain-shared.md is never
# allowlisted here and must never be inlined into a reread instruction.
xo_config_reread_is_allowlisted_item() {
  local item=$1 candidate
  for candidate in $XO_INHERITABLE_CONFIG; do
    [ "$candidate" = "$item" ] && return 0
  done
  return 1
}

# xo_config_reread_changed_items <report>
# Print bare allowlisted config item names whose report status is "pushed",
# in XO_INHERITABLE_CONFIG order (deterministic path order). Empty when none.
xo_config_reread_changed_items() {
...
xo_config_inherit_lock_path() {
  local dest_home=$1
  [ -n "$dest_home" ] || return 1
  printf '%s/%s\n' "$dest_home" "$XO_CONFIG_INHERIT_LOCK_REL"
}

xo_config_reread_retry_dir() {
  local source_home=$1 id=$2 token
  [ -n "$source_home" ] && [ -n "$id" ] || return 1
  token=${id//[^a-zA-Z0-9_.-]/_}
  [ -n "$token" ] || token=unknown
  printf '%s/%s/%s\n' "$source_home" "$XO_CONFIG_REREAD_RETRY_ROOT_REL" "$token"
}

xo_config_reread_pending_stages() {
  local source_home=$1 id=$2 retry_dir stage
  retry_dir=$(xo_config_reread_retry_dir "$source_home" "$id") || return 1
  for stage in "$retry_dir"/.xo-inherited-config-reread.*; do
    case "$stage" in
      *.report) continue ;;
    esac
...
xo_config_reread_pending_reports() {
  local source_home=$1 id=$2 retry_dir report
  retry_dir=$(xo_config_reread_retry_dir "$source_home" "$id") || return 1
  for report in "$retry_dir"/.xo-inherited-config-reread.*.report; do
    [ -f "$report" ] && [ ! -L "$report" ] || continue
    printf '%s\n' "$report"
  done | LC_ALL=C sort
}

xo_config_reread_has_staged() {
  local source_home=$1 id=$2 stage
  while IFS= read -r stage; do
    [ -n "$stage" ] && return 0
  done < <(xo_config_reread_pending_stages "$source_home" "$id")
  while IFS= read -r stage; do
    [ -n "$stage" ] && return 0
  done < <(xo_config_reread_pending_reports "$source_home" "$id")
  return 1
}

xo_config_reread_retry_queue_is_full() {
  local source_home=$1 id=$2 count report_count
  count=$(xo_config_reread_pending_stages "$source_home" "$id" | wc -l | tr -d ' ')
  report_count=$(xo_config_reread_pending_reports "$source_home" "$id" | wc -l | tr -d ' ')
  count=$((count + report_count))
  [ "$count" -ge "$XO_CONFIG_REREAD_MAX_PENDING" ]
}

xo_config_reread_retry_pending() {
  local id=$1 dest_home=$2 report retry_out rc
  report=$(mktemp "${TMPDIR:-/tmp}/xo-config-reread-retry.XXXXXX" 2>/dev/null) || {
    printf 'CONFIG_REREAD: secondmate %s: send failed: could not create retry report\n' "$id"
    return 1
  }
  retry_out=$(xo_config_send_reread_nudge "$id" "$dest_home" "$report" 2>&1)
  rc=$?
  rm -f "$report"
...
xo_config_reread_new_retry_stage_path() {
  ...
  stage=$(umask 077; mktemp "$retry_dir/.xo-inherited-config-reread.$generation.XXXXXX" 2>/dev/null) || return 1
  printf '%s\n' "$stage"
}

xo_config_reread_save_retry_report() {
  local report=$1 stage_path=$2 report_path tmp parent
  parent=${stage_path%/*}
  report_path="$stage_path.report"
  tmp=$(umask 077; mktemp "$parent/.xo-config-reread-report.XXXXXX" 2>/dev/null) || return 1
...
# xo_config_write_reread_instruction <dest-home> <report> <instruction-path>
...
xo_config_write_reread_instruction() {
  ...
  while IFS= read -r item; do
    [ -n "$item" ] || continue
    xo_config_reread_is_allowlisted_item "$item" || continue
    rel="config/$item"
    dest="$dest_home/config/$item"
    if [ "$first" = 1 ]; then
      printf '%s\n' "$XO_CONFIG_REREAD_FRAMING" >> "$tmp" || { rm -f "$tmp"; return 1; }
      first=0
    fi
    {
      printf '\n'
  ...
  done < <(xo_config_reread_changed_items "$report")
  if [ "$first" = 1 ]; then
    rm -f "$tmp"
    return 1
  fi
  if ! mv -f "$tmp" "$instruction_path" 2>/dev/null; then
    XO_CONFIG_REREAD_FAILED_TEMP="$tmp"
    return 1
  fi
  return 0
}

xo_config_reread_adopt_exact_temp() {
...
xo_config_reread_pending_instructions() {
  local state=$1 pending instruction
  for pending in "$state"/.xo-inherited-config-reread.*.pending; do
...
xo_config_reread_has_pending() {
  local dest_home=$1 state pending
  state="$dest_home/${XO_CONFIG_REREAD_INSTRUCTION_PREFIX_REL%/*}"
  for pending in "$state"/.xo-inherited-config-reread.*.pending; do
    [ -f "$pending" ] && [ ! -L "$pending" ] || continue
    return 0
  done
...
xo_config_reread_cleanup_sent() {
  ...
  sorted=$(printf '%s\n' "$paths" | LC_ALL=C sort)
  total=$(printf '%s\n' "$sorted" | wc -l | tr -d ' ')
  remove=$((total - XO_CONFIG_REREAD_MAX_SENT))
...
xo_config_reread_mark_pending() {
  local instruction_path=$1 pending_path=$2 parent tmp
  parent=${pending_path%/*}
  [ -n "$parent" ] && [ "$parent" != "$pending_path" ] || return 1
  mkdir -p "$parent" 2>/dev/null || return 1
  tmp=$(umask 077; mktemp "$parent/.xo-config-reread-pending.XXXXXX" 2>/dev/null) || return 1
...
xo_config_reread_publish_stage() {
  ...
  tmp=$(umask 077; mktemp "$state/.xo-config-reread-publish.XXXXXX" 2>/dev/null) || return 1
  if ! cat "$stage" > "$tmp" || ! chmod 0600 "$tmp" 2>/dev/null || ! mv -f "$tmp" "$final" 2>/dev/null; then
    rm -f "$tmp"
    return 1
  fi
  if ! xo_config_reread_mark_pending "$final" "$final.pending"; then
    rm -f "$final"
    return 1
  fi
  printf '%s\n' "$final"
...
xo_config_reread_send_failure() {
  local id=$1 instruction_path=$2 pending_path=$3 detail=$4
  if ! xo_config_reread_mark_pending "$instruction_path" "$pending_path"; then
...
# xo_config_reread_send_pointer <id> <instruction-path>
xo_config_reread_send_pointer() {
  ...
  selector="xo-$id"
  send_bin="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/xo-send.sh"
  if [ ! -x "$send_bin" ]; then
    xo_config_reread_send_failure "$id" "$instruction_path" "$pending_path" "xo-send.sh not executable at $send_bin"
    return 1
  fi
  if [ -z "${XO_HOME:-}" ]; then
    xo_config_reread_send_failure "$id" "$instruction_path" "$pending_path" "XO_HOME is not set"
    return 1
  fi
  message="CONFIG_REREAD: $instruction_path"
  out=$(XO_HOME="$XO_HOME" \
    XO_ROOT_OVERRIDE="${XO_ROOT_OVERRIDE:-}" \
    XO_STATE_OVERRIDE="${XO_STATE_OVERRIDE:-}" \
    XO_SEND_SETTLE="${XO_SEND_SETTLE:-0}" \
  ...
  out=${out%%$'\n'*}
  [ -n "$out" ] || out="xo-send exited $rc"
  xo_config_reread_send_failure "$id" "$instruction_path" "$pending_path" "$out"
  return 1
}

# xo_config_reread_discard_pending <dest-home>
xo_config_reread_discard_pending() {
  ...
  if [ -n "$id" ] && [ -n "$source_home" ]; then
    retry_dir=$(xo_config_reread_retry_dir "$source_home" "$id") || rc=1
    if [ -d "$retry_dir" ]; then
      for retry_stage in "$retry_dir"/.xo-inherited-config-reread.*; do
...
xo_config_reread_quarantine_prune() {
  local root=$1 keep=${2:-$XO_CONFIG_REREAD_MAX_QUARANTINE} dirs dir oldest path remove
  case "$keep" in
    ''|*[!0-9]*) keep=$XO_CONFIG_REREAD_MAX_QUARANTINE ;;
  esac
  [ -d "$root" ] || return 0
  dirs=""
...
xo_config_reread_quarantine_dir() {
  local home=$1 state root quarantine
  state="$home/${XO_CONFIG_REREAD_INSTRUCTION_PREFIX_REL%/*}"
  root="$state/.xo-inherited-config-reread-quarantine"
  mkdir -p "$root" 2>/dev/null || return 1
  chmod 0700 "$root" 2>/dev/null || return 1
  xo_config_reread_quarantine_prune "$root" $((XO_CONFIG_REREAD_MAX_QUARANTINE - 1)) || return 1
  quarantine=$(umask 077; mktemp -d "$root/generation.XXXXXX" 2>/dev/null) || return 1
  chmod 0700 "$quarantine" 2>/dev/null || return 1
  printf '%s\n' "$quarantine"
}

xo_config_reread_quarantine_pending() {
  ...
  if [ "$dest_has_artifacts" -eq 1 ]; then
    dest_quarantine=$(xo_config_reread_quarantine_dir "$dest_home" 2>/dev/null || true)
  fi
  for pending in "$state"/.xo-inherited-config-reread.*.pending; do
    [ -f "$pending" ] && [ ! -L "$pending" ] || continue
    instruction=${pending%.pending}
  ...
  if [ -n "$id" ] && [ -n "$source_home" ]; then
    retry_dir=$(xo_config_reread_retry_dir "$source_home" "$id") || retry_dir=
    ...
    if [ "$source_has_artifacts" -eq 1 ]; then
      source_quarantine=$(xo_config_reread_quarantine_dir "$source_home" 2>/dev/null || true)
    fi
    if [ -d "$retry_dir" ]; then
      for retry_stage in "$retry_dir"/.xo-inherited-config-reread.*; do
        [ -f "$retry_stage" ] && [ ! -L "$retry_stage" ] || continue
...
# xo_config_send_reread_nudge <id> <dest-home> <report>
...
# (xo-send). The files contain only changed config paths, clear delimiters, and
...
xo_config_send_reread_nudge() {
  ...
  state="$dest_home_abs/${XO_CONFIG_REREAD_INSTRUCTION_PREFIX_REL%/*}"
  changed_items=$(xo_config_reread_changed_items "$report")
  pending_paths=""
  stage_paths=""
  retry_report_paths=""
  if [ "${XO_CONFIG_REREAD_SKIP_PENDING:-0}" != 1 ]; then
    pending_paths=$(xo_config_reread_pending_instructions "$state")
    source_home_abs=$(cd "${XO_HOME:-}" 2>/dev/null && pwd -P || true)
    if [ -n "$source_home_abs" ]; then
      stage_paths=$(xo_config_reread_pending_stages "$source_home_abs" "$id")
      retry_report_paths=$(xo_config_reread_pending_reports "$source_home_abs" "$id")
    fi
  fi
  send_failures=0
  while IFS= read -r retry_report_path; do
    ...
    if xo_config_write_reread_instruction "$dest_home_abs" "$retry_report_path" "$retry_stage_path"; then
      rm -f "$retry_report_path" 2>/dev/null || send_failures=1
      ...
      exact_tmp=${XO_CONFIG_REREAD_FAILED_TEMP:-}
      if [ -n "$exact_tmp" ] \
        && xo_config_reread_adopt_exact_temp "$exact_tmp" "$retry_stage_path"; then
        rm -f "$retry_report_path" 2>/dev/null || send_failures=1
        if [ -n "$stage_paths" ]; then
          stage_paths+=$'\n'
        fi
...
EOF
  if [ "$send_failures" -ne 0 ]; then
    xo_config_reread_cleanup_sent "$dest_home_abs"
    return 1
  fi
  if [ -n "$changed_items" ]; then
    ...
    if xo_config_reread_retry_queue_is_full "$source_home_abs" "$id"; then
      printf 'CONFIG_REREAD: secondmate %s: send failed: retry instruction queue is full\n' "$id"
      return 1
    fi
    current_stage_path=$(xo_config_reread_new_retry_stage_path "$source_home_abs" "$id") || {
      printf 'CONFIG_REREAD: secondmate %s: send failed: could not reserve retry instruction\n' "$id"
      return 1
    }
    if ! xo_config_write_reread_instruction "$dest_home_abs" "$report" "$current_stage_path"; then
      exact_tmp=${XO_CONFIG_REREAD_FAILED_TEMP:-}
      if [ -n "$exact_tmp" ] \
        && xo_config_reread_adopt_exact_temp "$exact_tmp" "$current_stage_path"; then
        ...
        printf 'CONFIG_REREAD: secondmate %s: send failed: retained exact retry temporary %s\n' "$id" "$exact_tmp"
      elif retry_record_path=$(xo_config_reread_save_retry_report "$report" "$current_stage_path"); then
        rm -f "$current_stage_path" 2>/dev/null || true
  ...
  while IFS= read -r stage_path; do
    ...
    if instruction_path=$(xo_config_reread_publish_stage "$dest_home_abs" "$stage_path"); then
...
EOF
  ...
  while IFS= read -r instruction_path; do
    ...
    if xo_config_reread_send_pointer "$id" "$instruction_path"; then
      while IFS= read -r stage_path; do
        [ -n "$stage_path" ] || continue
