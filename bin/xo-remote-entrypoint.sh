#!/usr/bin/env bash
# Fixed remote entrypoint for bin/xo-on.sh.
#
# Install this tracked file as xo-remote-entrypoint.sh on the remote account's
# non-interactive SSH PATH. It accepts protocol metadata plus a base64-encoded
# NUL argv stream, validates one genuine tracked executable in <root>/bin/xo-*.sh,
# then stages it for the XO-owned remote job worker. It never accepts a
# shell command string.
#
# The readiness-owning xo-remote-doctor.sh runs in this plain SSH bootstrap so
...
# disconnect remains unknown completion to xo-on.sh, which preserves OpenSSH's
...
PROTOCOL=1
DOCTOR_SHA256=596e3cc6de4fbdc7b04cf9287f647ea72edd4da05ad4ede8142bea8f877d3b75
...
decode_text() { # <label> <encoded> <destination>
  ...
  controls=$(xo_remote_job_has_forbidden_text_bytes "$destination")
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/xo-remote-entrypoint.XXXXXX") || die "cannot create protocol staging directory" 70
...
# shellcheck disable=SC2329 # Invoked by xo_remote_job_wait through XO_REMOTE_JOB_DISCONNECT_PROBE.
...
entrypoint_cleanup() {
  ...
  if [ -n "$JOB_ID" ] && [ "$JOB_COMPLETED" -eq 0 ] && [ -n "$ACCOUNT_HOME" ]; then
    xo_remote_job_cancel "$ACCOUNT_HOME" "$JOB_ID" 2>/dev/null || true
  fi
}
...
ROOT=$(<"$TMP/root")
HOME_PATH=$(<"$TMP/home")
ROOT=$(xo_remote_job_canonical_existing_dir "$ROOT") || die "remote root is not a safe existing directory"
HOME_PATH=$(xo_remote_job_canonical_home "$HOME_PATH") || die "remote home is not a safe directory"
[ -f "$ROOT/AGENTS.md" ] && [ ! -L "$ROOT/AGENTS.md" ] || die "remote root is not an XO checkout"
...
COMMAND=${ARGV[0]}
case "$COMMAND" in xo-*.sh) ;; *) die "command is outside the xo-*.sh namespace: $COMMAND" ;; esac
...
elif [ "$COMMAND" = xo-remote-doctor.sh ]; then
...
if [ "$COMMAND" = xo-remote-doctor.sh ]; then
  ...
  DOCTOR_ENV=(
    ...
    XO_REMOTE_DOCTOR_BOOTSTRAP=1
  )
  if [ -n "${XO_REMOTE_JOB_PLATFORM_OVERRIDE:-}" ]; then
    DOCTOR_ENV+=("XO_REMOTE_JOB_PLATFORM_OVERRIDE=$XO_REMOTE_JOB_PLATFORM_OVERRIDE")
  fi
  if [ -n "${XO_REMOTE_JOB_STATE_ROOT:-}" ]; then
    DOCTOR_ENV+=("XO_REMOTE_JOB_STATE_ROOT=$XO_REMOTE_JOB_STATE_ROOT")
  fi
  trap - EXIT
  rm -rf -- "$TMP"
...
if ! xo_remote_job_ensure_worker "$ROOT" "$ACCOUNT_HOME"; then
  die "${XO_REMOTE_JOB_ERROR:-remote job worker is unavailable; run xo-on.sh <route> xo-remote-doctor.sh --fix}"
fi
if ! JOB_ID=$(xo_remote_job_stage "$ACCOUNT_HOME" "$ROOT" "$HOME_PATH" "$COMMAND" "${ARGV[@]:1}"); then
  JOB_ID=
  die "${XO_REMOTE_JOB_ERROR:-cannot stage remote job}" 70
fi
XO_REMOTE_JOB_DISCONNECT_PROBE=entrypoint_caller_connected
if ! xo_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID"; then
  die "${XO_REMOTE_JOB_ERROR:-remote job did not complete}" 70
fi
JOB_COMPLETED=1
cat "$XO_REMOTE_JOB_STDOUT"
cat "$XO_REMOTE_JOB_STDERR" >&2
RESULT=$XO_REMOTE_JOB_EXIT
xo_remote_job_reap "$ACCOUNT_HOME" "$JOB_ID" || true
trap - EXIT
rm -rf -- "$TMP"
exit "$RESULT"
