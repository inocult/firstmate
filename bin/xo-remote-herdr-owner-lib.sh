...
# bin/xo-remote-herdr-guard.sh (the launch agent's exec target) and
# bin/xo-remote-doctor.sh (the readiness check for that session).
#
...
# ("xo-remote server birth and login-keychain access") holds the dated
# evidence for every marker read here.
#
# Functions:
#   xo_remote_herdr_socket_owner <socket-path>
...
#   xo_remote_herdr_process_env <pid>
...
#   xo_remote_herdr_process_ancestry <pid>
#     Prints "<pid> <command>" for <pid> and each ancestor up to pid 1.
#   xo_remote_herdr_owner_birth <pid>
...
#       worker   XO_REMOTE_JOB_ACTIVE=1, with launchctl proving that
#                dev.xo.remote-job is loaded only in gui/<uid>
...
xo_remote_herdr_gui_job_proves_owner() { # <uid> <label> <pid>
...
xo_remote_herdr_gui_job_is_exclusive() { # <uid> <label>
  local uid=$1 label=$2
...
xo_remote_herdr_owner_birth() { # <pid>
  ...
  env=$(xo_remote_herdr_process_env "$pid") || { printf 'unknown\n'; return 0; }
  ...
  if [ -n "$uid" ] && [ -n "$xpc_line" ] \
    && xo_remote_herdr_gui_job_proves_owner "$uid" "$label" "$pid"; then
    printf 'launchd\n'
    return 0
  fi
  if printf '%s\n' "$env" | grep -q -E '^XO_REMOTE_JOB_ACTIVE=1$' \
    && [ -n "$uid" ] \
    && xo_remote_herdr_gui_job_is_exclusive "$uid" dev.xo.remote-job; then
    printf 'worker\n'
    return 0
  fi
  if xo_remote_herdr_process_ancestry "$pid" | xo_remote_herdr_ancestry_has_ssh_origin; then
    printf 'ssh\n'
    return 0
  fi
  printf 'unknown\n'
...
xo_remote_herdr_ancestry_has_ssh_origin() {
  awk '
    {
      argv0 = $2
      sub(/.*\//, "", argv0)
