#!/usr/bin/env bash
# bin/xo-composer-lib.sh - the ONE fleet-wide owner of composer classification:
...
# session-provider adapter (tmux via bin/xo-tmux-lib.sh, and
# bin/backends/{herdr,orca,cmux,zellij}.sh) and by xo-spawn.sh's kimi
# launch-readiness check.
#
# WHY THIS EXISTS (tasks xo-composer-shellglyph-safety and
# xo-composer-thin-adapter-refactor-r1): the adapters each carried their own
# copy of composer shape knowledge, and every copy drifted. The audited result
# (data/xo-composer-consolidation-audit-s1) was a 5-adapter x 6-harness matrix
...
# shape is taught to xo_composer_classify_screen below, once, and every backend
...
# captures in data/xo-composer-consolidation-audit-s1/report.md and
...
# xo_composer_strip_ghost is the ONE ANSI-aware extractor of "real typed
...
# xo_composer_normalize_trim_var is the one fix: it maps every code point
...
# include guard (matching bin/xo-tmux-lib.sh).

# xo_composer_strip_ansi: drop every CSI escape sequence, leaving plain text.
...
# uses xo_composer_strip_ghost instead. Reads the styled text on stdin and prints
# plain text (stdin-only, matching xo_composer_strip_ghost). The character class
# includes ':' so an ITU colon-form SGR (38:2::r:g:b) is stripped whole, not left
# with a dangling tail.
xo_composer_strip_ansi() {
...
# guard (bin/xo-test-run.sh, live-harness-optin) is what catches a harness
# that starts drawing its composer with a character outside this property.
XO_COMPOSER_UNICODE_SPACES=()
for _xo_composer_space_octal in \
  ...
  printf -v _xo_composer_space_utf8 '%b' "$_xo_composer_space_octal"
  XO_COMPOSER_UNICODE_SPACES+=("$_xo_composer_space_utf8")
done
unset -v _xo_composer_space_octal _xo_composer_space_utf8

# xo_composer_normalize_spaces_var: the ONE Unicode-whitespace mapping.
...
xo_composer_normalize_spaces_var() {  # <varname>
  local __fmns_name=$1 __fmns_text=${!1} __fmns_space
  for __fmns_space in "${XO_COMPOSER_UNICODE_SPACES[@]}"; do
    __fmns_text=${__fmns_text//"$__fmns_space"/ }
  done
  printf -v "$__fmns_name" '%s' "$__fmns_text"
}

# xo_composer_normalize_trim_var: the one whitespace-normalizing trim shared by
...
xo_composer_normalize_trim_var() {  # <varname>
  local __fmnt_name=$1 __fmnt_text
  xo_composer_normalize_spaces_var "$__fmnt_name"
...
# xo_composer_strip_ghost: the ONE fleet-wide ANSI-aware extractor of "real typed
...
#     XO_COMPOSER_GHOST_LUMA_MAX (default 128): how grok renders its placeholder
...
#     dark-foreground run. This assumes a DARK terminal theme, the xo
...
# Raising XO_COMPOSER_GHOST_LUMA_MAX is not free: muse draws its `⟩` prompt glyph
...
xo_composer_strip_ghost() {
  LC_ALL=C awk -v lumamax="${XO_COMPOSER_GHOST_LUMA_MAX:-128}" '
...
# proving a submitted Enter actually landed. Keeping them in bin/xo-tmux-lib.sh
...
# busy contract - what xo records and supervises on - is owned by
# bin/xo-busy-lib.sh, which forbids classifying a harness from rendered text.
...
XO_DELIVERY_BUSY_REGEX_DEFAULT='esc (to )?interrupt|Working(\.\.\.|…)|Ctrl\+c:cancel|ctrl\+c to stop'
XO_DELIVERY_CLAUDE_BUSY_REGEX_DEFAULT='esc to interrupt|…[[:space:]]+\([0-9]+[smh]'
XO_DELIVERY_CODEX_BUSY_REGEX_DEFAULT='esc to interrupt'
XO_DELIVERY_OPENCODE_BUSY_REGEX_DEFAULT='esc interrupt'
XO_DELIVERY_PI_BUSY_REGEX_DEFAULT='Working\.\.\.'
...
XO_OMP_SPINNER_FRAMES_RE='(⠋|⠙|⠹|⠸|⠼|⠴|⠦|⠧|⠇|⠏|⣾|⣽|⣻|⢿|⡿|⣟|⣯|⣷)'
XO_DELIVERY_OMP_BUSY_REGEX_DEFAULT='Working…|^[[:space:]]*'"$XO_OMP_SPINNER_FRAMES_RE"'[[:space:]]+[0-9]+[smh]'
XO_DELIVERY_GROK_BUSY_REGEX_DEFAULT='Ctrl\+c:cancel'
...
# bin/xo-busy-lib.sh, never from this row.
XO_DELIVERY_CURSOR_BUSY_REGEX_DEFAULT='ctrl\+c to stop'
XO_DELIVERY_KIMI_BUSY_REGEX_DEFAULT='^[[:space:]]*(🌑|🌒|🌓|🌔|🌕|🌖|🌗|🌘)[[:space:]]+·[[:space:]]+'

xo_busy_lines_match() {  # [harness]
  local harness=${1:-} lines regex
  IFS= read -r -d '' lines || true
  if [ -n "${XO_BUSY_REGEX:-}" ]; then
    regex=$XO_BUSY_REGEX
  else
    case "$harness" in
      claude) regex=$XO_DELIVERY_CLAUDE_BUSY_REGEX_DEFAULT ;;
      codex) regex=$XO_DELIVERY_CODEX_BUSY_REGEX_DEFAULT ;;
      opencode) regex=$XO_DELIVERY_OPENCODE_BUSY_REGEX_DEFAULT ;;
      pi|pi-signed) regex=$XO_DELIVERY_PI_BUSY_REGEX_DEFAULT ;;
      omp) regex=$XO_DELIVERY_OMP_BUSY_REGEX_DEFAULT ;;
      grok) regex=$XO_DELIVERY_GROK_BUSY_REGEX_DEFAULT ;;
      kimi) regex=$XO_DELIVERY_KIMI_BUSY_REGEX_DEFAULT ;;
      cursor) regex=$XO_DELIVERY_CURSOR_BUSY_REGEX_DEFAULT ;;
      '') regex=$XO_DELIVERY_BUSY_REGEX_DEFAULT ;;
      *)
...
XO_COMPOSER_AGENT_PROMPT_GLYPHS=$(printf '%s\n' '❯' '›' '⟩' '→')
XO_COMPOSER_SHELL_PROMPT_GLYPHS=$(printf '%s\n' '>' '$' '%' '#')
...
# 2026.08.11-e8db854). XO_COMPOSER_IDLE_RE overrides for an unverified harness;
# matching is case-insensitive.
XO_COMPOSER_IDLE_RE_DEFAULT='^Type a message\.\.\.$|^Ask anything\.\.\.|^Plan, search, build anything$|^Add a follow-up$'

# Opencode draws a mode/model footer line INSIDE its left-bar composer
# ("Build · GPT-5.5 Fast OpenAI · high"). It is composer furniture, not typed
# text, and only the run's LAST row is ever matched against it.
XO_COMPOSER_LEFTBAR_FOOTER_RE_DEFAULT='^(Build|Plan)[[:space:]]+·[[:space:]]+'
...
XO_COMPOSER_OMP_STATUS_RE_DEFAULT='^[[:space:]]*(π|󰵗)[[:space:]]+·[[:space:]]|^[[:space:]]*'"$XO_OMP_SPINNER_FRAMES_RE"'[[:space:]]+[0-9]+[smh]([[:space:]]|$)|[[:space:]]·[[:space:]].*[0-9]+(\.[0-9]+)?%/[0-9]+K'
...
XO_COMPOSER_CAPTURE_LINES=${XO_COMPOSER_CAPTURE_LINES:-20}

# Pi allows a multi-line composer between its horizontal separators. Bound the
# structural candidate so two unrelated transcript rules with an arbitrarily
# large region between them can never be promoted into a composer.
XO_COMPOSER_PI_MAX_LINES=${XO_COMPOSER_PI_MAX_LINES:-8}

# 0 when <content> is exactly one glyph drawn from <glyph-list>.
_xo_composer_is_prompt_glyph() {  # <content> <glyph-list>
...
# xo_composer_leading_prompt_glyph_var: set <out-varname> to the ONE prompt
...
xo_composer_leading_prompt_glyph_var() {  # <out-varname> <content>
...
$XO_COMPOSER_AGENT_PROMPT_GLYPHS
$XO_COMPOSER_SHELL_PROMPT_GLYPHS
EOF
  printf -v "$__fmpg_out" '%s' ''
  return 1
}

# xo_composer_leading_agent_glyph_var: like the above but AGENT glyphs only.
# The bare-row shape must never be anchored by a shell glyph (dead-shell rule).
xo_composer_leading_agent_glyph_var() {  # <out-varname> <content>
...
xo_composer_leading_shell_glyph_var() {  # <out-varname> <content>
...
# xo_composer_classify_content: the single shared composer-content verdict.
...
# trimmed with.
xo_composer_classify_content() {  # <bordered> <content> [idle_re] [idle_case] [plain_content] [placeholder-position] [styled]
  ...
  content=$2
  xo_composer_normalize_trim_var content
  plain_content=${5:-$2}
  xo_composer_normalize_trim_var plain_content
  if [ "$bordered" != 1 ] && [ -z "$content" ] && [ -n "$plain_content" ]; then
    if _xo_composer_is_prompt_glyph "$plain_content" "$XO_COMPOSER_AGENT_PROMPT_GLYPHS"; then
      printf 'empty'; return 0
    fi
    printf 'unknown'; return 0
  fi
  if _xo_composer_is_prompt_glyph "$content" "$XO_COMPOSER_AGENT_PROMPT_GLYPHS"; then
    printf 'empty'; return 0
  fi
  if _xo_composer_is_prompt_glyph "$content" "$XO_COMPOSER_SHELL_PROMPT_GLYPHS"; then
    if [ "$bordered" = 1 ]; then printf 'empty'; else printf 'unknown'; fi
    return 0
  fi
  [ -n "$content" ] || { printf 'empty'; return 0; }
  xo_composer_idle_matches "$content" "$idle_re" "$idle_case" && idle_collision=1
  if xo_composer_leading_prompt_glyph_var glyph "$content"; then
  ...
  if [ "$idle_collision" != 1 ] && [ "$styled" = 1 ] && [ -n "$plain_content" ]; then
    ...
    if xo_composer_leading_prompt_glyph_var plain_glyph "$plain_body"; then
      plain_body=${plain_body#*"$plain_glyph"}
    fi
    xo_composer_normalize_trim_var plain_body
    if [ "${#content}" -lt "${#plain_body}" ] \
       && xo_composer_idle_matches "$plain_body" "$idle_re" "$idle_case"; then
      case "$plain_body" in
...
#
# xo_composer_classify_screen <caps> <screen> [cursor_row] [identity]
...
# _xo_composer_pi_separator_row: a solid pi separator - nothing but `─`, at
# least 8 columns wide. The width floor is a literal substring test so it is
# byte-exact in every locale.
_xo_composer_pi_separator_row() {  # <trimmed-row>
  local row=$1
  [ -n "$row" ] || return 1
  [ -z "${row//─/}" ] || return 1
  case "$row" in
...
# Row-scan results are returned through XO_COMPOSER_SCAN_* globals (bash 3.2
# has no nameref); they are internal to this owner.
_xo_composer_scan_screen() {  # <plain-screen> <cursor-or-empty> [extract-wrap]
  local pane=$1 cy=${2:-}
  ...
  XO_COMPOSER_SCAN_BOX_TOP=-1
  XO_COMPOSER_SCAN_BOX_BOTTOM=-1
  XO_COMPOSER_SCAN_BOX_AMBIG=0
  XO_COMPOSER_SCAN_INCOMPLETE_BOX_FROM=-1
  XO_COMPOSER_SCAN_UNSAFE=0
  XO_COMPOSER_SCAN_CURSOR_EDGE=0
  XO_COMPOSER_SCAN_BARE_ROW=-1
  XO_COMPOSER_SCAN_SHELL_ROW=-1
  XO_COMPOSER_SCAN_LEFTBAR_START=-1
  XO_COMPOSER_SCAN_LEFTBAR_END=-1
  XO_COMPOSER_SCAN_PI_PAIR_FOUND=0
  XO_COMPOSER_SCAN_PI_PAIR_VALID=0
  XO_COMPOSER_SCAN_PI_OPEN=-1
  XO_COMPOSER_SCAN_PI_CLOSE=-1
  XO_COMPOSER_SCAN_PI_LAST_SEPARATOR=-1
  local leftbar_start=-1 pi_open=-1 pi_lines=0 pi_max
  pi_max=$XO_COMPOSER_PI_MAX_LINES
  case "$pi_max" in ''|*[!0-9]*|0) pi_max=8 ;; esac
  while IFS= read -r line; do
    indent=${line%%[![:space:]]*}
    left_stripped="${line#"${line%%[![:space:]]*}"}"
    trimmed=$left_stripped
    xo_composer_normalize_trim_var trimmed
    kind=
    family=
    case "$trimmed" in
    ...
    if _xo_composer_pi_separator_row "$trimmed"; then
      XO_COMPOSER_SCAN_PI_LAST_SEPARATOR=$row
      if [ "$pi_open" -ge 0 ]; then
        XO_COMPOSER_SCAN_PI_PAIR_FOUND=1
        XO_COMPOSER_SCAN_PI_OPEN=$pi_open
        XO_COMPOSER_SCAN_PI_CLOSE=$row
        if [ "$pi_lines" -le "$pi_max" ]; then
          XO_COMPOSER_SCAN_PI_PAIR_VALID=1
        else
          XO_COMPOSER_SCAN_PI_PAIR_VALID=0
        fi
      fi
    ...
    case "$trimmed" in
      ...
      '┃'*)
        ...
        XO_COMPOSER_SCAN_LEFTBAR_START=$leftbar_start
        XO_COMPOSER_SCAN_LEFTBAR_END=$row
        ;;
      *) leftbar_start=-1 ;;
    esac
    ...
    if [ "$top" -lt 0 ] && xo_composer_leading_shell_glyph_var glyph "$trimmed"; then
      XO_COMPOSER_SCAN_SHELL_ROW=$row
    elif xo_composer_leading_agent_glyph_var glyph "$trimmed"; then
      XO_COMPOSER_SCAN_BARE_ROW=$row
    fi
    # Cursor safety: a cursor sitting on a structural edge row is never an
    # input row.
    if [ -n "$cy" ] && [ "$row" -eq "$cy" ] && xo_composer_row_has_edge "$trimmed"; then
      XO_COMPOSER_SCAN_CURSOR_EDGE=1
    fi
    ...
    if [ "$kind" = top ] || { [ "$kind" = ascii ] && [ "$top" -lt 0 ]; }; then
      if [ -n "$cy" ] && [ "$top" -ge 0 ] && [ "$top" -lt "$cy" ] && [ "$cy" -le "$row" ]; then
        XO_COMPOSER_SCAN_UNSAFE=1
      fi
      top=$row
      XO_COMPOSER_SCAN_INCOMPLETE_BOX_FROM=$row
      ...
      if [ "$top" -ge 0 ] && [ "$family" = "$current_family" ] \
        ...
        if [ "$geometry_check" = 1 ]; then
          ...
          if [ "$bottom_spaces" != "$top_spaces" ]; then
            ...
            if ! _xo_composer_titled_bottom_ok "$family" "$bottom_inner" "$top_spaces"; then
        ...
        if [ -n "$cy" ]; then
          if [ "$top" -lt "$cy" ] && [ "$cy" -le "$row" ]; then
            XO_COMPOSER_SCAN_BOX_TOP=$top
            XO_COMPOSER_SCAN_BOX_BOTTOM=$row
            XO_COMPOSER_SCAN_BOX_AMBIG=$geometry_ambiguous
          fi
        else
          XO_COMPOSER_SCAN_BOX_TOP=$top
          XO_COMPOSER_SCAN_BOX_BOTTOM=$row
        ...
        if [ "$XO_COMPOSER_SCAN_INCOMPLETE_BOX_FROM" -lt 0 ]; then
          XO_COMPOSER_SCAN_INCOMPLETE_BOX_FROM=$row
        fi
        if [ -n "$cy" ]; then
      ...
      case "$current_family:$side_family" in
        rounded:single|light:single|heavy:heavy|double:double|ascii:ascii)
          ...
          if [ "$geometry_check" = 1 ]; then
            ...
            if content_spaces=$(xo_composer_geometry_spaces "$content_inner"); then
...
_xo_composer_titled_bottom_ok() {  # <family> <bottom-inner> <top-spaces>
  local family=$1 inner=$2 expected=$3 dash spaces
  xo_composer_normalize_trim_var inner
  case "$family" in
    rounded|light) dash='─' ;;
    double) dash='═' ;;
    heavy) dash='━' ;;
...
# xo_composer_row_has_edge: 0 when the trimmed row starts or ends with a
...
xo_composer_row_has_edge() {  # <trimmed-row>
  local row=$1
  xo_composer_normalize_trim_var row
  case "$row" in
    '│'*|*'│'|'┃'*|*'┃'|'║'*|*'║'|'╭'*|*'╭'|'╮'*|*'╮'|\
    '┌'*|*'┌'|'┐'*|*'┐'|'╔'*|*'╔'|'╗'*|*'╗'|'┏'*|*'┏'|'┓'*|*'┓'|\
    '╰'*|*'╰'|'╯'*|*'╯'|'└'*|*'└'|'┘'*|*'┘'|'╚'*|*'╚'|'╝'*|*'╝'|\
...
# xo_composer_geometry_spaces: prove a box content row blank to the same width
...
xo_composer_geometry_spaces() {  # <content-inner> -> spaces
  local content=$1 glyph
  xo_composer_normalize_spaces_var content
  if xo_composer_leading_prompt_glyph_var glyph "$content"; then
    content=${content/"$glyph"/ }
  fi
...
# _xo_composer_screen_row: print row <n> (zero-based) of <screen>.
_xo_composer_screen_row() {  # <n> <screen>
  printf '%s\n' "$2" | sed -n "$(($1 + 1))p"
}

# _xo_composer_row_content: extract the classification content of one raw row:
# ghost-strip when styled, plain otherwise, normalize-trim, and strip one
# matching pair of side border glyphs.
_xo_composer_row_content() {  # <raw-row> <styled> -> content on stdout
  local raw=$1 styled=$2 stripped
  if [ "$styled" = 1 ]; then
    stripped=$(printf '%s\n' "$raw" | xo_composer_strip_ghost)
  else
    stripped=$(printf '%s\n' "$raw" | xo_composer_strip_ansi)
  fi
  xo_composer_normalize_trim_var stripped
  case "$stripped" in
...
# _xo_composer_classify_rows: shared multi-row container verdict for the box
...
_xo_composer_classify_rows() {  # <screen> <styled> <ambiguous> <first-row> <last-row>
  local screen=$1 styled=$2 ambiguous=$3 first=$4 last=$5
  local row raw content plain state unknown_seen=0
  row=$first
  while [ "$row" -le "$last" ]; do
    raw=$(_xo_composer_screen_row "$row" "$screen")
    content=$(_xo_composer_row_content "$raw" "$styled")
    plain=$(_xo_composer_row_content "$raw" 0)
    state=$(xo_composer_classify_content 1 "$content" \
      "${XO_COMPOSER_IDLE_RE:-$XO_COMPOSER_IDLE_RE_DEFAULT}" insensitive "$plain" 1 "$styled")
    case "$state" in
      pending)
...
# _xo_composer_classify_bare_row: the bare agent-glyph row verdict, including
...
_xo_composer_classify_bare_row() {  # <screen> <styled> <row>
  local screen=$1 styled=$2 row=$3 raw content plain state
  raw=$(_xo_composer_screen_row "$row" "$screen")
  content=$(_xo_composer_row_content "$raw" "$styled")
  plain=$(_xo_composer_row_content "$raw" 0)
  state=$(xo_composer_classify_content 0 "$content" \
    "${XO_COMPOSER_IDLE_RE:-$XO_COMPOSER_IDLE_RE_DEFAULT}" insensitive "$plain" 0 "$styled")
...
# _xo_composer_row_is_omp_status: 0 when the trimmed row is omp's status line
# (XO_COMPOSER_OMP_STATUS_RE_DEFAULT above) - composer furniture that sits
# below a bare composer and must bound its wrap region exactly as an edge does.
_xo_composer_row_is_omp_status() {  # <trimmed-row>
  xo_composer_idle_matches "$1" "${XO_COMPOSER_OMP_STATUS_RE:-$XO_COMPOSER_OMP_STATUS_RE_DEFAULT}" sensitive
}

# _xo_composer_wrap_region_ok: 0 when every row STRICTLY BELOW <glyph-row>
...
_xo_composer_wrap_region_ok() {  # <plain-screen> <glyph-row> <cursor-row>
  local plain=$1 g=$2 cy=$3 row line trimmed glyph
  row=$((g + 1))
  while [ "$row" -le "$cy" ]; do
    line=$(_xo_composer_screen_row "$row" "$plain")
    trimmed=$line
    xo_composer_normalize_trim_var trimmed
    [ -n "$trimmed" ] || return 1
    if xo_composer_row_has_edge "$trimmed"; then return 1; fi
    if _xo_composer_row_is_omp_status "$trimmed"; then return 1; fi
    if xo_composer_leading_shell_glyph_var glyph "$trimmed"; then return 1; fi
    row=$((row + 1))
  done
  return 0
}

# _xo_composer_classify_bare_wrap: the bare composer plus its wrap region.
...
# glyph row itself).
_xo_composer_classify_bare_wrap() {  # <screen> <styled> <glyph-row> <cursor-row>
  local screen=$1 styled=$2 g=$3 cy=$4 row raw content glyph='' text_seen=0
  row=$g
  while [ "$row" -le "$cy" ]; do
    raw=$(_xo_composer_screen_row "$row" "$screen")
    content=$(_xo_composer_row_content "$raw" "$styled")
    if [ "$row" -eq "$g" ] && xo_composer_leading_agent_glyph_var glyph "$content"; then
      content=${content#*"$glyph"}
    fi
    xo_composer_normalize_trim_var content
...
# _xo_composer_classify_leftbar: opencode's left-bar composer. Blank rows and
...
_xo_composer_classify_leftbar() {  # <screen> <styled> <first-row> <last-row>
  ...
  footer_re=${XO_COMPOSER_LEFTBAR_FOOTER_RE:-$XO_COMPOSER_LEFTBAR_FOOTER_RE_DEFAULT}
  row=$first
  while [ "$row" -le "$last" ]; do
    raw=$(_xo_composer_screen_row "$row" "$screen")
    content=$(_xo_composer_row_content "$raw" "$styled")
    ...
    if [ "$placeholder_position" = 1 ] \
       && xo_composer_idle_matches "$content" "${XO_COMPOSER_IDLE_RE:-$XO_COMPOSER_IDLE_RE_DEFAULT}" insensitive; then
      row=$((row + 1)); continue
    fi
    if [ "$row" -eq "$last" ] \
       && xo_composer_idle_matches "$content" "$footer_re" sensitive; then
      row=$((row + 1)); continue
    fi
    pending_seen=1
    row=$((row + 1))
...
_xo_composer_leftbar_floor_row() {  # <trimmed-row>
  local row=$1 blocks
  case "$row" in
    '╹▀'*) blocks=${row#╹} ;;
    *) return 1 ;;
...
_xo_composer_select_cursorless() {
  local plain=$1 generic=-1 next boundary raw trimmed
  XO_COMPOSER_SELECTED_KIND=
  XO_COMPOSER_SELECTED_FIRST=-1
  XO_COMPOSER_SELECTED_LAST=-1
  XO_COMPOSER_SELECTED_AMBIG=0
  if [ "$XO_COMPOSER_SCAN_BOX_BOTTOM" -ge 0 ]; then
    generic=$XO_COMPOSER_SCAN_BOX_BOTTOM
    XO_COMPOSER_SELECTED_KIND=box
    XO_COMPOSER_SELECTED_FIRST=$((XO_COMPOSER_SCAN_BOX_TOP + 1))
    XO_COMPOSER_SELECTED_LAST=$((XO_COMPOSER_SCAN_BOX_BOTTOM - 1))
    XO_COMPOSER_SELECTED_AMBIG=$XO_COMPOSER_SCAN_BOX_AMBIG
  fi
  if [ "$XO_COMPOSER_SCAN_BARE_ROW" -gt "$generic" ]; then
    generic=$XO_COMPOSER_SCAN_BARE_ROW
    XO_COMPOSER_SELECTED_KIND=bare
    XO_COMPOSER_SELECTED_FIRST=$XO_COMPOSER_SCAN_BARE_ROW
    XO_COMPOSER_SELECTED_LAST=$XO_COMPOSER_SCAN_BARE_ROW
  fi
  if [ "$XO_COMPOSER_SCAN_LEFTBAR_END" -gt "$generic" ]; then
    generic=$XO_COMPOSER_SCAN_LEFTBAR_END
    XO_COMPOSER_SELECTED_KIND=leftbar
    XO_COMPOSER_SELECTED_FIRST=$XO_COMPOSER_SCAN_LEFTBAR_START
    XO_COMPOSER_SELECTED_LAST=$XO_COMPOSER_SCAN_LEFTBAR_END
  fi
  if [ "$XO_COMPOSER_SCAN_INCOMPLETE_BOX_FROM" -gt "$generic" ]; then
    XO_COMPOSER_SELECTED_KIND=
    return 1
  fi
  if [ "$XO_COMPOSER_SCAN_PI_PAIR_FOUND" = 1 ] \
     && [ "$XO_COMPOSER_SCAN_PI_CLOSE" -gt "$generic" ] \
     && [ "$generic" -lt "$XO_COMPOSER_SCAN_PI_OPEN" ]; then
    generic=$XO_COMPOSER_SCAN_PI_CLOSE
    XO_COMPOSER_SELECTED_KIND=pi
    XO_COMPOSER_SELECTED_FIRST=$((XO_COMPOSER_SCAN_PI_OPEN + 1))
    XO_COMPOSER_SELECTED_LAST=$((XO_COMPOSER_SCAN_PI_CLOSE - 1))
  fi
  if [ "$XO_COMPOSER_SCAN_PI_PAIR_FOUND" = 0 ] \
     && [ "$XO_COMPOSER_SCAN_PI_LAST_SEPARATOR" -gt "$generic" ]; then
    XO_COMPOSER_SELECTED_KIND=
    return 1
  fi
  if [ "$XO_COMPOSER_SCAN_SHELL_ROW" -gt "$generic" ]; then
    XO_COMPOSER_SELECTED_KIND=
    return 1
  fi
  if [ "$XO_COMPOSER_SELECTED_KIND" = bare ]; then
    next=$((XO_COMPOSER_SELECTED_LAST + 1))
    while :; do
      raw=$(_xo_composer_screen_row "$next" "$plain")
      trimmed=$raw
      xo_composer_normalize_trim_var trimmed
      [ -n "$trimmed" ] || break
      xo_composer_row_has_edge "$trimmed" && break
      _xo_composer_row_is_omp_status "$trimmed" && break
      XO_COMPOSER_SELECTED_LAST=$next
      next=$((next + 1))
    done
  fi
  if [ "$XO_COMPOSER_SELECTED_KIND" = box ] \
     || [ "$XO_COMPOSER_SELECTED_KIND" = leftbar ]; then
    boundary=$XO_COMPOSER_SELECTED_LAST
    if [ "$XO_COMPOSER_SELECTED_KIND" = box ]; then
      boundary=$XO_COMPOSER_SCAN_BOX_BOTTOM
    else
      next=$((boundary + 1))
      raw=$(_xo_composer_screen_row "$next" "$plain")
      trimmed=$raw
      xo_composer_normalize_trim_var trimmed
      if _xo_composer_leftbar_floor_row "$trimmed"; then
        boundary=$next
      fi
    fi
    next=$((boundary + 1))
    ...
    trimmed=$raw
    xo_composer_normalize_trim_var trimmed
    if [ -n "$trimmed" ] && ! xo_composer_row_has_edge "$trimmed"; then
      XO_COMPOSER_SELECTED_KIND=
      return 1
    fi
  fi
  [ -n "$XO_COMPOSER_SELECTED_KIND" ]
}

xo_composer_extract_selected_content() {  # <caps> <screen>
...
$caps
EOF
  plain=$(printf '%s\n' "$screen" | xo_composer_strip_ansi)
  _xo_composer_scan_screen "$plain" '' 1
  _xo_composer_select_cursorless "$plain" || return 1
  row=$XO_COMPOSER_SELECTED_FIRST
  while [ "$row" -le "$XO_COMPOSER_SELECTED_LAST" ]; do
    raw=$(_xo_composer_screen_row "$row" "$screen")
    content=$(_xo_composer_row_content "$raw" "$styled")
    placeholder_position=0
    case "$XO_COMPOSER_SELECTED_KIND" in
      bare)
        if [ "$row" -eq "$XO_COMPOSER_SELECTED_FIRST" ] \
           && xo_composer_leading_agent_glyph_var glyph "$content"; then
          content=${content#*"$glyph"}
        fi
        ;;
      leftbar)
        ...
        if [ -z "$content" ]; then
          :
        elif [ "$leading_blank" = 1 ] && [ "$row" -gt "$XO_COMPOSER_SELECTED_FIRST" ]; then
          placeholder_position=1
          leading_blank=0
        else
          leading_blank=0
      ...
      box)
        if [ "$prompt_row" -lt 0 ] \
           && xo_composer_leading_prompt_glyph_var glyph "$content"; then
          prompt_row=$row
          placeholder_position=1
          if _xo_composer_is_prompt_glyph "$glyph" "$XO_COMPOSER_SHELL_PROMPT_GLYPHS"; then
            prompt_is_shell=1
          fi
          content=${content#*"$glyph"}
    ...
    if [ -z "$content" ] \
       || { { [ "$XO_COMPOSER_SELECTED_KIND" = leftbar ] \
              || { [ "$XO_COMPOSER_SELECTED_KIND" = box ] && [ "$prompt_is_shell" = 1 ]; }; } \
            && [ "$placeholder_position" = 1 ] \
            && xo_composer_idle_matches "$content" "${XO_COMPOSER_IDLE_RE:-$XO_COMPOSER_IDLE_RE_DEFAULT}" insensitive; } \
       || { [ "$XO_COMPOSER_SELECTED_KIND" = leftbar ] \
            && [ "$row" -eq "$XO_COMPOSER_SELECTED_LAST" ] \
            && xo_composer_idle_matches "$content" "$footer_re" sensitive; }; then
      row=$((row + 1))
      continue
    fi
...
EOF
  ...
  plain=$(printf '%s\n' "$screen" | xo_composer_strip_ansi)
  _xo_composer_scan_screen "$plain" "$cy"
  if [ -n "$cy" ]; then
    # Cursor mode (tmux): the shape CONTAINING the cursor is the composer.
    if [ "$XO_COMPOSER_SCAN_UNSAFE" = 1 ]; then
      printf 'unknown'; return 0
    fi
    if [ "$XO_COMPOSER_SCAN_BOX_TOP" -ge 0 ]; then
      _xo_composer_classify_rows "$screen" "$styled" "$XO_COMPOSER_SCAN_BOX_AMBIG" \
        "$((XO_COMPOSER_SCAN_BOX_TOP + 1))" "$((XO_COMPOSER_SCAN_BOX_BOTTOM - 1))"
      return 0
    fi
    if [ "$XO_COMPOSER_SCAN_LEFTBAR_START" -ge 0 ] \
       && [ "$cy" -ge "$XO_COMPOSER_SCAN_LEFTBAR_START" ] \
       && [ "$cy" -le "$XO_COMPOSER_SCAN_LEFTBAR_END" ]; then
      _xo_composer_classify_leftbar "$screen" "$styled" \
        "$XO_COMPOSER_SCAN_LEFTBAR_START" "$XO_COMPOSER_SCAN_LEFTBAR_END"
      return 0
    fi
    if [ "$XO_COMPOSER_SCAN_BARE_ROW" -ge 0 ] && [ "$cy" -eq "$XO_COMPOSER_SCAN_BARE_ROW" ]; then
      if [ "$XO_COMPOSER_SCAN_PI_PAIR_FOUND" = 1 ] \
         && [ "$cy" -gt "$XO_COMPOSER_SCAN_PI_OPEN" ] \
         && [ "$cy" -lt "$XO_COMPOSER_SCAN_PI_CLOSE" ]; then
        _xo_composer_classify_bare_pi_overlap "$screen" "$styled" "$has_identity" "$identity" "$cy"
      else
        _xo_composer_classify_bare_row "$screen" "$styled" "$cy"
      fi
      return 0
    fi
    ...
    # and earns its retry.
    if [ "$XO_COMPOSER_SCAN_BARE_ROW" -ge 0 ] && [ "$cy" -gt "$XO_COMPOSER_SCAN_BARE_ROW" ] \
       && _xo_composer_wrap_region_ok "$plain" "$XO_COMPOSER_SCAN_BARE_ROW" "$cy"; then
      _xo_composer_classify_bare_wrap "$screen" "$styled" "$XO_COMPOSER_SCAN_BARE_ROW" "$cy"
      return 0
    fi
    if [ "$XO_COMPOSER_SCAN_PI_PAIR_FOUND" = 1 ] \
       && [ "$cy" -gt "$XO_COMPOSER_SCAN_PI_OPEN" ] \
       && [ "$cy" -lt "$XO_COMPOSER_SCAN_PI_CLOSE" ]; then
      _xo_composer_pi_verdict "$screen" "$styled" "$has_identity" "$identity"
      return 0
    fi
    if [ "$XO_COMPOSER_SCAN_CURSOR_EDGE" = 1 ]; then
      printf 'unknown'; return 0
    fi
  ...
  if ! _xo_composer_select_cursorless "$plain"; then
    printf 'unknown'
    return 0
  fi
  case "$XO_COMPOSER_SELECTED_KIND" in
    pi)
      ...
      ;;
    box)
      _xo_composer_classify_rows "$screen" "$styled" "$XO_COMPOSER_SELECTED_AMBIG" \
        "$XO_COMPOSER_SELECTED_FIRST" "$XO_COMPOSER_SELECTED_LAST"
      ;;
    bare)
      if [ "$XO_COMPOSER_SELECTED_LAST" -gt "$XO_COMPOSER_SELECTED_FIRST" ]; then
        _xo_composer_classify_bare_wrap "$screen" "$styled" \
          "$XO_COMPOSER_SELECTED_FIRST" "$XO_COMPOSER_SELECTED_LAST"
      elif [ "$XO_COMPOSER_SCAN_PI_PAIR_FOUND" = 1 ] \
         && [ "$XO_COMPOSER_SCAN_BARE_ROW" -gt "$XO_COMPOSER_SCAN_PI_OPEN" ] \
         && [ "$XO_COMPOSER_SCAN_BARE_ROW" -lt "$XO_COMPOSER_SCAN_PI_CLOSE" ]; then
        _xo_composer_classify_bare_pi_overlap "$screen" "$styled" "$has_identity" "$identity" \
          "$XO_COMPOSER_SCAN_BARE_ROW"
      else
        _xo_composer_classify_bare_row "$screen" "$styled" "$XO_COMPOSER_SCAN_BARE_ROW"
      fi
      ;;
    leftbar)
      _xo_composer_classify_leftbar "$screen" "$styled" \
...
# xo_composer_submit_retry_core: the ONE verify-and-retry-Enter submit loop
...
# xo_composer_queued_enter_verdict; no shape knowledge lives in any loop.
xo_composer_submit_retry_core() {  # <send-key-fn> <state-fn> <target> <retries> <enter-sleep> [expected-label]
...
# xo_composer_queued_enter_verdict: the ONE busy-queued-Enter policy.
...
# Adapters supply their own busy primitive (tmux: xo_pane_is_busy; herdr:
# native agent_status=working, or a rendered busy footer on an idle native
# baseline). This function does not read a pane.
xo_composer_queued_enter_verdict() {  # <composer-state> <busy|idle|unknown>
...
_xo_composer_classify_pi_rows() {  # <screen> <styled>
  local screen=$1 styled=$2 row raw content
  row=$((XO_COMPOSER_SCAN_PI_OPEN + 1))
  while [ "$row" -lt "$XO_COMPOSER_SCAN_PI_CLOSE" ]; do
    raw=$(_xo_composer_screen_row "$row" "$screen")
    content=$(_xo_composer_row_content "$raw" "$styled")
    xo_composer_normalize_trim_var content
    if [ -n "$content" ]; then
...
_xo_composer_classify_bare_pi_overlap() {  # <screen> <styled> <has-identity> <identity> <bare-row>
...
_xo_composer_pi_verdict() {  # <screen> <styled> <has_identity> <identity>
  ...
  if [ "$agent" != pi ] || [ "$XO_COMPOSER_SCAN_PI_PAIR_VALID" != 1 ]; then
    printf 'unknown'
    return 0
  fi
  state=$(_xo_composer_classify_pi_rows "$screen" "$styled")
