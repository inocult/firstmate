...
#
# An X-mode mention that spawned real work is linked to its task by xo-x-link.sh
...
# done, build started, shipped, failed), xo composes a public-safe outcome
...
#   xo-x-followup.sh --check <task-id>
...
#   xo-x-followup.sh --clear <task-id> [--expect-request <request-id>]
...
#   xo-x-followup.sh <task-id> [--image <path>] [--final] --text-file <path>
#   xo-x-followup.sh <task-id> [--image <path>] [--final] -
#     Linked, within window, and under the cap: posts ONE follow-up via
#       xo-x-reply.sh --followup.
...
#       xo-x-reply.sh): clears the link and skips quietly, exactly like a
...
#       of retrying forever.
#       On xo-x-reply's fail-safe refusal (exit 8: platform or explicit budget
...
usage() {
  echo "usage: xo-x-followup.sh --check <task-id> | --clear <task-id> [--expect-request <request-id>] | <task-id> [--image <path>] [--final] --text-file <path> | <task-id> [--image <path>] [--final] -" >&2
}

help() {
  cat <<'EOF'
usage: xo-x-followup.sh --check <task-id>
       xo-x-followup.sh --clear <task-id> [--expect-request <request-id>]
       xo-x-followup.sh <task-id> [--image <path>] [--final] --text-file <path>
       xo-x-followup.sh <task-id> [--image <path>] [--final] -
  ...
  while [ "$#" -gt 0 ]; do
    case "$1" in
      ...
      --image)
        ...
        shift
        if [ "$#" -lt 1 ] || [ -z "$1" ]; then
          echo "xo-x-followup: missing --image path" >&2
          usage
          exit 2
        fi
        TS_ARGS+=("$1")
...
case "$ID" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "xo-x-followup: unsafe task id: $ID" >&2; exit 2 ;;
esac

META="$STATE/$ID.meta"
if [ -e "$META" ] || [ -L "$META" ]; then
  xo_backlog_record_present "$META" "task record" "$STATE" \
    || { echo "xo-x-followup: unsafe task record in state/$ID.meta" >&2; exit 1; }
fi
if [ "$MODE" = clear ]; then
...
# a post is simply a no-op success (xo need not special-case it).
if [ -z "$RID" ]; then
  if [ "$MODE" = check ]; then
    exit 1
  fi
  echo "xo-x-followup: $ID is not X-linked; nothing to post" >&2
  exit 0
fi
...
if [ "$EXPIRED" = 1 ]; then
  ...
  echo "xo-x-followup: $REASON for $ID; skipped and cleared the link" >&2
  exit 0
fi
...
# Post the follow-up. xo-x-reply owns text reading, thread-split, dry-run, the
...
if [ "${#REPLY_ENV[@]}" -gt 0 ]; then
  ...
  "$XO_ROOT/bin/xo-x-reply.sh" "$RID" --followup "${TS_ARGS[@]}" >/dev/null
fi
post_rc=$?
...
case "$post_rc" in
  0)
    ...
    if [ "$FINAL" = 1 ] || [ "$NEWCOUNT" -ge "$MAX_COUNT" ]; then
      ...
      echo "xo-x-followup: warning: posted but could not record the follow-up count in state/$ID.meta; cleared the link to avoid duplicate follow-ups" >&2
    fi
    printf '%s\n' "$RID"
    exit 0
    ;;
  8)
    # xo-x-reply.sh refused this follow-up (exit 8) because it could not
    ...
    # link here.
    echo "xo-x-followup: follow-up for $ID held: reply context lacks an authoritative platform or explicit budget; left the link in place to retry once both values are recoverable" >&2
    exit 1
    ;;
  9)
    # xo-x-reply.sh distinguishes a relay rejection of this specific follow-up
  ...
  *)
    ...
    # so xo can retry on a later pass.
    echo "xo-x-followup: follow-up post failed for $ID; left the link in place to retry" >&2
    exit 1
    ;;
esac
