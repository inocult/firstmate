#!/usr/bin/env python3
"""xo_voice_records.py - what the voice agent is allowed to know, and how it hands work over.

The voice agent answers status questions from xo's durable records and
...
another region, and the other writes to xo's wake queue.
...
nothing in a full status answer named a customer. tests/xo-voice-relay.test.sh
...
Two readings here treat that differently, on purpose.
  ...
  answer, and it is the same meaning bin/xo-inbox.sh gives "workers" in the human
...
the model cannot present it as current truth. bin/xo-crew-state.sh remains the
...
bin/xo-inbox.sh `status` is the human rendering of the same records and stays
...
Usage:
  xo_voice_records.py status [--home <dir>] [--scope counts|full]
  xo_voice_records.py queue <text>... [--home <dir>]
...
# bin/xo-brief.sh states the first six to every crewmate and bin/xo-classify-lib.sh
...
def default_home():
    """Return the operational home, matching bin/xo-inbox.sh's resolution."""
    env = os.environ.get("XO_HOME")
    if env:
        return env
...
def state_dir(home):
    """Return the runtime state directory, resolved as bin/xo-inbox.sh resolves it.

    xo-inbox.sh reads ${XO_STATE_OVERRIDE:-$XO_HOME/state}, and the handover
    below queues through xo-inbox.sh with the ambient environment. A reader that
    ...
    """
    override = os.environ.get("XO_STATE_OVERRIDE")
    if override:
        return override
...
def data_dir(home):
    ...
    Every script that sets XO_DATA_OVERRIDE for a child sets XO_STATE_OVERRIDE
    ...
    wrong.
    """
    override = os.environ.get("XO_DATA_OVERRIDE")
    if override:
        return override
...
def config_dir(home):
    ...
    override = os.environ.get("XO_CONFIG_OVERRIDE")
    if override:
        return override
...
def _last_event(state_dir, task_id):
    ...
    bin/xo-classify-lib.sh remains the owner of status-verb normalization.
...
def queue_request(text, home=None, root=None):
    ...
    return {
        ...
        "note_id": note_id,
        "queued_text": body,
        "handover": "XO now owns this request and will pick it up at "
...
def main(argv):
    parser = argparse.ArgumentParser(
        prog="xo_voice_records.py", description=__doc__.splitlines()[0],
    ...
    queue = sub.add_parser("queue", help="hand a request to xo")
    ...
    try:
        ...
        sys.stderr.write("xo_voice_records: {}\n".format(exc))
        return 2
