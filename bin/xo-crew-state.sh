...
#      (xo-on.sh + xo-remote-secondmate-control.sh state). alive falls through
...
#      binds an ACTIVE run without head equality (xo_nm_run_is_pipeline_owned_active
#      in bin/xo-nm-run-lib.sh).
...
#      xo_nm_runs_status_for_worktree in bin/xo-nm-run-lib.sh).
#      More than one recorded run can bind to this worktree at once, and
#      bin/xo-nm-run-lib.sh also owns which of them wins: a LIVE run always
...
#      bin/xo-classify-lib.sh owns the record test). An unreachable endpoint is
...
ID=${1:-}
[ -n "$ID" ] || { echo "usage: xo-crew-state.sh <id>" >&2; exit 2; }
...
META=${XO_CREW_STATE_META_OVERRIDE:-"$STATE/$ID.meta"}
LOG=${XO_CREW_STATE_STATUS_OVERRIDE:-"$STATE/$ID.status"}
NM_TIMEOUT=${XO_CREW_STATE_NM_TIMEOUT:-10}
case "$NM_TIMEOUT" in ''|*[!0-9]*) NM_TIMEOUT=10 ;; esac
# How many of the most recent `no-mistakes runs` rows each ledger read
# (xo_nm_runs_status_for_worktree in bin/xo-nm-run-lib.sh) scans, whether it is
...
XO_CREW_STATE_RUNS_LIMIT=${XO_CREW_STATE_RUNS_LIMIT:-200}
case "$XO_CREW_STATE_RUNS_LIMIT" in ''|*[!0-9]*) XO_CREW_STATE_RUNS_LIMIT=200 ;; esac
SEP=' · '
...
# Last non-empty status line; xo-classify-lib.sh owns leading-verb normalization.
log_last_line() {
  [ -f "$LOG" ] || return 1
  grep -v '^[[:space:]]*$' "$LOG" 2>/dev/null | tail -1
}
# Map a status-log verb onto a canonical state for the fallback path. `paused` is
# the deliberate-external-wait verb (xo-classify-lib.sh's XO_CLASSIFY_PAUSED_VERB):
...
# recovery-grade state over the same xo-on.sh transport xo-send uses, then read
...
if [ -n "$REMOTE_HOST" ]; then
  if ! REMOTE_STATE=$(XO_HOME="$XO_HOME" "$SCRIPT_DIR/xo-on.sh" "$ID" \
    xo-remote-secondmate-control.sh state "$ID" < /dev/null 2>/dev/null); then
    REMOTE_STATE=
  fi
...
# (xo_backend_of_meta defaults absent backend= to tmux, the P1 contract): a
# herdr task is read through xo_backend_capture instead of a bare tmux probe.
...
TASK_BACKEND=$(xo_backend_of_meta "$META")
BACKEND_TARGET=$(xo_backend_target_of_meta "$META")
EXPECTED_LABEL="xo-$ID"
pane_readable() {  # <target>
  case "$TASK_BACKEND" in
    tmux) tmux list-panes -t "$1" -F '#{pane_id}' >/dev/null 2>&1 ;;
    *) xo_backend_capture "$TASK_BACKEND" "$1" 1 "$EXPECTED_LABEL" >/dev/null 2>&1 ;;
  esac
}
# crew_busy_verdict: the crew's semantic busy state from the one contract
# owner (bin/xo-busy-lib.sh), as "<busy|idle|unknown> <source>". A converted
...
crew_busy_verdict() {  # <target>
  local tail40=''
  case "$HARNESS" in
    grok*) tail40=$(xo_backend_capture "$TASK_BACKEND" "$1" 40 "$EXPECTED_LABEL" 2>/dev/null) || tail40='' ;;
  esac
  xo_busy_classify "$TASK_BACKEND" "$1" "$HARNESS" "$ID" "$STATE" "$tail40"
}
...
# bin/xo-nm-run-lib.sh, shared with xo-teardown.sh's pre-teardown run abort.

trim() { xo_nm_trim "$@"; }
strip_quotes() { xo_nm_strip_quotes "$@"; }
nm_run() {  # <args...>
  xo_nm_run "$WT" "$NM_TIMEOUT" "$@"
}

# Scalar value of a TOON key in the captured run output ($RUN_OUT).
RUN_OUT=""
nm_field() {  # <key>
  xo_nm_field "$RUN_OUT" "$1"
}
...
# than a second threshold invented in xo. Positive evidence is required:
...
# status` is the canonical down-probe (the same one xo-brief.sh hands crews
...
nm_daemon_probe_down() {
  xo_nm_run_checked "$WT" "$NM_TIMEOUT" daemon status >/dev/null || return 0
  return 1
}
...
# has no runs-listing subcommand; tests/xo-crew-state.test.sh owns the
# 2026-07-02 dead-code incident history this fallback replaced).
# xo_nm_runs_status_for_worktree in bin/xo-nm-run-lib.sh is the ONE owner of
...
nm_runs_list() {
  nm_run runs --limit "$XO_CREW_STATE_RUNS_LIMIT"
}
...
# xo_nm_head_matches_worktree in bin/xo-nm-run-lib.sh.
nm_run_head_matches_worktree() {
  local run_head
  run_head=$(strip_quotes "$(nm_field head)")
  xo_nm_head_matches_worktree "$WT" "$run_head"
}
...
if [ "$KIND" = ship ] && [ -n "$CREW_BRANCH" ] && command -v no-mistakes >/dev/null 2>&1; then
  ...
  if [ -n "$RUN_OUT" ]; then
    ...
    # (xo_nm_run_is_pipeline_owned_active in bin/xo-nm-run-lib.sh).
    if [ -n "$run_branch" ] && [ "$run_branch" = "$CREW_BRANCH" ] \
      && { nm_run_head_matches_worktree || xo_nm_run_is_pipeline_owned_active "$RUN_OUT"; }; then
      HAVE_RUN=1
      # Live-over-terminal (bin/xo-nm-run-lib.sh). Bare `axi status` answers
      ...
      if ! xo_nm_run_is_active "$RUN_OUT"; then
        live_status=$(xo_nm_runs_status_for_worktree "$WT" "$CREW_BRANCH" "$(nm_runs_list)")
        if [ "$(xo_nm_run_status_class "$live_status")" = live ]; then
      ...
      COARSE_STATUS=$(xo_nm_runs_status_for_worktree "$WT" "$CREW_BRANCH" "$(nm_runs_list)")
...
if [ "$HAVE_RUN" = 1 ]; then
  ...
  if [ "$RUN_SOURCE" = coarse ]; then
    ...
    # surfaced by each supervisor's span classification (xo-classify-lib.sh's
...
# for merge (task_held_for_merge_pr in bin/xo-classify-lib.sh): a delivered ship
...
if ! pane_readable "$BACKEND_TARGET"; then
  ...
  # recovery-grade classifier (xo_backend_agent_state), which separates the
  # outcomes:
  ...
  #             xo_backend_tmux_agent_state owns as death, since xo-bootstrap
  #             and xo-session-start depend on it to license a respawn after a
  ...
  case "$TASK_BACKEND" in
    tmux|herdr) AGENT_STATE=$(xo_backend_agent_state "$TASK_BACKEND" "$BACKEND_TARGET") ;;
    *) AGENT_STATE=none ;;
  esac
...
# run-state. A decision-closing event - resolved: (xo-classify-lib.sh's
# XO_CLASSIFY_RESOLVE_VERB), and any future decision-only sibling - is NOT a state:
