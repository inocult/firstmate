...
# Usage: . bin/xo-tasks-axi-lib.sh
#
# Compatible means tasks-axi --version reports XO_TASKS_AXI_MIN or newer,
...
# XO_TASKS_AXI_MIN follows the axi-family floor policy owned beside the floor
# constants in bin/xo-bootstrap.sh.
...
# `config/backlog-backend=manual` opts out of tasks-axi for routine xo
# backlog mutations, but validated secondmate handoffs always use `tasks-axi mv`.
# Absent or any other value keeps the default tasks-axi backend path, falling
# back to manual mutation when the tool is not compatible.
# xo_tasks_axi_backend_resolve owns backend precedence: TASKS_AXI_BACKEND when
...
# xo_tasks_axi_backend delegates to that resolver and preserves its status;
# callers must check it before selecting backend-specific flags or exemptions.
#
# This file is the single owner of XO_TASKS_AXI_MIN. bin/xo-bootstrap.sh turns a
# failing check into the operator-facing MISSING diagnostic.
#
# COMPATIBILITY VERDICT REUSE. xo_tasks_axi_compatible costs three tasks-axi
# subprocesses, and one session start needs the same verdict twice: once in
# bin/xo-session-start.sh's backlog listing and once in the bin/xo-bootstrap.sh
...
#     in XO_TASKS_AXI_COMPATIBLE=0|1. Sourcing this file CONSUMES that variable
...
XO_TASKS_AXI_COMPATIBLE_MEMO=${XO_TASKS_AXI_COMPATIBLE:-}
unset XO_TASKS_AXI_COMPATIBLE
case "$XO_TASKS_AXI_COMPATIBLE_MEMO" in
  0|1) ;;
  *) XO_TASKS_AXI_COMPATIBLE_MEMO= ;;
esac

xo_tasks_axi_version_parts() {
  local output
...
xo_tasks_axi_compatible() {
  ...
  if xo_tasks_axi_compatible_probe; then
    XO_TASKS_AXI_COMPATIBLE_MEMO=1
    return 0
  fi
  XO_TASKS_AXI_COMPATIBLE_MEMO=0
  return 1
}

xo_tasks_axi_compatible_probe() {
  local parts major minor patch extra
  local min_major min_minor min_patch min_extra
  parts=$(xo_tasks_axi_version_parts) || return 1
  [ -n "$parts" ] || return 1
  ...
  IFS='.' read -r min_major min_minor min_patch min_extra <<< "$XO_TASKS_AXI_MIN"
  ...
  if [ "$major" -gt "$min_major" ] ||
    ...
    xo_tasks_axi_update_has_archive_body && xo_tasks_axi_mv_has_multi_id
    return $?
  fi
  return 1
}

xo_tasks_axi_update_has_archive_body() {
  local output
  command -v tasks-axi >/dev/null 2>&1 || return 1
...
xo_tasks_axi_backend_from_toml() {  # <toml-path>
  local toml=$1
  [ -f "$toml" ] || return 1
  LC_ALL=C awk '
    function trim(value) {
...
xo_tasks_axi_backend_resolve() {  # <tasks-axi-working-directory>
  ...
  if backend=$(xo_tasks_axi_backend_from_toml "$config"); then
    printf '%s\n' "$backend"
    return 0
  fi
  if [ -n "${HOME:-}" ]; then
...
xo_tasks_axi_backend() {  # <tasks-axi-working-directory>
  xo_tasks_axi_backend_resolve "$1"
}

xo_backlog_backend_value() {
...
xo_backlog_backend_manual() {
  local config_dir=$1
  [ "$(xo_backlog_backend_value "$config_dir")" = manual ]
}

xo_tasks_axi_backend_available() {
  local config_dir=$1
  xo_backlog_backend_manual "$config_dir" && return 1
  xo_tasks_axi_compatible
}
