...
# Sourced by bin/xo-spawn.sh, bin/xo-harness.sh, bin/xo-busy-lib.sh, and
...
# let xo launch an unrelated executable with Cursor flags.
#
...
# bin/xo-composer-lib.sh, which every backend already delegates to; an
...
XO_CURSOR_PROBE_TIMEOUT=${XO_CURSOR_PROBE_TIMEOUT:-10}
...
xo_cursor_path_is_cursor() {  # <path>
  local path=$1 canonical
  [ -n "$path" ] || return 1
  canonical=$(xo_cursor_canonical_path "$path") || return 1
...
xo_cursor_bounded_output() {  # <path> <args...>
  local path=$1 runner=
  shift
  ...
  [ -n "$runner" ] || return 1
  "$runner" "$XO_CURSOR_PROBE_TIMEOUT" "$path" "$@" 2>/dev/null
}

xo_cursor_probe_is_cursor() {  # <path>
  local path=$1 out
  out=$(xo_cursor_bounded_output "$path" --help) || return 1
  [ -n "$out" ] || return 1
  case "$out" in
...
xo_cursor_verify_executable() {  # <path>
  local path=$1
  [ -n "$path" ] && [ -x "$path" ] || return 1
  case "${path##*/}" in cursor-agent) return 0 ;; esac
  xo_cursor_path_is_cursor "$path" && return 0
  xo_cursor_probe_is_cursor "$path"
}

xo_cursor_list_models() {  # <path>
  xo_cursor_bounded_output "$1" --list-models
}

xo_cursor_catalog_has_model() {  # <model>
  local wanted=$1
  awk -v wanted="$wanted" '
...
#
# Resolution order, shared by bin/xo-spawn.sh and bin/xo-remote-doctor.sh:
...
# xo_cursor_verify_executable before it is accepted, so an unrelated executable
...
xo_cursor_resolve_binary() {
  local name candidate
  for name in cursor-agent agent; do
    candidate=$(command -v "$name" 2>/dev/null || true)
    [ -n "$candidate" ] && [ -x "$candidate" ] || continue
    if xo_cursor_verify_executable "$candidate"; then
      printf '%s\n' "$candidate"
      return 0
    fi
  done
...
xo_cursor_argv0_for_pid() {  # <pid> [comm-fallback]
  local pid=$1 fallback=${2:-} proc_root=${XO_PROC_ROOT_OVERRIDE:-/proc} argv0=
...
xo_cursor_argv0_is_cursor() {  # <argv0>
  local argv0=$1
  [ -n "$argv0" ] || return 1
  case "$argv0" in
    ''|MainThread) return 1 ;;
  ...
  xo_cursor_path_is_cursor "$argv0"
}
...
# (bin/xo-session-lock-lib.sh), harness detection (bin/xo-harness.sh), pane
# liveness (bin/backends/tmux.sh), and worker-server discovery (bin/xo-spawn.sh).
#
...
xo_cursor_process_matches() {  # <comm> <args> [argv0]
  ...
  case "$base" in
    ...
    agent|MainThread|node|node-*|node[0-9]*|python|python[0-9]*|python[0-9].[0-9]*)
      xo_cursor_argv0_is_cursor "$argv0" && return 0
      # A legacy alias may also be reported by its own path in comm.
      xo_cursor_path_is_cursor "$comm" && return 0
      return 1
      ;;
  esac
  # A version-named or otherwise renamed executable still identifies through
  # its install path.
  case "$comm" in */*) xo_cursor_path_is_cursor "$comm" && return 0 ;; esac
  return 1
}
