#!/usr/bin/env bash
# xo-lease-lib.sh - the per-task supervision lease contract (one owner).
#
# WHY. On the Pi supervision branch (docs/pi-supervision-branch.md), two LLM
# actors share one xo home inside one pi process: MAIN (the captain's
...
#     lease-command lock; leases never coordinate across xo homes.
#   - Actors: exactly "main" and "branch". The current actor is
#     $XO_SUPERVISION_ACTOR when set, else "main". The branch's shell gets
#     XO_SUPERVISION_ACTOR=branch injected deterministically by the Pi branch
...
#     session-lock holder, or XO_LEASE_HOLDER_PID - see bin/xo-lease.sh), and
...
# CONFUSED-AGENT-GRADE, the same grade bin/xo-gate-refuse-lib.sh documents
...
#   - Guard semantics (xo_lease_guard): no lease, a same-actor lease, or a
#     provably stale lease passes; a live lease held by the OTHER actor
#     refuses with exit XO_LEASE_REFUSE_EXIT. In a Pi supervision context the
#     guard retains the lease-command lock until xo_lease_guard_release, so the
...
#   - Role partition (xo_lease_forbid_branch): actions MAIN alone owns -
...
#
# Sourced by bin/xo-send.sh, bin/xo-control.sh, bin/xo-teardown.sh,
# bin/xo-pr-merge.sh, bin/xo-merge-local.sh, bin/xo-spawn.sh, and
# bin/xo-lease.sh. Callers must have $STATE resolved before calling. No side
# effects on source. set -u / set -e safe.

# Distinct from usage errors (2), the gate refusal (3), and xo-send's
# unconfirmed submit (3): recognizable as "the other supervision actor holds
# this task right now - retry after the lease clears".
XO_LEASE_REFUSE_EXIT=6
XO_LEASE_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XO_LEASE_GUARD_LOCK=

xo_lease_lock_helpers() {
  command -v xo_lock_acquire_wait >/dev/null 2>&1 && return 0
  # xo-wake-lib.sh is a canonical lint root in its own right and is already
  ...
  . "$XO_LEASE_LIB_DIR/xo-wake-lib.sh"
}

# xo_lease_actor: print the current actor after validating it. Returns 1 (with
# stderr) for an unknown XO_SUPERVISION_ACTOR value.
xo_lease_actor() {
  local actor=${XO_SUPERVISION_ACTOR:-main}
  case "$actor" in
    main|branch) printf '%s\n' "$actor" ;;
    *)
      echo "error: unknown XO_SUPERVISION_ACTOR '$actor' (expected main or branch)" >&2
      return 1
      ;;
  esac
}

# xo_lease_valid_id <id>: 0 iff the task/resource id is safe to embed in a
# state filename.
xo_lease_valid_id() {
  case "${1:-}" in
...
# xo_lease_read <task>: read the lease into XO_LEASE_ACTOR/XO_LEASE_PID/
# XO_LEASE_EPOCH. Returns 1 when no lease file exists. A malformed lease
# (unreadable actor or pid) reads as actor "" so callers treat it as stale
# rather than blocking forever on a torn record.
xo_lease_read() {
  local file line
  file=$(xo_lease_path "$1")
  XO_LEASE_ACTOR=
  XO_LEASE_PID=
  XO_LEASE_EPOCH=
  [ -e "$file" ] || return 1
  IFS= read -r line < "$file" 2>/dev/null || line=
  XO_LEASE_ACTOR=$(printf '%s' "$line" | cut -f1)
  XO_LEASE_PID=$(printf '%s' "$line" | cut -f2)
  # shellcheck disable=SC2034 # Consumed by sourcing callers (bin/xo-lease.sh check).
  XO_LEASE_EPOCH=$(printf '%s' "$line" | cut -f3)
  case "$XO_LEASE_ACTOR" in
    main|branch) ;;
    *) XO_LEASE_ACTOR= ;;
  esac
  case "$XO_LEASE_PID" in
    '' | *[!0-9]*) XO_LEASE_PID= ;;
  esac
  return 0
}

# xo_lease_live <task>: 0 iff a well-formed lease exists in a Pi context, its
# recorded pid is alive, and that pid IS the current session-lock holder (see
# the staleness contract above).
xo_lease_live() {
  local lock_pid
  case "${PI_CODING_AGENT:-}:${XO_SUPERVISION_ACTOR:-}" in
    true:*|*:main|*:branch) ;;
    *) return 1 ;;
  esac
  xo_lease_read "$1" || return 1
  [ -n "$XO_LEASE_ACTOR" ] || return 1
  [ -n "$XO_LEASE_PID" ] || return 1
  kill -0 "$XO_LEASE_PID" 2>/dev/null || return 1
  lock_pid=$(head -n 1 "$STATE/.lock" 2>/dev/null || true)
  case "$lock_pid" in ''|0|1|*[!0-9]*) return 1 ;; esac
  [ "$XO_LEASE_PID" = "$lock_pid" ]
}

# xo_lease_clear_stale <task>: remove the lease file when it exists but is not
# live. Silent; never touches a live lease.
xo_lease_clear_stale() {
  local file
  file=$(xo_lease_path "$1")
  [ -e "$file" ] || return 0
  xo_lease_live "$1" && return 0
  rm -f -- "$file"
}

# xo_lease_guard <task> <action-label>: refuse (exit XO_LEASE_REFUSE_EXIT) when
...
# mutation; the caller must invoke xo_lease_guard_release from its EXIT cleanup.
...
xo_lease_guard() {
  ...
  xo_lease_valid_id "$task" || return 0
  actor=$(xo_lease_actor) || exit "$XO_LEASE_REFUSE_EXIT"
  case "${PI_CODING_AGENT:-}:${XO_SUPERVISION_ACTOR:-}" in
    true:*|*:main|*:branch) active=1 ;;
  esac
  [ "$active" = 1 ] || [ -e "$(xo_lease_path "$task")" ] || return 0
  xo_lease_lock_helpers
  lock="$STATE/.xo-lease-command.lock"
  ...
  if [ "$XO_LEASE_GUARD_LOCK" != "$lock" ]; then
    xo_lock_acquire_wait "$lock"
    XO_LEASE_GUARD_LOCK=$lock
  fi
  if ! xo_lease_live "$task"; then
    xo_lease_clear_stale "$task" || { xo_lease_guard_release; return 1; }
    if [ "$active" != 1 ]; then
      xo_lease_guard_release
    fi
    return 0
  fi
  lease_actor=$XO_LEASE_ACTOR
...
# Release the claim/guard serialization lock retained by xo_lease_guard.
# Idempotent so callers can use it unconditionally from existing EXIT cleanup.
xo_lease_guard_release() {
  local lock=$XO_LEASE_GUARD_LOCK
  [ -n "$lock" ] || return 0
  XO_LEASE_GUARD_LOCK=
  xo_lock_release "$lock"
}

# xo_lease_forbid_branch <action-label>: refuse (exit XO_LEASE_REFUSE_EXIT)
