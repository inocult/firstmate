"""xo_voice_frame.py - the wire format between the voice client and the relay.
...
Kinds the relay sends down to the client:
  ...
  M  JSON {"mark": ..., "since_talk_end": ..., "tool_calls": ...}, one relay-side
     ...
     tool_answered and reply_end; bin/xo-voice-relay.py owns what each means.
  B  bye, empty payload
...
This module is copied to the laptop beside xo-voice-client.py, so it imports
