#!/usr/bin/env bash
# xo-tool-update-check.sh - report watched tooling that has an update available,
# and tooling whose update is installed but not in effect.
#
# Usage:
#   xo-tool-update-check.sh [check]
#   xo-tool-update-check.sh arm
#   xo-tool-update-check.sh disarm
#   xo-tool-update-check.sh --help
#
...
# state/tool-updates.check.sh and binds its bytes with xo-check-register.sh, so
# the watcher dispatches it on its normal XO_CHECK_INTERVAL cadence and turns
...
# XO_TOOL_UPDATE_INTERVAL (default 900, 0 disables the gate, otherwise 60..86400)
# and stays silent in between. Each probe is bounded by
# XO_TOOL_UPDATE_PROBE_SECS (default 5, valid 1..30) and a whole sweep by
# XO_TOOL_UPDATE_BUDGET_SECS (default 20, valid 1..120).
#
...
# larger than XO_CHECK_TIMEOUT (default 30, read from this check's own
...
usage() {
  cat <<'EOF'
Usage:
  xo-tool-update-check.sh [check]   report watched tools needing attention (silent when current)
  xo-tool-update-check.sh arm       write and register state/tool-updates.check.sh
  xo-tool-update-check.sh disarm    remove the check shim, its trust binding, and the record
  xo-tool-update-check.sh --help    print this help
...
die_usage() {
  printf 'xo-tool-update-check: %s\n' "$1" >&2
  usage >&2
  exit 2
}

INTERVAL=${XO_TOOL_UPDATE_INTERVAL:-900}
case "$INTERVAL" in
  ''|*[!0-9]*)
    printf 'xo-tool-update-check: XO_TOOL_UPDATE_INTERVAL must be 0 or a whole number from 60 to 86400\n' >&2
    exit 2
    ;;
esac
...
PROBE_SECS=${XO_TOOL_UPDATE_PROBE_SECS:-5}
case "$PROBE_SECS" in
  ''|*[!0-9]*|0)
    printf 'xo-tool-update-check: XO_TOOL_UPDATE_PROBE_SECS must be a whole number from 1 to 30\n' >&2
    exit 2
    ;;
esac
...
BUDGET_SECS=${XO_TOOL_UPDATE_BUDGET_SECS:-20}
case "$BUDGET_SECS" in
  ''|*[!0-9]*|0)
    printf 'xo-tool-update-check: XO_TOOL_UPDATE_BUDGET_SECS must be a whole number from 1 to 120\n' >&2
    exit 2
    ;;
esac
...
# The smallest bound a probe can be given, because xo_run_timed treats a
...
# xo_run_timed asks its runner for -k 1, so a probe that does not stop on TERM is
...
# PROBE_MIN_SECS, because xo_run_timed treats a non-positive bound as no bound.
probe_bound() {
  local left
...
path_hits() {
  ...
  while IFS= read -r dir; do
    ...
    identity=$(xo_pr_file_identity "$candidate" 2>/dev/null) || identity=
...
probe_output() {
  local path=$1
  shift
  xo_run_timed "$(probe_bound)" "$path" "$@" 2>&1
}
...
git_probe() {
  ...
  xo_run_timed "$(probe_bound)" git -C "$repo" "$@"
}
...
action_check() {
  ...
  if [ -n "$FINDINGS" ]; then
    ...
    xo_cap_line_var "tool updates: $FINDINGS" "$MAX_LINE"
...
shim_content() {
  ...
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    '# Auto-generated by xo-tool-update-check.sh - watched tool update poll shim.' \
    '# The watcher validates these bytes, then dispatches the trusted check script.' \
    "export XO_HOME=$(printf '%q' "$home")" \
    "exec $(printf '%q' "$SCRIPT_DIR/xo-tool-update-check.sh") check"
}
...
shim_write() {
  ...
  tmp=$(umask 077; mktemp "$STATE/.xo-tool-updates-check.XXXXXX" 2>/dev/null) || return 1
  SHIM_WRITE_TMP=$tmp
  if ! printf '%s\n' "$want" > "$tmp" \
    || ! chmod 0700 "$tmp" \
...
# wakes xo about unauthenticated state checks. So the one rule after a
...
arm_interrupted() {
  arm_rollback
  printf 'xo-tool-update-check: arming was interrupted, so state/%s.check.sh is not armed\n' "$CHECK_ID" >&2
  exit 1
}
...
action_arm() {
  local want home
  if [ ! -f "$CONFIG" ]; then
    printf 'xo-tool-update-check: no watched tool registry at %s\n' "$CONFIG" >&2
    return 1
  fi
  if ! config_validate || ! config_announce_patterns_usable; then
    printf 'xo-tool-update-check: %s (%s)\n' "$CONFIG_PROBLEM" "$CONFIG" >&2
    return 1
  fi
  ...
  case "$XO_HOME" in
    ...
    *)
      home=$(CDPATH='' cd -- "$XO_HOME" 2>/dev/null && pwd -P) || {
        printf 'xo-tool-update-check: cannot resolve XO_HOME %s\n' "$XO_HOME" >&2
        return 1
      }
      ;;
  esac
  ...
  if [ -f "$CHECK_SHIM" ] && [ ! -L "$CHECK_SHIM" ]; then
    ARM_BACKUP=$(shim_backup) || {
      printf 'xo-tool-update-check: could not save the existing %s\n' "$CHECK_SHIM" >&2
      return 1
    }
  fi
  ...
  if ! shim_write "$want"; then
    trap - HUP INT TERM
    arm_rollback
    printf 'xo-tool-update-check: could not write %s\n' "$CHECK_SHIM" >&2
    return 1
  fi
  if ! XO_HOME="$home" "$REGISTER_BIN" "$CHECK_ID" >/dev/null; then
    trap - HUP INT TERM
    arm_rollback
    printf 'xo-tool-update-check: could not register %s\n' "$CHECK_SHIM" >&2
    return 1
  fi
  trap - HUP INT TERM
