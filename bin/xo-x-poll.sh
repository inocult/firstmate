...
# via a non-empty XOX_PAIRING_TOKEN (from the home's .env or the environment).
...
# Its contract is "output => wake xo, silence => keep sleeping", so the
...
# relay includes is preserved for xox-respond to handle with continuity; the
...
# Also brings in bin/xo-x-lib.sh, which this script's relay client uses.
...
# of one per cycle; bin/xo-public-followup.sh consume clears it.
if xo_pf_has_events "$STATE"; then
  PF_ROOT=$(xo_pf_root "$STATE")
  PF_SIG=$(xo_pf_events_signature "$STATE" 2>/dev/null) || PF_SIG=
  if [ -n "$PF_SIG" ] \
    && [ "$(cat "$PF_ROOT/$XO_PF_SURFACED_BASENAME" 2>/dev/null || true)" != "$PF_SIG" ]; then
    if printf '%s\n' "$PF_SIG" \
      | xox_private_artifact_publish_stdin "$PF_ROOT" "$XO_PF_SURFACED_BASENAME" 600 2>/dev/null; then
...
emit_error_once() {
  local msg=$1
  if xox_private_artifact_file_valid "$STATE" "x-poll.error" 600 \
    && [ "$(cat "$ERROR_FILE" 2>/dev/null)" = "$msg" ]; then
    return 0
  fi
  printf '%s\n' "$msg" \
    | xox_private_artifact_publish_stdin "$STATE" "x-poll.error" 600 2>/dev/null || true
  printf 'x-mode-error %s\n' "$msg"
}

clear_error() {
  xox_private_artifact_dir_device "$STATE" >/dev/null 2>&1 || return 0
  rm -f "$ERROR_FILE" 2>/dev/null || true
}

emit_claim_error_once() {
  local msg=$1
  if xox_private_artifact_file_valid "$STATE" "x-poll.claim-error" 600 \
    && [ "$(cat "$CLAIM_ERROR_FILE" 2>/dev/null)" = "$msg" ]; then
    return 0
  fi
  printf '%s\n' "$msg" \
    | xox_private_artifact_publish_stdin "$STATE" "x-poll.claim-error" 600 2>/dev/null || true
  printf 'x-mode-error %s\n' "$msg"
}
...
BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/xo-x-poll.XXXXXX") || exit 0
AUTH_HEADER_FILE=
trap 'rm -f "$BODY_FILE" "$AUTH_HEADER_FILE"' EXIT
AUTH_HEADER_FILE=$(xox_auth_header_file) || { emit_error_once "invalid token"; exit 0; }
...
code=$(curl -m 5 -s -o "$BODY_FILE" -w '%{http_code}' \
  ...
  "$XOX_RELAY/connector/poll" 2>/dev/null) || exit 0
...
# Semantic worthiness is decided by xox-respond, so acknowledgments can still be
...
# The offer marker outlives the inbox file, which xox-respond removes after a
...
if xox_private_artifact_file_valid "$STATE/x-context" "$REQ.offered.json" 600; then
  clear_error
  clear_claim_error
  exit 0
fi
...
if ! (set -o pipefail; jq '.' "$BODY_FILE" 2>/dev/null \
  | xox_private_artifact_publish_stdin "$INBOX" "$REQ.json" 600); then
...
# payload. xox_context_registry_set is a no-op when the platform is unknown.
POLL_CTX=$(xox_extract_reply_context "$BODY_FILE" 2>/dev/null) || POLL_CTX=
if [ -n "$POLL_CTX" ]; then
  ...
  xox_context_registry_set "$STATE" "$REQ" "$POLL_PLATFORM" "$POLL_MAX" 2>/dev/null || true
fi

xox_offer_registry_claim "$STATE" "$REQ"
offer_rc=$?
case "$offer_rc" in
