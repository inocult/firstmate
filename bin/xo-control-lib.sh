#!/usr/bin/env bash
# xo-control-lib.sh - the ONE executable owner of xo's agent lifecycle
# CONTROL-PLANE mechanics.
#
# Data plane vs control plane (captain-approved root architecture, 2026-07-13).
# bin/xo-send.sh is the DATA plane: conversational text for the agent to read,
...
#      bin/xo-send.sh's --key path reads the same table rather than a second
#      copy of it.
...
#      (bin/xo-backend.sh's xo_backend_agent_state) able to PROVE that an agent
...
# command records the command's basename instead (bin/xo-spawn.sh derives
...
xo_control_harness_family() {  # <recorded-harness>
  case "${1-}" in
    pi) printf 'pi' ;;
...
# and bin/xo-spawn.sh refuses a --secondmate launch on any of them. The control
# plane asks this BEFORE it stops anything, so an incompatible relaunch target is
# refused while the current agent is still running rather than after it has
# been stopped.
xo_control_harness_supports_kind() {  # <harness> <kind>
  local harness=${1-} kind=${2-}
  xo_control_harness_supported "$harness" || return 1
  case "$harness" in
...
# through Herdr).
xo_control_interrupt_key() {  # <harness>
  case "${1-}" in
...
xo_control_interrupt_repeat() {  # <harness>
  case "${1-}" in
    opencode) printf '2' ;;
...
# above.
xo_control_interrupt_clear_key() {  # <harness>
  case "${1-}" in
    muse) printf 'C-u' ;;
...
xo_control_interrupt_ack_source() {  # <harness>
  case "${1-}" in
...
xo_control_exit_command() {  # <harness>
  case "${1-}" in
...
# Ctrl+U (bin/backends/orca.sh's xo_backend_orca_send_key).
...
# herdr implement xo_backend_agent_state; zellij, orca, and cmux report
...
# line: worktree-resident hook files and xo-owned state tokens only,
# never a harness's own managed config.
xo_control_harness_wiring_paths() {  # <harness> <worktree> <state-dir> <id>
  ...
  case "$harness" in
    ...
    muse)
      ...
      # bound to the pane by these two xo-owned sidecars. A relaunch
    ...
    # gemini's busy-state and turn-end hooks live in an xo-owned
...
# The xo-owned global turn-end registry entry a harness mints per task.
...
# xo_control_harness_wiring_paths. Prints the registry path or nothing.
xo_control_harness_turnend_token_path() {  # <harness> <state-dir> <id>
...
xo_control_harness_turnend_auth_path() {  # <harness> <token>
  local harness=${1-} token=${2-}
  case "$token" in ''|*[!A-Za-z0-9._-]*) return 0 ;; esac
  case "$harness" in
    grok) printf '%s\n' "${GROK_HOME:-$HOME/.grok}/hooks/xo-turn-end.d/$token" ;;
    kimi) printf '%s\n' "$HOME/.kimi-code/xo-turn-end.d/$token" ;;
    *) return 0 ;;
  esac
}
