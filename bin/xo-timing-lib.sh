#!/usr/bin/env bash
# xo-timing-lib.sh - the single owner of the deferred network stage's elapsed-time
# instrumentation.
#
# Sourced, never executed.
#
# WHY THIS EXISTS. The deferred stage (bin/xo-startup-network.sh) publishes one
...
#
# OFF BY DEFAULT, AND INERT. Every helper is a no-op unless XO_TIMING_LOG names a
...
#   XO_TIMING_LOG       append-only record file. Unset or empty disables recording.
#                       Exported across process boundaries on purpose: the phases
#                       being measured run in bin/xo-bootstrap.sh and
#                       bin/xo-fleet-sync.sh, which are children of the stage.
#   XO_TIMING_EPOCH_MS  the run's start instant, so every record carries an offset
...
XO_TIMING_DETAIL_MAX=${XO_TIMING_DETAIL_MAX:-80}
...
# Ensure XO_TIMING_EPOCH_MS holds the origin every offset is measured from.
...
# as "$XO_TIMING_EPOCH_MS" and not through a command substitution: a substitution
...
xo_timing_epoch_ensure() {
  case "${XO_TIMING_EPOCH_MS:-}" in
    ''|*[!0-9]*)
      XO_TIMING_EPOCH_MS=$(xo_timing_now_ms)
      export XO_TIMING_EPOCH_MS
      ;;
  esac
}
...
xo_timing_start() {  # <file>
  local file=$1
  [ -n "$file" ] || return 0
  : > "$file" 2>/dev/null || return 0
  XO_TIMING_LOG=$file
  XO_TIMING_EPOCH_MS=$(xo_timing_now_ms)
  export XO_TIMING_LOG XO_TIMING_EPOCH_MS
}

xo_timing_sanitize() {  # <text>
  local text=${1:-}
  case "$text" in
    *[[:space:]]*) printf '%s\n' 'unrecordable'; return 0 ;;
  esac
  text=${text//[!A-Za-z0-9._@:\/+-]/_}
  printf '%s\n' "${text:0:$XO_TIMING_DETAIL_MAX}"
}
...
xo_timing_record() {  # <scope> <name> <start-ms> [detail]
  local scope=$1 name=$2 start=$3 detail=${4:-} now offset elapsed
  xo_timing_enabled || return 0
  case "$start" in ''|*[!0-9]*) return 0 ;; esac
  now=$(xo_timing_now_ms)
  xo_timing_epoch_ensure
  offset=$(( start - XO_TIMING_EPOCH_MS ))
  [ "$offset" -ge 0 ] || offset=0
  elapsed=$(( now - start ))
  [ "$elapsed" -ge 0 ] || elapsed=0
  printf 'v1\t%s\t%s\t%s\t%s\t%s\n' \
    "$(xo_timing_sanitize "$scope")" "$(xo_timing_sanitize "$name")" \
    "$offset" "$elapsed" "$(xo_timing_sanitize "$detail")" \
    >> "$XO_TIMING_LOG" 2>/dev/null || true
  return 0
}
