#!/usr/bin/env bash
# xo-task-inbox-lib.sh - the per-task steering inbox: durable records plus a
# constant doorbell.
#
...
# bin/xo-send.sh writes and rings locally, the host-local remote steer leg
# (bin/xo-remote-secondmate-control.sh cmd_send) writes idempotently and rings
# on the remote host, bin/xo-watch.sh polls and re-rings, and the brief
# scaffold (bin/xo-brief.sh) tells the worker how to read and acknowledge;
# none of them restates the format.
#
# Design (captain-adopted, data/xo-send-reliability-reframe-s1/report.md): the
...
#
# Record format (xo_task_inbox_write / xo_task_inbox_body):
#   schema=xo-task-inbox.v1
#   at=<utc timestamp>
...
#
# Re-ring ladder (xo_task_inbox_due_action): an unhandled message older than
# XO_TASK_INBOX_GRACE_SECS is due one delivery attempt per grace period; an
# attempt may ring or be skipped to protect proven pending composer text. After
# XO_TASK_INBOX_RING_MAX attempts without an acknowledgement it escalates. The
...
#
# xo_task_inbox_ring requires bin/xo-backend.sh's dispatch (sourced below); the
# other helpers are dependency-light. Sourced by bin/xo-send.sh, bin/xo-watch.sh,
# and tests. No side effects on source beyond its sourced libraries.
#
# Tunables (env):
#   XO_TASK_INBOX_GRACE_SECS   default 90; delivery-attempt grace and spacing
#   XO_TASK_INBOX_RING_MAX     default 3; delivery attempts before escalation

_XO_TASK_INBOX_LIB_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
...
. "$_XO_TASK_INBOX_LIB_DIR/xo-wake-lib.sh"
# shellcheck source=/dev/null
. "$_XO_TASK_INBOX_LIB_DIR/xo-backend.sh"

XO_TASK_INBOX_SCHEMA='xo-task-inbox.v1'
XO_TASK_INBOX_GRACE_DEFAULT=90
XO_TASK_INBOX_RING_MAX_DEFAULT=3
XO_TASK_INBOX_LOCK_WAIT_DEFAULT=5

xo_task_inbox_grace_secs() {
  local g=${XO_TASK_INBOX_GRACE_SECS:-$XO_TASK_INBOX_GRACE_DEFAULT}
  case "$g" in ''|*[!0-9]*) g=$XO_TASK_INBOX_GRACE_DEFAULT ;; esac
  printf '%s' "$g"
}

xo_task_inbox_ring_max() {
  local m=${XO_TASK_INBOX_RING_MAX:-$XO_TASK_INBOX_RING_MAX_DEFAULT}
  case "$m" in ''|*[!0-9]*) m=$XO_TASK_INBOX_RING_MAX_DEFAULT ;; esac
  printf '%s' "$m"
}

xo_task_inbox_dir() {  # <state-dir> <task-id>
  printf '%s/%s.inbox' "$1" "$2"
}

xo_task_inbox_handled_dir() {  # <state-dir> <task-id>
  printf '%s/%s.inbox/handled' "$1" "$2"
}

# Numeric sequence of one record basename, or fail for a non-record name.
xo_task_inbox_seq_of() {  # <basename>
  local n=${1%.msg}
  [ "$n" != "$1" ] || return 1
...
xo_task_inbox_next_seq() {  # <inbox-dir>
  local dir=$1 max=0 d f n
  for d in "$dir" "$dir/handled"; do
    for f in "$d"/*.msg; do
      [ -e "$f" ] || continue
      n=$(xo_task_inbox_seq_of "${f##*/}") || continue
      [ "$n" -le "$max" ] || max=$n
    done
  done
  printf '%03d' "$((max + 1))"
...
xo_task_inbox_lock_acquire() {  # <lock-path>
  local lock=$1 wait=${XO_TASK_INBOX_LOCK_WAIT_SECS:-$XO_TASK_INBOX_LOCK_WAIT_DEFAULT}
  local deadline probe
  case "$wait" in ''|*[!0-9]*) wait=$XO_TASK_INBOX_LOCK_WAIT_DEFAULT ;; esac
  probe=$(mktemp "${lock%/*}/.lock-probe.XXXXXX") || return 1
  rm -f "$probe" || return 1
  if [ ! -e "$lock" ] && [ ! -L "$lock" ]; then
    xo_lock_try_create "$lock" && return 0
  fi
...
_xo_task_inbox_write_record_locked() {  # <inbox-dir> <text> [delivery-mode]
  local dir=$1 text=$2 delivery_mode=${3:-} seq tmp rec status=0
  seq=$(xo_task_inbox_next_seq "$dir")
  rec="$dir/$seq.msg"
  tmp=$(mktemp "$dir/.staging.XXXXXX") || return 1
  {
    printf 'schema=%s\n' "$XO_TASK_INBOX_SCHEMA"
...
xo_task_inbox_write() {  # <state-dir> <task-id> <text> [delivery-mode]
  local state=$1 task=$2 text=$3 delivery_mode=${4:-} dir lock rec status=0
  dir=$(xo_task_inbox_dir "$state" "$task")
  mkdir -p "$dir/handled" || return 1
  lock="$dir/.seq.lock"
  xo_task_inbox_lock_acquire "$lock" || return 1
  rec=$(_xo_task_inbox_write_record_locked "$dir" "$text" "$delivery_mode") || status=1
...
# local plane keeps plain xo_task_inbox_write: its outcome is synchronous, so
# a repeated identical local steer is a deliberate new instruction.
xo_task_inbox_write_idempotent() {  # <state-dir> <task-id> <text> [delivery-mode]
    ...
    if printf '%s' "$text" > "$want"; then
      for f in "$dir"/*.msg "$dir/handled"/*.msg; do
        ...
        if [ "$delivery_mode" = fire-and-forget ]; then
          xo_task_inbox_is_fire_and_forget "$f" || continue
        elif xo_task_inbox_is_fire_and_forget "$f"; then
          continue
        fi
        if ! xo_task_inbox_body "$f" > "$have" 2>/dev/null; then
          case "$f" in
            "$dir"/*.msg)
              f="$dir/handled/${f##*/}"
              xo_task_inbox_body "$f" > "$have" 2>/dev/null || continue
              ;;
            *) continue ;;
          esac
        fi
...
xo_task_inbox_body() {  # <record-path>
  local line
  [ -f "$1" ] || return 1
...
xo_task_inbox_doorbell_line() {  # <record-path>
  ...
  printf ": XO instruction waiting: list '%s'/*.msg and, in numeric order, read and act on each, then mv each handled file to '%s'/handled/." \
    "$quoted" "$quoted"
}
...
xo_task_inbox_ring() {  # <backend> <target> <record-path> [expected-label]
  local backend=$1 target=$2 rec=$3 label=${4:-} line cstate verdict
  case "$(xo_backend_agent_state "$backend" "$target" 2>/dev/null || true)" in
    dead|missing) return 3 ;;
  esac
  if ! line=$(xo_task_inbox_doorbell_line "$rec"); then
    return 2
  fi
  cstate=$(xo_backend_composer_state "$backend" "$target" "$label" 2>/dev/null) || cstate=unknown
  case "$cstate" in
    pending) return 1 ;;
  esac
  ...
  if ! verdict=$(xo_backend_send_text_submit "$backend" "$target" "$line" 1 0.4 0.3 "$label" 2>/dev/null); then
    return 2
  fi
...
xo_task_inbox_is_fire_and_forget() {  # <record-path>
  local rec=$1
  if [ ! -f "$rec" ]; then
    rec="${rec%/*}/handled/${rec##*/}"
    [ -f "$rec" ] || return 1
...
xo_task_inbox_oldest_unhandled() {  # <state-dir> <task-id>
  local dir best='' best_n=0 f n
  dir=$(xo_task_inbox_dir "$1" "$2")
  for f in "$dir"/*.msg; do
    [ -e "$f" ] || continue
    xo_task_inbox_is_fire_and_forget "$f" && continue
    n=$(xo_task_inbox_seq_of "${f##*/}") || continue
...
# a fresh ladder.
xo_task_inbox_due_action() {  # <state-dir> <task-id>
  local dir oldest base now grace max ladder rec_base count last
  dir=$(xo_task_inbox_dir "$1" "$2")
  if ! oldest=$(xo_task_inbox_oldest_unhandled "$1" "$2"); then
    rm -f "$dir/.ring-state" "$dir/.escalated" 2>/dev/null || true
    printf 'quiet'
    return 0
  fi
  base=${oldest##*/}
  grace=$(xo_task_inbox_grace_secs)
  if [ "$(xo_path_age "$oldest")" -lt "$grace" ]; then
    printf 'quiet'
    return 0
  fi
  count=0
...
# unhandled.
xo_task_inbox_record_ring() {  # <state-dir> <task-id> <record-path>
...
xo_task_inbox_record_escalated() {  # <state-dir> <task-id> <record-path>
  local dir
  dir=$(xo_task_inbox_dir "$1" "$2")
  [ -d "$dir" ] || return 0
