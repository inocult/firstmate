#!/usr/bin/env bash
# Cursor `stop` hook adapter for an xo PRIMARY session: the park model.
#
...
# primary supervision:
#
#   PARK      while supervision is needed, foreground bin/xo-watch-arm.sh and
...
#   - XO_CURSOR_TURNEND_LOOP_CEILING bounds the payload's own loop_count from
#     inside, deliberately BELOW the registered loop_limit, so xo's bound
...
# xo_watch_arm_pi. Cursor identity keeps parking despite a leaked
...
LOOP_CEILING=${XO_CURSOR_TURNEND_LOOP_CEILING:-180}
BLOCK_BUDGET=${XO_CURSOR_TURNEND_BLOCK_BUDGET:-3}
ARM_ATTEMPTS=${XO_CURSOR_PARK_ATTEMPTS:-2}
POLL=${XO_CURSOR_PARK_POLL:-2}
LOCK_ATTEMPTS=${XO_CURSOR_LOCK_ATTEMPTS:-50}
...
lock_acquire_bounded() {  # <lock>
  ...
  while [ "$attempt" -lt "$LOCK_ATTEMPTS" ]; do
    xo_lock_try_acquire "$lock" && return 0
    attempt=$((attempt + 1))
...
emit_followup() {  # <kind> <body> [reset-budget]
  ...
  xo_operational_input_encode "$kind" "$body" encoded || exit 0
...
$reason"
  xo_operational_input_encode turn-end-guard "$body" encoded || exit 0
  response=$(jq -n --arg m "$encoded" '{followup_message:$m}' 2>/dev/null) || exit 0
...
current_session_still_ours() {
  ...
  xo_session_lock_owned_by_self "$STATE"
}
...
# case, delegated to bin/xo-lock.sh so acquisition keeps its single owner.
...
if [ "$LOOP_COUNT" -ge "$LOOP_CEILING" ]; then
  ...
  xo_supervision_needed "$STATE" "$GRACE" || exit 0
  emit_followup turn-end-guard "XO SUPERVISION FOLLOW-UP CEILING REACHED - this session has taken $LOOP_COUNT consecutive hook-driven turns without a captain message, so automatic wake delivery stops here to bound the loop. Queued wakes stay durable: run bin/xo-wake-drain.sh, handle them, and run its exact WAKE_ACK_REQUIRED command. Supervision resumes automatically at the next turn end after the captain's next message."
fi

# Away mode owns the watcher and its own triage; never park and never wake.
[ -e "$STATE/.afk" ] && exit 0

if ! xo_supervision_needed "$STATE" "$GRACE"; then
  budget_reset_if_ours
  exit 0
fi
...
while [ "$attempt" -lt "$ARM_ATTEMPTS" ]; do
  ...
  if [ -n "$ARM_OUT" ]; then
    "$SCRIPT_DIR/xo-watch-arm.sh" >"$ARM_OUT" 2>&1 &
  else
    "$SCRIPT_DIR/xo-watch-arm.sh" >/dev/null 2>&1 &
  fi
  ARM_PID=$!
...
if [ "$ACTIONABLE" -eq 1 ]; then
  ...
  emit_followup watcher "xo watcher wake - one supervision event needs a handling turn now.
$WAKE

Run bin/xo-wake-drain.sh first, handle the wake, then run its exact WAKE_ACK_REQUIRED --ack-through command. Until that post-handling acknowledgement, interruption leaves the wake durable for idempotent re-handling. This stop hook owns watcher continuity: when the handling turn ends, the next needed cycle parks automatically - do NOT run bin/xo-watch-arm.sh after an ordinary wake." reset-budget
fi
...
# time: bin/xo-turnend-guard.sh owns the block decision and its banner for every
# harness, and --cursor tells it this is Cursor's own registration rather than
# the Claude-settings duplicate.
GUARD_ERR=$(mktemp "${TMPDIR:-/tmp}/xo-turnend-cursor.XXXXXX") || exit 0
printf '%s' "$PAYLOAD" | "$SCRIPT_DIR/xo-turnend-guard.sh" --cursor 2>"$GUARD_ERR"
GUARD_RC=$?
