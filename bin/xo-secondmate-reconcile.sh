#!/usr/bin/env bash
# xo-secondmate-reconcile.sh - ask a secondmate to reconcile its own books, at
# most once per home per cooldown window.
#
# Usage:
#   xo-secondmate-reconcile.sh request --snapshot <file>|-
#   xo-secondmate-reconcile.sh process-requests
#   xo-secondmate-reconcile.sh notify [--snapshot <file>|-]
#   xo-secondmate-reconcile.sh nudged <mate-id>
#
...
# (bin/xo-backlog-transition-lib.sh), so what reaches here is what neither could
...
# home unreadable: bin/xo-fleet-snapshot.sh keeps its decisions, queued, landed,
...
#     (XO_RECONCILE_COOLDOWN_SECONDS, four hours). A recap or digest loop
...
#   - sending through bin/xo-send.sh's fire-and-forget plane, which records the
...
# xo-send under its final route lock, and before the cooldown commit so a retired
...
XO_RECONCILE_COOLDOWN_SECONDS=${XO_RECONCILE_COOLDOWN_SECONDS:-14400}
XO_RECONCILE_REQUEST_MAX_BYTES=${XO_RECONCILE_REQUEST_MAX_BYTES:-1048576}
case "$XO_RECONCILE_COOLDOWN_SECONDS" in
  ''|*[!0-9]*) echo "xo-secondmate-reconcile: XO_RECONCILE_COOLDOWN_SECONDS must be a whole number of seconds" >&2; exit 2 ;;
esac
case "$XO_RECONCILE_REQUEST_MAX_BYTES" in
  ''|*[!0-9]*|0) echo "xo-secondmate-reconcile: XO_RECONCILE_REQUEST_MAX_BYTES must be a positive whole number" >&2; exit 2 ;;
esac
...
release_active_locks() {
  [ -z "$ACTIVE_META_LOCK" ] || xo_lock_release "$ACTIVE_META_LOCK"
  ACTIVE_META_LOCK=
  [ -z "$ACTIVE_CONTROL_LOCK" ] || xo_lock_release "$ACTIVE_CONTROL_LOCK"
  ACTIVE_CONTROL_LOCK=
  [ -z "$ACTIVE_RECONCILE_LOCK" ] || xo_lock_release "$ACTIVE_RECONCILE_LOCK"
  ACTIVE_RECONCILE_LOCK=
  [ -z "$ACTIVE_REQUEST_LOCK" ] || xo_lock_release "$ACTIVE_REQUEST_LOCK"
  ACTIVE_REQUEST_LOCK=
}
...
usage() {
  cat <<'EOF'
usage: xo-secondmate-reconcile.sh request --snapshot <file>|-
       xo-secondmate-reconcile.sh process-requests
       xo-secondmate-reconcile.sh notify [--snapshot <file>|-]
       xo-secondmate-reconcile.sh nudged <mate-id>
...
notify   ask every secondmate home whose backlog disagrees with its own task
         ...
         Reads an xo-fleet-snapshot.v1 or xo-bearings.v1 document from
         --snapshot (or runs xo-fleet-snapshot.sh --json when omitted).
nudged   print the epoch second of the last reconcile nudge sent to <mate-id>.
EOF
}

fail() { echo "xo-secondmate-reconcile: $*" >&2; exit 2; }
...
cmd_request() {
  ...
  if [ "$snapshot_src" = - ]; then
    LC_ALL=C head -c "$((XO_RECONCILE_REQUEST_MAX_BYTES + 1))" > "$tmp" \
    ...
    LC_ALL=C head -c "$((XO_RECONCILE_REQUEST_MAX_BYTES + 1))" "$snapshot_src" > "$tmp" \
      || { rm -f -- "$tmp"; fail "cannot capture the snapshot"; }
  fi
  bytes=$(LC_ALL=C wc -c < "$tmp" | tr -d ' ')
  case "$bytes" in ''|*[!0-9]*) rm -f -- "$tmp"; fail "cannot size the captured snapshot" ;; esac
  if [ "$bytes" -gt "$XO_RECONCILE_REQUEST_MAX_BYTES" ]; then
    rm -f -- "$tmp"
    fail "captured snapshot exceeds XO_RECONCILE_REQUEST_MAX_BYTES"
  fi
  if ! jq -e -s '
    length == 1
    and (.[0].schema == "xo-bearings.v1" or .[0].schema == "xo-fleet-snapshot.v1")
  ' "$tmp" >/dev/null 2>&1; then
    rm -f -- "$tmp"
    fail "input is not exactly one xo-fleet-snapshot.v1 or xo-bearings.v1 document"
  fi
  if ! jq -e '
    if .schema == "xo-bearings.v1" then
...
cmd_process_requests() {
  ...
  if ! xo_lock_try_acquire "$process_lock"; then
    return 0
  fi
  ...
  for request in "$REQUEST_DIR"/.processing-request-*.json "$REQUEST_DIR"/request-*.json; do
    ...
    XO_HOME="$XO_HOME" XO_STATE_OVERRIDE="$STATE" \
      "$SCRIPT_DIR/xo-secondmate-reconcile.sh" notify --snapshot "$claimed" \
      > "$output" 2>&1 || rc=$?
    if [ "$rc" -eq 0 ] \
...
cmd_notify() {
  ...
  if [ -z "$snapshot_src" ]; then
    snapshot=$("$SCRIPT_DIR/xo-fleet-snapshot.sh" --json) || fail "cannot read the fleet snapshot"
  ...
  printf '%s' "$snapshot" | jq -e '
    .schema == "xo-fleet-snapshot.v1" or .schema == "xo-bearings.v1"
  ' >/dev/null 2>&1 || fail "input is not an xo-fleet-snapshot.v1 or xo-bearings.v1 document"
  ...
  # metadata never carries one (bin/xo-spawn.sh's spawn_remote_secondmate());
  ...
  while IFS=$'\037' read -r id sampled_spawn_gen sampled_host kind; do
    ...
    if ! xo_lock_try_acquire "$reconcile_lock"; then
    ...
    if [ -n "$last" ]; then
      ...
      if [ "$age" -ge 0 ] && [ "$age" -lt "$XO_RECONCILE_COOLDOWN_SECONDS" ]; then
        printf 'cooldown: %s %s\n' "$id" "$age"
        release_active_locks
        continue
      fi
    ...
    release_active_locks
    send_rc=0
    XO_TASK_INBOX_LOCK_WAIT_SECS=0 XO_SEND_EXPECTED_SPAWN_GEN="$sampled_spawn_gen" \
      XO_SEND_EXPECTED_REMOTE_HOST="$expected_remote_host" \
      "$SCRIPT_DIR/xo-send.sh" "$id" --fire-and-forget "$did" \
