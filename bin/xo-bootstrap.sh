...
#          without moving the host's XO copy. If either placement changes
...
#          recovery-grade state owned by bin/xo-backend.sh's
#          xo_backend_agent_state: skipped distinguishes an existing ambiguous
...
#          unless XO_BOOTSTRAP_VERBOSE_FACTS=1 requests BOOTSTRAP_INFO facts.
#          A TANGLE line means the xo primary checkout (XO_ROOT) is stranded
...
#          refresh relays any completed xo-fleet-sync.sh output before the
#          aggregate timeout skip line with timeout and elapsed seconds.
#          Set XO_FLEET_PRUNE=0 to skip branch pruning during that refresh.
...
#          (bin/xo-backlog-transition-lib.sh), so this sweep exists for the
...
#          bin/xo-secondmate-reconcile.sh's nudge stay as backstops. Replayed
#          transitions and restored In-flight rows print BOOTSTRAP_INFO facts.
#          Set XO_BOOTSTRAP_DETECT_ONLY=1 to skip the six MUTATING sweeps
...
#          xo-session-start.sh's read-only path when another live session holds
...
#          additive.
#          Set XO_BOOTSTRAP_NETWORK to split this run by whether a step talks to
...
#                 local pass.
#          XO_BOOTSTRAP_DETECT_ONLY composes with it unchanged, so `only` plus
#          detect-only is the read-only `gh auth status` probe on its own.
#          bin/xo-startup-network.sh owns the deferral: it runs the `only` phase
...
#          Set XO_BOOTSTRAP_LOCKED=1 alongside it when the sweeps are skipped
...
#        xo-bootstrap.sh install <tool>...
#          Install the named tools (only ones the captain approved).
#        xo-bootstrap.sh lavish-compatible
#          Exit 0 when lavish-axi meets LAVISH_AXI_MIN, 1 otherwise, printing
#          nothing; bin/xo-brief.sh uses it to gate scout Lavish hosting.
set -u
...
# shellcheck source=bin/xo-tasks-axi-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-tasks-axi-lib.sh"
# shellcheck source=bin/xo-backlog-transition-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-backlog-transition-lib.sh"
# shellcheck source=bin/xo-quota-axi-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-quota-axi-lib.sh"
# shellcheck source=bin/xo-tangle-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-tangle-lib.sh"
# shellcheck source=bin/xo-ff-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-ff-lib.sh"
# shellcheck source=bin/xo-cursor-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-cursor-lib.sh"
# shellcheck source=bin/xo-config-inherit-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-config-inherit-lib.sh"
# shellcheck source=bin/xo-secondmate-nudge-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-secondmate-nudge-lib.sh"
# shellcheck source=bin/xo-startup-memory-budget-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-startup-memory-budget-lib.sh"
# shellcheck source=bin/xo-x-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-x-lib.sh"
# shellcheck source=bin/xo-backend.sh disable=SC1091
. "$SCRIPT_DIR/xo-backend.sh"
# shellcheck source=bin/xo-remote-readiness-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-remote-readiness-lib.sh"
# xo-timing-lib.sh is inert unless XO_TIMING_LOG names a file, which only the
# deferred network stage sets, so an ordinary bootstrap run records nothing.
# shellcheck source=bin/xo-timing-lib.sh disable=SC1091
. "$SCRIPT_DIR/xo-timing-lib.sh"
...
# safety sweep.
case "${XO_BOOTSTRAP_NETWORK:-all}" in
  skip|only) XO_BOOTSTRAP_NETWORK_PHASE=${XO_BOOTSTRAP_NETWORK:-all} ;;
  *) XO_BOOTSTRAP_NETWORK_PHASE=all ;;
esac
local_phase() { [ "$XO_BOOTSTRAP_NETWORK_PHASE" != only ]; }
network_phase() { [ "$XO_BOOTSTRAP_NETWORK_PHASE" != skip ]; }

network_mutation_authorized() {
  local expected=${XO_BOOTSTRAP_NETWORK_LOCK_PID:-} current
  [ -n "$expected" ] || return 0
...
bootstrap_parallel_begin() {
  BOOTSTRAP_PAR_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xo-bootstrap-par.XXXXXX") || return 1
  BOOTSTRAP_PAR_N=0
  XO_BOOTSTRAP_PARALLEL_DIR=$BOOTSTRAP_PAR_DIR
  export XO_BOOTSTRAP_PARALLEL_DIR
}
...
bootstrap_parallel_finish() {
  ...
  unset XO_BOOTSTRAP_PARALLEL_DIR BOOTSTRAP_PAR_DIR BOOTSTRAP_PAR_N
}

secondmate_note_respawned() {  # <id>
  SECONDMATE_RESPAWNED_IDS="$SECONDMATE_RESPAWNED_IDS $1"
  [ -n "${XO_BOOTSTRAP_PARALLEL_DIR:-}" ] || return 0
  printf '%s\n' "$1" > "$XO_BOOTSTRAP_PARALLEL_DIR/respawned.$1"
}
...
fleet_sync_bootstrap_timeout() {
  local count timeout
  if [ -n "${XO_FLEET_SYNC_BOOTSTRAP_TIMEOUT:-}" ]; then
    case "$XO_FLEET_SYNC_BOOTSTRAP_TIMEOUT" in
      *[!0-9]*) echo 20 ;;
      *) echo "$XO_FLEET_SYNC_BOOTSTRAP_TIMEOUT" ;;
    esac
    return 0
  fi
...
fleet_sync() {
  [ -x "$XO_ROOT/bin/xo-fleet-sync.sh" ] || return 0
  [ -d "$PROJECTS" ] || return 0

  tmp=$(mktemp "${TMPDIR:-/tmp}/xo-fleet-sync.XXXXXX" 2>/dev/null) || return 0
  timeout=$(fleet_sync_bootstrap_timeout)
  monitor_was_on=0
  case $- in *m*) monitor_was_on=1 ;; esac
  set -m 2>/dev/null || true
  "$XO_ROOT/bin/xo-fleet-sync.sh" >"$tmp" 2>/dev/null &
  pid=$!
...
secondmate_sync() {
  # shellcheck source=bin/xo-wake-lib.sh disable=SC1091
  ...
  # the primary's commit (xo-ff-lib.sh), while a standalone clone without it is
  # skipped until /updatexo refreshes it from origin. A remote home is on
  # another machine, so its host is handed that same commit and imports it there
  # (bin/xo-remote-secondmate-control.sh); this side still fetches nothing.
  ...
  # /updatexo, startup owns the live-convergence send itself because it is
  ...
  [ -d "$STATE" ] || return 0
  local primary_head
  if ! primary_head=$(primary_head_commit "$XO_ROOT"); then
    local meta id
  ...
  FF_NUDGE_WINDOWS=""
  FF_SEEN_HOMES=""
  SECOND_MATE_NUDGE_MESSAGE=$XO_SECOND_MATE_NUDGE_MESSAGE
  REMOTE_SECOND_MATE_NUDGE_MESSAGE=$XO_REMOTE_SECOND_MATE_NUDGE_MESSAGE
  SECOND_MATE_NUDGE_PENDING_DIR="$STATE/.secondmate-nudge-pending"

  secondmate_nudge_marker_path() {
    xo_secondmate_nudge_marker_path "$STATE" "$1"
  }

  secondmate_write_nudge_marker() {
    local id=$1 home=$2 commit=$3 instr=$4 message=${5:-$SECOND_MATE_NUDGE_MESSAGE} remote=${6:-0}
    xo_secondmate_nudge_write "$STATE" "$id" "$home" "$commit" "$instr" "$message" "$remote"
  }
  ...
  secondmate_send_nudge() {
    ...
    if out=$(XO_HOME="$XO_HOME" XO_ROOT_OVERRIDE="$XO_ROOT" XO_STATE_OVERRIDE="$STATE" "$SCRIPT_DIR/xo-send.sh" "$selector" "$SECOND_MATE_NUDGE_MESSAGE" 2>&1); then
      rm -f "$marker"
  ...
  xo_ff_after_instruction_update() {
  ...
  secondmate_retry_pending_nudges() {
    ...
    for marker in "$SECOND_MATE_NUDGE_PENDING_DIR"/*.pending; do
      ...
      selector=$(xo_meta_get "$marker" selector)
      home=$(xo_meta_get "$marker" home)
      commit=$(xo_meta_get "$marker" commit)
      message=$(xo_meta_get "$marker" message)
      remote=$(xo_meta_get "$marker" remote)
      [ -n "$remote" ] || remote=0
      [ "$selector" = "xo-$id" ] || {
  ...
  local tmp line
  secondmate_retry_pending_nudges
  tmp=$(mktemp "${TMPDIR:-/tmp}/xo-secondmate-sync.XXXXXX" 2>/dev/null) || return 0
  ...
  rm -f "$tmp"
  unset -f xo_ff_after_instruction_update
  ...
  # FF_SEEN_HOMES is exactly that set, and xo-config-inherit-lib.sh owns the
  ...
  while IFS='|' read -r id home _window _meta; do
    ...
    home_lock=$(xo_config_inherit_lock_path "$home_real") || {
      echo "CONFIG_REREAD: secondmate $id: send failed: could not resolve per-home lock"
      continue
    }
    xo_lock_acquire_wait "$home_lock" || {
    ...
    if [ "$reread_skip_pending" -eq 0 ] \
      && xo_config_reread_retry_queue_is_full "$XO_HOME" "$id"; then
      xo_config_reread_retry_pending "$id" "$home_real" || true
      if xo_config_reread_retry_queue_is_full "$XO_HOME" "$id"; then
        echo "CONFIG_REREAD: secondmate $id: send failed: retry instruction queue is full"
        xo_lock_release "$home_lock" || true
        continue
      fi
    fi
    report=$(mktemp "${TMPDIR:-/tmp}/xo-bootstrap-inherit.XXXXXX" 2>/dev/null) || {
      echo "SECONDMATE_SYNC: secondmate $id: skipped: inheritance failed"
      xo_lock_release "$home_lock" || true
      continue
    }
    if XO_CONFIG_INHERIT_REPORT="$report" XO_CONFIG_INHERIT_LIVE=1 \
      propagate_secondmate_inheritance "$XO_HOME" "$home_real" "$CONFIG" "$DATA"; then
      :
    else
      echo "SECONDMATE_SYNC: secondmate $id: skipped: inheritance failed"
    fi
    if ! reread_out=$(XO_HOME="$XO_HOME" XO_ROOT_OVERRIDE="$XO_ROOT" \
      XO_STATE_OVERRIDE="$STATE" \
      XO_CONFIG_REREAD_SKIP_PENDING="$reread_skip_pending" \
      xo_config_send_reread_nudge "$id" "$home_real" "$report" 2>&1); then
  ...
  secondmate_sync_remote_one() {  # <id> <home> <remote-host>
    ...
    remote_lock=$(xo_remote_inherit_transaction_lock_path "$STATE" "$id" 2>/dev/null || true)
    if [ -z "$remote_lock" ] || ! xo_lock_acquire_wait "$remote_lock"; then
      echo "NUDGE_SECONDMATES: secondmate $id: send failed: cannot lock remote inheritance transaction"
      return 0
    fi
    if ! "$SCRIPT_DIR/xo-procevent-remote-reply.sh" arm "$id" >/dev/null 2>&1; then
    ...
    remote_pending=0
    if [ -f "$remote_marker" ] && [ "$(xo_meta_get "$remote_marker" remote)" = 1 ]; then remote_pending=1; fi
    ...
    if inherit_out=$(XO_CONFIG_INHERIT_LIVE=1 \
      "$SCRIPT_DIR/xo-remote-inherit-push.sh" "$id" "$remote_generation" 2>&1); then
    ...
    if [ "$converged" -eq 1 ] && [ "$nudge_needed" -eq 1 ]; then
      if out=$(XO_HOME="$XO_HOME" XO_ROOT_OVERRIDE="$XO_ROOT" XO_STATE_OVERRIDE="$STATE" \
        "$SCRIPT_DIR/xo-send.sh" "xo-$id" "$REMOTE_SECOND_MATE_NUDGE_MESSAGE" 2>&1); then
        rm -f "$remote_marker"
        [ "${XO_BOOTSTRAP_VERBOSE_FACTS:-0}" != 1 ] || echo "BOOTSTRAP_INFO: nudged remote xo-$id after convergence"
      else
        echo "NUDGE_SECONDMATES: secondmate $id: send failed: $(first_line "$out")"
      fi
  ...
  secondmate_sync_remote_one_timed() {  # <id> <home> <remote-host>
    local id=$1 home=$2 remote_host=$3 __xo_timing_stamp
    __xo_timing_stamp=$(xo_timing_now_ms)
    secondmate_sync_remote_one "$id" "$home" "$remote_host"
    xo_timing_record secondmate convergence "$__xo_timing_stamp" "$id@$remote_host"
  }
  ...
  local remote_host __xo_timing_stamp parallel=0
...
# XO_BOOTSTRAP_VERBOSE_FACTS as before; on the deferred network pass the digest
# is already out, so reporting it is what keeps the superseded record from being
# acted on.
report_relaunch() {  # <id> <cause> <where>
  [ "${XO_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ] || ! local_phase || return 0
...
secondmate_liveness_sweep() {
  ...
  # xo_backend_agent_state. A missing tmux pane is not enough: tmux must prove
  ...
  [ -d "$STATE" ] || return 0
  local meta id remote_host label __xo_timing_stamp parallel=0
...
secondmate_liveness_one_timed() {  # <meta> <id> <label>
  local meta=$1 id=$2 label=$3 __xo_timing_stamp
  __xo_timing_stamp=$(xo_timing_now_ms)
  secondmate_liveness_one "$meta" "$id"
  xo_timing_record secondmate liveness "$__xo_timing_stamp" "$label"
}
...
secondmate_liveness_one() {  # <meta> <id>
  ...
  if [ -n "$remote_host" ]; then
    remote_rc=0
    xo_remote_readiness_ensure "$SCRIPT_DIR" "$id" || remote_rc=$?
    ...
    if [ "$remote_rc" -ne 0 ]; then
      readiness_reason=$(printf '%s\n' "$XO_REMOTE_READINESS_OUT" \
        | awk '/^check [^=]+=(fixable|human):|^action:|^error:/ { print; exit }')
      [ -n "$readiness_reason" ] || readiness_reason=$(first_line "$XO_REMOTE_READINESS_OUT")
    ...
    if out=$("$SCRIPT_DIR/xo-on.sh" "$id" xo-remote-secondmate-control.sh state "$id" < /dev/null 2>/dev/null); then
      remote_rc=0
    else
      remote_rc=$?
    fi
    ...
    case "$agent_state" in
      alive)
        if route_out=$("$SCRIPT_DIR/xo-on.sh" "$id" xo-remote-secondmate-control.sh route "$id" < /dev/null 2>/dev/null); then
          remote_rc=0
        else
          remote_rc=$?
        fi
        ...
        [ "${XO_BOOTSTRAP_VERBOSE_FACTS:-0}" != 1 ] || echo "BOOTSTRAP_INFO: remote secondmate $id already live (host=$remote_host)"
        ;;
      dead|missing)
        cause="remote endpoint $agent_state on its configured host"
        if out=$(XO_SPAWN_NO_GUARD=1 "$XO_ROOT/bin/xo-spawn.sh" "$id" --secondmate 2>&1); then
          secondmate_note_respawned "$id"
  ...
  agent_state=$(xo_backend_agent_state "$backend" "$target" 2>/dev/null) || agent_state=unreadable
  case "$harness" in
  ...
  case "$agent_state" in
    alive)
      if [ "${XO_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ]; then
    ...
    dead|missing)
      if [ "$agent_state" = dead ]; then
        ...
        xo_backend_kill "$backend" "$target" 2>/dev/null || true
      else
...
secondmate_handoff_resume() {
  ...
  "$SCRIPT_DIR/xo-backlog-handoff.sh" --resume-pending >/dev/null 2>&1 || true
}
...
# xo_backend_required_tools (bin/xo-backend.sh). So a herdr/zellij/cmux home is
...
BACKEND_VALID=1
if ! BACKEND_TOOLS=$(xo_backend_required_tools "$BACKEND"); then
  BACKEND_VALID=0
  BACKEND_TOOLS=""
fi
...
#                            its bytes and invokes bin/xo-x-poll.sh directly
#   config/x-mode.env      - exports XO_CHECK_INTERVAL=30, sourced by the watcher
...
x_mode_setup() {
  ...
  x_mode_supervision_repair() {
    local out
    out=$("$SCRIPT_DIR/xo-supervision-instructions.sh" --repair-line 2>/dev/null) \
...
# xo-watch.sh polls the X check every 30s. Non-X instances have no such file and
...
# (bin/xo-backlog-transition-lib.sh), so remaining recovery cases include a
...
# snapshot's classifier and bin/xo-secondmate-reconcile.sh's nudge stay as
...
backlog_record_reconcile() {
  ...
  if ! xo_backlog_directory_present "$STATE" "state directory"; then
    echo "error: backlog reconciliation refused: $XO_BACKLOG_TRANSITION_ERROR" >&2
    return 2
  fi
  if xo_backlog_transition_applies "$CONFIG" "$DATA" "$BOOTSTRAP_BACKLOG_GATE_KIND"; then
    :
  else
    gate_status=$?
    if [ "$gate_status" -eq 2 ]; then
      echo "error: backlog reconciliation cannot access configured data directory $DATA ($XO_BACKLOG_TRANSITION_ERROR)" >&2
      return 2
    fi
    return 0
  fi
  # Keep the wake/lock library's source-time state-directory creation inside
  # this mutating sweep, so XO_BOOTSTRAP_DETECT_ONLY remains read-only.
  # shellcheck source=bin/xo-wake-lib.sh disable=SC1091
  . "$SCRIPT_DIR/xo-wake-lib.sh"
  ...
  for marker in "$STATE"/*.backlog-close; do
    ...
    if ! xo_backlog_record_present "$marker" "pending-close record" "$STATE"; then
      echo "BACKLOG_RECONCILE: unsafe pending close refused: $XO_BACKLOG_TRANSITION_ERROR"
      return 2
    fi
    label=$(basename "$marker" .backlog-close)
    control_lock="$STATE/.control-$label.lock"
    meta_lock=$(xo_meta_lock_path "$STATE/$label.meta") || continue
    xo_lock_try_acquire "$control_lock" || continue
    if ! xo_lock_try_acquire "$meta_lock"; then
      xo_lock_release "$control_lock"
      continue
    fi
    if xo_backlog_close_marker_replay "$STATE" "$marker" "$DATA"; then
      case "$XO_BACKLOG_CLOSE_REPLAY_RESULT" in
        closed)
      ...
      echo "BACKLOG_RECONCILE: $label: recorded backlog close could not be replayed: $XO_BACKLOG_TRANSITION_ERROR"
    fi
  ...
  for meta in "$STATE"/*.meta; do
    ...
    if ! xo_backlog_record_present "$meta" "task record" "$STATE"; then
      echo "BACKLOG_RECONCILE: unsafe worker record refused: $XO_BACKLOG_TRANSITION_ERROR"
      return 2
    fi
    has_record=1
    break
  ...
  for meta in "$STATE"/*.meta; do
    ...
    id=$(basename "$meta" .meta)
    meta_lock=$(xo_meta_lock_path "$meta") || continue
    xo_lock_try_acquire "$meta_lock" || continue
    if [ -e "$STATE/$id.backlog-close" ] || [ -L "$STATE/$id.backlog-close" ]; then
      xo_lock_release "$meta_lock"
      continue
    fi
    if ! xo_backlog_record_present "$meta" "task record" "$STATE"; then
      echo "BACKLOG_RECONCILE: $id: post-lock worker record check refused: $XO_BACKLOG_TRANSITION_ERROR"
      xo_lock_release "$meta_lock"
      return 2
    fi
    if [ "$(xo_meta_get "$meta" kind)" != secondmate ] \
       && [ "$(xo_meta_get "$meta" cleanup_recovery)" != orca ]; then
      row=
      if xo_backlog_row_probe "$DATA" "$id"; then
        row=$XO_BACKLOG_ROW_STATE
      elif [ "$XO_BACKLOG_ROW_RESULT" != not_found ]; then
        echo "BACKLOG_RECONCILE: $id: worker record exists but its backlog item could not be read: $XO_BACKLOG_ROW_ERROR"
      fi
      ...
      if [ "$row" = "queued no no" ]; then
        if xo_backlog_start "$DATA" "$id"; then
          echo "BOOTSTRAP_INFO: marked $id in flight to match the worker this home already owns"
        else
          echo "BACKLOG_RECONCILE: $id: worker record exists but its backlog item could not be moved to In flight: $XO_BACKLOG_TRANSITION_ERROR"
        fi
      fi
    fi
    xo_lock_release "$meta_lock"
...
startup_memory_budget_setup() {
  ...
  if ! xo_startup_memory_budget_materialize "$CONFIG"; then
    echo "STARTUP_MEMORY_BUDGET: invalid config/$XO_STARTUP_MEMORY_BUDGET_FILE - $XO_STARTUP_MEMORY_BUDGET_ERROR"
  fi
}
...
if [ "${1:-}" = "install" ]; then
  shift
  [ $# -gt 0 ] || { echo "usage: xo-bootstrap.sh install <tool>..." >&2; exit 1; }
  for t in "$@"; do
...
if [ "${XO_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ] && local_phase; then
  BOOTSTRAP_BACKLOG_GATE_KIND=secondmate
  if [ -e "$STATE" ] || [ -L "$STATE" ]; then
    if ! xo_backlog_directory_present "$STATE" "state directory"; then
      echo "error: bootstrap cannot reconcile task state ($XO_BACKLOG_TRANSITION_ERROR)" >&2
      exit 1
    fi
    for BOOTSTRAP_BACKLOG_MARKER in "$STATE"/*.backlog-close; do
      [ -e "$BOOTSTRAP_BACKLOG_MARKER" ] || [ -L "$BOOTSTRAP_BACKLOG_MARKER" ] || continue
      if ! xo_backlog_record_present "$BOOTSTRAP_BACKLOG_MARKER" "pending-close record" "$STATE"; then
        echo "error: bootstrap refused unsafe pending close ($XO_BACKLOG_TRANSITION_ERROR)" >&2
        exit 1
      fi
    ...
    if [ "$BOOTSTRAP_BACKLOG_GATE_KIND" = secondmate ]; then
      for BOOTSTRAP_BACKLOG_META in "$STATE"/*.meta; do
        ...
        if ! xo_backlog_record_present "$BOOTSTRAP_BACKLOG_META" "task record" "$STATE"; then
          echo "error: bootstrap refused unsafe worker record ($XO_BACKLOG_TRANSITION_ERROR)" >&2
          exit 1
        fi
        if [ "$(xo_meta_get "$BOOTSTRAP_BACKLOG_META" kind)" != secondmate ] \
           && [ "$(xo_meta_get "$BOOTSTRAP_BACKLOG_META" cleanup_recovery)" != orca ]; then
          BOOTSTRAP_BACKLOG_GATE_KIND=ship
          break
        fi
      done
  ...
  if xo_backlog_transition_applies "$CONFIG" "$DATA" "$BOOTSTRAP_BACKLOG_GATE_KIND"; then
    ...
    if [ "$BOOTSTRAP_BACKLOG_GATE_STATUS" -eq 2 ]; then
      echo "error: bootstrap cannot access configured backlog data directory $DATA ($XO_BACKLOG_TRANSITION_ERROR)" >&2
      exit 1
    fi
  fi
  startup_memory_budget_setup
...
detect_local_tools() {
  if [ "$BACKEND_VALID" -eq 0 ]; then
    echo "BACKEND_INVALID: $BACKEND (known: $XO_BACKEND_KNOWN)"
  fi
  for t in $BACKEND_TOOLS; do
    xo_backend_required_tool_available "$BACKEND" "$t" \
  ...
  if xo_backend_list_contains "$TOOLS" treehouse \
  ...
  if command -v quota-axi >/dev/null 2>&1 && ! xo_quota_axi_compatible; then
    echo "MISSING: quota-axi (install: $(install_cmd quota-axi))"
  fi
  if command -v tasks-axi >/dev/null 2>&1 && ! xo_tasks_axi_compatible; then
...
detect_skills_activation_links() {
  local entry files=""
  [ -d "$XO_ROOT/.agents/skills" ] || return 0
  while IFS= read -r entry; do
    [ -n "$entry" ] || continue
    if [ ! -L "$XO_ROOT/$entry" ] && [ -f "$XO_ROOT/$entry" ]; then
      files="${files:+$files, }$entry"
    fi
  done < <(git -C "$XO_ROOT" ls-files -s -- .agents/skills 2>/dev/null | awk -F'\t' '$1 ~ /^120000 / { print $2 }')
...
detect_local_config() {
  # Worktree-tangle check: the xo primary checkout (XO_ROOT) must sit on its
  # default branch, not a feature branch (see xo-tangle-lib.sh). Scoped to the
  # primary only; detached-HEAD worktrees and secondmate homes never trip it.
  tangle_branch=$(xo_primary_tangle_branch "$XO_ROOT" 2>/dev/null || true)
  if [ -n "$tangle_branch" ]; then
    tangle_default=$(xo_default_branch "$XO_ROOT" 2>/dev/null || echo main)
    if [ "${XO_BOOTSTRAP_DETECT_ONLY:-0}" = 1 ] && [ "${XO_BOOTSTRAP_LOCKED:-0}" != 1 ]; then
      ...
      echo "TANGLE: primary checkout on feature branch '$tangle_branch' (expected '$tangle_default'); the work is safe on that ref - restore the primary with: git -C $XO_ROOT checkout $tangle_default, then re-validate the branch in a proper worktree"
    fi
  fi
  crew=
  [ -f "$CONFIG/crew-harness" ] && crew=$(tr -d '[:space:]' < "$CONFIG/crew-harness" || true)
  if [ "${XO_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ] && [ -n "$crew" ] && [ "$crew" != "default" ]; then
  ...
  if [ "$crew" = cursor ] && ! xo_cursor_resolve_binary >/dev/null 2>&1; then
    echo "MISSING_MANUAL: cursor-agent (instructions: $(manual_install_url cursor-agent))"
  fi
  crew_dispatch_validate
  if [ "${XO_BOOTSTRAP_VERBOSE_FACTS:-0}" = 1 ] \
    && ! xo_backlog_backend_manual "$CONFIG" && xo_tasks_axi_compatible; then
...
# been (re)published, and at least XO_HOME_SUMMARY_FAILURE_REPORT attempts have
...
detect_home_summary_publication() {
  ...
  threshold=${XO_HOME_SUMMARY_FAILURE_REPORT:-2}
...
# xo-timing-lib.sh discards the record unless the caller asked for timings, and
...
if network_phase; then
  ...
  xo_timing_record phase gh-auth "$__xo_timing_stamp"
fi
local_phase && detect_local_config

if [ "${XO_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  if network_phase && network_sweep_authorized 'project clone refresh'; then
    fleet_sync_out=$(mktemp "${TMPDIR:-/tmp}/xo-bootstrap-fleet.XXXXXX") || fleet_sync_out=
    if [ -n "$fleet_sync_out" ]; then
      (
        __xo_timing_stamp=$(xo_timing_now_ms)
        fleet_sync
        xo_timing_record phase fleet-sync "$__xo_timing_stamp"
      ) >"$fleet_sync_out" 2>&1 &
      fleet_sync_pid=$!
    else
      __xo_timing_stamp=$(xo_timing_now_ms)
  ...
  if network_phase; then
    if network_sweep_authorized 'dead-secondmate relaunch'; then
      __xo_timing_stamp=$(xo_timing_now_ms)
      secondmate_liveness_sweep
      xo_timing_record phase secondmate-liveness "$__xo_timing_stamp"
    fi
    if network_sweep_authorized 'secondmate convergence'; then
      __xo_timing_stamp=$(xo_timing_now_ms)
      secondmate_sync
      xo_timing_record phase secondmate-sync "$__xo_timing_stamp"
    fi
    if network_sweep_authorized 'pending handoff delivery'; then
      __xo_timing_stamp=$(xo_timing_now_ms)
      secondmate_handoff_resume
      xo_timing_record phase handoff-delivery "$__xo_timing_stamp"
    fi
  fi
