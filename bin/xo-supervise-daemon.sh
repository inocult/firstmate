#!/usr/bin/env bash
# xo-supervise-daemon.sh — presence-gated sub-supervisor (closes #27's P2).
#
# Wraps bin/xo-watch.sh: runs it as a child, presents and classifies every
# durable wake after an actionable close, acknowledges only after routing, and
# either SELF-HANDLES the routine majority in bash (no xo turn) or
...
# signal/stale/heartbeat wakes cost zero xo context; only done/
...
# user message clears it and xo resumes full responsiveness.
# When afk is off, normal xo-watch.sh always-on triage is the active mechanism.
...
#
# IN-BAND OPERATIONAL INPUT. bin/xo-operational-input.sh constructs every
# current daemon injection as the typed away-supervisor kind after the stable
# XO_OPERATIONAL_PREFIX. A human cannot type its leading U+2063 from a normal
# keyboard at the start of a message, and Herdr transports it as text.
# XO's contract: a message that starts with the current prefix, or a
...
#     crash/restart/missed injection is recovered on the next xo-wake-drain.sh.
...
#     xo-classify-lib.sh's combined predicate - instead gets its own longer
...
#     undelivered past XO_MAX_DEFER_SECS, the daemon retries a normal flush and
...
#          XO_INJECT_SKIP           |-prefixes force-self-handle bypassing
#                                   classification (default "heartbeat"); empty
#                                   disables. Use sparingly: it overrides the
#                                   captain-relevant escalation for matching
#                                   kinds.
#          XO_STALE_ESCALATE_SECS   idle seconds before a stale pane escalates
#                                   as a possible wedge (default 240)
#          XO_PAUSE_RESURFACE_SECS  seconds a declared wait stays declared,
...
#          XO_ESCALATE_BATCH_SECS   buffer window for batched escalation
#                                   digests; 0 = flush immediately (default 90)
#          XO_HEARTBEAT_SCAN_SECS   cadence for the catch-all status scan
#                                   (default 300)
#          XO_HOUSEKEEPING_TICK     seconds between housekeeping passes while
#                                   the watcher is mid-cycle (default 15)
#          XO_BUSY_REGEX            optional rendered busy-signature override
#                                   for delivery guards and Grok's fallback
#          XO_COMPOSER_IDLE_RE      optional shared classifier override; see
#                                   docs/configuration.md for its safety gates
#          XO_MAX_DEFER_SECS        max seconds a buffered escalation may sit
#                                   undelivered before one normal flush attempt;
#                                   if that cannot confirm a submit, a wedge
#                                   alarm fires (default 300; 0 disables)
#          XO_WEDGE_ALARM_CHANNEL   override config/wedge-alarm with a single
...
#          XO_WEDGE_ALARM_EXEC      notifier seam: when set, every notifier
...
#          XO_WEDGE_ALARM_TIMEOUT_SECS seconds allowed for each notifier before
#                                   its watchdog terminates it and continues to the
#                                   next channel (default 10; invalid/zero uses the
#                                   default).
#          XO_INJECT_CONFIRM_RETRIES Enter-retry attempts on a swallowed Enter
#                                   (default 3); the digest is typed once, only
#                                   Enter is retried. Composer-empty detection is
#                                   structural and style-aware (bin/xo-tmux-lib.sh):
...
#          XO_INJECT_CONFIRM_SLEEP  seconds between daemon submit checks
#                                   (default 0.5)
#          XO_LOG_MAX_BYTES / XO_LOG_KEEP_LINES / XO_CRASH_*  log + crash guards
#          XO_STATE_OVERRIDE        alternate state dir (testing)
...
#          lock. A crashing xo-watch.sh is logged and restarted, never killing
#          the daemon; a tight crash-restart spin is detected and backed off.
set -u

XO_DAEMON_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XO_ROOT="${XO_ROOT_OVERRIDE:-$(cd "$XO_DAEMON_DIR/.." && pwd)}"
...
# corrected composer detection. Stale task rechecks use xo-backend.sh below.
# shellcheck source=bin/xo-tmux-lib.sh
. "$XO_DAEMON_DIR/xo-tmux-lib.sh"

# shellcheck source=bin/xo-backend.sh
. "$XO_DAEMON_DIR/xo-backend.sh"

# Canonical construction and parsing for every XO operational input.
# shellcheck source=bin/xo-operational-input.sh
. "$XO_DAEMON_DIR/xo-operational-input.sh"
...
. "$XO_DAEMON_DIR/xo-classify-lib.sh"
...
. "$XO_DAEMON_DIR/xo-afk-contract.sh"

# Supervisor-pane discovery (XO_SUPERVISOR_TARGET_DEFAULT,
# XO_SUPERVISOR_BACKEND_DEFAULT, discover_supervisor_target,
# discover_supervisor_backend). Shared with the script-owned away launcher
# (bin/xo-afk-launch.sh) so the captain-pane resolution has exactly one owner.
# shellcheck source=bin/xo-supervisor-target-lib.sh
. "$XO_DAEMON_DIR/xo-supervisor-target-lib.sh"

# The single owner of semantic busy state for recorded tasks
# (xo_busy_classify).
# shellcheck source=bin/xo-busy-lib.sh
. "$XO_DAEMON_DIR/xo-busy-lib.sh"

# --- tunables ---------------------------------------------------------------
# Supervisor backends this daemon knows how to inject into today. zellij, orca,
# and cmux are real backends elsewhere in xo (bin/xo-backend.sh) but this
...
# pane.
XO_SUPERVISOR_SUPPORTED_BACKENDS="tmux herdr"
...
# live in bin/xo-classify-lib.sh, shared with the always-on watcher.
# Composer-empty detection, submit acknowledgement, and the harness-scoped
# supervisor-pane busy guard live in bin/xo-tmux-lib.sh.
# XO_BUSY_REGEX also overrides Grok's isolated task-state fallback.
...
# --- presence-gating --------------------------------------------------------
# bin/xo-operational-input.sh owns the U+2063 XO_OP bytes and typed
...
# Resolve the effective state dir. XO_STATE_OVERRIDE wins (testing); otherwise
# $XO_HOME/state. Kept as a function so the pure
# classifiers can take an explicit state arg without depending on globals.
_state_root() { printf '%s' "${XO_STATE_OVERRIDE:-$XO_HOME/state}"; }

# --- portable stat (same trap as xo-watch.sh: no `stat -f || stat -c`) -------
...
# skill (enter) and by xo on user return (exit). Durable: a plain file,
...
# should_exit_afk: encodes xo's afk-exit contract as a testable function.
#   away posture inactive   -> 1 (nothing to exit; the posture is the record
#                              bin/xo-afk-contract.sh owns, or the legacy flag)
...
should_exit_afk() {  # <state> <message-text>
  local state=$1 msg=$2
  afk_active "$state" || xo_afk_contract_present "$state" || return 1
...
# marker (a daemon escalation), 1 otherwise (a real user message). XO's
...
message_is_injection() {  # <message-text>
  ...
  [ -n "$msg" ] || return 1
  case "$msg" in
    "$XO_INJECT_MARK"*) return 0 ;;
  esac
  return 1
}

# strip_injection_marker: remove a current typed away envelope, the landed
# untyped XO_OP prefix, or the legacy bare sentinel. Current grammar is
# delegated to its owner rather than reimplemented here.
strip_injection_marker() {  # <message-text>
  local msg=$1 body
  if xo_operational_input_body "$msg" body; then
    printf '%s' "$body"
    return
  fi
  case "$msg" in
    "$XO_OPERATIONAL_PREFIX"*) msg=${msg#"$XO_OPERATIONAL_PREFIX"} ;;
    "$XO_INJECT_MARK"*) msg=${msg#"$XO_INJECT_MARK"} ;;
  esac
  printf '%s' "$msg"
}
...
# bin/xo-supervisor-target-lib.sh (sourced above). xo_super_main below calls
# them exactly as before; the away launcher reuses the identical resolution to
# pass the captain pane in as XO_SUPERVISOR_TARGET.

# --- classification helpers (PURE: no side effects, testable) ---------------
# last_status_line, status_is_captain_relevant, window_to_task, and the
# status-span reader come from bin/xo-classify-lib.sh (sourced above),
# the single classifier shared with bin/xo-watch.sh. The decision-string wrappers
...
# summary xo would otherwise have to re-read.
...
classify_signal() {  # <reason-after-colon> <state>
  ...
  for f in $reason; do
    ...
    if [ "$rc" -eq 2 ]; then
      ...
      [ -n "${XO_STATUS_SPAN_ENDPOINT_FILE:-}" ] \
        && printf 'ERROR\t%s\t%s\n' "$task" "$sig" >> "$XO_STATUS_SPAN_ENDPOINT_FILE"
      rel=1
      continue
    fi
    endpoint=${record%%$'\t'*}
    ...
    [ -n "${XO_STATUS_SPAN_ENDPOINT_FILE:-}" ] \
      && printf '%s\t%s\t%s\n' "$task" "$endpoint" "$ident" >> "$XO_STATUS_SPAN_ENDPOINT_FILE"
    if [ "$rc" -eq 0 ]; then
      event=${rest#*$'\t'}
...
classify_stale() {  # <window> <state> [<span-record> <span-status>]
  ...
  if [ -n "$last" ] && status_is_paused_or_captain_held "$last"; then
    ...
    # (xo-classify-lib.sh owns which declarations qualify): an idle pane is
    ...
    # reuses the status line already read, no xo-crew-state.sh call, mirroring the
...
classify_check() {  # <full reason>  — check scripts print only when xo should wake
  printf 'escalate|%s' "$1"
}
...
migrate_watcher_pause_markers() {  # <state>
  ...
  for meta in "$state"/*.meta; do
    [ -e "$meta" ] || continue
    win=$(xo_backend_target_of_meta "$meta")
    [ -n "$win" ] || continue
...
# by bin/xo-composer-lib.sh), and future verdicts. The detector drops
...
# bin/xo-backend.sh's generic per-backend primitives rather than a hand-rolled
...
xo_daemon_primary_harness() {
  if [ -z "${XO_DAEMON_PRIMARY_HARNESS:-}" ]; then
    XO_DAEMON_PRIMARY_HARNESS=$("$XO_DAEMON_DIR/xo-harness.sh" 2>/dev/null || printf 'unknown')
    [ -n "$XO_DAEMON_PRIMARY_HARNESS" ] || XO_DAEMON_PRIMARY_HARNESS=unknown
  fi
  printf '%s' "$XO_DAEMON_PRIMARY_HARNESS"
}

pane_is_busy() {  # <target> [backend]
  local target=$1 backend=${2:-tmux} native tail40 harness
  harness=$(xo_daemon_primary_harness)
  native=$(xo_backend_busy_state "$backend" "$target" 2>/dev/null)
  case "$native" in
    busy) return 0 ;;
  esac
  tail40=$(xo_backend_capture "$backend" "$target" 40 2>/dev/null) || return 1
  printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -12 \
    | xo_busy_lines_match "$harness"
}

# pane_input_pending dispatches through xo_backend_composer_state and treats
...
pane_input_pending() {  # <target> [backend]
  ...
  [ "$(xo_backend_composer_state "$backend" "$target" 2>/dev/null)" != empty ]
}
...
# semantic busy-state contract (bin/xo-busy-lib.sh), 1 when it is not, and 2
...
stale_window_is_busy() {  # <window> <state>
  ...
  task=$(window_to_task "$win" "$state")
  label="xo-$task"
  tail40=$(xo_backend_capture "$backend" "$win" 40 "$label" 2>/dev/null) || return 2
  verdict=$(xo_busy_classify "$backend" "$win" "$harness" "$task" "$state" "$tail40")
  [ "${verdict%% *}" = busy ]
}
...
# non-empty, non-comment line. XO_WEDGE_ALARM_CHANNEL overrides the file with a
...
# Print the configured channel directives, one per line. XO_WEDGE_ALARM_CHANNEL
...
wedge_alarm_configured_channels() {
  ...
  if [ -n "${XO_WEDGE_ALARM_CHANNEL:-}" ]; then
    printf '%s\n' "$XO_WEDGE_ALARM_CHANNEL"
    return 0
  fi
  cfg="${XO_CONFIG_OVERRIDE:-$XO_HOME/config}/wedge-alarm"
  if [ -f "$cfg" ]; then
...
wedge_alarm_run_bounded() {
  ...
  shift
  timeout=${XO_WEDGE_ALARM_TIMEOUT_SECS:-$WEDGE_ALARM_TIMEOUT_SECS_DEFAULT}
  case "$timeout" in
...
# XO_WEDGE_ALARM_EXEC, when set, REPLACES the real notifier: the resolved channel
...
wedge_alarm_os_notifier_override() {  # <channel> <summary>
  local channel=$1 summary=$2 rc exec_override=${XO_WEDGE_ALARM_EXEC:-}
...
wedge_alarm_via_osascript() {  # <summary>
  ...
  wedge_alarm_run_bounded osascript osascript -e 'on run argv' \
    -e 'display notification (item 1 of argv) with title "xo: away-mode escalations WEDGED" sound name "Basso"' \
...
wedge_alarm_via_herdr() {  # <summary>
  ...
  wedge_alarm_run_bounded herdr herdr notification show "xo: away-mode escalations WEDGED" \
...
wedge_alarm_via_command() {  # <cmd> <summary>
  ...
  wedge_alarm_run_bounded command sh -c "$cmd" xo-wedge-alarm "$summary" \
...
wedge_alarm_emit() {  # <channel> <summary>
  local channel=$1 summary=$2 cmd=${3:-} rc exec_override=${XO_WEDGE_ALARM_EXEC:-} WEDGE_ALARM_EMIT_ACTIVE=1
  case "$exec_override" in
    '') ;;
    discard) return 0 ;;
    *)
...
# an ERROR, drops a durable marker xo/recovery can surface, flashes
...
inject_wedge_alarm() {  # <state> <age-seconds>
  ...
  max_defer="${XO_MAX_DEFER_SECS:-$MAX_DEFER_SECS_DEFAULT}"
  ...
  {
    printf 'xo away-mode inject WEDGED: %ss undelivered as of %s\n' "$age" "$(date '+%Y-%m-%dT%H:%M:%S%z')"
    printf 'The supervisor pane could not accept an escalation. Buffered items:\n'
    cat "$state/.subsuper-escalations" 2>/dev/null
  } 2>/dev/null > "$marker" || true
  target="${XO_SUPERVISOR_TARGET:-$XO_SUPERVISOR_TARGET_DEFAULT}"
  backend="${XO_SUPERVISOR_BACKEND:-$XO_SUPERVISOR_BACKEND_DEFAULT}"
  ...
  if [ "$backend" = tmux ]; then
    tmux display-message -t "$target" "xo: away-mode escalations WEDGED ${age}s — see $marker" 2>/dev/null || true
  fi
...
housekeeping() {  # <state>
  ...
  # (1) batch flush
  if [ "${XO_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ]; then
    escalate_flush "$state" || true
  else
    due=$(_oldest_line_age "$state/.subsuper-escalations")
    if [ "$due" -ge "${XO_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" ]; then
      escalate_flush "$state" || true
    fi
  fi
  ...
  for marker in "$state"/.subsuper-stale-*; do
    ...
    [ "$age" -ge "${XO_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}" ] || continue
  ...
  # (2b) pause re-surface recheck. A declared wait is waiting, not wedged (xo-classify-lib.sh's
  ...
  pause_secs=${XO_PAUSE_RESURFACE_SECS:-$XO_PAUSE_RESURFACE_SECS_DEFAULT}
  for marker in "$state"/.subsuper-paused-*; do
    ...
    until=
    bounded_until=0
    if status_is_captain_held "$last" && xo_afk_contract_present "$state"; then
      continue
    fi
  ...
  #     later routine append has already moved past; xo-classify-lib.sh's span
  #     read decides relevance, and the classified-through offset is the dedup.
  if [ "$(_file_age "$state/.subsuper-last-scan")" -ge "${XO_HEARTBEAT_SCAN_SECS:-$HEARTBEAT_SCAN_SECS_DEFAULT}" ]; then
...
window_for_task() {  # <task-key> [state]
  ...
  for w in $(tmux list-windows -a -F '#{session_name}:#{window_name}' 2>/dev/null | grep ':xo-' || true); do
...
inject_msg() {  # <message> [state]
  ...
  # daemon self-handles and stays quiet; xo drives the normal always-on
  ...
  xo_operational_input_encode away-supervisor "$msg" encoded || return 1
  msg=$encoded
  target="${XO_SUPERVISOR_TARGET:-$XO_SUPERVISOR_TARGET_DEFAULT}"
  # BACKEND-AWARE (previously a raw `tmux display-message` pane-exists probe):
  # dispatches through bin/xo-backend.sh so a herdr supervisor pane is checked
  # via the herdr adapter instead of always assuming tmux. Falls back to tmux
  # when unset (sourced/test contexts that never ran xo_super_main's startup
  # discovery), matching this function's pre-existing default assumption.
  backend="${XO_SUPERVISOR_BACKEND:-tmux}"
  xo_backend_target_exists "$backend" "$target" || return 1
  ...
  #      composer. The shared classifier (xo_backend_composer_state ->
  #      xo_composer_classify_content, bin/xo-composer-lib.sh) reports 'pending'
  ...
  composer=$(xo_backend_composer_state "$backend" "$target" 2>/dev/null)
  ...
  # Dispatches through xo_backend_send_text_submit (bin/xo-backend.sh): for
  # backend=tmux this calls xo_backend_tmux_send_text_submit, a verbatim
  # re-export of xo_tmux_submit_core - byte-identical to calling it directly.
  retries=${XO_INJECT_CONFIRM_RETRIES:-$INJECT_CONFIRM_RETRIES_DEFAULT}
  sleep_s=${XO_INJECT_CONFIRM_SLEEP:-$INJECT_CONFIRM_SLEEP_DEFAULT}
  verdict=$(xo_backend_send_text_submit "$backend" "$target" "$msg" "$retries" "$sleep_s" "$sleep_s")
...
should_force_self() {  # <reason>
  local reason=$1 skip="${XO_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}" prefix
  [ -n "$skip" ] || return 1
  local -a prefixes
...
# signal:<files> (bin/xo-watch.sh). Classify it as a signal so the capture file
...
handle_wake() {  # <reason> <state>
  ...
  if should_force_self "$reason"; then
    log "wake force-self (XO_INJECT_SKIP): $reason"
    rm -f "$capture"
    return
  fi
  case "$reason" in
    signal:*|needs-decision:*)
              ...
              decision=$(XO_STATUS_SPAN_ENDPOINT_FILE="$capture" classify_signal "$arg" "$state") ;;
  ...
  case "$action" in
    escalate)
      ...
      if escalate_add "$state" "$distilled"; then
        ...
        [ "${XO_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}" -le 0 ] && { escalate_flush "$state" || true; }
      else
...
handle_durable_wakes() {  # <watcher-reason> <state>
  ...
  if ! "$XO_DAEMON_DIR/xo-wake-drain.sh" > "$out" 2> "$err"; then
    cat "$err" >&2
    rm -f "$out" "$err"
    return 1
  fi
  ...
  "$XO_DAEMON_DIR/xo-wake-drain.sh" --ack-through "$ack_through" \
    --recovery-generation "$ack_generation"
}

# --- log --------------------------------------------------------------------
# Uses LOG set by xo_super_main; harmless no-op-ish if unset (tests source fns
...
trim_log() {
  ...
  [ "$sz" -ge "${XO_LOG_MAX_BYTES:-$LOG_MAX_BYTES_DEFAULT}" ] || return 0
  tmp=$(mktemp "${TMPDIR:-/tmp}/xo-daemon-log.XXXXXX") || return 0
  tail -n "${XO_LOG_KEEP_LINES:-$LOG_KEEP_LINES_DEFAULT}" "$LOG" >"$tmp" 2>/dev/null && mv -f "$tmp" "$LOG"
}

# ============================================================================
# Everything below runs only when the script is EXECUTED, not sourced. The pure
# classifiers above are sourceable for unit tests (tests/xo-daemon.test.sh).
# ============================================================================
...
xo_super_main() {
  ...
  # Export XO_STATE_OVERRIDE so the lib resolves the same state dir.
  # shellcheck source=bin/xo-wake-lib.sh
  XO_STATE_OVERRIDE="$STATE" . "$XO_DAEMON_DIR/xo-wake-lib.sh"

  local WATCH="$XO_DAEMON_DIR/xo-watch.sh"
  ...
  local INJECT_FAIL_SLEEP=${XO_INJECT_FAIL_SLEEP:-$INJECT_FAIL_SLEEP_DEFAULT}
  local CRASH_THRESHOLD=${XO_CRASH_THRESHOLD:-$CRASH_THRESHOLD_DEFAULT}
  local CRASH_WINDOW=${XO_CRASH_WINDOW:-$CRASH_WINDOW_DEFAULT}
  local CRASH_BACKOFF=${XO_CRASH_BACKOFF:-$CRASH_BACKOFF_DEFAULT}
  local CRASH_NORMAL_SLEEP=${XO_CRASH_NORMAL_SLEEP:-$CRASH_NORMAL_SLEEP_DEFAULT}
  ...
  if ! xo_lock_try_acquire "$LOCK"; then
    if [ -n "${XO_LOCK_HELD_PID:-}" ]; then
      echo "error: another xo-supervise-daemon is already running (pid $XO_LOCK_HELD_PID, lock $LOCK held)" >&2
    else
      echo "error: another xo-supervise-daemon is already running (lock $LOCK held)" >&2
    fi
    exit 1
  fi
  echo "$$" > "$PIDFILE"
  # The recorded identity is what proves this daemon still owns supervision after
  # its watcher child exits (xo_afk_daemon_owns_supervision, read by the turn-end
  ...
  if ! xo_pid_identity "${BASHPID:-$$}" > "$LOCK/pid-identity" 2>/dev/null; then
  ...
  # Priority: XO_SUPERVISOR_BACKEND override > $TMUX_PANE (tmux) > $HERDR_ENV=1
  ...
  # into XO_SUPERVISOR_BACKEND makes inject_msg/pane_is_busy/pane_input_pending
  ...
  backend_source="XO_SUPERVISOR_BACKEND"
  if [ -z "${XO_SUPERVISOR_BACKEND:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      backend_source="TMUX_PANE"
    elif [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
      backend_source="HERDR_ENV"
    else
      backend_source="FALLBACK($XO_SUPERVISOR_BACKEND_DEFAULT)"
    fi
  fi
  discovered_backend=$(discover_supervisor_backend) || true
  XO_SUPERVISOR_BACKEND="$discovered_backend"
  local BACKEND="$XO_SUPERVISOR_BACKEND"
  ...
  if ! xo_backend_list_contains "$XO_SUPERVISOR_SUPPORTED_BACKENDS" "$BACKEND"; then
    echo "error: away-mode daemon does not support supervisor backend '$BACKEND' yet (supported: $XO_SUPERVISOR_SUPPORTED_BACKENDS); set XO_SUPERVISOR_BACKEND=tmux|herdr and XO_SUPERVISOR_TARGET to run xo's own pane under a supported backend" >&2
    log "startup failed: unsupported supervisor backend '$BACKEND' (source=$backend_source)"
    xo_lock_release "$LOCK" 2>/dev/null || true
    rm -f "$PIDFILE" 2>/dev/null || true
    exit 1
  fi

  # --- auto-discover the supervisor target (the pane running xo) -----
  # Priority: XO_SUPERVISOR_TARGET override > $TMUX_PANE (tmux; inherited from
  # the pane that launched the daemon, normally xo's own) >
  # $HERDR_PANE_ID (herdr, composed into "<session>:<pane-id>") > xo:0
  # fallback. Exporting the result into XO_SUPERVISOR_TARGET makes inject_msg
  # (which reads that env var) use the discovered pane without an extra global.
  local discovered target_source
  target_source="XO_SUPERVISOR_TARGET"
  if [ -z "${XO_SUPERVISOR_TARGET:-}" ]; then
    if [ -n "${TMUX_PANE:-}" ]; then
      target_source="TMUX_PANE"
    elif [ "${HERDR_ENV:-}" = "1" ] && [ -n "${HERDR_PANE_ID:-}" ]; then
      target_source="HERDR_ENV(HERDR_PANE_ID)"
    else
      target_source="FALLBACK(xo:0)"
    fi
  fi
  if discovered=$(discover_supervisor_target); then
    : # resolved cleanly
  else
    echo "warn: could not auto-discover supervisor pane (no XO_SUPERVISOR_TARGET, TMUX_PANE, or HERDR_ENV/HERDR_PANE_ID); falling back to '$discovered' — verify this is xo's pane" >&2
  fi
  XO_SUPERVISOR_TARGET="$discovered"
  local TARGET="$XO_SUPERVISOR_TARGET"

  # --- validate supervisor target at startup (a missing target is a typo) ---
  # Dispatches through bin/xo-backend.sh instead of a raw `tmux display-message`
  ...
  if ! xo_backend_target_exists "$BACKEND" "$TARGET"; then
    echo "error: supervisor target '$TARGET' does not resolve to a $BACKEND pane; set XO_SUPERVISOR_TARGET" >&2
  ...
  log "daemon starting (pid $$); target=$TARGET; target_source=$target_source; backend=$BACKEND; backend_source=$backend_source; afk=$afk_status; inject_skip='${XO_INJECT_SKIP:-$INJECT_SKIP_DEFAULT}'; stale_escalate=${XO_STALE_ESCALATE_SECS:-$STALE_ESCALATE_SECS_DEFAULT}s; batch=${XO_ESCALATE_BATCH_SECS:-$ESCALATE_BATCH_SECS_DEFAULT}s"
  migrate_watcher_pause_markers "$STATE"
  ...
  start_watcher() {
    CUR_TMP=$(mktemp "${TMPDIR:-/tmp}/xo-watch.XXXXXX") || { log "error: mktemp failed; retrying in 5s"; sleep 5; return 1; }
  ...
  while true; do
    ...
    # has nowhere to go, and xo itself is the consumer of escalations.
    ...
    # The watcher child runs on its own XO_POLL cadence internally; we only need
    ...
    sleep 1
    if [ "$(_file_age "$STATE/.subsuper-last-housekeep")" -ge "${XO_HOUSEKEEPING_TICK:-$HOUSEKEEPING_TICK_DEFAULT}" ]; then
...
if [ "${BASH_SOURCE[0]}" = "${0}" ]; then
  ...
  # execs the daemon, see bin/xo-afk-start.sh). Make it structurally impossible
  # for a sourced context to fire a real desktop notification from the wedge
  # alarm: default the XO_WEDGE_ALARM_EXEC notifier seam to "discard" unless the
  ...
  : "${XO_WEDGE_ALARM_EXEC:=discard}"
  export XO_WEDGE_ALARM_EXEC
fi
