...
#
# Usage: xo-x-dismiss.sh <request_id>
#
# When xo decides NOT to reply to a mention (a pure acknowledgment, or any
...
usage() {
  echo "usage: xo-x-dismiss.sh <request_id>" >&2
}
...
case "$REQ" in
  ''|.*|*[!A-Za-z0-9._-]*) echo "xo-x-dismiss: unsafe request_id: $REQ" >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "xo-x-dismiss: jq not found" >&2; exit 1; }
