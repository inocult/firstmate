...
# Usage: xo-send.sh <target> [--resolve-key <key>]... [--fire-and-forget <delivery-id>] <text...>
#   <target> may be an exact task id, a legacy xo-<id> task label resolved
#   through this home's state/<id>.meta, or an explicit well-formed backend
#   target. xo-send refuses unresolved guesses rather than falling back to a
#   tmux window search, because a "successful" send to the wrong endpoint is
#   worse than a loud failure.
# Special keys instead of text: xo-send.sh <target> --key Enter
...
# local task, or the remote home's host-local inbox reached through xo-on.sh
...
# request onto the existing record (bin/xo-task-inbox-lib.sh), so after a lost
# transport (ssh exit 255, completion unknown) xo-send retries the same leg
# once itself. For an ordinary reply-bearing request, a later re-run is
# idempotent only through the printed XO_PENDING_REPLY_EXISTING_CORR=<corr>
...
# bin/xo-task-inbox-lib.sh owns the record format, the doorbell line, and the
...
# bin/xo-tmux-lib.sh. Tune with XO_SEND_RETRIES (default 3) / XO_SEND_SLEEP
...
# xo-send-secondmate-harness-invocation-r1 owns that behavior.
#
...
# delivery (bin/xo-pending-reply-lib.sh). Delivery
...
# Set XO_PENDING_REPLY_EXISTING_CORR=<id> when re-sending a recovery request
...
#
# Remote secondmate delivery: the send crosses xo-on.sh to a host-local leg
# (bin/xo-remote-secondmate-control.sh cmd_send) that writes the message as a
...
# locally: leg exit 0 means durably recorded (xo-send then exits 0, marks the
...
# so xo-send retries the identical leg once - safe because the remote write
...
# XO_PENDING_REPLY_EXISTING_CORR=<id> resend command that preserves the body
...
# retry. Every remote transport attempt is bounded by XO_SEND_REMOTE_BUDGET
...
# semantic callers may set XO_SEND_EXPECTED_SPAWN_GEN or
# XO_SEND_EXPECTED_REMOTE_HOST to require that sampled identity to still match
...
# blocked: record in the target task's state/<id>.status. xo-send itself
...
# (pending-reply-* today; bin/xo-classify-lib.sh's reserved-key guard) is
# closed with the owning library's vocabulary note
# (xo_pending_reply_close_note_for_key / xo_pending_reply_resolved_note), so
...
# open-decision ledger xo-wake-drain folds lives in this home's own state dir
...
# feeds bin/xo-captain-hold.sh's one keyed-answer intake for any key that names
# a captain-held task in this home - the key as a task id itself, or through
# the legacy `<task>-decision-<key>` identity for pre-collapse rows. xo-send
...
# in this home's status log per status_open_decisions (bin/xo-classify-lib.sh), or
...
#
# After a successful TYPED-plane submit xo-send pauses XO_SEND_SETTLE seconds
...
# a crewmate (see bin/xo-gate-refuse-lib.sh).
xo_refuse_if_gate_agent

if [ -z "${XO_HOME+x}" ] || [ -z "${XO_HOME:-}" ]; then
  echo "error: XO_HOME is not set; xo-send refuses to resolve targets without an explicit xo home" >&2
  exit 1
fi

STATE="${XO_STATE_OVERRIDE:-$XO_HOME/state}"
if [ ! -d "$XO_HOME" ]; then
  echo "error: XO_HOME '$XO_HOME' is not a directory; xo-send cannot resolve this home's state" >&2
  exit 1
fi
if [ ! -d "$STATE" ]; then
  echo "error: state dir '$STATE' is missing; xo-send cannot resolve targets for XO_HOME '$XO_HOME'" >&2
  exit 1
fi
...
XO_GUARD_CONTINUE_LINE='This is a supervision warning only; the requested message WILL still be sent.' "$SCRIPT_DIR/xo-guard.sh" || true

xo_send_id_from_meta() {  # <meta-file>
  local base
  base=${1##*/}
  printf '%s' "${base%.meta}"
}

# xo_send_clear_after_interrupt: muse RESTORES the interrupted prompt back into
...
# control-plane capability table (bin/xo-control-lib.sh) rather than a second
# copy here - the same table bin/xo-control.sh's interrupt verb reads.
xo_send_clear_after_interrupt() {  # <key>
  local key=$1 family clear
  [ "$key" = Escape ] || return 0
  family=$(xo_control_harness_family "$TARGET_HARNESS") || return 0
  clear=$(xo_control_interrupt_clear_key "$family") || return 0
  [ -n "$clear" ] || return 0
  [ "$TARGET_BACKEND" != remote ] || return 0
  if ! xo_backend_send_key "$TARGET_BACKEND" "$T" "$clear" "$EXPECTED_LABEL"; then
...
xo_send_record_interrupt() {  # <key>
  local key=$1 id gen
  [ "$key" = Escape ] || return 0
  case "$TARGET_HARNESS" in claude*) : ;; *) return 0 ;; esac
  [ -n "$TARGET_META" ] || return 0
  id=$(xo_send_id_from_meta "$TARGET_META")
  [ -f "$STATE/$id.busy-gen" ] || return 0
  gen=$(xo_meta_get "$TARGET_META" busy_gen)
  if [ -n "$gen" ]; then
    "$XO_ROOT/bin/xo-busy-event.sh" apply "$STATE" "$id" idle \
      --gen "$gen" --source xo-interrupt --event interrupt
  else
    "$XO_ROOT/bin/xo-busy-event.sh" apply "$STATE" "$id" idle \
      --current-gen --source xo-interrupt --event interrupt
  fi || {
...
xo_send_meta_for_key_value() {  # <state-dir> <key> <value>
  local state=$1 key=$2 value=$3 meta got
  for meta in "$state"/*.meta; do
    [ -e "$meta" ] || continue
    got=$(xo_meta_get "$meta" "$key")
    [ "$got" = "$value" ] || continue
    printf '%s' "$meta"
    return 0
  done
...
xo_send_count_colons() {  # <string>
  local s=$1 no_colons
  no_colons=${s//:/}
  printf '%s' $(( ${#s} - ${#no_colons} ))
}

xo_send_resolve_target() {  # <raw-target>
  ...
  if [ -n "$meta" ]; then
    if [ -n "$(xo_meta_get "$meta" remote_host)" ]; then
      id=$(xo_send_id_from_meta "$meta")
      RESOLVED_TARGET="remote:$id"
      TARGET_BACKEND=remote
      TARGET_META=$meta
      TARGET_HARNESS=$(xo_meta_get "$meta" harness)
      EXPECTED_LABEL="xo-$id"
      TARGET_SELECTOR=1
      TARGET_REMOTE_ID=$id
      TARGET_REMOTE_HOST=$(xo_meta_get "$meta" remote_host)
    ...
    TARGET_META=$meta
    TARGET_HARNESS=$(xo_meta_get "$meta" harness)
    EXPECTED_LABEL=$(xo_backend_expected_label_of_selector "$raw" "$STATE")
    TARGET_SELECTOR=1
    return 0
  fi
  ...
  case "$raw" in
    xo-*:*)
      # A named Herdr session may itself begin with "xo-". Keep that explicit
      # session:pane target on the validated backend-target path below rather
      # than mistaking it for an unresolved task selector.
      ;;
    xo-*)
      RESOLUTION_TRIED="meta=$STATE/$raw.meta; legacy-meta=$STATE/${raw#xo-}.meta; backend=none"
      echo "error: no metadata for $raw in $STATE (tried $RESOLUTION_TRIED); pass a well-formed explicit backend target only when targeting outside this xo home" >&2
      return 1
      ;;
  esac

  pane_meta=$(xo_send_meta_for_key_value "$STATE" herdr_pane_id "$raw" 2>/dev/null || true)
  if [ -n "$pane_meta" ]; then
    session=$(xo_meta_get "$pane_meta" herdr_session)
    hint="${session:-<herdr-session>}:$raw"
    id=$(xo_send_id_from_meta "$pane_meta")
    echo "error: target '$raw' matches herdr_pane_id in $pane_meta but is missing its herdr session prefix; expected <herdr-session>:<pane-id> such as '$hint' or use 'xo-$id' (tried meta=$STATE/$raw.meta; backend=herdr)" >&2
    return 1
  fi
  ...
  if [ -n "$meta" ]; then
    ...
    RESOLVED_TARGET=$target
    TARGET_BACKEND=$(xo_backend_of_meta "$meta")
    TARGET_META=$meta
    TARGET_HARNESS=$(xo_meta_get "$meta" harness)
  ...
  case "$raw" in
    *:*)
      colons=$(xo_send_count_colons "$raw")
      if [ "$colons" -ge 2 ]; then
        assumed=herdr
      else
        assumed=tmux
      fi
      if ! xo_backend_target_exists "$assumed" "$raw"; then
        echo "error: explicit target '$raw' is not a live $assumed endpoint (tried meta=$STATE/$raw.meta; metadata window/terminal lookup; backend=$assumed). Use xo-<id> for a recorded task/lane, or pass a target whose backend endpoint can be verified." >&2
        return 1
      fi
      RESOLVED_TARGET=$raw
  ...
  echo "error: target '$raw' is not resolvable (tried meta=$STATE/$raw.meta; metadata window/terminal lookup; backend=none). Use xo-$raw for a recorded task/lane, or pass a well-formed explicit backend target such as session:window." >&2
  return 1
}

RAW_TARGET=$1
xo_send_resolve_target "$RAW_TARGET" || exit 1
T=$RESOLVED_TARGET
shift
...
# untouched (contract: bin/xo-lease-lib.sh).
# shellcheck source=bin/xo-lease-lib.sh
. "$SCRIPT_DIR/xo-lease-lib.sh"
if [ -n "$TARGET_META" ]; then
  LEASE_GUARD_TASK=$(xo_send_id_from_meta "$TARGET_META")
  if [ -n "$LEASE_GUARD_TASK" ]; then
    xo_lease_guard "$LEASE_GUARD_TASK" "steer (xo-send)"
    trap 'xo_lease_guard_release' EXIT
  fi
fi
...
while :; do
  case "${1:-}" in
    --resolve-key)
      ...
      xo_send_add_resolve_key "$2" || exit 1
      shift 2
      ;;
    --resolve-key=*)
      xo_send_add_resolve_key "${1#--resolve-key=}" || exit 1
      shift
      ;;
    --fire-and-forget)
...
TARGET_TASK_ID=
xo_send_known_undelivered_cleanup() {
  [ -n "$PENDING_REPLY_CORR" ] || return 0
  if [ "$PENDING_REPLY_CREATED" = 1 ]; then
    xo_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR"
  else
    xo_pending_reply_reset_known_undelivered "$STATE" "$PENDING_REPLY_CORR"
  fi
}
...
# is owned by the status log: xo-captain-hold's `complete` closes that live copy
...
xo_send_hold_resolved_id() {  # <task-id> <decision-key>
  local show id state hold_kind
  command -v tasks-axi >/dev/null 2>&1 || return 1
  for id in "$2" "$1-decision-$2"; do
    show=$( (cd "$XO_HOME" && tasks-axi show "$id" --full) 2>/dev/null ) || continue
...
# fold actually closes it (xo_pending_reply_close_note_for_key).
xo_send_resolve_close_note() {  # <key> <excerpt>
  local k=$1 excerpt=$2 owned
  if owned=$(xo_pending_reply_close_note_for_key "$k" "$RESOLVE_TASK_ID" operator-resolve-key "$excerpt"); then
    printf '%s' "$owned"
    return 0
  fi
...
if [ -n "$RESOLVE_KEYS" ]; then
  ...
  RESOLVE_TASK_ID=$(xo_send_id_from_meta "$TARGET_META")
  ...
  for k in $RESOLVE_KEYS; do
    ...
    if resolved_hold_id=$(xo_send_hold_resolved_id "$RESOLVE_TASK_ID" "$k"); then
  ...
  for k in $RESOLVE_STATUS_KEYS; do
    probe=$(xo_send_resolve_close_note "$k" "$resolve_excerpt")
    if ! _xo_decision_key_transition_allowed "$k" "$probe"; then
    ...
    xo_cap_line_var "$probe_line"
    probe_key=$(_xo_decision_key "$XO_LINE_CAP_LINE") || probe_key=
    if [ "$(status_line_verb "$XO_LINE_CAP_LINE")" != resolved ] || [ "$probe_key" != "$k" ]; then
      echo "error: --resolve-key cannot close a decision key of length ${#k}: its ${#probe_line}-character close record exceeds the $XO_LINE_CAP_DEFAULT-character status-line cap, and truncation would remove the structural key delimiter. Refusing rather than writing an ineffective close; nothing was sent." >&2
      exit 1
    fi
  done
fi
...
# (bin/xo-wake-lib.sh) and does not wake this same session again; any
# concurrent foreign status bytes leave the watcher's wake path untouched.
xo_send_close_resolved_keys() {  # <answer-text>
  local note=$1 k line close_note append_rc still manual_close_cmd
  note=$(printf '%s' "$note" | tr '\n\r\t' '   ' | LC_ALL=C tr -d '\000-\037\177')
  for k in $RESOLVE_STATUS_KEYS; do
    close_note=$(xo_send_resolve_close_note "$k" "$note")
    line="resolved [key=$k]: $close_note"
    xo_cap_line_var "$line"
    printf -v manual_close_cmd "printf '%%s\\n' %q >> %q" "$XO_LINE_CAP_LINE" "$RESOLVE_STATUS_FILE"
    append_rc=0
    xo_wake_status_append_self_announced "$STATE" "$RESOLVE_STATUS_FILE" "$XO_LINE_CAP_LINE" || append_rc=$?
...
# lines, exactly the way every other channel does. xo-send decides nothing here:
...
xo_send_feed_resolved_holds() {  # <answer-text>
  ...
  if ! printf '%s' "$lines" | "$SCRIPT_DIR/xo-captain-hold.sh" answers \
    --source "an xo answer sent to $RESOLVE_TASK_ID" >/dev/null 2>&1; then
    echo "error: the answer was delivered to $T, but this captain-held task could not be closed: ${RESOLVE_HOLD_KEYS}. Close it with xo-captain-hold.sh answer - do not resend the answer." >&2
    return 1
  fi
}

# Resolve the target's harness from its meta (recorded by xo-spawn), used only to
...
if [ "${1:-}" = "--key" ]; then
  ...
  key=$2
  semantic_key=$(xo_send_normalize_key "$key")
  if [ "$TARGET_BACKEND" = remote ]; then
    XO_SEND_REMOTE_BUDGET=${XO_SEND_REMOTE_BUDGET:-30}
    case "$XO_SEND_REMOTE_BUDGET" in
      ''|*[!0-9]*|0)
        echo "error: XO_SEND_REMOTE_BUDGET must be a positive integer: $XO_SEND_REMOTE_BUDGET" >&2
        exit 1
        ;;
    esac
    if ! xo_run_timed "$XO_SEND_REMOTE_BUDGET" "$SCRIPT_DIR/xo-on.sh" "$TARGET_REMOTE_ID" \
      xo-remote-secondmate-control.sh key "$TARGET_REMOTE_ID" "$key" < /dev/null; then
      echo "error: key '$key' not sent to remote secondmate $TARGET_REMOTE_ID; completion may be unknown" >&2
      exit 1
    fi
  elif ! xo_backend_send_key "$TARGET_BACKEND" "$T" "$key" "$EXPECTED_LABEL"; then
    echo "error: key '$key' not sent to $T ($TARGET_BACKEND send failed; tried $RESOLUTION_TRIED)" >&2
    exit 1
  fi
  xo_send_clear_after_interrupt "$semantic_key" || exit 1
  xo_send_record_interrupt "$semantic_key" || exit 1
else
  MESSAGE=$*
  if [ "$TARGET_BACKEND" = remote ]; then
    XO_SEND_REMOTE_BUDGET=${XO_SEND_REMOTE_BUDGET:-30}
  ...
  if [ "$MARK_FROM_PRIMARY" = 1 ] && [ -n "$FIRE_AND_FORGET_ID" ]; then
    ...
    if [ "${XO_PENDING_REPLY_EXISTING_CORR+x}" = x ]; then
      existing_corr_explicit=1
      existing_corr=$XO_PENDING_REPLY_EXISTING_CORR
    else
      existing_corr=$(xo_pending_reply_extract_corr "$MESSAGE")
    fi
    if [ -n "$existing_corr" ] \
      && xo_pending_reply_corr_reusable "$STATE" "$existing_corr" "$TARGET_TASK_ID"; then
      ...
      PENDING_REPLY_CORR=$(xo_pending_reply_create "$XO_HOME" "$STATE" "$TARGET_TASK_ID" "$MESSAGE") \
        || { echo "error: failed to create parent pending-reply expectation for $TARGET_TASK_ID" >&2; exit 1; }
      PENDING_REPLY_CREATED=1
    fi
    xo_pending_reply_embed_corr "$MESSAGE" "$PENDING_REPLY_CORR" MESSAGE
    if [ "$PENDING_REPLY_CREATED" != 1 ] \
      && xo_pending_reply_delivery_attempt_unresolved "$STATE" "$PENDING_REPLY_CORR"; then
      if [ "$TARGET_BACKEND" = remote ]; then
        if ! xo_pending_reply_reset_known_undelivered "$STATE" "$PENDING_REPLY_CORR"; then
    ...
    if ! xo_pending_reply_prepare_delivery "$STATE" "$PENDING_REPLY_CORR"; then
      [ "$PENDING_REPLY_CREATED" != 1 ] \
        || xo_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
  ...
  if [ "$INBOX_PLANE" = 1 ] && [ "$TARGET_BACKEND" = remote ]; then
    ...
    # Every transport attempt is bounded by XO_SEND_REMOTE_BUDGET seconds
    ...
    REMOTE_META_LOCK=$(xo_meta_lock_path "$TARGET_META") || exit 1
    if ! xo_task_inbox_lock_acquire "$REMOTE_META_LOCK"; then
    ...
    if [ -f "$TARGET_META" ]; then
      CURRENT_REMOTE_ID=$(xo_send_id_from_meta "$TARGET_META")
      CURRENT_REMOTE_HOST=$(xo_meta_get "$TARGET_META" remote_host)
      CURRENT_REMOTE_SPAWN_GEN=$(xo_meta_get "$TARGET_META" spawn_gen)
    fi
    if [ "$CURRENT_REMOTE_ID" != "$TARGET_REMOTE_ID" ] \
      || { [ -n "${XO_SEND_EXPECTED_SPAWN_GEN:-}" ] \
        && [ "$CURRENT_REMOTE_SPAWN_GEN" != "$XO_SEND_EXPECTED_SPAWN_GEN" ]; } \
      || { [ -n "${XO_SEND_EXPECTED_REMOTE_HOST:-}" ] \
        && [ "$CURRENT_REMOTE_HOST" != "$XO_SEND_EXPECTED_REMOTE_HOST" ]; } \
      || [ -z "$CURRENT_REMOTE_HOST" ] \
      || [ "$CURRENT_REMOTE_HOST" != "$TARGET_REMOTE_HOST" ]; then
      xo_lock_release "$REMOTE_META_LOCK"
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
        xo_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
      fi
    ...
    # Each transport attempt is bounded by XO_SEND_REMOTE_BUDGET seconds.
    # xo_run_timed's 124 means the attempt was killed at the bound with remote
    ...
    xo_run_timed "$XO_SEND_REMOTE_BUDGET" "$SCRIPT_DIR/xo-on.sh" "$TARGET_REMOTE_ID" \
      xo-remote-secondmate-control.sh send "${REMOTE_SEND_ARGS[@]}" < /dev/null || remote_rc=$?
    ...
    if [ "$remote_rc" -ne 0 ] && [ "$remote_completion_unknown" -eq 1 ]; then
      ...
      if [ -n "$PENDING_REPLY_CORR" ]; then
        xo_pending_reply_mark_delivery_unknown "$STATE" "$PENDING_REPLY_CORR" || true
      fi
      if [ "$remote_rc" -eq 255 ]; then
        ...
        echo "error: steer to remote secondmate $TARGET_REMOTE_ID is unconfirmed (the remote transport did not complete within its ${XO_SEND_REMOTE_BUDGET}s budget; remote completion unknown). Only the correlation-reusing resend below is idempotent and lands on the same remote inbox record:" >&2
      else
      ...
      resend_home=$(cd "$XO_HOME" 2>/dev/null && pwd) || resend_home=$XO_HOME
      printf 'XO_HOME=%q ' "$resend_home" >&2
      if [ "${XO_STATE_OVERRIDE+x}" = x ]; then
        resend_state=$(cd "$STATE" 2>/dev/null && pwd) || resend_state=$STATE
        printf 'XO_STATE_OVERRIDE=%q ' "$resend_state" >&2
      fi
      printf 'XO_PENDING_REPLY_EXISTING_CORR=%q %q' "$PENDING_REPLY_CORR" "$SCRIPT_DIR/xo-send.sh" >&2
      for resend_arg in "${XO_SEND_ORIGINAL_ARGS[@]}"; do
    ...
    if [ -n "$PENDING_REPLY_CORR" ]; then
      if xo_pending_reply_confirm_delivery "$STATE" "$PENDING_REPLY_CORR"; then
        :
      else
    ...
    if [ -n "$RESOLVE_KEYS" ]; then
      xo_send_close_resolved_keys "$RESOLVE_ANSWER_TEXT" || exit 1
      xo_send_feed_resolved_holds "$RESOLVE_ANSWER_TEXT" || exit 1
    fi
    exit 0
  fi
  if [ "$INBOX_PLANE" = 1 ]; then
    INBOX_TASK_ID=$(xo_send_id_from_meta "$TARGET_META")
    INBOX_META_LOCK=$(xo_meta_lock_path "$TARGET_META") || exit 1
    if ! xo_task_inbox_lock_acquire "$INBOX_META_LOCK"; then
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
        xo_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
      fi
    ...
    if [ -f "$TARGET_META" ]; then
      CURRENT_INBOX_TARGET=$(xo_backend_target_of_meta "$TARGET_META")
      CURRENT_INBOX_BACKEND=$(xo_backend_of_meta "$TARGET_META")
      CURRENT_INBOX_SPAWN_GEN=$(xo_meta_get "$TARGET_META" spawn_gen)
    fi
    if [ "$CURRENT_INBOX_TARGET" != "$T" ] \
      || [ "$CURRENT_INBOX_BACKEND" != "$TARGET_BACKEND" ] \
      || { [ -n "${XO_SEND_EXPECTED_SPAWN_GEN:-}" ] \
        && [ "$CURRENT_INBOX_SPAWN_GEN" != "$XO_SEND_EXPECTED_SPAWN_GEN" ]; } \
      || [ -n "$(xo_meta_get "$TARGET_META" remote_host)" ]; then
      xo_lock_release "$INBOX_META_LOCK"
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
        xo_pending_reply_discard_undelivered "$STATE" "$PENDING_REPLY_CORR" || true
      fi
    ...
    if [ "${XO_SEND_IDEMPOTENT:-0}" = 1 ]; then
      INBOX_RECORD=$(xo_task_inbox_write_idempotent "$STATE" "$INBOX_TASK_ID" "$MESSAGE" \
        "${FIRE_AND_FORGET_ID:+fire-and-forget}") || inbox_write_rc=$?
    else
      INBOX_RECORD=$(xo_task_inbox_write "$STATE" "$INBOX_TASK_ID" "$MESSAGE" \
        "${FIRE_AND_FORGET_ID:+fire-and-forget}") || inbox_write_rc=$?
    fi
    ...
    ring_rc=0
    xo_task_inbox_ring "$TARGET_BACKEND" "$T" "$INBOX_RECORD" "$EXPECTED_LABEL" || ring_rc=$?
    case "$ring_rc" in
      1) echo "xo-send: doorbell skipped (composer visibly holds pending text); the steer is durably recorded at $INBOX_RECORD and the watcher will re-ring" >&2 ;;
      2) echo "xo-send: doorbell did not reach $T; the steer is durably recorded at $INBOX_RECORD and the watcher will re-ring" >&2 ;;
      3) echo "xo-send: doorbell not typed because the agent in $T has exited; the steer is durably recorded at $INBOX_RECORD for recovery (stuck-crewmate-recovery), and the watcher will not re-ring a dead pane" >&2 ;;
    esac
    exit 0
  fi
  ...
  retries=${XO_SEND_RETRIES:-3}
  sleep_s=${XO_SEND_SLEEP:-0.4}
  ...
  # earlier.
  send_rc=0
  if verdict=$(xo_backend_send_text_submit "$TARGET_BACKEND" "$T" "$MESSAGE" "$retries" "$sleep_s" "$settle" "$EXPECTED_LABEL"); then
    :
  else
    send_rc=$?
  fi
  ...
  case "$verdict" in
    ...
    pending)
      ...
      echo "xo-send: text delivered to $T but submission is unconfirmed (verdict=pending; tried $RESOLUTION_TRIED); do not retype or blindly resend - verify with xo-peek.sh, then re-send '--key Enter' only if the composer still holds the text" >&2
      exit 3
      ;;
    *)
      if [ "$PENDING_REPLY_CREATED" = 1 ] && [ -n "$PENDING_REPLY_CORR" ]; then
  ...
  # crewmate actually working instead of the stale idle pane. XO_SEND_SETTLE=0
  # disables it. Scoped to this path only, never the shared submit core.
  [ "${XO_SEND_SETTLE:-1}" = 0 ] || sleep "${XO_SEND_SETTLE:-1}"
fi
