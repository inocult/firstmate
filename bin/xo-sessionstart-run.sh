...
#
# Why running beats nudging: bin/xo-sessionstart-nudge.sh can only ASK the agent
...
# first turn is.
#
# Usage: xo-sessionstart-run.sh [--source <source>] [--pi-prerequisite]
...
xo_is_gate_agent "$XO_ROOT" && stand_down
xo_primary_scope_matches "$XO_ROOT" "$STATE" || stand_down

session_start_completed() {
  local lock_pid completion_pid
  [ -f "$STATE/.lock" ] && [ ! -L "$STATE/.lock" ] || return 1
  [ -f "$COMPLETION_FILE" ] && [ ! -L "$COMPLETION_FILE" ] || return 1
  xo_session_lock_owned_by_self "$STATE" || return 1
...
if [ -z "$SOURCE" ] && [ ! -t 0 ]; then
  ...
  # so a Cursor-delivered payload here is the duplicate: bin/xo-sessionstart-
  ...
  if xo_hook_payload_is_foreign_host "$PAYLOAD"; then
    exit 0
  fi
...
case "$SOURCE" in
  resume|reload|fork)
    exec "$SCRIPT_DIR/xo-sessionstart-nudge.sh"
    ;;
  clear|compact)
    if session_start_completed; then
      "$SCRIPT_DIR/xo-session-start.sh" --reemit --source "$SOURCE" || true
    else
      "$SCRIPT_DIR/xo-session-start.sh" --source "$SOURCE" || true
    fi
    ;;
  *)
    "$SCRIPT_DIR/xo-session-start.sh" --source "$SOURCE" || true
