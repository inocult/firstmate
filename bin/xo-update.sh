#!/usr/bin/env bash
# Self-update a running xo and its secondmates to the latest origin.
#
# Mechanical half of the /updatexo skill. Fast-forwards the running
# xo repo's default branch from origin, then fast-forwards every
...
# xo-fleet-sync.sh: never force, never create a merge commit, never stash;
...
#
# The fast-forward mechanics live in bin/xo-ff-lib.sh (base_mode "origin" here);
...
#   - reread-xo: yes|no    (did the running xo's instructions change)
#   - restart-secondmates: xo-<id>...|none (every live secondmate this pass left
#     on origin's tip - advanced OR already there - whose recorded runtime can
#     prove a restart)
#   - nudge-secondmates: xo-<id>...|none   (the residual: live secondmates on
...
#     (bin/xo-secondmate-restart-lib.sh owns that test), so it falls to the
...
SECONDMATES_MD="$XO_HOME/data/secondmates.md"
...
usage() { echo "usage: xo-update.sh [--help]" >&2; }
...
reread_xo="no"
ff_target "$XO_ROOT" "xo" origin no no
...
secondmate_agent_may_be_alive() {  # <id>
  ...
  if [ -n "$remote_host" ]; then
    state=$("$SCRIPT_DIR/xo-on.sh" "$id" \
      xo-remote-secondmate-control.sh state "$id" < /dev/null 2>/dev/null) || state=unreadable
  elif xo_backend_validate_task_endpoint "$meta" "$id" >/dev/null 2>&1; then
    state=$(xo_backend_agent_state "$XO_BACKEND_VALIDATED_BACKEND" \
      "$XO_BACKEND_VALIDATED_TARGET" 2>/dev/null) || state=unreadable
  fi
  case "$state" in
...
claim_settled_secondmate() {  # <id>
  local id=$1
  selector_claimed "xo-$id" && return 0
  secondmate_agent_may_be_alive "$id" || return 0
  if xo_secondmate_restart_capable "$STATE/$id.meta"; then
    FF_RESTART_WINDOWS="$FF_RESTART_WINDOWS xo-$id"
  else
    FF_STEER_WINDOWS="$FF_STEER_WINDOWS xo-$id"
  fi
}

# bin/xo-ff-lib.sh calls this for each local home it left AT the base with a live
...
if [ -f "$SECONDMATES_MD" ]; then
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    if [ "$SECONDMATE_REGISTRY_REMOTE" -eq 1 ]; then
      if remote_out=$("$SCRIPT_DIR/xo-on.sh" "$id" xo-remote-secondmate-control.sh update "$id" < /dev/null 2>&1); then
        remote_result=$(printf '%s\n' "$remote_out" | tail -1)
        case "$remote_result" in
          synced:*)
            remote_detail=${remote_result#synced: }
            # The host reports its advance as "<commit> instr=<paths>"; a host
            # whose XO copy predates that suffix reports the commit alone.
