...
# Usage: xo-remote-inherit-push.sh <secondmate-id> <generation>
#
# The item set is derived from the ONE declared owner
# (XO_INHERITABLE_CONFIG in bin/xo-config-inherit-lib.sh), the same declaration
# the receiving bin/xo-remote-inherit.sh enforces, so the two implementations in
# one code revision cannot drift silently. Different local and remote revisions
# fail closed as documented by that owner. XO_CONFIG_INHERIT_LIVE=1 marks a live
...
[ "$#" -eq 2 ] || { echo "usage: xo-remote-inherit-push.sh <secondmate-id> <generation>" >&2; exit 2; }
ID=$1
GENERATION=$2
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/xo-remote-inherit-push.XXXXXX") || die "cannot create inheritance staging directory"
trap 'rm -rf -- "$TMP"' EXIT
EMPTY="$TMP/empty"
: > "$EMPTY"
EMPTY_HASH=$(sha256_file "$EMPTY") || die "cannot hash empty inheritance payload"

ITEMS=$(xo_config_inherit_items)
while IFS= read -r rel; do
  [ -n "$rel" ] || continue
  if [ "${XO_CONFIG_INHERIT_LIVE:-0}" = 1 ]; then
    case "$rel" in
      config/*)
        if xo_config_inherit_item_session_scoped "${rel#config/}"; then
          printf 'unchanged: %s\n' "$rel"
          continue
        fi
        ;;
  ...
  source_present=$(xo_config_source_present "$source") || exit 1
  if [ "$source_present" = 1 ]; then
    ...
    "$SCRIPT_DIR/xo-on.sh" --stdin "$ID" xo-remote-inherit.sh put "$rel" "$bytes" "$hash" "$GENERATION" < "$snapshot"
  else
    # This loop's heredoc is its control stream, not remote command input.
    "$SCRIPT_DIR/xo-on.sh" "$ID" xo-remote-inherit.sh absent "$rel" 0 "$EMPTY_HASH" "$GENERATION" < /dev/null
  fi
done <<EOF
$ITEMS
EOF
