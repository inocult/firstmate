#!/usr/bin/env python3
"""xo-voice-relay.py - hold the Nova Sonic session on this desktop, on behalf of the laptop.
...
Bedrock bidirectional session, answers the model's tool calls from xo's
...
The voice agent this relay runs is NOT xo. It stands in front of
xo: it answers questions about the fleet from the records, and when the
...
Modes:
  --serve            read xo_voice_frame frames on stdin, write them on stdout.
...
Read scope, deny list and the handover queue all belong to bin/xo_voice_records.py.
bin/xo_voice_frame.py owns the wire contract between the two machines, and
...
an unconfigured home cannot start this relay at all.
  ...
  config/voice-region   XO_VOICE_REGION   Bedrock region.        required
  config/voice-model    XO_VOICE_MODEL    Nova Sonic model id.   required
  config/voice-profile  XO_VOICE_PROFILE  AWS profile.           optional
  config/voice-id       XO_VOICE_ID       output voice.          default matthew

An absent profile means the relay uses only credentials that are already in its
environment. An empty XO_VOICE_PROFILE, or an empty `--profile ""`, forces that
...
Usage:
  xo-voice-relay.py [--serve] [options]
  xo-voice-relay.py --self-test <file.pcm> [options]
...
Options:
  ...
  --home <dir>          xo home for records.  default $XO_HOME or this repo
...
import xo_voice_frame as frame              # noqa: E402
import xo_voice_records as records          # noqa: E402
...
VOICE = "matthew"
SETTINGS = {
    "region": ("voice-region", "XO_VOICE_REGION", "Bedrock region"),
    "model": ("voice-model", "XO_VOICE_MODEL", "Nova Sonic model id"),
    "profile": ("voice-profile", "XO_VOICE_PROFILE", "AWS profile"),
    "voice": ("voice-id", "XO_VOICE_ID", "output voice"),
}
...
SYSTEM_PROMPT = (
    "You are the captain's voice assistant. You are NOT the XO, and you "
    "must never claim to be. You stand in front of the XO and you are "
    ...
    "XO, then call hand_over_to_xo with the captain's request in "
...
TOOLS = {"tools": [
    {"toolSpec": {
        ...
        "description": (
            "Read the XO's durable records: how many jobs are in "
    ...
    {"toolSpec": {
        "name": "hand_over_to_xo",
        "description": (
            "Hand a request for real work to the XO, which will pick it "
...
def widen_path():
    """Put the toolbox directories on PATH, as bin/xo-inbox.sh does and for the same reason.
...
def profile_credentials(profile, verbose=False):
    ...
    if not profile:
        raise CredentialError(
            ...
            "write one into config/voice-profile, set XO_VOICE_PROFILE, or "
...
class Session:
    ...
    def __init__(self, options, down, credentials):
        ...
        # this relay, so "it called hand_over_to_xo and did not answer
        # for xo" has to be evidence in the run record, not an inference
        # from a count.
        self.tool_names = []
...
async def serve(options):
    ...
    # cannot desynchronise the client. See xo_voice_frame.MAGIC.
    ...
    try:
        ...
        sys.stderr.write(
            "xo-voice-relay: the uplink is not a frame stream any more: {}\n"
            .format(exc))
        reason = "{}: {}".format(type(exc).__name__, exc)
        status = 2
    finally:
...
async def self_test(options):
    ...
    if early:
        sys.stderr.write(
            "xo-voice-relay: {} came in before the end of the clip, so these "
...
def resolve_settings(options):
    ...
    if options.profile is None:
        # Presence, not truthiness: an empty XO_VOICE_PROFILE is the captain
        # saying "use the credentials I already have" and must not fall through
        # to a configured profile, which is how xo-inbox.sh reads its own
...
def main(argv):
    ...
    try:
        ...
        sys.stderr.write("xo-voice-relay: {}\n".format(exc))
        return 2
