...
# not call xo-guard.sh.
# Usage: xo-ensure-agents-md.sh [repo-or-worktree-dir]
set -eu

usage() {
  echo "usage: xo-ensure-agents-md.sh [repo-or-worktree-dir]" >&2
  cat >&2 <<'EOF'
...
ensure_maintenance_section() {
  ...
  if grep -Fqx -e '## Maintaining this file' -e $'## Maintaining this file\r' "$AGENTS" ||
    head -n 1 "$AGENTS" | grep -Fqx -e '<!-- xo:maintained-by-project -->' \
      -e $'<!-- xo:maintained-by-project -->\r'; then
    return 0
  fi
  local eol=$'\n' sep=''
