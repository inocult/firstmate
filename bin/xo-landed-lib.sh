...
# Usage: . bin/xo-landed-lib.sh; splice "$XO_LANDED_JQ_DEFS" ahead of a jq
...
# separate jq programs - bin/xo-bearings-snapshot.sh projects this home's own
# Done rows, and bin/xo-fleet-snapshot.sh projects each secondmate home's Done
...
# tests/xo-bearings-snapshot.test.sh covers both fresh and cached v1 summaries.
...
XO_LANDED_JQ_DEFS='
  ...
  # Pinned by tests/xo-captain-hold-lifecycle.test.sh on "released, retained, or
