...
if (@ARGV && $ARGV[0] eq 'handoff') {
  ...
  my $record = encode_json({
    schema => 'xo-procevent-capture-capability.v1', token => $reservation_token,
  ...
  delete @ENV{grep { /^XO_PROCEVENT_INTERNAL_CAPTURE_/ } keys %ENV};
...
sub write_reservation {
  ...
  my $record = encode_json({
    schema => 'xo-procevent-capture-reservation.v1', token => $token,
...
write_all($extension_file, join("\n", "schema=xo-procevent-extension-owner.v1", "extension_id=$extension_id", "extension_version=$extension_version", "capability_version=$capability_version", "package_digest=$package_digest", "binding_digest=$binding_digest", ""));
