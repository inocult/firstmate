#!/usr/bin/env python3
# xo-mail.py - the IMAP/SMTP engine behind bin/xo-mail.sh.
#
# A small mail client used by xo-mail.sh:
...
# is never marked seen before xo answers it.
import imaplib
import os
import re
import socket
...
USER = os.environ['XO_MAIL_USER']
PW = os.environ['XO_MAIL_PASS']
IMH = os.environ['XO_IMAP_HOST']
IMP = int(os.environ['XO_IMAP_PORT'])
STH = os.environ['XO_SMTP_HOST']
STP = int(os.environ['XO_SMTP_PORT'])
CTX = ssl.create_default_context()


def mail_timeout():
    """Seconds for IMAP/SMTP sockets. Invalid or non-positive values become 20."""
    raw = os.environ.get('XO_MAIL_TIMEOUT', '20')
    try:
        value = float(raw)
    ...
    try:
        ...
        for i in ids[-READ_LIMIT:]:
            ...
            if typ != 'OK' or not msg or not msg[0] or not msg[0][1]:
                ...
                print('Date:', '')
                print('Subj:', 'unfetchable body - see xo-mail read')
...
def cmd_poll_list():
    ...
    cap = int(os.environ.get('XO_MAIL_POLL_MAX_WAKES') or '20')
    if cap < 1:
        cap = 20
    stored_gen, seen = load_cursor(os.environ.get('XO_MAIL_CURSOR', ''))
    retry, retry_order = load_retry(os.environ.get('XO_MAIL_RETRY', ''))
    retry_pos_path = os.environ.get('XO_MAIL_RETRY_POS', '')
    ...
    try:
        ...
        turn_path = os.environ.get('XO_MAIL_TURN', '')
        next_turn = None
        ...
        for u in new_candidates + retry_candidates:
            ...
            try:
                ...
                out.append((clean(u), '', '(no header)',
                            'unfetchable header - see xo-mail read', 'degraded'))
                new_emitted += 1
                continue
        ...
        print('xo-mail poll error:', e, file=sys.stderr)
        return 1
    finally:
        if m is not None:
            try:
