#!/usr/bin/env bash
# xo-branch-outcome.sh - the durable outcome store for the Pi supervision
...
#     bin/xo-teardown.sh removes a retired task's cache with its other records,
...
#   xo-branch-outcome.sh processed-init [--held-lock]
...
#     of the process holding $STATE/.branch-outcomes.lock (xo-wake-drain.sh may
...
#   xo-branch-outcome.sh list [--recent <n>]
#     Print the last n records (default 20), read or not.
#   xo-branch-outcome.sh startup-replay
...
#     Run it only when the session holds the lock (xo-session-start.sh owns the
#     call site).
set -eu
...
OUTCOME_INDEX_VERSION=xo-branch-outcome-index-v1
OUTCOME_INDEX_MAX_BYTES=512
OUTCOME_INDEX_READY="$STATE/.branch-outcome-index-ready"

usage() {
  echo "usage: xo-branch-outcome.sh append --task <id> --verdict routine|captain --summary <text> [--wake <text>] [--silent true|false] | unread | mark-read --through <seq> | unprocessed | mark-processed --through <seq> | processed-init [--held-lock] | list [--recent <n>] | startup-replay" >&2
  exit 2
}
...
capture_status_position() { # <task>
  ...
  size=$(_xo_status_file_size "$f") || return 0
  size=${size//[[:space:]]/}
  ident=$(_xo_open_decisions_file_ident "$f") || return 0
  size_after=$(_xo_status_file_size "$f") || return 0
  size_after=${size_after//[[:space:]]/}
  ident_after=$(_xo_open_decisions_file_ident "$f") || return 0
...
rebuild_outcome_indexes() {
  ...
  while IFS=$(printf '\t') read -r task seq epoch endpoint ident; do
    ...
    if [ -z "$endpoint" ] || [ -z "$ident" ]; then
      ...
      ident=-
      if [ -f "$f" ] && [ -r "$f" ] && [ ! -L "$f" ]; then
        mtime=$(_xo_status_file_mtime "$f") || mtime=
        case "$mtime" in ''|*[!0-9]*) ;;
          *)
...
held_lock_owned_by_ancestor() {
  ...
  if [ -L "$LOCK" ]; then
    owner=$(xo_lock_link_owner "$LOCK" 2>/dev/null) || return 1
    xo_lock_points_to_owner "$LOCK" "$owner" || return 1
  elif [ -d "$LOCK" ]; then
    owner=$LOCK
  else
    return 1
  ...
  xo_pid_alive "$owner_pid" || return 1
...
case "$CMD" in
  append)
    ...
    SEQ=$(( LAST_SEQ + 1 ))
    capture_status_position "$TASK"
    rm -f -- "$OUTCOME_INDEX_READY" || { xo_lock_release "$LOCK"; exit 1; }
