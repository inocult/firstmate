#!/usr/bin/env bash
# xo-busy-lib.sh - the ONE owner of xo's semantic busy-state contract.
#
...
# by bin/xo-busy-event.sh (the only writer):
#
...
# wiring is armed (xo-spawn, or a documented recovery re-arm). Every event
...
# newer record.
#
# Semantic sources written by adapters (xo_busy_sources_for_harness owns the
...
#                    xo_busy_codex_semantic_source
#   kimi-wire, kimi-hook  reserved: standalone Kimi, gated by xo_busy_kimi_verified
# XO-owned sources accepted for every converted adapter:
#   xo-spawn         the launch-brief turn seeded at spawn
#   xo-interrupt     the legacy Claude xo-send --key Escape idle event
#   xo-recovery      a documented recovery reset after relaunch
...
#
# Classification (xo_busy_classify): busy | idle | unknown | dead, always
# with the producing source as the second token. Precedence:
#   1. dead endpoint (xo_busy_classify_live only) -> dead endpoint-gone
...
# path xo drives, see references/harness/rovo.md); each is scoped to
# its own harness= and can never classify another adapter. The delivery
# guards in bin/xo-composer-lib.sh match rendered footers for submit
...
# cleared. See xo_busy_muse_run_state for the fold.
#
...
# cleared. See xo_busy_cursor_turn_state for the fold. Cursor's rendered
# `ctrl+c to stop` footer is deliberately not a state source here.
#
# Codex negotiation (xo_busy_codex_appserver_observable,
# xo_busy_codex_hooks_verified): the approved contract prefers Codex's
...
# falling back to idle, and xo-spawn installs no Codex busy wiring.
...
# unknown kimi-unverified and xo-spawn wires no Kimi busy events. Kimi's
...
# real turn on an xo-launched worker including the interrupt path,
...
# and land the wiring in xo-spawn behind this same gate in the same change.
XO_BUSY_KIMI_VERIFIED_VERSIONS=""

xo_busy_kimi_verified() {
  [ -n "$XO_BUSY_KIMI_VERIFIED_VERSIONS" ]
}

# xo_busy_codex_appserver_observable: capability/version negotiation for the
...
xo_busy_codex_appserver_observable() {
  return 1
}

# xo_busy_codex_hooks_verified: the sanctioned intermediate - Codex's stable
...
# xo-launched worker. codex-cli 0.145.0 verdict (live, 2026-07-28):
# NOT verified. XO-written project hooks under <worktree>/.codex/
...
xo_busy_codex_hooks_verified() {
  return 1
}

# xo_busy_codex_semantic_source: 0 when ANY verified Codex semantic source
# exists. xo-spawn arms and wires Codex only behind this gate, and the
# classifier reports unknown codex-unverified until it opens.
xo_busy_codex_semantic_source() {
  xo_busy_codex_appserver_observable || xo_busy_codex_hooks_verified
}

xo_busy_record_path() {  # <state-dir> <id>
  printf '%s/%s.busy-state' "$1" "$2"
}

xo_busy_gen_path() {  # <state-dir> <id>
  printf '%s/%s.busy-gen' "$1" "$2"
}

# xo_busy_token_valid: conservative token charset shared by gen, source, and
# event fields. Anything else is malformed.
xo_busy_token_valid() {  # <value>
  case "${1:-}" in
...
# xo_busy_current_gen: the task's armed gen token, or failure when the busy
# contract has never been armed for this task.
xo_busy_current_gen() {  # <state-dir> <id>
  local gen_file gen
  gen_file=$(xo_busy_gen_path "$1" "$2")
  [ -f "$gen_file" ] || return 1
  IFS= read -r gen < "$gen_file" 2>/dev/null || gen=
  xo_busy_token_valid "$gen" || return 1
  printf '%s' "$gen"
}

# xo_busy_sources_for_harness: the semantic sources trusted to classify a
# task recorded with <harness>. One line, space-separated, possibly empty.
# The xo-owned sources are appended for every converted adapter.
...
xo_busy_sources_for_harness() {  # <harness>
  local adapter=
  case "${1:-}" in
    claude*) adapter=claude-hook ;;
    codex*)
      xo_busy_codex_semantic_source || { printf ''; return 0; }
    ...
    omp) adapter=omp-ext ;;
    kimi*)
      xo_busy_kimi_verified || { printf ''; return 0; }
      adapter='kimi-wire kimi-hook'
      ;;
    *) printf ''; return 0 ;;
  esac
  printf '%s xo-spawn xo-interrupt xo-recovery' "$adapter"
}

xo_busy_source_trusted() {  # <harness> <source>
  local trusted
  trusted=$(xo_busy_sources_for_harness "$1")
  case " $trusted " in
    *" $2 "*) return 0 ;;
  esac
  return 1
...
# xo_busy_record_read: parse and validate state/<id>.busy-state against the
...
xo_busy_record_read() {  # <state-dir> <id>
  local state=$1 id=$2 rec gen line extra ver f
  local r_gen='' r_seq='' r_state='' r_source='' r_event='' r_ts=''
  rec=$(xo_busy_record_path "$state" "$id")
  if [ ! -f "$rec" ]; then
    printf 'missing'
    return 1
  fi
  if ! gen=$(xo_busy_current_gen "$state" "$id"); then
  ...
  ver=${fields[0]:-}
  [ "$ver" = "$XO_BUSY_LIB_VERSION" ] || { printf 'malformed'; return 1; }
  ...
  xo_busy_token_valid "$r_gen" || { printf 'malformed'; return 1; }
  xo_busy_token_valid "$r_source" || { printf 'malformed'; return 1; }
  xo_busy_token_valid "$r_event" || { printf 'malformed'; return 1; }
...
# xo_busy_muse_binding_path: the per-task sidecar xo-spawn writes so the
...
# predates this pane.
xo_busy_muse_binding_path() {  # <state-dir> <id>
  printf '%s/%s.muse-session' "$1" "$2"
}

xo_busy_muse_cache_path() {  # <state-dir> <id>
  printf '%s/%s.muse-session-current' "$1" "$2"
}

# xo_busy_muse_binding_field: read one field from the sidecar, or fail.
xo_busy_muse_binding_field() {  # <state-dir> <id> <key>
  local path line key=$3
  path=$(xo_busy_muse_binding_path "$1" "$2")
  [ -f "$path" ] || return 1
...
# xo_busy_muse_matching_logs: every MAIN session log whose recorded
...
# parent's turn ended.
xo_busy_muse_matching_logs() {  # <sessions-root> <workspace-root>
  local root=$1 ws=$2
  [ -d "$root" ] || return 1
...
xo_busy_muse_binding_has_prior_log() {  # <state-dir> <id> <session-log>
  local path line
  path=$(xo_busy_muse_binding_path "$1" "$2")
  [ -f "$path" ] || return 1
  while IFS= read -r line || [ -n "$line" ]; do
...
xo_busy_muse_cache_field() {  # <state-dir> <id> <key>
  local path line key=$3
  path=$(xo_busy_muse_cache_path "$1" "$2")
  [ -f "$path" ] || return 1
  while IFS= read -r line || [ -n "$line" ]; do
    case "$line" in
      "$key="*)
...
xo_busy_muse_main_log_path_valid() {  # <sessions-root> <session-log>
...
xo_busy_muse_namespace_day() {  # <sessions-root>
  printf '%s/%s' "${1%/}" "$(date '+%Y/%m/%d')"
}

xo_busy_muse_namespace_signature() {  # <day-directory>
...
xo_busy_muse_cached_session_log() {  # <state-dir> <id> <root> <binding-id>
  local cache_binding log cache_day cache_signature day signature
  [ -n "$4" ] || return 1
  cache_binding=$(xo_busy_muse_cache_field "$1" "$2" binding_id) || return 1
  [ "$cache_binding" = "$4" ] || return 1
  log=$(xo_busy_muse_cache_field "$1" "$2" session_log) || return 1
  xo_busy_muse_main_log_path_valid "$3" "$log" || return 1
  xo_busy_muse_binding_has_prior_log "$1" "$2" "$log" && return 1
  cache_day=$(xo_busy_muse_cache_field "$1" "$2" namespace_day) || return 1
  cache_signature=$(xo_busy_muse_cache_field "$1" "$2" namespace_signature) || return 1
  day=$(xo_busy_muse_namespace_day "$3") || return 1
  [ "$cache_day" = "$day" ] || return 1
  signature=$(xo_busy_muse_namespace_signature "$day") || return 1
  [ "$cache_signature" = "$signature" ] || return 1
  printf '%s' "$log"
}

xo_busy_muse_cache_session_log() {  # <state-dir> <id> <binding-id> <session-log> <namespace-day> <namespace-signature>
  local cache tmp current
  [ -n "$3" ] || return 0
  current=$(xo_busy_muse_binding_field "$1" "$2" binding_id) || return 1
  [ "$current" = "$3" ] || return 1
  cache=$(xo_busy_muse_cache_path "$1" "$2")
  tmp="$cache.tmp.$$"
  {
    printf 'binding_id=%s\n' "$3"
...
# xo_busy_muse_session_log: the one matching MAIN session log that did not
# exist when xo-spawn created this pane's binding. Multiple candidates are
# ambiguous and fail closed rather than guessing which pane owns either log.
xo_busy_muse_session_log() {  # <state-dir> <id>
  local root ws binding_id='' candidate selected='' cache namespace_day namespace_before namespace_after
  root=$(xo_busy_muse_binding_field "$1" "$2" sessions_root) || return 1
  ws=$(xo_busy_muse_binding_field "$1" "$2" workspace_root) || return 1
  binding_id=$(xo_busy_muse_binding_field "$1" "$2" binding_id 2>/dev/null || true)
  if cache=$(xo_busy_muse_cached_session_log "$1" "$2" "$root" "$binding_id"); then
    printf '%s' "$cache"
    return 0
  fi
  rm -f "$(xo_busy_muse_cache_path "$1" "$2")"
  namespace_day=$(xo_busy_muse_namespace_day "$root") || return 1
  namespace_before=$(xo_busy_muse_namespace_signature "$namespace_day") || return 1
  while IFS= read -r candidate; do
    [ -n "$candidate" ] || continue
    xo_busy_muse_binding_has_prior_log "$1" "$2" "$candidate" && continue
    [ -z "$selected" ] || return 1
    selected=$candidate
  done <<EOF
$(xo_busy_muse_matching_logs "$root" "$ws")
EOF
  [ -n "$selected" ] || return 1
  namespace_after=$(xo_busy_muse_namespace_signature "$namespace_day") || return 1
  [ "$namespace_before" = "$namespace_after" ] || return 1
  xo_busy_muse_cache_session_log "$1" "$2" "$binding_id" "$selected" \
    "$namespace_day" "$namespace_after" || return 1
  printf '%s' "$selected"
}

xo_busy_muse_run_events() {  # <session-log>
  [ -f "$1" ] || return 1
  LC_ALL=C awk '
...
# xo_busy_muse_run_state: fold one session log to busy|settled|none.
...
xo_busy_muse_run_state() {  # <session-log>
  [ -f "$1" ] || return 1
  xo_busy_muse_run_events "$1" | LC_ALL=C awk -F '\t' '
...
xo_busy_muse_active_run_id() {  # <session-log>
  [ -f "$1" ] || return 1
  xo_busy_muse_run_events "$1" | LC_ALL=C awk -F '\t' '
    $2 == "started" { open[$1] = 1 }
    $2 == "terminal" { open[$1] = 0 }
...
xo_busy_muse_run_terminal() {  # <session-log> <run-id>
  [ -f "$1" ] && [ -n "${2:-}" ] || return 1
  xo_busy_muse_run_events "$1" | LC_ALL=C awk -F '\t' -v wanted="$2" '
...
#
# xo_busy_cursor_binding_path: the per-task sidecar xo-spawn writes. It records
...
xo_busy_cursor_binding_path() {  # <state-dir> <id>
  printf '%s/%s.cursor-session' "$1" "$2"
}

xo_busy_cursor_binding_field() {  # <state-dir> <id> <key>
  local path value
  path=$(xo_busy_cursor_binding_path "$1" "$2")
  [ -f "$path" ] || return 1
...
# xo_busy_cursor_project_dir: the project directory whose recorded
...
xo_busy_cursor_project_dir() {  # <projects-root> <workspace-root>
...
# xo_busy_cursor_transcript: the ONE transcript this pane owns, or failure.
...
xo_busy_cursor_transcript() {  # <state-dir> <id>
  local root workspace project dir conv found='' count=0 prior
  root=$(xo_busy_cursor_binding_field "$1" "$2" projects_root) || return 1
  workspace=$(xo_busy_cursor_binding_field "$1" "$2" workspace_root) || return 1
  project=$(xo_busy_cursor_project_dir "$root" "$workspace") || return 1
  prior=$(LC_ALL=C awk -F= '$1 == "prior_conversation" { sub(/^[^=]*=/, ""); print }' \
    "$(xo_busy_cursor_binding_path "$1" "$2")" 2>/dev/null)
...
# xo_busy_cursor_turn_state: fold the transcript into busy | settled | none.
...
xo_busy_cursor_turn_state() {  # <transcript>
  [ -f "$1" ] || return 1
...
# xo_busy_grok_tail_busy: the Grok-only temporary rendered-tail fallback.
# Consumes the tail on stdin; 0 when Grok's verified busy signature matches.
# XO_BUSY_REGEX still globally overrides the signature, mirroring the
# historical operator escape hatch.
xo_busy_grok_tail_busy() {
  grep -v '^[[:space:]]*$' | tail -12 \
    | grep -qiE "${XO_BUSY_REGEX:-${XO_DELIVERY_GROK_BUSY_REGEX_DEFAULT:-Ctrl\\+c:cancel}}"
}

# xo_busy_rovo_tail_busy: the Rovo-only temporary rendered-tail fallback.
...
# never armed as a semantic writer (xo_busy_sources_for_harness trusts
# nothing for rovo). XO_BUSY_ROVO_REGEX overrides the signature.
xo_busy_rovo_tail_busy() {
  grep -v '^[[:space:]]*$' | tail -12 \
    | grep -qiE "${XO_BUSY_ROVO_REGEX:-Rovo is thinking}"
}

# xo_busy_classify: semantic classification for a task whose endpoint the
# caller has already established as present. Prints "<verdict> <source>":
# busy|idle|unknown plus the producing source (see header). Never probes
# process state. <tail40> is optional pre-captured plain output used only by
# the Grok arm; when absent the Grok arm captures through xo_backend_capture
# if available, else reports unknown capture-failed.
xo_busy_classify() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  ...
  case "$harness" in
    ...
    cursor*)
      ...
      if ! log=$(xo_busy_cursor_transcript "$state" "$id"); then
        printf 'unknown cursor-transcript'
        return 0
      fi
      case "$(xo_busy_cursor_turn_state "$log" 2>/dev/null)" in
  ...
  out=$(xo_busy_record_read "$state" "$id") && rc=0 || rc=$?
  if [ "$rc" = 0 ]; then
    r_state=${out%% *}
    out=${out#* }
    r_source=${out%% *}
    if xo_busy_source_trusted "$harness" "$r_source"; then
  ...
  # unknown here.
  if [ "$backend" = herdr ] && command -v xo_backend_busy_state >/dev/null 2>&1; then
    native=$(xo_backend_busy_state "$backend" "$target" 2>/dev/null || true)
    if [ "$native" = busy ]; then
  ...
  case "$harness" in
    muse*)
      ...
      if ! log=$(xo_busy_muse_session_log "$state" "$id"); then
        printf 'unknown muse-session-log'
        return 0
      fi
      case "$(xo_busy_muse_run_state "$log" 2>/dev/null)" in
    ...
    grok*)
      if [ -z "$tail40" ]; then
        if command -v xo_backend_capture >/dev/null 2>&1; then
          tail40=$(xo_backend_capture "$backend" "$target" 40 2>/dev/null) || {
      ...
      if printf '%s' "$tail40" | xo_busy_grok_tail_busy; then
    ...
    rovo*)
      ...
      if printf '%s' "$tail40" | xo_busy_rovo_tail_busy; then
...
# xo_busy_classify_live: xo_busy_classify behind the one process-level
# override - a gone endpoint is dead, never busy. Requires xo-backend.sh to
# be sourced for xo_backend_target_exists.
xo_busy_classify_live() {  # <backend> <target> <harness> <id> <state-dir> [expected-label]
  local backend=$1 target=$2 harness=$3 id=$4 state=$5 label=${6-}
  if [ -z "$target" ]; then
    printf 'unknown no-target'
    return 0
  fi
  if ! xo_backend_target_exists "$backend" "$target" "$label" 2>/dev/null; then
    printf 'dead endpoint-gone'
    return 0
  fi
  xo_busy_classify "$backend" "$target" "$harness" "$id" "$state"
}

# xo_busy_classify_meta: classify a task from its recorded metadata, so every
# consumer resolves backend, target, and harness the same way instead of
# re-deriving them. Requires xo-backend.sh to be sourced. <tail40> is
# optional pre-captured plain output reused by the Grok arm.
xo_busy_classify_meta() {  # <meta-file> <id> <state-dir> [tail40]
  ...
  xo_busy_classify "$backend" "$target" "$harness" "$id" "$state" "$tail40"
}

# xo_busy_is_busy: boolean view for callers that only gate on provable
...
xo_busy_is_busy() {  # <backend> <target> <harness> <id> <state-dir> [tail40]
  local verdict
  verdict=$(xo_busy_classify "$@")
  [ "${verdict%% *}" = busy ]
}
