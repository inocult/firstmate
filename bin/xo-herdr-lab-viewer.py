...
bin/xo-herdr-lab.sh's ``viewer start`` is the only supported caller; run this
...
why xo could not drive the attached-viewer teardown cases live before
...
Usage: xo-herdr-lab-viewer.py <session> <pidfile>
...
SESSION_PATTERN = re.compile(r"\Axo-lab-[A-Za-z0-9][A-Za-z0-9_-]*\Z")
...
def main(argv):
    if len(argv) != 3:
        sys.stderr.write("xo-herdr-lab-viewer: usage: <session> <pidfile>\n")
        return 2
    session, pidfile = argv[1:]
    if session == "default" or not SESSION_PATTERN.match(session):
        sys.stderr.write("xo-herdr-lab-viewer: refusing session %r\n" % session)
        return 2
    if not os.path.isabs(pidfile):
        sys.stderr.write("xo-herdr-lab-viewer: pidfile must be an absolute path\n")
        return 2

    try:
        master, slave = os.openpty()
    except OSError as error:
        sys.stderr.write("xo-herdr-lab-viewer: could not create a pty: %s\n" % error)
        return 3
    ...
    try:
        ...
        signal.pthread_sigmask(signal.SIG_UNBLOCK, TERMINATION_SIGNALS)
        sys.stderr.write("xo-herdr-lab-viewer: could not fork the viewer: %s\n" % error)
        return 3
    if viewer_pid == 0:
    ...
    try:
        ...
        sys.stderr.write("xo-herdr-lab-viewer: could not record process identity: %s\n" % error)
        try:
            os.kill(viewer_pid, signal.SIGKILL)
        except OSError:
            pass
