...
import { runCommandAsync } from "./xo-async-exec.ts";
...
// XO_BRANCH_DISPATCH_EVENT. A live, enabled branch extension calls accept()
...
// main can repair the watcher cycle (xo_watch_arm_pi lives on main).

export const XO_BRANCH_DISPATCH_EVENT = "xo-branch-supervision:dispatch";
...
export interface UnreadWakeScope {
   ...
   * bin/xo-wake-drain.sh implements against this list). Empty whenever
   * `eligible` is false.
   */
  eligibleSeqs: string[];
  /**
   ...
   * scan excluded. Signal rows are marked by bin/xo-watch.sh; stale rows are
   * decision-owned when their task has an open needs-decision or its current
   * declaration is captain-held. xo-primary-pi-watch.ts cross-references these
...
// "Per-actor acknowledgement"). bin/xo-wake-drain.sh never reclassifies a row
...
// (xo-primary-pi-watch.ts forces every check-kind TRIGGER to main), so nothing
...
// and forced to main on its own triggering close (xo-primary-pi-watch.ts's
...
// this repo's xo_wake_append could never have produced (an unknown kind, or a
...
export function scopeForUnreadWake(state: string, heartbeat: boolean): UnreadWakeScope {
  ...
  const resolveVerb = process.env.XO_CLASSIFY_RESOLVE_VERB || "resolved";
  const heldVerb = process.env.XO_CLASSIFY_CAPTAIN_HELD_VERB || "captain-held";
  const reservedPrefixes = (process.env.XO_CLASSIFY_RESERVED_KEY_PREFIXES || "pending-reply-")
    .split(/\s+/)
    .filter(Boolean);
  ...
  for (const line of rows) {
    ...
    if (kind === "signal") {
      ...
      task = taskByKey.get(key) ?? taskByKey.get(key.replace(/^xo-/, "")) ?? "";
      project = metadata.get(key) ?? metadata.get(key.replace(/^xo-/, "")) ?? "";
      if (task) {
      ...
      // A kind xo_wake_append never emits: structural corruption, not an
...
// The exact state-relative filename bin/xo-wake-drain.sh reads for a
// XO_SUPERVISION_ACTOR=branch drain or ack (its header is the single owner of
...
// that draws the captain's TUI (lib/xo-async-exec.ts). The grant script itself
...
async function runGrantScript(
  ...
  const result = await runCommandAsync("bash", [grantScript, ...args], {
    env: {
      ...
      XO_WAKE_QUEUE: `${state}/.wake-queue`,
      XO_WAKE_QUEUE_LOCK: `${state}/.wake-queue.lock`,
    },
  });
