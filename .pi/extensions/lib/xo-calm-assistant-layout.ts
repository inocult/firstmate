...
// if it is missing; xo-calm.ts catches that and skips only this adapter with a diagnostic
...
// ./xo-calm-visibility.ts owns which classes Calm hides.
...
const CALM_ASSISTANT_LAYOUT_PATCH = Symbol.for(
  "xo:calm-assistant-layout:pi-0.81.1",
);
...
export function installCalmAssistantLayout(): void {
  ...
  if (typeof AssistantMessageComponent !== "function") {
    throw new Error("XO Calm requires Pi AssistantMessageComponent");
  }
  const originalUpdateContent = AssistantMessageComponent.prototype.updateContent;
  if (typeof originalUpdateContent !== "function") {
    throw new Error("XO Calm requires Pi AssistantMessageComponent.updateContent");
  }
