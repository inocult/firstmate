...
const operationalInputScript =
  ...
  resolve(dirname(fileURLToPath(import.meta.url)), "../../../bin/xo-operational-input.sh");

export const XO_CURRENT_OPERATIONAL_KINDS = [
  "session-start",
  "watcher",
  "turn-end-guard",
  "away-supervisor",
...
export type XoCurrentOperationalKind =
  (typeof XO_CURRENT_OPERATIONAL_KINDS)[number];

type OperationalInputCommand = "encode" | "classify" | "kind";

export function xoShellInvocation(
  script: string,
...
function encodeFailure(kind: XoCurrentOperationalKind): Error {
  return new Error(`could not encode XO operational input kind ${kind}`);
}

export function encodeXoOperationalInput(
  kind: XoCurrentOperationalKind,
  content: string,
): string {
...
export async function encodeXoOperationalInputWith(
...
export function classifyXoOperationalText(content: string): string | undefined {
  return runOperationalInputCommand("classify", content);
}

export function classifyXoCurrentOperationalText(
  content: string,
): string | undefined {
