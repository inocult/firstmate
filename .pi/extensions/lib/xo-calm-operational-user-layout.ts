...
// and throws if it is missing; xo-calm.ts catches that and skips only this adapter with a
...
import { calmPresentationHides } from "./xo-calm-visibility.ts";
import { classifyXoCurrentOperationalText } from "./xo-operational-input.ts";
...
const CALM_OPERATIONAL_USER_LAYOUT_PATCH = Symbol.for(
  "xo:calm-operational-user-layout:pi-0.81.1",
);
...
export function installCalmOperationalUserLayout(): void {
  ...
  const isOperationalInput = (text: string): boolean => {
    ...
    return (
      classifyXoCurrentOperationalText(text) !== undefined ||
  ...
  if (typeof InteractiveMode !== "function") {
    throw new Error("XO Calm requires Pi InteractiveMode");
  }
  ...
  if (typeof originalAddMessageToChat !== "function") {
    throw new Error("XO Calm requires Pi InteractiveMode.addMessageToChat");
  }

  const UserMessageComponent = PiCodingAgent.UserMessageComponent;
  if (typeof UserMessageComponent !== "function") {
    throw new Error("XO Calm requires Pi UserMessageComponent");
  }
