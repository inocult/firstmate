// XO's home-persistent Pi transcript presentation toggle.
//
...
// ./lib/xo-calm-working-ship.ts owns the animated working presentation this file
...
import { installCalmAssistantLayout } from "./lib/xo-calm-assistant-layout.ts";
import { installCalmOperationalUserLayout } from "./lib/xo-calm-operational-user-layout.ts";
import {
  CALM_WORKING_SHIP_WIDGET_KEY,
  createCalmWorkingShipAnimation,
  createCalmWorkingShipWidget,
} from "./lib/xo-calm-working-ship.ts";
import {
  calmPresentationHides,
  calmPresentationIsActive,
  XO_CALM_PRESENTATION_EVENT,
  registerXoSyntheticPresentation,
  setCalmPresentation,
...
function installCalmPresentationAdapter(name: string, install: () => void): void {
  try {
    ...
    console.error(`XO Calm: ${name} presentation adapter unavailable, skipping. ${reason}`);
  }
}
...
export default function (pi: ExtensionAPI) {
  ...
  const configDirectory = process.env.XO_CONFIG_OVERRIDE || resolve(fmHome, "config");
  ...
  const publishPresentationState = (): void => {
    pi.events.emit(XO_CALM_PRESENTATION_EVENT, {
      active: calmPresentationIsActive(),
      stockExportRendering: exportRendering,
    });
  };

  registerXoSyntheticPresentation(pi);
  ...
  function wrapBuiltIn<TParams extends TSchema, TDetails, TState>(
    ...
    if (!originalRenderCall || !originalRenderResult) {
      throw new Error(`XO calm mode requires both render slots for Pi built-in tool ${original.name}`);
    }
  ...
  function contestedBuiltIns(): ToolDefinition<any, any, any>[] {
    ...
    try {
      ...
      const reason = error instanceof Error ? error.message : String(error);
      console.error(`XO Calm: built-in ownership check unavailable, claiming every built-in unconditionally. ${reason}`);
      return [];
    }
  ...
  function activateBuiltInsIfNeeded(ui: ExtensionUIContext): void {
    ...
    ui.notify(
      `XO Calm: the ${names} built-in tool${plural ? "s are" : " is"} already provided by another extension, so Calm may not fully function for ${plural ? "them" : "it"} this session.`,
      "warning",
    );
    for (const tool of contested) {
      console.error(`XO Calm: skipped claiming built-in "${tool.name}" because another extension already owns it.`);
    }
  }
  ...
  function reportBuiltInLosses(): void {
    ...
    try {
      ...
      const reason = error instanceof Error ? error.message : String(error);
      console.error(`XO Calm: built-in ownership check unavailable. ${reason}`);
      return;
    }
    for (const tool of wrappedBuiltIns) {
      const owner = registered.find((info) => info.name === tool.name)?.sourceInfo;
      if (owner && owner.source !== "builtin" && realpathOrSelf(owner.path) !== extensionRealFile) {
        console.error(
          `XO Calm: another extension (${owner.path}) also claimed the built-in "${tool.name}" tool and won; Calm's presentation for it is unavailable this session.`,
        );
      }
    }
  }
  ...
  pi.on("session_start", (_event, ctx) => {
    ...
    ctx.ui.setStatus("xo-calm", undefined);
  ...
  pi.registerCommand("calm", {
    description: "Toggle XO's supported conversation-only transcript presentation.",
