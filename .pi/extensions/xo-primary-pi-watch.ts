// XO primary watcher bridge for Pi.
//
...
const armReadyTimeoutMs = positiveInteger(
  "XO_PI_ARM_READY_TIMEOUT_MS",
  process.platform === "win32" ? 35000 : 12000,
);
const armRetireTimeoutMs = positiveInteger("XO_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const repairOnlyHint = "call xo_watch_arm_pi again only after a later notification says the cycle is missing, failed, or unhealthy";
...
type ReplacementCoordinatorGlobal = typeof globalThis & {
  __xoPiWatchReplacements?: Map<string, ReplacementCoordinator>;
};
const replacementCoordinatorGlobal = globalThis as ReplacementCoordinatorGlobal;
const replacementCoordinators = replacementCoordinatorGlobal.__xoPiWatchReplacements ??= new Map<string, ReplacementCoordinator>();
...
export default function (pi: ExtensionAPI) {
  ...
  function offerWakeToBranch(message: string): Promise<void> | null {
    ...
    const taskIdentity = (key: string): string =>
      scope.taskByWakeKey[key] ?? scope.taskByWakeKey[key.replace(/^xo-/, "")] ?? key;
    ...
    pi.events?.emit?.(XO_BRANCH_DISPATCH_EVENT, offer);
  ...
  async function processPendingActionables(owner: SessionGeneration): Promise<void> {
    ...
    try {
      ...
      if (generationIsLive(owner)) {
        ...
        // bounded retries, which hands repair to main through xo_watch_arm_pi
  ...
  function startArm(owner: SessionGeneration, predecessorArmPid = ""): ArmResult {
    ...
    if (ownership === "missing") {
      return {
        ok: false,
        message: "watcher: not armed - no live session holds the lock; run bin/xo-session-start.sh to reclaim it, then call xo_watch_arm_pi to re-arm",
      };
    }
    markLoaded();
    if (owner.child) {
  ...
  pi.registerCommand?.("xo-watch-arm-pi", {
    description: "Arm xo watcher supervision through the Pi extension instead of foreground bash.",
  ...
  registerXoTool(pi, {
    name: "xo_watch_arm_pi",
    label: "Arm xo watcher",
    description: "Start the first required Pi watcher cycle, or repair one only after a notification says the cycle is missing, failed, or unhealthy. Do not call after ordinary work or ordinary notifications; the Pi extension re-arms automatically. Never run bin/xo-watch-arm.sh through bash.",
    ...
    promptGuidelines: [
      "Call xo_watch_arm_pi only for the first required cycle or after a notification says the cycle is missing, failed, or unhealthy. Do not call it after ordinary work, turn completion, or ordinary signal, stale, check, or heartbeat handling because the Pi extension owns re-arming. Never run bin/xo-watch-arm.sh through bash.",
    ],
