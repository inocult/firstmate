// XO supervision branch for Pi (docs/pi-supervision-branch.md).
//
...
// actionable wake here (lib/xo-branch-dispatch.ts); the branch handles it with
// real tools and reports through the xo_branch_report custom tool, which
// writes the durable outcome store FIRST (bin/xo-branch-outcome.sh), then
...
// xo-main-mirror context from Pi's
...
//
// Prefix stability (the cache contract, owner: bin/xo-branch-prompt.sh
...
// Pi start acquires the lock only when the session runs xo-session-start.sh,
...
// inside the branch's own shell. bin/xo-lease-lib.sh documents the grade and
...
import {
  ...
  XO_CALM_PRESENTATION_EVENT,
} from "./lib/xo-calm-visibility.ts";
import {
...
} from "./lib/xo-branch-dispatch.ts";
import {
...
} from "./lib/xo-branch-model-picker.ts";
import {
  classifyXoOperationalText,
  encodeXoOperationalInputWith,
} from "./lib/xo-operational-input.ts";

const extensionFile = fileURLToPath(import.meta.url);
const extensionDir = dirname(extensionFile);
const root = resolve(extensionDir, "../..");
const fmHome = process.env.XO_HOME || process.env.XO_ROOT_OVERRIDE || root;
const fmRoot = process.env.XO_ROOT_OVERRIDE || root;
const state = process.env.XO_STATE_OVERRIDE || `${fmHome}/state`;
const config = process.env.XO_CONFIG_OVERRIDE || `${fmHome}/config`;
...
const promptScript = join(fmRoot, "bin", "xo-branch-prompt.sh");
const outcomeScript = join(fmRoot, "bin", "xo-branch-outcome.sh");
const leaseScript = join(fmRoot, "bin", "xo-lease.sh");
const wakeGrantScript = join(fmRoot, "bin", "xo-wake-grant.sh");
...
const BRANCH_TOOL_NAMES = ["read", "bash", "xo_branch_report"] as const;
...
const branchCacheKey = `xo-branch-${createHash("sha256").update(fmHome).digest("hex").slice(0, 24)}`;

const MIRROR_MESSAGE_CAP = 4000;
const MERGE_NOTE_BOAT = "⛵";
const VISIBLE_OUTCOME_ANCHOR = "⚓";
const VISIBLE_OUTCOME_ENTRY_TYPE = "xo-branch-visible-outcome";
...
// and only main's explicit sequence-bound acknowledgement (xo_branch_processed)
...
const PROCESSING_MESSAGE_TYPE = "xo-branch-process";
...
const PROCESSING_INSTRUCTION =
  ...
  "Process each outcome now as xo: give the captain a visible response where one is due, answer or escalate a decision, act on a blocker or failure, or record that no further action is needed. " +
  "When every outcome below is processed, call xo_branch_processed with through={N} exactly once. " +
...
// package (tests/xo-pi-primary-types.test.sh) the moment Pi adds or removes a
// level, in either direction, so the list cannot drift into a stale XO
// catalog.
...
function isOperationalUserText(text: string): boolean {
  return classifyXoOperationalText(text) !== undefined;
}
...
export default function (pi: ExtensionAPI) {
  ...
  // A revision advances only after xo_branch_report has appended successfully,
  ...
  // to. xo_branch_report refuses every other task id during such a prompt,
  ...
  // awaited rather than synchronous now (lib/xo-async-exec.ts), which means a
  ...
  function deliverBranchHealthNote(text: string): void {
    const message = { customType: "xo-branch-merge", content: `${MERGE_NOTE_BOAT} ${text}`, display: true };
  ...
  async function releaseBranchLeases(expectedGeneration: number): Promise<boolean> {
    ...
    const result = await runCommandAsync("bash", [leaseScript, "release-actor", "--actor", "branch"], {
      cwd: fmRoot,
      env: { ...scriptEnv, XO_SUPERVISION_ACTOR: "branch" },
    });
  ...
  async function runOutcomeScript(args: string[]): Promise<{ ok: boolean; stdout: string; detail: string }> {
    ...
    return {
      ok: false,
      stdout: "",
      detail: `xo-branch-outcome.sh exited ${result.status ?? "none"}: ${(result.stderr || "").trim()}`,
    };
  }
  ...
  async function processingRequestInput(rows: OutcomeRow[]): Promise<string> {
    ...
    try {
      return await encodeXoOperationalInputWith(runCommandAsync, "branch-outcome", body);
    } catch {
      return body;
    }
  }
  ...
  // xo_branch_processed does, keyed to the sequence main acknowledges.
  ...
  async function reconcileUnreadOutcomes(expectedGeneration: number, present = true): Promise<boolean> {
    ...
    if (unread.stdout) {
      ...
      for (const line of unread.stdout.split("\n")) {
        ...
        // xo-pi-routine-delivery-idempotency-followup-r1: if the mark-read
  ...
  async function createBranch(
    ...
    if (prompt.status !== 0 || !prompt.stdout || prompt.stdout.length < 1024) {
      throw new Error(
        `xo-branch-prompt.sh did not produce a usable branch prompt (status=${prompt.status ?? "none"}): ${(prompt.stderr || "").trim()}`,
      );
    }
    ...
    const bashTool = createBashToolDefinition(fmRoot, {
      spawnHook: (context) => {
        ...
        return {
          ...
          // model lives in bin/xo-lease-lib.sh.
          command: `readonly XO_SUPERVISION_ACTOR XO_LEASE_HOLDER_PID
(
${context.command}
)`,
          env: {
            ...
            XO_LEASE_HOLDER_PID: leaseHolderPid,
          },
        };
      },
    });
  ...
  async function flushMirror(session: AgentSession, expectedGeneration: number): Promise<void> {
    ...
    while (pendingMirror.length > 0) {
      ...
      await session.sendCustomMessage(
        { customType: "xo-main-mirror", content: `[${item.tag}] ${item.text}`, display: false },
        {},
      );
      ...
      .then(async () => {
        ...
        try {
          await session.prompt(
            `XO SUPERVISION WAKE: ${message}\n\nHandle this per your operating procedure and finish with xo_branch_report.`,
          );
        } finally {
          wakeTaskScope = null;
        }
  ...
  // the moment emit returns (lib/xo-branch-dispatch.ts owns that handshake).
  ...
  pi.events?.on?.(XO_BRANCH_DISPATCH_EVENT, (data) => {
  ...
  // thinking levels for the model just chosen, with no parallel XO
  ...
  pi.registerCommand?.("supervision-model", {
    description: "Pick the model and reasoning effort XO's Pi supervision branch uses, or follow main's.",
  ...
  // branch, and it has no room for the "follow main" row or for XO's
  // branch-runtime eligibility filter. Ordering and filtering live in
  // lib/xo-branch-model-picker.ts; everything here is Pi's own rendering.
  ...
  // parallel XO picker catalog. Cancelling leaves the current effort
  ...
  pi.events?.on?.(XO_CALM_PRESENTATION_EVENT, (data) => {
  ...
  const getStockOutcomesPreviewLines = (): number | undefined => {
    ...
    const probeTokens = Array.from(
      { length: 64 },
      (_, index) => `XO_OUTCOMES_PREVIEW_PROBE_${String(index).padStart(2, "0")}`,
    );
    try {
      const probeDefinition: ToolDefinition = {
        name: "xo_outcomes_preview_probe",
        label: "Preview probe",
  ...
  pi.registerMessageRenderer?.("xo-branch-merge", (message, _options, theme) => {
