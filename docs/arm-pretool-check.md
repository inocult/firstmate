...
`bin/xo-arm-command-policy.mjs` is the single semantic owner.
`bin/xo-arm-pretool-check.sh` is only the stable harness transport and output renderer.
The tracked harness adapters forward command text without classifying it.
`bin/xo-arm-command-policy.mjs` is also the sole owner of XO's shell classification: it exports the tokenizer and command-position analysis, which the sibling cd-guard seatbelt (`bin/xo-cd-pretool-check.sh`, `docs/cd-guard.md`) reuses instead of duplicating shell lexing.

## Purpose and boundary

An XO primary must arm `bin/xo-watch-arm.sh` or run `bin/xo-watch-checkpoint.sh` through an observable harness call.
...
`bin/xo-guard.sh` and `bin/xo-turnend-guard.sh` apply their respective post-arm supervision predicates to the watcher lock and beacon after an allowed call.
...
`bin/xo-arm-pretool-check.sh` supports these entry forms:
...
The active XO home is `${XO_HOME:-<code-root>}`.
It passes both roots and the exact command string to the Node policy owner.

The wrapper fast-allows a command without invoking the Node policy owner only when the command cannot contain the `xo-watch` byte sequence even after the classifier's decoders run.
The fast path may allow only when both of these hold:

1. The stripped text lacks the `xo-watch` watcher substring, after mirroring the classifier's cheapest byte normalizations - dropping line-continuation and escape backslashes, quotes, and newlines.
...
Any `xo-watch` match or any quoting-decoder marker delegates to the classifier.
Normalizing first keeps this a strict superset: a protected watcher path obfuscated as `xo-watc\<newline>h-arm.sh` or `xo-"watch"-arm.sh` still delegates, and stripping only those non-alphanumeric bytes can never destroy an existing `xo-watch` run.
The quoting-decoder marker closes the case the byte strip cannot: `bin/xo-$'\x77'atch-arm.sh` and `bin/xo-$"watch"-arm.sh` both resolve to `bin/xo-watch-arm.sh` only after the classifier decodes the encoded character, so a cheap byte strip would otherwise lose the `xo-watch` bytes and fast-allow them.
This marker set is coupled to the classifier's decoder set in `bin/xo-arm-command-policy.mjs`: adding any new quote or expansion form the classifier decodes requires extending this marker set in the same change, or the prefilter stops being a strict superset.
...
```text
bin/xo-watch-arm.sh          (arm; blessed entry point)
bin/xo-watch-checkpoint.sh   (checkpoint; blessed entry point)
bin/xo-watch.sh              (watch; protected but never blessed)
```

The relative form, the `<code-root>`-anchored absolute form, and any word ending in `/bin/<script>` all resolve to that identity.
Suffix matching recognizes an expanded-path prefix statically, so `$XO_HOME/bin/xo-watch-arm.sh`, `$HOME/xo/bin/xo-watch-arm.sh`, and `~/xo/bin/xo-watch-arm.sh` are the arm identity.
The classifier never expands the variable or tilde; it matches the literal bytes only.
Static quote forms are cooked before the suffix match, so a command word split by ordinary quotes (`xo-"watch"-arm.sh`), ANSI-C quoting (`xo-$'\x77'atch-arm.sh`), or a bash locale string (`xo-$"watch"-arm.sh`) all resolve to the same identity; this reads the fixed literal bytes as the shell would cook them and never runs an expansion or a command.
...
`bin/xo-watch.sh` is protected but is not a blessed entry point.
A direct `bin/xo-watch.sh` execution - relative, `<code-root>`-anchored, `$VAR`-prefixed, or `~`-prefixed - always denies with `watcher-direct`, whose reason points the caller at `bin/xo-watch-arm.sh` and `bin/xo-watch-checkpoint.sh`.
...
`bin/xo-watch-arm.sh` and `bin/xo-watch-checkpoint.sh` are the only blessed final nodes, including their expanded-path forms; a `bin/xo-watch.sh` final node is never blessed and denies with `watcher-direct`.
...
The allowed x-mode paths are `config/x-mode.env`, `./config/x-mode.env`, and an absolute path that normalizes to `<active-xo-home>/config/x-mode.env`.
...
An actually executed `pkill` command is denied when its parsed pattern arguments target `xo-watch`.
Path-qualified `pkill`, `command pkill`, and `sudo pkill` are recognized.

`kill "$(pgrep -f '/bin/xo-watch.sh')"` is also denied because the executed `kill` consumes an executed watcher-wide `pgrep` substitution.
A standalone read-only `pgrep` is allowed.
Quoted text such as `echo 'pkill -f xo-watch'` is data and is allowed.
...
When the command carries such grammar and its raw bytes reference both a `xo-watch` target and a `pkill` or `kill` verb, the classifier cannot prove which command position the kill occupies, so it denies with `broad-watcher-kill` rather than allowing.
This backstop mirrors the protected-execution fail-closed rule and covers forms like `while true; do pkill -f xo-watch; done`, `for x in 1; do pkill -f xo-watch; done`, `case x in x) pkill -f xo-watch ;; esac`, and `until false; do kill $(pgrep -f xo-watch); done`.
It is gated on the grammar being unsupported: in grammar the classifier does model, command-position analysis is authoritative, so data mentions such as `echo 'pkill -f xo-watch'` and a loop that only names the watcher without a kill verb such as `for f in 1; do echo xo-watch; done` remain allowed.
...
| `watcher-direct` | A direct `bin/xo-watch.sh` execution; the watcher must be reached through `bin/xo-watch-arm.sh` or `bin/xo-watch-checkpoint.sh`. |
...
| Grok | `.toolInput.command` | `.grok/hooks/xo-primary-pretool-check.json` forwards stdin and Grok consumes the stdout `decision=deny` object. |
| OpenCode | `output.args.command` | `.opencode/plugins/xo-primary-pretool-check.js` passes one `--command` argument and throws only for exit 2. |
| Pi / pi-signed | `event.input.command` | `.pi/extensions/xo-primary-turnend-guard.ts` passes one `--command` argument and returns `{block: true}` only for exit 2. |
| omp | `event.input.command` | `.omp/extensions/xo-primary-turnend-guard.ts` passes one `--command` argument and returns `{block: true, reason}` only for exit 2; omp surfaces the reason verbatim to the model (verified 18.1.2). |
...
Validation ran in a git-initialized scratch XO-shaped project under this task worktree.
...
The OpenCode interactive check used the dedicated tmux socket `xo-pretool-smoke`.
...
pgrep -fl '/bin/xo-watch.sh' || true
source '<scratch-project>/config/x-mode.env'; bin/xo-watch-checkpoint.sh --seconds 180
...
pi -p -e .pi/extensions/xo-primary-turnend-guard.ts --no-context-files --no-session "$PROMPT"
```
...
The Grok debug transcript showed four exit-0 results from `project/xo-primary-pretool-check`, then exit 2 with 145 stdout bytes, 214 stderr bytes, and `hook denied` for the backgrounded arm.
OpenCode displayed the four allowed command outputs and then `bin/xo-watch-arm.sh & failed` with the stderr deny object.
...
- Claude ran `bin/xo-watch-arm.sh --restart` with its native tracked background option and produced `watcher: started pid=<scratch> (scratch)`.
...
- OpenCode ran in an interactive TUI on `tmux -L xo-pretool-smoke`, reached `session.idle`, and its unchanged watch-arm plugin created the scratch automatic-arm marker.
- Pi loaded both primary extensions, called `xo_watch_arm_pi`, and created the scratch automatic-arm marker.

Every native-path automatic marker was present and every deny sentinel remained absent.

## Automated validation

`tests/xo-arm-pretool-check.test.sh` owns the adversarial acceptance matrix.
...
```sh
bash -n bin/xo-arm-pretool-check.sh
shellcheck bin/xo-arm-pretool-check.sh tests/xo-arm-pretool-check.test.sh
node --check bin/xo-arm-command-policy.mjs
tests/xo-arm-pretool-check.test.sh
bin/xo-test-run.sh --all
```
