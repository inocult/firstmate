...
This document is the authoritative human-readable contract for the guard that stops an XO primary from delegating work outside the fleet.

The shipped mechanism is `bin/xo-subagent-pretool-check.sh`, a PreToolUse guard that denies a delegation-SHAPED tool name in a genuine primary home.
...
On 2026-07-22 an XO primary ran four workers through Claude Code's built-in subagent tool instead of `bin/xo-spawn.sh`.
...
Only `bin/xo-spawn.sh` writes `state/<id>.meta`, so untracked project work contributes nothing to the in-flight count used by `bin/xo-supervision-lib.sh` and `bin/xo-turnend-guard.sh`.
...
`bin/xo-subagent-pretool-check.sh` is the shipped layer.
...
- `PLAN_ONLY_TOOLS`: the exact names `taskcreate` and `taskupdate` are allowed.
  ...
  That list has no executor: it spawns no agent, allocates no worktree, registers no schedule, and starts nothing that could outlive the session or escape an XO guard.
  So it is not the "work, agent, schedule, or isolated workspace that XO would not know about" the guard exists to stop, and the stem match on `task` is a false positive rather than a policy.
  The cost of the false positive was concrete: the primary could not track its own plan, and the deny text told it to run `bin/xo-brief.sh` and `bin/xo-spawn.sh` to create a todo entry.
...
The shipped hook fires only in a genuine XO primary home, using the shared predicate `xo_primary_scope_matches` from `bin/xo-primary-scope-lib.sh`.
This is the same predicate `bin/xo-sessionstart-nudge.sh` and `bin/xo-turnend-guard.sh` use, so the three tracked primary-scoped hooks cannot drift apart.

A home is in scope when it has `AGENTS.md`, a `bin/` directory, an existing state directory, and either a plain checkout where git-dir equals git-common-dir or a valid `.xo-secondmate-home` marker.
...
A crewmate's disposable task worktree is a linked git worktree, which is the shape `bin/xo-spawn.sh` always hands out, so it is out of scope.
A crewmate using delegation tools inside its own task worktree is legitimate and stays allowed.
A non-XO repo is out of scope.
...
`XO_ALLOW_SUBAGENT=1` in the session environment allows the call at the shipped hook.
...
When `bin/xo-scout.sh` exists in the home the message first defers to the `AGENTS.md` intake classification, then routes work already classified as a scout there and authorized ship work with its bounded research to `bin/xo-brief.sh` then `bin/xo-spawn.sh`.
When that script is absent the message still defers to intake classification and degrades to naming `bin/xo-brief.sh` then `bin/xo-spawn.sh` for dispatched work, rather than pointing at a script that is not there.
...
Applicability turns on one question: does the harness expose built-in delegation tools that a primary session could use instead of `bin/xo-spawn.sh`?
...
| omp | present, per bundled material | Not wired and unverified. omp ships a built-in task delegation tool: its bundled docs list `tools/task.md` and the captain-level `task.maxConcurrency` setting governs it. No XO delegation seatbelt is wired for it yet, and its status stays unverified until a live tool enumeration is recorded the way the Codex row was. |
...
- Grok's tracked hooks (`.grok/hooks/xo-primary-pretool-check.json`, `.grok/hooks/xo-primary-cd-check.json`) use a `PreToolUse` matcher, currently `Bash`, and pipe stdin to a checker.
...
This repo's rule in the `xo-coding-guidelines` skill is that a harness hook must be validated in a scratch project before it is trusted, and `arm-pretool-check.md` records the concrete cost of guessing: a Grok hook whose `command` string is even slightly wrong fails to launch the hook at all.
...
`bin/xo-subagent-pretool-check.sh` needs no change for any of them: it already accepts Grok's stdin shape and the `--tool` CLI form OpenCode and Pi use, and it already emits the Grok stdout decision object by default.
...
Run in a scratch XO-shaped project containing `AGENTS.md`, `state/`, a full copy of `bin/`, and a Claude settings file containing the local deny list exactly as recommended on that date, which was the 18-name form that still included `TaskCreate` and `TaskUpdate`.
...
> [subagent-dispatch] the xo primary dispatches through the fleet, not the harness's own
> delegation tools... (blocked tool: Workflow). Launch the session with XO_ALLOW_SUBAGENT=1 for a
...
The same `Workflow` prompt in the scratch primary home, launched as `XO_ALLOW_SUBAGENT=1 claude -p ...`:
...
`tests/xo-subagent-pretool-check.test.sh` asserts stdout is empty on every `--claude` deny and that default mode still emits the Grok object on stdout.
...
`tests/xo-subagent-pretool-check.test.sh` owns the acceptance matrix and is registered in the `pure-contract-unit` family in `bin/xo-test-run.sh`.
It covers the tracked Claude settings boundary that forbids a `permissions` key; the match-all Claude hook registration; denial of every work-creating delegation tool by shape; denial of twelve hypothetical future tool names that appear on no list; the observe-or-stop, plan-only, and MCP exclusions; the exactness of the plan-only exclusion against six near-miss names a substring or shorter-stem widening would release; the scout-present and scout-absent message variants; the escape hatch including its fail-closed values; inertness in a linked task worktree and in a non-XO repo; in-scope enforcement for a marked secondmate home; both stdin transports; the empty-stdout requirement; fail-open transport behavior; and the preserved `Bash` seatbelts and `Stop` guard.

Run:

```sh
bash -n bin/xo-subagent-pretool-check.sh
bin/xo-lint.sh
...
Every XO guard's in-flight-work branch keys off `state/<id>.meta`, and only `bin/xo-spawn.sh` writes that record.
`bin/xo-supervision-lib.sh` also recognizes a Relay poll as supervision need, but unaccounted primary work still contributes nothing to that predicate.
...
That is a separate change to `bin/xo-supervision-lib.sh` and `bin/xo-turnend-guard.sh` and is out of scope here.
