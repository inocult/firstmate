...
XO talks to a running agent two ways, and they are not the same channel.

The **data plane** is [`bin/xo-send.sh`](../bin/xo-send.sh): conversational text for the agent to read.
For a `kind=secondmate` target it always prepends the from-primary routing marker, because a secondmate is itself an XO and its reply must come back through the status path rather than a chat nobody reads.

The **control plane** is [`bin/xo-control.sh`](../bin/xo-control.sh): allowlisted lifecycle verbs addressed to an exact task id.
...
A routing-marked `/quit` arrives as ordinary chat - `[xo-from-primary] /quit` - which the agent reasons about instead of executing.
...
`bin/xo-control-lib.sh` is the single executable owner of three capability tables, with no side effects, so it can be read as a contract:
...
- **Per-harness mechanics**: the key that cancels a running turn, how many times it must be delivered, whether the composer needs clearing afterwards, the command that exits the agent, and which task kinds the adapter is verified to run.
  These were previously carried only in the [`harness-adapters`](../skills/playbooks/harness-adapters/SKILL.md) skill's tool references, which now point here.
  `bin/xo-send.sh`'s `--key` path reads the composer-clear table from this owner too, rather than keeping a second copy of it.
...
`xo_control_harness_family` is the one place that prefix rule is stated, and an unrecognized value resolves to no adapter rather than being guessed into one.
...
Removing a worktree, closing an endpoint, or discarding work stays with [`bin/xo-teardown.sh`](../bin/xo-teardown.sh), which owns the landed-work test.
...
1. **Resolve the profile.**
   ...
   A ship or scout keeps the harness already recorded for it, because that harness comes from XO's dispatch-profile judgment at intake and must not be silently re-read from configuration.
...
5. **Launch the replacement** through its single owner, `bin/xo-spawn.sh --relaunch`, which adopts the recorded endpoint and worktree instead of creating either, clears the previous harness's per-task wiring, and arms a fresh busy generation.
...
- Targeting is exact.
  ...
  A legacy `xo-<id>` window label, an explicit `session:window` endpoint, and a record whose `endpoint_task_id` names another task are all refused.
- A remotely placed secondmate is refused by name.
  ...
  For `relaunch` that host-side drive is `bin/xo-on.sh <id> xo-remote-secondmate-control.sh relaunch ...`, whose host-local leg runs this same plane against a record that is ordinary and local there, so every checkpoint, journal, rollback, and postcondition below applies unchanged ([`docs/remote-secondmates.md`](remote-secondmates.md)); `interrupt` and `exit` have no such route.
...
- `xo-spawn --relaunch` independently refuses unless the recorded endpoint is positively agent-free, so a replacement can never join a live agent.
...
Per-harness interrupt keys, repeat counts, composer clears, exit commands, and supported task kinds live in `bin/xo-control-lib.sh` and are exercised for every verified harness by `tests/xo-control.test.sh`.
The empirical basis for each adapter's value is the `harness-adapters` skill's verification record for that adapter.

## Verification

- `tests/xo-control.test.sh` - the adapter contract for every verified harness, the backend capability matrix, exact-id scoping, the closed verb list, the busy, idle, dead, and idempotent lifecycle cases, and marker non-regression, all against a stubbed session provider.
- `tests/xo-control-relaunch.test.sh` - the relaunch transaction: identity preservation, harness switching, the progress note, checkpoint refusals, and rollback after a failed launch.
- `tests/xo-control-herdr-smoke.test.sh` - the second state-verified backend against the real herdr binary, on an isolated throwaway lab session.
