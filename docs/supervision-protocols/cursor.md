...
2. Routine watcher arm and re-arm are owned by the `stop` hook (`bin/xo-turnend-guard-cursor.sh`), never by you.
...
3. An actionable close wakes you as a follow-up turn carrying the `watcher` operational kind.
   On that wake, run `bin/xo-wake-drain.sh` first and handle it.
   Do not run `bin/xo-watch-arm.sh` after an ordinary wake; the next turn end parks again automatically when supervision is still needed.
...
5. On a `turn-end-guard` follow-up, the park could not establish a live cycle.
   Inspect the watcher startup path rather than turning the notice into a repeating manual-arm loop; the nag is bounded by `XO_CURSOR_TURNEND_BLOCK_BUDGET` (default 3) and then stops on its own.
...
The watcher itself remains `bin/xo-watch.sh`, and `bin/xo-watch-arm.sh` remains the verified arm wrapper that the `stop` hook runs as its own tracked child.
