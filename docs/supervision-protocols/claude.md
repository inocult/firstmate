...
2. Routine watcher arm and re-arm are owned by the Stop `asyncRewake` hook (`bin/xo-claude-stop-autoarm.sh`), never by you.
...
3. On a `Stop hook feedback` wake (`signal:`, `stale:`, `check:`, or `heartbeat`), run `bin/xo-wake-drain.sh` first and handle the wake.
   Do not run `bin/xo-watch-arm.sh` after an ordinary wake; the next turn end re-arms automatically when supervision is still needed.
...
4. On the one `Stop hook feedback` automatic-mechanism failure notice (`xo watcher auto-arm FAILED ...`), drain, inspect the automatic mechanism failure, and do not turn the notice into a repeating manual-arm loop.
...
8. The turn-end guard (`bin/xo-turnend-guard.sh --claude`) remains the final backstop.
...
The watcher itself remains `bin/xo-watch.sh`, and `bin/xo-watch-arm.sh` remains the verified arm wrapper that the Stop hook foregrounds.
