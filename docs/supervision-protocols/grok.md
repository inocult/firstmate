...
2. Source `__XO_X_MODE_ENV__` first when Relay is active.
3. First cycle: arm with Grok's tracked background tool, as its own call:

   `run_terminal_command` with `background: true` on:
   `[ -f __XO_X_MODE_ENV_SH__ ] && . __XO_X_MODE_ENV_SH__; exec bin/xo-watch-arm.sh`
...
8. Waiting is silent.
9. Never use shell `&` for XO supervision.
10. Never bundle the arm onto another command.
    A shell `&`, a truncating pipe, or bundling is denied automatically by the PreToolUse seatbelt (`bin/xo-arm-pretool-check.sh`) whenever this project's Grok hooks are trusted.
...
1. Run `bin/xo-wake-drain.sh` first.
...
4. Ordinary wake: re-arm the next cycle with the same background `bin/xo-watch-arm.sh` call if the home still needs supervision, as `bin/xo-supervision-lib.sh` defines it.
...
The primary project Stop hook runs `bin/xo-turnend-guard-grok.sh` as a backstop, not the normal wake path.
...
Headless `grok -p` may wait for background process exit but does not reliably surface full auto-wake model output; do not run the primary XO as a one-shot headless process.
