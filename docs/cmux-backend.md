...
[`configuration.md`](configuration.md#runtime-backend-configbackend--xo_backend) owns shared selection and metadata semantics.

## Setup

Pick cmux when you already use the app as your terminal and want task workspaces in its sidebar.
cmux is macOS-only, GUI-first, and unsuitable for a headless or SSH-only XO session.
...
cmux defaults to a control mode that rejects external shells, while XO always controls it from an external process.
Open Settings > Automation and choose a viable Socket Control Mode before the first cmux-backed spawn.

| Setting | Value | XO support | Security boundary |
| --- | --- | --- | --- |
...
For Password mode, store the password as the first line of local gitignored `config/cmux-socket-password` or provide `CMUX_SOCKET_PASSWORD` in XO's environment.
...
Select cmux with local `config/backend` containing `cmux`, `XO_BACKEND=cmux` for one launch, or an explicit request to XO.
It can also be runtime auto-detected when XO itself runs inside cmux.
...
Routine supervision uses `bin/xo-peek.sh <id>` and `XO_HOME=<home> bin/xo-send.sh <id> '<text>'` without bringing the cmux window forward.
...
The caller-facing label remains `xo-<id>`, while the visible workspace title is `xo-<home-label>-<id>`.
The home label is `xo` or `2ndmate-<id>` plus a stable short hash of the resolved XO root.
cmux does not enforce title uniqueness, so create, recovery, list, and cleanup paths all validate this scoped title.
Relocating the XO installation changes the hash and leaves old titles unmatched, consistent with recorded worktree paths also becoming stale.
...
An ordinary metadata-routed `xo-send.sh` text steer becomes a durable steering-inbox record, and only its best-effort constant doorbell passes through cmux's submit machinery.
On the typed plane, literal send and Enter are separate calls.
Enter, Escape, and Ctrl-C are supported.
The composer verifier is a thin adapter: it captures a bounded plain-text tail and hands it with cmux's capability facts to the fleet-wide classifier in `bin/xo-composer-lib.sh`, which owns every shape, including Claude's borderless `❯` row with its U+00A0 separator.
...
When the task is last in its window, XO creates one unfocused unnamed sibling workspace in that same window, closes the task workspace, and leaves the window with cmux's fresh default workspace.
The sibling never carries an `xo-` title and is ignored by recovery.
...
XO does not attempt to close the macOS window because cmux's socket cannot close a window holding a live terminal.

Real tests share the captain's running app rather than creating an isolated cmux session.
`tests/cmux-test-safety.sh` permits cleanup only for an exact currently listed `xo-test-` workspace and never enumerates and closes unrelated workspaces or relaunches the app.
