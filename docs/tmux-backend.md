...
tmux is XO's verified reference runtime backend and the fully supported baseline for secondmate homes.
...
Select it explicitly with local `config/backend` containing `tmux`, with `XO_BACKEND=tmux` for one launch, or by asking XO to use tmux.
...
If the primary harness runs outside tmux, XO creates or reuses a detached session named `xo`:

```sh
tmux attach -t xo
```

Each task window is named `xo-<id>`.

```sh
tmux list-windows -t <session-name>
tmux select-window -t <session-name>:xo-<id>
```

Typing into an attached task window is authoritative direct intervention.
Routine supervision does not require attachment: `bin/xo-peek.sh <id>` captures a bounded tail and `XO_HOME=<home> bin/xo-send.sh <id> '<text>'` steers the recorded endpoint.

Verify setup by spawning a small task and confirming its `xo-<id>` window appears in the selected session.
...
The process-name vocabulary behind those verdicts is owned by `bin/xo-agent-process-lib.sh` and shared with the Herdr adapter, which proves a registered agent against the same names ([herdr-backend.md](herdr-backend.md) "Restart and liveness behavior").
...
The CI-enforced portable regression and opt-in real-harness drift guard follow the split owned by `.agents/skills/xo-coding-guidelines/SKILL.md`.
...
A task's busy, idle, unknown, or dead verdict comes from the semantic busy-state contract owned by `bin/xo-busy-lib.sh`; [architecture](architecture.md#busy-state-is-semantic-per-adapter) owns its boundaries.
...
`bin/xo-tmux-lib.sh` owns exact type-and-submit mechanics.
...
An ordinary local `xo-send.sh` text steer and every remote text steer no longer ride this verified submit at all: they become durable steering-inbox records plus best-effort constant doorbell lines (`bin/xo-task-inbox-lib.sh`).
The verdicts above are delivery-critical only for the local typed plane - harness-native invocations and explicit backend targets - where `xo-send.sh` still never retypes or assumes a confirmed submit for an unconfirmed verdict; its header owns the distinct delivered-unconfirmed exit status and operator response.
...
`tests/xo-tmux-submit-busy.test.sh` covers busy and idle panes with proven, ambiguous, and cleared composers.
...
```sh
tests/xo-backend-tmux-smoke.test.sh
tests/xo-tmux-agent-liveness.test.sh
tests/xo-harness-liveness-drift-live-e2e.test.sh
tests/xo-composer-ghost.test.sh
tests/xo-kimi-harness.test.sh
tests/xo-cursor-harness.test.sh
tests/xo-muse-harness.test.sh
tests/xo-omp-harness.test.sh
