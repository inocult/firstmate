...
- The branch lives in `.pi/extensions/xo-branch-supervision.ts`, which only a Pi primary ever loads; no other harness gains branch supervision behavior.
- The bash-side additions (leases, the outcome store, session-start recovery) are inert in a home with no branch state: no lease files exist, no actor variable is set, every guard passes silently, and no new state appears (`tests/xo-branch-supervision.test.sh` holds this).
...
- Wake dispatch: `.pi/extensions/xo-primary-pi-watch.ts` stays the dispatcher; `.pi/extensions/lib/xo-branch-dispatch.ts` owns the offer handshake and row eligibility, while [`watcher-continuity.md`](watcher-continuity.md#per-actor-acknowledgement) owns the per-actor consume contract.
  ...
  A decision-owned event surfaced by `bin/xo-watch.sh`'s signal path gets the identical treatment even though it keeps the ordinary `signal` kind.
...
- The branch itself: `.pi/extensions/xo-branch-supervision.ts` creates the branch session, serializes wakes, mirrors dialog, and merges outcomes.
  ...
  A prompt from the current branch generation and model or effort selection that appends a durable `xo_branch_report` and then settles without a provider error clears both the latch and provider-error streak and surfaces a one-line recovery note; a provider error settled after that report wins instead, re-latches the branch, and extends the cooldown.
...
- Branch system prompt: `bin/xo-branch-prompt.sh`; its header owns the byte-stable-prefix contract (no timestamps, no fleet snapshot, no per-wake content).
- Outcome store: `bin/xo-branch-outcome.sh`; its header owns the append-only format, read cursor, and bounded per-task status-coverage indexes.
  ...
  That asymmetry is a known limitation of the routine delivery representation rather than of the ordering above, it predates delivery moving off Pi's render thread, and closing it means giving routine delivery a durable idempotent record - tracked as follow-up `xo-pi-routine-delivery-idempotency-followup-r1` and pinned meanwhile by `tests/xo-pi-branch-extension.test.sh`.
- Consistency: `bin/xo-lease-lib.sh` owns the per-task lease contract, the main-only role partition, and the deliberate CONFUSED-AGENT-GRADE threat model these guards target (captain-decided; adversarial-grade separation is out of scope and tracked as follow-up design work); `bin/xo-lease.sh` is the command surface.
  The guards are wired into `xo-send.sh`, `xo-control.sh`, and `xo-teardown.sh` (overlap, lease-checked, with claim serialization retained through the mutation) and `xo-pr-merge.sh`, `xo-merge-local.sh`, and `xo-spawn.sh` (main-owned, branch refused; a relaunch through `xo-control` stays branch-legal recovery).
...
Subprocess work reached through Pi's asynchronous APIs is now awaited instead: `.pi/extensions/lib/xo-async-exec.ts` owns that awaited-spawn replacement and preserves the status, captured-output, and failure semantics its callers used from the synchronous form.

Awaiting yields the thread, so what the single thread used to guarantee for free is now an explicit queue in `.pi/extensions/xo-branch-supervision.ts`.
...
Main's captain and assistant text - never tool calls, tool results, operational injections, or the branch's own merged notes - is mirrored into the branch as read-only `xo-main-mirror` messages.
...
Stage two is the branch's verdict on each handled event, reported through its `xo_branch_report` tool: `routine` keeps the existing custom-message path without a follow-up turn, while `captain` appends a versioned `xo-branch-visible-outcome` custom session entry.
...
After the visible entry exists and the read cursor has passed it, the extension hands every still-unprocessed captain row to main as one hidden, typed `xo-branch-process` request (kind `branch-outcome`) listing each `[seq N] task: summary`, and that request opens exactly one main turn.
Main closes it only by calling `xo_branch_processed` with the highest sequence the request listed, which advances a processed marker that `bin/xo-branch-outcome.sh` keeps separately from the read cursor and never moves past it or backwards.
...
The branch runs its normal operating procedure for the wake (`bin/xo-branch-prompt.sh` "Handling a wake") and performs the deeper fleet review that main previously performed.
...
The captain accepted the normal provider prompt-caching strategy: a byte-identical branch prefix generated once per XO version, the same tool set in the same order on every request, and one shared `prompt_cache_key` per home for all branch sessions (set in a `before_provider_request` hook, and only for providers whose requests already carry that field); main keeps its own per-session key.
...
That carve-out is scoped to provider registration alone: the branch keeps its `noExtensions`, `noSkills`, and `noContextFiles` isolation, the copy is never persisted, a provider whose registration fails to compose is simply unavailable, and `tests/xo-pi-branch-extension.test.sh` pins the pin-and-fallthrough behavior.
No caching machinery beyond this exists, deliberately: any later dynamic content in the branch prefix silently removes most of the cache benefit, which is why `bin/xo-branch-prompt.sh`'s header is the contract's single owner and `tests/xo-branch-supervision.test.sh` pins the output to byte identity.

## Away mode

On Pi the away daemon is no longer launched: `/afk` writes the away-posture record (`state/.afk-contract`, owned by `bin/xo-afk-contract.sh`) and never the `state/.afk` daemon flag, so the branch keeps its attended shape under the record until the posture-aware dispatch lands in a later phase.
...
`tests/xo-watch-triage.test.sh` covers `bin/xo-watch.sh`'s side of the contract end to end: needs-decision, no-verb captain-held, and pending-reply second-mate escalation signal rows are marked `needs-decision:`, a needs-decision whose key transition was rejected by the reserved-key vocabulary (`xo-classify-lib.sh`'s `reconciliation-required:` wrapper) is still marked, and ordinary blocked or captain-relevant signals stay unmarked.
Live guards: `XO_PI_BRANCH_LIVE_E2E=1 tests/xo-pi-branch-live-e2e.test.sh` exercises the real installed Pi SDK's immediate active-transcript appendEntry rendering, persistence, custom-entry model exclusion, branch-session surfaces, and watcher-owned fallback after rejected branch settlement.
`XO_PI_BRANCH_RESPONSIVENESS_E2E=1 tests/xo-pi-branch-responsiveness-live-e2e.test.sh` answers the question only a real TUI can: it types into an isolated Pi pane while outcomes are delivered and fails if keystroke echo leaves the class of the same machine's extension-free floor.
Record dated current results in [docs/verification/runtime-backends.md](verification/runtime-backends.md).
The strict typecheck in `tests/xo-pi-primary-types.test.sh` pins the extension against the installed Pi package.
