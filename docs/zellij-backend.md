...
Select it with local `config/backend` containing `zellij`, `XO_BACKEND=zellij` for one launch, or an explicit request to XO.
...
XO uses one shared session named `xo` by default.
`XO_ZELLIJ_SESSION` can select another name for isolated verification.
Attach with:
...
Use `bin/xo-peek.sh <id>` and `XO_HOME=<home> bin/xo-send.sh <id> '<text>'` against the metadata-routed endpoint.
...
The caller-facing label remains `xo-<id>`, while the visible title is home-scoped as `xo-<home-label>-<id>`.
The home label is `xo` or `2ndmate-<id>` plus a short stable hash of the resolved XO root.
This prevents task-id collisions between a primary, secondmates, and separate XO installations sharing one session.

Zellij does not enforce tab-name uniqueness, so the adapter performs its own duplicate check against the scoped title.
Create, recover, list, and cleanup paths all use the same scoped title owner in `bin/xo-backend-hometag-lib.sh`.
Moving an XO installation changes its path hash and leaves old titles unmatched, consistent with worktree paths also becoming stale after a move.
...
An ordinary metadata-routed `xo-send.sh` text steer becomes a durable steering-inbox record, and only its best-effort constant doorbell passes through Zellij's submit machinery.
...
Zellij exposes no cursor-row or native agent-state signal, but `dump-screen --ansi` (verified at 0.44.0) preserves styling, so the composer is read through the same fleet-wide classifier as tmux and herdr (`bin/xo-composer-lib.sh`), with ghost and placeholder text stripped before the verdict.
...
Real test cleanup uses only an isolated non-`xo` session and the guard in `tests/zellij-test-safety.sh`; it never calls all-session deletion commands.
