...
Exercised by `tests/xo-procevent.test.sh` against a fake blocking source whose completion is a process event, not a timer; for the supervision-delivery and headline rows below, by `tests/xo-watch-triage.test.sh` driving a real `bin/xo-watch.sh` over a real capture and over queued strand and launch-failure keys, with `tests/xo-watch-arm.test.sh` covering the arm-time refusal; and for adapter-owned application, by `tests/xo-remote-reply.test.sh` driving the real remote-reply relay end to end in an isolated home:
...
| strand and failure headlines | a real `bin/xo-watch.sh` surfaces queued `stranded` and `launch-failed` keys under `process-event source stranded` and `process-event source failed to start` rather than as a captured result, joins a mixed cycle's headlines, never re-delivers a key it has already surfaced, and delivers each new failure episode; `bin/xo-watch-arm.sh` refuses to arm on an unusable confirm window, naming the variable and range, with no beacon and no running watcher |
...
| condition->action process bounds | the same suite proves action timeout terminates descendants and command-output staging remains within `XO_WHEN_OUTPUT_TAIL_BYTES` while the command runs |
...
| absent extension registry parity | `tests/xo-extension-binding.test.sh` drives `list` and `verify` in a fresh home while the current directory contains project files and Pi packages and an environment variable names fake package data; both commands report no bindings, create no home path, and discover nothing outside `config/extensions.d` |
...
```sh
node --version
bin/xo-test-run.sh tests/xo-extension-binding.test.sh
XO_EXTENSION_BINDING_SEGMENT=lifecycle-invocation-cleanup bin/xo-test-run.sh tests/xo-extension-binding.test.sh
bin/xo-test-run.sh tests/xo-procevent.test.sh
XO_BEARINGS_LAVISH_LIVE=1 bin/xo-test-run.sh tests/xo-bearings-board-lavish-live-e2e.test.sh
...
The 2026-08-27 review inspected `bin/xo-harness.sh`, `bin/xo-supervision-instructions.sh`, `bin/xo-supervision-lib.sh`, the process-event delivery and reconcile boundaries in `bin/xo-watch.sh`, `bin/xo-backend.sh`, and `bin/xo-config-inherit-lib.sh` before marking integration axes not applicable.
...
| Claude, Codex, OpenCode, Pi, pi-signed, Grok, Kimi, Cursor, and Muse task workers | Not applicable after inspecting harness detection and launch ownership, because source registration has no task metadata or worker endpoint and the package is never launched through `xo-spawn`. |
...
The circular lock wait described at `release_start_claim` in [`bin/xo-procevent.sh`](../../bin/xo-procevent.sh) explains why healthy runners required the forced signal; the measured delay was the stop waiting for exit cleanup that could not acquire its lock.
...
Refresh the regressions with `bash tests/xo-procevent.test.sh`; the dated measurements above are recorded observations, not fixed timing thresholds.
...
Built-in adapters extend the runner through `bin/xo-procevent-<adapter>.sh`; the `when` adapter also uses the runner library's locked registration publisher so its private trust state and source registration are serialized under one source boundary.
...
The watcher reports a queued process-event result through the one shared actionable-exit path (`wake` in `bin/xo-push-transition-lib.sh`) that every existing signal, stale, and check wake already uses, so it reads no pane, queries no backend, and names no harness.
