...
```sh
pi -p -e .pi/extensions/xo-primary-turnend-guard.ts \
...
The omp Run-tier adapter was verified on 2026-09-05 with omp 18.1.11 and the openai-codex `gpt-6-astra` model through `XO_OMP_LIVE_E2E=1 tests/xo-omp-primary-live-e2e.test.sh`, which drives a real omp in its JSON-RPC stdio mode inside an isolated lab clone.
...
omp's `session_start` payload carries no reason field, so the adapter derives the source: the first start of the process is `startup` (or `resume` from a `--continue`/`--resume` launch line) and a later in-process start is `clear`; `tests/xo-omp-harness.test.sh` pins that mapping over a fake omp API.
...
The first two were measured on 2026-08-05 against a throwaway XO-shaped lab carrying each harness's own tracked registration with a recorder standing in for `bin/xo-sessionstart-run.sh`.
...
The interactive TUI is a known uncovered gap: XO has no tracked session-open, compaction, or re-emit channel there, ships no global hook, and does not claim instruction-refresh delivery for that surface.
...
The current adapter classification and baseline mechanics are owned by [`../sessionstart-nudge.md`](../sessionstart-nudge.md#harness-transports) and the `bin/xo-session-start.sh` header.
...
Its missing-native branch deliberately requests `bin/xo-session-start.sh`, so an escaped first call reproduces the duplicate-producing manual path rather than passing vacuously.

```sh
XO_PI_SESSIONSTART_RACE_LIVE_E2E=1 \
  tests/xo-sessionstart-hook-live-e2e.test.sh
```
...
# xo-sessionstart-hook-live-e2e.test.sh: offline Pi /new race assertions passed
```
...
The portable public-event regression in `tests/xo-sessionstart-nudge.test.sh` separately covers interruption, process-tree retirement, two rapid replacements, stale completion, empty output, spawn error, timeout output, truncation, ineligible stand-down, and compaction cancellation.
...
It used a scratch `XO_HOME`, a private tmux socket, and a disposable XO checkout.
...
```sh
XO_SESSIONSTART_INSTRUCTION_REFRESH_LIVE_E2E=1 \
XO_SESSIONSTART_INSTRUCTION_REFRESH_REF=origin/main \
XO_SESSIONSTART_INSTRUCTION_REFRESH_EXPECT=stale \
tests/xo-sessionstart-instruction-refresh-live-e2e.test.sh
...
Session start composes its digest from local reads and runs every external-network call in a worker detached by the hook (`bin/xo-startup-network.sh`), so a harness that reaped the hook's process tree would silently stop running the sweeps rather than merely delaying them.
Verified on 2026-08-06 with Claude Code 2.1.222 in a throwaway lab whose `bin/xo-bootstrap.sh` sleeps 6s before writing a marker, so the marker can exist only if the worker outlived the hook and the whole `claude -p` process.
...
The latency this buys was re-measured on 2026-08-06 against default-branch tip `8398d31`, in a throwaway home holding one remote secondmate whose host hangs 25s per SSH connection (an `XO_SSH_BIN`-shaped stub; no real host was contacted).
Both runs used the same fixture and the same `bin/xo-session-start.sh` invocation, differing only in which checkout supplied the script:
...
Codex and Pi were not installed as run-tier labs in this measurement, so their evidence for this fact is NOT refreshed; `tests/xo-sessionstart-hook-live-e2e.test.sh` asserts it for each installed Claude, Codex exec, and Pi adapter and is the command that refreshes their record.
...
```sh
tests/xo-sessionstart-nudge.test.sh
tests/xo-session-start.test.sh
tests/xo-startup-network.test.sh
XO_SESSIONSTART_HOOK_LIVE_E2E=1 tests/xo-sessionstart-hook-live-e2e.test.sh
XO_PI_SESSIONSTART_RACE_LIVE_E2E=1 tests/xo-sessionstart-hook-live-e2e.test.sh
XO_SESSIONSTART_INSTRUCTION_REFRESH_LIVE_E2E=1 tests/xo-sessionstart-instruction-refresh-live-e2e.test.sh
XO_PI_LIVE_E2E=1 tests/xo-pi-primary-live-e2e.test.sh
XO_OPENCODE_LIVE_E2E=1 tests/xo-opencode-primary-live-e2e.test.sh
```

`tests/xo-sessionstart-hook-live-e2e.test.sh` is the command that refreshes the Claude, Codex exec, and Pi table above; run it after upgrading any of those harnesses.
...
Cursor's refresh command is `XO_CURSOR_PRIMARY_LIVE_E2E=1 tests/xo-cursor-primary-live-e2e.test.sh`, recorded under [Cursor primary park](#cursor-primary-park-2026-08-13).
...
| --- | --- | --- | --- |
| Pi | 0.82.0 | Extension `agent_start` / `agent_settled` with `ctx.isIdle()` | The spawn seed `busy source=xo-spawn`, then `busy source=pi-ext event=agent-start`, then `idle source=pi-ext event=agent-settled`; the turn-end marker was still touched. |
| omp | 18.1.11 | Extension `agent_start` / `agent_end` without `willContinue` | Live Herdr scout on `openai-codex/gpt-6-astra` (2026-09-05): the spawn seed `busy source=xo-spawn`, then `busy source=omp-ext event=agent-start`, then `idle source=omp-ext event=agent-end` at the natural end of the brief; a steer through `xo-send` reopened `busy … agent-start`, and a control-plane interrupt closed it with `idle … agent-end` (omp fires `agent_end` on an interrupted turn). `ctx.isIdle()` is deliberately not consulted because it reads false at a natural TUI `agent_end`. |
...
| Claude | 2.1.220 (Claude Code) | Hooks `UserPromptSubmit`, `Stop`, `StopFailure`, `SessionEnd` | `UserPromptSubmit` fired for the argv launch prompt and each steer, and `Stop` closed every completed turn. A mid-stream Escape interrupt fired no closing hook, which is why the XO-controlled clear exists. `StopFailure` and `SessionEnd` are wired from the four hook names present in the installed binary; only the abnormal paths they cover were not reproduced live. |
...
In this 2026-07-28 Codex 0.145.0 semantic-busy probe, XO-written lifecycle project hooks under `<worktree>/.codex/hooks.json` fired for neither an interactive pane whose directory trust was granted nor `codex exec`, in both cases with `--dangerously-bypass-hook-trust`, while an untracked global probe fired in the same runs; XO does not ship, install, recommend, or depend on that global path.
...
```sh
tests/xo-busy-state.test.sh
tests/xo-busy-adapter-wiring.test.sh
...
| omp | 18.1.11 | Blocking `session_stop` hook returning `{ continue: true, additionalContext }` | In the isolated rpc lab (2026-09-05), the successor watcher was frozen with `SIGSTOP` until its beacon passed the lab `XO_GUARD_GRACE` of 20s while its arm child stayed attached (a killed watcher closes its arm child and the extension re-arms before the guard can fire); the next turn end raised the guard, the guard spy recorded `rc=2` followed by a stop carrying `stop_hook_active: true`, omp compelled a continuation carrying the `turn-end-guard` operational text, the `xo_watch_arm_omp` invocation count then rose to at least two, and a live watcher held the home lock after the thaw; the flagged stop was allowed, so exactly one continuation ran. `session_stop` never fired for an interrupted turn. |
...
Cursor was validated as a primary on 2026-08-13 against the installed CLI on macOS 26.5.2 arm64 with tmux 3.6a, in a throwaway XO home on a private tmux socket, never against a live home and never with a user-scope hook.
...
```sh
XO_CURSOR_PRIMARY_LIVE_E2E=1 tests/xo-cursor-primary-live-e2e.test.sh
```
...
The live run proved that session start acquires the fleet lock through Cursor's structural process identity in `bin/xo-cursor-lib.sh`; `tests/xo-session-lock-ancestry.test.sh` pins the same ancestry path portably.
It also proved that Cursor's `autoarm` supervision model lets the mid-turn pull guard accept a fresh beacon after the between-turn watcher closes; `tests/xo-guard-stale-banner.test.sh` pins that model-aware verdict.
...
It is therefore recorded as uncovered in the same sense as the Codex interactive TUI, and `tests/xo-cursor-primary.test.sh` asserts `preCompact` stays unregistered so it cannot return unnoticed without its own design and evidence.
...
```sh
XO_GROK_STOP_LIVE_E2E=1 \
  XO_GROK_NATIVE_BIN="$native_grok_0_2_112" \
  XO_GROK_LEGACY_BIN="$official_pre_native_grok_0_2_73" \
  tests/xo-grok-stop-live-e2e.test.sh
```
...
`tests/xo-turnend-guard.test.sh` now pins every tracked `.claude/settings.json` hook entry against a real grok 1.0.0 hook environment so the inertness contract is covered deterministically rather than only by the opt-in live matrix.

The secondmate-home scope and manual-repair wake path were measured with Claude Code 2.1.207 on 2026-07-12, when a native background completion re-invoked the idle model with no human input.
The current Stop-owned main/secondmate inclusion and child-worktree exclusion are covered deterministically by `tests/xo-claude-stop-autoarm.test.sh`.
Session-lock ownership in `bin/xo-session-lock-lib.sh` is decided against a session's whole contiguous harness ancestry rather than one chosen pid, so the Stop auto-arm reaches its lock owner wherever that owner sits: the outermost pid of Claude Code's multi-level `bg-spare` hook worker chain, or an inner pid when a harness-named daemon parents the session.
...
`tests/xo-session-lock-ancestry.test.sh` pins both platforms' reporting semantics behind a deterministic process table and runs the real Stop auto-arm in version-named, daemon-parented, and combined real process trees.
...
```sh
claude --version
XO_CLAUDE_LIVE_E2E=1 tests/xo-claude-stop-autoarm-live-e2e.test.sh
```
...
```sh
tests/xo-turnend-guard.test.sh
tests/xo-supervision-instructions.test.sh
XO_PI_LIVE_E2E=1 tests/xo-pi-primary-live-e2e.test.sh
XO_GROK_STOP_LIVE_E2E=1 XO_GROK_NATIVE_BIN="$native_grok" XO_GROK_LEGACY_BIN="$pre_native_grok" tests/xo-grok-stop-live-e2e.test.sh
```
...
```sh
bin/xo-lint.sh
bin/xo-doc-audience-check.sh
bin/xo-test-run.sh tests/xo-claude-stop-autoarm.test.sh tests/xo-guard-stale-banner.test.sh tests/xo-turnend-guard.test.sh tests/xo-supervision-instructions.test.sh
```

Observed output:

```text
xo-lint.sh: ShellCheck 0.11.0 (pinned 0.11.0)
xo-doc-audience-check: ok surfaces=61 local_links=174
XO_TEST_SUMMARY total=4 failed=0 skipped_gate=0 duration_ms=102585
```

The fresh-beacon portion of the model-aware pull-guard predicate (`bin/xo-guard.sh` accepts a beacon within grace without a live watcher under the Claude Stop auto-arm model) was verified on 2026-08-04 with the installed ShellCheck 0.11.0 and the same isolated behavior suites.
...
```text
xo-lint.sh: ShellCheck 0.11.0 (pinned 0.11.0)
xo-doc-audience-check: ok surfaces=64 local_links=188
XO_TEST_SUMMARY total=4 failed=0 skipped_gate=0 duration_ms=80078
```

The Pi extension-model pull-guard correction (`bin/xo-guard.sh` no longer reports a false watcher-down on a Pi primary during the extension's own watcher hand-off) was verified on 2026-08-13 with the installed ShellCheck 0.11.0 and isolated behavior suites.
The guard verdict itself reads only state files and process liveness, so the portable suites are the enforcing evidence; `bin/xo-harness.sh`'s Pi marker detection, which selects the model, is exercised in the same suite through `PI_CODING_AGENT`.

```sh
bin/xo-lint.sh
bin/xo-doc-audience-check.sh
bin/xo-test-run.sh tests/xo-guard-stale-banner.test.sh tests/xo-turnend-guard.test.sh tests/xo-session-start.test.sh tests/xo-pi-watch-extension.test.sh tests/xo-watch-arm.test.sh
```
...
```text
xo-lint.sh: ShellCheck 0.11.0 (pinned 0.11.0)
xo-doc-audience-check: ok surfaces=67 local_links=243
XO_TEST_SUMMARY total=5 failed=0 skipped_gate=0 duration_ms=280160
```
...
```sh
XO_SUPERVISION_MODEL=persistent XO_GUARD_READ_ONLY=1 bin/xo-guard.sh
XO_SUPERVISION_MODEL=extension XO_GUARD_READ_ONLY=1 bin/xo-guard.sh
```
...
```sh
bin/xo-test-run.sh tests/xo-watch-triage.test.sh tests/xo-watcher-lock.test.sh tests/xo-afk-inject-e2e.test.sh tests/xo-afk-return.test.sh tests/xo-x-mode.test.sh tests/xo-backend.test.sh tests/xo-backend-tmux-smoke.test.sh tests/xo-secondmate-safety.test.sh
```

Observed output:

```text
XO_TEST_SUMMARY total=8 failed=0 skipped_gate=0 duration_ms=617507
```

The actionable-close ordering correction was reverified on 2026-08-02 against an identity-matched live successor.

```sh
tests/xo-claude-stop-autoarm.test.sh >/dev/null && echo "xo-claude-stop-autoarm: ok"
```

Observed output:

```text
xo-claude-stop-autoarm: ok
```
...
| --- | --- | --- |
| Claude | `XO_CLAUDE_LIVE_E2E=1 tests/xo-claude-stop-autoarm-live-e2e.test.sh` | Session start reclaimed a stale owner before two Stop-owned cycles, and a competing live owner prevented arm, rewake, epoch write, or lock replacement. |
| Codex | `XO_CODEX_LIVE_E2E=1 tests/xo-codex-continuity-live-e2e.test.sh` | The one-second foreground checkpoint returned without switching to the arm wrapper. |
| OpenCode | `XO_OPENCODE_LIVE_E2E=1 tests/xo-opencode-primary-live-e2e.test.sh` | A verified successor existed before prompt handling, with no model re-arm or turn-end fallback. |
| Pi | `XO_PI_LIVE_E2E=1 tests/xo-pi-primary-live-e2e.test.sh` | One initial tool call led to extension-owned successors and clean child retirement on exit. |
| omp | `XO_OMP_LIVE_E2E=1 tests/xo-omp-primary-live-e2e.test.sh` | One initial `xo_watch_arm_omp` invocation (the openai-codex model reaches extension tools through omp's `xd://` virtual-file bridge, a `write` to `xd://xo_watch_arm_omp`, counted as the same invocation) started a live watcher; an actionable close spawned a ledger-linked successor and woke main exactly once; the lab is reaped by path, and omp 18.1.11 did not exit within 30s of its rpc stdin closing, recorded as a note. omp 18.1.11, 2026-09-05. |
| Grok | `XO_GROK_LIVE_E2E=1 tests/xo-grok-continuity-live-e2e.test.sh` | Native task completion surfaced the actionable close and the cycle ledger recorded `reason=actionable-signal`. |
...
```sh
pi --version
tests/xo-pi-watch-extension.test.sh
...
Plain Pi and pi-signed share the same tracked `.pi/extensions/xo-primary-pi-watch.ts` path, so both inherit the generation owner; other primary harnesses are not applicable because they do not use this Pi extension lifecycle.
...
```sh
bin/xo-test-run.sh tests/xo-watch-recovery-loop.test.sh
```
...
XO_TEST_SUMMARY total=1 failed=0 skipped_gate=0 duration_ms=59357
```
...
tests/xo-pi-watch-extension.test.sh
tests/xo-pi-primary-types.test.sh
tests/xo-watcher-lock.test.sh
tests/xo-watch-arm.test.sh
tests/xo-watch-recovery-loop.test.sh
tests/xo-wake-queue.test.sh
tests/xo-subagent-pretool-check.test.sh
tests/xo-claude-stop-autoarm.test.sh
tests/xo-turnend-guard.test.sh
```
...
/usr/bin/osascript \
  -e 'on run argv' \
  -e 'display notification (item 1 of argv) with title "XO TEST - IGNORE" sound name "Basso"' \
  -e 'end run' \
  'XO TEST - IGNORE (wedge-alarm channel verification)'
```
...
```sh
herdr notification show 'XO TEST - IGNORE' \
  --body 'XO TEST - IGNORE (wedge-alarm channel verification)' \
  --sound request
```
...
The safe command-channel contract is covered without a notification by `tests/xo-daemon.test.sh`: the summary reaches both `$1` and stdin, every channel is process-group bounded, and a failed channel falls through.
