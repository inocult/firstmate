...
Refresh it by rerunning the fixture below after changing any publisher named in `bin/xo-parent-channel-lib.sh`.
...
The mate home carries `.xo-secondmate-home` (`mate`) and a local `.xo-secondmate-parent` binding to `$P`; the parent home carries `state/mate.meta` (`kind=secondmate`, `home=$M`).
The mate's child task `child` is a real tmux pane (`fmlive:xo-child`) recorded in `$M/state/child.meta` with `mode=no-mistakes` and `yolo=off`; the mate itself is a second real pane (`fmlive:xo-mate`).
Both homes run the real `bin/xo-watch.sh` (`XO_POLL=2`), re-armed after every wake through `bin/xo-wake-drain.sh` acknowledgement, exactly as fleet supervision re-arms a watcher after a handled turn.
...
The mate's only actions are the scripts an XO runs when it registers a PR and when it holds a task for the captain and records the answer.
...
$ tmux send-keys -t fmlive:xo-child "echo 'done: PR https://github.com/kunchenguid/firstmate/pull/9999 checks green' >> $M/state/child.status" Enter
...
$ # Step 2: the mate registers the PR with xo-pr-check; the ready line with the canonical URL reaches the parent from the script itself

$ XO_HOME=$M XO_STATE_OVERRIDE=$M/state bin/xo-pr-check.sh child https://github.com/kunchenguid/firstmate/pull/9999
...
$ # Step 3: the mate holds a task for the captain; the hold reaches the parent from xo-captain-hold itself

$ XO_HOME=$M bin/xo-captain-hold.sh hold child-call --title 'Pick the rollout window' --reason 'rollout window choice pending' --repo alpha
child-call

$ # Step 4: the captain's answer is recorded in the mate home; the close reaches the parent from xo-captain-hold itself

$ XO_HOME=$M bin/xo-captain-hold.sh answer child-call --decision-file decision.txt   (decision: roll out on Monday)
answered: child-call
...
- Each line was written by the script that recorded the underlying fact: `bin/xo-inactive-reconcile.sh` on the mate watcher's poll, `bin/xo-pr-check.sh`, and `bin/xo-captain-hold.sh`.
- Each line produced one `signal:` wake in the real parent watcher, which is the event that starts the parent XO's turn and therefore the captain-facing report.
