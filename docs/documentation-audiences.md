...
`bin/xo-doc-audience-check.sh` validates exact inventory coverage, README setup routing, required owner pointers, and local link targets.
...
- `agent-runtime` is loaded or rendered as an operating contract for XO agents rather than read as product documentation.

The knowledge-placement policy is owned by [`xo-coding-guidelines`](../skills/playbooks/xo-coding-guidelines/SKILL.md).
