...
Remote second mates place a whole persistent XO home on another SSH-reachable host.
The primary still owns routing and supervision, while the remote home owns its own projects, backlog, and workers.
XO does not support placing an individual worker remotely or failing a remote route over to a local replacement.

The remote second-mate agent itself always runs on the [Herdr backend](herdr-backend.md) in the shared `xo-remote` session, and every path that provisions or launches one refuses a host that is not ready for it.
`xo-remote` is reserved for remote fleet work and must not be used for personal work.
...
Do not enable agent forwarding for XO.
`xo-on.sh` also disables agent forwarding, forwarding setup, and configured `SendEnv` patterns on every call, and arms bounded SSH dead-peer detection so a vanished host (a reboot, a dropped link) fails within a bounded window instead of hanging indefinitely; its [script header](../bin/xo-on.sh) owns the keepalive defaults and environment overrides.

Clone XO on the remote host at an absolute code-root path.
Expose that clone's fixed entrypoint on the account's non-interactive SSH `PATH`, for example:

```sh
mkdir -p ~/.local/bin
ln -s /absolute/path/to/xo/bin/xo-remote-entrypoint.sh ~/.local/bin/xo-remote-entrypoint.sh
```

The entrypoint accepts encoded argv for genuine executable `bin/xo-*.sh` files only.
...
After setup, every other command verifies XO's account-owned remote job worker, stages the encoded argv and stdin bytes, waits for its result, and relays stdout, stderr, and the exit status separately.
On macOS the worker is `dev.xo.remote-job`, an Aqua-scoped LaunchAgent at `~/Library/LaunchAgents/dev.xo.remote-job.plist` with logs under `~/Library/Logs/`.
After that bootstrap every non-doctor `xo-on.sh` target runs through that worker in the remote account's GUI session, never in the SSH process or a Herdr pane.
The worker serves one lane per staged home: jobs for the same home follow the staging-order contract owned by [`bin/xo-remote-job-lib.sh`](../bin/xo-remote-job-lib.sh), while different homes' lanes run concurrently so one home's long job never delays another home's commands.
...
`bin/xo-remote-job-lib.sh` owns that preemption contract and distinguishes preemption from a wait window that closes with no data, so only a genuinely quiet window proves channel freshness while either outcome can re-arm without losing data.
...
A worker stops itself once its configured code root stops being an XO checkout, so a worker started from a worktree cannot outlive that worktree, and `bin/xo-remote-job-reap-orphans.sh` clears any worker already left behind that way without ever touching one whose checkout still exists.
...
`bin/xo-remote-job-lib.sh` is the single owner of the worker `PATH` and builds it by filesystem discovery rather than by evaluating shell startup files.
...
Because `~/.local/bin` precedes the package-manager directories, a stale self-updated `herdr` there shadows the one the account's login shell may resolve; the Herdr adapter steps around a client the running server refuses and `xo-remote-doctor.sh` names which client it selected ([`herdr-backend.md`](herdr-backend.md#client-selection)).
...
When a required tool remains discoverable only through one of those managers, `xo-remote-doctor.sh --fix` may create an XO-owned wrapper in `~/.local/bin` that executes its selected absolute target.
...
`bin/xo-remote-doctor.sh` is the single owner of what "ready for a remote second mate" means.
Check any host against it directly:

```sh
bin/xo-on.sh <secondmate-id|ssh-alias> xo-remote-doctor.sh
```
...
```sh
bin/xo-on.sh <secondmate-id|ssh-alias> xo-remote-doctor.sh --fix
```

Over the plain SSH doctor bootstrap, it writes and reloads the XO-owned `dev.xo.remote-job` and `dev.xo.herdr.xo-remote` launch agents on macOS, both scoped with `LimitLoadToSessionType=Aqua` and bootstrapped in `gui/<uid>`.
The Herdr agent runs [`bin/xo-remote-herdr-guard.sh`](../bin/xo-remote-herdr-guard.sh) through a shell in login mode with separate `-l` and `-c` arguments, resolving the remote account's executable labeled Directory Services `UserShell`, then an executable `$SHELL`, and finally `/bin/sh`, so the server inherits the account's own environment.
...
Herdr's own SSH remote attach starts such a server when it finds none, and at boot it wins the `xo-remote` socket because sshd accepts connections before the login session exists, so the guard is what makes the launch agent converge: it execs the server in the foreground under launchd when nothing owns the socket, exits 0 when an Aqua-born server already does, and otherwise stops the foreign server and takes the session over, closing its panes so the parent XO relaunches its mates into the Aqua-born server.
`KeepAlive={SuccessfulExit=false}` lets that exit 0 rest instead of respawning against a held socket; the guard's header owns the decision table and [`bin/xo-remote-herdr-owner-lib.sh`](../bin/xo-remote-herdr-owner-lib.sh) owns the birth markers it reads.
It starts the same workers directly on Linux, recreates the `~/.local/bin/xo-remote-entrypoint.sh` symlink when it is absent, and creates only XO-owned required-tool wrappers that it can prove resolve to a version-manager target, stopping after one harness satisfies the at-least-one requirement.
It never installs packages or overwrites a non-XO file at a reserved wrapper path.
The dedicated Herdr launch agent owns only the remote-secondmate `xo-remote` server and does not inspect, rewrite, start, stop, or require the user's interactive `default` session or its `dev.xo.herdr` launch agent.
...
XO never writes an auto-login password, never changes FileVault, and never stores an account password.
A file at `~/.local/bin/xo-remote-entrypoint.sh` that is not XO's own symlink is reported for the operator to inspect and is never overwritten.
...
```sh
bin/xo-remote-home-seed.sh <id> <ssh-alias> <remote-root> <remote-home> {<project>[=<origin-url>]...|--no-projects}
```

`<remote-root>` is the remote XO code clone that supplies tracked scripts.
...
[`bin/xo-project-origin-lib.sh`](../bin/xo-project-origin-lib.sh) owns which URLs are accepted; it decides on structure and safety alone, so no forge, domain, or host is privileged and a self-hosted server works exactly as a hosted one does.
...
The seed records `host:`, `root:`, and `home:` in `data/secondmates.md`, gates the host on readiness, sends a bounded manifest, and lets the remote host clone its own XO home and project origins.
...
Seeding also writes a durable `.xo-secondmate-parent` record next to the home's `.xo-secondmate-home` identity marker, naming this home's route to its parent as `local` or `remote`.
The promised-public-reply subsystem is same-filesystem by construction, so a remote route can never carry a delegated public-reply promise; `bin/xo-teardown.sh`'s cleanup gate reads this record to treat a remote parent as out of scope rather than an unresolved binding.

Local secondmates keep the existing route form and need no migration.
A fleet may contain local and remote routes together.
Use `bin/xo-home-seed.sh validate` to validate either form.
...
```sh
bin/xo-spawn.sh <id> --secondmate
```

The primary resolves the verified secondmate harness and optional model and effort, runs the same readiness gate the seed runs, transfers the inherited-material allowlist, and asks the remote host to launch on Herdr in `xo-remote`.
All remote secondmates on one host share `xo-remote` and retain separate `2ndmate-<id>` workspaces inside it.
...
The Bearings inventory-reconcile hook therefore accepts these markerless routes, revalidates the sampled host at delivery, and refuses a route that changed hosts; [`xo-secondmate-reconcile.sh`](../bin/xo-secondmate-reconcile.sh) owns the exact cooldown, identity, and reporting contract.

Send routed requests normally:

```sh
XO_HOME=<primary-home> bin/xo-send.sh xo-<id> '<request>'
```

The [`xo-send.sh` header](../bin/xo-send.sh) owns the exact delivery-status contract.
...
Every remote transport attempt is bounded by `XO_SEND_REMOTE_BUDGET`; that header owns the setting's default and validation contract.
...
If delivery remains unconfirmed, only the exact `XO_PENDING_REPLY_EXISTING_CORR=<id>` resend command printed by `xo-send` is safe to run later because it preserves the request body and lets the remote enqueue deduplicate onto the same record; a plain rerun mints a different correlation and is not idempotent.
...
`xo-peek.sh` and `xo-crew-state.sh` route remote-secondmate reads to the endpoint's host instead of consulting local worktree or backend state.
...
The remote home's own outcome publishers append there too, through the channel contract in `bin/xo-parent-channel-lib.sh` ([secondmate-parent-channel.md](secondmate-parent-channel.md)).
...
The underlying `xo-on` transport never retries automatically, but `xo-send` retries its correlation-preserving steering-inbox leg exactly once.
...
```sh
bin/xo-backlog-handoff.sh <id> <item-key>...
```
...
The outbox is then copied to the remote handoff scratch directory and `xo-backlog-receive.sh` atomically ingests every destination-absent key under the remote backlog's own lock.
The [`bin/xo-backlog-handoff.sh`](../bin/xo-backlog-handoff.sh) header owns remote outbox release after receipt and stable wake-correlation retry behavior.
...
Locked startup convergence and `bin/xo-config-push.sh` transfer only the declared inherited-material allowlist.
...
Session start and every remote launch converge the persistent remote home on the primary's own default-branch commit rather than on the XO copy that host keeps.
The [`secondmate-provisioning` skill](../skills/playbooks/secondmate-provisioning/SKILL.md) owns the guarded convergence contract, including the distinct `/updatexo` behavior, and [`bin/xo-remote-secondmate-control.sh`](../bin/xo-remote-secondmate-control.sh) owns the commit-import mechanics.
Neither session start nor launch moves the host's own XO copy, and an unsafe or unavailable target is reported and left untouched.
...
It closes only the retiring secondmate's panes or `2ndmate-<id>` workspace in `xo-remote`; it never stops the shared session or removes a sibling secondmate's workspace or panes.
...
```sh
bin/xo-test-run.sh tests/xo-on.test.sh
bin/xo-test-run.sh tests/xo-send-remote-delivery.test.sh
bin/xo-test-run.sh tests/xo-secondmate-reconcile.test.sh
bin/xo-test-run.sh tests/xo-peek-remote.test.sh
bin/xo-test-run.sh tests/xo-crew-state.test.sh
bin/xo-test-run.sh tests/xo-remote-job.test.sh
bin/xo-test-run.sh tests/xo-remote-transport-lanes.test.sh
bin/xo-test-run.sh tests/xo-remote-doctor.test.sh
bin/xo-test-run.sh tests/xo-remote-herdr-guard.test.sh
bin/xo-test-run.sh tests/xo-project-origin.test.sh
bin/xo-test-run.sh tests/xo-secondmate-sync.test.sh
bin/xo-test-run.sh tests/xo-remote-reply.test.sh
bin/xo-test-run.sh tests/xo-remote-backlog-handoff.test.sh
bin/xo-test-run.sh tests/xo-remote-secondmate-lifecycle-e2e.test.sh
bin/xo-test-run.sh tests/xo-remote-secondmate-trace-context.test.sh
```
...
The audit-session facts the guard relies on are recorded with their commands in [runtime backend verification](verification/runtime-backends.md#xo-remote-server-birth-and-login-keychain-access).
