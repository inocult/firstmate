...
Use `/rig` to prepare this home and a repository for missions before the first one is dispatched.
...
`/rig` performs this setup and owns the split between the home half Control writes directly and the project half a worker delivers.
