...
`tests/xo-pr-check-security.test.sh` proves the host-agnostic path through a non-default-host sidecar and verifies that `glab` receives the reconstructed project URL.
...
Its JSON would need a JSON processor, and `jq` is not one of XO's common tools, so the state is read from glab's own field output instead.
Only an exact `merged` wakes XO, so a changed output format produces no wake rather than a false merge.
...
```
$ xo-pr-check.sh e1 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/1
armed: state/e1.check.sh
$ xo-pr-check.sh e2 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
armed: state/e2.check.sh
$ xo-pr-check.sh e3 https://gitlab.example/group/subgroup/project/-/merge_requests/7
...
```
$ PATH="$noglab" xo-pr-poll.sh --validated $(tr '\n' ' ' < state/e1.pr-poll)
$ PATH="$noglab" xo-pr-poll.sh --validated $(tr '\n' ' ' < state/e3.pr-poll)
```
...
```
$ PATH="$noglab" xo-pr-check.sh e5 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/1
...
```
$ PATH="$noglab" xo-pr-check.sh e6 https://github.com/kunchenguid/firstmate/pull/750
...
`bin/xo-pr-merge.sh` now merges a GitLab merge request through the shared recording helper and GitLab's own live pre-merge guards.
Every run below used a throwaway `XO_HOME`, so no live task record was touched, and a `glab` wrapper that refused any `merge` subcommand outright, so no merge could reach the forge even if a check were wrong.
...
```
$ PATH="$noglab" xo-pr-merge.sh e5 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
error: merging a GitLab merge request requires glab on PATH
$ echo $?
1
$ PATH="$nojq" xo-pr-merge.sh e6 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
...
`jq` is not one of XO's common tools, which is why the watch poll reads glab's field output instead.
...
```
$ xo-pr-merge.sh e1 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/1
armed: state/e1.check.sh
...
```
$ xo-pr-merge.sh e2 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
armed: state/e2.check.sh
...
```
$ xo-pr-merge.sh e4 https://gitlab.com/KarotKris/gitlab-merge-watch-fixture/-/merge_requests/2
...
The remaining refusal conditions, and the merge itself, are covered by `tests/xo-pr-merge.test.sh` against fixtures.
...
`bin/xo-pr-check.sh` records `pr_head=` only for GitHub, where `gh` exposes the head commit as a selectable field.
It is optional by design, and the other consumers already treat it that way: `bin/xo-teardown.sh` reads the head from the forge at teardown and falls back to its provider-agnostic content check, and `bin/xo-review-diff.sh` resolves the head from the remote when none is recorded.
