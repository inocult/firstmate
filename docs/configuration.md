...
6. In each secondmate home, rename the private `.fm-secondmate-home` and `.fm-secondmate-parent` markers to `.xo-secondmate-home` and `.xo-secondmate-parent`, because the runtime recognizes a secondmate home by the new marker names; then rewrite the renamed parent record's contents, its `schema=` line to `schema=xo-secondmate-parent.v1` and, when it carries `route=local`, its `parent_home=` line to the renamed primary home path, because `bin/xo-secondmate-parent-lib.sh` parses the record strictly and a secondmate whose record still names the old schema or the moved primary home runs but can no longer report upward; the `.xo-secondmate-home` marker holds only the id and needs no edit.
