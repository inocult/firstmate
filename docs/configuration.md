...
2. Retire every standing process-event source from the old home before anything moves, because a built-in registration stores its adapter's argv verbatim (`xo_procevent_registration_publish_locked`, bin/xo-procevent-lib.sh) and a standing source is not task-keyed, so the empty-fleet precondition does not clear it: run `bin/xo-procevent.sh list` in each home to enumerate what is registered, record what each source was so step 19 can re-arm it, then retire it through its own adapter, `bin/xo-procevent-when.sh retire <name>` for a `when` watch registered as source id `when-<name>`, and `bin/xo-procevent-lavish.sh retire <artifact.html>` for an armed Lavish board. A `when` watch's stored argv names the absolute `<old-home>/bin/fm-procevent-when.sh`, and its private `state/when/when-<name>.spec` and `.trust` records open with the bare tokens `fm-when-spec-v1` and `fm-when-trust-v1` that `spec_load` rejects and that step 9's `schema=` sweep cannot reach, so a source left armed is relaunched by every `xo-procevent.sh reconcile` against a path that no longer exists, never polls its condition, and never fires its action. Two kinds that `list` also shows need no retire: the remote-reply source, which bootstrap re-arms on every pass, and an extension-owned source, which stores `argc=0` and no argv at all because the tracked extension host constructs its command at run time, so step 9's sweep rewriting its `extension_schema=` token is the whole of its migration.
...
19. Re-arm every standing process-event source recorded in step 2, from the renamed home and after the restart, with the same adapter command that armed it: `bin/xo-procevent-when.sh arm <name> [options] --condition <argv>... --action <argv>...` for a `when` watch, and `bin/xo-procevent-lavish.sh arm <artifact.html>` for a Lavish board. Arming republishes the registration against the renamed `bin/xo-*` path and writes a fresh `state/when/` spec and trust record under the current tokens; each source starts on the watcher's next cycle, or immediately with `bin/xo-procevent.sh reconcile`. A `when` watch's deadline restarts from this arming, and a Lavish board must be armed against the artifact path under the renamed home.
