...
3. Repair the treehouse pool slots of every project under the renamed home, because each returned slot's `.git` file holds an absolute `gitdir:` pointer into the old home path and treehouse skips a broken slot and allocates a new one instead of reusing it: run `git worktree repair` with no arguments from each `projects/<project>` checkout under the renamed home, which rewrites every slot's pointer to the new path so the next `treehouse get` reuses it (verified with git 2.55.0 and treehouse v2.3.0).
