...
Migrate with an empty fleet: land and clean up every task first, because live task records hold the old home path and the old hook paths and are not rewritten by this procedure, and retire every treehouse-leased secondmate home, one seeded with `-`, with `bin/xo-teardown.sh <id>` from the old primary before step 1, because treehouse derives its pool from the checkout directory name and the origin URL, so the renamed primary resolves a new, empty pool and a lease left behind in the old pool can never be returned from the renamed checkout; those secondmates are re-seeded into the new pool from their regenerated charters in step 17.
