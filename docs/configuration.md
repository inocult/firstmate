...
Migrate with an empty fleet: land and clean up every task first, because live task records hold the old home path and the old hook paths and are not rewritten by this procedure, and retire every treehouse-leased secondmate home, one seeded with `-`, with `bin/xo-teardown.sh <id>` from the old primary before step 1, because treehouse derives its pool from the checkout directory name and the origin URL, so the renamed primary resolves a new, empty pool and a lease left behind in the old pool can never be returned from the renamed checkout; those secondmates are re-seeded into the new pool from their regenerated charters in step 14.
...
5. In each secondmate home, rename the private `.fm-secondmate-home` and `.fm-secondmate-parent` markers to `.xo-secondmate-home` and `.xo-secondmate-parent`, because the runtime recognizes a secondmate home by the new marker names; then rewrite the renamed parent record's contents, its `schema=` line to `schema=xo-secondmate-parent.v1` and, when it carries `route=local`, its `parent_home=` line to the renamed primary home path, because `bin/xo-secondmate-parent-lib.sh` parses the record strictly and a secondmate whose record still names the old schema or the moved primary home runs but can no longer report upward; the `.xo-secondmate-home` marker holds only the id and needs no edit.
6. Regenerate each secondmate charter with the renamed scaffold before any secondmate is relaunched, because `bin/xo-spawn.sh` reads the persisted `data/charter.md` as the secondmate's instruction source on every launch and a charter written before the rename tells the agent to run `bin/fm-*` scripts and to expect the `[fm-from-firstmate]` marker: in the primary home, move the old `data/<id>/brief.md` aside because the scaffold refuses to overwrite it, run `XO_SECONDMATE_CHARTER='<charter>' XO_SECONDMATE_SCOPE='<scope>' bin/xo-brief.sh <id> --secondmate {<project>...|--no-projects}` with the charter text, the Routing scope text, and the project list the old brief carried, so the regenerated charter matches the `scope:` the registry routes on; then re-seed a local secondmate by copying the regenerated `data/<id>/brief.md` to `data/charter.md` in that secondmate home, because `bin/xo-home-seed.sh` refuses to re-seed a populated home, leave a remote secondmate's regenerated brief in place for step 12, which re-seeds it only after its host is prepared, and leave a retired leased secondmate's regenerated brief in place for step 14.
7. Update the hook paths that name `bin/fm-*`: the merged `.claude/settings.json` already names `bin/xo-*`, so check any user-level hook file such as `~/.claude/settings.json` and any private `.claude/settings.local.json` in the home, and replace each `bin/fm-` path with `bin/xo-` under the renamed home directory.
8. Re-run `no-mistakes init` in the renamed home, because the gate registration is keyed to the repository path and the old registration points at a directory that no longer exists.
9. Rename the GitHub repository with `gh repo rename xo` from the renamed home, so the old `inocult/firstmate` URLs redirect to `inocult/xo`.
10. Update the origin remote with `git remote set-url origin` to the renamed repository URL, in the primary home and in every secondmate home that clones from it.
11. Prepare every remote host before any remote command runs, over an interactive SSH session rather than `bin/xo-on.sh`, because that entrypoint invokes `xo-remote-entrypoint.sh` by bare name and a host provisioned before the rename exposes only `fm-remote-entrypoint.sh`, so every routed command, including the doctor whose `--fix` would otherwise recreate the symlink, fails with command not found until the symlink exists: rename the code-root clone to the path the updated `root:` names, advance it and the remote home clone to the rename commit with `git remote set-url origin` and a fast-forward pull, because re-seeding refreshes an existing home's owned files but never advances its checkout, and replace the entrypoint with `rm ~/.local/bin/fm-remote-entrypoint.sh && ln -s <remote-root>/bin/xo-remote-entrypoint.sh ~/.local/bin/xo-remote-entrypoint.sh`; on a macOS host also boot out and delete the old-label launch agents and move the old remote job state root now, because the readiness gate in step 12 runs `xo-remote-doctor.sh --fix`, which boots out and rewrites only the new labels `dev.xo.herdr.xo-remote` and `dev.xo.remote-job`, so the old KeepAlive agents would otherwise stay loaded, keep relaunching against the moved paths, and run a duplicate worker that `xo-remote-herdr-owner-lib.sh` cannot detect; run `launchctl bootout gui/$(id -u)/dev.firstmate.herdr.fm-remote` and `launchctl bootout gui/$(id -u)/dev.firstmate.remote-job`, delete `~/Library/LaunchAgents/dev.firstmate.herdr.fm-remote.plist` and `~/Library/LaunchAgents/dev.firstmate.remote-job.plist`, then `mkdir -p ~/.xo && mv ~/.firstmate/remote-job ~/.xo/remote-job` or remove it when no queued job matters.
12. Re-seed each remote secondmate from the primary home by re-running `bin/xo-remote-home-seed.sh <id> <ssh-alias> <remote-root> <remote-home> {<project>[=<origin-url>]...|--no-projects}` with the same host, root, and home its updated record names, never by copying the brief verbatim, because that seed rewrites the charter's parent channel to the remote home's `state/parent-replies.status`, which the remote reply relay reads, and refreshes the owned files, including the parent record, of an existing home marked for that id; expect its readiness gate to find the new-label agents absent and bootstrap `dev.xo.remote-job` and `dev.xo.herdr.xo-remote` through `xo-remote-doctor.sh --fix`, which is the one time the new labels are loaded, so no separate doctor call is needed afterwards.
13. Restart the session and confirm that the session-start digest runs from `bin/xo-session-start.sh` and reports no missing tool or hook, then run the read-only `bin/xo-on.sh <secondmate-id> xo-remote-doctor.sh` for each remote host and confirm it reports no remaining gap.
14. Re-seed each retired leased secondmate from the renamed primary home with `bin/xo-home-seed.sh <id> - {<project>...|--no-projects}`, which takes a fresh lease in the pool the renamed checkout derives and copies the regenerated charter from step 6 into the new home; then remove the old pool directory under `~/.treehouse` once nothing references it.
