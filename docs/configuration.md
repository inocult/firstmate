...
Migrate with an empty fleet: land and clean up every task first, because live task records hold the old home path and the old hook paths and are not rewritten by this procedure, and retire every treehouse-leased secondmate home, one seeded with `-`, with `bin/xo-teardown.sh <id>` from the old primary before step 1, because treehouse derives its pool from the checkout directory name and the origin URL, so the renamed primary resolves a new, empty pool and a lease left behind in the old pool can never be returned from the renamed checkout; those secondmates are re-seeded into the new pool from their regenerated charters in step 16.
...
15. Restart the session and confirm that the session-start digest runs from `bin/xo-session-start.sh` and reports no missing tool or hook, then run the read-only `bin/xo-on.sh <secondmate-id> xo-remote-doctor.sh` for each remote host and confirm it reports no remaining gap.
16. Re-seed each retired leased secondmate from the renamed primary home with `bin/xo-home-seed.sh <id> - {<project>...|--no-projects}`, which takes a fresh lease in the pool the renamed checkout derives and copies the regenerated charter from step 8 into the new home; then remove the old pool directory under `~/.treehouse` once nothing references it, and run `git worktree prune` in the renamed primary checkout afterwards so its dangling worktree records for the removed pool are dropped.
