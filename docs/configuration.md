...
The category expresses stability: `core` holds the operating contracts `AGENTS.md` loads at its named triggers and every home relies on, `missions` holds the Plane ticket tier (`prep`, `operation`, `breach`, `overwatch`, and `plane-missions`), `experimental` holds backend- or integration-specific skills, and `deprecated` holds redirect stubs and aliases kept for in-flight briefs, while `public` holds the portable, installer-facing skills that reference no private path of this home.
...
Audience follows location rather than hand-maintained copies: a skill name exists exactly once across the tree, because installers such as skills.sh resolve `--skill <name>` by frontmatter name across every category and would otherwise install whichever same-named variant they walk first, so a portable skill and one that assumes this home's private paths are two audience variants under distinct names, such as the public `stow-notes` beside the internal `stow`, the activation links choose which skills this home loads, and `metadata.internal: true` in a skill's frontmatter still marks the ones that assume a live firstmate home for installers.
...
The setting is inherited into secondmate homes under the primary-authoritative contract owned by [`secondmate-provisioning`](../skills/core/secondmate-provisioning/SKILL.md).
...
`config/backend` is inherited into secondmate homes under the primary-authoritative contract owned by [`secondmate-provisioning`](../skills/core/secondmate-provisioning/SKILL.md).
...
The [`firstmate-coding-guidelines` skill](../skills/core/firstmate-coding-guidelines/SKILL.md#no-mistakes-test-configuration) owns why `commands.test` stays absent and targeted validation belongs to the evidence path.
...
Before changing it, inspect the current file and curate the matching bullet in place under the internal [`stow` skill's](../skills/core/stow/SKILL.md) tiering and archive contract; add a new bullet only for a genuinely new durable preference.
...
The file is created lazily on first learning and follows the internal [`stow` skill's](../skills/core/stow/SKILL.md) aging-tier and cold-archive contract: inspect the current file first and curate it instead of appending forever.
...
A secondmate does not create an independent default and instead receives the primary value through the inherited-local-material contract in [`secondmate-provisioning`](../skills/core/secondmate-provisioning/SKILL.md).
...
The internal [`/stow` skill](../skills/core/stow/SKILL.md) owns curation and its automatic secondmate cascade, which accounts every home against this same per-home allowance separately rather than against a fleet total.
The helper's header owns exact parsing, publication, and report output mechanics.

## Stow pass horizon (config/stow-pass-horizon)

`config/stow-pass-horizon` is an optional local, gitignored presence flag that opts this home in to the pass-count decay horizon in the internal [`/stow` skill](../skills/core/stow/SKILL.md).
...
The concise single-line route contract is owned by the [`secondmate-provisioning` skill](../skills/core/secondmate-provisioning/SKILL.md#routing-table), including the parser-compatible fields, one-sentence summary requirement, `home:` pointer to the seeded charter, and limit on extra registry prose.
...
gemini is likewise refused for secondmates because it has no primary supervision protocol; [its adapter reference](../skills/core/harness-adapters/references/harness/gemini.md) owns the credential precondition, canonical-launch wiring, and raw-launch limitations.
...
The verified adapter evidence - each harness's busy-state source, interrupt and exit behavior, skill-invocation syntax, and per-harness quirks - lives in the skill tree rooted at [`.agents/skills/harness-adapters/SKILL.md`](../skills/core/harness-adapters/SKILL.md).
...
The inherited-local-material contract is owned by [`secondmate-provisioning`](../skills/core/secondmate-provisioning/SKILL.md); its harness-relevant consequence is that a secondmate's own crewmates use the primary's dispatch profiles and static harness value.
...
The file is a captain-wide safety preference, so it is inherited into secondmate homes under the [`secondmate-provisioning`](../skills/core/secondmate-provisioning/SKILL.md) inherited-local-material contract; a secondmate's own Claude crewmates then launch on the same posture.
The [Claude adapter reference](../skills/core/harness-adapters/references/harness/claude.md) records the verified shape of both launches and which once-per-machine dialog each one can meet.
...
The file is inherited into secondmate homes through the [primary-authoritative configuration contract](../skills/core/secondmate-provisioning/SKILL.md).
...
An absent or incompatible `lavish-axi` reports `PRESENTATION_UNAVAILABLE` with its required floor, install command, and explicit text fallback; [`bootstrap-diagnostics`](../skills/core/bootstrap-diagnostics/SKILL.md) owns the response and compatibility check before visual use.
...
When an allowlisted config item changes for an already-running local home, it sends the literal-content reread pointer described in [`secondmate-provisioning`](../skills/core/secondmate-provisioning/SKILL.md); unchanged allowlisted config sends no pointer unless a previous delivery is pending.
