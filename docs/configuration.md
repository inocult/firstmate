...
2. Rename the home directory, for example `mv ~/firstmate ~/xo`, and rename each git-cloned secondmate home, one seeded with an explicit path, the same way; a treehouse-leased home, one seeded with `-`, stays in its pool slot untouched because the slot follows the primary checkout's rename, and only needs `git worktree repair` run from the renamed primary checkout to re-point its `.git` file at the moved main repository, while a git-cloned home needs no worktree repair. Then, before any secondmate is launched or any remote command runs, update every secondmate record in the gitignored `data/secondmates.md` whose directories moved, local and remote alike, so its recorded `home:` path, and `root:` for a remote one, names the renamed location, because `bin/xo-spawn.sh` resolves a local secondmate's home from that field, `bin/xo-on.sh` resolves every remote command through those recorded paths, and the rename does not rewrite that private file.
