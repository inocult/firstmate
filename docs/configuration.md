...
6. Regenerate each secondmate charter with the renamed scaffold before any secondmate is relaunched, because `bin/xo-spawn.sh` reads the persisted `data/charter.md` as the secondmate's instruction source on every launch and a charter written before the rename tells the agent to run `bin/fm-*` scripts and to expect the `[fm-from-firstmate]` marker: in the primary home, move the old `data/<id>/brief.md` aside because the scaffold refuses to overwrite it, run `XO_SECONDMATE_CHARTER='<charter>' bin/xo-brief.sh <id> --secondmate {<project>...|--no-projects}` with the charter text and project list the old brief carried, then re-seed it by copying the regenerated `data/<id>/brief.md` to `data/charter.md` in that secondmate home, over the recorded route for a remote one, because `bin/xo-home-seed.sh` refuses to re-seed a populated home.
