...
The category is derived mechanically from each skill's frontmatter, stability first, in this precedence: a redirect stub or retired alias goes to `deprecated`; a newly added skill or a backend- or integration-specific one goes to `in-progress`; a skill without the `metadata.internal` marker is portable and goes to `fieldcraft`, the skills a worker can install into a project and call there; otherwise `user-invocable: true` goes to `orders`, the slash commands only the commander issues; otherwise `playbooks`, the procedures the XO loads at its triggers; and `misc` is only for a skill that fits none of those, so it is empty today.
