# XO test isolation proof

This record owns concurrent isolation evidence for the portable parallel candidate set and admitted runner families.
`bin/xo-test-isolation-proof.sh` is the authoritative harness and `docs/xo-test-isolation-proof.json` is the portable pool's machine-readable result.
`bin/xo-test-run.sh` owns production lane partitioning and family concurrency admission.

## Verification

- Date: 2026-08-20
- Command: `bin/xo-test-isolation-proof.sh --jobs 4 --json /tmp/xo-isolation-proof.json`
- Result: `XO_ISOLATION_SUMMARY total=24 failed=0 concurrency=4 duration_ms=113278`

| Field | Value |
|---|---|
| `run_id` | `xo-isolation-1787273044622-10250` |
...
|---:|---:|---:|---|
| 45356 | 0 | 2 | `tests/xo-backend-herdr.test.sh` |
| 35415 | 0 | 24 | `tests/xo-x-mode.test.sh` |
| 35095 | 0 | 4 | `tests/xo-captain-hold-lifecycle.test.sh` |
| 27529 | 0 | 1 | `tests/xo-arm-pretool-check.test.sh` |
| 20922 | 0 | 21 | `tests/xo-test-run.test.sh` |
| 17558 | 0 | 8 | `tests/xo-crew-state.test.sh` |
| 16582 | 0 | 5 | `tests/xo-cd-pretool-check.test.sh` |
| 9766 | 0 | 12 | `tests/xo-lint.test.sh` |
| 9562 | 0 | 11 | `tests/xo-herdr-lab.test.sh` |
| 6768 | 0 | 10 | `tests/xo-grok-harness.test.sh` |
| 6290 | 0 | 14 | `tests/xo-pr-merge.test.sh` |
| 5569 | 0 | 6 | `tests/xo-composer-ghost.test.sh` |
| 4563 | 0 | 16 | `tests/xo-send-popup-settle.test.sh` |
| 4021 | 0 | 22 | `tests/xo-tmux-submit-busy.test.sh` |
| 3544 | 0 | 7 | `tests/xo-composer-lib.test.sh` |
| 3025 | 0 | 18 | `tests/xo-send-strict.test.sh` |
| 2753 | 0 | 17 | `tests/xo-send-settle.test.sh` |
| 2166 | 0 | 15 | `tests/xo-review-diff.test.sh` |
| 1315 | 0 | 3 | `tests/xo-brief.test.sh` |
| 975 | 0 | 19 | `tests/xo-spawn-batch.test.sh` |
| 598 | 0 | 13 | `tests/xo-pi-primary-types.test.sh` |
| 513 | 0 | 9 | `tests/xo-ensure-agents-md.test.sh` |
| 331 | 0 | 20 | `tests/xo-supervision-instructions.test.sh` |
| 99 | 0 | 23 | `tests/xo-transition-lib.test.sh` |

## Family concurrency proofs

`bin/xo-test-isolation-proof.sh --pool <family>` runs the same concurrent proof over a whole `bin/xo-test-run.sh` family, for a stateful family that stays serial on CI but can earn bounded local concurrency.
A family is admitted to `list_concurrent_safe_families` in `bin/xo-test-run.sh` only by a passing proof recorded here.

### watcher-wake-lock: admitted

- Date: 2026-08-28
- Command: `bin/xo-test-isolation-proof.sh --pool watcher-wake-lock --jobs 4`
- Archived harness result: two consecutive runs, 18 candidates, 0 failures.

| Run | Summary |
|---|---|
| 1 | `XO_ISOLATION_SUMMARY total=18 failed=0 concurrency=4 duration_ms=394675` |
| 2 | `XO_ISOLATION_SUMMARY total=18 failed=0 concurrency=4 duration_ms=374869` |
...
Admission is also supported by three independent runs of the production scheduler using `bin/xo-test-run.sh --changed --base HEAD`.
...
An earlier attempt on the same host measured three failures (`xo-watch-checkpoint`, `xo-watch-recovery-loop`, `xo-watch-arm`) while six unrelated busy processes were running, at roughly ten runnable processes against fourteen cores.
...
The archived harness runs showed why ordering matters: the candidate sum was 818s and the balanced four-worker target 205s, but alphabetical order finished in 395s because the 193s `xo-watch-triage` started last and ran alone at the tail.
Both `bin/xo-test-run.sh` and the current proof harness therefore order concurrent runs longest-hint-first.

### pure-contract-unit: admitted

- Date: 2026-08-28
- Command: `bin/xo-test-isolation-proof.sh --pool pure-contract-unit --jobs 4`
- Result: two consecutive runs, 32 candidates, 0 failures.

| Run | Summary |
|---|---|
| 1 | `XO_ISOLATION_SUMMARY total=32 failed=0 concurrency=4 duration_ms=161837` |
| 2 | `XO_ISOLATION_SUMMARY total=32 failed=0 concurrency=4 duration_ms=156462` |

The current runner-change selection is owned by [`bin/xo-test-run.sh`](../bin/xo-test-run.sh)'s changed-file map.
Before admission, 14 of the family's scripts fell to the serial tail and the 33-script selection measured 327.3s against a 300s budget: the concurrent group was 19 scripts totalling 273.4s while the tail alone was 215.7s, dominated by `xo-calm-pi-extension` (77.5s), `xo-vendor-auth-probe` (51.0s), and `xo-muse-harness` (39.7s).
...
Because the runner uses `#!/usr/bin/env bash` and invokes each test with `bash` from `PATH`, the stock macOS measurement used `PATH=/bin:$PATH bin/xo-test-run.sh --changed --max-wall-ms 300000` so both resolved to `/bin/bash` 3.2.57.
Two runs selected all 33 scripts, passed the five-minute result check in 153.5s and 166.8s, and reported the same two failures as `main`: `tests/xo-muse-harness.test.sh` and `tests/xo-composer-lib.test.sh`.
With Bash 5.3.9 on `PATH`, three runs of `bin/xo-test-run.sh --changed --max-wall-ms 300000` selected the same 33 scripts, completed with 0 failures, and reported 163.8s, 172.0s, and 166.9s.
...
- Date: 2026-09-03
- Command: `bin/xo-test-isolation-proof.sh --pool pr-forge --jobs 4`
- Result: two consecutive runs, 6 candidates, 0 failures.

| Run | Summary |
|---|---|
| 1 | `XO_ISOLATION_SUMMARY total=6 failed=0 concurrency=4 duration_ms=198594` |
| 2 | `XO_ISOLATION_SUMMARY total=6 failed=0 concurrency=4 duration_ms=186796` |
...
The family's clock is two long scripts that do not contend: `xo-pr-check-security` (198.5s) and `xo-teardown` (194.1s) each own a worker for nearly the whole run, and `xo-pr-merge` (118.5s) plus `xo-x-mode` (79.4s) fill the other two.
`bin/xo-test-isolation-proof.sh`'s own `--list-exclusions` keeps `xo-pr-check-security` and `xo-teardown` out of the mixed PORTABLE pool, where they would share a machine with unrelated lock and forge stress.
...
- Date: 2026-09-03
- Command: `bin/xo-test-isolation-proof.sh --pool secondmate --jobs 4`
- Result: two consecutive runs, 21 candidates, 0 failures.

| Run | Summary |
|---|---|
| 1 | `XO_ISOLATION_SUMMARY total=21 failed=0 concurrency=4 duration_ms=536586` |
| 2 | `XO_ISOLATION_SUMMARY total=21 failed=0 concurrency=4 duration_ms=571247` |

An earlier proof on 2026-09-03 refused this family on `tests/xo-backlog-handoff.test.sh`, failing two runs of three with `Task "pre-move-crash" not found in this backlog`.
...
`xo_fake_crash_injector` in `tests/lib.sh` drops an `xo-crash-inject <pid>` shim that signals the target and returns only once that process is observably gone, and the pre-move fake never delegates the move at all.
...
- Date: 2026-09-03
- Command: `bin/xo-test-isolation-proof.sh --pool session-bootstrap --jobs 4`
- Result: two consecutive runs, 11 candidates, 0 failures.

| Run | Summary |
|---|---|
| 1 | `XO_ISOLATION_SUMMARY total=11 failed=0 concurrency=4 duration_ms=337928` |
| 2 | `XO_ISOLATION_SUMMARY total=11 failed=0 concurrency=4 duration_ms=335204` |

The earlier refusal was `tests/xo-session-start.test.sh` reporting `the digest waited 9s for inactive reconciliation's 8s state read`.
...
Its scan budget was also raised to the maximum, because the previous value left two seconds of margin over the fixed sleep and was measuring the host rather than the deadline that `tests/xo-inactive-reconcile.test.sh` owns.
...
- Date: 2026-09-03
- Command: `bin/xo-test-isolation-proof.sh --pool standalone --jobs 4`
- Result: two consecutive runs, 28 candidates, 0 failures.

| Run | Summary |
|---|---|
| 1 | `XO_ISOLATION_SUMMARY total=28 failed=0 concurrency=4 duration_ms=301792` |
| 2 | `XO_ISOLATION_SUMMARY total=28 failed=0 concurrency=4 duration_ms=250230` |
...
`tests/xo-test-run.test.sh` covers that split behaviorally: two `standalone` members run concurrently while an unmapped basename is refused under `--jobs` and still runs serially.

Two scripts left the residual set rather than joining it.
`tests/xo-backend-herdr-focus-flash-e2e.test.sh` is a real-Herdr lab regression and is now `real-herdr-gated`, which also moves it out of the portable serial lane and into the required Herdr lane; it had been gate-skipping on Linux CI, so that real-Herdr regression was not running anywhere.
...
Each family measured with `bin/xo-test-run.sh --family <name> --jobs <n>` on the same host, back to back, every run reporting 0 failures.
...
Curated `--family`, `--lane`, and `--all` selections remain serial unless the caller explicitly requests an admissible `--jobs` value, as documented by `bin/xo-test-run.sh --help`.
...
The harness cleared ambient `XO_HOME` and `XO_*_OVERRIDE` values for every worker and verified that global Git configuration was unchanged.
A candidate failure fails the aggregate run and requires investigation rather than a retry.

## Re-run

```sh
bin/xo-test-isolation-proof.sh --list
bin/xo-test-isolation-proof.sh --jobs 4 --json /tmp/xo-isolation-proof.json
bin/xo-test-run.sh --check-coverage
```

To re-run a family proof:

```sh
bin/xo-test-isolation-proof.sh --pool watcher-wake-lock --jobs 4
```
