...
When called with `XO_HOME=<this-xo-home>` or when `XO_HOME` is already set to the active XO home, metadata-routed `xo-send.sh` requests to a live `kind=secondmate` use the live-charter-compatible `from-xo` carrier owned by `bin/xo-operational-input.sh`, so the secondmate returns terse answers through status lines and detailed answers through docs plus status pointers instead of replying only in its own chat.
