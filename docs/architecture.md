...
So a delivered ordinary crew task whose last line stays `done: PR ...`, outside the held-for-merge exemption above, bounds repeated alarms from new pane hashes to the `FM_PAUSE_RESURFACE_SECS` cadence for the length of the captain's decision.
...
The procedure and outcome vocabulary are owned by the [`/updatefirstmate` skill](../skills/orders/updatefirstmate/SKILL.md); the relevant script headers own the mechanics.
