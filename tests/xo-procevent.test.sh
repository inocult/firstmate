...
TMP_ROOT=$(xo_test_tmproot xo-procevent-tests)
...
trigger=$1; shift
while [ ! -e "$trigger" ]; do
  [ "$SECONDS" -lt "${XO_TEST_STUB_MAX_BLOCK_SECONDS:-120}" ] || exit 75
  sleep 0.05
done
...
pe() { XO_HOME="$1" "$ROOT/bin/xo-procevent.sh" "${@:2}"; }
...
hold_source_lock() {  # <source-id> <ready-file> <release-file>
  ...
  XO_HOME="$TMP_ROOT/lock-helper-home" bash -c '
    . "$1/bin/xo-pr-lib.sh"
    . "$1/bin/xo-wake-lib.sh"
    . "$1/bin/xo-procevent-lib.sh"
    xo_procevent_source_lock_acquire "$2" || exit 1
    trap "xo_procevent_source_lock_release \"$2\"" EXIT
    printf "ready\n" > "$3"
    while [ ! -e "$4" ]; do
...
hold_source_lock_then_handle() {  # <home> <source-id> <sequence> <ready-file> <release-file>
  ...
  XO_HOME="$home" bash -c '
    ...
    xo_procevent_mark_handled "$3/state" "$2" "$7"
...
sup=$(PATH="${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/xo-supervision-lib.sh"; xo_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$IDLE/state")
...
sup=$(PATH="${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/xo-supervision-lib.sh"; xo_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$H1/state")
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/src-one.claim" || fail "reconcile never claimed the registered source"
...
mode=$(PATH="${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/xo-pr-lib.sh"; xo_pr_file_mode "$2"' _ "$ROOT" "$RESULT")
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/symlinked-src.claim" \
...
direct_runner=$!
wait_for "$XO_PROCEVENT_CLAIM_ROOT/direct-src.claim" || fail "direct start never claimed its source"
direct_leader=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/direct-src.claim")
...
XO_HOME="$HPG" perl "$SHARED_LAUNCHER" "$SHARED_SIBLING" \
  "$ROOT/bin/xo-procevent.sh" start shared-src > "$TMP_ROOT/shared-start.out" &
shared_launcher=$!
wait_for "$SHARED_SIBLING" || fail "shared caller group never started its unrelated sibling"
wait_for "$XO_PROCEVENT_CLAIM_ROOT/shared-src.claim" || fail "shared-group start never claimed its source"
...
private_mode=$(PATH="${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/xo-pr-lib.sh"; xo_pr_file_mode "$2"' _ "$ROOT" "$HPRIVATE/state/procevent-inbox/private-src.1.handled")
...
cat > "$ADAPTER_ROOT/bin/xo-procevent-endnow.sh" <<'SH'
#!/usr/bin/env bash
...
exit 2
SH
cat > "$ADAPTER_ROOT/bin/xo-procevent-openended.sh" <<'SH'
#!/usr/bin/env bash
# Fixture adapter with no terminal knowledge at all: nothing ever ends it.
exit 2
SH
cat > "$ADAPTER_ROOT/bin/xo-procevent-applying.sh" <<'SH'
#!/usr/bin/env bash
case "${1-}" in
  autohandle)
    printf '%s %s\n' "$2" "$3" >> "$XO_HOME/state/applied"
    "$XO_PROCEVENT_UNDER_TEST" handled "$2" "$3" >/dev/null
    ;;
  *) exit 2 ;;
esac
SH
cat > "$ADAPTER_ROOT/bin/xo-procevent-selfann.sh" <<'SH'
#!/usr/bin/env bash
# Fixture adapter that declares a durable downstream announcement of its own.
# XO_HOME/state/selfann-fail makes its application fail so the fallback
# publication path stays provable.
case "${1-}" in
  self-announcing) exit 0 ;;
  autohandle)
    [ ! -e "$XO_HOME/state/selfann-fail" ] || exit 1
    printf '%s %s\n' "$2" "$3" >> "$XO_HOME/state/applied"
    "$XO_PROCEVENT_UNDER_TEST" handled "$2" "$3" >/dev/null
    ;;
  *) exit 2 ;;
...
SH
chmod +x "$ADAPTER_ROOT/bin/xo-procevent-endnow.sh" "$ADAPTER_ROOT/bin/xo-procevent-openended.sh" \
  "$ADAPTER_ROOT/bin/xo-procevent-applying.sh" "$ADAPTER_ROOT/bin/xo-procevent-selfann.sh"

pe_adapter() {  # <home> <command>...: run the runner against the fixture adapters
  local home=$1
  shift
  XO_ROOT_OVERRIDE="$ADAPTER_ROOT" XO_PROCEVENT_UNDER_TEST="$ROOT/bin/xo-procevent.sh" \
    XO_HOME="$home" "$ROOT/bin/xo-procevent.sh" "$@"
}

HPUBLISH="$TMP_ROOT/hpublish"; new_home "$HPUBLISH"
xo_test_track_procevent_home "$HPUBLISH"
...
XO_HOME="$HPUBLISH" XO_PROCEVENT_UNDER_TEST="$ROOT/bin/xo-procevent.sh" \
  "$ADAPTER_ROOT/bin/xo-procevent-applying.sh" autohandle publish-src 1 \
...
xo_test_track_procevent_home "$HSELF"
...
xo_test_track_procevent_home "$HTERM"
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/ends-src.claim" "terminal retirement releases this runner's own claim"
...
xo_test_track_procevent_home "$HOPEN"
...
xo_test_track_procevent_home "$HREPLACE"
...
xo_test_track_procevent_home "$HRETFAIL"
FAIL_RM_BIN=$(xo_fakebin "$TMP_ROOT/retire-fail-bin")
...
assert_present "$XO_PROCEVENT_CLAIM_ROOT/retire-fail-src.claim" \
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/retire-fail-src.claim" \
...
LAVISH_BIN=$(xo_fakebin "$TMP_ROOT/lavish-stub")
...
printf '<h1>review</h1>\n' > "$REVIEW_ART"
lavish_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$REVIEW_ART")
xo_test_track_procevent_home "$HLT"
PATH="$LAVISH_BIN:$PATH" XO_HOME="$HLT" "$ROOT/bin/xo-procevent-lavish.sh" arm "$REVIEW_ART" >/dev/null
for _ in $(seq 1 6); do
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/$lavish_id.claim" "the ended review releases its owned claim"
...
out=$(PATH="$LAVISH_BIN:$PATH" XO_HOME="$HLT" "$ROOT/bin/xo-procevent-lavish.sh" retire "$REVIEW_ART")
...
EMPTY_BIN=$(xo_fakebin "$TMP_ROOT/lavish-empty-stub")
...
printf '<h1>quiet</h1>\n' > "$QUIET_ART"
quiet_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$QUIET_ART")
xo_test_track_procevent_home "$HEMPTY"
PATH="$EMPTY_BIN:$PATH" XO_HOME="$HEMPTY" \
  "$ROOT/bin/xo-procevent-lavish.sh" arm "$QUIET_ART" >/dev/null
...
ANSWER_BIN=$(xo_fakebin "$TMP_ROOT/lavish-answer-stub")
...
answer_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$ANSWER_ART")
xo_test_track_procevent_home "$HANSWER"
PATH="$ANSWER_BIN:$PATH" XO_HOME="$HANSWER" \
  "$ROOT/bin/xo-procevent-lavish.sh" arm "$ANSWER_ART" >/dev/null
...
#     code: SERVER_ERROR
# while the board's marks remain available. XO registered raw poll output,
...
LAVISH_SCRIPTED_BIN=$(xo_fakebin "$TMP_ROOT/lavish-scripted-stub")
...
PATH="$LAVISH_SCRIPTED_BIN:$PATH" LAVISH_COUNT="$DEFAULT_RATE_COUNT" LAVISH_SCRIPT=interrupt \
  XO_LAVISH_POLL_RETRY_DELAY='' \
  "$ROOT/bin/xo-procevent-lavish.sh" poll "$DEFAULT_RATE_ART" >/dev/null 2>&1 &
DEFAULT_RATE_PID=$!
...
export XO_LAVISH_POLL_RETRY_DELAY=1
...
printf '<h1>retry</h1>\n' > "$RETRY_ART"
retry_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$RETRY_ART")
xo_test_track_procevent_home "$HRETRY"
LAVISH_COUNT="$TMP_ROOT/retry-count"; LAVISH_SCRIPT="interrupt interrupt feedback"
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HRETRY" \
  "$ROOT/bin/xo-procevent-lavish.sh" arm "$RETRY_ART" >/dev/null
...
exh_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$EXH_ART")
xo_test_track_procevent_home "$HEXH"
LAVISH_COUNT="$TMP_ROOT/exhaust-count"; LAVISH_SCRIPT="interrupt"
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HEXH" \
  "$ROOT/bin/xo-procevent-lavish.sh" arm "$EXH_ART" >/dev/null
...
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HEXH" \
  "$ROOT/bin/xo-procevent-lavish.sh" retire "$EXH_ART" >/dev/null
...
printf '<h1>other</h1>\n' > "$OTHER_ART"
other_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$OTHER_ART")
xo_test_track_procevent_home "$HOTHER"
LAVISH_COUNT="$TMP_ROOT/other-count"; LAVISH_SCRIPT="other-server-error"
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HOTHER" \
  "$ROOT/bin/xo-procevent-lavish.sh" arm "$OTHER_ART" >/dev/null
...
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HOTHER" \
  "$ROOT/bin/xo-procevent-lavish.sh" retire "$OTHER_ART" >/dev/null
pass "only the exact interruption is retried; an unrelated SERVER_ERROR still surfaces"
unset XO_LAVISH_POLL_RETRY_DELAY
...
printf '<h1>near</h1>\n' > "$NEAR_ART"
near_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$NEAR_ART")
xo_test_track_procevent_home "$HNEAR"
LAVISH_COUNT="$TMP_ROOT/near-count"; LAVISH_SCRIPT="near-interrupt feedback"
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HNEAR" XO_LAVISH_POLL_RETRY_DELAY=1 \
  "$ROOT/bin/xo-procevent-lavish.sh" arm "$NEAR_ART" >/dev/null
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HNEAR" pe "$HNEAR" start "$near_id" >/dev/null
[ "$(cat "$LAVISH_COUNT")" = 1 ] \
...
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HNEAR" \
  "$ROOT/bin/xo-procevent-lavish.sh" retire "$NEAR_ART" >/dev/null
...
invalid_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$INVALID_ART")
for invalid_delay in 0 61 invalid; do
  invalid_status=0
  invalid_out=$(PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HINVALID" \
    XO_LAVISH_POLL_RETRY_DELAY="$invalid_delay" \
    "$ROOT/bin/xo-procevent-lavish.sh" arm "$INVALID_ART" 2>&1) || invalid_status=$?
  [ "$invalid_status" -ne 0 ] \
...
PATH="$LAVISH_SCRIPTED_BIN:$PATH" TMPDIR="$QUOTED_TMPDIR" \
  "$ROOT/bin/xo-procevent-lavish.sh" poll "$NEAR_ART" >/dev/null
quoted_staged=("$QUOTED_TMPDIR"/xo-lavish-poll.*)
[ ! -e "${quoted_staged[0]}" ] \
...
printf '<h1>stream</h1>\n' > "$STREAM_ART"
stream_id=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$STREAM_ART")
xo_test_track_procevent_home "$HSTREAM"
LAVISH_COUNT="$TMP_ROOT/stream-count"; LAVISH_SCRIPT="stream"
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HSTREAM" \
  "$ROOT/bin/xo-procevent-lavish.sh" arm "$STREAM_ART" >/dev/null
PATH="$LAVISH_SCRIPTED_BIN:$PATH" TMPDIR="$STREAM_TMPDIR" \
  ...
  XO_PROCEVENT_MAX_OUTPUT_BYTES=100 pe "$HSTREAM" reconcile >/dev/null
wait_for "$LAVISH_STREAM_READY" || fail "streaming poll did not start"
stream_staged=("$STREAM_TMPDIR"/xo-lavish-poll.*)
...
PATH="$LAVISH_SCRIPTED_BIN:$PATH" XO_HOME="$HSTREAM" \
  "$ROOT/bin/xo-procevent-lavish.sh" retire "$STREAM_ART" >/dev/null
...
# Simulate a replacement XO session: reconcile runs cold, as it would on
...
pending=$(bash -c '. "$1/bin/xo-procevent-lib.sh"; xo_procevent_pending "$2"' _ "$ROOT" "$HP/state")
expected=$(printf '%s\n' \
...
deduped=$(XO_HOME="$HP" bash -c '
  . "$1/bin/xo-wake-lib.sh"
  xo_wake_print_deduped "$2/state/.wake-queue" | awk -F "\t" "{print \$5}"
' _ "$ROOT" "$HP")
expected=$(printf '%s\n' \
...
runner_pid=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/shared-src.claim" 2>/dev/null)
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/shared-src.claim" "retire releases the claim"
...
sleep 0.5
orphan_pid=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/orphan-src.claim" 2>/dev/null)
...
CLAIM="$XO_PROCEVENT_CLAIM_ROOT/stale-src.claim"
mkdir -p "$XO_PROCEVENT_CLAIM_ROOT"
...
printf '%s\n%s\ncross-home-token\ncross-home-identity\n%s\n' \
  "$HC_OLD" "999999" "$HC_OLD_STATE/procevent" > "$XO_PROCEVENT_CLAIM_ROOT/cross-home-src.claim"
chmod 0600 "$XO_PROCEVENT_CLAIM_ROOT/cross-home-src.claim"
...
printf '%s\n%s\nold-token\nold-identity\n' "$TMP_ROOT/gone-home" 999999 > "$XO_PROCEVENT_CLAIM_ROOT/race-src.claim"
chmod 0600 "$XO_PROCEVENT_CLAIM_ROOT/race-src.claim"
race_pids=()
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/orphan-src.claim" || fail "leader-crash fixture never claimed its source"
wait_for "$ORPHAN_LOG" || fail "leader-crash fixture source never started"
orphan_leader=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/orphan-src.claim")
...
[ -e "$XO_PROCEVENT_CLAIM_ROOT/orphan-src.claim" ] \
...
orphan_token=$(sed -n '3p' "$XO_PROCEVENT_CLAIM_ROOT/orphan-src.claim")
...
printf '%s\n%s\ndead-token\ndead-identity\n%s\n' "$HG2" 999999 "$HG2/state/procevent" \
  > "$XO_PROCEVENT_CLAIM_ROOT/dead-gen-src.claim"
chmod 0600 "$XO_PROCEVENT_CLAIM_ROOT/dead-gen-src.claim"
...
sr_new=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/state-root-src.claim")
...
sr2_identity=$(bash -c '. "$1/bin/xo-pr-lib.sh"; xo_pr_file_identity "$2"' _ \
...
} > "$XO_PROCEVENT_CLAIM_ROOT/wedged-src.claim"
chmod 0600 "$XO_PROCEVENT_CLAIM_ROOT/wedged-src.claim"
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/wedged-src.claim" "retire left the dead generation owning the source"
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/live-drift-src.claim" \
  || fail "live-drift fixture never claimed its source"
wait_for "$SR3_LOG" || fail "live-drift fixture source never started"
sr3_leader=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/live-drift-src.claim")
...
[ "$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/live-drift-src.claim")" = "$sr3_leader" ] \
...
assert_contains "$sr4_wake" "bin/xo-procevent.sh start reused-group-src" \
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/reused-plain-src.claim" \
...
sr5_claim="$XO_PROCEVENT_CLAIM_ROOT/reused-plain-src.claim"
...
# `bin/xo-watch.sh` discards reconcile's `failed=` count and exit status, so a
...
ep_reconcile() {  # <expected-fragment> <expected-exit-nonzero:0|1> <msg>; sets ep_out
  local rc=0
  ep_out=$(XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS=2 pe "$HEP" reconcile) || rc=$?
...
# episode of a registration (tests/xo-watch-triage.test.sh proves delivery).
...
assert_contains "$ep_wake" "did not prove it took the source's claim within XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS" \
  "the launch-failed wake does not state what confirmation observed: $ep_wake"
assert_contains "$ep_wake" "attached bin/xo-procevent.sh start episode-src to reproduce a refusal" \
...
for _ in $(seq 1 100); do
  [ -e "$XO_PROCEVENT_CLAIM_ROOT/episode-src.claim" ] || break
  sleep 0.1
done
[ ! -e "$XO_PROCEVENT_CLAIM_ROOT/episode-src.claim" ] \
...
# bin/xo-watch.sh names the marker for a surfaced key `.seen-procevent-<hex>`,
...
lk_out=$(XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS=2 pe "$HLK" reconcile) || true
...
uf_rc=0
uf_out=$(XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS=2 pe "$HUF" reconcile) || uf_rc=$?
...
(
  for _ in $(seq 1 600); do
    if first_result "$HFC" aa-fast-src >/dev/null 2>&1 \
      ...
      && [ ! -e "$XO_PROCEVENT_CLAIM_ROOT/aa-fast-src.claim" ]; then
      break
    fi
    sleep 0.05
  done
...
FC_RELEASER=$!
fc_rc=0
fc_out=$(XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS=2 pe "$HFC" reconcile) || fc_rc=$?
...
zp_rc=0
zp_out=$(XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS=08 pe "$HZP" reconcile 2>/dev/null) || zp_rc=$?
...
for iw_value in 5s 0 700; do
  iw_rc=0
  iw_out=$(XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS="$iw_value" pe "$HIW" reconcile 2>&1) || iw_rc=$?
  [ "$iw_rc" -ne 0 ] \
    || fail "reconcile accepted the unusable confirm window '$iw_value': $iw_out"
  assert_contains "$iw_out" "XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS" \
  ...
  [ ! -e "$XO_PROCEVENT_CLAIM_ROOT/invalid-window-src.claim" ] \
...
: > "$UW_REG_FILE"
uw_identity=$(bash -c '. "$1/bin/xo-pr-lib.sh"; xo_pr_file_identity "$2"' _ \
  "$ROOT" "$HUW/state/procevent/untidyable-src.source") \
  || fail "could not read the untidyable fixture registration identity"
UW_CLAIM="$XO_PROCEVENT_CLAIM_ROOT/untidyable-src.claim"
{
  printf '%s\n%s\nuntidyable-token\nuntidyable-identity\n' "$HUW" 999999
  printf '%s\n%s\nactive\n' "$UW_REG_FILE" "$uw_identity"
  printf '%s\n%s\n%s\n%s\n%s\n' "$HUW/state" \
    "$(bash -c '. "$1/bin/xo-pr-lib.sh"; xo_pr_file_device "$2"' _ "$ROOT" "$HUW/state")" \
    "$(bash -c '. "$1/bin/xo-pr-lib.sh"; xo_pr_file_inode "$2"' _ "$ROOT" "$HUW/state")" \
    "$(id -u)" 755
} > "$UW_CLAIM"
chmod 0600 "$UW_CLAIM"
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/torn-src.claim" || fail "torn-read fixture runner did not claim its source"
awk 'NR == 3 { print "replacement-token"; next } { print }' \
  "$XO_PROCEVENT_CLAIM_ROOT/torn-src.claim" > "$TMP_ROOT/torn-next.claim"
...
mv "$TMP_ROOT/torn-next.claim" "$XO_PROCEVENT_CLAIM_ROOT/torn-src.claim"
: > "$TORN_RELEASE"
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/retire-start-src.claim" "retirement cannot leave a late claim"
...
printf '%s\n%s\nreused-token\nnot-the-live-process-identity\n' \
  "$HI" "$innocent_pid" > "$XO_PROCEVENT_CLAIM_ROOT/reused-src.claim"
chmod 0600 "$XO_PROCEVENT_CLAIM_ROOT/reused-src.claim"
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/reused-src.claim" "retirement releases the exact reused-pid claim"
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/identity-src.claim" || fail "identity fixture runner did not claim its source"
identity_pid=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/identity-src.claim")
IDENTITY_FAKEBIN=$(xo_fakebin "$TMP_ROOT/identity-tools")
...
identity_status=0
identity_out=$(PATH="$IDENTITY_FAKEBIN:$PATH" XO_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-proc" \
...
assert_present "$XO_PROCEVENT_CLAIM_ROOT/identity-src.claim" "uncertain retirement preserves claim generation"
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/sweep-one.claim" || fail "home sweep fixture one did not start"
wait_for "$XO_PROCEVENT_CLAIM_ROOT/sweep-two.claim" || fail "home sweep fixture two did not start"
sweep_pid_one=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/sweep-one.claim")
sweep_pid_two=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/sweep-two.claim")
...
assert_present "$XO_PROCEVENT_CLAIM_ROOT/sweep-two.claim" \
...
assert_present "$XO_PROCEVENT_CLAIM_ROOT/sweep-one.claim" "home sweep preflight does not release claims"
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/sweep-one.claim" "home sweep releases the first claim"
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/sweep-two.claim" "home sweep releases a claim with no registration"
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/foreign-src.claim" || fail "foreign-owner fixture did not start"
foreign_pid=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/foreign-src.claim")
...
assert_present "$XO_PROCEVENT_CLAIM_ROOT/foreign-src.claim" "home sweep preserves a foreign-home claim"
[ "$(sed -n '1p' "$XO_PROCEVENT_CLAIM_ROOT/foreign-src.claim")" = "$HN" ] || fail "home sweep changed foreign claim ownership"
...
wait_for "$XO_PROCEVENT_CLAIM_ROOT/sweep-uncertain.claim" || fail "uncertain sweep fixture did not start"
sweep_uncertain_pid=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/sweep-uncertain.claim")
sweep_status=0
sweep_out=$(PATH="$IDENTITY_FAKEBIN:$PATH" XO_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-sweep-proc" \
...
assert_present "$XO_PROCEVENT_CLAIM_ROOT/sweep-uncertain.claim" "uncertain home sweep preserves the claim"
...
sup=$(bash -c '. "$1/bin/xo-supervision-lib.sh"; xo_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$HV/state")
...
XO_PROCEVENT_MAX_OUTPUT_BYTES=100 XO_HOME="$HF" "$ROOT/bin/xo-procevent.sh" start big-src >/dev/null 2>&1
...
XO_PROCEVENT_MAX_OUTPUT_BYTES=100 pe "$HG" reconcile >/dev/null
...
printf '%s\n' "$$" > "$1"
while [ "$SECONDS" -lt "${XO_TEST_STUB_MAX_BLOCK_SECONDS:-120}" ]; do sleep 1; done
SH
  chmod +x "$POST_TERM_SOURCE"
  POST_TERM_BIN=$(xo_fakebin "$HPOST_TERM/tools")
  ...
  XO_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-post-term-proc" \
    XO_PROCEVENT_OWNER_CHECK_SECONDS=5 pe "$HPOST_TERM" reconcile >/dev/null
  wait_for "$POST_TERM_PID" || fail "the post-TERM reuse fixture did not start"
  wait_for "$XO_PROCEVENT_CLAIM_ROOT/post-term-src.claim" \
    || fail "the post-TERM reuse fixture did not claim its source"
  POST_TERM_RUNNER=$(sed -n '2p' "$XO_PROCEVENT_CLAIM_ROOT/post-term-src.claim")
  ...
  XO_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-post-term-proc" pe "$HPOST_TERM" list >/dev/null \
...
SH
  ...
  post_term_out=$(PATH="$POST_TERM_BIN:$PATH" XO_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-post-term-proc" \
  ...
  case "$post_term_case" in
    mismatch|nonleader)
      ...
      assert_present "$XO_PROCEVENT_CLAIM_ROOT/post-term-src.claim" \
    ...
    *)
      if [ ! -s "$POST_TERM_SIGNALS" ]; then
        post_term_evidence "$post_term_case" "$POST_TERM_RUNNER" \
          "$XO_PROCEVENT_CLAIM_ROOT/post-term-src.claim" "$POST_TERM_SIGNALS" \
      ...
      assert_absent "$XO_PROCEVENT_CLAIM_ROOT/post-term-src.claim" \
...
bad_limit_status=0
bad_limit_out=$(XO_PROCEVENT_MAX_OUTPUT_BYTES=invalid pe "$HBAD" start bad-limit 2>&1) || bad_limit_status=$?
...
assert_absent "$XO_PROCEVENT_CLAIM_ROOT/bad-limit.claim" "invalid output bound leaves no source claim"
...
sid=$(XO_HOME="$TMP_ROOT/hg" "$ROOT/bin/xo-procevent-lavish.sh" source-id "$ART")
case "$sid" in lavish-*) : ;; *) fail "adapter source id has an unexpected shape: $sid" ;; esac
sid2=$(XO_HOME="$TMP_ROOT/hg" "$ROOT/bin/xo-procevent-lavish.sh" source-id "$ART")
[ "$sid" = "$sid2" ] || fail "adapter source id is not stable"
ART_ALIAS="$TMP_ROOT/artifact-alias.html"
ln -s "$ART" "$ART_ALIAS"
sid3=$(XO_HOME="$TMP_ROOT/hg" "$ROOT/bin/xo-procevent-lavish.sh" source-id "$ART_ALIAS")
...
newline_artifact_out=$("$ROOT/bin/xo-procevent-lavish.sh" source-id "$ART_NEWLINE" 2>&1) || newline_artifact_status=$?
...
guard_out=$(XO_ROOT_OVERRIDE="$TMP_ROOT/guard-root" XO_HOME="$HS" XO_GUARD_GRACE=1 \
  "$ROOT/bin/xo-guard.sh" 2>&1)
...
CLS="$TMP_ROOT/cls"
printf 'session:\n  file: /a.html\n  status: feedback\nprompts[1]{uid}:\n  p1\n' > "$CLS"
out=$("$ROOT/bin/xo-procevent-lavish.sh" classify "$CLS")
assert_contains "$out" feedback "the adapter reads the indented session status"
printf 'session:\n  file: /a.html\n  status: feedback\nprompts[1]{text}:\n  No active Lavish Editor session; code: NOT_FOUND\n' > "$CLS"
assert_contains "$("$ROOT/bin/xo-procevent-lavish.sh" classify "$CLS")" feedback "prompt text cannot override a valid session status"
printf 'session:\n  file: /a.html\n  status: ended\n' > "$CLS"
assert_contains "$("$ROOT/bin/xo-procevent-lavish.sh" classify "$CLS")" ended "an ended session classifies as ended"
printf 'error: No active Lavish Editor session for this file\ncode: NOT_FOUND\n' > "$CLS"
assert_contains "$("$ROOT/bin/xo-procevent-lavish.sh" classify "$CLS")" missing "an explicit missing session classifies as missing"
printf 'garbage that is not a session block\n' > "$CLS"
assert_contains "$("$ROOT/bin/xo-procevent-lavish.sh" classify "$CLS")" unknown "malformed output classifies as unknown rather than a lifecycle state"
...
printf 'session:\n  file: /a.html\n  status: feedback\n  session_ended: true\n  ended_by: user\n' > "$TRM"
assert_contains "$("$ROOT/bin/xo-procevent-lavish.sh" classify "$TRM")" feedback \
  "a final feedback delivery still classifies as feedback for the handler"
"$ROOT/bin/xo-procevent-lavish.sh" terminal "$TRM" \
...
printf 'session:\n  file: /a.html\n  status: ended\n  ended_by: user\n' > "$TRM"
"$ROOT/bin/xo-procevent-lavish.sh" terminal "$TRM" || fail "an ended session was not reported terminal"
printf 'error: No active Lavish Editor session for this file\ncode: NOT_FOUND\n' > "$TRM"
"$ROOT/bin/xo-procevent-lavish.sh" terminal "$TRM" || fail "a missing session was not reported terminal"
printf 'session:\n  file: /a.html\n  status: waiting\n' > "$TRM"
"$ROOT/bin/xo-procevent-lavish.sh" terminal "$TRM" && fail "a waiting session was reported terminal"
printf 'garbage that is not a session block\n' > "$TRM"
"$ROOT/bin/xo-procevent-lavish.sh" terminal "$TRM" && fail "an unreadable result was reported terminal"
printf 'session:\n  file: /a.html\n  status: feedback\nfeedback[1]{text}:\n  session_ended: true\n' > "$TRM"
"$ROOT/bin/xo-procevent-lavish.sh" terminal "$TRM" \
...
silent_says() {  # <expected: yes|no> <description>
  if "$ROOT/bin/xo-procevent-lavish.sh" silent "$SIL" >/dev/null 2>&1; then
...
read_out() { "$ROOT/bin/xo-procevent-lavish.sh" read "$READ"; }
cat > "$READ" <<'EOF'
session:
  file: /review.html
  status: feedback
...
for adapter in remote-reply when; do
  ! "$ROOT/bin/xo-procevent-$adapter.sh" silent "$SIL" >/dev/null 2>&1 \
...
adapter_help=$("$ROOT/bin/xo-procevent-lavish.sh" --help 2>&1 || true)
...
runner_help=$("$ROOT/bin/xo-procevent.sh" --help 2>&1 || true)
...
#!/usr/bin/env bash
perl -MTime::HiRes=time -e 'printf "%.6f\n", time' >> "$1"
XO_HOME="$2" perl -MPOSIX=setsid -e '
  my @command = @ARGV;
  ...
  exec @command;
' "$3/bin/xo-procevent.sh" reconcile
exit 1
SH
chmod +x "$STORM_SOURCE"

HFLOOR="$TMP_ROOT/launch-floor"; new_home "$HFLOOR"
xo_test_track_procevent_home "$HFLOOR"
pe_register "$HFLOOR" lavish floor-src -- \
  "$STORM_SOURCE" "$TMP_ROOT/launch-times" "$HFLOOR" "$ROOT"
XO_PROCEVENT_OWNER_LEASE_SECONDS=4 XO_PROCEVENT_OWNER_CHECK_SECONDS=1 \
  XO_PROCEVENT_LAUNCH_FLOOR_SECONDS=1 pe "$HFLOOR" reconcile >/dev/null
...
xo_test_track_procevent_home "$HPACE"
PACE_LOG="$TMP_ROOT/registration-pacing.log"
pe_register "$HPACE" lavish pace-src -- "$FAST_SOURCE" "$PACE_LOG" >/dev/null
XO_PROCEVENT_LAUNCH_FLOOR_SECONDS=3600 pe "$HPACE" start pace-src >/dev/null
pe "$HPACE" retire pace-src >/dev/null
pe_register "$HPACE" lavish pace-src -- "$FAST_SOURCE" "$PACE_LOG" >/dev/null
XO_PROCEVENT_LAUNCH_FLOOR_SECONDS=3600 pe "$HPACE" start pace-src > "$TMP_ROOT/replacement-pacing.out" 2>&1 &
PACE_START_PID=$!
...
xo_test_track_procevent_home "$HPACE_RACE"
...
XO_PROCEVENT_LAUNCH_FLOOR_SECONDS=3 pe "$HPACE_RACE" start pace-race-src >/dev/null
XO_PROCEVENT_LAUNCH_FLOOR_SECONDS=3 \
  pe "$HPACE_RACE" start pace-race-src > "$TMP_ROOT/registration-pacing-race.out" 2>&1 &
PACE_RACE_PID=$!
wait_for "$XO_PROCEVENT_CLAIM_ROOT/pace-race-src.claim" \
...
xo_test_track_procevent_home "$HCOMMIT"
...
XO_PROCEVENT_LAUNCH_FLOOR_SECONDS=1 pe "$HCOMMIT" start commit-src >/dev/null \
...
XO_PROCEVENT_LAUNCH_FLOOR_SECONDS=3600 pe "$HROLLBACK" start rollback-src > "$TMP_ROOT/rollback.out" 2>&1 &
ROLLBACK_START_PID=$!
...
xo_test_track_procevent_home "$HRECREATED"
...
XO_PROCEVENT_OWNER_LEASE_SECONDS=30 XO_PROCEVENT_OWNER_CHECK_SECONDS=1 \
...
xo_test_track_procevent_home "$HGUARDFAIL"
pe_register "$HGUARDFAIL" lavish guard-fail-src -- "$FAST_SOURCE" "$TMP_ROOT/unguarded-launches"
guard_fail_status=0
guard_fail_out=$(XO_PROCEVENT_OWNER_LEASE_SECONDS=invalid \
...
xo_test_track_procevent_home "$HATTACHED"
...
XO_PROCEVENT_OWNER_LEASE_SECONDS=1 XO_PROCEVENT_OWNER_CHECK_SECONDS=1 \
...
xo_test_track_procevent_home "$HCLOCK"
CLOCK_TRIGGER="$TMP_ROOT/lease-clock.trigger"
CLOCK_STATE="$TMP_ROOT/lease-clock-state"
CLOCK_BIN=$(xo_fakebin "$TMP_ROOT/lease-clock-bin")
...
PATH="$CLOCK_BIN:$PATH" XO_PROCEVENT_OWNER_LEASE_SECONDS=1 XO_PROCEVENT_OWNER_CHECK_SECONDS=1 \
...
xo_test_track_procevent_home "$HDETACHED"
...
XO_PROCEVENT_OWNER_LEASE_SECONDS=1 XO_PROCEVENT_OWNER_CHECK_SECONDS=1 XO_HOME="$HDETACHED" \
  perl -MPOSIX=setsid -e 'setsid() >= 0 or exit 1; exec @ARGV' \
    "$ROOT/bin/xo-procevent.sh" start detached-attached-src \
...
PATH="$REUSED_GROUP_BIN:$PATH" XO_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-reused-group-proc" \
  XO_PROCEVENT_OWNER_LEASE_SECONDS=30 XO_PROCEVENT_OWNER_CHECK_SECONDS=1 \
...
marker=$1
( while [ "$SECONDS" -lt "${XO_TEST_STUB_MAX_BLOCK_SECONDS:-120}" ]; do
    printf 'tick\n' >> "$marker.ticks"
    sleep 0.1
  done ) &
printf '%s\n' "$!" > "$marker.descendant"
...
#!/usr/bin/env bash
marker=$1
( sleep "${XO_TEST_STUB_MAX_BLOCK_SECONDS:-120}" ) &
printf '%s\n' "$!" > "$marker.descendant"
while [ ! -e "$marker.trigger" ]; do
  [ "$SECONDS" -lt "${XO_TEST_STUB_MAX_BLOCK_SECONDS:-120}" ] || exit 75
  sleep 0.1
...
orphan_pe() {  # <home> <command...>
  local home=$1
  shift
  XO_PROCEVENT_OWNER_LEASE_SECONDS="$PROOF_LEASE_SECONDS" \
    XO_PROCEVENT_OWNER_CHECK_SECONDS="$PROOF_CHECK_SECONDS" \
    XO_HOME="$home" "$ROOT/bin/xo-procevent.sh" "$@"
}
...
xo_test_track_procevent_home "$HORPHAN"
HKEEP="$TMP_ROOT/orphan-live-owner"; new_home "$HKEEP"
xo_test_track_procevent_home "$HKEEP"
...
xo_test_track_procevent_home "$RETRY_HOME"
RETRY_STATE="$TMP_ROOT/stop-retry-state"; mkdir -p "$RETRY_STATE"
RETRY_BIN=$(xo_fakebin "$TMP_ROOT/stop-retry-bin")
...
retry_pe() {  # <command...>
  PATH="$RETRY_BIN:$PATH" STOP_RETRY_STATE="$RETRY_STATE" \
    XO_PROCEVENT_OWNER_LEASE_SECONDS=2 XO_PROCEVENT_OWNER_CHECK_SECONDS=1 \
    XO_HOME="$RETRY_HOME" "$ROOT/bin/xo-procevent.sh" "$@"
}
...
trap '[ -z "${PROOF_RELEASE:-}" ] || touch "$PROOF_RELEASE"; xo_test_cleanup' EXIT
for proof_state in absent zombie; do
  ...
  if [ "$proof_state" = zombie ]; then
    ...
    XO_HOME="$HPROOF" XO_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-proof-proc" \
      perl - "$PROOF_RELEASE" "$ROOT/bin/xo-procevent.sh" _start proof-src >"$HPROOF/start.log" 2>&1 <<'PL' &
my $release = shift @ARGV;
defined(my $pid = fork) or exit 125;
if ($pid == 0) {
  setpgrp(0, 0) or exit 125;
  $ENV{XO_PROCEVENT_RUNNER_GROUP} = $$;
  exec @ARGV;
  exit 125;
}
my $deadline = time + ($ENV{XO_TEST_STUB_MAX_BLOCK_SECONDS} // 120);
while (!-e $release && time < $deadline) { select undef, undef, undef, 0.05; }
waitpid($pid, 0) == $pid or exit 125;
PL
  else
    XO_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-proof-proc" \
...
xo_test_track_procevent_home "$HPGUARD"
...
xo_test_track_procevent_home "$HBOUND"
BOUND_STATE="$TMP_ROOT/guard-bound-state"; mkdir -p "$BOUND_STATE"
BOUND_BIN=$(xo_fakebin "$TMP_ROOT/guard-bound-bin")
  ...
  case \$arg in
    *'int(\$now - \$value)'*)
      ...
      if [ ! -s "\$GUARD_BOUND_STATE/reference" ]; then
        printf '%s\\n' "\$finished" > "\$XO_HOME/state/procevent/.owner-lease" || exit 1
...
bound_pe() {
  PATH="$BOUND_BIN:$PATH" GUARD_BOUND_STATE="$BOUND_STATE" \
    XO_PROCEVENT_OWNER_LEASE_SECONDS="$BOUND_LEASE_SECONDS" \
    XO_PROCEVENT_OWNER_CHECK_SECONDS="$BOUND_CHECK_SECONDS" \
    XO_HOME="$HBOUND" "$ROOT/bin/xo-procevent.sh" "$@"
}
...
INTERVAL_BIN=$(xo_fakebin "$TMP_ROOT/decimal-interval-bin")
...
for interval in 08 010; do
  ...
  PATH="$INTERVAL_BIN:$PATH" INTERVAL_SLEEP_LOG="$HINTERVAL/sleeps" \
    XO_PROCEVENT_OWNER_CHECK_SECONDS="$interval" \
...
xo_test_track_procevent_home "$DEBOUNCE_HOME"
DEBOUNCE_STATE="$TMP_ROOT/lease-debounce-state"; mkdir -p "$DEBOUNCE_STATE"
DEBOUNCE_BIN=$(xo_fakebin "$TMP_ROOT/lease-debounce-bin")
...
debounce_pe() {
  PATH="$DEBOUNCE_BIN:$PATH" LEASE_DEBOUNCE_STATE="$DEBOUNCE_STATE" \
    XO_PROCEVENT_OWNER_LEASE_SECONDS="$DEBOUNCE_LEASE_SECONDS" \
    XO_PROCEVENT_OWNER_CHECK_SECONDS="$DEBOUNCE_CHECK_SECONDS" \
    XO_HOME="$DEBOUNCE_HOME" "$ROOT/bin/xo-procevent.sh" "$@"
}
