...
#
# The failure this suite pins: xo promises a public final reply in an X or
...
PF="$ROOT/bin/xo-public-followup.sh"
EMIT="$ROOT/bin/xo-public-followup-emit.sh"
POLL="$ROOT/bin/xo-x-poll.sh"
TEARDOWN="$ROOT/bin/xo-teardown.sh"
PROMOTE="$ROOT/bin/xo-promote.sh"
SESSION_START="$ROOT/bin/xo-session-start.sh"
TMP_ROOT=$(xo_test_tmproot xo-public-followup)
...
seed_commitment() {
  ...
  jq -n '{type:"pr-merged", project:"xo",
...
seed_repro_commitment() {   # <home> <obligation> <request> <work-home> <work-id>
  ...
  jq -n '{type:"report-ready", project:"xo",
...
test_restart_e2e_delivers_exactly_once() {
  ...
  printf '%s\n' fmdev > "$child/.xo-secondmate-home"
  xo_write_meta "$home/state/fmdev.meta" "kind=secondmate" "home=$child"
  seed_commitment "$home" pf-restart req-restart discord secondmate:fmdev work-code-q1
  xo_write_meta "$child/state/work-code-q1.meta" \
...
test_late_receipt_closes_the_exact_attempt_without_reposting() {
  ...
  xo_write_meta "$home/state/work-late.meta" \
...
test_typed_terminal_clear_only_removes_legacy_link() {
  ...
  out=$(PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" "$ROOT/bin/xo-x-followup.sh" --clear work-clear) \
...
test_outward_delivery_stays_with_the_owning_home() {
  ...
  printf '%s\n' child > "$child/.xo-secondmate-home"
  xo_write_meta "$owner/state/child.meta" "kind=secondmate" "home=$child"
  seed_commitment "$owner" pf-own req-own discord secondmate:child work-child
  xo_write_meta "$child/state/work-child.meta" \
  ...
  PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
...
test_delivery_requires_registration_before_posting() {
  ...
  xo_write_meta "$home/state/work-missing.meta" \
...
test_secondmate_teardown_requires_parent_binding() {
  ...
  printf '%s\n' mate > "$child/.xo-secondmate-home"
  seed_commitment "$parent" pf-teardown req-teardown x secondmate:mate work-child
  xo_write_meta "$parent/state/mate.meta" "kind=secondmate" "home=$child"
  xo_write_meta "$child/state/work-child.meta" \
    "window=xo:xo-work-child" "endpoint_task_id=work-child" \
    "worktree=$child" "project=$child" "kind=ship" "mode=local-only" "spawn_gen=public-followup-fixture"

  PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
    XO_STATE_OVERRIDE="$child/state" XO_DATA_OVERRIDE="$child/data" \
  ...
  XO_HOME="$parent" "$ROOT/bin/xo-home-seed.sh" validate >/dev/null \
...
# The three tests below exercise the durable .xo-secondmate-parent record a real
# bin/xo-home-seed.sh seed now writes next to .xo-secondmate-home (xo-remote-sm-
...
# XO_PUBLIC_FOLLOWUP_PRIMARY_HOME env var: a restart that dropped that prefix made
...
# the real bin/xo-teardown.sh cleanup path against a home real xo-home-seed.sh
# produced, never a hand-crafted marker.

assert_local_secondmate_parent_record() {
  local child=$1 parent=$2
  cmp -s "$child/.xo-secondmate-parent" <(
...
test_local_secondmate_seed_publishes_parent_before_identity() {
  ...
  fakebin=$(xo_fakebin "$TMP_ROOT/seed-publication-fake")
...
case "$destination" in
  */.xo-secondmate-home)
    touch "$XO_TEST_PUBLISH_ENTERED"
    wait_count=0
    while [ ! -f "$XO_TEST_PUBLISH_RELEASE" ]; do
...
SH
  chmod +x "$fakebin/mv"
  PATH="$fakebin:$PATH" XO_HOME="$parent" \
    XO_SECONDMATE_CHARTER='Local publication-order regression charter.' \
    XO_TEST_REAL_MV="$real_mv" XO_TEST_PUBLISH_ENTERED="$entered" \
    XO_TEST_PUBLISH_RELEASE="$release" \
    "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects > "$manifest_out" 2>&1 &
  seed_pid=$!
  wait_count=0
  while [ ! -f "$entered" ]; do
  ...
  assert_absent "$child/.xo-secondmate-home" \
  ...
  assert_present "$child/.xo-secondmate-home" \
...
test_secondmate_teardown_resolves_parent_from_durable_record_when_env_lost() {
  ...
  XO_SECONDMATE_CHARTER='Durable-record regression charter.' \
    XO_HOME="$parent" "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  parent_resolved=$(cd "$parent" && pwd -P)
  make_fake_curl "$child" >/dev/null
  xo_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
  ...
  # No XO_PUBLIC_FOLLOWUP_PRIMARY_HOME at all here: a restart of the secondmate
...
test_secondmate_teardown_durable_record_missing_parent_registration_still_refuses() {
  ...
  XO_SECONDMATE_CHARTER='Durable-record missing-registration regression charter.' \
    XO_HOME="$parent" "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  parent_resolved=$(cd "$parent" && pwd -P)
...
test_secondmate_teardown_durable_record_with_unknown_field_succeeds() {
  ...
  XO_SECONDMATE_CHARTER='Durable-record clean-cleanup regression charter.' \
    XO_HOME="$parent" "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  parent_resolved=$(cd "$parent" && pwd -P)
  ...
  xo_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
  assert_local_secondmate_parent_record "$child" "$parent_resolved"
  printf 'some_future_field=value\n' >> "$child/.xo-secondmate-parent"
  parent_alias="$TMP_ROOT/teardown-durable-clean-parent-alias"
  ln -s "$parent" "$parent_alias"
  xo_write_meta "$parent/state/mate.meta" "kind=secondmate" "home=$child"
  xo_git_init_commit "$child/projects/worktree"
  printf 'manual\n' > "$child/config/backlog-backend"
  xo_write_meta "$child/state/work-clean.meta" \
    "window=xo:xo-work-clean" "endpoint_task_id=work-clean" \
  ...
  rc=0
  out=$(PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
    XO_STATE_OVERRIDE="$child/state" XO_DATA_OVERRIDE="$child/data" \
    XO_CONFIG_OVERRIDE="$child/config" XO_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent_alias" \
...
test_secondmate_teardown_rejects_conflicting_live_and_durable_parent_bindings() {
  ...
  XO_SECONDMATE_CHARTER='Durable-record conflict regression charter.' \
    XO_HOME="$durable_parent" "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  ...
  xo_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
  assert_local_secondmate_parent_record "$child" "$parent_resolved"
  xo_write_meta "$durable_parent/state/mate.meta" "kind=secondmate" "home=$child"
  xo_git_init_commit "$child/projects/worktree"
  printf 'manual\n' > "$child/config/backlog-backend"
  xo_write_meta "$child/state/work-conflict.meta" \
    "window=xo:xo-work-conflict" "endpoint_task_id=work-conflict" \
    "worktree=$child/projects/worktree" "project=$child/projects/worktree" \
    "kind=ship" "mode=local-only" "spawn_gen=public-followup-fixture"

  PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
    XO_STATE_OVERRIDE="$child/state" XO_DATA_OVERRIDE="$child/data" \
    XO_CONFIG_OVERRIDE="$child/config" XO_PUBLIC_FOLLOWUP_PRIMARY_HOME="$live_parent" \
...
test_secondmate_teardown_rejects_unsafe_durable_parent_records() {
  ...
  for case_name in symlink invalid-route duplicate-route remote-parent-home local-parent-host; do
    ...
    XO_SECONDMATE_CHARTER='Unsafe durable-record regression charter.' \
      XO_HOME="$parent" "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null \
    ...
    xo_write_meta "$child/state/work-child.meta" \
      "window=xo:xo-work-child" "endpoint_task_id=work-child" \
      "worktree=$child" "project=$child" "kind=ship" "mode=local-only" "spawn_gen=public-followup-fixture"
    parent_record="$child/.xo-secondmate-parent"
    case "$case_name" in
      symlink)
        mv "$parent_record" "$parent_record.valid"
        ln -s .xo-secondmate-parent.valid "$parent_record"
        ;;
      invalid-route)
        printf 'schema=xo-secondmate-parent.v1\nroute=garbage\n' > "$parent_record"
        ;;
...
test_secondmate_teardown_rejects_nul_bearing_durable_parent_record() {
  ...
  XO_SECONDMATE_CHARTER='Durable-record NUL regression charter.' \
    XO_HOME="$parent" "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null \
    || fail "real secondmate seeding failed"
  child=$(cd "$child" && pwd -P)
  parent_resolved=$(cd "$parent" && pwd -P)
  ...
  record="$child/.xo-secondmate-parent"
  {
    printf 'schema=xo-secondmate-parent.v1\nroute=local\n'
  ...
  PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
    XO_STATE_OVERRIDE="$child/state" XO_DATA_OVERRIDE="$child/data" \
    XO_CONFIG_OVERRIDE="$child/config" \
...
test_relay_disabled_unmarked_teardown_skips_public_path() {
  ...
  xo_git_init_commit "$home/projects/worktree"
...
SH
  ...
  xo_write_meta "$home/state/work-disabled.meta" \
    "window=xo:xo-work-disabled" "endpoint_task_id=work-disabled" \
  ...
  out=$(PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    ...
    XO_CONFIG_OVERRIDE="$home/config" FAKE_TASKS_AXI_LOG="$tasks_log" \
...
test_relay_disabled_parent_allows_marked_child_teardown() {
  ...
  xo_git_init_commit "$child/projects/worktree"
  printf '%s\n' disabled-mate > "$child/.xo-secondmate-home"
  printf -- '- disabled-mate - synthetic (home: %s; scope: synthetic; projects: ; added 2026-07-30)\n' \
    "$child" > "$parent/data/secondmates.md"
  xo_write_meta "$parent/state/disabled-mate.meta" "kind=secondmate" "home=$child"
...
SH
  ...
  xo_write_meta "$child/state/work-disabled.meta" \
    "window=xo:xo-work-disabled" "endpoint_task_id=work-disabled" \
    "worktree=$child/projects/worktree" "project=$child/projects/worktree" \
    "kind=ship" "mode=local-only" "spawn_gen=public-followup-fixture"
  ...
  out=$(PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
    XO_STATE_OVERRIDE="$child/state" XO_DATA_OVERRIDE="$child/data" \
    XO_CONFIG_OVERRIDE="$child/config" \
    XO_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent" FAKE_TASKS_AXI_LOG="$tasks_log" \
    "$TEARDOWN" work-disabled 2>&1) || rc=$?
...
test_secondmate_parent_binding_matches_literal_id() {
  ...
  printf '%s\n' 'mate.id' > "$child/.xo-secondmate-home"
  ...
  xo_write_meta "$parent/state/mate.id.meta" "kind=secondmate" "home=$child"
  xo_write_meta "$child/state/work-literal.meta" \
    "window=xo:xo-work-literal" "endpoint_task_id=work-literal" \
    "worktree=$child" "project=$child" "kind=ship" "mode=local-only" "spawn_gen=public-followup-fixture"

  PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
    XO_STATE_OVERRIDE="$child/state" XO_DATA_OVERRIDE="$child/data" \
    XO_CONFIG_OVERRIDE="$child/config" XO_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent" \
...
test_cleanup_refuses_while_a_public_reply_is_owed() {
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" ship-task \
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" ship-task >/dev/null 2>&1 || rc=$?
...
SH
  ...
  for cmd in "consume" "pending" "guard-work main any-task" "retire anything"; do
    ...
    out=$(PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
      XO_STATE_OVERRIDE="$home/state" FAKE_TASKS_AXI_LOG="$tasks_log" "$PF" $cmd 2>&1) || rc=$?
  ...
  # shellcheck disable=SC1091
  . "$ROOT/bin/xo-public-followup-lib.sh"
  xo_pf_has_open_loops "$home/state" \
    && fail "a relay-disabled home must not grow an open-loop registry"
  xo_pf_has_delivered_open_loops "$home/state" \
    && fail "a relay-disabled home must not grow a delivered open-loop registry"

  rc=0
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" FAKE_TASKS_AXI_LOG="$tasks_log" "$PF" active || rc=$?
...
test_relay_poll_stays_inert_and_surfaces_once() {
  ...
  out=$(PATH="$off/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$off" \
    XO_STATE_OVERRIDE="$off/state" "$POLL" 2>&1)
  ...
  out=$(PATH="$on/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$on" \
    XO_STATE_OVERRIDE="$on/state" "$POLL" 2>&1)
  ...
  first=$(PATH="$on/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$on" \
    XO_STATE_OVERRIDE="$on/state" "$POLL" 2>&1)
  ...
  second=$(PATH="$on/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$on" \
    XO_STATE_OVERRIDE="$on/state" "$POLL" 2>&1)
  assert_not_contains "$second" "public-followup" \
    "an unchanged pending set must not wake xo again every cycle"
...
test_session_start_surfaces_only_when_owed() {
  ...
  out=$(PATH="$off/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$off" \
    XO_STATE_OVERRIDE="$off/state" XO_DATA_OVERRIDE="$off/data" \
    XO_CONFIG_OVERRIDE="$off/config" "$SESSION_START" 2>&1)
  ...
  out=$(PATH="$on/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$on" \
    XO_STATE_OVERRIDE="$on/state" XO_DATA_OVERRIDE="$on/data" \
    XO_CONFIG_OVERRIDE="$on/config" "$SESSION_START" 2>&1)
...
test_dropped_baton_now_surfaces_open_loop() {
  ...
  XO_SECONDMATE_CHARTER='Baton repro charter.' XO_HOME="$parent" \
    "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null || fail "seed failed"
  child=$(cd "$child" && pwd -P)
  make_fake_curl "$child" >/dev/null
  xo_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
  ...
  xo_write_meta "$child/state/pi-rearm-loop-fix-r1.meta" \
    "window=xo:xo-pi-rearm-loop-fix-r1" "endpoint_task_id=pi-rearm-loop-fix-r1" \
    "worktree=$child" "project=$child" "kind=ship" "mode=local-only" "spawn_gen=public-followup-fixture"

  PATH="$parent/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$parent" \
    XO_STATE_OVERRIDE="$parent/state" "$PF" guard-work secondmate:mate pi-rearm-loop-fix-r1 \
  ...
  PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
    XO_STATE_OVERRIDE="$child/state" XO_DATA_OVERRIDE="$child/data" FAKE_CURL_LOG="$log" \
...
test_control_registered_followon_is_guarded() {
  ...
  XO_SECONDMATE_CHARTER='Baton control charter.' XO_HOME="$parent" \
    "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null || fail "seed failed"
  child=$(cd "$child" && pwd -P)
  make_fake_curl "$child" >/dev/null
  xo_fake_exit0 "$child/fakebin" tmux treehouse no-mistakes gh gh-axi
...
SH
  ...
  command=$(printf '%s\n' "$out" | awk '
    index($0, "/bin/xo-public-followup-emit.sh") { capture=1 }
  ...
  command=${command/"$ROOT/bin/xo-public-followup-emit.sh"/"$parent/fakebin/record-emit"}
...
test_retire_after_secondmate_home_removal() {
  ...
  mkdir -p "$child/state"
  printf 'mate\n' > "$child/.xo-secondmate-home"
  xo_write_meta "$home/state/mate.meta" "kind=secondmate" "home=$child"
  log="$home/curl.log"; : > "$log"
...
test_retire_refuses_unbound_existing_secondmate() {
  ...
  xo_write_meta "$child/state/scout-unbound.meta" "status=working" "x_request=req-unbound-mate"
...
test_retire_refuses_reassigned_secondmate_home() {
  ...
  printf 'mate\n' > "$original/.xo-secondmate-home"
  xo_write_meta "$home/state/mate.meta" "kind=secondmate" "home=$original"
  log="$home/curl.log"; : > "$log"
  ...
  printf 'mate\n' > "$replacement/.xo-secondmate-home"
  xo_write_meta "$replacement/state/scout-reassigned.meta" \
    "status=working" "x_request=req-unrelated-replacement"
  xo_write_meta "$home/state/mate.meta" "kind=secondmate" "home=$replacement"
...
test_rechain_refuses_unclaimed_existing_destination() {
  ...
  jq -n '{type:"pr-merged", project:"xo", required_deliverables:["pr_url"],
...
test_pending_skips_concurrent_retirement() {
  ...
  XO_RACE_HOME="$home" XO_RACE_ROOT="$ROOT" bash -c '
    . "$XO_RACE_ROOT/bin/xo-public-followup-lib.sh"
    xo_pf_registry_lock_acquire "$XO_RACE_HOME/state" pf-race || exit 1
    : > "$XO_RACE_HOME/lock-ready"
    while [ ! -e "$XO_RACE_HOME/release-lock" ]; do sleep 0.02; done
    sleep 0.1
    mkdir -p "$XO_RACE_HOME/state/public-followup/retired"
    printf "reason=concurrent close\nretired_at=2026-08-01T00:00:00Z\n" \
      > "$XO_RACE_HOME/state/public-followup/retired/pf-race"
    chmod 600 "$XO_RACE_HOME/state/public-followup/retired/pf-race"
    rm -f "$XO_RACE_HOME/state/public-followup/registry/pf-race"
    xo_pf_registry_lock_release "$XO_RACE_HOME/state" pf-race
  ' &
  locker_pid=$!
...
test_retention_creates_no_false_teardown_refusal() {
  ...
  xo_write_meta "$home/state/ship-retain.meta" \
    "window=xo:xo-ship-retain" \
    "worktree=$home/projects/gone" \
    "project=$home/projects/sample" \
    "harness=codex" \
    "kind=ship" \
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" ship-retain \
...
test_x_request_teardown_warns_when_final_unposted() {
  ...
  xo_write_meta "$home/state/linked-task.meta" \
    "window=xo:xo-linked-task" \
    "worktree=$home/projects/gone" \
    "project=$home/projects/sample" \
    "kind=ship" \
    "mode=local-only" \
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" linked-task \
    > "$home/td.out" 2> "$home/td.err" || rc=$?
...
test_secondmate_promotion_uses_teardown_parent_resolution() {
  ...
  printf '%s\n' mate > "$child/.xo-secondmate-home"
  printf 'schema=xo-secondmate-parent.v1\nroute=local\nparent_home=%s\n' \
    "$stale" > "$child/.xo-secondmate-parent"
  ...
  xo_write_meta "$child/state/promote-conflict.meta" \
    "window=xo:xo-promote-conflict" "kind=scout"
  write_promotion_brief "$child" promote-conflict
  out=$(PATH="$child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$child" \
    XO_STATE_OVERRIDE="$child/state" XO_PUBLIC_FOLLOWUP_PRIMARY_HOME="$parent" \
  ...
  rm -f "$child/.xo-secondmate-parent"
  xo_write_meta "$child/state/promote-legacy.meta" \
    "window=xo:xo-promote-legacy" "kind=scout"
  ...
  assert_contains "$out" "next: XO_HOME=" \
  ...
  out=$(PATH="$remote_child/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$remote_child" \
    XO_STATE_OVERRIDE="$remote_child/state" \
...
#
# The transport is faked at the XO_SSH_BIN process seam and then runs the REAL
...
# The remote root is a real git repo holding the real bin/, because both xo-on.sh
...
remote_fixture_prepare() {
  ...
  cp "$ROOT"/bin/xo-*.sh "$REMOTE_FIXTURE_ROOT/bin/"
  ...
  fakebin=$(xo_fakebin "$TMP_ROOT/remote-transport")
...
# machine. The parent-side task record carries the same route xo-spawn writes.
...
- $id - remote lane (host: remote-mac; root: $REMOTE_FIXTURE_ROOT; home: $remote_home; scope: relay work; projects: xo; added 2026-08-02)
EOF
  xo_write_meta "$home/state/$id.meta" "kind=secondmate" "home=$remote_home" \
...
run_pf_remote() {  # <home> <args...>
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    ...
    XO_REMOTE_JOB_STATE_ROOT="$REMOTE_FIXTURE_JOBS" \
    "$PF" "$@"
}
...
test_remote_secondmate_loop_delivers_and_retires() {
  ...
  xo_write_meta "$remote/state/work-remote.meta" \
...
test_delivered_remote_registration_skips_offline_route() {
  ...
  xo_write_meta "$remote/state/work-delivered.meta" \
  ...
  out=$(XO_FAKE_SSH_MODE=unreachable run_pf_remote "$home" consume) \
...
test_remote_retire_force_semantics_unchanged() {
  ...
  xo_write_meta "$remote/state/work-open.meta" \
    "x_request=req-remote-open" "x_request_ts=1700000000" "x_followups=1"
  xo_write_meta "$remote/state/work-forced.meta" \
...
test_remote_retire_refuses_reassigned_route() {
  ...
  xo_write_meta "$original/state/work-reused.meta" \
  ...
  replacement=$(cd "$replacement" && pwd -P)
  cat > "$home/data/secondmates.md" <<EOF
- mate - replacement lane (host: remote-mac; root: $REMOTE_FIXTURE_ROOT; home: $replacement; scope: relay work; projects: xo; added 2026-08-03)
EOF
  xo_write_meta "$home/state/mate.meta" "kind=secondmate" "home=$replacement" \
    "remote_host=remote-mac" "remote_root=$REMOTE_FIXTURE_ROOT"
  xo_write_meta "$replacement/state/work-reused.meta" \
...
test_remote_retire_accepts_nonwritable_absence() {
  ...
  xo_write_meta "$remote/state/work-no-link.meta" "status=done"
...
# xo-on.sh passes ssh's status through, so 255 is unknown remote completion, not
...
test_remote_unconfirmed_clear_is_unknown_completion() {
  ...
  xo_write_meta "$remote/state/work-unknown.meta" \
    "x_request=req-remote-unknown" "x_request_ts=1700000000" "x_followups=1"

  XO_FAKE_SSH_MODE=unreachable \
...
test_remote_work_home_emit_reaches_owning_home() {
  ...
  case "$command" in
    *"$ROOT/bin/xo-public-followup-emit.sh"*)
...
# than resolved by argument order, and a staging path that is not an xo
...
test_stage_in_refuses_ambiguous_or_unusable_homes() {
  ...
  assert_contains "$EXPECT_OUT" "xo home" \
  ...
  pass "staging requires the matching secondmate xo home"
}
...
test_remote_collection_transport_failure_is_loud() {
  ...
  XO_FAKE_SSH_MODE=unreachable expect_failure \
...
test_remote_brief_rejects_traversal_route_paths() {
  ...
  cat > "$home/data/secondmates.md" <<EOF
- mini-default - remote lane (host: remote-mac; root: $REMOTE_FIXTURE_ROOT/../remote-root; home: $remote; scope: relay work; projects: xo; added 2026-08-02)
EOF
  ...
  cat > "$home/data/secondmates.md" <<EOF
- mini-default - remote lane (host: remote-mac; root: $REMOTE_FIXTURE_ROOT; home: //$remote; scope: relay work; projects: xo; added 2026-08-02)
EOF
...
test_local_work_home_emit_path_is_unchanged() {
  ...
  assert_contains "$command" "$ROOT/bin/xo-public-followup-emit.sh" \
...
# CI's stock macOS Bash lane sets XO_TEST_ONLY to run just the bash-3.2 empty-lock
# register regression. The rest of this file is not a 3.2 snapshot suite.
if [ -n "${XO_TEST_ONLY:-}" ]; then
  "$XO_TEST_ONLY"
  exit 0
fi
