#!/usr/bin/env bash
# tests/xo-afk-inject-herdr-e2e.test.sh - real-herdr end-to-end test for the
# away-mode daemon's herdr transport (bin/xo-supervise-daemon.sh), the herdr
# counterpart of tests/xo-afk-inject-e2e.test.sh's private-socket tmux e2e.
# Mirrors tests/xo-backend-herdr-smoke.test.sh and tests/herdr-test-safety.sh's
...
# at XO_SUPERVISOR_BACKEND=herdr, XO_SUPERVISOR_TARGET="<session>:<pane-id>",
...
# bordered branch of xo_backend_herdr_composer_state, and logs every submitted
# line (hex + text + injection/user classification) - the same technique
# tests/xo-afk-inject-e2e.test.sh uses for its tmux supervisor pane, so this
...
# xo_backend_herdr_send_text_submit's confirmation is native agent-state
...
# --- source the daemon (for afk_enter/afk_exit/XO_INJECT_MARK) + the backend -
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-herdr-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
...
TASK_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "xo-afk-e2e-supervisor" /tmp "$SEEDED_TAB_ID") \
...
for _ in $(seq 1 100); do
  PROCESS_INFO=$(xo_backend_herdr_cli "$SESSION" pane process-info --pane "$PANE_ID" 2>/dev/null || true)
...
# e2e's fake xo-fake-c1 crewmate window - not required by scan_signals (which
...
FAKE_CREW_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "xo-fake-c1" /tmp) \
...
# Mirrors tests/xo-afk-inject-e2e.test.sh's supervisor-loop.sh, but draws the
...
# submission: xo_backend_herdr_send_text_submit's confirmation is now native
...
MARK=$'\xE2\x81\xA3'
LOG="$1"
AGENT_SOURCE=xo-test-supervisor
AGENT_LABEL=xo-test-supervisor
...
submit_line() {
  ...
  # xo_backend_herdr_send_text_submit now confirms against. The 0.6s "working"
  # window comfortably covers the daemon's XO_INJECT_CONFIRM_SLEEP=0.5
...
xo_backend_herdr_send_text_line "$SUPERVISOR_TARGET" "bash '$LOOP_SCRIPT' '$LOG_FILE'" \
...
HERDR_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xo-herdr-shim.XXXXXX")
...
start_daemon() {
  ...
  XO_STATE_OVERRIDE="$STATE_DIR" \
  XO_SUPERVISOR_BACKEND=herdr \
  XO_SUPERVISOR_TARGET="$SUPERVISOR_TARGET" \
  XO_ESCALATE_BATCH_SECS=0 \
  XO_HOUSEKEEPING_TICK=1 \
  XO_POLL=1 \
  XO_SIGNAL_GRACE=1 \
  XO_HEARTBEAT=999999 \
  XO_CHECK_INTERVAL=999999 \
  XO_INJECT_CONFIRM_SLEEP=0.5 \
  XO_INJECT_CONFIRM_RETRIES=6 \
  XO_STALE_ESCALATE_SECS=999999 \
...
# Verify pane_input_pending (dispatched through xo_backend_composer_state for
...
selfcheck_pane_input_pending() {
  ...
  xo_backend_herdr_send_literal "$SUPERVISOR_TARGET" "$check_text" \
  ...
  if PATH="$HERDR_SHIM_DIR:$PATH" pane_input_pending "$SUPERVISOR_TARGET" herdr; then
    xo_backend_herdr_send_key "$SUPERVISOR_TARGET" Enter
    sleep 0.5
    return 0
  fi
  echo "pane_input_pending cannot detect typed text in this real-herdr environment" >&2
  xo_backend_herdr_capture "$SUPERVISOR_TARGET" 10 | sed 's/^/    /' >&2
  xo_backend_herdr_send_key "$SUPERVISOR_TARGET" Enter
...
test_scenario_a() {
  ...
  xo_backend_herdr_send_literal "$SUPERVISOR_TARGET" "human draft text"
  sleep 0.5
...
# must never crash or hot-loop. Exercises xo_backend_composer_state(herdr, ...)
...
test_scenario_d_max_defer() {
  ...
  xo_backend_herdr_send_literal "$SUPERVISOR_TARGET" "stuck-in-the-box"
  sleep 0.5
  ...
  XO_CHECK_INTERVAL=999999 \
  XO_MAX_DEFER_SECS=3 \
  XO_INJECT_CONFIRM_SLEEP=0.3 \
  XO_INJECT_CONFIRM_RETRIES=2 \
  XO_STALE_ESCALATE_SECS=999999 \
  nohup "$DAEMON" >"$STATE_DIR/daemon.out" 2>"$STATE_DIR/daemon.err" &
  DAEMON_PID=$!
  ...
  xo_backend_herdr_send_key "$SUPERVISOR_TARGET" C-c >/dev/null 2>&1 || true
...
xo_backend_herdr_kill "$SUPERVISOR_TARGET" 2>/dev/null || true
xo_backend_herdr_kill "$SESSION:$FAKE_CREW_PANE_ID" 2>/dev/null || true
cleanup_all
trap - EXIT
