...
# bin/xo-herdr-lab.sh against a named non-default lab session. Only the
...
xo_live_gate default-on XO_HERDR_VERSION_FLOOR_LIVE_E2E herdr jq curl shasum
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/xo-herdr-version-floor.XXXXXX")
ORIGINAL_PATH=$PATH
LAB_SESSION=$("$LAB_HELPER" name xo-herdr-version-floor)
cleanup() {
  local status=$?
  rm -rf "$TMP_ROOT"
  exit "$status"
