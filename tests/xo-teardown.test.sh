#!/usr/bin/env bash
# Tests for bin/xo-teardown.sh's landed-work safety and stale-lock recovery.
#
...
#   (o) xo-pr-check rerun after HEAD moved                      -> no stale pr_head
#   (p) xo-pr-check when local HEAD lags                        -> record remote PR head
...
# killed crew process (bin/xo-teardown.sh's teardown_treehouse_return).
...
TMP_ROOT=$(xo_test_tmproot xo-teardown-tests)
...
#   $CASE/state/        - xo state dir (with a fresh watcher beacon)
...
#   $CASE/project/      - clone of origin; acts as the xo project dir
...
exit 0
SH
  # Default hermetic no-mistakes stub: `axi status` answers XO_FAKE_AXI_STATUS
  # verbatim (empty by default, i.e. no active run - the pre-teardown run-abort
  # step is then a no-op), `axi abort` appends one line to
  # XO_FAKE_NM_ABORT_LOG when set, the top-level `runs` listing answers
  # XO_FAKE_NM_RUNS_LIST verbatim (the real `no-mistakes runs --limit N` is
  # plain text with no run id and no quoting - see the ledger fixtures below),
  # and `runs` appends its own invocation to XO_FAKE_NM_RUNS_LOG when set, so
  ...
  # XO_FAKE_AXI_STATUS/XO_FAKE_NM_ABORT_LOG/XO_FAKE_NM_RUNS_LIST before
  # run_teardown.
  ...
  axi)
    ...
    case "${1:-}" in
      status)
        ...
        if [ -n "${XO_FAKE_NM_ABORT_LOG:-}" ] \
           && grep -Fxq "abort --run $run_id" "$XO_FAKE_NM_ABORT_LOG" 2>/dev/null \
           && [ "${XO_FAKE_NM_ABORT_NOOP:-0}" != 1 ]; then
          if [ "${XO_FAKE_NM_NOT_FOUND_AFTER_ABORT:-0}" = 1 ]; then
            printf 'error: "run \\"%s\\" not found"\n' "$run_id" >&2
            exit 1
          elif [ "${XO_FAKE_NM_EMPTY_AFTER_ABORT:-0}" = 1 ]; then
            exit 0
          elif [ -n "${XO_FAKE_AXI_STATUS_AFTER_ABORT:-}" ]; then
            printf '%s\n' "$XO_FAKE_AXI_STATUS_AFTER_ABORT"
          else
            printf 'run:\n  id: "%s"\n  outcome: cancelled\n' "$run_id"
          fi
        else
      ...
      abort)
        shift
        [ -z "${XO_FAKE_NM_ABORT_LOG:-}" ] || printf 'abort %s\n' "$*" >> "$XO_FAKE_NM_ABORT_LOG"
        exit 0 ;;
    esac
    ;;
  runs)
    [ -z "${XO_FAKE_NM_RUNS_LOG:-}" ] || printf 'runs %s\n' "$*" >> "$XO_FAKE_NM_RUNS_LOG"
    printf '%s\n' "${XO_FAKE_NM_RUNS_LIST:-}" ;;
esac
exit 0
SH
  ...
  git -C "$case_dir/project" worktree add -q -b xo/task-x1 "$case_dir/wt" main

  # Fresh watcher beacon so xo-guard stays quiet.
...
write_meta() {
  ...
  xo_write_meta "$case_dir/state/task-x1.meta" \
    "window=xo:xo-task-x1" \
...
add_fork_with_pushed_branch() {
  ...
  # so refs/remotes/fork/xo-task-x1 is visible from the worktree (shared object db).
  git -C "$case_dir/wt" push -q fork xo/task-x1
...
setup_squash_rebased_history() {
  ...
  git clone -q "$case_dir/origin.git" "$tmp"
  git -C "$tmp" checkout -q -b xo/task-x1
  ...
  git -C "$tmp" merge -q --squash xo/task-x1 >/dev/null
  git -C "$tmp" -c user.email=t@t -c user.name=t commit -q -m "feat: squash (#7)"
  git -C "$tmp" push -q origin main
  rm -rf "$tmp"

  git -C "$case_dir/project" fetch -q origin
  git -C "$case_dir/wt" fetch -q origin "refs/pull/7/head:refs/xo-test/pr-head"
  case "$local_mode" in
    rebased)
...
assert_refusal_retained_task_state() {
  ...
  [ "$(git -C "$case_dir/wt" rev-parse --abbrev-ref HEAD 2>/dev/null)" = xo/task-x1 ] \
...
# xo-teardown.sh's own retry-then-stale-cleanup logic (teardown_treehouse_return
# in bin/xo-teardown.sh) rather than hand-simulating that logic in the test.
...
run_teardown() {
  ...
  # XO_DATA_OVERRIDE is pinned to the case dir because teardown closes this
  ...
  XO_STATE_OVERRIDE="$case_dir/state" \
  XO_DATA_OVERRIDE="$case_dir/data" \
  XO_CONFIG_OVERRIDE="$case_dir/config" \
  PATH="$case_dir/fakebin:${XO_TEARDOWN_TEST_PATH:-$PATH}" \
    "$TEARDOWN" task-x1 "$@"
}
...
test_local_only_fork_remote_allows() {
  ...
  # task's records are gone (bin/xo-branch-prompt.sh); that report must be
  # stored, must publish its ready sequence, and must not recreate the index.
  post_seq=$(XO_STATE_OVERRIDE="$case_dir/state" "$ROOT/bin/xo-branch-outcome.sh" append \
...
test_no_mistakes_origin_remote_allows() {
  ...
  git -C "$case_dir/wt" push -q origin xo/task-x1
  git -C "$case_dir/project" fetch -q origin
...
test_no_pr_recorded_discovers_merged_pr_by_branch_allows() {
  ...
  # recorded in meta at all (xo-pr-check.sh was never run, e.g. a yolo merge on
  ...
  # never pushing xo/task-x1 at all, so no refs/remotes/origin/xo/task-x1
...
test_squash_merged_pr_allows_replayed_unpushed_patch() {
  ...
  git -C "$case_dir/wt" push -q origin "$parent_head:refs/heads/xo/task-x1"
  git -C "$case_dir/project" fetch -q origin xo/task-x1
  wt_commit_file "$case_dir" feature.txt hello "add feature"
  append_pr_meta_url "$case_dir"
...
test_pr_check_does_not_refresh_stale_pr_head() {
  ...
  pass "xo-pr-check does not refresh PR head after HEAD moves"
}
...
test_pr_check_records_remote_head_when_local_lags() {
  ...
  pass "xo-pr-check records the remote PR head when the local worktree lags"
}
...
test_content_in_default_fallback_allows() {
  ...
  cat > "$case_dir/fakebin/treehouse" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" > "${XO_TEST_TREEHOUSE_LOG:?}"
exit 0
SH
  chmod +x "$case_dir/fakebin/treehouse"

  set +e
  XO_TEST_TREEHOUSE_LOG="$case_dir/treehouse.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_non_linked_index_lock_path_is_checked_from_worktree() {
  ...
  git -C "$case_dir/wt" checkout -q -b xo/task-x1
  write_meta "$case_dir" no-mistakes ship
...
test_transient_index_lock_clears_after_first_attempt_and_retry_succeeds() {
  ...
  XO_TREEHOUSE_RETURN_LOCK_RETRIES=2 \
  XO_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS=0 \
  XO_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_empty_retry_wait_uses_default_without_aborting() {
  ...
  set +e
  TREEHOUSE_ATTEMPT_FILE="$attempt_file" \
  XO_TREEHOUSE_RETURN_LOCK_RETRIES=1 \
  XO_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS='' \
  XO_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS='' \
  XO_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
test_fractional_legacy_retry_wait_refuses_without_arithmetic_error() {
  ...
  XO_TREEHOUSE_RETURN_LOCK_RETRIES=1 \
  XO_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS='' \
  XO_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS=0.1 \
  XO_STALE_WORKTREE_LOCK_AGE_SECS=3600 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
# xo-pr-check run with XO_HOME="$case_dir/home" so the parent-channel
...
# with the canonical URL on the parent channel from xo-pr-check itself, once;
...
test_secondmate_pr_registration_publishes_ready_line() {
  ...
  XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$case_dir/state" \
    PATH="$case_dir/fakebin:$PATH" "$PR_CHECK" task-x1 "$url" > "$case_dir/pr-check.out" 2> "$case_dir/pr-check.err" \
    || fail "mate-pr-ready: xo-pr-check failed: $(cat "$case_dir/pr-check.err")"
  ...
  XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$case_dir/state" \
    PATH="$case_dir/fakebin:$PATH" "$PR_CHECK" task-x1 "$url" >/dev/null 2> "$case_dir/pr-check.err" \
    || fail "main-pr-ready: xo-pr-check failed"
  ! grep -q '^actionable:' "$case_dir/pr-check.err" \
  ...
  pass "xo-pr-check publishes the PR-ready line on a secondmate's parent channel once"
}
...
test_secondmate_home_teardown_delivers_final_line_or_refuses() {
  ...
  set +e
  XO_HOME="$case_dir/home" run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
  ...
  rmdir "$channel"
  err=$(XO_HOME="$case_dir/home" XO_STATE_OVERRIDE="$case_dir/state" \
    "$ROOT/bin/xo-wake-drain.sh" 2>&1 >/dev/null)
  ...
  [ -z "$seq" ] || XO_HOME="$case_dir/home" XO_STATE_OVERRIDE="$case_dir/state" \
    "$ROOT/bin/xo-wake-drain.sh" --ack-through "$seq" --recovery-generation "$generation" >/dev/null
  set +e
  XO_HOME="$case_dir/home" run_teardown "$case_dir" > "$case_dir/stdout2" 2> "$case_dir/stderr2"
  rc=$?
  set -e
...
test_teardown_missing_busy_sidecar_completes() {
  ...
  write_meta "$case_dir" local-only ship
  gen=$("$ROOT/bin/xo-busy-event.sh" arm "$case_dir/state" task-x1)
...
case "\${1:-} \${2:-}" in
  ...
  "status --json")
    ...
    ;;
  "session list")
    if [ "\${XO_FAKE_HERDR_SESSION_LIST_GARBAGE:-0}" = 1 ]; then
      printf '%s\n' 'not-json'
    else
      printf '%s\n' '{"sessions":[{"name":"default","running":true,"socket_path":"$case_dir/herdr.sock"}]}'
    fi
    ;;
  "pane close")
    : > "\${XO_FAKE_HERDR_CLOSED:?}"
    ;;
  "pane get")
    if [ "\${XO_FAKE_HERDR_PANE_GET_GARBAGE:-0}" = 1 ]; then
      printf '%s\n' 'not-json'
      exit 0
    fi
    if [ -e "\${XO_FAKE_HERDR_CLOSED:?}" ]; then
...
SH
  ...
  lock=$(XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" PATH="$case_dir/fakebin:$PATH" \
    bash -c '. "$0/bin/backends/herdr.sh"; xo_backend_herdr_presentation_session_lock_path default' "$ROOT") \
  ...
  rc=0
  XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" \
  ...
  if [ "$(git -C "$case_dir/wt" rev-parse --abbrev-ref HEAD 2>/dev/null)" != "xo/task-x1" ]; then
  ...
  XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" XO_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
...
test_herdr_flat_teardown_refuses_records_on_unparseable_presence() {
  ...
  : > "$case_dir/state/task-x1.status"
  rc=0
  XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" XO_FAKE_HERDR_PANE_GET_GARBAGE=1 \
    XO_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  [ "$rc" -ne 0 ] \
  ...
  case "$mode" in
    missing-adapter|missing-parser|missing-explicit-close-helper)
      ...
      if [ "$mode" = missing-adapter ]; then
        ...
        sed -i.bak 's/^xo_backend_herdr_explicit_close_pane_confirmed()/xo_backend_herdr_explicit_close_pane_confirmed_unavailable()/' \
          "$case_dir/test-root/bin/backends/herdr.sh"
        rm -f "$case_dir/test-root/bin/backends/herdr.sh.bak"
      else
        sed -i.bak 's/^xo_backend_herdr_parse_target()/xo_backend_herdr_parse_target_unavailable()/' \
          "$case_dir/test-root/bin/backends/herdr.sh"
        rm -f "$case_dir/test-root/bin/backends/herdr.sh.bak"
      fi
      teardown_bin="$case_dir/test-root/bin/xo-teardown.sh"
      ;;
  esac
  rc=0
  XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$case_dir/state" XO_DATA_OVERRIDE="$case_dir/data" \
    XO_CONFIG_OVERRIDE="$case_dir/config" XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" \
    XO_FAKE_HERDR_SESSION_LIST_GARBAGE="$([ "$mode" = unresolvable-lock ] && printf 1 || printf 0)" \
    PATH="$case_dir/fakebin:$PATH" \
...
configure_secondmate_with_herdr_child() {  # <case-dir>
  ...
  printf '%s\n' task-x1 > "$home/.xo-secondmate-home"
  printf '%s\n' "home=$home" >> "$case_dir/state/task-x1.meta"
  xo_write_meta "$home/state/child-herdr.meta" \
...
case "\${1:-} \${2:-}" in
  ...
  "pane get")
    if [ -e "\${XO_FAKE_HERDR_CLOSED:?}" ]; then
      if [ "\${XO_FAKE_HERDR_PRESENCE_UNKNOWN:-0}" = 1 ]; then
        printf '%s\n' 'not-json'
      else
        printf '%s\n' '{"error":{"code":"pane_not_found"}}' >&2
        exit 1
    ...
    ;;
  "pane close") : > "\${XO_FAKE_HERDR_CLOSED:?}" ;;
esac
SH
  chmod +x "$case_dir/fakebin/herdr"
}
...
SH
  ...
  rc=0
  XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" \
    XO_FAKE_HERDR_SESSION_LIST_GARBAGE=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
configure_secondmate_with_tmux_children() {  # <case-dir>
  ...
  for child in child-a child-b; do
    ...
    git -C "$case_dir/project" worktree add -q -b "xo/$child" "$child_wt" main
    xo_write_meta "$home/state/$child.meta" \
      "window=xo:xo-$child" \
...
SH
  ...
  ROOT="$ROOT" LOCK="$lock" READY="$ready" RELEASE="$release" \
    ...
    export XO_STATE_OVERRIDE="$HOME_STATE"
    . "$ROOT/bin/xo-wake-lib.sh"
    xo_lock_try_acquire "$LOCK" || exit 1
    : > "$READY"
...
test_forced_secondmate_herdr_child_retains_records_when_close_unconfirmed() {
  ...
  rc=0
  XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" XO_FAKE_HERDR_PRESENCE_UNKNOWN=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
configure_nested_secondmate_with_herdr_grandchild() {  # <case-dir>
  ...
  printf '%s\n' task-x1 > "$home/.xo-secondmate-home"
  printf '%s\n' nested-sm > "$nested_home/.xo-secondmate-home"
  printf '%s\n' "home=$home" >> "$case_dir/state/task-x1.meta"
  xo_write_meta "$home/state/nested-sm.meta" \
    "window=xo:xo-nested-sm" \
    ...
    "mode=local-only" \
    "home=$nested_home"
  xo_write_meta "$nested_home/state/grandchild-herdr.meta" \
...
case "${1:-} ${2:-}" in
  "workspace list")
    if [ -e "${XO_FAKE_HERDR_RESTORED:?}" ]; then
    ...
    elif [ -e "${XO_FAKE_HERDR_CLOSED:?}" ]; then
      ...
      printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","active_tab_id":"w1:t2","label":"xo/task-x1 · p:AbCdEfGhIjKlMnOpQrStUv","focused":false},{"workspace_id":"w2","active_tab_id":"w2:t2","label":"2ndmate-bravo","focused":true},{"workspace_id":"w3","active_tab_id":"w3:t1","label":"2ndmate-alpha","focused":false}]}}'
    fi
    ;;
  "tab list")
    case "$*" in
  ...
  "session list")
    ...
    ;;
  "pane close")
    if [ "${XO_FAKE_HERDR_CLOSE_FAIL:-0}" = 1 ]; then
      exit 1
    fi
    : > "${XO_FAKE_HERDR_CLOSED:?}"
    ;;
  ...
  "tab get")
    ...
    ;;
  "tab focus")
    if [ "${XO_FAKE_HERDR_RESTORE_FAIL:-0}" = 1 ]; then
      exit 1
    fi
    : > "${XO_FAKE_HERDR_RESTORED:?}"
...
test_herdr_projection_teardown_retires_journal_only_after_confirmed_close() {
  ...
  XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" XO_FAKE_HERDR_RESTORED="$restored" \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" \
...
test_herdr_projection_teardown_retains_journal_when_close_unconfirmed() {
  ...
  local rc=0
  XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_CLOSED="$closed" XO_FAKE_HERDR_RESTORED="$restored" XO_FAKE_HERDR_PRESENCE_UNKNOWN=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  [ "$rc" -ne 0 ] \
...
# path (bin/xo-teardown.sh), never by matching its source text. ------------

# A parked-at-a-gate `axi status` TOON payload for <branch>/<head>, matching
# the shape no-mistakes actually emits (see tests/xo-crew-state.test.sh's
# run_parked fixture, the same shape bin/xo-crew-state.sh's own tests pin).
...
# (the same shape tests/xo-crew-state.test.sh's runs-list fixtures pin).
...
make_unfetched_pipeline_heads() {
  ...
  git -C "$case_dir/pipeline-clone" checkout -q xo/task-x1
...
test_parked_own_run_is_aborted_before_teardown() {
  ...
  local rc=0
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$head")" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
...
# newest xo/task-x1 row is active at the unfetched head, anchored by the
# immediately older xo/task-x1 row ending exactly at this worktree's HEAD.
...
test_parked_run_advanced_past_unfetched_head_is_still_aborted() {
  ...
  rc=0
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$advanced_short")" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/other-task aaaaaaa 2026-09-03 22:10)
$(ledger_row running xo/task-x1 "$advanced_short" 2026-09-03 07:55)
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-09-02 06:36)
$(ledger_row completed xo/third-task bbbbbbb 2026-09-01 11:00)
EOF
)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_parked_run_with_mismatched_ledger_head_is_never_aborted() {
  ...
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$advanced_short")" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/task-x1 deadbee 2026-09-03 07:55)
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-09-02 06:36)
EOF
)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
...
test_parked_run_with_malformed_ledger_row_is_never_aborted() {
  ...
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$advanced_short")" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
running xo/task-x1 $advanced_short
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-09-02 06:36)
EOF
)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
...
test_parked_run_with_impossible_ledger_date_is_never_aborted() {
  ...
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$advanced_short")" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/task-x1 "$advanced_short" 2026-02-31 07:55)
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-02-28 06:36)
EOF
)" \
  XO_FAKE_NM_RUNS_LOG="$case_dir/nm-runs.log" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_terminal_status_with_gate_never_queries_or_aborts_ledger_fallback() {
  ...
  terminal_status=$(parked_axi_status_toon xo/task-x1 "$advanced_short")
  terminal_status=${terminal_status/status: awaiting_approval/status: completed}

  rc=0
  XO_FAKE_AXI_STATUS="$terminal_status" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/task-x1 "$advanced_short" 2026-09-03 07:55)
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-09-02 06:36)
EOF
...
test_parked_run_advanced_head_locally_fetched_is_still_aborted() {
  ...
  # moving any ref, so xo_nm_resolve_commit sees the head again.
  git -C "$case_dir/project" fetch -q "$case_dir/pipeline-clone" xo/task-x1
...
test_parked_advanced_run_ancestor_anchor_is_never_aborted() {
  ...
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/task-x1 "$advanced_short" 2026-09-03 07:55)
$(ledger_row failed xo/task-x1 "$parent_short" 2026-09-02 06:36)
EOF
)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_parked_terminal_unfetched_row_is_never_aborted() {
  ...
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$advanced_short")" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row failed xo/task-x1 "$advanced_short" 2026-09-03 08:20)
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-09-02 06:36)
EOF
)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
...
test_parked_run_terminal_newest_row_at_own_head_is_never_aborted() {
  ...
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/other-task aaaaaaa 2026-09-03 22:10)
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-09-03 06:36)
EOF
)" \
  XO_FAKE_NM_RUNS_LOG="$case_dir/nm-runs.log" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
...
test_parked_run_behind_diverged_newer_row_is_never_aborted() {
  ...
  # A newer xo/task-x1 head that THIS copy resolves but that diverged from
  ...
  git -C "$case_dir/newer-clone" push -q --force origin HEAD:xo/task-x1
  git -C "$case_dir/project" fetch -q origin
  diverged_short=$(git -C "$case_dir/wt" rev-parse --short=7 origin/xo/task-x1)
  ...
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$advanced_short")" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/task-x1 "$diverged_short" 2026-09-03 09:00)
$(ledger_row failed xo/task-x1 "$advanced_short" 2026-09-03 07:55)
EOF
)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_parked_advanced_run_ambiguous_rows_are_never_aborted() {
  ...
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$advanced_short")" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/task-x1 "$advanced_short" 2026-09-03 08:30)
$(ledger_row failed xo/other-task ccccccc 2026-09-03 08:10)
$(ledger_row running xo/task-x1 ddddddd 2026-09-03 07:55)
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-09-02 06:36)
EOF
)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
...
test_ledger_proven_continuation_never_aborts_active_run() {
  ...
  rc=0
  XO_FAKE_AXI_STATUS="$(running_axi_status_toon xo/task-x1 "$advanced_short")" \
  XO_FAKE_NM_RUNS_LIST="$(cat <<EOF
$(ledger_row running xo/task-x1 "$advanced_short" 2026-09-03 07:55)
$(ledger_row failed xo/task-x1 "$anchor_short" 2026-09-02 06:36)
EOF
...
test_mismatched_run_after_abort_refuses_unconfirmed() {
  ...
  rc=0
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$head" 01RUN)" \
  XO_FAKE_AXI_STATUS_AFTER_ABORT="$(parked_axi_status_toon xo/task-x1 "$head" 02RUN)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_empty_status_after_abort_refuses_unconfirmed() {
  ...
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$head")" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
  XO_FAKE_NM_EMPTY_AFTER_ABORT=1 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_not_found_status_after_abort_confirms_completion() {
  ...
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$head")" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
  XO_FAKE_NM_NOT_FOUND_AFTER_ABORT=1 \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_another_branchs_parked_run_is_never_touched() {
  ...
  # teardown.
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/some-other-task deadbeef)" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_own_autonomous_run_is_left_alone() {
  ...
  rc=0
  XO_FAKE_AXI_STATUS="$(running_axi_status_toon xo/task-x1 "$head")" \
  XO_FAKE_NM_ABORT_LOG="$case_dir/nm-abort.log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  ...
  rc=0
  XO_TEARDOWN_TEST_PATH="$path_without_lsof" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${XO_FAKE_REUSED_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
  count=0
  [ ! -f "$XO_FAKE_PS_COUNT" ] || count=$(cat "$XO_FAKE_PS_COUNT")
  count=$((count + 1))
  printf '%s\n' "$count" > "$XO_FAKE_PS_COUNT"
...
SH
  ...
  rc=0
  XO_PROC_ROOT_OVERRIDE="$case_dir/no-proc" \
  XO_FAKE_REUSED_PID="$pid" XO_FAKE_PS_COUNT="$case_dir/ps-count" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_exec_changed_process_is_still_reaped() {
  ...
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${XO_FAKE_EXEC_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
  out=$("$REAL_PS_FOR_TEST" "$@") || exit $?
  [ -e "$XO_FAKE_EXEC_MARKER" ] || : > "$XO_FAKE_EXEC_MARKER"
  printf '%s\n' "$out"
  exit 0
fi
exec "$REAL_PS_FOR_TEST" "$@"
...
#!/usr/bin/env bash
count=0
[ ! -f "$XO_FAKE_LSOF_COUNT" ] || count=$(cat "$XO_FAKE_LSOF_COUNT")
count=$((count + 1))
printf '%s\n' "$count" > "$XO_FAKE_LSOF_COUNT"
if [ "$count" -eq 2 ]; then
  i=0
  while [ "$i" -lt 100 ]; do
    [ ! -e "$XO_FAKE_EXEC_DONE" ] || break
    sleep 0.01
    i=$((i + 1))
  done
fi
...
SH
  ...
  rc=0
  XO_PROC_ROOT_OVERRIDE="$case_dir/no-proc" \
  XO_FAKE_EXEC_PID="$pid" XO_FAKE_EXEC_MARKER="$marker" \
  XO_FAKE_EXEC_DONE="$done_flag" XO_FAKE_LSOF_COUNT="$case_dir/lsof-count" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${XO_FAKE_PERSISTENT_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
...
SH
  ...
  rc=0
  XO_PROC_ROOT_OVERRIDE="$case_dir/no-proc" XO_FAKE_PERSISTENT_PID="$fake_pid" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${XO_FAKE_EXITED_PID:-}" ]; then
  exit 1
fi
exec "$REAL_PS_FOR_TEST" "$@"
SH
...
EOF
  ...
  rc=0
  XO_PROC_ROOT_OVERRIDE="$case_dir/no-proc" XO_FAKE_EXITED_PID="$fake_pid" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  ...
  rc=0
  XO_FAKE_AXI_STATUS="$(parked_axi_status_toon xo/task-x1 "$head")" \
  XO_FAKE_NM_ABORT_LOG="$abort_log" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
