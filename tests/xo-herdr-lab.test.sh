#!/usr/bin/env bash
# Behavior tests for bin/xo-herdr-lab.sh using a stateful fake Herdr client.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(xo_test_tmproot xo-herdr-lab)
...
case "$1 ${2:-}" in
  "session list")
    ...
    ;;
  "server --session")
    if [ "${XO_FAKE_HERDR_SERVER_DELAY:-0}" != 0 ]; then
      "$XO_FAKE_HERDR_REAL_SLEEP" "$XO_FAKE_HERDR_SERVER_DELAY"
    fi
  ...
  "session delete")
    [ "$3" = "$session" ] || exit 92
    [ "${XO_FAKE_HERDR_DELETE_FAIL:-}" != 1 ] || exit 93
    printf '%s\n' deleted > "$state/$session"
    ;;
  "terminal title")
    [ "${XO_FAKE_HERDR_TITLE_FAIL:-}" != 1 ] || exit 94
...
. "$ROOT/bin/xo-herdr-lab.sh"

run_with_fake() {
  PATH="$FAKEBIN:$PATH" \
    XO_FAKE_HERDR_STATE="$FAKE_STATE" \
    XO_FAKE_HERDR_LOG="$FAKE_LOG" \
    XO_FAKE_HERDR_REAL_SLEEP="$REAL_SLEEP" \
    XO_FAKE_HERDR_SERVER_DELAY="${XO_FAKE_HERDR_SERVER_DELAY:-0}" \
    XO_FAKE_HERDR_FAST_POLL="${XO_FAKE_HERDR_FAST_POLL:-}" \
    XO_FAKE_HERDR_DELETE_FAIL="${XO_FAKE_HERDR_DELETE_FAIL:-}" \
    XO_FAKE_HERDR_TITLE_FAIL="${XO_FAKE_HERDR_TITLE_FAIL:-}" \
    XO_HERDR_LAB_STATE_DIR="$TRIPWIRES" \
    "$@"
}

test_refuses_unsafe_names() {
  local status=0 generated
  xo_herdr_lab_validate_name default >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "literal default must be refused"
  status=0
  xo_herdr_lab_validate_name arbitrary-session >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "non-lab prefix must be refused"
  xo_herdr_lab_validate_name xo-lab-safe-123 || fail "valid lab session name was refused"
  generated=$(xo_herdr_lab_name xo-autodetect-smoke-concurrency-h3)
  xo_herdr_lab_validate_name "$generated" || fail "generated lab session name was refused"
  [ "${#generated}" -le 40 ] || fail "generated lab session name is too long for Herdr socket paths: $generated"
  pass "xo-herdr-lab: names fail closed and require the lab prefix"
}

test_provision_run_and_guarded_teardown() {
  local name='' line_count status=0 stop_line delete_line
  name="xo-lab-behavior-$$"
  : > "$FAKE_LOG"
  run_with_fake xo_herdr_lab_provision "$name" || fail "provision failed"
  ...
  run_with_fake xo_herdr_lab_cli "$name" workspace list >/dev/null || fail "safe run command failed"
  run_with_fake xo_herdr_lab_cli "$name" server >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "bare server start outside provision must be refused"
  status=0
  run_with_fake xo_herdr_lab_cli "$name" server stop >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "server-global stop must be refused"
  status=0
  run_with_fake xo_herdr_lab_cli "$name" session delete "$name" >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "direct session delete must be refused"
  status=0
  run_with_fake xo_herdr_lab_cli "$name" status --session default >/dev/null 2>&1 || status=$?
  ...
  status=0
  run_with_fake xo_herdr_lab_cli "$name" --handoff server stop >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "a leading option shifting server stop past the guard must be refused"
  status=0
  run_with_fake xo_herdr_lab_cli "$name" --no-session session delete "$name" >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "a leading option shifting session delete past the guard must be refused"
  status=0
  run_with_fake xo_herdr_lab_cli "$name" --remote host workspace list >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "a leading option subverting session isolation must be refused"

  run_with_fake xo_herdr_lab_teardown "$name" || fail "guarded teardown failed"
  ...
  pass "xo-herdr-lab: provisioning, scoped calls, guarded teardown, and fleet tripwire are deterministic"
}

test_missing_tripwire_blocks_destruction() {
  local name="xo-lab-no-tripwire-$$" status=0 before after
  printf '%s\n' running > "$FAKE_STATE/$name"
  : > "$FAKE_LOG"
  before=$(wc -l < "$FAKE_LOG")
  run_with_fake xo_herdr_lab_teardown "$name" >/dev/null 2>&1 || status=$?
  ...
  pass "xo-herdr-lab: missing tripwire refuses teardown before any Herdr call"
}

test_changed_default_trips_after_teardown() {
  local name="xo-lab-tripwire-change-$$" status=0
  : > "$FAKE_LOG"
  run_with_fake xo_herdr_lab_provision "$name" || fail "tripwire fixture provision failed"
  ...
  pass "xo-herdr-lab: changed default fleet state is a hard failure"
}

test_stopped_owned_lab_can_reprovision() {
  local name="xo-lab-reprovision-$$"
  : > "$FAKE_LOG"
  run_with_fake xo_herdr_lab_provision "$name" || fail "initial provision failed"
  run_with_fake xo_herdr_lab_stop "$name" || fail "guarded stop failed"
  ...
  run_with_fake xo_herdr_lab_provision "$name" || fail "re-provision after guarded stop failed"
  ...
  run_with_fake xo_herdr_lab_teardown "$name" || fail "teardown after re-provision failed"
  pass "xo-herdr-lab: an owned stopped lab can re-provision safely"
}

test_failed_delete_retains_tripwire() {
  local name="xo-lab-delete-failure-$$" status=0
  : > "$FAKE_LOG"
  run_with_fake xo_herdr_lab_provision "$name" || fail "delete-failure fixture provision failed"
  XO_FAKE_HERDR_DELETE_FAIL=1 run_with_fake xo_herdr_lab_teardown "$name" >/dev/null 2>&1 || status=$?
  ...
  run_with_fake xo_herdr_lab_teardown "$name" || fail "retry after failed delete did not clean up the lab session"
  assert_absent "$TRIPWIRES/$name.fleet-state.json" "successful retry left the ownership tripwire behind"
  pass "xo-herdr-lab: failed deletion retains ownership until absence is confirmed"
}

test_timed_out_provision_cancels_late_launch() {
  local name="xo-lab-late-launch-$$" status=0
  cat > "$FAKEBIN/sleep" <<'SH'
#!/usr/bin/env bash
if [ "${XO_FAKE_HERDR_FAST_POLL:-}" = 1 ]; then
  while [ -n "${XO_FAKE_HERDR_WAIT_MARKER:-}" ] && [ ! -f "$XO_FAKE_HERDR_WAIT_MARKER" ]; do
    "$XO_FAKE_HERDR_REAL_SLEEP" 0.01
  done
  exit 0
fi
exec "$XO_FAKE_HERDR_REAL_SLEEP" "$@"
SH
  chmod +x "$FAKEBIN/sleep"
  : > "$FAKE_LOG"
  XO_FAKE_HERDR_FAST_POLL=1 XO_FAKE_HERDR_SERVER_DELAY=30 \
    run_with_fake xo_herdr_lab_provision "$name" >/dev/null 2>&1 || status=$?
  ...
  run_with_fake xo_herdr_lab_teardown "$name" || fail "teardown after timed-out provision failed"
  ...
  pass "xo-herdr-lab: timed-out provisioning cancels the launch before teardown"
}


# The pty attachment itself needs a real Herdr client, so the live guard
# tests/xo-herdr-attached-viewer-live-e2e.test.sh owns that proof. What is
# portable is who the helper will ever attach to, and who it will signal.
test_viewer_refuses_unowned_sessions() {
  local name="xo-lab-viewer-guard-$$" status=0 out
  : > "$FAKE_LOG"
  out=$(run_with_fake xo_herdr_lab_viewer_start "$name" 2>&1) || status=$?
  ...
  status=0
  run_with_fake xo_herdr_lab_viewer_start default >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "the default session must never be attached to"
  pass "xo-herdr-lab: the viewer attaches only to a session this lab owns"
}
...
write_viewer_record() {
  ...
  launcher_start=$(xo_herdr_lab_process_start "$launcher_pid") || fail "could not identify launcher fixture process"
  viewer_start=$(xo_herdr_lab_process_start "$viewer_pid") || fail "could not identify viewer fixture process"
  printf 'launcher_pid=%s\nlauncher_start=%s\nviewer_pid=%s\nviewer_start=%s\n' \
    "$launcher_pid" "$launcher_start" "$viewer_pid" "$viewer_start" > "$record"
}

test_viewer_start_cancels_an_unrecorded_launcher() {
  local name="xo-lab-viewer-late-$$" out status=0 launcher_pid
  local started="$TMP_ROOT/viewer-launcher-started"
  run_with_fake xo_herdr_lab_provision "$name" || fail "viewer-late fixture provision failed"
  cat > "$FAKEBIN/python3" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$$" > "$XO_FAKE_VIEWER_STARTED"
exec "$XO_FAKE_HERDR_REAL_SLEEP" 20
SH
  chmod +x "$FAKEBIN/python3"
  out=$(XO_FAKE_HERDR_FAST_POLL=1 XO_FAKE_HERDR_WAIT_MARKER="$started" \
    XO_FAKE_VIEWER_STARTED="$started" run_with_fake xo_herdr_lab_viewer_start "$name" 2>&1) || status=$?
  ...
  run_with_fake xo_herdr_lab_teardown "$name" || fail "viewer-late fixture teardown failed"
  pass "xo-herdr-lab: timed-out viewer startup cancels its exact launcher"
}
...
#!/usr/bin/env bash
trap 'printf "" > "$XO_FAKE_VIEWER_TERMINATING"; "$XO_FAKE_HERDR_REAL_SLEEP" 1.2; printf "" > "$XO_FAKE_VIEWER_COMPLETED"; exit 0' TERM
printf '' > "$XO_FAKE_VIEWER_STARTED"
while :; do
  "$XO_FAKE_HERDR_REAL_SLEEP" 0.1
done
SH
  chmod +x "$FAKEBIN/viewer-launcher"
  XO_FAKE_HERDR_REAL_SLEEP="$REAL_SLEEP" XO_FAKE_VIEWER_STARTED="$started" \
    XO_FAKE_VIEWER_TERMINATING="$terminating" XO_FAKE_VIEWER_COMPLETED="$completed" \
    "$FAKEBIN/viewer-launcher" &
  launcher_pid=$!
  while [ ! -f "$started" ]; do
    "$REAL_SLEEP" 0.01
  done
  run_with_fake xo_herdr_lab_cancel_viewer_launcher "$launcher_pid"
  ...
  pass "xo-herdr-lab: startup timeout allows launcher child escalation"
}

test_viewer_start_requires_its_owned_process() {
  local name="xo-lab-viewer-ownership-$$" out status=0 marker="$TMP_ROOT/viewer-launched"
  run_with_fake xo_herdr_lab_provision "$name" || fail "viewer-ownership fixture provision failed"
  printf '%s\n' cleared > "$FAKE_STATE/$name.foreground"
  cat > "$FAKEBIN/python3" <<'SH'
#!/usr/bin/env bash
: > "$XO_FAKE_VIEWER_MARKER"
exit 0
SH
  chmod +x "$FAKEBIN/python3"
  out=$(XO_FAKE_HERDR_FAST_POLL=1 XO_FAKE_VIEWER_MARKER="$marker" \
    run_with_fake xo_herdr_lab_viewer_start "$name" 2>&1) || status=$?
  rm -f "$FAKEBIN/python3"
  ...
  run_with_fake xo_herdr_lab_teardown "$name" || fail "viewer-ownership fixture teardown failed"
  pass "xo-herdr-lab: viewer start requires an identity-matched owned process"
}

test_viewer_stop_only_signals_owned_processes() {
  local name="xo-lab-viewer-stop-$$" record status=0 holder_pid pair="$TMP_ROOT/viewer-stop-pair"
  run_with_fake xo_herdr_lab_provision "$name" || fail "viewer-stop fixture provision failed"
  record=$(run_with_fake xo_herdr_lab_viewer_record_path "$name")

  # No record: a client attached by someone else is not ours to kill.
  printf '%s\n' cleared > "$FAKE_STATE/$name.foreground"
  run_with_fake xo_herdr_lab_viewer_stop "$name" \
  ...
  status=0
  XO_FAKE_HERDR_FAST_POLL=1 run_with_fake xo_herdr_lab_viewer_stop "$name" \
    >/dev/null 2>&1 || status=$?
  ...
  printf '%s\n' no_foreground_client > "$FAKE_STATE/$name.foreground"
  run_with_fake xo_herdr_lab_viewer_stop "$name" || fail "stop rejected a stale process record"
  kill -0 "$holder_pid" 2>/dev/null || fail "stop signalled a PID whose recorded identity did not match"
  kill "$holder_pid" 2>/dev/null || true
  wait "$holder_pid" 2>/dev/null || true

  run_with_fake xo_herdr_lab_viewer_stop "$name" || fail "stop failed once the client had detached"
  assert_absent "$record" "a confirmed detach left the viewer record behind"
  run_with_fake xo_herdr_lab_teardown "$name" || fail "teardown after viewer stop failed"
  pass "xo-herdr-lab: viewer stop signals only recorded processes and confirms the detach"
}

test_viewer_stop_requires_the_recorded_parent() {
  local name="xo-lab-viewer-parent-$$" record launcher_pid viewer_pid
  run_with_fake xo_herdr_lab_provision "$name" || fail "viewer-parent fixture provision failed"
  record=$(run_with_fake xo_herdr_lab_viewer_record_path "$name")
  sleep 20 &
  launcher_pid=$!
  sleep 20 &
  ...
  printf '%s\n' no_foreground_client > "$FAKE_STATE/$name.foreground"
  run_with_fake xo_herdr_lab_viewer_stop "$name" || fail "parent-mismatch stop failed"
  ...
  run_with_fake xo_herdr_lab_teardown "$name" || fail "viewer-parent fixture teardown failed"
  pass "xo-herdr-lab: viewer ownership requires the recorded parent"
}

test_interrupted_viewer_start_cancels_launcher() {
  local name="xo-lab-viewer-interrupt-$$" command_pid launcher_pid status=0
  local started="$TMP_ROOT/viewer-interrupt-started" attached="$TMP_ROOT/viewer-interrupt-attached"
  run_with_fake xo_herdr_lab_provision "$name" || fail "viewer-interrupt fixture provision failed"
  cat > "$FAKEBIN/python3" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$$" > "$XO_FAKE_VIEWER_STARTED"
"$XO_FAKE_HERDR_REAL_SLEEP" 0.5
: > "$XO_FAKE_VIEWER_ATTACHED"
printf '%s\n' cleared > "$XO_FAKE_HERDR_STATE/$XO_FAKE_VIEWER_SESSION.foreground"
exec "$XO_FAKE_HERDR_REAL_SLEEP" 20
SH
  chmod +x "$FAKEBIN/python3"
  XO_FAKE_VIEWER_STARTED="$started" XO_FAKE_VIEWER_ATTACHED="$attached" \
    XO_FAKE_VIEWER_SESSION="$name" run_with_fake exec "$ROOT/bin/xo-herdr-lab.sh" \
  ...
  run_with_fake xo_herdr_lab_teardown "$name" || fail "viewer-interrupt fixture teardown failed"
  pass "xo-herdr-lab: interrupted viewer start cancels its launcher"
}

test_teardown_refuses_while_viewer_attached() {
  local name="xo-lab-viewer-teardown-$$" record status=0 pair="$TMP_ROOT/viewer-teardown-pair"
  run_with_fake xo_herdr_lab_provision "$name" || fail "viewer-teardown fixture provision failed"
  record=$(run_with_fake xo_herdr_lab_viewer_record_path "$name")
  printf '%s\n' cleared > "$FAKE_STATE/$name.foreground"
  start_viewer_fixture "$pair"
  write_viewer_record "$record" "$FIXTURE_LAUNCHER_PID" "$FIXTURE_VIEWER_PID"
  : > "$FAKE_LOG"
  XO_FAKE_HERDR_FAST_POLL=1 run_with_fake xo_herdr_lab_teardown "$name" \
    >/dev/null 2>&1 || status=$?
  ...
  printf '%s\n' no_foreground_client > "$FAKE_STATE/$name.foreground"
  run_with_fake xo_herdr_lab_teardown "$name" || fail "teardown after the viewer detached failed"
  pass "xo-herdr-lab: teardown refuses to destroy a session an attached viewer still holds"
}

test_viewer_stop_retains_record_when_detach_is_unreadable() {
  local name="xo-lab-viewer-unreadable-$$" record status=0
  run_with_fake xo_herdr_lab_provision "$name" || fail "unreadable-detach fixture provision failed"
  record=$(run_with_fake xo_herdr_lab_viewer_record_path "$name")
  printf 'launcher_pid=99999999\nlauncher_start=stale\nviewer_pid=99999999\nviewer_start=stale\n' > "$record"
  XO_FAKE_HERDR_FAST_POLL=1 XO_FAKE_HERDR_TITLE_FAIL=1 \
    run_with_fake xo_herdr_lab_viewer_stop "$name" >/dev/null 2>&1 || status=$?
  ...
  printf '%s\n' no_foreground_client > "$FAKE_STATE/$name.foreground"
  run_with_fake xo_herdr_lab_teardown "$name" || fail "teardown after a confirmed detach failed"
  pass "xo-herdr-lab: unreadable detach results fail closed on running sessions"
}

test_viewer_launcher_refuses_unsafe_arguments() {
  local launcher="$ROOT/bin/xo-herdr-lab-viewer.py" status=0
  command -v python3 >/dev/null 2>&1 || { pass "xo-herdr-lab: viewer launcher argument guard (skipped, no python3)"; return; }
  ...
  status=0
  python3 "$launcher" xo-lab-args relative-pidfile >/dev/null 2>&1 || status=$?
  ...
  pass "xo-herdr-lab: the viewer launcher refuses unsafe sessions and pidfiles"
}
