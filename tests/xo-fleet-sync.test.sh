#!/usr/bin/env bash
# Behavior tests for xo-fleet-sync.sh drift handling.
#
# xo-fleet-sync fast-forwards a clone that is cleanly on its default branch. This
...
# xo home, the xo checkout itself - so it must be skipped by name
...
# (fetch_with_packed_refs_lock_guard, backed by bin/xo-lock-lib.sh's shared
...
TMP_ROOT=$(xo_test_tmproot xo-fleet-sync-tests)
HOME_N=0

# --- fixtures ---------------------------------------------------------------

# new_home: fresh isolated XO_HOME with an empty projects/ dir. Each test gets its
...
run_sync() {
  local home=$1
  shift
  XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/xo-fleet-sync.sh" "$@" 2>/dev/null
}

# build_enclosing_home <name>: an XO_HOME that is itself nested inside another git
# repository - xo's own layout, where projects/ sits inside the xo
...
# lsof shims mirror tests/xo-teardown.test.sh: no-holder (provably free), a live
...
# (XO_FLEET_SYNC_PACKED_REFS_LOCK_*, GIT_FETCH_COUNTER) are read from the caller's
...
run_sync_guarded() {
  ...
  XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/xo-fleet-sync.sh" "$@" >"$outf" 2>"$errf"
}
...
test_bootstrap_relays_recovered_and_stuck() {
  ...
  out=$(XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null)
...
test_orphaned_stale_packed_refs_lock_recovers() {
  ...
  set +e
  XO_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=2 \
  XO_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
  XO_FLEET_SYNC_PACKED_REFS_LOCK_AGE_SECS=0 \
...
test_transient_packed_refs_lock_self_clears() {
  ...
  XO_FLEET_SYNC_PACKED_REFS_LOCK_RETRIES=3 \
  XO_FLEET_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
