#!/usr/bin/env bash
# tests/xo-wake-drain-open-decisions-cursor.test.sh - end-to-end behavior tests
# for the incremental, cursor-backed OPEN DECISIONS scan
# (xo-classify-lib.sh's status_open_decisions_incremental /
# scan_open_decisions_incremental, wired into bin/xo-wake-drain.sh). These drive
...
# property, not the fold's own source text. tests/xo-wake-drain-open-decisions.test.sh
...
TMP_ROOT=$(xo_test_tmproot xo-wake-drain-open-decisions-cursor-tests)
...
test_buried_decision_survives_many_growing_drains_and_resolution_clears_it() {
  ...
  XO_STATE_OVERRIDE="$state" XO_OPEN_DECISIONS_READ_PROBE="$probe" "$DRAIN" > "$out" \
...
test_read_failure_preserves_state_for_retry() {
  ...
  XO_STATE_OVERRIDE="$state" XO_STATUS_SPAN_READER="$reader" "$DRAIN" > "$out" \
...
SH
  ...
  XO_STATE_OVERRIDE="$state" XO_OPEN_DECISIONS_READ_PROBE="$probe" PATH="$fakebin:$PATH" "$DRAIN" > "$out" \
