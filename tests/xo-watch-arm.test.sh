#!/usr/bin/env bash
# tests/xo-watch-arm.test.sh - the arm layer's cycle-close contract when the arm
...
# bin/xo-watch.sh holds the singleton, a real bin/xo-watch-arm.sh attaches to it,
...
TMP_ROOT=$(xo_test_tmproot xo-watch-arm-tests)
...
start_seed_watcher() {  # <state> <fakebin> <watch-out>
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=5 XO_SIGNAL_GRACE=1 \
...
start_attached_arm() {  # <state> <fakebin> <arm-out> <confirm-timeout>
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_ARM_ATTACH_POLL=0.1 \
    XO_ARM_CONFIRM_TIMEOUT="$confirm" "$WATCH_ARM" > "$armout" &
  ARM_PID=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
write_remote_delta() {  # <result-path> <status-line>
  ...
  {
    printf 'schema=xo-remote-delta.v1\n'
    printf 'status=delta\n'
...
status_signature() {  # <status-path>
  bash -c '
    ...
    size=$(_xo_status_file_size "$2") || exit 1
    ident=$(_xo_open_decisions_file_ident "$2") || exit 1
    printf "v2\t%s\t%s@%s" "$reported" "$size" "$ident"
  ' _ "$ROOT/bin/xo-classify-lib.sh" "$1"
}
...
start_rearm_arm() {  # <home> <state> <fakebin> <arm-out> [predecessor-arm-pid]
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$state" \
    XO_POLL=1 XO_SIGNAL_GRACE=0 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    XO_WATCH_PREDECESSOR_ARM_PID="$predecessor" \
...
test_attached_arm_reports_the_delivered_wake_after_drain() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "drain failed"
...
test_rearm_resurfaces_durable_queue_and_remote_open_decision() {
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" XO_DATA_OVERRIDE="$home/data" \
    "$ROOT/bin/xo-procevent-remote-reply.sh" ingest ios "$result" >/dev/null \
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/baseline-drain.out" \
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drainout" \
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/decision-only-drain.out" \
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/replayed-decision-drain.out" \
...
test_delivery_gap_wake_is_recovered_once() {
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/first-drain.out" \
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/gap-drain.out" \
...
test_interrupted_handling_is_redrained_on_rearm() {
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$WATCH_ARM" --handling-delivered "$handling_generation" \
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/interrupted-drain.out" \
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/replay-drain.out" \
...
test_malformed_marker_is_quarantined_once() {
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/recovery-drain.out" \
...
test_recovery_consumption_serializes_queue_publication() {
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/drain.out" \
...
test_handling_window_close_keeps_the_acknowledgement_valid() {
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/drain.out" 2> "$dir/drain.err" \
    || fail "handling drain did not present the durable wake"
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/remaining-drain.out" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "${pair%%$'\t'*}" \
...
test_moved_generation_acknowledgement_is_self_healing() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$first_sequence" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through 999 \
  ...
  XO_HOME="$home" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/redrain.out" \
...
# The watcher validates XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS when it arms and
...
test_arm_refuses_an_unusable_launch_confirm_window() {
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$state" \
    XO_POLL=1 XO_SIGNAL_GRACE=0 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    XO_ARM_CONFIRM_TIMEOUT=5 XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS=5s \
  ...
  grep -qF 'XO_PROCEVENT_LAUNCH_CONFIRM_SECONDS' "$armout" \
