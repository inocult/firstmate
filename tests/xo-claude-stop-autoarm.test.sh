...
# (bin/xo-claude-stop-autoarm.sh, docs/watcher-continuity.md).
#
...
# through the real xo-lock.sh path. The arm wrapper is a per-test fixture, so no
# real watcher, model, or fleet state is touched.
# shellcheck disable=SC2016 # single quotes are deliberate: $XO_HOME expands inside the fake harness child, and grep needles are literal strings
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(xo_test_tmproot xo-claude-stop-autoarm)
...
make_crewmate_worktree_dir() {
  local base=$1 dir=$2
  xo_git_worktree "$base" "$dir" xo/autoarm-test-branch
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_autoarm_scripts "$dir"
  printf '%s\n' "$dir"
...
# Arm fixture variants, installed per test as <dir>/bin/xo-watch-arm.sh.
write_arm_fixture() {
  local dir=$1 kind=$2
  case "$kind" in
    actionable)
...
#!/usr/bin/env bash
echo "$$" >> "$XO_HOME/state/arm-ran"
printf 'pending:downtime:fixture-generation\n' > "$XO_HOME/state/.watcher-down"
touch "$XO_HOME/state/.last-watcher-beat"
...
#!/usr/bin/env bash
echo "$$" >> "$XO_HOME/state/arm-ran"
: > "$XO_HOME/state/arm-waiting"
while [ ! -e "$XO_HOME/state/arm-release" ]; do sleep 0.02; done
printf 'watcher: FAILED - cycle ended without an actionable reason\n'
exit 1
SH
      ;;
...
printf 'epoch=999 owner_pid=1 outcome=arming updated_at=%s\nfixture-superseder-identity\n' "$(date +%s)" \
  > "$XO_HOME/state/.claude-autoarm-epoch"
printf 'watcher: FAILED - no live watcher with a fresh beacon\n'
exit 1
SH
      ;;
...
#!/usr/bin/env bash
echo "$$" >> "$XO_HOME/state/arm-ran"
printf '%s\n' "${XO_GUARD_GRACE:-unset}" > "$XO_HOME/state/arm-received-grace"
printf 'watcher: attached pid=%s (beacon 2s)\n' "$$"
exit 0
SH
      ;;
...
test_inert_without_session_lock() {
  ...
  out=$(printf '%s\n' '{"session_id":"s"}' | XO_HOME="$dir" bash "$dir/bin/xo-claude-stop-autoarm.sh" 2>&1); status=$?
...
test_reclaims_stale_session_lock_before_arming() {
  ...
  pass "auto-arm: a demonstrably dead recorded session owner is reclaimed through xo-lock.sh before arming"
}
...
test_inert_when_lock_held_by_other_harness() {
  ...
  out=$(printf '%s\n' '{"session_id":"s"}' | XO_HOME="$dir" "$FAKE_CLAUDE" -c '"$XO_HOME/bin/xo-claude-stop-autoarm.sh"' 2>&1); status=$?
...
test_stale_lock_recovery_preserves_afk_and_need_gates() {
  ...
  out=$(printf '%s\n' '{"session_id":"stale-afk"}' | XO_HOME="$afk_dir" "$FAKE_CLAUDE" -c '"$XO_HOME/bin/xo-claude-stop-autoarm.sh"' 2>&1); status=$?
  ...
  out=$(printf '%s\n' '{"session_id":"stale-idle"}' | XO_HOME="$idle_dir" "$FAKE_CLAUDE" -c '"$XO_HOME/bin/xo-claude-stop-autoarm.sh"' 2>&1); status=$?
...
test_actionable_close_rewakes_with_reason() {
  ...
  assert_contains "$out" "xo watcher wake" "rewake must carry the wake banner"
  assert_contains "$out" "stale: fixture-win actionable" "rewake must carry the arm's reason line"
  assert_contains "$out" "bin/xo-wake-drain.sh" "rewake must direct the drain-first protocol"
  assert_contains "$out" "do NOT run bin/xo-watch-arm.sh" "rewake must forbid a duplicate model re-arm"
...
test_actionable_close_with_live_successor_rewakes_once() {
  ...
  [ "$(printf '%s\n' "$out" | grep -c '^xo watcher wake')" -eq 1 ] \
...
test_failed_close_rewakes_with_failure_banner() {
  ...
  assert_not_contains "$out" "bin/xo-watch-arm.sh" "failure rewake must not create a manual arm loop"
...
test_arms_for_registered_custom_check_without_inflight() {
  ...
  XO_STATE_OVERRIDE="$dir/state" "$ROOT/bin/xo-check-register.sh" issue-comments >/dev/null \
    || fail "xo-check-register.sh could not register the custom check"
  write_arm_fixture "$dir" actionable
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
...
test_single_flight_admits_exactly_one_owner() {
  ...
  XO_HOME="$dir" "$FAKE_CLAUDE" -c '
    printf "%s\n" "$$" > "$XO_HOME/state/.lock"
    printf "%s\n" "{\"session_id\":\"s\"}" | "$XO_HOME/bin/xo-claude-stop-autoarm.sh" >/dev/null 2>"$XO_HOME/state/err1" &
    p1=$!
    printf "%s\n" "{\"session_id\":\"s\"}" | "$XO_HOME/bin/xo-claude-stop-autoarm.sh" >/dev/null 2>"$XO_HOME/state/err2" &
    p2=$!
    wait "$p1"; echo $? > "$XO_HOME/state/rc1"
    wait "$p2"; echo $? > "$XO_HOME/state/rc2"
  '
...
record_autoarm_owner_identity() {
  ...
  identity=$(xo_test_pid_identity "$pid") || return 1
  [ -n "$identity" ] || return 1
...
test_abandoned_owner_claim_is_reclaimed_and_rearms() {
  ...
  assert_contains "$out" "xo watcher wake" "the reclaimed cycle must still translate its wake"
...
record_autoarm_v2_claim() {
  ...
  identity=$(xo_test_pid_identity "$identity_pid") || return 1
  [ -n "$identity" ] || return 1
  printf 'epoch=%s owner_pid=%s outcome=%s updated_at=1\n%s\n' \
...
test_stuck_generation_claim_is_superseded_and_rearms() {
  ...
  assert_contains "$out" "xo watcher wake" "the superseding generation must still translate its wake"
...
test_superseded_owner_goes_silent_and_never_double_translates() {
  ...
  assert_contains "$c_out" "xo watcher wake" "the superseding generation must carry the rewake banner"
  wait "$a_pid"
  a_status=$?
...
test_long_poll_grace_reaches_arm_wrapper() {
  ...
  out=$(unset XO_GUARD_GRACE; XO_POLL=900 run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 2 "$status" "an unverified close without a healthy watcher must still fail closed"
  [ -e "$dir/state/arm-received-grace" ] || fail "arm wrapper never recorded XO_GUARD_GRACE"
  ...
  pass "auto-arm: a long XO_POLL with XO_GUARD_GRACE unset reaches xo-watch-arm.sh with the derived grace"
}

test_xo_lock_status_still_works_with_shared_lib() {
  local out
  out=$(XO_HOME="$TMP_ROOT/lock-status-home" bash "$ROOT/bin/xo-lock.sh" status 2>&1)
  assert_contains "$out" "lock: free" "xo-lock.sh status must keep working after the session-lock lib extraction"
  pass "xo-lock: shared session-lock lib preserves the status path"
}
