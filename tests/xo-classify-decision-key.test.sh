#!/usr/bin/env bash
# tests/xo-classify-decision-key.test.sh - decision-key position tolerance in
# the open-decisions fold (bin/xo-classify-lib.sh). A "[key=<slug>]" token is
...
# from one handed to a durable captain-held task (bin/xo-captain-hold.sh
# diverged). Cross-drain cursor persistence and the incremental
# cost bound live in tests/xo-wake-drain-open-decisions-cursor.test.sh; the
# drain wiring lives in tests/xo-wake-drain-open-decisions.test.sh.
set -u
...
TMP_ROOT=$(xo_test_tmproot xo-classify-decision-key-tests)
...
test_resolution_closes_across_positions() {
  ...
  # Opened colon-first, closed in the documented form (what xo-send's
