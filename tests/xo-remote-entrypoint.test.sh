#!/usr/bin/env bash
# xo-remote-entrypoint.sh installs as a PATH symlink under ~/.local/bin
# (docs/remote-secondmates.md). SCRIPT_DIR must resolve to the real bin/
# directory so it can source its sibling xo-remote-job-lib.sh, not to the
...
TMP_ROOT=$(xo_test_tmproot xo-remote-entrypoint)
REAL_BIN="$TMP_ROOT/real-root/bin"
LOCAL_BIN="$TMP_ROOT/local-bin"
mkdir -p "$REAL_BIN" "$LOCAL_BIN"
cp "$ROOT/bin/xo-remote-entrypoint.sh" "$ROOT/bin/xo-remote-job-lib.sh" "$REAL_BIN/"
chmod +x "$REAL_BIN/xo-remote-entrypoint.sh"
ln -s "$REAL_BIN/xo-remote-entrypoint.sh" "$LOCAL_BIN/xo-remote-entrypoint.sh"
...
test_symlink_invocation_resolves_sibling_lib() {
  ...
  code=$(run_entrypoint "$LOCAL_BIN/xo-remote-entrypoint.sh" "$out" "$err")
  ...
  assert_no_grep 'No such file or directory' "$err" \
    "invoking xo-remote-entrypoint.sh through a symlink failed to source its sibling lib"
  ...
  pass "xo-remote-entrypoint.sh invoked via a PATH symlink resolves SCRIPT_DIR to the real bin/ directory"
}
...
test_direct_invocation_still_works() {
  ...
  code=$(run_entrypoint "$REAL_BIN/xo-remote-entrypoint.sh" "$out" "$err")

  expect_code 64 "$code" "direct invocation exit code"
  assert_grep 'remote entrypoint expects protocol, root, home, and argv' "$err" \
    "direct invocation did not reach argument validation"
  pass "xo-remote-entrypoint.sh invoked directly still resolves SCRIPT_DIR correctly"
}
