#!/usr/bin/env bash
# xo-peek remote-secondmate capture routing.
#
...
# xo-peek + xo-on executables with a stubbed ssh transport (XO_SSH_BIN seam)
...
TMP_ROOT=$(xo_test_tmproot xo-peek-remote)
...
#!/usr/bin/env bash
cat > /dev/null
[ -z "${XO_FAKE_REMOTE_CAPTURE:-}" ] || printf '%s\n' "$XO_FAKE_REMOTE_CAPTURE"
exit "${XO_FAKE_SSH_RC:-0}"
SH
  chmod +x "$fb/fake-ssh"
  cat > "$fb/tmux" <<'SH'
#!/usr/bin/env bash
printf 'tmux\n' >> "${XO_FAKE_TMUX_TOUCHED:?}"
exit 1
SH
  chmod +x "$fb/tmux"
  printf '%s\n' "$fb"
...
test_remote_peek_reads_remote_pane() {
  ...
  out=$(env PATH="$fb:$PATH" \
    ...
    XO_SSH_BIN="$fb/fake-ssh" XO_FAKE_SSH_RC=0 \
    XO_FAKE_REMOTE_CAPTURE='● the remote mate is mid-refactor' \
    XO_FAKE_TMUX_TOUCHED="$touched" \
  ...
  pass "xo-peek remote: the capture routes over the remote transport, local adapters untouched"
}
...
test_remote_peek_unreachable_fails_loudly_without_death_claim() {
  ...
  env PATH="$fb:$PATH" \
    XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_SSH_BIN="$fb/fake-ssh" XO_FAKE_SSH_RC=255 \
    XO_FAKE_TMUX_TOUCHED="$touched" \
  ...
  pass "xo-peek remote: an unreachable host fails loudly without a false death claim"
}

test_remote_peek_reads_remote_pane
test_remote_peek_unreachable_fails_loudly_without_death_claim

echo "all xo-peek-remote tests passed"
