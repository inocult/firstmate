#!/usr/bin/env bash
# Opt-in live guard for Cursor Agent CLI as an xo PRIMARY.
#
...
#
# tests/xo-cursor-primary.test.sh and the Cursor cases in
# tests/xo-tmux-agent-liveness.test.sh are the portable regressions that run
...
#
# Isolation: a throwaway xo home under a temp dir, a private tmux socket,
...
xo_live_gate opt-in XO_CURSOR_PRIMARY_LIVE_E2E tmux jq

REAL_TMUX=$(command -v tmux)
CURSOR_BIN=${XO_CURSOR_BIN:-$(command -v cursor-agent || true)}
[ -n "$CURSOR_BIN" ] && [ -x "$CURSOR_BIN" ] \
  || fail "cursor-agent not found; install it or set XO_CURSOR_BIN. This guard refuses to pass without checking the real harness."
...
SOCKET="xo-cursor-primary-$$"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-cursor-primary.XXXXXX")
HOME_DIR="$LAB/home"
MARKER="XO_CURSOR_LIVE_MARKER_$$"
...
"$REAL_TMUX" -L "$SOCKET" new-session -d -s primary -x 220 -y 60 -c "$HOME_DIR" \
  "cd '$HOME_DIR' && XO_HOME='$HOME_DIR' XO_HEARTBEAT=30 XO_HEARTBEAT_MAX=30 exec '$CURSOR_BIN' --trust --yolo --workspace '$HOME_DIR'" \
...
wait_for_pane "XO_OP: v1 watcher:" 300 "a watcher wake delivered as a stop-hook follow-up"
...
sleep 5
LIVE_PARKS=$(pgrep -f "$HOME_DIR/bin/xo-turnend-guard-cursor.sh" 2>/dev/null | wc -l | tr -d ' ')
[ "${LIVE_PARKS:-0}" -le 1 ] \
...
export XO_STATE_OVERRIDE="$HOME_DIR/state"
export XO_SUPERVISOR_TARGET=primary
export XO_SUPERVISOR_BACKEND=tmux
export XO_DAEMON_PRIMARY_HARNESS=cursor
. "$HOME_DIR/bin/xo-supervise-daemon.sh"
composer=\$(xo_backend_composer_state tmux primary)
printf 'composer=%s\n' "\$composer"
