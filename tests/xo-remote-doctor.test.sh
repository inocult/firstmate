#!/usr/bin/env bash
# tests/xo-remote-doctor.test.sh - the remote second-mate readiness gate.
#
# Drives the real bin/xo-remote-doctor.sh against a controlled account fixture:
# a private HOME, a fake launchctl backed by state files, a fake herdr CLI, a
# fake lsof that names a real holder process as the xo-remote socket owner, and
...
# birth markers bin/xo-remote-herdr-owner-lib.sh reads, so the Aqua-versus-SSH
...
TMP_ROOT=$(xo_test_tmproot xo-remote-doctor)
LABEL=dev.xo.herdr.xo-remote
INTERACTIVE_LABEL=dev.xo.herdr
mkdir -p "$TMP_ROOT"
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
JOB_LABEL=dev.xo.remote-job
CASE_N=0
DOCTOR_WORKER_PID=
HOLDER_PIDS=()
trap 'if [ -n "$DOCTOR_WORKER_PID" ]; then kill "$DOCTOR_WORKER_PID" 2>/dev/null || true; fi; if [ "${#HOLDER_PIDS[@]}" -gt 0 ]; then kill "${HOLDER_PIDS[@]}" 2>/dev/null || true; fi; xo_test_cleanup || true' EXIT
GUARD="$ROOT/bin/xo-remote-herdr-guard.sh"
...
#!/usr/bin/env bash
set -u
printf '%s\n' "$*" >> "$XO_FAKE_LAUNCHCTL_LOG"
domain=${2:-}
label=${domain##*/}
loaded="$XO_FAKE_STATE/loaded-$label"
case "${1:-}" in
  print)
    case "$domain" in
      user/*/*)
        [ -f "$XO_FAKE_STATE/user-loaded-$label" ] || exit 113
        cat "$XO_FAKE_STATE/user-loaded-$label"
        ;;
      */dev.xo.herdr.xo-remote)
        [ -f "$loaded" ] || exit 113
        cat "$loaded"
        ;;
      */dev.xo.herdr)
        [ -f "$XO_FAKE_STATE/interactive-loaded" ] || exit 113
        printf 'interactive default job\n'
        ;;
      */*/*) [ -f "$loaded" ] || exit 113; cat "$loaded" ;;
      *) [ -f "$XO_FAKE_STATE/gui-session" ] || exit 113 ;;
    esac
    exit 0
    ;;
  bootout)
    [ ! -f "$XO_FAKE_STATE/bootout-fail" ] || { printf 'Boot-out failed: operation not permitted\n' >&2; exit 6; }
    case "$domain" in
      */dev.xo.herdr.xo-remote) rm -f "$loaded" ;;
      */dev.xo.herdr) rm -f "$XO_FAKE_STATE/interactive-loaded" ;;
      *) rm -f "$loaded" ;;
    esac
    exit 0
    ;;
  bootstrap)
    ...
    [ -f "$XO_FAKE_STATE/gui-session" ] || { printf 'Bootstrap failed: 5: Input/output error\n' >&2; exit 5; }
    ...
    case "$label" in
      dev.xo.remote-job)
        cat > "$loaded" <<EOF
path = $XO_FAKE_JOB_PLIST
program = $XO_FAKE_JOB_WORKER
...
EOF
      ...
      *)
        cat > "$loaded" <<EOF
path = $XO_FAKE_PLIST
program = $XO_FAKE_LOGIN_SHELL
arguments = {
	$XO_FAKE_LOGIN_SHELL
	-l
	-c
	exec '$XO_FAKE_GUARD' '$XO_FAKE_HERDR_BIN' 'xo-remote'
}
stdout path = $XO_FAKE_LAUNCH_AGENT_LOG
stderr path = $XO_FAKE_LAUNCH_AGENT_LOG
semaphores = {
	successful exit => 0
}
properties = runatload | inferred program
EOF
        if [ ! -f "$XO_FAKE_STATE/bootstrap-does-not-start" ]; then
          printf 'true\n' > "$XO_FAKE_HERDR_RUNNING"
          printf '%s\n' "$XO_FAKE_AQUA_PID" > "$XO_FAKE_STATE/socket-owner"
        fi
        ;;
    esac
    exit 0
    ;;
  kickstart)
    [ ! -f "$XO_FAKE_STATE/kickstart-fail" ] || { printf 'Kickstart failed: service unavailable\n' >&2; exit 6; }
    case "$label" in
      dev.xo.remote-job) : ;;
      *)
        ...
        printf '%s\n' "$XO_FAKE_AQUA_PID" > "$XO_FAKE_STATE/socket-owner"
        if [ -f "$XO_FAKE_STATE/kickstart-delay" ]; then
          cp "$XO_FAKE_STATE/kickstart-delay" "$XO_FAKE_STATE/herdr-delay"
        else
          printf 'true\n' > "$XO_FAKE_HERDR_RUNNING"
        fi
        ;;
...
#!/usr/bin/env bash
pid=$(cat "$XO_FAKE_STATE/socket-owner" 2>/dev/null || printf '%s' "$XO_FAKE_AQUA_PID")
[ -n "$pid" ] || exit 0
printf 'p%s\n' "$pid"
printf 'n%s\n' "$XO_FAKE_HERDR_SOCKET"
SH
...
#!/usr/bin/env bash
set -u
[ "${XO_FAKE_DSCL_FAIL:-0}" != 1 ] || exit 1
[ "${XO_FAKE_DSCL_HANG:-0}" != 1 ] || exec /bin/sleep 30
if [ "${1:-}" = . ] && [ "${2:-}" = -read ] && [ "${4:-}" = UserShell ]; then
  count_file="$XO_FAKE_STATE/dscl-count"
  count=$(cat "$count_file" 2>/dev/null || printf 0)
  count=$((count + 1))
  printf '%s\n' "$count" > "$count_file"
  shell=${XO_FAKE_LOGIN_SHELL:-/bin/sh}
  if [ "$count" -gt 1 ] && [ -n "${XO_FAKE_SECOND_LOGIN_SHELL:-}" ]; then
    shell=$XO_FAKE_SECOND_LOGIN_SHELL
  fi
  printf 'UserShell: %s\n' "$shell"
  exit 0
fi
...
#!/usr/bin/env bash
set -u
running=$(cat "$XO_FAKE_HERDR_RUNNING" 2>/dev/null || printf 'false')
case "${1:-} ${2:-}" in
  "status --json")
    if [ -f "$XO_FAKE_STATE/herdr-delay" ]; then
      delay=$(cat "$XO_FAKE_STATE/herdr-delay")
      if [ "$delay" -gt 0 ]; then
        printf '%s\n' "$((delay - 1))" > "$XO_FAKE_STATE/herdr-delay"
        running=false
      else
        rm -f "$XO_FAKE_STATE/herdr-delay"
        printf 'true\n' > "$XO_FAKE_HERDR_RUNNING"
        running=true
      fi
    fi
    printf '{"client":{"version":"0.7.5","protocol":16},"server":{"running":%s,"socket":"%s"}}\n' "$running" "$XO_FAKE_HERDR_SOCKET"
    ;;
  "server "*|"server ")
    printf 'true\n' > "$XO_FAKE_HERDR_RUNNING"
    ;;
...
doctor() {
  ...
  DOCTOR_OUT=$(
    HOME="$CASE_HOME" \
    XO_HOME="$CASE_PROJECT_HOME" \
    PATH="$CASE_HOME/.local/bin:$CASE_BIN:$BASE_PATH" \
    XO_FAKE_STATE="$CASE_STATE" \
    XO_FAKE_LAUNCHCTL_LOG="$CASE_LAUNCHCTL_LOG" \
    XO_FAKE_FORBIDDEN_LOG="$CASE_FORBIDDEN_LOG" \
    XO_FAKE_HERDR_RUNNING="$CASE_HERDR_RUNNING" \
    XO_FAKE_HERDR_BIN="$CASE_BIN/herdr" \
    XO_FAKE_HERDR_SOCKET="$CASE_STATE/herdr.sock" \
    XO_FAKE_GUARD="$GUARD" \
    XO_FAKE_AQUA_PID="$AQUA_HOLDER_PID" \
    XO_FAKE_PLIST="$CASE_PLIST" \
    XO_FAKE_JOB_PLIST="$CASE_JOB_PLIST" \
    XO_FAKE_JOB_WORKER="$ROOT/bin/xo-remote-job-worker.sh" \
    XO_FAKE_LAUNCH_AGENT_LOG="$CASE_HOME/Library/Logs/$LABEL.log" \
    XO_FAKE_LOGIN_SHELL="${CASE_LOGIN_SHELL:-/bin/sh}" \
    XO_FAKE_SECOND_LOGIN_SHELL="${CASE_SECOND_LOGIN_SHELL:-}" \
    XO_FAKE_DSCL_FAIL="${CASE_DSCL_FAIL:-0}" \
    XO_FAKE_DSCL_HANG="${CASE_DSCL_HANG:-0}" \
    XO_LAUNCH_AGENT_SHELL="$([ "${CASE_RESOLVE_DSCL:-0}" = 1 ] || printf '%s' "$CASE_LOGIN_SHELL")" \
    SHELL="${CASE_ENV_SHELL-${SHELL-}}" \
    XO_REMOTE_JOB_PLATFORM_OVERRIDE="${CASE_PLATFORM_OVERRIDE-}" \
    XO_REMOTE_JOB_ACTIVE="${CASE_REMOTE_JOB_ACTIVE-1}" \
    "$ROOT/bin/xo-remote-doctor.sh" "$@" 2>&1
  )
  DOCTOR_RC=$?
  set -e
}
...
write_loaded_contract() { # <herdr-path> [properties] [exec-command]
  ...
  exec_cmd=${3:-"exec '$GUARD' '$herdr_bin' 'xo-remote'"}
...
# argv contract. The plist is XO's output, so semantic structure is
...
assert_herdr_launch_agent_contract() { # <plist> <herdr-bin> [login-shell]
  ...
  [ "$cmd" = "exec '$GUARD' '$herdr_bin' 'xo-remote'" ] \
    || fail "ProgramArguments[3] is not exec of the guard with $herdr_bin for session xo-remote: $cmd"
...
[ "$(plist_value "$CASE_JOB_PLIST" Label)" = "$JOB_LABEL" ] \
  || fail "the worker plist does not carry the XO label"
[ "$(plist_value "$CASE_JOB_PLIST" LimitLoadToSessionType)" = Aqua ] \
  || fail "the worker plist is not Aqua-scoped"
[ "$(plist_first_value "$CASE_JOB_PLIST" ProgramArguments)" = "$ROOT/bin/xo-remote-job-worker.sh" ] \
...
cmp -s "$CASE_STATE/interactive-before.plist" "$CASE_INTERACTIVE_PLIST" \
  || fail "the xo-remote repair rewrote the interactive default launch agent"
assert_present "$CASE_STATE/interactive-loaded" "the xo-remote repair unloaded the interactive default launch agent"
assert_no_grep "gui/$(id -u)/$INTERACTIVE_LABEL$" "$CASE_LAUNCHCTL_LOG" \
  "the xo-remote repair inspected or controlled the interactive default launch agent"
assert_no_dangerous_calls "the repair reached for auto-login, FileVault, or the keychain"
pass "--fix installs the dedicated xo-remote launch agent without touching default"
...
XML
write_loaded_contract /obsolete/bin/herdr 'keepalive | runatload | inferred program' "exec '/obsolete/bin/herdr' server --session 'xo-remote'"
printf 'true\n' > "$CASE_HERDR_RUNNING"
doctor
...
assert_contains "$DOCTOR_OUT" "check herdr-server=ok: session xo-remote is running in the Aqua login session (pid $AQUA_HOLDER_PID, launchd)" \
...
assert_contains "$DOCTOR_OUT" "check herdr-server=fixable: session xo-remote is served by pid $BACKGROUND_HOLDER_PID born outside the Aqua login session (unknown)" \
...
assert_contains "$DOCTOR_OUT" "check herdr-server=fixable: session xo-remote is served by pid $XPC_ZERO_HOLDER_PID born outside the Aqua login session (unknown)" \
...
assert_contains "$DOCTOR_OUT" "check herdr-server=ok: session xo-remote is running in the Aqua login session (pid $WORKER_HOLDER_PID, worker)" \
...
assert_contains "$DOCTOR_OUT" "check herdr-server=fixable: session xo-remote is served by pid $SSH_HOLDER_PID born outside the Aqua login session (ssh)" \
...
assert_contains "$DOCTOR_OUT" "check herdr-server=fixable: session xo-remote is served by pid $SSH_HOLDER_PID" \
...
assert_contains "$DOCTOR_OUT" 'check herdr-server=fixable: session xo-remote is running but no herdr process can be shown to own its socket' \
...
assert_grep '# XO remote tool wrapper v1' "$CASE_HOME/.local/bin/tasks-axi" \
  "the generated wrapper is not marked XO-owned"
...
assert_contains "$DOCTOR_OUT" 'fix required-treehouse=failed:' \
  "the non-XO wrapper refusal was not reported"
...
HOME="$CASE_HOME" XO_ROOT_OVERRIDE="$ROOT" XO_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  "$ROOT/bin/xo-remote-job-worker.sh" > "$CASE_STATE/worker.out" 2> "$CASE_STATE/worker.err" &
DOCTOR_WORKER_PID=$!
for _ in $(seq 1 100); do
  [ -f "$CASE_HOME/.xo/remote-job/worker.ready" ] && break
  sleep 0.05
done
assert_present "$CASE_HOME/.xo/remote-job/worker.ready" "the stale-identity fixture worker did not start"
printf 'stale-worker-identity\n' > "$CASE_HOME/.xo/remote-job/worker.identity"
doctor
expect_code 1 "$DOCTOR_RC" "doctor accepted a live worker with stale code identity"
assert_contains "$DOCTOR_OUT" 'check remote-job-worker=fixable: the running remote job worker does not match the current XO code' \
...
DOCTOR_WORKER_PID=$(cat "$CASE_HOME/.xo/remote-job/worker.pid")
...
printf '#!/usr/bin/env bash\n' > "$REMOTE_ROOT/bin/xo-remote-entrypoint.sh"
export XO_ROOT_OVERRIDE="$REMOTE_ROOT"
doctor
...
[ "$(readlink "$CASE_HOME/.local/bin/xo-remote-entrypoint.sh")" = "$REMOTE_ROOT/bin/xo-remote-entrypoint.sh" ] \
  || fail "the entrypoint symlink does not point at this code root"
printf 'not a symlink\n' > "$CASE_HOME/.local/bin/other"
rm -f "$CASE_HOME/.local/bin/xo-remote-entrypoint.sh"
printf 'operator wrapper\n' > "$CASE_HOME/.local/bin/xo-remote-entrypoint.sh"
doctor --fix
assert_contains "$DOCTOR_OUT" 'check entrypoint-link=human:' "an operator-owned entrypoint file was not left to the operator"
[ "$(cat "$CASE_HOME/.local/bin/xo-remote-entrypoint.sh")" = 'operator wrapper' ] \
