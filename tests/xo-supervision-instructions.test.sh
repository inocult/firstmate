...
TMP_ROOT=$(xo_test_tmproot xo-supervision-instructions)
RENDER="$ROOT/bin/xo-supervision-instructions.sh"
...
test_selected_harness_block_only() {
  ...
  assert_contains "$out" "bin/xo-watch-checkpoint.sh" "codex checkpoint helper missing"
...
test_conditional_stanzas() {
  ...
  out=$(XO_HOME="$home" XO_CONFIG_OVERRIDE="$config" "$RENDER" --harness codex --read-only 1 --afk 1 --x-mode 1)
...
test_repair_lines() {
  ...
  out=$(XO_HOME="$home" XO_CODEX_WATCH_CHECKPOINT=7 "$RENDER" --harness codex --repair-line)
  assert_contains "$out" "bin/xo-watch-checkpoint.sh --seconds 7" "codex repair line did not use checkpoint helper and env override"

  out=$(XO_HOME="$home" "$RENDER" --harness claude --queue-pending 1 --repair-line)
  ...
  assert_not_contains "$out" "bin/xo-watch-arm.sh" "claude pre-verification repair line directed an arm command"

  : > "$home/config/x-mode.env"
  out=$(XO_HOME="$home" XO_CODEX_WATCH_CHECKPOINT=7 "$RENDER" --harness codex --x-mode 1 --repair-line)
  assert_contains "$out" "source '$home/config/x-mode.env' first" "x-mode repair line did not source the effective cadence config"
  assert_contains "$out" "bin/xo-watch-checkpoint.sh --seconds 7" "x-mode codex repair line lost the checkpoint helper"

  out=$(XO_HOME="$home" "$RENDER" --harness opencode --read-only 1 --repair-line)
  assert_contains "$out" "session holding the fleet lock" "read-only repair line missing"

  out=$(XO_HOME="$home" "$RENDER" --harness pi --repair-line)
  assert_contains "$out" "Pi tool xo_watch_arm_pi" "pi repair line does not direct the model to the extension-owned tool"
  assert_not_contains "$out" "extension command /xo-watch-arm-pi" "pi repair line still directs the model to the human slash command"
  out=$(XO_HOME="$home" "$RENDER" --harness omp --repair-line)
  assert_contains "$out" "omp tool xo_watch_arm_omp" "omp repair line does not direct the model to the extension-owned tool"
  assert_contains "$out" ".omp/extensions/xo-primary-turnend-guard.ts" "omp repair line does not name its own turn-end extension"
  assert_not_contains "$out" "xo_watch_arm_pi" "omp repair line must not borrow the Pi tool"
...
test_cross_harness_ordinary_continuation_and_repair_matrix() {
  ...
  assert_not_contains "$ordinary" "xo_watch_arm_pi" "pi ordinary-wake line incorrectly calls the recovery tool"
  out=$("$RENDER" --harness pi --repair-line)
  assert_contains "$out" "xo_watch_arm_pi" "pi recovery line lost the extension-owned repair tool"
  ...
  assert_contains "$out" ".omp/extensions/xo-primary-omp-watch.ts" "omp snippet did not substitute its watch extension path"
  assert_not_contains "$out" "__XO_OMP_EXT__" "omp snippet left a placeholder unsubstituted"
  assert_not_contains "$out" "__XO_OMP_TURNEND_EXT__" "omp snippet left the turn-end placeholder unsubstituted"
  ...
  assert_contains "$out" "xo_watch_arm_omp" "omp recovery line lost the extension-owned repair tool"
  ...
  assert_not_contains "$ordinary" "bin/xo-watch-arm.sh" "opencode ordinary-wake line incorrectly calls the recovery probe"
  ...
  assert_contains "$ordinary" "bin/xo-claude-stop-autoarm.sh" "claude ordinary-wake line lost the auto-arm script name"
  assert_contains "$ordinary" "do not arm another cycle" "claude ordinary-wake line does not forbid a model re-arm"
  assert_not_contains "$ordinary" "bin/xo-watch-arm.sh" "claude ordinary-wake line incorrectly calls the manual arm"
  ...
  assert_not_contains "$out" "bin/xo-watch-arm.sh" "claude recovery line must not create a repeatable manual arm loop"
  ...
  assert_contains "$ordinary" "bin/xo-watch-arm.sh" "grok ordinary-wake line lost the background arm command"
  ...
  assert_contains "$out" "bin/xo-watch-arm.sh" "grok recovery line lost the arm command"

  out=$("$RENDER" --harness codex)
  ordinary=$(printf '%s\n' "$out" | grep -F -- '- Ordinary wake:')
  assert_contains "$ordinary" "next foreground" "codex ordinary-wake line lost its foreground checkpoint"
  assert_contains "$ordinary" "bin/xo-watch-checkpoint.sh" "codex ordinary-wake line lost the checkpoint command"
  assert_not_contains "$ordinary" "bin/xo-watch-arm.sh" "codex ordinary-wake line incorrectly uses a background arm"
  ...
  assert_contains "$out" "bin/xo-watch-checkpoint.sh" "codex recovery line lost the checkpoint command"
...
test_pi_signed_preserves_identity_with_pi_supervision_protocol() {
  ...
  assert_contains "$out" "Pi tool xo_watch_arm_pi" \
...
test_grok_is_background_notify() {
  ...
  assert_contains "$out" "bin/xo-watch-arm.sh" "grok snippet missing watcher arm"
  assert_not_contains "$out" "__XO_X_MODE_ENV" "renderer leaked an x-mode path placeholder"
...
test_grok_command_sources_effective_config() {
  ...
  out=$(XO_HOME="$home" XO_CONFIG_OVERRIDE="$config" "$RENDER" --harness grok --x-mode 1)
  assert_contains "$out" "[ -f '$config/x-mode.env' ] && . '$config/x-mode.env'; exec bin/xo-watch-arm.sh" "grok arm command did not use the effective x-mode config path"
...
test_pi_snippet_uses_effective_extension_path() {
  ...
  turnend="$ROOT/.pi/extensions/xo-primary-turnend-guard.ts"
  watch="$ROOT/.pi/extensions/xo-primary-pi-watch.ts"
  mkdir -p "$home/state" "$home/config"
  out=$(XO_HOME="$home" "$RENDER" --harness pi)
  ...
  assert_not_contains "$out" "__XO_PI_EXT__" "renderer leaked the Pi extension path placeholder"
  assert_not_contains "$out" "__XO_PI_TURNEND_EXT__" "renderer leaked the Pi turn-end extension path placeholder"
  assert_not_contains "$out" "state/xo-primary-pi-watch.ts" "pi snippet kept the old generated state-relative extension path"
