...
TMP_ROOT=$(xo_test_tmproot xo-remote-handoff)
mkdir -p "$TMP_ROOT"
...
#   rm: cannot remove '/tmp/xo-remote-handoff.XXXXXX': Directory not empty
...
trap xo_remote_handoff_teardown EXIT
printf 'fixture\n' > "$REMOTE_ROOT/AGENTS.md"
cp "$ROOT/bin/xo-remote-entrypoint.sh" "$ROOT/bin/xo-remote-job-lib.sh" \
  "$ROOT/bin/xo-remote-job-worker.sh" "$ROOT/bin/xo-remote-file.sh" \
  "$ROOT/bin/xo-backlog-receive.sh" "$ROOT/bin/xo-tasks-axi-lib.sh" \
  "$ROOT/bin/xo-wake-lib.sh" "$REMOTE_ROOT/bin/"
...
case "${XO_FAKE_SSH_MODE:-normal}:$command_name" in
  *:xo-remote-secondmate-control.sh)
    printf '%s\n' "$command_name" >> "$XO_FAKE_REMOTE_WAKE_LOG"
    [ "${XO_FAKE_REMOTE_WAKE_RC:-0}" -eq 0 ] || printf 'remote receiver wake failed\n' >&2
    exit "${XO_FAKE_REMOTE_WAKE_RC:-0}"
    ;;
  unreachable:*) exit 255 ;;
  serialize:xo-backlog-receive.sh)
    if mkdir "$XO_FAKE_SERIALIZE_ONCE" 2>/dev/null; then
      touch "$XO_FAKE_SERIALIZE_ENTERED"
      while [ ! -f "$XO_FAKE_SERIALIZE_RELEASE" ]; do sleep 0.02; done
    fi
    exec "$XO_FAKE_REMOTE_ENTRYPOINT" "$@"
    ;;
  after-put:xo-remote-file.sh)
    "$XO_FAKE_REMOTE_ENTRYPOINT" "$@"
    exit 255
    ;;
  after-receive:xo-backlog-receive.sh)
    "$XO_FAKE_REMOTE_ENTRYPOINT" "$@"
    exit 255
    ;;
  *) exec "$XO_FAKE_REMOTE_ENTRYPOINT" "$@" ;;
...
handoff_env() {
  ...
  XO_FAKE_REMOTE_WAKE_LOG="$WAKE_LOG" \
  XO_FAKE_REMOTE_WAKE_RC="${XO_FAKE_REMOTE_WAKE_RC:-0}" \
  XO_FAKE_SERIALIZE_ONCE="$TMP_ROOT/serialize.once" \
  XO_FAKE_SERIALIZE_ENTERED="$TMP_ROOT/serialize.entered" \
  XO_FAKE_SERIALIZE_RELEASE="$TMP_ROOT/serialize.release" \
...
if printf 'complete' | XO_HOME="$REMOTE" "$REMOTE_ROOT/bin/xo-remote-file.sh" \
...
XO_HOME="$REMOTE" "$REMOTE_ROOT/bin/xo-remote-file.sh" \
...
if XO_HOME="$REMOTE" "$REMOTE_ROOT/bin/xo-remote-file.sh" \
...
: > "$SSH_COUNT"
set +e
XO_FAKE_SSH_MODE=after-receive handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios ios-a ios-b \
...
out=$(handoff_env "$ROOT/bin/xo-backlog-handoff.sh" --resume-pending)
assert_contains "$out" 'received: ios moved=0 already=2' "retry did not classify already-delivered keys idempotently"
[ "$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")" -eq 1 ] \
...
: > "$SSH_COUNT"
set +e
XO_FAKE_SSH_MODE=after-put handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios transfer-cut \
...
handoff_env "$ROOT/bin/xo-backlog-handoff.sh" --resume-pending >/dev/null \
...
XO_FAKE_SSH_MODE=serialize handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios serialized-a \
...
XO_FAKE_SSH_MODE=serialize handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios serialized-b \
...
handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios stale-lock-item >/dev/null \
...
set +e
XO_FAKE_SSH_MODE=unreachable handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios pending-offline \
  > "$TMP_ROOT/offline.out" 2>&1
rc=$?
set -e
[ "$rc" -ne 0 ] || fail "offline handoff claimed success"
bootstrap_out=$(XO_HOME="$PARENT" XO_ROOT_OVERRIDE="$ROOT" XO_BACKEND=tmux \
  XO_BOOTSTRAP_DETECT_ONLY=1 "$ROOT/bin/xo-bootstrap.sh" 2>&1)
...
XO_FAKE_REMOTE_WAKE_RC=1 handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios remote-wake-fail \
...
wakes_before=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
XO_FAKE_REMOTE_WAKE_RC=255 handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios wake-escalated \
...
# The watcher's pending-reply tick (bin/xo-watch.sh -> xo_pending_reply_tick)
# escalates an undelivered delivery-unknown correlation before any resume runs.
bash -c '. "$1"; xo_pending_reply_tick "$2"' _ "$ROOT/bin/xo-pending-reply-lib.sh" "$PARENT/state" \
...
set +e
handoff_env "$ROOT/bin/xo-backlog-handoff.sh" --resume-pending > "$TMP_ROOT/wake-escalated-resume.out" 2>&1
rc=$?
set -e
if [ "$rc" -ne 0 ]; then
...
[ "$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")" -eq $((wakes_before + 3)) ] \
...
handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios after-escalated >/dev/null \
...
wakes_before=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
XO_FAKE_REMOTE_WAKE_RC=255 handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios wake-permanent-a \
...
wakes_after_first=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
[ "$wakes_after_first" -gt "$wakes_before" ] || fail "first permanently lost wake was not attempted"
set +e
XO_FAKE_REMOTE_WAKE_RC=255 handoff_env "$ROOT/bin/xo-backlog-handoff.sh" --resume-pending \
...
wakes_after_resume=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
...
XO_FAKE_REMOTE_WAKE_RC=255 handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios wake-permanent-b \
...
[ "$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")" -gt "$wakes_after_resume" ] \
...
wakes_before=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
set +e
PATH="$RM_FAKEBIN:$PATH" XO_REAL_RM="$REAL_RM" \
  XO_FAIL_RM_PATH="$PARENT/data/handoff/ios.outbox.md" \
  handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios cleanup-retry \
...
wakes_after=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
...
handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios after-cleanup >/dev/null \
  || fail "fresh handoff did not converge an older confirmed cleanup failure"
[ "$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")" -eq $((wakes_after + 1)) ] \
...
wakes_before=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
PATH="$RM_FAKEBIN:$PATH" XO_REAL_RM="$REAL_RM" \
  XO_FAIL_RM_PATH="$PARENT/state/.backlog-handoff-ios.wake-pending" \
  handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios confirmed-marker-stale \
...
[ "$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")" -eq $((wakes_before + 1)) ] \
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "$XO_CONFIRM_MV_PATH" ]; then
  count=$(cat "$XO_CONFIRM_MV_COUNT" 2>/dev/null || echo 0)
  count=$((count + 1))
  printf '%s\n' "$count" > "$XO_CONFIRM_MV_COUNT"
  [ "$count" -ne 2 ] || exit 1
fi
exec "$XO_REAL_MV" "$@"
SH
...
wakes_before=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
PATH="$CONFIRM_MV_FAKEBIN:$PATH" XO_REAL_MV="$REAL_MV" \
  XO_CONFIRM_MV_PATH="$PARENT/state/.backlog-handoff-ios.wake-pending" \
  XO_CONFIRM_MV_COUNT="$TMP_ROOT/confirm-mv.count" \
  handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios delivered-pending-old \
...
handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios delivered-pending-new >/dev/null \
  || fail "delivered pending marker blocked the next handoff"
[ "$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")" -eq $((wakes_before + 2)) ] \
...
wakes_before=$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")
PATH="$MV_FAKEBIN:$PATH" XO_REAL_MV="$REAL_MV" \
  XO_FAIL_MV_PATH="$PARENT/state/.backlog-handoff-ios.wake-pending" \
  handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios wake-state-drop \
...
[ "$(grep -cF xo-remote-secondmate-control.sh "$WAKE_LOG")" -eq "$wakes_before" ] \
...
handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios after-wake-state-drop >/dev/null \
...
PATH="$RM_FAKEBIN:$PATH" XO_REAL_RM="$REAL_RM" XO_FAIL_RM_PATH="$stale_wake_marker" \
  handoff_env "$ROOT/bin/xo-backlog-handoff.sh" --resume-pending \
...
PATH="$RM_FAKEBIN:$PATH" XO_REAL_RM="$REAL_RM" XO_FAIL_RM_PATH="$stale_wake_marker" \
  handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios after-stale-wake \
...
XO_HOME="$PARENT" /bin/bash -c '
  ...
  xo_lock_release "$3"
  xo_lock_release "$2"
' _ "$ROOT/bin/xo-wake-lib.sh" "$registry_lock" "$handoff_lock" \
...
handoff_env "$ROOT/bin/xo-backlog-handoff.sh" ios route-race \
...
: > "$SSH_COUNT"
fresh_out=$(XO_HOME="$FRESH" XO_ROOT_OVERRIDE="$ROOT" XO_BACKEND=tmux \
  XO_BOOTSTRAP_DETECT_ONLY=1 "$ROOT/bin/xo-bootstrap.sh" 2>&1)
