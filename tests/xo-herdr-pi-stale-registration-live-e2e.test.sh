...
# one (tests/herdr-test-safety.sh; bin/xo-herdr-lab.sh owns the isolation).
set -u
...
xo_live_gate default-on XO_HERDR_PI_STALE_REGISTRATION_LIVE_E2E herdr pi jq
...
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/xo-pi-stale.XXXXXX")
...
xo_backend_herdr_server_ensure "$SESSION" || fail "could not start the isolated Herdr lab server"
WS=$(lab workspace create --label xo-pi-stale --cwd "$SCRATCH/cwd" 2>&1) \
...
wait_process_state() {  # <expected> <tries>
  ...
  while [ "$i" -lt "$tries" ]; do
    got=$(xo_backend_herdr_pane_process_state "$SESSION" "$PANE_ID")
...
wait_process_state agent 100 || version_fail \
  "pi is running and registered ($STATUS) but pane process-info reads '$(xo_backend_herdr_pane_process_state "$SESSION" "$PANE_ID")', not 'agent'. Observed foreground: $(herdr pane process-info --pane "$PANE_ID" --session "$SESSION" 2>/dev/null | jq -c '.result.process_info.foreground_processes'). Teach bin/xo-agent-process-lib.sh's xo_agent_process_classify the identity this release actually reports"
...
STATE=$(xo_backend_agent_state herdr "$TARGET")
[ "$STATE" = alive ] || version_fail "a running, registered pi reads '$STATE' rather than 'alive' (registration '$(registered_status)', pane state '$(xo_backend_herdr_pane_agent_state "$SESSION" "$PANE_ID")', process state '$(xo_backend_herdr_pane_process_state "$SESSION" "$PANE_ID")', agent get: $(herdr agent get "$PANE_ID" --session "$SESSION" 2>&1 | tr -d '\n'))"
...
if ! wait_process_state shell 50; then
  ...
  wait_process_state shell 150 || version_fail \
    "pi did not exit to a shell within 40s of /quit; pane process-info reads '$(xo_backend_herdr_pane_process_state "$SESSION" "$PANE_ID")'"
fi
...
STATE=$(xo_backend_agent_state herdr "$TARGET")
BUSY=$(xo_backend_herdr_busy_state "$TARGET")
