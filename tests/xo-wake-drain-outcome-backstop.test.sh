#!/usr/bin/env bash
# tests/xo-wake-drain-outcome-backstop.test.sh - executable regressions for the
...
OUTCOMES="$ROOT/bin/xo-branch-outcome.sh"
TMP_ROOT=$(xo_test_tmproot xo-wake-drain-outcome-backstop-tests)
...
append_outcome() {  # <state> <task> <summary>
  XO_STATE_OVERRIDE="$1" "$OUTCOMES" append \
...
test_branch_annotation_cannot_consume_the_main_resurfacing_backstop() {
  ...
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$$" mode5-backstop \
    || fail "branch owner activation failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" publish mode5-backstop 1 \
    || fail "branch grant publication failed"

  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" > "$branch_out" 2> "$branch_err" \
  ...
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$main_out" \
...
test_same_second_outcome_uses_status_causal_position() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$first_out" \
  ...
  set_mtime "$epoch" "$state/same-second.status"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$second_out" \
...
test_successful_backstop_is_idempotent_without_consuming_delayed_annotation() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$signal_out" 2>/dev/null \
...
SH
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$retry_out" \
...
SH
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$final_out" \
...
test_held_lock_mode_rejects_an_unlocked_caller() {
  ...
  if XO_STATE_OVERRIDE="$state" "$OUTCOMES" processed-init --held-lock > "$out" 2>&1; then
...
test_held_lock_mode_accepts_a_lock_owner_descendant() {
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    ...
    . "$1"
    xo_lock_acquire_wait "$STATE/.branch-outcomes.lock" || exit 1
    trap "xo_lock_release \"$STATE/.branch-outcomes.lock\"" EXIT
    sh -c '"'"'$1 processed-init --held-lock'"'"' _ "$2"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$OUTCOMES" \
...
test_index_self_heal_runs_under_the_outcome_lock() {
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    # shellcheck source=bin/xo-wake-lib.sh
    . "$1"
    xo_lock_try_acquire "$STATE/.branch-outcomes.lock" || exit 1
    trap "xo_lock_release \"$STATE/.branch-outcomes.lock\"" EXIT
    sleep 30
  ...
  XO_STATUS_PRESENTATION_LOCK_TIMEOUT=1 XO_STATE_OVERRIDE="$state" "$DRAIN" > "$busy_out" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$healed_out" \
...
test_backstop_output_is_bounded() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "main drain failed for bounded output"
