#!/usr/bin/env bash
# tests/xo-afk-contract.test.sh - the away-posture record owner
# (bin/xo-afk-contract.sh): the mandate-clause fields, the structural refusal
...
TMP_ROOT=$(xo_test_tmproot xo-afk-contract-tests)
...
contract() {  # <home> <args...>
  local home=$1
  shift
  XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$CONTRACT" "$@"
}
