#!/usr/bin/env bash
# Behavior tests for the semantic busy-state contract (bin/xo-busy-lib.sh and
# its only writer bin/xo-busy-event.sh).
#
...
TMP_ROOT=$(xo_test_tmproot xo-busy-state)
EV="$ROOT/bin/xo-busy-event.sh"
...
test_arm_seeds_busy_spawn() {
  ...
  out=$(xo_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy xo-spawn" ] || fail "seed should classify 'busy xo-spawn', got '$out'"
  pass "arm mints a gen sidecar and seeds busy xo-spawn at seq=1"
}
...
test_apply_advances_seq_and_source() {
  ...
  seq=$(xo_busy_record_read "$state" t1 | awk '{print $4}')
...
test_apply_current_gen_reset() {
  ...
  "$EV" apply "$state" t1 idle --current-gen --source xo-interrupt --event interrupt \
    || fail "apply --current-gen failed"
  out=$(xo_busy_classify tmux w1 pi t1 "$state")
  [ "$out" = "idle xo-interrupt" ] || fail "expected 'idle xo-interrupt', got '$out'"
  "$EV" apply "$state" t1 unknown --current-gen --source xo-recovery --event relaunch \
    || fail "apply unknown failed"
  out=$(xo_busy_classify tmux w1 pi t1 "$state")
  [ "$out" = "unknown xo-recovery" ] || fail "expected 'unknown xo-recovery', got '$out'"
  pass "xo-owned interrupt and recovery events bind to the current gen"
}
...
test_retire_serializes_and_rejects_stale_gen() {
  ...
  out=$(xo_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy xo-spawn" ] || fail "stale retirement changed the new incarnation, got '$out'"
...
# xo-teardown.sh died there after returning the worktree, leaving state/<id>.meta
...
test_stale_lock_broken_under_gnu_stat() {
  ...
  gen=$("$EV" arm "$state" t1)
  fakebin=$(xo_fakebin "$TMP_ROOT/gnu-stat-lock")
...
test_stale_gen_event_rejected() {
  ...
  out=$(xo_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy xo-spawn" ] || fail "stale event must not change the record, got '$out'"
...
test_missing_record_unknown_not_idle() {
  ...
  for h in claude opencode pi pi-signed; do
    out=$(xo_busy_classify tmux w1 "$h" t1 "$state")
    [ "$out" = "unknown missing" ] || fail "$h with no record must be 'unknown missing', got '$out'"
  done
  out=$(xo_busy_classify tmux w1 codex t1 "$state")
...
test_source_mismatch_cross_adapter() {
  ...
  out=$(xo_busy_classify tmux w1 grok t1 "$state")
...
Ctrl+c:cancel'
  for h in claude opencode pi pi-signed; do
    out=$(xo_busy_classify tmux w1 "$h" t1 "$state" "$tail")
    [ "$out" = "unknown missing" ] || fail "$h must never classify from footer text, got '$out'"
  done
  out=$(xo_busy_classify tmux w1 codex t1 "$state" "$tail")
...
test_grok_regex_isolated() {
  ...
  out=$(xo_busy_classify tmux w1 grok t1 "$state" 'thinking hard
Ctrl+c:cancel')
  [ "$out" = "busy grok-regex" ] || fail "grok busy tail must classify 'busy grok-regex', got '$out'"
  out=$(xo_busy_classify tmux w1 grok t1 "$state" 'done.
> ')
  ...
  out=$(xo_busy_classify tmux w1 grok t1 "$state" '• Working (6s • esc to interrupt)')
...
test_codex_unverified_gate() {
  ...
  [ -z "$(xo_busy_sources_for_harness codex)" ] \
...
test_kimi_unverified_gate() {
  ...
  out=$(xo_busy_classify tmux w1 kimi t1 "$state")
  [ "$out" = "unknown kimi-unverified" ] || fail "unverified kimi must classify unknown, got '$out'"
  out=$(xo_busy_classify tmux w1 kimi t1 "$state" '🌒 · thinking')
...
test_cursor_ignores_rendered_and_native_signals() {
  ...
  out=$(xo_busy_classify tmux w1 cursor t1 "$state" 'Working')
  [ "$out" = "unknown cursor-transcript" ] \
    || fail "cursor must not classify from its rendered footer, got '$out'"
  out=$(xo_busy_classify tmux w1 cursor t1 "$state" 'ctrl+c to stop')
  [ "$out" = "unknown cursor-transcript" ] \
  ...
  # shellcheck disable=SC2329 # invoked indirectly through xo_busy_classify
  xo_backend_busy_state() { printf '%s' busy; }
  out=$(xo_busy_classify herdr s:p cursor t1 "$state")
  [ "$out" = "unknown cursor-transcript" ] \
    || fail "cursor must not borrow herdr's native busy verdict, got '$out'"
  unset -f xo_backend_busy_state
  # The fold is a PULL source: nothing is armed, so no stored record is trusted.
  [ -z "$(xo_busy_sources_for_harness cursor)" ] \
...
test_dead_endpoint_overrides() {
  ...
  gen=$("$EV" arm "$state" t1)
  # shellcheck disable=SC2329 # invoked indirectly through xo_busy_classify_live
  xo_backend_target_exists() { return 1; }
  out=$(xo_busy_classify_live tmux w1 claude t1 "$state")
  [ "$out" = "dead endpoint-gone" ] || fail "gone endpoint must classify dead, got '$out'"
  # shellcheck disable=SC2329 # invoked indirectly through xo_busy_classify_live
  xo_backend_target_exists() { return 0; }
  out=$(xo_busy_classify_live tmux w1 claude t1 "$state")
  [ "$out" = "busy xo-spawn" ] || fail "live endpoint must fall through to the record, got '$out'"
  out=$(xo_busy_classify_live tmux '' claude t1 "$state")
  [ "$out" = "unknown no-target" ] || fail "empty target must classify unknown, got '$out'"
  unset -f xo_backend_target_exists
...
test_herdr_native_busy_only() {
  ...
  # shellcheck disable=SC2329 # invoked indirectly through xo_busy_classify
  xo_backend_busy_state() { printf '%s' "$FAKE_NATIVE"; }
  FAKE_NATIVE=busy
  out=$(xo_busy_classify herdr s:p claude t1 "$state")
...
test_record_read_leaves_caller_shell_intact() {
  ...
  out=$(bash -c '
    set -f
    . "$1/bin/xo-busy-lib.sh"
    set -- keepme second
    xo_busy_record_read "$2" t1 >/dev/null
    printf "%s|%s|%s" "$1" "$#" "$-"
  ' _ "$ROOT" "$state")
  case "$out" in
    keepme\|2\|*f*) : ;;
...
test_boolean_view_never_promotes_unknown() {
  ...
  gen=$("$EV" arm "$state" t1)
  xo_busy_is_busy tmux w1 claude t1 "$state" || fail "busy record must read busy"
  "$EV" apply "$state" t1 idle --gen "$gen" --source claude-hook --event stop
  if xo_busy_is_busy tmux w1 claude t1 "$state"; then
...
echo "all xo-busy-state tests passed"
