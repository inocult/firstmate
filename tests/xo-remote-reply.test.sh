#!/usr/bin/env bash
# End-to-end remote reply relay through xo-on and the process-event runner.
set -u
...
TMP_ROOT=$(xo_test_tmproot xo-remote-reply)
mkdir -p "$TMP_ROOT"
...
cleanup() {
  ...
  if [ -f "$TMP_ROOT/remote-jobs/worker.pid" ]; then
    ...
    xo_remote_job_stop_worker_tree "$worker_pid" || true
  fi
  rm -rf -- "$TMP_ROOT"
}
trap cleanup EXIT
...
remote_env() {
  ...
  XO_FAKE_REMOTE_ENTRYPOINT="$ROOT/bin/xo-remote-entrypoint.sh" \
  XO_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  XO_REMOTE_JOB_STATE_ROOT="$TMP_ROOT/remote-jobs" \
  XO_REMOTE_REPLY_WAIT_SECONDS=10 \
  "$@"
}
...
ADAPTER="$ROOT/bin/xo-procevent-remote-reply.sh"
...
remote_env "$ROOT/bin/xo-procevent.sh" start "$SID" > "$TMP_ROOT/start-one.out" 2>&1 &
RUNNER=$!
...
XO_STATE_OVERRIDE="$PARENT/state" bash -c '
  . "$1/bin/xo-wake-lib.sh"
  xo_wake_signal_seen_current "$2/state" "$2/state/ios.status"
...
reconcile_out=$(remote_env "$ROOT/bin/xo-procevent.sh" reconcile)
...
PENDING_CORR=$(xo_pending_reply_create "$PARENT" "$PARENT/state" ios 'audit the release chain')
[ -n "$PENDING_CORR" ] || fail "could not create the parent pending-reply record"
xo_pending_reply_mark_delivered "$PARENT/state" "$PENDING_CORR" \
...
[ "$(xo_pending_reply_get "$PARENT/state/pending-replies/$PENDING_CORR" phase)" = resolved ] \
...
remote_env "$ROOT/bin/xo-procevent.sh" start "$SID" >/dev/null 2>&1 \
...
ESCALATED_CORR=$(xo_pending_reply_create "$PARENT" "$PARENT/state" ios 'confirm the notarization')
[ -n "$ESCALATED_CORR" ] || fail "could not create the pending-reply record to escalate"
xo_pending_reply_mark_delivered "$PARENT/state" "$ESCALATED_CORR" \
  || fail "could not mark the escalating request delivered"
xo_pending_reply_mark_turn_completed "$PARENT/state" "$ESCALATED_CORR" request
XO_PENDING_REPLY_SEND_HOOK=true \
  xo_pending_reply_send_recovery "$PARENT/state" "$ESCALATED_CORR" \
  || fail "the one automatic recovery repost was not sent"
xo_pending_reply_mark_turn_completed "$PARENT/state" "$ESCALATED_CORR" recovery
xo_pending_reply_maybe_escalate "$PARENT/state" "$ESCALATED_CORR" \
...
[ "$(xo_pending_reply_get "$PARENT/state/pending-replies/$ESCALATED_CORR" phase)" = resolved ] \
  || fail "the correlated reply left its escalated request unresolved"
xo_pending_reply_tick "$PARENT/state" || fail "supervision tick failed"
...
unset XO_PENDING_REPLY_GRACE_SECS
...
for _ in $(seq 1 100); do
  for job in "$TMP_ROOT"/remote-jobs/jobs/job-*; do
    [ -d "$job" ] || continue
    if [ "$(xo_remote_job_read_state "$job" 2>/dev/null || true)" = running ]; then
      running_poll=$job
      break 2
    fi
  done
...
remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-file.sh get data/reply/report.md 262144 >/dev/null
set +e
wait "$PREEMPTED_SOURCE"
preempted_rc=$?
set -e
[ "$preempted_rc" -eq "$XO_REMOTE_JOB_PREEMPTED_EXIT" ] \
...
set +e
XO_REMOTE_REPLY_WAIT_SECONDS=1 remote_env "$ADAPTER" source ios >/dev/null 2>&1
quiet_rc=$?
set -e
[ "$quiet_rc" -eq 75 ] || fail "a quiet reply window exited with an unexpected status: $quiet_rc"
watermark_after=$(date +%s)
caught_up=$(XO_STATE_OVERRIDE="$PARENT/state" bash -c '
  . "$1/bin/xo-pending-reply-lib.sh"
  xo_pending_reply_remote_channel_epoch "$2/state" ios
' _ "$ROOT" "$PARENT")
...
XO_STATE_OVERRIDE="$PARENT/state" bash -c '
  . "$1/bin/xo-wake-lib.sh"
  xo_wake_status_mark_current "$2/state" "$2/state/ios.status"
...
remote_env "$ROOT/bin/xo-procevent.sh" start "$SID" > "$TMP_ROOT/start-two.out" 2>&1 &
RUNNER=$!
