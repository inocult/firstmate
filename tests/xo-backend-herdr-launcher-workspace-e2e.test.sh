#!/usr/bin/env bash
# tests/xo-backend-herdr-launcher-workspace-e2e.test.sh - mandatory ISOLATED
...
# workspace of the xo or secondmate process that launched it, identified
...
# "xo", and the previous label-first-match resolution put the worker in
# whichever one sorted first - visibly the wrong space whenever the launcher was
# not in it.
#
# This drives the REAL bin/xo-spawn.sh and bin/xo-teardown.sh, because the
# guarantee spans the whole spawn handoff (xo-spawn.sh's herdr arm ->
# xo_backend_herdr_container_ensure -> xo_backend_herdr_workspace_ensure ->
# xo_backend_herdr_launcher_identity) and no adapter primitive holds it alone.
# The headline duplicate-label case additionally runs xo-spawn.sh INSIDE a real
...
# operation goes through bin/xo-herdr-lab.sh, which appends the named session
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/xo-herdr-launcher-e2e.XXXXXX")
HERDR_LAB_HELPER="$ROOT/bin/xo-herdr-lab.sh"
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name xo-herdr-launcher-ws) || {
  rm -rf "$TMP_ROOT"
...
# spawn_from_launcher <launcher-pane|""> <home> <task-id> <project> [extra xo-spawn args...]
# Composes exactly the Herdr identity Herdr itself injects into a pane's
# processes. An empty launcher pane means "this xo is not running inside
# Herdr at all".
...
spawn_from_launcher() {
  ...
  if [ -n "$pane" ]; then
    env HERDR_ENV=1 HERDR_PANE_ID="$pane" HERDR_SESSION="$HERDR_LAB_SESSION" \
      ...
      XO_SPAWN_NO_GUARD=1 XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
      "$ROOT/bin/xo-spawn.sh" "$id" "$proj" "sh -c 'echo launcher-ws-ok'" --backend herdr "$@" \
...
printf '%s\n' "$SM_ID" > "$SM_HOME/.xo-secondmate-home"
...
printf '# scratch secondmate home AGENTS.md placeholder\n' > "$SM2_HOME/AGENTS.md"
printf '%s\n' "$SM2_ID" > "$SM2_HOME/.xo-secondmate-home"
...
[ "$(label_of_workspace "$WS_PRIMARY")" = xo ] || fail "uniqA did not land in a 'xo' workspace"
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "the spawn stole focus from the captain's workspace"
pass "real herdr E2E: with one 'xo' workspace and no herdr parent, a crewmate still lands in this home's own workspace without stealing focus"
...
EOF
[ -n "$LAUNCH_PRIMARY_PANE" ] || fail "could not create a launcher pane inside the 'xo' workspace"
...
[ "$(workspace_of_pane "$UNIQB_PANE")" = "$WS_PRIMARY" ] \
  || fail "a crewmate launched from the 'xo' workspace must stay in it"
...
$(make_workspace xo)
EOF
[ -n "$WS_PRIMARY_DUP" ] || fail "could not create the second 'xo' workspace"
[ "$WS_PRIMARY_DUP" != "$WS_PRIMARY" ] || fail "the two 'xo' workspaces must be distinct"
DUP_COUNT=$(lab workspace list 2>/dev/null | jq -r '[.result.workspaces[]? | select(.label == "xo")] | length')
[ "$DUP_COUNT" = 2 ] || fail "expected exactly two 'xo' workspaces, got $DUP_COUNT"
...
XO_SPAWN_NO_GUARD=1 XO_HOME="$PRIMARY_HOME" XO_ROOT_OVERRIDE="$ROOT" \\
  "$ROOT/bin/xo-spawn.sh" dupC "$PROJ" "sh -c 'echo launcher-ws-ok'" --mode no-mistakes --yolo off --backend herdr \\
...
lab pane run "$LAUNCH_DUP_PANE" "$TMP_ROOT/spawn-in-pane.sh" >/dev/null 2>&1 \
  || fail "could not run xo-spawn.sh inside the launcher's herdr pane"
i=0
while [ ! -f "$TMP_ROOT/dupC.rc" ] && [ "$i" -lt 120 ]; do sleep 2; i=$((i + 1)); done
[ -f "$TMP_ROOT/dupC.rc" ] || fail "xo-spawn.sh never finished inside the launcher's herdr pane"
...
[ "$DUPC_WS" = "$WS_PRIMARY_DUP" ] \
  || fail "a worker launched from the second 'xo' workspace ($WS_PRIMARY_DUP) landed in '$DUPC_WS' instead"
...
pass "real herdr E2E: with two 'xo' workspaces, a worker spawned from inside the second one lands in that exact workspace"
...
[ "$(label_of_workspace "$WS_PRIMARY")" = xo ] \
...
assert_contains_local "$(cat "$SPAWN_ERR")" "labeled 'xo'" \
...
DUP_TABS=$(lab tab list --workspace "$WS_PRIMARY" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "xo-dupD")] | length')
DUP_TABS2=$(lab tab list --workspace "$WS_PRIMARY_DUP" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "xo-dupD")] | length')
...
# An xo whose own pane was closed under it has an identity that no longer
...
STALE_TABS=$(lab tab list --workspace "$WS_PRIMARY_DUP" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "xo-staleF")] | length')
...
XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$PRIMARY_HOME/state" XO_DATA_OVERRIDE="$PRIMARY_HOME/data" \
  ...
  "$ROOT/bin/xo-teardown.sh" dupC >"$TMP_ROOT/teardown.out" 2>&1
status=$?
[ "$status" -eq 0 ] || fail "xo-teardown.sh failed for dupC"$'\n'"$(cat "$TMP_ROOT/teardown.out")"
[ ! -f "$DUPC_META" ] || fail "xo-teardown.sh did not remove dupC's meta"
if lab pane get "$DUPC_PANE" >/dev/null 2>&1; then
  fail "xo-teardown.sh did not close dupC's own pane"
fi
...
[ "$(label_of_workspace "$WS_PRIMARY_DUP")" = xo ] || fail "teardown removed or renamed the launcher's workspace"
