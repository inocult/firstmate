...
xo_live_gate opt-in XO_GROK_LIVE_E2E grok tmux
...
"$TMUX" -L "$SOCKET" new-session -d -s "$SESSION" -c "$PROJECT" \
  "env XO_HOME='$HOME_DIR' XO_ROOT_OVERRIDE='$PROJECT' XO_POLL=1 XO_SIGNAL_GRACE=0 XO_HEARTBEAT=600 bash -lc 'printf \"%s\\n\" \"\$\$\" > \"\$XO_HOME/state/.lock\"; grok --trust --always-approve --reasoning-effort low; rc=\$?; printf \"GROK_EXIT=%s\\n\" \"\$rc\"; sleep 300'"

wait_for_text "Grok Build" 180 || fail "Grok did not reach its ready composer"
sleep 1
# shellcheck disable=SC2016 # Backticks are literal prompt markup.
PROMPT='Use run_terminal_command with background=true to run exactly `bin/xo-watch-arm.sh`. Never use a shell ampersand. Once it reports started, respond briefly.'
