...
. "$ROOT/bin/xo-pr-lib.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/xo-check-lib.sh"

PR_CHECK="$ROOT/bin/xo-pr-check.sh"
PR_MERGE="$ROOT/bin/xo-pr-merge.sh"
POLL="$ROOT/bin/xo-pr-poll.sh"
WATCH="$ROOT/bin/xo-watch.sh"
TEARDOWN="$ROOT/bin/xo-teardown.sh"
REGISTER="$ROOT/bin/xo-check-register.sh"
TMP_ROOT=$(xo_test_tmproot xo-pr-check-security)
...
ack_watcher_cycle() {  # <state>
  ...
  XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-wake-drain.sh" >/dev/null 2> "$err" || return 1
  sequence=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through \([0-9][0-9]*\) --recovery-generation [A-Za-z0-9._-][A-Za-z0-9._-]*$/\1/p' "$err")
  generation=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through [0-9][0-9]* --recovery-generation \([A-Za-z0-9._-][A-Za-z0-9._-]*\)$/\1/p' "$err")
  rm -f "$err"
  [ -n "$sequence" ] && [ -n "$generation" ] || return 1
  XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-wake-drain.sh" --ack-through "$sequence" \
...
make_case() {
  ...
  cat > "$fake_root/bin/xo-guard.sh" <<'SH'
#!/usr/bin/env bash
printf 'guard\n' >> "$XO_TEST_GUARD_LOG"
SH
  chmod +x "$fake_root/bin/xo-guard.sh"
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$XO_TEST_GH_LOG"
case "${1:-} ${2:-}" in
  ...
  "pr view")
    case " $* " in
      *statusCheckRollup*)
        printf '%s\n' "{\"state\":\"OPEN\",\"isDraft\":false,\"mergeable\":\"MERGEABLE\",\"mergeStateStatus\":\"CLEAN\",\"headRefOid\":\"${XO_TEST_GH_HEAD:-0123456789abcdef0123456789abcdef01234567}\",\"statusCheckRollup\":[{\"__typename\":\"CheckRun\",\"name\":\"ci\",\"status\":\"COMPLETED\",\"conclusion\":\"SUCCESS\"}]}"
        exit 0
        ;;
    esac
    ;;
...
case " $* " in
  *" headRefOid "*) printf '%s\n' "${XO_TEST_GH_HEAD:-0123456789abcdef0123456789abcdef01234567}" ;;
  *" state "*)
    [ "${XO_TEST_GH_FAIL:-0}" = 0 ] || exit 1
    [ "${XO_TEST_GH_SLEEP:-0}" = 0 ] || sleep "$XO_TEST_GH_SLEEP"
    printf '%s\n' "${XO_TEST_GH_STATE:-OPEN}"
    ;;
esac
SH
...
case "${1:-} ${2:-}" in
  "pr view")
    [ "$#" -eq 5 ] && [ "${4:-}" = --repo ] || exit 2
    printf 'pull_request:\n  number: %s\n  state: %s\n' "$3" "${XO_TEST_GH_MERGE_STATE:-merged}"
    ;;
esac
exit "${XO_TEST_GH_AXI_RC:-0}"
SH
...
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$XO_TEST_GLAB_LOG"
[ "${XO_TEST_GLAB_FAIL:-0}" = 0 ] || exit 1
[ "${XO_TEST_GLAB_SLEEP:-0}" = 0 ] || sleep "$XO_TEST_GLAB_SLEEP"
printf 'title:\tfixture merge request\nstate:\t%s\nauthor:\tsomeone\n' "${XO_TEST_GLAB_STATE:-opened}"
SH
...
write_task_meta() {
  ...
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=xo:xo-$id" \
    "endpoint_task_id=$id" \
    "worktree=$dir/wt" \
    "project=$dir/project" \
...
run_check_entry() {
  local dir=$1
  shift
  XO_ROOT_OVERRIDE="$dir/root" XO_HOME="$dir/home" \
    XO_TEST_GUARD_LOG="$dir/guard.log" XO_TEST_GH_LOG="$dir/gh.log" \
    XO_TEST_GH_AXI_LOG="$dir/gh-axi.log" XO_TEST_GLAB_LOG="$dir/glab.log" \
...
test_parser_matrix() {
  ...
  while IFS='|' read -r url owner repo number; do
    [ -n "$url" ] || continue
    xo_pr_url_parse "$url" || fail "parser rejected canonical URL"
    [ "$XO_PR_URL" = "$url" ] || fail "parser changed canonical URL"
    [ "$XO_PR_OWNER" = "$owner" ] || fail "parser returned wrong owner"
    [ "$XO_PR_REPO" = "$repo" ] || fail "parser returned wrong repository"
    [ "$XO_PR_NUMBER" = "$number" ] || fail "parser returned wrong PR number"
  done <<'EOF'
...
EOF
  while IFS='|' read -r url host path number; do
    [ -n "$url" ] || continue
    xo_pr_url_parse "$url" || fail "parser rejected a canonical merge request URL"
    [ "$XO_PR_PROVIDER" = gitlab ] || fail "parser did not tag a merge request URL as gitlab"
    [ "$XO_PR_URL" = "$url" ] || fail "parser changed a canonical merge request URL"
    [ "$XO_PR_HOST" = "$host" ] || fail "parser returned wrong GitLab host"
    [ "$XO_PR_PATH" = "$path" ] || fail "parser returned wrong GitLab project path"
    [ "$XO_PR_NUMBER" = "$number" ] || fail "parser returned wrong merge request number"
    [ -z "$XO_PR_OWNER" ] && [ -z "$XO_PR_REPO" ] \
...
EOF
  xo_pr_url_parse https://github.com/a/b/pull/1 || fail "parser rejected canonical URL"
  [ "$XO_PR_PROVIDER" = github ] || fail "parser did not tag a pull request URL as github"
  [ "$XO_PR_HOST" = github.com ] || fail "parser returned wrong GitHub host"
  [ "$XO_PR_PATH" = a/b ] || fail "parser returned wrong GitHub project path"
  for row in "${INVALID_URLS[@]}"; do
    ! xo_pr_url_parse "$row" || fail "parser accepted a rejected raw-byte URL class"
  done
  for id in -task task- task--a Task-a task_a task.a; do
    xo_pr_task_id_valid "$id" || fail "task ID validator rejected a safe lifecycle-compatible slug"
  done
  xo_task_id_creation_valid _noncanonical \
  ...
  xo_pr_task_id_valid "$id" || fail "operational validator rejected a path-safe legacy task ID"
  ! xo_task_id_creation_valid "$id" || fail "creation validator accepted an overlong task ID"
...
test_invalid_entrypoints_have_zero_side_effects() {
  ...
  for value in "${UNSAFE_LIFECYCLE_IDS[@]}"; do
    before=$(state_snapshot "$dir/home/state")
    set +e
    XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$dir/root" XO_TEST_GUARD_LOG="$dir/guard.log" \
...
test_valid_recording_and_merge_derivation() {
  ...
  XO_TEST_GH_HEAD=$expected run_check_entry "$dir" task-a https://github.com/my-org/repo_name.with-dots/pull/37 \
  ...
  [ "$(xo_pr_file_link_count "$dir/home/state/task-a.check.sh")" = 1 ] \
    && [ "$(xo_pr_file_link_count "$dir/home/state/task-a.pr-poll")" = 1 ] \
    && [ "$(xo_pr_file_link_count "$dir/home/state/task-a.pr-poll-registration")" = 1 ] \
    || fail "published poll artifacts were not single-link files"
  xo_pr_poll_artifacts_valid "$dir/home/state" task-a "$POLL" \
  ...
  set +e
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/merged-watch.out" 2> "$dir/merged-watch.err"
  rc=$?
  set -e
  ...
  xo_pr_poll_artifacts_valid "$dir/home/state" Task_A.1 "$POLL" \
...
SH
  ...
  XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" PATH="$dir/fakebin:$BASE_PATH" \
...
SH
    ...
    XO_HOME="$dir/home" "$ROOT/bin/xo-x-link.sh" "$id" req-legacy \
    ...
    xo_pr_poll_artifacts_valid "$dir/home/state" "$id" "$POLL" \
...
run_watcher_bounded() {
  local home=$1 fakebin=$2 check_interval=${XO_TEST_CHECK_INTERVAL:-0} watch_root=${XO_TEST_WATCH_ROOT:-$ROOT}
  shift 2
  perl -e 'my $pid=fork; die unless defined $pid; if (!$pid) { exec @ARGV } local $SIG{ALRM}=sub { kill "TERM", $pid; waitpid $pid, 0; exit 124 }; alarm 10; waitpid $pid, 0; alarm 0; exit($? >> 8)' \
    env XO_HOME="$home" XO_ROOT_OVERRIDE="$watch_root" XO_CHECK_INTERVAL="$check_interval" XO_CHECK_TIMEOUT=1 \
      XO_POLL=0.02 XO_HEARTBEAT=999999 XO_SIGNAL_GRACE=0 PATH="$fakebin:$BASE_PATH" "$WATCH" "$@"
}
...
test_rejected_metacharacter_bytes_are_inert() {
  ...
  for family in "${families[@]}"; do
    ...
    xo_pr_poll_prepare "$dir/home/state" safe-check github https://github.com/o/r/pull/99 github.com o/r 99 "$POLL" \
      || fail "could not prepare bounded watcher poll"
    xo_pr_poll_publish_prepared || fail "could not publish bounded watcher poll"

    set +e
    XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
    rc=$?
    set -e
  ...
  XO_TEST_GH_STATE=OPEN run_check_entry "$dir" task-a https://github.com/o/r/pull/1 >/dev/null 2>/dev/null \
...
run_poll() {
  local dir=$1
  XO_TEST_GH_LOG="$dir/gh.log" XO_TEST_GLAB_LOG="$dir/glab.log" \
    PATH="$dir/fakebin:$BASE_PATH" \
...
test_static_poll_contract() {
  ...
  make_poll_fixture "$dir"
  set +e
  out=$(XO_STATE_OVERRIDE="$dir/home/state" XO_CHECK_TIMEOUT=1 XO_TEST_GH_LOG="$dir/gh.log" \
    XO_TEST_GH_SLEEP=3 PATH="$dir/fakebin:$BASE_PATH" \
  ...
  xo_pr_poll_prepare "$dir/home/state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
    || fail "could not prepare authenticated watcher poll"
  xo_pr_poll_publish_prepared || fail "could not publish authenticated watcher poll"
  rm -f "$dir/home/state/.last-check"
  set +e
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
...
SH
  ...
  ! find "$dir/home/state" -name '.xo-pr-poll-*' -print | grep . >/dev/null \
...
SH
    ...
    direct_pid=$!
    i=0
    while [ "$i" -lt 100 ] && ! find "$dir/home/state" -name '.xo-pr-poll-check.*' -print | grep . >/dev/null; do
      sleep 0.01
      i=$((i + 1))
    done
...
test_poll_publication_refuses_unsafe_destinations() {
  ...
  for artifact in task-a.pr-poll task-a.pr-poll-registration task-a.check.sh; do
    for kind in regular dangling directory; do
      ...
      xo_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
        || fail "could not stage poll symlink refusal fixture"
      destination="$state/$artifact"
      make_private_symlink "$dir" "$destination" "$kind"
      if xo_pr_poll_publish_prepared; then
...
test_live_artifact_single_link_and_privacy_validation() {
  ...
  for artifact in check.sh pr-poll pr-poll-registration; do
    ...
    xo_pr_poll_artifacts_valid "$state" task-a "$POLL" \
  ...
  set +e
  XO_HOME="$dir/home" "$REGISTER" custom > "$dir/register.out" 2> "$dir/register.err"
  rc=$?
  set -e
...
#!/usr/bin/env bash
last=${!#}
"${XO_TEST_REAL_MV:?}" "$@" || exit $?
[ "$last" = "${XO_TEST_FINAL_PATH:?}" ] || exit 0
case "${XO_TEST_FINAL_ACTION:?}" in
  type)
    rm -f -- "$last"
    ln -s "${XO_TEST_FAULT_LINK_TARGET:?}" "$last"
    ;;
  mode) "${XO_TEST_REAL_CHMOD:?}" 0644 "$last" ;;
  content) printf 'faulted final bytes\n' > "$last" ;;
  device) : > "${XO_TEST_FAULT_GATE:?}" ;;
  *) exit 2 ;;
esac
SH
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${XO_TEST_FINAL_PATH:-}" ] && [ -e "${XO_TEST_FAULT_GATE:-/nonexistent}" ]; then
  case " $* " in
    *" %d "*) printf '%s\n' 999999; exit 0 ;;
  esac
fi
exec "${XO_TEST_REAL_STAT:?}" "$@"
SH
...
test_postrename_poll_validation_revokes_and_retries() {
  ...
  for artifact in data registration check; do
    for action in type mode device content; do
      ...
      xo_pr_poll_publish_prepared || fail "could not publish prior poll"
      write_poll_meta "$state" task-a https://github.com/o/r/pull/2
      xo_pr_poll_prepare "$state" task-a github https://github.com/o/r/pull/2 github.com o/r 2 "$POLL" \
      ...
      if XO_TEST_FINAL_PATH="$destination" XO_TEST_FINAL_ACTION="$action" \
        XO_TEST_FAULT_LINK_TARGET="$link_target" XO_TEST_FAULT_GATE="$gate" \
        XO_TEST_REAL_MV="$REAL_MV" XO_TEST_REAL_STAT="$REAL_STAT" XO_TEST_REAL_CHMOD="$REAL_CHMOD" \
        PATH="$dir/fakebin:$BASE_PATH" xo_pr_poll_publish_prepared; then
      ...
      PATH="$BASE_PATH" xo_pr_poll_publish_prepared || fail "poll retry did not recover after final validation fault"
      xo_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "poll retry did not publish a valid pair"
    done
  done
...
test_bootstrap_leaves_unauthenticated_checks() {
  ...
  state="$dir/home/state"
  xo_write_meta "$state/task-a.meta" \
    'window=xo-task-a' \
  ...
  XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" XO_BOOTSTRAP_NETWORK=skip \
    PATH="$dir/fakebin:$BASE_PATH" \
    "$ROOT/bin/xo-bootstrap.sh" > "$dir/bootstrap.out" 2> "$dir/bootstrap.err" \
...
test_custom_snapshot_cleanup_on_signal() {
  ...
  printf '%s\n' '#!/usr/bin/env bash' 'trap "" TERM' \
    'printf "%s\n" "$$" > "$XO_TEST_CUSTOM_CHILD_PID"' 'while :; do sleep 1; done' \
    > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
...
SH
  ...
  XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" XO_POLL=0 XO_CHECK_INTERVAL=0 \
    XO_SIGNAL_GRACE=0 XO_TEST_CUSTOM_CHILD_PID="$child_pid_file" \
  ...
  find "$state" -maxdepth 1 -name '.xo-custom-check.*' -print | grep . >/dev/null \
  ...
  ! find "$state" -maxdepth 1 -name '.xo-check-output.*' -print | grep . >/dev/null \
...
#!/usr/bin/env bash
perl -e '$SIG{TERM}="IGNORE"; open my $ready, ">", $ENV{XO_TEST_DESCENDANT_READY} or die $!; print {$ready} "ready\n"; close $ready; select undef, undef, undef, 4; open my $sentinel, ">", $ENV{XO_TEST_DESCENDANT_SENTINEL} or die $!; print {$sentinel} "late\n"; close $sentinel; select undef, undef, undef, 1' &
printf '%s\n' "$!" > "$XO_TEST_DESCENDANT_PID"
while [ ! -s "$XO_TEST_DESCENDANT_READY" ]; do sleep 0.01; done
: > "$XO_TEST_DIRECT_DONE"
SH
    chmod 0700 "$state/custom.check.sh"
    XO_HOME="$dir/home" "$REGISTER" custom >/dev/null \
...
SH
    ...
    XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" XO_POLL=0.1 XO_CHECK_INTERVAL=999999 \
      XO_CHECK_TIMEOUT=10 XO_HEARTBEAT=999999 XO_SIGNAL_GRACE=0 \
      XO_CHECK_FORCE_FALLBACK="$force_fallback" XO_TEST_DESCENDANT_READY="$ready" \
      XO_TEST_DESCENDANT_SENTINEL="$sentinel" XO_TEST_DESCENDANT_PID="$child_pid_file" \
      XO_TEST_DIRECT_DONE="$direct_done" PATH="$fakebin:$BASE_PATH" "$WATCH" \
      > "$dir/watch.out" 2> "$dir/watch.err" &
    watcher_pid=$!
    i=0
    while [ "$i" -lt 200 ]; do
...
test_teardown_removes_poll_artifacts() {
  ...
  fakebin="$dir/fakebin"
  xo_write_meta "$dir/home/state/task-a.meta" \
    'window=xo:xo-task-a' \
...
SH
  ...
  XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" PATH="$fakebin:$BASE_PATH" \
  ...
  xo_pr_poll_snapshot_capture "$dir/home/state" task-a "$POLL" \
    || fail "could not snapshot teardown receipt fixture"
  xo_pr_poll_retirement_publish "$dir/home/state" task-a "$POLL" merged \
...
SH
    ...
    touch "$dir/home/state/.last-watcher-beat"
    set +e
    XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" XO_FAKE_TMUX_LOG="$dir/tmux.log" \
...
test_gitlab_merge_watch() {
  ...
  xo_pr_poll_prepare "$state" task-a gitlab "$url" gitlab.example group/subgroup/project 7 "$POLL" \
    || fail "could not prepare a GitLab poll"
  xo_pr_poll_publish_prepared || fail "could not publish a GitLab poll"
  xo_pr_poll_artifacts_valid "$state" task-a "$POLL" \
...
7" ] || fail "published GitLab sidecar bytes were not exact"
  ...
  # Only an exact merged state wakes xo. Every other reading, including
  ...
  for value in opened closed locked '' not-a-state MERGED merged-but-not; do
    out=$(XO_TEST_GLAB_STATE="$value" run_poll "$dir")
    [ -z "$out" ] || fail "GitLab poll emitted for a non-merged state"
  done
  out=$(XO_TEST_GLAB_STATE=merged run_poll "$dir")
  [ "$out" = merged ] || fail "GitLab poll did not emit exactly one merged line"
  out=$(XO_TEST_GLAB_FAIL=1 run_poll "$dir")
...
EOF
  ...
  out=$(XO_TEST_GLAB_STATE=merged XO_TEST_GH_LOG="$dir/gh.log" XO_TEST_GLAB_LOG="$dir/glab.log" \
    PATH="$noglab" \
  ...
  set +e
  out=$(XO_ROOT_OVERRIDE="$dir/root" XO_HOME="$dir/home" \
    XO_TEST_GUARD_LOG="$dir/guard.log" PATH="$noglab" \
...
seed_canonical_poll() {
  ...
  state="$dir/home/state"
  xo_pr_url_parse "$url" || fail "retirement fixture URL was invalid"
  provider=$XO_PR_PROVIDER
  host=$XO_PR_HOST
  path=$XO_PR_PATH
  number=$XO_PR_NUMBER
  xo_pr_poll_prepare "$state" "$id" "$provider" "$url" "$host" "$path" "$number" "$template" \
    || fail "could not prepare retirement fixture"
  xo_pr_poll_publish_prepared || fail "could not publish retirement fixture"
}
...
add_stop_custom_check() {
  ...
  XO_HOME="$dir/home" "$REGISTER" z-stop >/dev/null \
...
test_merged_poll_retires_once() {
  ...
  set +e
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-1.out" 2> "$dir/watch-1.err"
  rc=$?
  set -e
  ...
  set +e
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-2.out" 2> "$dir/watch-2.err"
  rc=$?
  set -e
...
# surfaced (e.g. bin/xo-pr-check.sh re-armed after the fact). The per-task
# merge-notified marker (bin/xo-pr-lib.sh) is what stops that re-registration
...
test_merged_poll_reregistration_after_notification_is_absorbed() {
  ...
  # Re-registration: xo-pr-check.sh re-armed for a task whose PR is already
...
# this home performed: bin/xo-merge-outcome-lib.sh carries both to the parent on
...
seed_secondmate_home() {  # <dir> [<route>]
  ...
  printf '%s\n' mate-x > "$dir/home/.xo-secondmate-home"
  printf 'schema=xo-secondmate-parent.v1\nroute=%s\n' "$route" \
    > "$dir/home/.xo-secondmate-parent"
}
...
test_merged_poll_retries_a_failed_upward_report() {
  ...
  set +e
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" \
...
exec "$XO_TEST_REAL_MV" "$@"
SH
  chmod +x "$dir/fakebin/mv"
  set +e
  XO_TEST_REAL_MV="$REAL_MV" run_merge_entry "$dir" task-a "$url" \
    >"$dir/merge.out" 2>"$dir/merge.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] \
  ...
  xo_pr_poll_merge_already_notified "$state" task-a github github.com o/r 1 \
...
test_different_merged_pr_for_same_task_is_not_absorbed() {
  ...
  xo_pr_poll_merge_already_notified "$state" task-a github github.com o/r 2 \
...
test_persistent_secondmate_retirement_is_poll_only() {
  ...
  state="$dir/home/state"
  xo_write_meta "$state/domain.meta" \
    'window=session:xo-domain' \
...
test_retirement_crash_recovery() {
  ...
  XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_wake_append check "$2" "$3"' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$state/task-a.check.sh" "check: $state/task-a.check.sh: merged" \
    || fail "could not seed post-queue crash"
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/recovery.out" 2> "$dir/recovery.err" \
  ...
  XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" "$ROOT/bin/xo-wake-drain.sh" > "$dir/drain.out" 2>/dev/null
  ...
  xo_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt crash fixture"
  xo_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish crash receipt"
  add_stop_custom_check "$dir"
  set +e
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/restart.out" 2> "$dir/restart.err"
  rc=$?
  set -e
  ...
  xo_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot partial removal fixture"
  xo_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish partial removal receipt"
  ...
  assert_poll_absent "$state" task-a
  xo_pr_poll_retirement_recover_one "$state" task-a "$POLL" || fail "completed retirement was not idempotent"

  dir=$(make_case retirement-after-registration-removal)
  state="$dir/home/state"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/5
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/5
  xo_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot sidecar-removal fixture"
  xo_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish sidecar-removal receipt"
  ...
  write_poll_meta "$state" task-a https://github.com/o/r/pull/5
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/5
  xo_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt-only fixture"
  xo_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish receipt-only fixture"
  ...
  state="$dir/home/state"
  historical_poll="$dir/historical-xo-pr-poll.sh"
  ...
  xo_pr_poll_snapshot_capture "$state" task-a "$historical_poll" \
    || fail "could not snapshot pre-update retirement fixture"
  xo_pr_poll_retirement_publish "$state" task-a "$historical_poll" merged \
    || fail "could not publish pre-update retirement receipt"
  add_stop_custom_check "$dir"
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/template-recovery.out" 2> "$dir/template-recovery.err" \
...
test_external_merge_transition_retires_only_terminal_poll() {
  ...
  for label in open-green open-red closed-unmerged forge-error malformed; do
    ...
    case "$label" in
      open-green|open-red)
        XO_TEST_GH_STATE=OPEN run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      closed-unmerged)
        XO_TEST_GH_STATE=CLOSED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      forge-error)
        XO_TEST_GH_FAIL=1 run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      malformed)
        XO_TEST_GH_STATE=NOT_A_FORGE_STATE run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
    esac
    rc=$?
    set -e
  ...
  set +e
  XO_TEST_GH_STATE=MERGED run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/merged.out" 2> "$dir/merged.err"
  rc=$?
  set -e
...
test_retirement_refuses_replacement_and_nonterminal_results() {
  ...
  xo_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot replacement fixture"
  write_poll_meta "$state" task-a https://github.com/o/r/pull/7
  seed_canonical_poll "$dir" task-a https://github.com/o/r/pull/7
  before=$(state_snapshot "$state")
  xo_pr_poll_retirement_publish "$state" task-a "$POLL" merged \
  ...
  xo_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "replacement poll was not left canonical"

  xo_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot nonterminal fixture"
  for result in OPEN CLOSED ERROR 'merged extra'; do
    xo_pr_poll_retirement_publish "$state" task-a "$POLL" "$result" \
  ...
  xo_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot rearm-race fixture"
  xo_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish rearm-race receipt"
  ...
  xo_pr_poll_retirement_recover_one "$state" task-a "$POLL" || fail "stale receipt did not yield to a canonical replacement"
  ...
  xo_pr_poll_artifacts_valid "$state" task-a "$POLL" || fail "replacement poll lost canonical provenance"

  dir=$(make_case retirement-template-update-rearm-race)
  state="$dir/home/state"
  historical_poll="$dir/historical-xo-pr-poll.sh"
  current_poll="$dir/current-xo-pr-poll.sh"
  cp "$POLL" "$historical_poll"
  ...
  replacement_registration=$(shasum -a 256 "$state/task-a.pr-poll-registration")
  replacement_meta=$(shasum -a 256 "$state/task-a.meta")
  xo_pr_poll_retirement_recover_one "$state" task-a "$current_poll" \
  ...
  xo_pr_poll_artifacts_valid "$state" task-a "$current_poll" || fail "updated replacement poll lost canonical provenance"

  dir=$(make_case custom-merged-not-retired)
  state="$dir/home/state"
  printf '#!/usr/bin/env bash\nprintf "merged\\n"\n' > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
  XO_HOME="$dir/home" "$REGISTER" custom >/dev/null || fail "could not register merged custom check"
  set +e
...
test_retirement_queue_failure_and_receipt_tampering() {
  ...
  before=$(poll_artifact_snapshot "$state" task-a)
  set +e
  XO_TEST_GH_LOG="$dir/gh.log" XO_TEST_GH_STATE=MERGED \
  ...
  xo_pr_poll_snapshot_capture "$state" task-a "$POLL" || fail "could not snapshot receipt tamper fixture"
  xo_pr_poll_retirement_publish "$state" task-a "$POLL" merged || fail "could not publish receipt tamper fixture"
  printf 'extra\n' >> "$state/task-a.pr-poll-retirement"
  before=$(state_snapshot "$state")
  xo_pr_poll_retirement_recover_one "$state" task-a "$POLL" \
...
test_gitlab_merged_poll_retires() {
  ...
  seed_canonical_poll "$dir" task-a "$url"
  set +e
  XO_TEST_GLAB_STATE=merged run_watcher_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
