#!/usr/bin/env bash
# xo-control.sh relaunch: the transactional replace-the-agent verb.
#
...
#   6. xo-spawn --relaunch refuses on its own: a live agent, a contradicting
...
. "$ROOT/bin/xo-control-lib.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/xo-trace-context-lib.sh"

CONTROL="$ROOT/bin/xo-control.sh"
SPAWN="$ROOT/bin/xo-spawn.sh"
PROMOTE="$ROOT/bin/xo-promote.sh"
X_LINK="$ROOT/bin/xo-x-link.sh"
# xo_test_tmproot's own cleanup trap fires when its command substitution exits,
# so recreate the root before resolving it and clean it up from this file's trap.
TMP_ROOT=$(xo_test_tmproot xo-control-relaunch)
mkdir -p "$TMP_ROOT"
...
# The same lifecycle-modelling tmux stub as tests/xo-control.test.sh: the
    ...
    if [ "$literal" = 1 ]; then
      ...
      case "$payload" in
        /exit|/quit)
          ...
          [ -z "${XO_FAKE_EXIT_TRANSPORT_FAIL_AFTER_STOP:-}" ] || exit 1
          ;;
        *'encode launch-brief'*)
          cat "$D/becomes" > "$D/command"
          [ -z "${XO_FAKE_LAUNCH_TRANSPORT_FAIL_AFTER_START:-}" ] || exit 1
          ;;
      esac
    else
      printf '%s\n' "$payload" >> "$D/keys"
      case "$payload" in
        'export GOTMPDIR='*)
          if [ -n "${XO_FAKE_TRACE_PREPARE:-}" ]; then
            : > "$XO_FAKE_TRACE_PREPARE"
            while [ ! -e "$XO_FAKE_TRACE_RELEASE" ]; do /bin/sleep 0.01; done
          fi
          ;;
        'export TRACEPARENT='*)
          [ -z "${XO_FAKE_TRACE_EXPORTED:-}" ] || : > "$XO_FAKE_TRACE_EXPORTED"
          ;;
      esac
    fi
    exit 0 ;;
  display-message)
    for a in "$@"; do
      case "$a" in
        ...
        *pane_current_path*)
          if [ -n "${XO_FAKE_CWD_RACE_READY:-}" ]; then
            : > "$XO_FAKE_CWD_RACE_READY"
            /bin/sleep 1
          fi
          cat "$D/cwd"; printf '\n'; exit 0 ;;
      esac
...
SH
  ...
  cat > "$fb/sleep" <<'SH'
#!/usr/bin/env bash
[ -z "${XO_FAKE_LOCK_WAITING:-}" ] || : > "$XO_FAKE_LOCK_WAITING"
exit 0
SH
  chmod +x "$fb/sleep"
}
...
add_ship_task() {
  ...
  xo_git_worktree "$proj" "$wt" "task-$id"
...
run_control() {  # <case-dir> <args...>
  ...
  # store (bin/xo-claude-trust.sh), and a relaunch reaches it through xo-control.sh, so this runs against a throwaway HOME;
  # without it this suite would write the developer's real ~/.claude.json.
  mkdir -p "$dir/user-home"
  env PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" XO_FAKE_DIR="$dir/fake" \
    HOME="$dir/user-home" CLAUDE_CONFIG_DIR='' \
    XO_SPAWN_NO_GUARD=1 GROK_HOME="$dir/grokhome" \
    XO_CONTROL_POLL=0.01 XO_CONTROL_EXIT_WAIT=0.05 XO_CONTROL_LAUNCH_WAIT=0.05 \
    XO_REAL_GIT="${XO_REAL_GIT:-}" XO_FAKE_GIT_FAILURE="${XO_FAKE_GIT_FAILURE:-}" \
    XO_REAL_MV="${XO_REAL_MV:-}" XO_FAKE_COMPLETE_JOURNAL_MV_FAIL="${XO_FAKE_COMPLETE_JOURNAL_MV_FAIL:-}" \
    XO_FAKE_META_PUBLISH_MV_FAIL="${XO_FAKE_META_PUBLISH_MV_FAIL:-}" \
    XO_FAKE_TRACE_PREPARE="${XO_FAKE_TRACE_PREPARE:-}" \
    XO_FAKE_TRACE_RELEASE="${XO_FAKE_TRACE_RELEASE:-}" \
    XO_FAKE_META_WRITER_READY="${XO_FAKE_META_WRITER_READY:-}" \
    XO_FAKE_TRACE_EXPORTED="${XO_FAKE_TRACE_EXPORTED:-}" \
    "$CONTROL" "$@" 2>&1
}
...
run_spawn() {  # <case-dir> <args...>
  local dir=$1; shift
  # A claude spawn pre-registers workspace trust in the launching user's own
  # store (bin/xo-claude-trust.sh), so it runs against a throwaway HOME;
  # without it this suite would write the developer's real ~/.claude.json.
  mkdir -p "$dir/user-home"
  env PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" XO_FAKE_DIR="$dir/fake" \
    HOME="$dir/user-home" CLAUDE_CONFIG_DIR='' \
...
make_mv_failure_stub() {  # <case-dir>
  cat > "$1/fakebin/mv" <<'SH'
#!/usr/bin/env bash
if [ -n "${XO_FAKE_COMPLETE_JOURNAL_MV_FAIL:-}" ]; then
  for path in "$@"; do
...
if [ -n "${XO_FAKE_META_PUBLISH_MV_FAIL:-}" ]; then
  for path in "$@"; do
    [ "$path" != "$XO_FAKE_META_PUBLISH_MV_FAIL" ] || exit 1
  done
fi
source_path=
target_path=
...
if [ -n "${XO_FAKE_META_WRITER_TARGET:-}" ] \
   && [ "$target_path" = "$XO_FAKE_META_WRITER_TARGET" ] \
   && grep -q '^x_request=' "$source_path" 2>/dev/null; then
  : > "$XO_FAKE_META_WRITER_READY"
  while [ ! -e "$XO_FAKE_META_WRITER_RELEASE" ]; do /bin/sleep 0.01; done
fi
exec "$XO_REAL_MV" "$@"
SH
...
#!/usr/bin/env bash
for arg in "$@"; do
  if [ -n "${XO_FAKE_RM_FAIL_PATH:-}" ] && [ "$arg" = "$XO_FAKE_RM_FAIL_PATH" ]; then
    exit 1
  fi
done
exec "$XO_REAL_RM" "$@"
...
# backlog transition (bin/xo-backlog-transition-lib.sh) is live rather than
...
test_same_harness_relaunch_keeps_identity_and_reuses_the_endpoint() {
  ...
  gen_before=$("$ROOT/bin/xo-busy-event.sh" arm "$dir/home/state" rl1)
  ...
  [ "$(meta_field "$dir" rl1 window)" = "fmses:xo-rl1" ] \
  ...
  pass "xo-control relaunch: a same-harness relaunch replaces the agent in the same endpoint and worktree"
}
...
test_relaunch_from_linked_home_preserves_recorded_worktree() {
  ...
  git -C "$dir/wt" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm task-work
  ...
  if [ "${XO_TEST_EVIDENCE:-0}" = 1 ]; then
    printf '# evidence begin: linked-home relaunch\n'
    printf '$ bin/xo-control.sh rl42 relaunch --note "continue from linked home"\n%s\nexit=%s\n' "$out" "$rc"
  ...
  pass "xo-control relaunch: a linked spawning home preserves committed and unfinished work in the recorded copy"
}
...
test_relaunch_preserves_durable_task_metadata() {
  ...
  pass "xo-control relaunch: durable task metadata survives replacement launch publication"
}
...
test_relaunch_serializes_concurrent_durable_metadata_publication() {
  ...
  XO_REAL_MV=$(command -v mv) \
    XO_FAKE_TRACE_PREPARE="$prepare" \
    XO_FAKE_TRACE_RELEASE="$launch_release" \
  ...
  env PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_REAL_MV="$(command -v mv)" \
    XO_FAKE_LOCK_WAITING="$waiting" \
    XO_FAKE_META_WRITER_TARGET="$dir/home/state/rl28.meta" \
    XO_FAKE_META_WRITER_READY="$ready" \
    XO_FAKE_META_WRITER_RELEASE="$release" \
  ...
  xo_trace_context_valid "$traceparent" \
    || fail "concurrent metadata publication erased the replacement's trace carrier"
  pass "xo-control relaunch: delivery and concurrent task metadata publication serialize"
}
...
test_disabled_relaunch_clears_prior_trace_context() {
  ...
  pass "xo-control relaunch: disabling tracing clears metadata and pane context"
}
...
test_relaunch_appends_the_progress_note_to_the_instructions() {
  ...
  pass "xo-control relaunch: the progress note lands in the instructions the replacement reads"
}
...
test_relaunch_requires_a_note_for_a_ship_task() {
  ...
  pass "xo-control relaunch: a ship task refuses without the progress note its replacement needs"
}
...
test_harness_switch_moves_the_record_and_clears_prior_wiring() {
  ...
  pass "xo-control relaunch: switching harness is one ordinary relaunch, and the old wiring goes with the old agent"
}
...
test_harness_switch_does_not_carry_the_old_profile_axes() {
  ...
  pass "xo-control relaunch: a harness switch resets model and effort unless they are named too"
}
...
test_harness_switch_resolves_a_prefixed_recorded_harness() {
  ...
  mkdir -p "$dir/grokhome/hooks/xo-turn-end.d"
  printf 'xo.abcdefabcdef\n' > "$dir/home/state/rl32.grok-turnend-token"
  auth="$dir/grokhome/hooks/xo-turn-end.d/xo.abcdefabcdef"
  printf '%s\n' "$dir/home/state/rl32.turn-ended" > "$auth"
  printf 'token=xo.abcdefabcdef\n' > "$dir/wt/.xo-grok-turnend"
  ...
  [ ! -e "$auth" ] && [ ! -e "$dir/home/state/rl32.grok-turnend-token" ] \
    && [ ! -e "$dir/wt/.xo-grok-turnend" ] \
    || fail "relaunch should retire wiring owned by the prefixed prior harness"
  pass "xo-control relaunch: a prefixed recorded harness can switch adapters transactionally"
}
...
test_prefixed_recorded_harness_requires_explicit_replacement() {
  ...
  pass "xo-control relaunch: a prefixed command requires an explicit replacement harness"
}
...
test_same_harness_relaunch_keeps_the_profile_axes() {
  ...
  pass "xo-control relaunch: a same-harness relaunch keeps the profile axes it was running with"
}
...
test_explicit_model_wins_over_the_recorded_one() {
  ...
  pass "xo-control relaunch: explicit model and effort win over the recorded ones"
}
...
test_relaunch_onto_an_unverified_harness_is_refused() {
  ...
  [ "$(cat "$dir/fake/command")" = claude ] || fail "a refused relaunch must not stop the agent"
  pass "xo-control relaunch: refuses to relaunch onto an adapter with no verified mechanics"
}
...
test_prior_harness_turnend_registry_entry_is_cleared() {
  ...
  pass "xo-control relaunch: the retired incarnation's global turn-end token is revoked"
}
...
test_wiring_removal_failure_refuses_before_replacement_arm() {
  ...
  out=$(XO_REAL_RM="$real_rm" XO_FAKE_RM_FAIL_PATH="$hook" \
  ...
  pass "xo-control relaunch: wiring cleanup failure refuses replacement arming"
}

test_turnend_auth_paths_are_owned_by_the_control_adapter() {
  local dir state grok_path kimi_path token_path
  dir=$(xo_test_tmproot xo-control-auth)
  state="$dir/state"
  mkdir -p "$state"
  printf 'xo.111111111111\n' > "$state/x.grok-turnend-token"
  printf 'xo.222222222222\n' > "$state/x.kimi-turnend-token"
  token_path=$(xo_control_harness_turnend_token_path grok "$state" x)
  [ "$token_path" = "$state/x.grok-turnend-token" ] \
    || fail "the grok token path should be computed without reading it"
  grok_path=$(GROK_HOME="$dir/gh" xo_control_harness_turnend_auth_path grok xo.111111111111)
  [ "$grok_path" = "$dir/gh/hooks/xo-turn-end.d/xo.111111111111" ] \
    || fail "grok's registry path should resolve under GROK_HOME, got '$grok_path'"
  kimi_path=$(HOME="$dir/kh" xo_control_harness_turnend_auth_path kimi xo.222222222222)
  [ "$kimi_path" = "$dir/kh/.kimi-code/xo-turn-end.d/xo.222222222222" ] \
    || fail "kimi's registry path should resolve under the home store, got '$kimi_path'"
  grok_path=$(GROK_HOME="$dir/gh" xo_control_harness_turnend_auth_path grok 'not a token/../..')
  [ -z "$grok_path" ] || fail "a malformed token must resolve to no path, got '$grok_path'"
  pass "xo-control-lib: one owner resolves each harness's turn-end registry entry, and refuses a malformed token"
}
...
test_secondmate_relaunch_picks_up_the_configured_harness_pin() {
  ...
  xo_git_worktree "$dir/proj" "$dir/smhome" sm-branch
  mkdir -p "$dir/smhome/state" "$dir/smhome/data" "$dir/smhome/bin"
  printf 'sm3\n' > "$dir/smhome/.xo-secondmate-home"
  printf '# agents\n' > "$dir/smhome/AGENTS.md"
  {
    echo "window=fmses:xo-sm3"
  ...
  pass "xo-control relaunch: a secondmate relaunch re-resolves its durable configured harness pin"
}
...
test_secondmate_relaunch_ignores_invalid_configured_effort_before_stop() {
  ...
  pass "xo-control relaunch: invalid configured effort is ignored before stop"
}

# muse is a verified adapter, but only for crewmates and scouts: it has no
# primary supervision protocol, so bin/xo-spawn.sh refuses it for a secondmate.
...
test_secondmate_relaunch_onto_a_crewmate_only_adapter_refuses_before_stop() {
  ...
  pass "xo-control relaunch: an adapter unverified for this task kind refuses before the agent is stopped"
}
...
test_explicit_secondmate_harness_ignores_configured_profile_axes() {
  ...
  pass "xo-control relaunch: explicit secondmate harness resets unnamed profile axes"
}
...
test_ship_relaunch_ignores_the_crew_harness_config() {
  ...
  pass "xo-control relaunch: a ship task keeps its recorded harness instead of re-reading crew config"
}
...
test_spawn_relaunch_without_a_harness_reuses_the_recorded_one() {
  ...
  [ "$(meta_field "$dir" rl21 harness)" = claude ] \
    || fail "xo-spawn --relaunch without --harness must reuse the recorded harness, got '$(meta_field "$dir" rl21 harness)'"
  assert_contains "$out" "spawned rl21 harness=claude" "the launch should report the recorded harness"
  pass "xo-spawn --relaunch: with no explicit harness it reuses the task's recorded one, never the crew default"
}

# xo-spawn arms per-task wiring on harness PREFIXES, because a task launched
...
test_prefixed_prior_harness_wiring_is_still_retired() {
  ...
  pass "xo-spawn --relaunch: wiring armed under a prefixed harness name is still retired"
}

# muse installs no hook; its busy source is its own session event log, bound to
# the pane by two xo-owned sidecars. Relaunching AWAY from muse must
...
test_muse_session_binding_is_retired_on_a_harness_switch() {
  ...
  pass "xo-spawn --relaunch: switching away from muse retires its session binding"
}
...
test_cursor_session_binding_is_retired_on_a_harness_switch() {
  ...
  pass "xo-spawn --relaunch: switching away from cursor retires its session binding"
}
...
test_missing_worktree_refuses_before_stopping_anything() {
  ...
  [ "$(cat "$dir/fake/command")" = claude ] || fail "a refused relaunch must not stop the agent"
  [ -z "$(cat "$dir/fake/literal")" ] || fail "a refused relaunch must send nothing"
  pass "xo-control relaunch: an unaccountable local copy refuses before the agent is touched"
}
...
test_missing_instructions_refuse_before_stopping_anything() {
  ...
  [ "$(cat "$dir/fake/command")" = claude ] || fail "a refused relaunch must not stop the agent"
  pass "xo-control relaunch: a worker with nothing to work from is never launched"
}
...
test_checkpoint_refusal_leaves_the_record_byte_identical() {
  ...
  pass "xo-control relaunch: a refusal before the agent is stopped leaves the durable record untouched"
}
...
test_checkpoint_refuses_uninspectable_head_and_status() {
  ...
  out=$(XO_REAL_GIT="$real_git" XO_FAKE_GIT_FAILURE=head \
  ...
  make_git_failure_stub "$dir"
  out=$(XO_REAL_GIT="$real_git" XO_FAKE_GIT_FAILURE=status \
  ...
  pass "xo-control relaunch: checkpoint inspection failures refuse before stopping"
}
...
test_launch_failure_keeps_the_prior_record_and_reports_it() {
  ...
  pass "xo-control relaunch: a launch failure after the stop keeps the prior record and reports the real state"
}
...
test_prepublication_failure_keeps_concurrent_durable_metadata() {
  ...
  printf '%s' "$dir/proj" > "$dir/fake/cwd"
  XO_FAKE_CWD_RACE_READY="$dir/cwd-race-ready" \
  ...
  link_out=$(env PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" \
  ...
  pass "xo-control relaunch: unpublished rollback keeps concurrent durable metadata"
}
...
test_post_publication_launch_failure_keeps_the_new_record() {
  ...
  printf 'codex' > "$dir/fake/becomes"
  out=$(XO_FAKE_LAUNCH_TRANSPORT_FAIL_AFTER_START=1 \
  ...
  pass "xo-control relaunch: post-publication failure keeps the new durable record"
}
...
test_stop_transport_failure_reconciles_a_dead_agent() {
  ...
  out=$(XO_FAKE_EXIT_TRANSPORT_FAIL_AFTER_STOP=1 \
  ...
  pass "xo-control relaunch: partial stop reconciles actual agent state"
}
...
test_complete_journal_failure_rolls_back_from_durable_phase() {
  ...
  make_mv_failure_stub "$dir"
  out=$(XO_REAL_MV="$real_mv" XO_FAKE_COMPLETE_JOURNAL_MV_FAIL=1 \
  ...
  pass "xo-control relaunch: failed journal replacement preserves durable phase"
}
...
test_prepublication_abort_retires_replacement_wiring_and_busy_state() {
  ...
  real_mv=$(command -v mv)
  make_mv_failure_stub "$dir"
  out=$(XO_REAL_MV="$real_mv" XO_FAKE_META_PUBLISH_MV_FAIL="$meta" \
  ...
  pass "xo-spawn relaunch: prepublication abort removes replacement state"
}
...
test_journal_records_the_checkpoint_it_proved() {
  ...
  pass "xo-control relaunch: the checkpoint records the exact unlanded work it preserved"
}
...
test_secondmate_relaunch_checkpoints_child_work_and_spares_the_charter() {
  ...
  printf '# agents\n' > "$dir/smhome/AGENTS.md"
  printf 'window=x:xo-c1\n' > "$dir/smhome/state/c1.meta"
  printf 'window=x:xo-c2\n' > "$dir/smhome/state/c2.meta"
  {
    echo "window=fmses:xo-sm1"
  ...
  pass "xo-control relaunch: a secondmate's child work is accounted for and its charter is left alone"
}
...
test_secondmate_relaunch_refuses_an_unmarked_home() {
  ...
  [ "$(cat "$dir/fake/command")" = claude ] || fail "a refused relaunch must not stop the agent"
  pass "xo-control relaunch: a secondmate home that is not this secondmate's is refused"
}
...
SH
  ...
  pass "xo-control relaunch: unreadable and untraversable child state fails checkpoint"
}
...
test_concurrent_relaunch_is_refused() {
  ...
  pass "xo-control relaunch: two control actions on one task serialize instead of interleaving"
}
...
test_direct_spawn_relaunch_participates_in_the_lifecycle_lock() {
  ...
  pass "xo-spawn relaunch: direct entry participates in lifecycle serialization"
}
...
test_promotion_participates_in_the_lifecycle_lock_before_metadata_resolution() {
  ...
  out=$(XO_HOME="$dir/home" "$PROMOTE" rl29 --mode direct-PR --yolo on 2>&1); rc=$?
  kill "$holder" 2>/dev/null || true
  wait "$holder" 2>/dev/null || true
  ...
  pass "xo-promote: promotion participates in lifecycle serialization"
}

# --- 6. xo-spawn --relaunch's own refusals -----------------------------------
...
test_spawn_relaunch_refuses_a_live_agent() {
  ...
  assert_contains "$out" "xo-control.sh rl15 exit" "the refusal should point at the way to stop it"
  pass "xo-spawn --relaunch: refuses to launch a second agent into a live endpoint"
}
...
SH
  ...
  pass "xo-spawn --relaunch: symlinked records refuse before inspection"
}
...
SH
  ...
  pass "xo-spawn --relaunch: keeps its early meta lock continuous"
}
...
test_spawn_relaunch_refuses_a_pending_authoritative_close() {
  ...
  pass "xo-spawn --relaunch: pending closes refuse before replacement begins"
}
...
test_spawn_relaunch_refuses_contradicting_flags() {
  ...
  pass "xo-spawn --relaunch: every identity axis comes from the record, and a contradicting flag refuses"
}
...
test_spawn_relaunch_refuses_an_unrecorded_task() {
  ...
  pass "xo-spawn --relaunch: an unrecorded task is refused"
}
...
test_spawn_relaunch_refuses_a_pane_outside_the_worktree() {
  ...
  pass "xo-spawn --relaunch: refuses to start a replacement outside the copy holding its work"
}
