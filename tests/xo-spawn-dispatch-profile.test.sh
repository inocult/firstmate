#!/usr/bin/env bash
# Behavior tests for xo-spawn.sh concrete dispatch profile flags.
#
# These tests drive xo-spawn through meta writing and launch construction with a
...
# command xo would run without starting any real harness.
set -u
...
TMP_ROOT=$(xo_test_tmproot xo-spawn-dispatch-profile)
...
set -u
if [ "${1:-}" = --help ]; then
  if [ "${XO_FAKE_PI_VERSION:-0.84.0}" = 0.82.0 ]; then
    printf '%s\n' 'Pi 0.82.0' 'Options: --help'
  else
    printf '%s\n' "Pi ${XO_FAKE_PI_VERSION:-0.84.0}" 'Options: --help --tui-mode <mode>'
  fi
fi
exit 0
SH
...
make_spawn_fakebin() {
  local dir=$1 fakebin
  fakebin=$(xo_test_make_spawn_fakebin "$dir")
...
if [ "${1:-}" = --list-models ]; then
  [ "${XO_FAKE_CURSOR_LIST_STATUS:-0}" -eq 0 ] || exit "${XO_FAKE_CURSOR_LIST_STATUS}"
  printf '%b\n' "${XO_FAKE_CURSOR_MODELS:-Available models\ncursor-grok-4.5-high - Grok 4.5 High}"
fi
exit 0
SH
...
run_spawn() {
  ...
  shift 4
  : > "$launchlog"
  # CLAUDE_CONFIG_DIR is forwarded onto claude launches by xo-spawn, so pin it
  # explicitly (empty by default) instead of leaking the invoking shell's value,
  # which would make launch assertions depend on the developer's environment.
  # A test opts in to the set case via XO_TEST_CLAUDE_CONFIG_DIR.
  CLAUDE_CONFIG_DIR="${XO_TEST_CLAUDE_CONFIG_DIR:-}" \
    XO_FAKE_LAUNCH_LOG="$launchlog" XO_FAKE_PI_VERSION="${XO_TEST_PI_VERSION:-0.84.0}" \
    XO_FAKE_CURSOR_MODELS="${XO_TEST_CURSOR_MODELS:-}" \
    XO_FAKE_CURSOR_LIST_STATUS="${XO_TEST_CURSOR_LIST_STATUS:-0}" \
...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' \"\$('${ROOT}/bin/xo-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/launch-brief.md')\""
...
test_relative_home_overrides_launch_with_absolute_cross_process_paths() {
  ...
  out=$(
    cd "$CASE_DIR" || exit 1
    CDPATH="$CASE_DIR/cdpath" XO_ROOT_OVERRIDE='' XO_HOME=home \
      XO_STATE_OVERRIDE=home/state XO_DATA_OVERRIDE=home/data \
      XO_PROJECTS_OVERRIDE=home/projects XO_CONFIG_OVERRIDE=home/config \
      XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
      CLAUDE_CONFIG_DIR='' XO_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
  ...
  assert_contains "$launch" "-e '$home_real/state/$id.pi-ext.ts'" \
    "relative XO_STATE_OVERRIDE leaked into Pi's cross-process extension path"
  assert_contains "$launch" "< '$home_real/data/$id/launch-brief.md'" \
    "relative XO_DATA_OVERRIDE leaked into the cross-process brief path"
...
test_home_defaults_preserve_absolute_or_resolve_relative_paths() {
  ...
  out=$(
    cd "$CASE_DIR" || exit 1
    XO_ROOT_OVERRIDE='' XO_HOME=home \
      XO_STATE_OVERRIDE='' XO_DATA_OVERRIDE='' \
      XO_PROJECTS_OVERRIDE=home/projects XO_CONFIG_OVERRIDE=home/config \
      XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
      CLAUDE_CONFIG_DIR='' XO_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
      GROK_HOME=home/grok-home PATH="$FAKEBIN_DIR:$PATH" \
  ...
  status=$?
  expect_code 0 "$status" "spawn with relative XO_HOME defaults should succeed"
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "-e '$home_real/state/$relative_id.pi-ext.ts'" \
    "relative XO_HOME leaked into Pi's default cross-process extension path"
  assert_contains "$launch" "< '$home_real/data/$relative_id/launch-brief.md'" \
    "relative XO_HOME leaked into the default cross-process brief path"

  linked_home="$CASE_DIR/home-link"
  ln -s "$HOME_DIR" "$linked_home"
  : > "$LAUNCH_LOG"
  out=$(
    XO_ROOT_OVERRIDE='' XO_HOME="$linked_home" \
      XO_STATE_OVERRIDE='' XO_DATA_OVERRIDE='' \
      XO_PROJECTS_OVERRIDE="$linked_home/projects" XO_CONFIG_OVERRIDE="$linked_home/config" \
      XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
      CLAUDE_CONFIG_DIR='' XO_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
  ...
  status=$?
  expect_code 0 "$status" "spawn with absolute symlink-spelled XO_HOME defaults should succeed"
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "-e '$linked_home/state/$absolute_id.pi-ext.ts'" \
    "absolute XO_HOME spelling changed in Pi's default cross-process extension path"
  assert_contains "$launch" "< '$linked_home/data/$absolute_id/launch-brief.md'" \
    "absolute XO_HOME spelling changed in the default cross-process brief path"
  pass "XO_HOME defaults resolve relative paths and preserve absolute spellings"
}
...
test_absolute_override_spelling_is_preserved_in_launch_paths() {
  ...
  out=$(
    XO_ROOT_OVERRIDE='' XO_HOME="$linked_home" \
      XO_STATE_OVERRIDE="$linked_home/state" XO_DATA_OVERRIDE="$linked_home/data" \
      XO_PROJECTS_OVERRIDE="$linked_home/projects" XO_CONFIG_OVERRIDE="$linked_home/config" \
      XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
      CLAUDE_CONFIG_DIR='' XO_FAKE_LAUNCH_LOG="$LAUNCH_LOG" \
      GROK_HOME="$linked_home/grok-home" PATH="$FAKEBIN_DIR:$PATH" \
  ...
  assert_contains "$launch" "-e '$linked_home/state/$id.pi-ext.ts'" \
    "absolute XO_STATE_OVERRIDE spelling changed in Pi's cross-process extension path"
  assert_contains "$launch" "< '$linked_home/data/$id/launch-brief.md'" \
    "absolute XO_DATA_OVERRIDE spelling changed in the cross-process brief path"
...
test_unresolvable_relative_overrides_fail_loudly() {
  ...
  out=$(
    cd "$CASE_DIR" || exit 1
    XO_ROOT_OVERRIDE='' XO_HOME=missing-home \
      XO_STATE_OVERRIDE='' XO_DATA_OVERRIDE='' \
      "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
  expect_code 1 "$status" "spawn with an unresolvable relative home should fail"
  assert_contains "$out" "XO_HOME directory cannot be resolved: missing-home" \
    "spawn did not name the unresolvable XO_HOME"

  out=$(
    cd "$CASE_DIR" || exit 1
    XO_ROOT_OVERRIDE='' XO_HOME=home \
      XO_STATE_OVERRIDE=missing-state XO_DATA_OVERRIDE=home/data \
      "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
  expect_code 1 "$status" "spawn with an unresolvable relative state override should fail"
  assert_contains "$out" "XO_STATE_OVERRIDE directory cannot be resolved: missing-state" \
    "spawn did not name the unresolvable XO_STATE_OVERRIDE"

  out=$(
    cd "$CASE_DIR" || exit 1
    XO_ROOT_OVERRIDE='' XO_HOME=home \
      XO_STATE_OVERRIDE=home/state XO_DATA_OVERRIDE=missing-data \
      "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1
  )
  status=$?
  expect_code 1 "$status" "spawn with an unresolvable relative data override should fail"
  assert_contains "$out" "XO_DATA_OVERRIDE directory cannot be resolved: missing-data" \
    "spawn did not name the unresolvable XO_DATA_OVERRIDE"
...
test_grok_omits_invalid_max_reasoning_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "grok --always-approve --model 'grok-4' \"\$('${ROOT}/bin/xo-operational-input.sh' encode launch-brief < " \
...
test_cursor_failed_catalog_probe_does_not_block_spawn() {
  ...
  XO_TEST_CURSOR_LIST_STATUS=124 \
    out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR" \
...
test_native_effort_validator_keeps_axes_separate() {
  ...
  for harness in pi pi-signed; do
    "$ROOT/bin/xo-harness.sh" validate-native-effort "$harness" codex-native/gpt-6-astra ultra \
      || fail "native validator refused supported harness $harness"
  done
  if "$ROOT/bin/xo-harness.sh" validate-native-effort 'pi:codex-native/forged' '' ultra 2>/dev/null; then
...
test_pi_threads_model_and_max_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "XO_PI_HARNESS=pi '$FAKEBIN_DIR/pi' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' -e" \
    "pi launch did not force the regular TUI while threading the requested model and max thinking level"
  assert_not_contains "$launch" "XO_XO_PI_LAUNCH_BRIEF=" \
    "pi launch still exports the removed Calm input-reroute binding"
  assert_contains "$launch" "xo-operational-input.sh' encode launch-brief" \
...
test_pi_signed_threads_shared_pi_profile_and_preserves_identity() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "XO_PI_HARNESS=pi-signed '$FAKEBIN_DIR/pi-signed' --tui-mode regular --model 'openai-codex/gpt-5.6-sol' --thinking 'max' -e" \
  ...
  assert_contains "$(cat "$HOME_DIR/state/$id.busy-state")" "state=busy source=xo-spawn" \
...
test_pi_tui_mode_probe_is_safe_for_old_and_new_pi() {
  ...
  for harness in pi pi-signed; do
    for version in 0.82.0 0.84.0; do
      ...
      out=$(XO_TEST_PI_VERSION="$version" \
      ...
      assert_not_contains "$launch" "XO_PI_HARNESS=$harness $harness" \
...
test_pi_signed_missing_binary_refuses_before_endpoint_or_metadata() {
  ...
  out=$(XO_ROOT_OVERRIDE='' XO_HOME="$HOME_DIR" \
    XO_STATE_OVERRIDE="$HOME_DIR/state" XO_DATA_OVERRIDE="$HOME_DIR/data" \
    XO_PROJECTS_OVERRIDE="$HOME_DIR/projects" XO_CONFIG_OVERRIDE="$HOME_DIR/config" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$WT_DIR" TMUX="fake,1,0" \
    XO_FAKE_LAUNCH_LOG="$LAUNCH_LOG" PATH="$FAKEBIN_DIR:/usr/bin:/bin:/usr/sbin:/sbin" \
    "$SPAWN" "$id" "$PROJ_DIR" --mode no-mistakes --yolo off 2>&1)
  status=$?
...
test_pi_signed_persistent_secondmate_uses_pi_extensions_and_identity() {
  ...
  assert_contains "$launch" "XO_PI_HARNESS=pi-signed '$FAKEBIN_DIR/pi-signed' --tui-mode regular -e '$sm/.pi/extensions/xo-primary-turnend-guard.ts' -e '$sm/.pi/extensions/xo-primary-pi-watch.ts'" \
...
test_claude_forwards_xo_config_dir_when_set() {
  local rec id out status launch
  ...
  # (bin/xo-claude-trust.sh), so an unwritable directory is a genuine blocker.
  # The forwarding assertion below is what this case proves and is unchanged.
  out=$(XO_TEST_CLAUDE_CONFIG_DIR="$CASE_DIR/claude-work" \
    run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  ...
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='$CASE_DIR/claude-work' env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}'" \
    "claude launch did not forward xo's CLAUDE_CONFIG_DIR to the crewmate pane"
  pass "claude forwards xo's CLAUDE_CONFIG_DIR so the crewmate uses the same credential store"
}
...
test_claude_omits_config_dir_prefix_when_unset() {
  ...
  # default path where xo-spawn adds no prefix.
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  expect_code 0 "$status" "claude spawn without CLAUDE_CONFIG_DIR should succeed"
  launch=$(cat "$LAUNCH_LOG")
  assert_not_contains "$launch" "CLAUDE_CONFIG_DIR=" \
    "claude launch must not add a config-dir prefix when xo has no CLAUDE_CONFIG_DIR set"
  pass "claude omits the config-dir prefix when xo runs with the single-store default"
}
...
test_non_claude_harness_ignores_config_dir() {
  ...
  out=$(XO_TEST_CLAUDE_CONFIG_DIR="/opt/test/claude-work" \
    run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
...
test_launch_environment_allowlist() {
  ...
  for setting in absent missing-config enabled empty; do
    ...
    cat > "$probe" <<'SH'
#!/bin/sh
printf '%s\n' "${XO_TEST_AMBIENT_SENTINEL-unset}" "${XO_TEST_ALLOWED-unset}" \
  "${XO_TEST_EMPTY-unset}" "${XO_TEST_UNSET-unset}" "$HOME" "$PATH" "$TERM" "$TMUX" "$GOTMPDIR"
SH
    out=$(XO_TEST_AMBIENT_SENTINEL=synthetic-unrelated \
      run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" \
    ...
    for pane_shell in /bin/sh /bin/bash /bin/zsh; do
      ...
      TMUX=synthetic-pane GOTMPDIR=/synthetic/gotmp \
      XO_TEST_AMBIENT_SENTINEL=synthetic-unrelated XO_TEST_ALLOWED="$value" XO_TEST_EMPTY='' \
...
test_launch_environment_invalid_config_refuses() {
  ...
  rec=$(make_spawn_case "$id" codex "$id")
  read_case_record "$rec"
  for bad in 'XO_TEST_ALLOWED=value' 'NAME;false' '1INVALID' '*'; do
...
#!/bin/sh
printf '%s\n' "${XO_TEST_AMBIENT_SENTINEL-unset}" "$XO_TEST_ALLOWED" "$XO_HOME" "${XO_STATE_OVERRIDE-unset}"
SH
  chmod +x "$FAKEBIN_DIR/codex"
  result=$(env -i HOME="$HOME_DIR/user-home" PATH="$FAKEBIN_DIR:$PATH" \
    XO_TEST_AMBIENT_SENTINEL=synthetic-unrelated XO_TEST_ALLOWED=synthetic-provider \
...
run_launch_environment_inheritance() {
  ...
  if [ "$route" = local ]; then
    (
      # shellcheck source=/dev/null
      . "$ROOT/bin/xo-config-inherit-lib.sh"
      XO_INHERITABLE_CONFIG=launch-env-allowlist \
        propagate_inheritable_config "$home/config" "$dest/config"
    )
  else
    XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_CONFIG_OVERRIDE="$home/config" \
      XO_DATA_OVERRIDE="$home/data" XO_INHERITABLE_CONFIG=launch-env-allowlist \
      XO_SSH_BIN="$fakebin/inherit-ssh" \
      "$ROOT/bin/xo-remote-inherit-push.sh" inherited-env "$generation"
  fi
}
...
[ "$#" -eq 6 ] && [ "$1" = inherit-host ] && [ "$2" = xo-remote-entrypoint.sh ] && [ "$3" = 1 ] || exit 91
...
[ "${args[0]}" = xo-remote-inherit.sh ] || exit 92
XO_HOME="$remote_home" XO_STATE_OVERRIDE="$remote_home/state" \
...
SH
    ...
    [ "$(cat "$dest/config/launch-env-allowlist")" = XO_TEST_ALLOWED ] \
...
test_worker_launch_delivers_role_scope() {
  ...
  for kind in no-mistakes direct-PR local-only scout; do
    ...
    if [ "$brief_kind" != scaffold ]; then
      xo_test_spawn_brief "$HOME_DIR" "$id"
      if [ "$brief_kind" = heading ]; then
        printf '\n# Worker role\nFollow the project instructions.\n' >> "$HOME_DIR/data/$id/brief.md"
      fi
    else
      if [ "$kind" = scout ]; then
        XO_HOME="$HOME_DIR" "$ROOT/bin/xo-brief.sh" "$id" arbitrary-project-name --scout >/dev/null || fail "scout scaffold failed"
      else
        XO_HOME="$HOME_DIR" "$ROOT/bin/xo-brief.sh" "$id" arbitrary-project-name --mode "$kind" >/dev/null || fail "$kind scaffold failed"
      fi
      brief="$HOME_DIR/data/$id/brief.md"
      content=$(cat "$brief")
      content=${content//'{TASK}'/brief for $id}
      content=${content//'{XO_SPEC}'/Exercise the spawn behavior under test.}
      printf '%s\n' "$content" > "$brief"
    fi
...
SH
    ...
    XO_ROLE_PROMPT="$prompt" PATH="$FAKEBIN_DIR:$PATH" bash -c "$launch" || fail "could not consume $kind launch command"
  ...
  pass "xo-spawn: actual ship/scout launch commands deliver the worker role contract"
}

# config/claude-permission-mode (bin/xo-spawn.sh header): absent and `bypass`
...
claude_expected_launch() {  # <home> <id> <permission-flag>
  ...
  printf '%s' "env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude $flag --settings '{\"feedbackDrafts\":\"off\",\"attribution\":{\"commit\":\"\",\"pr\":\"\",\"sessionUrl\":false}}' \"\$('${ROOT}/bin/xo-operational-input.sh' encode launch-brief < '$home/data/$id/launch-brief.md')\""
}
...
echo "# all xo-spawn-dispatch-profile tests passed"
