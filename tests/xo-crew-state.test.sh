#!/usr/bin/env bash
# Behavior tests for bin/xo-crew-state.sh - the deterministic crew-current-state
# helper.
#
# The status file (state/<id>.status) is a best-effort append-only EVENT LOG, so
# `tail -1` of it reports the last event, not the current state. xo-crew-state
...
#       fake xo-crew-state.sh verdict): cross-branch attribution via the runs
...
. "$ROOT/bin/xo-classify-lib.sh"

CREW_STATE="$ROOT/bin/xo-crew-state.sh"
TMP_ROOT=$(xo_test_tmproot xo-crew-state)
...
make_repo_on_branch() {  # <dir> <branch>
  ...
  # Real worktree HEAD for run head-binding (fixtures read XO_FAKE_RUN_HEAD).
  XO_FAKE_RUN_HEAD=$(git -C "$dir" rev-parse HEAD)
  export XO_FAKE_RUN_HEAD
}
...
case "${1:-}" in
  axi)
    ...
    case "${1:-}" in
      status)
        shift
        if [ "${1:-}" = --run ]; then printf '%s\n' "${XO_FAKE_AXI_STATUS_RUN:-}"
        else printf '%s\n' "${XO_FAKE_AXI_STATUS:-}"; fi ;;
      logs)
        printf '%s\n' "${XO_FAKE_CI_LOGS:-}" ;;
    esac
    ;;
  runs)
    printf '%s\n' "${XO_FAKE_RUNS_LIST:-}" ;;
  daemon)
    # XO_FAKE_DAEMON_DOWN: the explicit down-probe fails, as the real
    # `no-mistakes daemon status` does when the daemon is not running.
    [ "${XO_FAKE_DAEMON_DOWN:-0}" = 1 ] && exit 1
...
#!/usr/bin/env bash
set -u
# XO_FAKE_TMUX_MISSING: the window is authoritatively gone while the server and
...
# XO_FAKE_TMUX_UNREADABLE: tmux itself cannot answer - it fails to execute (a
...
# responses that xo_backend_tmux_agent_state owns as death.
[ "${XO_FAKE_TMUX_UNREADABLE:-0}" = 1 ] && { printf 'no current client\n' >&2; exit 1; }
case "${1:-}" in
  list-windows)
    ...
    ;;
  list-panes)
    [ "${XO_FAKE_TMUX_MISSING:-0}" = 1 ] && { printf "can't find window: %s\n" "${3:-}" >&2; exit 1; }
    printf '%%1\n' ;;
  display-message)
    printf '%%1\n' ;;
  capture-pane)
    [ "${XO_FAKE_TMUX_MISSING:-0}" = 1 ] && exit 1
    if [ "${XO_FAKE_BUSY:-0}" = 1 ]; then printf 'work in progress\n%s\n' "${XO_FAKE_BUSY_TEXT:-esc to interrupt}"
...
case "${1:-}" in
  ...
  pane)
    case "${2:-}" in
      read)
        [ "${XO_FAKE_HERDR_MISSING:-0}" = 1 ] && exit 1
        [ "${XO_FAKE_HERDR_READ_FAIL:-0}" = 1 ] && exit 1
        if [ "${XO_FAKE_HERDR_BUSY:-0}" = 1 ]; then printf 'work in progress\nesc to interrupt\n'
        else printf 'all quiet\n> \n'; fi
        exit 0 ;;
      get)
        if [ "${XO_FAKE_HERDR_MISSING:-0}" = 1 ]; then
  ...
  agent)
    case "${2:-}" in
      get)
        if [ "${XO_FAKE_HERDR_HUSK:-0}" = 1 ]; then
          printf '{"error":{"code":"agent_not_found","message":"no agent in pane"}}\n'
          exit 0
        fi
        [ -n "${XO_FAKE_HERDR_AGENT_STATUS:-}" ] || exit 1
        printf '{"result":{"agent":{"agent_status":"%s"}}}\n' "$XO_FAKE_HERDR_AGENT_STATUS"
        exit 0 ;;
    esac ;;
esac
exit 0
...
# Run the helper for one case dir. XO_FAKE_* env (run output, busy flag) are read
# from the caller's environment by the fakes above.
run_crew_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" XO_STATE_OVERRIDE="$1/state" "$CREW_STATE" "$2"
}
...
arm_idle_record() {  # <state-dir> <id>
  ...
  gen=$("$ROOT/bin/xo-busy-event.sh" arm "$state" "$id")
  "$ROOT/bin/xo-busy-event.sh" apply "$state" "$id" idle --gen "$gen" \
...
reset_fakes() {
  ...
  XO_FAKE_BUSY=0
  XO_FAKE_BUSY_TEXT=
  XO_FAKE_TMUX_MISSING=0
  XO_FAKE_TMUX_UNREADABLE=0
  XO_FAKE_HERDR_BUSY=0
  XO_FAKE_HERDR_MISSING=0
  XO_FAKE_HERDR_READ_FAIL=0
  XO_FAKE_HERDR_HUSK=0
  XO_FAKE_HERDR_AGENT_STATUS=""
  XO_FAKE_HERDR_PROCESS=agent
  XO_FAKE_HERDR_SHELL_PID=$$
  XO_FAKE_CI_LOGS=""
  XO_FAKE_DAEMON_DOWN=0
  export XO_FAKE_AXI_STATUS XO_FAKE_AXI_STATUS_RUN XO_FAKE_RUNS_LIST XO_FAKE_BUSY XO_FAKE_BUSY_TEXT XO_FAKE_TMUX_MISSING XO_FAKE_TMUX_UNREADABLE
  export XO_FAKE_HERDR_BUSY XO_FAKE_HERDR_MISSING XO_FAKE_HERDR_READ_FAIL XO_FAKE_HERDR_HUSK XO_FAKE_HERDR_AGENT_STATUS XO_FAKE_HERDR_PROCESS XO_FAKE_HERDR_SHELL_PID XO_FAKE_CI_LOGS
  export XO_FAKE_DAEMON_DOWN
}
...
test_active_run_is_authoritative() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-a
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-a.meta" "window=xo:xo-feat-a" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running xo/feat-a)"
...
test_stale_needs_decision_superseded() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-b
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-b.meta" "window=xo:xo-feat-b" "worktree=$d/wt" "kind=ship"
  printf 'working: started\nneeds-decision: pick A or B\n' > "$d/state/feat-b.status"
  XO_FAKE_AXI_STATUS="$(run_fixing xo/feat-b)"
...
test_stale_blocked_superseded() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-bb
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-bb.meta" "window=xo:xo-feat-bb" "worktree=$d/wt" "kind=ship"
  printf 'blocked: waiting on review answer\n' > "$d/state/feat-bb.status"
  XO_FAKE_AXI_STATUS="$(run_running xo/feat-bb)"
...
test_daemon_claim_over_live_run_reads_run_alive() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-dl
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-dl.meta" "window=xo:xo-feat-dl" "worktree=$d/wt" "kind=ship"
  printf 'blocked: no-mistakes daemon unreachable, drive run: read response: i/o timeout\n' \
    > "$d/state/feat-dl.status"
  XO_FAKE_AXI_STATUS="$(run_fixing_active_recent xo/feat-dl)"
...
test_socket_refusal_over_stale_fixing_run_reports_blocked() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-dq
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-dq.meta" "window=xo:xo-feat-dq" "worktree=$d/wt" "kind=ship"
  printf 'blocked: no-mistakes daemon socket refused connections\n' \
    > "$d/state/feat-dq.status"
  XO_FAKE_AXI_STATUS="$(run_fixing_active_quiet xo/feat-dq)"
...
test_socket_refusal_over_terminal_run_reports_blocked() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-dqt
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-dqt.meta" "window=xo:xo-feat-dqt" "worktree=$d/wt" "kind=ship"
  printf 'blocked: no-mistakes daemon socket refused connections\n' \
    > "$d/state/feat-dqt.status"
  XO_FAKE_AXI_STATUS="$(run_failed xo/feat-dqt)"
...
test_ordinary_blocked_over_live_run_keeps_plain_superseded() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-ob
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-ob.meta" "window=xo:xo-feat-ob" "worktree=$d/wt" "kind=ship"
  printf 'blocked: database upload failed with broken pipe\n' > "$d/state/feat-ob.status"
  XO_FAKE_AXI_STATUS="$(run_fixing_active_recent xo/feat-ob)"
...
test_genuine_daemon_down_reports_blocked() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-dd
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-dd.meta" "window=xo:xo-feat-dd" "worktree=$d/wt" "kind=ship" "harness=claude"
...
test_genuine_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-c
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-c.meta" "window=xo:xo-feat-c" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-c.status"
  XO_FAKE_AXI_STATUS="$(run_parked xo/feat-c)"
...
test_scalar_gate_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-cs
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cs.meta" "window=xo:xo-feat-cs" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cs.status"
  XO_FAKE_AXI_STATUS="$(run_parked_scalar_gate_running xo/feat-cs)"
...
test_gate_block_parked_not_superseded() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-cb
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cb.meta" "window=xo:xo-feat-cb" "worktree=$d/wt" "kind=ship"
  printf 'needs-decision: review gate\n' > "$d/state/feat-cb.status"
  XO_FAKE_AXI_STATUS="$(run_parked_in_gate_block xo/feat-cb)"
...
test_ci_ready_done_log_beats_monitoring_run() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-ci
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-ci.meta" "window=xo:xo-feat-ci" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-ci.status"
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/feat-ci)"
...
# merge/close. xo-crew-state must surface this as done, not "validating
# (running)", so a green PR is never silently absorbed as still-in-progress.
test_ci_monitoring_checks_green_surfaces_done() {
  reset_fakes
  local d; d=$(new_case ci-green)
  make_repo_on_branch "$d/wt" xo/feat-cigreen
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cigreen.meta" "window=xo:xo-feat-cigreen" "worktree=$d/wt" "kind=ship"
  # No status-log line at all: the crew never reported its own checks-green line.
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/feat-cigreen)"
  XO_FAKE_CI_LOGS=$(cat <<'EOF'
...
test_top_level_ci_checks_green_surfaces_done() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-topcigreen
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-topcigreen.meta" "window=xo:xo-feat-topcigreen" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_top_level_ci xo/feat-topcigreen)"
  XO_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
...
test_ci_monitoring_no_checks_terminal_surfaces_done() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-cinochecks
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cinochecks.meta" "window=xo:xo-feat-cinochecks" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/feat-cinochecks)"
  XO_FAKE_CI_LOGS="no CI checks reported - still monitoring until merged or closed"
...
test_ci_monitoring_green_then_rearm_stays_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-cirearm
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cirearm.meta" "window=xo:xo-feat-cirearm" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/feat-cirearm)"
  XO_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
...
test_ci_monitoring_no_checks_yet_stays_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-cinochecksyet
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cinochecksyet.meta" "window=xo:xo-feat-cinochecksyet" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/feat-cinochecksyet)"
  XO_FAKE_CI_LOGS=$(cat <<'EOF'
...
test_ci_monitoring_still_waiting_stays_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-ciwait
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-ciwait.meta" "window=xo:xo-feat-ciwait" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/feat-ciwait)"
  XO_FAKE_CI_LOGS="CI checks running, waiting for results..."
...
test_ci_monitoring_green_then_new_issue_stays_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-cirelapse
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cirelapse.meta" "window=xo:xo-feat-cirelapse" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/feat-cirelapse)"
  XO_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
...
test_ci_ready_done_log_relapse_stays_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-cireadyrelapse
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cireadyrelapse.meta" "window=xo:xo-feat-cireadyrelapse" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-cireadyrelapse.status"
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/feat-cireadyrelapse)"
  XO_FAKE_CI_LOGS=$(cat <<'EOF'
all CI checks passed - still monitoring until merged or closed
base branch advanced (aaaaaaa..bbbbbbb), re-arming CI monitor timeout
CI checks running, waiting for results...
...
test_ci_fixing_after_green_stays_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-cifixing
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-cifixing.meta" "window=xo:xo-feat-cifixing" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-cifixing.status"
  XO_FAKE_AXI_STATUS="$(run_ci_fixing xo/feat-cifixing)"
  XO_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
...
test_top_level_fixing_ci_running_after_green_stays_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-topfixingci
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-topfixingci.meta" "window=xo:xo-feat-topfixingci" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_fixing_ci_running xo/feat-topfixingci)"
  XO_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
...
test_top_level_fixing_done_log_stays_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-topfixing
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-topfixing.meta" "window=xo:xo-feat-topfixing" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/2 checks green\n' > "$d/state/feat-topfixing.status"
  XO_FAKE_AXI_STATUS="$(run_fixing xo/feat-topfixing)"
  XO_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed"
...
test_terminal_passed() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-d
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-d.meta" "window=xo:xo-feat-d" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_passed xo/feat-d)"
...
test_terminal_failed() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-e
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-e.meta" "window=xo:xo-feat-e" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_failed xo/feat-e)"
...
test_terminal_failed_ci_orphan_after_green_reads_done() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-ci-orphan
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-ci-orphan.meta" "window=xo:xo-feat-ci-orphan" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_failed_ci_orphan xo/feat-ci-orphan)"
  XO_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed
...
test_terminal_failed_ci_orphan_status_only_reads_done() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-ci-orphan2
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-ci-orphan2.meta" "window=xo:xo-feat-ci-orphan2" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_failed_ci_orphan_status_only xo/feat-ci-orphan2)"
  XO_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed
daemon shutting down"
...
test_terminal_failed_ci_genuine_red_stays_failed() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-ci-red
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-ci-red.meta" "window=xo:xo-feat-ci-red" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_failed_ci_orphan xo/feat-ci-red)"
  XO_FAKE_CI_LOGS="CI checks running
...
test_terminal_failed_ci_orphan_second_failed_step_stays_failed() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-ci-2fail
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-ci-2fail.meta" "window=xo:xo-feat-ci-2fail" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_failed_ci_orphan_second_failure xo/feat-ci-2fail)"
  XO_FAKE_CI_LOGS="all CI checks passed - still monitoring until merged or closed
daemon shutting down"
...
test_cross_branch_attribution_via_runs_list() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-f
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-f.meta" "window=xo:xo-feat-f" "worktree=$d/wt" "kind=ship"
  # The repo-wide active/most-recent run belongs to a different crew's branch.
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  # Real `no-mistakes runs` shape: plain text, newest-first, no run id, no
  # quoting - "<status> <branch> <short-sha> <date> [<pr-url>]".
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-07-02 22:10
  running    xo/feat-f ${short}  2026-07-02 22:05
EOF
)"
...
test_coarse_socket_refusal_reports_blocked() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-coarse-down
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-coarse-down.meta" "window=xo:xo-feat-coarse-down" "worktree=$d/wt" "kind=ship"
  printf 'blocked: no-mistakes daemon connection refused\n' > "$d/state/feat-coarse-down.status"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-07-02 22:10
  running    xo/feat-coarse-down ${short}  2026-07-02 22:05
EOF
)"
...
test_coarse_failed_ledger_with_daemon_down_reports_unknown() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-coarsedown
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-coarsedown.meta" "window=xo:xo-feat-coarsedown" "worktree=$d/wt" "kind=ship"
  ...
  # worktree's own head.
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="  failed     xo/feat-coarsedown ${short}  2026-09-05 21:00"
  XO_FAKE_DAEMON_DOWN=1
...
test_cross_branch_attribution_picks_most_recent_row() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-fq
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-fq.meta" "window=xo:xo-feat-fq" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-07-02 22:10
  running    xo/feat-fq ${short}  2026-07-02 21:50
  completed  xo/feat-fq bbbbbbb  2026-07-02 20:00  https://github.com/o/r/pull/1
EOF
)"
...
# Live-over-terminal selection (bin/xo-nm-run-lib.sh). Reproduces the proven
...
test_terminal_corpse_loses_to_live_run_on_same_branch() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-corpse
  ...
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/corpse.meta" "window=xo:xo-corpse" "worktree=$d/wt" "kind=ship"
  # The corpse is the most-recently-touched run, so it is what `axi status`
  # reports, at this worktree's own commit.
  XO_FAKE_RUN_HEAD="$base_head"
  XO_FAKE_AXI_STATUS="$(run_failed xo/feat-corpse)"
  ...
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  failed     xo/feat-corpse ${short_base}  2026-08-05 11:20
  running    xo/feat-corpse ${short_live}  2026-08-05 10:05
EOF
)"
...
test_runs_list_live_row_outranks_newer_terminal_row() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-liverow
  base_head=$(git -C "$d/wt" rev-parse HEAD)
  git -C "$d/wt" commit -q --allow-empty -m 'live run advanced the tip'
  live_head=$(git -C "$d/wt" rev-parse HEAD)
  git -C "$d/wt" reset -q --hard "$base_head"
  ...
  short_live=$(git -C "$d/wt" rev-parse --short=7 "$live_head")
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/liverow.meta" "window=xo:xo-liverow" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-08-05 11:30
  failed     xo/feat-liverow ${short_base}  2026-08-05 11:20
  running    xo/feat-liverow ${short_live}  2026-08-05 10:05
EOF
)"
...
test_unfetched_live_sibling_outranks_terminal_row_at_exact_head() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-unfetched
  base_head=$(git -C "$d/wt" rev-parse HEAD)
  short_base=$(git -C "$d/wt" rev-parse --short=7 "$base_head")
  unfetched=0123abc
  ...
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/unfetched.meta" "window=xo:xo-unfetched" "worktree=$d/wt" "kind=ship"
  XO_FAKE_RUN_HEAD="$base_head"
  XO_FAKE_AXI_STATUS="$(run_failed xo/feat-unfetched)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  failed     xo/feat-unfetched ${short_base}  2026-08-05 11:20
  running    xo/feat-unfetched ${unfetched}  2026-08-05 10:05
EOF
)"
...
test_only_terminal_rows_keep_newest_first_precedence() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-allterminal
  base_head=$(git -C "$d/wt" rev-parse HEAD)
  ...
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/allterminal.meta" "window=xo:xo-allterminal" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-08-05 11:30
  cancelled  xo/feat-allterminal ${short_base}  2026-08-05 11:20
  completed  xo/feat-allterminal ${short_older}  2026-08-05 10:05
EOF
)"
...
test_unknown_status_row_keeps_newest_first_precedence() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-unknownrow
  base_head=$(git -C "$d/wt" rev-parse HEAD)
  ...
  short_live=$(git -C "$d/wt" rev-parse --short=7 "$live_head")
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/unknownrow.meta" "window=xo:xo-unknownrow" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-08-05 11:30
  quarantined xo/feat-unknownrow ${short_base}  2026-08-05 11:20
  running    xo/feat-unknownrow ${short_live}  2026-08-05 10:05
EOF
)"
...
test_terminal_run_without_live_sibling_is_unchanged() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-nosibling
  base_head=$(git -C "$d/wt" rev-parse HEAD)
  ...
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/nosibling.meta" "window=xo:xo-nosibling" "worktree=$d/wt" "kind=ship"
  XO_FAKE_RUN_HEAD="$base_head"
  XO_FAKE_AXI_STATUS="$(run_failed xo/feat-nosibling)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  failed     xo/feat-nosibling ${short_base}  2026-08-05 11:20
  completed  xo/feat-nosibling ${short_other}  2026-08-05 10:05
EOF
)"
...
test_coarse_run_does_not_probe_other_branch_ci_log_for_ready_status() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-coarseready
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-coarseready.meta" "window=xo:xo-feat-coarseready" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/4 checks green\n' > "$d/state/feat-coarseready.status"
  XO_FAKE_AXI_STATUS="$(run_ci_monitoring xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-07-02 22:10
  running    xo/feat-coarseready ${short}  2026-07-02 22:05
EOF
)"
  XO_FAKE_CI_LOGS="CI checks running, waiting for results..."
...
test_other_branch_run_ignored() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-g
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-g.meta" "window=xo:xo-feat-g" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'done: implemented, ready to validate\n' > "$d/state/feat-g.status"
  XO_FAKE_AXI_STATUS="$(run_running xo/some-other)"
  XO_FAKE_RUNS_LIST="$(cat <<'EOF'
  running    xo/some-other aaaaaaa  2026-07-02 22:10
EOF
)"
  XO_FAKE_BUSY=0
...
test_no_run_busy_pane() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-h
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-h.meta" "window=xo:xo-feat-h" "worktree=$d/wt" "kind=ship" "harness=claude"
  # No matching run anywhere. The busy verdict comes from the crew's own
  # semantic lifecycle record (bin/xo-busy-lib.sh), not from rendered text.
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$d/state" feat-h)
  "$ROOT/bin/xo-busy-event.sh" apply "$d/state" feat-h busy --gen "$gen" \
...
test_no_run_footer_text_alone_is_not_working() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-h2
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-h2.meta" "window=xo:xo-feat-h2" "worktree=$d/wt" "kind=ship" "harness=claude"
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_BUSY=1
...
test_no_run_grok_uses_isolated_fallback() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-h3
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-h3.meta" "window=xo:xo-feat-h3" "worktree=$d/wt" "kind=ship" "harness=grok"
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_BUSY=1
  XO_FAKE_BUSY_TEXT='Ctrl+c:cancel'
  export XO_FAKE_BUSY_TEXT
...
test_no_run_herdr_unknown_uses_backend_capture() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-herdr
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-herdr.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_TMUX_MISSING=1
  XO_FAKE_HERDR_BUSY=1
  XO_FAKE_HERDR_AGENT_STATUS=working
...
test_no_run_herdr_cli_failure_reads_unreachable_not_gone() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-herdr-cli
  make_fakebin "$d" >/dev/null
...
SH
  ...
  xo_write_meta "$d/state/feat-herdr-cli.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_TMUX_MISSING=1
...
test_no_run_herdr_alive_with_failed_read_stays_live() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-herdr-alive
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-herdr-alive.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_TMUX_MISSING=1
...
test_no_run_herdr_stale_registration_over_shell_reads_agent_gone() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-herdr-stale
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-herdr-stale.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=pi"
  XO_FAKE_TMUX_MISSING=1
  XO_FAKE_HERDR_READ_FAIL=1
  XO_FAKE_HERDR_AGENT_STATUS=idle
  XO_FAKE_HERDR_PROCESS=shell
...
test_no_run_herdr_stale_working_record_is_never_busy() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-herdr-stale-working
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-herdr-stale-working.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=pi"
  XO_FAKE_TMUX_MISSING=1
  XO_FAKE_HERDR_AGENT_STATUS=working
  XO_FAKE_HERDR_PROCESS=shell
...
test_no_run_herdr_husk_dead_still_reads_gone() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-herdr-husk
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-herdr-husk.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_TMUX_MISSING=1
...
test_no_run_herdr_idle_agent_status_outranked_by_record() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-herdr-idle
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-herdr-idle.meta" "window=default:w1:p3" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  ...
  XO_FAKE_HERDR_AGENT_STATUS=idle
  XO_FAKE_HERDR_BUSY=0
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$d/state" feat-herdr-idle)
  "$ROOT/bin/xo-busy-event.sh" apply "$d/state" feat-herdr-idle busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
...
test_no_run_herdr_idle_agent_status_and_idle_record_stays_idle() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-herdr-stopped
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-herdr-stopped.meta" "window=default:w1:p4" "worktree=$d/wt" "kind=ship" \
    "backend=herdr" "harness=claude"
  ...
  XO_FAKE_HERDR_AGENT_STATUS=idle
  XO_FAKE_HERDR_BUSY=0
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$d/state" feat-herdr-stopped)
  "$ROOT/bin/xo-busy-event.sh" apply "$d/state" feat-herdr-stopped idle --gen "$gen" \
    --source claude-hook --event stop
...
test_no_run_idle_pane_uses_log() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-i
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-i.meta" "window=xo:xo-feat-i" "worktree=$d/wt" "kind=ship" "harness=claude"
...
test_no_run_idle_pane_uses_keyed_log() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-keyed
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-keyed.meta" "window=xo:xo-feat-keyed" "worktree=$d/wt" "kind=ship" "harness=claude"
...
test_no_run_idle_pane_paused() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-pause
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-pause.meta" "window=xo:xo-feat-pause" "worktree=$d/wt" "kind=ship" "harness=claude"
...
test_no_run_idle_pane_custom_paused_verb() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-custom-pause
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-custom-pause.meta" "window=xo:xo-feat-custom-pause" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'awaiting: vendor maintenance window\n' > "$d/state/feat-custom-pause.status"
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_BUSY=0
  arm_idle_record "$d/state" feat-custom-pause
  local out; out=$(XO_CLASSIFY_PAUSED_VERB=awaiting run_crew_state "$d" feat-custom-pause)
  ...
  out=$(XO_CLASSIFY_PAUSED_VERB=awaiting run_crew_state "$d" feat-custom-pause)
...
# state=unknown with resolution prose. The one-owner keyed fold in xo-classify-lib.sh
...
test_no_run_idle_secondmate_resolved_event_not_state() {
  ...
  mkdir -p "$d/wt"
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/mate.meta" "window=xo:xo-mate" "worktree=$d/wt" "kind=secondmate" "home=$d/wt"
...
test_dead_window_ignores_stale_status_log() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-dead
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-dead.meta" "window=xo:xo-feat-dead" "worktree=$d/wt" "kind=ship"
...
# xo_backend_tmux_agent_state classifies it as `missing` so xo-bootstrap and
# xo-session-start can respawn after a genuine server death.)
test_no_run_tmux_unreadable_reads_unreachable_not_gone() {
  reset_fakes
  local d; d=$(new_case tmux-unreadable)
  make_repo_on_branch "$d/wt" xo/feat-tmux-unread
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-tmux-unread.meta" "window=xo:xo-feat-tmux-unread" \
    "worktree=$d/wt" "kind=ship"
  printf 'done: old completion event\n' > "$d/state/feat-tmux-unread.status"
  XO_FAKE_AXI_STATUS=""
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_TMUX_UNREADABLE=1
...
# PR leaves a gone endpoint while xo has already recorded pr= and armed
...
# The fixtures lay out the records exactly as bin/xo-pr-check.sh leaves them - a
...
arm_held_records() {  # <case-dir> <id> [worktree] [extra meta kv...]
  ...
  xo_write_meta "$d/state/$id.meta" "window=xo:xo-$id" "worktree=$wt" "kind=ship" "pr=$HELD_PR" "$@"
...
test_gone_window_held_for_merge_reads_done() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-held-gone
  make_fakebin "$d" >/dev/null
...
test_gone_window_held_for_merge_does_not_depend_on_status_log() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-held-resolved
  make_fakebin "$d" >/dev/null
  arm_held_records "$d" feat-held-resolved "$d/wt"
  printf 'done: PR %s checks green\nresolved [key=nm-01RUN-review]: xo accepted the finding\n' "$HELD_PR" \
...
test_herdr_husk_held_for_merge_reads_done() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-held-herdr
  make_fakebin "$d" >/dev/null
  arm_held_records "$d" feat-held-herdr "$d/wt" "backend=herdr" "harness=claude"
  # bin/xo-pr-check.sh records pr= as the last line; a meta whose backend keys
  # follow it must still read as the herdr task it is.
  xo_write_meta "$d/state/feat-held-herdr.meta" "window=default:w1:p2" "worktree=$d/wt" "kind=ship" \
...
test_gone_window_needs_both_held_records() {
  ...
  do
    ...
    make_repo_on_branch "$d/wt" "xo/$id"
    make_fakebin "$d" >/dev/null
    case "${spec##*|}" in
      pr-only)
        xo_write_meta "$d/state/$id.meta" "window=xo:xo-$id" "worktree=$d/wt" "kind=ship" "pr=$HELD_PR" ;;
      check-only)
        xo_write_meta "$d/state/$id.meta" "window=xo:xo-$id" "worktree=$d/wt" "kind=ship"
        printf '#!/usr/bin/env bash\nexit 0\n' > "$d/state/$id.check.sh"
        chmod 0700 "$d/state/$id.check.sh" ;;
      scout)
        arm_held_records "$d" "$id" "$d/wt"
        xo_write_meta "$d/state/$id.meta" "window=xo:xo-$id" "worktree=$d/wt" "kind=scout" "pr=$HELD_PR" ;;
    esac
...
test_unreachable_backend_with_held_records_stays_unknown() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-held-unreach
  make_fakebin "$d" >/dev/null
...
test_held_records_do_not_override_live_pane_or_run_step() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-held-live
  make_fakebin "$d" >/dev/null
  arm_held_records "$d" feat-held-live "$d/wt" "harness=claude"
  xo_write_meta "$d/state/feat-held-live.meta" "window=xo:xo-feat-held-live" "worktree=$d/wt" "kind=ship" \
  ...
  make_repo_on_branch "$d/wt" xo/feat-held-run
  make_fakebin "$d" >/dev/null
  ...
  XO_FAKE_AXI_STATUS=$(run_running xo/feat-held-run)
  XO_FAKE_TMUX_MISSING=1
...
test_dead_window_still_reports_terminal_run_step() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-dead-done
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-dead-done.meta" "window=xo:xo-feat-dead-done" "worktree=$d/wt" "kind=ship"
  printf 'done: PR https://github.com/o/r/pull/3 checks green\n' > "$d/state/feat-dead-done.status"
  XO_FAKE_AXI_STATUS="$(run_passed xo/feat-dead-done)"
  XO_FAKE_TMUX_MISSING=1   # the crew's window has closed
...
test_dead_window_still_reports_active_run_step() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-dead-act
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-dead-act.meta" "window=xo:xo-feat-dead-act" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running xo/feat-dead-act)"
  XO_FAKE_TMUX_MISSING=1
...
test_no_timeout_uses_perl_bound() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-timeout
  make_fakebin "$d" >/dev/null
  calls_file="$d/no-mistakes.calls"
  : > "$calls_file"
  cat > "$d/fakebin/no-mistakes" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "${XO_FAKE_NM_CALLS:-/dev/null}"
while :; do :; done
SH
  chmod +x "$d/fakebin/no-mistakes"
  toolbin=$(make_no_timeout_toolbin "$d")
  xo_write_meta "$d/state/feat-timeout.meta" "window=xo:xo-feat-timeout" "worktree=$d/wt" "kind=ship" \
    "harness=claude"
  XO_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$d/state" feat-timeout)
  "$ROOT/bin/xo-busy-event.sh" apply "$d/state" feat-timeout busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
  start=$SECONDS
  out=$(XO_FAKE_NM_CALLS="$calls_file" PATH="$d/fakebin:$toolbin" XO_STATE_OVERRIDE="$d/state" XO_CREW_STATE_NM_TIMEOUT=1 "$CREW_STATE" feat-timeout)
...
test_scout_skips_run_lookup() {
  ...
  make_repo_on_branch "$d/wt" xo/scout-j
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/scout-j.meta" "window=xo:xo-scout-j" "worktree=$d/wt" "kind=scout" \
    "harness=claude"
  # Even if a run existed on this branch, a scout must not read it.
  XO_FAKE_AXI_STATUS="$(run_running xo/scout-j)"
  XO_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$d/state" scout-j)
  "$ROOT/bin/xo-busy-event.sh" apply "$d/state" scout-j busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
...
test_torn_down_worktree() {
  ...
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/gone-k.meta" "window=xo:xo-gone-k" "worktree=$d/no-such-worktree" "kind=ship"
  local out rc
...
# "worktree gone". These cases drive the real helper over the real xo-on.sh
# route with a stubbed ssh transport (XO_SSH_BIN seam): the stub prints
# XO_FAKE_REMOTE_STATE_OUT as the remote endpoint's recovery-grade state and
# exits XO_FAKE_SSH_RC.

setup_remote_case() {  # <name> -> echoes case dir with remote meta + registry
  local d
  d=$(new_case "$1")
  mkdir -p "$d/data" "$d/fakebin"
  xo_write_meta "$d/state/rsm.meta" \
    "window=remote:rsm" \
    ...
    "remote_herdr_session=xo-remote" \
    "remote_target=xo-remote:w1:p1"
...
#!/usr/bin/env bash
cat > /dev/null
[ -z "${XO_FAKE_REMOTE_STATE_OUT:-}" ] || printf '%s\n' "$XO_FAKE_REMOTE_STATE_OUT"
exit "${XO_FAKE_SSH_RC:-0}"
SH
...
run_remote_crew_state() {  # <case-dir> <id>
  PATH="$1/fakebin:$PATH" XO_HOME="$1" XO_STATE_OVERRIDE="$1/state" \
    XO_SSH_BIN="$1/fakebin/fake-ssh" "$CREW_STATE" "$2"
}
...
test_remote_alive_with_log_uses_status_log() {
  ...
  out=$(XO_FAKE_REMOTE_STATE_OUT=alive XO_FAKE_SSH_RC=0 run_remote_crew_state "$d" rsm); rc=$?
  ...
  pass "xo-crew-state remote: alive endpoint falls through to the routed status log"
}
...
test_remote_alive_idle_is_healthy_not_gone() {
  ...
  pass "xo-crew-state remote: an idle alive endpoint reads alive, never gone or dead"
}
...
test_remote_unreachable_is_unknown_remote_not_dead() {
  ...
  make_fakebin "$d" >/dev/null
  printf 'working: refactoring the quota adapter\n' > "$d/state/rsm.status"
  out=$(XO_FAKE_SSH_RC=255 run_remote_crew_state "$d" rsm); rc=$?
  ...
  pass "xo-crew-state remote: an unreachable host reads unknown-remote, never gone or dead"
}
...
test_remote_dead_reports_remote_verdict() {
  ...
  make_fakebin "$d" >/dev/null
  out=$(XO_FAKE_REMOTE_STATE_OUT=dead XO_FAKE_SSH_RC=0 run_remote_crew_state "$d" rsm); rc=$?
  ...
  pass "xo-crew-state remote: the remote host's own dead verdict is reported truthfully"
}
...
# (k) crew_is_provably_working end-to-end over the REAL xo-crew-state.sh (not a
# canned fake verdict, unlike tests/xo-watch-triage.test.sh's classifier
...
test_provably_working_via_runs_list_fallback() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-provable
  short=$(git -C "$d/wt" rev-parse --short=7 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-provable.meta" "window=xo:xo-feat-provable" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-07-02 22:10
  running    xo/feat-provable ${short}  2026-07-02 22:05
EOF
)"
  PATH="$d/fakebin:$PATH" XO_STATE_OVERRIDE="$d/state" crew_is_provably_working feat-provable \
...
test_not_provably_working_when_stopped() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-stopped
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-stopped.meta" "window=xo:xo-feat-stopped" "worktree=$d/wt" "kind=ship"
...
EOF
  ...
  XO_FAKE_BUSY=0
  PATH="$d/fakebin:$PATH" XO_STATE_OVERRIDE="$d/state" crew_is_provably_working feat-stopped \
...
test_historical_same_branch_rewritten_head_not_current() {
  ...
  make_repo_on_branch "$d/wt" xo/todo-flag
  ...
  git -C "$d/wt" branch -q -M xo/todo-flag
  new_head=$(git -C "$d/wt" rev-parse HEAD)
  [ "$old_head" != "$new_head" ] || fail "rewrite did not produce a new head"
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/wishlist.meta" "window=xo:xo-wishlist" "worktree=$d/wt" "kind=ship" "harness=claude"
  ...
  XO_FAKE_RUN_HEAD="$old_head"
  XO_FAKE_AXI_STATUS="$(run_parked xo/todo-flag)"
  XO_FAKE_BUSY=0
...
test_active_run_descendant_fix_head_remains_current() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-pipeline
  base_head=$(git -C "$d/wt" rev-parse HEAD)
  ...
  git -C "$d/wt" reset -q --hard "$base_head"
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/pipe.meta" "window=xo:xo-pipe" "worktree=$d/wt" "kind=ship"
  XO_FAKE_RUN_HEAD="$fix_head"
  XO_FAKE_AXI_STATUS="$(run_fixing xo/feat-pipeline)"
...
test_local_advanced_past_run_head_invalidates() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-adv
  run_head=$(git -C "$d/wt" rev-parse HEAD)
  git -C "$d/wt" commit -q --allow-empty -m 'local stage-2 work after prior run'
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/adv.meta" "window=xo:xo-adv" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: stage 2 implementation in progress\n' > "$d/state/adv.status"
  XO_FAKE_RUN_HEAD="$run_head"
  XO_FAKE_AXI_STATUS="$(run_parked xo/feat-adv)"
  XO_FAKE_BUSY=0
...
test_pipeline_owned_active_run_beats_superseded_failed_row() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-f10
  short=$(git -C "$d/wt" rev-parse --short=8 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-f10.meta" "window=xo:xo-feat-f10" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running_pipeline_owned xo/feat-f10 f0f0f0f0)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/feat-f10 f0f0f0f0  2026-08-27 13:53
  failed     xo/feat-f10 ${short}  2026-08-27 12:09
EOF
)"
...
test_failed_run_with_no_later_run_still_surfaces() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-f10b
  short=$(git -C "$d/wt" rev-parse --short=8 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-f10b.meta" "window=xo:xo-feat-f10b" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_failed xo/feat-f10b)"
  XO_FAKE_RUNS_LIST="  failed     xo/feat-f10b ${short}  2026-08-27 12:09"
...
test_coarse_unresolvable_active_row_never_falls_to_older_row() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-f10c
  short=$(git -C "$d/wt" rev-parse --short=8 HEAD)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-f10c.meta" "window=xo:xo-feat-f10c" "worktree=$d/wt" "kind=ship" "harness=claude"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-08-27 14:00
  running    xo/feat-f10c f0f0f0f0  2026-08-27 13:53
  failed     xo/feat-f10c ${short}  2026-08-27 12:09
EOF
)"
  XO_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$d/state" feat-f10c)
  "$ROOT/bin/xo-busy-event.sh" apply "$d/state" feat-f10c busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
...
test_coarse_mismatched_anchor_falls_to_pane_not_older_row() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-f10g
  git -C "$d/wt" commit -q --allow-empty -m 'second local commit'
  old_short=$(git -C "$d/wt" rev-parse --short=8 HEAD~1)
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-f10g.meta" "window=xo:xo-feat-f10g" "worktree=$d/wt" "kind=ship" "harness=claude"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-08-27 14:00
  running    xo/feat-f10g f0f0f0f0  2026-08-27 13:53
  failed     xo/feat-f10g ${old_short}  2026-08-27 12:09
EOF
)"
  XO_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$d/state" feat-f10g)
  "$ROOT/bin/xo-busy-event.sh" apply "$d/state" feat-f10g busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
...
test_non_pipeline_owned_unresolvable_head_not_attributed() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-f10d
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-f10d.meta" "window=xo:xo-feat-f10d" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: implementing\n' > "$d/state/feat-f10d.status"
  XO_FAKE_AXI_STATUS="$(run_running_pipeline_owned xo/feat-f10d f0f0f0f0 synced)"
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_BUSY=0
...
test_pipeline_owned_terminal_run_not_exempt() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-f10e
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/feat-f10e.meta" "window=xo:xo-feat-f10e" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: stage 2 in progress\n' > "$d/state/feat-f10e.status"
  XO_FAKE_AXI_STATUS="$(run_running_pipeline_owned xo/feat-f10e f0f0f0f0)
outcome: failed"
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_BUSY=0
...
test_missing_run_head_falls_back_to_current_state() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-no-head
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/no-head.meta" "window=xo:xo-no-head" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: current stage still in progress\n' > "$d/state/no-head.status"
  XO_FAKE_AXI_STATUS=$(run_parked xo/feat-no-head | grep -v '^  head:')
  XO_FAKE_RUNS_LIST=""
  XO_FAKE_BUSY=0
...
test_active_fix_round_unfetched_pipeline_head_reports_current() {
  ...
  xo_write_meta "$d/state/unfetched.meta" "window=xo:xo-unfetched" "worktree=$d/wt" "kind=ship"
  XO_FAKE_RUN_HEAD="$h2"
  XO_FAKE_AXI_STATUS="$(run_fixing xo/feat-unfetched)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other aaaaaaa  2026-07-30 22:10
  running    xo/feat-unfetched $(git -C "$d/wt.pipe" rev-parse --short=7 HEAD)  2026-07-30 22:05
  failed     xo/feat-unfetched $(git -C "$d/wt" rev-parse --short=7 HEAD)  2026-07-29 20:00
EOF
)"
  out=$(run_crew_state "$d" unfetched)
...
test_unanchored_unfetched_active_row_does_not_match() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-noanchor
  ...
  h2=$(mint_unfetched_fix_head "$d/wt")
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/noanchor.meta" "window=xo:xo-noanchor" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'failed: earlier stage run\n' > "$d/state/noanchor.status"
  XO_FAKE_RUN_HEAD="$h2"
  XO_FAKE_AXI_STATUS="$(run_fixing xo/feat-noanchor)"
  ...
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other aaaaaaa  2026-07-30 22:10
  running    xo/feat-noanchor $(git -C "$d/wt.pipe" rev-parse --short=7 HEAD)  2026-07-30 22:05
  failed     xo/feat-noanchor $(git -C "$d/wt" rev-parse --short=7 HEAD~1)  2026-07-29 20:00
EOF
)"
  XO_FAKE_BUSY=0
...
test_unresolved_terminal_row_is_history_not_current() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-hist
  ...
  git -C "$d/wt" checkout -q --orphan tmp-rewrite
  git -C "$d/wt" commit -q --allow-empty -m 'rewritten tip'
  git -C "$d/wt" branch -q -M xo/feat-hist
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/hist.meta" "window=xo:xo-hist" "worktree=$d/wt" "kind=ship" "harness=claude"
  printf 'working: stage 2 in progress\n' > "$d/state/hist.status"
  XO_FAKE_RUN_HEAD="$h_old"
  XO_FAKE_AXI_STATUS="$(run_failed xo/feat-hist)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  failed     xo/feat-hist $(git -C "$d/wt.pipe" rev-parse --short=7 HEAD)  2026-07-01 20:00
EOF
)"
  XO_FAKE_BUSY=0
...
test_runs_list_continuation_found_when_axi_answers_other_branch() {
  ...
  make_repo_on_branch "$d/wt" xo/feat-coarsefix
  h1=$(git -C "$d/wt" rev-parse HEAD)
  h2=$(mint_unfetched_fix_head "$d/wt")
  make_fakebin "$d" >/dev/null
  xo_write_meta "$d/state/coarsefix.meta" "window=xo:xo-coarsefix" "worktree=$d/wt" "kind=ship"
  XO_FAKE_AXI_STATUS="$(run_running xo/other-crew)"
  XO_FAKE_RUNS_LIST="$(cat <<EOF
  running    xo/other-crew aaaaaaa  2026-07-30 22:10
  running    xo/feat-coarsefix $(git -C "$d/wt.pipe" rev-parse --short=7 HEAD)  2026-07-30 22:05
  failed     xo/feat-coarsefix $(git -C "$d/wt" rev-parse --short=7 HEAD)  2026-07-29 20:00
EOF
)"
...
echo "all xo-crew-state tests passed"
