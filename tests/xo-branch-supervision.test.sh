...
# (bin/xo-branch-prompt.sh), the append-only outcome store
# (bin/xo-branch-outcome.sh), the per-task lease contract (bin/xo-lease.sh,
# bin/xo-lease-lib.sh), the lease and role-partition guards wired into the
...
TMP_ROOT=$(xo_test_tmproot xo-branch-supervision)
...
test_branch_prompt_is_byte_stable_and_above_cache_floor() {
  ...
  out_a=$(cd "$TMP_ROOT" && XO_HOME="$home_a" TZ=UTC "$ROOT/bin/xo-branch-prompt.sh") \
    || fail "branch prompt generator failed for home A"
  out_b=$(cd / && XO_HOME="$home_b" TZ=Australia/Eucla "$ROOT/bin/xo-branch-prompt.sh") \
    || fail "branch prompt generator failed for home B"
  sleep 1
  out_c=$("$ROOT/bin/xo-branch-prompt.sh") || fail "branch prompt generator failed on re-run"
...
test_outcome_store_is_append_only_with_cursor_reads() {
  ...
  seq1=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" append \
  ...
  seq2=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" append \
...
PY
  ...
  snapshot=$(cat "$store")
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 1 || fail "mark-read failed"
  unread=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unread) || fail "unread failed"
  case "$unread" in
    '{"seq":2,'*) ;;
  ...
  replay=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" startup-replay) || fail "startup-replay failed"
  ...
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 2 \
  ...
  snapshot=$(cat "$store")
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" append \
...
test_outcome_startup_replay_preserves_silence() {
  ...
  replay=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" startup-replay) || fail "mixed startup replay failed"
  ...
  [ -z "$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unread)" ] \
  ...
  replay=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" startup-replay) || fail "legacy startup replay failed"
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unread 2>&1)
  status=$?
...
test_outcome_startup_replay_stops_at_captain_barrier() {
  ...
  replay=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" startup-replay) || fail "barrier replay failed"
  ...
  unread=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unread) || fail "barrier unread failed"
...
test_outcome_cursor_corruption_fails_closed() {
  ...
  snapshot=$(cat "$store")
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 01 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 1 2>&1)
  status=$?
...
test_cursor_advancement_refuses_ahead_processed_marker() {
  ...
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 1 || fail "fixture mark-read failed"
  printf '3\n' > "$marker"

  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 3 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" startup-replay 2>&1)
  status=$?
...
test_outcome_sequence_conflicts_fail_closed() {
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" list --recent 2 2>&1)
  status=$?
...
test_outcome_non_jsonl_layout_fails_closed() {
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" list 2>&1)
  status=$?
...
test_outcome_processed_marker_is_sequence_bound() {
  ...
  [ -z "$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unprocessed)" ] \
    || fail "an unread captain row was reported as unprocessed"
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 2 || fail "mark-read failed"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unprocessed) || fail "unprocessed failed"
  case "$out" in
    '{"seq":2,'*) ;;
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-processed --through 3 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-processed --through 1 2>&1)
  status=$?
  ...
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-processed --through 2 || fail "mark-processed failed"
  ...
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 3 || fail "second mark-read failed"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unprocessed) || fail "second unprocessed failed"
  case "$out" in
    '{"seq":3,'*) ;;
  ...
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" processed-init || fail "processed-init failed on a present marker"
  [ "$(cat "$marker")" = 2 ] || fail "processed-init rewrote a present marker"
  printf '2x\n' > "$marker"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unprocessed 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" processed-init 2>&1)
  status=$?
  ...
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" mark-read --through 1 || fail "migration mark-read failed"
  assert_contains "$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unprocessed)" '"seq":1' \
    "an absent marker hid a delivered captain row instead of reading as zero"
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" processed-init || fail "migration processed-init failed"
...
test_lease_exclusivity_release_stale_and_sweep() {
  ...
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-1 --actor branch \
    || fail "branch claim failed"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-lease.sh" check task-1) || fail "check missed a held lease"
  case "$out" in
    "branch $$ "*" live") ;;
    *) fail "check misreported the lease: $out" ;;
  esac
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=main XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-1 2>&1)
  status=$?
  ...
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-1 \
    || fail "same-actor refresh was refused"

  # Release by the calling holder; release of an unheld lease stays a silent no-op.
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-lease.sh" release task-1 --actor branch || fail "release failed"
  XO_HOME="$home" "$ROOT/bin/xo-lease.sh" check task-1 >/dev/null && fail "released lease still reported"
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-lease.sh" release task-1 --actor branch || fail "idempotent release failed"
  ...
  XO_HOME="$home" XO_SUPERVISION_ACTOR=main XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-dead \
    || fail "stale lease blocked a live claim"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-lease.sh" check task-dead)
  case "$out" in "main $$ "*) ;; *) fail "stale lease was not taken over: $out" ;; esac
  printf 'branch\t999999\t123\n' > "$home/state/.lease-task-dead2"
  XO_HOME="$home" "$ROOT/bin/xo-lease.sh" sweep || fail "sweep failed"
  ...
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim backlog --actor branch \
    || fail "backlog lease claim failed"
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=main XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim backlog 2>&1)
...
test_mutating_scripts_refuse_the_other_actors_lease() {
  ...
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-held --actor branch \
    || fail "fixture lease claim failed"

  # xo-control: refused while the branch holds the lease; the ordinary no-task
  # error (a different failure) proves pass-through once the lease is gone.
  out=$(XO_HOME="$home" "$ROOT/bin/xo-control.sh" task-held interrupt 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "leased xo-control exited $status, not 6: $out"
  assert_contains "$out" "leased to the branch supervision actor" "xo-control refusal lost the holder"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-control.sh" task-unheld interrupt 2>&1)
  status=$?
  [ "$status" -ne 6 ] || fail "unleased xo-control still hit the lease refusal"
  assert_contains "$out" "no task 'task-unheld'" "unleased xo-control lost its ordinary error"

  # xo-teardown: same refusal shape before any teardown work.
  out=$(XO_HOME="$home" "$ROOT/bin/xo-teardown.sh" task-held 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "leased xo-teardown exited $status, not 6: $out"
  assert_contains "$out" "teardown (xo-teardown) refused" "xo-teardown refusal lost its action label"

  # The same lease refuses the BRANCH actor when MAIN holds it - the guard is
  # symmetric, not a branch-only fence.
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-lease.sh" release task-held --actor branch
  XO_HOME="$home" XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-held --actor main \
    || fail "main fixture claim failed"
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-control.sh" task-held interrupt 2>&1)
  status=$?
  ...
  pass "xo-control and xo-teardown refuse the other actor's live lease and pass through otherwise"
}
...
test_main_owned_actions_refuse_the_branch_actor() {
  ...
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-pr-merge.sh" task-x https://github.com/o/r/pull/1 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "branch xo-pr-merge exited $status, not 6: $out"
  assert_contains "$out" "the supervision branch never performs this action" "pr-merge refusal lost the partition wording"

  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-merge-local.sh" task-x 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "branch xo-merge-local exited $status, not 6: $out"

  out=$(XO_HOME="$home" XO_ROOT_OVERRIDE="$root" XO_SUPERVISION_ACTOR=branch \
    "$ROOT/bin/xo-spawn.sh" task-new --mode no-mistakes --yolo off 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "branch xo-spawn exited $status, not 6: $out"
  assert_contains "$out" "new-task spawn (xo-spawn) refused" "spawn refusal lost its action label"

  # The same calls as MAIN fail on their ORDINARY validation instead - the
  # partition guard never fires for the main actor.
  out=$(XO_HOME="$home" "$ROOT/bin/xo-merge-local.sh" task-x 2>&1)
  status=$?
  [ "$status" -ne 6 ] || fail "main xo-merge-local hit the partition refusal"
  assert_contains "$out" "no meta for task task-x" "main xo-merge-local lost its ordinary error"
...
test_home_without_branch_is_untouched() {
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-control.sh" task-any interrupt 2>&1)
  status=$?
  [ "$status" -ne 6 ] || fail "no-branch home hit a lease refusal in xo-control"
  assert_contains "$out" "no task 'task-any'" "no-branch xo-control lost its ordinary error"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-pr-merge.sh" 2>&1)
  status=$?
  [ "$status" -eq 2 ] || fail "no-branch xo-pr-merge usage error changed: $status: $out"
  ...
  out=$(STATE="$home/state" bash -c '. "$1"; xo_lease_guard task-reused "probe"; xo_lease_forbid_branch "probe"; echo silent-pass' _ "$ROOT/bin/xo-lease-lib.sh" 2>&1)
  ...
  out=$(env -u PI_CODING_AGENT -u XO_SUPERVISION_ACTOR CLAUDECODE=1 STATE="$home/state" bash -c '. "$1"; xo_lease_guard task-reused "probe"; echo silent-pass' _ "$ROOT/bin/xo-lease-lib.sh" 2>&1)
...
test_lease_liveness_binds_to_the_session_lock() {
  ...
  printf 'branch\t%s\t123\n' "$PPID" > "$home/state/.lease-task-reused"
  printf '%s\n' "$$" > "$home/state/.lock"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-lease.sh" check task-reused) || fail "check missed the leftover lease"
  case "$out" in
    *" stale") ;;
  ...
  XO_HOME="$home" XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-current --actor main \
    || fail "claim under the current lock holder failed"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-lease.sh" check task-current)
  case "$out" in
    "main $$ "*" live") ;;
  ...
  printf '%sjunk\n' "$$" > "$home/state/.lock"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-lease.sh" check task-current) || fail "check missed the lease under a malformed lock"
  case "$out" in
    *" stale") ;;
    *) fail "a malformed lock proved lease liveness: $out" ;;
  esac
  XO_HOME="$home" "$ROOT/bin/xo-lease.sh" sweep || fail "sweep under malformed lock failed"
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "$XO_TEST_LEASE_PATH" ] && mkdir "$XO_TEST_GATE.once" 2>/dev/null; then
  : > "$XO_TEST_GATE.ready"
  while [ ! -e "$XO_TEST_GATE.release" ]; do sleep 0.01; done
fi
exec "$XO_TEST_REAL_MV" "$@"
SH
  chmod +x "$fakebin/mv"

  PATH="$fakebin:$PATH" XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ \
    XO_TEST_REAL_MV="$real_mv" XO_TEST_LEASE_PATH="$home/state/.lease-task-race" XO_TEST_GATE="$home/state/gate" \
    "$ROOT/bin/xo-lease.sh" claim task-race --actor branch >/dev/null 2>&1 &
  branch_pid=$!
  while [ ! -e "$home/state/gate.ready" ]; do sleep 0.01; done
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_SUPERVISION_ACTOR=main XO_LEASE_HOLDER_PID=$$ \
    XO_TEST_REAL_MV="$real_mv" XO_TEST_LEASE_PATH="$home/state/.lease-task-race" XO_TEST_GATE="$home/state/gate" \
    "$ROOT/bin/xo-lease.sh" claim task-race --actor main >/dev/null 2>&1 &
  main_pid=$!
  sleep 0.1
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "$XO_TEST_STALE_PATH" ] && mkdir "$XO_TEST_GATE.once" 2>/dev/null; then
  : > "$XO_TEST_GATE.ready"
  while [ ! -e "$XO_TEST_GATE.release" ]; do sleep 0.01; done
fi
exec "$XO_TEST_REAL_RM" "$@"
SH
  chmod +x "$fakebin/rm"

  PATH="$fakebin:$PATH" STATE="$home/state" XO_TEST_REAL_RM="$real_rm" \
    XO_TEST_STALE_PATH="$home/state/.lease-task-race" XO_TEST_GATE="$home/state/gate" \
    bash -c '. "$1"; xo_lease_guard task-race "probe"' _ "$ROOT/bin/xo-lease-lib.sh" &
  guard_pid=$!
  while [ ! -e "$home/state/gate.ready" ]; do sleep 0.01; done
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ \
    XO_TEST_REAL_RM="$real_rm" XO_TEST_STALE_PATH="$home/state/.lease-task-race" XO_TEST_GATE="$home/state/gate" \
    "$ROOT/bin/xo-lease.sh" claim task-race --actor branch >/dev/null 2>&1 &
  claim_pid=$!
  sleep 0.1
  : > "$home/state/gate.release"
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-lease.sh" check task-race) || fail "guard deleted the newer lease claim"
  case "$out" in
    "branch $$ "*" live") ;;
...
test_guard_holds_exclusivity_through_mutation() {
  ...
  PI_CODING_AGENT=true STATE="$home/state" XO_TEST_READY="$home/operation-ready" \
    XO_TEST_RELEASE="$home/operation-release" bash -c '
      . "$1"
      xo_lease_guard task-race "probe"
      trap "xo_lease_guard_release" EXIT
      : > "$XO_TEST_READY"
      while [ ! -e "$XO_TEST_RELEASE" ]; do sleep 0.01; done
    ' _ "$ROOT/bin/xo-lease-lib.sh" &
  operation_pid=$!
  while [ ! -e "$home/operation-ready" ]; do sleep 0.01; done

  PI_CODING_AGENT=true XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ \
    "$ROOT/bin/xo-lease.sh" claim task-race --actor branch >/dev/null 2>&1 &
  claim_pid=$!
  sleep 0.2
...
test_claim_refuses_the_other_actors_name_loudly() {
  ...
  mkdir -p "$home/state"
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ \
    "$ROOT/bin/xo-lease.sh" claim task-z --actor main 2>&1)
  status=$?
...
test_release_actor_drops_only_that_actors_leases() {
  ...
  mkdir -p "$home/state"
  printf '%s\n' "$$" > "$home/state/.lock"
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-a --actor branch \
    || fail "branch claim failed"
  XO_HOME="$home" XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-b --actor main \
    || fail "main claim failed"
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-lease.sh" release task-b --actor main 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-lease.sh" release-actor --actor main 2>&1)
  status=$?
  ...
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-lease.sh" release-actor --actor branch || fail "release-actor failed"
...
test_branch_cannot_force_teardown_or_directly_relaunch() {
  ...
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-teardown.sh" task-x --force 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" XO_SUPERVISION_ACTOR=branch "$ROOT/bin/xo-teardown.sh" task-x 2>&1)
  status=$?
  [ "$status" -ne 6 ] || fail "ordinary branch teardown hit the forced-discard refusal: $out"

  # A branch relaunch is legal only through xo-control's owned transaction,
  # never by direct xo-spawn invocation.
  out=$(XO_HOME="$home" XO_ROOT_OVERRIDE="$root" XO_SUPERVISION_ACTOR=branch \
    "$ROOT/bin/xo-spawn.sh" task-x --relaunch 2>&1)
  status=$?
  [ "$status" -eq 6 ] || fail "direct branch relaunch exited $status, not 6: $out"
  assert_contains "$out" "must relaunch through xo-control" "relaunch refusal lost its wording"
  pass "the branch cannot force a teardown or bypass xo-control for a relaunch"
}
