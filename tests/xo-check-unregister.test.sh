#!/usr/bin/env bash
# Behavior tests for xo-check-unregister.sh: refuse empty-variable retirement,
...
UNREGISTER="$ROOT/bin/xo-check-unregister.sh"
REGISTER="$ROOT/bin/xo-check-register.sh"
TMP_ROOT=$(xo_test_tmproot xo-check-unregister)
...
SH
  ...
  XO_HOME="$home" "$REGISTER" "$id" >/dev/null \
...
test_empty_id_and_empty_state_refuse_without_stray_rm() {
  ...
  status=0
  PATH="$home/fakebin:$PATH" STATE='' ID='' XO_HOME="$home" \
  ...
  status=0
  : > "$log"
  PATH="$home/fakebin:$PATH" STATE='' ID='' XO_HOME="$home/nostate-home" \
  ...
  : > "$log"
  PATH="$home/fakebin:$PATH" STATE='' ID='' XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/missing-state" \
    "$UNREGISTER" demo-check >"$out" 2>"$err" || status=$?
  ...
  status=0
  : > "$log"
  PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE='' \
...
test_happy_path_removes_only_check_and_trust() {
  ...
  status=0
  STATE='' ID='' XO_HOME="$home" "$UNREGISTER" demo-check >"$out" 2>"$err" || status=$?
...
test_unsafe_hardlink_or_symlink_is_refused() {
  ...
  status=0
  XO_HOME="$home" "$UNREGISTER" custom >"$out" 2>"$err" || status=$?
