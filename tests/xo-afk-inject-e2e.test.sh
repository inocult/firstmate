#!/usr/bin/env bash
# tests/xo-afk-inject-e2e.test.sh - private-socket end-to-end test for the afk
...
# private socket. The daemon points at a throwaway state dir (XO_STATE_OVERRIDE)
# and the test pane (XO_SUPERVISOR_TARGET). Nothing touches the live fleet.
# XO_SUPERVISOR_BACKEND=tmux is passed explicitly (not left to auto-detection):
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
LOG_FILE="$STATE_DIR/submitted.log"
: > "$LOG_FILE"

# Source the daemon to get XO_INJECT_MARK, afk_enter, afk_exit.
...
TMUX_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xo-shim.XXXXXX")
...
# Create a fake crewmate window (the watcher lists xo-* windows for stale
# detection). The pane is an inert shell - it just needs to exist.
"$REAL_TMUX" -L "$SOCKET" new-window -d -n xo-fake-c1 -t supervisor

start_daemon() {
  PATH="$TMUX_SHIM_DIR:$PATH" \
  XO_STATE_OVERRIDE="$STATE_DIR" \
  XO_SUPERVISOR_TARGET="$SUPERVISOR_PANE" \
  XO_SUPERVISOR_BACKEND=tmux \
