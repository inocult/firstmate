...
TMP_ROOT=$(xo_test_tmproot xo-calm-pi-extension)
EXT="$ROOT/.pi/extensions/xo-calm.ts"
ASSISTANT_LAYOUT="$ROOT/.pi/extensions/lib/xo-calm-assistant-layout.ts"
OPERATIONAL_USER_LAYOUT="$ROOT/.pi/extensions/lib/xo-calm-operational-user-layout.ts"
VISIBILITY="$ROOT/.pi/extensions/lib/xo-calm-visibility.ts"
WORKING_SHIP="$ROOT/.pi/extensions/lib/xo-calm-working-ship.ts"
...
# .pi/extensions/xo-calm.ts) instead of relying on version inference, so a version
...
find_chrome() {
  local candidate
  if [ -n "${XO_CHROME_BIN:-}" ] && [ -x "$XO_CHROME_BIN" ]; then
    printf '%s\n' "$XO_CHROME_BIN"
    return 0
  fi
  for candidate in \
...
render_export_dom() {
  ...
  wait_limit=${XO_CHROME_RENDER_WAIT_TICKS:-300}
  : >"$report"
...
test_home_resolution() {
  ...
  mkdir -p \
    ...
    "$fixture/override" \
    "$fixture/launch-cwd"
  cp "$EXT" "$fixture/project/.pi/extensions/xo-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/project/.pi/extensions/lib/xo-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/project/.pi/extensions/lib/xo-calm-operational-user-layout.ts"
  ...
  out=$(cd "$fixture/launch-cwd" && \
    EXT="$fixture/project/.pi/extensions/xo-calm.ts" \
...
delete process.env.XO_HOME;
delete process.env.XO_CONFIG_OVERRIDE;
process.env.XO_ROOT_OVERRIDE = process.env.OVERRIDE_HOME;
...
if (readFileSync(`${process.env.OVERRIDE_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm ignored XO_ROOT_OVERRIDE when XO_HOME was unset");
}

delete process.env.XO_ROOT_OVERRIDE;
calm = registerCalm();
calm.sessionStart({ reason: "startup" }, context);
await calm.calmCommand.handler("", context);
if (readFileSync(`${process.env.EXTENSION_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not derive the XO home from its extension path");
}
...
const assistant = await import("./.pi/extensions/lib/xo-calm-assistant-layout.ts");
const operational = await import("./.pi/extensions/lib/xo-calm-operational-user-layout.ts");
...
process.env.XO_HOME = process.env.HOME_OFF;
const offRun = fakePi();
...
process.env.XO_HOME = process.env.HOME_ON;
const onRun = fakePi();
...
// captain-accepted, documented bound on the gate-at-load fix (see xo-calm.ts's file
...
test_rendering_and_session_lifecycle() {
  ...
  cp "$EXT" "$fixture/xo-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/lib/xo-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/lib/xo-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/lib/xo-calm-visibility.ts"
  cp "$WORKING_SHIP" "$fixture/lib/xo-calm-working-ship.ts"
  cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" "$fixture/lib/xo-operational-input.ts"
  cp "$ROOT/.pi/extensions/lib/xo-branch-dispatch.ts" "$fixture/lib/xo-branch-dispatch.ts"
...
#!/usr/bin/env bash
printf '%s\n' "${1-}" >>"$XO_OPERATIONAL_INPUT_CALLS"
exec "$XO_OPERATIONAL_INPUT_OWNER" "$@"
SH
...
// xo-calm.ts derives its own identity the same way (fileURLToPath(import.meta.url)),
...
extension.default(pi);
const visibility = await import(`${pathToFileURL(`${process.cwd()}/lib/xo-calm-visibility.ts`).href}?policy=${Date.now()}`);
const operationalInput = await import(`${pathToFileURL(`${process.cwd()}/lib/xo-operational-input.ts`).href}?input=${Date.now()}`);

// Registration is gated on config/calm at load (see xo-calm.ts's file header); this
...
if (
  ...
  "Toggle XO's supported conversation-only transcript presentation."
) {
...
const watcherBody =
  "XO WATCHER WAKE: signal: /tmp/probe.status\n\n" +
  "Run bin/xo-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
const watcherMessage = operationalInput.encodeXoOperationalInput("watcher", watcherBody);
...
const callsBeforePlainReplay = readFileSync(process.env.XO_OPERATIONAL_INPUT_CALLS, "utf8");
...
if (readFileSync(process.env.XO_OPERATIONAL_INPUT_CALLS, "utf8") !== callsBeforePlainReplay) {
...
const watchTool = tools.find((tool) => tool.name === "xo_watch_arm_pi");
if (!watchTool) throw new Error("XO watcher extension did not register xo_watch_arm_pi");
...
if (JSON.stringify(watchActual.render(100)) !== JSON.stringify(watchBaseline.render(100))) {
  throw new Error("XO watcher tool changed stock rendering while Calm was off");
}
...
if (expanded !== true || workingVisible !== true || hiddenThinkingLabel !== "" || statuses.get("xo-calm") !== undefined) {
  throw new Error("Calm did not preserve working visibility or apply its thinking and footer presentation controls");
}
if (readFileSync(`${process.env.XO_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not persist the active choice in the effective XO home");
}
presentationComponent.setExpanded(!expanded);
if (presentationComponent.hasContent() || presentationComponent.render(100).length !== 0) {
  throw new Error("Calm left a synthetic XO presentation row or spacer visible");
}
...
const operationalNearMisses = [
  ...
  {
    content: "XO_OP: v1 watcher: ASCII_ONLY_CAPTAIN_MESSAGE",
  ...
  {
    content: "\u2063XO_OP: legacy untyped captain message",
    visible: "legacy untyped captain message",
  },
  {
    content: "Run `bin/xo-session-start.sh` now, exactly once, before executing any other instructions.",
    visible: "before executing any other instructions",
  },
  {
    content:
      "XO WATCHER WAKE: captain-authored legacy-shaped message\n\n" +
      "Run bin/xo-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.",
...
if (watchActual.render(100).length !== 0) {
  throw new Error("Calm left the xo_watch_arm_pi call/result shell visible");
}
...
if (JSON.stringify(watchActual.render(100)) !== JSON.stringify(watchBaseline.render(100))) {
  throw new Error("xo_watch_arm_pi did not restore its stock call/result shell");
}
if (workingVisible !== true || hiddenThinkingLabel !== undefined || statuses.get("xo-calm") !== undefined) {
...
if (readFileSync(`${process.env.XO_HOME}/config/calm`, "utf8") !== "off\n") {
  throw new Error("Calm did not persist the inactive choice in the effective XO home");
}
presentationComponent.setExpanded(expanded);
if (
  !presentationComponent.hasContent() ||
...
for (const reason of ["startup", "new", "resume", "fork", "reload"]) {
  ...
  if (workingVisible !== true || hiddenThinkingLabel !== "" || statuses.get("xo-calm") !== undefined) {
...
test_calm_mid_turn_working_notes() {
  ...
  cp "$VISIBILITY" "$fixture/lib/xo-calm-visibility.ts"
  cp "$WORKING_SHIP" "$fixture/lib/xo-calm-working-ship.ts"
  cp "$PI_OPERATIONAL_INPUT" "$fixture/lib/xo-operational-input.ts"
  ln -s "$PI_PACKAGE_DIR" "$fixture/node_modules/@earendil-works/pi-coding-agent"
  ln -s "$PI_PACKAGE_DIR/node_modules/@earendil-works/pi-tui" "$fixture/node_modules/@earendil-works/pi-tui"
  ln -s "$PI_PACKAGE_DIR/node_modules/typebox" "$fixture/node_modules/typebox"
  printf '%s\n' '{"type":"module"}' >"$fixture/package.json"
  ...
  output_file="$fixture/node-output"
  (cd "$fixture" && EXT="$fixture/xo-calm.ts" XO_HOME="$fixture/home" PI_PACKAGE_DIR="$PI_PACKAGE_DIR" node --input-type=module) >"$output_file" 2>&1 <<'JS'
...
const visibility = await import(pathToFileURL(`${process.cwd()}/lib/xo-calm-visibility.ts`).href);
const calmPreferencePath = `${process.env.XO_HOME}/config/calm`;
const components = [];
const ui = {
  getEditorText: () => "",
  getToolsExpanded: () => false,
...
test_operational_followup_turn_e2e() {
  ...
  xo_git_init_commit "$project"
  cp "$EXT" "$project/.pi/extensions/xo-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$project/.pi/extensions/lib/xo-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$project/.pi/extensions/lib/xo-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$project/.pi/extensions/lib/xo-calm-visibility.ts"
  cp "$WORKING_SHIP" "$project/.pi/extensions/lib/xo-calm-working-ship.ts"
  cp "$PI_OPERATIONAL_INPUT" "$project/.pi/extensions/lib/xo-operational-input.ts"
...
import { encodeXoOperationalInput } from "./.pi/extensions/lib/xo-operational-input.ts";
...
const EXACT_WATCHER_INPUT =
  "\u2063XO_OP: v1 watcher: XO WATCHER WAKE: signal: /home/fixture/github/kunchenguid/firstmate/state/oss-triage-t4.status\n\n" +
  "Run bin/xo-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
...
function monitorInput(suffix: "ONE" | "TWO"): string {
  ...
  return encodeXoOperationalInput("watcher", `MONITOR_${label}_${suffix}`);
}
...
TS
  ...
  run_followup_case() {
    ...
    if [ "$calm_state" = absent ]; then
      ...
      rm -f "$home/config/calm"
      extensions='-e ./.pi/extensions/xo-calm.ts -e ./followup-e2e.ts'
    else
    ...
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env XO_HOME='$home' PI_CODING_AGENT_DIR='$config' XO_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions $extensions $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
    ...
    if [ "$calm_state" = on ]; then
      ...
      if [ "$label" = exact_watcher ]; then
        assert_not_contains "$pane" "XO WATCHER WAKE: signal: /home/fixture/github/kunchenguid/firstmate/state/oss-triage-t4.status" \
          "Pi exact watcher case rendered the Calm-hidden authoritative payload"
        assert_not_contains "$pane" "Run bin/xo-wake-drain.sh first and handle the queued wake." \
...
const expectedOperationalTexts = Array.from({ length: expected }, (_, index) => {
  ...
  return label === "exact_watcher" && suffix === "ONE"
    ...
    : label === "legacy_away" && suffix === "ONE"
      ...
      : `\u2063XO_OP: v1 watcher: MONITOR_${label}_${suffix}`;
});
...
JS
  ...
  replay_exact_case() {
    ...
    printf '%s\n' on >"$home/config/calm"
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 160 -y 36 \
      "cd '$project' && env XO_HOME='$home' PI_CODING_AGENT_DIR='$config' XO_OPERATIONAL_INPUT_SCRIPT='$OPERATIONAL_INPUT' PI_OFFLINE=1 pi --approve --no-context-files --no-skills --no-prompt-templates --no-extensions -e ./.pi/extensions/xo-calm.ts -e ./followup-e2e.ts --session '$exact_session'; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
    i=0
    while [ "$i" -lt 120 ]; do
      pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" -S - 2>/dev/null || true)
...
const text = (content) => typeof content === "string"
  ? content
  : (content ?? []).filter((item) => item.type === "text").map((item) => item.text).join("");
const exact = "\u2063XO_OP: v1 watcher: XO WATCHER WAKE: signal: /home/fixture/github/kunchenguid/firstmate/state/oss-triage-t4.status\n\nRun bin/xo-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
...
TS
  ...
  start_geometry_pi() {
    ...
    tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -x 100 -y 44 \
      "cd '$project' && env XO_HOME='$home' PI_CODING_AGENT_DIR='$config' PI_OFFLINE=1 pi --approve --no-context-files --no-prompt-templates --no-extensions -e ./.pi/extensions/xo-calm.ts -e ./geometry-provider.ts $session_arg; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 20"
  }
...
test_working_ship_geometry_and_lifecycle() {
  ...
  out=$(cd "$fixture" && EXT="$fixture/xo-calm.ts" XO_HOME="$fixture/home" PI_PACKAGE_DIR="$PI_PACKAGE_DIR" node --input-type=module 2>&1 <<'JS'
import { pathToFileURL } from "node:url";
...
const ship = await import(
  `${pathToFileURL(`${process.cwd()}/lib/xo-calm-working-ship.ts`).href}?ship=${Date.now()}`
);
const {
...
echo attempt >>"$XO_FAKE_CHROME_ATTEMPTS"
...
case "${1:-}" in --version) echo "FakeChrome 1.2.3"; exit 0 ;; esac
echo attempt >>"$XO_FAKE_CHROME_ATTEMPTS"
if [ "$(wc -l <"$XO_FAKE_CHROME_ATTEMPTS")" -lt 3 ]; then
...
SH
  ...
  : >"$dir/attempts-ok"
  XO_FAKE_CHROME_ATTEMPTS="$dir/attempts-ok" \
  ...
  : >"$dir/attempts-flaky"
  : >"$out_file"
  XO_FAKE_CHROME_ATTEMPTS="$dir/attempts-flaky" \
  ...
  : >"$dir/attempts-broken"
  : >"$out_file"
  if XO_FAKE_CHROME_ATTEMPTS="$dir/attempts-broken" \
  ...
  : >"$dir/attempts-hang"
  : >"$out_file"
  if XO_FAKE_CHROME_ATTEMPTS="$dir/attempts-hang" XO_CHROME_RENDER_WAIT_TICKS=3 \
...
test_interactive_terminal_e2e() {
  ...
  cp \
    ...
    "$ROOT/bin/xo-sessionstart-nudge.sh" \
    "$ROOT/bin/xo-primary-scope-lib.sh" \
    "$ROOT/bin/xo-gate-refuse-lib.sh" \
  ...
  cat >"$project/bin/xo-session-start.sh" <<'SH'
#!/usr/bin/env bash
printf 'CALM_E2E_SESSION_START_DIGEST\n'
exit 0
SH
  chmod +x "$project/bin/"*.sh
  cat >"$project/.pi/extensions/xo-calm-e2e-inject.ts" <<'TS'
import {
  type AssistantMessage,
  createAssistantMessageEventStream,
} from "@earendil-works/pi-ai";
import type { ExtensionAPI } from "@earendil-works/pi-coding-agent";
import { encodeXoOperationalInput } from "./lib/xo-operational-input.ts";
...
export default function (pi: ExtensionAPI): void {
  ...
  pi.registerCommand("calm-inject-e2e", {
    ...
    handler: async (args, ctx) => {
      ...
      await pi.sendUserMessage(encodeXoOperationalInput(kind, body), {
        deliverAs: "followUp",
      });
    },
  });
...
{"type":"message","id":"a0000007","parentId":"a0000006","timestamp":"$now","message":{"role":"assistant","content":[{"type":"thinking","thinking":"third internal reasoning block"},{"type":"toolCall","id":"call_watch_e2e","name":"xo_watch_arm_pi","arguments":{}}],"api":"anthropic-messages","provider":"anthropic","model":"claude-sonnet-4-5","usage":{"input":2,"output":1,"cacheRead":0,"cacheWrite":0,"totalTokens":3,"cost":{"input":0,"output":0,"cacheRead":0,"cacheWrite":0,"total":0}},"stopReason":"toolUse","timestamp":7}}
{"type":"message","id":"a0000008","parentId":"a0000007","timestamp":"$now","message":{"role":"toolResult","toolCallId":"call_watch_e2e","toolName":"xo_watch_arm_pi","content":[{"type":"text","text":"watcher: started Pi extension arm child 1"}],"details":{"ok":true,"message":"watcher: started Pi extension arm child 1"},"isError":false,"timestamp":8}}
{"type":"custom","id":"a0000009","parentId":"a0000008","timestamp":"$now","customType":"xo-synthetic-input-presentation","data":{"content":"XO WATCHER WAKE: signal: /tmp/probe.status\\n\\nRun bin/xo-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.","kind":"watcher"}}
{"type":"custom_message","id":"a0000010","parentId":"a0000009","timestamp":"$now","customType":"xo-synthetic-input","content":"XO WATCHER WAKE: signal: /tmp/probe.status\\n\\nRun bin/xo-wake-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.","display":false,"details":{"kind":"watcher"}}
{"type":"message","id":"a0000011","parentId":"a0000010","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"XO WATCHER WAKE: can you explain this phrase?"}],"timestamp":11}}
{"type":"message","id":"a0000012","parentId":"a0000011","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Captain quote: \u2063XO_OP: v1 watcher: QUOTED_CURRENT_NEAR_MISS"}],"timestamp":12}}
{"type":"message","id":"a0000013","parentId":"a0000012","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"XO_OP: v1 watcher: ASCII_ONLY_NEAR_MISS"}],"timestamp":13}}
{"type":"message","id":"a0000014","parentId":"a0000013","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Ordinary captain text before \u2063XO_OP: v1 watcher: EMBEDDED_CURRENT_NEAR_MISS"}],"timestamp":14}}
...
JSON
  ...
  assert_contains "$(cat "$default_snapshot")" "xo_watch_arm_pi" "Calm-off transcript did not show the XO watcher tool"
  assert_contains "$(cat "$default_snapshot")" "XO WATCHER WAKE: signal: /tmp/probe.status" "Calm-off transcript did not show the synthetic XO presentation row"
  assert_contains "$(cat "$default_snapshot")" "Thinking..." "reasoning fixture did not render Pi's collapsed thinking label"
  assert_contains "$(cat "$default_snapshot")" "xo-calm.ts" "project-local Pi calm extension did not auto-load"
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$default_snapshot")" 'Run `bin/xo-session-start.sh` now' \
  ...
  while [ "$active_screen_wait" -lt 120 ]; do
    ...
    # first-ever activation are a separate, documented exception (see xo-calm.ts's
    ...
    if ! grep -Fq "Thinking..." "$hidden_snapshot" &&
      ...
      grep -Fq "XO WATCHER WAKE: can you explain this phrase?" "$hidden_snapshot" &&
  ...
  # xo-calm.ts's file header and docs/calm.md): the alternative was letting Calm
  ...
  assert_not_contains "$(cat "$hidden_snapshot")" "xo_watch_arm_pi" "/calm left the XO watcher tool call shell in the transcript"
  assert_not_contains "$(cat "$hidden_snapshot")" "watcher: started Pi extension arm child" "/calm left the XO watcher tool result in the transcript"
  assert_not_contains "$(cat "$hidden_snapshot")" "XO WATCHER WAKE: signal: /tmp/probe.status" "/calm left a synthetic XO user-role presentation in the transcript"
  ...
  assert_contains "$(cat "$hidden_snapshot")" "XO WATCHER WAKE: can you explain this phrase?" "/calm hid a genuine near-miss user prompt"
  for near_miss in \
...
const nativeSessionStart = entries.find((entry) =>
  entry.type === "custom_message" &&
  entry.customType === "xo-sessionstart-nudge"
);
if (
  !nativeSessionStart ||
  nativeSessionStart.display !== false ||
  nativeSessionStart.details?.kind !== "session-start" ||
  !nativeSessionStart.content?.startsWith("\u2063XO_OP: v1 session-start: ")
) {
...
JS
  ...
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$active_hidden_snapshot")" 'Run `bin/xo-session-start.sh` now' \
...
if (!serialized.includes("xo-synthetic-input") || !serialized.includes("/tmp/probe.status")) process.exit(1);
const synthetic = entries.find((entry) => entry.type === "custom_message" && entry.customType === "xo-synthetic-input");
if (!synthetic || synthetic.display) process.exit(1);
JS
  chrome=$(find_chrome) \
    || fail "Chrome or Chromium is required for rendered export DOM assertions; set XO_CHROME_BIN to one"
...
JS
  ...
  assert_not_contains "$(cat "$export_settled_snapshot")" "xo_watch_arm_pi" \
    "/export left the XO watcher tool call shell in the Calm transcript"
  assert_not_contains "$(cat "$export_settled_snapshot")" "watcher: started Pi extension arm child" \
    "/export left the XO watcher tool result in the Calm transcript"
  assert_not_contains "$(cat "$export_settled_snapshot")" "XO WATCHER WAKE: signal: /tmp/probe.status" \
    "/export left a synthetic XO user-role presentation in the Calm transcript"
  ...
  assert_contains "$(cat "$restored_snapshot")" "xo_watch_arm_pi" "second /calm did not restore the XO watcher tool shell"
  assert_contains "$(cat "$restored_snapshot")" "XO WATCHER WAKE: signal: /tmp/probe.status" "second /calm did not restore the synthetic XO user row"
  for restored in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
  ...
  assert_not_contains "$(cat "$boat_frame_one")" "XO WATCHER WAKE: signal: /tmp/probe.status" "the real provider wait restored a hidden operational row"
  ...
  assert_not_contains "$(cat "$restarted_snapshot")" "xo_watch_arm_pi" "restart/resume reset Calm and restored the XO watcher tool"
  assert_not_contains "$(cat "$restarted_snapshot")" "XO WATCHER WAKE: signal: /tmp/probe.status" "restart/resume reset Calm and restored a legacy presentation row"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
