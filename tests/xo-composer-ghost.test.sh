...
# the one shared ANSI-aware owner (xo_composer_strip_ghost, bin/xo-composer-lib.sh,
# reached here through the xo_tmux_strip_ghost thin adapter):
...
# These tests pin:
#   1. xo_tmux_strip_ghost drops dim/faint AND dark-truecolor runs, keeping
#      normal-intensity, brightly-coloured text.
#   2. xo_pane_input_pending reads a ghost-only composer (either style) as NOT
...
#   4. The human/LLM-facing capture path (xo-peek.sh) stays PLAIN - no escape codes
#      ever reach xo's context.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

LIB="$ROOT/bin/xo-tmux-lib.sh"
PEEK="$ROOT/bin/xo-peek.sh"

# shellcheck source=/dev/null
. "$LIB"

TMP_ROOT=$(xo_test_tmproot xo-ghost-tests)
...
# from XO_FAKE_CY. The fake deliberately returns the complete fixture for every
...
case "${1:-}" in
  ...
  capture-pane)
    ...
    f="${XO_FAKE_STYLED:-/dev/null}"
    if [ -n "${XO_FAKE_ROW:-}" ] \
       && [ "$start" = "${XO_FAKE_CY:-0}" ] \
       && [ "$end" = "${XO_FAKE_CY:-0}" ]; then
      f=$XO_FAKE_ROW
    fi
    if [ "$has_e" = 1 ]; then
      cat "$f" 2>/dev/null
...
test_strip_ghost_drops_dim_keeps_normal() {
  ...
  pass "xo_tmux_strip_ghost drops dim/faint runs, keeps normal and bold text"
}
...
test_strip_ghost_handles_combined_and_boundary_codes() {
  ...
  pass "xo_tmux_strip_ghost handles combined SGR, ESC[22m, and reset-then-dim"
}
...
test_strip_ghost_keeps_colored_text_with_2_payloads() {
  ...
  pass "xo_tmux_strip_ghost keeps bright colored text with 2 payloads"
}
...
test_strip_ghost_drops_dark_truecolor_ghost() {
  ...
  pass "xo_tmux_strip_ghost drops a dark/muted truecolor foreground (grok placeholder)"
}
...
test_strip_ghost_keeps_muse_composer_colors() {
  ...
  out=$(printf '\033[38;2;90;160;255m\033[48;2;38;56;84m\xe2\x9f\xa9 \033[38;2;204;211;219mhello from xo\033[39m\n' | xo_tmux_strip_ghost)
  [ "$out" = "$(printf '%s hello from xo' "$glyph")" ] \
  ...
  pass "xo_tmux_strip_ghost keeps muse's near-threshold glyph and its typed text"
}

# --- xo_pane_input_pending: dim ghost is not pending ------------------------
...
test_dim_ghost_only_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY=0 \
     xo_pane_input_pending "fakepane"; then
    fail "dim ghost-only composer falsely read as pending"
  fi
  pass "xo_pane_input_pending: a dim ghost-only composer is NOT pending"
}
...
test_dim_ghost_inside_bordered_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY=1 \
     xo_pane_input_pending "fakepane"; then
    fail "dim ghost in a bordered composer falsely read as pending"
  fi
  pass "xo_pane_input_pending: dim ghost inside a bordered composer is NOT pending"
}
...
test_normal_text_still_pending() {
  ...
  PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY=0 \
    xo_pane_input_pending "fakepane" \
    || fail "real typed text was not detected as pending"
  pass "xo_pane_input_pending: normal-intensity typed text is still pending"
}
...
test_colored_text_with_2_payload_still_pending() {
  ...
  pass "xo_pane_input_pending: bright colored text with 2 payloads is still pending"
}
...
test_dark_truecolor_ghost_only_composer_is_not_pending() {
  ...
  pass "xo_pane_input_pending: a dark truecolor ghost-only composer (grok placeholder) is NOT pending"
}
...
test_dark_truecolor_bare_shell_prompt_is_unknown() {
  ...
  for prompt in '$' 'user@host $'; do
    ...
    out=$(PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY=0 \
      xo_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "dark truecolor bare shell prompt '$prompt' must read unknown, got '$out'"
  done
  pass "xo_tmux_composer_state: dark truecolor shell prompts read unknown"
}
...
test_real_text_with_trailing_ghost_is_pending() {
  ...
  pass "xo_pane_input_pending: real text plus a trailing ghost run is still pending"
}

# --- xo_tmux_composer_state: structural multi-row box scan ------------------
...
╰────────────────────────────────────────────────────╯
EOF
  out=$(PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY=2 \
    xo_tmux_composer_state "fakepane")
  [ "$out" = pending ] \
    || fail "two-row composer text above the empty cursor row should be pending, got '$out'"
  pass "xo_tmux_composer_state: text above an empty cursor row is pending"
}
...
╰────────────────────────────────────────────╯
EOF
  out=$(PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY=4 \
    xo_tmux_composer_state "fakepane")
  [ "$out" = pending ] \
    || fail "a three-row wrapped composer should be pending, got '$out'"
  pass "xo_tmux_composer_state: a message wrapped across three rows is pending"
}
...
test_proven_box_bottom_border_cursor_classifies_content() {
  ...
  pass "xo_tmux_composer_state: a proven titled box tolerates a bottom-border cursor"
}
...
test_pi_identity_requires_readable_busy_state() (
  ...
  # shellcheck disable=SC2329 # Mock invoked indirectly by the sourced adapter.
  xo_pane_busy_state() { printf 'unknown'; }
  if out=$(xo_tmux_composer_identity fakepane); then
    fail "a live Pi process with unreadable busy state must not produce identity, got '$out'"
  fi
  pass "xo_tmux_composer_identity: unknown busy state cannot become idle identity"
)
...
test_bordered_busy_signatures_are_pending() {
  ...
  for signature in 'Working...' 'Ctrl+c:cancel'; do
    printf '╭────────────────────╮\n│ %-18s │\n╰────────────────────╯\n' "$signature" > "$capture"
    out=$(PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY=1 \
      xo_tmux_composer_state "fakepane")
    [ "$out" = pending ] \
      || fail "typed bordered busy signature '$signature' should be pending, got '$out'"
  done
  pass "xo_tmux_composer_state: typed Pi and Grok busy signatures inside a box are pending"
}
...
test_non_bordered_busy_footer_is_unknown_strict() {
  ...
  # core's baseline-idle turn-started conversion (xo_tmux_submit_core), which
  ...
  pass "xo_tmux_composer_state: a bare busy-footer row reads unknown (strict container-proof rule)"
}
...
test_clipped_bordered_box_is_unknown() {
  ...
  pass "xo_tmux_composer_state: an unbounded bordered box fails closed as unknown"
}
...
test_asymmetric_composer_edges_are_unknown() {
  ...
  for row in '│ > text' 'text │' '│ > text ┃' '──────' '+----' 'text +'; do
    printf '%s\n' "$row" > "$capture"
    out=$(PATH="$fb:$PATH" LC_ALL=C XO_FAKE_STYLED="$capture" XO_FAKE_CY=0 \
      xo_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "unbounded composer-edge row '$row' should be unknown, got '$out'"
  done
  pass "xo_tmux_composer_state: clipped and asymmetric composer edges fail closed"
}
...
test_mismatched_box_families_are_unknown() {
  ...
  pass "xo_tmux_composer_state: inconsistent box geometry fails closed"
}
...
test_misaligned_box_is_unknown() {
  ...
  pass "xo_tmux_composer_state: misaligned box bounds fail closed"
}
...
test_unproved_empty_geometry_fails_closed() {
  ...
  for fixture in ghost idle malformed-top; do
    case "$fixture" in
      ...
      idle)
        ...
        printf '╭────────────╮\n│ idle hint │\n╰────────────╯\n' > "$capture"
        out=$(PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY=1 \
          XO_COMPOSER_IDLE_RE='^idle hint$' xo_tmux_composer_state "fakepane")
        ;;
      malformed-top)
        expected=unknown
        printf '╭────x───────╮\n│            │\n╰────────────╯\n' > "$capture"
  ...
  pass "xo_tmux_composer_state: unproved ghost and malformed geometry stay unknown while styled placeholder-like text stays pending-unproven"
}
...
test_differing_widths_use_asymmetric_verdicts() {
  ...
  pass "xo_tmux_composer_state: differing widths prefer pending or unknown, never empty"
}
...
test_wide_composer_text_is_pending() {
  ...
  for text in '修复登录问题' 'fix the bug 🔧'; do
    printf '╭────────────────────╮\n│ > %s │\n╰────────────────────╯\n' "$text" > "$capture"
    out=$(PATH="$fb:$PATH" LC_ALL=C XO_FAKE_STYLED="$capture" XO_FAKE_CY=1 \
      xo_tmux_composer_state "fakepane")
    [ "$out" = pending-unproven ] \
      || fail "wide composer text '$text' should be pending-unproven, got '$out'"
  done
  pass "xo_tmux_composer_state: emoji and CJK text remain pending under the C locale"
}
...
test_all_tmux_harness_composers_share_classification() {
  ...
  pass "xo_tmux_composer_state: all tmux harnesses share empty and pending classification"
}

test_unrecognized_state_defers_input_guard() {
  (
    # shellcheck disable=SC2329
    xo_tmux_composer_state() { printf 'future-state'; }
    xo_pane_input_pending "fakepane"
  ) || fail "an unrecognized composer state should defer the input guard"
  pass "xo_pane_input_pending: unrecognized states defer by default"
}
...
test_single_capture_leaves_no_fallback_race() {
  ...
  # capture (bin/xo-composer-lib.sh, xo_composer_classify_screen), so the
  # race is structurally gone: a divergent band-capture row (served via
  # XO_FAKE_ROW, which only a band capture would read) must have no effect on
  # the verdict.
  ...
  printf '│ > │\n' > "$row_capture"
  out=$(PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_ROW="$row_capture" XO_FAKE_CY=0 \
    xo_tmux_composer_state "fakepane")
  [ "$out" = pending ] \
    || fail "the verdict must come from the one full capture (agent glyph + typed text = pending), got '$out'"
  pass "xo_tmux_composer_state: one capture feeds the classifier; no band-capture race remains"
}
...
test_absent_tmux_identity_keeps_enclosed_bare_verdict() {
  ...
  pass "xo_tmux_composer_state: absent Pi identity preserves Claude's enclosed bare verdict"
}
...
test_legitimate_empty_routes_remain_empty() {
  ...
  # unknown, pinned by tests/xo-daemon.test.sh and tests/xo-composer-lib.test.sh.
  for fixture in bordered double-bordered agent-prompt; do
    ...
    out=$(PATH="$fb:$PATH" XO_FAKE_STYLED="$capture" XO_FAKE_CY="$cursor" \
      xo_tmux_composer_state "fakepane")
    [ "$out" = empty ] \
      || fail "legitimate empty route '$fixture' should remain empty, got '$out'"
  done
  pass "xo_tmux_composer_state: only proven structural and non-bordered empty routes stay empty"
}
...
test_non_bordered_composer_uses_compatibility_fallback() {
  ...
  pass "xo_tmux_composer_state: panes without bordered structure retain compatibility fallback"
}
...
test_non_bordered_interior_edges_are_pending() {
  ...
  pass "xo_tmux_composer_state: interior edge glyphs retain non-bordered fallback"
}

# --- xo-peek.sh stays escape-free (LLM-facing path) -------------------------
...
test_peek_output_is_escape_free() {
  ...
  # surface NONE of these escape codes into xo's context.
  printf 'normal output line\n\xe2\x9d\xaf \033[2mpredicted next prompt\033[0m\n' > "$capture"
  # Empty XO_HOME so xo-guard.sh finds no in-flight task and stays silent.
  home="$dir/home"; mkdir -p "$home/state"
  # Pass an explicit session:window so resolution needs no metadata.
  out=$(PATH="$fb:$PATH" XO_HOME="$home" XO_FAKE_STYLED="$capture" \
        "$PEEK" "sess:win" 2>/dev/null)
  case "$out" in
    *"$ESC"*) fail "xo-peek surfaced ANSI escape codes into LLM-facing output" ;;
  esac
  # And it should still carry the real content.
  case "$out" in
    *"predicted next prompt"*) : ;;
    *) fail "xo-peek dropped pane content (expected the ghost text body as plain text)" ;;
  esac
  pass "xo-peek output is escape-free (no raw -e bytes reach xo context)"
}
