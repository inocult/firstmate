#!/usr/bin/env bash
# tests/xo-backend-orca.test.sh - fake-Orca-CLI unit tests for the Orca
...
TMP_ROOT=$(xo_test_tmproot xo-backend-orca-tests)
...
if [ "${1:-}" = status ] && [ "${XO_ORCA_STATUS_RESPONSE:-ready}" != sequence ]; then
...
neutral_xo_root() {  # <dir> -> echoes a minimal root with a quiet guard
  local root="$1/root"
  mkdir -p "$root/bin"
  cat > "$root/bin/xo-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$root/bin/xo-guard.sh"
  printf '%s\n' "$root"
}
...
test_capture_reads_terminal_tail_json() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_capture term-123 40' "$ROOT" )
  ...
  pass "xo_backend_orca_capture: parses result.terminal.tail and calls terminal read"
}
...
test_capture_falls_back_to_text_fields() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_capture term-abc 5' "$ROOT" )
  [ "$out" = "plain text output" ] || fail "capture should fall back to result.text, got '$out'"
  pass "xo_backend_orca_capture: falls back to result text fields"
}
...
test_capture_fails_on_orca_error_json() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_capture term-stale 5' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "xo_backend_orca_capture: fails closed on Orca read error JSON"
}
...
test_runtime_check_accepts_ready_orca_status() {
  ...
  printf '{"ok":true,"result":{"runtime":{"reachable":true,"state":"ready"}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" XO_ORCA_STATUS_RESPONSE=sequence \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_runtime_check' "$ROOT" )
  ...
  pass "xo_backend_orca_runtime_check: accepts reachable ready runtime"
}
...
test_runtime_check_refuses_unready_orca_status() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" XO_ORCA_STATUS_RESPONSE=sequence \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_runtime_check' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "xo_backend_orca_runtime_check: fails closed when runtime is not ready"
}
...
test_send_text_submit_verifies_empty_composer_after_enter() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_text_submit term-123 "hello captain" 3 0.01 0.01' "$ROOT" )
  ...
  # composer (audit xo-composer-consolidation-audit-s1, section 3.3).
  assert_not_contains "$(cat "$LOG")" $'\x1f''--cursor'$'\x1f' \
    "the composer read must never page backward into scrollback"
  pass "xo_backend_orca_send_text_submit: verifies empty composer after Enter with one bounded read"
}
...
test_send_text_submit_borderless_claude_confirms() {
  ...
  pass "xo_backend_orca_send_text_submit: a borderless claude composer confirms delivery (the missing #2029 shape)"
}
...
test_composer_state_stale_banner_never_wins() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_composer_state term-123' "$ROOT" )
  ...
  pass "xo_backend_orca_composer_state: a stale startup banner cannot outrank the live composer row"
}
...
test_send_text_submit_retries_when_composer_stays_pending() {
  ...
  pass "xo_backend_orca_send_text_submit: retries Enter while composer remains pending"
}
...
test_composer_state_popup_placeholder_fill_is_pending() {
  ...
  pass "xo_backend_orca_composer_state: a slash-command popup's argument-hint placeholder still reads pending"
}

# Dead-shell injection safety (task xo-composer-shellglyph-safety): a pane whose
...
test_composer_state_bare_shell_prompt_is_unknown() {
  ...
  printf '{"ok":true,"result":{"terminal":{"tail":["some earlier output","kunchen@mac xo $ "]}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_composer_state term-123' "$ROOT" )
  [ "$out" = unknown ] || fail "a bare dead-shell prompt (no bordered composer row) must read unknown, got '$out'"
  pass "xo_backend_orca_composer_state: a bare dead-shell prompt reads unknown (unsafe-for-injection), never empty"
}
...
test_send_text_submit_popup_autocomplete_requires_second_enter() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_text_submit term-123 "/compact" 3 0.01 1.2' "$ROOT" )
  ...
  pass "xo_backend_orca_send_text_submit: a slash-command popup's placeholder fill on Enter #1 does not short-circuit as submitted; Enter #2 is retried and lands it"
}

test_send_literal_constructs_non_enter_send() {
  orca_case send-literal
  PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_literal term-123 "typed only"' "$ROOT"
  ...
  pass "xo_backend_orca_send_literal: sends text without submitting"
}
...
test_send_text_submit_reports_send_failed() {
  ...
  printf '1\n' > "$RESP/1.exit"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_text_submit term-123 "hello" 1 0.01 0.01' "$ROOT" )
  [ "$out" = send-failed ] || fail "failed Orca send should report send-failed, got '$out'"
  pass "xo_backend_orca_send_text_submit: reports send-failed when Orca send fails"
}
...
test_send_helpers_reject_orca_error_json() {
  ...
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_text_line term-stale "hello"' "$ROOT" 2>&1 )
  status=$?
  ...
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/2.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_literal term-stale "typed"' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "send_literal should fail on Orca ok:false JSON"
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/3.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_key term-stale Enter' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "send_key should fail on Orca ok:false JSON"
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/4.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_text_submit term-stale "hello" 1 0.01 0.01' "$ROOT" 2>/dev/null )
...
test_send_key_enter_and_interrupt() {
  orca_case send-key
  PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_key term-123 Enter; xo_backend_orca_send_key term-123 C-c' "$ROOT"
  ...
  pass "xo_backend_orca_send_key: Enter maps to empty enter, C-c maps to interrupt"
}
...
test_send_key_refuses_unknown_key() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_key term-123 F12' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "xo_backend_orca_send_key: refuses unsupported keys loudly"
}
...
test_send_key_refuses_escape_until_supported() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_send_key term-123 Escape' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "xo_backend_orca_send_key: refuses Escape instead of mapping it to interrupt"
}
...
test_kill_is_best_effort_close() {
  ...
  printf '1\n' > "$RESP/1.exit"
  PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_kill term-123' "$ROOT"
  ...
  pass "xo_backend_orca_kill: calls terminal close and stays best-effort"
}
...
test_remove_worktree_refuses_empty_id() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_remove_worktree ""' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "xo_backend_orca_remove_worktree: refuses empty worktree ids"
}
...
test_remove_worktree_rejects_orca_error_json() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_remove_worktree wt-gone' "$ROOT" 2>&1 )
  status=$?
  ...
  pass "xo_backend_orca_remove_worktree: fails closed on ok:false JSON"
}
...
test_worktree_path_resolves_id() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_worktree_path wt-123' "$ROOT" )
  ...
  pass "xo_backend_orca_worktree_path: resolves an Orca worktree id to its path"
}
...
test_json_get_ignores_undocumented_terminal_id_shapes() {
  ...
  out=$( printf '{"ok":true,"result":{"id":"term-root-id"}}\n' | \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_json_get terminal-handle' "$ROOT" )
  status=$?
  set +e
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_worktree_create /repo/path xo-task' "$ROOT" )
  wt_id=${out%%$'\t'*}
  wt_path=${out#*$'\t'}
  term=${wt_path#*$'\t'}
  wt_path=${wt_path%%$'\t'*}
  ...
  pass "xo_backend_orca_json_get: ignores undocumented terminal id shapes"
}
...
test_worktree_and_terminal_helpers_parse_json() {
  ...
  term=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_terminal_create wt-123 xo-task' "$ROOT" )
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''create'$'\x1f''--repo'$'\x1f''id:repo-123'$'\x1f''--name'$'\x1f''xo-task'$'\x1f''--no-parent'$'\x1f''--setup'$'\x1f''skip'$'\x1f''--json' \
    "worktree helper did not create an independent no-hook worktree"
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''create'$'\x1f''--worktree'$'\x1f''id:wt-123'$'\x1f''--title'$'\x1f''xo-task'$'\x1f''--json' \
...
test_worktree_create_removes_worktree_when_path_missing() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; xo_backend_orca_worktree_create /repo/path xo-task' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "worktree helper should fail when Orca omits the worktree path"
  assert_contains "$out" "orca worktree create did not return a path for xo-task" \
  ...
  pass "xo_backend_orca_worktree_create: removes created worktree when path is missing"
}
...
test_spawn_preserves_orca_metadata_when_pathless_worktree_cleanup_fails() {
  ...
  printf '{"ok":false,"error":{"code":"worktree_not_removed","message":"worktree not removed"}}\n' > "$RESP/5.out"
  out=$( HOME="$SPAWN_HOME" CLAUDE_CONFIG_DIR='' PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$state" XO_DATA_OVERRIDE="$data" XO_CONFIG_OVERRIDE="$config" \
    XO_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" "$id" "$proj" claude --mode no-mistakes --yolo off --backend orca 2>&1 )
  status=$?
  ...
  assert_grep "window=xo-$id" "$state/$id.meta" "preserved pathless metadata missing stable window alias"
  ...
  pass "xo-spawn.sh --backend orca: preserves metadata when pathless cleanup fails"
}
...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  out=$( HOME="$SPAWN_HOME" CLAUDE_CONFIG_DIR='' PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    ...
    XO_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" "$id" "$proj" claude --mode no-mistakes --yolo off --backend orca 2>&1 )
  expect_code 0 $? "xo-spawn.sh --backend orca should succeed with fake Orca"$'\n'"$out"
  assert_contains "$out" "spawned $id harness=claude kind=ship mode=no-mistakes yolo=off window=xo-$id worktree=$wt" \
    "spawn output missing Orca window/worktree summary"
  assert_grep "backend=orca" "$state/$id.meta" "meta missing backend=orca"
  assert_grep "window=xo-$id" "$state/$id.meta" "meta missing stable Orca window alias"
  ...
  assert_contains "$(cat "$log")" $'orca\x1f''terminal'$'\x1f''send'$'\x1f''--terminal'$'\x1f''term-spawn'$'\x1f''--text'$'\x1f''export GOTMPDIR=/tmp/xo-orcaspawnz1/gotmp'$'\x1f''--enter'$'\x1f''--json' \
  ...
  rm -rf "/tmp/xo-$id"
  pass "xo-spawn.sh --backend orca: reuses implicit terminal, records metadata, launches harness"
}
...
test_spawn_refuses_orca_secondmate_before_home_mutation() {
  ...
  printf '%s\n' "$id" > "$subhome/.xo-secondmate-home"
  printf 'xo\n' > "$subhome/AGENTS.md"
  printf 'claude\n' > "$config/crew-harness"
  touch "$state/.last-watcher-beat"
  set +e
  out=$( XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" XO_STATE_OVERRIDE="$state" XO_DATA_OVERRIDE="$data" XO_CONFIG_OVERRIDE="$config" \
    XO_PROJECTS_OVERRIDE="$home/projects" XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" "$id" "$subhome" claude --backend orca --secondmate 2>&1 )
  status=$?
  set +e
  ...
  pass "xo-spawn.sh --backend orca --secondmate: refuses before secondmate-home mutation"
}
...
test_spawn_refuses_orca_when_runtime_not_ready() {
  ...
  printf '{"ok":true,"result":{"runtime":{"reachable":false,"state":"starting"}}}\n' > "$RESP/1.out"
  out=$( HOME="$SPAWN_HOME" CLAUDE_CONFIG_DIR='' PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" XO_ORCA_STATUS_RESPONSE=sequence \
    XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$state" XO_DATA_OVERRIDE="$data" XO_CONFIG_OVERRIDE="$config" \
    XO_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" "$id" "$proj" claude --mode no-mistakes --yolo off --backend orca 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "xo-spawn.sh --backend orca should refuse when Orca runtime is not ready"
  ...
  pass "xo-spawn.sh --backend orca: refuses before mutation when Orca runtime is not ready"
}
...
test_spawn_refuses_orca_nonisolated_worktree() {
  ...
  out=$( HOME="$SPAWN_HOME" CLAUDE_CONFIG_DIR='' PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    ...
    "$ROOT/bin/xo-spawn.sh" "$id" "$proj" claude --mode no-mistakes --yolo off --backend orca 2>&1 )
  status=$?
  expect_code 1 "$status" "xo-spawn.sh --backend orca should refuse a primary checkout worktree"
  ...
  pass "xo-spawn.sh --backend orca: refuses non-isolated worktrees and closes implicit terminals"
}
...
test_spawn_removes_orca_worktree_when_terminal_create_fails() {
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''create'$'\x1f''--worktree'$'\x1f''id:wt-terminal-fail'$'\x1f''--title'$'\x1f'"xo-$id"$'\x1f''--json' \
  ...
  pass "xo-spawn.sh --backend orca: removes worktree when terminal creation fails"
}
...
test_spawn_preserves_orca_metadata_when_abort_cleanup_fails() {
  ...
  assert_grep "window=xo-$id" "$state/$id.meta" "preserved metadata missing stable window alias"
  ...
  pass "xo-spawn.sh --backend orca: preserves metadata when abort cleanup fails"
}
...
test_spawn_releases_orca_resources_when_metadata_write_fails() {
  ...
  pass "xo-spawn.sh --backend orca: releases terminal and worktree on later aborts"
}
...
test_peek_send_and_crew_state_route_through_orca_meta() {
  ...
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-io" "worktree=$wt" "project=$wt" "harness=claude" "kind=scout" "backend=orca"
  touch "$state/.last-watcher-beat"
  orca_case io-path
  neutral=$(neutral_xo_root "$CASE_DIR/neutral")
  printf '{"ok":true,"result":{"terminal":{"tail":["ready"]}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    XO_ROOT_OVERRIDE="$neutral" XO_STATE_OVERRIDE="$state" XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-peek.sh" "xo-$id" 10 )
  [ "$out" = ready ] || fail "xo-peek should read through Orca metadata, got '$out'"
  printf '{"ok":true,"result":{"send":{"handle":"term-io","accepted":true}}}\n' > "$RESP/2.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-io","accepted":true}}}\n' > "$RESP/3.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["│ > │"]}}}\n' > "$RESP/4.out"
  PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    XO_ROOT_OVERRIDE="$neutral" XO_HOME="$neutral" XO_STATE_OVERRIDE="$state" XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-send.sh" "xo-$id" "hello orca"
  printf '{"ok":true,"result":{"terminal":{"tail":["idle prompt"]}}}\n' > "$RESP/5.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-crew-state.sh" "$id" )
  ...
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''read'$'\x1f''--terminal'$'\x1f'"xo-$id" \
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''send'$'\x1f''--terminal'$'\x1f''term-io'$'\x1f''--text'$'\x1f'': XO instruction waiting:' \
  ...
  pass "xo-peek/xo-send/xo-crew-state route through backend=orca metadata and its durable inbox"
}
...
test_peek_and_crew_state_fail_closed_on_orca_error_json() {
  ...
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-stale" "worktree=$wt" "project=$wt" "harness=claude" "kind=scout" "backend=orca"
  touch "$state/.last-watcher-beat"
  orca_case read-error-json
  neutral=$(neutral_xo_root "$CASE_DIR/neutral")
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    XO_ROOT_OVERRIDE="$neutral" XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-peek.sh" "xo-$id" 10 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "xo-peek should fail when Orca reports a stale terminal"
  assert_contains "$out" "terminal handle stale" "xo-peek should surface the Orca read error message"
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/2.out"
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-crew-state.sh" "$id" )
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''read'$'\x1f''--terminal'$'\x1f''term-stale' \
    "xo-peek/xo-crew-state did not read the recorded Orca terminal"
  pass "xo-peek/xo-crew-state: Orca read error JSON fails closed"
}
...
test_target_exists_rejects_orca_error_json() {
  ...
  set +e
  PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/xo-backend.sh"; xo_backend_target_exists orca term-stale xo-task' "$ROOT"
  status=$?
  set -e
  [ "$status" -ne 0 ] || fail "xo_backend_target_exists should reject Orca ok:false read JSON"
  pass "xo_backend_target_exists: Orca ok:false read JSON is not live"
}
...
test_scout_teardown_removes_orca_worktree_via_helper() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-teardown" "worktree=$wt" "project=$proj" \
  ...
  set +e
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    XO_ROOT_OVERRIDE="$neutral" XO_STATE_OVERRIDE="$state" XO_DATA_OVERRIDE="$data" XO_CONFIG_OVERRIDE="$config" \
    "$ROOT/bin/xo-teardown.sh" "$id" 2>&1 )
  rc=$?
  set -e
  ...
  pass "xo-teardown.sh backend=orca: scout report gate then helper-backed worktree removal"
}
...
test_scout_teardown_refuses_orca_id_path_mismatch() {
  ...
  xo_git_worktree "$proj" "$wt" "xo/$id"
  git -C "$proj" worktree add --quiet -b "xo/$id-other" "$other_wt"
  mkdir -p "$data/$id" "$state" "$config"
  printf 'report\n' > "$data/$id/report.md"
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-scout-mismatch" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=scout" "mode=no-mistakes" "yolo=off" \
  ...
  pass "xo-teardown.sh backend=orca: scout teardown refuses id/path mismatches"
}
...
test_teardown_removes_orca_worktree_when_path_missing() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-missing-path" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=scout" "mode=no-mistakes" "yolo=off" \
  ...
  pass "xo-teardown.sh backend=orca: releases terminal/worktree when path is absent"
}
...
test_teardown_preserves_metadata_when_orca_remove_error_json() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-remove-error" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=scout" "mode=no-mistakes" "yolo=off" \
  ...
  pass "xo-teardown.sh backend=orca: preserves metadata on remove ok:false JSON"
}
...
test_scout_teardown_refuses_orca_missing_report_when_path_missing() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-missing-report" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=scout" "mode=no-mistakes" "yolo=off" \
  ...
  pass "xo-teardown.sh backend=orca: scout report gate precedes pathless helper cleanup"
}
...
test_ship_teardown_refuses_orca_missing_worktree_path() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-missing-ship" "worktree=$wt" "project=$proj" \
  ...
  pass "xo-teardown.sh backend=orca: ship teardown fails closed when worktree path is missing"
}
...
test_ship_teardown_removes_orca_worktree_when_id_path_matches() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-ship-match" "worktree=$wt" "project=$proj" \
  ...
  pass "xo-teardown.sh backend=orca: ship teardown requires a matching Orca id path"
}
...
test_ship_teardown_refuses_orca_unresolvable_worktree_id() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-ship-unresolved" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=ship" "mode=local-only" "yolo=off" \
  ...
  pass "xo-teardown.sh backend=orca: ship teardown fails closed when id resolution fails"
}
...
test_ship_teardown_refuses_orca_id_path_mismatch() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-ship-mismatch" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=ship" "mode=local-only" "yolo=off" \
  ...
  pass "xo-teardown.sh backend=orca: ship teardown refuses id/path mismatches"
}
...
test_teardown_refuses_orca_missing_worktree_id() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-missing-id" "worktree=$wt" "project=$proj" \
  ...
  pass "xo-teardown.sh backend=orca: refuses missing worktree ids before cleanup"
}
...
test_teardown_refuses_orca_worktree_without_terminal_handle() {
  ...
  touch "$state/.last-watcher-beat"
  xo_write_meta "$state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=scout" "mode=no-mistakes" "yolo=off" \
  ...
  pass "xo-teardown.sh backend=orca: refuses incomplete worktree-only endpoint metadata before runtime dispatch"
}
...
test_secondmate_force_teardown_removes_orca_child_via_orca() {
  ...
  xo_git_worktree "$childproj" "$childwt" "xo/$child_id"
  xo_write_meta "$home/state/domain.meta" \
    "window=xo:xo-domain" "worktree=$subhome" "project=$subhome" \
  ...
  xo_write_meta "$subhome/state/$child_id.meta" \
    "window=xo-$child_id" "endpoint_task_id=$child_id" \
  ...
  set +e
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    XO_ROOT_OVERRIDE="$neutral" XO_HOME="$home" "$ROOT/bin/xo-teardown.sh" domain --force 2>&1 )
  rc=$?
  set -e
  ...
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''close'$'\x1f''--terminal'$'\x1f'"xo-$child_id" \
  ...
  pass "xo-teardown.sh --force: removes Orca secondmate children through Orca"
}
...
test_secondmate_force_teardown_refuses_orca_child_id_path_mismatch() {
  ...
  printf 'domain\n' > "$subhome/.xo-secondmate-home"
  xo_git_worktree "$childproj" "$childwt" "xo/$child_id"
  git -C "$childproj" worktree add --quiet -b "xo/$child_id-other" "$other_wt"
  xo_write_meta "$home/state/domain.meta" \
    "window=xo:xo-domain" "worktree=$subhome" "project=$subhome" \
    "harness=echo" "kind=secondmate" "mode=secondmate" "yolo=off" \
    "home=$subhome" "projects=alpha"
  ...
  pass "xo-teardown.sh --force: refuses Orca child id/path mismatches"
}
...
test_secondmate_force_teardown_refuses_partial_orca_child() {
  ...
  pass "xo-teardown.sh --force: refuses partial Orca secondmate children before runtime dispatch"
}
...
test_dispatcher_sources_orca_and_routes_primitives() {
  ...
  out=$( PATH="$FB:$PATH" XO_ORCA_LOG="$LOG" XO_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/xo-backend.sh"; xo_backend_validate orca; xo_backend_capture orca term-123 9' "$ROOT" )
  [ "$out" = "via dispatch" ] || fail "dispatcher should route capture to the Orca adapter, got '$out'"
  pass "xo-backend dispatcher: accepts orca and routes capture through bin/backends/orca.sh"
}
