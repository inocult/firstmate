#!/usr/bin/env bash
# Regression test for the xo-spawn.sh treehouse-get worktree-detection settle
# loop (bin/xo-spawn.sh, the `for _ in $(seq 1 60)` loop after `treehouse get`).
#
...
TMP_ROOT=$(xo_test_tmproot xo-spawn-worktree-settle)

# make_settle_fakebin <dir> builds a fake tmux whose `#{pane_current_path}`
# query returns XO_FAKE_PANE_STALE for the first XO_FAKE_PANE_STALE_READS
# calls, then XO_FAKE_PANE_PATH forever after - reproducing a pane that
...
case "$*" in
  *"#{pane_current_path}"*)
    countfile="${XO_FAKE_PANE_COUNTFILE:?XO_FAKE_PANE_COUNTFILE unset}"
    n=0
    [ -f "$countfile" ] && n=$(cat "$countfile")
    n=$((n + 1))
    printf '%s\n' "$n" > "$countfile"
    if [ "$n" -le "${XO_FAKE_PANE_STALE_READS:-0}" ]; then
      printf '%s\n' "${XO_FAKE_PANE_STALE:-}"
    else
      printf '%s\n' "${XO_FAKE_PANE_PATH:-}"
    fi
...
# live incident where the stale read was another real xo home).
make_settle_case() {
...
run_settle_spawn() {
  local id=$1
  XO_ROOT_OVERRIDE='' XO_HOME="$HOME_DIR" \
    ...
    XO_SPAWN_NO_GUARD=1 TMUX="fake,1,0" \
    XO_FAKE_PANE_PATH="$WT_DIR" XO_FAKE_PANE_STALE="$STALE_DIR" \
    XO_FAKE_PANE_STALE_READS="$STALE_READS" XO_FAKE_PANE_COUNTFILE="$COUNTFILE" \
...
make_primary_case() {
  ...
  countfile="$case_dir/pane-call-count"
  fakebin=$(make_settle_fakebin "$case_dir/fake")
  xo_test_spawn_home "$home" codex
  xo_git_worktree "$primary" "$proj" "mate-$name"
  git -C "$primary" worktree add --quiet -b "slot-$name" "$wt"
  xo_test_spawn_brief "$home" "$id" "Exercise primary-checkout transient detection for $id."
...
echo "# all xo-spawn-worktree-settle tests passed"
