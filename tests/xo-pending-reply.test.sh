#!/usr/bin/env bash
# Parent-owned secondmate pending-reply guards (bin/xo-pending-reply-lib.sh).
#
...
#  10. xo-send secondmate path embeds corr and creates durable pending records
...
# shellcheck source=bin/xo-marker-lib.sh
...
REPORT="$ROOT/bin/xo-secondmate-report.sh"
TMP_ROOT=$(xo_test_tmproot xo-pending-reply)

export XO_PENDING_REPLY_GRACE_SECS=0
export XO_SEND_SETTLE=0
...
bind_local_mate() {  # <parent-home> <id> -> mate-home
  ...
  mkdir -p "$mate/state"
  printf '%s\n' "$id" > "$mate/.xo-secondmate-home"
  cat > "$mate/.xo-secondmate-parent" <<EOF
schema=xo-secondmate-parent.v1
route=local
parent_home=$parent
EOF
  printf '%s\n' "$mate"
...
run_send() {
  ...
  env PATH="$fb:$PATH" \
    ...
    XO_PENDING_REPLY_GRACE_SECS=0 \
    "$SEND" "$@" 2>/dev/null
}

phase_of() {  # <state> <corr>
  xo_pending_reply_get "$(xo_pending_reply_path "$1" "$2")" phase
}
...
# the production owner (bin/xo-task-inbox-lib.sh).
latest_record_body() {  # <home> <task>
  local rec
  rec=$(find "$1/state/$2.inbox" -maxdepth 1 -name '*.msg' 2>/dev/null | sort | tail -1)
  [ -n "$rec" ] || return 1
  bash -c '. "$1"; xo_task_inbox_body "$2"' _ "$ROOT/bin/xo-task-inbox-lib.sh" "$rec"
}
...
test_normal_correlated_reply_resolves_once() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=1000
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "audit the ledger")
  xo_pending_reply_mark_delivered "$state" "$corr"
  status="$state/hibit.status"
  if xo_pending_reply_try_resolve "$state" "$corr"; then
    fail "missing status must not resolve"
  fi
  printf 'done [corr=%s]: ledger clean\n' "$corr" > "$status"
  xo_pending_reply_try_resolve "$state" "$corr" || fail "correlated status should resolve"
  [ "$(phase_of "$state" "$corr")" = resolved ] || fail "phase should be resolved"
  # Idempotent second resolve.
  xo_pending_reply_try_resolve "$state" "$corr" || fail "second resolve must stay successful"
  [ "$(phase_of "$state" "$corr")" = resolved ] || fail "phase must remain resolved"
  rec=$(xo_pending_reply_path "$state" "$corr")
  [ "$(xo_pending_reply_get "$rec" resolved_via)" = status ] \
...
test_completed_turn_no_report_triggers_one_recovery() {
  ...
  : > "$hook_log"
  export XO_PENDING_REPLY_NOW=2000
  export XO_PENDING_REPLY_SEND_HOOK='printf "%s\t%s\n" >>"'"$hook_log"'"'
  # The hook above is wrong for eval form - use a function.
  # Invoked indirectly through XO_PENDING_REPLY_SEND_HOOK.
  # shellcheck disable=SC2329
  recovery_hook() {
    printf '%s\t%s\n' "$1" "$2" >> "$hook_log"
  }
  export -f recovery_hook
  export XO_PENDING_REPLY_SEND_HOOK='recovery_hook'

  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "status of phase 7")
  xo_pending_reply_mark_delivered "$state" "$corr"
  # Turn completes with no parent report (the Hi Bit missed-report shape).
  xo_pending_reply_observe_busy "$state" "$corr" busy
  xo_pending_reply_observe_busy "$state" "$corr" idle
  xo_pending_reply_send_recovery "$state" "$corr" \
  ...
  if xo_pending_reply_send_recovery "$state" "$corr" 2>/dev/null; then
...
test_recovery_attempt_is_never_reinjected() {
  ...
  : > "$hook_log"
  export XO_PENDING_REPLY_NOW=2500
  recovery_fail_hook() {
    printf 'attempted\n' >> "$hook_log"
    return 1
  }
  export -f recovery_fail_hook
  export XO_PENDING_REPLY_SEND_HOOK=recovery_fail_hook
  corr=$(xo_pending_reply_create "$home" "$state" hibit "at most once")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  if xo_pending_reply_send_recovery "$state" "$corr"; then
  ...
  [ -z "$(xo_pending_reply_get "$(xo_pending_reply_path "$state" "$corr")" recovery_sent_epoch)" ] \
  ...
  xo_pending_reply_maybe_escalate "$state" "$corr" \
  ...
  live_corr=$(xo_pending_reply_create "$home" "$state" hibit "live recovery")
  xo_pending_reply_mark_delivered "$state" "$live_corr"
  xo_pending_reply_mark_turn_completed "$state" "$live_corr" request
  live_rec=$(xo_pending_reply_path "$state" "$live_corr")
  live_pid=${BASHPID:-$$}
  live_identity=$(xo_pending_reply_pid_identity "$live_pid") \
    || fail "live sender identity should be observable"
  xo_pending_reply_set "$live_rec" recovery_attempted_epoch 2500 || fail "live attempt precommit failed"
  xo_pending_reply_set "$live_rec" recovery_sender_pid "$live_pid" || fail "live sender pid commit failed"
  xo_pending_reply_set "$live_rec" recovery_sender_identity "$live_identity" \
    || fail "live sender identity commit failed"
  xo_pending_reply_set "$live_rec" phase recovery_sending || fail "live sending phase failed"
  xo_pending_reply_tick_one "$state" "$live_corr" unknown || fail "live recovery tick failed"
  [ "$(phase_of "$state" "$live_corr")" = recovery_sending ] \
    || fail "live recovery must remain in progress without elapsed-time inference"
  corr=$(xo_pending_reply_create "$home" "$state" hibit "crashed recovery")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  rec=$(xo_pending_reply_path "$state" "$corr")
  xo_pending_reply_set "$rec" recovery_attempted_epoch 2500 || fail "attempt precommit failed"
  xo_pending_reply_set "$rec" phase recovery_sending || fail "sending phase precommit failed"
  xo_pending_reply_tick_one "$state" "$corr" unknown || fail "recovery reconciliation failed"
  [ "$(phase_of "$state" "$corr")" = escalated ] \
    || fail "interrupted recovery attempt should escalate unknown delivery"
  [ "$(xo_pending_reply_get "$rec" recovery_delivery_outcome)" = unknown ] \
    || fail "interrupted recovery should preserve unknown delivery outcome"
  [ -z "$(xo_pending_reply_get "$rec" recovery_sent_epoch)" ] \
  ...
  unset XO_PENDING_REPLY_SEND_HOOK
...
test_recovery_reply_resolves_original() {
  ...
  export -f recovery_hook
  export XO_PENDING_REPLY_SEND_HOOK='recovery_hook'
  export XO_PENDING_REPLY_NOW=3000

  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "phase 7 status")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  xo_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  printf 'done [corr=%s]: phase 7 is Done (reposted)\n' "$corr" > "$state/hibit.status"
  xo_pending_reply_try_resolve "$state" "$corr" || fail "recovery reply should resolve original"
...
test_second_missed_turn_escalates_once_and_stays_durable() {
  ...
  export -f recovery_hook
  export XO_PENDING_REPLY_SEND_HOOK='recovery_hook'
  export XO_PENDING_REPLY_NOW=4000
  # Do not export STATE into the test process: xo-send resolves
  # XO_STATE_OVERRIDE/STATE from the environment and a leak breaks later cases.

  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "why is phase 7 stuck")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  xo_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  # Recovery turn also completes with no correlated report.
  xo_pending_reply_mark_turn_completed "$state" "$corr" recovery
  xo_pending_reply_maybe_escalate "$state" "$corr" || fail "escalation should fire"
  ...
  if xo_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null; then
  ...
  [ "$(xo_pending_reply_get "$rec" parent_status)" = "$state/hibit.status" ] \
...
seen_gate() {  # <state> <file>: 0 when every byte is already announced
  XO_STATE_OVERRIDE="$1" bash -c '. "$1"; xo_wake_signal_seen_current "$2" "$3"' \
    _ "$ROOT/bin/xo-wake-lib.sh" "$1" "$2"
}
prime_seen() {  # <state> <file>
  XO_STATE_OVERRIDE="$1" bash -c '
    . "$1"; xo_wake_status_mark_current "$2" "$3"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$1" "$2"
}
...
test_escalation_wakes_and_its_close_stays_quiet() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_SEND_HOOK='true'
  export XO_PENDING_REPLY_NOW=4200
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "confirm the notarization")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  xo_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  xo_pending_reply_mark_turn_completed "$state" "$corr" recovery
  ...
  xo_pending_reply_try_resolve "$state" "$corr" || fail "correlated reply should resolve"
...
test_escalation_publication_failure_retries() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=4500
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "retry escalation")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  export XO_PENDING_REPLY_SEND_HOOK='true'
  xo_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  ...
  mkdir -p "$target"
  xo_pending_reply_set "$rec" parent_status "$target" || fail "failed to set escalation target"
  if xo_pending_reply_maybe_escalate "$state" "$corr" 2>/dev/null; then
  ...
  rmdir "$target"
  xo_pending_reply_maybe_escalate "$state" "$corr" || fail "escalation retry should succeed"
...
test_legacy_escalation_closes_default_decision() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=4725
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "legacy close")
  xo_pending_reply_mark_delivered "$state" "$corr"
  rec=$(xo_pending_reply_path "$state" "$corr")
  xo_pending_reply_set "$rec" phase escalated
  xo_pending_reply_set "$rec" escalated_epoch 4700
  ...
  xo_pending_reply_try_resolve "$state" "$corr" || fail "legacy reply should resolve its record"
  ...
  [ -n "$(xo_pending_reply_get "$rec" escalation_closed_epoch)" ] \
...
test_legacy_escalation_does_not_close_taken_default_decision() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=4750
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "legacy escalation")
  xo_pending_reply_mark_delivered "$state" "$corr"
  rec=$(xo_pending_reply_path "$state" "$corr")
  xo_pending_reply_set "$rec" phase escalated
  xo_pending_reply_set "$rec" escalated_epoch 4700
  ...
  xo_pending_reply_tick "$state" || fail "legacy close retry failed"
  open=$(status_open_decisions "$state/hibit.status")
...
test_foreign_blocker_is_not_selected_as_escalation() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=4775
  export XO_PENDING_REPLY_SEND_HOOK=true
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "foreign blocker")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  xo_pending_reply_send_recovery "$state" "$corr" || fail "recovery send failed"
  xo_pending_reply_mark_turn_completed "$state" "$corr" recovery
  xo_pending_reply_maybe_escalate "$state" "$corr" || fail "genuine escalation failed"
  rec=$(xo_pending_reply_path "$state" "$corr")
  printf 'blocked [key=release]: foreign decision pending-reply-id=%s corr=%s\n' \
    "$corr" "$corr" >> "$state/hibit.status"

  xo_pending_reply_try_resolve "$state" "$corr" || fail "correlated foreign blocker should resolve the record"
  open=$(status_open_decisions "$state/hibit.status")
...
test_concurrent_resolution_closes_escalation_once() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=4800
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "concurrent resolution")
  xo_pending_reply_mark_delivered "$state" "$corr"
  rec=$(xo_pending_reply_path "$state" "$corr")
  xo_pending_reply_set "$rec" phase escalated
  xo_pending_reply_set "$rec" escalated_epoch 4750
  ...
  for _ in 1 2 3 4 5 6 7 8; do
    xo_pending_reply_try_resolve "$state" "$corr" &
  done
  wait
...
test_concurrent_escalation_yields_to_late_reply() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=4900
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "concurrent escalation")
  xo_pending_reply_mark_delivered "$state" "$corr"
  rec=$(xo_pending_reply_path "$state" "$corr")
  xo_pending_reply_set "$rec" phase recovery_sent
  xo_pending_reply_set "$rec" recovery_turn_completed_epoch 4850
  ...
  [ -z "$(xo_pending_reply_get "$rec" escalated_epoch)" ] \
...
test_transport_success_is_not_reply_success() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=5000
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "ping")
  xo_pending_reply_mark_delivered "$state" "$corr" || fail "mark delivered failed"
...
test_undelivered_records_are_scan_immutable() {
  (
    ...
    export XO_PENDING_REPLY_NOW=5500
    corr=$(xo_pending_reply_create "$home" "$state" hibit "not delivered yet")
    rec=$(xo_pending_reply_path "$state" "$corr")
    printf 'done [corr=%s]: arrived too early\n' "$corr" > "$state/hibit.status"
    printf 'done [corr=%s]: wrong home too early\n' "$corr" > "$sm_home/state/hibit.status"
    xo_write_secondmate_meta "$state/hibit.meta" "$sm_home" "sess:xo-hibit"
    before=$(cat "$rec")
    if xo_pending_reply_try_resolve "$state" "$corr"; then
      fail "undelivered expectation must not resolve"
    fi
    xo_pending_reply_detect_wrong_home "$state" "$corr" "$sm_home" \
      || fail "undelivered wrong-home check should be inert"
    xo_pending_reply_tick_one "$state" "$corr" busy "$sm_home" \
      || fail "undelivered direct tick should be inert"
    xo_backend_busy_state() { fail "undelivered watcher tick must not probe the backend"; }
    xo_backend_capture() { fail "undelivered watcher tick must not capture the backend"; }
    xo_pending_reply_tick "$state" || fail "undelivered watcher tick should succeed"
    after=$(cat "$rec")
    [ "$after" = "$before" ] || fail "scan paths must not mutate an undelivered record"
    xo_pending_reply_mark_delivered "$state" "$corr" || fail "delivery marker should succeed"
    xo_pending_reply_tick_one "$state" "$corr" unknown "$sm_home" \
...
test_delivery_confirmation_fallback_reconciles() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export XO_PENDING_REPLY_NOW=5750
    corr=$(xo_pending_reply_create "$home" "$state" hibit "confirmed delivery")
    rec=$(xo_pending_reply_path "$state" "$corr")
    xo_pending_reply_mark_delivered() { return 1; }
    if xo_pending_reply_confirm_delivery "$state" "$corr"; then
      fail "primary delivery commit failure should be reported"
    else
      rc=$?
    fi
    [ "$rc" = 2 ] || fail "durable fallback should return status 2, got $rc"
    marker=$(xo_pending_reply_delivery_confirmation_path "$state" "$corr")
    [ -f "$marker" ] || fail "delivery confirmation fallback marker should persist"
    [ -z "$(xo_pending_reply_get "$rec" delivered_epoch)" ] \
      || fail "failed primary commit should leave delivered_epoch empty"
    . "$ROOT/bin/xo-pending-reply-lib.sh"
    xo_pending_reply_tick_one "$state" "$corr" unknown \
      || fail "watcher should reconcile the delivery marker"
    [ "$(xo_pending_reply_get "$rec" delivered_epoch)" = 5750 ] \
      || fail "watcher should restore the confirmed delivery epoch"
    [ ! -e "$marker" ] || fail "reconciled delivery marker should be removed"
    prepared_corr=$(xo_pending_reply_create "$home" "$state" hibit "prepared delivery")
    prepared_rec=$(xo_pending_reply_path "$state" "$prepared_corr")
    xo_pending_reply_prepare_delivery "$state" "$prepared_corr" \
      || fail "delivery preparation should persist before transport"
    xo_pending_reply_set "$prepared_rec" grace_secs 10 \
      || fail "delivery-unknown grace fixture should persist"
    prepared_marker=$(xo_pending_reply_delivery_confirmation_path "$state" "$prepared_corr")
    [ -f "$prepared_marker" ] || fail "prepared delivery marker should persist"
    xo_pending_reply_tick_one "$state" "$prepared_corr" unknown \
    ...
    [ -z "$(xo_pending_reply_get "$prepared_rec" delivered_epoch)" ] \
    ...
    export XO_PENDING_REPLY_NOW=5760
    xo_pending_reply_write_delivery_confirmation \
    ...
    xo_pending_reply_tick "$state" || fail "watcher should accept a late delivery report"
    [ "$(phase_of "$state" "$prepared_corr")" = resolved ] \
      || fail "late report should resolve escalated delivery-unknown"
    [ "$(xo_pending_reply_get "$prepared_rec" delivered_epoch)" = 5760 ] \
      || fail "late report should provide delivery evidence"
    escalations=$(grep -Fc "blocked [key=pending-reply-$prepared_corr]:" "$state/hibit.status")
    [ "$escalations" = 1 ] || fail "late report must not re-escalate delivery-unknown"
    xo_pending_reply_tick "$state" || fail "resolved late report should remain idempotent"
    [ "$(phase_of "$state" "$prepared_corr")" = resolved ] \
      || fail "late report resolution should remain durable"
    export XO_PENDING_REPLY_NOW=5800
    reported_corr=$(xo_pending_reply_create "$home" "$state" hibit "reported delivery")
    reported_rec=$(xo_pending_reply_path "$state" "$reported_corr")
    xo_pending_reply_prepare_delivery "$state" "$reported_corr" \
      || fail "reported delivery attempt should persist"
    reported_marker=$(xo_pending_reply_delivery_confirmation_path "$state" "$reported_corr")
    printf 'done [corr=%s]: report proves delivery\n' "$reported_corr" >> "$state/hibit.status"
    xo_pending_reply_try_resolve "$state" "$reported_corr" \
...
test_delivery_confirmation_serializes_with_reconciliation() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export XO_PENDING_REPLY_NOW=5900
    corr=$(xo_pending_reply_create "$home" "$state" hibit "serialized delivery")
    rec=$(xo_pending_reply_path "$state" "$corr")
    calls="$home/mark-delivered.calls"
    entered="$home/mark-delivered.entered"
    release="$home/mark-delivered.release"
    xo_pending_reply_mark_delivered() {
      local pending_state=$1 pending_corr=$2 epoch=$3 pending_rec phase
      printf '%s\n' "${BASHPID:-$$}" >> "$calls"
      : > "$entered"
      while [ ! -e "$release" ]; do /bin/sleep 0.01; done
      pending_rec=$(xo_pending_reply_path "$pending_state" "$pending_corr")
      xo_pending_reply_set "$pending_rec" delivered_epoch "$epoch" || return 1
      phase=$(xo_pending_reply_get "$pending_rec" phase)
      [ "$phase" != delivery_unknown ] \
        || xo_pending_reply_set "$pending_rec" phase awaiting_report
    }
    xo_pending_reply_confirm_delivery "$state" "$corr" &
    ...
    xo_pending_reply_reconcile_delivery "$state" "$corr" &
    # The background PID is consumed within this isolated test subshell.
    # shellcheck disable=SC2031
    reconcile_pid=$!
    /bin/sleep 0.1
    ...
    [ "$(xo_pending_reply_get "$rec" delivered_epoch)" = 5900 ] \
    ...
    [ ! -e "$(xo_pending_reply_delivery_confirmation_path "$state" "$corr")" ] \
...
test_unrelated_and_stale_corr_cannot_resolve() {
  ...
  # shellcheck disable=SC2031
  export XO_PENDING_REPLY_NOW=6000
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "need answer")
  xo_pending_reply_mark_delivered "$state" "$corr"
  other=$(xo_pending_reply_new_id)
...
test_restart_preserves_expectation_and_parent_destination() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=7000
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "survive restart")
  xo_pending_reply_mark_delivered "$state" "$corr"
  rec=$(xo_pending_reply_path "$state" "$corr")
  parent_status=$(xo_pending_reply_get "$rec" parent_status)
  parent_home=$(xo_pending_reply_get "$rec" parent_home)
  # Simulate process restart: re-source library and re-read the same record.
  # shellcheck source=bin/xo-pending-reply-lib.sh
  . "$ROOT/bin/xo-pending-reply-lib.sh"
  [ -f "$rec" ] || fail "record must survive restart"
  [ "$(xo_pending_reply_get "$rec" parent_status)" = "$parent_status" ] \
    || fail "parent_status must be stable across restart"
  [ "$(xo_pending_reply_get "$rec" parent_home)" = "$parent_home" ] \
...
test_wrong_home_detected_not_acknowledged() {
  ...
  mkdir -p "$sm_home/state"
  export XO_PENDING_REPLY_NOW=8000
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "report to parent")
  xo_pending_reply_mark_delivered "$state" "$corr"
  ...
  rec=$(xo_pending_reply_path "$state" "$corr")
  hits=$(xo_pending_reply_get "$rec" wrong_home_hits)
...
test_xo_send_marked_secondmate_creates_pending_and_embeds_corr() {
  local dir fb log home rc got corr rec
  dir="$TMP_ROOT/send-pending"; mkdir -p "$dir"
  fb=$(make_stubs "$dir"); log="$dir/send.log"
  home=$(setup_parent send-pending)
  xo_write_secondmate_meta "$home/state/hibit.meta" "$home/sm" "sess:xo-hibit"
  ...
  rec=$(xo_pending_reply_path "$home/state" "$corr")
  [ -f "$rec" ] || fail "pending-reply record must exist after marked send"
  [ "$(xo_pending_reply_get "$rec" phase)" = awaiting_report ] \
    || fail "phase should be awaiting_report after delivery"
  [ -n "$(xo_pending_reply_get "$rec" delivered_epoch)" ] \
    || fail "delivered_epoch must be set after successful send"
  [ "$(xo_pending_reply_get "$rec" task_id)" = hibit ] \
    || fail "task_id must match secondmate id"
  pass "xo-send marked secondmate path creates pending and embeds corr"
}
...
test_document_pointer_resolves() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=9000
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "deep audit")
  xo_pending_reply_mark_delivered "$state" "$corr"
  printf 'done [corr=%s]: see data/hibit/report.md\n' "$corr" > "$state/hibit.status"
  xo_pending_reply_try_resolve "$state" "$corr" || fail "document pointer status should resolve"
  [ "$(xo_pending_reply_get "$(xo_pending_reply_path "$state" "$corr")" resolved_via)" = document ] \
...
test_helper_report_resolves() {
  ...
  export XO_PENDING_REPLY_NOW=9100
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "quick answer")
  xo_pending_reply_mark_delivered "$state" "$corr"
  XO_HOME="$sm_home" "$REPORT" "done" "$corr" "all good" \
  ...
  xo_pending_reply_try_resolve "$state" "$corr" || fail "helper report should resolve"
  [ "$(xo_pending_reply_get "$(xo_pending_reply_path "$state" "$corr")" resolved_via)" = helper ] \
...
test_busy_idle_observation_via_backend_abstraction() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=9200
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "backend turn")
  xo_pending_reply_mark_delivered "$state" "$corr"
  # Simulates Pi/Claude secondmate busy_state from xo_backend_busy_state without
  # reading conversation text (herdr native idle/busy or tmux unknown fallback).
  xo_pending_reply_observe_busy "$state" "$corr" unknown
  [ -z "$(xo_pending_reply_get "$(xo_pending_reply_path "$state" "$corr")" request_turn_completed_epoch)" ] \
    || fail "unknown busy_state must not prove turn completion"
  xo_pending_reply_observe_busy "$state" "$corr" busy
  xo_pending_reply_observe_busy "$state" "$corr" idle
  [ -n "$(xo_pending_reply_get "$(xo_pending_reply_path "$state" "$corr")" request_turn_completed_epoch)" ] \
...
test_unknown_backend_state_uses_capture_fallback() {
  ...
  for backend in tmux zellij; do
    (
      ...
      sm_home="$home/sm"
      mkdir -p "$sm_home/state"
      export XO_PENDING_REPLY_GRACE_SECS=10
      # These fixture overrides are intentionally scoped to the isolated subshell.
      # shellcheck disable=SC2030,SC2031
      export XO_PENDING_REPLY_NOW=10000
      corr=$(xo_pending_reply_create "$home" "$state" "hibit" "$backend fallback")
      xo_pending_reply_mark_delivered "$state" "$corr"
      xo_write_secondmate_meta "$state/hibit.meta" "$sm_home" "session:xo-hibit" alpha pi
      [ "$backend" = tmux ] || printf 'backend=%s\n' "$backend" >> "$state/hibit.meta"
      xo_backend_busy_state() { printf 'unknown'; }
      xo_backend_capture() { printf '%s' "$XO_PENDING_TEST_CAPTURE"; }
      # Invoked indirectly through XO_PENDING_REPLY_SEND_HOOK.
      # shellcheck disable=SC2329
      recovery_hook() { :; }
      ...
      # shellcheck disable=SC2030,SC2031
      export XO_PENDING_REPLY_SEND_HOOK=recovery_hook
      export XO_PENDING_TEST_CAPTURE='idle footer'
      xo_pending_reply_tick "$state"
      rec=$(xo_pending_reply_path "$state" "$corr")
      [ -z "$(xo_pending_reply_get "$rec" request_turn_completed_epoch)" ] \
        || fail "$backend fallback must not accept stale idle before grace"
      # Continue advancing the subshell-local fixture clock.
      # shellcheck disable=SC2030,SC2031
      export XO_PENDING_REPLY_NOW=10010
      xo_pending_reply_tick "$state"
      [ "$(phase_of "$state" "$corr")" = recovery_sent ] \
        || fail "$backend fallback idle should trigger recovery after grace"
      export XO_PENDING_REPLY_NOW=10011
      export XO_PENDING_TEST_CAPTURE='Working...'
      xo_pending_reply_tick "$state"
      export XO_PENDING_REPLY_NOW=10012
      export XO_PENDING_TEST_CAPTURE='idle footer'
      xo_pending_reply_tick "$state"
      [ "$(phase_of "$state" "$corr")" = escalated ] \
...
test_kimi_capture_fallback_uses_recorded_harness() (
  ...
  # This fixture clock is intentionally scoped to the isolated subshell.
  # shellcheck disable=SC2030,SC2031
  export XO_PENDING_REPLY_NOW=10020
  corr=$(xo_pending_reply_create "$home" "$state" hibit "kimi fallback")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_write_secondmate_meta "$state/hibit.meta" "$sm_home" "session:xo-hibit" alpha kimi
  xo_backend_busy_state() { printf 'unknown'; }
  xo_backend_capture() { printf '%s' "$XO_PENDING_KIMI_CAPTURE"; }
  export XO_PENDING_KIMI_CAPTURE=' 🌑 · Tip: ask Kimi to schedule tasks, e.g. "remind me at 5pm"'

  [ "$(xo_pending_reply_backend_observation tmux session:xo-hibit xo-hibit codex)" = fallback-idle ] \
    || fail "Kimi spinner leaked into another harness"
  export XO_PENDING_KIMI_CAPTURE='Ctrl+c:cancel'
  [ "$(xo_pending_reply_backend_observation tmux session:xo-hibit xo-hibit kimi)" = fallback-idle ] \
    || fail "Grok's exact busy token leaked into Kimi pending-reply observation"
  export XO_PENDING_KIMI_CAPTURE=' 🌑 · Tip: ask Kimi to schedule tasks, e.g. "remind me at 5pm"'
  xo_pending_reply_tick "$state"
  rec=$(xo_pending_reply_path "$state" "$corr")
  [ "$(xo_pending_reply_get "$rec" turn_seen_busy)" = 1 ] \
...
test_tick_skips_terminal_and_reuses_target_observation() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export XO_PENDING_REPLY_NOW=10100
    open1=$(xo_pending_reply_create "$home" "$state" hibit "first open request")
    open2=$(xo_pending_reply_create "$home" "$state" hibit "second open request")
    xo_pending_reply_mark_delivered "$state" "$open1"
    xo_pending_reply_mark_delivered "$state" "$open2"
    resolved=$(xo_pending_reply_create "$home" "$state" resolved "resolved request")
    xo_pending_reply_mark_delivered "$state" "$resolved"
    printf 'done [corr=%s]: complete\n' "$resolved" > "$state/resolved.status"
    xo_pending_reply_try_resolve "$state" "$resolved" || fail "resolved fixture should resolve"
    escalated=$(xo_pending_reply_create "$home" "$state" escalated "escalated request")
    xo_pending_reply_mark_delivered "$state" "$escalated"
    rec=$(xo_pending_reply_path "$state" "$escalated")
    xo_pending_reply_set "$rec" phase escalated || fail "escalated fixture should transition"
    mkdir -p "$home/escalated/state"
    printf 'done [corr=%s]: wrong home\n' "$escalated" > "$home/escalated/state/child.status"
    xo_write_secondmate_meta "$state/hibit.meta" "$home/hibit" "sess:xo-hibit"
    xo_write_secondmate_meta "$state/resolved.meta" "$home/resolved" "sess:xo-resolved"
    xo_write_secondmate_meta "$state/escalated.meta" "$home/escalated" "sess:xo-escalated"
    ...
    # shellcheck disable=SC2329
    xo_backend_capture() { fail "native busy observations should not capture"; }
    # shellcheck disable=SC2329
    xo_pending_reply_find_resolve_line() {
      local status_file=$1 corr=$2 line
      printf '%s\t%s\n' "$status_file" "$corr" >> "$scan_log"
      [ -f "$status_file" ] || return 0
      while IFS= read -r line || [ -n "$line" ]; do
        xo_pending_reply_line_resolves "$line" "$corr" || continue
        printf '%s' "$line"
        return 0
      done < "$status_file"
      return 0
    ...
    rec=$(xo_pending_reply_path "$state" "$open1")
    [ "$(xo_pending_reply_get "$rec" turn_seen_busy)" = 1 ] \
      || fail "cached observation should update the first open record"
    rec=$(xo_pending_reply_path "$state" "$open2")
    [ "$(xo_pending_reply_get "$rec" turn_seen_busy)" = 1 ] \
      || fail "cached observation should update the second open record"
    rec=$(xo_pending_reply_path "$state" "$escalated")
    snapshot=$(xo_pending_reply_get "$rec" wrong_home_scan_signature)
    ...
    [ "$(xo_pending_reply_get "$rec" wrong_home_scan_signature)" = "$snapshot" ] \
...
test_correlations_reuse_only_for_matching_open_task() {
  ...
  state="$home/state"
  xo_write_secondmate_meta "$state/domain.meta" "$home/domain" "sess:xo-domain"
  xo_write_secondmate_meta "$state/other.meta" "$home/other" "sess:xo-other"
  run_send "$fb" "$home" "$log" domain "first request" || fail "first marked send failed"
  got=$(latest_record_body "$home" domain)
  corr1=$(xo_pending_reply_extract_corr "$got")
  export XO_PENDING_REPLY_EXISTING_CORR=$corr1
  if run_send "$fb" "$home" "$log" other "forwarded request"; then
    fail "an explicit cross-task correlation must be refused"
  fi
  unset XO_PENDING_REPLY_EXISTING_CORR
  ...
  corr2=$(xo_pending_reply_extract_corr "$(latest_record_body "$home" other)")
  [ -n "$corr2" ] && [ "$corr2" != "$corr1" ] \
    || fail "cross-task send must receive a new correlation"
  rec=$(xo_pending_reply_path "$state" "$corr2")
  [ "$(xo_pending_reply_get "$rec" task_id)" = other ] \
  ...
  rec=$(xo_pending_reply_path "$state" "$corr3")
  [ "$(xo_pending_reply_get "$rec" task_id)" = domain ] \
...
test_tick_end_to_end_missed_then_escalate() {
  ...
  export XO_PENDING_REPLY_SEND_HOOK='recovery_hook'
  # shellcheck disable=SC2031
  export XO_PENDING_REPLY_NOW=9300

  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "e2e miss")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_write_secondmate_meta "$state/hibit.meta" "$sm_home" "sess:xo-hibit"
  # Override backend busy via direct tick_one (backend may be unknown in hermetic home).
  xo_pending_reply_tick_one "$state" "$corr" busy "$sm_home"
  xo_pending_reply_tick_one "$state" "$corr" idle "$sm_home"
  [ "$(phase_of "$state" "$corr")" = recovery_sent ] \
  ...
  export XO_PENDING_REPLY_NOW=999999
  xo_pending_reply_tick_one "$state" "$corr" idle "$sm_home"
  [ -f "$(xo_pending_reply_path "$state" "$corr")" ] \
...
test_remote_repost_waits_for_the_reply_channel() {
  ...
  export XO_PENDING_REPLY_SEND_HOOK=remote_repost_hook

  xo_write_meta "$state/ios.meta" \
    "window=xo-remote:w1:p1" "harness=claude" "kind=secondmate" "mode=secondmate" \
    "remote_host=remote-mac" "remote_root=/remote/root" "remote_backend=herdr"
  corr=$(xo_pending_reply_create "$home" "$state" "ios" "status of the iOS build")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_observe_busy "$state" "$corr" busy
  xo_pending_reply_observe_busy "$state" "$corr" idle
  rec=$(xo_pending_reply_path "$state" "$corr")
  ...
  xo_pending_reply_note_remote_channel_caught_up "$state" ios 4000
  if xo_pending_reply_send_recovery "$state" "$corr" 2>/dev/null; then
  ...
  xo_pending_reply_note_remote_channel_caught_up "$state" ios \
    "$(xo_pending_reply_get "$rec" request_turn_completed_epoch)"
  xo_pending_reply_send_recovery "$state" "$corr" \
...
test_mirrored_remote_reply_never_triggers_a_repost() {
  ...
  export XO_PENDING_REPLY_SEND_HOOK=mirrored_reply_hook

  xo_write_meta "$state/ios.meta" \
    "window=xo-remote:w1:p1" "harness=claude" "kind=secondmate" "mode=secondmate" \
    "remote_host=remote-mac" "remote_root=/remote/root" "remote_backend=herdr"
  corr=$(xo_pending_reply_create "$home" "$state" "ios" "did the build go green")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  # The mirror caught up AND carried the mate's correlated answer.
  printf 'done [corr=%s]: build is green\n' "$corr" > "$state/ios.status"
  xo_pending_reply_note_remote_channel_caught_up "$state" ios 6000

  xo_pending_reply_tick_one "$state" "$corr" idle || fail "tick should succeed"
  [ "$(phase_of "$state" "$corr")" = resolved ] \
...
test_same_basename_self_home_corr_resolves_on_tick() {
  ...
  export -f recovery_hook
  export XO_PENDING_REPLY_SEND_HOOK=recovery_hook
  export XO_PENDING_REPLY_NOW=11000

  corr=$(xo_pending_reply_create "$home" "$state" mate "status of the audit")
  xo_pending_reply_mark_delivered "$state" "$corr"
  rec=$(xo_pending_reply_path "$state" "$corr")
  parent_status=$(xo_pending_reply_get "$rec" parent_status)
  ...
  [ -n "$(xo_pending_reply_get "$rec" resolved_epoch)" ] \
  ...
  [ "$(xo_pending_reply_get "$rec" wrong_home_hits)" = 1 ] \
    || fail "the stranded file should still count as one wrong-home sighting"
  [ "$(xo_pending_reply_sighting_display \
    "$(xo_pending_reply_get "$rec" wrong_home_first_sighting)")" = \
...
test_same_basename_reply_resolves_after_recovery_failure() {
  ...
  state="$home/state"
  sm_home=$(bind_local_mate "$home" mate)
  export XO_PENDING_REPLY_NOW=11050
  export XO_PENDING_REPLY_SEND_HOOK=false

  corr=$(xo_pending_reply_create "$home" "$state" mate "status after failed recovery")
  xo_pending_reply_mark_delivered "$state" "$corr"
  xo_pending_reply_mark_turn_completed "$state" "$corr" request
  if xo_pending_reply_send_recovery "$state" "$corr" 2>/dev/null; then
  ...
  rec=$(xo_pending_reply_path "$state" "$corr")
  parent_status=$(xo_pending_reply_get "$rec" parent_status)
  xo_write_secondmate_meta "$state/mate.meta" "$sm_home"
...
test_child_status_wrong_home_is_not_copied() {
  ...
  export -f recovery_hook
  export XO_PENDING_REPLY_SEND_HOOK=recovery_hook
  export XO_PENDING_REPLY_NOW=11100
  ...
  [ -z "$(xo_pending_reply_get "$rec" resolved_epoch)" ] \
  ...
  stored_first=$(xo_pending_reply_get "$rec" wrong_home_first_sighting)
  [ "$(xo_pending_reply_sighting_display "$stored_first")" = "$expected_display" ] \
...
test_mechanical_helper_writes_parent_channel() {
  ...
  state="$home/state"
  sm_home=$(bind_local_mate "$home" mate)
  export XO_PENDING_REPLY_NOW=11200
  corr=$(xo_pending_reply_create "$home" "$state" mate "status of the audit")
  xo_pending_reply_mark_delivered "$state" "$corr"
  XO_HOME="$sm_home" "$REPORT" "done" "$corr" "audit clean" \
  ...
  [ "$(phase_of "$state" "$corr")" = resolved ] || fail "phase should be resolved"
  empty_corr=$(xo_pending_reply_create "$home" "$state" mate "answer must not be empty")
  xo_pending_reply_mark_delivered "$state" "$empty_corr"
  rc=0
  XO_HOME="$sm_home" "$REPORT" "done" "$empty_corr" "" 2>/dev/null || rc=$?
  [ "$rc" -ne 0 ] || fail "helper must reject an empty status note"
  if xo_pending_reply_try_resolve "$state" "$empty_corr"; then
    fail "an empty helper report must not resolve an expectation"
  fi
  rc=0
  env -u XO_HOME "$REPORT" "done" "$empty_corr" "must require XO_HOME" \
    2>/dev/null || rc=$?
  [ "$rc" -ne 0 ] || fail "helper must require XO_HOME"
  rc=0
  XO_HOME="$home" "$REPORT" "done" "$corr" "from a main home" 2>/dev/null || rc=$?
...
test_remote_parent_replies_is_not_wrong_home() {
  ...
  mkdir -p "$sm_home/state"
  printf '%s\n' mate > "$sm_home/.xo-secondmate-home"
  cat > "$sm_home/.xo-secondmate-parent" <<EOF
schema=xo-secondmate-parent.v1
route=remote
parent_host=remote.example
EOF
  export XO_PENDING_REPLY_NOW=11300
  corr=$(xo_pending_reply_create "$home" "$state" mate "did the build go green")
  xo_pending_reply_mark_delivered "$state" "$corr"
  rec=$(xo_pending_reply_path "$state" "$corr")
...
test_local_parent_replies_is_wrong_home_evidence() {
  ...
  state="$home/state"
  sm_home=$(bind_local_mate "$home" mate)
  export XO_PENDING_REPLY_NOW=11350
  corr=$(xo_pending_reply_create "$home" "$state" mate "did the build go green")
  xo_pending_reply_mark_delivered "$state" "$corr"
  rec=$(xo_pending_reply_path "$state" "$corr")
  ...
  first=$(xo_pending_reply_get "$rec" wrong_home_first_sighting)
  [ "$(xo_pending_reply_sighting_display "$first")" = \
...
test_failed_send_discards_undelivered_expectation() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=9400
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "never lands")
  # Not delivered: discard is allowed.
  xo_pending_reply_discard_undelivered "$state" "$corr" || fail "discard undelivered failed"
  [ ! -f "$(xo_pending_reply_path "$state" "$corr")" ] \
    || fail "undelivered record should be removed"
  # Delivered records must not be discarded by this path.
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "landed")
  xo_pending_reply_mark_delivered "$state" "$corr"
  if xo_pending_reply_discard_undelivered "$state" "$corr" 2>/dev/null; then
    fail "delivered record must not be discarded"
  fi
  [ -f "$(xo_pending_reply_path "$state" "$corr")" ] || fail "delivered record must remain"
...
test_escalated_undelivered_correlation_stays_retryable() {
  ...
  state="$home/state"
  export XO_PENDING_REPLY_NOW=9600
  corr=$(xo_pending_reply_create "$home" "$state" "hibit" "wake after lost transport")
  rec=$(xo_pending_reply_path "$state" "$corr")
  marker=$(xo_pending_reply_delivery_confirmation_path "$state" "$corr")
  ...
  xo_pending_reply_prepare_delivery "$state" "$corr" || fail "prepare delivery failed"
  xo_pending_reply_mark_delivery_unknown "$state" "$corr" || fail "mark delivery unknown failed"
  xo_pending_reply_maybe_escalate "$state" "$corr" || fail "delivery-unknown escalation should fire"
  [ "$(phase_of "$state" "$corr")" = escalated ] || fail "phase should be escalated"
  [ -z "$(xo_pending_reply_get "$rec" delivered_epoch)" ] || fail "escalation must not invent delivery"
  [ "$(grep -cF "blocked [key=pending-reply-$corr]:" "$state/hibit.status")" = 1 ] \
    || fail "delivery-unknown escalation should publish once"
  xo_pending_reply_corr_reusable "$state" "$corr" hibit \
    || fail "an escalated undelivered correlation must stay reusable by its owner"
  if xo_pending_reply_corr_reusable "$state" "$corr" other 2>/dev/null; then
    fail "an escalated correlation must not be reusable for another task"
  fi
  xo_pending_reply_reset_known_undelivered "$state" "$corr" \
  ...
  [ -n "$(xo_pending_reply_get "$rec" escalated_epoch)" ] \
  ...
  export XO_PENDING_REPLY_NOW=9601
  xo_pending_reply_prepare_delivery "$state" "$corr" || fail "resend prepare failed"
  xo_pending_reply_confirm_delivery "$state" "$corr" || fail "resend confirm failed"
  [ -n "$(xo_pending_reply_get "$rec" delivered_epoch)" ] || fail "resend should confirm delivery"
  printf 'done [corr=%s]: routed work picked up\n' "$corr" >> "$state/hibit.status"
  xo_pending_reply_try_resolve "$state" "$corr" || fail "correlated report should resolve"
  [ "$(phase_of "$state" "$corr")" = resolved ] || fail "phase should be resolved"
  open=$(status_open_decisions "$state/hibit.status")
  ...
  export XO_PENDING_REPLY_NOW=9700
  delivered_corr=$(xo_pending_reply_create "$home" "$state" "hibit" "delivered then missed")
  delivered_rec=$(xo_pending_reply_path "$state" "$delivered_corr")
  xo_pending_reply_mark_delivered "$state" "$delivered_corr" || fail "mark delivered failed"
  xo_pending_reply_set "$delivered_rec" phase escalated
  xo_pending_reply_set "$delivered_rec" escalated_epoch 9700
  if xo_pending_reply_corr_reusable "$state" "$delivered_corr" hibit 2>/dev/null; then
    fail "a delivered escalated correlation must not be reusable"
  fi
  if xo_pending_reply_reset_known_undelivered "$state" "$delivered_corr" 2>/dev/null; then
  ...
  [ "$(xo_pending_reply_get "$delivered_rec" delivered_epoch)" = 9700 ] \
    || fail "refused reset must keep the confirmed delivery"
  unset XO_PENDING_REPLY_NOW
