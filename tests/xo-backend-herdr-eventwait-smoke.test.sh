#!/usr/bin/env bash
# tests/xo-backend-herdr-eventwait-smoke.test.sh - REAL-herdr smoke test for the
# native pane.agent_status_changed push escalation (xo_backend_herdr_wait_transition,
...
# herdr_safe_stop_and_delete on a private xo-lab-* session, never a bare/ambient
# `herdr server stop`. Every lifecycle op goes through bin/xo-herdr-lab.sh, which
...
SESSION="xo-lab-eventwait-smoke-$$"
...
xo_herdr_lab_prepare "$SESSION" || fail "could not prepare the isolated Herdr lab session"
...
if ! xo_backend_herdr_events_capable "$SESSION"; then
...
IDS=$(xo_backend_herdr_create_task "$CONTAINER" "xo-evwait1" /tmp "$SEEDED_TAB_ID") || fail "create_task failed"
...
# scratch xo state so window_to_task and the wake queue resolve
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/xo-evwait.XXXXXX")
...
xo_herdr_lab_cli "$SESSION" pane report-agent "$PANE_ID" --source xo-evwait-test --agent claude --state idle >/dev/null 2>&1 \
...
( xo_backend_herdr_wait_transition "$SESSION" 8 "$STATE" "$TARGET" > "$OUT"; echo $? > "$RCF" ) &
WPID=$!
sleep 0.5   # let it connect, subscribe, and reconcile the idle baseline

START=$(python3 -c 'import time; print(time.time())')
xo_herdr_lab_cli "$SESSION" pane report-agent "$PANE_ID" --source xo-evwait-test --agent claude --state blocked >/dev/null 2>&1 \
...
REC_PANE=$(xo_transition_pane_id "$REC")
REC_TO=$(xo_transition_to_status "$REC")
...
export XO_STATE_OVERRIDE="$STATE"
export XO_ROOT_OVERRIDE="$ROOT"
# shellcheck source=/dev/null
. "$ROOT/bin/xo-push-transition-lib.sh"
wake() { return 0; }
