#!/usr/bin/env bash
# tests/xo-tmux-agent-liveness.test.sh - portable regression for the tmux
...
# live per-harness counterpart is tests/xo-harness-liveness-drift-live-e2e.test.sh.
#
...
SOCKET="xo-liveness-$$"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-liveness.XXXXXX")
SESSION=liveness
...
wait_for_state() {  # <target> <expected> [tries]
  ...
  while [ "$i" -lt "$tries" ]; do
    got=$(xo_backend_agent_state tmux "$target")
  ...
  printf 'last verdict for %s was %s (expected %s); title=%s comms=[%s]\n' \
    ...
    "$(xo_backend_tmux_foreground_comms "$target" | tr '\n' ' ')" >&2
  return 1
}
...
title_classifies_agent() {  # <target>
  local name
  name=$(xo_backend_tmux_current_command "$1" 2>/dev/null)
  [ "$(xo_agent_process_classify_name "$name")" = agent ]
}
...
comms_classify_agent() {  # <target>
  ...
  while IFS= read -r name; do
    [ -n "$name" ] || continue
    [ "$(xo_agent_process_classify_name "$name")" = agent ] && return 0
  done <<EOF
$(xo_backend_tmux_foreground_comms "$1")
EOF
  while IFS= read -r name; do
    [ -n "$name" ] || continue
    [ "$(xo_agent_process_classify_name '' "$name")" = agent ] && return 0
  done <<EOF
$(xo_backend_tmux_foreground_argv0s "$1")
EOF
  return 1
}
...
assert_sources_disagree() {  # <target> <label>
  ...
  [ $((t + c)) -eq 1 ] || fail \
    "$2: the two name sources were expected to disagree, but title=$t comms=$c (title='$(xo_backend_tmux_current_command "$1")' comms='$(xo_backend_tmux_foreground_comms "$1" | tr '\n' ' ')')"
}
...
xo_backend_tmux_foreground_comms "$SESSION:no-such-window" >/dev/null \
  || fail "the foreground-comms read must stay best-effort for an absent window"
[ "$(xo_backend_agent_state tmux "$SESSION:no-such-window")" = missing ] \
...
cursor_anchored_verdict() {  # <target>
  local cy pane
  cy=$(xo_tmux_composer_cursor_row "$1")
  pane=$(xo_tmux_composer_capture "$1")
  xo_composer_classify_screen "$(xo_tmux_composer_caps)" "$pane" "$cy"
}

open_composer_pane cursor-idle "$LAB/bin/cursor-agent" 'Plan, search, build anything' 1
xo_tmux_pane_is_cursor "$SESSION:cursor-idle" \
...
[ "$(xo_tmux_composer_state "$SESSION:cursor-idle")" = empty ] \
...
[ "$(xo_tmux_composer_state "$SESSION:cursor-typed")" = pending ] \
...
if xo_tmux_pane_is_cursor "$SESSION:notcursor-idle"; then
  fail "a pane running a non-Cursor binary must not be identified as Cursor"
fi
[ "$(xo_tmux_composer_state "$SESSION:notcursor-idle")" = unknown ] \
...
if xo_tmux_pane_is_cursor "$SESSION:cursor-exited"; then
  fail "a pane whose Cursor process exited must not still identify as Cursor"
fi
[ "$(xo_tmux_composer_state "$SESSION:cursor-exited")" != empty ] \
