...
# Bearings before XO owned remediation. The shared script now stops,
...
install_runner() {  # <case-dir>
  ...
  cp "$ROOT/bin/xo-afk-contract.sh" "$dir/bin/"
  cp "$ROOT/bin/xo-branch-outcome.sh" "$dir/bin/"
  cp "$ROOT/bin/xo-tasks-axi-lib.sh" "$dir/bin/"
  cp "$ROOT/bin/xo-backlog-transition-lib.sh" "$dir/bin/"
  ...
  cat > "$dir/bin/xo-afk-launch.sh" <<'SH'
#!/usr/bin/env bash
[ "${1:-}" = stop ] || exit 2
printf 'stop\n' >> "$XO_HOME/stop.log"
rm -f "$XO_HOME/state/.afk"
if [ -e "$XO_HOME/state/.fail-terminal-stop-once" ]; then
  rm -f "$XO_HOME/state/.fail-terminal-stop-once"
  exit 1
fi
rm -f "$XO_HOME/state/.afk-daemon-terminal"
"$(dirname "$0")/xo-afk-contract.sh" archive >/dev/null
SH
  cat > "$dir/bin/xo-wake-drain.sh" <<'SH'
#!/usr/bin/env bash
file="$XO_HOME/state/.fake-drain"
if [ "${1:-}" = --ack-through ]; then
  [ "${3:-}" = --recovery-generation ] && [ "${4:-}" = fixture-generation ] || exit 2
  printf '%s\n' "$2" >> "$XO_HOME/state/.fake-drain-acks"
  : > "$file"
  exit 0
fi
if [ -s "$file" ]; then
  ...
  printf 'WAKE_ACK_REQUIRED: after handling completes run bin/xo-wake-drain.sh --ack-through %s --recovery-generation fixture-generation\n' "$sequence" >&2
fi
SH
  chmod +x "$dir/bin/"*.sh
}
...
run_return() {  # <case-dir> <mode>
  local dir=$1 mode=$2
  XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" "$dir/bin/xo-afk-return.sh" "$mode" 2>&1
}
...
ack_return() {  # <case-dir> <return-output>
  ...
  XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" \
    "$dir/bin/xo-wake-drain.sh" --ack-through "$sequence" --recovery-generation "$generation"
}

seed_live_blocker() {  # <case-dir> <backend> <key>
  local dir=$1 backend=$2 key=$3 target
  case "$backend" in
    tmux) target='synthetic:xo-repair-task' ;;
    herdr) target='xo-lab-synthetic:w1:p2' ;;
  esac
...
kind=ship
EOF
  printf 'blocked [key=%s]: xo can refresh the synthetic token\n' "$key" > "$dir/home/state/repair-task.status"
}
...
test_return_gate_orders_catchup_before_bearings() {
  ...
  printf 'xo away-mode inject WEDGED: 4555s undelivered\n' > "$dir/home/state/.subsuper-inject-wedged"
  {
  ...
  assert_contains "$out" 'xo-actionable blocker: repair-task [key=synthetic-dependency]' "return output did not assign blocker remediation to XO"
  ...
  grep -F $'evidence\twedge\txo away-mode inject WEDGED: 4555s undelivered' "$gate" >/dev/null || fail "wedge evidence was not retained in the durable gate"
  ...
  set +e
  out=$(XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" "$ROOT/bin/xo-bearings-snapshot.sh" --json 2>&1)
  rc=$?
  set -e
...
test_captain_decision_does_not_masquerade_as_xo_blocker() {
  local dir out
  dir="$TMP_ROOT/captain-decision"
  install_runner "$dir"
  cat > "$dir/home/state/decision-task.meta" <<'EOF'
window=synthetic:xo-decision-task
backend=tmux
kind=ship
EOF
  ...
  out=$(run_return "$dir" begin) || fail "approval decision should not be treated as an xo blocker: $out"
  assert_contains "$out" 'catch-up wake:' "approval decision notification was not surfaced in catch-up"
  [ ! -e "$dir/home/state/.afk-return-catchup" ] || fail "approval decision incorrectly opened an xo blocker gate"
  pass "needs-decision remains reportable without masquerading as an xo-actionable blocker"
}
...
test_evidence_publication_failure_preserves_wake_for_redrain() {
  ...
  set +e
  XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" \
    "$dir/bin/xo-afk-return.sh" begin 3< "$dir/read-only-output" >&3 2> "$dir/failed.err"
  rc=$?
  set -e
...
test_away_reentry_refuses_pending_return_gate() {
  ...
  set +e
  out=$(XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" "$ROOT/bin/xo-afk-launch.sh" start-native 2>&1)
  rc=$?
  set -e
...
contract_in() {  # <case-dir> <args...>
  local dir=$1
  shift
  XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" "$dir/bin/xo-afk-contract.sh" "$@"
}

outcome_in() {  # <case-dir> <args...>
  local dir=$1
  shift
  XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" "$dir/bin/xo-branch-outcome.sh" "$@"
}
...
test_return_brief_composes_from_record_store_and_held_set() {
  ...
  outcome_in "$dir" append --task other --verdict routine \
    --summary 'resent the steer; worker resumed' --wake 'stale: synthetic:xo-other' >/dev/null \
  ...
  assert_contains "$out" 'fix-windows [key=token] still blocked, xo remediates before ordinary work' "the blocker sharing a task with a captain outcome was exempted"
  assert_contains "$out" 'other [key=dep] still blocked, xo remediates before ordinary work' "the unreached blocker was not listed as could-not-fix"
  ...
  assert_contains "$out" 'xo-actionable blocker: other [key=dep]' "the unreached blocker did not gate"
  assert_contains "$out" 'xo-actionable blocker: fix-windows [key=token]' "a captain outcome incorrectly exempted an open blocker"
  ...
  XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" "$dir/bin/xo-afk-return.sh" guard \
...
SH
  ...
  gate="$dir/home/state/.afk-return-catchup"
  set +e
  out=$(PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" \
    "$dir/bin/xo-afk-return.sh" begin 2>&1)
  rc=$?
  set -e
...
test_unreadable_status_file_keeps_catchup_gated() {
  ...
  printf 'window=synthetic:xo-unreadable\nbackend=tmux\nkind=ship\n' > "$dir/home/state/unreadable.meta"
...
test_return_guard_refuses_while_the_record_exists() {
  ...
  set +e
  out=$(XO_HOME="$dir/home" XO_STATE_OVERRIDE="$dir/home/state" "$dir/bin/xo-afk-return.sh" guard 2>&1)
  rc=$?
  set -e
