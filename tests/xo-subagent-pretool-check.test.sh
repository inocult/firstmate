...
CHECK="$ROOT/bin/xo-subagent-pretool-check.sh"
TMP_ROOT=$(xo_test_tmproot xo-subagent-pretool-tests)
...
BRIEF_ONLY_ROUTE='first classify the work under the AGENTS.md intake contract, then use bin/xo-brief.sh followed by bin/xo-spawn.sh for dispatched work'
SCOUT_ROUTE='first classify the work under the AGENTS.md intake contract: work already classified as a scout goes to bin/xo-scout.sh "<question>" [project], while authorized ship work and its bounded research go to bin/xo-brief.sh then bin/xo-spawn.sh'
...
run_tool() {
  ...
  : > "$OUT"
  : > "$ERR"
  env XO_ROOT_OVERRIDE="$PRIMARY" XO_HOME="$PRIMARY" XO_STATE_OVERRIDE="$STATE" "$@" \
...
test_deny_message_defers_to_intake_classification() {
  local actual
  printf '#!/usr/bin/env bash\n' > "$PRIMARY/bin/xo-scout.sh"
  run_tool Agent && fail "scout-present case must still deny"
  actual=$(jq -r '.systemMessage' "$ERR")
  case "$actual" in
    *"$SCOUT_ROUTE"*) ;;
    *) fail "deny must reserve bin/xo-scout.sh for classified scout work: $actual" ;;
  esac
  case "$actual" in
    *'investigation or diagnosis goes to bin/xo-scout.sh'*) fail "deny must not classify all investigation or diagnosis as scout work: $actual" ;;
  esac
  rm -f "$PRIMARY/bin/xo-scout.sh"
  run_tool Agent && fail "scout-absent case must still deny"
  actual=$(jq -r '.systemMessage' "$ERR")
  case "$actual" in
    *"$BRIEF_ONLY_ROUTE"*) ;;
    *) fail "deny must degrade to brief-then-spawn when xo-scout.sh is absent: $actual" ;;
  esac
  pass "deny defers to intake classification and degrades gracefully without xo-scout.sh"
}

test_escape_hatch_allows_deliberate_use() {
  local rc value
  expect_allow "escape hatch set" Agent XO_ALLOW_SUBAGENT=1
  expect_deny "escape hatch unset" Agent
  for value in '' 0 yes true 11; do
    rc=0
    run_tool Agent "XO_ALLOW_SUBAGENT=$value" || rc=$?
    [ "$rc" -eq 2 ] || fail "XO_ALLOW_SUBAGENT='$value' must not release the guard, got exit $rc"
  done
  pass "the single documented escape hatch releases the guard only on the exact opt-in value"
}

test_task_worktree_and_non_xo_repo_are_inert() {
  ...
  : > "$OUT"
  : > "$ERR"
  XO_ROOT_OVERRIDE="$child" XO_HOME="$child" XO_STATE_OVERRIDE="$child/state" \
  ...
  git -C "$plain" init -q
  rc=0
  XO_ROOT_OVERRIDE="$plain" XO_HOME="$plain" XO_STATE_OVERRIDE="$plain/state" \
    "$CHECK" --claude --tool Agent > "$OUT" 2> "$ERR" || rc=$?
  [ "$rc" -eq 0 ] || fail "a non-xo repo must be out of scope, got exit $rc"
  pass "the guard is inert in a crewmate task worktree and in a non-xo repo"
}
...
test_secondmate_home_is_in_scope() {
  ...
  printf '# fixture\n' > "$second/AGENTS.md"
  printf 'sm-fixture\n' > "$second/.xo-secondmate-home"
  XO_ROOT_OVERRIDE="$second" XO_HOME="$second" XO_STATE_OVERRIDE="$second/state" \
    "$CHECK" --claude --tool Agent > "$OUT" 2> "$ERR" || rc=$?
...
test_stdin_transports_and_output_shapes() {
  ...
  printf '%s' '{"tool_name":"Agent","tool_input":{"prompt":"go"}}' \
    | XO_ROOT_OVERRIDE="$PRIMARY" XO_HOME="$PRIMARY" XO_STATE_OVERRIDE="$STATE" \
...
test_missing_jq_stdin_transport_fails_open() {
  ...
  printf '%s' '{"tool_name":"Agent"}' \
    | env PATH="$fakebin" XO_ROOT_OVERRIDE="$PRIMARY" XO_HOME="$PRIMARY" XO_STATE_OVERRIDE="$STATE" \
      "$CHECK" --claude > "$OUT" 2> "$ERR" || rc=$?
