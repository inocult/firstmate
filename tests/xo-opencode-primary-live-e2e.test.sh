...
# XO_HOME. Existing OpenCode credentials stay in their managed store.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

xo_live_gate opt-in XO_OPENCODE_LIVE_E2E opencode tmux sqlite3
...
xo_operational_input_encode session-start "$LEGACY_START" CURRENT_START \
...
run_ahoy_transcript_regressions() {
  ...
  cp "$ROOT/.opencode/plugins/xo-primary-sessionstart-nudge.js" \
...
run_native_ahoy_regressions() {
  ...
  (
    cd "$AHOY_PROJECT" &&
      OPENCODE_DB="$first_db" XO_HOME="$first_home" \
  ...
  startup_text=$(sqlite3 -json "$first_db" \
    "select json_extract(p.data,'$.text') text from message m join part p on p.message_id=m.id where json_extract(m.data,'$.role')='user' and json_extract(p.data,'$.text') like '%XO_OP:%' order by m.time_created limit 1;" \
    | jq -r '.[0].text')
  ...
  "$TMUX" -L "$SOCKET" new-session -d -s "$native_session" -c "$AHOY_PROJECT" \
    "env OPENCODE_DB='$later_db' XO_HOME='$later_home' OPENCODE_DISABLE_AUTOUPDATE=1 OPENCODE_DISABLE_LSP_DOWNLOAD=1 OPENCODE_CONFIG_CONTENT='{\"permission\":{\"*\":\"allow\"}}' opencode --auto"
  i=0
  while [ "$i" -lt 120 ]; do
  ...
  (
    cd "$AHOY_PROJECT" &&
      OPENCODE_DB="$later_db" XO_HOME="$later_home" \
        OPENCODE_DISABLE_AUTOUPDATE=1 OPENCODE_DISABLE_LSP_DOWNLOAD=1 \
        OPENCODE_CONFIG_CONTENT='{"permission":{"*":"allow"}}' \
...
cp "$ROOT/.opencode/plugins/xo-primary-watch-arm.js" "$PROJECT/.opencode/plugins/xo-primary-watch-arm.js"
cp "$ROOT/.opencode/plugins/lib/xo-operational-input.js" "$PROJECT/.opencode/plugins/lib/xo-operational-input.js"
...
# shellcheck disable=SC2016 # The model, not this test shell, expands XO_HOME.
PROMPT='Use the terminal to run `printf ready > "$XO_HOME/state/opencode-model-initial"`, then respond briefly. If a later watcher wake arrives, run bin/xo-wake-drain.sh, then run `printf handled > "$XO_HOME/state/opencode-model-handled"`. Never run or request any watcher arm command.'
"$TMUX" -L "$SOCKET" new-session -d -s "$SESSION" -c "$PROJECT" \
  "env OPENCODE_CONFIG_CONTENT='{\"permission\":{\"*\":\"allow\"}}' XO_HOME='$HOME_DIR' XO_ROOT_OVERRIDE='$PROJECT' XO_POLL=1 XO_SIGNAL_GRACE=0 XO_HEARTBEAT=600 bash -lc 'printf \"%s\\n\" \"\$\$\" > \"\$XO_HOME/state/.lock\"; opencode --auto; rc=\$?; printf \"OPENCODE_EXIT=%s\\n\" \"\$rc\"; sleep 300'"
...
if printf '%s\n' "$pane" | grep -Fq '$ bin/xo-watch-arm.sh'; then
