...
# `xo-bearings-board.sh build` and then executed under the minimal DOM shim in
...
TMP_ROOT=$(xo_test_tmproot xo-bearings-board-render)
...
make_home() {  # <name>
  ...
  # tests/xo-bearings-board.test.sh owns.
...
render() {  # <home> <charted-json> [charted_more] [charted_warning_more]
  ...
  jq -n --argjson charted "$charted" --argjson more "$more" --argjson warning_more "$warning_more" '{
    schema:"xo-bearings-board.v1", home:"render-home", generated:"2026-08-26T00:00Z",
...
test_omitted_warnings_never_count_as_more_queued() {
  ...
  printf '%s' "$out" | jq -e '
    (.empty | length) == 1 and (.empty[0] | test("Nothing is queued"))
      and (.more == ["+1 more repair warning - ask xo for the full chart"])
