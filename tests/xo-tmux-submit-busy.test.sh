#!/usr/bin/env bash
# tests/xo-tmux-submit-busy.test.sh - regression: busy pane + pending composer
...
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xo-tmux-submit-busy.XXXXXX")
trap 'rm -rf "$TMP_ROOT"' EXIT

# Override xo_pane_is_busy for testing: XO_FAKE_PANE_BUSY=1 means busy.
xo_pane_is_busy() {
  [ "${XO_FAKE_PANE_BUSY:-0}" = 1 ]
}
...
case "${1:-}" in
  display-message)
    ...
    exit 0 ;;
  capture-pane)
    if [ -n "${XO_FAKE_CAPTURE_COUNT:-}" ]; then
      count=0
      [ ! -f "$XO_FAKE_CAPTURE_COUNT" ] || count=$(cat "$XO_FAKE_CAPTURE_COUNT")
      count=$((count + 1))
      printf '%s\n' "$count" > "$XO_FAKE_CAPTURE_COUNT"
      if [ "${XO_FAKE_FAIL_FIRST_CAPTURE:-0}" = 1 ] && [ "$count" -eq 1 ]; then
        exit 1
      fi
    fi
...
test_busy_pane_pending_returns_empty() {
  ...
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" xo_tmux_composer_state "win" > "$vfile" 2>/dev/null
  ...
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" XO_FAKE_SENT="$sent" \
    XO_FAKE_SWALLOW="$dir/.swallow" XO_FAKE_PERSIST_SWALLOW=1 XO_FAKE_PANE_BUSY=1 \
    xo_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  ...
  pass "xo_tmux_submit_enter_core: busy pane + pending composer returns empty (message queued)"
}
...
test_idle_pane_pending_returns_pending() {
  ...
  touch "$dir/.swallow"
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" XO_FAKE_SENT="$sent" \
    XO_FAKE_SWALLOW="$dir/.swallow" XO_FAKE_PERSIST_SWALLOW=1 XO_FAKE_PANE_BUSY=0 \
    xo_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  [ "$(cat "$vfile")" = pending ] || fail "idle-pane pending should return pending, got '$(cat "$vfile")'"
  pass "xo_tmux_submit_enter_core: idle pane + pending composer stays pending (genuine swallow preserved)"
}
...
test_wrapped_continuation_retries_swallowed_enter() {
  ...
  pass "xo_tmux_submit_enter_core: wrapped input retains swallowed-Enter retries"
}
...
test_placeholder_like_bare_input_retries_swallowed_enter() {
  ...
  pass "xo_tmux_submit_enter_core: placeholder-like bare input retains swallowed-Enter retries"
}
...
test_busy_pane_composer_clears_first_try() {
  ...
  printf '╭────────────╮\n│ > fix      │\n╰────────────╯\n' > "$composer"
  : > "$sent"
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" XO_FAKE_SENT="$sent" XO_FAKE_PANE_BUSY=1 \
    xo_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  [ "$(cat "$vfile")" = empty ] || fail "busy-pane with cleared composer should return empty, got '$(cat "$vfile")'"
  pass "xo_tmux_submit_enter_core: busy pane clears composer on first Enter - returns empty"
}
...
test_idle_pane_composer_clears_first_try() {
  ...
  printf '╭────────────╮\n│ > fix      │\n╰────────────╯\n' > "$composer"
  : > "$sent"
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" XO_FAKE_SENT="$sent" XO_FAKE_PANE_BUSY=0 \
    xo_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  [ "$(cat "$vfile")" = empty ] || fail "idle-pane with cleared composer should return empty, got '$(cat "$vfile")'"
  pass "xo_tmux_submit_enter_core: idle pane clears composer on first Enter - returns empty as before"
}
...
test_busy_pane_unknown_stays_unknown() {
  ...
  printf '│ > unbounded\n' > "$composer"
  touch "$dir/.swallow"
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" XO_FAKE_PANE_BUSY=1 \
  ...
  pass "xo_tmux_submit_enter_core: busy conversion is limited to proven pending input"
}
...
test_failed_baseline_capture_keeps_busy_unknown_unconfirmed() {
  ...
  printf '│ > unbounded\n' > "$composer"
  touch "$dir/.swallow"
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" \
    XO_FAKE_CAPTURE_COUNT="$dir/captures" XO_FAKE_FAIL_FIRST_CAPTURE=1 \
    XO_FAKE_SWALLOW="$dir/.swallow" XO_FAKE_PERSIST_SWALLOW=1 XO_FAKE_APPEND_BUSY=1 \
    xo_tmux_submit_core "win" "fix" 3 0.05 0.05 > "$vfile" 2>/dev/null
  [ "$(cat "$vfile")" = unknown ] \
  ...
  pass "xo_tmux_submit_core: failed baseline capture disables busy unknown conversion"
}
...
test_busy_pane_ambiguous_pending_retries_without_conversion() {
  ...
  pass "xo_tmux_submit_enter_core: pending-unproven retries without busy conversion"
}
...
test_unrecognized_state_skips_busy_conversion() {
  ...
  (
    ...
    # shellcheck disable=SC2329
    xo_pane_is_busy() { touch "$busy_called"; return 0; }
    PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" \
      xo_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
  ...
  pass "xo_tmux_submit_enter_core: unrecognized states skip busy conversion"
}
...
test_claude_busy_signature_uses_real_capture_shapes() {
  ...
  pane_busy() {
    PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$composer" \
      bash -c '. "$1/bin/xo-tmux-lib.sh"; xo_pane_is_busy "$2" "$3"' \
      _ "$ROOT" "$1" "${2:-}"
  }
  ...
  pass "xo_pane_is_busy: Claude spinner is scoped, multi-frame, and backward-compatible"
}
