...
TMP_ROOT=$(xo_test_tmproot xo-rovo-harness)
...
state=$(cat "$XO_FAKE_ROVO_STATE" 2>/dev/null || true)
fake_screen() {
  case "$state" in
    ready)
  ...
  send-keys)
    ...
    case " $* " in
      *' Enter '*)
        case "$state" in
          launched)
            if [ "${XO_FAKE_ROVO_READY:-yes}" = yes ]; then
              printf 'ready\n' > "$XO_FAKE_ROVO_STATE"
            fi
            ;;
          pointer-typed)
            if [ "${XO_FAKE_ROVO_DELIVERY:-yes}" = yes ]; then
              printf 'delivered\n' > "$XO_FAKE_ROVO_STATE"
            else
              printf 'ready\n' > "$XO_FAKE_ROVO_STATE"
            fi
...
run_spawn() {
  ...
  HOME="$home" XO_ROOT_OVERRIDE='' XO_HOME="$home" \
    ...
    XO_FAKE_ROVO_STATE="$case_dir/rovo.state" \
    XO_FAKE_TMUX_CALL_LOG="$case_dir/tmux-calls.log" \
    XO_FAKE_BRIEF_REAL="$(cd "$home/data/$id" && pwd -P)/launch-brief.md" \
    XO_FAKE_ROVO_READY="${XO_FAKE_ROVO_READY:-yes}" \
    XO_FAKE_ROVO_DELIVERY="${XO_FAKE_ROVO_DELIVERY:-yes}" \
    XO_ROVO_READY_POLLS=3 XO_ROVO_DELIVERY_POLLS=3 XO_ROVO_POLL_INTERVAL=0 \
...
test_rovo_launch_then_send_is_verified() {
  ...
  assert_contains "$launch" "env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u XO_PI_HARNESS" \
  ...
  pass "xo-spawn: rovo launches bare, waits for readiness, and delivers its brief pointer"
}
...
test_rovo_effort_xhigh_is_recorded_but_omitted() {
  ...
  pass "xo-spawn: rovo omits efficiencyLevel for xhigh but keeps its allowedExternalPaths grant, recording xhigh in task metadata"
}
...
test_rovo_effort_high_sets_config_override() {
  ...
  pass "xo-spawn: rovo's supported effort values merge into the same --config-override as its allowedExternalPaths grant"
}
...
test_rovo_readiness_gate_precedes_pointer() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(XO_FAKE_ROVO_READY=no run_spawn \
  ...
  grep -q "kill-window.*xo-$id" "$CASE_DIR/tmux-calls.log" \
  ...
  pass "xo-spawn: rovo never sends the brief pointer before an observable ready signal, and tears down the created endpoint on failure"
}
...
test_rovo_unconfirmed_delivery_fails_loudly() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(XO_FAKE_ROVO_DELIVERY=no run_spawn \
    "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$id") || rc=$?
  ...
  pass "xo-spawn: rovo treats a silent pointer drop as a failed spawn, and tears down the created endpoint"
}
...
test_rovo_missing_binary_refuses_before_pane_creation() {
  ...
  pass "xo-spawn: missing rovo executable refuses before pane creation"
}
...
test_rovo_secondmate_is_refused() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(HOME="$HOME_DIR" XO_ROOT_OVERRIDE='' XO_HOME="$HOME_DIR" \
  ...
  pass "xo-spawn: rovo cannot be launched as a secondmate"
}
...
SH
  ...
  pass "xo-harness: rovo's markers outrank an inherited CLAUDECODE, and markerless ancestry still resolves rovo"
}

test_rovo_control_lib_table() {
  # shellcheck source=/dev/null
  . "$ROOT/bin/xo-control-lib.sh"
  [ "$(xo_control_interrupt_key rovo)" = Escape ] || fail "rovo interrupt key is not Escape"
  [ "$(xo_control_interrupt_repeat rovo)" = 1 ] || fail "rovo interrupt repeat is not 1"
  [ -z "$(xo_control_interrupt_clear_key rovo)" ] || fail "rovo should need no interrupt clear key"
  [ "$(xo_control_interrupt_ack_source rovo)" = none ] || fail "rovo interrupt ack source is not none"
  [ "$(xo_control_exit_command rovo)" = /exit ] || fail "rovo exit command is not /exit"
  [ "$(xo_control_harness_family rovo-anything)" = rovo ] || fail "rovo harness family prefix match failed"
  xo_control_harness_supports_kind rovo ship || fail "rovo should support ship tasks"
  xo_control_harness_supports_kind rovo scout || fail "rovo should support scout tasks"
  if xo_control_harness_supports_kind rovo secondmate; then
    fail "rovo should never support secondmate tasks"
  fi
  pass "xo-control-lib: rovo's lifecycle table matches its verified facts"
}
...
test_rovo_busy_regex_isolated() {
  ...
  local out
  out=$(xo_busy_classify tmux fake:0 rovo taskid /nonexistent-state '⬢ Rovo is thinking...')
  [ "$out" = "busy rovo-regex" ] || fail "xo_busy_classify did not read a real rovo busy tail as busy rovo-regex, got '$out'"
  ...
  out=$(xo_busy_classify tmux fake:0 rovo taskid /nonexistent-state 'Context: 1% 2K/900K')
  [ "$out" = "unknown rovo-regex" ] || fail "xo_busy_classify misread a marker-absent rovo tail as definitive idle instead of unknown, got '$out'"
...
test_rovo_busy_marker_scrolled_out_of_tail_is_unknown() {
  ...
  . "$ROOT/bin/xo-busy-lib.sh"
  local tail40 out i
  # xo_busy_rovo_tail_busy only inspects the last 12 nonblank lines. Build a
...
tool output line $i"
  ...
  out=$(xo_busy_classify tmux fake:0 rovo taskid /nonexistent-state "$tail40")
