#!/usr/bin/env bash
# tests/xo-control-herdr-smoke.test.sh - real-herdr smoke test for the agent
# lifecycle control plane (bin/xo-control.sh).
#
# tmux is the control plane's reference backend and is covered hermetically in
# tests/xo-control.test.sh. herdr is the OTHER backend whose recovery-grade
...
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/xo-control-herdr.XXXXXX")
...
CONTAINER_RAW=$(xo_backend_herdr_container_ensure "$WT") || fail "container_ensure failed"
CONTAINER=${CONTAINER_RAW%%$'\t'*}
SEEDED_TAB_ID=${CONTAINER_RAW#*$'\t'}
WORKSPACE_ID=${CONTAINER#*:}
TASK_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "xo-hsmoke" "$WT" "$SEEDED_TAB_ID") \
...
run_control() {
  env XO_HOME="$HOME_DIR" HERDR_SESSION="$SESSION" XO_SPAWN_NO_GUARD=1 \
    XO_CONTROL_POLL=0.2 XO_CONTROL_EXIT_WAIT=2 \
    "$ROOT/bin/xo-control.sh" "$@" 2>&1
}
...
# tests/xo-backend-herdr.test.sh; this is the check that notices when the real
...
STATE=$(xo_backend_agent_state herdr "$SESSION:$PANE_ID")
[ "$STATE" = dead ] \
...
# session.
[ "$(xo_backend_herdr_server_running_state "$SESSION")" = running ] \
  || version_fail "this run's own live lab session does not report .server.running=true through status --json"
[ "$(xo_backend_herdr_server_running_state "xo-lab-never-started-$$")" = stopped ] \
...
[ "$(xo_backend_agent_state herdr "xo-lab-never-started-$$:w1:p2")" = missing ] \
...
[ "$(xo_backend_agent_state herdr "no-separator-here")" = unreadable ] \
...
xo_backend_herdr_send_text_line "$SESSION:$PANE_ID" "export PATH=$FAKEBIN_Q:\$PATH" \
  || fail "could not put the inert test harness on the pane PATH"
xo_backend_herdr_send_text_line "$SESSION:$PANE_ID" "cd -- $PROJ_Q" \
  || fail "could not move the agent-free pane out of its recorded worktree"
for _ in $(seq 1 20); do
  [ "$(xo_backend_herdr_current_path "$SESSION:$PANE_ID" 2>/dev/null || true)" != "$PROJ_REAL" ] || break
  sleep 0.1
done
[ "$(xo_backend_herdr_current_path "$SESSION:$PANE_ID" 2>/dev/null || true)" = "$PROJ_REAL" ] \
  || fail "the real Herdr pane did not drift out of its recorded worktree"

OUT=$(env XO_HOME="$HOME_DIR" HERDR_SESSION="$SESSION" XO_SPAWN_NO_GUARD=1 \
  "$ROOT/bin/xo-spawn.sh" hsmoke --relaunch --harness codex) \
...
[ "$(xo_backend_herdr_current_path "$SESSION:$PANE_ID" 2>/dev/null || true)" = "$WT_REAL" ] \
...
# tests/xo-tmux-agent-liveness.test.sh uses (a copied platform binary fails code
...
wait_process_state() {  # <expected> <tries>
  ...
  while [ "$i" -lt "$tries" ]; do
    [ "$(xo_backend_herdr_pane_process_state "$SESSION" "$PANE_ID")" != "$expected" ] || return 0
    sleep 0.1
    i=$((i + 1))
  done
  return 1
...
start_agent_process() {
  xo_backend_herdr_send_text_line "$SESSION:$PANE_ID" "$AGENT_Q 900" \
    || fail "could not start the agent-named foreground process in the task pane"
  wait_process_state agent 50 \
    || version_fail "a real agent-named foreground process reads '$(xo_backend_herdr_pane_process_state "$SESSION" "$PANE_ID")' rather than 'agent' through pane process-info"
}

start_agent_process
herdr pane report-agent "$PANE_ID" --source xo-control-smoke --agent xo-control-smoke-agent \
...
wait_process_state shell 50 \
  || version_fail "after the agent process exited the pane reads '$(xo_backend_herdr_pane_process_state "$SESSION" "$PANE_ID")' rather than 'shell' through pane process-info. Raw process-info: $(herdr pane process-info --pane "$PANE_ID" --session "$SESSION" 2>&1 | tr -d '\n')"
...
PANE_STATE=$(xo_backend_herdr_pane_agent_state "$SESSION" "$PANE_ID")
...
xo_backend_herdr_kill "$SESSION:$PANE_ID" 2>/dev/null || true
