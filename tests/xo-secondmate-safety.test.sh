#!/usr/bin/env bash
# tests/xo-secondmate-safety.test.sh - secondmate home safety invariants:
...
# operator flow lives in xo-secondmate-lifecycle-e2e.test.sh; this file keeps the
...
TMP_ROOT=$(xo_test_tmproot xo-secondmate-safety)
...
install_fake_process_event_sweep() {
  local home=$1 log=$2
  mkdir -p "$home/bin"
  cat > "$home/bin/xo-procevent.sh" <<'SH'
#!/usr/bin/env bash
set -eu
case "${1:-}" in
  sweep-home)
    ...
    [ "$#" -eq 1 ] || exit 2
    printf '%s\n' "$XO_HOME" >> "$XO_FAKE_PROCEVENT_SWEEP_LOG"
    rm -f -- "$XO_HOME"/state/procevent/*.source "$XO_HOME"/state/procevent/*.runner
    ;;
  reconcile)
    printf '%s\n' "$XO_HOME" >> "$XO_FAKE_PROCEVENT_REARM_LOG"
    [ -z "${XO_FAKE_PROCEVENT_REARM_FAIL:-}" ] || exit 1
    ;;
  *) exit 2 ;;
esac
SH
  chmod +x "$home/bin/xo-procevent.sh"
  : > "$log"
}

test_xo_home_parameterization() {
  ...
  out=$(XO_HOME="$home_one" "$ROOT/bin/xo-project-mode.sh" app)
  [ "$out" = "local-only on" ] || fail "xo-project-mode did not read projects.md from XO_HOME"
  out=$(XO_HOME="$home_two" "$ROOT/bin/xo-project-mode.sh" app 2>/dev/null)
  [ "$out" = "no-mistakes off" ] || fail "xo-project-mode did not isolate missing registry by home"

  XO_HOME="$home_one" "$ROOT/bin/xo-brief.sh" task-a app --mode no-mistakes >/dev/null || fail "brief scaffold failed under XO_HOME"
  brief="$home_one/data/task-a/brief.md"
  [ -f "$brief" ] || fail "brief was not written under XO_HOME/data"
  grep -F ">> '$home_one/state/task-a.status'" "$brief" >/dev/null || fail "brief did not shell-quote XO_HOME state path"

  XO_HOME="$home_one" "$ROOT/bin/xo-brief.sh" task-b app --scout >/dev/null || fail "scout brief scaffold failed under XO_HOME"
  brief="$home_one/data/task-b/brief.md"
  grep -F ">> '$home_one/state/task-b.status'" "$brief" >/dev/null || fail "scout brief did not shell-quote XO_HOME state path"

  XO_HOME="$home_one" XO_SECONDMATE_CHARTER='ops domain' "$ROOT/bin/xo-brief.sh" task-c --secondmate app >/dev/null \
    || fail "secondmate brief scaffold failed under XO_HOME"
  brief="$home_one/data/task-c/brief.md"
  grep -F ">> '$home_one/state/task-c.status'" "$brief" >/dev/null || fail "secondmate brief did not shell-quote XO_HOME state path"

  printf 'project=x\n' > "$home_one/state/task-a.meta"
  XO_HOME="$home_one" XO_GUARD_GRACE=999999 "$ROOT/bin/xo-pr-check.sh" task-a https://github.com/example/repo/pull/1 >/dev/null 2>/dev/null \
    || fail "xo-pr-check failed under XO_HOME"
  [ -f "$home_one/state/task-a.check.sh" ] || fail "pr check was not written under XO_HOME/state"
  [ ! -e "$home_two/state/task-a.check.sh" ] || fail "pr check leaked into another home"
  pass "XO_HOME parameterizes data and state paths"
}
...
test_seed_allows_overlapping_clones_and_drops_owner() {
  ...
  # init, modes preserved - is asserted by xo-secondmate-lifecycle-e2e.
  local home design other
  home="$TMP_ROOT/overlap-main"
  design="$TMP_ROOT/overlap-design"
  other="$TMP_ROOT/overlap-other"
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_init_commit "$home/projects/beta"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/seed-overlap-alpha.git"
  xo_git_add_origin "$home/projects/beta" "$TMP_ROOT/remotes/seed-overlap-beta.git"
  cat > "$home/data/projects.md" <<EOF
- alpha [direct-PR] - alpha project (added 2026-06-22)
- beta [direct-PR] - beta project (added 2026-06-22)
EOF

  XO_HOME="$home" XO_SECONDMATE_CHARTER='feature design for alpha beta' \
    XO_SECONDMATE_SCOPE='feature design for alpha beta' \
    "$ROOT/bin/xo-home-seed.sh" design "$design" alpha beta >/dev/null \
  ...
  XO_HOME="$home" XO_SECONDMATE_CHARTER='issue triage for beta' \
    XO_SECONDMATE_SCOPE='issue triage for beta' \
    "$ROOT/bin/xo-home-seed.sh" other "$other" beta >/dev/null 2>&1 \
  ...
  XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" validate >/dev/null || fail "registry validation rejected overlapping clones"

  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" owner alpha >/dev/null 2>&1; then
...
test_home_seed_validate_rejects_unparseable_registry_entry() {
  ...
  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" validate >/dev/null 2>"$err"; then
...
test_home_seed_refuses_broken_registry_symlink() {
  ...
  if XO_HOME="$home" XO_SECONDMATE_CHARTER='design domain' \
    "$ROOT/bin/xo-home-seed.sh" design "$sub" alpha >/dev/null 2>"$err"; then
...
test_home_seed_uses_treehouse_acquired_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/dash-alpha.git"
  ...
  out=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TREEHOUSE_HOME="$acquired" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TREEHOUSE_LEASE_FILE="$lease" \
    XO_SECONDMATE_CHARTER='dash acquired scope' XO_SECONDMATE_SCOPE='dash acquired scope' \
    "$ROOT/bin/xo-home-seed.sh" dash - alpha) \
  ...
  [ -f "$acquired/.xo-secondmate-home" ] || fail "seed did not mark acquired home"
  [ "$(cat "$acquired/.xo-secondmate-home")" = dash ] || fail "seed wrote wrong acquired-home marker"
...
test_home_seed_returns_treehouse_acquired_home_on_assignment_failure() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/dash-fail-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  git clone --quiet "$ROOT" "$acquired"
  acquired_abs=$(cd "$acquired" && pwd -P)
  printf 'other\n' > "$acquired/.xo-secondmate-home"
  fakebin=$(make_fake_tmux "$TMP_ROOT/dash-fail-fake")
  log="$TMP_ROOT/dash-fail-fake/tmux.log"

  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TREEHOUSE_HOME="$acquired" XO_FAKE_TMUX_LOG="$log" \
    XO_SECONDMATE_CHARTER='dash acquired scope' XO_SECONDMATE_SCOPE='dash acquired scope' \
    "$ROOT/bin/xo-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
...
test_home_seed_warns_when_acquired_home_return_fails() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/dash-return-fail-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  git clone --quiet "$ROOT" "$acquired"
  acquired_abs=$(cd "$acquired" && pwd -P)
  printf 'other\n' > "$acquired/.xo-secondmate-home"
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TREEHOUSE_HOME="$acquired" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TREEHOUSE_LEASE_FILE="$lease" XO_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    XO_SECONDMATE_CHARTER='dash acquired scope' XO_SECONDMATE_SCOPE='dash acquired scope' \
    "$ROOT/bin/xo-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
...
test_home_seed_does_not_return_unsafe_acquired_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/dash-active-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  fakebin=$(make_fake_tmux "$TMP_ROOT/dash-active-fake")
  log="$TMP_ROOT/dash-active-fake/tmux.log"

  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TREEHOUSE_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/xo-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
    fail "seed accepted an acquired home matching the active xo home"
  fi
  grep -F 'secondmate home cannot be the active xo home' "$err" >/dev/null \
  ...
  : > "$log"
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TREEHOUSE_HOME="$descendant" XO_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/xo-home-seed.sh" dash - alpha >/dev/null 2>"$err"; then
    fail "seed accepted an acquired home inside the active xo home"
  fi
  grep -F 'secondmate home cannot be inside the active xo home' "$err" >/dev/null \
...
test_home_seed_rolls_back_failed_clone() {
  ...
  xo_git_init_commit "$home/projects/alpha"
  xo_git_init_commit "$home/projects/beta"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/rollback-alpha.git"
...
EOF
  ...
  if XO_HOME="$home" XO_SECONDMATE_CHARTER='rollback scope' XO_SECONDMATE_SCOPE='rollback scope' \
    "$ROOT/bin/xo-home-seed.sh" rollback "$subhome" alpha beta >/dev/null 2>"$err"; then
  ...
  [ ! -e "$subhome/.xo-secondmate-home" ] || fail "failed seed left a subhome marker"
...
test_home_seed_refuses_missing_filled_charter() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/missing-charter-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"

  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
...
test_home_seed_refuses_placeholder_charter() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/placeholder-charter-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  XO_HOME="$home" "$ROOT/bin/xo-brief.sh" design --secondmate alpha >/dev/null \
...
test_home_seed_refuses_empty_charter_fields() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/empty-charter-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"

  if XO_HOME="$home" XO_SECONDMATE_CHARTER='   ' "$ROOT/bin/xo-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
  ...
  XO_SECONDMATE_SCOPE='   ' scaffold_secondmate_charter "$home" design 'filled charter' alpha \
...
test_home_seed_no_projects_end_to_end() {
  # A domain whose subject is the xo repo itself needs no project clones:
  ...
  out=$(XO_HOME="$home" XO_SECONDMATE_CHARTER='xo self-development' \
    XO_SECONDMATE_SCOPE='xo repo work' \
    "$ROOT/bin/xo-home-seed.sh" fdev "$sub" --no-projects) \
  ...
  assert_grep '- fdev - xo self-development' "$home/data/secondmates.md" "project-less registry line missing"
  assert_grep 'scope: xo repo work' "$home/data/secondmates.md" "project-less registry scope missing"
  assert_grep 'projects: ;' "$home/data/secondmates.md" "project-less registry did not render an empty projects field"
  [ "$(cat "$sub/.xo-secondmate-home")" = fdev ] || fail "project-less seed did not mark the subhome"
  ...
  XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" validate >/dev/null || fail "registry validation failed after project-less seed"
  ...
  : > "$log"
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/no-projects-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" fdev "$sub" codex --secondmate >/dev/null 2>&1 \
...
test_secondmate_spawn_resolves_punctuated_registry_projects() {
  ...
  XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" validate >/dev/null \
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/punctuated-spawn-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" punctuated codex --secondmate >/dev/null 2>&1 \
...
EOF
    ...
    if [ "$case_name" = metadata-mismatch ]; then
      xo_write_secondmate_meta "$home/state/domain.meta" "$sub"
      meta_before="$TMP_ROOT/spawn-binding-$case_name.meta.before"
      cp "$home/state/domain.meta" "$meta_before"
      if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
        XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-binding-$case_name-fake/pane.txt" \
        "$ROOT/bin/xo-spawn.sh" domain codex --secondmate >/dev/null 2>"$err"; then
      ...
      if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
        XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-binding-$case_name-fake/pane.txt" \
        "$ROOT/bin/xo-spawn.sh" domain "$sub" codex --secondmate >/dev/null 2>"$err"; then
        fail "secondmate spawn accepted $case_name registry binding"
      fi
...
test_home_seed_refuses_projectful_reused_charter_for_projectless_home() {
  ...
  mark_xo_home "$reusable_sub"
  mark_xo_home "$stale_sub"

  scaffold_secondmate_charter "$home" reusable 'xo self-development' --no-projects \
  ...
  XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" reusable "$reusable_sub" --no-projects >/dev/null \
  ...
  scaffold_secondmate_charter "$home" stale 'xo self-development. None. This is a project-less domain.' alpha \
  ...
  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" stale "$stale_sub" --no-projects >/dev/null 2>"$err"; then
  ...
  grep -F 'xo-brief.sh stale --secondmate --no-projects' "$err" >/dev/null \
  ...
  assert_absent "$stale_sub/.xo-secondmate-home" "project-less charter refusal wrote a home marker"
...
test_home_seed_refuses_projectless_conversion_of_populated_home() {
  ...
  mark_xo_home "$sub"
  xo_git_init_commit "$sub/projects/existing-clone"
...
EOF
  ...
  if XO_HOME="$home" XO_SECONDMATE_CHARTER='xo self-development' \
    XO_SECONDMATE_SCOPE='xo repo work' \
    "$ROOT/bin/xo-home-seed.sh" fdev "$sub" --no-projects >/dev/null 2>"$err"; then
  ...
  assert_absent "$sub/.xo-secondmate-home" "project-less conversion refusal wrote a home marker"
...
test_home_seed_refuses_projectless_home_with_uninspectable_projects() {
  ...
  mark_xo_home "$sub"
  xo_git_init_commit "$sub/projects/hidden-clone"
  ...
  assert_absent "$sub/.xo-secondmate-home" "project-less inspection refusal wrote a home marker"
...
test_home_seed_refuses_projectless_home_with_symlinked_projects() {
  ...
  mark_xo_home "$sub"
  xo_git_init_commit "$target/hidden-clone"
  ...
  assert_absent "$sub/.xo-secondmate-home" "project-less symlink refusal wrote a home marker"
...
test_home_seed_refuses_projectless_home_with_non_directory_projects() {
  ...
  assert_absent "$sub/.xo-secondmate-home" "project-less non-directory refusal wrote a home marker"
...
test_home_seed_refuses_projectless_home_with_uninspectable_registry() {
  ...
  assert_absent "$sub/.xo-secondmate-home" "project-less registry inspection refusal wrote a home marker"
...
test_home_seed_refuses_missing_projects_without_signal() {
  ...
  if XO_HOME="$home" XO_SECONDMATE_CHARTER='some scope' \
    "$ROOT/bin/xo-home-seed.sh" fdev "$sub" >/dev/null 2>"$err"; then
  ...
  if XO_HOME="$home" XO_SECONDMATE_CHARTER='some scope' \
    "$ROOT/bin/xo-home-seed.sh" fdev "$sub" --no-projects alpha >/dev/null 2>"$err"; then
...
test_home_seed_refuses_registry_delimiter_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/delimiter-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"

  if XO_HOME="$home" XO_SECONDMATE_CHARTER='delimiter charter' "$ROOT/bin/xo-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
  ...
  [ ! -e "$subhome/.xo-secondmate-home" ] || fail "delimiter home seed wrote a marker"
...
test_home_seed_refuses_active_home_and_root() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/active-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondmate_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for active-home seed test"

  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$home" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home to reuse active XO_HOME"
  fi
  grep -F 'secondmate home cannot be the active xo home' "$err" >/dev/null \
    || fail "seed did not explain active XO_HOME rejection"

  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$active_descendant" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home inside active XO_HOME"
  fi
  grep -F 'secondmate home cannot be inside the active xo home' "$err" >/dev/null \
    || fail "seed did not explain active XO_HOME descendant rejection"
  [ ! -e "$home/nested" ] || fail "seed created a directory inside active XO_HOME before descendant rejection"

  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$active_ancestor" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home to contain active XO_HOME"
  fi
  grep -F 'secondmate home cannot be an ancestor of the active xo home' "$err" >/dev/null \
    || fail "seed did not explain active XO_HOME ancestor rejection"
  [ ! -f "$active_ancestor/.xo-secondmate-home" ] || fail "seed marked an ancestor of active XO_HOME"

  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$ROOT" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home to reuse XO_ROOT"
  fi
  grep -F 'secondmate home cannot be the xo repo' "$err" >/dev/null \
    || fail "seed did not explain XO_ROOT rejection"

  git clone --quiet "$ROOT" "$root_clone"
  if XO_HOME="$home" XO_ROOT_OVERRIDE="$root_clone" "$ROOT/bin/xo-home-seed.sh" design "$root_descendant" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home inside XO_ROOT"
  fi
  grep -F 'secondmate home cannot be inside the xo repo' "$err" >/dev/null \
    || fail "seed did not explain XO_ROOT descendant rejection"
  [ ! -e "$root_clone/tmp" ] || fail "seed created a directory inside XO_ROOT before descendant rejection"

  git clone --quiet "$ROOT" "$root_ancestor"
  git clone --quiet "$ROOT" "$root_inside"
  if XO_HOME="$home" XO_ROOT_OVERRIDE="$root_inside" "$ROOT/bin/xo-home-seed.sh" design "$root_ancestor" alpha >/dev/null 2>"$err"; then
    fail "seed allowed secondmate home to contain XO_ROOT"
  fi
  grep -F 'secondmate home cannot be an ancestor of the xo repo' "$err" >/dev/null \
    || fail "seed did not explain XO_ROOT ancestor rejection"
  [ ! -f "$root_ancestor/.xo-secondmate-home" ] || fail "seed marked an ancestor of XO_ROOT"
...
test_home_seed_refuses_home_marked_for_another_id() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/marked-alpha.git"
  ...
  [ "$(cat "$subhome/.xo-secondmate-home")" = "other" ] || fail "seed overwrote another secondmate marker"
...
test_home_seed_refuses_home_registered_to_another_id() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/registered-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  subhome_abs=$(cd "$subhome" && pwd -P)
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  ...
  [ ! -e "$subhome/.xo-secondmate-home" ] || fail "seed wrote a marker before rejecting a registered home"
...
test_home_seed_refuses_reassigning_existing_id_to_different_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/reassign-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"

  XO_HOME="$home" XO_SECONDMATE_CHARTER='design domain' XO_SECONDMATE_SCOPE='design domain' \
    "$ROOT/bin/xo-home-seed.sh" design "$first" alpha >/dev/null \
    || fail "initial seed failed for reassigning-id test"
  first_abs=$(cd "$first" && pwd -P)

  if XO_HOME="$home" XO_SECONDMATE_CHARTER='design domain' XO_SECONDMATE_SCOPE='design domain' \
    "$ROOT/bin/xo-home-seed.sh" design "$second" alpha >/dev/null 2>"$err"; then
  ...
  [ "$(cat "$first/.xo-secondmate-home")" = design ] || fail "failed id reassignment changed the original marker"
...
test_home_seed_refuses_home_overlapping_registered_home() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/overlap-alpha.git"
...
EOF
  ...
  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$nested" alpha >/dev/null 2>"$err"; then
  ...
  if XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$parent" alpha >/dev/null 2>"$err"; then
    fail "seed accepted a home containing a registered secondmate home"
  fi
  grep -F 'overlaps registered secondmate home' "$err" >/dev/null \
    || fail "seed did not explain registered descendant overlap"
  [ ! -f "$parent/.xo-secondmate-home" ] || fail "seed marked a home containing a registered home"
...
test_home_seed_refuses_existing_remote_backed_project_with_wrong_origin() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/wrong-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  subhome_abs=$(cd "$subhome" && pwd -P)
...
test_home_seed_resolves_relative_source_origins() {
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$subhome" alpha)
  subhome_abs=$(cd "$subhome" && pwd -P)
  ...
  XO_HOME="$home" "$ROOT/bin/xo-home-seed.sh" design "$subhome" alpha >/dev/null \
...
test_home_seed_skips_initialized_existing_no_mistakes_projects() {
  ...
  xo_git_init_commit "$home/projects/alpha"
  xo_git_init_commit "$home/projects/beta"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/existing-alpha.git"
  xo_git_add_origin "$home/projects/beta" "$TMP_ROOT/remotes/existing-beta.git"
  git clone --quiet "$ROOT" "$subhome"
  mkdir -p "$subhome/projects"
  ...
  if PATH="$fakebin:$PATH" XO_FAKE_NO_MISTAKES_LOG="$log" XO_FAKE_NO_MISTAKES_FAIL_PROJECT=beta \
    XO_HOME="$home" XO_SECONDMATE_CHARTER='existing init rollback scope' XO_SECONDMATE_SCOPE='existing init rollback scope' \
    "$ROOT/bin/xo-home-seed.sh" design "$subhome" alpha beta >/dev/null 2>"$err"; then
...
test_home_seed_refuses_uninitialized_existing_no_mistakes_project() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/uninitialized-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  mkdir -p "$subhome/projects"
  origin=$(git -C "$home/projects/alpha" remote get-url origin)
  git clone --quiet "$origin" "$subhome/projects/alpha"
  ...
  if PATH="$fakebin:$PATH" XO_FAKE_NO_MISTAKES_LOG="$log" \
    XO_HOME="$home" XO_SECONDMATE_CHARTER='existing uninitialized scope' \
    "$ROOT/bin/xo-home-seed.sh" design "$subhome" alpha >/dev/null 2>"$err"; then
...
test_home_seed_refuses_project_destinations_outside_subhome() {
  ...
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/symlink-alpha.git"
  git clone --quiet "$ROOT" "$subhome"
  ...
  [ ! -f "$subhome/.xo-secondmate-home" ] || fail "seed marked subhome after unsafe project destination rejection"
...
test_home_seed_refuses_operational_dirs_outside_subhome() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/symlink-opdir-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  ...
  for opdir in data state config; do
    ...
    [ ! -f "$subhome/.xo-secondmate-home" ] || fail "seed marked subhome after unsafe $opdir directory rejection"
  done
...
test_home_seed_refuses_unsafe_leaf_files() {
  ...
  mkdir -p "$home/projects" "$home/data" "$home/state"
  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$TMP_ROOT/remotes/symlink-leaf-alpha.git"
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_secondmate_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink leaf seed test"

  for leaf in data/projects.md data/charter.md .xo-secondmate-home .xo-secondmate-parent; do
    ...
    expected=outside
    if [ "$leaf" = ".xo-secondmate-home" ]; then
      expected=design
    fi
    printf '%s\n' "$expected" > "$sink"
    ...
    [ ! -f "$subhome/.xo-secondmate-home" ] || [ "$leaf" = ".xo-secondmate-home" ] || fail "seed marked subhome after symlinked leaf refusal"
  done
  for leaf in data/projects.md data/charter.md .xo-secondmate-home .xo-secondmate-parent; do
...
test_home_seed_preserves_existing_parent_binding() {
  ...
  XO_HOME="$parent_a" XO_SECONDMATE_CHARTER='Durable parent reseed charter.' \
    XO_SECONDMATE_SCOPE='durable parent reseed scope' \
    "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects >/dev/null \
  ...
  if XO_HOME="$parent_b" XO_SECONDMATE_CHARTER='Replacement parent charter.' \
    XO_SECONDMATE_SCOPE='replacement parent scope' \
    "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects > /dev/null 2>"$err"; then
  ...
  out=$(XO_HOME="$parent_a" "$ROOT/bin/xo-home-seed.sh" mate "$child" --no-projects) \
    || fail "matching-parent reseed failed"
  printf '%s\n' "$out" | grep -F "home=$child_abs" >/dev/null \
    || fail "matching-parent reseed did not report success"
  cmp -s "$before/.xo-secondmate-parent" "$child/.xo-secondmate-parent" \
...
test_secondmate_spawn_requires_seeded_matching_home() {
  ...
  cat > "$fakeroot/bin/xo-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$fakeroot/bin/xo-guard.sh"
  mkdir -p "$ancestor_active_home/data" "$ancestor_active_home/state" "$active_ancestor/data" "$root_ancestor/data" "$root_inside/bin"
  cat > "$root_inside/bin/xo-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$root_inside/bin/xo-guard.sh"
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$subhome" codex --secondmate >/dev/null 2>"$err"; then
  ...
  printf 'other\n' > "$wronghome/.xo-secondmate-home"
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$wronghome" codex --secondmate >/dev/null 2>"$err"; then
  ...
  printf 'domain\n' > "$marker_only/.xo-secondmate-home"
  printf 'charter\n' > "$marker_only/data/charter.md"
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$marker_only" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a marked home missing AGENTS.md"
  fi
  grep -F 'not an xo home (missing AGENTS.md)' "$err" >/dev/null || fail "spawn did not explain missing AGENTS.md"
  ...
  grep -F 'not an xo home (missing bin/)' "$err" >/dev/null || fail "spawn did not explain missing bin"

  printf 'domain\n' > "$home/.xo-secondmate-home"
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$home" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted the active home"
  fi
  grep -F 'secondmate home cannot be the active xo home' "$err" >/dev/null || fail "spawn did not reject active home"

  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$ROOT" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted the xo repo root"
  fi
  grep -F 'secondmate home cannot be the xo repo' "$err" >/dev/null || fail "spawn did not reject xo repo root"

  printf 'domain\n' > "$active_descendant/.xo-secondmate-home"
  printf 'charter\n' > "$active_descendant/data/charter.md"
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$active_descendant" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a home inside the active xo home"
  fi
  grep -F 'secondmate home cannot be inside the active xo home' "$err" >/dev/null || fail "spawn did not reject active home descendant"

  printf 'domain\n' > "$active_ancestor/.xo-secondmate-home"
  printf 'charter\n' > "$active_ancestor/data/charter.md"
  if PATH="$fakebin:$PATH" XO_HOME="$ancestor_active_home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$active_ancestor" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a home containing the active xo home"
  fi
  grep -F 'secondmate home cannot be an ancestor of the active xo home' "$err" >/dev/null || fail "spawn did not reject active home ancestor"

  printf 'domain\n' > "$root_descendant/.xo-secondmate-home"
  printf 'charter\n' > "$root_descendant/data/charter.md"
  if PATH="$fakebin:$PATH" XO_ROOT_OVERRIDE="$fakeroot" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$root_descendant" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a home inside the xo repo"
  fi
  grep -F 'secondmate home cannot be inside the xo repo' "$err" >/dev/null || fail "spawn did not reject repo root descendant"

  printf 'domain\n' > "$root_ancestor/.xo-secondmate-home"
  printf 'charter\n' > "$root_ancestor/data/charter.md"
  if PATH="$fakebin:$PATH" XO_ROOT_OVERRIDE="$root_inside" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-validate-fake/pane.txt" \
    "$ROOT/bin/xo-spawn.sh" domain "$root_ancestor" codex --secondmate >/dev/null 2>"$err"; then
    fail "secondmate spawn accepted a home containing the xo repo"
  fi
  grep -F 'secondmate home cannot be an ancestor of the xo repo' "$err" >/dev/null || fail "spawn did not reject repo ancestor"
...
test_secondmate_spawn_refuses_operational_dirs_outside_subhome() {
  ...
  for opdir in data state config projects; do
    ...
    : > "$log"
    if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/spawn-opdir-fake/pane.txt" \
      "$ROOT/bin/xo-spawn.sh" domain "$subhome" codex --secondmate >/dev/null 2>"$err"; then
...
test_xo_send_refuses_bare_window_without_home_meta() {
  # The happy path (a bare xo-<id> resolves the window recorded in THIS home's
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_WINDOW="other-session:xo-missing" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/send-fake/pane.txt" \
    "$ROOT/bin/xo-send.sh" xo-missing 'wrong home' >/dev/null 2>"$err"; then
    fail "xo-send sent to a bare xo window without home metadata"
  fi
  grep -F "no metadata for xo-missing in $home/state" "$err" >/dev/null \
    || fail "xo-send did not explain missing home metadata"
  grep -F 'send-keys -t other-session:xo-missing' "$log" >/dev/null \
    && fail "xo-send fell back to a foreign same-name window"
  pass "xo-send refuses a bare xo window with no metadata in this home"
}
...
EOF
  ...
  printf 'domain\n' > "$lease"
  PATH="$fakebin:$PATH" XO_ROOT_OVERRIDE="$fmroot" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-fake/pane.txt" \
    XO_FAKE_TREEHOUSE_LEASE_FILE="$lease" \
    "$ROOT/bin/xo-teardown.sh" domain >/dev/null 2>/dev/null \
...
EOF
    ...
    if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
      XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-binding-$case_name-fake/pane.txt" \
      "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
test_secondmate_teardown_sweeps_process_events_before_removal() {
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/procevent-teardown-fake/pane.txt" \
    XO_FAKE_PROCEVENT_SWEEP_LOG="$sweep_log" \
    "$ROOT/bin/xo-teardown.sh" domain >/dev/null 2>/dev/null \
...
test_secondmate_teardown_refuses_process_events_without_sweep_script() {
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_PROCEVENT_CLAIM_ROOT="$claim_root" \
      XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/procevent-refusal-fake/pane.txt" \
      "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed process-event state without a sweep-capable child script"
  fi
  grep -F 'no sweep-capable bin/xo-procevent.sh' "$err" >/dev/null || fail "missing sweep capability refusal was not explained"
...
SH
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
      XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/procevent-later-refusal-fake/pane.txt" \
      XO_FAKE_PROCEVENT_SWEEP_LOG="$sweep_log" \
      "$ROOT/bin/xo-teardown.sh" domain >/dev/null 2>"$err"; then
...
test_secondmate_force_teardown_sweeps_nested_homes() {
  ...
  mark_xo_home "$subhome"
  mark_xo_home "$childhome"
  printf 'domain\n' > "$subhome/.xo-secondmate-home"
  printf 'nested\n' > "$childhome/.xo-secondmate-home"
  ...
  xo_write_secondmate_meta "$home/state/domain.meta" "$subhome"
  xo_write_secondmate_meta "$subhome/state/nested.meta" "$childhome"
  cat > "$home/data/secondmates.md" <<EOF
...
EOF
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/procevent-force-fake/pane.txt" \
    XO_FAKE_PROCEVENT_SWEEP_LOG="$sweep_log" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>/dev/null \
...
EOF
  ...
  set +e
  PATH="$fakebin:$PATH" XO_ROOT_OVERRIDE="$fmroot" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/procevent-nested-fail-fake/pane.txt" \
    XO_FAKE_PROCEVENT_SWEEP_LOG="$sweep_log" XO_FAKE_PROCEVENT_REARM_LOG="$rearm_log" \
    XO_FAKE_TREEHOUSE_RETURN_FAIL=1 XO_FAKE_PROCEVENT_REARM_FAIL=1 \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"
  rc=$?
  set -e
  ...
  backup=$(find "$TMP_ROOT" -maxdepth 1 -type d -name '.xo-procevent-restore.*' \
...
EOF
  ...
  set +e
  PATH="$fakebin:$PATH" XO_ROOT_OVERRIDE="$fmroot" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-return-fail-fake/pane.txt" \
    XO_FAKE_PROCEVENT_SWEEP_LOG="$sweep_log" XO_FAKE_PROCEVENT_REARM_LOG="$rearm_log" \
    XO_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    "$ROOT/bin/xo-teardown.sh" domain >/dev/null 2>"$err"
  rc=$?
  set -e
...
EOF
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/plain-clone-teardown-fake/pane.txt" \
    XO_FAKE_TREEHOUSE_RETURN_FAIL=1 \
    "$ROOT/bin/xo-teardown.sh" domain >/dev/null 2>/dev/null \
...
test_secondmate_force_teardown_discards_child_work() {
  ...
  mkdir -p "$home/state" "$home/data" "$subhome/state"
  xo_git_worktree "$childproj" "$childwt" force-child
  printf 'domain\n' > "$subhome/.xo-secondmate-home"
  cat > "$home/state/domain.meta" <<EOF
window=xo:xo-domain
worktree=$subhome
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/force-teardown-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain >/dev/null 2>&1; then
    fail "teardown allowed a secondmate with in-flight child work"
  fi
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/force-teardown-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>/dev/null \
  ...
  grep -F 'kill-window -t =xo:=xo-child' "$log" >/dev/null || fail "force teardown did not kill child window"
  grep -F 'kill-window -t =xo:=xo-domain' "$log" >/dev/null || fail "force teardown did not kill parent window"
...
test_secondmate_force_teardown_refuses_duplicated_child_slot() {
  ...
  xo_git_worktree "$childproj" "$childwt" duplicate-child
...
EOF
  ...
  set +e
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/force-duplicate-slot-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"
  rc=$?
  set -e
...
test_secondmate_force_teardown_preserves_child_on_unproven_lock() {
  ...
  mkdir -p "$home/state" "$home/data" "$subhome/state" "$(dirname "$childwt")"
  xo_git_worktree "$childproj" "$childwt" force-child-lock
  printf '{"worktrees":[{"name":"1","path":"%s"}]}\n' "$childwt" \
...
SH
  ...
  set +e
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/force-lock-child-fake/pane.txt" \
    XO_STALE_WORKTREE_LOCK_RETRY_WAIT_SECS=0 XO_STALE_WORKTREE_LOCK_AGE_SECS=1 \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"
  rc=$?
  set -e
...
EOF
    ...
    PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/symlink-inside-teardown-fake-$opdir/pane.txt" \
      "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err" \
    ...
    grep -F 'kill-window -t =xo:=xo-domain' "$log" >/dev/null || fail "force teardown did not kill parent window for inside $opdir symlink"
  done
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/symlink-state-teardown-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
test_secondmate_teardown_path_boundary_matrix() {
  ...
  # an ancestor of the active xo home, inside the active xo home,
  # or inside the xo repo. One row per hazard, one shared assertion block.
  local row base home subhome fmroot fakebin log err expect tid
  while IFS='|' read -r row expect; do
    [ -n "$row" ] || continue
    base="$TMP_ROOT/td-pb-$row"
    fmroot="$ROOT"   # real xo repo unless a row overrides it
    tid=domain
    case "$row" in
      unmarked)
        home="$base/main"; subhome="$base/sub"
        mkdir -p "$home/state" "$home/data" "$subhome/state"
        # No .xo-secondmate-home marker on purpose.
        ;;
      ancestor)
        # The home being torn down is an ANCESTOR of the active xo home.
      ...
      repo-descendant)
        ...
        cat > "$fmroot/bin/xo-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
        chmod +x "$fmroot/bin/xo-guard.sh"
        printf 'repo-domain\n' > "$subhome/.xo-secondmate-home"
        ;;
    esac
    xo_write_secondmate_meta "$home/state/$tid.meta" "$subhome"
    ...
    if PATH="$fakebin:$PATH" XO_ROOT_OVERRIDE="$fmroot" XO_HOME="$home" \
      XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$base/fake/pane.txt" \
      "$ROOT/bin/xo-teardown.sh" "$tid" >/dev/null 2>"$err"; then
...
ancestor|ancestor of the active xo home
active-descendant|inside the active xo home
repo-descendant|inside the xo repo
ROWS
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/nested-teardown-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain >/dev/null 2>"$err"; then
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-registry-teardown-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain >/dev/null 2>"$err"; then
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/prevalidate-teardown-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
seed_task_set_lock_home() {  # <tag> -> echoes "<home>|<subhome>"
  ...
  xo_git_worktree "$childproj" "$childwt" "child-$tag"
  printf '%s\n' domain > "$subhome/.xo-secondmate-home"
  cat > "$home/state/domain.meta" <<EOF
window=xo:xo-domain
worktree=$subhome
project=$subhome
...
task_set_lock_path() {  # <state-dir>
  ...
  ( . "$ROOT/bin/xo-wake-lib.sh"; xo_task_set_lock_path "$state" )
}

# The holder must stay ALIVE: xo_lock_try_acquire reclaims a lock whose owning
...
hold_task_set_lock() {  # <state-dir> -> echoes "<holder-pid> <lock-path>"
  ...
  (
    ...
    xo_lock_try_acquire "$lock" || exit 1
    sleep 30
  ) >/dev/null 2>&1 &
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/taskset-state-file-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/taskset-state-symlink-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
$rec
EOF
  claim_root="$TMP_ROOT/taskset-state-absent-xdg/xo/procevent-claims"
  ...
  cat > "$subhome/bin/xo-procevent.sh" <<'SH'
#!/usr/bin/env bash
set -u
if [ "${1:-}" = sweep-home ] && [ "${2:-}" = --preflight ]; then
  : > "$XO_TASK_SET_TEST_READY"
  while [ ! -e "$XO_TASK_SET_TEST_RELEASE" ]; do sleep 0.05; done
  exit 1
fi
exit 1
SH
  chmod +x "$subhome/bin/xo-procevent.sh"
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/taskset-state-absent-fake/pane.txt" \
    XDG_STATE_HOME="$TMP_ROOT/taskset-state-absent-xdg" \
    XO_TASK_SET_TEST_READY="$ready" XO_TASK_SET_TEST_RELEASE="$release" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err" &
  # shellcheck disable=SC2031 # The background PID is captured immediately in this shell.
  pid=$!
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/taskset-teardown-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
EOF
  ...
  holder=${held%% *}
  lock=${held#* }
  if XO_HOME="$subhome" XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" newtask "$subhome/projects/alpha" --scout >/dev/null 2>"$err"; then
    kill "$holder" 2>/dev/null || true
...
test_fresh_remote_secondmate_spawn_refuses_while_task_set_is_owned() {
  ...
  holder=${held%% *}
  lock=${held#* }
  if XO_HOME="$home" XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" remote-new --secondmate >/dev/null 2>"$err"; then
    kill "$holder" 2>/dev/null || true
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-active-descendant-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed a child worktree inside active XO_HOME"
  fi
  ...
  grep -F 'inside the active xo home' "$err" >/dev/null || fail "force teardown did not explain active home descendant rejection"
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_ROOT_OVERRIDE="$fakeroot" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/child-repo-descendant-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
    fail "force teardown removed a child worktree inside XO_ROOT"
  fi
  ...
  grep -F 'inside the xo repo' "$err" >/dev/null || fail "force teardown did not explain repo descendant rejection"
  pass "force teardown refuses child worktrees inside the xo repo"
}
...
EOF
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_LOG="$log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/unregistered-child-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" domain --force >/dev/null 2>"$err"; then
...
EOF
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_LOG="$TMP_ROOT/watch-fake/tmux.log" XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/watch-fake/pane.txt" \
    XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$ROOT/bin/xo-watch.sh" > "$out" &
  # shellcheck disable=SC2031 # The background PID is captured immediately in this shell.
  pid=$!
...
test_secondmate_charter_brief_is_idle_by_default() {
  ...
  grep -F 'go idle and wait silently for the main xo' "$brief" >/dev/null \
    || fail "charter brief does not tell the secondmate to go idle and wait for routed work"
  grep -F 'Act only on tasks the main xo routes to you' "$brief" >/dev/null \
  ...
  grep -F 'run normal xo bootstrap and recovery' "$brief" >/dev/null \
...
EOF
  ...
  if XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design bug-z no-such-key >/dev/null 2>&1; then
  ...
  before=$(cat "$home/data/backlog.md")
  if XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design live-task >/dev/null 2>&1; then
  ...
  if XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" ghost bug-z >/dev/null 2>&1; then
    fail "handoff accepted an unregistered secondmate id"
  fi
  pass "xo-backlog-handoff aborts atomically on unmatched, in-flight, and unregistered targets"
}
...
test_backlog_handoff_refuses_done_items_and_non_secondmate_homes() {
  ...
  if out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" archive shipped-task 2>&1); then
  ...
  if XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" proj-sm shipped-task >/dev/null 2>&1; then
...
EOF
  if XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" marker-sm marker-task >/dev/null 2>&1; then
...
EOF
  if XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" symlink-sm symlink-task >/dev/null 2>&1; then
  ...
  pass "xo-backlog-handoff refuses Done items under whitespace section headings and unsafe homes"
}
