...
VIEW="$ROOT/bin/xo-fleet-view.sh"
TMP_ROOT=$(xo_test_tmproot xo-fleet-snapshot)
...
EOF
  ...
  # (bin/xo-busy-lib.sh), which is what the snapshot's current-state read
  # consults; rendered pane text is no longer a state source.
  fixture_gen=$("$ROOT/bin/xo-busy-event.sh" arm "$home/state" ship-task)
  "$ROOT/bin/xo-busy-event.sh" apply "$home/state" ship-task busy --gen "$fixture_gen" \
    --source claude-hook --event user-prompt-submit
  xo_write_meta "$home/state/scout-task.meta" \
    "window=xo:xo-scout-task" \
  ...
  xo_write_meta "$home/state/secondmate-task.meta" \
    "window=xo:xo-secondmate-task" \
  ...
  xo_write_meta "$home/state/cmux-task.meta" \
    "backend=cmux" \
...
test_empty_fleet_json() {
  ...
  out=$(XO_HOME="$home" "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
...
test_fixture_snapshot_json() {
  ...
  out=$(PATH="$fakebin:$PATH" XO_HOME="$home" "$SNAPSHOT" --json)
  ...
  printf '%s' "$out" | jq -e '
    ...
    | .secondmate_projects == ["alpha","beta","gamma"]
      ...
      and (.actions.watch | contains("do not routinely xo-peek"))
...
EOF
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_DATA_OVERRIDE="$home/data"     XO_SNAPSHOT_NOW=2026-07-25T00:00:00Z "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
...
## Done
EOF
  xo_write_meta "$home/state/worker.meta" \
    "window=xo:xo-worker" "worktree=$home/projects/worker" "project=alpha" \
...
test_event_hints_follow_reconciled_current_state() {
  ...
  xo_write_meta "$home/state/active-decision.meta" \
    "window=xo:xo-active-decision" \
  ...
  xo_write_meta "$home/state/active-blocked.meta" \
    "window=xo:xo-active-blocked" \
  ...
  xo_write_meta "$home/state/stale-decision.meta" \
    "window=xo:xo-stale-decision-ship-task" \
    "worktree=$home/projects/stale-decision" \
    "project=alpha" \
    "harness=claude" \
    "kind=ship" \
    "mode=ship"
  hint_gen=$("$ROOT/bin/xo-busy-event.sh" arm "$home/state" stale-decision)
  "$ROOT/bin/xo-busy-event.sh" apply "$home/state" stale-decision busy --gen "$hint_gen" \
    --source claude-hook --event user-prompt-submit
  printf 'needs-decision: already answered\n' > "$home/state/stale-decision.status"
  xo_write_meta "$home/state/stale-blocked.meta" \
    "window=xo:xo-stale-blocked-ship-task" \
    "worktree=$home/projects/stale-blocked" \
    "project=alpha" \
    "harness=claude" \
    "kind=ship" \
    "mode=ship"
  hint_gen=$("$ROOT/bin/xo-busy-event.sh" arm "$home/state" stale-blocked)
  "$ROOT/bin/xo-busy-event.sh" apply "$home/state" stale-blocked busy --gen "$hint_gen" \
    --source claude-hook --event user-prompt-submit
...
EOF
  ...
  xo_write_meta "$home/state/bold-task.meta" \
    "window=xo:xo-bold-task" \
  ...
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_DATA_OVERRIDE="$data" XO_PROJECTS_OVERRIDE="$projects" \
    XO_SNAPSHOT_NOW=2026-07-14T00:00:00Z "$SNAPSHOT" --json)
  ...
  view=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_DATA_OVERRIDE="$data" XO_PROJECTS_OVERRIDE="$projects" "$VIEW")
...
EOF
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_DATA_OVERRIDE="$home/data" \
    XO_SNAPSHOT_NOW=2026-07-25T00:00:00Z "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
  ...
  out=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_DATA_OVERRIDE="$home/data" \
    XO_SNAPSHOT_NOW=2026-07-25T00:00:00Z XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS=30 "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
    .backlog.records[] | select(.id == "aged-call")
    | .hold_bucket == "live" and .hold_age_days == 24
  ' >/dev/null || fail "raising the age threshold must leave a 24-day hold unaged: $out"
  out=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_DATA_OVERRIDE="$home/data" \
    XO_SNAPSHOT_NOW=2026-07-25T00:00:00Z XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS=5 "$SNAPSHOT" --json)
  printf '%s' "$out" | jq -e '
...
test_view_renders_snapshot() {
  ...
  write_fixture "$home"
  fakebin=$(make_fakebin "$home")
  view=$(PATH="$fakebin:$PATH" XO_HOME="$home" "$VIEW")
  ...
  assert_contains "$view" "bin/xo-send.sh xo-secondmate-task" \
  ...
  assert_not_contains "$view" "xo-peek.sh xo-secondmate-task" \
    "view must not tell xo to routinely peek secondmates"
...
test_view_renders_dead_secondmate_agent_status() {
  ...
  xo_write_meta "$home/state/dead-secondmate.meta" \
    "window=xo:xo-dead-secondmate" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
    "mode=secondmate" \
...
# durable keyed fold (xo-classify-lib.sh) keeps it open until an explicit resolution.
test_open_decision_survives_later_unrelated_event() {
  local home fakebin out
  home=$(make_home masking)
  mkdir -p "$home/secondmate-home"
  xo_write_meta "$home/state/masked-decision.meta" \
    "window=xo:xo-masked-decision" \
    "worktree=$home/secondmate-home" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
...
test_secondmate_open_decision_survives_live_endpoint() {
  ...
  mkdir -p "$home/secondmate-home"
  xo_write_meta "$home/state/active-secondmate.meta" \
    "window=xo:xo-active-secondmate" \
    "worktree=$home/secondmate-home" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
...
test_open_decision_transfers_to_captain_hold() {
  ...
  mkdir -p "$home/secondmate-home"
  xo_write_meta "$home/state/transferred-decision.meta" \
    "window=xo:xo-transferred-decision" \
    "worktree=$home/secondmate-home" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
...
test_open_decision_clears_on_keyed_resolution() {
  ...
  mkdir -p "$home/secondmate-home"
  xo_write_meta "$home/state/resolved-decision.meta" \
    "window=xo:xo-resolved-decision" \
    "worktree=$home/secondmate-home" \
    "project=$home/secondmate-home" \
    "harness=codex" \
    "kind=secondmate" \
...
test_parked_scout_decision_stays_pending() {
  ...
  xo_write_meta "$home/state/parked-scout.meta" \
    "window=xo:xo-parked-scout" \
...
EOF
  ...
  fakebin=$(make_fakebin "$home")
  out=$(PATH="$fakebin:$PATH" XO_HOME="$home" "$SNAPSHOT" --secondmate-home-summary)
  printf '%s' "$out" | jq -e '
...
EOF
  ...
  xo_write_meta "$home/state/unowned-ship.meta" \
    "window=xo:xo-unowned-ship" \
...
## Done
EOF
  xo_write_meta "$home/state/terminal-ship.meta" \
    "window=xo:xo-terminal-ship" \
