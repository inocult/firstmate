#!/usr/bin/env bash
# Live guard for the real, installed Rovo CLI (bin/xo-test-run.sh's
...
# harness-dependent facts bin/xo-busy-lib.sh and bin/xo-control-lib.sh encode for
...
# send-keys shape). The launch-then-send shape is what xo-spawn.sh actually
...
xo_live_gate opt-in XO_ROVO_SIGNALS_LIVE python3

[ -x "$ROVO_BIN" ] || fail "XO_ROVO_SIGNALS_LIVE=1 but no real rovo executable is installed"
...
LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-rovo-signals.XXXXXX") || fail "could not create the isolated Rovo lab"
cleanup() { rm -rf -- "$LAB"; }
trap cleanup EXIT
...
printf '%s\n' "Rovo is thinking..." | xo_busy_rovo_tail_busy \
  || fail "xo_busy_rovo_tail_busy did not classify the real busy line as busy"
...
# xo-spawn.sh's rovo_config_override_flag grant, and that the same access is
# blocked by default (AGENTS.md xo-rovo-external-paths finding). rovo's
