#!/usr/bin/env bash
# tests/xo-wake-drain-unread-status.test.sh - drain must surface every still-
...
TMP_ROOT=$(xo_test_tmproot xo-wake-drain-unread-status-tests)
...
prime_cursor() {  # <state> <status-file>
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>/dev/null \
...
test_incident_note_answer_buried_under_routine_note_surfaces_both() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on the incident shape"
...
test_already_presented_notes_are_not_replayed() {
  ...
  printf 'note: captain said use REST not RPC\n' >> "$status"
  printf 'note: re-read acknowledgement\n' >> "$status"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "first drain of unread notes failed"
  grep -F 'captain said use REST not RPC' "$out" >/dev/null \
    || fail "setup error: first drain did not surface the answer note"

  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "second drain after presentation failed"
...
test_brand_new_note_after_presentation_is_surfaced() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain of the first note failed"
  grep -F 'task3 note: first answer' "$out" >/dev/null \
    || fail "setup error: first note was not presented"

  printf 'note: follow-up after ack\n' >> "$status"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain of the brand-new note failed"
...
test_signal_annotation_surfaces_every_unread_note_not_only_the_newest() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" \
...
test_pending_reply_resolution_surfaces_once() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a pending-reply resolution"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "second drain after pending-reply presentation failed"
...
test_unread_output_over_cap_remains_recoverable() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed for unread output over the former cap"
...
test_retired_task_id_starts_new_status_unread() {
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    . "$1/bin/xo-wake-lib.sh"
    . "$1/bin/xo-classify-lib.sh"
    _xo_open_decisions_file_ident "$STATE/reused.status" > "$2"
  ...
  offset=$(bash -c '
    . "$1/bin/xo-wake-lib.sh"
    . "$1/bin/xo-classify-lib.sh"
    xo_wake_signal_seen_size "$2" "$2/reused.status"
  ' _ "$ROOT" "$state")
...
test_weak_identity_still_presents_and_advances() {
  ...
  XO_STATUS_IDENTITY_READER="$reader" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" \
  ...
  XO_STATUS_IDENTITY_READER="$reader" XO_STATE_OVERRIDE="$state" "$DRAIN" > "$second" \
...
test_open_decisions_fold_is_unchanged() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a buried needs-decision plus a note"
  ...
  grep -F "close one by answering it: bin/xo-send.sh <task> --resolve-key <key>" "$out" >/dev/null \
    || fail "OPEN DECISIONS lost its answerer-closes hint"

  printf 'resolved [key=api-shape]: went with REST\n' >> "$state/task6.status"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed after resolving the keyed decision"
  if grep -F 'OPEN DECISIONS' "$out" >/dev/null; then
...
test_routine_working_and_covered_done_stay_silent_on_the_empty_queue() {
  ...
  XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-branch-outcome.sh" append \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed with routine working and covered done lines"
