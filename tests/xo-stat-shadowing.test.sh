#!/usr/bin/env bash
# tests/xo-stat-shadowing.test.sh - verify Darwin BSD-stat helpers ignore a GNU
...
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xo-stat-shadowing.XXXXXX") || exit 1
...
. "$ROOT/bin/xo-startup-memory-budget-lib.sh"

TESTFILE="$TMP_ROOT/testfile"
printf 'hello world\n' > "$TESTFILE"

# 1. xo_sup_stat_mtime from bin/xo-supervision-lib.sh
RESULT_MTIME=$(xo_sup_stat_mtime "$TESTFILE") || true
EXPECTED_MTIME=$(/usr/bin/stat -f %m "$TESTFILE" 2>/dev/null)
if [ -z "$RESULT_MTIME" ] || [ "$RESULT_MTIME" != "$EXPECTED_MTIME" ]; then
  PATH="$ORIGINAL_PATH"
  fail "xo_sup_stat_mtime: expected $EXPECTED_MTIME, got '$RESULT_MTIME'"
fi
pass "xo_sup_stat_mtime returns correct epoch mtime under GNU stat shadowing"

# 2. xo_startup_memory_budget_link_count from bin/xo-startup-memory-budget-lib.sh
RESULT_LINKS=$(xo_startup_memory_budget_link_count "$TESTFILE") || true
EXPECTED_LINKS=$(/usr/bin/stat -f %l "$TESTFILE" 2>/dev/null)
if [ -z "$RESULT_LINKS" ] || [ "$RESULT_LINKS" != "$EXPECTED_LINKS" ]; then
  PATH="$ORIGINAL_PATH"
  fail "xo_startup_memory_budget_link_count: expected $EXPECTED_LINKS, got '$RESULT_LINKS'"
fi
pass "xo_startup_memory_budget_link_count returns correct link count under GNU stat shadowing"

# 3. _xo_status_file_size from bin/xo-classify-lib.sh
...
if [ -z "$RESULT_SIZE" ]; then
  PATH="$ORIGINAL_PATH"
  fail "_xo_status_file_size: could not get size via /usr/bin/stat"
fi
# Verify the helper itself works by checking that calling it with /usr/bin/stat prefix matches
. "$ROOT/bin/xo-classify-lib.sh"
HELPER_SIZE=$(_xo_status_file_size "$TESTFILE") || true
if [ -z "$HELPER_SIZE" ] || [ "$HELPER_SIZE" != "$RESULT_SIZE" ]; then
  PATH="$ORIGINAL_PATH"
  fail "_xo_status_file_size: expected $RESULT_SIZE, got '$HELPER_SIZE'"
fi
pass "_xo_status_file_size returns correct byte size under GNU stat shadowing"

# 4. stat_mtime from bin/xo-watch.sh
# xo-watch.sh runs a top-level `mkdir -p` on its state dir when sourced; pin it
# to the temp root via XO_STATE_OVERRIDE so no artifact escapes into the repo's
# git-ignored state/ directory.
export XO_STATE_OVERRIDE="$TMP_ROOT/state"
...
if [ -z "$RESULT_WATCH_MTIME" ] || [ "$RESULT_WATCH_MTIME" != "$EXPECTED_MTIME" ]; then
  PATH="$ORIGINAL_PATH"
  fail "stat_mtime (xo-watch.sh): expected $EXPECTED_MTIME, got '$RESULT_WATCH_MTIME'"
fi
pass "stat_mtime from xo-watch.sh returns correct epoch mtime under GNU stat shadowing"
