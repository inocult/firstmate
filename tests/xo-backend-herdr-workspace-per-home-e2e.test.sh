#!/usr/bin/env bash
# tests/xo-backend-herdr-workspace-per-home-e2e.test.sh - mandatory ISOLATED
# end-to-end real-herdr test for the P3 "workspace-per-home" pass (AGENTS.md
# task herdr-sm-spaces-k4). Drives the REAL bin/xo-spawn.sh and
# bin/xo-teardown.sh (not just adapter primitives), because the requirement
...
# too - only exists at xo-spawn.sh's own home-shadowing logic (the herdr case
# arm) and at xo_backend_herdr_workspace_label's XO_HOME read; neither is
# exercised by the adapter-primitive smoke test.
#
# Mirrors tests/xo-backend-autodetect-smoke.test.sh's isolated-session
# convention: a private throwaway HERDR_SESSION (never the captain's
# default), scratch XO_HOME(s), and scratch local-only projects.
#
...
#   - a primary-shaped home (no .xo-secondmate-home marker) spawning a
#     crewmate into the "xo" workspace
#   - a secondmate-shaped home (with .xo-secondmate-home) getting its own
#     labeled workspace when the PRIMARY spawns it (xo-spawn.sh's XO_HOME
#     shadow for --secondmate)
#   - a crewmate spawned FROM that secondmate-shaped home (the secondmate
#     running its OWN xo-spawn.sh) landing in the secondmate's own workspace -
...
command -v treehouse >/dev/null 2>&1 || { echo "skip: treehouse not found (required by xo-spawn.sh)"; exit 0; }
...
# tests/xo-backend-autodetect-smoke.test.sh.
# xo-spawn no longer needs this as a symlink workaround: xo-spawn-symlink-guard-s8
# canonicalized project and backend cwd comparisons in the worktree-discovery
# poll.
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/xo-herdr-e2e.XXXXXX")
SESSION="xo-lab-herdr-e2e-$$"
...
# --- 1. primary-shaped home: a crewmate spawns into the "xo" space ---

CM1_OUT="$TMP_ROOT/cm1.out"; CM1_ERR="$TMP_ROOT/cm1.err"
XO_SPAWN_NO_GUARD=1 XO_HOME="$PRIMARY_HOME" XO_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/xo-spawn.sh" cm1 "$PROJ1" "sh -c 'echo primary-crew-ok'" --mode no-mistakes --yolo off --backend herdr \
  >"$CM1_OUT" 2>"$CM1_ERR"
rc=$?
...
sleep 1
CM1_CAPTURE=$(xo_backend_herdr_capture "$SESSION:$CM1_PANE" 30) || fail "capture failed on cm1's pane"
...
[ "$CM1_WS_LABEL" = "xo" ] || fail "a primary-shaped home's crewmate should land in the 'xo' workspace, got '$CM1_WS_LABEL'"
pass "real herdr E2E: the primary-shaped home's crewmate landed in the 'xo' workspace"

# --- 2. the PRIMARY spawns a secondmate: its tab lands in the SECONDMATE's own space ---
# (xo-spawn.sh's herdr case arm shadows XO_HOME to the secondmate's home for
# exactly this call - AGENTS.md task herdr-sm-spaces-k4, requirement 3.)

SM_OUT="$TMP_ROOT/sm.out"; SM_ERR="$TMP_ROOT/sm.err"
XO_SPAWN_NO_GUARD=1 XO_HOME="$PRIMARY_HOME" XO_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/xo-spawn.sh" e2esm1 "$SM_HOME" "sh -c 'echo secondmate-launch-ok'" --secondmate --backend herdr \
  >"$SM_OUT" 2>"$SM_ERR"
rc=$?
...
XO_SPAWN_NO_GUARD=1 XO_HOME="$SM_HOME" XO_ROOT_OVERRIDE="$ROOT" \
  "$ROOT/bin/xo-spawn.sh" cm2 "$PROJ2" "sh -c 'echo sm-crew-ok'" --mode no-mistakes --yolo off --backend herdr \
  >"$CM2_OUT" 2>"$CM2_ERR"
rc=$?
...
pass "real herdr E2E: a crewmate spawns successfully FROM a secondmate-shaped home's own xo-spawn.sh process"

sleep 1
CM2_CAPTURE=$(xo_backend_herdr_capture "$SESSION:$CM2_PANE" 30) || fail "capture failed on cm2's pane"
...
PRIMARY_LIVE=$(XO_HOME="$PRIMARY_HOME" xo_backend_herdr_list_live "$SESSION")
assert_contains_local "$PRIMARY_LIVE" "xo-cm1" "the primary home's list_live did not see its own task"
assert_not_contains_local "$PRIMARY_LIVE" "xo-e2esm1" "the primary home's list_live must not see the secondmate's own task"
assert_not_contains_local "$PRIMARY_LIVE" "xo-cm2" "the primary home's list_live must not see the secondmate-owned crewmate's task"
...
assert_contains_local "$SM_LIVE" "xo-e2esm1" "the secondmate home's list_live did not see its own task"
assert_contains_local "$SM_LIVE" "xo-cm2" "the secondmate home's list_live did not see the crewmate spawned from it"
assert_not_contains_local "$SM_LIVE" "xo-cm1" "the secondmate home's list_live must not see the primary's task"
...
XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$PRIMARY_HOME/state" XO_DATA_OVERRIDE="$PRIMARY_HOME/data" \
  XO_CONFIG_OVERRIDE="$PRIMARY_HOME/config" \
  "$ROOT/bin/xo-teardown.sh" cm1 >"$TD1_OUT" 2>&1
rc=$?
[ "$rc" -eq 0 ] || fail "xo-teardown.sh failed for the primary-shaped crewmate cm1"$'\n'"$(cat "$TD1_OUT")"
[ -f "$CM1_META" ] && fail "xo-teardown.sh did not remove cm1's meta"
if herdr pane get "$CM1_PANE" --session "$SESSION" >/dev/null 2>&1; then
  fail "xo-teardown.sh did not close cm1's pane"
fi
...
XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$SM_HOME/state" XO_DATA_OVERRIDE="$SM_HOME/data" \
  XO_CONFIG_OVERRIDE="$SM_HOME/config" \
  "$ROOT/bin/xo-teardown.sh" cm2 >"$TD2_OUT" 2>&1
rc=$?
[ "$rc" -eq 0 ] || fail "xo-teardown.sh failed for the secondmate-owned crewmate cm2"$'\n'"$(cat "$TD2_OUT")"
[ -f "$CM2_META" ] && fail "xo-teardown.sh did not remove cm2's meta"
if herdr pane get "$CM2_PANE" --session "$SESSION" >/dev/null 2>&1; then
  fail "xo-teardown.sh did not close cm2's pane"
fi
if ! herdr pane get "$SM_PANE" --session "$SESSION" >/dev/null 2>&1; then
...
xo_backend_herdr_kill "$SESSION:$SM_PANE"
