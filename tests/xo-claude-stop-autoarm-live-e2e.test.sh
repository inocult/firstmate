...
# (bin/xo-claude-stop-autoarm.sh + bin/xo-turnend-guard.sh --claude).
...
# session lock can run xo-session-start.sh first; session start reclaims the
...
# The project and XO_HOME are isolated; Claude keeps using its existing managed
...
xo_live_gate opt-in XO_CLAUDE_LIVE_E2E claude
...
printf '%s\n' "$P" | jq -r '.tool_input.command // "unknown"' >> "$XO_HOME/state/tool-calls.log" 2>/dev/null
exit 0
SH
...
# harness owner under xo_harness_pid_alive, matching the reproduced incident.
printf '9999999\n' > "$HOME_DIR/state/.lock"
...
cat > "$PROJECT/bin/xo-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
N=$(cat "$XO_HOME/state/arm-count" 2>/dev/null || echo 0); N=$((N+1)); echo "$N" > "$XO_HOME/state/arm-count"
echo "arm-run=$N pid=$$" >> "$XO_HOME/state/arm-ran"
if [ "$N" -ge 3 ]; then
...
# Stop-owned cycles.
cat > "$PROJECT/bin/xo-wake-drain.sh" <<'SH'
#!/usr/bin/env bash
N=$(cat "$XO_HOME/state/drain-count" 2>/dev/null || echo 0); N=$((N+1)); echo "$N" > "$XO_HOME/state/drain-count"
echo "drain-run=$N" >> "$XO_HOME/state/drain-ran"
if [ "$N" -ge 3 ]; then
  rm -f "$XO_HOME/state/task.meta"
fi
printf 'stale: fixture-rapid drained\n'
SH
chmod +x "$PROJECT/bin/xo-watch-arm.sh" "$PROJECT/bin/xo-wake-drain.sh"

PROMPT='Run exactly `bin/xo-session-start.sh` with Bash as your first tool call. After reading its complete digest, reply with exactly CYCLE0 and stop. Whenever a Stop hook feedback message wakes you, run exactly `bin/xo-wake-drain.sh` once with Bash, then reply with exactly ACK and stop. Never run bin/xo-watch-arm.sh or any other arm command, and never use any other tool.'

(
  cd "$PROJECT" || exit 1
  XO_HOME="$HOME_DIR" CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 \
...
[ "$(sed -n '1p' "$HOME_DIR/state/tool-calls.log" 2>/dev/null)" = 'bin/xo-session-start.sh' ] \
...
if [ -f "$HOME_DIR/state/tool-calls.log" ]; then
  ! grep -q 'xo-watch-arm.sh' "$HOME_DIR/state/tool-calls.log" \
...
printf '%s\n' '{"session_id":"live-owner-control"}' \
  | XO_HOME="$LIVE_OWNER_HOME" XO_ROOT_OVERRIDE="$PROJECT" "$FAKE_CLAUDE" -c '"$XO_ROOT_OVERRIDE/bin/xo-claude-stop-autoarm.sh"' \
