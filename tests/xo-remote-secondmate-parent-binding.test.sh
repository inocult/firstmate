#!/usr/bin/env bash
# tests/xo-remote-secondmate-parent-binding.test.sh - regression coverage for the
# xo-remote-sm-cleanup-parent-binding-s1 scout report: finished-worker cleanup
...
# child the remote code checkout as its parent home (bin/xo-spawn.sh's sole
# writer of XO_PUBLIC_FOLLOWUP_PRIMARY_HOME receives XO_HOME=$XO_ROOT from
# bin/xo-remote-secondmate-control.sh's host-local launch), and that path can
# never carry the parent's real state or registry.
#
# The fix (report section 7, captain-approved same-machine scope): a durable
# .xo-secondmate-parent record, written once at seeding next to the
# .xo-secondmate-home identity marker, names this home's route to its parent as
# "local" or "remote". bin/xo-teardown.sh's cleanup gate reads it and treats a
...
#
# This drives the REAL remote route (xo-remote-home-seed.sh -> xo-on.sh ->
# xo-remote-entrypoint.sh -> the host-local xo-remote-secondmate-control.sh ->
# the real bin/xo-spawn.sh --secondmate) across the repo's own deterministic SSH
# boundary and Herdr fixture, then runs the real bin/xo-teardown.sh for a
...
TMP_ROOT=$(xo_test_tmproot xo-remote-parent-binding)
mkdir -p "$TMP_ROOT"
...
PUBLISH_FAKEBIN=$(xo_fakebin "$TMP_ROOT/publication-fake")
...
case "$destination" in
  */.xo-secondmate-home)
    ...
    while [ ! -f "$XO_TEST_PUBLISH_RELEASE" ]; do sleep 0.02; done
    ;;
esac
...
PATH="$PUBLISH_FAKEBIN:$PATH" XO_HOME="$PUBLISH_HOME" XO_ROOT_OVERRIDE="$ROOT" \
  XO_TEST_REAL_MV="$REAL_MV" XO_TEST_PUBLISH_ENTERED="$PUBLISH_ENTERED" \
  XO_TEST_PUBLISH_RELEASE="$PUBLISH_RELEASE" \
  "$ROOT/bin/xo-remote-home-provision.sh" < "$PUBLISH_MANIFEST" >/dev/null 2>&1 &
PUBLISH_PID=$!
publish_wait=0
...
cmp -s "$PUBLISH_HOME/.xo-secondmate-parent" <(
...
assert_absent "$PUBLISH_HOME/.xo-secondmate-home" \
...
PUBLISH_PID=
assert_present "$PUBLISH_HOME/.xo-secondmate-home" \
...
if [ "$command_name" = xo-remote-secondmate-control.sh ] \
   && [ "$_command_action" = launch ] \
   && [ -n "${XO_TEST_PUBLICATION_TARGET:-}" ]; then
  out=$("$XO_FAKE_REMOTE_ENTRYPOINT" "$@")
  rc=$?
  rm -f "$XO_TEST_PUBLICATION_TARGET"
  ln -s "$XO_TEST_PUBLICATION_FOREIGN" "$XO_TEST_PUBLICATION_TARGET" || exit 94
  printf '%s\n' "$out"
  exit "$rc"
fi
...
XO_SECONDMATE_CHARTER='Own iOS delivery on the build Mac.' \
...
assert_present "$REMOTE_HOME/.xo-secondmate-parent" \
  "real remote provisioning must write a durable parent record"
cmp -s "$REMOTE_HOME/.xo-secondmate-parent" <(
  printf 'schema=xo-secondmate-parent.v1\nroute=remote\nparent_host=remote-mac\n'
) || fail "real remote provisioning must write the exact durable remote parent record"

remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate >/dev/null \
  || fail "real remote secondmate launch failed"

DELIVERED_LINE=$(grep -F 'XO_PUBLIC_FOLLOWUP_PRIMARY_HOME' "$HERDR_LOG" | tail -1 || true)
DELIVERED=$(printf '%s\n' "$DELIVERED_LINE" | tr ' ' '\n' \
  | sed -n "s/^XO_PUBLIC_FOLLOWUP_PRIMARY_HOME='\{0,1\}\([^']*\)'\{0,1\}\$/\1/p" | tail -1)
...
write_child_meta() {
  xo_write_meta "$REMOTE_HOME/state/work-child.meta" \
...
run_child_teardown() { # <extra env assignments...>
  ...
  out=$(env "$@" PATH="$TMP_ROOT/childfake:$PATH" \
    ...
    XO_DATA_OVERRIDE="$REMOTE_HOME/data" XO_CONFIG_OVERRIDE="$REMOTE_HOME/config" \
    "$REMOTE_ROOT/bin/xo-teardown.sh" work-child 2>&1) || rc=$?
...
rm -f "$LOCAL_META"
PUBLICATION_RC=0
PUBLICATION_OUT=$(XO_TEST_PUBLICATION_TARGET="$LOCAL_META" \
  XO_TEST_PUBLICATION_FOREIGN="$FOREIGN_META" \
  remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate 2>&1) || PUBLICATION_RC=$?
[ "$PUBLICATION_RC" -ne 0 ] \
