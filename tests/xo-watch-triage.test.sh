#!/usr/bin/env bash
# tests/xo-watch-triage.test.sh - the always-on wake triage built into
# bin/xo-watch.sh and the shared classifier (bin/xo-classify-lib.sh). The watcher
# now absorbs the benign majority of wakes in bash and exits ONLY on an actionable
# wake, so xo's LLM re-arms once per actionable event instead of once per
# wake. These tests cover the classifier predicates as pure functions, then drive
# a real xo-watch.sh subprocess to assert the behavioral contract:
...
#
# Daemon-side classification/injection lives in xo-daemon.test.sh; watcher/lock
# liveness in xo-watcher-lock.test.sh; the durable-queue safety matrix in
# xo-wake-queue.test.sh.
set -u
...
WATCH="$ROOT/bin/xo-watch.sh"
DRAIN="$ROOT/bin/xo-wake-drain.sh"

TMP_ROOT=$(xo_test_tmproot xo-watch-triage-tests)

ack_stopped_cycle() {  # <state>
  local state=$1 err sequence generation
  err="$state/.test-cycle-drain.err"
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2> "$err" || return 1
  sequence=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through \([0-9][0-9]*\) --recovery-generation [A-Za-z0-9._-][A-Za-z0-9._-]*$/\1/p' "$err")
  generation=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through [0-9][0-9]* --recovery-generation \([A-Za-z0-9._-][A-Za-z0-9._-]*\)$/\1/p' "$err")
  rm -f "$err"
  [ -n "$sequence" ] && [ -n "$generation" ] || return 1
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$sequence" \
    --recovery-generation "$generation"
}

# Common watcher knobs: tight poll/grace, no check or heartbeat cadence unless a
# test overrides them, so a test only exercises the path it targets. XO_CREW_STATE_BIN
# points at the case's hermetic fake xo-crew-state.sh (installed by make_case) so the
# absorb-only-when-provably-working triage reads a canned verdict; a test fixes that
# verdict via XO_FAKE_CREW_STATE in its environment before calling watch_bg.
watch_bg() {  # <state> <fakebin> <out> [extra env assignments...]
  local state=$1 fakebin=$2 out=$3
  shift 3
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$@" "$WATCH" > "$out" &
}
...
# A fixed wait_live budget only proves the process is still ALIVE: xo-watch.sh
...
# watcher takes that long to decide, but because xo-watch.sh does bounded
...
# fallback (which writes a partial filesystem dump on Linux; see xo-watch.sh).
file_mtime() {
...
# fire on a pre-existing status (mirrors xo-watch.sh's stat_sig exactly).
seen_sig() {
  ...
  case "$1" in
    *.status)
      ...
      size=$(size_of "$1")
      ident=$(_xo_open_decisions_file_ident "$1")
...
record_pi_busy() {  # <state-dir> <id>
  ...
  "$ROOT/bin/xo-busy-event.sh" apply "$state" "$id" busy --gen "$gen" \
    --source pi-ext --event agent-start
}

reap() { kill "$1" 2>/dev/null || true; wait "$1" 2>/dev/null || true; }

# --- pure classifier predicates (xo-classify-lib.sh) ------------------------
...
test_malformed_seen_signature_reads_the_whole_log() {
  ...
  printf '40' > "$marker"
  offset=$(bash -c '. "$1"; xo_wake_signal_seen_size "$2" "$3"' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$state" "$f")
  [ "$offset" = 0 ] \
...
test_stale_is_terminal_classifier() {
  ...
  printf 'done: ready in branch xo/x\n' > "$state/term.status"
  stale_is_terminal "sess:xo-term" "$state" || fail "terminal stale status not classified terminal"
  xo_write_meta "$state/herdr-term.meta" "window=default:w1:p2" "backend=herdr"
  printf 'done: ready in branch xo/herdr\n' > "$state/herdr-term.status"
  ...
  stale_is_terminal "sess:xo-nonterm" "$state" && fail "non-terminal stale classified terminal"
  stale_is_terminal "sess:xo-missing" "$state" && fail "stale with no status classified terminal"
...
test_classifier_primitives() {
  ...
  [ "$(window_to_task "sess:xo-fix-login-k3")" = "fix-login-k3" ] || fail "window_to_task did not strip session+xo- prefix"
  ...
  XO_CAPTAIN_RE='custom-verb:' status_is_captain_relevant "custom-verb: x" || fail "XO_CAPTAIN_RE override not honored"
  XO_CAPTAIN_RE='custom-verb:' status_is_captain_relevant "done: x" && fail "XO_CAPTAIN_RE override did not replace the default verb set"
  XO_CAPTAIN_RE='merged|custom-verb:' status_is_captain_relevant "working: rebased onto merged #76" \
    && fail "XO_CAPTAIN_RE override bypassed working: suppression"
  XO_CAPTAIN_RE='checks green|custom-verb:' status_is_captain_relevant "paused: checks green pending approval" \
    && fail "XO_CAPTAIN_RE override bypassed paused: suppression"
  XO_CAPTAIN_RE='custom-verb:' status_is_captain_relevant "custom-verb: x" \
...
# benign (absorb) ONLY when xo-crew-state.sh reports the crew as working from an
...
# fake xo-crew-state.sh (XO_CREW_STATE_BIN) returns a canned verdict per case.
...
# crew_absorb_class: the single xo-crew-state.sh read that returns BOTH absorb
...
test_crew_absorb_class_classifier() {
  ...
  XO_FAKE_CREW_STATE='state: paused · source: status-log · awaiting upstream'
...
test_crew_worktree_written_since_classifier() {
  ...
  # Only .git churn (what xo's own read-only git commands touch): pruned.
  ...
  # A secondmate records a provisioned xo home, not a code tree, and such a
  ...
  printf 'window=test:xo-sm2\nkind=ship\nworktree=%s\n' "$home" > "$state/sm2.meta"
  ! crew_worktree_written_since sm2 "$state" "$anchor" \
    || fail "a marked xo home reported crew write evidence"
...
# XO_WORKTREE_WRITE_PRUNE is a skip list, so clearing it skips nothing and is the
...
test_empty_write_prune_widens_the_probe() {
  ...
  : > "$anchor"
  set_mtime "$(( $(date +%s) - 120 ))" "$anchor"
  printf 'window=test:xo-e\nkind=ship\nworktree=%s\n' "$wt" > "$state/e.meta"
  saved=$XO_WORKTREE_WRITE_PRUNE
  XO_WORKTREE_WRITE_PRUNE=''
  ...
  XO_WORKTREE_WRITE_PRUNE=$saved
  ! crew_worktree_written_since e "$state" "$anchor" \
    || fail "the default prune list stopped keeping .git out of the probe"
  pass "an empty XO_WORKTREE_WRITE_PRUNE widens the probe to the whole depth-bounded tree instead of disabling it"
}
...
test_empty_write_prune_from_the_environment_widens_the_probe() {
  ...
  : > "$anchor"
  set_mtime "$(( $(date +%s) - 120 ))" "$anchor"
  printf 'window=test:xo-wenv\nkind=ship\nworktree=%s\n' "$wt" > "$state/wenv.meta"
  # The one thing written since the anchor sits exactly where the DEFAULT list prunes.
  printf 'pack\n' > "$wt/.git/objects/fresh"
  env -u XO_WORKTREE_WRITE_PRUNE \
    bash -c '. "$1"; crew_worktree_written_since wenv "$2" "$3"' _ \
    "$ROOT/bin/xo-classify-lib.sh" "$state" "$anchor" \
    && fail "the default skip list let .git churn count as write evidence"
  XO_WORKTREE_WRITE_PRUNE='' \
    bash -c '. "$1"; crew_worktree_written_since wenv "$2" "$3"' _ \
    "$ROOT/bin/xo-classify-lib.sh" "$state" "$anchor" \
    || fail "an empty XO_WORKTREE_WRITE_PRUNE in the environment fell back to the default skip list instead of widening the probe"
  pass "an empty XO_WORKTREE_WRITE_PRUNE exported into the environment prunes nothing, widening the probe"
}
...
SH
  ...
  started=$(date +%s)
  PATH="$slowbin:$PATH" XO_WORKTREE_WRITE_TIMEOUT=1 \
    bash -c '. "$1"; crew_worktree_written_since slow "$2" "$3"' _ \
    "$ROOT/bin/xo-classify-lib.sh" "$state" "$anchor" \
...
test_signal_crew_provably_working_classifier() {
  ...
  export XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh"
  export XO_FAKE_CREW_STATE_a='state: working · source: run-step · running'
  export XO_FAKE_CREW_STATE_b='state: done · source: run-step · run passed'
  ...
  unset XO_FAKE_CREW_STATE_a XO_FAKE_CREW_STATE_b
...
test_secondmate_status_signal_never_absorbed_classifier() {
  ...
  export XO_FAKE_CREW_STATE_sm='state: working · source: run-step · running'
  ...
  export XO_FAKE_CREW_STATE_crew='state: working · source: run-step · running'
  printf 'kind=ship\n' > "$state/crew.meta"
  ...
  unset XO_FAKE_CREW_STATE_sm XO_FAKE_CREW_STATE_crew
...
test_provably_working_signal_absorbed() {
  ...
  export XO_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
...
test_turn_ended_provably_working_absorbed() {
  ...
  export XO_FAKE_CREW_STATE='state: working · source: pane · harness busy'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_turn_ended_not_working_surfaced() {
  ...
  export XO_FAKE_CREW_STATE='state: unknown · source: none · no current-state source available'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced turn-end failed"
...
# turn boundary woke xo. Pane content that changed since the previous poll
...
test_turn_ended_churning_pane_absorbed() {
  ...
  export XO_FAKE_CREW_STATE='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_turn_ended_churn_resets_prior_stale_classification() {
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_STALE_ESCALATE_SECS=999 \
    XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_absorbed "$state" "$pid" "absorbed benign signal:" \
...
test_turn_ended_churn_resets_wedge_state_before_stale_poll() {
  ...
  window="test:xo-codexfreshinterval"
  ...
  export XO_FAKE_CREW_STATE='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_FAKE_TMUX_CAPTURE_COUNT_FILE="$capture_count" XO_FAKE_TMUX_CAPTURE_FAIL_AFTER=1 \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_turn_ended_still_pane_surfaced() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the still-pane turn-end failed"
...
test_turn_ended_malformed_prior_hash_surfaced() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null \
...
test_secondmate_turn_ended_churning_pane_surfaced() {
  ...
  printf '0\n' > "$state/.count-$key"
  export XO_FAKE_CREW_STATE='state: unknown · source: pane · harness state unavailable'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
...
test_turn_ended_colliding_window_key_surfaced() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:xo-a.b"; colliding="test:xo-a_b"
...
test_turn_ended_mixed_positive_evidence_batch_absorbed() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"
  first_window="test:xo-first"; second_window="test:xo-second"
  : > "$state/first.turn-ended"
  ...
  export XO_FAKE_CREW_STATE_first='state: working · source: run-step · running'
  export XO_FAKE_CREW_STATE_second='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOWS="$(printf 'xo-first\nxo-second')" \
    XO_FAKE_TMUX_CAPTURE="$capture_file" XO_FAKE_TMUX_FORBIDDEN_TARGET="$first_window" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  reap "$pid"
  unset XO_FAKE_CREW_STATE_first XO_FAKE_CREW_STATE_second
...
test_turn_ended_mixed_positive_evidence_batch_default_off() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  first_window="test:xo-firstoff"; second_window="test:xo-secondoff"
  ...
  printf '0\n' > "$state/.count-$first_key"
  printf '0\n' > "$state/.count-$second_key"
  export XO_FAKE_CREW_STATE_firstoff='state: working · source: run-step · running'
  export XO_FAKE_CREW_STATE_secondoff='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOWS="$(printf 'xo-firstoff\nxo-secondoff')" \
    XO_FAKE_TMUX_CAPTURE="$capture_file" XO_CONFIG_OVERRIDE="$(churn_config "$dir" off)" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  unset XO_FAKE_CREW_STATE_firstoff XO_FAKE_CREW_STATE_secondoff
...
test_status_and_turn_end_batch_never_uses_churn_evidence() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  first_window="test:xo-firststatus"; second_window="test:xo-secondturn"
  ...
  printf '%s' "$(hash_text 'second task previous render')" > "$state/.hash-$second_key"
  printf '0\n' > "$state/.count-$second_key"
  export XO_FAKE_CREW_STATE_firststatus='state: working · source: run-step · running'
  export XO_FAKE_CREW_STATE_secondturn='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOWS="$(printf 'xo-firststatus\nxo-secondturn')" \
    XO_FAKE_TMUX_CAPTURE="$capture_file" XO_CONFIG_OVERRIDE="$(churn_config "$dir")" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  unset XO_FAKE_CREW_STATE_firststatus XO_FAKE_CREW_STATE_secondturn
...
test_turn_ended_churn_absorb_off_by_default() {
  ...
  export XO_FAKE_CREW_STATE='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir" off)" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_turn_ended_churn_absorb_bounded() {
  ...
  export XO_FAKE_CREW_STATE='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" XO_TURNEND_CHURN_ABSORB_SECS=60 \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  wait_for_exit "$pid" 100 \
...
test_turn_ended_churn_timer_write_failure_surfaced() {
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" 2>/dev/null &
  pid=$!
...
test_turn_ended_invalid_churn_bound_surfaced() {
  ...
  export XO_FAKE_CREW_STATE='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" XO_TURNEND_CHURN_ABSORB_SECS=bogus \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" 2>/dev/null &
  pid=$!
...
test_turn_ended_oversized_churn_bound_surfaced() {
  ...
  export XO_FAKE_CREW_STATE='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_CONFIG_OVERRIDE="$(churn_config "$dir")" XO_TURNEND_CHURN_ABSORB_SECS=999999999999999999999999999999999999 \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" 2>/dev/null &
  pid=$!
...
test_turn_ended_surfaced_batch_opens_no_partial_deadline() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  first_window="test:xo-codexfirst"; second_window="test:xo-codexsecond"
  : > "$state/first.turn-ended"
  : > "$state/second.turn-ended"
  printf 'window=%s\nkind=ship\nharness=codex\n' "$first_window" > "$state/first.meta"
  printf 'window=%s\nkind=ship\nharness=codex\n' "$second_window" > "$state/second.meta"
  ...
  export XO_FAKE_CREW_STATE='state: unknown · source: pane · harness state unavailable (unknown codex-unverified)'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOWS="$(printf 'xo-codexfirst\nxo-codexsecond')" \
    XO_FAKE_TMUX_CAPTURE="$capture_file" XO_CONFIG_OVERRIDE="$(churn_config "$dir")" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=3 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" 2>/dev/null &
  pid=$!
...
test_working_note_not_working_surfaced() {
  ...
  status_file="$state/task.status"
  printf 'working: compiling step 2\n' > "$status_file"
  # A non-no-mistakes crew (no run) whose pane went idle: xo-crew-state falls back
  ...
  export XO_FAKE_CREW_STATE='state: working · source: status-log · working: compiling step 2'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced working: note failed"
...
test_secondmate_status_note_surfaced_despite_busy_agent() {
  ...
  export XO_FAKE_CREW_STATE='state: working · source: run-step · running'
  XO_CONFIG_OVERRIDE="$(churn_config "$dir")" watch_bg "$state" "$fakebin" "$out"
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the surfaced note failed"
...
test_self_announced_close_does_not_rewake_but_next_note_does() {
  ...
  rc=0
  XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    xo_wake_status_append_self_announced "$2" "$3" "resolved [key=k1]: answered: closed by this home"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$state" "$status_file" || rc=$?
  [ "$rc" -eq 0 ] || fail "the bookkeeping close was not self-announced (rc=$rc)"
  export XO_FAKE_CREW_STATE='state: unknown · source: none · idle worker'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_actionable_signal_surfaced() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the actionable signal failed"
...
# queues as an ordinary signal-kind wake - xo-branch-dispatch.ts's
...
# (xo-classify-lib.sh's status_span_first_actionable_record), but it is still a
...
test_captain_held_signal_payload_marked_for_branch_exclusion() {
  ...
  export XO_FAKE_CREW_STATE='state: working · source: run-step · still wrapping up'
  watch_bg "$state" "$fakebin" "$out"
  pid=$!
...
test_actionable_signal_survives_a_later_routine_append() {
  ...
  grep -F "signal: $status_file" "$out" >/dev/null || fail "watcher did not print the actionable signal reason"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the masked signal failed"
...
test_release_completion_survives_a_later_routine_append() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the masked completion failed"
  grep "$(printf '\tsignal\t')" "$drain_out" | grep -F "$status_file" >/dev/null \
...
test_permission_recovery_surfaces_preserved_status() {
  ...
  before_ident=$(_xo_open_decisions_file_ident "$status_file")
  ...
  chmod 600 "$status_file"
  after_ident=$(_xo_open_decisions_file_ident "$status_file")
...
test_terminal_stale_surfaced() {
  ...
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the terminal stale failed"
...
# log gets no new entry once xo hands it to a no-mistakes validation
...
test_stale_terminal_status_overridden_by_active_run() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:xo-validating"
  printf 'no-mistakes axi run: validating...' > "$capture_file"
  printf 'window=%s\nkind=ship\n' "$window" > "$state/validating.meta"
  # The crew reported done BEFORE xo triggered no-mistakes validation;
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_STALE_ESCALATE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  : > "$out"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_STALE_ESCALATE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_nonterminal_stale_provably_working_absorbed_then_escalated() {
  ...
  export XO_FAKE_CREW_STATE='state: working · source: run-step · ci running'
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the wedge escalation failed"
...
test_nonterminal_stale_not_working_surfaced() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the immediate stale failed"
...
test_nonterminal_stale_paused_absorbed_then_resurfaced() {
  ...
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  # crew_absorb_class reads the declared pause from xo-crew-state.sh.
  export XO_FAKE_CREW_STATE='state: paused · source: status-log · holding for the upstream tool release'

  # Phase A: a fresh pause (status file just written) under a high re-surface
  # threshold is absorbed - no wake, no wedge timer.
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_FAKE_TMUX_CURRENT_COMMAND=zsh \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_PAUSE_RESURFACE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_FAKE_TMUX_CURRENT_COMMAND=zsh \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_PAUSE_RESURFACE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the paused re-surface failed"
  grep "$(printf '\tstale\t')" "$drain_out" | grep -F "$window" >/dev/null || fail "paused re-surface was not queued"
  pass "a declared pause is absorbed on first sight, then re-surfaced as a recheck past the threshold, never wedge-escalated"
}

# A captain-held crew can leave a stable backend endpoint after its agent exits.
# xo-crew-state then authoritatively reports stopped rather than paused, but the
...
test_exited_declared_pause_is_bounded_but_live_gate_surfaces() {
  ...
  while [ "$round" -le 6 ]; do
    PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
      XO_FAKE_TMUX_CURRENT_COMMAND=zsh XO_FAKE_CREW_STATE='state: stopped · source: pane · bare shell' \
      XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_PAUSE_RESURFACE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
      XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" >> "$out" &
    pid=$!
    if wait_poll_cycle "$state" "$pid"; then
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_FAKE_TMUX_CURRENT_COMMAND=grok XO_FAKE_CREW_STATE='state: paused · source: status-log · waiting at an active external-decision gate' \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_PAUSE_RESURFACE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_FAKE_TMUX_CURRENT_COMMAND=grok XO_FAKE_CREW_STATE='state: paused · source: status-log · waiting at an active external-decision gate' \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_STALE_ESCALATE_SECS=240 XO_PAUSE_RESURFACE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
    reap "$pid"
...
test_absorbed_replacement_wait_does_not_inherit_the_old_throttle() {
  ...
  do
    ...
    PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
      XO_FAKE_TMUX_CURRENT_COMMAND=zsh XO_FAKE_CREW_STATE='state: stopped · source: pane · bare shell' \
      XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
      XO_PAUSE_RESURFACE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
      XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" >> "$out" &
    pid=$!
...
# in the pane contents the case just wrote. Armed the way xo-watch-arm.sh arms a
# successor after xo handled a wake, because that is what a supervision
...
parked_watch_round() {  # <state> <fakebin> <out> <capture> <window> <exit|absorb>
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture" \
    XO_FAKE_TMUX_CURRENT_COMMAND=grok \
    XO_FAKE_CREW_STATE='state: paused · source: status-log · parked' \
    XO_WATCH_HANDLING_SUCCESSOR=1 \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_PAUSE_RESURFACE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" >> "$out" &
  pid=$!
  if [ "$mode" = exit ]; then
...
# wake xo - the throttle was written by the very wake it should have
...
test_live_declared_wait_churn_honors_the_resurface_throttle() {
  ...
  do
    ...
    # inconclusive and xo has to look at it.
...
# itself lives in the BACKLOG, written there by bin/xo-captain-hold.sh. No line
...
#
# The backlog is real rather than a fixture file: bin/xo-captain-hold.sh is the
...
# The window key every hold fixture uses, derived the way xo-watch.sh derives it.
hold_key() {
  printf '%s' test:xo-held-merge | tr ':/.' '___'
}

# bin/xo-captain-hold.sh against a hold fixture's own home.
run_hold() {  # <dir> <args...>
  local dir=$1
  shift
  XO_HOME="$dir" XO_STATE_OVERRIDE="$dir/state" XO_DATA_OVERRIDE="$dir/data" \
    XO_CONFIG_OVERRIDE="$dir/config" "$ROOT/bin/xo-captain-hold.sh" "$@" >/dev/null 2>&1
}
...
make_hold_home() {  # <name> <status-line> <hold|nohold>
  ...
  printf 'window=test:xo-held-merge\nkind=ship\nharness=grok\nbackend=tmux\n' \
...
hold_watch_launch() {  # <dir> <out> <capture>
  ...
  PATH="$dir/fakebin:$PATH" XO_FAKE_TMUX_WINDOW=test:xo-held-merge \
    XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURRENT_COMMAND=zsh \
    XO_FAKE_CREW_STATE='state: stopped · source: pane · bare shell' \
    XO_WATCH_HANDLING_SUCCESSOR=1 \
    XO_HOME="$dir" XO_DATA_OVERRIDE="$dir/data" XO_CONFIG_OVERRIDE="$dir/config" \
    XO_STATE_OVERRIDE="$dir/state" XO_CREW_STATE_BIN="$dir/fakebin/xo-crew-state.sh" \
    XO_PAUSE_RESURFACE_SECS="${XO_HOLD_PAUSE_RESURFACE_SECS:-999}" XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" >> "$out" 2>&1 &
  HOLD_WATCH_PID=$!
}
...
hold_stale_wakes() {  # <state>
  awk -F '\t' '$3 == "stale" && $4 == "test:xo-held-merge" { n++ } END { print n + 0 }' \
...
test_secondmate_paused_resurfaces_in_normal_mode() {
  ...
  window="test:xo-secondmate-held"
  ...
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  export XO_FAKE_CREW_STATE='state: paused · source: status-log · awaiting the upstream release'
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_PAUSE_RESURFACE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_secondmate_captain_held_resurfaces_in_normal_mode() {
  ...
  window="test:xo-secondmate-hold"
...
test_secondmate_nonpaused_stale_remains_suppressed() {
  ...
  window="test:xo-secondmate-working"
...
test_secondmate_unpause_clears_pause_tracking() {
  ...
  out="$dir/watch.out"; statusf="$state/secondmate-resumed.status"; window="test:xo-secondmate-resumed"
...
test_nonterminal_stale_pause_transitions_reclassify_unchanged_hash() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-transition"
  printf 'idle awaiting external\n' > "$capture_file"
...
test_nonterminal_paused_rechecks_authoritative_state() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-pause-recheck"
  printf 'idle awaiting external\n' > "$capture_file"
...
test_paused_authoritative_working_preserves_wedge_timer() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-paused-working"
  printf 'idle awaiting external\n' > "$capture_file"
...
# XO_WEDGE_DEMAND_INSPECT_COUNT consecutive escalations on the SAME pane, the
...
# string of stale and possible-wedge alarms every XO_STALE_ESCALATE_SECS until
# the captain merged. Each worker had exited after reporting its PR, xo
...
# The fixtures lay out exactly those records - the pr= line bin/xo-pr-check.sh
...
# dead agent. xo-crew-state's canned verdict is left at its unknown default:
...
test_held_for_merge_dead_agent_absorbs_first_sight_stale() {
  ...
  for spec in \
    ...
    'nonterminal|resolved [key=nm-01RUN-review]: xo accepted the finding'
  do
    name=${spec%%|*}; status_line=${spec#*|}
    dir=$(make_case "held-merge-$name"); state="$dir/state"; fakebin="$dir/fakebin"
    out="$dir/watch.out"; capture_file="$dir/pane.txt"
    window="test:xo-held-$name"
...
# escalated again every XO_STALE_ESCALATE_SECS. A held-for-merge task must drop
...
test_held_for_merge_needs_dead_agent_and_both_records() {
  ...
  do
    ...
    PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
      XO_FAKE_TMUX_CURRENT_COMMAND="$command" \
      XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_STALE_ESCALATE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
      XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
    pid=$!
...
test_busy_pane_below_turn_age_bound_is_absorbed() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-busy-fresh"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_BUSY_TURN_MAX_SECS=999 XO_STALE_ESCALATE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_busy_pane_stable_hash_escalates_past_turn_age_bound() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-busy-stable"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_BUSY_TURN_MAX_SECS=1 XO_STALE_ESCALATE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  : > "$out"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_BUSY_TURN_MAX_SECS=1 XO_STALE_ESCALATE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_busy_pane_changing_hash_escalates_past_turn_age_bound() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-busy-ticking"
...
test_busy_pane_turn_end_touch_resets_age() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-busy-reset"
  printf 'Working...' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_BUSY_TURN_MAX_SECS=3600 XO_STALE_ESCALATE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_busy_pane_repeated_escalation_reaches_demand_deep_inspection() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-busy-demand-inspect"
  printf 'Working...' > "$capture_file"
...
test_busy_declared_pause_is_rechecked_not_wedge_escalated() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-review-scout"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    XO_BUSY_TURN_MAX_SECS=1 XO_STALE_ESCALATE_SECS=1 XO_PAUSE_RESURFACE_SECS=999 \
    XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    XO_BUSY_TURN_MAX_SECS=1 XO_STALE_ESCALATE_SECS=1 XO_PAUSE_RESURFACE_SECS=240 \
    XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    XO_BUSY_TURN_MAX_SECS=1 XO_STALE_ESCALATE_SECS=999 XO_PAUSE_RESURFACE_SECS=999 \
    XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    XO_BUSY_TURN_MAX_SECS=1 XO_STALE_ESCALATE_SECS=240 XO_PAUSE_RESURFACE_SECS=999 \
    XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
# XO_STALE_ESCALATE_SECS for as long as the wait lasted, with the escalation count
...
test_afk_busy_declared_pause_hands_off_plain_stale() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-afk-review-scout"
...
test_afk_busy_declared_pause_ticking_pane_hands_off_once() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; window="test:xo-afk-ticking-scout"
...
case "${1:-}" in
  list-windows)
    [ -n "${XO_FAKE_TMUX_WINDOW:-}" ] && printf '%s\n' "${XO_FAKE_TMUX_WINDOW#*:}"
    exit 0 ;;
  capture-pane)
    n=$(( $(cat "$XO_FAKE_TMUX_TICKS" 2>/dev/null || echo 0) + 1 ))
    echo "$n" > "$XO_FAKE_TMUX_TICKS"
    printf 'Working... (%d.%ds) lavish-axi poll' "$(( 7200 + n ))" "$(( n % 10 ))"
    exit 0 ;;
  display-message)
    case "$*" in
      *pane_current_command*) printf '%s\n' "${XO_FAKE_TMUX_CURRENT_COMMAND:-}"; exit 0 ;;
    esac ;;
esac
exit 1
SH
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_TICKS="$ticks" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_FAKE_CREW_STATE='state: working · source: pane · harness busy (pi-ext)' \
    XO_BUSY_TURN_MAX_SECS=1 XO_STALE_ESCALATE_SECS=1 XO_PAUSE_RESURFACE_SECS=999 \
    XO_POLL=0.2 XO_SIGNAL_GRACE=1 \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || true
...
# Behavioral proof that the production default (no XO_BUSY_TURN_MAX_SECS override
...
test_busy_pane_default_turn_age_bound_is_3600s() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"; window="test:xo-busy-default"
  printf 'Working...' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_STALE_ESCALATE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
test_wedge_escalation_deferred_while_worktree_is_written() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:xo-writing"; wt="$dir/wt"
  mkdir -p "$wt/src"
  printf 'idle building output' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_STALE_ESCALATE_SECS=240 \
    XO_PAUSE_RESURFACE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the stalled-crew escalation failed"
...
test_write_deferral_resurfaces_on_the_bounded_cadence() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:xo-churn"; wt="$dir/wt"
  mkdir -p "$wt/src"
  printf 'idle building output' > "$capture_file"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the write-deferral recheck failed"
...
# The worktree recorded for a secondmate is a provisioned xo home, and that
...
test_secondmate_home_supervision_churn_is_not_write_evidence() {
  ...
  out="$dir/watch.out"; drain_out="$dir/drain.out"; capture_file="$dir/pane.txt"
  window="test:xo-mate"; home="$dir/mate-home"
  mkdir -p "$home/state"
  printf 'sm-mate\n' > "$home/.xo-secondmate-home"
  printf 'Working... (12.3s)' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_STALE_ESCALATE_SECS=240 XO_BUSY_TURN_MAX_SECS=1 XO_PAUSE_RESURFACE_SECS=999 \
    XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the mate escalation failed"
...
test_timer_repair_drops_a_finished_write_deferral_chain() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"
  window="test:xo-chain-repair"; wt="$dir/wt"
  mkdir -p "$wt/src"
  printf 'idle building output' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_STALE_ESCALATE_SECS=240 XO_PAUSE_RESURFACE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
...
test_terminal_first_sight_drops_a_finished_write_deferral_chain() {
  ...
  out="$dir/watch.out"; capture_file="$dir/pane.txt"
  window="test:xo-chain-firstsight"; wt="$dir/wt"
  mkdir -p "$wt/src"
  printf 'no-mistakes axi run: validating...' > "$capture_file"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_STALE_ESCALATE_SECS=999 XO_PAUSE_RESURFACE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  : > "$state/.writing-since-$key"
  set_mtime "$back" "$state/.writing-since-$key"
  XO_FAKE_CREW_STATE='state: unknown · source: none · no run, no busy pane'
  : > "$out"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_STALE_ESCALATE_SECS=999 XO_PAUSE_RESURFACE_SECS=240 XO_POLL=1 XO_SIGNAL_GRACE=1 \
...
SH
  ...
  export XO_FAKE_CREW_STATE='state: working · source: run-step · validating (running)'
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 XO_WATCH_TRIAGE_LOG_MAX_BYTES=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
...
# Run the runner against a case home. XO_ROOT_OVERRIDE (exported by the shared
...
pe_case() {  # <dir> <command>...
  ...
  shift
  (unset XO_ROOT_OVERRIDE
   XO_PROCEVENT_CLAIM_ROOT="$dir/claims" XO_HOME="$dir" "$ROOT/bin/xo-procevent.sh" "$@")
}
...
# The watcher, scoped by XO_HOME rather than XO_STATE_OVERRIDE, so the
# per-cycle reconcile it launches resolves the same home's state.
procevent_watch_bg() {  # <dir> <out>
  local dir=$1 out=$2
  dir=$(cd "$dir" && pwd -P) || return 1
  PATH="$dir/fakebin:$PATH" XO_HOME="$dir" XO_PROCEVENT_CLAIM_ROOT="$dir/claims" \
    XO_CREW_STATE_BIN="$dir/fakebin/xo-crew-state.sh" \
    XO_POLL=0.2 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
}
...
test_procevent_captured_result_surfaces_proactively() {
  ...
  beacon_age=$(XO_STATE_OVERRIDE="$state" bash -c \
    '. "$1/bin/xo-wake-lib.sh"; xo_path_age "$2"' _ "$ROOT" "$state/.last-watcher-beat")
  [ "$beacon_age" -lt 60 ] || fail "the surfacing watcher was not a healthy one (beacon age ${beacon_age}s)"

  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the process-event wake failed"
...
test_procevent_unacknowledged_result_redrains_until_handled() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "drain after the first process-event wake failed"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$replay_out" 2> "$replay_err" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$sequence" --recovery-generation "$generation" \
...
test_procevent_marker_keys_are_injective() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "marker identity fixture drain failed"
  pass "complete process-event queue keys map to distinct seen markers"
}

# The reason line is the headline xo reads before the payload. Every
...
test_procevent_headlines_classify_queue_keys() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "captured headline fixture drain failed"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "stranded headline fixture drain failed"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "joined headline fixture drain failed"
...
# xo. The watcher remembers every procevent key it has surfaced for
...
case "$dest" in
  */.seen-procevent-*)
    case "${XO_MARKER_MV_MODE:-}" in
      pause)
        printf '1\n' > "$XO_MARKER_MV_READY"
        while [ ! -e "$XO_MARKER_MV_RELEASE" ]; do sleep 0.02; done
        ;;
...
test_procevent_surface_serializes_with_drain() {
  ...
  install_marker_mv_fault "$dir"
  XO_MARKER_MV_MODE=pause XO_MARKER_MV_READY="$ready" XO_MARKER_MV_RELEASE="$release" \
    procevent_watch_bg "$dir" "$out"
  pid=$!
  wait_numeric_file "$ready" 100 || fail "the watcher never reached its marker commit boundary"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" &
  drain_pid=$!
...
test_procevent_surface_crash_boundaries() {
  ...
  PATH="$dir/fakebin:$PATH" XO_HOME="$dir" XO_PROCEVENT_CLAIM_ROOT="$dir/claims" \
    XO_CREW_STATE_BIN="$dir/fakebin/xo-crew-state.sh" XO_POLL=0.2 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$fifo" &
  pid=$!
  wait "$reader" || true
  wait_for_exit "$pid" 100
  exit_status=$?
  ...
  install_marker_mv_fault "$dir"
  XO_MARKER_MV_MODE=kill-before procevent_watch_bg "$dir" "$out"; pid=$!
  wait_for_exit "$pid" 100
  exit_status=$?
  ...
  install_marker_mv_fault "$dir"
  XO_MARKER_MV_MODE=kill-after procevent_watch_bg "$dir" "$out"; pid=$!
  wait_for_exit "$pid" 100
  exit_status=$?
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out.replay.drain" 2> "$replay_err" \
...
test_procevent_marker_failure_exits_and_replays() {
  ...
  install_marker_mv_fault "$dir"
  XO_MARKER_MV_MODE=fail procevent_watch_bg "$dir" "$out"
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2>&1 || fail "marker-failure fixture drain failed"
...
test_heartbeat_no_change_absorbed() {
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=1 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid"; then
  ...
  # XO_HEARTBEAT, which need not be the first completed cycle, so wait for the
...
test_heartbeat_backstop_surfaces_unsurfaced_status() {
  ...
  # fleet-scan backstop must catch it and wake xo.
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the backstop heartbeat failed"
...
test_beacon_stays_fresh_while_absorbing() {
  ...
  reap "$pid"
  pass "the liveness beacon stays fresh while the watcher absorbs benign wakes (xo-guard never false-alarms)"
}
...
test_afk_present_reverts_watcher_to_one_shot() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after the afk-mode signal failed"
  grep "$(printf '\tsignal\t')" "$drain_out" | grep -F "$status_file" >/dev/null \
...
test_afk_paused_changed_pane_hands_off_plain_stale() {
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_FAKE_CREW_STATE='state: paused · source: status-log · awaiting the upstream tool release' \
    XO_STATE_OVERRIDE="$state" XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" XO_PAUSE_RESURFACE_SECS=240 XO_POLL=0.2 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2>/dev/null || fail "drain after AFK paused stale failed"
...
# While state/.afk-contract exists (bin/xo-afk-contract.sh) nobody is there to
...
write_away_record() {  # <state>
  if ! XO_HOME="$(dirname "$1")" XO_STATE_OVERRIDE="$1" "$ROOT/bin/xo-afk-contract.sh" propose >/dev/null 2>&1 \
    || ! XO_HOME="$(dirname "$1")" XO_STATE_OVERRIDE="$1" "$ROOT/bin/xo-afk-contract.sh" confirm >/dev/null 2>&1; then
    fail "could not write the away-posture record in $1"
  fi
}

archive_away_record() {  # <state>
  XO_HOME="$(dirname "$1")" XO_STATE_OVERRIDE="$1" "$ROOT/bin/xo-afk-contract.sh" archive >/dev/null 2>&1 \
...
test_afk_one_shot_never_hands_off_captain_held_under_away_record() {
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    XO_FAKE_TMUX_CURRENT_COMMAND=zsh \
    XO_STATE_OVERRIDE="$state" XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  if ! wait_poll_cycle "$state" "$pid" || ! wait_poll_cycle "$state" "$pid" || ! wait_poll_cycle "$state" "$pid"; then
...
until_watch() {  # <dir> <cadence> -> pid in UNTIL_PID
  local dir=$1
  PATH="$dir/fakebin:$PATH" XO_FAKE_TMUX_WINDOW=test:xo-until XO_FAKE_TMUX_CAPTURE="$dir/pane.txt" \
    XO_FAKE_CREW_STATE='state: unknown · source: none · no current-state source available' \
    XO_STATE_OVERRIDE="$dir/state" XO_CREW_STATE_BIN="$dir/fakebin/xo-crew-state.sh" \
    XO_PAUSE_RESURFACE_SECS="$2" XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$dir/watch.out" 2>&1 &
  UNTIL_PID=$!
}
...
test_paused_until_wrong_year_is_bounded_by_the_cadence() {
  ...
  grep -F 'stale: test:xo-until' "$dir/watch.out" >/dev/null \
...
test_paused_until_that_passed_is_rechecked_before_the_cadence() {
  ...
  grep -F 'stale: test:xo-until' "$dir/watch.out" >/dev/null || fail "the due recheck did not print a stale wake: $(cat "$dir/watch.out")"
  grep -F 'declared clearing time has passed' "$dir/watch.out" >/dev/null \
