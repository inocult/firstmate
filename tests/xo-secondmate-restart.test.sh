#!/usr/bin/env bash
# bin/xo-secondmate-restart.sh: persist-then-restart, and the honest fallback.
#
# What these pin, all through the real commands (real xo-send, real durable
# steering inbox, real parent-owned reply expectation, real xo-control
...
#      its host, over the xo-on transport, with the profile resolved from the
#      PARENT's own pin rather than the remote home's copy of it.
#   6. End to end with bin/xo-update.sh: a live mate whose home needed no
...
RESTART="$ROOT/bin/xo-secondmate-restart.sh"

xo_git_identity fmtest fmtest@example.com
TMP_ROOT=$(xo_test_tmproot xo-secondmate-restart)
mkdir -p "$TMP_ROOT"
...
add_local_mate() {
  ...
  xo_git_worktree "$dir/$id-repo" "$smhome" "sm-$id"
  mkdir -p "$smhome/state" "$smhome/data" "$smhome/bin" "$home/data/$id"
  printf '%s\n' "$id" > "$smhome/.xo-secondmate-home"
  printf '# agents\n' > "$smhome/AGENTS.md"
...
# Like add_local_mate, but the world is the one /updatexo actually runs
# against: a bare origin, an xo repo clone on its default branch, and the
...
add_repo_backed_mate() {  # <case-dir> <id> [harness] [backend]
  ...
  if [ ! -d "$repo" ]; then
    ...
    # The operational dirs a live home carries are gitignored in a real xo
    # checkout; without that the home would read as dirty and be skipped.
    printf '/data/\n/state/\n/config/\n/projects/\n/.no-mistakes/\n.xo-secondmate-home\n' \
      > "$dir/seed/.gitignore"
    git -C "$dir/seed" add -A
    git -C "$dir/seed" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm c1
...
# run_update_in_case <case-dir>: the real /updatexo mechanics over that world.
run_update_in_case() {
  local dir=$1
  env PATH="$dir/fakebin:$PATH" XO_FAKE_DIR="$dir/fake" \
    XO_ROOT_OVERRIDE="$dir/fmrepo" XO_HOME="$dir/home" \
    XO_SSH_BIN="${XO_TEST_SSH_BIN:-ssh}" \
    "$ROOT/bin/xo-update.sh" 2>/dev/null
}
...
run_restart() {  # <case-dir> <args...>
  ...
  env PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" XO_FAKE_DIR="$dir/fake" \
    XO_SPAWN_NO_GUARD=1 XO_SECONDMATE_PERSIST_POLL=1 \
    XO_SECONDMATE_PERSIST_WAIT="${XO_TEST_PERSIST_WAIT:-30}" \
...
test_persist_gates_and_asks_only_for_open_records() {
  ...
  out=$(XO_TEST_PERSIST_WAIT=0 run_restart "$dir" xo-sm1); rc=$?
...
test_persist_precedes_restart() {
  ...
  doorbell_line=$(grep -n '^: XO instruction waiting: ' "$dir/fake/literal" | head -1 | cut -d: -f1)
...
test_arrived_answer_precedes_deadline_check() {
  ...
  out=$(XO_TEST_PERSIST_WAIT=0 run_restart "$dir" sm1); rc=$?
...
target=${!#}
case "$target" in
  "${XO_FAKE_DIR%/fake}"/home/state/pending-replies/*)
    if [ ! -e "$XO_FAKE_DIR/answer-after-scan" ] \
      && grep -q '^parent_status_scan_signature=.' "$target"; then
      : > "$XO_FAKE_DIR/answer-after-scan"
      corr=${target##*/}
...
# --- T6: a remote mate restarts over the xo-on hop, on the parent's pin -------
# The seam decodes what xo-on.sh actually put on the wire, so this pins the
...
setup_remote_case() {  # <case-dir> <id> <ssh-mode>
  ...
  mkdir -p "$dir/$id-home"
  {
    ...
    echo "remote_target=xo-remote:2ndmate-$id"
...
printf '%s\n' "${rargs[*]}" >> "$XO_FAKE_SSH_LOG"
case "${XO_FAKE_SSH_MODE:-ok}" in
...
case "${rargs[1]:-}" in
  send)
    ...
    if [ -n "${XO_FAKE_ANSWER_STATUS:-}" ]; then
      corr=$(printf '%s' "${rargs[3]:-}" | grep -oE 'corr=[0-9a-f]{16}' | head -1)
      [ -z "$corr" ] || printf 'done [%s]: open records written down\n' "$corr" \
        >> "$XO_FAKE_ANSWER_STATUS"
    fi
    ;;
  relaunch)
    case "${XO_FAKE_SSH_MODE:-ok}" in
      slow-relaunch)
        : > "$XO_FAKE_DIR/remote-relaunch-start"
        /bin/sleep 2
        : > "$XO_FAKE_DIR/remote-relaunch-end"
        ;;
    esac
    printf 'relaunched %s\n' "${rargs[2]}"
    ;;
...
SH
  chmod +x "$fb/fake-ssh"
  : > "$dir/ssh.log"
  export XO_FAKE_SSH_LOG="$dir/ssh.log"
  export XO_FAKE_SSH_MODE="$mode"
  export XO_TEST_SSH_BIN="$fb/fake-ssh"
}
...
test_remote_mate_restarts_over_the_transport_hop() {
  ...
  export XO_FAKE_ANSWER_STATUS="$dir/home/state/sm2.status"
  ...
  out=$(run_restart "$dir" xo-sm2); rc=$?
  unset XO_FAKE_ANSWER_STATUS
  ...
  relaunch_line=$(grep '^xo-remote-secondmate-control.sh relaunch' "$dir/ssh.log" | head -1)
  [ -n "$relaunch_line" ] || fail "no relaunch crossed the transport hop"$'\n'"$(cat "$dir/ssh.log")"
  [ "$relaunch_line" = "xo-remote-secondmate-control.sh relaunch sm2 codex big-model high" ] \
  ...
  [ "$(grep -n '^xo-remote-secondmate-control.sh send' "$dir/ssh.log" | head -1 | cut -d: -f1)" \
     -lt "$(grep -n '^xo-remote-secondmate-control.sh relaunch' "$dir/ssh.log" | head -1 | cut -d: -f1)" ] \
    || fail "the remote mate was restarted before it was asked to persist"$'\n'"$(cat "$dir/ssh.log")"
  pass "T6 a remote mate restarts through the host-local control plane over the xo-on hop"
}
...
test_native_ultra_restart_keeps_local_and_remote_profiles() {
  ...
  relaunch_line=$(grep '^xo-remote-secondmate-control.sh relaunch' "$dir/ssh.log" | head -1)
  [ "$relaunch_line" = "xo-remote-secondmate-control.sh relaunch sm2 pi-signed codex-native/gpt-6-astra ultra" ] \
...
test_persist_waits_are_polled_together() {
  ...
  out=$(XO_TEST_PERSIST_WAIT=3 run_restart "$dir" sm1 sm2); rc=$?

  expect_code 3 "$rc" "the unanswered mate should fall back after the confirmed mate restarts"$'\n'"$out"
  exit_line=$(grep -n '^/exit$' "$dir/fake/literal" | head -1 | cut -d: -f1)
  nudge_line=$(grep -n '^: XO instruction waiting: ' "$dir/fake/literal" | tail -1 | cut -d: -f1)
  [ -n "$exit_line" ] && [ -n "$nudge_line" ] && [ "$exit_line" -lt "$nudge_line" ] \
    || fail "the XO's timeout held the confirmed second mate behind it: $out"
...
test_relaunches_do_not_block_persist_polling() {
  ...
  export XO_FAKE_ANSWER_STATUS="$dir/home/state/sm1.status"
  arm_answer "$dir" sm2

  out=$(XO_TEST_PERSIST_WAIT=5 run_restart "$dir" sm1 sm2); rc=$?
  unset XO_FAKE_ANSWER_STATUS
  ...
  assert_grep 'xo-remote-secondmate-control.sh relaunch sm1 claude default default' "$dir/ssh.log" \
...
#!/usr/bin/env bash
if [ -e "$XO_FAKE_DIR/remote-relaunch-start" ] && [ ! -e "$XO_FAKE_DIR/result-race-injected" ]; then
  result=$(find "$XO_HOME/state" -maxdepth 2 -name '0.result' -print -quit)
  if [ -z "$result" ]; then
    result_dir=$(find "$XO_HOME/state" -maxdepth 1 -type d -name '.secondmate-restart.*' -print -quit)
    if [ -n "$result_dir" ]; then
      printf 'restarted: sm1 on remote-mac (claude)\n' > "$result_dir/0.result"
      : > "$XO_FAKE_DIR/result-race-injected"
      printf 'Z\n'
      exit 0
    fi
  fi
...
test_already_current_unprovable_mate_stays_on_the_nudge_path() {
  ...
  assert_contains "$nudge_line" "xo-sm1" \
...
echo "# all xo-secondmate-restart tests passed"
