#!/usr/bin/env bash
# tests/lib.sh - shared primitives for xo behavior tests.
#
...
#
# ROOT is exported as the xo repo root (this file lives in tests/), so a
...
if [ -n "${XO_TEST_LIB_SOURCED:-}" ]; then
  return 0
fi
XO_TEST_LIB_SOURCED=1

# Pin the fixture umask. XO's state-root and process-event contracts
...
# Exempt xo's own test suite from the gate-lifecycle refusal
# (bin/xo-gate-refuse-lib.sh). The no-mistakes gate runs this suite FROM a gate
# worktree - the exact environment that guard refuses - so without this every
# test that drives the real xo-spawn/xo-send/xo-teardown would be refused during
# xo's own validation. A confused gate agent never sources this helper, so
# the boundary against the real hazard is unaffected. tests/xo-gate-refuse.test.sh
# strips this to verify real refusal.
export XO_GATE_REFUSE_BYPASS=1

# Clear the task-worker marker bin/xo-spawn.sh exports into ship and scout
# panes. This suite builds git-init fixture repositories whose primary checkout
# it runs a copied bin/xo-test-run.sh in, and that runner refuses the primary
# under the marker. A case that verifies the refusal sets XO_TASK_ID itself.
unset XO_TASK_ID
...
#
# xo_test_tmproot <prefix> echoes a fresh temp dir and registers it for removal
# on EXIT/INT/TERM. A test file that needs extra teardown (e.g. killing a
# daemon) should define its own EXIT trap and call xo_test_cleanup from inside
# it so registered dirs are still removed.
#
# The call site is almost always `TMP_ROOT=$(xo_test_tmproot prefix)`, which
...
# subshell's - see `man bash` on `$$`), so xo_test_tmproot records the
...
XO_TEST_CLEANUP_DIRS=()
XO_TEST_CLEANUP_REGISTRY=$(mktemp "${TMPDIR:-/tmp}/.xo-test-cleanup.$$.XXXXXX") || return 1

xo_test_pid_identity() {
  local pid=$1
  XO_STATE_OVERRIDE="${TMPDIR:-/tmp}" bash -c \
    '. "$1"; xo_pid_identity "$2"' _ "$ROOT/bin/xo-wake-lib.sh" "$pid"
}

XO_TEST_OWNER_IDENTITY=$(xo_test_pid_identity "$$") || {
  rm -f "$XO_TEST_CLEANUP_REGISTRY"
  return 1
}
...
XO_TEST_PROCEVENT_REGISTRY=$(mktemp "${TMPDIR:-/tmp}/.xo-test-procevent.$$.XXXXXX") || return 1

xo_test_track_procevent_home() {  # <home> [claim-root]
  [ -n "${1:-}" ] || return 1
  printf '%s\t%s\n' "$1" "${2-}" >> "$XO_TEST_PROCEVENT_REGISTRY"
}

xo_test_reap_procevent_homes() {
  local home claim_root seen=$'\n'
  [ -f "$XO_TEST_PROCEVENT_REGISTRY" ] || return 0
  while IFS=$'\t' read -r home claim_root; do
    ...
    if [ -n "$claim_root" ]; then
      XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" XO_PROCEVENT_CLAIM_ROOT="$claim_root" \
        "$ROOT/bin/xo-procevent.sh" sweep-home >/dev/null 2>&1 || true
    else
      XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
        "$ROOT/bin/xo-procevent.sh" sweep-home >/dev/null 2>&1 || true
    fi
  done < "$XO_TEST_PROCEVENT_REGISTRY"
  rm -f "$XO_TEST_PROCEVENT_REGISTRY"
}
...
XO_TEST_STUB_MAX_BLOCK_SECONDS=${XO_TEST_STUB_MAX_BLOCK_SECONDS:-120}
export XO_TEST_STUB_MAX_BLOCK_SECONDS

xo_test_cleanup() {
  local d
  xo_test_reap_procevent_homes
  for d in "${XO_TEST_CLEANUP_DIRS[@]:-}"; do
    [ -n "$d" ] && rm -rf "$d"
  done
  if [ -f "$XO_TEST_CLEANUP_REGISTRY" ]; then
    while IFS= read -r d; do
      [ -n "$d" ] && rm -rf "$d"
    done < "$XO_TEST_CLEANUP_REGISTRY"
    rm -f "$XO_TEST_CLEANUP_REGISTRY"
  fi
}

xo_test_tmproot() {
  local prefix=${1:-xo-test} root tmp_base
  tmp_base=${TMPDIR:-/tmp}
  tmp_base=${tmp_base%/}
  root=$(mktemp -d "$tmp_base/${prefix}.XXXXXX") || return 1
  root=$(cd -P -- "$root" && pwd -P) || return 1
  if ! printf '%s\n%s\n' "$$" "$XO_TEST_OWNER_IDENTITY" > "$root/.xo-test-fixture" ||
    ! printf '%s\n' "$root" >> "$XO_TEST_CLEANUP_REGISTRY"; then
    rm -rf "$root"
    return 1
  fi
  printf '%s\n' "$root"
...
trap 'xo_test_cleanup; exit 130' INT
trap 'xo_test_cleanup; exit 143' TERM
trap 'xo_test_cleanup; exit 129' HUP
trap 'xo_test_cleanup; exit 131' QUIT

# xo_test_reap_orphans: best-effort sweep for fixture roots left behind by a
# prior run that was killed hard enough to skip the traps above (e.g. a
# SIGKILL timeout). Only removes directories carrying the .xo-test-fixture
# marker xo_test_tmproot writes, so it never touches unrelated xo-* tmp dirs
# from real (non-test) xo commands. The marker identifies the owning
# shell across PID reuse, so the same live owner always wins over the age
# fallback for dead or unowned roots.
XO_TEST_ORPHAN_MAX_AGE_SECONDS=${XO_TEST_ORPHAN_MAX_AGE_SECONDS:-3600}

xo_test_reap_orphans() {
  local marker dir mtime now owner_pid owner_identity current_identity
  now=$(date +%s)
  for marker in "${TMPDIR:-/tmp}"/xo-*/.xo-test-fixture; do
    [ -e "$marker" ] || continue
    ...
    case "$owner_pid" in
      '' | *[!0-9]*) ;;
      *)
        current_identity=$(xo_test_pid_identity "$owner_pid" 2>/dev/null) || current_identity=
    ...
    [ $((now - mtime)) -ge "$XO_TEST_ORPHAN_MAX_AGE_SECONDS" ] || continue
    dir=$(dirname "$marker")
    if [ -d "$dir" ] && [ ! -L "$dir" ]; then
...
if [ "${XO_TEST_SKIP_ORPHAN_REAP:-0}" != 1 ]; then
  xo_test_reap_orphans
fi

# --- live-capability gate ---------------------------------------------------
#
# xo_live_gate <policy> <vars> [tool ...]
#
...
#
# Setting any of those variables to 1 (or XO_LIVE=1, for every guard at once)
...
# actually ran. Setting one to 0 (or XO_LIVE=0) turns it off; a guard's own
# variable wins over XO_LIVE.
#
# Sourcing this library also exports XO_GATE_REFUSE_BYPASS=1, which is what
# lets a live guard drive the real xo-spawn/xo-send/xo-teardown from inside a
# no-mistakes gate worktree instead of being refused by
# bin/xo-gate-refuse-lib.sh.
...
xo_live_gate() {
  ...
  case "$policy" in
    default-on | opt-in) ;;
    *) fail "xo_live_gate: unknown policy '$policy' (expected default-on or opt-in)" ;;
  esac
  ...
  [ "${#var_list[@]}" -gt 0 ] || fail "xo_live_gate: at least one control variable is required"
  primary=${var_list[0]}
  ...
  if [ "$requested" -eq 0 ]; then
    ...
    case "${XO_LIVE:-}" in
      0)
        printf 'skip: live: disabled by XO_LIVE=0\n'
        exit 0
        ;;
      1) requested=1 ;;
      *)
...
# --- fakebin / PATH shims ---------------------------------------------------
#
# xo_fakebin <dir> creates <dir>/fakebin and echoes it; prepend it to PATH to
# shadow real tools with stubs. xo_fake_exit0 drops trivial exit-0 stubs for the
# named tools into a fakebin dir. xo_fake_crash_injector drops the shim a fake
# uses to crash the process under test deterministically. xo_fake_version_tool
...
# Drops an `xo-crash-inject <pid>` shim that a PATH fake calls to simulate a
...
xo_fake_crash_injector() {
  local fakebin=$1
  cat > "$fakebin/xo-crash-inject" <<'SH'
#!/usr/bin/env bash
set -u
target=${1:?xo-crash-inject: <pid> required}
case "$target" in
  ''|*[!0-9]*)
    echo "xo-crash-inject: '$target' is not a pid" >&2
    exit 1
    ;;
esac
...
echo "xo-crash-inject: pid $target still running 30s after SIGKILL" >&2
exit 1
SH
  chmod +x "$fakebin/xo-crash-inject"
}

# xo_fake_version_tool <fakebin> <tool> <override-env-var> <default-version>
...
# xo_touch_epoch <epoch> <path> [path...]: set each path's modification time to
...
xo_touch_epoch() {
  ...
  stamp=$(TZ=UTC0 date -d "@$epoch" +%Y%m%d%H%M.%S 2>/dev/null) \
    ...
    || fail "xo_touch_epoch: date(1) accepted neither -d @<epoch> nor -r <epoch>"
  TZ=UTC0 touch -t "$stamp" "$@" \
    || fail "xo_touch_epoch: touch -t $stamp failed for $*"
}

# --- deterministic git identity and fixtures --------------------------------

# xo_git_identity [name] [email]: export a fixed author/committer identity so
...
# xo_git_init_commit <dir>: create a git repo at <dir> with a README and one
# commit. Uses an inline identity so it works whether or not xo_git_identity was
...
xo_git_init_commit() {
  ...
  git -C "$dir" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm initial
}

# xo_git_add_origin <repo> <bare>: clone <repo> bare into <bare> and register it
...
# xo_git_worktree <repo> <worktree> <branch>: initialize <repo> with one commit
# and a local bare origin, then add a worktree on a fresh branch.
xo_git_worktree() {
  local repo=$1 worktree=$2 branch=$3
  xo_git_init_commit "$repo"
  xo_git_add_origin "$repo" "$repo.origin.git"
  git -C "$repo" worktree add --quiet -b "$branch" "$worktree"
}

# --- state/<id>.meta writers ------------------------------------------------

# xo_write_meta <file> <key=val> ...: write the given key=val lines to a meta
...
# xo_write_secondmate_meta <file> <home> [window] [projects] [harness]: write the
# standard kind=secondmate meta block used across the secondmate suites. Window
# defaults to xo:xo-<id>, projects defaults to alpha, and harness defaults
# to echo to match the common case.
xo_write_secondmate_meta() {
...
# xo_test_base_path_sans <base_path> <tool...>: returns the path to a single
...
# bare BASE_PATH.
xo_test_base_path_sans() {
  local base_path=$1 dir src entry name tool skip
  shift
  local tools=("$@")
  dir=$(xo_test_tmproot xo-base-path-sans) || return 1
  local dirs
