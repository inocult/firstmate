...
# xo_test_run_spawn through the real xo-spawn.sh; this file pins the shared
...
# tests/git-config-helpers.sh - the shared helpers, bin/xo-test-run.sh's
...
TMP_ROOT=$(xo_test_tmproot xo-test-fixtures)
...
test_git_config_isolation() (
  ...
  cp "$ROOT/bin/xo-test-run.sh" "$ROOT/bin/xo-timeout-lib.sh" "$dir/runner/bin/"
  cp "$ROOT/tests/git-config-helpers.sh" "$dir/runner/tests/"
  fakebin=$(xo_fakebin "$dir/standalone")
...
while [ "$#" -gt 0 ]; do
  if [ "$1" = -c ]; then
    git -C "$2" log -1 --format=%s > "${XO_TEST_STANDALONE_COMMIT:?}"
    exit 1
  fi
  shift
done
SH
  ...
  cat > "$dir/runner/tests/xo-test-run.test.sh" <<'SH'
#!/usr/bin/env bash
set -eu
repo=$(mktemp -d "${TMPDIR:-/tmp}/xo-git-runner.XXXXXX")
...
[ "$(GIT_CONFIG_GLOBAL="$XO_TEST_GIT_CONFIG" git config --global --get commit.gpgsign)" = true ]
SH
  chmod +x "$dir/runner/tests/xo-test-run.test.sh"
...
SH
  for jobs in 1 2; do
    for timeout in 0 30; do
      GIT_CONFIG_COUNT=1 GIT_CONFIG_KEY_0=fixture.input GIT_CONFIG_VALUE_0=preserved \
        XO_TEST_GIT_CONFIG="$dir/global" \
        "$dir/runner/bin/xo-test-run.sh" --jobs "$jobs" --per-script-timeout-secs "$timeout" \
        tests/xo-test-run.test.sh > "$dir/runner.log" 2>&1 \
        || fail "runner inherited global config (jobs=$jobs, timeout=$timeout): $(cat "$dir/runner.log")"
      assert_grep 'XO_TEST_SUMMARY total=1 failed=0 skipped_gate=0' "$dir/runner.log" \
        "runner did not execute the Git fixture"
    done
  done
  rc=0
  XO_SESSIONSTART_INSTRUCTION_REFRESH_LIVE_E2E=1 XO_SESSIONSTART_INSTRUCTION_REFRESH_REF=HEAD \
    XO_SESSIONSTART_INSTRUCTION_REFRESH_EXPECT=updated \
    XO_TEST_STANDALONE_COMMIT="$dir/global-standalone-commit" PATH="$fakebin:$PATH" \
    bash "$ROOT/tests/xo-sessionstart-instruction-refresh-live-e2e.test.sh" \
  ...
  bash "$ROOT/tests/xo-gitignore-config.test.sh" > "$dir/gitignore.log" 2>&1 \
...
test_touch_epoch_preserves_repeated_dst_hour() {
  ...
  for epoch in 1761438600 1761442200; do
    xo_touch_epoch "$epoch" "$TMP_ROOT/epoch-one" "$TMP_ROOT/epoch two"
    for path in "$TMP_ROOT/epoch-one" "$TMP_ROOT/epoch two"; do
      ...
      [ "$actual" = "$epoch" ] \
        || fail "xo_touch_epoch should preserve epoch $epoch, got $actual"
    done
  done
  pass "xo_touch_epoch preserves both epochs in the repeated DST hour"
}

test_no_mistakes_version_constant() {
  local fakebin out
  fakebin=$(xo_fakebin "$TMP_ROOT/nm")
  xo_test_fake_no_mistakes "$fakebin"
  out=$("$fakebin/no-mistakes" --version)
  [ "$out" = "$XO_TEST_NO_MISTAKES_FAKE_VERSION" ] || \
    fail "fake no-mistakes --version should be the shared constant, got '$out'"
  out=$(XO_FAKE_NO_MISTAKES_VERSION="$XO_TEST_NO_MISTAKES_FAKE_VERSION_TS" \
    "$fakebin/no-mistakes" --version)
  [ "$out" = "$XO_TEST_NO_MISTAKES_FAKE_VERSION_TS" ] || \
    fail "timestamped banner override should round-trip, got '$out'"
  case "$out" in
    "$XO_TEST_NO_MISTAKES_FAKE_VERSION "*) ;;
    *) fail "timestamped banner '$out' is not the shared constant plus a suffix" ;;
  esac
  out=$(XO_FAKE_NO_MISTAKES_VERSION='no-mistakes version v9.9.9 (fake)' \
    "$fakebin/no-mistakes" --version)
  [ "$out" = 'no-mistakes version v9.9.9 (fake)' ] || \
    fail "XO_FAKE_NO_MISTAKES_VERSION should override the default banner, got '$out'"
...
test_fake_gh_and_gh_axi() {
  local fakebin out
  fakebin=$(xo_fakebin "$TMP_ROOT/gh")
  ...
  [ "$out" = "$XO_TEST_GH_AXI_VERSION" ] || \
    fail "fake gh-axi --version should be $XO_TEST_GH_AXI_VERSION, got '$out'"
  out=$(XO_FAKE_GH_AXI_VERSION=0.9.9 "$fakebin/gh-axi" --version)
  [ "$out" = 0.9.9 ] || fail "XO_FAKE_GH_AXI_VERSION should override, got '$out'"
...
test_spawn_tmux_and_fakebin() {
  ...
  : > "$log"
  out=$(XO_FAKE_PANE_PATH=/tmp/wt "$fakebin/tmux" display-message -p '#{pane_current_path}')
  [ "$out" = /tmp/wt ] || fail "spawn tmux pane path should be XO_FAKE_PANE_PATH, got '$out'"
  out=$(unset XO_FAKE_PANE_PATH; "$fakebin/tmux" display-message -p '#{pane_current_path}')
  [ -z "$out" ] || fail "spawn tmux pane path should default to empty, got '$out'"
  out=$("$fakebin/tmux" display-message -p '#S')
  [ "$out" = xo ] || fail "spawn tmux session name should be xo, got '$out'"
  XO_FAKE_LAUNCH_LOG="$log" "$fakebin/tmux" send-keys -t @w -l 'codex --yolo'
...
test_send_stubs_and_ssh() {
  ...
  : > "$log"
  xo_test_fake_ssh "$fakebin"
  XO_SEND_LOG="$log" "$fakebin/tmux" send-keys -t sess:w -l 'hello steer'
  ...
  printf 'ignored\n' | XO_SSH_LOG="$ssh_log" "$fakebin/fake-ssh" host -- cmd
  assert_grep 'host -- cmd' "$ssh_log" "fake ssh did not record argv"
  XO_FAKE_SSH_RC=7 "$fakebin/fake-ssh" x < /dev/null
  expect_code 7 $? "fake ssh should honor XO_FAKE_SSH_RC"
...
test_spawn_home_layout() {
  ...
  xo_test_spawn_home "$home" claude
  xo_test_spawn_brief "$home" t1 'do the thing'
