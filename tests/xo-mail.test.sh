#!/usr/bin/env bash
# Behavior tests for bin/xo-mail.sh.
#
# xo-mail.sh is a network mail client, so these tests exercise only the paths
...
# through the executable public interface of bin/xo-mail.sh and never assert
...
MAIL="$ROOT/bin/xo-mail.sh"
TMP_ROOT=$(xo_test_tmproot xo-mail)
HOME_DIR="$TMP_ROOT/home"
mkdir -p "$HOME_DIR"

test_missing_secret_fails_cleanly() {
  local out rc
  env -u XO_MAIL_USER -u XO_MAIL_PASS -u XO_IMAP_HOST -u XO_SMTP_HOST \
    XO_HOME="$HOME_DIR" "$MAIL" status >"$TMP_ROOT/out" 2>"$TMP_ROOT/err"
  rc=$?
  expect_code 1 "$rc" "status without configuration must fail"
  out=$(cat "$TMP_ROOT/err")
  assert_contains "$out" "XO_MAIL_USER" "missing-config error names the missing variable"
  assert_contains "$out" "XO_MAIL_*" "missing-config error names the configuration family"
  assert_not_contains "$out" "test-pass" "missing-config error never leaks a secret"
  pass "xo-mail: missing required configuration fails cleanly naming the variable"
}
...
XO_MAIL_USER=fromfile@example.com
XO_MAIL_PASS=filepass
XO_IMAP_HOST=imap.file.invalid
XO_SMTP_HOST=smtp.file.invalid
EOF
...
test_status_without_network() {
  local out rc
  out=$(XO_MAIL_USER="test@example.com" XO_MAIL_PASS="test-pass" \
    XO_IMAP_HOST="imap.test.invalid" XO_SMTP_HOST="smtp.test.invalid" \
    XO_HOME="$HOME_DIR" "$MAIL" status 2>&1)
  rc=$?
  ...
  pass "xo-mail: status succeeds without network and prints configuration"
}
...
test_help_plumbing() {
  ...
  out=$(XO_MAIL_USER="test@example.com" XO_MAIL_PASS="test-pass" \
    XO_IMAP_HOST="imap.test.invalid" XO_SMTP_HOST="smtp.test.invalid" \
    XO_HOME="$HOME_DIR" "$MAIL" --help 2>&1)
  rc=$?
  ...
  pass "xo-mail: --help prints usage for every subcommand"
}
...
test_unknown_subcommand_prints_usage() {
  ...
  out=$(XO_MAIL_USER="test@example.com" XO_MAIL_PASS="test-pass" \
    XO_IMAP_HOST="imap.test.invalid" XO_SMTP_HOST="smtp.test.invalid" \
    XO_HOME="$HOME_DIR" "$MAIL" bogus 2>&1)
  rc=$?
  ...
  pass "xo-mail: unknown subcommand prints usage and exits non-zero"
}
...
SH
  ...
  export XO_MAIL_TEST_STDIN_FILE="$stdin_file"
  local out rc=0
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=h XO_SMTP_HOST=h \
    XO_HOME="$HOME_DIR" PATH="$fakebin:$PATH" \
  ...
  pass "xo-mail: send passes body not empty through stdin"
}
...
test_poll_error_propagates() {
  ...
  mkdir -p "$homedir_bin"
  [ -e "$homedir_bin/xo-wake-lib.sh" ] || ln -s "$ROOT/bin/xo-wake-lib.sh" "$homedir_bin/xo-wake-lib.sh"

  # Fake python3 that exits with an error (simulating IMAP failure).
  cat > "$fakebin/python3" <<'SH'
#!/usr/bin/env bash
echo "xo-mail poll error: connection refused" >&2
exit 1
SH
  chmod +x "$fakebin/python3"

  local out rc=0
  out=$(env -u XO_MAIL_USER -u XO_MAIL_PASS -u XO_IMAP_HOST -u XO_SMTP_HOST \
    XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$HOME_DIR" PATH="$fakebin:$PATH" \
    "$MAIL" poll 2>&1) || rc=$?
  expect_code 1 "$rc" "poll must propagate python3 errors"
  assert_contains "$out" "connection refused" "poll error message is visible"
  pass "xo-mail: poll propagates errors instead of swallowing them"
}
...
SH
  ...
  local out rc=0
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$HOME_DIR" PATH="$fakebin:$PATH" \
    "$MAIL" poll 2>&1) || rc=$?
  ...
  pass "xo-mail: poll surfaces each new uid exactly once"
}
...
SH
  ...
  pass "xo-mail: generation change prevents a reused uid from being suppressed"
}
...
SH
  ...
  pass "xo-mail: poll heals a wake whose cursor record was interrupted"
}
...
SH
  ...
  pass "xo-mail: the poll lock serializes overlapping polls so mail wakes exactly once"
}
...
SH
  ...
  pass "xo-mail: journal recovers a wake the drain already acknowledged"
}
...
SH
  ...
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$interrupted_home" PATH="$fakebin:$PATH" \
    "$MAIL" poll 2>&1) || rc=$?
  expect_code 0 "$rc" "first poll must succeed"
  ...
  pass "xo-mail: a poll interrupted before evidence writes does not duplicate on recovery"
}
...
SH
  ...
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$acked_home" PATH="$fakebin:$PATH" \
    "$MAIL" poll 2>&1) || rc=$?
  expect_code 0 "$rc" "first poll must succeed"
  ...
  pass "xo-mail: an acknowledged wake whose cursor record was lost is recovered from the journal"
}
...
SH
  ...
  pass "xo-mail: a legacy generation-less wake never marks a reused uid surfaced"
}
...
test_poll_missing_wake_lib_does_not_suppress() {
  ...
  # Hide the script-relative wake library: poll sources xo-wake-lib.sh from
  # next to xo-mail.sh, not from $XO_HOME/bin. Copy only the plane scripts.
  cp "$ROOT/bin/xo-mail.sh" "$miss_bin/xo-mail.sh"
  cp "$ROOT/bin/xo-mail.py" "$miss_bin/xo-mail.py"
  chmod +x "$miss_bin/xo-mail.sh"
...
SH
  ...
  local out rc=0
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$miss_home" PATH="$fakebin:$PATH" \
    "$miss_bin/xo-mail.sh" poll 2>&1) || rc=$?
  expect_code 1 "$rc" "poll must stop when the wake library is missing"
  assert_contains "$out" "xo-wake-lib.sh missing" "missing-lib error names the wake library"
  assert_not_contains "$(cat "$miss_home/state/.mail-seen" 2>/dev/null)" "33" "a failed wake must never be committed to the cursor"
  pass "xo-mail: a missing wake library fails the poll instead of suppressing mail"
}
...
SH
  ...
  local out rc=0
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$roll_home" PATH="$fakebin:$PATH" \
    "$MAIL" poll 2>&1) || rc=$?
  ...
  pass "xo-mail: a wake with no durable record is rolled back, not left ackable"
}
...
test_poll_rollback_failure_never_leaves_unrecorded_ackable_wake() {
  ...
  [ -e "$roll_home/bin/xo-wake-lib.sh" ] || ln -s "$ROOT/bin/xo-wake-lib.sh" "$roll_home/bin/xo-wake-lib.sh"
...
SH
  ...
  pass "xo-mail: a rollback failure never releases a wake the drain could acknowledge without a durable record"
}
...
import os, sys
os.environ.update({
    'XO_MAIL_USER': 't', 'XO_MAIL_PASS': 'p',
    'XO_IMAP_HOST': 'imap.test', 'XO_IMAP_PORT': '993',
    'XO_SMTP_HOST': 'smtp.test', 'XO_SMTP_PORT': '465',
    'XO_MAIL_CURSOR': sys.argv[1],
    'XO_MAIL_RETRY': sys.argv[2],
    'XO_MAIL_POLL_MAX_WAKES': '4',
})
class FakeConn:
...
spec = importlib.util.spec_from_file_location('xo_mail', sys.argv[3])
...
PYEOF
  ...
  out=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" "$ROOT/bin/xo-mail.py" 2>&1)
  assert_contains "$out" $'41\t\ta@b.c\tgood\tretry' "recovered metadata surfaces despite the new-mail flood"
  pass "xo-mail: the reserved retry budget survives a new-mail flood"
}
...
os.environ.update({
    ...
    'XO_MAIL_CURSOR': sys.argv[1],
    'XO_MAIL_RETRY': sys.argv[2],
    'XO_MAIL_TURN': sys.argv[3],
    'XO_MAIL_POLL_MAX_WAKES': '1',
})
class FakeConn:
    untagged_responses = {'UIDVALIDITY': [b'90009']}
    def __init__(self, *a, **k):
...
imaplib.IMAP4_SSL = lambda *a, **k: FakeConn()
import importlib.util
spec = importlib.util.spec_from_file_location('xo_mail', sys.argv[4])
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)
sys.exit(mod.cmd_poll_list())
PYEOF
  ...
  out1=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-turn" "$ROOT/bin/xo-mail.py" 2>&1) || rc1=$?
  ...
  out2=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-turn" "$ROOT/bin/xo-mail.py" 2>&1) || rc2=$?
  ...
  pass "xo-mail: a single contended slot alternates between new mail and retry recovery"
}
...
imaplib.IMAP4_SSL = lambda *a, **k: FakeConn()
import importlib.util
spec = importlib.util.spec_from_file_location('xo_mail', sys.argv[5])
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)
sys.exit(mod.cmd_poll_list())
PYEOF
  ...
  out1=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-turn" "$HOME_DIR/state/.mail-retry-pos" "$ROOT/bin/xo-mail.py" 2>&1) || rc1=$?
  expect_code 0 "$rc1" "first contended poll must succeed"
  assert_contains "$out1" $'100\t\ta@b.c\tgood\tok' "the first contended slot surfaces new mail"
  ...
  out2=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-turn" "$HOME_DIR/state/.mail-retry-pos" "$ROOT/bin/xo-mail.py" 2>&1) || rc2=$?
  expect_code 0 "$rc2" "second contended poll must succeed"
  ...
  pass "xo-mail: a zero retry budget leaves the retry-scan position unchanged"
}
...
PYEOF
  ...
  pass "xo-mail: a cap of one never suppresses new mail while retries exist"
}
...
test_poll_restores_retry_when_recovered_wake_cannot_append() {
  ...
  [ -e "$HOME_DIR/bin/xo-wake-lib.sh" ] || ln -s "$ROOT/bin/xo-wake-lib.sh" "$HOME_DIR/bin/xo-wake-lib.sh"
  cat > "$fakebin/python3" <<'SH'
#!/usr/bin/env bash
printf 'uidvalidity\t90009\n'
...
SH
  ...
  pass "xo-mail: a recovered wake that cannot append restores the retry instead of stranding the metadata"
}
...
SH
  ...
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$test_home" PATH="$fakebin:$PATH" \
    "$MAIL" poll 2>&1) || rc=$?
  ...
  pass "xo-mail: a death between retry remove and wake publish cannot strand recovered metadata"
}
...
test_poll_fails_closed_when_poll_list_fails() {
  ...
  cat > "$fakebin/python3" <<'SH'
#!/usr/bin/env bash
if [ -f "$XO_POLL_FAIL_MARKER" ]; then
  echo 'xo-mail poll error: simulated failure' >&2
  exit 1
fi
printf 'uidvalidity\t90009\n'
...
SH
  ...
  out=$(XO_POLL_FAIL_MARKER="$HOME_DIR/fail.marker" XO_MAIL_USER=test XO_MAIL_PASS=pass \
    XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$HOME_DIR" PATH="$fakebin:$PATH" \
    "$MAIL" poll 2>&1) || rc=$?
  ...
  pass "xo-mail: a failed poll_list fails closed, wakes nothing, and releases the lock"
}
...
SH
  ...
  pass "xo-mail: a failed retry clear fails the poll; the published wake stays and the retry entry remains"
}
...
SH
  ...
  pass "xo-mail: a failed stale-retry clear fails the poll instead of silently leaving a duplicate-wake entry"
}
...
#!/usr/bin/env bash
printf 'uidvalidity\t90009\n'
printf '77\t\t(no header)\tunfetchable header - see xo-mail read\tdegraded\n'
SH
  chmod +x "$fakebin/python3"
  printf 'uidvalidity=90009\n' > "$HOME_DIR/state/.mail-seen"
  : > "$HOME_DIR/state/.mail-retry"
  ...
  pass "xo-mail: a failed retry write fails the poll instead of losing recovery"
}
...
SH
  ...
  pass "xo-mail: a journal write failure never cursor-records a suppressed mail"
}
...
PYEOF
  ...
  out=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$ROOT/bin/xo-mail.py" 2>&1) || rc=$?
  expect_code 0 "$rc" "poll must succeed"
  assert_contains "$out" $'90\t\t(no header)\tunfetchable header - see xo-mail read\tdegraded' "a retry uid the cursor never recorded is re-surfaced degraded"
  pass "xo-mail: a retry uid whose degraded wake never recorded is re-surfaced, not dropped"
}
...
imaplib.IMAP4_SSL = lambda *a, **k: FakeConn()
import importlib.util
spec = importlib.util.spec_from_file_location('xo_mail', sys.argv[2])
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)
sys.exit(mod.cmd_poll_list())
PYEOF
  printf 'uidvalidity=90009\n' > "$HOME_DIR/state/.mail-seen"
  out=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$ROOT/bin/xo-mail.py" 2>&1) || rc=$?
  expect_code 0 "$rc" "a raised fetch must not abort the scan"
  assert_contains "$out" $'1\t\t(no header)\tunfetchable header - see xo-mail read\tdegraded' "the raising uid is surfaced degraded"
  assert_contains "$out" $'2\t\ta@b.c\tgood\tok' "the scan advances past the raising uid"
  pass "xo-mail: a raised FETCH surfaces that uid degraded and advances"
}
...
PYEOF
  ...
  out1=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "71,72,73,74,75,76,77,78,79,80,81" "$ROOT/bin/xo-mail.py" "$HOME_DIR/state/.mail-retry-pos" 2>&1) || rc1=$?
  ...
  out2=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "71,72,73,74,75,76,77,78,79,80,81" "$ROOT/bin/xo-mail.py" "$HOME_DIR/state/.mail-retry-pos" 2>&1) || rc2=$?
  ...
  pass "xo-mail: the retry-scan cursor advances past persistent failures"
}
...
PYEOF
  ...
  out=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-retry-pos" "$ROOT/bin/xo-mail.py" 2>&1) || rc=$?
  ...
  pass "xo-mail: retry-scan position does not advance past an unpublished wake"
}
...
PYEOF
  ...
  out=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-retry-pos" "$ROOT/bin/xo-mail.py" "101,102,103" 2>&1) || rc=$?
  ...
  pass "xo-mail: retry budget advances the scan by examined candidates, not the window"
}
...
PYEOF
  ...
  out1=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-retry-pos" "$ROOT/bin/xo-mail.py" 2>&1) || rc1=$?
  ...
  out2=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-retry-pos" "$ROOT/bin/xo-mail.py" 2>&1) || rc2=$?
  ...
  pass "xo-mail: a retry window of unseen uids never stalls the cursor"
}
...
PYEOF
  ...
  pass "xo-mail: an unfetchable retry window still advances while new mail fills out"
}
...
PYEOF
  ...
  pass "xo-mail: an unseen retry window still advances while new mail fills out"
}
...
PYEOF
  ...
  pass "xo-mail: hung logout cannot advance the retry-scan position"
}
...
PYEOF
  ...
  out=$(python3 "$harness" "$marker" "$HOME_DIR/state/.mail-seen" "$ROOT/bin/xo-mail.py" 2>&1) || rc=$?
  ...
  pass "xo-mail: poll_list logs out when IMAP work fails after login"
}
...
PYEOF
  ...
  pass "xo-mail: retry-scan position is not saved when a row is emitted"
}
...
PYEOF
  ...
  pass "xo-mail: small retry window rotates past an unfetchable prefix"
}
...
PYEOF
  ...
  out=$(python3 "$harness" "$HOME_DIR/state/.mail-seen" "$HOME_DIR/state/.mail-retry" \
    "$HOME_DIR/state/.mail-turn" "$ROOT/bin/xo-mail.py" 2>&1) || rc=$?
  ...
  pass "xo-mail: cap-one turn is not saved before emit/flush"
}
...
PYEOF
  ...
  pass "xo-mail: a failed retry-pos write does not spend the cap-one retry turn"
}
...
PYEOF
  ...
  assert_contains "$out" $'1\t\t(no header)\tunfetchable header - see xo-mail read\tdegraded' \
  ...
  pass "xo-mail: an unfetchable uid is surfaced degraded and cannot starve later mail"
}
...
class FakeConn:
    ...
    def uid(self, cmd, *args):
        if cmd == 'search':
            return ('OK', [b'41'])
        if cmd == 'fetch':
            n = int(open(os.environ['XO_MAIL_TEST_FETCH_COUNT']).read() or '0')
            if n < 3:
                return ('NO', None)
...
imaplib.IMAP4_SSL = lambda *a, **k: FakeConn()
spec = importlib.util.spec_from_file_location('xo_mail', sys.argv[1])
mod = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mod)
if len(sys.argv) > 2 and sys.argv[2] == 'poll_list':
    path = os.environ['XO_MAIL_TEST_FETCH_COUNT']
...
EOF
  ...
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$retry_home" PATH="$fakebin:$PATH" \
    XO_MAIL_TEST_FETCH_COUNT="$control" \
    "$MAIL" poll 2>&1) || rc=$?
  ...
  pass "xo-mail: a transient fetch failure recovers real metadata on a later poll"
}
...
SH
  ...
  pass "xo-mail: the journal survives when the heal cannot commit a uid"
}
...
SH
  ...
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$heal_home" PATH="$fakebin:$PATH" \
    "$MAIL" poll 2>&1) || rc=$?
  ...
  pass "xo-mail: heal failure with unseen mail fails the poll instead of re-waking"
}
...
test_body_preview_falls_back_from_empty_plain() {
  ...
  cat > "$harness" <<'PYEOF'
import os, sys
os.environ.update({'XO_MAIL_USER':'t','XO_MAIL_PASS':'p','XO_IMAP_HOST':'h','XO_IMAP_PORT':'993','XO_SMTP_HOST':'s','XO_SMTP_PORT':'465'})
...
PYEOF
  out=$(python3 "$harness" "$ROOT/bin/xo-mail.py")
  assert_contains "$out" "Hello" "empty plain-text alternative falls back to the html preview"
  pass "xo-mail: an empty plain-text alternative falls back to the html preview"
}
...
PYEOF
  out=$(python3 "$harness" "$ROOT/bin/xo-mail.py" 2>&1) || rc=$?
  ...
  pass "xo-mail: read does not silently hide an unfetchable uid"
}
...
PYEOF
  ...
  pass "xo-mail: read tolerates a None payload without crashing"
}

test_invalid_port_fails_cleanly() {
  local out rc=0
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=h XO_SMTP_HOST=h \
    XO_IMAP_PORT=abc XO_HOME="$HOME_DIR" "$MAIL" status 2>&1) || rc=$?
  expect_code 1 "$rc" "a non-numeric IMAP port must fail"
  assert_contains "$out" "XO_IMAP_PORT" "invalid IMAP port names the variable"
  assert_not_contains "$out" "ValueError" "invalid port must not leak a python traceback"
  rc=0
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=h XO_SMTP_HOST=h \
    XO_SMTP_PORT=abc XO_HOME="$HOME_DIR" "$MAIL" status 2>&1) || rc=$?
  expect_code 1 "$rc" "a non-numeric SMTP port must fail"
  assert_contains "$out" "XO_SMTP_PORT" "invalid SMTP port names the variable"
  pass "xo-mail: a non-numeric port fails cleanly in bash"
}
...
SH
  ...
  out=$(XO_MAIL_USER=test XO_MAIL_PASS=pass XO_IMAP_HOST=imap.test XO_SMTP_HOST=smtp.test \
    XO_HOME="$HOME_DIR" PATH="$fakebin:$PATH" XO_MAIL_POLL_MAX_WAKES=2 \
    "$MAIL" poll 2>&1) || rc=$?
  ...
  pass "xo-mail: per-poll wake cap bounds the durable queue without missing mail"
}
...
EOF
  ...
  pass "xo-mail: poll sanitizes tabs and newlines in header fields"
}
...
test_poll_bounded_fetch_progresses_large_backlog() {
  ...
  # Fake python3 emulating the bounded poll_list: read XO_MAIL_CURSOR, return
  ...
  cat > "$fakebin/python3" <<'SH'
#!/usr/bin/env bash
cursor="$XO_MAIL_CURSOR"
cap="${XO_MAIL_POLL_MAX_WAKES:-20}"
seen=""
...
SH
  ...
  pass "xo-mail: bounded fetch makes progress through a large backlog without re-surfacing mail"
}
