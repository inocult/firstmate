...
#
# bin/xo-cd-command-policy.mjs is the single owner of the block/allow decision;
# it reuses the shell classifier owned by bin/xo-arm-command-policy.mjs.
# bin/xo-cd-pretool-check.sh is the stable transport: it scopes the guard to the
...
TMP_ROOT=$(xo_test_tmproot xo-cd-pretool-check)

# A primary-shaped checkout: plain (non-worktree) git repo, AGENTS.md, bin/ with
# the transport plus both policy files (xo-cd-command-policy.mjs imports the
# shared classifier from xo-arm-command-policy.mjs). This is what the transport's
# scoping treats as the real primary xo checkout.
install_cd_scripts() {
  local dir=$1
  mkdir -p "$dir/bin"
  cp "$ROOT/bin/xo-cd-pretool-check.sh" "$dir/bin/xo-cd-pretool-check.sh"
  ...
  chmod +x "$dir/bin/xo-cd-pretool-check.sh" "$dir/bin/xo-cd-command-policy.mjs"
}
...
# Same shape as primary plus the .xo-secondmate-home marker: a secondmate's own
...
# A genuine linked git worktree - the shape bin/xo-spawn.sh hands crewmate/scout
# tasks. git-dir and git-common-dir differ, so the guard must be inert.
make_child_worktree_fixture() {
  local base=$1 dir=$2
  xo_git_worktree "$base" "$dir" xo/cd-guard-test-branch
  : > "$dir/AGENTS.md"
  install_cd_scripts "$dir"
  printf '%s\n' "$dir"
}
...
CHECK="$PRIMARY/bin/xo-cd-pretool-check.sh"
...
MATRIX_TMP=$(mktemp -d "${TMPDIR:-/tmp}/xo-cd-policy-matrix.XXXXXX")
...
test_fires_in_secondmate_home() {
  ...
  out=$("$dir/bin/xo-cd-pretool-check.sh" --claude --command 'cd projects/foo' 2>&1); rc=$?
...
test_inert_when_not_xo_repo() {
  ...
  out=$("$dir/bin/xo-cd-pretool-check.sh" --claude --command 'cd projects/foo' 2>&1); rc=$?
  expect_code 0 "$rc" "cd-guard must be inert without AGENTS.md (not an xo checkout)"
  [ -z "$out" ] || fail "cd-guard produced output outside an xo checkout: $out"
  pass "cd-guard: inert in a non-xo repo (no AGENTS.md)"
}
...
test_e2e_cwd_leak_regression() {
  ...
  # `cd projects/clone` makes the next xo-owned backlog write land in the
...
test_fail_open_missing_node() {
  ...
  fakebin=$(xo_fakebin "$TMP_ROOT/nonode")
...
test_fail_open_missing_jq_on_stdin() {
  local fakebin tool tool_path out rc
  fakebin=$(xo_fakebin "$TMP_ROOT/nojq")
...
test_prefilter_skips_node_without_cd_substring() {
  ...
  make_primary_fixture "$dir" >/dev/null
  fakebin=$(xo_fakebin "$TMP_ROOT/prefilter-fake")
...
EOF
  ...
  out=$(PATH="$fakebin" "$dir/bin/xo-cd-pretool-check.sh" --command 'git status' 2>&1); rc=$?
...
test_policy_cli_direct() {
  local policy
  policy="$ROOT/bin/xo-cd-command-policy.mjs"
  ...
  pass "cd-guard: xo-cd-command-policy.mjs CLI honors the deny/allow output contract"
}

# --- per-harness wiring -----------------------------------------------------

# Delegated to bin/xo-lint.sh, the single owner of the lint definition including
...
test_scripts_are_shellcheck_clean() {
  ...
  out=$("$ROOT/bin/xo-lint.sh" "$ROOT/bin/xo-cd-pretool-check.sh" 2>&1) \
    || fail "bin/xo-cd-pretool-check.sh is not lint-clean under the pinned definition: $out"
  pass "bin/xo-cd-pretool-check.sh is clean under bin/xo-lint.sh"
}
