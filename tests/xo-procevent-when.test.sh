...
# runner (bin/xo-procevent-when.sh).
#
...
TMP_ROOT=$(xo_test_tmproot xo-procevent-when-tests)
export XO_PROCEVENT_CLAIM_ROOT="$TMP_ROOT/claims"

pe()   { XO_HOME="$1" "$ROOT/bin/xo-procevent.sh" "${@:2}"; }
when() { XO_HOME="$1" "$ROOT/bin/xo-procevent-when.sh" "${@:2}"; }
...
new_home() { mkdir -p "$1/state"; xo_test_track_procevent_home "$1"; }
...
mode=$(PATH="${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/xo-pr-lib.sh"; xo_pr_file_mode "$2"' _ "$ROOT" "$H/state/when/when-arm-test.spec")
...
pass "a repeatedly erroring condition wakes xo instead of firing"
...
SH
chmod +x "$NOISY"
XO_WHEN_OUTPUT_TAIL_BYTES=128 when "$H" arm bounded-output --stable 1 \
  --condition true --action "$NOISY" "$NOISY_READY" >/dev/null
XO_WHEN_OUTPUT_TAIL_BYTES=128 pe "$H" reconcile >/dev/null
...
printf 'all xo-procevent-when tests passed\n'
