...
HERDR_LAB_HELPER=${HERDR_LAB_HELPER:-$ROOT/bin/xo-herdr-lab.sh}
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/xo-herdr-presentation.XXXXXX")
...
export XO_BACKEND_HERDR_WORKSPACE_MOVER="$FAKEBIN/herdr-workspace-mover"
...
HERDR_LAB_SESSION=$(PATH="$HERDR_ORIGINAL_PATH" \
  "$HERDR_LAB_HELPER" name xo-herdr-presentation-projection)
...
# defect is demonstrated by tests/xo-backend-herdr-focus-flash-e2e.test.sh);
...
spawn_task() {  # <id> <home> <project>
  ...
  XO_GATE_REFUSE_BYPASS=1 XO_SPAWN_NO_GUARD=1 XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/xo-spawn.sh" "$id" "$project" "sh -c 'while :; do sleep 60; done'" --mode no-mistakes --yolo off --backend herdr
}
...
spawn_secondmate_task() {
  local id=$1 home=$2
  XO_GATE_REFUSE_BYPASS=1 XO_SPAWN_NO_GUARD=1 XO_HOME="$HOME_DIR" XO_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/xo-spawn.sh" "$id" "$home" "sh -c 'while :; do sleep 60; done'" --secondmate --backend herdr
}

teardown_task() {  # <id> <home>
  local id=$1 home=$2
  XO_GATE_REFUSE_BYPASS=1 XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/xo-teardown.sh" "$id" --force
}
...
session_presentation_lock_path() {
  PATH="$FAKEBIN:$PATH" HERDR_SESSION="$HERDR_LAB_SESSION" bash -c '
    ...
    xo_backend_herdr_presentation_session_lock_path "$1"
...
mkdir -p "$HOME_DIR/state" "$HOME_DIR/config" \
  ...
  "$HOME_DIR/data/order-fail" "$HOME_DIR/data/xo-hibit-resume-r1" \
...
write_ship_brief "$HOME_DIR" xo-hibit-resume-r1 'Hi Bit-style projection restart fixture.'
...
# Keep one ordinary primary task live so the durable xo workspace is
...
XO_WSID=$(grep '^herdr_workspace_id=' "$ANCHOR_META" | cut -d= -f2-)
[ -n "$XO_WSID" ] || fail "anchor metadata did not record the xo workspace"
...
if [ "$FLOOR_VERDICT" = 0 ]; then
  ...
  [ -n "$DEFAULT_ON_WSID" ] && [ "$DEFAULT_ON_WSID" != "$XO_WSID" ] \
    || fail "an unconfigured home reused the flat xo workspace instead of projecting"
  ...
  [ "$DEFAULT_ON_WSID" = "$XO_WSID" ] \
    || fail "an unconfigured home did not land in the flat xo workspace on below-floor herdr $FLOOR_VERSION (got '${DEFAULT_ON_WSID:-<empty>}')"
...
printf '%s' "$PROJECTED_TABS" | jq -e --arg tab "$PROJECTED_TAB" \
  '.result.tabs[0].tab_id == $tab and .result.tabs[0].label == "xo-shape"' >/dev/null 2>&1 \
  || fail "projected workspace's only tab was not the normal xo-shape task tab"
...
[ "$LOCK_CONTENTION_WSID" = "$XO_WSID" ] \
  || fail "bounded lock contention did not use the ordinary flat xo workspace"
...
EXPECTED_LABELS=$(printf 'xo\n%s\n%s\n2ndmate-alpha\n2ndmate-bravo' "$PROJECTED_LABEL" "$CREATED_LABELS")
ACTUAL_LABELS=$(printf '%s' "$ORDER_LIST" | jq -r '.result.workspaces[].label')
[ "$ACTUAL_LABELS" = "$EXPECTED_LABELS" ] || fail "workspace order was not xo, stable primary block, secondmates: $ACTUAL_LABELS"
PRIMARY_IDS=$(printf '%s' "$ORDER_LIST" | jq -r '
  .result.workspaces[]
  | select((.label | startswith("└ ")) or (.label | startswith("xo/")))
  | .workspace_id
')
...
XO_BACKEND_HERDR_WORKSPACE_MOVER="$FAIL_MOVER" \
...
for ROUND in 1 2 3; do
  ...
  WAVE_EXPECTED=$(printf 'xo\n%s\n2ndmate-alpha\n2ndmate-bravo' "$WAVE_LABELS")
  WAVE_ACTUAL=$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "xo" or (.label | startswith("└ ")) or (.label | startswith("2ndmate-"))) | .label')
...
# ------------------------------------------------------------------
# Multi-home topology: real secondmate XO_HOME spawn paths, inheritance,
...
printf 'alpha\n' > "$SECOND_HOME_A/.xo-secondmate-home"
printf 'bravo\n' > "$SECOND_HOME_B/.xo-secondmate-home"
touch "$SECOND_HOME_A/state/.last-watcher-beat" "$SECOND_HOME_B/state/.last-watcher-beat"
# Ensure the secondmate homes look like gitignored xo homes so inheritance
...
git -C "$SECOND_HOME_A" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm init
git -C "$SECOND_HOME_B" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm init
mkdir -p "$SECOND_HOME_A/bin"
printf '# XO secondmate fixture\n' > "$SECOND_HOME_A/AGENTS.md"
...
case "$PCW_LABEL" in $'└ pcw · p:'*|xo) ;; *) fail "cross-home primary label wrong: $PCW_LABEL" ;; esac
...
# Exercise both the leading xo- identity style seen in Hi Bit work and the
# project-name identity style used by Wheelhouse work.
for RESTART_ID in xo-hibit-resume-r1 wheelhouse-healing-r1; do
  ...
  EXPECTED_CONCISE=${RESTART_ID#xo-}
  ...
  if [ "$RESTART_ID" = xo-hibit-resume-r1 ]; then
...
LEGACY_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "xo/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" --no-focus) \
...
FLAT_TAB_OUT=$(lab tab create --workspace "$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "2ndmate-alpha") | .workspace_id' | head -1)" --cwd "$PROJECT_DIR" --label xo-flat-legacy-tab --no-focus) \
...
[ "$(lab workspace get "$LEGACY_WSID" | jq -r '.result.workspace.label')" = "xo/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" ] \
...
xo_backend_herdr_projection_journal_create "$MISSING_STATE" missing1 >/dev/null
MISSING_JOURNAL=$(xo_backend_herdr_projection_journal_path "$MISSING_STATE" missing1)
START=$(log_line_count)
xo_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$MISSING_JOURNAL" missing1 \
...
RENAMED_TOKEN=$(xo_backend_herdr_projection_journal_create "$RENAMED_STATE" renamed1)
RENAMED_JOURNAL=$(xo_backend_herdr_projection_journal_path "$RENAMED_STATE" renamed1)
RENAMED_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "xo/renamed1 · p:$RENAMED_TOKEN" --no-focus)
...
START=$(log_line_count)
xo_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$RENAMED_JOURNAL" renamed1 \
...
DUP_TOKEN=$(xo_backend_herdr_projection_journal_create "$DUP_STATE" duplicate1)
DUP_JOURNAL=$(xo_backend_herdr_projection_journal_path "$DUP_STATE" duplicate1)
DUP1=$(lab workspace create --cwd "$PROJECT_DIR" --label "xo/duplicate1 · p:$DUP_TOKEN" --no-focus)
...
START=$(log_line_count)
xo_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$DUP_JOURNAL" duplicate1 \
...
lab pane report-agent "$DUP1_PANE" --source xo-projection-e2e --agent test-agent --state idle >/dev/null \
  || fail "could not register the duplicate-live-agent risk fixture"
START=$(log_line_count)
if xo_backend_herdr_projection_recovery_allows_flat "$HERDR_LAB_SESSION" "$DUP_JOURNAL" duplicate1; then
