...
from xo_plane.registry import AdapterError, Registry
from xo_plane.mcp_client import Plane, rows
from xo_plane.cli import Missions, bind_task, check_task, ensure_label, key_for, load_config, main, run, state_id
...
class AdapterTests(unittest.TestCase):
    ...
    def ensure(self, **overrides):
        ...
        with patch("xo_plane.cli.Plane", return_value=self.plane):
    ...
    def test_provisioning_precedes_the_lifecycle_mapping_that_ticket_commands_require(self):
        ...
        argv = ["xo-plane.py", "--config", str(path), "ensure-label", "--name", "needs-triage"]
        with patch("xo_plane.cli.Plane", return_value=self.plane), patch.object(sys, "argv", argv), \
    ...
    def overwatch(self, *args, expected=0):
        ...
        env = dict(os.environ, XO_HOME=str(local), XO_STATE_OVERRIDE=str(local / "state"))
        env.pop("XO_ROOT_OVERRIDE", None)
        result = subprocess.run([sys.executable, str(ROOT / "bin/xo-overwatch.py"), *args],
    ...
    def test_overwatch_registers_durable_wake_and_off_preserves_work(self):
        ...
        check = subprocess.run(["bash", "-c", '. "$1"; . "$2"; xo_custom_check_registered "$3" overwatch',
                                "test", str(ROOT / "bin/xo-pr-lib.sh"), str(ROOT / "bin/xo-check-lib.sh"),
    ...
    def test_bind_creates_valid_brief_and_shared_dispatch_check(self):
        ...
        with patch.dict(os.environ, {"XO_HOME": local}):
            ...
            result = subprocess.run(["bash", "-c", '. "$1"; xo_brief_task_content_valid "$2"', "test",
                                     str(ROOT / "bin/xo-dod-lib.sh"), str(brief)])
            self.assertEqual(result.returncode, 0)
            # Verify the public parser sees the injected heading only as quoted data.
            result = subprocess.run(["bash", "-c", '. "$1"; xo_brief_task_heading_body "$2" "## Captain\x27s intent"',
                                     "test", str(ROOT / "bin/xo-dod-lib.sh"), str(brief)], capture_output=True, text=True)
    ...
    def test_done_requires_merge_and_acceptance_then_is_retryable(self):
        ...
        with patch("xo_plane.cli.Plane", return_value=self.plane):
            with patch("xo_plane.cli.merged_pr", side_effect=AdapterError("not merged")):
                with self.assertRaises(AdapterError):
                    asyncio.run(run(args, self.config))
                self.assertEqual(self.plane.ticket["state"], "review")
            with patch("xo_plane.cli.merged_pr", return_value={"merged": True}):
    ...
    def test_a_conflict_over_the_real_transport_still_names_the_label_and_its_id(self):
        ...
        err = io.StringIO()
        argv = ["xo-plane.py", "--config", str(path), "ensure-label", "--name", "ready_for_agent"]
    ...
    def test_no_adapter_lifecycle_command_moves_a_ticket_without_a_claim_record(self):
        ...
        err = io.StringIO()
        argv = ["xo-plane.py", "--config", str(path), "complete", "--item", "item-1",
                "--execution", execution, "--acceptance-verified"]
        with patch("xo_plane.cli.merged_pr", return_value={"merged": True}), patch.object(sys, "argv", argv), \
