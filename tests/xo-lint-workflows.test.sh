#!/usr/bin/env bash
# GitHub workflow lint gate owned by bin/xo-lint-workflows.sh.
#
...
LINT_WF="$ROOT/bin/xo-lint-workflows.sh"
LINT="$ROOT/bin/xo-lint.sh"
INSTALLER="$ROOT/bin/xo-install-actionlint.sh"
...
xo_install_stub_tar_actionlint() {
  local fakebin=$1
  cat > "$fakebin/tar" <<'SH'
#!/usr/bin/env bash
while [ "$#" -gt 0 ]; do
...
test_col0_heredoc_fails_with_clear_error() {
  local tmp out rc
  tmp=$(xo_test_tmproot xo-lint-wf-col0)
...
test_valid_fixture_passes() {
  local tmp out rc
  tmp=$(xo_test_tmproot xo-lint-wf-ok)
  mkdir -p "$tmp/.github/workflows"
...
test_empty_workflows_dir_fails() {
  local tmp out rc
  tmp=$(xo_test_tmproot xo-lint-wf-empty)
  mkdir -p "$tmp/.github/workflows"
  rc=0
  out=$("$LINT_WF" --root "$tmp" 2>&1) || rc=$?
...
test_explicit_broken_path_fails() {
  ...
  tmp=$(xo_test_tmproot xo-lint-wf-path)
...
test_non_mapping_root_fails() {
  local tmp out rc
  tmp=$(xo_test_tmproot xo-lint-wf-scalar)
  mkdir -p "$tmp/.github/workflows"
...
test_missing_actionlint_fails_closed() {
  ...
  tmp=$(xo_test_tmproot xo-lint-wf-noactionlint)
  ...
  assert_contains "$out" "xo-install-actionlint.sh" \
    "missing actionlint did not name the pinned installer"
  pass "missing actionlint fails closed"
}

test_pins_an_explicit_version() {
  [ -n "$REQUIRED" ] || fail "xo-lint-workflows.sh --required-version printed nothing"
  assert_contains "$REQUIRED" "1.7.12" "xo-lint-workflows.sh must pin actionlint 1.7.12"
  pass "xo-lint-workflows.sh pins an explicit actionlint version ($REQUIRED)"
}

test_rejects_wrong_actionlint_version() {
  local tmp fakebin out rc
  tmp=$(xo_test_tmproot xo-lint-wf-ver)
  fakebin=$(xo_fakebin "$tmp")
  mkdir -p "$tmp/.github/workflows"
  write_valid_workflow "$tmp/.github/workflows/ci.yml"
...
SH
  ...
  [ "$rc" -ne 0 ] || fail "xo-lint-workflows.sh accepted an actionlint version other than the pin"$'\n'"$out"
  assert_contains "$out" "$REQUIRED" "xo-lint-workflows.sh did not name the required version on mismatch"
  assert_contains "$out" "0.0.0" "xo-lint-workflows.sh did not report the resolved (wrong) version"
  pass "xo-lint-workflows.sh refuses to lint under a non-pinned actionlint version"
}

test_installer_retries_transient_download_failure() {
  local tmp fakebin destination out
  tmp=$(xo_test_tmproot xo-actionlint-download)
  fakebin=$(xo_fakebin "$tmp")
  destination="$tmp/bin"
  ...
  xo_install_stub_tar_actionlint "$fakebin"
...
test_installer_selects_platform_archive_url_and_checksum() {
  ...
  tmp=$(xo_test_tmproot xo-actionlint-platform)
  fakebin=$(xo_fakebin "$tmp")
  destination="$tmp/bin"
...
test_installer_rejects_wrong_checksum() {
  ...
  tmp=$(xo_test_tmproot xo-actionlint-badsum)
  fakebin=$(xo_fakebin "$tmp")
  destination="$tmp/bin"
...
test_installer_falls_back_to_shasum() {
  ...
  tmp=$(xo_test_tmproot xo-actionlint-shasum)
  fakebin=$(xo_fakebin "$tmp")
  destination="$tmp/bin"
...
test_installer_prefers_sha256sum_over_shasum() {
  ...
  tmp=$(xo_test_tmproot xo-actionlint-sha256sum-pref)
  fakebin=$(xo_fakebin "$tmp")
  destination="$tmp/bin"
  hasher_log="$tmp/hasher.log"
...
test_installer_rejects_unsupported_platform() {
  local tmp fakebin destination out rc
  tmp=$(xo_test_tmproot xo-actionlint-unsupported)
  fakebin=$(xo_fakebin "$tmp")
  destination="$tmp/bin"
...
# Prove the no-mistakes/local owner (bin/xo-lint.sh with no paths) catches a
# self-broken ci.yml. Copy the lint scripts into a fake repo so the default
# workflow root is the fixture, not this worktree.
test_xo_lint_default_path_catches_broken_ci_yml() {
  local tmp fakebin log diff_file out rc
  tmp=$(xo_test_tmproot xo-lint-wf-default)
  mkdir -p "$tmp/bin" "$tmp/.github/workflows"
  cp "$LINT" "$tmp/bin/xo-lint.sh"
  cp "$LINT_WF" "$tmp/bin/xo-lint-workflows.sh"
  chmod +x "$tmp/bin/xo-lint.sh" "$tmp/bin/xo-lint-workflows.sh"
  write_col0_heredoc_workflow "$tmp/.github/workflows/ci.yml"
...
case "$*" in
  ...
  "diff --name-only --diff-filter=ACMR -z fakebase123 --")
    [ -n "${XO_TEST_GIT_DIFF_FILE:-}" ] && cat "${XO_TEST_GIT_DIFF_FILE}"
    exit 0
    ;;
  *) exit 0 ;;
esac
...
SH
  ...
  out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' XO_LINT_JOBS=1 \
    XO_TEST_GIT_DIFF_FILE="$diff_file" "$tmp/bin/xo-lint.sh" 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "xo-lint.sh default path missed a broken ci.yml"$'\n'"$out"
  assert_contains "$out" "could not parse as YAML" \
    "xo-lint.sh default path did not surface the workflow YAML error"
  assert_contains "$out" "ci.yml" \
    "xo-lint.sh default path did not name the broken workflow"
  pass "xo-lint.sh default path catches a self-broken ci.yml"
}
