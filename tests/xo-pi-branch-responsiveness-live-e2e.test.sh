...
#
# The portable regression in tests/xo-pi-branch-extension.test.sh pins the
...
#
# Nothing global is touched: a scratch XO_HOME, a scratch project holding a
...
xo_live_gate default-on XO_PI_BRANCH_RESPONSIVENESS_E2E pi tmux node

PI_VERSION=$(pi --version 2>/dev/null || printf 'unknown')
TMUX=$(command -v tmux)
SOCKET="xo-pi-branch-responsiveness-$$"
SESSION=pi-branch-responsiveness
TMP_ROOT=$(xo_test_tmproot xo-pi-branch-responsiveness)
LAB="$TMP_ROOT/lab"
PROJECT="$LAB/project"
HOME_DIR="$LAB/fmhome"
...
EXT="$PROJECT/.pi/extensions/xo-branch-supervision.ts"
OUTCOME_SCRIPT="$ROOT/bin/xo-branch-outcome.sh"

# The store this guard drives is the real one; only the rows are synthetic.
append_outcome() {
  XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$HOME_DIR/state" XO_ROOT_OVERRIDE="$ROOT" \
...
run_arm() {
  ...
  "$TMUX" -L "$SOCKET" new-session -d -s "$SESSION" -x 200 -y 50 \
    "XO_HOME='$HOME_DIR' XO_STATE_OVERRIDE='$HOME_DIR/state' XO_CONFIG_OVERRIDE='$HOME_DIR/config' \
     XO_ROOT_OVERRIDE='$ROOT' LAB_PROJECT='$PROJECT' '$LAB/launch.sh' ${pi_args[*]@Q}"
