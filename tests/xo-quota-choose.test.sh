#!/usr/bin/env bash
# Unit tests for bin/xo-quota-choose.sh.
...
LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-quota-choose.XXXXXX")
...
call_choose() {
  ...
  output=$(QUOTA_AXI_CALLS="$CALLS" QUOTA_AXI_FIXTURE="$FIXTURE" \
    PATH="$FAKEBIN:$PATH" "$BIN/xo-quota-choose.sh" "$@")
  rc=$?
...
if help=$("$BIN/xo-quota-choose.sh" --help 2>&1); then
...
printf '# all xo-quota-choose tests passed\n'
