#!/usr/bin/env bash
# Behavioral regressions for ask-user authority instructions generated by xo-brief.
set -u
...
TMP_ROOT=$(xo_test_tmproot xo-ask-user-authority)
...
test_primary_and_secondmate_instruction_generation() {
  ...
  assert_grep "XO applies \`ask-user-authority\` and obtains any required captain decision" "$ship" \
  ...
  XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_SECONDMATE_CHARTER='Handle sample work.' \
