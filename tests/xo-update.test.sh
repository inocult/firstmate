#!/usr/bin/env bash
# Tests for bin/xo-update.sh: fast-forward-only self-update of a running
# xo repo and every registered secondmate home.
#
# The guarantees under test mirror xo-fleet-sync.sh and prime directive #3:
#   - The running xo repo (on its default branch) fast-forwards from
...
#   - The caller-action summary is correct: reread-xo flips to yes only
...
#     data/secondmates.md registry, deduped, and the xo repo is never
...
UPDATE="$ROOT/bin/xo-update.sh"

# Deterministic, isolated git identity for fixture commits.
xo_git_identity fmtest fmtest@example.com

TMP_ROOT=$(xo_test_tmproot xo-update-tests)

# Build a fresh world: a bare origin seeded with one commit, an xo repo
...
set -u
case "${1:-}" in
  list-windows) cat "$XO_FAKE_DIR/windows" ;;
  display-message)
    target=
    for arg in "$@"; do
      case "$arg" in main:xo-*) target=$arg ;; esac
    done
    case "${*: -1}" in
      *pane_current_command*)
        id=${target##*xo-}
        if [ -e "$XO_FAKE_DIR/dead-$id" ]; then printf 'zsh\n'; else printf 'claude\n'; fi
        ;;
      *) printf '\n' ;;
    esac
    ;;
...
SH
  ...
  # Fresh watcher beacon keeps xo-guard quiet.
  ...
  printf 'echo a\n' > "$w/seed/bin/tool.sh"
  printf '#!/usr/bin/env bash\nexit 0\n' > "$w/seed/bin/xo-remote-secondmate-control.sh"
  chmod +x "$w/seed/bin/xo-remote-secondmate-control.sh"
...
# Add a secondmate home as a DETACHED worktree of the xo repo (matching
...
run_update() {
  local w=$1
  PATH="$w/fakebin:$PATH" XO_FAKE_DIR="$w/fake" \
    XO_SSH_BIN="${XO_TEST_SSH_BIN:-ssh}" \
    XO_ROOT_OVERRIDE="$w/main" XO_HOME="$w/home" "$UPDATE" 2>/dev/null
}
...
test_updates_main_and_secondmate() {
  ...
  assert_contains "$out" "xo: updated " "xo fast-forwarded"
  assert_contains "$out" "secondmate sm1: updated " "secondmate fast-forwarded"
  assert_contains "$out" "reread-xo: yes" "instruction change triggers reread"
  assert_contains "$out" "restart-secondmates: xo-sm1" "a changed AGENTS.md must move the secondmate into the restart set"
  ...
  # XO stays on its default branch; secondmate stays detached.
  [ "$(git -C "$w/main" symbolic-ref --short HEAD 2>/dev/null)" = "main" ] \
    || fail "xo left its default branch"
...
test_reread_gate_is_instruction_only() {
  ...
  assert_contains "$out" "xo: updated " "xo still advanced"
  assert_contains "$out" "reread-xo: no" "non-instruction change skips reread"
  # The running xo reads nothing new, but the mate's agent still holds its
  # launch-time wiring from before the pass, which only a restart re-resolves.
  assert_contains "$out" "restart-secondmates: xo-sm1" \
...
test_bin_only_advance_restarts() {
  ...
  assert_contains "$out" "reread-xo: yes" "a bin/ change is still an instruction-surface advance"
  assert_contains "$out" "restart-secondmates: xo-sm1" "a bin/-only advance must still restart the live mate"
  assert_contains "$out" "nudge-secondmates: none" "a restarted secondmate must not also be nudged"
...
test_skills_only_advance_rereads() {
  ...
  assert_contains "$out" "reread-xo: yes" "a skills/ change is an instruction-surface advance"
  assert_contains "$out" "restart-secondmates: xo-sm1" "a skills/-only advance must still restart the live mate"
...
test_unprovable_runtime_gets_fallback_nudge() {
  ...
  assert_contains "$out" "nudge-secondmates: xo-sm1" "an unverifiable runtime must retain the fallback re-read nudge"
...
EOF
  ...
  out=$(XO_TEST_SSH_BIN="$fake_ssh" run_update "$w")
...
test_dirty_secondmate_skipped() {
  ...
  assert_not_contains "$out" "xo-sm1" "skipped secondmate is not nudged"
...
test_diverged_secondmate_skipped() {
  ...
  assert_not_contains "$out" "xo-sm1" "diverged secondmate is not nudged"
...
test_already_current_secondmate_still_restarts() {
  ...
  assert_contains "$out" "xo: already current" "xo already current"
  assert_contains "$out" "secondmate sm1: already current" "secondmate already current"
  assert_contains "$out" "reread-xo: no" "no reread when nothing changed"
  restart_line=$(printf '%s\n' "$out" | grep '^restart-secondmates:')
  assert_contains "$restart_line" "xo-sm1" \
...
test_already_current_unprovable_mate_is_nudged() {
  ...
  assert_contains "$nudge_line" "xo-sm1" "an unprovable runtime must keep the honest re-read steer"
...
#   selfish - a bogus registry line pointing the xo repo at itself.
# Asserts: reg1 advances but is NOT nudged (no live metadata); sm1 advances,
# is processed once, and IS nudged; the xo repo is never re-processed.
test_registry_backstop_dedup_and_self_exclusion() {
  local w out count
  w=$(new_world t7)
  add_sm "$w" sm1
  git -C "$w/main" worktree add -q --detach "$w/reg1" main
  printf 'reg1\n' > "$w/reg1/.xo-secondmate-home"
  {
  ...
  assert_not_contains "$out" "secondmate selfish" "xo repo re-processed as its own secondmate"
  ...
  local restart_line
  restart_line=$(printf '%s\n' "$out" | grep '^restart-secondmates:')
  assert_contains "$restart_line" "xo-sm1" "live-meta secondmate is restarted"
  ...
  pass "T7 registry backstop resolves, dedups meta+registry, excludes the xo repo"
}

# --- T9: xo repo on a feature branch is skipped ---------------------
test_xo_wrong_branch_skipped() {
  local w out before
  w=$(new_world t9)
  bump_origin "$w" instr
  # Simulate xo mid-shipping its own change: not on the default branch.
  git -C "$w/main" checkout -q -b feature/wip
  before=$(git -C "$w/main" rev-parse HEAD)

  out=$(run_update "$w")

  assert_contains "$out" "xo: skipped: on feature/wip, expected main" "off-default xo skipped"
  assert_contains "$out" "reread-xo: no" "no reread when xo was skipped"
  [ "$(git -C "$w/main" rev-parse HEAD)" = "$before" ] \
    || fail "skipped xo HEAD moved"
  pass "T9 xo off its default branch is skipped, not forced"
}

test_xo_detached_head_skipped() {
  local w out before
  w=$(new_world t10)
  bump_origin "$w" instr
  ...
  assert_contains "$out" "xo: skipped: detached HEAD, expected main" "detached xo skipped"
  assert_contains "$out" "reread-xo: no" "no reread when detached xo was skipped"
  [ "$(git -C "$w/main" rev-parse HEAD)" = "$before" ] \
    || fail "detached xo HEAD moved"
  pass "T10 xo detached HEAD is skipped"
}
...
test_unsafe_secondmate_home_skipped_before_git_update() {
  ...
  assert_contains "$out" "secondmate bad: skipped: unsafe home: secondmate home cannot be inside the active xo home" \
...
echo "# all xo-update tests passed"
