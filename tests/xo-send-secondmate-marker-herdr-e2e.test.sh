...
# xo-spawn.sh --secondmate launch, captures Pi's before_agent_start prompt bytes,
# and proves both sides of the routing boundary:
#   - exact task id through explicit XO_HOME receives exactly one marker;
#   - direct terminal input remains unmarked.
#
# Every Herdr call, including calls made inside the production backend adapter,
# is routed through bin/xo-herdr-lab.sh. The PATH shim strips only the adapter's
...
xo_live_gate opt-in XO_SEND_MARKER_HERDR_E2E git herdr jq pi

LAB_HELPER=${HERDR_LAB_HELPER:-$ROOT/bin/xo-herdr-lab.sh}
SESSION=$("$LAB_HELPER" name xo-send-secondmate-marker-v7)
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/xo-send-marker-herdr-e2e.XXXXXX")
...
ID='marker-pi-sm'
REQUEST='XO_MARKER_HERDR_E2E exact-id request'
DIRECT='XO_MARKER_HERDR_DIRECT captain input'
...
printf '%s\n' "$ID" > "$SECOND_HOME/.xo-secondmate-home"
...
CAPTURE_EXTENSION="$TMP_ROOT/xo-send-marker-capture.ts"
...
PATH="$FAKEBIN:$ORIGINAL_PATH" XO_GATE_REFUSE_BYPASS=1 XO_HOME="$SENDER_HOME" HERDR_SESSION="$SESSION" \
  "$ROOT/bin/xo-spawn.sh" "$ID" "$SECOND_HOME" --secondmate --harness pi --backend herdr >/dev/null

META="$SENDER_HOME/state/$ID.meta"
[ -f "$META" ] || fail "real secondmate spawn did not write exact-id metadata"
[ "$(xo_meta_get "$META" kind)" = secondmate ] || fail "real secondmate metadata did not record kind=secondmate"
...
PATH="$FAKEBIN:$ORIGINAL_PATH" XO_GATE_REFUSE_BYPASS=1 XO_HOME="$SENDER_HOME" \
