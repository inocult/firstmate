...
#
# Two layers:
#   PREDICATE  - bin/xo-supervision-lib.sh, the shared beacon/status computation
#                used by xo-guard.sh and by the hook's banner details.
#   HOOK       - bin/xo-turnend-guard.sh, the shared primary hook predicate that
...
. "$ROOT/bin/xo-supervision-lib.sh"

TMP_ROOT=$(xo_test_tmproot xo-turnend-guard)
...
# --- PREDICATE: bin/xo-supervision-lib.sh -----------------------------------

test_predicate_healthy_no_inflight() {
  local state="$TMP_ROOT/pred-empty/state"
  mkdir -p "$state"
  if xo_supervision_unhealthy "$state" 300; then
    fail "predicate reported unhealthy with zero in-flight tasks"
  fi
  [ "$XO_SUP_IN_FLIGHT" -eq 0 ] || fail "expected zero in-flight, got $XO_SUP_IN_FLIGHT"
  pass "xo_supervision_unhealthy: false with no state/*.meta at all"
}
...
test_predicate_unhealthy_no_beacon() {
  ...
  mkdir -p "$state"
  : > "$state/task1.meta"
  xo_supervision_unhealthy "$state" 300 || fail "predicate did not fire: in-flight task, beacon never seen"
  [ "$XO_SUP_IN_FLIGHT" -eq 1 ] || fail "expected 1 in-flight, got $XO_SUP_IN_FLIGHT"
  [ "$XO_SUP_WATCHER_FRESH" = false ] || fail "beacon absent must not read as fresh"
  [ "$XO_SUP_BEACON_DESC" = never ] || fail "beacon description should be 'never', got $XO_SUP_BEACON_DESC"
  pass "xo_supervision_unhealthy: true with in-flight task and no beacon ever"
}
...
test_predicate_unhealthy_stale_beacon() {
  ...
  xo_supervision_unhealthy "$state" 300 || fail "predicate did not fire: in-flight task, beacon far outside grace"
  [ "$XO_SUP_WATCHER_FRESH" = false ] || fail "an ancient beacon must not read as fresh"
  pass "xo_supervision_unhealthy: true with in-flight task and a beacon far outside the grace window"
}
...
test_predicate_healthy_fresh_beacon() {
  ...
  [ "$XO_SUP_WATCHER_FRESH" = true ] || fail "a beacon touched just now must read as fresh"
  pass "xo_supervision_unhealthy: false with in-flight task and a fresh beacon"
}

test_predicate_queue_pending_flag() {
  local state="$TMP_ROOT/pred-queue/state"
  mkdir -p "$state"
  xo_supervision_status "$state" 300
  [ "$XO_SUP_QUEUE_PENDING" = false ] || fail "empty/absent wake queue must not read as pending"
  printf 'record\n' > "$state/.wake-queue"
  xo_supervision_status "$state" 300
  [ "$XO_SUP_QUEUE_PENDING" = true ] || fail "a non-empty wake queue must read as pending"
  pass "xo_supervision_status: XO_SUP_QUEUE_PENDING tracks state/.wake-queue"
}
...
test_predicate_x_mode_needs_supervision() {
  ...
  xo_supervision_needed "$state" 300 || fail "X-mode relay poll did not register as supervision need"
  [ "$XO_SUP_IN_FLIGHT" -eq 0 ] || fail "X-mode relay poll must not count as an in-flight task"
  [ "$XO_SUP_NEEDED" = true ] || fail "X-mode relay poll must set XO_SUP_NEEDED"
  xo_supervision_unhealthy "$state" 300 || fail "X-mode relay poll with no beacon must be unhealthy"
  pass "xo_supervision_needed: X-mode relay poll needs supervision"
}
...
test_predicate_source_needs_supervision() {
  ...
  xo_supervision_unhealthy "$state" 300 || fail "registered source with no beacon must be unhealthy"
  [ "$XO_SUP_IN_FLIGHT" -eq 0 ] || fail "a process-event source must not count as a task"
  [ "$XO_SUP_SOURCES" -eq 1 ] || fail "expected one registered process-event source"
  pass "xo_supervision_unhealthy: source-only home needs supervision"
}

# Register a custom check the way an operator does, through the real
# bin/xo-check-register.sh, so these cases bind to the shipped registration
# artifacts rather than to a hand-written imitation of them.
register_custom_check() {
  local state=$1 id=$2
  printf '#!/usr/bin/env bash\nexit 0\n' > "$state/$id.check.sh"
  chmod 700 "$state/$id.check.sh"
  XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-check-register.sh" "$id" >/dev/null \
    || fail "xo-check-register.sh could not register $id"
}
...
test_predicate_registered_check_needs_supervision() {
  ...
  xo_supervision_needed "$state" 300 || fail "a registered custom check did not register as supervision need"
  [ "$XO_SUP_IN_FLIGHT" -eq 0 ] || fail "a registered custom check must not count as an in-flight task"
  [ "$XO_SUP_CHECKS" -eq 1 ] || fail "expected one registered custom check, got $XO_SUP_CHECKS"
  xo_supervision_unhealthy "$state" 300 || fail "a registered custom check with no beacon must be unhealthy"
  pass "xo_supervision_needed: a registered custom check needs supervision with no task in flight"
}
...
test_predicate_registered_check_survives_rebinding_drift() {
  ...
  xo_supervision_needed "$state" 300 \
    || fail "an edited registered check must keep supervision on so the sweep can report the rejection"
  [ "$XO_SUP_CHECKS" -eq 1 ] || fail "expected the edited check to stay counted, got $XO_SUP_CHECKS"
  pass "xo_supervision_needed: a registered check whose bytes drifted still needs supervision"
}
...
test_predicate_unregistered_check_needs_nothing() {
  ...
  if xo_supervision_needed "$state" 300; then
    fail "a check with no trust binding must not arm supervision"
  fi
  [ "$XO_SUP_CHECKS" -eq 0 ] || fail "an unregistered check must not be counted, got $XO_SUP_CHECKS"
  pass "xo_supervision_needed: false for a check.sh with no registration binding"
}
...
test_predicate_task_pr_poll_is_not_a_custom_check() {
  ...
  : > "$state/task1.pr-poll"
  xo_supervision_needed "$state" 300 || fail "the in-flight task itself must need supervision"
  [ "$XO_SUP_IN_FLIGHT" -eq 1 ] || fail "expected the task to be the one in-flight need, got $XO_SUP_IN_FLIGHT"
  [ "$XO_SUP_CHECKS" -eq 0 ] || fail "a task PR poll must not count as a registered custom check"
  pass "xo_supervision_needed: a task PR poll without a trust binding is not a registered custom check"
}
...
test_predicate_relay_shim_is_not_a_custom_check() {
  ...
  mkdir -p "$state"
  : > "$state/x-watch.check.sh"
  xo_supervision_needed "$state" 300 || fail "the relay poll must still need supervision"
  [ "$XO_SUP_CHECKS" -eq 0 ] || fail "the relay shim keeps its own trust path and must not be counted as a custom check"
  pass "xo_supervision_status: the relay shim is not counted as a registered custom check"
}

# --- HOOK: bin/xo-turnend-guard.sh ------------------------------------------
#
# Each scenario gets its own directory carrying a copy of the two guard scripts
# under bin/, so the hook (invoked by absolute path) resolves its own XO_ROOT to
# that scenario dir regardless of the test's cwd.

install_guard_scripts() {
  local dir=$1
  mkdir -p "$dir/bin"
  cp "$ROOT/bin/xo-turnend-guard.sh" "$dir/bin/xo-turnend-guard.sh"
  cp "$ROOT/bin/xo-turnend-guard-grok.sh" "$dir/bin/xo-turnend-guard-grok.sh"
  cp "$ROOT/bin/xo-operational-input.sh" "$dir/bin/xo-operational-input.sh"
  cp "$ROOT/bin/xo-supervision-instructions.sh" "$dir/bin/xo-supervision-instructions.sh"
  cp "$ROOT/bin/xo-harness.sh" "$dir/bin/xo-harness.sh"
  cp "$ROOT/bin/xo-primary-scope-lib.sh" "$dir/bin/xo-primary-scope-lib.sh"
  cp "$ROOT/bin/xo-supervision-lib.sh" "$dir/bin/xo-supervision-lib.sh"
  cp "$ROOT/bin/xo-wake-lib.sh" "$dir/bin/xo-wake-lib.sh"
  cp "$ROOT/bin/xo-hook-host-lib.sh" "$dir/bin/xo-hook-host-lib.sh"
  mkdir -p "$dir/docs"
  cp -R "$ROOT/docs/supervision-protocols" "$dir/docs/supervision-protocols"
  chmod +x "$dir/bin/xo-turnend-guard.sh" "$dir/bin/xo-turnend-guard-grok.sh" "$dir/bin/xo-operational-input.sh" "$dir/bin/xo-supervision-instructions.sh" "$dir/bin/xo-harness.sh"
}

mark_codex_hook_root() {
  local dir=$1
  mkdir -p "$dir/.codex"
  printf '{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"xo-turnend-guard.sh"}]}]}}\n' > "$dir/.codex/hooks.json"
}
...
# Same shape as primary, plus the .xo-secondmate-home marker bin/xo-home-seed.sh
# writes at seed time (regardless of treehouse-lease or git-clone acquisition).
make_secondmate_dir() {
  local dir=$1
  make_primary_dir "$dir" >/dev/null
  printf 'sm-test-1\n' > "$dir/.xo-secondmate-home"
  printf '%s\n' "$dir"
}

# A genuine linked `git worktree` of a base repo - the shape bin/xo-spawn.sh
# always hands crewmate/scout tasks working on xo itself. git-dir and
# git-common-dir differ here, unlike a plain checkout.
make_crewmate_worktree_dir() {
  local base=$1 dir=$2
  xo_git_worktree "$base" "$dir" xo/turnend-guard-test-branch
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_guard_scripts "$dir"
  printf '%s\n' "$dir"
...
# .xo-secondmate-home marker, so the marker force-include never fires for it and
# it stays exempt through the linked-worktree git-dir test.
make_secondmate_child_worktree_dir() {
  local home=$1 dir=$2
  git -C "$home" worktree add --quiet -b xo/turnend-secondmate-child "$dir"
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_guard_scripts "$dir"
  printf '%s\n' "$dir"
...
# valid .xo-secondmate-home marker. This is the production topology the plain
...
make_secondmate_linked_home_dir() {
  local base=$1 dir=$2
  xo_git_worktree "$base" "$dir" xo/turnend-secondmate-linked-home
  mkdir -p "$dir/state"
  : > "$dir/AGENTS.md"
  install_guard_scripts "$dir"
  printf 'sm-linked-1\n' > "$dir/.xo-secondmate-home"
...
run_hook() {
  ...
  home=$(cd "$dir" && pwd)
  printf '{"stop_hook_active":%s}' "$stop_active" | CLAUDECODE=1 XO_HOME="$home" bash "$dir/bin/xo-turnend-guard.sh" 2>&1
}
...
watcher_identity() {
  local dir=$1 pid=$2
  XO_STATE_OVERRIDE="$dir/state" bash -c '. "$1"; xo_pid_identity "$2"' _ "$dir/bin/xo-wake-lib.sh" "$pid"
}
...
record_watcher_lock() {
  ...
  printf '%s\n' "$root" > "$dir/state/.watch.lock/xo-home"
  printf '%s\n' "$bin_dir/xo-watch.sh" > "$dir/state/.watch.lock/watcher-path"
...
test_hook_silent_when_no_work_in_flight() {
  ...
  pass "xo-turnend-guard: silent no-op with nothing in flight"
}
...
test_hook_blocks_when_fresh_beacon_has_no_live_lock() {
  ...
  pass "xo-turnend-guard: blocks when a fresh beacon has no live watcher lock"
}
...
test_hook_blocks_source_only_home() {
  ...
  pass "xo-turnend-guard: non-Claude path blocks a source-only home"
}
...
test_hook_blocks_when_dead_lock_has_fresh_beacon() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "xo-turnend-guard: blocks on a dead watcher lock even when the beacon is fresh"
}
...
test_hook_silent_with_live_lock_and_fresh_beacon() {
  ...
  pass "xo-turnend-guard: silent no-op with a live watcher lock and fresh beacon"
}
...
test_hook_non_claude_health_ignores_claude_budget_contention() {
  ...
  while IFS='|' read -r harness payload; do
    out=$(printf '%s' "$payload" | XO_HOME="$home" bash "$dir/bin/xo-turnend-guard.sh" 2>&1); status=$?
...
EOF
  ...
  pass "xo-turnend-guard: healthy non-Claude harness paths ignore Claude episode contention"
}
...
test_hook_blocks_with_live_lock_and_stale_beacon() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "xo-turnend-guard: blocks on a live watcher lock with an ancient beacon"
}
...
test_hook_blocks_when_unhealthy_in_primary() {
  ...
  pass "xo-turnend-guard: blocks with the exact required reason in the primary when unhealthy"
}

test_hook_blocks_from_xo_home_state() {
  local dir home out status
  dir=$(make_primary_dir "$TMP_ROOT/hook-xo-home")
  home="$TMP_ROOT/hook-xo-home-op"
  mkdir -p "$home/state"
  : > "$home/state/task1.meta"
  out=$(printf '{"stop_hook_active":false}' | CLAUDECODE=1 XO_HOME="$home" bash "$dir/bin/xo-turnend-guard.sh" 2>&1); status=$?
  expect_code 2 "$status" "hook must inspect the active XO_HOME state dir"
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "xo-turnend-guard: blocks from active XO_HOME state, not only repo-root state"
}
...
test_hook_x_mode_reason_sources_cadence() {
  ...
  pass "xo-turnend-guard: X-mode repair reason sources the cadence config"
}
...
test_hook_x_mode_only_blocks_in_default_mode() {
  ...
  pass "xo-turnend-guard: X-mode-only supervision remains guarded in default mode"
}
...
test_hook_registered_check_only_blocks_with_check_banner() {
  ...
  pass "xo-turnend-guard: registered-check-only supervision is named in the block banner"
}

test_hook_ignores_repo_state_when_xo_home_set() {
  local dir home out status
  dir=$(make_primary_dir "$TMP_ROOT/hook-xo-home-ignore-root")
  home="$TMP_ROOT/hook-xo-home-quiet"
  mkdir -p "$home/state"
  : > "$dir/state/task1.meta"
  out=$(printf '{"stop_hook_active":false}' | XO_HOME="$home" bash "$dir/bin/xo-turnend-guard.sh" 2>&1); status=$?
  expect_code 0 "$status" "hook must ignore repo-root state when XO_HOME selects another state dir"
  [ -z "$out" ] || fail "hook produced output from stale repo-root state despite XO_HOME: $out"
  pass "xo-turnend-guard: ignores stale repo-root state when XO_HOME is set"
}
...
test_hook_uses_state_override() {
  ...
  : > "$state/task1.meta"
  out=$(printf '{"stop_hook_active":false}' | CLAUDECODE=1 XO_HOME="$home" XO_STATE_OVERRIDE="$state" bash "$dir/bin/xo-turnend-guard.sh" 2>&1); status=$?
  expect_code 2 "$status" "hook must let XO_STATE_OVERRIDE win over XO_HOME/state"
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "xo-turnend-guard: uses XO_STATE_OVERRIDE ahead of XO_HOME/state"
}
...
test_hook_loop_guard_allows_retry() {
  ...
  pass "xo-turnend-guard: stop_hook_active=true always allows the stop (never blocks twice in one turn)"
}

# A secondmate's OWN home runs a primary xo session and must be guarded
# exactly like the main primary. This was the guard's proven blind spot: the
# .xo-secondmate-home marker used to early-exit here, so an overnight secondmate
...
test_hook_blocks_in_secondmate_own_home() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  assert_contains "$out" "TURN WOULD END BLIND" "block banner must read as an alarm"
  pass "xo-turnend-guard: blocks a blind turn end in a secondmate's own home (.xo-secondmate-home no longer excludes it)"
}
...
test_hook_silent_in_idle_secondmate_home() {
  ...
  pass "xo-turnend-guard: idle-by-default - silent in a secondmate home with nothing in flight"
}
...
test_hook_secondmate_loop_guard_allows_retry() {
  ...
  pass "xo-turnend-guard: stop_hook_active=true allows the stop in a secondmate home (never blocks twice in one turn)"
}
...
test_hook_secondmate_reinvoke_recovery_loop() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "xo-turnend-guard: secondmate deferred-death recovery - silent while watched, forces re-arm once the watcher exits"
}
...
test_hook_silent_in_secondmate_child_worktree() {
  ...
  pass "xo-turnend-guard: inert in a secondmate's own child worktree (linked git worktree) even when unhealthy"
}
...
test_hook_blocks_in_treehouse_leased_secondmate_home() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  assert_contains "$out" "TURN WOULD END BLIND" "block banner must read as an alarm"
  pass "xo-turnend-guard: blocks a blind turn end in a treehouse-leased LINKED secondmate home (marker force-include)"
}
...
test_hook_exempts_linked_worktree_with_stray_marker() {
  ...
  pass "xo-turnend-guard: an invalid (empty) marker cannot spoof inclusion; linked worktree stays exempt"
}
...
test_hook_exempts_linked_worktree_with_non_ascii_marker() {
  ...
  pass "xo-turnend-guard: a non-ASCII marker cannot spoof inclusion; linked worktree stays exempt"
}
...
test_hook_silent_in_crewmate_worktree() {
  ...
  pass "xo-turnend-guard: inert in a crewmate/scout task worktree (linked git worktree) even when unhealthy"
}
...
test_hook_runs_fast() {
  ...
  pass "xo-turnend-guard: runs well under the generous timing margin (${elapsed_s}s)"
}
...
test_grok_adapter_forces_one_resume_when_unhealthy() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(xo_fakebin "$TMP_ROOT/grok-adapter-fakebin")
...
EOF
  ...
  out=$(printf '{"sessionId":"session-test","hookEventName":"stop"}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  assert_contains "$(cat "$log")" 'XO_OP: v1 turn-end-guard: TURN WOULD END BLIND' "grok adapter must retain the typed guard kind"
  pass "xo-turnend-guard-grok: forces one explicitly marked same-session resume when the shared predicate blocks"
}
...
test_grok_adapter_loop_guard_skips_resume() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(xo_fakebin "$TMP_ROOT/grok-adapter-loop-fakebin")
...
EOF
  chmod +x "$fakebin/grok"
  out=$(printf '{"sessionId":"session-test","hookEventName":"stop"}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" GROK_TURNEND_GUARD_ACTIVE=1 bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "xo-turnend-guard-grok: legacy environment loop guard prevents a nested resume loop"
}
...
test_grok_adapter_native_false_blocks_without_resume() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(xo_fakebin "$TMP_ROOT/grok-native-false-bin")
  log="$TMP_ROOT/grok-native-false.log"
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":false}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "xo-turnend-guard-grok: native false delegates blocking feedback with zero resume processes"
}
...
test_grok_adapter_native_true_allows_without_resume() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(xo_fakebin "$TMP_ROOT/grok-native-true-bin")
  log="$TMP_ROOT/grok-native-true.log"
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":true}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "xo-turnend-guard-grok: native true remains bounded and starts no resume process"
}
...
test_grok_adapter_snake_case_native_and_camel_precedence() {
  ...
  : > "$dir/state/task1.meta"
  out=$(printf '%s' '{"sessionId":"native","stop_hook_active":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":true,"stop_hook_active":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  expect_code 0 "$status" "camelCase true must win over snake_case false"
  out=$(printf '%s' '{"sessionId":"native","stopHookActive":false,"stop_hook_active":true}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  expect_code 2 "$status" "camelCase false must win over snake_case true"
  pass "xo-turnend-guard-grok: both spellings are typed and camelCase has deterministic precedence"
}
...
test_grok_adapter_invalid_inputs_start_neither_path() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(xo_fakebin "$TMP_ROOT/grok-invalid-bin")
  ...
  for payload in \
    ...
    '{"sessionId":"x","sessionId":"y"}'
  do
    out=$(printf '%s' "$payload" | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin:$PATH" GROK_WORKSPACE_ROOT="$TMP_ROOT/missing-grok-root" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "xo-turnend-guard-grok: malformed, invalidly typed, and missing-prerequisite payloads start neither path"
}
...
test_grok_adapter_missing_jq_and_no_supervision_allow() {
  ...
  : > "$dir/state/task1.meta"
  fakebin=$(xo_fakebin "$TMP_ROOT/grok-nojq-bin")
  ...
  printf '#!/usr/bin/env bash\nprintf called >> %q\n' "$log" > "$fakebin/grok"
  chmod +x "$fakebin/grok"
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | PATH="$fakebin" GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' '{"sessionId":"x","stopHookActive":false}' | GROK_WORKSPACE_ROOT="$dir" bash "$dir/bin/xo-turnend-guard-grok.sh" 2>&1); status=$?
  ...
  pass "xo-turnend-guard-grok: missing jq and no-supervision-needed stops stay silent and bounded"
}
...
#
# bin/xo-subagent-pretool-check.sh is the deliberate exception: Grok has no
...
test_tracked_claude_entries_inert_under_grok() {
  ...
  mkdir -p "$dir/bin"
  for script in xo-turnend-guard.sh xo-claude-stop-autoarm.sh xo-sessionstart-run.sh \
    xo-arm-pretool-check.sh xo-cd-pretool-check.sh xo-subagent-pretool-check.sh; do
  ...
  while IFS= read -r cmd; do
    ...
    if [ "$target" = xo-subagent-pretool-check.sh ]; then
...
test_codex_hook_uses_process_pwd_when_payload_cwd_is_outside_root() {
  ...
  mkdir -p "$outside"
  cat > "$dir/bin/xo-turnend-guard.sh" <<'EOF'
#!/usr/bin/env bash
printf 'guard=%s\n' "$0"
cat
EOF
  chmod +x "$dir/bin/xo-turnend-guard.sh"
  payload=$(jq -cn --arg cwd "$outside" '{cwd:$cwd,stop_hook_active:false}')
  out=$(printf '%s' "$payload" | (cd "$dir" && bash -c "$command") 2>&1); status=$?
  expect_code 0 "$status" "codex hook must execute successfully when payload cwd is outside the xo root"
  assert_contains "$out" "guard=$expected_root/bin/xo-turnend-guard.sh" "codex hook must use the hook process root"
...
test_codex_hook_ignores_nested_git_root_guard() {
  ...
  cat > "$nested/bin/xo-turnend-guard.sh" <<'EOF'
#!/usr/bin/env bash
printf 'nested guard executed\n'
exit 99
EOF
  chmod +x "$nested/bin/xo-turnend-guard.sh"
  cat > "$dir/bin/xo-turnend-guard.sh" <<'EOF'
#!/usr/bin/env bash
printf 'guard=%s\n' "$0"
cat
EOF
  ...
  assert_contains "$out" "guard=$expected_root/bin/xo-turnend-guard.sh" "codex hook must keep using the outer xo guard"
...
test_opencode_plugin_anchors_guard_to_worktree() {
  ...
  plugin="$ROOT/.opencode/plugins/xo-primary-turnend-guard.js"
  ...
  cat > "$worktree_dir/bin/xo-turnend-guard.sh" <<'EOF'
#!/usr/bin/env bash
cat >/dev/null
printf 'guard-fired\n' >&2
exit 2
EOF
  chmod +x "$worktree_dir/bin/xo-turnend-guard.sh"
...
const hooks = await mod.XoPrimaryTurnendGuard({
  client,
  directory: process.env.DIRECTORY,
  worktree: process.env.WORKTREE,
});
await hooks.event({ event: { type: "session.idle", properties: { sessionID: "session-test" } } });
if (!promptBody.startsWith("\u2063XO_OP: v1 turn-end-guard: ")) {
...
test_pi_extension_injects_once_per_logical_agent_run() {
  ...
  ext="$repo/.pi/extensions/xo-primary-turnend-guard.ts"
  log="$TMP_ROOT/pi-logical-run-guard.log"
  mkdir -p "$repo/.pi/extensions/lib" "$repo/bin" "$home/state"
  cp "$ROOT/.pi/extensions/xo-primary-turnend-guard.ts" "$ext"
  cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" "$repo/.pi/extensions/lib/xo-operational-input.ts"
  cp "$ROOT/bin/xo-operational-input.sh" "$repo/bin/xo-operational-input.sh"
  cat > "$repo/bin/xo-turnend-guard.sh" <<'SH'
#!/usr/bin/env bash
cat >/dev/null
printf 'guard\n' >> "${XO_GUARD_LOG:?}"
...
exit 2
SH
  cat > "$repo/bin/xo-arm-pretool-check.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$repo/bin/xo-turnend-guard.sh" "$repo/bin/xo-arm-pretool-check.sh"
  out=$(PLUGIN="$ext" XO_HOME="$home" XO_GUARD_LOG="$log" node --input-type=module 2>&1 <<'EOF'
...
const pi = {
  ...
  async sendUserMessage(message, options) {
    prompts += 1;
    if (!message.startsWith("\u2063XO_OP: v1 turn-end-guard: ")) throw new Error(`untyped operational prompt: ${message}`);
...
const guardRuns = readFileSync(process.env.XO_GUARD_LOG, "utf8").trim().split("\n").length;
...
SH
  chmod +x "$repo/bin/xo-turnend-guard.sh" "$repo/bin/xo-arm-pretool-check.sh"
  out=$(PLUGIN="$ext" XO_HOME="$home" node --input-type=module 2>&1 <<'EOF'
import { pathToFileURL } from "node:url";
...
run_hook_claude() {
  local dir=$1 stop_active=$2 home
  home=$(cd "$dir" && pwd)
  printf '{"stop_hook_active":%s,"session_id":"sess-claude-mode"}' "$stop_active" | CLAUDECODE=1 XO_HOME="$home" bash "$dir/bin/xo-turnend-guard.sh" --claude 2>&1
}
...
install_integrated_autoarm() {
  local dir=$1
  cp "$ROOT/bin/xo-claude-stop-autoarm.sh" "$dir/bin/xo-claude-stop-autoarm.sh"
  cp "$ROOT/bin/xo-primary-scope-lib.sh" "$dir/bin/xo-primary-scope-lib.sh"
  cp "$ROOT/bin/xo-supervision-lib.sh" "$dir/bin/xo-supervision-lib.sh"
  cp "$ROOT/bin/xo-wake-lib.sh" "$dir/bin/xo-wake-lib.sh"
  cp "$ROOT/bin/xo-hook-host-lib.sh" "$dir/bin/xo-hook-host-lib.sh"
  cp "$ROOT/bin/xo-session-lock-lib.sh" "$dir/bin/xo-session-lock-lib.sh"
  cp "$ROOT/bin/xo-cursor-lib.sh" "$dir/bin/xo-cursor-lib.sh"
  cp "$ROOT/bin/xo-lock.sh" "$dir/bin/xo-lock.sh"
  chmod +x "$dir/bin/xo-claude-stop-autoarm.sh" "$dir/bin/xo-lock.sh"
...
run_integrated_autoarm() {
  local dir=$1 home
  home=$(cd "$dir" && pwd)
  # shellcheck disable=SC2016 # the fake harness expands XO_HOME inside its child shell.
  printf '{"session_id":"sess-claude-mode","stop_hook_active":false}\n' \
    | XO_HOME="$home" "$dir/fake-claude" -c '
        printf "%s\n" "$$" > "$XO_HOME/state/.lock"
        "$XO_HOME/bin/xo-claude-stop-autoarm.sh"
      ' 2>&1
}
...
run_integrated_autoarm_unowned() {
  ...
  # shellcheck disable=SC2016 # the fake harness expands XO_HOME inside its child shell.
  printf '{"session_id":"sess-claude-mode","stop_hook_active":false}\n' \
    | XO_HOME="$home" "$dir/fake-claude" -c '"$XO_HOME/bin/xo-claude-stop-autoarm.sh"' 2>&1
}

write_integrated_failed_arm() {
  local dir=$1
  cat > "$dir/bin/xo-watch-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'watcher: FAILED - persistent fixture failure\n'
exit 1
SH
  chmod +x "$dir/bin/xo-watch-arm.sh"
}
...
test_hook_claude_mode_reblocks_stop_hook_active_when_unhealthy() {
  ...
  : > "$dir/state/task1.meta"
  out=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=200 run_hook_claude "$dir" true); status=$?
  ...
  pass "xo-turnend-guard --claude: re-blocks a loop-guarded stop while unhealthy and unclaimed (incident regression)"
}
...
test_hook_claude_mode_reblocks_x_mode_without_tasks() {
  ...
  pass "xo-turnend-guard --claude: X-mode-only homes re-block when auto-arm recovery is absent"
}
...
test_hook_claude_mode_allows_when_autoarm_owner_alive() {
  ...
  pass "xo-turnend-guard --claude: a live arming epoch advances once and repeated observation is idempotent"
}
...
test_hook_claude_mode_repeated_failed_to_arming_interleavings_reach_fail_open() {
  ...
  assert_contains "$out" 'XO SUPERVISION IS GENUINELY DOWN' "arming interleavings stalled before the bounded fail-open"
  assert_present "$dir/state/.claude-autoarm-failure-alarmed" "arming interleavings did not consume the one-time alarm"
  pass "xo-turnend-guard --claude: repeated failed-to-arming races make bounded monotonic progress"
}
...
test_hook_claude_mode_terminal_boundary_excludes_starting_owner() {
  ...
  cat > "$fakebin/cat" <<'SH'
#!/usr/bin/env bash
if [ "$1" = "$XO_TERMINAL_ROLE_PATH" ] \
  && [ "$(/bin/cat "$1" 2>/dev/null || true)" = terminal-check ] \
  && (set -C; : > "$XO_TERMINAL_ONCE") 2>/dev/null; then
  printf 'ready\n' > "$XO_TERMINAL_READY"
  IFS= read -r _ < "$XO_TERMINAL_RELEASE"
fi
exec /bin/cat "$@"
SH
  chmod +x "$fakebin/cat"
  (
    printf '{"stop_hook_active":true,"session_id":"sess-claude-mode"}' \
      | PATH="$fakebin:$PATH" \
        XO_TERMINAL_ROLE_PATH="$dir/state/.claude-autoarm.lock/role" \
        XO_TERMINAL_READY="$ready" \
        XO_TERMINAL_RELEASE="$release" \
        XO_TERMINAL_ONCE="$once" \
        CLAUDECODE=1 XO_HOME="$dir" bash "$dir/bin/xo-turnend-guard.sh" --claude \
          > "$guard_out" 2>&1
    printf '%s\n' "$?" > "$guard_status"
  ) &
  guard_pid=$!
  ...
  assert_contains "$(cat "$guard_out")" 'XO SUPERVISION IS GENUINELY DOWN' "terminal boundary did not produce the one-time alarm"
  assert_absent "$dir/state/.claude-autoarm.lock" "terminal boundary left its owner lock behind"
  pass "xo-turnend-guard --claude: terminal owner boundary excludes a concurrent start without deadlock"
}
...
test_hook_claude_mode_allows_on_fresh_rewake_epoch() {
  ...
  pass "xo-turnend-guard --claude: fresh rewake epoch prevents a duplicate continuation for the same event"
}
...
test_hook_claude_mode_blocks_on_abandoned_autoarm_claim() {
  ...
  pass "xo-turnend-guard --claude: an abandoned auto-arm claim no longer allows a blind stop (incident regression)"
}
...
test_hook_claude_mode_blocks_on_pid_reused_arming_claim() {
  ...
  identity=$(xo_test_pid_identity "$$") || fail "could not compute a claim pid-identity"
  ...
  pass "xo-turnend-guard --claude: a claim whose pid was reused stops counting as recovery even while its entry reads arming"
}
...
test_hook_claude_mode_blocks_on_stuck_arming_claim() {
  ...
  pid=$!
  record_autoarm_owner "$dir" "$pid"
  identity=$(xo_test_pid_identity "$pid") || fail "could not compute a claim pid-identity"
  printf '%s\n' "$identity" > "$dir/state/.claude-autoarm.lock/pid-identity"
  printf 'epoch=464 owner_pid=%s outcome=arming updated_at=1\n' "$pid" > "$dir/state/.claude-autoarm-epoch"
  touch -t 202001010000 "$dir/state/.claude-autoarm-epoch"
  touch -t 202001010000 "$dir/state/.last-watcher-beat"
  ...
  pass "xo-turnend-guard --claude: a hung owner frozen at arming with no watcher beat no longer allows a blind stop"
}
...
test_hook_claude_mode_allows_on_open_generation_claim() {
  ...
  pass "xo-turnend-guard --claude: a live open generation claim owns recovery with no lock held"
}
...
test_hook_claude_mode_blocks_on_stuck_generation_claim() {
  ...
  pass "xo-turnend-guard --claude: a stuck generation claim no longer allows a blind stop"
}
...
test_hook_claude_mode_terminal_fail_open_clears_abandoned_claim() {
  ...
  assert_contains "$out" 'XO SUPERVISION IS GENUINELY DOWN' "an abandoned claim suppressed the episode's attended alarm"
  ...
  pass "xo-turnend-guard --claude: the terminal path clears an abandoned claim instead of stepping aside silently"
}
...
test_hook_claude_mode_preserves_fresh_failed_progression() {
  ...
  pass "xo-turnend-guard --claude: fresh failed epochs preserve and advance monotonic fail-open progression"
}
...
test_hook_claude_mode_integrated_monotonic_fail_open() {
  ...
  guard_out=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true); guard_status=$?
  ...
  for i in 1 2 3 4; do
    ...
    if [ "$i" -lt 4 ]; then
      ...
      assert_not_contains "$guard_out" 'XO SUPERVISION IS GENUINELY DOWN' "fail-open fired before the bounded progression ended"
    else
      expect_code 0 "$guard_status" "the bounded failure progression must reach the attended fail-open"
      assert_contains "$guard_out" 'XO SUPERVISION IS GENUINELY DOWN' "the integrated fail-open alarm is missing"
  ...
  assert_not_contains "$guard_out" 'XO SUPERVISION IS GENUINELY DOWN' "the attended alarm repeated in the same episode"
  ...
  guard_out=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false); guard_status=$?
  ...
  pass "xo-turnend-guard --claude: integrated fresh failures reach one bounded fail-open, stop continuation, and reset on recovery"
}
...
test_hook_claude_mode_frozen_epoch_reaches_bounded_fail_open() {
  ...
  for i in 1 2 3 4; do
    ...
    if [ "$i" -lt 4 ]; then
      ...
      assert_not_contains "$guard_out" 'XO SUPERVISION IS GENUINELY DOWN' "fail-open fired before the frozen-epoch budget was spent"
      ...
      assert_contains "$guard_out" 'XO SUPERVISION IS GENUINELY DOWN' "the frozen-epoch fail-open alarm is missing"
  ...
  assert_not_contains "$guard_out" 'XO SUPERVISION IS GENUINELY DOWN' "the attended alarm repeated against the frozen epoch"
  ...
  pass "xo-turnend-guard --claude: an inert auto-arm's frozen epoch reaches one bounded fail-open and resets on recovery"
}
...
test_hook_claude_mode_frozen_epoch_without_verified_failure_spends_budget_and_keeps_blocking() {
  ...
  for i in 1 2 3 4 5; do
    out=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false); status=$?
  ...
  pass "xo-turnend-guard --claude: a frozen unverified epoch spends the budget yet still blocks"
}
...
test_hook_claude_mode_recovery_contention_is_not_ordinary_allow() {
  ...
  pass "xo-turnend-guard --claude: reset contention preserves all episode state until retry"
}
...
test_hook_claude_mode_concurrent_recovery_resets_are_idempotent() {
  ...
  auto_pid=$!
  (XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" false > "$dir/guard.out"; printf '%s\n' "$?" > "$dir/guard.status") &
  guard_pid=$!
  wait "$auto_pid"
  wait "$guard_pid"
  kill "$pid" 2>/dev/null || true
  ...
  pass "xo-turnend-guard --claude: concurrent auto-arm and guard resets are idempotent and deadlock-free"
}
...
test_hook_claude_mode_stale_rewake_epoch_blocks() {
  ...
  pass "xo-turnend-guard --claude: stale rewake epoch does not allow a blind stop"
}
...
test_hook_claude_mode_budget_without_verified_failure_keeps_blocking() {
  ...
  pass "xo-turnend-guard --claude: budget exhaustion alone cannot permit a blind stop"
}
...
test_hook_claude_mode_verified_failure_alarm_is_loud_and_once() {
  ...
  seed_claude_failure "$dir"
  seed_claude_budget "$dir" 3
  out=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true); status=$?
  expect_code 0 "$status" "verified failure with exhausted budget must take the bounded attended fail-open"
  assert_contains "$out" 'XO SUPERVISION IS GENUINELY DOWN' "bounded fail-open alarm was not unmistakable"
  ...
  assert_not_contains "$out" 'xo-watch-arm.sh' "bounded fail-open alarm assigned a manual watcher launch"
  assert_present "$dir/state/.claude-autoarm-failure-alarmed" "bounded fail-open did not consume the episode alarm"
  out2=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$dir" true); status2=$?
  expect_code 2 "$status2" "a consumed attended alarm must make later unhealthy stops block again"
  assert_not_contains "$out2" 'XO SUPERVISION IS GENUINELY DOWN' "attended failure alarm repeated in one episode"
  pass "xo-turnend-guard --claude: verified fail-open is loud, bounded, attended, and non-repeating"
}
...
test_hook_claude_mode_fail_open_requires_notice_and_failure_epoch() {
  ...
  out=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$no_notice" true); status=$?
  ...
  out=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=100 run_hook_claude "$notice_only" true); status=$?
  expect_code 2 "$status" "a consumed notice without an exhausted failure epoch must remain blocking"
  pass "xo-turnend-guard --claude: fail-open requires both exhausted retries and consumed notice"
}
...
test_hook_claude_mode_away_mode_never_uses_stop_autoarm_fail_open() {
  ...
  pass "xo-turnend-guard --claude: away ownership excludes the Stop-autoarm fail-open"
}
...
test_hook_claude_mode_allow_resets_budget() {
  ...
  pass "xo-turnend-guard --claude: positive watcher recovery resets failure episode state"
}
...
test_hook_claude_mode_waits_for_late_claim() {
  ...
  helper=$!
  out=$(XO_CLAUDE_AUTOARM_SYNC_WAIT_MS=3000 run_hook_claude "$dir" false); status=$?
  ...
  pass "xo-turnend-guard --claude: bounded claim wait avoids a token-consuming forced continuation"
}
...
test_hook_claude_mode_secondmate_reblocks_like_primary() {
  ...
  pass "xo-turnend-guard --claude: secondmate home re-blocks unclaimed and allows auto-arm-claimed stops"
}

# --- AWAY MODE: the daemon owns supervision ----------------------------------
#
# While state/.afk exists, bin/xo-supervise-daemon.sh owns supervision and runs
# bin/xo-watch.sh ONE-SHOT: the watcher exits on every wake and the daemon
...
# computed for itself (watcher_identity is that same xo_pid_identity read).
...
test_hook_away_daemon_allows_between_watcher_cycles() {
  ...
  pass "xo-turnend-guard: a live away-mode daemon satisfies supervision with no watcher holding the lock"
}
...
test_hook_away_daemon_allows_over_dead_watcher_lock() {
  ...
  [ -z "$out" ] || fail "away-mode daemon ownership still produced a block banner: $out"
  pass "xo-turnend-guard: away-mode daemon ownership survives a leftover dead watcher lock"
}
...
test_hook_away_mode_blocks_without_any_supervisor() {
  ...
  pass "xo-turnend-guard: away mode with no daemon and no watcher still blocks"
}
...
test_hook_away_mode_blocks_on_dead_daemon() {
  ...
  assert_contains "$out" "$AWAY_REQUIRED_REASON" "away-mode block must point at the daemon, not normal supervision"
  pass "xo-turnend-guard: away mode blocks on a dead away-mode daemon"
}
...
test_hook_away_mode_blocks_on_pid_reused_daemon() {
  ...
  assert_contains "$out" "$AWAY_REQUIRED_REASON" "away-mode block must point at the daemon, not normal supervision"
  pass "xo-turnend-guard: away mode blocks on a pid-reused away-mode daemon lock"
}
...
test_hook_away_mode_blocks_on_stale_beacon() {
  ...
  assert_contains "$out" "$AWAY_REQUIRED_REASON" "away-mode block must point at the daemon, not normal supervision"
  pass "xo-turnend-guard: away-mode daemon ownership never substitutes for a fresh beacon"
}
...
test_hook_daemon_lock_is_ignored_without_away_mode() {
  ...
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "xo-turnend-guard: a daemon lock proves nothing while away mode is off"
}
...
# daemon perfectly healthy throughout (xo-turnend-guard-afk-race). The guard
# must accept a live daemon there once XO_POLL justifies the wider window, but
...
test_hook_away_daemon_allows_beacon_within_poll_derived_grace() {
  ...
  # grace (max(300, XO_POLL + 60) = 660 at XO_POLL=600) - a live daemon that
  # simply has not finished restarting its watcher yet.
  beat=$(( $(date +%s) - 400 ))
  xo_touch_epoch "$beat" "$dir/state/.last-watcher-beat"
  out=$(XO_GUARD_GRACE='' XO_POLL=600 run_hook "$dir" false); status=$?
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  ...
  pass "xo-turnend-guard: away-mode beacon freshness uses the poll-derived grace, not the flat default"
}
...
test_hook_away_daemon_blocks_dead_daemon_despite_poll_derived_grace() {
  ...
  assert_contains "$out" "$AWAY_REQUIRED_REASON" "away-mode block must point at the daemon, not normal supervision"
  pass "xo-turnend-guard: a dead away-mode daemon still blocks under the poll-derived grace"
}
...
test_hook_away_daemon_blocks_beacon_older_than_poll_derived_grace() {
  ...
  # 700s exceeds even the wider poll-derived grace (660 at XO_POLL=600), so a
  ...
  assert_contains "$out" "$AWAY_REQUIRED_REASON" "away-mode block must point at the daemon, not normal supervision"
  pass "xo-turnend-guard: the poll-derived grace is bounded, not unlimited"
}
...
test_hook_no_afk_ignores_poll_derived_grace() {
  ...
  # flat default govern instead - old behavior, unaffected by XO_POLL.
  beat=$(( $(date +%s) - 400 ))
  xo_touch_epoch "$beat" "$dir/state/.last-watcher-beat"
  out=$(XO_GUARD_GRACE='' XO_POLL=600 run_hook "$dir" false); status=$?
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  expect_code 2 "$status" "without .afk, XO_POLL must not widen the strict watcher predicate's grace"
  assert_contains "$out" "$REQUIRED_REASON" "block reason must contain the exact required instruction"
  pass "xo-turnend-guard: with away mode off, the poll-derived grace never applies"
}
