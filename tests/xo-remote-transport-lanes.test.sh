...
#
# Pins, against the real worker and the real xo-on -> entrypoint transport
# (through the deterministic XO_SSH_BIN seam tests/xo-on.test.sh proves
...
#   T6: a non-payload xo-on call with an OPEN stdin pipe completes instead of
...
TMP_ROOT=$(xo_test_tmproot xo-remote-transport-lanes)
mkdir -p "$TMP_ROOT"
...
cp "$ROOT/bin/xo-remote-job-lib.sh" "$ROOT/bin/xo-remote-job-worker.sh" \
  "$ROOT/bin/xo-remote-entrypoint.sh" "$ROOT/bin/xo-remote-delta-read.sh" \
  "$ROOT/bin/xo-remote-secondmate-control.sh" "$ROOT/bin/xo-backend.sh" \
  "$ROOT/bin/xo-pending-reply-lib.sh" "$ROOT/bin/xo-task-inbox-lib.sh" \
  "$ROOT/bin/xo-wake-lib.sh" "$ROOT/bin/xo-marker-lib.sh" \
  "$ROOT/bin/xo-operational-input.sh" "$ROOT/bin/xo-tmux-lib.sh" \
  "$ROOT/bin/xo-composer-lib.sh" "$ROOT/bin/xo-cursor-lib.sh" \
  "$ROOT/bin/xo-classify-lib.sh" "$ROOT/bin/xo-timeout-lib.sh" \
  "$ROOT/bin/xo-ff-lib.sh" "$ROOT/bin/xo-secondmate-registry-lib.sh" \
  "$REMOTE_ROOT/bin/"
...
cat > "$REMOTE_ROOT/bin/xo-mark-job.sh" <<'SH'
#!/bin/bash
printf '%s\n' "$1" >> "$2"
sleep "${3:-0}"
SH
...
cat > "$REMOTE_ROOT/bin/xo-two-phase-job.sh" <<'SH'
#!/bin/bash
printf 'started\n' > "$1"
sleep "$3"
printf 'finished\n' > "$2"
SH
cat > "$REMOTE_ROOT/bin/xo-stdin-probe.sh" <<'SH'
#!/bin/bash
...
export XO_REMOTE_JOB_QUEUE_TIMEOUT=60
export XO_REMOTE_JOB_TIMEOUT=30
export XO_REMOTE_JOB_STAGE_REAP_SECONDS=1
# shellcheck source=bin/xo-remote-job-lib.sh
. "$ROOT/bin/xo-remote-job-lib.sh"

xo_remote_job_prepare_state "$ACCOUNT_HOME" || fail "$XO_REMOTE_JOB_ERROR"
rm -f -- "$STATE_ROOT/seq"
SEQ_PIDS=()
for i in $(seq 1 20); do
  xo_remote_job_next_seq > "$TMP_ROOT/seq-$i" &
  SEQ_PIDS+=("$!")
done
...
xo_remote_job_reap_stale "$ACCOUNT_HOME" || fail "sequence claim reaping failed"
...
xo_remote_job_reap_stale "$ACCOUNT_HOME" || fail "rate-limited sequence claim reaping failed"
...
xo_remote_job_reap_stale "$ACCOUNT_HOME" || fail "expired sequence claim reaping failed"
...
job_state() { # <id>
  xo_remote_job_read_state "$STATE_ROOT/jobs/$1" 2>/dev/null || true
}
...
xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_A" xo-mark-job.sh a1 "$LOG_A" 4 < /dev/null > /dev/null
A1=$XO_REMOTE_JOB_ID
wait_for_state "$A1" running || fail "home A's long job did not begin running"
xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_A" xo-mark-job.sh a2 "$LOG_A" 0 < /dev/null > /dev/null
A2=$XO_REMOTE_JOB_ID
xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_EDGE" xo-mark-job.sh b1 "$LOG_B" 0 < /dev/null > /dev/null
B1=$XO_REMOTE_JOB_ID
B_BEGAN=$(date +%s)
xo_remote_job_wait "$ACCOUNT_HOME" "$B1" || fail "$XO_REMOTE_JOB_ERROR"
B_ELAPSED=$(( $(date +%s) - B_BEGAN ))
[ "$XO_REMOTE_JOB_EXIT" -eq 0 ] || fail "home B's job behind home A's long job did not complete"
...
xo_remote_job_reap "$ACCOUNT_HOME" "$B1" || fail "home B's job could not be reaped"
xo_remote_job_wait "$ACCOUNT_HOME" "$A1" || fail "$XO_REMOTE_JOB_ERROR"
xo_remote_job_wait "$ACCOUNT_HOME" "$A2" || fail "$XO_REMOTE_JOB_ERROR"
[ "$(printf '%s' "$(cat "$LOG_A")")" = "$(printf 'a1\na2')" ] \
  || fail "home A's jobs did not execute in stage order: $(cat "$LOG_A")"
xo_remote_job_reap "$ACCOUNT_HOME" "$A1" || fail "home A's first job could not be reaped"
xo_remote_job_reap "$ACCOUNT_HOME" "$A2" || fail "home A's second job could not be reaped"
...
: > "$LOG_A"
xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_A" xo-mark-job.sh hold "$LOG_A" 2 < /dev/null > /dev/null
HOLD=$XO_REMOTE_JOB_ID
wait_for_state "$HOLD" running || fail "the lane-holding job did not begin running"
RAPID_IDS=()
for tag in r1 r2 r3 r4 r5; do
  xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_A" xo-mark-job.sh "$tag" "$LOG_A" 0 < /dev/null > /dev/null
  RAPID_IDS+=("$XO_REMOTE_JOB_ID")
done
xo_remote_job_wait "$ACCOUNT_HOME" "$HOLD" || fail "$XO_REMOTE_JOB_ERROR"
xo_remote_job_reap "$ACCOUNT_HOME" "$HOLD" || true
for id in "${RAPID_IDS[@]}"; do
  xo_remote_job_wait "$ACCOUNT_HOME" "$id" || fail "$XO_REMOTE_JOB_ERROR"
...
: > "$LOG_A"
xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_A" xo-mark-job.sh publish-hold "$LOG_A" 3 < /dev/null > /dev/null
PUBLISH_HOLD=$XO_REMOTE_JOB_ID
wait_for_state "$PUBLISH_HOLD" running || fail "the publication-order lane holder did not begin running"
(
  {
    printf 'delayed payload\n'
    sleep 5
  } | xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_A" \
    xo-mark-job.sh delayed "$LOG_A" 0
...
xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_A" xo-mark-job.sh fast "$LOG_A" 0 < /dev/null > /dev/null
FAST_STAGE=$XO_REMOTE_JOB_ID
wait "$DELAYED_STAGE_PID" || fail "the delayed stdin stage failed to publish"
DELAYED_STAGE=$(cat "$TMP_ROOT/delayed-stage-id")
FAST_SEQ=$(xo_remote_job_read_number "$STATE_ROOT/jobs/$FAST_STAGE" seq) \
  || fail "the fast stage lost its sequence"
DELAYED_SEQ=$(xo_remote_job_read_number "$STATE_ROOT/jobs/$DELAYED_STAGE" seq) \
...
xo_remote_job_wait "$ACCOUNT_HOME" "$PUBLISH_HOLD" || fail "$XO_REMOTE_JOB_ERROR"
xo_remote_job_wait "$ACCOUNT_HOME" "$FAST_STAGE" || fail "$XO_REMOTE_JOB_ERROR"
xo_remote_job_wait "$ACCOUNT_HOME" "$DELAYED_STAGE" || fail "$XO_REMOTE_JOB_ERROR"
[ "$(cat "$LOG_A")" = "$(printf 'publish-hold\nfast\ndelayed')" ] \
  || fail "execution order diverged from publication sequence: $(tr '\n' ' ' < "$LOG_A")"
xo_remote_job_reap "$ACCOUNT_HOME" "$PUBLISH_HOLD" || true
xo_remote_job_reap "$ACCOUNT_HOME" "$FAST_STAGE" || true
xo_remote_job_reap "$ACCOUNT_HOME" "$DELAYED_STAGE" || true
...
# never executes it.
xo_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$HOME_A" xo-mark-job.sh hold2 "$LOG_A" 4 < /dev/null > /dev/null
HOLD2=$XO_REMOTE_JOB_ID
...
xo_on ios xo-touch-job.sh "$QUEUED_EFFECT" > /dev/null 2>&1 &
QUEUED_CALLER=$!
QUEUED_JOB=
for _ in $(seq 1 200); do
...
xo_remote_job_wait "$ACCOUNT_HOME" "$HOLD2" || fail "$XO_REMOTE_JOB_ERROR"
xo_remote_job_reap "$ACCOUNT_HOME" "$HOLD2" || true
sleep 1
...
xo_on build xo-two-phase-job.sh "$RUN_START" "$RUN_FINISH" 8 > /dev/null 2>&1 &
RUNNING_CALLER=$!
for _ in $(seq 1 200); do
  [ -f "$RUN_START" ] && break
  sleep 0.05
...
env XO_HOME="$LOCAL_HOME" XO_ROOT_OVERRIDE="$REMOTE_ROOT" \
  XO_SSH_BIN="$FAKEBIN/fake-ssh" \
  XO_FAKE_REMOTE_ENTRYPOINT="$REMOTE_ROOT/bin/xo-remote-entrypoint.sh" \
  XO_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" XO_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  bash -c '
    "$1/bin/xo-on.sh" build xo-two-phase-job.sh "$2" "$3" 12 >/dev/null 2>&1 &
...
for tag in c1 c2 c3; do
  rc=0
  xo_run_timed 15 env XO_HOME="$LOCAL_HOME" XO_ROOT_OVERRIDE="$REMOTE_ROOT" \
    XO_SSH_BIN="$FAKEBIN/fake-ssh" \
    XO_FAKE_REMOTE_ENTRYPOINT="$REMOTE_ROOT/bin/xo-remote-entrypoint.sh" \
    XO_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" XO_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
    "$ROOT/bin/xo-on.sh" ios xo-touch-job.sh "$TMP_ROOT/burst-$tag" >/dev/null 2>&1 || rc=$?
...
printf 'rsm\n' > "$HOME_A/.xo-secondmate-home"
printf '# fixture secondmate home\n' > "$HOME_A/AGENTS.md"
mkdir -p "$HOME_A/state" "$HOME_A/bin"
rc=0
xo_run_timed 20 env XO_HOME="$LOCAL_HOME" XO_ROOT_OVERRIDE="$REMOTE_ROOT" \
  XO_SSH_BIN="$FAKEBIN/fake-ssh" \
  XO_FAKE_REMOTE_ENTRYPOINT="$REMOTE_ROOT/bin/xo-remote-entrypoint.sh" \
  XO_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" XO_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh state rsm \
...
} | xo_on --stdin ios xo-stdin-probe.sh > "$TMP_ROOT/slow-payload-out" 2> "$TMP_ROOT/slow-payload-err" || rc=$?
...
xo_on --stdin ios xo-stdin-probe.sh < "$TMP_ROOT/payload" > "$TMP_ROOT/payload-out" 2>/dev/null \
...
xo_remote_job_process_start "$$" > "$LIVE_STAGE_BUILD/.owner-start" \
