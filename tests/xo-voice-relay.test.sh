#!/usr/bin/env bash
# tests/xo-voice-relay.test.sh - the spoken interface's wire format, read scope and handover.
#
...
# handed to xo rather than done by the voice agent. The latency work that
...
TMP_ROOT=$(xo_test_tmproot xo-voice-relay)
...
EOF
  ...
  xo_write_meta "$HOME_FIXTURE/state/alpha-one.meta" \
    kind=ship mode=no-mistakes window=xo:xo-alpha-one \
    pr=https://github.com/example/alpha/pull/7
  xo_write_meta "$HOME_FIXTURE/state/gamma-three.meta" kind=ship mode=direct-PR
  printf 'working: reading the failing test\n' > "$HOME_FIXTURE/state/alpha-one.status"
  # The bracketed shape, which is what bin/xo-secondmate-report.sh writes and
...
records_status() {
  python3 "$ROOT/bin/xo_voice_records.py" status --home "$HOME_FIXTURE" "$@"
}
...
import xo_voice_frame as frame
...
spec = importlib.util.spec_from_file_location(
    "relay", str(pathlib.Path(sys.argv[1]) / "xo-voice-relay.py"))
...
relay = importlib.util.module_from_spec(spec)
spec.loader.exec_module(relay)
import xo_voice_records as records

home = sys.argv[2]
for name in ("XO_VOICE_REGION", "XO_VOICE_MODEL", "XO_VOICE_PROFILE", "XO_VOICE_ID",
             "XO_CONFIG_OVERRIDE"):
...
os.environ["XO_VOICE_REGION"] = "eu-elsewhere-2"
os.environ["XO_VOICE_ID"] = "amy"
options = relay.resolve_settings(relay.parse_args(["--serve", "--home", home]))
...
os.environ.pop("XO_VOICE_REGION", None)
os.environ.pop("XO_VOICE_ID", None)

# THE PROFILE IS READ BY PRESENCE, NOT BY TRUTHINESS. An empty XO_VOICE_PROFILE is
...
# they just opted out of. xo-inbox.sh reads its own equivalent that way and
...
os.environ["XO_VOICE_PROFILE"] = ""
options = relay.resolve_settings(relay.parse_args(["--serve", "--home", home]))
check(options.profile == "",
      "an empty XO_VOICE_PROFILE must force ambient credentials: %r" % options.profile)

os.environ["XO_VOICE_PROFILE"] = "another-profile"
options = relay.resolve_settings(relay.parse_args(["--serve", "--home", home]))
check(options.profile == "another-profile",
      "a set XO_VOICE_PROFILE should override the file: %r" % options.profile)
os.environ.pop("XO_VOICE_PROFILE", None)

# An empty region, by contrast, is not a choice about anything, so it still falls
# through to the file rather than refusing.
os.environ["XO_VOICE_REGION"] = ""
options = relay.resolve_settings(relay.parse_args(["--serve", "--home", home]))
check(options.region == "eu-somewhere-1",
...
set +e
help_out=$(python3 "$ROOT/bin/xo-voice-relay.py" --help 2>&1)
help_code=$?
set -e
...
#
# EVERY XO_INBOX_ VARIABLE IS NEUTRALIZED HERE, at the harness rather than in each
...
done < <(env | sed -n 's/^\(XO_INBOX_[A-Za-z0-9_]*\)=.*/\1/p' | sort -u)
inbox_env+=(XO_HOME="$CONFIG_HOME" XO_STATE_OVERRIDE="$CONFIG_HOME/state"
            XO_CONFIG_OVERRIDE="$CONFIG_HOME/config")
...
INBOX_FAKEBIN=$(xo_fakebin "$TMP_ROOT/inbox-fake")
...
set +e
ask_out=$(env "${inbox_env[@]}" "$ROOT/bin/xo-inbox.sh" ask "how is the fleet" 2>&1)
ask_code=$?
set -e
...
set +e
say_out=$(printf '' | env "${inbox_env[@]}" "$ROOT/bin/xo-inbox.sh" say 2>&1)
say_code=$?
set -e
...
unconfigured_note=$(env "${inbox_env[@]}" \
  "$ROOT/bin/xo-inbox.sh" note "the handover must work with no configuration") \
...
inbox_help=$("$ROOT/bin/xo-inbox.sh" --help) || fail "xo-inbox.sh --help failed"
...
assert_contains "$inbox_help" 'XO_HOME' \
...
assert_contains "$inbox_help" 'xo-inbox.sh note' \
  "the help must still open with the usage it always had"
pass "xo-inbox.sh --help prints its whole header, privacy paragraph included"

# --- the tool surface the two sides share -----------------------------------
#
# The relay declares the tools and xo_voice_records implements them. Renaming one
...
if names != ["get_fleet_status", "hand_over_to_xo"]:
...
handover = [t["toolSpec"] for t in relay.TOOLS["tools"]
            if t["toolSpec"]["name"] == "hand_over_to_xo"][0]
import json
schema = json.loads(handover["inputSchema"]["json"])
if schema.get("required") != ["request"]:
    sys.exit("hand_over_to_xo must require the request text")
...
import importlib.util, pathlib
spec = importlib.util.spec_from_file_location(
    "client", str(pathlib.Path(sys.argv[1]) / "xo-voice-client.py"))
...
os.environ.pop("XO_VOICE_RELAY", None)
try:
...
os.environ["XO_VOICE_RELAY"] = "/desktop/xo/bin/xo-voice-relay.py"
check(client.parse_args(["--host", "desk"]).relay
      == "/desktop/xo/bin/xo-voice-relay.py",
      "XO_VOICE_RELAY should supply the relay path for a whole shell")
...
def half_start():
    ...
    raise SystemExit("xo-voice-client: the relay closed the connection")
...
try:
    ...
    check("not xo-voice-relay.py" in str(exc),
...
mkdir -p "$LAPTOP"
cp "$ROOT/bin/xo-voice-client.py" "$ROOT/bin/xo_voice_frame.py" "$LAPTOP/"

set +e
copied_out=$(cd "$LAPTOP" && env -u PYTHONPATH python3 ./xo-voice-client.py --help 2>&1)
copied_code=$?
set -e
expect_code 0 "$copied_code" \
  "the client must start with only the two copied files: $copied_out"
assert_contains "$copied_out" 'xo-voice-client.py' \
...
mkdir -p "$SHORT"
cp "$ROOT/bin/xo-voice-client.py" "$SHORT/"
set +e
short_out=$(cd "$SHORT" && env -u PYTHONPATH python3 ./xo-voice-client.py --help 2>&1)
short_code=$?
set -e
[ "$short_code" -ne 0 ] || fail "the client started without xo_voice_frame.py beside it"
assert_contains "$short_out" 'xo_voice_frame' \
...
OPENMIC_FAKEBIN=$(xo_fakebin "$TMP_ROOT/openmic-fake")
...
set +e
openmic_out=$(PATH="$OPENMIC_FAKEBIN:$PATH" python3 "$ROOT/bin/xo-voice-client.py" \
  --host a-desktop --relay /desktop/bin/xo-voice-relay.py --listen open-mic 2>&1)
openmic_code=$?
set -e
...
set +e
PATH="$OPENMIC_FAKEBIN:$PATH" python3 "$ROOT/bin/xo-voice-client.py" \
  --host a-desktop --relay /desktop/bin/xo-voice-relay.py --talk-seconds 0 \
  >/dev/null 2>&1
set -e
...
EOF
xo_write_meta "$LEAK_HOME/state/omega-nine.meta" \
  kind=captain pr=https://github.com/example/omega/pull/11
xo_write_meta "$LEAK_HOME/state/sigma-ten.meta" \
  kind=ship pr=https://github.com/example/sigma/pull/12
xo_write_meta "$LEAK_HOME/state/zeta-eight.meta" \
  kind=ship pr=https://github.com/example/zeta/pull/99

leak_status() {
  python3 "$ROOT/bin/xo_voice_records.py" status --home "$LEAK_HOME" --scope full
}
...
# therefore closed to the states bin/xo-brief.sh gives every crewmate plus the two
# bin/xo-classify-lib.sh adds when a decision closes, and anything else is a note.
...
EOF
xo_write_meta "$VERB_HOME/state/one.meta" kind=ship
xo_write_meta "$VERB_HOME/state/two.meta" kind=ship
xo_write_meta "$VERB_HOME/state/three.meta" kind=ship
...
verb_status() {
  python3 "$ROOT/bin/xo_voice_records.py" status --home "$VERB_HOME" "$@"
}
...
# XO_DATA_OVERRIDE sets XO_STATE_OVERRIDE beside it, so a reader that resolved one
...
EOF
moved=$(XO_DATA_OVERRIDE="$alt_data" verb_status --scope full) \
...
inbox_moved=$(XO_HOME="$VERB_HOME" XO_STATE_OVERRIDE="$VERB_HOME/state" \
  XO_DATA_OVERRIDE="$alt_data" "$ROOT/bin/xo-inbox.sh" status) \
  || fail "xo-inbox status with an overridden data directory failed"
...
EOF
xo_write_meta "$DONE_HOME/state/still-open.meta" \
  kind=ship pr=https://github.com/example/a/pull/1
xo_write_meta "$DONE_HOME/state/ticked-two.meta" \
  kind=ship pr=https://github.com/example/b/pull/2
xo_write_meta "$DONE_HOME/state/older-three.meta" \
  kind=ship pr=https://github.com/example/c/pull/3

done_status() {
  python3 "$ROOT/bin/xo_voice_records.py" status --home "$DONE_HOME" --scope full
}
...
# --- handover ---------------------------------------------------------------
#
# The point of the boundary: real work is queued for xo, not done by the
# voice agent. It reuses bin/xo-inbox.sh rather than carrying a second queue.
...
handed=$(XO_HOME="$HOME_FIXTURE" python3 "$ROOT/bin/xo_voice_records.py" queue \
...
# Exactly one wake, so a spoken request is presented once at xo's next
# check rather than queued twice or lost.
assert_present "$HOME_FIXTURE/state/.wake-queue" \
  "handover should wake xo"
...
pass "handover queues the request for xo and wakes it exactly once"

# The same pairing when the state directory is moved. bin/xo-inbox.sh resolves
# ${XO_STATE_OVERRIDE:-$XO_HOME/state} and the handover queues through it with
...
XO_STATE_OVERRIDE="$alt_state" python3 "$ROOT/bin/xo_voice_records.py" queue \
...
overridden=$(XO_STATE_OVERRIDE="$alt_state" python3 \
  "$ROOT/bin/xo_voice_records.py" status --home "$alt_home") \
...
set +e
empty_out=$(python3 "$ROOT/bin/xo_voice_records.py" queue "   " \
...
bare="$TMP_ROOT/bare"
mkdir -p "$bare"
bare_out=$(python3 "$ROOT/bin/xo_voice_records.py" status --home "$bare") \
...
# real work to xo, and the reply audio and the timing come back down the
...
# client, the frame format, the relay, the reader and bin/xo-inbox.sh.
#
...
cp "$ROOT/bin/xo-voice-client.py" "$ROOT/bin/xo_voice_frame.py" "$E2E/laptop/"
...
- [x] old-six - Shipped the $NEVER_TOKEN integration (repo: alpha) (done 2026-07-01)
EOF
xo_write_meta "$E2E/home/state/alpha-one.meta" kind=ship mode=no-mistakes \
  pr=https://github.com/example/alpha/pull/7
...
# The model stand-in, at exactly the import boundary bin/xo-voice-relay.py uses.
...
here is derived from xo's records rather than from a fixture string, and
it appends one JSON line per session describing what that session was opened
with and what it was asked. tests/xo-voice-relay.test.sh reads that record.

  XO_FAKE_SCRIPT   comma-separated turn kinds: status | handover | clean-end
  XO_FAKE_THINK    seconds before the reply begins, standing in for the model
  XO_FAKE_STATE    file holding the turn counter across the relay's reconnects
  XO_FAKE_LOG      where to append the per-session record
  XO_FAKE_REQUEST  the words the captain uses when asking for real work
  XO_FAKE_EARLY    1 to answer from the first audio in, not from the talk end
...
XO_FAKE_EARLY stands in for the model's own end-of-speech detector firing inside
...
THINK = float(os.environ.get("XO_FAKE_THINK", "0.4"))
REPLY_SECONDS = float(os.environ.get("XO_FAKE_REPLY_SECONDS", "0.4"))
SCRIPT = [s.strip() for s in os.environ.get("XO_FAKE_SCRIPT", "status").split(",")
          if s.strip()]
STATE = os.environ.get("XO_FAKE_STATE", "")
LOG = os.environ.get("XO_FAKE_LOG", "")
REQUEST = os.environ.get("XO_FAKE_REQUEST", "open a pull request for the retry")
EARLY = os.environ.get("XO_FAKE_EARLY", "") == "1"
...
def _status_sentence(result):
    ...
    if notes:
        said += " {} note is queued for the XO.".format(notes)
...
def _queued_sentence(result):
    ...
    return ("That is queued with the XO as {}. I have not done any of it "
...
class _Stream:
    ...
    async def _reply(self):
        ...
        if self.kind == "handover":
            self._say("I am not the XO, so I am handing that to it.")
            result = await self._call_tool("hand_over_to_xo",
                                           {"request": REQUEST})
...
XO_HOME=$E2E/home
XO_FAKE_STATE=$E2E/turn-counter
XO_FAKE_LOG=$E2E/model-sessions.jsonl
XO_FAKE_THINK=0.4
XO_FAKE_REPLY_SECONDS=0.4
XO_FAKE_SCRIPT=status,handover
XO_FAKE_REQUEST=$E2E_REQUEST
...
# interpreter that can reach Bedrock, no xo home.
...
env -i PATH="$E2E/bin:$PATH" HOME="$E2E/laptop-home" PYTHONDONTWRITEBYTECODE=1 \
  python3 "$E2E/laptop/xo-voice-client.py" \
    --host desktop.example \
    --relay "$ROOT/bin/xo-voice-relay.py" \
...
independent=$(python3 "$ROOT/bin/xo_voice_records.py" status \
...
for session in sessions:
    ...
    check(session["tool_names_offered"] ==
          ["get_fleet_status", "hand_over_to_xo"],
...
# The handover turn queues real work and says so. The note is xo's own
# queue, written by bin/xo-inbox.sh, and the agent's confirmation carries the id
# that queue gave it, so it cannot be claiming to have queued something it did
# not.
handover = sessions[1]
check([c["name"] for c in handover["tool_calls"]] == ["hand_over_to_xo"],
...
check("handed to the XO" in transcript,
...
PY
pass "a spoken turn goes out and comes back: the records answer, xo gets the work"
...
grep -v '^XO_FAKE_' "$E2E/bin/desktop.env" > "$SURVIVE/bin/desktop.env"
cat >> "$SURVIVE/bin/desktop.env" <<EOF
XO_FAKE_STATE=$SURVIVE/turn-counter
XO_FAKE_LOG=$SURVIVE/model-sessions.jsonl
XO_FAKE_THINK=0.4
XO_FAKE_REPLY_SECONDS=0.4
XO_FAKE_SCRIPT=clean-end,status
EOF
...
env -i PATH="$PATH" HOME="$E2E/desktop-home" PYTHONPATH="$E2E/fakesdk" \
  PYTHONDONTWRITEBYTECODE=1 XO_HOME="$E2E/home" \
  AWS_ACCESS_KEY_ID="$E2E_KEY" \
  AWS_SECRET_ACCESS_KEY=desktop-secret-not-a-real-key \
  python3 - "$ROOT/bin" <<'PY'
...
relay = os.path.join(sys.argv[1], "xo-voice-relay.py")
...
#
# `xo-voice-relay.py --self-test <clip.pcm>` is what docs/voice-relay.md tells the
...
relay_self_test() {
  ...
  env -i PATH="$PATH" HOME="$E2E/desktop-home" PYTHONPATH="$E2E/fakesdk" \
    PYTHONDONTWRITEBYTECODE=1 XO_HOME="$E2E/home" \
    XO_FAKE_STATE="$SELFTEST/turn-counter" XO_FAKE_LOG="$SELFTEST/sessions.jsonl" \
    XO_FAKE_THINK=0.4 XO_FAKE_REPLY_SECONDS=0.4 XO_FAKE_SCRIPT=status \
    AWS_ACCESS_KEY_ID="$E2E_KEY" \
    AWS_SECRET_ACCESS_KEY=desktop-secret-not-a-real-key \
    "$@" python3 "$ROOT/bin/xo-voice-relay.py" --self-test "$clip"
}
...
set +e
early_out=$(relay_self_test "$SELFTEST/ends-in-silence.pcm" XO_FAKE_EARLY=1 \
