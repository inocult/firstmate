...
WRITER="$ROOT/bin/xo-home-summary-refresh.sh"
SNAPSHOT="$ROOT/bin/xo-fleet-snapshot.sh"
WATCH="$ROOT/bin/xo-watch.sh"
TMP_ROOT=$(xo_test_tmproot xo-home-summary-refresh)
...
#!/usr/bin/env bash
if [ -n "${XO_TEST_NM_MARKER:-}" ]; then
  printf '%s\n' "$$" > "$XO_TEST_NM_MARKER"
  sleep "${XO_TEST_NM_SLEEP:-30}"
fi
exit 0
SH
...
printf '# Seeded XO home\n' > "$HOME_DIR/AGENTS.md"
printf 'mate\n' > "$HOME_DIR/.xo-secondmate-home"
xo_git_init_commit "$HOME_DIR/projects/task"
git -C "$HOME_DIR/projects/task" checkout -q -b xo/ledger-task
cat > "$HOME_DIR/data/backlog.md" <<'EOF'
## In flight
- [ ] ledger-task - Publish the home ledger (repo: xo) (kind: ship) (since 2026-08-28)

## Queued

## Done
EOF
xo_write_meta "$HOME_DIR/state/ledger-task.meta" \
  "window=fmtest:xo-ledger-task" \
  "worktree=$HOME_DIR/projects/task" \
  "project=xo" \
  "harness=claude" \
  "kind=ship" \
  "mode=no-mistakes" \
  "spawn_gen=xo.ledger123456"
busy_gen=$("$ROOT/bin/xo-busy-event.sh" arm "$HOME_DIR/state" ledger-task)
"$ROOT/bin/xo-busy-event.sh" apply "$HOME_DIR/state" ledger-task idle \
...
run_writer() {  # <now> <epoch> [writer args...]
  ...
  shift 2
  PATH="$FAKEBIN:$PATH" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" \
    XO_SNAPSHOT_NOW="$now" XO_SNAPSHOT_NOW_EPOCH="$epoch" \
    "$WRITER" "$@"
}
...
run_producer() {  # <now> <epoch>
  PATH="$FAKEBIN:$PATH" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" \
    XO_SNAPSHOT_NOW="$1" XO_SNAPSHOT_NOW_EPOCH="$2" \
...
PATH="$FAKEBIN:$PATH" \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" \
  XO_SNAPSHOT_NOW="$NOW_TWO" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_TWO" \
  XO_POLL=1 XO_SIGNAL_GRACE=0 XO_CHECK_INTERVAL=9999999 XO_HEARTBEAT=9999999 \
...
printf 'large\n' > "$LARGE_HOME/.xo-secondmate-home"
large_id_suffix=$(printf 'i%.0s' $(seq 1 110))
{
  printf '%s\n' '## In flight'
  i=1
  while [ "$i" -le 1200 ]; do
    printf '%s\n' "- [ ] orphan-$i-$large_id_suffix - Missing metadata (repo: xo) (kind: ship)"
    i=$((i + 1))
  done
  printf '%s\n' '' '## Queued' '' '## Done'
...
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$LARGE_HOME" \
  XO_SNAPSHOT_NOW="$NOW_ONE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_ONE" \
  "$SNAPSHOT" --json > "$TMP_ROOT/large-snapshot.json" \
  || fail "fleet snapshot json mode failed for a large backlog"
jq -e '.schema == "xo-fleet-snapshot.v1"
...
jq -e '.schema == "xo-secondmate-home-summary.v1"' "$TMP_ROOT/large-summary.json" \
...
jq -e '.schema == "xo-secondmate-home-summary.v1"' \
...
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$STATELESS_HOME" \
  XO_SNAPSHOT_NOW="$NOW_ONE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_ONE" \
  "$SNAPSHOT" --json > "$TMP_ROOT/stateless-snapshot.json" \
  || fail "fleet snapshot json mode failed without a state directory"
jq -e '.schema == "xo-fleet-snapshot.v1" and (.tasks | length) == 0' \
...
printf 'large-child\n' > "$LARGE_CHILD_HOME/.xo-secondmate-home"
{
  printf '%s\n' '## In flight'
  i=1
  while [ "$i" -le 600 ]; do
...
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$LARGE_CHILD_HOME" \
  XO_SNAPSHOT_NOW="$NOW_ONE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_ONE" \
...
printf -- '- large-child - fixture domain (home: %s; scope: fixture work; projects: xo; added 2026-08-28)\n' \
  "$LARGE_CHILD_HOME" > "$LARGE_PARENT_HOME/data/secondmates.md"
printf '%s\n' '## In flight' '' '## Queued' '' '## Done' \
  > "$LARGE_PARENT_HOME/data/backlog.md"
xo_write_secondmate_meta "$LARGE_PARENT_HOME/state/large-child.meta" \
  "$LARGE_CHILD_HOME" "fmtest:xo-large-child" xo claude
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$LARGE_PARENT_HOME" \
  XO_SNAPSHOT_NOW="$NOW_ONE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_ONE" \
...
printf 'cadence\n' > "$CADENCE_HOME/.xo-secondmate-home"
...
## Done
EOF
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$CADENCE_HOME" \
  XO_SNAPSHOT_NOW="$NOW_TWO" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_TWO" \
  "$WRITER" || fail "could not seed the cadence ledger"
touch -t 203801010000 "$CADENCE_HOME/state/home-summary.json"
PATH="$FAKEBIN:$PATH" \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$CADENCE_HOME" \
  XO_SNAPSHOT_NOW="$NOW_THREE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_THREE" \
  XO_POLL=1 XO_HOME_SUMMARY_INTERVAL=1 XO_SIGNAL_GRACE=0 \
  XO_CHECK_INTERVAL=9999999 XO_HEARTBEAT=9999999 \
...
text = path.read_text()
path.write_text(text.replace("## Queued\n\n## Done", "## Queued\n- [ ] cadence-task - Publish without a status signal (repo: xo) (kind: ship)\n\n## Done"))
PY
i=0
...
printf -- '- mate - fixture domain (home: %s; scope: fixture work; projects: xo; added 2026-08-28)\n' \
...
## Done
EOF
xo_write_secondmate_meta "$PARENT_HOME/state/mate.meta" "$HOME_DIR" \
  "fmtest:xo-mate" xo claude
PATH="$FAKEBIN:$PATH" \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$PARENT_HOME" \
  XO_SNAPSHOT_NOW="$NOW_TWO" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_TWO" \
...
PATH="$FAKEBIN:$PATH" \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" \
  XO_SNAPSHOT_NOW="$NOW_THREE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_THREE" \
  XO_TEST_NM_MARKER="$SLOW_MARKER" XO_TEST_NM_SLEEP=30 \
...
PATH="$FAKEBIN:$PATH" \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" \
  XO_SNAPSHOT_NOW="$NOW_THREE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_THREE" \
  XO_TEST_NM_MARKER="$SUCCESS_MARKER" XO_TEST_NM_SLEEP=1 \
...
XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" bash -c '
  . "$1/bin/xo-wake-lib.sh"
  xo_lock_acquire_wait "$2/state/.home-summary-refresh.lock"
  : > "$3"
  sleep 30
...
started=$(date +%s)
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" \
  XO_HOME_SUMMARY_TIMEOUT=1 "$WRITER" --best-effort \
...
exec "$XO_TEST_REAL_JQ" "$@"
SH
chmod +x "$HANGBIN/jq"
started=$(date +%s)
PATH="$HANGBIN:$FAKEBIN:$PATH" XO_TEST_REAL_JQ="$REAL_JQ" \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" XO_HOME_SUMMARY_TIMEOUT=1 \
  "$WRITER" --best-effort \
...
#!/usr/bin/env bash
for arg in "$@"; do
  if [ "$arg" = "$XO_TEST_STALLED_STATE" ]; then
    sleep 30
  fi
done
exec "$XO_TEST_REAL_MKDIR" "$@"
SH
chmod +x "$MKBIN/mkdir"
started=$(date +%s)
PATH="$MKBIN:$FAKEBIN:$PATH" XO_TEST_REAL_MKDIR="$REAL_MKDIR" \
  XO_TEST_STALLED_STATE="$HOME_DIR/state" XO_ROOT_OVERRIDE="$ROOT" \
  XO_HOME="$HOME_DIR" XO_HOME_SUMMARY_TIMEOUT=1 \
...
#!/usr/bin/env bash
if [ ! -e "$XO_TEST_SIGNAL_MARKER" ]; then
  : > "$XO_TEST_SIGNAL_MARKER"
  exit 143
fi
exec "$XO_TEST_REAL_ENV" "$@"
SH
chmod +x "$SIGNALBIN/env"
rm -f "$HOME_DIR/state/.home-summary-refresh.log"
PATH="$SIGNALBIN:$FAKEBIN:$PATH" XO_TEST_REAL_ENV="$REAL_ENV" \
  XO_TEST_SIGNAL_MARKER="$SIGNAL_MARKER" XO_TIMEOUT_MECHANISM_OVERRIDE=bash \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" "$WRITER" --best-effort \
...
if ! PATH="$SIGNALBIN:$FAKEBIN:$PATH" XO_TEST_REAL_ENV="$REAL_ENV" \
  XO_TEST_SIGNAL_MARKER="$SIGNAL_MARKER" XO_TIMEOUT_MECHANISM_OVERRIDE=bash \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" WRITER="$WRITER" python3 - <<'PY'
import os
import subprocess
import time
...
printf '# Seeded XO home\n' > "$COST_HOME/AGENTS.md"
printf 'cost\n' > "$COST_HOME/.xo-secondmate-home"
xo_git_init_commit "$COST_HOME/projects/task"
cat > "$COST_HOME/data/backlog.md" <<'EOF'
## In flight
- [ ] cost-task - Publish from an accumulated home (repo: xo) (kind: ship) (since 2026-08-28)

## Queued

## Done
EOF
xo_write_meta "$COST_HOME/state/cost-task.meta" \
  "window=fmtest:xo-cost-task" \
  "worktree=$COST_HOME/projects/task" \
  "project=xo" \
  "harness=claude" \
  "kind=ship" \
  "mode=no-mistakes" \
  "spawn_gen=xo.cost123456"
cost_busy_gen=$("$ROOT/bin/xo-busy-event.sh" arm "$COST_HOME/state" cost-task)
"$ROOT/bin/xo-busy-event.sh" apply "$COST_HOME/state" cost-task idle \
...
PY
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$COST_HOME" \
  XO_SNAPSHOT_NOW="$NOW_ONE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_ONE" \
  XO_HOME_SUMMARY_TIMEOUT=30 "$WRITER" --best-effort \
...
printf 'remote\n' > "$REMOTE_HOME/.xo-secondmate-home"
cat > "$REMOTE_HOME/data/backlog.md" <<'EOF'
## In flight
- [ ] rsm - Read remote current state (repo: xo) (kind: ship) (since 2026-08-28)
...
EOF
xo_write_meta "$REMOTE_HOME/state/rsm.meta" \
  "window=remote:rsm" \
...
started=$(date +%s)
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$REMOTE_HOME" \
  XO_SSH_BIN="$TMP_ROOT/sshbin/stalled-ssh" XO_TEST_SSH_CALLED="$TMP_ROOT/stalled-ssh.called" \
  XO_SNAPSHOT_NOW="$NOW_TWO" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_TWO" \
  XO_SNAPSHOT_CREW_STATE_TIMEOUT=2 \
...
XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$BEAT_HOME" bash -c '
  . "$1/bin/xo-wake-lib.sh"
  xo_lock_acquire_wait "$2/state/.home-summary-refresh.lock"
  : > "$3"
  sleep 120
...
PATH="$FAKEBIN:$PATH" \
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$BEAT_HOME" \
  XO_SNAPSHOT_NOW="$NOW_THREE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_THREE" \
  XO_POLL=1 XO_HOME_SUMMARY_INTERVAL=1 XO_HOME_SUMMARY_TIMEOUT=90 \
  XO_SIGNAL_GRACE=0 XO_CHECK_INTERVAL=9999999 XO_HEARTBEAT=9999999 \
...
printf 'restart\n' > "$RESTART_HOME/.xo-secondmate-home"
xo_git_init_commit "$RESTART_HOME/projects/task"
cat > "$RESTART_HOME/data/backlog.md" <<'EOF'
## In flight
- [ ] restart-task - Preserve publication single flight (repo: xo) (kind: ship) (since 2026-08-28)

## Queued

## Done
EOF
xo_write_meta "$RESTART_HOME/state/restart-task.meta" \
  "window=fmtest:xo-restart-task" \
  "worktree=$RESTART_HOME/projects/task" \
  "project=xo" \
  "harness=claude" \
  "kind=ship" \
  "mode=no-mistakes" \
  "spawn_gen=xo.restart123456"
RESTART_LOCK_MARKER="$TMP_ROOT/restart-lock-held"
XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$RESTART_HOME" bash -c '
  . "$1/bin/xo-wake-lib.sh"
  xo_lock_acquire_wait "$2/state/.home-summary-refresh.lock"
  : > "$3"
  sleep 30
...
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$RESTART_HOME" \
  XO_POLL=1 XO_HOME_SUMMARY_INTERVAL=999999 XO_HOME_SUMMARY_TIMEOUT=2 \
  XO_SIGNAL_GRACE=0 XO_CHECK_INTERVAL=9999999 XO_HEARTBEAT=9999999 \
...
LOCK_HOLDER_PID=
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$RESTART_HOME" \
  XO_HOME_SUMMARY_IF_IDLE=1 "$WRITER" --best-effort \
...
printf 'report\n' > "$REPORT_HOME/.xo-secondmate-home"
...
run_bootstrap_detect() {
  local threshold=${2:-2}
  PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$1" \
    XO_HOME_SUMMARY_FAILURE_REPORT="$threshold" \
    XO_BOOTSTRAP_DETECT_ONLY=1 XO_BOOTSTRAP_NETWORK=skip \
...
printf 'compat\n' > "$COMPAT_HOME/.xo-secondmate-home"
...
## Done
EOF
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$COMPAT_HOME" \
  XO_SNAPSHOT_NOW="$NOW_ONE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_ONE" \
...
if [ "$#" -eq 2 ] && [ "$1" = -u ] && [ "$2" = +%Y-%m-%dT%H:%M:%SZ ]; then
  python3 - "$XO_TEST_ORDER_START" "$XO_TEST_ORDER_EARLY" "$XO_TEST_ORDER_LATE" <<'PY'
import sys
import time
...
exec "$XO_TEST_REAL_DATE" "$@"
SH
chmod +x "$ORDER_DATE_BIN/date"
ORDER_LOCK_MARKER="$TMP_ROOT/order-lock-held"
XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$ORDER_HOME" bash -c '
  . "$1/bin/xo-wake-lib.sh"
  xo_lock_acquire_wait "$2/state/.home-summary-refresh.lock"
  : > "$3"
  sleep 30
...
PATH="$ORDER_DATE_BIN:$FAKEBIN:$PATH" XO_TEST_REAL_DATE="$REAL_DATE" \
  XO_TEST_ORDER_START="$order_started" XO_TEST_ORDER_EARLY="$NOW_ONE" \
  XO_TEST_ORDER_LATE="$NOW_THREE" XO_ROOT_OVERRIDE="$ROOT" \
  XO_HOME="$ORDER_HOME" XO_HOME_SUMMARY_TIMEOUT=2 \
  "$WRITER" --best-effort \
  || fail "ordered timeout changed the best-effort caller result"
kill "$LOCK_HOLDER_PID" >/dev/null 2>&1 || true
wait "$LOCK_HOLDER_PID" >/dev/null 2>&1 || true
LOCK_HOLDER_PID=
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$ORDER_HOME" \
  XO_SNAPSHOT_NOW="$NOW_TWO" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_TWO" \
...
PATH="$FAKEBIN:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$REPORT_HOME" \
  XO_SNAPSHOT_NOW="$NOW_THREE" XO_SNAPSHOT_NOW_EPOCH="$EPOCH_THREE" \
