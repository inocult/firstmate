#!/usr/bin/env bash
# Contract tests for bin/xo-test-run.sh - the single owner of behavior suite
...
# focused scheduler checks, not the complete XO suite.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

RUNNER="$ROOT/bin/xo-test-run.sh"

assert_present "$RUNNER" "bin/xo-test-run.sh is missing"
[ -x "$RUNNER" ] || fail "bin/xo-test-run.sh must be executable"
...
test_family_selection() {
  ...
  printf '%s\n' "$listed" | grep -Fq 'tests/xo-test-run.test.sh' \
    || fail "pure-contract-unit must include xo-test-run.test.sh"
...
test_single_script_selection() {
  local listed
  listed=$("$RUNNER" --list tests/xo-lint.test.sh)
  [ "$listed" = "tests/xo-lint.test.sh" ] \
    || fail "single-script list expected tests/xo-lint.test.sh, got: $listed"
...
test_changed_file_selection_is_conservative() {
  ...
  listed=$("$RUNNER" --list tests/xo-brief.test.sh)
  [ "$listed" = "tests/xo-brief.test.sh" ] \
...
init_changed_fixture_repo() {
  ...
  cp "$RUNNER" "$repo/bin/xo-test-run.sh"
  cp "$ROOT/tests/git-config-helpers.sh" "$repo/tests/"
  chmod +x "$repo/bin/xo-test-run.sh"
  for script in \
    xo-brief.test.sh \
    xo-ask-user-authority.test.sh \
    xo-documentation-audiences.test.sh \
    xo-test-isolation-proof.test.sh \
    xo-test-run.test.sh \
    xo-test-fixtures.test.sh \
    xo-cd-pretool-check.test.sh \
    xo-daemon.test.sh \
    xo-harness-adapter-instructions-live-e2e.test.sh \
    xo-harness-adapter-references.test.sh \
    xo-skills-installer-live-e2e.test.sh \
    xo-backend-herdr-smoke.test.sh \
    xo-secondmate-safety.test.sh \
    xo-session-start.test.sh \
    xo-afk-pi-herdr-return-e2e.test.sh \
    xo-backend.test.sh \
    xo-pr-merge.test.sh \
    xo-procevent-quota.test.sh \
    xo-quota-choose.test.sh \
    xo-pi-watch-extension.test.sh \
    xo-pi-windows-shell-invocation.test.sh \
    xo-afk-return.test.sh \
    xo-bearings-snapshot.test.sh \
    xo-backend-cmux.test.sh \
    xo-backend-zellij.test.sh \
    xo-control-herdr-smoke.test.sh \
    xo-backend-orca.test.sh; do
    printf '#!/usr/bin/env bash\n# tests/lib.sh\n' >"$repo/tests/$script"
    chmod +x "$repo/tests/$script"
  done
  : >"$repo/tests/lib.sh"
  : >"$repo/tests/xo-backend-herdr-eventwait.test.py"
  : >"$repo/bin/xo-supervisor-target-lib.sh"
  : >"$repo/bin/xo-control-lib.sh"
  : >"$repo/bin/xo-timeout-lib.sh"
  : >"$repo/bin/xo-procevent-quota.sh"
  : >"$repo/bin/xo-quota-axi-lib.sh"
  : >"$repo/bin/xo-quota-choose.sh"
  ...
  printf '# shared-probe-lib.sh\n' >>"$repo/tests/xo-backend-herdr-smoke.test.sh"
  ...
  printf '. "$ROOT/bin/shared-probe-lib.sh"\n' >"$repo/bin/xo-watch-probe.sh"
  printf '# .claude/settings.json\n# .pi/extensions/xo-primary-turnend-guard.ts\n' \
    >>"$repo/tests/xo-cd-pretool-check.test.sh"
  printf '# .pi/extensions/xo-primary-pi-watch.ts\n' >>"$repo/tests/xo-pi-watch-extension.test.sh"
  ...
  mkdir -p \
    "$repo/skills/playbooks/example" \
    "$repo/skills/playbooks/harness-adapters/references/common" \
    "$repo/skills/playbooks/quota-array-dispatch" \
    "$repo/skills/playbooks/board/assets"
  : >"$repo/skills/playbooks/example/SKILL.md"
  : >"$repo/skills/playbooks/example/LICENSE"
  : >"$repo/skills/playbooks/harness-adapters/SKILL.md"
  : >"$repo/skills/playbooks/harness-adapters/references/common/dispatch.md"
  : >"$repo/skills/playbooks/quota-array-dispatch/SKILL.md"
  : >"$repo/skills/playbooks/board/assets/template.html"
  ln -s ../../skills/playbooks/example "$repo/.agents/skills/example"
  ln -s ../../skills/playbooks/harness-adapters "$repo/.agents/skills/harness-adapters"
  printf '# .agents/skills/board/assets/template.html\n' >>"$repo/tests/xo-bearings-snapshot.test.sh"
  : >"$repo/.claude/settings.json"
  : >"$repo/.pi/extensions/xo-primary-pi-watch.ts"
  : >"$repo/.pi/extensions/xo-primary-turnend-guard.ts"
  mkdir -p "$repo/.pi/extensions/lib"
  : >"$repo/.pi/extensions/lib/xo-operational-input.ts"
  : >"$repo/docs/xo-test-isolation-proof.md"
...
test_task_marker_refuses_the_primary_checkout() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-primary.XXXXXX")
  repo="$tmp/repo"
  linked="$tmp/linked"
  init_primary_and_linked_worktree "$repo" "$linked"

  # Marker set, primary checkout: refuse, name the primary, and run nothing.
  out=$(XO_TASK_ID=probe-task "$repo/bin/xo-test-run.sh" tests/probe.test.sh 2>&1) && rc=0 || rc=$?
  ...
  assert_contains "$out" "XO_TASK_ID=probe-task" "refusal did not name the task marker"
  assert_contains "$out" "task worktree" "refusal did not point at the task worktree"
  assert_not_contains "$out" "XO_TEST_BEGIN" "the refusal must happen before any suite runs"
  ...
  XO_TASK_ID=probe-task "$linked/bin/xo-test-run.sh" tests/probe.test.sh >/dev/null 2>&1 \
  ...
  "$repo/bin/xo-test-run.sh" tests/probe.test.sh >/dev/null 2>&1 \
  ...
  rm -f "$repo/ran"
  out=$(XO_TASK_ID=probe-task "$repo/bin/xo-test-run.sh" --list tests/probe.test.sh 2>&1) \
...
test_changed_runner_surfaces_select_their_family() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-owner-scope.XXXXXX")
  repo="$tmp/repo"
  ...
  printf '\n' >>"$repo/bin/xo-test-run.sh"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD | LC_ALL=C sort)
  case "$listed" in
  ...
  case "$listed" in
    *tests/xo-ask-user-authority.test.sh*) ;;
    *) fail "runner change did not select its pure-contract-unit family: $listed" ;;
  esac
  ...
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD | LC_ALL=C sort)
  case "$listed" in
    *tests/xo-documentation-audiences.test.sh*) ;;
...
test_shell_line_ending_policy_selects_runner_contract() {
  local tmp repo listed
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-attributes.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"
  printf '*.sh text eol=lf\n' >"$repo/.gitattributes"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-test-run.test.sh" \
...
test_changed_dependency_selection_and_unmapped_failure() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-changed.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"

  printf '\n' >>"$repo/tests/lib.sh"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-pr-merge.test.sh" "shared helper selects pr-forge dependents"
  assert_contains "$listed" "tests/xo-secondmate-safety.test.sh" "shared helper selects secondmate dependents"
  assert_contains "$listed" "tests/xo-bearings-snapshot.test.sh" "shared helper selects snapshot dependents"
  ...
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-pr-merge.test.sh" "git-config helper selects lib.sh dependents"
  assert_contains "$listed" "tests/xo-secondmate-safety.test.sh" "git-config helper selects secondmate dependents"
  assert_contains "$listed" "tests/xo-bearings-snapshot.test.sh" "git-config helper selects snapshot dependents"
  ...
  printf '\n' >>"$repo/tests/xo-backend-herdr-eventwait.test.py"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-backend-herdr-smoke.test.sh" "eventwait test selects Herdr coverage"
  assert_contains "$listed" "tests/xo-backend.test.sh" "eventwait test selects backend coverage"
  git -C "$repo" add tests/xo-backend-herdr-eventwait.test.py
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm eventwait-change

  printf '\n' >>"$repo/bin/xo-supervisor-target-lib.sh"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-daemon.test.sh" "supervisor target selects daemon coverage"
  assert_contains "$listed" "tests/xo-afk-return.test.sh" "supervisor target selects afk coverage"
  git -C "$repo" add bin/xo-supervisor-target-lib.sh
  ...
  printf '\n' >>"$repo/.pi/extensions/xo-primary-turnend-guard.ts"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-ask-user-authority.test.sh" "skill source selects pure contract coverage"
  assert_contains "$listed" "tests/xo-skills-installer-live-e2e.test.sh" "skill source selects the opt-in installer guard"
  assert_contains "$listed" "tests/xo-cd-pretool-check.test.sh" "Claude and Pi source selects hook coverage"
  assert_contains "$listed" "tests/xo-pi-watch-extension.test.sh" "Pi source selects watcher coverage"
  assert_contains "$listed" "tests/xo-pi-windows-shell-invocation.test.sh" \
  ...
  git -C "$repo" add .pi/extensions/lib/xo-operational-input.ts
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm operational-input-source-change

  printf '\n' >>"$repo/skills/playbooks/harness-adapters/references/common/dispatch.md"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-harness-adapter-references.test.sh" "harness adapter reference selects portable structural coverage"
  assert_contains "$listed" "tests/xo-harness-adapter-instructions-live-e2e.test.sh" "harness adapter reference selects opt-in instruction coverage"
  git -C "$repo" add skills/playbooks/harness-adapters
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm harness-adapter-reference-change

  printf '\n' >>"$repo/skills/playbooks/harness-adapters/SKILL.md"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-harness-adapter-references.test.sh" "harness adapter router selects portable structural coverage"
  assert_contains "$listed" "tests/xo-harness-adapter-instructions-live-e2e.test.sh" "harness adapter router selects opt-in instruction coverage"
  git -C "$repo" add skills/playbooks/harness-adapters/SKILL.md
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm harness-adapter-router-change

  printf '\n' >>"$repo/skills/playbooks/quota-array-dispatch/SKILL.md"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-ask-user-authority.test.sh" "quota dispatch owner selects pure contract coverage"
  assert_contains "$listed" "tests/xo-harness-adapter-instructions-live-e2e.test.sh" "quota dispatch owner selects opt-in live coverage"
  git -C "$repo" add skills/playbooks/quota-array-dispatch/SKILL.md
  ...
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-ask-user-authority.test.sh" "a removed activation link selects pure contract coverage"
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm activation-link-removed

  printf '\n' >>"$repo/skills/playbooks/board/assets/template.html"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-bearings-snapshot.test.sh" "a skill asset named by its activation-link spelling selects the referencing suite"
  git -C "$repo" add skills/playbooks/board/assets/template.html
  ...
  assert_contains "$listed" "tests/xo-ask-user-authority.test.sh" "a skill file no suite names selects the skills-tree structural coverage"
  git -C "$repo" add skills/playbooks/example/LICENSE
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm skill-unreferenced-file-change

  printf '\n' >>"$repo/bin/xo-procevent-quota.sh"
  printf '\n' >>"$repo/bin/xo-quota-choose.sh"
  listed=$(cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD)
  assert_contains "$listed" "tests/xo-procevent-quota.test.sh" \
    "quota process-event source selects its focused test"
  assert_contains "$listed" "tests/xo-quota-choose.test.sh" \
    "quota chooser source selects its focused test"
  git -C "$repo" add bin/xo-procevent-quota.sh bin/xo-quota-choose.sh
  ...
  git -C "$repo" add bin/xo-timeout-lib.sh
  git -C "$repo" -c user.name=test -c user.email=test@example.invalid commit -qm timeout-lib-change

  printf '\n' >>"$repo/src/unmapped.ts"
  set +e
  (cd "$repo" && bin/xo-test-run.sh --list --changed --base HEAD) >"$tmp/out" 2>"$tmp/err"
  rc=$?
  set -e
...
test_changed_bin_reference_selects_per_script_not_per_family() {
  local tmp repo listed
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-changed-scope.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"
  ...
  assert_contains "$listed" "tests/xo-backend-herdr-smoke.test.sh" \
  ...
  assert_contains "$listed" "tests/xo-daemon.test.sh" \
    "a curated consumer of the helper must still select its whole family"
  assert_contains "$listed" "tests/xo-pi-watch-extension.test.sh" \
    "a curated consumer of the helper must still select its whole family"
...
test_changed_uses_bounded_automatic_concurrency() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-changed-consent.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"
  cp "$ROOT/bin/xo-timeout-lib.sh" "$repo/bin/xo-timeout-lib.sh"
  for script in xo-backend-herdr-smoke.test.sh xo-daemon.test.sh xo-pi-watch-extension.test.sh; do
...
SH
  ...
  (cd "$repo" && bin/xo-test-run.sh --changed --base HEAD --json "$tmp/parallel.json") \
    >"$tmp/parallel.out" 2>"$tmp/parallel.err" \
    || fail "default changed fixture run failed: $(cat "$tmp/parallel.err")"
  parallel_shape=$(grep -E '^XO_TEST_(BEGIN|END)' "$tmp/parallel.out" | head -n 2 | awk '{print $1}' | paste -sd, -)
  [ "$parallel_shape" = XO_TEST_BEGIN,XO_TEST_BEGIN ] \
    || fail "plain --changed did not use bounded concurrent scheduling: $parallel_shape"

  (cd "$repo" && bin/xo-test-run.sh --changed --base HEAD --jobs 1 --json "$tmp/serial.json") \
    >"$tmp/serial.out" 2>"$tmp/serial.err" \
    || fail "explicit serial changed fixture run failed: $(cat "$tmp/serial.err")"
  serial_shape=$(grep -E '^XO_TEST_(BEGIN|END)' "$tmp/serial.out" | head -n 2 | awk '{print $1}' | paste -sd, -)
  [ "$serial_shape" = XO_TEST_BEGIN,XO_TEST_END ] \
...
PY
  ...
  timeout_script=tests/xo-calm-pi-extension.test.sh
  mkdir -p "$timeout_repo/bin" "$timeout_repo/tests"
  cp "$RUNNER" "$timeout_repo/bin/xo-test-run.sh"
  cp "$ROOT/tests/git-config-helpers.sh" "$timeout_repo/tests/"
  cat >"$timeout_repo/bin/xo-timeout-lib.sh" <<'SH'
xo_run_timed() {
  [ "$1" -eq 900 ] || return 99
  return 124
}
...
SH
  chmod +x "$timeout_repo/bin/xo-test-run.sh" "$timeout_repo/$timeout_script"
  ...
  set +e
  (cd "$timeout_repo" && bin/xo-test-run.sh --changed --base HEAD) \
    >"$tmp/timeout.out" 2>"$tmp/timeout.err"
  rc=$?
  set -e
  [ "$rc" -eq 1 ] || fail "single-script automatic timeout must fail the run, got $rc"
  grep -Eq '^XO_TEST_END .+ tests/xo-calm-pi-extension\.test\.sh exit=124 ' "$tmp/timeout.out" \
...
test_windows_posix_mode_emulation_does_not_fail_parallel_runs() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-windows-modes.XXXXXX")
  repo="$tmp/repo"
  fakebin="$tmp/fakebin"
...
SH
  ...
  out=$(cd "$repo" && PATH="$fakebin:$PATH" REAL_STAT="$real_stat" \
    bin/xo-test-run.sh --jobs 2 \
      tests/xo-cd-pretool-check.test.sh tests/xo-ask-user-authority.test.sh 2>&1)
  rc=$?
  set -e
  expect_code 0 "$rc" "native-Windows POSIX-mode emulation"
  assert_contains "$out" "XO_TEST_SUMMARY total=2 failed=0" \
...
test_script_list_uses_bounded_automatic_concurrency() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-script-list.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"
  rm -f "$repo/bin/xo-timeout-lib.sh"
  # xo-cd-pretool-check and xo-pr-merge are individually proven isolated;
  # xo-backend-orca is not, so it must still land in the serial tail.
  for script in xo-cd-pretool-check.test.sh xo-pr-merge.test.sh xo-backend-orca.test.sh; do
    cat >"$repo/tests/$script" <<'SH'
#!/usr/bin/env bash
sleep 1
...
SH
  ...
  (cd "$repo" && bin/xo-test-run.sh tests/xo-cd-pretool-check.test.sh tests/xo-pr-merge.test.sh \
  ...
  (cd "$repo" && bin/xo-test-run.sh tests/xo-cd-pretool-check.test.sh tests/xo-pr-merge.test.sh \
      tests/xo-backend-orca.test.sh) >"$tmp/mixed.out" 2>"$tmp/mixed.err" \
    || fail "mixed proven/unproven script list failed: $(cat "$tmp/mixed.err")"
  mixed_shape=$(grep -E '^XO_TEST_(BEGIN|END)' "$tmp/mixed.out" | awk '{print $1}' | paste -sd, -)
  [ "$mixed_shape" = XO_TEST_BEGIN,XO_TEST_BEGIN,XO_TEST_END,XO_TEST_END,XO_TEST_BEGIN,XO_TEST_END ] \
...
PYJSON
  ...
  (cd "$repo" && bin/xo-test-run.sh tests/xo-backend-orca.test.sh) \
    >"$tmp/named.out" 2>"$tmp/named.err" \
    || fail "a named script unexpectedly required a timeout helper: $(cat "$tmp/named.err")"
  grep -Eq '^XO_TEST_END .+ tests/xo-backend-orca\.test\.sh exit=0 ' "$tmp/named.out" \
...
test_family_proofs_run_in_separate_concurrent_phases() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-family-phases.XXXXXX")
  repo="$tmp/repo"
  mkdir -p "$repo/bin" "$repo/tests"
  cp "$RUNNER" "$repo/bin/xo-test-run.sh"
  cp "$ROOT/tests/git-config-helpers.sh" "$repo/tests/"
  ...
  chmod +x "$repo/bin/xo-test-run.sh"
  for script in \
    xo-calm-pi-extension.test.sh xo-vendor-auth-probe.test.sh \
    xo-pr-check-security.test.sh xo-teardown.test.sh; do
    cat >"$repo/tests/$script" <<'SH'
#!/usr/bin/env bash
sleep 1
...
SH
  ...
  (cd "$repo" && bin/xo-test-run.sh \
      tests/xo-pr-check-security.test.sh tests/xo-calm-pi-extension.test.sh \
      tests/xo-teardown.test.sh tests/xo-vendor-auth-probe.test.sh --jobs 4) \
    >"$tmp/out" 2>"$tmp/err" \
...
for line in open(sys.argv[1], encoding="utf-8"):
    if line.startswith("XO_TEST_BEGIN "):
        ...
        if sum(value == family for value in active.values()) > 1:
            overlap[family] = True
    elif line.startswith("XO_TEST_END "):
...
test_empty_selection_emits_summary() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-empty.XXXXXX")
  repo="$tmp/repo"
  init_changed_fixture_repo "$repo"
  printf 'documentation only\n' >"$repo/README.md"
  out=$(cd "$repo" && bin/xo-test-run.sh --changed --base HEAD --json "$tmp/artifacts/timing.json" 2>"$tmp/err") \
    || fail "empty valid changed selection must pass"
  printf '%s\n' "$out" | grep -Eq \
    '^XO_TEST_SUMMARY total=0 failed=0 skipped_gate=0 duration_ms=[0-9]+$' \
...
test_timing_markers_and_json() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-timing.XXXXXX")
...
SH
  ...
  begin_n=$(grep -c '^XO_TEST_BEGIN ' "$out" || true)
  end_n=$(grep -c '^XO_TEST_END ' "$out" || true)
  [ "$begin_n" -eq 1 ] || fail "expected one XO_TEST_BEGIN, got $begin_n"
  [ "$end_n" -eq 1 ] || fail "expected one XO_TEST_END, got $end_n"
  grep -Eq '^XO_TEST_BEGIN .+ family=unclassified expected_gate_skip=none$' "$out" \
    || fail "BEGIN line missing family/expected_gate_skip: $(grep '^XO_TEST_BEGIN' "$out")"
  grep -Eq '^XO_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=false$' "$out" \
    || fail "END line missing exit/duration/gate_skip: $(grep '^XO_TEST_END' "$out")"
  summary=$(grep '^XO_TEST_SUMMARY ' "$out" || true)
  ...
  grep -q '^XO_TEST_SLOWEST rank=1 ' "$out" \
    || fail "expected XO_TEST_SLOWEST rank=1"
...
test_aggregate_exit_behavior() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-agg.XXXXXX")
...
SH
  ...
  grep -q 'XO_TEST_SUMMARY total=2 failed=1' "$tmp/out.txt" \
    || fail "summary should report total=2 failed=1: $(grep XO_TEST_SUMMARY "$tmp/out.txt")"
  # All-green stays 0.
  set +e
...
test_gate_skip_accounting() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-skip.XXXXXX")
...
SH
  ...
  grep -Eq '^XO_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=true$' "$out" \
    || fail "END must mark gate_skip=true: $(grep '^XO_TEST_END' "$out")"
  grep -q 'XO_TEST_SUMMARY total=1 failed=0 skipped_gate=1' "$out" \
    || fail "summary must count skipped_gate=1: $(grep XO_TEST_SUMMARY "$out")"
  python3 -c '
import json, sys
doc = json.load(open(sys.argv[1]))
...
test_gate_skip_reason_is_recorded() {
  local tmp skip_f out json
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-skipreason.XXXXXX")
  skip_f="$tmp/skip.test.sh"
  out="$tmp/out.txt"
  json="$tmp/timing.json"
  cat >"$skip_f" <<'SH'
...
test_a_run_that_ran_records_no_skip_reason() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-ranreason.XXXXXX")
...
test_live_guards_expect_a_capability_skip_class() {
  local tmp out
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-liveclass.XXXXXX")
  out="$tmp/out.txt"
  # XO_LIVE=0 makes every live guard refuse without touching a harness, so this
  # exercises the real family through the real runner in bounded time.
  XO_LIVE=0 "$RUNNER" --json "$tmp/timing.json" \
    tests/xo-composer-matrix-live-e2e.test.sh >"$out" 2>"$tmp/err.txt" \
    || fail "a disabled live guard must not fail the runner: $(cat "$tmp/err.txt")"
  grep -q 'expected_gate_skip=live-capability' "$out" \
    || fail "the live-harness family must expect a capability skip: $(grep XO_TEST_BEGIN "$out")"
  python3 -c '
import json, sys
doc = json.load(open(sys.argv[1]))
record = doc["scripts"][0]
...
test_fail_on_gate_skip_token() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-fail-skip.XXXXXX")
  skip_f="$tmp/skip.test.sh"
  out="$tmp/out.txt"
  cat >"$skip_f" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  grep -q 'XO_TEST_SUMMARY total=1 failed=1' "$out" \
    || fail "summary must report failed=1 under fail-on-gate-skip: $(grep XO_TEST_SUMMARY "$out")"
...
test_exclude_family() {
  ...
  printf '%s\n' "$listed" | grep -Fq 'tests/xo-backend-herdr-smoke.test.sh' \
    && fail "exclude-family real-herdr-gated left a real-herdr script"
  printf '%s\n' "$listed" | grep -Fq 'tests/xo-lint.test.sh' \
...
test_list_scheduled_proven_isolated_uses_serial_weights() {
  local tmp
  tmp=$(xo_test_tmproot xo-test-run-proven-schedule)
...
test_list_scheduled_non_lane_selections_use_serial_weights() {
  ...
  local -a scripts=(
    tests/xo-operational-input.test.sh
    tests/xo-lint.test.sh
    tests/xo-muse-harness.test.sh
    tests/xo-captain-hold-lifecycle.test.sh
    tests/xo-kimi-harness.test.sh
    tests/xo-brief.test.sh
  )
  tmp=$(xo_test_tmproot xo-test-run-non-lane-schedule)
  repo="$tmp/repo"
  mkdir -p "$repo/bin" "$repo/tests"
  cp "$RUNNER" "$repo/bin/xo-test-run.sh"
  ...
  printf '%s\n' \
    ...
    tests/xo-lint.test.sh \
    tests/xo-kimi-harness.test.sh \
    tests/xo-operational-input.test.sh >"$tmp/expected"
  for selection in family all changed scripts; do
    ...
    "$repo/bin/xo-test-run.sh" --list-scheduled "$@" >"$tmp/actual" \
...
test_portable_shard_union_and_coverage_guard() {
  ...
  printf '%s\n' "$s1" "$s2" "$serial" | grep -Fq 'tests/xo-backend-herdr-smoke.test.sh' \
    && fail "portable lanes must not include real-herdr-gated smoke"
  printf '%s\n' "$herdr" | grep -Fq 'tests/xo-backend-herdr-smoke.test.sh' \
    || fail "herdr family must include smoke"
  out=$("$RUNNER" --check-coverage)
  assert_contains "$out" "XO_TEST_COVERAGE ok" "coverage guard success marker"
  all_count=$("$RUNNER" --list --all | wc -l | tr -d ' ')
...
test_portable_serial_hint_coverage_is_reported_and_bounded() {
  ...
  # this trips (docs/xo-test-portable-shards.md).
...
test_portable_serial_shard_lane_refusals() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-shard-lane.XXXXXX")
...
test_jobs_requires_proven_isolated() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-jobs.XXXXXX")
  set +e
  ...
  set +e
  "$RUNNER" --jobs 2 tests/xo-afk-inject-e2e.test.sh >"$tmp/out2" 2>"$tmp/err2"
  rc=$?
  set -e
...
test_jobs_admits_a_concurrent_safe_family() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-jobs-admit.XXXXXX")
...
test_unmapped_new_test_never_inherits_family_concurrency() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-unmapped.XXXXXX")
  repo="$tmp/repo"
  mkdir -p "$repo/bin" "$repo/tests"
  cp "$RUNNER" "$repo/bin/xo-test-run.sh"
  cp "$ROOT/tests/git-config-helpers.sh" "$repo/tests/"
  ...
  for script in xo-procevent.test.sh xo-quota-choose.test.sh xo-zz-unmapped-fixture.test.sh; do
    printf '#!/usr/bin/env bash\necho "ok - %s fixture"\n' "$script" >"$repo/tests/$script"
    chmod +x "$repo/tests/$script"
  done

  set +e
  (cd "$repo" && bin/xo-test-run.sh --jobs 2 \
    tests/xo-procevent.test.sh tests/xo-quota-choose.test.sh) \
    >"$tmp/family.out" 2>"$tmp/family.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] \
    || fail "two members of the proven residual family must be admitted, got $rc: $(cat "$tmp/family.err")"
  grep -Fq 'XO_TEST_SUMMARY total=2 failed=0' "$tmp/family.out" \
    || fail "the admitted residual-family run did not report both scripts green: $(cat "$tmp/family.out")"

  set +e
  (cd "$repo" && bin/xo-test-run.sh --jobs 2 \
    tests/xo-procevent.test.sh tests/xo-zz-unmapped-fixture.test.sh) \
  ...
  grep -Fq 'xo-zz-unmapped-fixture.test.sh' "$tmp/unmapped.err" \
  ...
  set +e
  (cd "$repo" && bin/xo-test-run.sh tests/xo-zz-unmapped-fixture.test.sh) \
    >"$tmp/serial.out" 2>"$tmp/serial.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] \
    || fail "an unclassified test must still run serially, got $rc: $(cat "$tmp/serial.err")"
  grep -Eq '^XO_TEST_BEGIN .+ family=unclassified expected_gate_skip=none$' "$tmp/serial.out" \
...
test_concurrent_runs_are_ordered_longest_first() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-order.XXXXXX")
  set +e
  ...
  set +e
  "$RUNNER" --jobs 2 \
    tests/xo-session-lock-ancestry.test.sh tests/xo-task-inbox.test.sh \
    >"$tmp/out" 2>"$tmp/err"
  set -e
  first=$(grep -m1 '^XO_TEST_BEGIN' "$tmp/out" | awk '{print $3}')
  [ "$first" = tests/xo-task-inbox.test.sh ] \
...
test_per_script_timeout_bounds_a_hang() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-hang.XXXXXX")
  repo="$tmp/repo"
  runner="$repo/bin/xo-test-run.sh"
  hang=tests/xo-hang-fixture.test.sh
  mkdir -p "$repo/bin" "$repo/tests"
  cp "$RUNNER" "$runner"
  cp "$ROOT/tests/git-config-helpers.sh" "$repo/tests/"
  cp "$ROOT/bin/xo-timeout-lib.sh" "$repo/bin/xo-timeout-lib.sh"
...
SH
  ...
  grep -Eq 'XO_TEST_END .* exit=124 ' "$tmp/out" \
  ...
  grep -Fq 'XO_TEST_SUMMARY total=1 failed=1' "$tmp/out" \
...
test_max_wall_ms_is_a_result_not_advice() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-budget.XXXXXX")
  repo="$tmp/repo"
  runner="$repo/bin/xo-test-run.sh"
  fast=tests/xo-budget-fixture.test.sh
  mkdir -p "$repo/bin" "$repo/tests"
  cp "$RUNNER" "$runner"
  cp "$ROOT/tests/git-config-helpers.sh" "$repo/tests/"
  cat >"$repo/$fast" <<'SH'
...
SH
  ...
  grep -Eq '^XO_TEST_BUDGET max_wall_ms=60000 duration_ms=[0-9]+$' "$tmp/under" \
  ...
  grep -Eq '^XO_TEST_SUMMARY total=1 failed=0 skipped_gate=0 duration_ms=[0-9]+$' "$tmp/over" \
    || fail "an over-budget run omitted its summary: $(cat "$tmp/over")"
  grep -Eq '^XO_TEST_SUMMARY_FAMILY .+$' "$tmp/over" \
    || fail "an over-budget run omitted its family summary: $(cat "$tmp/over")"
  grep -Eq '^XO_TEST_SLOWEST rank=1 .+$' "$tmp/over" \
    || fail "an over-budget run omitted its slowest result: $(cat "$tmp/over")"
  grep -Eq '^XO_TEST_BUDGET max_wall_ms=500 duration_ms=[0-9]+$' "$tmp/over" \
    || fail "an over-budget run omitted its budget result: $(cat "$tmp/over")"
  summary_duration=$(awk '/^XO_TEST_SUMMARY / { for (i=1;i<=NF;i++) if ($i ~ /^duration_ms=/) { sub(/^duration_ms=/, "", $i); print $i } }' "$tmp/over")
  budget_duration=$(awk '/^XO_TEST_BUDGET / { for (i=1;i<=NF;i++) if ($i ~ /^duration_ms=/) { sub(/^duration_ms=/, "", $i); print $i } }' "$tmp/over")
...
test_jobs_parallel_scheduler_and_failure_propagation() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-jobs-sched.XXXXXX")
  repo="$tmp/repo"
  runner="$repo/bin/xo-test-run.sh"
  evidence="$tmp/evidence"
  fake_bin="$tmp/fake-bin"
  a=tests/xo-brief.test.sh
  b=tests/xo-composer-lib.test.sh
  c=tests/xo-lint.test.sh
  d=tests/xo-supervision-instructions.test.sh
...
SH
  ...
  begin_n=$(grep -c '^XO_TEST_BEGIN ' "$tmp/out" || true)
  end_n=$(grep -c '^XO_TEST_END ' "$tmp/out" || true)
  [ "$begin_n" -eq 3 ] || fail "expected 3 BEGIN markers, got $begin_n"
  [ "$end_n" -eq 3 ] || fail "expected 3 END markers, got $end_n"
  grep -q 'XO_TEST_SUMMARY total=3 failed=0' "$tmp/out" \
    || fail "summary missing for jobs run: $(grep XO_TEST_SUMMARY "$tmp/out")"
  python3 -c '
import json,sys
doc=json.load(open(sys.argv[1]))
...
SH
  ...
  grep -q 'XO_TEST_SUMMARY total=2 failed=1' "$tmp/out4" \
    || { rm -rf "$tmp"; fail "jobs failure summary wrong: $(grep XO_TEST_SUMMARY "$tmp/out4")"; }
...
SH
  ...
  grep -q 'XO_TEST_SUMMARY total=1 failed=1' "$tmp/out5" \
    || { rm -rf "$tmp"; fail "parallel stderr hard-fail summary wrong: $(grep XO_TEST_SUMMARY "$tmp/out5")"; }

  "$runner" --jobs 2 "$d" >"$tmp/out6" 2>"$tmp/err6" \
    || { rm -rf "$tmp"; fail "ordinary parallel stderr gate skip must remain successful"; }
  grep -Eq '^XO_TEST_END .+ exit=0 duration_ms=[0-9]+ gate_skip=true$' "$tmp/out6" \
    || { rm -rf "$tmp"; fail "parallel stderr gate skip was not recorded"; }
  grep -q 'XO_TEST_SUMMARY total=1 failed=0 skipped_gate=1' "$tmp/out6" \
    || { rm -rf "$tmp"; fail "parallel stderr skip summary wrong: $(grep XO_TEST_SUMMARY "$tmp/out6")"; }
...
test_aggregate_json() {
  local tmp a b
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-run-aggjson.XXXXXX")
  cat >"$tmp/a.json" <<'JSON'
{
  "run_id": "a",
...
JSON
  ...
  assert_contains "$out" "XO_TEST_AGGREGATE lanes=2 total=3 failed=1" "aggregate summary line"
  python3 -c '
import json,sys
doc=json.load(open(sys.argv[1]))
