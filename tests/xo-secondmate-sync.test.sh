...
# PRIMARY xo checkout's current default-branch commit by a purely LOCAL
# fast-forward (no origin fetch). Two hook points drive it - bin/xo-spawn.sh
# (before launching a secondmate) and bin/xo-bootstrap.sh (a startup sweep of
...
#     XO copy on its own host: the parent resolves the commit and the host
...
#     host's own copy is never moved, /updatexo's code-root-relative sync
...
TMP_ROOT=$(xo_test_tmproot xo-secondmate-sync)
export XO_BACKEND=tmux

# --- world builders --------------------------------------------------------

# new_world <name>: a PRIMARY xo repo on `main` with one commit (the
...
new_world() {
  ...
  # Fresh watcher beacon keeps xo-guard quiet for the spawn path.
...
bump_primary() {
  ...
  if [ "$mode" = instr ]; then
    ...
    mkdir -p "$w/main/skills/orders/note"
    printf 's-%s\n' "$mode" > "$w/main/skills/orders/note/SKILL.md"
  fi
  if [ "$mode" = bin ]; then
...
# the tracked .gitignore and commit it on main. The marker (.xo-secondmate-home)
# is xo-generic, written by bin/xo-home-seed.sh into every seeded home; once
...
test_bootstrap_sweep_nudges_only_instruction_change() {
  ...
  printf 'sm-nonlive\n' > "$w/sm-nonlive/.xo-secondmate-home"
  ...
  info_line=$(printf '%s\n' "$out" | grep '^BOOTSTRAP_INFO: nudged xo-sm-instr ' || true)
  [ -n "$info_line" ] || fail "no BOOTSTRAP_INFO nudge line emitted (got: $out)"
  assert_contains "$info_line" "xo was updated to the latest - please re-read your AGENTS.md to pick up the new instructions." \
    "successful nudge report should include the exact message sent"
  assert_not_contains "$out" "NUDGE_SECONDMATES:" "successful nudge must not leave an xo action item"
  ...
  assert_contains "$(cat "$log")" "XO instruction waiting" \
...
test_bootstrap_nudge_send_uses_state_override() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    XO_STATE_OVERRIDE="$override_state" XO_SEND_SETTLE=0 XO_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null)

  assert_contains "$out" "BOOTSTRAP_INFO: nudged xo-sm-instr with" \
  ...
  pass "T8a bootstrap nudge send respects XO_STATE_OVERRIDE"
}
...
test_bootstrap_nudge_retry_rejects_malformed_marker_id() {
  ...
  printf '../escape\n' > "$evil/.xo-secondmate-home"
  mkdir -p "$w/home/state/.secondmate-nudge-pending"
  marker="$w/home/state/.secondmate-nudge-pending/bad.pending"
  {
    printf 'id=../escape\n'
    printf 'selector=xo-../escape\n'
    printf 'home=%s\n' "$evil"
    printf 'commit=%s\n' "$c1"
    printf 'instructions=AGENTS.md\n'
    printf 'message=xo was updated to the latest - please re-read your AGENTS.md to pick up the new instructions.\n'
  } > "$marker"
  {
    printf 'window=xo:xo-evil\n'
    printf 'kind=secondmate\n'
    printf 'home=%s\n' "$evil"
  } > "$w/home/escape.meta"
  fakebin=$(make_fake_toolchain "$w")
  ...
  assert_not_contains "$out" "BOOTSTRAP_INFO: nudged xo-../escape" \
  ...
  assert_absent "$log" "malformed retry marker should not invoke xo-send"
...
test_bootstrap_nudge_failure_records_retry_marker() {
  ...
  assert_grep "selector=xo-sm-instr" "$marker" "retry marker should pin the stable selector"
  assert_grep "message=xo was updated to the latest - please re-read your AGENTS.md to pick up the new instructions." \
...
test_bootstrap_nudge_retry_is_idempotent() {
  ...
  out2=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    XO_SEND_SETTLE=0 "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null)
...
test_bootstrap_nudge_retry_refuses_changed_home() {
  ...
  printf '%s\n' sm-instr > "$other/.xo-secondmate-home"
...
# --- T8b: stale herdr nudge failures retry through current xo-<id> metadata ---
...
# (default:wA:p2), so xo-send with the printed target fell back to tmux and failed
# while xo-<id> resolved through current meta.
...
SH
  chmod +x "$spawn_stub"
  cp "$spawn_stub" "$w/main/bin/xo-spawn.sh"
  ...
  out=$(PATH="$herdrfb:$toolchain:$BASE_PATH" HERDR_ENV=1 XO_BACKEND=herdr \
    XO_SEND_SETTLE=0 \
    XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null)
  ...
  assert_contains "$out" "BOOTSTRAP_INFO: nudged xo-sm-instr" \
  ...
  resolved=$(bash -c '. "$0/bin/xo-backend.sh"; xo_backend_resolve_selector xo-sm-instr "$1"' "$ROOT" "$w/home/state")
  [ "$resolved" = "$fresh" ] || fail "xo-<id> should resolve through post-respawn meta, got '$resolved'"

  # shellcheck disable=SC2016  # $0/$1 belong to the inner bash -c process.
  stale_send=$(PATH="$herdrfb:$toolchain:$BASE_PATH" bash -c \
    '. "$0/bin/xo-backend.sh"; xo_backend_source herdr; xo_backend_herdr_send_literal "$1" "nudge"' "$ROOT" "$stale" 2>/dev/null; printf '%s' "$?")
  [ "$stale_send" != 0 ] || fail "explicit stale herdr endpoint send should fail"

  # shellcheck disable=SC2016  # $0/$1 belong to the inner bash -c process.
  fresh_send=$(PATH="$herdrfb:$toolchain:$BASE_PATH" bash -c \
    '. "$0/bin/xo-backend.sh"; xo_backend_source herdr; xo_backend_herdr_send_literal "$1" "nudge"' "$ROOT" "$fresh" 2>/dev/null; printf '%s' "$?")
  [ "$fresh_send" = 0 ] || fail "send through xo-<id>-resolved fresh endpoint should succeed"

  pass "T8b a stale herdr endpoint cannot lose a durably enqueued nudge, and xo-<id> resolves through post-respawn metadata"
}
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    XO_ROOT_OVERRIDE="$w/main" XO_HOME="$w/home" \
    ...
    XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" sm "$w/sm" codex --secondmate >/dev/null 2>&1 || true
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    XO_PROJECTS_OVERRIDE="$w/home/projects" XO_CONFIG_OVERRIDE="$w/home/config" \
    XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" sm "$w/sm" codex --secondmate >/dev/null 2>"$err" || true
...
# The seed marker used to leave every home permanently dirty: bin/xo-fleet-sync.sh
...
test_seed_marker_clean_when_gitignored() {
  ...
  # The exact dirtiness signal bin/xo-fleet-sync.sh reads (its line: dirty=yes when
...
# Standalone-clone homes converge through /updatexo's origin fetch instead.
...
# host-local leg (bin/xo-remote-secondmate-control.sh) directly.

# new_remote_world <name>: a PRIMARY xo repo with a bare forge origin, a
# host "XO copy" clone (the code root), and a persistent remote home clone
# of that copy - the topology bin/xo-remote-home-provision.sh lays down. Echoes
...
# make_remote_leg_ssh_stub <w>: a fake ssh that decodes xo-on.sh's payload and
...
shift 2  # host, xo-remote-entrypoint.sh
home_b64=$3
argv_b64=$4
...
cmd=${rargs[0]}
[ "$cmd" != xo-remote-doctor.sh ] || exit 0
# An older remote XO copy rejects a command shape it does not know with
# the usage status, which is exactly what a parent-targeted sync meets there.
if [ "${XO_TEST_REMOTE_LEG_REJECT_SYNC:-0}" = 1 ] \
  && [ "$cmd" = xo-remote-secondmate-control.sh ] && [ "${rargs[1]:-}" = sync ]; then
  printf 'Host-local lifecycle control for the remote secondmate home selected by xo-on.\n'
  exit 2
fi
rc=0
env XO_HOME="$remote_home" XO_ROOT_OVERRIDE="$XO_REMOTE_CODE_ROOT" \
  "$XO_TEST_REPO_ROOT/bin/$cmd" "${rargs[@]:1}" || rc=$?
exit "$rc"
SH
  chmod +x "$fb/fake-ssh"
  printf '%s\n' "$fb"
...
remote_sync() {
  ...
  shift 2
  REMOTE_SYNC_RC=0
  REMOTE_SYNC_OUT=$(XO_HOME="$w/$id" XO_ROOT_OVERRIDE="$w/coderoot" \
    "$ROOT/bin/xo-remote-secondmate-control.sh" sync "$id" "$@" 2>&1) || REMOTE_SYNC_RC=$?
}

# --- R1: a remote home follows the PARENT primary, not the host's own copy ----
# The reported incident: the home had already advanced past the host's XO
...
test_remote_sync_targets_primary_not_host_copy() {
  ...
  [ "$(head_of "$w/coderoot")" = "$coderoot_before" ] \
    || fail "remote sync moved the host's own XO copy"
  pass "R1 a remote home ahead of the host's XO copy still advances to the primary's commit"
}
...
# the only place that fact can come from. /updatexo needs it to decide
...
test_remote_sync_imports_from_host_copy() {
  ...
  pass "R2 a missing target is imported from the host's XO copy without moving it"
}
...
test_remote_sync_skips_unimportable_target() {
  ...
  assert_contains "$REMOTE_SYNC_OUT" "/updatexo" \
...
test_remote_sync_skips_dirty_diverged_and_feature_branch() {
  ...
  git -C "$w/feature" checkout -q -b xo/in-flight
  ...
  assert_contains "$REMOTE_SYNC_OUT" "on xo/in-flight, expected main" \
...
# --- R7: /updatexo's contract is unchanged ------------------------------
# That path refreshes the host's own XO copy from origin first and then
...
test_remote_sync_without_target_follows_host_copy() {
  local w c1 c2
  w=$(new_remote_world remote-updatexo)
  c1=$(head_of "$w/main")
  add_remote_home "$w" sm "$w/coderoot" "$c1"
  bump_primary "$w" instr
  c2=$(head_of "$w/main")
  git -C "$w/main" push -q origin main
  git -C "$w/coderoot" pull -q --ff-only        # what /updatexo does first
  ...
  pass "R7 a sync with no target still follows the host's own refreshed XO copy"
}
...
# so this drives the real bin/xo-bootstrap.sh network phase across the real
...
test_bootstrap_syncs_remote_home_to_primary_commit() {
  ...
  # xo-on.sh only routes commands this primary checkout genuinely tracks, so the
  # fixture primary carries the real tooling the parent legs invoke.
  cp "$ROOT"/bin/xo-remote-*.sh "$w/main/bin/"
  git -C "$w/main" add -A
  ...
  xo_write_secondmate_meta "$home/state/sm.meta" "$w/sm"
  printf 'remote_host=host-sm\n' >> "$home/state/sm.meta"
  mkdir -p "$w/sm/state/parent-route"
  xo_write_meta "$w/sm/state/parent-route/sm.meta" \
    'window=xo-remote:p1' 'endpoint_task_id=sm' 'worktree=-' 'project=-' \
    'backend=herdr' 'harness=codex' 'herdr_session=xo-remote' \
    'herdr_workspace_id=w1' 'herdr_tab_id=t1' 'herdr_pane_id=p1'

  fakebin=$(make_remote_leg_ssh_stub "$w")
  xo_fake_exit0 "$fakebin" gh treehouse tmux node
  out=$(PATH="$fakebin:$BASE_PATH" \
    XO_HOME="$home" XO_ROOT_OVERRIDE="$w/main" \
    XO_BOOTSTRAP_NETWORK=only \
    XO_SSH_BIN="$fakebin/fake-ssh" XO_REMOTE_CODE_ROOT="$w/coderoot" \
    XO_TEST_REPO_ROOT="$ROOT" \
    XO_INHERITABLE_CONFIG='' XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-bootstrap.sh" 2>&1)

  [ "$(head_of "$w/sm")" = "$c2" ] \
    || fail "session start left the remote home off the primary's commit (out: $out)"
  [ "$(head_of "$w/coderoot")" = "$coderoot_before" ] \
    || fail "session start moved the host's own XO copy"
...
# A host still running an older XO copy rejects a command shape it does
...
test_bootstrap_reports_outdated_host_actionably() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" \
    ...
    XO_BOOTSTRAP_NETWORK=only \
    XO_SSH_BIN="$fakebin/fake-ssh" XO_REMOTE_CODE_ROOT="$w/coderoot" \
    XO_TEST_REPO_ROOT="$ROOT" XO_TEST_REMOTE_LEG_REJECT_SYNC=1 \
    XO_INHERITABLE_CONFIG='' XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-bootstrap.sh" 2>&1)

  assert_contains "$out" "SECONDMATE_SYNC: secondmate sm: skipped:" \
    "an outdated host did not produce its own convergence line"
  assert_contains "$out" "too old to sync to this primary's commit; run /updatexo" \
...
# --- R9: a remote launch never re-targets the host's own XO copy --------
# The launch leg runs a host-local spawn whose XO_ROOT is that host's XO
...
test_remote_launch_does_not_retarget_host_copy() {
  ...
  fakebin=$(xo_fakebin "$w/launchfake")
  ...
  launch_out=$(PATH="$fakebin:$BASE_PATH" \
    XO_HOME="$w/launched" XO_ROOT_OVERRIDE="$w/coderoot" XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-remote-secondmate-control.sh" launch launched codex - - herdr 2>&1) || true
  [ "$(head_of "$w/launched")" = "$c1" ] \
    || fail "a remote launch moved the home onto the host's own XO copy (out: $launch_out)"

  # Divergence control: the ordinary secondmate spawn contract DOES follow its
  # XO_ROOT, so the suppressed step above is not vacuously absent.
  control_out=$(PATH="$fakebin:$BASE_PATH" \
    XO_HOME="$w/coderoot" XO_ROOT_OVERRIDE="$w/coderoot" \
    XO_STATE_OVERRIDE="$w/control/state" XO_DATA_OVERRIDE="$w/control/data" \
    XO_CONFIG_OVERRIDE="$w/control/config" XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" control "$w/control" --secondmate --harness codex --backend herdr 2>&1) || true
...
echo "# all xo-secondmate-sync tests passed"
