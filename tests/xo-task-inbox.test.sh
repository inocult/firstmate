#!/usr/bin/env bash
# tests/xo-task-inbox.test.sh - the per-task steering inbox
# (bin/xo-task-inbox-lib.sh) and the watcher's re-ring ladder.
#
...
#   5. A real xo-watch.sh subprocess re-rings the doorbell for an unhandled
#      aged message on an idle pane WITHOUT waking xo, waits on a busy
...
TMP_ROOT=$(xo_test_tmproot xo-task-inbox)
...
inbox_lib() {  # <state> <function> [args...]
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    ...
    shift 2
    "$fn" "$@"
  ' _ "$ROOT/bin/xo-task-inbox-lib.sh" "$@"
}

# A fake tmux for the watcher cases: capture-pane replays XO_FAKE_TMUX_CAPTURE,
# display-message yields a numeric cursor row, and every literal send-keys is
# logged to XO_SEND_LOG so a doorbell ring is observable. With
# XO_FAKE_TMUX_AGENT set, the inventory lists window xo-t1 and its
# #{pane_current_command} answers with that value, so `zsh` makes
# xo_backend_tmux_agent_state read the pane as a dead bare shell.
...
case "${1:-}" in
  send-keys)
    ...
    if [ "$literal" = 1 ]; then
      printf '%s\n' "${1:-}" >> "${XO_SEND_LOG:-/dev/null}"
      if [ -n "${XO_ACK_RECORD:-}" ] && [ -f "$XO_ACK_RECORD" ]; then
        mv "$XO_ACK_RECORD" "${XO_ACK_RECORD%/*}/handled/"
      fi
    fi
    exit 0 ;;
  display-message)
    for a in "$@"; do
      case "$a" in
        *cursor_y*) printf '1\n'; exit 0 ;;
        *pane_current_command*) [ -z "${XO_FAKE_TMUX_AGENT:-}" ] || { printf '%s\n' "$XO_FAKE_TMUX_AGENT"; exit 0; } ;;
        *pane_tty*) [ -z "${XO_FAKE_TMUX_AGENT:-}" ] || { printf '\n'; exit 0; } ;;
      esac
    done
    printf 'fakepane\n'; exit 0 ;;
  capture-pane)
    if [ -n "${XO_FAKE_TMUX_CAPTURE:-}" ] && [ -f "$XO_FAKE_TMUX_CAPTURE" ]; then
      cat "$XO_FAKE_TMUX_CAPTURE"
    else
      printf '╭────╮\n│    │\n╰────╯\n'
    fi
    exit 0 ;;
  list-windows) [ "${XO_FAKE_TMUX_MISSING:-0}" = 1 ] || printf 'xo-t1\n'; exit 0 ;;
esac
exit 0
SH
  chmod +x "$fb/tmux"
...
watch_bg() {  # <state> <fakebin> <out> [extra env assignments...]
  ...
  shift 3
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" \
    XO_CREW_STATE_BIN="$fakebin/xo-crew-state.sh" \
    XO_FAKE_CREW_STATE='state: working · source: run-step · validating (running)' \
    XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    XO_TASK_INBOX_GRACE_SECS=1 \
...
test_write_is_durable_and_exact() {
  ...
  rec=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "$text") \
  ...
  inbox_lib "$state" xo_task_inbox_body "$rec" > "$actual" \
    || fail "record body could not be read"
  cmp -s "$expected" "$actual" \
    || fail "record body did not preserve trailing and blank-line bytes"
  rec2=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "no trailing newline") \
  ...
  inbox_lib "$state" xo_task_inbox_body "$rec2" > "$actual2" \
    || fail "second record body could not be read"
  cmp -s "$expected2" "$actual2" \
    || fail "record body added a trailing newline"
  doorbell=$(inbox_lib "$state" xo_task_inbox_doorbell_line "$rec")
  doorbell2=$(inbox_lib "$state" xo_task_inbox_doorbell_line "$rec2")
  [ "$doorbell" = "$doorbell2" ] \
  ...
  assert_contains "$doorbell" "XO instruction waiting" "doorbell should be self-describing"
  case "$doorbell" in
...
test_doorbell_is_a_shell_noop() {
  ...
  marker="$state/marker"
  mkdir -p "$state"
  rec=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "please continue")
  doorbell=$(inbox_lib "$state" xo_task_inbox_doorbell_line "$rec")
  case "$doorbell" in
    ': '*) ;;
...
test_doorbell_rejects_terminal_controls() {
  ...
  for label in etx esc; do
    ...
    doorbell=
    rc=0
    doorbell=$(inbox_lib "$state" xo_task_inbox_doorbell_line "$rec") || rc=$?
    ...
    rc=0
    PATH="$dir/fakebin:$PATH" XO_SEND_LOG="$log" \
      inbox_lib "$state" xo_task_inbox_ring tmux sess:xo-t1 "$rec" xo-t1 || rc=$?
...
# xo_task_inbox_ring against a backend whose agent classifies dead or missing:
...
test_ring_skips_dead_agent() {
  ...
  rc=0
  PATH="$dir/fakebin:$PATH" XO_SEND_LOG="$log" XO_FAKE_TMUX_AGENT=zsh \
    inbox_lib "$state" xo_task_inbox_ring tmux sess:xo-t1 "$rec" xo-t1 || rc=$?
  ...
  rc=0
  PATH="$dir/fakebin:$PATH" XO_SEND_LOG="$log" XO_FAKE_TMUX_MISSING=1 \
    inbox_lib "$state" xo_task_inbox_ring tmux sess:xo-t1 "$rec" xo-t1 || rc=$?
  ...
  rc=0
  PATH="$dir/fakebin:$PATH" XO_SEND_LOG="$log" XO_FAKE_TMUX_AGENT=claude \
    inbox_lib "$state" xo_task_inbox_ring tmux sess:xo-t1 "$rec" xo-t1 || rc=$?
  [ "$rc" = 0 ] || fail "a live agent should still be rung, got $rc"
  grep -qF 'XO instruction waiting' "$log" || fail "a live agent did not receive the doorbell"
  : > "$log"
  rc=0
  PATH="$dir/fakebin:$PATH" XO_SEND_LOG="$log" \
    inbox_lib "$state" xo_task_inbox_ring tmux sess:xo-t1 "$rec" xo-t1 || rc=$?
  [ "$rc" = 0 ] || fail "an endpoint the classifier cannot see should still be rung, got $rc"
  grep -qF 'XO instruction waiting' "$log" || fail "an unclassifiable endpoint did not receive the doorbell"
...
test_idempotent_write_dedups_exact_body() {
  ...
  r1=$(inbox_lib "$state" xo_task_inbox_write_idempotent "$state" t1 "$text") \
  ...
  r2=$(inbox_lib "$state" xo_task_inbox_write_idempotent "$state" t1 "$text") \
  ...
  r3=$(inbox_lib "$state" xo_task_inbox_write_idempotent "$state" t1 $'re-runnable steer\nsecond line changed') \
  ...
  r4=$(inbox_lib "$state" xo_task_inbox_write_idempotent "$state" t1 "$text") \
...
test_idempotent_write_follows_concurrent_ack() {
  ...
  rec=$(inbox_lib "$state" xo_task_inbox_write_idempotent "$state" t1 "$text") \
    || fail "race fixture write failed"
  result=$(XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    eval "$(declare -f xo_task_inbox_body | sed "1s/xo_task_inbox_body/_original_xo_task_inbox_body/")"
    xo_task_inbox_body() {
      candidate=$1
      case "$candidate" in
        */handled/*) ;;
      ...
      _original_xo_task_inbox_body "$candidate"
    }
    xo_task_inbox_write_idempotent "$2" t1 "$3"
  ' _ "$ROOT/bin/xo-task-inbox-lib.sh" "$state" "$text") \
...
test_handled_mv_dedups_by_sequence() {
  ...
  r1=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "first")
  r2=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "second")
  [ "$r2" = "$state/t1.inbox/002.msg" ] || fail "second record should be 002.msg, got $r2"
  oldest=$(inbox_lib "$state" xo_task_inbox_oldest_unhandled "$state" t1)
  ...
  if inbox_lib "$state" xo_task_inbox_oldest_unhandled "$state" t1 >/dev/null; then
  ...
  r3=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "third")
...
test_concurrent_writers_never_clobber() {
  ...
  for i in 1 2 3 4 5 6; do
    inbox_lib "$state" xo_task_inbox_write "$state" t1 "steer number $i" >/dev/null &
    pids+=($!)
  done
  for i in "${pids[@]}"; do
...
SH
  ...
  rec=$(PATH="$fakebin:$PATH" XO_REAL_LN="$real_ln" XO_FAKE_LN_MARKER="$marker" \
    inbox_lib "$state" xo_task_inbox_write "$state" t1 "steer after collision") \
...
test_ladder_writes_ignore_vanished_inbox() {
  ...
  rec=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "retired task")
  rm -rf "$state/t1.inbox"
  inbox_lib "$state" xo_task_inbox_record_ring "$state" t1 "$rec" \
    || fail "ring bookkeeping should ignore a concurrently removed inbox"
  inbox_lib "$state" xo_task_inbox_record_escalated "$state" t1 "$rec" \
...
test_fire_and_forget_records_never_enter_the_ladder() {
  ...
  fire=$(inbox_lib "$state" xo_task_inbox_write_idempotent "$state" t1 "one-shot steer" fire-and-forget)
  age_path "$fire"
  action=$(XO_TASK_INBOX_GRACE_SECS=0 XO_TASK_INBOX_RING_MAX=0 \
    inbox_lib "$state" xo_task_inbox_due_action "$state" t1)
  [ "$action" = quiet ] || fail "a fire-and-forget record entered the re-ring ladder: $action"
  tracked=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "tracked steer")
  age_path "$tracked"
  action=$(XO_TASK_INBOX_GRACE_SECS=0 XO_TASK_INBOX_RING_MAX=0 \
    inbox_lib "$state" xo_task_inbox_due_action "$state" t1)
...
test_ring_ladder_policy() {
  ...
  rec=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "do the thing")
  # Within grace: quiet.
  action=$(XO_TASK_INBOX_GRACE_SECS=3600 inbox_lib "$state" xo_task_inbox_due_action "$state" t1)
  [ "$action" = quiet ] || fail "a fresh unhandled message inside grace should be quiet, got: $action"
  # Past grace: one ring is due.
  age_path "$rec"
  action=$(XO_TASK_INBOX_GRACE_SECS=60 inbox_lib "$state" xo_task_inbox_due_action "$state" t1)
  ...
  action=$(XO_TASK_INBOX_GRACE_SECS=60 XO_TASK_INBOX_RING_MAX=3 inbox_lib "$state" xo_task_inbox_due_action "$state" t1)
  ...
  rec=$(inbox_lib "$state" xo_task_inbox_write "$state" t1 "next thing")
  age_path "$rec"
  action=$(XO_TASK_INBOX_GRACE_SECS=60 XO_TASK_INBOX_RING_MAX=3 inbox_lib "$state" xo_task_inbox_due_action "$state" t1)
...
setup_watch_case() {  # <name> -> echoes case dir; state in <dir>/state
  ...
  mkdir -p "$dir/state"
  make_watch_stubs "$dir" >/dev/null
  xo_write_meta "$dir/state/t1.meta" "window=sess:xo-t1" "kind=ship" "harness=grok"
  printf '%s\n' "$dir"
}
...
test_watcher_rerings_idle_pane_quietly() {
  ...
  watch_bg "$state" "$dir/fakebin" "$out" \
    XO_SEND_LOG="$log" XO_FAKE_TMUX_CAPTURE="$(idle_capture "$dir")" \
    XO_TASK_INBOX_RING_MAX=99
  pid=$!
  local i=0
  while [ "$i" -lt 100 ]; do
    grep -qF 'XO instruction waiting' "$log" 2>/dev/null && break
    kill -0 "$pid" 2>/dev/null || break
    sleep 0.1
    i=$((i + 1))
  done
  grep -qF "XO instruction waiting: list '$state/t1.inbox'/*.msg" "$log" \
    || { kill "$pid" 2>/dev/null; fail "the watcher never re-rang the doorbell:"$'\n'"$(cat "$log")"; }
  kill -0 "$pid" 2>/dev/null \
    || fail "a healthy re-ring must not wake xo (watcher exited):"$'\n'"$(cat "$out")"
  [ ! -s "$state/.wake-queue" ] \
  ...
  pass "watcher: an unhandled aged message on an idle pane re-rings without waking xo, and the ack silences it"
}
...
test_watcher_waits_on_busy_pane() {
  ...
  age_path "$rec"
  watch_bg "$state" "$dir/fakebin" "$out" \
    XO_SEND_LOG="$log" XO_FAKE_TMUX_CAPTURE="$dir/busy.capture" \
    XO_BUSY_REGEX=BUSYTOKEN XO_TASK_INBOX_RING_MAX=99
  pid=$!
  sleep 4
  kill "$pid" 2>/dev/null; wait "$pid" 2>/dev/null
...
test_watcher_ack_silences_unwritable_ladder() {
  ...
  watch_bg "$state" "$dir/fakebin" "$out" \
    XO_SEND_LOG="$log" XO_FAKE_TMUX_CAPTURE="$(idle_capture "$dir")" \
    XO_ACK_RECORD="$rec" XO_TASK_INBOX_RING_MAX=99
  pid=$!
  while [ "$i" -lt 100 ]; do
  ...
  kill "$pid" 2>/dev/null; wait "$pid" 2>/dev/null
  rings=$(grep -cF 'XO instruction waiting' "$log" || true)
...
test_watcher_escalates_once_after_budget() {
  ...
  grep -qF 'unread xo instruction' "$state/.wake-queue" \
  ...
  [ "$(grep -cF 'unread xo instruction' "$state/.wake-queue")" = 1 ] \
...
test_watcher_dead_pane_escalates_once_without_ringing() {
  ...
  watch_bg "$state" "$dir/fakebin" "$out" \
    XO_SEND_LOG="$log" XO_FAKE_TMUX_CAPTURE="$(idle_capture "$dir")" \
    XO_FAKE_TMUX_AGENT=zsh XO_TASK_INBOX_RING_MAX=99
  pid=$!
  wait_watcher_gone "$pid" \
    || { kill "$pid" 2>/dev/null; fail "the watcher never surfaced a dead pane's unhandled instruction"; }
  [ ! -s "$log" ] || fail "a dead pane was typed into:"$'\n'"$(cat "$log")"
  [ "$(grep -cF 'unread xo instruction' "$state/.wake-queue" 2>/dev/null || true)" = 1 ] \
  ...
  [ "$(inbox_lib "$state" xo_task_inbox_due_action "$state" t1)" = quiet ] \
...
test_watcher_dead_pane_ignores_stale_busy_state() {
  ...
  watch_bg "$state" "$dir/fakebin" "$out" \
    XO_SEND_LOG="$log" XO_FAKE_TMUX_CAPTURE="$dir/busy.capture" \
    XO_FAKE_TMUX_AGENT=zsh XO_BUSY_REGEX=BUSYTOKEN XO_TASK_INBOX_RING_MAX=99
  pid=$!
  wait_watcher_gone "$pid" \
