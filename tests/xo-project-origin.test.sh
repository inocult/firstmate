#!/usr/bin/env bash
# tests/xo-project-origin.test.sh - which project origins seeding accepts.
#
# XO supplies a project's origin instead of discovering it from a local
...
. "$ROOT/bin/xo-project-origin-lib.sh"

TMP_ROOT=$(xo_test_tmproot xo-project-origin)
FAKEBIN=$(xo_fakebin "$TMP_ROOT/fake")

accepts() {
  xo_project_origin_safe "$1" || fail "refused an ordinary clone URL: $1"
}
refuses() {
  ! xo_project_origin_safe "$1" || fail "accepted an origin git must never be handed: $1"
}

xo_git_init_commit "$TMP_ROOT/source"
git clone --quiet --bare "$TMP_ROOT/source" "$TMP_ROOT/source.git"

# XO is a shared template, so acceptance is decided by structure alone.
