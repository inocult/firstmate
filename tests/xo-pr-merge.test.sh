#!/usr/bin/env bash
# Tests for bin/xo-pr-merge.sh: the one path xo uses to merge a task's
# PR, which must record pr= and any available pr_head= into the task's meta so
# xo-teardown.sh's landed-check has a PR reference to verify against, even on
# repos with no PR CI where the usual "checks green" xo-pr-check.sh trigger
# never fires.
#
...
TMP_ROOT=$(xo_test_tmproot xo-pr-merge-tests)
BASE_PATH=$PATH
...
make_case() {
  ...
  : > "$case_dir/gh.log"
  # No worktree/project on disk; xo-pr-check.sh tolerates a worktree it cannot
...
case "${1:-} ${2:-}" in
  "pr view")
    case " $* " in
      *statusCheckRollup*)
        cat "$XO_TEST_GH_VIEW_JSON"
        if [ -f "${XO_TEST_AWAY_RECORD_AFTER_VIEW:-}" ]; then
          cp "$XO_TEST_AWAY_RECORD_AFTER_VIEW" "$XO_STATE_OVERRIDE/.afk-contract"
        fi
        exit 0
        ;;
      *headRefOid*)
    ...
    ;;
  "pr merge")
    if [ -n "${XO_TEST_META_AT_MERGE:-}" ] && [ -f "${XO_STATE_OVERRIDE:-}/task-x1.meta" ]; then
      cat "$XO_STATE_OVERRIDE/task-x1.meta" > "$XO_TEST_META_AT_MERGE"
    fi
    if [ -n "${XO_TEST_GH_MERGE_OUTPUT:-}" ]; then
      printf '%s\n' "$XO_TEST_GH_MERGE_OUTPUT"
    else
      printf 'merged:\n  number: %s\n  status: ok\n' "${3:-}"
    fi
    merge_rc=0
    if [ -f "${XO_TEST_GH_MERGE_RC_FILE:-}" ]; then
      merge_rc=$(cat "$XO_TEST_GH_MERGE_RC_FILE")
    fi
    exit "$merge_rc"
    ;;
  "api graphql")
    if [ -f "${XO_TEST_GH_GRAPHQL_FAIL:-}" ]; then
      echo 'error: could not reach the GitHub API' >&2
      exit 1
    fi
    cat "$XO_TEST_GH_OUTCOME"
    exit 0
    ;;
  api\ *)
    if [ -f "${XO_TEST_GH_RULES_FAIL:-}" ]; then
      exit 1
    fi
    cat "$XO_TEST_GH_RULES"
    exit 0
    ;;
esac
exit 0
...
#!/usr/bin/env bash
printf 'GITLAB_HOST=%s %s\n' "${GITLAB_HOST-<unset>}" "$*" >> "$XO_TEST_GLAB_LOG"
case_dir=$(dirname "$XO_TEST_GLAB_JSON")
case "${1:-} ${2:-}" in
  "mr view")
...
run_pr_merge() {
  ...
  XO_HOME="${XO_TEST_HOME:-$case_dir/home}" \
  XO_STATE_OVERRIDE="$case_dir/state" \
  XO_TEST_GH_AXI_LOG="$case_dir/gh-axi.log" \
  XO_TEST_GH_LOG="$case_dir/gh.log" \
  XO_TEST_GH_OUTCOME="$case_dir/github-outcome" \
  XO_TEST_GH_RULES="$case_dir/github-rules" \
  XO_TEST_GH_VIEW_JSON="$case_dir/github-view.json" \
  XO_TEST_GH_HEAD="$case_dir/github-head" \
  XO_TEST_GH_MERGE_RC_FILE="$case_dir/github-merge-rc" \
  XO_TEST_GH_MERGE_OUTPUT="$(cat "$case_dir/github-merge-output" 2>/dev/null || true)" \
  XO_TEST_GH_GRAPHQL_FAIL="$case_dir/github-graphql-fail" \
  XO_TEST_GH_RULES_FAIL="$case_dir/github-rules-fail" \
  XO_TEST_META_AT_MERGE="$case_dir/meta-at-merge" \
  XO_TEST_AWAY_RECORD_AFTER_VIEW="$case_dir/away-record-after-view" \
  XO_TEST_REAL_MV="$REAL_MV" \
  XO_TEST_GLAB_LOG="$case_dir/glab.log" \
  XO_TEST_GLAB_JSON="$case_dir/mr.json" \
  HOME="${XO_TEST_USER_HOME:-$case_dir/user-home}" \
...
write_away_record() {
  local case_dir=$1
  shift
  XO_HOME="$case_dir/home" XO_STATE_OVERRIDE="$case_dir/state" \
    "$ROOT/bin/xo-afk-contract.sh" propose "$@" >/dev/null
  XO_HOME="$case_dir/home" XO_STATE_OVERRIDE="$case_dir/state" \
    "$ROOT/bin/xo-afk-contract.sh" confirm >/dev/null
}
...
test_verified_merge_records_pr_and_head() {
  ...
  expect_code 0 "$rc" "records-before-merge: xo-pr-merge should succeed"
  assert_grep 'pr=https://github.com/example/repo/pull/9' "$case_dir/state/task-x1.meta" \
    "records-before-merge: pr= was not recorded"
  assert_grep 'pr_head=deadbeefcafefeed0000000000000000deadbeef' "$case_dir/state/task-x1.meta" \
    "records-before-merge: pr_head= was not recorded"
  assert_logged_gh_merge "$case_dir" 9 example/repo --squash
  pass "xo-pr-merge records pr= and pr_head= for a verified GitHub merge"
}
...
test_pr_metadata_is_recorded_before_the_forge_call() {
  ...
  expect_code 0 "$rc" "records-ahead-of-forge-call: xo-pr-merge should succeed"
  ...
  pass "xo-pr-merge records pr= before the forge call can land the merge"
}
...
test_merge_failure_propagates_after_recording() {
  ...
  expect_code 1 "$rc" "merge-fails: xo-pr-merge should propagate the gh-axi merge failure"
  assert_grep 'pr=https://github.com/example/repo/pull/13' "$case_dir/state/task-x1.meta" \
    "merge-fails: pr= should already be recorded even though the merge itself failed"
  pass "xo-pr-merge propagates a real merge failure without silently succeeding"
}
...
test_github_merged_outcome_is_verified() {
  ...
  pass "xo-pr-merge verifies a genuinely merged GitHub pull request"
}
...
test_github_verified_merge_requires_poll_recording() {
  ...
  pass "xo-pr-merge refuses to claim a merge when poll recording fails"
}
...
test_github_open_unqueued_outcome_refuses() {
  ...
  pass "xo-pr-merge refuses a GitHub merge call that leaves the PR open and unqueued"
}
...
test_github_unreadable_outcome_keeps_pr_bookkeeping() {
  ...
  pass "xo-pr-merge keeps PR bookkeeping when it cannot read a successful merge call's outcome"
}
...
test_github_refusal_quotes_the_forge_output() {
  ...
  pass "xo-pr-merge refuses with the forge's own output quoted apart from its verdict"
}
...
test_github_auto_merge_without_queue_refuses_legibly() {
  ...
  pass "xo-pr-merge explains an armed auto-merge that landed nothing on a queue-less base"
}
...
test_github_failed_merge_never_claims_armed_auto_merge() {
  ...
  pass "xo-pr-merge never reports auto-merge as armed when the merge command failed"
}
...
test_github_failed_merge_with_queue_flags_never_claims_acceptance() {
  ...
  pass "xo-pr-merge claims no acceptance for a failed merge command carrying queue flags"
}
...
test_github_accepted_queue_flags_do_not_echo_back_the_same_command() {
  ...
  pass "xo-pr-merge does not echo back queue flags the caller already used"
}
...
test_github_mismatched_queue_flags_still_name_the_retry() {
  ...
  pass "xo-pr-merge still names retry flags when the caller used a different method"
}
...
test_github_unrecognised_queue_method_still_names_the_queue() {
  ...
  pass "xo-pr-merge names the queue requirement even when its method is unrecognised"
}
...
test_github_unreadable_queue_rules_are_not_reported_as_no_queue() {
  ...
  pass "xo-pr-merge distinguishes unreadable branch rules from a base with no merge queue"
}
...
test_github_no_queue_rule_says_nothing_about_a_queue() {
  ...
  pass "xo-pr-merge says nothing about a merge queue when the base branch has no queue rule"
}
...
SH
  ...
  pass "xo-pr-merge accepts only a proved merge from the gh-axi fallback"
}
...
test_github_unreadable_outcome_refusal_quotes_the_forge_output() {
  ...
  pass "xo-pr-merge quotes the forge output when it cannot read the outcome either"
}
...
test_github_failed_gh_read_falls_back_to_gh_axi() {
  ...
  pass "xo-pr-merge falls back to the gh-axi view when gh's read fails"
}
...
test_github_failed_merge_names_an_observed_landed_state() {
  ...
  pass "xo-pr-merge names a landed state hiding behind a failed GitHub merge command"
}
...
test_github_without_gh_still_uses_gh_axi_merge() {
  ...
  pass "xo-pr-merge refuses a GitHub merge when gh is missing, before recording"
}
...
test_github_without_gh_failed_read_keeps_bookkeeping() {
  ...
  pass "xo-pr-merge refuses a GitHub merge when gh is missing rather than merging blind"
}
...
test_github_zero_exit_queue_required_refuses_with_exact_retry() {
  ...
  pass "xo-pr-merge reports exact queue retry flags after a zero-exit false success"
}
...
test_github_closed_unqueued_outcome_omits_retry_flags() {
  ...
  pass "xo-pr-merge omits merge-queue retry guidance for a closed GitHub PR"
}
...
test_github_queued_outcome_is_verified() {
  ...
  pass "xo-pr-merge accepts and accurately reports a GitHub merge-queue entry"
}
...
test_github_queue_required_refusal_names_retry_flags() {
  ...
  pass "xo-pr-merge explains how to retry with the required GitHub merge queue method"
}
...
test_github_agreeing_queue_rules_keep_retry_guidance() {
  ...
  pass "xo-pr-merge aggregates agreeing merge-queue rules"
}
...
test_github_conflicting_queue_rules_report_ambiguity() {
  ...
  pass "xo-pr-merge reports ambiguity for conflicting merge-queue rules"
}
...
test_extra_merge_args_forwarded() {
  ...
  pass "xo-pr-merge refuses branch deletion unless --attended-override is passed"
}
...
test_missing_meta_refuses_before_merge() {
  ...
  expect_code 1 "$rc" "missing-meta: xo-pr-merge should refuse"
  assert_grep 'error: task metadata is unavailable' "$case_dir/stderr" \
    "missing-meta: refusal did not explain missing meta"
  [ ! -s "$case_dir/gh.log" ] || fail "missing-meta: gh pr merge was invoked"
  assert_absent "$case_dir/state/missing-x1.check.sh" \
    "missing-meta: xo-pr-check should not arm a poll for an unknown task"
  pass "xo-pr-merge refuses before merging when task meta is missing"
}
...
test_malformed_url_refuses_before_merge() {
  ...
  expect_code 2 "$rc" "malformed-url: xo-pr-merge should refuse a malformed merge request URL"
  ...
  pass "xo-pr-merge refuses malformed PR URLs before calling gh-axi"
}
...
test_rejects_unsafe_url_segments_before_recording() {
  ...
  expect_code 1 "$rc" "unsafe-url-segment: xo-pr-merge should refuse unsafe owner/repo characters"
  ...
  pass "xo-pr-merge refuses unsafe PR URL segments before recording state"
}
...
test_repo_override_args_refuse_before_recording() {
  ...
  expect_code 1 "$rc" "repo-override: xo-pr-merge should refuse repo override flags"
  ...
  pass "xo-pr-merge refuses repo override args before recording state"
}
...
test_bundled_repo_override_args_refuse_before_recording() {
  ...
  expect_code 1 "$rc" "bundled-repo-override: xo-pr-merge should refuse a bundled repo override"
  assert_grep 'extra merge arguments must not override the repository' "$case_dir/stderr" \
  ...
  expect_code 1 "$rc" "bundled-repo-override-gitlab: xo-pr-merge should refuse a bundled instance override"
  assert_grep 'extra merge arguments must not override the repository' "$case_dir/stderr" \
  ...
  pass "xo-pr-merge refuses a bundled short-option repo override and refuses -d unless attended"
}
...
test_explicit_merge_method_not_overridden() {
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/22 -- --merge \
    > "$case_dir/stdout" 2> "$case_dir/stderr" || fail "explicit-merge-method: xo-pr-merge failed"

  assert_logged_gh_merge "$case_dir" 22 example/repo --merge
  pass "xo-pr-merge does not add default --squash when the caller passes an explicit merge method"
}
...
test_method_equals_merge_method_not_overridden() {
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/23 -- --method=merge \
    > "$case_dir/stdout" 2> "$case_dir/stderr" || fail "method-equals-merge-method: xo-pr-merge failed"

  assert_logged_gh_merge "$case_dir" 23 example/repo --method=merge
  pass "xo-pr-merge respects --method=<value> as an explicit merge method"
}
...
test_parses_pr_url_for_gh_axi() {
  ...
  run_pr_merge "$case_dir" task-x1 https://github.com/my-org/my-repo/pull/126 \
    > "$case_dir/stdout" 2> "$case_dir/stderr" || fail "url-parsing: xo-pr-merge failed"

  assert_logged_gh_merge "$case_dir" 126 my-org/my-repo --squash
  pass "xo-pr-merge parses a GitHub PR URL into gh-axi number and --repo arguments"
}
...
test_gitlab_url_resolves_and_merges() {
  ...
  pass "xo-pr-merge merges a GitLab merge request through glab instead of refusing it"
}
...
test_gitlab_host_comes_from_the_url() {
  ...
  pass "xo-pr-merge takes the GitLab instance from the URL rather than assuming one"
}
...
test_gitlab_imposes_no_merge_method() {
  ...
  pass "xo-pr-merge imposes no merge method on GitLab, leaving the project's own one"
}
...
test_gitlab_extra_args_forwarded() {
  ...
  pass "xo-pr-merge refuses GitLab source-branch deletion unless --attended-override is passed"
}
...
test_gitlab_merge_failure_propagates() {
  ...
  pass "xo-pr-merge propagates a real glab merge failure without silently succeeding"
}
...
test_gitlab_each_condition_refuses_independently() {
  ...
  for spec in "$@"; do
    ...
    expect_code 1 "$rc" "gitlab-refuse-$name: xo-pr-merge should refuse"
  ...
  pass "xo-pr-merge refuses on each GitLab pre-merge condition independently"
}
...
test_gitlab_reports_every_failing_condition() {
  ...
  expect_code 1 "$rc" "gitlab-refuse-all: xo-pr-merge should refuse"
  for expected in \
  ...
  pass "xo-pr-merge reports every failing GitLab condition, not only the first"
}
...
test_gitlab_stale_recorded_head_is_reported() {
  ...
  # xo-pr-check.sh rewrites the metadata, which drops a head it cannot resolve
  ...
  pass "xo-pr-merge reports a stale recorded head and verifies the live one"
}
...
test_gitlab_unreadable_state_refuses() {
  ...
  for name in view-fails not-an-object split-value; do
    ...
    expect_code 1 "$rc" "gitlab-unreadable-$name: xo-pr-merge should refuse"
  ...
  pass "xo-pr-merge refuses an unreadable GitLab merge request state rather than merging blind"
}
...
test_gitlab_invalid_head_refuses() {
  ...
  expect_code 1 "$rc" "gitlab-invalid-head: xo-pr-merge should refuse"
  ...
  pass "xo-pr-merge refuses a GitLab head commit it cannot validate"
}
...
test_gitlab_missing_tool_refuses_before_recording() {
  ...
  for tool in glab jq; do
    ...
    expect_code 1 "$rc" "gitlab-no-$tool: xo-pr-merge should refuse"
  ...
  pass "xo-pr-merge refuses before recording anything when glab or jq is absent"
}
...
test_gitlab_head_override_args_refuse_before_recording() {
  ...
  expect_code 1 "$rc" "gitlab-head-override: xo-pr-merge should refuse a caller head override"
  ...
  pass "xo-pr-merge refuses a GitLab head override before recording state"
}
...
test_github_still_forwards_sha_arg() {
  ...
  pass "xo-pr-merge refuses a caller --sha on GitHub because the head comes from the live read"
}

# --- durable merge outcome ---------------------------------------------------
# A merge that lands must leave a record outside the merging agent's memory.
# bin/xo-merge-outcome-lib.sh owns where that record goes; these cases pin the
...
make_home_case() {
  ...
  if [ -n "$route" ]; then
    printf '%s\n' mate-x >"$home/.xo-secondmate-home"
    {
...
test_secondmate_merge_reports_upward_once() {
  ...
  XO_TEST_HOME="$case_dir/home" run_pr_merge "$case_dir" task-x1 "$url" \
  ...
  # child's ready line on the same channel from xo-pr-check itself.
...
test_failed_merge_reports_nothing() {
  ...
  set +e
  XO_TEST_HOME="$case_dir/home" run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/63 \
    >"$case_dir/stdout" 2>"$case_dir/stderr"
  rc=$?
  set -e
...
test_gitlab_refusal_reports_nothing() {
  ...
  printf '%s\n' mate-x >"$case_dir/home/.xo-secondmate-home"
  printf 'schema=xo-secondmate-parent.v1\nroute=remote\n' >"$case_dir/home/.xo-secondmate-parent"

  set +e
  XO_TEST_HOME="$case_dir/home" run_pr_merge "$case_dir" task-x1 "$MR_URL" \
    >"$case_dir/stdout" 2>"$case_dir/stderr"
  rc=$?
  set -e
...
test_queued_github_merge_leaves_the_poll_armed() {
  ...
  XO_TEST_GH_MERGE_STATE=open XO_TEST_HOME="$case_dir/home" \
    run_pr_merge "$case_dir" task-x1 "$url" \
      >"$case_dir/stdout" 2>"$case_dir/stderr" \
...
test_distinct_merged_prs_keep_distinct_wakes() {
  ...
  XO_TEST_HOME="$case_dir/home" run_pr_merge "$case_dir" task-x1 "$first_url" \
  ...
  # Reused tasks re-bind through xo-pr-check before the next merge. Merge
  ...
  XO_TEST_HOME="$case_dir/home" run_pr_merge "$case_dir" task-x1 "$second_url" \
  ...
  XO_STATE_OVERRIDE="$case_dir/state" "$ROOT/bin/xo-wake-drain.sh" \
...
case "${!#}" in
  *.pr-poll-merge-notified)
    if mkdir "$XO_TEST_MARKER_FAILURE.claim" 2>/dev/null; then
      exit 1
    fi
    ;;
esac
...
SH
  chmod +x "$case_dir/fakebin/mv"
  export XO_TEST_MARKER_FAILURE="$case_dir/marker-failure"
  export XO_TEST_REAL_MV
  XO_TEST_REAL_MV=$(command -v mv)
  ...
  unset XO_TEST_MARKER_FAILURE XO_TEST_REAL_MV
  count=$(grep -c -F "$url" "$case_dir/state/.wake-queue")
  [ "$count" -ge 1 ] \
...
test_absent_backlog_still_merges() {
  ...
  pass "xo-pr-merge proceeds when the home carries no backlog at all"
}
...
test_unreadable_backlog_refuses_the_merge() {
  ...
  pass "xo-pr-merge refuses when the backlog exists but cannot be read"
}
...
test_unreadable_backend_config_refuses_the_merge() {
  ...
  pass "xo-pr-merge refuses when its configured backend cannot be read"
}
...
test_unreadable_user_backend_config_refuses_the_merge() {
  ...
  set +e
  XO_TEST_USER_HOME="$case_dir/user-home" \
    run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/64 \
      > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
  ...
  pass "xo-pr-merge refuses when its user backend configuration cannot be read"
}
...
test_untraversable_user_backend_config_directory_refuses_the_merge() {
  ...
  pass "xo-pr-merge refuses when its user backend configuration directory cannot be traversed"
}
...
test_absent_user_backend_config_directory_and_backlog_still_merge() {
  ...
  pass "xo-pr-merge proceeds once when its user configuration directory and backlog are genuinely absent"
}
...
test_backend_override_bypasses_unreadable_user_config() {
  ...
  set +e
  TASKS_AXI_BACKEND=markdown XO_TEST_USER_HOME="$case_dir/user-home" \
    run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/65 \
      > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
  ...
  pass "xo-pr-merge honors a backend override over an unreadable user configuration"
}
...
test_github_red_checks_refuse_and_allow_red_waives_named() {
  ...
  pass "xo-pr-merge refuses red GitHub checks and waives only a named --allow-red check"
}
...
test_allow_red_is_refused_while_away() {
  ...
  pass "xo-pr-merge rechecks away presence before an attended red merge"
}
...
test_allow_red_requires_one_separate_name() {
  ...
  pass "xo-pr-merge accepts exactly one separately named red-check waiver"
}
...
test_allow_red_refused_on_gitlab() {
  ...
  pass "xo-pr-merge refuses --allow-red on GitLab"
}
