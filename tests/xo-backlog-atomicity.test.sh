...
#
# bin/xo-backlog-transition-lib.sh states the contract; the three scripts that
...
#
#   dispatch    bin/xo-spawn.sh moves the row In flight in the same run that
#               publishes the record, so a live worker the backlog does not own
#               cannot arise on the ordinary path.
#   completion  bin/xo-teardown.sh closes the row before it reports success, so
#               a finished task cannot be left showing as running.
#   recovery    bin/xo-bootstrap.sh reconciles THIS home's own books at session
...
# in xo_tasks_axi_backend, so the backend cases must start from a clean slate.
unset TASKS_AXI_BACKEND || :

SPAWN="$ROOT/bin/xo-spawn.sh"
TEARDOWN="$ROOT/bin/xo-teardown.sh"
BOOTSTRAP="$ROOT/bin/xo-bootstrap.sh"
TMP_ROOT=$(xo_test_tmproot xo-backlog-atomicity)
...
# xo_tasks_axi_backend reads <addressing-root>/.tasks.toml and otherwise falls
...
make_home() {  # <name> [task-id...]
  ...
  # Pin the adapter per case: without it xo_tasks_axi_backend would fall through
...
EOF
  ...
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
case "$*" in *"#{pane_current_path}"*) printf '%s\n' "${XO_FAKE_PANE_PATH:-}"; exit 0 ;; esac
case "${1:-}" in display-message) printf 'xo\n'; exit 0 ;; esac
exit 0
SH
  chmod +x "$fakebin/tmux"
  xo_fake_exit0 "$fakebin" treehouse gh gh-axi no-mistakes

  xo_git_init_commit "$case_dir/project"
  xo_git_add_origin "$case_dir/project" "$case_dir/project.origin.git"
...
# path's own read-back repair) never answers. With XO_TASKS_AXI_TIMEOUT
...
# --- xo_tasks_axi's own bound -------------------------------------------------

# A PATH with no timeout variant on it - the stock-macOS shape, where GNU
# timeout is absent and coreutils does not ship gtimeout. xo_tasks_axi must
...
run_bounded_xo_tasks_axi() {  # <fallback-bin> <bound> [args...]
  ...
  PATH="$fb"
  out=$(
    . "$ROOT/bin/xo-backlog-transition-lib.sh"
    XO_TASKS_AXI_TIMEOUT="$bound" xo_tasks_axi "$@" 2>&1
  ) || rc=$?
  PATH=$saved_path
  printf '%s' "$out"
  return "$rc"
...
test_xo_tasks_axi_fallback_bounds_the_call_without_a_timeout_binary() {
  local case_dir fb out rc=0 started
  case_dir=$(make_home xo-tasks-axi-fallback)
  fb=$(make_fallback_bin "$case_dir" '#!/bin/bash
exec sleep 300')
  started=$SECONDS
  out=$(run_bounded_xo_tasks_axi "$fb" 2 show never-answers) || rc=$?
  [ "$rc" -eq 124 ] \
  ...
  pass "xo_tasks_axi bounds the call through its perl watchdog when no timeout binary exists"
}

test_xo_tasks_axi_fallback_passes_the_child_status_and_output_through() {
  local case_dir fb out rc=0
  case_dir=$(make_home xo-tasks-axi-passthrough)
  fb=$(make_fallback_bin "$case_dir" '#!/bin/bash
echo "stub failed"
exit 7')
  out=$(run_bounded_xo_tasks_axi "$fb" 5 show x) || rc=$?
  [ "$rc" -eq 7 ] \
  ...
  pass "xo_tasks_axi's perl watchdog passes the child status and output through unchanged"
}

test_xo_tasks_axi_fails_closed_when_nothing_can_bound_the_call() {
  local case_dir fb out rc=0
  case_dir=$(make_home xo-tasks-axi-unboundable)
  ...
  out=$(run_bounded_xo_tasks_axi "$fb" 5 show x) || rc=$?
  [ "$rc" -eq 127 ] \
    || fail "xo_tasks_axi ran the call although nothing could bound it (rc=$rc)"
  assert_contains "$out" "cannot bound tasks-axi" \
    "the fail-closed diagnostic did not say why the call was refused"
  pass "xo_tasks_axi fails closed rather than running unbounded when no bounding mechanism exists"
}

test_xo_tasks_axi_gnu_timeout_forces_termination_of_a_sigterm_ignoring_child() {
  local case_dir fb out rc=0 started
  if ! command -v timeout >/dev/null 2>&1; then
    pass "xo_tasks_axi's GNU timeout forces termination (skipped: no timeout binary on this host)"
    return 0
  fi
  case_dir=$(make_home xo-tasks-axi-kill-after)
  ...
  pass "xo_tasks_axi's GNU timeout kills a child that ignores SIGTERM after one further bound"
}
...
interrupt_kimi_readiness() {  # <case-dir>
  ...
  xo_fake_exit0 "$case_dir/fakebin" kimi
  cat > "$case_dir/fakebin/tmux" <<SH
#!/usr/bin/env bash
case "\$*" in
...
write_task_meta() {  # <case-dir> <id> <kind> <mode> [extra-line...]
  ...
  shift 4
  xo_write_meta "$(home_of "$case_dir")/state/$id.meta" \
    "window=xo:xo-$id" \
    "endpoint_task_id=$id" \
...
run_spawn() {  # <case-dir> <args...>
  ...
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$(home_of "$case_dir")" HOME="$case_dir/user-home" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$case_dir/wt" TMUX="fake,1,0" \
    CLAUDE_CONFIG_DIR='' \
...
# backlog transition rather than re-testing tests/xo-teardown.test.sh's matrix.
run_teardown() {  # <case-dir> <id> [args...]
  local case_dir=$1
  shift
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$(home_of "$case_dir")" \
    PATH="$case_dir/fakebin:$PATH" \
    "$TEARDOWN" "$@" 2>&1
}
...
test_backend_resolution_preserves_config_errors() {
  ...
  probe='. "$1/bin/xo-tasks-axi-lib.sh"; "$2" "$3"'
  ...
  for config in "$project_config" "$user_config"; do
    ...
    for resolver in xo_tasks_axi_backend xo_tasks_axi_backend_resolve; do
      rc=0
...
test_backlog_callers_refuse_unreadable_backend_config() (
  ...
  . "$ROOT/bin/xo-tasks-axi-lib.sh"
  . "$ROOT/bin/xo-backlog-transition-lib.sh"
  unset TASKS_AXI_BACKEND
  for operation in xo_backlog_transition_applies xo_backlog_row_show xo_backlog_row_list xo_backlog_row_probe xo_backlog_mutate; do
    case "$operation" in
      xo_backlog_transition_applies) args=("$case_dir/config" "$data" ship) ;;
      xo_backlog_row_list) args=("$data") ;;
      xo_backlog_mutate) args=("$data" start "$id") ;;
      *) args=("$data" "$id") ;;
    esac
    rc=0
    ...
    case "$operation" in
      xo_backlog_row_probe) diagnostic=$XO_BACKLOG_ROW_ERROR ;;
      xo_backlog_transition_applies|xo_backlog_mutate) diagnostic=$XO_BACKLOG_TRANSITION_ERROR ;;
...
test_captain_hold_preserves_relocated_backlog_on_backend_error() {
  ...
  for config_state in dangling absent readable; do
    ...
    rc=0
    out=$(env -u TASKS_AXI_BACKEND HOME="$case_dir/user-home" XO_HOME="$home" \
      XO_DATA_OVERRIDE="$data" "$ROOT/bin/xo-captain-hold.sh" hold "$id" \
...
test_recovery_uses_the_parent_of_a_trailing_slash_data_record() {
  ...
  out=$(XO_DATA_OVERRIDE="$relocated/" run_bootstrap "$case_dir")
...
test_completion_targets_a_nested_relative_data_directory() {
  ...
  out=$(cd "$case_dir" && \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$(home_of "$case_dir")" \
    XO_DATA_OVERRIDE="$relative_data" PATH="$case_dir/fakebin:$PATH" \
    "$TEARDOWN" "$id" 2>&1) \
...
test_immediate_child_absolute_data_dispatches_and_completes() {
  ...
  pin_markdown_backend "$case_dir"
  data="$case_dir/xo-records"
  mv "$(home_of "$case_dir")/data" "$data"
  data_resolved=$(cd "$data" && pwd -P)
  backlog="$data/backlog.md"
  tasks-axi add "$id" "item for $id" --kind ship --file "$backlog" >/dev/null

  out=$(XO_DATA_OVERRIDE="$data" run_ship_spawn "$case_dir" "$id") \
  ...
  out=$(XO_DATA_OVERRIDE="$data" run_teardown "$case_dir" "$id") \
...
test_bare_relative_data_dispatches_and_completes() {
  ...
  out=$(cd "$case_dir" && XO_DATA_OVERRIDE=records run_ship_spawn "$case_dir" "$id") \
  ...
  out=$(cd "$case_dir" && XO_DATA_OVERRIDE=records run_teardown "$case_dir" "$id") \
...
test_deferred_signal_verification_outlives_an_unresponsive_tasks_axi() {
  ...
  # (XO_TASKS_AXI_TIMEOUT=3), print the attempted wording naming the timeout,
  # and exit - the outer `timeout -k 5 30` only turns a regression back into
  # the lock-held-forever hang it exists to catch.
  mkdir -p "$case_dir/user-home"
  out=$(XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$(home_of "$case_dir")" \
    HOME="$case_dir/user-home" XO_SPAWN_NO_GUARD=1 \
    XO_FAKE_PANE_PATH="$case_dir/wt" TMUX="fake,1,0" CLAUDE_CONFIG_DIR='' \
    XO_TASKS_AXI_TIMEOUT=3 PATH="$case_dir/fakebin:$PATH" \
...
test_dispatch_interruption_during_kimi_readiness_fails_before_commit() {
  ...
  out=$(HOME="$home" XO_KIMI_READY_POLLS=2 XO_KIMI_POLL_INTERVAL=0 \
...
test_completion_closes_a_scout_with_its_report() {
  ...
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$(home_of "$case_dir")" \
    PATH="$case_dir/fakebin:$PATH" \
    "$ROOT/bin/xo-captain-hold.sh" complete "$id" --none >/dev/null \
...
test_completion_records_a_relative_report_for_relocated_data() {
  ...
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$(home_of "$case_dir")" \
    XO_DATA_OVERRIDE="$relocated////" PATH="$case_dir/fakebin:$PATH" \
    "$ROOT/bin/xo-captain-hold.sh" complete "$id" --none >/dev/null \
    || fail "could not record the relocated scout's captain-call inventory"

  out=$(XO_DATA_OVERRIDE="$relocated////" run_teardown "$case_dir" "$id") \
...
test_space_containing_scout_report_marker_replays() {
  ...
  printf 'findings\n' > "$data/$id/report.md"
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$(home_of "$case_dir")" \
    XO_DATA_OVERRIDE="$data" PATH="$case_dir/fakebin:$PATH" \
    "$ROOT/bin/xo-captain-hold.sh" complete "$id" --none >/dev/null \
    || fail "could not record the space-path scout's captain-call inventory"
  break_verb "$case_dir" "done"
  marker="$(home_of "$case_dir")/state/$id.backlog-close"

  out=$(XO_DATA_OVERRIDE="$data" run_teardown "$case_dir" "$id") || rc=$?
  ...
  out=$(XO_DATA_OVERRIDE="$data" run_bootstrap "$case_dir")
  [ "$(tasks-axi show "$id" --file "$backlog" 2>/dev/null | sed -n 's/^  state: *//p' | head -1)" = "done" ] \
...
test_trailing_newline_data_path_fails_closed() {
  ...
  out=$(XO_DATA_OVERRIDE="$data" run_ship_spawn "$case_dir" "$id") || rc=$?
...
test_lifecycle_refuses_ancestor_symlinks_outside_home_roots() {
  ...
  out=$(XO_DATA_OVERRIDE="$home/foreign-link/data" run_ship_spawn "$backlog_case" "$id") || rc=$?
  ...
  xo_write_meta "$foreign/state/$id.meta" "kind=ship" "spawn_gen=foreign-worker"
  ln -s "$foreign" "$home/foreign-link"
  rc=0
  out=$(XO_STATE_OVERRIDE="$home/foreign-link/state" run_bootstrap "$worker_case") || rc=$?
  ...
  ln -s "$foreign" "$home/foreign-link"
  rc=0
  out=$(XO_STATE_OVERRIDE="$home/foreign-link/state" run_bootstrap "$close_case") || rc=$?
...
test_same_home_state_override_remains_supported() {
  ...
  out=$(XO_STATE_OVERRIDE="$state" run_bootstrap "$case_dir") \
...
test_no_backlog_teardown_refuses_a_symlinked_task_record_at_entry() {
  ...
  mkdir -p "$target_dir"
  xo_write_meta "$target" \
    "window=xo:xo-$id" "endpoint_task_id=$id" \
...
test_teardown_rechecks_record_parent_after_lock_acquisition() {
  ...
  xo_write_meta "$foreign_state/$id.meta" \
    "window=xo:xo-$id" "endpoint_task_id=$id" \
    "worktree=$foreign_worktree" "project=$case_dir/foreign-project" \
    "harness=claude" "kind=ship" "mode=local-only" "yolo=off"
  track_teardown_resource_actions "$case_dir"
...
test_teardown_refuses_a_symlinked_state_directory_at_entry() {
  ...
  xo_write_meta "$external_state/$id.meta" \
    "window=xo:xo-$id" "endpoint_task_id=$id" \
...
test_spawn_refuses_a_special_file_tasks_config() {
  ...
  out=$(XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$case_dir/wt" TMUX="fake,1,0" \
    CLAUDE_CONFIG_DIR='' \
    PATH="$case_dir/fakebin:$PATH" \
...
test_dispatch_and_completion_are_structural() {
  ...
  id=xo-structural-b15
  pr=https://github.com/example/xo/pull/15
...
test_a_secondmate_home_keeps_its_own_books() {
  ...
  # The mate's home is an xo home in its own right; the invariant is
  # single-host, so its own dispatch and completion keep its own two records
  # paired with no parent involved.
  printf '%s\n' mate-h1 > "$(home_of "$case_dir")/.xo-secondmate-home"
  add_item "$case_dir" "$id"
