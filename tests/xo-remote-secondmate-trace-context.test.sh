#!/usr/bin/env bash
# tests/xo-remote-secondmate-trace-context.test.sh - trace-context regressions for
# the REMOTE second mate route, over the deterministic generic SSH boundary.
#
# The local spawn path's coverage lives in tests/xo-trace-context-spawn.test.sh.
# A remote second mate never reaches that path: bin/xo-spawn.sh routes it through
# spawn_remote_secondmate, which hands the launch to the remote host. These
# assertions drive the real chain - parent xo-spawn -> xo-on -> the real remote
# entrypoint -> xo-remote-secondmate-control -> the remote host's own xo-spawn -
...
TMP_ROOT=$(xo_test_tmproot xo-remote-trace-context)
mkdir -p "$TMP_ROOT"
...
trap 'XO_HOME="$PARENT" XO_PROCEVENT_CLAIM_ROOT="$CLAIMS" "$ROOT/bin/xo-procevent.sh" sweep-home >/dev/null 2>&1 || true; if [ -f "$TMP_ROOT/remote-jobs/worker.pid" ]; then kill "$(cat "$TMP_ROOT/remote-jobs/worker.pid")" 2>/dev/null || true; fi; rm -rf -- "$TMP_ROOT"' EXIT

# The remote host's tracked code root is this branch, as a real git repository:
# xo-on and the remote entrypoint both require the dispatched command to be
...
# literal's XO_TRACE_CONTEXT prefix are both observable exactly as the pane
...
# would inspect the RUNNER's own account; tests/xo-remote-doctor.test.sh owns
# the doctor's behavior against controlled account fixtures.
if printf '%s' "$4" | base64 --decode 2>/dev/null | tr '\0' '\n' | head -1 | grep -q '^xo-remote-doctor.sh$'; then
...
# does, hermetically against an ambient XO_TRACE_CONTEXT.
freeze_parent_session() {
  printf '%s\n' "$$" > "$PARENT/state/.lock"
  (
    unset XO_TRACE_CONTEXT
    xo_trace_context_session_start "$PARENT/config" "$PARENT/state/.trace-context-effective"
  )
}
...
remote_launch_snapshot() {
  grep -o 'XO_TRACE_CONTEXT=[a-z]*' "$HERDR_LOG" | tail -1 | cut -d= -f2
}
...
XO_SECONDMATE_CHARTER='Own iOS delivery on the build Mac.' \
  ...
  remote_env "$ROOT/bin/xo-remote-home-seed.sh" ios remote-mac "$REMOTE_ROOT" "$REMOTE_HOME" --no-projects >/dev/null \
  || fail "remote seed did not provision the traced route"

# --- disabled: the remote route must stay byte-identically untraced ----------
freeze_parent_session
: > "$HERDR_LOG"
remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate >/dev/null 2>&1 \
...
[ "$(remote_launch_snapshot)" = off ] \
  || fail "default-off remote spawn must deliver XO_TRACE_CONTEXT=off (got '$(remote_launch_snapshot)')"
...
[ "$(remote_launch_snapshot)" = on ] \
  || fail "an enabled remote spawn must deliver XO_TRACE_CONTEXT=on (got '$(remote_launch_snapshot)')"
...
LAUNCH_LINE=$(grep -n 'XO_TRACE_CONTEXT=' "$HERDR_LOG" | tail -1 | cut -d: -f1)
...
XO_SECONDMATE_CHARTER='Own the second build Mac.' \
  XO_SECONDMATE_SCOPE='second remote domain' \
  TRACEPARENT="$AMBIENT" \
  remote_env "$ROOT/bin/xo-remote-home-seed.sh" ios2 remote-mac "$REMOTE_ROOT" "$SECOND_HOME" --no-projects >/dev/null \
  || fail "second remote seed failed"
reset_remote_herdr_fixture "$HERDR_STATE"
: > "$HERDR_LOG"
TRACEPARENT="$AMBIENT" remote_env "$ROOT/bin/xo-spawn.sh" ios2 --secondmate >/dev/null 2>&1 \
  || fail "second remote secondmate spawn failed"
SECOND_TP=$(meta_traceparent "$PARENT/state/ios2.meta")
xo_trace_context_valid "$SECOND_TP" \
...
try_flag() { # <expect-substring> <message> [extra args...]
  ...
  shift 2
  if out=$(XO_SPAWN_NO_GUARD=1 XO_HOME="$FLAG_HOME" "$ROOT/bin/xo-spawn.sh" \
