...
# bin/xo-harness.sh checks verified ENV markers before ancestry. A suite run
...
KIMI_HOOK="$ROOT/bin/xo-kimi-turnend-hook.sh"
TMP_ROOT=$(xo_test_tmproot xo-kimi-harness)
KIMI_RUNTIME_TASK_TMP=
PYTHON_BIN=$(command -v python3) || fail "test needs python3"
PYTHON_BIN_DIR=$(dirname "$PYTHON_BIN")
JQ_BIN=$(command -v jq) || fail "test needs jq"
BASE_PATH=${XO_TEST_BASE_PATH:-$PYTHON_BIN_DIR:/usr/bin:/bin:/usr/sbin:/sbin}
...
#!/usr/bin/env bash
set -u
printf '%s\n' "$*" >> "$XO_FAKE_TMUX_CALL_LOG"
state=$(cat "$XO_FAKE_KIMI_STATE" 2>/dev/null || true)
fake_screen() {
  case "$state" in
    ready)
...
case "${1:-}" in
  ...
  send-keys)
    ...
    if [ -n "$literal" ]; then
      case "$literal" in
        *' --auto')
          printf '%s\n' "$literal" >> "$XO_FAKE_LAUNCH_LOG"
          printf 'launched\n' > "$XO_FAKE_KIMI_STATE"
          ;;
        *)
          printf '%s\n' "$literal" >> "$XO_FAKE_POINTER_LOG"
    ...
    case " $* " in
      *' Enter '*)
        case "$state" in
          launched)
            if [ "${XO_FAKE_KIMI_READY:-yes}" = yes ]; then
              printf 'ready\n' > "$XO_FAKE_KIMI_STATE"
            fi
            ;;
          pointer-typed)
            if [ "${XO_FAKE_KIMI_DELIVERY:-yes}" = yes ]; then
              if [ "${XO_FAKE_KIMI_SWALLOW_FIRST:-no}" = yes ] \
                 && [ ! -f "$XO_FAKE_KIMI_SWALLOWED" ]; then
                : > "$XO_FAKE_KIMI_SWALLOWED"
              else
                printf 'delivered\n' > "$XO_FAKE_KIMI_STATE"
              fi
            else
...
SH
  ...
  xo_fake_exit0 "$fakebin" treehouse gh-axi gh
  xo_fake_exit0 "$fakebin" kimi
...
run_spawn() {
  ...
  shift 6
  HOME="$home" XO_ROOT_OVERRIDE='' XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_PROJECTS_OVERRIDE="$home/projects" XO_CONFIG_OVERRIDE="$home/config" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$wt" TMUX="fake,1,0" \
    XO_FAKE_LAUNCH_LOG="$case_dir/launch.log" \
    XO_FAKE_POINTER_LOG="$case_dir/pointer.log" \
    XO_FAKE_KIMI_STATE="$case_dir/kimi.state" \
    XO_FAKE_KIMI_SWALLOWED="$case_dir/kimi.swallowed" \
    XO_FAKE_KIMI_SWALLOW_FIRST="${XO_FAKE_KIMI_SWALLOW_FIRST:-no}" \
    XO_FAKE_TMUX_CALL_LOG="$case_dir/tmux-calls.log" \
    XO_FAKE_BRIEF_REAL="$(cd "$home/data/$id" && pwd -P)/launch-brief.md" \
    XO_KIMI_READY_POLLS=2 XO_KIMI_DELIVERY_POLLS=2 XO_KIMI_POLL_INTERVAL=0 \
...
test_kimi_launch_then_send_is_verified() {
  ...
  out=$(XO_FAKE_KIMI_SWALLOW_FIRST=yes run_spawn \
  ...
  assert_grep "export XO_TASK_ID=$id" "$CASE_DIR/tmux-calls.log" \
    "kimi spawn did not mark the pane with its task id"
  assert_grep 'BEGIN XO KIMI TURN-END HOOK' "$HOME_DIR/.kimi-code/config.toml" \
    "kimi spawn did not install its guarded global hook region"
  assert_grep 'token=' "$WT_DIR/.xo-kimi-turnend" "kimi spawn did not write its token pointer"
  assert_present "$HOME_DIR/state/$id.kimi-turnend-token" "kimi spawn did not record its token"
  pass "xo-spawn: kimi launches, delivers its brief, and registers a guarded turn-end token"
}
...
EOF
  ...
  count=$(grep -c '^# BEGIN XO KIMI TURN-END HOOK' "$config")
  [ "$count" -eq 1 ] || fail "idempotent install left $count XO regions"

  HOME="$home" "$KIMI_HOOK" remove || fail "Kimi hook removal failed"
  cp "$config" "$stripped"
  cmp -s "$original" "$stripped" \
    || fail "config with the XO region excised was not byte-identical to the original"
  assert_absent "$home/.kimi-code/xo-turn-end.sh" "removal left the XO hook script"
  assert_absent "$home/.kimi-code/xo-turn-end.d" "removal left the XO registry"
...
test_kimi_hook_fails_closed_on_missing_malformed_or_partial_config() {
  ...
  assert_absent "$missing/.kimi-code/xo-turn-end.sh" "missing config refusal wrote the hook script"
  ...
  assert_absent "$malformed/.kimi-code/xo-turn-end.sh" "malformed config refusal wrote the hook script"

  printf '# BEGIN XO KIMI TURN-END HOOK\n' > "$partial/.kimi-code/config.toml"
  cp "$partial/.kimi-code/config.toml" "$partial/before"
  rc=0
  out=$(HOME="$partial" "$KIMI_HOOK" install 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "partial XO marker was accepted"
...
test_kimi_hook_install_refuses_without_jq() {
  ...
  assert_absent "$home/.kimi-code/xo-turn-end.sh" "missing-jq refusal wrote the hook script"
  assert_absent "$home/.kimi-code/xo-turn-end.d" "missing-jq refusal wrote the registry"
...
test_kimi_hook_is_silent_and_requires_registered_workspace_token() {
  ...
  hook="$HOME_DIR/.kimi-code/xo-turn-end.sh"
  target="$HOME_DIR/state/$id.turn-ended"
  token=$(sed -n 's/^token=//p' "$WT_DIR/.xo-kimi-turnend")
  assert_present "$HOME_DIR/.kimi-code/xo-turn-end.d/$token" "Kimi registry token is missing"
  ...
  rm "$target"
  fakebin=$(xo_fakebin "$CASE_DIR/no-jq")
  ln -s "$(command -v bash)" "$fakebin/bash"
  out=$(printf '{"hook_event_name":"Stop","session_id":"crew","cwd":"%s","stop_hook_active":false}\n' "$WT_DIR" \
  ...
  pass "Kimi hook stays silent and inert without an XO registry token"
}
...
test_kimi_spawn_refuses_unsafe_global_config_before_pane_creation() {
  ...
  pass "xo-spawn: unsafe Kimi global config refuses before pane creation"
}
...
test_kimi_teardown_removes_pointer_and_registry_token() {
  ...
  HOME="$HOME_DIR" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$HOME_DIR" \
    ...
    XO_SPAWN_NO_GUARD=1 PATH="$FAKEBIN_DIR:$BASE_PATH" \
    "$TEARDOWN" "$id" --force >/dev/null 2>&1 || fail "Kimi teardown failed"
  assert_absent "$WT_DIR/.xo-kimi-turnend" "Kimi token pointer survived teardown"
  assert_absent "$HOME_DIR/.kimi-code/xo-turn-end.d/$token" "Kimi registry token survived teardown"
  assert_absent "$HOME_DIR/state/$id.kimi-turnend-token" "Kimi token state survived teardown"
  pass "xo-teardown: Kimi task pointer and registry token are removed"
}
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  xo_fake_exit0 "$(dirname "$fallback")" kimi
  out=$(run_spawn "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$id")
  rc=$?
  ...
  pass "xo-spawn: Kimi fallback expands the active HOME"
}
...
test_kimi_missing_binary_refuses_before_pane_creation() {
  ...
  pass "xo-spawn: missing Kimi executable refuses before pane creation"
}
...
test_kimi_unconfirmed_delivery_fails_loudly() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(XO_FAKE_KIMI_DELIVERY=no run_spawn \
  ...
  pass "xo-spawn: kimi treats a silent pointer drop as a failed spawn"
}
...
test_kimi_readiness_gate_precedes_pointer() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(XO_FAKE_KIMI_READY=no run_spawn \
    "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$id") || rc=$?
  ...
  pass "xo-spawn: kimi never sends the brief pointer before an observable ready signal"
}
...
SH
  ...
  out=$(env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
    ...
    PATH="$fakebin:$BASE_PATH" XO_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/xo-harness.sh")
  [ "$out" = kimi ] || fail "kimi ancestry detection returned '$out'"
  out=$(env -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI \
    CLAUDECODE=1 PATH="$fakebin:$BASE_PATH" XO_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/xo-harness.sh")
  [ "$out" = claude ] || fail "verified env-marker precedence changed, got '$out'"
  pass "xo-harness: markerless kimi is detected by ancestry after env-marker precedence"
}

test_kimi_session_lock_identity() {
  local home fakebin out
  home="$TMP_ROOT/session-lock-home"
  fakebin=$(xo_fakebin "$TMP_ROOT/session-lock-fake")
  mkdir -p "$home/state"
  cat > "$fakebin/ps" <<'SH'
#!/usr/bin/env bash
case "$*" in
...
SH
  ...
  XO_HOME="$home" PATH="$fakebin:$BASE_PATH" "$ROOT/bin/xo-lock.sh" \
    || fail "xo-lock did not acquire from Kimi ancestry"
  case "$(cat "$home/state/.lock")" in
    ''|*[!0-9]*) fail "xo-lock did not record the Kimi harness ancestor" ;;
  esac
  printf '%s\n' "$$" > "$home/state/.lock"
  out=$(XO_HOME="$home" PATH="$fakebin:$BASE_PATH" "$ROOT/bin/xo-lock.sh" status)
  assert_contains "$out" "lock: held by live harness pid" \
    "xo-lock did not recognize Kimi as a live holder"
  pass "xo-lock recognizes Kimi ancestry and live lock holders"
}

test_kimi_busy_signature_is_scoped_to_spinner_lines() {
  local capture
  # shellcheck source=/dev/null
  . "$ROOT/bin/xo-tmux-lib.sh"
  unset XO_BUSY_REGEX
  ...
  for phase in 🌑 🌒 🌓 🌔 🌕 🌖 🌗 🌘; do
    ...
    xo_pane_is_busy fake kimi || fail "Kimi spinner phase $phase was not recognized as busy"
  done
  printf 'ordinary response ending with 🌕\n│ > │\n' > "$capture"
  if xo_pane_is_busy fake kimi; then
  ...
  if xo_pane_is_busy fake codex; then
...
test_watcher_never_classifies_kimi_from_its_spinner() (
  ...
  unset XO_BUSY_REGEX
  XO_HOME="$TMP_ROOT/watch-home"
  XO_STATE_OVERRIDE="$state"
  export XO_HOME XO_STATE_OVERRIDE
  # shellcheck source=/dev/null
  . "$ROOT/bin/xo-watch.sh"
  ...
  if window_is_busy fake "$busy_capture"; then
    fail "xo-watch classified a Kimi task busy from its spinner instead of unknown"
  fi
  [ "$(xo_busy_classify tmux fake kimi kimi-watch "$state" "$busy_capture")" = "unknown kimi-unverified" ] \
    || fail "a Kimi task must classify unknown kimi-unverified"
  printf 'window=fake\nharness=codex\n' > "$state/kimi-watch.meta"
  if window_is_busy fake "$busy_capture"; then
    fail "xo-watch applied Kimi's spinner to a recorded Codex task"
  fi
  printf 'window=fake\nharness=grok\n' > "$state/kimi-watch.meta"
  if window_is_busy fake "$busy_capture"; then
  ...
  pass "xo-watch classifies Kimi as unknown rather than from its spinner, and Grok's fallback stays isolated"
)
