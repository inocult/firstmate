...
# the adapter package with actual XO extensions and durable scripts.
...
# PI_CODEX_NATIVE_PACKAGE selects the installed adapter package; XO_PI_BIN
# selects Pi. XO_NATIVE_TEST_KEEP=1 preserves the isolated fixture for diagnosis.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"
xo_live_gate default-on XO_PI_CODEX_NATIVE_LIVE node "${XO_PI_BIN:-pi}"
...
if [ ! -f "$PI_CODEX_NATIVE_PACKAGE/index.ts" ]; then
  if [ "${XO_PI_CODEX_NATIVE_LIVE:-${XO_LIVE:-0}}" = 1 ]; then
...
XO_NATIVE_TEST_ROOT="$ROOT" node --input-type=module <<'JS'
import fs from "node:fs";
import path from "node:path";
import assert from "node:assert/strict";
import { spawn, execFileSync } from "node:child_process";
import { homedir, tmpdir } from "node:os";
const root = process.env.XO_NATIVE_TEST_ROOT;
const nativePackage =
  process.env.PI_CODEX_NATIVE_PACKAGE ||
  path.join(homedir(), ".pi/agent/packages/pi-codex-native");
const piVersion = execFileSync(process.env.XO_PI_BIN || "pi", ["--version"], {
...
const fixture = fs.mkdtempSync(path.join(tmpdir(), "xo-native-primary."));
...
script(
  "xo-sessionstart-run.sh",
  '#!/usr/bin/env bash\nprintf "NATIVE_PRIMARY_STARTUP_SENTINEL\\n"\n',
);
script("xo-turnend-guard.sh", "#!/usr/bin/env bash\nexit 0\n");
script(
  "xo-watch-arm.sh",
  `#!/usr/bin/env bash
if [ "\${1:-}" = --handling-delivered ]; then printf 'confirmed\\n' >> "$XO_HOME/state/arm.log"; exit 0; fi
printf 'arm\\n' >> "$XO_HOME/state/arm.log"
printf 'watcher: started pid=%s (beacon fresh) recovery-generation=native-smoke\\n' "$$"
trap 'exit 0' TERM INT
while :; do
 if [ -f "$XO_HOME/state/trigger" ]; then rm -f "$XO_HOME/state/trigger"; printf 'check: native-worker-complete\\n'; exit 0; fi
 sleep 0.05
done
`,
);
...
fs.writeFileSync(
  own,
  `import {writeFileSync,appendFileSync} from 'node:fs'; export default function(pi){pi.on('session_start',()=>writeFileSync(process.env.XO_HOME+'/state/.lock',String(process.pid)+'\\n'));pi.events.on('codex-native:progress',event=>appendFileSync(process.env.XO_HOME+'/state/progress-events',JSON.stringify(event)+'\\n'));}`,
);
...
const log=(data)=>fs.appendFileSync(process.env.XO_HOME+'/state/native.log',JSON.stringify(data)+'\\n');
...
async function handle(q){
 const reply=(result)=>emit({id:q.id,result});const thread={id:'xo-native-primary-thread',cwd:process.cwd(),turns:[],status:{type:'idle'}};
 ...
 if(q.method==='thread/start'||q.method==='thread/resume'){config=q.params.config.mcp_servers.pi_xo;log({kind:'thread',method:q.method});return reply({thread,model:'gpt-6-astra',modelProvider:'openai',cwd:process.cwd(),approvalPolicy:'never',sandbox:{type:'dangerFullAccess'}});}
 ...
 if(text.includes('ARM_PRIMARY'))await control('xo_watch_arm_pi');
 const seq=text.match(/\\[seq (\\d+)\\]/);
 if(seq){await control('xo_branch_outcomes',{recent:1});await control('xo_branch_processed',{through:Number(seq[1])});await control('xo_branch_processed',{through:Number(seq[1])});}
...
async function start(resume) {
  ...
  const args = [
    ...
    ...[
      "xo-primary-turnend-guard.ts",
      "xo-primary-pi-watch.ts",
      "xo-branch-supervision.ts",
  ...
  child = spawn(process.env.XO_PI_BIN || "pi", args, {
    cwd: repo,
    env: {
      ...process.env,
      XO_HOME: home,
      XO_ROOT_OVERRIDE: repo,
...
try {
  ...
  execFileSync(
    path.join(root, "bin/xo-branch-outcome.sh"),
    [
      "append",
      "--task",
      "worker-smoke",
  ...
  await wait(
    () => log().filter((e) => e.name === "xo_branch_processed").length === 2,
    "duplicate acknowledgement refusal",
  );
  let calls = log().filter((e) => e.kind === "control");
  assert.equal(
    calls.filter((e) => e.name === "xo_branch_processed" && !e.result.isError)
      .length,
    1,
  );
  assert.equal(
  ...
  assert(
    calls
      .find((e) => e.name === "xo_branch_outcomes")
  ...
  await stop();
  const prior = log().filter((e) => e.name === "xo_branch_processed").length;
  await start(session);
  ...
  assert.equal(
    log().filter((e) => e.name === "xo_branch_processed").length,
    prior,
  ...
  const progress = fs.readFileSync(path.join(state, "progress-events"), "utf8")
    .trim().split("\n").map(JSON.parse);
  assert(progress.some((event) => event.threadId === "xo-native-primary-thread" && event.phase === "output"),
    ...
    JSON.stringify(
      {
        ...
        piVersion,
        adapterVersion,
        ...(process.env.XO_NATIVE_TEST_KEEP === "1" ? { fixture } : {}),
        checks: [
  ...
  await stop();
  if (passed && process.env.XO_NATIVE_TEST_KEEP !== "1")
    fs.rmSync(fixture, { recursive: true, force: true });
  else console.error("Native XO test fixture: " + fixture);
}
