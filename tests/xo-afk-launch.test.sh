#!/usr/bin/env bash
# tests/xo-afk-launch.test.sh - the script-owned, backend-aware away-daemon
# launch (bin/xo-afk-launch.sh) and the away-mode stale-artifact lifecycle fixes
# (bin/xo-afk-start.sh). Two layers:
#
...
#   byte-identical via the xo-herdr-lab.sh fleet-state tripwire; the tmux path
#   uses uniquely-named throwaway sessions killed by exact name. A harmless
#   sleeper replaces the real daemon (XO_AFK_LAUNCH_ENTRY) so the test observes
#   only the terminal lifecycle.
set -u

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LAUNCH="$ROOT/bin/xo-afk-launch.sh"
START="$ROOT/bin/xo-afk-start.sh"
CONTRACT="$ROOT/bin/xo-afk-contract.sh"
...
unset PI_CODING_AGENT XO_PI_HARNESS CURSOR_AGENT CURSOR_INVOKED_AS GEMINI_CLI ATLASSIAN_AGENT_TYPE ROVODEV_CLI
export CLAUDECODE=1
...
SLEEPER=$(mktemp "${TMPDIR:-/tmp}/xo-afk-sleeper.XXXXXX")
...
confirm_posture() {  # <home>
  XO_HOME="$1" XO_STATE_OVERRIDE="$1/state" "$CONTRACT" propose >/dev/null 2>&1 \
    && XO_HOME="$1" XO_STATE_OVERRIDE="$1/state" "$CONTRACT" confirm >/dev/null 2>&1
}
...
unit_propose_confirm_records_the_posture_without_a_daemon() {
  local st out rc
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-propose.XXXXXX")
  mkdir -p "$st/state"
  out=$(XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" propose \
  ...
  out=$(XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" confirm 2>&1)
  rc=$?
  ...
  printf 'schema\txo-afk-return.v1\nphase\tblocked\n' > "$st/state/.afk-return-catchup"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" propose --action merge --object 'task a PR' --when 'checks green' >/dev/null 2>&1; then
...
unit_pi_never_launches_the_daemon() {
  ...
  for harness in pi pi-signed; do
    st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-pi.XXXXXX")
    mkdir -p "$st/state"
    out=$(XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_TEST_HARNESS="$harness" \
      XO_SUPERVISOR_TARGET=unused XO_SUPERVISOR_BACKEND=tmux XO_AFK_LAUNCH_ENTRY="$SLEEPER" \
      bash -c '. "$1"; xo_afk_launch_primary_harness() { printf "%s" "$XO_TEST_HARNESS"; }; xo_afk_launch_main start' _ "$LAUNCH" 2>&1)
    rc=$?
    ...
    out=$(XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_TEST_HARNESS="$harness" \
      bash -c '. "$1"; xo_afk_launch_primary_harness() { printf "%s" "$XO_TEST_HARNESS"; }; xo_afk_launch_main start-native' _ "$LAUNCH" 2>&1)
    rc=$?
...
unit_daemon_entry_requires_confirmation() {
  local st out rc
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-entry-record.XXXXXX")
  mkdir -p "$st/state"
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$CONTRACT" propose --action merge --object 'task a PR' --when 'checks green' >/dev/null 2>&1
  out=$(XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" start-native 2>&1)
  rc=$?
  ...
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$CONTRACT" confirm >/dev/null 2>&1
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" start-native >/dev/null 2>&1 \
    && [ -e "$st/state/.afk" ]; then
  ...
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" stop >/dev/null 2>&1
  rm -rf "$st"
}

unit_failed_daemon_launch_preserves_confirmed_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-failed-record.XXXXXX")
  mkdir -p "$st/state"
  confirm_posture "$st" || fail "failed start: could not confirm fixture posture"
  if ! XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_SUPERVISOR_TARGET=unused \
    XO_SUPERVISOR_BACKEND=unsupported "$LAUNCH" start >/dev/null 2>&1 \
...
unit_stop_archives_the_record_last() {
  local st epoch
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-stop-archive.XXXXXX")
  mkdir -p "$st/state"
  confirm_posture "$st" || fail "stop archive: could not confirm fixture posture"
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" start-native >/dev/null 2>&1 || fail "stop archive: native entry failed"
  epoch=$(XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$CONTRACT" field entered_epoch)
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" stop >/dev/null 2>&1 \
...
# ---------------------------------------------------------------------------
# UNIT 1: xo_afk_clear_stale_artifacts removes exactly the three stale artifacts.
# ---------------------------------------------------------------------------
unit_clear_stale() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-clear.XXXXXX")
  mkdir -p "$st/state"
  ...
  # Source xo-afk-start.sh inside a child bash (it sets `set -eu` and would
  # otherwise leak that into this test shell) and call the clear helper.
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" \
    bash -c '. "$1"; xo_afk_clear_stale_artifacts "$2"' _ "$START" "$st/state"
...
unit_relative_paths_are_absolute_before_daemon_launch() {
  ...
  root=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-relative-home.XXXXXX")
  mkdir -p "$root/home/state" "$root/cdpath/home/state"
  home=$(cd "$root/home" && pwd -P)
  state="$home/state"
  out=$(
    cd "$root" || exit 1
    CDPATH="$root/cdpath" XO_HOME=home XO_STATE_OVERRIDE=home/state \
      bash -c '. "$1"; printf "%s\n%s\n" "$XO_HOME" "$XO_AFK_LAUNCH_STATE"' _ "$LAUNCH"
  )
  if [ "$out" = "$home"$'\n'"$state" ]; then
  ...
  out=$(XO_HOME="$linked_home" XO_STATE_OVERRIDE="$linked_home/state" \
    bash -c '. "$1"; printf "%s\n%s\n" "$XO_HOME" "$XO_AFK_LAUNCH_STATE"' _ "$LAUNCH")
  ...
  out=$(
    cd "$root" || exit 1
    XO_HOME=missing-home "$LAUNCH" help 2>&1
  )
  status=$?
  if [ "$status" -ne 0 ] && printf '%s\n' "$out" | grep -F "XO_HOME directory cannot be resolved: missing-home" >/dev/null; then
    pass "launcher paths: unresolved relative XO_HOME fails loudly"
  else
    fail "launcher paths: unresolved relative XO_HOME did not name the bad input ($out)"
  fi
  out=$(
    cd "$root" || exit 1
    XO_HOME=home XO_STATE_OVERRIDE=missing-state "$LAUNCH" help 2>&1
  )
  status=$?
  if [ "$status" -ne 0 ] && printf '%s\n' "$out" | grep -F "XO_STATE_OVERRIDE directory cannot be resolved: missing-state" >/dev/null; then
    pass "launcher paths: unresolved relative XO_STATE_OVERRIDE fails loudly"
  else
    fail "launcher paths: unresolved relative XO_STATE_OVERRIDE did not name the bad input ($out)"
  fi
  rm -rf "$root"
}
...
unit_fresh_vs_refresh() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-refresh.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  : > "$st/state/.subsuper-inject-wedged"
  ...
  ( . "$ROOT/bin/xo-wake-lib.sh"; xo_pid_identity "$sleep_pid" > "$lock/pid-identity" 2>/dev/null ) || true
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$START" >/dev/null 2>&1
...
# ---------------------------------------------------------------------------
# UNIT 3: exit ordering - xo_afk_launch_stop SIGTERMs the daemon WHILE .afk is
# still present (so its flush is not a no-op), and clears .afk last.
# ---------------------------------------------------------------------------
unit_stop_ordering() {
  local st lock marker daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-stop.XXXXXX")
  mkdir -p "$st/state"
  date '+%s' > "$st/state/.afk"
  ...
  # shellcheck source=/dev/null
  ( . "$ROOT/bin/xo-wake-lib.sh"; xo_pid_identity "$daemon_pid" > "$lock/pid-identity" 2>/dev/null ) || true
...
unit_stop_rejects_reused_pid() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-pid-reuse.XXXXXX")
  mkdir -p "$st/state"
  date '+%s' > "$st/state/.afk"
  sleep 600 &
  # shellcheck disable=SC2031 # The background PID is captured immediately in this shell.
...
unit_failed_start_rolls_back_state() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-failed-start.XXXXXX")
  mkdir -p "$st/state"
  printf 'pending\n' > "$st/state/.subsuper-escalations"
  printf 'wedged\n' > "$st/state/.subsuper-inject-wedged"
  confirm_posture "$st" || fail "failed start: could not confirm fixture posture"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_SUPERVISOR_TARGET=unused \
    XO_SUPERVISOR_BACKEND=unsupported "$LAUNCH" start >/dev/null 2>&1; then
...
unit_concurrent_start_serialized() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-concurrent.XXXXXX")
  cap_session="xo-afk-concurrent-cap-$$"
  ...
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_SUPERVISOR_TARGET="$cap_pane" \
    XO_SUPERVISOR_BACKEND=tmux XO_AFK_LAUNCH_ENTRY="$SLEEPER" "$LAUNCH" start >/dev/null 2>&1 &
  # shellcheck disable=SC2031 # The background PID is captured immediately in this shell.
  first=$!
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_SUPERVISOR_TARGET="$cap_pane" \
    XO_SUPERVISOR_BACKEND=tmux XO_AFK_LAUNCH_ENTRY="$SLEEPER" "$LAUNCH" start >/dev/null 2>&1 &
...
unit_lock_initialization_grace() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-lock-init.XXXXXX")
  ...
  (
    ...
    if [ -d "$st/state/.afk-launch.lock" ]; then
      ...
      # shellcheck source=/dev/null
      ( . "$ROOT/bin/xo-wake-lib.sh"; xo_pid_identity "$$" > "$st/state/.afk-launch.lock/pid-identity" 2>/dev/null ) || true
  ...
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
...
unit_signal_exits_with_lock_cleanup() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-signal.XXXXXX")
  marker="$st/resumed"
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    xo_afk_launch_start() { sleep 30; }
    xo_afk_launch_main start
    : > "$2"
  ' _ "$LAUNCH" "$marker" &
  # shellcheck disable=SC2031 # The background PID is captured immediately in this shell.
  child=$!
...
unit_herdr_partial_create_recovery() {
  local st recorded
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-herdr-partial.XXXXXX")
  recorded="$st/recorded"
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_AFK_LAUNCH_ENTRY=/bin/true \
    XO_AFK_LAUNCH_LABEL=afk-exact-label RECORDED="$recorded" bash -c '
    . "$1"
    xo_backend_source() { return 0; }
    xo_backend_herdr_server_ensure() { return 0; }
    ...
    xo_afk_launch_record_write() { printf "%s:%s:%s" "$1" "$2" "$3" > "$RECORDED"; }
    xo_afk_launch_create_herdr lab:captain herdr
  ' _ "$LAUNCH"
...
unit_herdr_error_with_exact_ids_closes_exact() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-herdr-error-exact.XXXXXX")
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    xo_backend_source() { return 0; }
    xo_backend_herdr_server_ensure() { return 0; }
...
unit_herdr_run_failure_preserves_unconfirmed_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-herdr-run-fail.XXXXXX")
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    xo_backend_source() { return 0; }
    xo_backend_herdr_server_ensure() { return 0; }
...
unit_record_failure_closes_terminal() {
  local st closed
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-record-fail.XXXXXX")
  closed="$st/closed"
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" CLOSED="$closed" bash -c '
    . "$1"
    xo_afk_launch_record_write() { return 1; }
    xo_afk_launch_close_terminal() { printf "%s:%s" "$1" "$2" > "$CLOSED"; }
    ! xo_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_readiness_failure_rolls_back_terminal() {
  local st closed
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-not-ready.XXXXXX")
  closed="$st/closed"
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" CLOSED="$closed" bash -c '
    . "$1"
    xo_afk_launch_wait_ready() { return 1; }
    xo_afk_launch_close_terminal() { printf "%s:%s" "$1" "$2" > "$CLOSED"; }
    xo_afk_launch_terminal_absent() { [ -e "$CLOSED" ]; }
    ! xo_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_readiness_failure_preserves_unconfirmed_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-not-ready-unconfirmed.XXXXXX")
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    xo_afk_launch_wait_ready() { return 1; }
    xo_afk_launch_close_terminal() { return 1; }
    xo_afk_launch_terminal_absent() { return 1; }
    ! xo_afk_launch_commit_terminal tmux exact-session ""
  ' _ "$LAUNCH"
...
unit_tmux_absence_distinguishes_probe_failure() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-tmux-probe.XXXXXX")
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() { printf "%s" "can'\''t find session: exact-session" >&2; return 1; }
    xo_afk_launch_terminal_absent tmux exact-session
...
unit_native_lifecycle() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-native.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
...
unit_native_entry_preserves_prepared_state() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-native-entry.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  : > "$st/state/.subsuper-escalations"
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_AFK_STATE_PREPARED=1 bash -c '
    . "$1"
    XO_AFK_DAEMON=/bin/true
    xo_afk_start_main
...
unit_close_failure_preserves_record() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-close-fail.XXXXXX")
  mkdir -p "$st/state"
...
unit_record_publication_atomic() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-record-atomic.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\told-session\towned\n' > "$st/state/.afk-daemon-terminal"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    mv() { return 1; }
    ! xo_afk_launch_record_write tmux new-session owned
  ' _ "$LAUNCH" \
...
unit_malformed_record_fails_closed() {
  local st acted
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-record-malformed.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  acted="$st/acted"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" ACTED="$acted" bash -c '
    . "$1"
    xo_afk_launch_close_terminal() { : > "$ACTED"; }
    ! xo_afk_launch_reconcile
  ' _ "$LAUNCH" \
...
unit_stop_malformed_record_fails_closed() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-stop-malformed.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
...
unit_tmux_planned_record_and_collision() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-tmux-plan.XXXXXX")
  mkdir -p "$st/state"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() {
      if [ "$1" = new-session ]; then
        [ -s "$XO_AFK_LAUNCH_RECORD" ] || return 9
        printf "%s" "$4" > "$XO_HOME/created-name"
        return 1
      fi
      [ "$1" != kill-session ] || : > "$XO_HOME/killed"
      return 1
    }
    ! xo_afk_launch_create_tmux captain:0 tmux
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-tmux-unique.XXXXXX")
  mkdir -p "$st/state"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    tmux() {
      [ "$1" != new-session ] || { printf "%s" "$4" > "$XO_HOME/created-name"; return 1; }
      [ "$1" != kill-session ] || : > "$XO_HOME/killed"
      return 1
    }
    ! xo_afk_launch_create_tmux captain:0 tmux
...
unit_stop_validates_before_signal() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-stop-validate.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  sleep 30 &
  ...
  # shellcheck source=/dev/null
  ( . "$ROOT/bin/xo-wake-lib.sh"; xo_pid_identity "$sleeper_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" "$LAUNCH" stop >/dev/null 2>&1 || true
...
unit_lock_requires_complete_metadata() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-lock-metadata.XXXXXX")
  mkdir -p "$st/state"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    xo_pid_identity() { return 1; }
    ! xo_afk_launch_lock_acquire
...
unit_stop_surfaces_afk_removal_failure() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-stop-remove.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    rm() { local last=${!#}; [ "$last" != "$XO_AFK_LAUNCH_STATE/.afk" ]; }
    ! xo_afk_launch_stop
  ' _ "$LAUNCH"; then
...
unit_stop_confirms_daemon_exit() {
  local st daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-stop-live.XXXXXX")
  mkdir -p "$st/state/.supervise-daemon.lock"
  : > "$st/state/.afk"
  printf 'none\t-\tnative\n' > "$st/state/.afk-daemon-terminal"
  ...
  # shellcheck source=/dev/null
  ( . "$ROOT/bin/xo-wake-lib.sh"; xo_pid_identity "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    seq() { printf "1\n"; }
    sleep() { :; }
    kill() {
      command kill "$@"
      if [ "$1" = -TERM ]; then
        rm -rf "$XO_AFK_LAUNCH_STATE/.supervise-daemon.lock"
      fi
    }
    ! xo_afk_launch_stop
...
unit_refresh_validates_record() {
  local st daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-refresh-record.XXXXXX")
  mkdir -p "$st/state/.supervise-daemon.lock"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  sleep 30 &
  # shellcheck disable=SC2031 # The background PID is captured immediately in this shell.
  ...
  ( . "$ROOT/bin/xo-wake-lib.sh"; xo_pid_identity "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid-identity" )
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" XO_SUPERVISOR_TARGET=unused \
    XO_SUPERVISOR_BACKEND=tmux bash -c '
      . "$1"
      ! xo_afk_launch_start && ! xo_afk_launch_start_native
...
unit_clear_failure_aborts_entry() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-clear-fail.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    xo_afk_launch_reconcile() { return 0; }
    xo_afk_clear_stale_artifacts() { return 1; }
...
unit_confirmed_absence_succeeds() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-confirmed-absent.XXXXXX")
  mkdir -p "$st/state"
  printf 'tmux\texact-session\towned\n' > "$st/state/.afk-daemon-terminal"
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    xo_afk_launch_close_terminal() { return 1; }
    xo_afk_launch_terminal_absent() { return 0; }
    xo_afk_launch_reconcile
...
unit_incomplete_restore_retains_backup() {
  local st backup
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-restore-fail.XXXXXX")
  mkdir -p "$st/state"
  ...
  if XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
    cp() { return 1; }
    ! xo_afk_launch_restore_backup "$2" 1
...
unit_flag_write_failure_aborts() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-flag-fail.XXXXXX")
  mkdir -p "$st/state"
  XO_HOME="$st" XO_STATE_OVERRIDE="$st/state" bash -c '
    . "$1"
...
e2e_herdr() {
  ...
  SESSION="xo-lab-afk-launch-e2e-$$"
  export HERDR_SESSION="$SESSION"
  home_tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-e2e-home.XXXXXX")
  E2E_HERDR_CLEANUP() {
    # shellcheck disable=SC2031 # Cleanup reads the caller's resolved target; it does not reassign it.
    XO_HOME="$home_tmp" XO_STATE_OVERRIDE="$home_tmp/state" \
      XO_SUPERVISOR_TARGET="$target" XO_SUPERVISOR_BACKEND=herdr "$LAUNCH" stop >/dev/null 2>&1 || true
    herdr_safe_stop_and_delete "$SESSION" >/dev/null 2>&1 || true
    rm -rf "$home_tmp" 2>/dev/null || true
  }
  xo_herdr_lab_prepare "$SESSION" || { fail "herdr e2e: could not prepare isolated lab session"; return 0; }
  xo_backend_source herdr || { E2E_HERDR_CLEANUP; fail "herdr e2e: xo_backend_source herdr failed"; return 0; }
  xo_backend_herdr_server_ensure "$SESSION" || { E2E_HERDR_CLEANUP; fail "herdr e2e: lab server did not start"; return 0; }

  out=$(xo_backend_herdr_cli "$SESSION" workspace create --cwd "$ROOT" --label captain --no-focus 2>/dev/null)
  ...
  before=$(xo_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_before=$(xo_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')

  XO_HOME="$home_tmp" XO_STATE_OVERRIDE="$home_tmp/state" \
    XO_SUPERVISOR_TARGET="$target" XO_SUPERVISOR_BACKEND=herdr XO_AFK_LAUNCH_ENTRY="$SLEEPER" \
    "$LAUNCH" start >/dev/null 2>&1

  during=$(xo_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_during=$(xo_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')
  dtgt=$(cut -f2 "$home_tmp/state/.afk-daemon-terminal" 2>/dev/null || true)
  dtab=$(xo_backend_herdr_cli "$SESSION" pane get "${dtgt#*:}" 2>/dev/null | jq -r '.result.pane.tab_id // empty')
  ...
  XO_HOME="$home_tmp" XO_STATE_OVERRIDE="$home_tmp/state" \
    XO_SUPERVISOR_TARGET="$target" XO_SUPERVISOR_BACKEND=herdr "$LAUNCH" stop >/dev/null 2>&1

  after=$(xo_backend_herdr_cli "$SESSION" pane list --workspace "$cap_ws" 2>/dev/null | jq --arg t "$cap_tab" '[.result.panes[]?|select(.tab_id==$t)]|length')
  ws_after=$(xo_backend_herdr_cli "$SESSION" workspace list 2>/dev/null | jq '[.result.workspaces[]?]|length')
...
e2e_tmux() {
  ...
  cap_session="xo-afk-launch-cap-$$"
  home_tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-afk-tmux-home.XXXXXX")
  ...
  XO_HOME="$home_tmp" XO_STATE_OVERRIDE="$home_tmp/state" \
    XO_SUPERVISOR_TARGET="$cap_pane" XO_SUPERVISOR_BACKEND=tmux XO_AFK_LAUNCH_ENTRY="$SLEEPER" \
    "$LAUNCH" start >/dev/null 2>&1
  ...
  XO_HOME="$home_tmp" XO_STATE_OVERRIDE="$home_tmp/state" \
    XO_SUPERVISOR_TARGET="$cap_pane" XO_SUPERVISOR_BACKEND=tmux "$LAUNCH" stop >/dev/null 2>&1
