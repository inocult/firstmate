#!/usr/bin/env bash
# Behavior tests for X mode: the relay poll client (xo-x-poll.sh), the answer
# poster (xo-x-reply.sh), and bootstrap's .env-presence activation.
#
...
TMP_ROOT=$(xo_test_tmproot xo-x-mode-tests)
...
make_sample_image() {
  ...
  case "$path" in
    *.png) printf '\211PNG\r\n\032\nxo-test-png' > "$path" ;;
    *.jpg|*.jpeg) printf '\377\330\377xo-test-jpeg' > "$path" ;;
    *.gif) printf 'GIF89axo-test-gif' > "$path" ;;
    *.webp) printf 'RIFF....WEBPxo-test-webp' > "$path" ;;
    *) printf 'xo-test-image' > "$path" ;;
  esac
}
...
assert_no_private_artifact_temps() {
  ...
  [ -d "$dir" ] || return 0
  leftovers=$(find "$dir" -name '*.xo-x.*' -print 2>/dev/null)
...
test_poll_no_token_is_hard_noop() {
  ...
  pass "xo-x-poll is a hard no-op without a token (inert default)"
}
...
test_poll_empty_env_token_overrides_env_file() {
  ...
  pass "xo-x-poll treats an explicitly empty env token as configured"
}
...
test_poll_204_is_silent() {
  ...
  pass "xo-x-poll stays silent on HTTP 204 (the common case)"
}
...
test_poll_auth_error_reports_once() {
  ...
  pass "xo-x-poll surfaces auth/config errors once and clears on recovery"
}
...
test_poll_error_private_publication_rejects_unsafe_paths() {
  ...
  pass "xo-x-poll diagnostic markers use private guarded publication"
}
...
test_poll_question_stashes_and_marks() {
  ...
  pass "xo-x-poll stashes the question and prints the compact marker"
}
...
test_poll_mentions_wake_once_per_durable_offer() {
  ...
  pass "xo-x-poll wakes once per durable request offer across inbox cleanup"
}
...
test_poll_offer_claim_failure_reports_once() {
  ...
  pass "xo-x-poll retains offer claim diagnostics until recovery"
}
...
test_poll_preserves_conversation_context() {
  ...
  pass "xo-x-poll preserves in_reply_to conversation context in the inbox"
}
...
test_poll_preserves_inbound_attachment_urls() {
  ...
  pass "xo-x-poll preserves inbound attachment URLs for the responder"
}
...
SH
  ...
  pass "xo-x-poll reports inbox commit failures without emitting a mention wake"
}
...
test_poll_inbox_private_publication_rejects_unsafe_paths() {
  ...
  pass "xo-x-poll publishes inbox records only through private guarded artifacts"
}
...
test_poll_rejects_unsafe_request_id() {
  ...
  pass "xo-x-poll rejects an unsafe request_id (path-traversal guard)"
}
...
test_reply_non_2xx_fails() {
  ...
  pass "xo-x-reply exits non-zero on a non-2xx relay response"
}
...
SH
  ...
  pass "xo-x-reply cleans up auth header temp files on interrupted posts"
}
...
test_reply_usage_error() {
  ...
  err="$home/err.txt"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-reply.sh" "only-one" >/dev/null 2>"$err"; rc=$?
  expect_code 2 "$rc" "reply usage error exit"
  assert_grep "--image <path>" "$err" "reply usage must mention --image"
  pass "xo-x-reply rejects missing arguments with a usage error"
}

test_reply_help_mentions_image() {
  local home out rc
  home="$TMP_ROOT/reply-help"; mkdir -p "$home"
  out=$(PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-reply.sh" --help); rc=$?
  ...
  pass "xo-x-reply --help makes image support discoverable"
}
...
test_reply_whitespace_text_rejected() {
  ...
  pass "xo-x-reply rejects whitespace-only reply text"
}
...
test_bootstrap_activates_on_env_token() {
  ...
  assert_grep "export XO_CHECK_INTERVAL=30" "$home/config/x-mode.env" "cadence must be 30s"
  # Cadence inheritance: sourcing the config exports the 30s interval to a child,
  # exactly how xo-watch-arm.sh's forked watcher inherits it.
  local inherited
  # shellcheck source=/dev/null
  inherited=$( . "$home/config/x-mode.env" && bash -c 'echo "${XO_CHECK_INTERVAL:-300}"' )
  [ "$inherited" = "30" ] \
    || fail "sourcing the cadence config must export XO_CHECK_INTERVAL=30 to a child"
...
test_bootstrap_relative_home_writes_absolute_poll_shim() {
  ...
  pass "bootstrap ignores CDPATH when writing absolute XO_HOME into the durable X-mode poll shim"
}
...
test_bootstrap_reports_missing_x_dependency() {
  ...
  fakebin=$(xo_fakebin "$home")
  xo_fake_exit0 "$fakebin" tmux node no-mistakes chrome-devtools-axi curl
...
test_poll_empty_text_is_silent() {
  ...
  pass "xo-x-poll requires a non-empty question before waking"
}
...
test_reply_text_file_and_stdin() {
  ...
  pass "xo-x-reply accepts the reply via --text-file and stdin (safe, unexpanded)"
}
...
test_reply_dry_run_records_not_posts() {
  ...
  pass "xo-x-reply dry-run records the would-be reply and never posts"
}
...
test_reply_dry_run_needs_no_token() {
  ...
  pass "xo-x-reply dry-run works without a token"
}
...
test_reply_empty_env_dry_run_overrides_env_file() {
  ...
  pass "xo-x-reply lets an explicitly empty dry-run env override .env"
}
...
test_reply_dry_run_fails_when_outbox_unwritable() {
  ...
  pass "xo-x-reply dry-run fails when it cannot record the preview"
}
...
test_reply_dry_run_outbox_private_publication_rejects_unsafe_paths() {
  ...
  assert_no_private_artifact_temps "$home/state/x-outbox"
  pass "xo-x-reply dry-run publishes outbox records only through private guarded artifacts"
}
...
test_reply_single_no_texts() {
  ...
  pass "xo-x-reply keeps a concise reply as a single unnumbered tweet"
}
...
test_reply_thread_dry_run() {
  ...
  pass "xo-x-reply auto-splits a long reply into a numbered thread (texts[])"
}
...
TXT
  ...
  pass "xo-x-reply uses the Discord inbox platform budget instead of the X tweet budget"
}
...
test_reply_x_inbox_still_uses_x_budget() {
  ...
  pass "xo-x-reply keeps numeric X requests on the X tweet budget"
}
...
test_reply_inbox_explicit_limit_wins() {
  ...
  pass "xo-x-reply prefers an explicit relay-provided reply limit"
}
...
test_reply_rejects_unsafe_inbox_context_reads() {
  ...
  pass "xo-x-reply rejects unsafe inbox context artifacts"
}
...
test_reply_max_chars_floor_clamps_to_minimum() {
  ...
  pass "xo-x-reply clamps a below-floor max to 50 characters"
}
...
test_reply_thread_live_posts_texts() {
  ...
  pass "xo-x-reply posts a thread payload (texts[]) to the relay"
}
...
test_reply_image_live_posts_image_object() {
  ...
  pass "xo-x-reply --image posts an image object on answer"
}
...
test_reply_image_live_streams_payload_file() {
  ...
  pass "xo-x-reply streams large image payloads outside curl argv"
}
...
test_reply_image_thread_dry_run_records_compact_marker() {
  ...
  pass "xo-x-reply dry-run records compact image metadata for threaded replies"
}
...
test_reply_image_dry_run_cleans_payload_temp_files() {
  ...
  leftovers=$(find "$tmpdir" -type f -name 'xo-x-reply.*' -print)
  [ -z "$leftovers" ] || fail "reply temp files must be cleaned (left: $leftovers)"
  pass "xo-x-reply cleans image and payload temp files"
}
...
test_reply_image_path_errors_are_clear() {
  ...
  pass "xo-x-reply --image rejects missing and unsupported image paths clearly"
}
...
test_reply_followup_live_posts_to_followup_endpoint() {
  ...
  pass "xo-x-reply --followup posts to /connector/followup with the same request-bound body"
}
...
test_reply_followup_409_marker_exits_distinctly() {
  ...
  pass "xo-x-reply maps a followup_unavailable follow-up 409 to exit 9"
}
...
test_reply_followup_409_without_marker_still_exits_distinctly() {
  ...
  pass "xo-x-reply maps every follow-up 409 to exit 9 even without the marker"
}
...
test_reply_answer_409_is_generic_failure() {
  ...
  pass "xo-x-reply treats answer-endpoint 409 as a generic failure"
}
...
test_reply_followup_image_live_posts_image_object() {
  ...
  pass "xo-x-reply --followup --image posts an image object"
}
...
test_reply_followup_flag_position_is_flexible() {
  ...
  pass "xo-x-reply --followup is accepted in any position and leaves the answer path default"
}
...
test_reply_followup_thread_dry_run() {
  ...
  pass "xo-x-reply --followup auto-splits a long follow-up into a marked thread"
}
...
test_reply_followup_image_dry_run_marks_endpoint_and_compacts_image() {
  ...
  pass "xo-x-reply followup dry-run keeps endpoint marker and compact image metadata"
}
...
# poll records a durable per-request reply-context registry, xo-x-reply resolves
...
test_poll_records_context_registry_from_relay_platform() {
  ...
  pass "xo-x-poll records the durable per-request reply context from the relay payload"
}
...
test_dismiss_clears_context_registry() {
  ...
  pass "xo-x-dismiss clears the durable per-request context (a dismissed mention gets no follow-up)"
}

# --- xo-x-dismiss: drop a mention at the relay without replying ---------------
...
test_dismiss_success_posts_request_only() {
  ...
  pass "xo-x-dismiss posts a request-bound dismiss and echoes only the request_id"
}
...
test_dismiss_dry_run_records_not_posts() {
  ...
  pass "xo-x-dismiss dry-run records the would-be body and never posts"
}
...
test_dismiss_dry_run_needs_no_token() {
  ...
  pass "xo-x-dismiss dry-run works without a token"
}
...
test_dismiss_dry_run_outbox_private_publication_rejects_unsafe_paths() {
  ...
  assert_no_private_artifact_temps "$home/state/x-outbox"
  pass "xo-x-dismiss dry-run publishes outbox records only through private guarded artifacts"
}
...
test_dismiss_non_2xx_fails() {
  ...
  pass "xo-x-dismiss exits non-zero on a non-2xx relay response"
}
...
SH
  ...
  pass "xo-x-dismiss exits non-zero on a transport failure"
}
...
test_dismiss_unsafe_request_id_rejected() {
  ...
  pass "xo-x-dismiss rejects an unsafe request_id (path-traversal guard)"
}

test_dismiss_usage_error() {
  local home rc
  home="$TMP_ROOT/dismiss-usage"; mkdir -p "$home"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-dismiss.sh" >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss missing-arg usage exit"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-dismiss.sh" req-1 extra >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "dismiss extra-arg usage exit"
  pass "xo-x-dismiss rejects missing or extra arguments with a usage error"
}

# --- xo-x-link: task <-> X-request association in meta -----------------------
...
test_link_records_request_and_timestamp() {
  ...
  pass "xo-x-link records and refreshes the X-request link without disturbing meta"
}
...
TXT
  ...
  pass "xo-x-link records Discord platform context so follow-ups keep the Discord budget"
}
...
TXT
  ...
  pass "xo-x-link resolves the platform by request_id so a post-cleanup link keeps the Discord budget"
}

# Criterion 2 loud-warning branch: when the inbox and relay cannot resolve both
# axes, the link is still recorded but xo-x-link warns loudly and every follow-up
# is refused.
test_link_warns_loudly_when_platform_unresolvable() {
  ...
  pass "xo-x-link warns loudly and the follow-up is held (not wrongly split) when the platform is unknown"
}
...
test_link_carry_count_and_ts_preserve_followup_binding() {
  ...
  pass "xo-x-link paired carry flags preserve a prior task's follow-up binding onto a successor"
}
...
TXT
  ...
  pass "xo-x-link recovery relink preserves Discord platform context after inbox drain"
}
...
test_link_carry_count_validation() {
  ...
  err="$home/err.txt"
  printf 'window=w\nkind=ship\n' > "$home/state/ok.meta"
  PATH="$BASE_PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-x-link.sh" ok req-1 --carry-count abc >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-x-link.sh" ok req-1 --carry-ts abc >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-x-link.sh" ok req-1 --carry-count >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link --carry-count missing value exit"
  PATH="$BASE_PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-x-link.sh" ok req-1 --carry-count 1 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-x-link.sh" ok req-1 --carry-ts 1700000000 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-x-link.sh" ok req-1 --carry-count 1 --carry-ts 1700000000 >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-x-link.sh" ok req-1 --carry-platform discord >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-x-link.sh" ok req-1 --carry-count 1 --carry-ts 1700000000 --carry-max 49 >/dev/null 2>"$err"; rc=$?
  ...
  pass "xo-x-link rejects malformed or unpaired carry flags"
}
...
test_meta_helpers_refuse_a_symlinked_task_record() {
  ...
  assert_symlink_untouched() {
    ...
    leftover=$(find "$home/state" -maxdepth 1 -name '.*.xo-x.*' -print 2>/dev/null || true)
  ...
  XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" --clear sym-task >/dev/null 2>&1; rc=$?
...
test_link_rejects_unsafe_and_missing() {
  ...
  printf 'kind=ship\n' > "$home/state/ok.meta"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-link.sh" "../evil" req-1 >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link unsafe task id exit"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-link.sh" ok "../../etc/x" >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-link.sh" no-such req-1 >/dev/null 2>&1; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-link.sh" ok >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "link missing arg exit"
  pass "xo-x-link rejects unsafe ids, missing meta, and missing arguments"
}
...
test_link_missing_task_without_secondmates_stays_plain() {
  ...
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-link.sh" no-such req-1 >/dev/null 2>"$err"; rc=$?
  ...
  assert_no_grep "xo-public-followup.sh register" "$err" \
    "a home with no second mates must not be pointed at the promised-final path"
  pass "xo-x-link keeps the plain missing-task error when no second mate is registered"
}
...
test_link_refuses_secondmate_routed_task_with_promised_final_pointer() {
  ...
  out=$(PATH="$BASE_PATH" XO_HOME="$main" "$ROOT/bin/xo-x-link.sh" routed-k1 req-routed 2>"$err"); rc=$?
  ...
  assert_grep "xo-public-followup.sh register" "$err" \
  ...
  pass "xo-x-link refuses a second-mate-routed task and points at the promised-final path"
}
...
test_link_missing_task_with_secondmates_points_at_promised_final() {
  ...
  printf '# Second mates\n\n- sm-far - owns the far domain (host: box; root: /srv/fm; home: /srv/xo/home; scope: far things; projects: far; added 2026-01-01)\n' \
    > "$home/data/secondmates.md"
  err="$TMP_ROOT/link-remote-secondmate.err"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-link.sh" unknown-k1 req-unknown >/dev/null 2>"$err"; rc=$?
  ...
  pass "xo-x-link points an unlocatable task at the promised-final path when second mates exist"
}

# --- xo-x-followup: detect, post up to 3 follow-ups, manage the link --------
...
test_followup_check_states() {
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" --check plain 2>/dev/null); rc=$?
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" --check nope 2>/dev/null); rc=$?
  expect_code 1 "$rc" "check missing-meta exit"
  pass "xo-x-followup --check reports postable / not-linked correctly"
}
...
test_followup_check_expired_prunes_link() {
  ...
  pass "xo-x-followup --check prunes a link past the 7-day window"
}
...
test_followup_check_cap_reached_prunes_link() {
  ...
  pass "xo-x-followup --check prunes a link that already reached the follow-up cap"
}
...
test_followup_post_increments_counter_keeps_link() {
  ...
  pass "xo-x-followup posts a follow-up, increments the counter, and keeps the link under the cap"
}
...
test_followup_post_final_clears_link_immediately() {
  ...
  pass "xo-x-followup --final clears the link after one post regardless of the remaining count"
}
...
test_followup_post_cap_reached_clears_link() {
  ...
  pass "xo-x-followup clears the link once the third follow-up reaches the cap"
}
...
test_followup_post_forwards_image_to_reply_client() {
  ...
  pass "xo-x-followup --image forwards the attachment through xo-x-reply --followup"
}
...
test_followup_post_failure_keeps_link() {
  ...
  pass "xo-x-followup keeps the link and counter when the post fails"
}
...
SH
  ...
  pass "xo-x-followup tombstones the link when a post-success counter write fails"
}
...
test_followup_post_relay_rejection_degrades_gracefully() {
  ...
  pass "xo-x-followup treats a relay cap/window rejection as an already-exhausted link, not a retry"
}
...
test_followup_post_expired_skips_and_clears() {
  ...
  pass "xo-x-followup skips silently and clears the link past the 7-day window"
}
...
test_followup_post_not_linked_is_noop() {
  ...
  printf 'kind=ship\n' > "$home/state/plain.meta"
  out=$(XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" plain - <<<"nothing to do" 2>/dev/null); rc=$?
  ...
  pass "xo-x-followup is a no-op for a task with no X link"
}
...
test_followup_post_dry_run_increments_counter_keeps_link() {
  ...
  pass "xo-x-followup dry-run records the follow-up and increments the counter, keeping the link"
}
...
test_followup_post_dry_run_final_clears_link() {
  ...
  pass "xo-x-followup dry-run --final clears the link just as a live post would"
}
...
test_followup_usage_errors() {
  ...
  err="$home/err.txt"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" >/dev/null 2>"$err"; rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" --check >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup --check no-id exit"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" some-task >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup post no-text-source exit"
  out=$(PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" --help); rc=$?
  ...
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" "../evil" --text-file /dev/null >/dev/null 2>&1; rc=$?
  expect_code 2 "$rc" "followup unsafe-id exit"
  PATH="$BASE_PATH" XO_HOME="$home" "$ROOT/bin/xo-x-followup.sh" some-task --image >/dev/null 2>"$err"; rc=$?
  ...
  pass "xo-x-followup rejects malformed invocations"
}
