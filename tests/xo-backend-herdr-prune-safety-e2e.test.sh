#!/usr/bin/env bash
# tests/xo-backend-herdr-prune-safety-e2e.test.sh - isolated real-herdr
...
# "xo" - with a live long-running process in that pane, exactly as
...
# xo-<id> task tab), mirroring tests/xo-backend-herdr-smoke.test.sh's broader
...
SESSION="xo-lab-prune-safety-e2e-$$"
export HERDR_SESSION="$SESSION"
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/xo-herdr-prune-safety.XXXXXX")
cleanup_all() {
...
# Explicitly label the startup workspace "xo" to create the collision
...
xo_backend_herdr_server_ensure "$SESSION" || fail "could not start the isolated session's server"

CREATE_OUT=$(xo_backend_herdr_cli "$SESSION" workspace create --cwd "$LIVE_CWD" --label xo --no-focus) \
...
[ "$LIVE_LABEL" = xo ] || fail "the startup workspace label should be 'xo', got '$LIVE_LABEL' - repro setup is wrong"
pass "repro setup: a pre-existing workspace labeled 'xo' collides with the primary home's own label"
...
for _ in $(seq 1 100); do
  PROCESS_INFO=$(xo_backend_herdr_cli "$SESSION" pane process-info --pane "$LIVE_PANE_ID" 2>/dev/null || true)
...
xo_backend_herdr_cli "$SESSION" pane run "$LIVE_PANE_ID" \
...
RAW=$(xo_backend_herdr_container_ensure "$LIVE_CWD") || fail "container_ensure failed"
CONTAINER=${RAW%%$'\t'*}
SEEDED_TAB_ID=${RAW#*$'\t'}
...
TASK_IDS=$(xo_backend_herdr_create_task "$CONTAINER" xo-prunesafety-e2e "$LIVE_CWD" "$SEEDED_TAB_ID") \
...
xo_backend_herdr_kill "$SESSION:$NEW_PANE_ID"
xo_backend_herdr_kill "$SESSION:$LIVE_PANE_ID"

# --- 4. happy path still works: a genuinely fresh workspace gets its seeded -
# default tab pruned, leaving exactly one clean xo-<id> task tab -------------

HAPPY_CWD="$SCRATCH/happy-project"
mkdir -p "$HAPPY_CWD"
HAPPY_RAW=$(xo_backend_herdr_container_ensure "$HAPPY_CWD") || fail "happy-path container_ensure failed"
...
HAPPY_TASK_IDS=$(xo_backend_herdr_create_task "$HAPPY_CONTAINER" xo-prunesafety-happy "$HAPPY_CWD" "$HAPPY_SEEDED") \
...
pass "happy path: a genuinely fresh workspace's seeded default tab is still pruned, leaving exactly one clean xo-<id> task tab"

xo_backend_herdr_kill "$SESSION:$HAPPY_PANE"
