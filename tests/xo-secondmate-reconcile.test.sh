#!/usr/bin/env bash
# tests/xo-secondmate-reconcile.test.sh - the cooldown-limited reconcile ask.
#
...
RECONCILE="$ROOT/bin/xo-secondmate-reconcile.sh"
TMP_ROOT=$(xo_test_tmproot xo-secondmate-reconcile)
...
# fake tmux backend, so xo-send's real inbox plane is exercised end to end.
...
write_snapshot() {  # <path> <mate-id> <invalidity-json> [state]
  jq -n --arg id "$2" --argjson inv "$3" --arg state "${4:-captain_decision}" '{
    schema:"xo-fleet-snapshot.v1", generated:"2026-08-26T00:00:00Z",
...
# A fake ssh that decodes xo-on.sh's base64 remote-home/argv payload and
# EXECUTES the real host-local leg (xo-remote-secondmate-control.sh) against a
# genuinely seeded remote-home fixture, exactly like the ssh stub proven in
# tests/xo-send-remote-delivery.test.sh.
...
cmd=${rargs[0]}
rc=0
if [ "${XO_TEST_RECONCILE_REMOTE_DELAY:-0}" -gt 0 ]; then
  sleep "$XO_TEST_RECONCILE_REMOTE_DELAY"
fi
...
make_remote_parent_home() {  # <name> <mate-id> <remote-home> <host> -> echoes home dir
  ...
  mkdir -p "$home/data" "$home/state"
  # Canonicalize: xo-on.sh's registry route parser rejects an empty path
...
run_remote_notify() {  # <home> <fakebin> <snapshot>
  ...
  XO_SSH_BIN="$fakebin/fake-ssh" XO_REMOTE_CODE_ROOT="$ROOT" \
    PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$home/state" \
...
run_notify() {  # <home> <fakebin> <name> <snapshot> [extra args...]
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$home/state" \
    XO_FAKE_TMUX_WINDOW="xo:xo-mate" \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/$name-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/$name-fake/pane.txt" \
    "$RECONCILE" notify --snapshot "$snap" "$@"
}
...
hold_lock_until_released() {  # <lock> <ready> <release>
  bash -c '
    ...
    xo_lock_release "$2"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$1" "$2" "$3" &
}
...
test_the_window_is_four_hours() {
  ...
  cat > "$fakebin/date" <<'SH'
#!/usr/bin/env bash
if [ -n "${XO_TEST_DATE_NOW:-}" ] && [ "${1:-}" = +%s ]; then
  printf '%s\n' "$XO_TEST_DATE_NOW"
  exit 0
fi
exec /bin/date "$@"
SH
  ...
  out=$(XO_TEST_DATE_NOW=$now run_notify "$home" "$fakebin" fourhours "$snap")
...
test_each_home_carries_its_own_cooldown() {
  ...
  sed -i.bak 's/xo-mate/xo-other/' "$home/state/other.meta" && rm -f "$home/state/other.meta.bak"
  snap="$home/snapshot.json"
  jq -n '{schema:"xo-fleet-snapshot.v1", generated:"2026-08-26T00:00:00Z",
    secondmate_current:{records:[
...
test_the_ask_never_arms_a_reply_expectation_or_a_re_ring() {
  ...
  ladder=$(XO_TASK_INBOX_GRACE_SECS=0 bash -c '. "$1"; xo_task_inbox_due_action "$2" "$3"' _ \
    "$ROOT/bin/xo-task-inbox-lib.sh" "$home/state" mate 2>&1 || true)
  [ "$ladder" = quiet ] \
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$home/state" \
    XO_FAKE_TMUX_WINDOW="xo:xo-mate" \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/fireforget-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/fireforget-fake/pane.txt" \
    "$ROOT/bin/xo-send.sh" mate "an ordinary steer that does expect handling" >/dev/null 2>&1 \
...
set -u
if [ "${1:-}" = send-keys ]; then
  : > "$XO_FAKE_TMUX_SEND_SIGNAL"
  while [ ! -f "$XO_FAKE_TMUX_SEND_RELEASE" ]; do sleep 0.01; done
fi
exec "$(dirname "$0")/tmux-real" "$@"
SH
  chmod +x "$fakebin/tmux"

  XO_FAKE_TMUX_SEND_SIGNAL="$signal" XO_FAKE_TMUX_SEND_RELEASE="$release" \
  ...
  (
    ...
    xo_lock_acquire_wait "$home/state/.control-mate.lock"
    xo_lock_acquire_wait "$home/state/.meta-mate.lock"
...
worktree=$mate
META
    xo_lock_release "$home/state/.meta-mate.lock"
    xo_lock_release "$home/state/.control-mate.lock"
    : > "$lifecycle_done"
  ) &
...
# (bin/xo-spawn.sh's spawn_remote_secondmate() never writes one). This is the
...
test_route_replacement_during_send_is_refused() {
  ...
  snap="$home/snapshot.json"
  signal="$home/xo-send-started"
  release="$home/release-xo-send"
...
case "${1:-}" in
  */xo-send.sh)
    : > "$XO_RECONCILE_RACE_SIGNAL"
    while [ ! -f "$XO_RECONCILE_RACE_RELEASE" ]; do sleep 0.01; done
    ;;
esac
case "${1:-}" in
  */*) printf '%s\n' "${1%/*}" ;;
...
SH
  ...
  rc=0
  XO_RECONCILE_RACE_SIGNAL="$signal" XO_RECONCILE_RACE_RELEASE="$release" \
    run_remote_notify "$home" "$fakebin" "$snap" > "$home/notify.out" 2>&1 &
  # shellcheck disable=SC2031 # The background PID is captured immediately in this shell.
  notify_pid=$!
  while [ ! -f "$signal" ]; do
    kill -0 "$notify_pid" 2>/dev/null || fail "reconcile exited before entering xo-send"
    sleep 0.01
  done

  # shellcheck source=/dev/null
  . "$ROOT/bin/xo-wake-lib.sh"
  xo_lock_acquire_wait "$home/state/.control-remote-send-race-mate.lock"
  xo_lock_acquire_wait "$home/state/.meta-remote-send-race-mate.lock"
  ...
  xo_lock_release "$home/state/.meta-remote-send-race-mate.lock"
  xo_lock_release "$home/state/.control-remote-send-race-mate.lock"
  : > "$release"
  wait "$notify_pid" || rc=$?
  out=$(cat "$home/notify.out")
  ...
  pass "a route replacement between reconcile and xo-send cannot receive a stale ask"
}
...
test_reconcile_request_rejects_an_unbounded_input_without_filling_storage() {
  ...
  started=$(date +%s)
  if yes x | XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$home/state" \
      XO_RECONCILE_REQUEST_MAX_BYTES=64 "$RECONCILE" request --snapshot - \
...
test_reconcile_request_requires_one_snapshot_document() {
  ...
  if XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$home/state" \
...
META
  ...
  i=0
  while [ "$i" -lt 4 ]; do
    XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$home/state" \
  ...
  jq '{schema:"xo-bearings.v1",secondmate_reconcile:[.secondmate_current.records[] | {
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$home/state" \
      XO_FAKE_TMUX_WINDOW='' XO_FAKE_TMUX_LOG="$home/tmux.log" \
      XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/coalesced-requests-fake/pane.txt" \
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$home/state" \
    XO_FAKE_TMUX_WINDOW='xo:xo-coalesce-a' XO_FAKE_TMUX_LOG="$home/tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/coalesced-requests-fake/pane.txt" \
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$home/state" \
    XO_FAKE_TMUX_WINDOW='xo:xo-coalesce-b' XO_FAKE_TMUX_LOG="$home/tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/coalesced-requests-fake/pane.txt" \
...
test_bearings_request_returns_before_remote_delivery_and_supervision_sends_later() {
  ...
  jq -n --arg home "$rhome" '{
    schema:"xo-secondmate-home-summary.v1",hold_classifier_schema:"xo-captain-hold-buckets.v1",
  ...
  warm=$(XO_SSH_BIN="$fakebin/fake-ssh" XO_REMOTE_CODE_ROOT="$ROOT" \
    PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$home/state" XO_SNAPSHOT_BUDGET=3 XO_SNAPSHOT_NOW_EPOCH=2000 \
    XO_BEARINGS_NOW=2026-09-01T22:00:00Z "$ROOT/bin/xo-bearings-snapshot.sh" --json) \
  ...
  started=$(date +%s)
  snap=$(XO_TEST_RECONCILE_REMOTE_DELAY=30 \
    XO_SSH_BIN="$fakebin/fake-ssh" XO_REMOTE_CODE_ROOT="$ROOT" \
    PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$home/state" XO_SNAPSHOT_BUDGET=1 XO_SNAPSHOT_NOW_EPOCH=2000 \
    XO_BEARINGS_NOW=2026-09-01T22:00:00Z "$ROOT/bin/xo-bearings-snapshot.sh" --json) \
    || fail "Bearings failed while the remote queue was delayed"
  printf '%s\n' "$snap" | XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$home/state" \
  ...
  XO_TEST_RECONCILE_REMOTE_DELAY=4 \
    XO_SSH_BIN="$fakebin/fake-ssh" XO_REMOTE_CODE_ROOT="$ROOT" \
    PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$home/state" XO_POLL=1 XO_HOME_SUMMARY_INTERVAL=999999 \
    "$ROOT/bin/xo-watch.sh" > "$home/watch.out" 2> "$home/watch.err" &
  watcher=$!
  i=0
  processing=''
  while [ "$i" -lt 100 ]; do
