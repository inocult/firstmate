#!/usr/bin/env bash
# tests/xo-wake-drain-open-decisions.test.sh - behavior tests for the OPEN
# DECISIONS section bin/xo-wake-drain.sh prints on every drain (including the
# empty-queue fast path). The section is pure wiring around
# xo-classify-lib.sh's status_open_decisions fold (the ONE authoritative
...
TMP_ROOT=$(xo_test_tmproot xo-wake-drain-open-decisions-tests)
...
test_buried_decision_still_surfaces() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a buried decision"
...
test_explicit_resolution_closes_it() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed after an explicit resolution"
...
test_reserved_key_namespace_is_owned_by_its_library() {
  ...
  state="$dir/state"
  out="$dir/drain.out"
  # `pending-reply-<id>` names a decision bin/xo-pending-reply-lib.sh raises and
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on reserved-key lines"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed after the owner closed its decision"
  if grep -F 'OPEN DECISIONS' "$out" >/dev/null; then
...
test_later_unrelated_terminal_line_does_not_close_it() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed after an unrelated terminal line"
...
test_no_open_decisions_prints_nothing() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed with no open decisions"
...
test_open_decision_surfaces_even_with_an_unrelated_queued_wake() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed with a mixed fleet"
...
test_buried_decision_surfaces_on_the_empty_queue_fast_path() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "empty-queue drain failed"
...
test_status_symlink_is_not_followed() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed with a symlinked status file"
...
# The per-item cut now comes from bin/xo-line-cap-lib.sh, shared with the
...
test_over_long_decision_note_is_capped_with_a_marker() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on an over-long decision note"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a short decision note"
