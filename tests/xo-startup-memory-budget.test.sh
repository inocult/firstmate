...
TMP_ROOT=$(xo_test_tmproot xo-startup-memory-budget)
BUDGET="$ROOT/bin/xo-startup-memory-budget.sh"
BOOTSTRAP="$ROOT/bin/xo-bootstrap.sh"
CONFIG_PUSH="$ROOT/bin/xo-config-push.sh"
...
expect_rejected_read() {
  ...
  set +e
  out=$(XO_HOME="$home" "$BUDGET" read 2>&1)
  rc=$?
  set -e
...
test_safe_parser_rejects_ambiguous_and_unsafe_values() {
  ...
  printf '42\n' > "$home/config/startup-memory-budget"
  [ "$(XO_HOME="$home" "$BUDGET" read)" = 42 ] || fail "valid positive decimal budget was rejected"
...
test_budget_accounting_reports_all_three_files_and_safe_failure() {
  ...
  out=$(XO_HOME="$home" "$BUDGET" report)
  ...
  set +e
  out=$(XO_HOME="$home" "$BUDGET" report 2>&1)
  rc=$?
  set -e
...
run_config_push() {
  ...
  PATH="$fakebin:$BASE_PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$root" XO_SEND_SETTLE=0 \
    XO_FAKE_TMUX_LOG="$log" "$CONFIG_PUSH"
}
...
test_primary_budget_converges_with_exact_reread_and_safe_failures() {
  ...
  assert_contains "$(<"$log")" "XO instruction waiting: list " \
...
echo '# all xo-startup-memory-budget tests passed'
