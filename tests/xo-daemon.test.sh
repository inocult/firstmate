#!/usr/bin/env bash
# tests/xo-daemon.test.sh - supervise-daemon classifiers, the captain-relevant
...
# (persistent-Enter-swallow, max-defer wedge alarms, xo-send typed-plane swallow
# reporting, composer-pending ANSI parsing). The operator-visible inject flow
# lives in xo-afk-inject-e2e and xo-wake-daemon-lifecycle-e2e.
set -u

# shellcheck source=tests/wake-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/wake-helpers.sh"

DAEMON="$ROOT/bin/xo-supervise-daemon.sh"
AFK_START="$ROOT/bin/xo-afk-start.sh"
...
if [ -z "${XO_TEST_DAEMON_SOURCED:-}" ]; then
  export XO_TEST_DAEMON_SOURCED=1
  # shellcheck source=bin/xo-supervise-daemon.sh
  . "$DAEMON"
fi

TMP_ROOT=$(xo_test_tmproot xo-daemon-tests)
XO_DAEMON_PRIMARY_HARNESS=claude
export XO_DAEMON_PRIMARY_HARNESS
...
test_afk_start_refuses_when_flag_cannot_be_written() {
  ...
  out=$(XO_STATE_OVERRIDE="$state" XO_SUPERVISOR_BACKEND=unsupported "$AFK_START" 2>&1)
  status=$?

  [ "$status" -ne 0 ] || fail "xo-afk-start.sh should fail when state/.afk cannot be written"
  assert_not_contains "$out" "starting supervise daemon" "xo-afk-start.sh continued into daemon startup after .afk write failure"
  assert_absent "$state/.supervise-daemon.log" "xo-afk-start.sh started the daemon after .afk write failure"
  pass "xo-afk-start.sh fails before daemon startup when the afk flag cannot be written"
}
...
test_afk_start_ignores_stale_pidfile_without_lock() {
  ...
  [ "$status" -ne 0 ] || fail "xo-afk-start.sh should attempt daemon startup instead of trusting a pidfile-only live pid"
  assert_contains "$out" "starting supervise daemon" "xo-afk-start.sh did not attempt daemon startup"
  assert_contains "$out" "does not support supervisor backend 'unsupported'" "daemon startup did not reach backend validation"
  assert_not_contains "$out" "daemon already running" "xo-afk-start.sh trusted a stale pidfile-only live pid"
  pass "xo-afk-start.sh ignores stale pidfile-only live pids"
}
...
test_afk_start_reclaims_stale_daemon_lock_reused_pid() {
  ...
  [ "$status" -ne 0 ] || fail "xo-afk-start.sh should attempt daemon startup after rejecting a reused-pid lock"
  assert_contains "$out" "starting supervise daemon" "xo-afk-start.sh did not attempt daemon startup after rejecting the stale lock"
  ...
  assert_not_contains "$out" "daemon already running" "xo-afk-start.sh trusted a stale daemon lock with a reused pid"
  assert_not_contains "$out" "another xo-supervise-daemon is already running" "daemon singleton lock still trusted the reused pid"
  pass "xo-afk-start.sh reclaims stale daemon locks whose live pid identity no longer matches"
}

test_daemon_state_root_uses_xo_home() {
  local dir home override out
  dir=$(make_supercase daemon-xo-home)
  home="$dir/xo-home"
  override="$dir/override-state"
  mkdir -p "$home" "$override"

  out=$(XO_HOME="$home" XO_STATE_OVERRIDE='' _state_root)
  [ "$out" = "$home/state" ] || fail "daemon state root ignored XO_HOME: $out"

  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$override" _state_root)
  [ "$out" = "$override" ] || fail "daemon state root ignored XO_STATE_OVERRIDE: $out"

  pass "supervise daemon state root is scoped by XO_HOME"
}
...
seen_through() {  # <state> <task>
  ...
  ident=$(_xo_open_decisions_file_ident "$state/$task.status")
...
test_classify_signal_skips_turn_end_markers() {
  ...
  out=$(XO_STATUS_IDENTITY_READER="$reader" classify_signal "$turn" "$state")
...
test_classify_signal_survives_a_later_routine_append() {
  ...
  out=$(XO_STATE_OVERRIDE="$state" classify_signal "$state/mask-b1.status" "$state")
  case "$out" in
    escalate\|*) ;;
  ...
  out=$(XO_STATE_OVERRIDE="$state" classify_signal "$state/mask-r1.status" "$state")
  case "$out" in
    escalate\|*) ;;
...
test_classification_commits_its_captured_endpoint() {
  ...
  out=$(XO_STATUS_SPAN_ENDPOINT_FILE="$capture" \
...
test_stale_masked_event_escalates_at_captured_endpoint() {
  ...
  XO_ESCALATE_BATCH_SECS=999 handle_wake "stale: sess:xo-stale-r2" "$state"
...
test_stale_read_failure_surfaces_without_advancing_seen() {
  ...
  printf '3@%s' "$(_xo_open_decisions_file_ident "$state/stale-r3.status")" \
    > "$state/.subsuper-seen-status-$key"
  (
    # shellcheck disable=SC2329 # Invoked indirectly by the function under test.
    _xo_status_read_span() { return 1; }
    XO_ESCALATE_BATCH_SECS=999 handle_wake "stale: sess:xo-stale-r3" "$state"
  )
  out=$(cat "$state/.subsuper-escalations" 2>/dev/null || true)
...
EOF
  ...
  record=$(XO_STATUS_IDENTITY_READER="$reader" \
  ...
  XO_STATUS_IDENTITY_READER="$reader" mark_status_seen "$state" reused-r4 "$endpoint" "$ident"
  [ ! -e "$state/.subsuper-seen-status-reused-r4" ] \
    || fail "a stale captured identity advanced the replacement task marker"
  out=$(XO_STATUS_IDENTITY_READER="$reader" classify_signal "$state/reused-r4.status" "$state")
...
test_unverifiable_identity_surfaces_without_marker() {
  ...
  (
    XO_STATUS_IDENTITY_READER="$reader" XO_ESCALATE_BATCH_SECS=999 \
...
test_status_read_failure_surfaces_without_advancing_seen() {
  ...
  printf '3@%s' "$(_xo_open_decisions_file_ident "$state/read-r1.status")" \
    > "$state/.subsuper-seen-status-$key"
  (
    # shellcheck disable=SC2329 # Invoked indirectly by the function under test.
    ...
    # shellcheck disable=SC2329 # Invoked indirectly by the function under test.
    _xo_status_read_span() { return 1; }
    XO_ESCALATE_BATCH_SECS=999 handle_wake "signal: $state/read-r1.status" "$state"
  )
  out=$(cat "$state/.subsuper-escalations" 2>/dev/null || true)
...
test_catchall_advances_routine_then_surfaces_append() {
  ...
  XO_STATE_OVERRIDE="$state" housekeeping "$state"
...
test_escalation_buffer_failure_retains_wake_and_position() {
  ...
  cat > "$fakebin/xo-wake-drain.sh" <<EOF
#!/usr/bin/env bash
...
EOF
  chmod +x "$fakebin/xo-wake-drain.sh"

  ! XO_DAEMON_DIR="$fakebin" handle_durable_wakes fallback "$state" 2>/dev/null \
  ...
  rmdir "$buffer"
  XO_DAEMON_DIR="$fakebin" handle_durable_wakes fallback "$state" \
...
test_catchall_buffer_failure_preserves_position() {
  ...
  mkdir "$buffer"
  rm -f "$state/.subsuper-last-scan"
  XO_STATE_OVERRIDE="$state" housekeeping "$state" 2>/dev/null || true
...
#!/usr/bin/env bash
if [ "\${1:-}" = --ack-through ]; then printf '%s\n' ack >> "$dir/acked"; exit 0; fi
printf '1\t1\tstale\tmissing-r8\tstale: sess:xo-missing-r8\n'
...
test_permission_recovery_reclassifies_catchall_status() {
  ...
  before_ident=$(_xo_open_decisions_file_ident "$status")
  chmod 000 "$status"
  if [ -r "$status" ]; then
    chmod 600 "$status"
  ...
  chmod 600 "$status"
  after_ident=$(_xo_open_decisions_file_ident "$status")
...
test_classify_routine_signal_self() {
  ...
  out=$(XO_STATE_OVERRIDE="$state" classify_signal "$state/foo-x1.status" "$state")
...
test_classify_terminal_signal_escalates() {
  ...
  for kw in "done: PR https://x/y/pull/1" "needs-decision: pick A" "blocked: no perms" \
            ...
            "ready in branch xo/t1" "merged"; do
    printf 'working\n%s\n' "$kw" > "$state/t.status"
    out=$(XO_STATE_OVERRIDE="$state" classify_signal "$state/t.status" "$state")
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:xo-qux-w4" "$state"
  out=$(XO_STATE_OVERRIDE="$state" classify_stale "sess:xo-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:xo-qux-w4")" | tr ':/.' '___')
...
test_stale_diagnostic_wedge_survives_busy_housekeeping() {
  ...
  for case_name in working prior-terminal paused; do
    ...
    xo_write_meta "$state/$task.meta" "window=$win" "backend=tmux"
    case "$case_name" in
    ...
    (
      kill() { printf 'kill %s\n' "$*" >> "$action_log"; }
      xo_backend_send_text_submit() { printf 'interrupt %s\n' "$*" >> "$action_log"; }
      LOG="$dir/daemon.log" XO_STATE_OVERRIDE="$state" handle_wake "$reason" "$state"
      PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$pane" \
        XO_STATE_OVERRIDE="$state" XO_ESCALATE_BATCH_SECS=999999 housekeeping "$state"
    )
    case "$case_name" in
      paused)
...
test_enriched_wedge_under_declared_wait_uses_pause_cadence() {
  ...
  task=paused-wedge-w1; win="sess:xo-$task"; pane="$dir/pane.txt"
  key=$(printf '%s' "$task" | tr ':/.' '___')
  xo_write_meta "$state/$task.meta" "window=$win" "backend=tmux"
  printf 'working: dispatching the long audit\npaused: the audit engine is running to completion\n' \
    > "$state/$task.status"
  printf 'idle prompt $\n' > "$pane"
  case "$(XO_STATE_OVERRIDE="$state" classify_stale "$win" "$state")" in
    pause\|*) ;;
  ...
  for i in 2 3 4 5; do
    if [ "$i" -ge 3 ]; then
      # Past XO_WEDGE_DEMAND_INSPECT_COUNT the watcher enriches the same reason with
    ...
    LOG="$dir/daemon.log" XO_STATE_OVERRIDE="$state" handle_wake "$reason" "$state"
    PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$pane" \
      XO_STATE_OVERRIDE="$state" XO_ESCALATE_BATCH_SECS=999999 \
      XO_STALE_ESCALATE_SECS=240 XO_PAUSE_RESURFACE_SECS=3600 housekeeping "$state"
  done
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$pane" \
    XO_STATE_OVERRIDE="$state" XO_ESCALATE_BATCH_SECS=999999 XO_PAUSE_RESURFACE_SECS=3600 \
    housekeeping "$state"
  escalations=0
  [ -s "$state/.subsuper-escalations" ] \
...
test_stale_terminal_escalates() {
  ...
  state="$dir/state"
  printf 'done: ready in branch xo/t1\n' > "$state/fin-t5.status"
  out=$(XO_STATE_OVERRIDE="$state" classify_stale "sess:xo-fin-t5" "$state")
  case "$out" in escalate\|*) ;; *) fail "terminal stale did not escalate: $out" ;; esac
  xo_write_meta "$state/herdr-t5.meta" "window=default:w1:p2" "backend=herdr"
  printf 'done: ready in branch xo/herdr\n' > "$state/herdr-t5.status"
  out=$(XO_STATE_OVERRIDE="$state" classify_stale "default:w1:p2" "$state")
...
test_stale_actionable_wait_escalates_and_keeps_pause_cadence() {
  ...
  win="sess:xo-waiting-r10"; key=$(printf '%s' waiting-r10 | tr ':/.' '___')
  printf 'blocked [key=release]: need captain approval\npaused: waiting for release access\n' \
    > "$state/waiting-r10.status"

  out=$(XO_STATE_OVERRIDE="$state" classify_stale "$win" "$state")
  ...
  XO_ESCALATE_BATCH_SECS=999 handle_wake "$reason" "$state"
  out=$(cat "$state/.subsuper-escalations" 2>/dev/null || true)
  ...
  resumed_win="sess:xo-resumed-r10"; resumed_key=$(printf '%s' resumed-r10 | tr ':/.' '___')
  ...
  XO_ESCALATE_BATCH_SECS=999 handle_wake "stale: $resumed_win" "$state"
...
test_stale_paused_classifies_pause() {
  ...
  out=$(XO_STATE_OVERRIDE="$state" classify_stale "sess:xo-held-w9" "$state")
...
test_stale_captain_held_classifies_pause() {
  ...
  out=$(XO_STATE_OVERRIDE="$state" classify_stale "sess:xo-held-w9h" "$state")
...
test_handle_wake_paused_records_pause_marker() {
  ...
  XO_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
...
test_handle_wake_paused_signal_records_pause_marker() {
  ...
  state="$dir/state"
  win="sess:xo-held-w10-signal"
  printf 'window=%s\nkind=ship\n' "$win" > "$state/held-w10-signal.meta"
  printf 'paused: awaiting the vendor rate-limit reset\n' > "$state/held-w10-signal.status"
  key=$(printf '%s' "held-w10-signal" | tr ':/.' '___')
  date +%s > "$state/.subsuper-stale-$key"
  XO_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-signal.status" "$state"
...
test_handle_wake_terminal_signal_clears_pause_tracking() {
  ...
  XO_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-terminal.status" "$state"
...
test_housekeeping_migrates_watcher_unpaused_marker_to_clear() {
  ...
  state="$dir/state"
  win="sess:xo-held-w10-migrate-unpaused"
...
test_housekeeping_paused_resurfaces_and_resets() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:xo-held-w11"; pane="$dir/pane.txt"
  printf 'paused: holding for the upstream tool release\n' > "$state/held-w11.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "held-w11" | tr ':/.' '___')
  echo $(( $(date +%s) - 5000 )) > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$pane" \
    XO_STATE_OVERRIDE="$state" XO_PAUSE_RESURFACE_SECS=240 housekeeping "$state"
...
# xo-classify-lib.sh's cadence comment is explicit that a forgotten hold cannot rot
...
test_housekeeping_captain_held_resurfaces_and_resets() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:xo-held-w11h"; pane="$dir/pane.txt"
...
test_housekeeping_paused_resumed_cleared() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:xo-held-w12"; pane="$dir/pane.txt"
  printf 'paused: holding for the upstream tool release\nworking: upstream landed, resuming\n' \
    > "$state/held-w12.status"
  printf 'Working...\n' > "$pane"
  xo_write_meta "$state/held-w12.meta" "window=$win" "worktree=$dir/wt" "kind=ship" "harness=pi"
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$state" held-w12)
  "$ROOT/bin/xo-busy-event.sh" apply "$state" held-w12 busy --gen "$gen" \
    --source pi-ext --event agent-start
  key=$(printf '%s' "held-w12" | tr ':/.' '___')
  echo $(( $(date +%s) - 5000 )) > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$pane" \
    XO_STATE_OVERRIDE="$state" stale_window_is_busy "$win" "$state" \
...
# declared wait to the daemon exactly once per declaration (bin/xo-watch.sh's
...
test_housekeeping_busy_declared_wait_matures_its_window() {
  ...
  for case_name in paused captain-held; do
    ...
    state="$dir/state"; fakebin="$dir/fakebin"
    task="held-w12b-$case_name"; win="sess:xo-$task"; pane="$dir/pane.txt"
    case "$case_name" in
    ...
    printf 'Working...\n' > "$pane"
    xo_write_meta "$state/$task.meta" "window=$win" "worktree=$dir/wt" "kind=ship" "harness=pi"
    gen=$("$ROOT/bin/xo-busy-event.sh" arm "$state" "$task")
    "$ROOT/bin/xo-busy-event.sh" apply "$state" "$task" busy --gen "$gen" \
      --source pi-ext --event agent-start
    key=$(printf '%s' "$task" | tr ':/.' '___')
    PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$pane" \
      XO_STATE_OVERRIDE="$state" stale_window_is_busy "$win" "$state" \
...
test_housekeeping_declared_time_controls_pause_recheck() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  task='held-until'; win="sess:xo-$task"; pane="$dir/pane.txt"
  printf 'idle prompt $\n' > "$pane"
  xo_write_meta "$state/$task.meta" "window=$win" "worktree=$dir/wt" "kind=ship" "harness=pi"
  key=$(printf '%s' "$task" | tr ':/.' '___')
  now=$(date +%s)
...
test_housekeeping_paused_unpaused_cleared() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:xo-held-w13"; pane="$dir/pane.txt"
...
test_housekeeping_captain_held_resolved_cleared() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:xo-held-w13h"; pane="$dir/pane.txt"
...
test_housekeeping_stale_marker_transitions_to_pause() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:xo-held-w14"; pane="$dir/pane.txt"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$pane" \
    XO_STATE_OVERRIDE="$state" XO_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_captain_held_stale_marker_transitions_to_pause() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:xo-held-w14h"; pane="$dir/pane.txt"
...
test_housekeeping_pause_marker_transitions_to_clear() {
  ...
  state="$dir/state"; fakebin="$dir/fakebin"; win="sess:xo-held-w15"; pane="$dir/pane.txt"
  printf 'working: upstream landed, resuming\n' > "$state/held-w15.status"
  printf 'idle prompt $\n' > "$pane"
  key=$(printf '%s' "held-w15" | tr ':/.' '___')
  date +%s > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$pane" \
    XO_STATE_OVERRIDE="$state" XO_PAUSE_RESURFACE_SECS=999999 housekeeping "$state"
...
test_housekeeping_resumed_stale_cleared() {
  ...
  # record (bin/xo-busy-lib.sh), not through the pane's rendered footer.
  xo_write_meta "$state/res-w6.meta" "window=$win" "worktree=$dir/wt" "kind=ship" "harness=pi"
  local gen; gen=$("$ROOT/bin/xo-busy-event.sh" arm "$state" res-w6)
  "$ROOT/bin/xo-busy-event.sh" apply "$state" res-w6 busy --gen "$gen" \
    --source pi-ext --event agent-start
...
test_housekeeping_herdr_persistent_stale_resolves_meta() {
  ...
  state="$dir/state"
  xo_write_meta "$state/herdr-w7.meta" "window=default:w1:p2" "backend=herdr"
  ...
  (
    ...
    xo_backend_busy_state() {
      [ "$1" = herdr ] || fail "expected herdr busy backend, got $1"
      [ "$2" = "default:w1:p2" ] || fail "expected herdr busy target, got $2"
      printf 'idle'
    }
    xo_backend_capture herdr default:w1:p2 40 >/dev/null
    [ "$(xo_backend_busy_state herdr default:w1:p2)" = idle ] || fail "herdr busy stub did not report idle"
    XO_STATE_OVERRIDE="$state" XO_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_herdr_idle_busy_record_clears_stale() {
  ...
  state="$dir/state"
  xo_write_meta "$state/herdr-footer.meta" "window=default:w1:p4" "backend=herdr" "harness=claude"
  printf 'working\n' > "$state/herdr-footer.status"
  gen=$("$ROOT/bin/xo-busy-event.sh" arm "$state" herdr-footer)
  "$ROOT/bin/xo-busy-event.sh" apply "$state" herdr-footer busy --gen "$gen" \
  ...
  (
    ...
    xo_backend_capture herdr default:w1:p4 40 >/dev/null
    [ "$(xo_backend_busy_state herdr default:w1:p4)" = idle ] || fail "herdr busy stub did not report idle"
    XO_STATE_OVERRIDE="$state" XO_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_herdr_resumed_stale_cleared() {
  ...
  state="$dir/state"
  xo_write_meta "$state/herdr-busy.meta" "window=default:w1:p3" "backend=herdr"
  ...
  (
    ...
    xo_backend_capture herdr default:w1:p3 40 >/dev/null
    [ "$(xo_backend_busy_state herdr default:w1:p3)" = busy ] || fail "herdr busy stub did not report busy"
    XO_STATE_OVERRIDE="$state" XO_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_orca_persistent_stale_resolves_terminal() {
  ...
  state="$dir/state"
  xo_write_meta "$state/orca-w8.meta" "window=xo-orca-w8" "terminal=term-orca-w8" "backend=orca"
  ...
  (
    ...
    xo_backend_capture orca term-orca-w8 40 >/dev/null
    [ "$(xo_backend_busy_state orca term-orca-w8)" = idle ] || fail "Orca busy stub did not report idle"
    XO_STATE_OVERRIDE="$state" XO_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_escalate_batches_into_one_digest() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_PANE_ALIVE=1 XO_FAKE_TMUX_SENT="$sent" \
    XO_FAKE_TMUX_CAPTURE="$capture" XO_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
    || fail "escalate_flush failed"
  grep -F 'XO_OP: v1 away-supervisor: ' "$sent" >/dev/null \
...
test_escalate_batch_age_uses_first_append() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_PANE_ALIVE=1 XO_FAKE_TMUX_SENT="$sent" \
    XO_FAKE_TMUX_CAPTURE="$capture" XO_ESCALATE_BATCH_SECS=90 XO_HOUSEKEEPING_TICK=0 \
    housekeeping "$state"
  grep -F 'event A: done: PR 1 | event B: done: PR 2' "$sent" >/dev/null \
...
test_handle_wake_routes_self_and_escalate() {
  ...
  XO_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "routine signal was escalated by handle_wake"
  printf 'done: PR 1\n' > "$state/h-done.status"
  XO_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-done.status" "$state"
...
EOF
  ...
  XO_DAEMON_DIR="$fakebin" XO_STATE_OVERRIDE="$state" XO_ESCALATE_BATCH_SECS=999 \
    handle_durable_wakes fallback "$state" \
  ...
  XO_STATE_OVERRIDE="$state" handle_wake "signal: $state/ordinary-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] \
  ...
  XO_STATE_OVERRIDE="$state" handle_wake "signal: $state/ordinary-done.status" "$state"
  out=$(cat "$state/.subsuper-escalations" 2>/dev/null || true)
  case "$out" in
...
# captain-held transfer. xo-captain-hold.sh complete writes that line through the
...
test_inject_skip_forces_self() {
  ...
  state="$dir/state"
  printf 'done: PR 1\n' > "$state/s1.status"
  XO_STATE_OVERRIDE="$state" XO_INJECT_SKIP="signal" handle_wake "signal: $state/s1.status" "$state"
...
test_is_wake_reason_distinguishes_status_stdout() {
  ...
  is_wake_reason "stale: s:xo-x" || fail "stale: not recognized as wake"
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  XO_STATE_OVERRIDE="$state" handle_wake "signal: $state/sig-t8.status" "$state"
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" XO_FAKE_TMUX_PANE_ALIVE=1 XO_FAKE_TMUX_SENT="$sent" \
    XO_FAKE_TMUX_CAPTURE="$capture" XO_ESCALATE_BATCH_SECS=0 escalate_flush "$state"; then
...
test_marker_detection() {
  local marker_hex
  marker_hex=$(printf '%s' "$XO_INJECT_MARK" | od -An -tx1 | tr -d ' \n')
  [ "$marker_hex" = e281a3 ] \
    || fail "XO_INJECT_MARK must use terminal-safe U+2063 bytes, got $marker_hex"
  [ "$XO_OPERATIONAL_PREFIX" = "${XO_INJECT_MARK}XO_OP: " ] \
  ...
  message_is_injection "${XO_OPERATIONAL_PREFIX}Supervisor escalate: done" \
    || fail "operationally-prefixed message not detected as injection"
  message_is_injection "${XO_INJECT_MARK}Supervisor escalate: done" \
  ...
  state="$dir/state"
  afk_enter "$state"
  should_exit_afk "$state" "${XO_INJECT_MARK}escalate" \
...
test_should_exit_afk_when_afk_inactive() {
  ...
  should_exit_afk "$state" "${XO_INJECT_MARK}test" \
...
test_strip_injection_marker() {
  ...
  xo_operational_input_encode away-supervisor "Supervisor escalate: done" encoded \
  ...
  stripped=$(strip_injection_marker "${XO_OPERATIONAL_PREFIX}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
    || fail "landed untyped operational prefix not stripped: '$stripped'"
  stripped=$(strip_injection_marker "${XO_INJECT_MARK}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
  ...
  # Only marker → empty.
  stripped=$(strip_injection_marker "$XO_INJECT_MARK")
...
test_pane_input_pending_detects_partial_input() {
  ...
  printf 'line one\nline two\nhuman draft text\n' > "$capture"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURSOR_Y=2 \
...
test_pane_input_pending_requires_proven_empty_prompt() {
  ...
  for prompt in '❯' '›'; do
    printf 'output\noutput\n%s \n' "$prompt" > "$capture"
    if PATH="$fakebin:$PATH" XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURSOR_Y=2 \
...
# The safety fix at the tmux classifier (task xo-composer-shellglyph-safety): a
...
test_tmux_composer_state_bare_shell_is_unknown() {
  ...
  for g in '$' '%' '#' '>'; do
    printf 'output\noutput\n%s \n' "$g" > "$capture"
    out=$(PATH="$fakebin:$PATH" XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURSOR_Y=2 \
      xo_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
      || fail "bare shell prompt '$g' must classify unknown (dead shell, unsafe), got '$out'"
  done
  pass "xo_tmux_composer_state: a bare shell prompt (\$/%/#/>) reads unknown, never empty (dead-shell injection safety)"
}
...
test_tmux_composer_state_bordered_and_agent_rows_are_empty() {
  ...
  fakebin="$dir/fakebin"; capture="$dir/pane.txt"
  printf '╭────────────────────────╮\n│ >                      │\n╰────────────────────────╯\n' > "$capture"
  out=$(PATH="$fakebin:$PATH" XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURSOR_Y=1 \
    xo_tmux_composer_state "fakepane")
  [ "$out" = empty ] || fail "a bordered '│ > │' composer should read empty, got '$out'"
  printf '%s\n' "❯ " > "$capture"
  out=$(PATH="$fakebin:$PATH" XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURSOR_Y=0 \
    xo_tmux_composer_state "fakepane")
  ...
  pass "xo_tmux_composer_state: a bordered composer box and bare agent glyphs (❯/›) still read empty"
}
...
test_tmux_composer_state_requires_matching_box_borders() {
  ...
  pass "xo_tmux_composer_state: only matching edge borders form a composer box"
}
...
test_pane_input_pending_preserves_bright_placeholder_like_draft() {
  ...
  capture="$dir/pane.txt"
  printf '╭────────────────╮\n│ custom idle>   │\n╰────────────────╯\n' > "$capture"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURSOR_Y=1 \
    XO_COMPOSER_IDLE_RE='^custom idle>$' pane_input_pending "fakepane" \
...
test_classify_signal_dedup_against_scan() {
  ...
  out=$(XO_STATE_OVERRIDE="$state" classify_signal "$state/dup-s9.status" "$state")
...
test_classify_stale_dedup_against_signal() {
  ...
  out=$(XO_STATE_OVERRIDE="$state" classify_stale "sess:xo-dup-s10" "$state")
...
test_afk_genuine_done_still_terminal_stale() {
  ...
  state="$dir/state"
  printf 'done: PR https://example.com/pull/76 checks green; stage 1 of 4 ready for xo merge\n' \
    > "$state/stage1-w2.status"
  out=$(XO_STATE_OVERRIDE="$state" classify_stale "sess:xo-stage1-w2" "$state")
...
test_pane_input_pending_bordered_idle_not_pending() {
  ...
  for line in '>' '❯' ''; do
    ...
    if PATH="$fakebin:$PATH" XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURSOR_Y=1 \
      pane_input_pending "fakepane"; then
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$dir/composer" XO_FAKE_SENT="$sent" \
    xo_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, xo-send exits non-zero —
  ...
  touch "$dir/.swallow"
  verdict=$(PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$dir/composer" XO_FAKE_SENT="$sent" \
    XO_FAKE_SWALLOW="$dir/.swallow" XO_FAKE_PERSIST_SWALLOW=1 \
    xo_tmux_submit_core "win" "the digest" 3 0.05 0.05)
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$dir/composer" XO_FAKE_SENT="$sent" \
    XO_FAKE_SWALLOW="$dir/.swallow" XO_FAKE_PERSIST_SWALLOW=1 XO_INJECT_CONFIRM_SLEEP=0.05 \
    XO_ESCALATE_BATCH_SECS=99999 XO_MAX_DEFER_SECS=60 housekeeping "$state"
...
test_max_defer_flushes_empty_idle_pane() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$dir/composer" XO_FAKE_SENT="$sent" \
    XO_ESCALATE_BATCH_SECS=99999 XO_MAX_DEFER_SECS=60 XO_INJECT_CONFIRM_SLEEP=0.05 \
    housekeeping "$state"
...
test_normal_flush_clears_stale_wedge_marker() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" XO_FAKE_COMPOSER="$dir/composer" XO_FAKE_SENT="$sent" \
    XO_INJECT_CONFIRM_SLEEP=0.05 escalate_flush "$state" \
...
test_below_max_defer_does_nothing() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_PANE_ALIVE=1 XO_FAKE_TMUX_SENT="$sent" \
    XO_FAKE_TMUX_CAPTURE="$capture" XO_FAKE_TMUX_CURSOR_Y=0 \
    XO_ESCALATE_BATCH_SECS=99999 XO_MAX_DEFER_SECS=300 housekeeping "$state"
...
# the XO_WEDGE_ALARM_EXEC seam, which tests/wake-helpers.sh forces to a recorder
# ($XO_WEDGE_ALARM_LOG logs "<channel>\t<summary>"); the daemon also defaults
...
#!/usr/bin/env bash
printf '%s\n' "${XO_FAKE_UNAME:-Darwin}"
SH
  # Fakes keep command discovery deterministic on any CI host.
  cat > "$fakebin/osascript" <<'SH'
#!/usr/bin/env bash
printf '%s\n' osascript >> "${XO_WEDGE_ALARM_REAL_LOG:-/dev/null}"
exit 0
SH
  cat > "$fakebin/herdr" <<'SH'
#!/usr/bin/env bash
printf '%s\n' herdr >> "${XO_WEDGE_ALARM_REAL_LOG:-/dev/null}"
exit 0
SH
...
test_wedge_alarm_library_mode_defaults_to_discard() {
  ...
  # XO_WEDGE_ALARM_EXEC to "discard", so a sourced context (every test) cannot
  # fire a real notification even if it forgets to stub. Checked in a clean
  # subshell that first unsets this harness's recorder.
  local out
  # shellcheck disable=SC2016  # $1/$XO_WEDGE_ALARM_EXEC must expand in the child, not here
  out=$(env -u XO_WEDGE_ALARM_EXEC bash -c '. "$1"; printf "%s" "${XO_WEDGE_ALARM_EXEC:-UNSET}"' _ "$DAEMON")
  [ "$out" = discard ] \
    || fail "sourcing the daemon did not default the notifier seam to discard (got: $out)"
  pass "library mode: sourcing the daemon defaults XO_WEDGE_ALARM_EXEC to discard (no test can fire a real notification)"
}
...
test_wake_helpers_replace_inherited_notifier_override() {
  ...
  cat > "$unsafe" <<'SH'
#!/usr/bin/env bash
printf '%s\n' invoked >> "${XO_WEDGE_ALARM_UNSAFE_LOG:?}"
SH
  chmod +x "$unsafe"
  XO_WEDGE_ALARM_EXEC="$unsafe" XO_WEDGE_ALARM_UNSAFE_LOG="$unsafe_log" \
    XO_WEDGE_ALARM_LOG="$alert_log" XO_WEDGE_ALARM_CHANNEL=osascript \
...
test_wedge_alarm_discard_seam_fires_nothing() {
  ...
  PATH="$dir/fakebin:$PATH" XO_WEDGE_ALARM_LOG="$log" XO_WEDGE_ALARM_EXEC=discard \
    XO_WEDGE_ALARM_CHANNEL=$'osascript\nherdr\n'"$channel" \
...
test_wedge_alarm_direct_notifiers_honor_discard_seam() {
  ...
  PATH="$dir/fakebin:$PATH" XO_WEDGE_ALARM_REAL_LOG="$real_log" XO_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_osascript "away-mode WEDGED 900s"
  PATH="$dir/fakebin:$PATH" XO_WEDGE_ALARM_REAL_LOG="$real_log" XO_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_herdr "away-mode WEDGED 900s"
  XO_WEDGE_ALARM_EXEC=discard wedge_alarm_via_command "$command" "away-mode WEDGED 900s"
...
test_wedge_alarm_osascript_channel_selected() {
  ...
  XO_WEDGE_ALARM_LOG="$log" XO_WEDGE_ALARM_CHANNEL=osascript \
...
test_wedge_alarm_herdr_channel_selected() {
  ...
  XO_WEDGE_ALARM_LOG="$log" XO_WEDGE_ALARM_CHANNEL=herdr \
...
test_wedge_alarm_command_channel_receives_summary() {
  ...
  XO_WEDGE_ALARM_EXEC='' XO_WEDGE_ALARM_CHANNEL="$chan" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_command_failure_hides_configured_command() {
  ...
  LOG="$daemon_log" XO_WEDGE_ALARM_EXEC='' XO_WEDGE_ALARM_CHANNEL="command:exit 73 # $secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_unknown_channel_hides_configured_directive() {
  ...
  secret="https://alerts.example.invalid/hook?token=private-wedge-token"
  LOG="$daemon_log" XO_WEDGE_ALARM_CHANNEL="webhook:$secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_off_disables_active_alert_regardless_of_position() {
  ...
  for directives in $'osascript\noff' $'off\nosascript'; do
    : > "$log"
    XO_WEDGE_ALARM_LOG="$log" XO_WEDGE_ALARM_CHANNEL="$directives" \
      wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_darwin_selects_osascript() {
  ...
  PATH="$dir/fakebin:$PATH" XO_WEDGE_ALARM_LOG="$log" XO_FAKE_UNAME=Darwin XO_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_non_darwin_has_no_os_channel() {
  ...
  PATH="$dir/fakebin:$PATH" XO_WEDGE_ALARM_LOG="$log" XO_FAKE_UNAME=Linux XO_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_config_file_multi_channel() {
  ...
  XO_WEDGE_ALARM_LOG="$log" XO_CONFIG_OVERRIDE="$cfgdir" \
...
test_wedge_alarm_failing_channel_degrades_gracefully() {
  ...
  XO_WEDGE_ALARM_LOG="$log" XO_WEDGE_ALARM_FAIL=osascript \
    XO_WEDGE_ALARM_CHANNEL=$'osascript\nherdr' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_hung_channel_times_out_and_falls_through() {
  ...
  start=$SECONDS
  LOG="$daemon_log" XO_WEDGE_ALARM_EXEC='' XO_WEDGE_ALARM_TIMEOUT_SECS=1 \
    XO_WEDGE_ALARM_CHANNEL=$'command:sleep 30\n'"$channel" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
SH
  chmod +x "$blocker"
  start=$SECONDS
  LOG="$daemon_log" XO_WEDGE_ALARM_EXEC="$blocker" XO_WEDGE_ALARM_TIMEOUT_SECS=1 \
    XO_WEDGE_ALARM_CHANNEL=$'osascript\nherdr' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  elapsed=$((SECONDS - start))
...
test_inject_wedge_alarm_fires_active_alert_on_non_tmux_backend() {
  ...
  XO_WEDGE_ALARM_LOG="$log" XO_STATE_OVERRIDE="$state" \
    XO_WEDGE_ALARM_CHANNEL=osascript XO_SUPERVISOR_BACKEND=herdr \
...
test_inject_wedge_alarm_throttles_when_marker_cannot_be_written() {
  ...
  chmod u-w "$state"
  WEDGE_ALARM_LAST_EPOCH=0
  LOG="$daemon_log" XO_WEDGE_ALARM_LOG="$log" XO_MAX_DEFER_SECS=600 \
    XO_WEDGE_ALARM_CHANNEL=osascript XO_SUPERVISOR_BACKEND=herdr \
    inject_wedge_alarm "$state" 30600
  LOG="$daemon_log" XO_WEDGE_ALARM_LOG="$log" XO_MAX_DEFER_SECS=600 \
    XO_WEDGE_ALARM_CHANNEL=osascript XO_SUPERVISOR_BACKEND=herdr \
...
test_xo_send_reports_delivered_unconfirmed_submit() {
  # When typed-plane text was typed and Enter sent but the submit read-back
  # remains pending, xo-send must return its documented delivered-unconfirmed status and prevent
  ...
  # Clean submit -> exit 0.
  PATH="$fakebin:$PATH" XO_HOME="$dir" XO_STATE_OVERRIDE="$dir/state" XO_FAKE_COMPOSER="$dir/composer" \
    XO_SEND_SLEEP=0.05 "$ROOT/bin/xo-send.sh" sess:win 'route this work' >/dev/null 2>"$err" \
    || fail "xo-send exited non-zero on a clean submit: $(cat "$err")"
  ...
  printf '╭─────╮\n│ >   │\n╰─────╯\n' > "$dir/composer"
  touch "$dir/.swallow"
  if PATH="$fakebin:$PATH" XO_HOME="$dir" XO_STATE_OVERRIDE="$dir/state" XO_FAKE_COMPOSER="$dir/composer" \
    XO_FAKE_SWALLOW="$dir/.swallow" XO_FAKE_PERSIST_SWALLOW=1 XO_SEND_SLEEP=0.05 \
    "$ROOT/bin/xo-send.sh" sess:win 'fix findings 1 and 3, skip 2' >/dev/null 2>"$err"; then
    rc=0
  else
    rc=$?
  fi
  [ "$rc" -eq 3 ] || fail "xo-send returned $rc instead of delivered-unconfirmed exit 3: $(cat "$err")"
  grep -F 'submission is unconfirmed' "$err" >/dev/null \
    || fail "xo-send did not explain the pending confirmation: $(cat "$err")"
  grep -F 'do not retype or blindly resend' "$err" >/dev/null \
    || fail "xo-send did not prevent a duplicate resend: $(cat "$err")"
  if grep -F 'error:' "$err" >/dev/null; then
    fail "xo-send mislabeled delivered-unconfirmed as an error: $(cat "$err")"
  fi
  pass "xo-send returns 3 with a non-error no-resend warning when confirmation stays pending"
}

test_xo_send_exits_nonzero_on_initial_send_failure() {
  local dir fakebin err
  dir=$(make_bordered_case send-type-failure)
  fakebin="$dir/fakebin"; err="$dir/send.err"
  if PATH="$fakebin:$PATH" XO_HOME="$dir" XO_STATE_OVERRIDE="$dir/state" XO_FAKE_COMPOSER="$dir/composer" \
    XO_FAKE_SEND_FAIL=1 XO_SEND_SLEEP=0.05 \
    "$ROOT/bin/xo-send.sh" sess:win 'route this work' >/dev/null 2>"$err"; then
    fail "xo-send exited zero despite initial tmux send-keys failure"
  fi
  grep -F 'text not sent' "$err" >/dev/null || fail "xo-send did not explain initial send failure: $(cat "$err")"
  pass "xo-send exits non-zero when initial text send fails"
}

test_xo_send_exits_nonzero_on_unproven_submit() {
  local dir fakebin err
  ...
  if PATH="$fakebin:$PATH" XO_HOME="$dir" XO_STATE_OVERRIDE="$dir/state" XO_FAKE_COMPOSER="$dir/composer" \
    XO_FAKE_SWALLOW="$dir/.swallow" XO_FAKE_PERSIST_SWALLOW=1 XO_SEND_SLEEP=0.05 \
    "$ROOT/bin/xo-send.sh" sess:win '修复' >/dev/null 2>"$err"; then
    fail "xo-send exited zero when submit proof remained pending-unproven"
  fi
  grep -F 'verdict=pending-unproven' "$err" >/dev/null \
    || fail "xo-send did not preserve the unproven-submit verdict: $(cat "$err")"
  pass "xo-send exits non-zero unless delivery is proven empty"
}

# --- herdr backend-awareness (xo-turnend-guard-h6-adjacent transport fix) ----
...
test_discover_supervisor_backend_precedence() {
  local out
  out=$(XO_SUPERVISOR_BACKEND=herdr TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "explicit XO_SUPERVISOR_BACKEND override was not honored: $out"

  out=$(XO_SUPERVISOR_BACKEND='' TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = tmux ] || fail "TMUX_PANE should win over HERDR_ENV (tmux nested in herdr resolves to tmux): $out"

  out=$(XO_SUPERVISOR_BACKEND='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "HERDR_ENV=1 with HERDR_PANE_ID present should resolve to herdr: $out"

  if out=$(XO_SUPERVISOR_BACKEND='' TMUX_PANE='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_backend); then
...
test_discover_supervisor_target_herdr() {
  local out
  out=$(XO_SUPERVISOR_TARGET=explicit:target TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = "explicit:target" ] || fail "explicit XO_SUPERVISOR_TARGET override was not honored: $out"

  out=$(XO_SUPERVISOR_TARGET='' TMUX_PANE='%3' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = '%3' ] || fail "TMUX_PANE should win over herdr markers: $out"

  out=$(XO_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION='' discover_supervisor_target)
  [ "$out" = "default:w1:p9" ] || fail "herdr target should default HERDR_SESSION to 'default': $out"

  out=$(XO_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION=iso1 discover_supervisor_target)
  [ "$out" = "iso1:w1:p9" ] || fail "herdr target should use an explicit HERDR_SESSION: $out"

  if out=$(XO_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV='' HERDR_PANE_ID='' discover_supervisor_target); then
    fail "bare fallback should return non-zero"
  fi
  [ "$out" = "xo:0" ] || fail "bare fallback should still print xo:0: $out"

  pass "discover_supervisor_target: override > TMUX_PANE > herdr '<session>:<pane-id>' composition > xo:0 fallback"
}
...
test_pane_is_busy_herdr_native_busy_state() {
  ...
  (
    xo_backend_busy_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected busy_state args: $1 $2"; printf 'busy'; }
    xo_backend_capture() { fail "capture should not be consulted when busy_state is conclusive"; }
    XO_STATE_OVERRIDE="$dir/state" XO_DAEMON_PRIMARY_HARNESS=claude pane_is_busy "default:w1:p2" herdr \
...
test_primary_busy_guard_is_harness_scoped() {
  (
    xo_backend_busy_state() { printf 'unknown'; }
    xo_backend_capture() { printf 'esc interrupt\n'; }
    if XO_DAEMON_PRIMARY_HARNESS=claude pane_is_busy "default:w1:p2" herdr; then
      fail "OpenCode's rendered signature must not classify a Claude primary busy"
    fi
    XO_DAEMON_PRIMARY_HARNESS=opencode pane_is_busy "default:w1:p2" herdr \
...
test_pane_is_busy_defaults_to_tmux_when_backend_omitted() {
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_CAPTURE="$capture" XO_STATE_OVERRIDE="$dir/state" XO_DAEMON_PRIMARY_HARNESS=grok pane_is_busy "fakepane" \
...
test_pane_input_pending_herdr_dispatch() {
  (
    xo_backend_composer_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected composer_state args: $1 $2"; printf 'pending'; }
  ...
  (
    xo_backend_composer_state() { printf 'empty'; }
  ...
  (
    xo_backend_composer_state() { printf 'future-state'; }
  ...
  pass "pane_input_pending: dispatches through xo_backend_composer_state for backend=herdr"
}
...
test_inject_msg_herdr_busy_guard_defers() {
  ...
  afk_enter "$state"
  (
    xo_backend_target_exists() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected target_exists args: $1 $2"; return 0; }
    pane_is_busy() { return 0; }
    xo_backend_composer_state() { fail "composer_state should not be consulted once the busy-guard already deferred"; }
    xo_backend_send_text_submit() { fail "send_text_submit should not run when the busy-guard defers"; }
    if XO_SUPERVISOR_BACKEND=herdr XO_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_composer_guard_defers() {
  ...
  afk_enter "$state"
  (
    xo_backend_target_exists() { return 0; }
    pane_is_busy() { return 1; }
    xo_backend_composer_state() { [ "$1" = herdr ] && [ "$2" = "default:w1:p2" ] || fail "unexpected composer_state args: $1 $2"; printf 'pending'; }
    xo_backend_send_text_submit() { fail "send_text_submit should not run when the composer-guard defers"; }
    if XO_SUPERVISOR_BACKEND=herdr XO_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_pane_gone_defers() {
  ...
  afk_enter "$state"
  (
    xo_backend_target_exists() { return 1; }
    pane_is_busy() { fail "busy guard should not be consulted once the pane-exists check already failed"; }
    xo_backend_send_text_submit() { fail "send_text_submit should not run when the pane does not exist"; }
    if XO_SUPERVISOR_BACKEND=herdr XO_SUPERVISOR_TARGET="default:w1:gone" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_submits_through_backend_dispatch() {
  ...
  (
    ...
    XO_SUPERVISOR_BACKEND=herdr XO_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state" \
...
# Safety-critical (task xo-composer-shellglyph-safety): the away-mode injector
...
test_inject_msg_defers_on_dead_shell_unknown() {
  ...
  (
    xo_backend_target_exists() { return 0; }
    pane_is_busy() { return 1; }
    xo_backend_composer_state() { printf 'unknown'; }
    xo_backend_send_text_submit() { fail "send_text_submit must NOT run when the composer is a dead shell (unknown)"; }
    if XO_SUPERVISOR_BACKEND=herdr XO_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_defers_on_unrecognized_composer_state() {
  ...
  (
    ...
    pane_is_busy() { return 1; }
    xo_backend_composer_state() { printf 'future-state'; }
    xo_backend_send_text_submit() { fail "send_text_submit must not run for an unrecognized composer state"; }
    if XO_SUPERVISOR_BACKEND=herdr XO_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
