...
TMP_ROOT=$(xo_test_tmproot xo-watch-recovery-loop)
...
SH
  ...
  out=$(
    PLUGIN="$plugin" XO_HOME="$home" XO_ROOT_OVERRIDE="$repo" \
      XO_STATE_OVERRIDE="$home/state" PATH="$fakebin:$PATH" \
...
const lockPid = existsSync(`${process.env.XO_HOME}/state/.watch.lock/pid`)
  ? readFileSync(`${process.env.XO_HOME}/state/.watch.lock/pid`, "utf8").trim()
  : "";
...
const marker = readFileSync(`${process.env.XO_HOME}/state/.watcher-down`, "utf8").trim();
...
test_handling_successor_does_not_go_blind() {
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$state" \
    XO_POLL=1 XO_SIGNAL_GRACE=0 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=600 \
    XO_WATCH_HANDLING_SUCCESSOR=1 "$WATCH" > "$out" 2>&1 &
  child=$!
  now=0
  while [ "$now" -lt 40 ]; do
