...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/xo-herdr-session-cleanup-e2e.XXXXXX")
...
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name xo-herdr-session-start-stale-projection-cleanup-r1)
...
production_process_proof() {
  XO_HOME="$HOME_DIR" XO_BACKEND=herdr HERDR_SESSION="$HERDR_LAB_SESSION" \
    XO_HERDR_SESSION_CLEANUP_SOURCE_ONLY=1 PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" \
    bash -c '. "$1"; xo_backend_herdr_pane_idle_shell_pid "$2" "$3" >/dev/null' \
      _ "$ROOT/bin/xo-herdr-session-cleanup.sh" "$HERDR_LAB_SESSION" "$PANE"
}
focus_snapshot() {
...
XO_HOME="$HOME_DIR" XO_BACKEND=herdr HERDR_SESSION="$HERDR_LAB_SESSION" \
  PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" "$ROOT/bin/xo-herdr-session-cleanup.sh" \
