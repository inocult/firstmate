...
xo_live_gate default-on XO_HERDR_AGENT_EXIT_SHELL_E2E herdr jq pi
...
TMP_ROOT=$(xo_test_tmproot xo-herdr-agent-exit-shell-e2e)
...
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name xo-herdr-agent-exit-shell)
...
classify_pane() {  # <pane_id>
  PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" bash -c '
    ...
    xo_backend_herdr_pane_agent_state "$2" "$3"
...
classify_recovery() {  # <pane_id>
  PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" bash -c '
    set -u
    . "$1/bin/backends/herdr.sh"
    xo_backend_herdr_agent_state "$2:$3"
  ' _ "$ROOT" "$HERDR_LAB_SESSION" "$1"
}
