...
# bin/xo-spawn.sh installs under the contract owned by bin/xo-busy-lib.sh.
#
# These tests run the REAL xo-spawn against a fake tmux pane and an isolated
...
# bin/xo-busy-event.sh writer, and the real classifier are exercised together
...
TMP_ROOT=$(xo_test_tmproot xo-busy-adapter-wiring)
...
run_spawn() {  # <home> <wt> <fakebin> <spawn-args...>
  ...
  GROK_HOME="$home/grok-home" \
    xo_test_run_spawn "$home" "$wt" "$fakebin" "$@" --mode no-mistakes --yolo off
}
...
classify() {  # <harness> <id> <state-dir>
  xo_busy_classify tmux fake:w "$1" "$2" "$3"
}
...
test_pi_extension_semantic_lifecycle() {
  ...
  [ "$out" = "busy xo-spawn" ] || fail "seed after spawn must be 'busy xo-spawn', got '$out'"
  ...
  out=$(classify pi "$id" "$state")
  [ "$out" = "busy xo-spawn" ] || fail "native progress changed semantic state: $out"
  ...
  out=$(classify pi "$id" "$state")
  [ "$out" = "busy xo-spawn" ] || fail "turn_end must stay a notification, not a state edge, got '$out'"
...
test_pi_extension_stale_incarnation_rejected() {
  ...
  "$ROOT/bin/xo-busy-event.sh" arm "$state" "$id" >/dev/null
  out=$(drive_pi_ext "$ext" settle-idle) || fail "stale settle drive failed: $out"
  out=$(classify pi "$id" "$state")
  [ "$out" = "busy xo-spawn" ] || fail "a stale extension event must not change state, got '$out'"
...
const hooks = await mod.XoBusyState({});
...
test_opencode_plugin_semantic_lifecycle() {
  ...
  state="$HOME_DIR/state"
  plugin="$WT_DIR/.opencode/plugins/xo-busy-state.js"
...
test_claude_hooks_stale_incarnation_harmless() {
  ...
  out=$(classify claude "$id" "$state")
  [ "$out" = "busy xo-spawn" ] || fail "a stale-gen hook event must not change state, got '$out'"
...
test_codex_unverified_until_a_semantic_source_exists() {
  ...
  out=$(xo_busy_classify tmux fake:w codex "$id" "$state" '• Working (6s • esc to interrupt)')
...
test_gemini_hooks_semantic_lifecycle() {
  ...
  # xo must never write it, or a project's configuration is clobbered.
...
test_gemini_is_refused_as_a_secondmate() {
  ...
  out=$(GROK_HOME="$HOME_DIR/grok-home" \
    xo_test_run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" --secondmate "$id" gemini) && {
...
echo "all xo-busy-adapter-wiring tests passed"
