#!/usr/bin/env bash
# Behavioral tests for bin/xo-procevent-quota.sh.
set -u

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
XO_ROOT="${XO_ROOT_OVERRIDE:-$(cd "$SCRIPT_DIR/.." && pwd)}"
BIN="$XO_ROOT/bin"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-procevent-quota.XXXXXX")
FAKEBIN="$LAB/fakebin"
COUNT="$LAB/count"
...
if help=$("$BIN/xo-procevent-quota.sh" --help 2>&1); then
  fail "help unexpectedly exited zero"
fi
printf '%s\n' "$help" | grep -Fq 'xo-procevent-quota.sh retire [--provider <provider>]' \
...
out=$(QUOTA_AXI_EXHAUSTED_DETAIL=1 QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" \
  "$BIN/xo-procevent-quota.sh" poll)
...
out=$(QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 0.01 --threshold 10 --provider codex --timeout 1)
...
out=$(QUOTA_AXI_EXHAUSTED_DETAIL=1 QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" \
  "$BIN/xo-procevent-quota.sh" poll --interval 1 --threshold 10 --provider codex --timeout 1)
...
rm -f "$COUNT"
out=$(QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 0.01 --threshold 10 --provider '' --timeout 1)
...
rm -f "$COUNT"
out=$(QUOTA_AXI_EMPTY_FIRST=1 QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 0.01 --threshold 10 --provider '' --timeout 1)
...
if err=$(QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" arm --provider 2>&1); then
...
for provider in -- codex-; do
  if err=$(QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" arm --provider "$provider" 2>&1); then
...
out=$(XO_HOME="$LAB/retire-home" XO_STATE_OVERRIDE="$LAB/retire-state" \
  "$BIN/xo-procevent-quota.sh" retire --provider codex)
...
if err=$(QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 1 --threshold 100.5 --provider codex --timeout 1 2>&1); then
...
rm -f "$COUNT"
out=$(QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 0.01 --threshold 010 --provider codex --timeout 1)
...
rm -f "$COUNT"
out=$(QUOTA_AXI_AT_THRESHOLD=1 QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 0.01 --threshold 10 --provider codex --timeout 1)
...
if err=$(QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --provider 2>&1); then
...
rm -f "$COUNT"
out=$(XO_TIMEOUT_MECHANISM_OVERRIDE=bash QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 0.01 --threshold 10 --provider codex --timeout 1)
...
for malformed in schema duplicate types range runway availability known-empty semantics-mismatch identity; do
  out=$(QUOTA_AXI_MALFORMED="$malformed" QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 1 --threshold 10 --provider codex --timeout 1)
...
rm -f "$COUNT"
out=$(QUOTA_AXI_UNKNOWN_FIRST=1 QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 0.01 --threshold 10 --provider codex --timeout 1)
...
rm -f "$COUNT"
out=$(QUOTA_AXI_KNOWN_UNKNOWN_FIRST=1 QUOTA_AXI_COUNT="$COUNT" PATH="$FAKEBIN:$PATH" "$BIN/xo-procevent-quota.sh" poll --interval 0.01 --threshold 10 --provider codex --timeout 1)
...
printf '# all xo-procevent-quota tests passed\n'
