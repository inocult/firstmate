...
# Run explicitly with XO_CMUX_CLAUDE_COMPOSER_LIVE=1; it creates and cleans up
# only one exact xo-test- workspace through the normal scout lifecycle.
set -u
...
TASK="xo-test-cmux-claude-composer-$$"
LAB=
SPAWNED=0
...
cleanup() {
  [ "$SPAWNED" -eq 0 ] || {
    ...
    XO_HOME="$LAB" "$ROOT/bin/xo-decision-hold.sh" complete "$TASK" --none >/dev/null 2>&1 || true
    XO_HOME="$LAB" "$ROOT/bin/xo-teardown.sh" "$TASK" >/dev/null 2>&1 || true
  }
  [ -z "$LAB" ] || rm -rf -- "$LAB"
}

xo_live_gate opt-in XO_CMUX_CLAUDE_COMPOSER_LIVE claude cmux jq treehouse python3

cmux ping >/dev/null 2>&1 || fail "XO_CMUX_CLAUDE_COMPOSER_LIVE=1 but the cmux socket is unavailable"

LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-cmux-claude-composer.XXXXXX") || fail "could not create an isolated cmux Claude lab"
trap cleanup EXIT
...
XO_HOME="$LAB" "$ROOT/bin/xo-brief.sh" "$TASK" comms --scout || fail "could not scaffold the Claude probe brief"
...
Then append exactly `needs-decision [key=probe-decision]: awaiting codeword` to that file and stop to wait for an xo message.
When you receive an xo message containing `ALBATROSS`, append `done: received ALBATROSS` to that status file and stop.
Do not change project files or make a commit.'''))
PY

XO_HOME="$LAB" "$ROOT/bin/xo-spawn.sh" "$TASK" "$LAB/projects/comms" --scout --harness claude --model haiku --backend cmux \
...
xo_backend_source cmux || fail "could not source the cmux adapter"
TARGET=$(awk -F= '/^window=/{print $2}' "$LAB/state/$TASK.meta")
[ -n "$TARGET" ] || fail "the cmux probe did not record its endpoint"

for _ in $(seq 1 45); do
  CAPTURE=$(xo_backend_cmux_capture "$TARGET" 200 "$TASK" 2>/dev/null || true)
  case "$CAPTURE" in
    *'Yes, I trust this folder'*) XO_HOME="$LAB" "$ROOT/bin/xo-send.sh" "$TASK" --key Enter || fail "could not accept Claude's folder-trust prompt" ;;
  esac
...
COMPOSER=$(xo_backend_cmux_composer_state "$TARGET" "$TASK")
...
XO_SEND_SETTLE=0 XO_HOME="$LAB" "$ROOT/bin/xo-send.sh" "$TASK" --resolve-key probe-decision ALBATROSS \
...
CAPTURE=$(xo_backend_cmux_capture "$TARGET" 200 "$TASK")
