#!/usr/bin/env bash
# xo-send remote-secondmate delivery: the remote inbox leg.
#
# A remote steer is delivered by durable record, never by typing its payload:
# xo-send crosses xo-on.sh to the host-local leg (xo-remote-secondmate-control.sh
...
# the real xo-send + xo-on executables with a stubbed ssh transport that
# EXECUTES the real host-local leg against a seeded remote-home fixture (the
# XO_SSH_BIN seam tests/xo-on.test.sh proves preserves exit status), and pin:
...
#      (executed remotely, then ssh exit 255) makes xo-send retry the same
...
. "$ROOT/bin/xo-task-inbox-lib.sh"

SEND="$ROOT/bin/xo-send.sh"
DRAIN="$ROOT/bin/xo-wake-drain.sh"

TMP_ROOT=$(xo_test_tmproot xo-send-remote-delivery)
TMP_ROOT=$(cd "$TMP_ROOT" && pwd)

# Stub tmux for the local typed-plane legs: logs literal typed text to
# XO_SEND_LOG. The default composer reads empty (clean submit);
# XO_FAKE_TMUX_PENDING=1 keeps a proven pending composer with no busy footer,
...
# fails with XO_FAKE_SSH_RC (emitting XO_FAKE_SSH_STDERR as the remote
# stderr), or decodes the entrypoint argv and executes the REAL host-local
# command against the decoded remote home - with XO_FAKE_SSH_AMBIGUOUS=1
...
case "${1:-}" in
  ...
  display-message)
    ...
    printf 'fakepane\n'; exit 0 ;;
  capture-pane)
    if [ "${XO_FAKE_TMUX_PENDING:-0}" = 1 ]; then
      printf '╭────────────╮\n│ > steer    │\n╰────────────╯\n'
    else
      printf '╭────╮\n│    │\n╰────╯\n'
    fi
...
set -u
cat > /dev/null
count=$(cat "$XO_SSH_COUNT" 2>/dev/null || echo 0)
count=$((count + 1))
printf '%s\n' "$count" > "$XO_SSH_COUNT"
printf '%s\n' "$*" >> "$XO_SSH_LOG"
if [ -n "${XO_FAKE_SSH_HANG:-}" ]; then
  ...
  /bin/sleep "$XO_FAKE_SSH_HANG"
  exit 255
fi
if [ "${XO_FAKE_SSH_AFTER_AMBIGUOUS_RC:-0}" -ne 0 ] && [ "$count" -gt 1 ]; then
  exit "$XO_FAKE_SSH_AFTER_AMBIGUOUS_RC"
fi
if [ "${XO_FAKE_SSH_RC:-0}" -ne 0 ]; then
  [ -z "${XO_FAKE_SSH_STDERR:-}" ] || printf '%s\n' "$XO_FAKE_SSH_STDERR" >&2
  exit "${XO_FAKE_SSH_RC}"
fi
while [ "$#" -gt 0 ]; do
...
env XO_HOME="$remote_home" XO_ROOT_OVERRIDE="$XO_REMOTE_CODE_ROOT" \
  "$XO_REMOTE_CODE_ROOT/bin/$cmd" "${rargs[@]:1}" || rc=$?
if [ "${XO_FAKE_SSH_AMBIGUOUS:-0}" = 1 ] \
  || { [ "${XO_FAKE_SSH_AFTER_AMBIGUOUS_RC:-0}" -ne 0 ] && [ "$count" -eq 1 ]; }; then
  exit 255
fi
exit "$rc"
SH
...
# writes into: identity marker, XO-checkout shape, and a parent-route
# endpoint record on Herdr in the dedicated xo-remote session.
setup_remote_secondmate_home() {  # <name> -> echoes remote home dir
  local rh="$TMP_ROOT/$1-rhome"
  mkdir -p "$rh/state/parent-route" "$rh/bin"
  printf 'rsm\n' > "$rh/.xo-secondmate-home"
  printf '# remote secondmate home fixture\n' > "$rh/AGENTS.md"
  xo_write_meta "$rh/state/parent-route/rsm.meta" \
    "window=xo-remote:p1" \
    "worktree=-" \
    "project=-" \
    "backend=herdr" \
...
# xo-on.sh resolves the ssh route from, pointing at <remote-home>.
setup_remote_parent_home() {  # <name> <remote-home> -> echoes home dir
  local home
  home=$(setup_home "$1")
  mkdir -p "$home/data"
  xo_write_meta "$home/state/rsm.meta" \
    "window=xo-remote:p1" \
    "endpoint_task_id=rsm" \
    "harness=claude" \
    "kind=secondmate" \
...
drain_out() {  # <home>
  XO_STATE_OVERRIDE="$1/state" "$DRAIN" 2>/dev/null
}
...
send_env() {  # <fakebin> <parent-home> <ssh-log> [extra env...] -- <cmd...>
  ...
  shift 3
  env PATH="$fb:$PATH" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" XO_SEND_SETTLE=0 \
    XO_SSH_BIN="$fb/fake-ssh" XO_SSH_LOG="$ssh_log" \
    XO_SSH_COUNT="$ssh_log.count" XO_REMOTE_CODE_ROOT="$ROOT" \
    "$@"
}
...
test_remote_steer_lands_in_remote_inbox() {
  ...
  assert_grep 'xo-remote-entrypoint.sh' "$ssh_log" "the steer should cross the remote transport"
  ...
  pass "xo-send remote: a text steer lands as a durable remote inbox record and exits 0 at enqueue"
}
...
test_remote_rerun_is_idempotent() {
  ...
  # 255 (completion unknown). xo-send retries the identical leg once; the
  # idempotent remote write must land both executions on the same record.
  rc=0
  (
    cd "$TMP_ROOT" || exit 1
    send_env "$fb" "${home#"$TMP_ROOT/"}" "$ssh_log" XO_FAKE_SSH_AMBIGUOUS=1 \
  ...
  [ "$(cat "$ssh_log.count")" = 2 ] \
    || fail "xo-send must retry the identical remote leg exactly once after ssh 255, got $(cat "$ssh_log.count") attempts"
  ...
  corr=$(xo_pending_reply_get "$pend" corr_id)
  printf -v quoted '%q' "$home"
  expected_cmd="XO_HOME=$quoted XO_PENDING_REPLY_EXISTING_CORR=$corr"
  ...
  xo_pending_reply_set "$pend" delivered_epoch "" \
    || fail "could not restore the undelivered fixture expectation"
  [ "$(xo_pending_reply_get "$pend" phase)" = escalated ] \
  ...
  (
    unset XO_HOME
    env PATH="$fb:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_SEND_SETTLE=0 \
      XO_SSH_BIN="$fb/fake-ssh" XO_SSH_LOG="$ssh_log" \
      XO_SSH_COUNT="$ssh_log.count" XO_REMOTE_CODE_ROOT="$ROOT" \
      bash -c "$resend_cmd"
  ) >"$dir/resend.out" 2>"$dir/resend.err" || rc=$?
  expect_code 0 "$rc" "the printed correlation-reusing resend must succeed without an inherited XO_HOME"
  ...
  [ -n "$(xo_pending_reply_get "$pend" delivered_epoch)" ] \
    || fail "the successful resend did not confirm pending-reply delivery: $(cat "$pend")"
  [ "$(xo_pending_reply_get "$pend" phase)" = awaiting_report ] \
    || fail "the successful resend did not restore awaiting_report: $(cat "$pend")"
  pass "xo-send remote: the printed correlation-reusing resend deduplicates onto the same record"
}
...
test_remote_retry_failure_preserves_ambiguous_expectation() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" XO_FAKE_SSH_AFTER_AMBIGUOUS_RC=1 \
    "$SEND" rsm "please rename the metric" >"$dir/out" 2>"$dir/err" || rc=$?
  err=$(cat "$dir/err")
  ...
  pass "xo-send remote: a failed retry cannot erase an earlier ambiguous delivery"
}
...
test_remote_fire_and_forget_never_arms_reply_recovery() {
  ...
  action=$(XO_TASK_INBOX_GRACE_SECS=0 XO_TASK_INBOX_RING_MAX=0 \
    xo_task_inbox_due_action "$rhome/state/parent-route" rsm)
  ...
  pass "xo-send remote: fire-and-forget delivery is idempotent without reply recovery"
}
...
test_remote_send_revalidates_after_retirement_lock() {
  ...
  XO_STATE_OVERRIDE="$rhome/state/parent-route" bash -c '
    . "$1"
    xo_task_inbox_lock_acquire "$2" || exit 91
    : > "$3"
    while [ ! -e "$4" ]; do sleep 0.05; done
    rm -f "$5"
    xo_lock_release "$2"
  ' _ "$ROOT/bin/xo-task-inbox-lib.sh" "$lock" "$ready" "$release" "$meta" &
  holder_pid=$!
  while [ ! -e "$ready" ]; do kill -0 "$holder_pid" 2>/dev/null || fail "metadata-lock holder exited early"; sleep 0.05; done

  rc=0
  env XO_HOME="$rhome" XO_ROOT_OVERRIDE="$ROOT" \
    "$ROOT/bin/xo-remote-secondmate-control.sh" send rsm "retirement-race steer" \
    >"$dir/out" 2>"$dir/err" &
  sender_pid=$!
  sleep 0.2
  : > "$release"
...
test_remote_send_revalidates_parent_route_after_retirement_lock() {
  ...
  pass "xo-send remote: enqueue revalidates the parent route under its metadata lock"
}
...
test_remote_expected_host_revalidates_final_route() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" \
    XO_SEND_EXPECTED_SPAWN_GEN="" XO_SEND_EXPECTED_REMOTE_HOST=remote-mac \
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" \
    XO_SEND_EXPECTED_SPAWN_GEN="" XO_SEND_EXPECTED_REMOTE_HOST=retired-mac \
  ...
  pass "xo-send remote: expected host is enforced by final route validation"
}
...
test_remote_resolve_key_closes_at_enqueue() {
  ...
  pass "xo-send remote: a --resolve-key answer closes its decision at enqueue"
}
...
test_remote_slash_rides_inbox() {
  ...
  pass "xo-send remote: every remote text, harness-native included, rides the inbox"
}
...
test_remote_real_failure_still_fails() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" XO_FAKE_SSH_RC=1 \
    XO_FAKE_SSH_STDERR='error: remote secondmate rsm endpoint metadata is invalid; refusing access until it is explicitly migrated' \
    "$SEND" rsm "please rename the metric" >"$dir/out" 2>"$dir/err" || rc=$?
  err=$(cat "$dir/err")
  ...
  pass "xo-send remote: a real remote failure still fails loudly with the remote diagnostics"
}
...
test_remote_exit3_no_longer_delivered() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" XO_FAKE_SSH_RC=3 \
    "$SEND" rsm "please rename the metric" >"$dir/out" 2>"$dir/err" || rc=$?
  err=$(cat "$dir/err")
  ...
  pass "xo-send remote: exit 3 from the remote leg is a failure, never a delivery claim"
}
...
test_remote_transport_loss_preserves_expectation() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" XO_FAKE_SSH_RC=255 \
    "$SEND" rsm "please rename the metric" >"$dir/out" 2>"$dir/err" || rc=$?
  err=$(cat "$dir/err")
  [ "$rc" -ne 0 ] || fail "an unknown-completion transport loss must exit nonzero"
  [ "$(cat "$ssh_log.count")" = 2 ] \
    || fail "xo-send must retry the identical remote leg once on ssh 255, got $(cat "$ssh_log.count") attempts"
  assert_contains "$err" "Only the correlation-reusing resend below is idempotent" \
  ...
  pass "xo-send remote: ssh 255 fails with resend-safe guidance and preserves the expectation"
}
...
test_remote_send_budget_bounds_busy_lane() {
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" XO_SEND_REMOTE_BUDGET=invalid \
  ...
  began=$(date +%s)
  rc=0
  send_env "$fb" "$home" "$ssh_log" XO_FAKE_SSH_HANG=60 XO_SEND_REMOTE_BUDGET=2 \
  ...
  rc=0
  send_env "$fb" "$home" "$ssh_log" XO_SEND_REMOTE_BUDGET=invalid \
    XO_PENDING_REPLY_EXISTING_CORR="$corr" \
  ...
  pass "xo-send remote: the remote leg is budget-bounded and stays idempotent across the bound"
}
...
test_local_secondmate_pending_keeps_expectation_armed() {
  ...
  xo_write_meta "$home/state/lsm.meta" \
    "window=sess:xo-lsm" "harness=claude" "kind=secondmate" "mode=secondmate" "home=$home/sm"
  ...
  : > "$log"
  rc=0
  env PATH="$fb:$PATH" XO_FAKE_TMUX_PENDING=1 \
  ...
  [ "$(xo_pending_reply_get "$rec" phase)" = awaiting_report ] \
    || fail "the surviving expectation must stay armed, got $(xo_pending_reply_get "$rec" phase)"
  ...
  xo_pending_reply_try_resolve "$home/state" "$corr" \
    || fail "a correlated report must still resolve the preserved expectation"
  pass "xo-send local: an unconfirmed secondmate send keeps its reply expectation armed"
}
...
test_local_pending_reports_delivered_unconfirmed() {
  ...
  pass "xo-send local: an unconfirmed submit exits 3 with an honest non-error report"
}
...
test_local_pending_does_not_close_resolve_key() {
  ...
  xo_write_meta "$home/state/t2.meta" "window=sess:xo-t2" "kind=ship"
  ...
  pass "xo-send local: an unconfirmed submit still never closes a --resolve-key decision"
}
...
echo "all xo-send-remote-delivery tests passed"
