...
CASCADE="$ROOT/bin/xo-stow-cascade.sh"
TMP_ROOT=$(xo_test_tmproot xo-stow-cascade)
mkdir -p "$TMP_ROOT"
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
FAKEBIN=$(xo_fakebin "$TMP_ROOT/fakebin")
BASE_PATH=${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}

# A fake ssh standing in for the whole remote transport. It decodes xo-on.sh's
...
case "$command" in
  xo-startup-memory-budget.sh)
    cat "$XO_FAKE_REMOTE_BUDGET"
    ;;
  xo-remote-secondmate-control.sh)
    printf '%s\n' "${XO_FAKE_REMOTE_AGENT_STATE:-alive}"
    ;;
  *) exit 91 ;;
esac
SH
...
test_every_registered_home_is_enumerated_exactly_once() {
  ...
  xo_write_secondmate_meta "$primary/state/enum-a.meta" "$a"
  xo_write_secondmate_meta "$primary/state/enum-a-stale.meta" "$a"
...
test_transport_routes_by_placement_and_liveness() {
  ...
  xo_write_secondmate_meta "$primary/state/live-local.meta" "$live" 'xo:xo-live-local' alpha claude
  xo_write_secondmate_meta "$primary/state/remote-live.meta" "$remote" 'xo-remote:xo-remote-live' alpha claude
  printf 'role=secondmate\neffective_budget_tokens=7500\ntotal_estimated_tokens=100\nbudget_status=within-budget\n' \
    > "$TMP_ROOT/remote-budget.txt"

  set +e
  out=$(run_cascade "$primary" \
    XO_FAKE_TMUX_WINDOW='xo-live-local' \
    XO_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt" \
    XO_FAKE_REMOTE_AGENT_STATE=alive)
  set -e
  ...
  out=$(run_cascade "$primary" \
    XO_FAKE_TMUX_WINDOW='xo-live-local' \
    XO_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt" \
    XO_FAKE_REMOTE_AGENT_STATE=dead)
  set -e
  s=$(stanza "$out" remote-live)
...
test_a_slow_remote_is_bounded_and_the_rest_still_report() {
  ...
  xo_write_secondmate_meta "$primary/state/bounded-remote.meta" "$remote" 'xo-remote:xo-bounded-remote'
  printf 'role=secondmate\n' > "$TMP_ROOT/remote-budget.txt"

  started=$(date +%s)
  set +e
  out=$(run_cascade "$primary" \
    XO_STOW_CASCADE_TIMEOUT=2 \
    XO_FAKE_SSH_MODE=hang \
    XO_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt")
  rc=$?
  set -e
