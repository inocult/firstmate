...
# compaction. It runs an isolated tmux server, throwaway XO checkout,
# and scratch XO_HOME, so it never drives the caller's Pi session or fleet.
#
...
xo_live_gate opt-in XO_SESSIONSTART_INSTRUCTION_REFRESH_LIVE_E2E pi tmux git

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
TMUX_SOCKET="xo-sessionstart-instruction-refresh-$$"
TMUX_SESSION="instruction-refresh"
LAB=${TMPDIR:-/tmp}
LAB="${LAB%/}/xo-sessionstart-instruction-refresh-live-e2e.$$"
PROJECT="$LAB/project"
HOME_DIR="$LAB/home"
...
TEST_REF=${XO_SESSIONSTART_INSTRUCTION_REFRESH_REF:-HEAD}
TEST_COMMIT=$(git -C "$ROOT" rev-parse --verify "$TEST_REF^{commit}" 2>/dev/null) || {
  printf 'not ok - could not resolve isolated test ref %s\n' "$TEST_REF" >&2
  exit 2
}
EXPECTATION=${XO_SESSIONSTART_INSTRUCTION_REFRESH_EXPECT:-updated}
case "$EXPECTATION" in
  updated|stale) ;;
  *) printf 'not ok - expected XO_SESSIONSTART_INSTRUCTION_REFRESH_EXPECT=updated or stale, got: %s\n' "$EXPECTATION" >&2; exit 2 ;;
esac
...
mkdir -p "$LAB"
git clone --quiet --no-hardlinks "$ROOT" "$PROJECT" || fail "could not create isolated XO checkout"
...
mv "$PROJECT/bin/xo-sessionstart-run.sh" "$PROJECT/bin/.xo-sessionstart-run.real.sh"
cat > "$PROJECT/bin/xo-sessionstart-run.sh" <<'SH'
#!/usr/bin/env bash
set -o pipefail
set -u
state="${XO_HOME:?}/state"
printf 'argv=%s pi=%s root=%s home=%s\n' "$*" "${PI_CODING_AGENT:-absent}" "${XO_ROOT_OVERRIDE:-absent}" "${XO_HOME:-absent}" \
  >> "$state/.sessionstart-e2e-sources"
"$(dirname "$0")/.xo-sessionstart-run.real.sh" "$@" | tee -a "$state/.sessionstart-e2e-output"
exit "${PIPESTATUS[0]}"
SH
chmod +x "$PROJECT/bin/xo-sessionstart-run.sh"
...
tmux -L "$TMUX_SOCKET" new-session -d -s "$TMUX_SESSION" -c "$PROJECT" -x 220 -y 55 \
  -e "XO_HOME=$HOME_DIR" -e "XO_ROOT_OVERRIDE=$PROJECT" -e "XO_GATE_REFUSE_BYPASS=1" \
  pi --no-tools -e "$PROJECT/.pi/extensions/xo-primary-turnend-guard.ts" \
...
echo "# xo-sessionstart-instruction-refresh-live-e2e.test.sh: all live assertions passed"
