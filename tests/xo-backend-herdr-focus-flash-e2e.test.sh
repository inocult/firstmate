...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/xo-herdr-focus-flash-e2e.XXXXXX")
FAKEBIN="$TMP_ROOT/fakebin"
mkdir -p "$FAKEBIN"

HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name xo-herdr-focus-flash-regression-r1)
...
: > "$B_OPERATION_ACTIVE"
B_OUT=$(PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" XO_FLASH_CALL_LOG="$CALL_LOG" bash -c '
  ...
  xo_backend_herdr_projection_close_pane_focus_preserving "$2" "$3"
...
C_PROOF_POLLS=3
C_OUT=$(PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" XO_FLASH_CALL_LOG="$C_CALL_LOG" \
  XO_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS="$C_PROOF_POLLS" bash -c '
  . "$1/bin/backends/herdr.sh"
  xo_backend_herdr_cli() {
    local session=$1
    shift
