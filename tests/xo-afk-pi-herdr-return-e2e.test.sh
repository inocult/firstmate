...
# through xo-herdr-lab.sh. The scenario proves, against a real Pi primary:
...
# tests/xo-afk-inject-herdr-e2e.test.sh for the harnesses that still run it.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

xo_live_gate opt-in XO_AFK_PI_HERDR_E2E herdr jq pi python3

# shellcheck source=/dev/null
. "$ROOT/bin/xo-supervise-daemon.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/xo-backend.sh"

LAB_HELPER=${HERDR_LAB_HELPER:-$ROOT/bin/xo-herdr-lab.sh}
SESSION=$("$LAB_HELPER" name xo-afk-pi-return-e2e)
TMP_ROOT=$(xo_test_tmproot xo-afk-pi-return-e2e)
...
cleanup() {
  ...
  if [ -f "$STATE/.afk-contract" ] || [ -e "$STATE/.afk" ]; then
    PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
      PI_CODING_AGENT=true "$ROOT/bin/xo-afk-launch.sh" stop >/dev/null 2>&1 || true
  fi
...
printf '# Synthetic isolated XO primary\n' > "$PROJECT/AGENTS.md"
...
const capturePath = process.env.XO_PI_CAPTURE_PATH!;
...
EXT="$CAPTURE_EXT"
PI_CMD=$(printf 'exec env PI_CODING_AGENT_DIR=%q XO_HOME=%q XO_PI_CAPTURE_PATH=%q pi -e %q --no-context-files --no-session' "$PI_DIR" "$HOME_DIR" "$CAPTURE" "$EXT")
...
CHILD_OUT=$("$LAB_HELPER" run "$SESSION" tab create --workspace "$WORKSPACE" --cwd "$PROJECT" --label xo-repair-task --no-focus)
...
set +e
START_OUT=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  PI_CODING_AGENT=true XO_SUPERVISOR_BACKEND=herdr XO_SUPERVISOR_TARGET="$PRIMARY_TARGET" \
  "$ROOT/bin/xo-afk-launch.sh" start 2>&1)
START_RC=$?
set -e
...
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  PI_CODING_AGENT=true "$ROOT/bin/xo-afk-launch.sh" propose >/dev/null || fail "the away posture read-back failed on Pi"
CONFIRM_OUT=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  PI_CODING_AGENT=true "$ROOT/bin/xo-afk-launch.sh" confirm 2>&1) || fail "the away posture could not be recorded on Pi: $CONFIRM_OUT"
...
sleep 0.5
composer=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" xo_backend_composer_state herdr "$PRIMARY_TARGET")
[ "$composer" = pending ] || fail "real Pi draft did not classify pending (got $composer)"
CHILD_CMD=$(printf "printf 'blocked [key=synthetic-dependency]: xo can refresh the synthetic token\\n' >> %q; exec sleep 120" "$STATE/repair-task.status")
...
set +e
RETURN_OUT=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_ROOT_OVERRIDE="$PROJECT" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  PI_CODING_AGENT=true "$ROOT/bin/xo-afk-return.sh" begin 2>&1)
RETURN_RC=$?
set -e
[ "$RETURN_RC" -eq 3 ] || fail "return catch-up did not gate the still-live blocker (rc=$RETURN_RC): $RETURN_OUT"
assert_contains "$RETURN_OUT" 'xo-actionable blocker: repair-task [key=synthetic-dependency]' "return gate did not assign remediation"
...
set +e
BEARINGS_OUT=$(PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_ROOT_OVERRIDE="$PROJECT" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  "$ROOT/bin/xo-bearings-snapshot.sh" --json 2>&1)
BEARINGS_RC=$?
set -e
...
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_ROOT_OVERRIDE="$PROJECT" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  "$ROOT/bin/xo-afk-return.sh" check >/dev/null || fail "remediated blocker did not clear return catch-up"
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_ROOT_OVERRIDE="$PROJECT" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  "$ROOT/bin/xo-bearings-snapshot.sh" --json >/dev/null || fail "Bearings remained gated after blocker remediation"
...
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  PI_CODING_AGENT=true "$ROOT/bin/xo-afk-launch.sh" propose >/dev/null || fail "clean away re-entry read-back failed"
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  PI_CODING_AGENT=true "$ROOT/bin/xo-afk-launch.sh" confirm >/dev/null || fail "clean away re-entry failed"
PATH="$FAKEBIN:$ORIGINAL_PATH" HERDR_SESSION="$SESSION" XO_ROOT_OVERRIDE="$PROJECT" XO_HOME="$HOME_DIR" XO_STATE_OVERRIDE="$STATE" \
  PI_CODING_AGENT=true "$ROOT/bin/xo-afk-return.sh" begin >/dev/null \
