#!/usr/bin/env bash
# tests/xo-composer-lib.test.sh - the shared composer-content classifier
# (bin/xo-composer-lib.sh), the ONE fleet-wide owner every backend adapter
# delegates its empty|pending|unknown verdict to.
#
# The load-bearing contract, task xo-composer-shellglyph-safety:
...
classify() { xo_composer_classify_content "$@"; }
...
test_bare_shell_glyphs_are_unknown() {
  ...
  pass "xo_composer_classify_content: a bare shell prompt glyph (>/\$/%/#) reads unknown, never empty"
}
...
test_stripped_unbordered_content_uses_plain_content() {
  ...
  pass "xo_composer_classify_content: stripped unbordered content is unknown except verified agent glyphs"
}
...
test_bare_shell_prompt_with_command_is_not_empty() {
  ...
  pass "xo_composer_classify_content: a bare shell prompt carrying a command is not empty"
}
...
test_bordered_shell_glyph_is_empty() {
  ...
  pass "xo_composer_classify_content: a bare prompt glyph inside a bordered composer box reads empty (claude's own idle composer)"
}
...
test_agent_glyphs_are_empty_bordered_and_bare() {
  ...
  pass "xo_composer_classify_content: agent prompt glyphs (❯ claude, › codex, ⟩ muse) read empty bordered or bare"
}
...
test_empty_content_is_empty() {
  ...
  pass "xo_composer_classify_content: an empty composer reads empty"
}
...
test_idle_placeholder_is_empty() {
  ...
  pass "xo_composer_classify_content: idle matching is limited to proven placeholder positions"
}
...
test_idle_placeholder_case_mode_is_explicit() {
  ...
  pass "xo_composer_classify_content: idle matching preserves the caller's case mode"
}
...
test_real_text_is_pending() {
  ...
  pass "xo_composer_classify_content: real unsubmitted text reads pending (including a popup argument-hint fill)"
}

# =============================================================================
# xo_composer_classify_screen: the adapter-facing screen classifier and the
# correctness matrix (audit data/xo-composer-consolidation-audit-s1, task
# xo-composer-thin-adapter-refactor-r1).
#
...
assert_screen() {
  ...
  shift 2
  out=$(xo_composer_classify_screen "$@")
  [ "$out" = "$want" ] || fail "$label: expected $want, got '$out'"
  out=$(LC_ALL=C xo_composer_classify_screen "$@")
...
test_matrix_muse_truecolor_glyph_survives_signal_loss() {
  ...
  out=$(XO_COMPOSER_GHOST_LUMA_MAX=200 xo_composer_classify_screen "$CAPS_STYLED" "$screen")
...
test_matrix_cursor_reverse_video_placeholder_remnant() {
  ...
  stripped=$(printf '%s' "$row" | xo_composer_strip_ghost)
  ...
  out=$(xo_composer_classify_screen "$CAPS_PLAIN" "$typed_plain")
  [ "$out" != empty ] \
...
test_matrix_herdr_halfblock_rule_bounds_bare_wrap() {
  ...
  out=$(xo_composer_classify_screen "$CAPS_STYLED" "$screen")
  [ "$out" = empty ] \
...
❯ fix the flaky test
 π  · ◔ GPT-6-Astra · 🌳 …-workspace · ⑂ detached · ◫ 15.4%/272K ⟲ · (sub)'
  ...
  _xo_composer_row_is_omp_status ' π  · ◔ GPT-6-Astra · 🌳 …-workspace' \
    || fail "the unicode-preset omp status row must be recognized as furniture"
  _xo_composer_row_is_omp_status ' 󰵗  ·  qwen3:8b ·  kun-agent-workspace/… ·  detached ?1 ·  36.7%/41K' \
    || fail "the nerd-preset omp status row must be recognized as furniture"
  _xo_composer_row_is_omp_status ' ⠧ 11s  · ◔ GPT-6-Astra' \
    || fail "the busy omp spinner row must be recognized as furniture"
  _xo_composer_row_is_omp_status 'fix the flaky test' \
    && fail "ordinary typed text must not be mistaken for omp status furniture"
  _xo_composer_row_is_omp_status 'please rerun the suite and report' \
  ...
  _xo_composer_row_is_omp_status 'fix · tests before pushing' \
  ...
  _xo_composer_row_is_omp_status 'pi · e · phi as the three constants' \
    && fail "typed text opening 'pi ·' must not be mistaken for omp status furniture"
  _xo_composer_row_is_omp_status ' ⣾ 3s  · ◔ GPT-6-Astra' \
...
test_matrix_pi_separated_needs_identity() {
  ...
  [ "$(xo_composer_classify_screen "$CAPS_STYLED" "$screen")" = need-identity ] \
...
test_matrix_opencode_leftbar_signals() {
  ...
  out=$(XO_COMPOSER_IDLE_RE='^NEVER-MATCHES$' xo_composer_classify_screen "$CAPS_TMUX" "$dim_screen" 1)
...
test_matrix_kimi_bordered_shell_glyph_box() {
  # Kimi's bordered `│ > │` composer - the shape xo-spawn.sh's retired
...
test_strict_blank_row_divergence() {
  ...
  out=$(xo_composer_classify_screen "$CAPS_PLAIN" $'\n\n')
...
test_bare_wrap_region_classifies() {
  ...
  out=$(xo_composer_classify_screen "$CAPS_TMUX" "$ghost_wrapped" 1)
  ...
  out=$(xo_composer_classify_screen "$CAPS_TMUX" $'❯ text\n────────────────\nbelow the rule' 2)
  ...
  pass "xo_composer_classify_screen: the bare composer's wrap region stays identified; structure breaks it"
}
...
test_contiguous_transcript_reanchors_on_live_prompt() {
  ...
  pass "xo_composer_classify_screen: a row-leading agent glyph reanchors the live composer"
}
...
test_lower_dead_shell_invalidates_cursorless_candidate() {
  ...
  out=$(xo_composer_classify_screen "$CAPS_TMUX" "$stale" 1)
  [ "$out" = empty ] \
  ...
  pass "xo_composer_classify_screen: a lower dead shell invalidates only cursorless stale composers"
}
...
test_cursorless_bare_wrap_region_classifies() {
  ...
  out=$(xo_composer_classify_screen "$CAPS_STYLED" "$ghost")
  [ "$out" = empty ] || fail "cursorless ghost wrap on herdr should be empty, got '$out'"
  out=$(xo_composer_classify_screen "$CAPS_STYLED_NOID" "$ghost")
  [ "$out" = empty ] || fail "cursorless ghost wrap on zellij should be empty, got '$out'"
  pass "xo_composer_classify_screen: cursorless bare wrap regions participate in verdicts"
}
...
test_cursorless_container_rejects_contiguous_lower_activity() {
  ...
  pass "xo_composer_classify_screen: cursorless containers reject only contiguous unclaimed activity"
}
...
test_bottom_most_candidate_wins() {
  ...
  pass "xo_composer_classify_screen: the bottom-most candidate wins; stale banners cannot"
}

test_incomplete_lower_box_invalidates_stale_candidate() {
  local screen out
  screen=$'╭────────────────────────╮\n│ ❯                      │\n╰────────────────────────╯\nstartup complete\n╭────────────────────────╮\n│ ❯ clipped live draft  '
  out=$(xo_composer_classify_screen "$CAPS_PLAIN" "$screen")
  [ "$out" = unknown ] \
    || fail "an incomplete lower box must invalidate an earlier empty box, got '$out'"
  pass "xo_composer_classify_screen: incomplete lower structure invalidates stale boxes"
}

test_titled_bottom_requires_matching_width() {
  local screen out
  screen=$'╭────────────────────────╮\n│ ❯                      │\n╰─ Grok ─╯'
  out=$(xo_composer_classify_screen "$CAPS_TMUX" "$screen" 1)
  [ "$out" = unknown ] \
    || fail "a short titled bottom must not prove an empty box, got '$out'"
  pass "xo_composer_classify_screen: titled bottoms retain full box geometry"
}

test_cursor_on_proven_box_bottom_classifies_content() {
  local screen out
  screen=$'╭────────────────────────╮\n│ ❯                      │\n╰────────────────────────╯'
  out=$(xo_composer_classify_screen "$CAPS_TMUX" "$screen" 2)
  [ "$out" = empty ] \
    || fail "a cursor on a proven box bottom must classify its content, got '$out'"
  pass "xo_composer_classify_screen: a proven box tolerates a bottom-border cursor"
}

test_selected_content_is_composer_scoped_and_wrap_normalized() {
  local screen out
  screen=$'hello captain in transcript\n╭────────────────────╮\n│ unrelated          │\n│ draft               │\n╰────────────────────╯'
  out=$(xo_composer_extract_selected_content "$CAPS_STYLED_NOID" "$screen")
  [ "$out" = 'unrelated draft' ] \
  ...
  if out=$(xo_composer_extract_selected_content "$CAPS_STYLED_NOID" "$screen"); then
  ...
  pass "xo_composer_extract_selected_content: scopes user content and excludes furniture"
}
...
test_queued_enter_verdict_busy_pending_is_empty() {
  local out
  out=$(xo_composer_queued_enter_verdict pending busy)
  [ "$out" = empty ] || fail "busy + proven pending must be queued delivery (empty), got '$out'"
  pass "xo_composer_queued_enter_verdict: pending + busy returns empty (queued Enter)"
}

test_queued_enter_verdict_idle_pending_stays_pending() {
  local out
  out=$(xo_composer_queued_enter_verdict pending idle)
  [ "$out" = pending ] || fail "idle + proven pending must stay a genuine swallow, got '$out'"
  out=$(xo_composer_queued_enter_verdict pending unknown)
  [ "$out" = pending ] || fail "unknown busy is not proof of a queue, got '$out'"
  pass "xo_composer_queued_enter_verdict: pending + idle/unknown stays pending"
}

test_queued_enter_verdict_does_not_convert_other_states() {
  local state out
  for state in empty pending-unproven unknown send-failed future-state; do
    out=$(xo_composer_queued_enter_verdict "$state" busy)
    [ "$out" = "$state" ] || fail "busy must not convert '$state', got '$out'"
    out=$(xo_composer_queued_enter_verdict "$state" idle)
    [ "$out" = "$state" ] || fail "idle must not convert '$state', got '$out'"
  done
  pass "xo_composer_queued_enter_verdict: only proven pending is converted"
}
