...
#      crewmate harness). xo-harness.sh secondmate resolves the fallback chain
...
#      this knob existed (full backward-compat). xo-spawn.sh resolves a secondmate
...
#      xo-harness.sh secondmate-model / secondmate-effort. A bare harness-only
#      line (today's format) yields empty model/effort - full backward-compat.
#      xo-spawn.sh populates MODEL/EFFORT from those tokens for a --secondmate
...
unset CLAUDECODE PI_CODING_AGENT XO_PI_HARNESS GROK_AGENT CURSOR_AGENT CURSOR_INVOKED_AS

BASE_PATH=${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
xo_git_identity fmtest fmtest@example.com
TMP_ROOT=$(xo_test_tmproot xo-secondmate-harness)
export XO_BACKEND=tmux

# ===========================================================================
# A) xo-harness.sh secondmate resolution + fallback (deterministic detect_own)
# ===========================================================================
...
test_harness_resolution() {
  ...
  while IFS='^' read -r label crew sm exp_sm exp_crew; do
    ...
    got_sm=$(CLAUDECODE=1 XO_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/xo-harness.sh" secondmate)
    got_crew=$(CLAUDECODE=1 XO_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/xo-harness.sh" crew)
...
ROWS
  pass "A1 xo-harness.sh secondmate resolves the fallback chain; crew mode unchanged"
}
...
SH
  ...
  got=$(env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT \
    PATH="$fakebin:$BASE_PATH" CURSOR_INVOKED_AS=cursor-agent "$ROOT/bin/xo-harness.sh")
  [ "$got" = cursor ] || fail "Cursor's exact launcher marker resolved '$got', expected cursor"
  got=$(env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT \
    PATH="$fakebin:$BASE_PATH" CURSOR_INVOKED_AS=cursor "$ROOT/bin/xo-harness.sh")
  [ "$got" != cursor ] || fail "an inexact Cursor marker value was accepted as Cursor Agent CLI"
  pass "xo-harness detects only Cursor Agent CLI's exact invocation marker"
}

# ===========================================================================
# C) xo-harness.sh secondmate-model / secondmate-effort token resolution
# ===========================================================================
...
test_secondmate_model_effort_tokens() {
  ...
  while IFS='^' read -r label line exp_harness exp_model exp_effort; do
    ...
    got_h=$(CLAUDECODE=1 XO_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/xo-harness.sh" secondmate)
    got_m=$(CLAUDECODE=1 XO_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/xo-harness.sh" secondmate-model)
    got_e=$(CLAUDECODE=1 XO_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/xo-harness.sh" secondmate-effort)
...
ROWS
  pass "C1 xo-harness.sh secondmate-model/secondmate-effort resolve the optional tokens; bare harness stays empty (backward-compat)"
}
...
case "$pid:$field:${XO_TEST_SIGNED_SHAPE:-exact}" in
  100:comm=:*) printf '%s\n' '/test/Pi.app/bin/pi' ;;
  100:args=:*) printf '%s\n' 'Pi' ;;
  100:ppid=:*) printf '%s\n' 200 ;;
...
SH
  ...
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true "$ROOT/bin/xo-harness.sh")
  [ "$got" = pi ] || fail "unmarked shared signed-wrapper ancestry resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true XO_PI_HARNESS=pi-signed "$ROOT/bin/xo-harness.sh")
  [ "$got" = pi-signed ] || fail "selected signed wrapper resolved '$got', expected pi-signed"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true XO_PI_HARNESS=pi "$ROOT/bin/xo-harness.sh")
  [ "$got" = pi ] || fail "selected plain Pi resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true XO_PI_HARNESS=pi-signed-helper "$ROOT/bin/xo-harness.sh")
  [ "$got" = pi ] || fail "inexact signed selection marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT -u PI_CODING_AGENT PATH="$fakebin:$BASE_PATH" XO_PI_HARNESS=pi-signed "$ROOT/bin/xo-harness.sh")
  [ "$got" = pi ] || fail "signed selection marker without Pi's family marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true XO_TEST_SIGNED_SHAPE=plain "$ROOT/bin/xo-harness.sh")
  [ "$got" = pi ] || fail "plain Pi marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true XO_TEST_SIGNED_SHAPE=helper "$ROOT/bin/xo-harness.sh")
  [ "$got" = pi ] || fail "unrelated pi-signed-helper ancestry resolved '$got', expected pi"

  got=$(PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/xo-session-lock-lib.sh"; xo_harness_ancestry_pid' "$ROOT")
  [ "$got" = 100 ] || fail "session-lock ancestry selected '$got', expected the inner Pi engine pid 100"
  PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/xo-session-lock-lib.sh"; kill() { return 0; }; xo_harness_pid_alive 200' "$ROOT" \
    || fail "session-lock liveness rejected exact pi-signed holder"
  if PATH="$fakebin:$BASE_PATH" XO_TEST_SIGNED_SHAPE=helper bash -c \
    '. "$0/bin/xo-session-lock-lib.sh"; kill() { return 0; }; xo_harness_pid_alive 200' "$ROOT"; then
...
SH
  ...
  err="$dir/xo-harness.err"
  got=$(env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT \
    PATH="$fakebin:$BASE_PATH" "$ROOT/bin/xo-harness.sh" 2>"$err")
  [ "$got" = codex ] || fail "dash-leading shell ancestry resolved '$got', expected codex"
  [ ! -s "$err" ] || fail "xo-harness wrote basename option noise for literal -zsh: $(cat "$err")"

  err="$dir/xo-session-lock-ancestry.err"
  got=$(PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/xo-session-lock-lib.sh"; xo_harness_ancestry_pid' "$ROOT" 2>"$err")
  ...
  err="$dir/xo-session-lock-alive.err"
  PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/xo-session-lock-lib.sh"; kill() { return 0; }; xo_harness_pid_alive 5252' \
    "$ROOT" 2>"$err"; status=$?
...
test_propagate_lib() {
  ...
  assert_contains "$(cat "$stderr")" "xo-config-inherit: error: failed to remove crew-harness" \
  ...
  XO_INHERITABLE_CONFIG=crew-dispatch.json propagate_inheritable_config "$src" "$guard_repo/config" >"$stdout" 2>"$stderr" \
    || fail "guard skip should not make propagation fail"
  [ ! -s "$stdout" ] || fail "guard skip wrote to stdout"
  err_text=$(cat "$stderr")
  assert_contains "$err_text" "xo-config-inherit: warning: skipped crew-dispatch.json" \
...
# A minimal seeded secondmate home (validate_xo_home_for_spawn needs the
...
# Runs xo-spawn.sh in secondmate mode. XO_ROOT is the real repo (so xo-harness.sh
...
spawn_secondmate() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$world/home" \
    XO_STATE_OVERRIDE="$world/home/state" XO_DATA_OVERRIDE="$world/home/data" \
    XO_PROJECTS_OVERRIDE="$world/home/projects" XO_CONFIG_OVERRIDE="$world/home/config" \
    XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" "${spawn_args[@]}" >/dev/null 2>&1 || true
}
...
test_spawn_unverified_secondmate_harness_refused() {
  ...
  rc=0
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$w/home" \
    XO_STATE_OVERRIDE="$w/home/state" XO_DATA_OVERRIDE="$w/home/data" \
    XO_PROJECTS_OVERRIDE="$w/home/projects" XO_CONFIG_OVERRIDE="$w/home/config" \
    XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" sm "$sm" --secondmate >/dev/null 2>"$err" || rc=$?
...
test_spawn_cursor_secondmate_launches_with_its_primary_contract() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    XO_STATE_OVERRIDE="$w/home/state" XO_DATA_OVERRIDE="$w/home/data" \
    XO_PROJECTS_OVERRIDE="$w/home/projects" XO_CONFIG_OVERRIDE="$w/home/config" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_LAUNCH_LOG="$launchlog" XO_FAKE_PANE_PATH="$sm" \
    "$ROOT/bin/xo-spawn.sh" sm "$sm" --secondmate >/dev/null 2>&1 || rc=$?
  ...
  assert_contains "$launch" "XO_SUPERVISION_MODEL=autoarm" \
...
# `send-keys -l <cmd>` launch command into XO_FAKE_LAUNCH_LOG, mirroring the
# capture technique in xo-spawn-dispatch-profile.test.sh so the constructed
# launch command (not just meta) can be asserted on. Also answers the
# `#{pane_current_path}` probe from XO_FAKE_PANE_PATH so this same stub works
...
set -u
case "$*" in
  *"#{pane_current_path}"*) printf '%s\n' "${XO_FAKE_PANE_PATH:-}"; exit 0 ;;
esac
case "${1:-}" in
  display-message) printf 'xo\n'; exit 0 ;;
  list-windows) exit 0 ;;
  has-session|new-session|new-window|kill-window) exit 0 ;;
  send-keys)
    if [ -n "${XO_FAKE_LAUNCH_LOG:-}" ]; then
      prev=
      for a in "$@"; do
        if [ "$prev" = "-l" ]; then
          printf '%s\n' "$a" >> "$XO_FAKE_LAUNCH_LOG"
        fi
        prev=$a
      done
    fi
...
# spawn_secondmate_capture <world> <id> <home> <launchlog> [extra xo-spawn.sh args...]
...
spawn_secondmate_capture() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    XO_STATE_OVERRIDE="$world/home/state" XO_DATA_OVERRIDE="$world/home/data" \
    XO_PROJECTS_OVERRIDE="$world/home/projects" XO_CONFIG_OVERRIDE="$world/home/config" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_LAUNCH_LOG="$launchlog" \
    "$ROOT/bin/xo-spawn.sh" "$id" "$home" "$@" --secondmate
}
...
test_spawn_backend_precedence_over_inherited_config() {
  ...
  out=$(XO_BACKEND=tmux spawn_secondmate_capture \
    "$w" sm "$sm" "$launchlog" 2>&1); status=$?
  expect_code 0 "$status" \
    "XO_BACKEND=tmux should beat inherited config/backend=herdr"$'\n'"$out"
  ...
  assert_no_grep '^backend=' "$meta" \
    "XO_BACKEND=tmux did not beat inherited config/backend=herdr"
  pass "B5b spawn: XO_BACKEND wins over inherited config/backend"
}
...
test_spawn_explicit_backend_precedence_over_env_and_inherited_config() {
  ...
  out=$(XO_BACKEND=zellij spawn_secondmate_capture \
    "$w" sm "$sm" "$launchlog" --backend tmux 2>&1); status=$?
  expect_code 0 "$status" \
    "explicit --backend tmux should beat XO_BACKEND=zellij and inherited config/backend=herdr"$'\n'"$out"

  meta="$w/home/state/sm.meta"
  [ "$(cat "$sm/config/backend")" = herdr ] \
    || fail "explicit backend precedence fixture did not inherit config/backend=herdr"
  assert_no_grep '^backend=' "$meta" \
    "explicit --backend tmux did not beat XO_BACKEND=zellij and inherited config/backend=herdr"
  pass "B5c spawn: explicit --backend wins over XO_BACKEND and inherited config/backend"
}
...
test_spawned_secondmate_uses_its_harness_supervision_model() {
  ...
  for harness in codex claude; do
    ...
    make_seeded_home "$sm" sm
    spawn_secondmate_capture "$w" sm "$sm" "$launchlog" >/dev/null 2>&1
    xo_write_meta "$sm/state/task.meta" "window=xo:xo-task" "kind=ship"
...
#!/usr/bin/env bash
XO_ROOT_OVERRIDE="$sm" "$ROOT/bin/xo-guard.sh"
SH
...
test_spawn_fallback_chain_and_crew_scout_unaffected() {
  ...
  xo_git_worktree "$proj" "$wt" "wt-crew"
...
EOF
  ...
  PATH="$fakebin:$BASE_PATH" TMUX="fake,1,0" CLAUDECODE=1 \
    ...
    XO_PROJECTS_OVERRIDE="$home/projects" XO_CONFIG_OVERRIDE="$home/config" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$wt" XO_FAKE_LAUNCH_LOG="$launchlog" \
    "$ROOT/bin/xo-spawn.sh" "$id" "$proj" --mode no-mistakes --yolo off >/dev/null 2>&1
...
# A PRIMARY xo repo on main with one commit + a home dir, mirroring the
...
record_live_watcher_fixture() {
  ...
  identity=$(XO_STATE_OVERRIDE="$home/state" bash -c '. "$1"; xo_pid_identity "$2"' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$$") || fail "could not identify the live watcher fixture"
  mkdir "$home/state/.watch.lock"
  printf '%s\n' "$$" > "$home/state/.watch.lock/pid"
  printf '%s\n' "$home" > "$home/state/.watch.lock/xo-home"
  printf '%s\n' "$ROOT/bin/xo-watch.sh" > "$home/state/.watch.lock/watcher-path"
...
make_fake_toolchain() {
  ...
  fakebin="$dir/fakebin"
  mkdir -p "$fakebin"
  xo_fake_exit0 "$fakebin" node chrome-devtools-axi
...
SH
  ...
  # tmux fake supports xo-send's composer-verified submit path and optional
  # XO_FAKE_TMUX_LOG / XO_FAKE_TMUX_FAIL_LITERAL for reread-nudge assertions.
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
if [ -n "${XO_FAKE_TMUX_LOG:-}" ]; then
  printf '%s\n' "$*" >> "$XO_FAKE_TMUX_LOG"
fi
case "$*" in
  list-windows*)
    sed -n 's/^window=[^:]*://p' "${XO_HOME:?}"/state/*.meta
    exit 0
    ;;
  ...
  *capture-pane*) printf '❯\n'; exit 0 ;;
  *'send-keys'*' -l '*)
    [ "${XO_FAKE_TMUX_FAIL_LITERAL:-0}" = 1 ] && exit 1
    exit 0
    ;;
  *send-keys*)
    [ "${XO_FAKE_TMUX_FAIL_LITERAL:-0}" = 1 ] && exit 1
...
run_bootstrap() {
  ...
  if [ -n "$log" ]; then
    PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
      XO_SEND_SETTLE=0 XO_FAKE_TMUX_LOG="$log" \
      "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null
  else
    PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
      XO_SEND_SETTLE=0 "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null
  fi
}
...
# Config-reread pointers now ride xo-send's durable steering inbox: the typed
...
reread_instruction_path() {
  ...
  for path in "$state"/.xo-inherited-config-reread.*; do
    case "$path" in
      *.pending) continue ;;
    esac
...
reread_retry_stage_path() {
  ...
  retry_dir="$home/state/.xo-inherited-config-reread-retry/$id"
  for path in "$retry_dir"/.xo-inherited-config-reread.*; do
    [ -f "$path" ] && [ ! -L "$path" ] || continue
    latest="$path"
  done
  [ -n "$latest" ] || return 1
...
reread_retry_report_path() {
  local home=$1 id=$2 retry_dir path latest=
  retry_dir="$home/state/.xo-inherited-config-reread-retry/$id"
  for path in "$retry_dir"/.xo-inherited-config-reread.*.report; do
    [ -f "$path" ] && [ ! -L "$path" ] || continue
    latest="$path"
  done
  [ -n "$latest" ] || return 1
...
assert_no_reread_pending() {
  local home=$1 state path
  state="$home/state"
  for path in "$state"/.xo-inherited-config-reread.*.pending; do
...
# inheritance, so it is pinned in tests/xo-backend-herdr.test.sh instead.
sm_presentation_verdict() {  # <config-dir> -> on|off
  bash -c '
    . "$0/bin/backends/herdr.sh"
    case "$(xo_backend_herdr_presentation_preference "$1")" in
      off) printf "off\n" ;;
      *) printf "on\n" ;;
    esac
  ' "$ROOT" "$1" 2>/dev/null
...
test_config_push_exits_nonzero_on_copy_error() {
  ...
  err_text=$(cat "$err")
  assert_contains "$err_text" "xo-config-inherit: error: failed to copy crew-harness" \
...
This file is main-authoritative in the main xo home.
In secondmate homes it is read-only in secondmate homes and must not be edited there.
Route new captain-preference discoveries to the main xo through marked status or a document pointer.
EOF
}
...
test_config_reread_per_home_changed_sets_and_exact_bytes() {
  ...
  # Path order follows XO_INHERITABLE_CONFIG.
  awk '
...
test_config_reread_isolation_and_absent_and_send_failure() {
  ...
  xo_config_write_reread_instruction "$w/beta" "$report" "$w/beta/state/.xo-inherited-config-reread-absent" \
    || fail "ABSENT instruction write failed"
  assert_contains "$(cat "$w/beta/state/.xo-inherited-config-reread-absent")" \
    $'-----BEGIN config/crew-dispatch.json-----\nABSENT\n-----END config/crew-dispatch.json-----' \
    "helper ABSENT representation"
  assert_not_contains "$(cat "$w/beta/state/.xo-inherited-config-reread-absent")" "captain-shared" \
    "helper must ignore captain-shared even when report says pushed"
  assert_not_contains "$(cat "$w/beta/state/.xo-inherited-config-reread-absent")" "config/crew-harness" \
  ...
  out=$(PATH="$(make_fake_toolchain "$w"):$BASE_PATH" \
    XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-config-push.sh" 2>"$err"); status=$?
  ...
  out2=$(PATH="$(make_fake_toolchain "$w"):$BASE_PATH" \
    XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-config-push.sh" 2>"$err"); status2=$?
...
#!/usr/bin/env bash
case "\$*" in
  *"$alpha_state/.xo-inherited-config-reread."*) exit 1 ;;
esac
exec "$real_mv" "\$@"
SH
  chmod +x "$fakebin/mv"
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    XO_SEND_SETTLE=0 "$ROOT/bin/xo-config-push.sh" 2>&1); status=$?
...
test_config_reread_write_failure_retains_exact_retry_generation() {
  ...
  fakebin=$(make_fake_toolchain "$w")
  real_mv=$(command -v mv)
  mkdir -p "$w/home/state/.xo-inherited-config-reread-retry/sm"
  retry_dir=$(cd "$w/home/state/.xo-inherited-config-reread-retry/sm" && pwd -P)
  cat > "$fakebin/mv" <<SH
#!/usr/bin/env bash
target=
for arg in "\$@"; do target="\$arg"; done
case "\$target" in
  *"$retry_dir"/.xo-inherited-config-reread.*)
    case "\$target" in
      *.exact) ;;
      *) exit 1 ;;
    esac
...
for arg in "\$@"; do target="\$arg"; done
case "\$target" in
  *"$retry_dir"/.xo-inherited-config-reread.*) exit 1 ;;
esac
exec "$real_mv" "\$@"
SH
  chmod +x "$fakebin/mv"
...
SH
  ...
  (
    PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
      XO_SEND_SETTLE=0 XO_FAKE_TMUX_LOG="$log" \
      "$ROOT/bin/xo-config-push.sh" > "$first_out" 2>&1
  ) &
  first_pid=$!
  ...
  PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    XO_SEND_SETTLE=0 XO_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/xo-config-push.sh" > "$second_out" 2>&1
  second_status=$?
...
test_config_reread_full_retry_queue_drains_before_new_push() {
  ...
  printf 'old\n' > "$w/sm/config/crew-harness"
  printf 'new\n' > "$w/home/config/crew-harness"
  retry_dir="$w/home/state/.xo-inherited-config-reread-retry/sm"
  mkdir -p "$retry_dir"
  for n in $(seq -w 1 16); do
    path="$retry_dir/.xo-inherited-config-reread.20260721T000000.$n"
    printf 'generation-%s\n' "$n" > "$path"
    chmod 0600 "$path"
  done
  fakebin=$(make_fake_toolchain "$w")
  ...
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    XO_SEND_SETTLE=0 XO_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/xo-config-push.sh" 2>&1); status=$?
...
test_config_reread_cleanup_runs_after_mixed_delivery_failure() {
  ...
  fail_path="$state_real/.xo-inherited-config-reread.9999-fail"
  for n in $(seq -w 1 18); do
    path="$state_real/.xo-inherited-config-reread.00$n"
    printf 'generation-%s\n' "$n" > "$path"
    chmod 0600 "$path"
  done
  printf 'failed-generation\n' > "$fail_path"
  chmod 0600 "$fail_path"
  for path in "$state_real"/.xo-inherited-config-reread.*; do
    xo_config_reread_mark_pending "$path" "$path.pending" \
...
SH
  ...
  : > "$report"
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    XO_SEND_SETTLE=0 xo_config_send_reread_nudge sm "$w/sm" "$report" 2>&1); status=$?
...
test_config_reread_stops_after_failed_generation() {
  ...
  mkdir -p "$w/sm/state"
  state_real=$(cd "$w/sm/state" && pwd -P)
  old="$state_real/.xo-inherited-config-reread.0000-fail"
  new="$state_real/.xo-inherited-config-reread.0001-new"
  printf 'old-generation\n' > "$old"
  printf 'new-generation\n' > "$new"
  chmod 0600 "$old" "$new"
  xo_config_reread_mark_pending "$old" "$old.pending" \
    || fail "could not mark older generation pending"
  xo_config_reread_mark_pending "$new" "$new.pending" \
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_SEND_SETTLE=0 XO_FAKE_TMUX_LOG="$log" \
    xo_config_send_reread_nudge sm "$w/sm" "$report" 2>&1); status=$?
...
test_bootstrap_detect_only_does_not_create_state() {
  ...
  fakebin=$(make_fake_toolchain "$w")
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    XO_STATE_OVERRIDE="$detect_state" XO_BOOTSTRAP_DETECT_ONLY=1 \
    "$ROOT/bin/xo-bootstrap.sh" 2>&1); status=$?
...
test_config_reread_skips_when_unchanged_and_reads_after_push() {
  ...
  instr="$w/sm/state/.xo-inherited-config-reread-dest"
  xo_config_write_reread_instruction "$w/sm" "$report" "$instr" \
...
-----END config/crew-harness-----' \
  ...
  pending_instruction="$w/sm/state/.xo-inherited-config-reread.20260721T000000.01"
  printf '%s\n' generation > "$pending_instruction"
  xo_config_reread_mark_pending "$pending_instruction" "$pending_instruction.pending" \
    || fail "could not create bounded-lifecycle pending marker"
  for n in $(seq -w 2 18); do
    path="$w/sm/state/.xo-inherited-config-reread.20260721T000000.$n"
    printf '%s\n' generation > "$path"
    chmod 0600 "$path"
  done
  xo_config_reread_cleanup_sent "$w/sm"
  count=0
  for path in "$w/sm/state"/.xo-inherited-config-reread.*; do
    case "$path" in
      *.pending) continue ;;
    esac
    [ -f "$path" ] && [ ! -L "$path" ] || continue
...
test_config_reread_bootstrap_path_and_spawn_flexibility() {
  ...
  # xo-spawn still permits a conscious explicit runtime outside the config
  ...
  printf '%s\n' $'crew-harness\tpushed\t' > "$report"
  stale="$sm/state/.xo-inherited-config-reread.spawn-stale"
  xo_config_write_reread_instruction "$sm" "$report" "$stale" \
    || fail "could not create spawn stale reread generation"
  xo_config_reread_mark_pending "$stale" "$stale.pending" \
...
test_bootstrap_respawns_before_config_reread() {
  ...
  printf '%s\n' $'crew-harness\tpushed\t' > "$report"
  stale="$w/sm/state/.xo-inherited-config-reread.stale-generation"
  xo_config_write_reread_instruction "$w/sm" "$report" "$stale" \
...
cat > "$w/main/bin/xo-spawn.sh" <<SH
#!/usr/bin/env bash
. '$w/main/bin/xo-config-inherit-lib.sh'
printf '%s' spawn >> '$log'
printf '%s' codex > '$w/sm/config/crew-harness'
printf '%s\n' 7500 > '$w/sm/config/startup-memory-budget'
SH
  chmod +x "$w/main/bin/xo-spawn.sh"
  fakebin=$(make_fake_toolchain "$w")
  cat > "$fakebin/tmux" <<SH
#!/usr/bin/env bash
case "\$*" in
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" XO_HOME="$w/home" XO_ROOT_OVERRIDE="$w/main" \
    XO_SEND_SETTLE=0 XO_FAKE_TMUX_LOG="$log" \
    "$ROOT/bin/xo-bootstrap.sh" >/dev/null 2>&1
  ...
  xo_config_reread_discard_pending "$w/sm" || fail "could not clean respawn test generation"
...
test_spawn_quarantines_pending_rereads_on_cleanup_failure() {
  ...
  quarantine_root="$sm/state/.xo-inherited-config-reread-quarantine"
...
#!/usr/bin/env bash
case "\$*" in
  *'.xo-inherited-config-reread.'*) exit 1 ;;
esac
exec "$real_rm" "\$@"
SH
  chmod +x "$fakebin/rm"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    XO_PROJECTS_OVERRIDE="$w/home/projects" XO_CONFIG_OVERRIDE="$w/home/config" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_LAUNCH_LOG="$launchlog" \
    "$ROOT/bin/xo-spawn.sh" sm "$sm" --secondmate 2>&1); status=$?
  ...
  xo_config_reread_quarantine_pending "$sm" sm "$w/home" || true
...
echo "# all xo-secondmate-harness tests passed"
