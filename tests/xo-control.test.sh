#!/usr/bin/env bash
# xo-control.sh: the agent lifecycle CONTROL plane.
#
...
TMP_ROOT=$(xo_test_tmproot xo-control)
mkdir -p "$TMP_ROOT"
...
#
# A tmux stub whose whole model is four files under $XO_FAKE_DIR:
...
# `becomes` (a new agent came up). XO_FAKE_NEVER_DIES suppresses the first, so
...
case "${1:-}" in
  send-keys)
    ...
    if [ "$literal" = 1 ]; then
      ...
      printf '%s\n' "$payload" >> "$D/keys"
      if [ -n "${XO_FAKE_INTERRUPT_STOPS_AGENT:-}" ] \
         && { [ "$payload" = Escape ] || [ "$payload" = C-c ]; }; then
        printf 'zsh' > "$D/command"
      fi
      if [ "$payload" = Escape ] && [ -n "${XO_FAKE_MUSE_LOG:-}" ]; then
        if [ -n "${XO_FAKE_MUSE_DISAPPEAR_BEFORE_ACK:-}" ]; then
          : > "$D/muse-ack-pending"
        else
          printf '%s\n' '{"schema_version":1,"payload_type":"runtime.session","payload":{"kind":"run","run_id":"run-1","event":{"kind":"terminal","terminal":"cancelled","reason":null}}}' >> "$XO_FAKE_MUSE_LOG"
        fi
      fi
    fi
    exit 0 ;;
...
SH
  ...
  cat > "$fb/sleep" <<'SH'
#!/usr/bin/env bash
if [ -n "${XO_FAKE_MUSE_DISAPPEAR_BEFORE_ACK:-}" ] \
   && [ -e "$XO_FAKE_DIR/muse-ack-pending" ]; then
  rm -f "$XO_FAKE_DIR/muse-ack-pending"
  printf 'zsh' > "$XO_FAKE_DIR/command"
  printf '%s\n' '{"schema_version":1,"payload_type":"runtime.session","payload":{"kind":"run","run_id":"run-1","event":{"kind":"terminal","terminal":"cancelled","reason":null}}}' >> "$XO_FAKE_MUSE_LOG"
fi
exit 0
SH
...
# run_control <case-dir> <args...>: run xo-control against the case's home with
...
run_control() {
  ...
  env PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" XO_FAKE_DIR="$dir/fake" \
    XO_CONTROL_POLL=0.01 XO_CONTROL_SETTLE_WAIT=0.05 \
    XO_CONTROL_EXIT_WAIT=0.05 XO_CONTROL_LAUNCH_WAIT=0.05 \
    XO_FAKE_MUSE_LOG="${XO_FAKE_MUSE_LOG:-}" \
    XO_FAKE_MUSE_DISAPPEAR_BEFORE_ACK="${XO_FAKE_MUSE_DISAPPEAR_BEFORE_ACK:-}" \
    XO_FAKE_INTERRUPT_STOPS_AGENT="${XO_FAKE_INTERRUPT_STOPS_AGENT:-}" \
    "$CONTROL" "$@" 2>&1
}
...
test_exit_types_each_harness_verified_command() {
  ...
  pass "xo-control exit: every verified harness gets its own verified exit command"
}
...
test_interrupt_sends_each_harness_verified_key() {
  ...
  pass "xo-control interrupt: every verified harness gets its own verified key and repeat count"
}
...
test_harness_family_resolution() {
  ...
  [ "$(xo_control_harness_family pi-signed)" != "$(xo_control_harness_family pi)" ] \
    || fail "pi-signed must not collapse into pi"
  # omp is exact: an omp* prefix would claim unrelated commands such as ompd.
  xo_control_harness_family ompd \
    && fail "ompd must not be guessed into the omp adapter"
  xo_control_harness_family comp \
    && fail "comp must not be guessed into the omp adapter"
  pass "xo-control-lib: a recorded harness resolves to its verified adapter without guessing"
}
...
test_prefixed_recorded_harness_reaches_each_control_verb() {
  ...
  pass "xo-control: prefixed recorded harnesses reach interrupt and exit mechanics"
}
...
test_opencode_interrupts_twice_and_others_once() {
  ...
  pass "xo-control interrupt: opencode needs a double Escape, claude a single one"
}
...
test_unverified_harness_is_refused() {
  ...
  pass "xo-control: a harness with no verified control mechanics is refused, not guessed at"
}
...
test_backend_key_capability_matrix() {
  ...
  for backend in tmux herdr zellij cmux; do
    ...
    for key in Escape Enter C-c C-u; do
      xo_control_backend_supports_key "$backend" "$key" \
        || fail "$backend should be able to deliver $key"
    done
  done
  xo_control_backend_supports_key orca Escape \
    && fail "orca's terminal API has no Escape and must not claim it"
  xo_control_backend_supports_key orca C-u \
    && fail "orca's terminal API has no composer clear and must not claim one"
  xo_control_backend_supports_key orca C-c || fail "orca should deliver C-c"
  xo_control_backend_supports_key orca Enter || fail "orca should deliver Enter"
  pass "xo-control-lib: the backend key matrix matches each adapter's real send-key surface"
}
...
# supervision protocol, so bin/xo-spawn.sh refuses it for a secondmate, and
...
test_harness_kind_capability() {
  local harness
  for harness in $VERIFIED_HARNESSES; do
    xo_control_harness_supports_kind "$harness" ship \
      || fail "$harness should be able to run a ship task"
    xo_control_harness_supports_kind "$harness" scout \
      || fail "$harness should be able to run a scout task"
  done
  xo_control_harness_supports_kind muse secondmate \
  ...
  for harness in claude codex opencode pi pi-signed grok kimi omp; do
    xo_control_harness_supports_kind "$harness" secondmate \
      || fail "$harness should be able to run a secondmate"
  done
  xo_control_harness_supports_kind someagent ship \
    && fail "an unverified harness must not claim any kind"
  pass "xo-control-lib: adapter capability is per task kind, not per adapter alone"
}
...
test_orca_refuses_an_escape_harness_interrupt() {
  ...
  sed 's|^window=.*|window=xo-t1|' "$dir/home/state/t1.meta.new" > "$dir/home/state/t1.meta"
  out=$(run_control "$dir" t1 interrupt); rc=$?
  ...
  pass "xo-control interrupt: a backend that cannot deliver the harness's key refuses instead of sending another"
}
...
test_unverified_state_backends_refuse_stop_verbs() {
  ...
  pass "xo-control: a backend that cannot prove an agent stopped refuses exit and relaunch"
}

test_state_verified_backends_are_exactly_tmux_and_herdr() {
  xo_control_backend_state_verified tmux || fail "tmux has a recovery-grade classifier"
  xo_control_backend_state_verified herdr || fail "herdr has a recovery-grade classifier"
  local backend
  ...
  pass "xo-control-lib: stop-proving verbs are gated on the backends that really classify agent state"
}
...
test_window_label_is_refused_with_the_exact_id() {
  ...
  add_task "$dir" t1 claude
  alive_as "$dir" claude
  out=$(run_control "$dir" xo-t1 exit); rc=$?
  ...
  pass "xo-control: a legacy window label is refused and the exact task id is named"
}
...
test_explicit_endpoint_is_refused() {
  ...
  add_task "$dir" t1 claude
  alive_as "$dir" claude
  out=$(run_control "$dir" "fmses:xo-t1" exit); rc=$?
  ...
  [ -z "$(literals "$dir")" ] || fail "a refused target must receive no bytes"
  pass "xo-control: an explicit backend endpoint is never a control target"
}
...
test_unknown_task_is_refused() {
  ...
  pass "xo-control: an unrecorded task id is refused"
}
...
test_record_bound_to_another_task_is_refused() {
  ...
  pass "xo-control: a record whose endpoint identity names another task is refused"
}
...
test_remote_secondmate_is_refused_by_placement() {
  ...
  pass "xo-control: a remotely placed secondmate is refused by placement, not by a metadata complaint"
}

hold_lifecycle_lock() {  # <lock-path>
  local lifecycle_lock_path=$1
  . "$ROOT/bin/xo-wake-lib.sh"
  xo_lock_try_acquire "$lifecycle_lock_path" || return 1
  sleep 30
}
...
test_interrupt_and_exit_lock_before_task_state_resolution() {
  ...
  pass "xo-control: interrupt and exit lock before task-state resolution"
}
...
test_verb_allowlist_is_closed() {
  ...
  pass "xo-control: the verb list is closed - no raw keys, arbitrary text, or clear verb"
}
...
test_resume_is_refused_with_its_reason() {
  ...
  pass "xo-control: resume is refused with the determinism reason and the alternative"
}
...
test_relaunch_only_flags_are_rejected_on_other_verbs() {
  ...
  pass "xo-control: profile and note flags belong to relaunch only"
}
...
test_already_stopped_exit_is_idempotent() {
  ...
  pass "xo-control exit: an already-stopped agent is idempotent success with no bytes sent"
}
...
test_missing_endpoint_refuses() {
  ...
  pass "xo-control exit: a vanished endpoint refuses instead of silently succeeding"
}
...
test_interrupt_refuses_when_no_agent_runs() {
  ...
  pass "xo-control interrupt: refuses when no agent is running rather than keying a shell"
}
...
test_ambiguous_endpoint_refuses() {
  ...
  pass "xo-control exit: an endpoint whose process cannot be attributed refuses"
}
...
test_busy_agent_is_interrupted_before_the_exit_command() {
  ...
  gen=$("$ROOT/bin/xo-busy-event.sh" arm "$dir/home/state" t1)
  ...
  pass "xo-control exit: a busy agent receives interrupt delivery before the exit command"
}
...
test_idle_agent_is_not_interrupted() {
  ...
  add_task "$dir" t1 claude
  alive_as "$dir" claude
  gen=$("$ROOT/bin/xo-busy-event.sh" arm "$dir/home/state" t1 --state idle --source xo-spawn --event seed)
  printf 'busy_gen=%s\n' "$gen" >> "$dir/home/state/t1.meta"
  out=$(run_control "$dir" t1 exit); rc=$?
  ...
  pass "xo-control exit: an idle agent goes straight to its exit command"
}
...
test_interrupt_without_acknowledgement_preserves_busy_state() {
  ...
  pass "xo-control interrupt: unconfirmed delivery preserves observed busy state"
}
...
test_muse_interrupt_confirms_adapter_acknowledgement() {
  ...
  out=$(XO_FAKE_MUSE_LOG="$log" run_control "$dir" t1 interrupt); rc=$?
  ...
  pass "xo-control interrupt: muse confirms cancellation from its session log"
}
...
test_interrupt_revalidates_agent_after_acknowledgement_wait() {
  ...
  printf 'sessions_root=%s\nworkspace_root=%s\nbinding_id=test\n' \
    "$root" "$dir/wt-t1" > "$dir/home/state/t1.muse-session"
  out=$(XO_FAKE_MUSE_LOG="$log" XO_FAKE_MUSE_DISAPPEAR_BEFORE_ACK=1 \
  ...
  pass "xo-control interrupt: postconditions are revalidated after acknowledgement polling"
}
...
test_exit_accepts_agent_stopped_by_busy_interrupt() {
  ...
  gen=$("$ROOT/bin/xo-busy-event.sh" arm "$dir/home/state" t1)
  printf 'busy_gen=%s\n' "$gen" >> "$dir/home/state/t1.meta"
  out=$(XO_FAKE_INTERRUPT_STOPS_AGENT=1 run_control "$dir" t1 exit); rc=$?
  ...
  pass "xo-control exit: an interrupt-stopped agent satisfies the gone-state postcondition"
}
...
test_agent_that_does_not_stop_fails_closed() {
  ...
  gen=$("$ROOT/bin/xo-busy-event.sh" arm "$dir/home/state" t1)
  printf 'busy_gen=%s\n' "$gen" >> "$dir/home/state/t1.meta"
  out=$(env XO_FAKE_NEVER_DIES=1 PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" \
    XO_FAKE_DIR="$dir/fake" XO_CONTROL_POLL=0.01 XO_CONTROL_EXIT_WAIT=0.05 \
  ...
  pass "xo-control exit: a stubborn agent reports delivered input and an unconfirmed exit"
}
...
test_grok_interrupt_without_acknowledgement_reports_unconfirmed() {
  ...
  pass "xo-control interrupt: grok reports delivery without claiming cancellation"
}
...
test_grok_idle_footer_does_not_confirm_cancellation() {
  ...
  pass "xo-control interrupt: grok's idle footer does not confirm cancellation"
}
...
test_secondmate_control_command_carries_no_marker() {
  ...
  printf '%s\n' domain > "$dir/wt-domain/.xo-secondmate-home"
  alive_as "$dir" claude
  ...
  pass "xo-control: a lifecycle command to a secondmate is unmarked and opens no reply expectation"
}

test_xo_send_still_marks_the_same_secondmate_task() {
  local dir log out rc
  dir=$(new_case sm-send)
  add_task "$dir" domain claude secondmate
  log="$dir/fake/sendlog"
  : > "$log"
  out=$(env PATH="$dir/fakebin:$PATH" XO_HOME="$dir/home" XO_FAKE_DIR="$dir/fake" \
    XO_SEND_SETTLE=0 XO_ROOT_OVERRIDE="$dir/home" \
    "$SEND" domain "audit the build" 2>&1); rc=$?
  expect_code 0 "$rc" "xo-send to a secondmate should still succeed"$'\n'"$out"
  # The marked steer rides xo-send's durable inbox plane; only the doorbell is
