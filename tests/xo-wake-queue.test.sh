#!/usr/bin/env bash
# tests/xo-wake-queue.test.sh - wake-queue losslessness (the queue safety matrix):
...
# lives in xo-watcher-lock.test.sh; daemon classification/injection in
# xo-daemon.test.sh.
set -u
...
GRANT="$ROOT/bin/xo-wake-grant.sh"
GUARD="$ROOT/bin/xo-guard.sh"

TMP_ROOT=$(xo_test_tmproot xo-wake-tests)
...
test_concurrent_append_and_drain() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" &
  pids="$pids $!"
  for pid in $pids; do
    wait "$pid" || fail "concurrent append/drain subprocess failed"
  done
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" 2> "$dir/drain-two.err" || fail "final drain failed"
...
test_signal_catchup_without_running_watcher() {
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2> "$drain_err" || fail "drain after first signal failed"
...
test_stale_enqueue_before_suppressor() {
  ...
  printf 'done: ready in branch xo/stale\n' > "$state/stale.status"
  prime_status_seen "$state" "$state/stale.status"
  key=$(printf '%s' "$window" | tr ':/.' '___')
  pane_hash=$(hash_text "idle prompt")
  printf '%s' "$pane_hash" > "$state/.hash-$key"
  printf '1\n' > "$state/.count-$key"
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" XO_STATE_OVERRIDE="$state" XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after stale wake failed"
...
test_not_working_stale_enqueue_before_suppressor() {
  ...
  # fake xo-crew-state.sh the watcher reads via XO_CREW_STATE_BIN.)
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$window" XO_FAKE_TMUX_CAPTURE="$capture_file" \
    ...
    XO_STALE_ESCALATE_SECS=999 XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after the immediate stale wake failed"
...
SH
  ...
  XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-check-register.sh" task >/dev/null \
    || fail "could not register queue custom check"
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=0 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after check wake failed"
...
test_atomic_double_drain() {
  ...
  append_wake "$state" stale 's:xo-task' 'stale: s:xo-task' || fail "stale append failed"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out1" 2> "$dir/drain-one.err" &
  pid1=$!
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" 2> "$dir/drain-two.err" &
  pid2=$!
  ...
  leftover=$(XO_STATE_OVERRIDE="$state" "$DRAIN" | awk -F '\t' 'NF == 5 { count++ } END { print count + 0 }')
...
test_drain_dedupes_obvious_duplicates() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "dedupe drain failed"
...
# watcher liveness via xo-guard.sh: a lapsed re-arm chain then surfaces even on a
...
SH
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$state" XO_FAKE_TMUX_WINDOW='xo:xo-mate' \
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$dir/watch-first.out" 2> "$dir/watch-first.err" || true
  [ ! -s "$state/.wake-queue" ] \
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    ...
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$dir/watch-progress.out" 2> "$dir/watch-progress.err" || true
  [ ! -s "$state/.wake-queue" ] \
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    ...
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$out" 2> "$dir/watch-stalled.err" || true
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/drain.out" 2> "$dir/drain.err" \
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    ...
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$dir/watch-next.out" 2> "$dir/watch-next.err" || true
  [ ! -s "$state/.wake-queue" ] \
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    ...
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$dir/watch-refrozen.out" 2> "$dir/watch-refrozen.err" || true
...
EOF
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    ...
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$dir/watch-second.out" 2> "$dir/watch-second.err" || true
  [ ! -s "$state/.wake-queue" ] \
...
SH
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    ...
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$dir/watch-old.out" 2> "$dir/watch-old.err" || true
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    ...
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$dir/watch-regen.out" 2> "$dir/watch-regen.err" || true
  [ ! -s "$state/.wake-queue" ] \
  ...
  PATH="$fakebin:$PATH" XO_FAKE_NOW_FILE="$dir/now" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    ...
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 1 > "$dir/watch-regen-frozen.out" 2> "$dir/watch-regen-frozen.err" || true
...
SH
  ...
  "$ROOT/bin/xo-busy-event.sh" arm "$state" mate >/dev/null \
    || fail "could not arm the mate's busy contract"

  PATH="$fakebin:$PATH" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$state" XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 \
    XO_SIGNAL_GRACE=0 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 4 \
  ...
  "$ROOT/bin/xo-busy-event.sh" apply "$state" mate idle --current-gen \
...
SH
  ...
  PATH="$fakebin:$PATH" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$state" XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 \
    XO_SIGNAL_GRACE=0 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 2 \
...
test_acknowledged_stall_publication_survives_pre_marker_crash() {
  ...
  PATH="$fakebin:$PATH" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
    XO_STATE_OVERRIDE="$state" XO_FAKE_TMUX_WINDOW='xo:xo-mate' \
    XO_FAKE_TMUX_LOG="$dir/tmux.log" XO_FAKE_TMUX_CAPTURE="$dir/fake-tmux/pane.txt" \
    XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 2 > "$out" 2> "$dir/watch.err" || true
...
test_empty_prefix_mate_preserves_other_mate_receipt() {
  ...
  printf 'ios\n' > "$empty/.xo-secondmate-home"
  printf 'ios-ui\n' > "$stalled/.xo-secondmate-home"
  printf 'window=xo:xo-ios\nkind=secondmate\nhome=%s\n' "$empty" > "$state/ios.meta"
  printf 'window=xo:xo-ios-ui\nkind=secondmate\nhome=%s\n' "$stalled" > "$state/ios-ui.meta"
  ...
  while [ "$round" -le 2 ]; do
    PATH="$fakebin:$PATH" XO_HOME="$dir" XO_ROOT_OVERRIDE="$ROOT" \
      XO_STATE_OVERRIDE="$state" XO_FAKE_TMUX_WINDOW='' \
      XO_FAKE_TMUX_LOG="$dir/tmux.log" XO_FAKE_TMUX_CAPTURE="$dir/fake-tmux/pane.txt" \
      XO_SECONDMATE_WAKE_STALL_SECS=1 XO_POLL=1 XO_SIGNAL_GRACE=0 \
      XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
      "$ROOT/bin/xo-watch-checkpoint.sh" --seconds 2 \
...
test_drain_asserts_watcher_liveness() {
  ...
  state="$dir/state"
  err="$dir/drain.err"
  printf 'window=test:xo-x\nkind=ship\n' > "$state/x.meta"
  XO_STATE_OVERRIDE="$state" "$DRAIN" >/dev/null 2> "$err" || fail "drain failed while asserting liveness"
  ...
  : > "$err"
  identity=$(XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_pid_identity "$2"' _ "$ROOT/bin/xo-wake-lib.sh" "$$") \
    || fail "could not identify the live watcher fixture"
  mkdir "$state/.watch.lock"
  printf '%s\n' "$$" > "$state/.watch.lock/pid"
  printf '%s\n' "$dir" > "$state/.watch.lock/xo-home"
  printf '%s\n' "$WATCH" > "$state/.watch.lock/watcher-path"
  printf '%s\n' "$identity" > "$state/.watch.lock/pid-identity"
  touch "$state/.last-watcher-beat"
  XO_HOME="$dir" XO_STATE_OVERRIDE="$state" XO_GUARD_GRACE=300 "$DRAIN" >/dev/null 2> "$err" \
...
if [ "${1:-}" = -MFcntl=:DEFAULT ]; then
  for arg in "$@"; do
    if [ "$arg" = "${XO_WAKE_ENRICH_SWAP_PATH:-}" ]; then
      rm -f "$arg"
      ln -s "$XO_WAKE_ENRICH_SWAP_TARGET" "$arg"
      break
    fi
  done
fi
exec "$XO_WAKE_ENRICH_REAL_PERL" "$@"
SH
  ...
  append_wake "$state" stale test:xo-task "stale: test:xo-task" || fail "stale wake append failed"
  append_wake "$state" heartbeat heartbeat heartbeat || fail "heartbeat wake append failed"

  XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_wake_print_deduped "$2"' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$state/.wake-queue" > "$expected"
  PATH="$dir/fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_WAKE_ENRICH_SWAP_PATH="$state/task.status" \
    XO_WAKE_ENRICH_SWAP_TARGET="$outside" XO_WAKE_ENRICH_REAL_PERL="$perl_bin" "$DRAIN" > "$out" \
...
test_enrichment_preserves_all_unread_lines_and_status_file_failures() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" \
...
test_slow_annotation_does_not_block_append_and_deleted_file_fails_open() {
  ...
  XO_STATE_OVERRIDE="$state" XO_WAKE_ENRICH_TEST_DELAY=3 "$DRAIN" > "$out1" &
  pid=$!
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out2" || fail "follow-up drain after concurrent append failed"
...
# the real bin/xo-wake-drain.sh, independent of the Pi SDK. This is the core
...
test_branch_actor_scoped_ack_never_swallows_a_main_owned_row() {
  ...
  append_wake "$state" stale "xo-window" "stale: xo-window" || fail "stale append failed"

  # The extension's own job (xo-branch-dispatch.ts) is granting exactly the
  # two task-local rows; this test drives the bash consume contract those
  # sequence numbers gate, independent of the Pi SDK.
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$$" actor-scope || fail "branch owner activation failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" publish actor-scope 2 3 || fail "branch grant publication failed"

  out="$dir/branch-drain.out"
  err="$dir/branch-drain.err"
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" > "$out" 2> "$err" \
  ...
  grep -Fq "$(printf '\tstale\txo-window\t')" "$out" || fail "branch drain omitted its eligible stale row"
  ...
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" --ack-through "$sequence" --recovery-generation "$generation" \
  ...
  grep -Fq "$(printf '\tstale\txo-window\t')" "$state/.wake-queue" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "main drain failed: $(cat "$err")"
...
test_main_drain_excludes_rows_already_granted_to_branch() {
  ...
  append_wake "$state" check "some-poll.check.sh" "check: some-poll.check.sh: merged" \
    || fail "main-only append failed"
  append_wake "$state" signal "task-a.status" "signal: task-a" || fail "signal append failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$$" main-excludes || fail "branch owner activation failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" publish main-excludes 2 || fail "branch grant publication failed"
...
test_main_is_never_told_to_drain_rows_only_the_branch_owns() {
  ...
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$$" held-by-branch || fail "branch owner activation failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" publish held-by-branch 1 || fail "branch grant publication failed"
  ...
  XO_STATE_OVERRIDE="$state" "$GUARD" 2> "$dir/guard-held.err" || fail "guard failed while the branch held the rows"
  ...
  XO_STATE_OVERRIDE="$state" "$GRANT" release held-by-branch || fail "branch grant release failed"
  out="$dir/main-drain-after.out"
  err="$dir/main-drain-after.err"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "main drain failed after release: $(cat "$err")"
...
SH
  ...
  PATH="$awkbin:$PATH" XO_STATE_OVERRIDE="$state" "$GUARD" 2> "$dir/unreadable.err" \
  ...
  : > "$state/.wake-queue"
  PATH="$awkbin:$PATH" XO_STATE_OVERRIDE="$state" "$GUARD" 2> "$dir/empty.err" \
...
test_unconsumable_rows_are_retired_instead_of_wedging_the_queue() {
  ...
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$$" retire-scope || fail "branch owner activation failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" publish retire-scope 1 || fail "branch grant publication failed"
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" > "$dir/branch.out" 2> "$dir/branch.err" \
  ...
  XO_STATE_OVERRIDE="$state" "$GRANT" release retire-scope || fail "branch grant release failed"

  XO_STATE_OVERRIDE="$state" "$GUARD" 2> "$dir/guard-before.err" || fail "guard failed with unusable rows queued"
  ...
  XO_STATE_OVERRIDE="$state" "$GUARD" 2> "$dir/guard-after.err" || fail "guard failed after the queue drained"
...
test_branch_grant_refuses_rows_already_claimed_by_main() {
  ...
  append_wake "$state" signal "task-a.status" "signal: task-a" || fail "signal append failed"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/main.out" 2> "$dir/main.err" \
    || fail "main presentation failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$$" branch-refuses || fail "branch owner activation failed"
  rc=0
  XO_STATE_OVERRIDE="$state" "$GRANT" publish branch-refuses 1 || rc=$?
...
test_actor_filter_precedes_same_key_deduplication() {
  ...
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$$" actor-dedup || fail "branch owner activation failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" publish actor-dedup 1 || fail "branch grant publication failed"
  append_wake "$state" signal "task-a.status" "signal: main version" || fail "main row append failed"

  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/main.out" 2> "$dir/main.err" || fail "main drain failed"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$main_sequence" --recovery-generation "$main_generation" \
    || fail "main same-key acknowledgement failed"
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" --ack-through "$branch_sequence" --recovery-generation "$branch_generation" \
...
test_main_reclaims_a_grant_whose_branch_owner_exited() {
  ...
  sleep 30 &
  owner=$!
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$owner" stale-owner || {
    kill "$owner" 2>/dev/null || true
    fail "branch owner activation failed"
  }
  XO_STATE_OVERRIDE="$state" "$GRANT" publish stale-owner 1 || {
    kill "$owner" 2>/dev/null || true
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/main.out" 2> "$dir/main.err" || fail "main reclaim drain failed"
  grep -Fq "$(printf '\tsignal\ttask-a.status\t')" "$dir/main.out" \
...
test_branch_actor_without_eligible_snapshot_refuses() {
  ...
  if XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" >/dev/null 2>"$dir/err"; then
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${XO_TEST_PUBLISH_MARKER:-}" ]; then
  exit 1
fi
...
SH
  ...
  set +e
  PATH="$fakebin:$PATH" XO_TEST_REAL_MV="$real_mv" XO_TEST_PUBLISH_MARKER="$state/.watcher-down" \
  ...
  PATH="$fakebin:$PATH" XO_TEST_REAL_MV="$real_mv" \
...
test_legacy_generationless_wake_is_adopted() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/first.out" 2> "$dir/first.err" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/replay.out" 2> "$dir/replay.err" \
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/after-ack.out" 2> "$dir/after-ack.err" \
...
test_stale_recovery_generation_cannot_touch_a_newer_episode() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$newer_sequence" \
...
stale_ack_remedy() {  # <stderr-file> -> "<seq>\t<generation>"
  ...
  seq=$(sed -n 's/^wake drain: nothing was acknowledged through [0-9][0-9]*.*run bin\/xo-wake-drain.sh --ack-through \([0-9][0-9]*\) --recovery-generation [A-Za-z0-9._-][A-Za-z0-9._-]* after handling it$/\1/p' "$1")
  generation=$(sed -n 's/^wake drain: nothing was acknowledged through [0-9][0-9]*.*run bin\/xo-wake-drain.sh --ack-through [0-9][0-9]* --recovery-generation \([A-Za-z0-9._-][A-Za-z0-9._-]*\) after handling it$/\1/p' "$1")
...
test_stale_ack_that_consumes_nothing_names_the_current_wake() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/first.out" 2> "$dir/first.err" || fail "first drain failed"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "$first_seq" --recovery-generation "$first_gen" \
    || fail "first acknowledgement failed"

  append_wake "$state" check second 'check: second wake' || fail "second append failed"
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/second.out" 2> "$dir/second.err" || fail "second drain failed"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through "${remedy%%$'\t'*}" --recovery-generation "${remedy##*$'\t'}" \
...
test_branch_stale_ack_that_consumes_nothing_names_its_granted_wake() {
  ...
  XO_STATE_OVERRIDE="$state" "$GRANT" activate "$$" branch-stale || fail "branch owner activation failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" publish branch-stale 1 || fail "first grant publication failed"
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" > "$dir/first.out" 2> "$dir/first.err" \
    || fail "first branch drain failed: $(cat "$dir/first.err")"
  first_seq=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through \([0-9][0-9]*\) --recovery-generation .*/\1/p' "$dir/first.err")
  first_gen=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--recovery-generation \([A-Za-z0-9._-][A-Za-z0-9._-]*\)$/\1/p' "$dir/first.err")
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" --ack-through "$first_seq" --recovery-generation "$first_gen" \
    || fail "first branch acknowledgement failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" release branch-stale || fail "first grant release failed"

  # The next prompt: a stale escalation for another pane, granted on its own.
  append_wake "$state" stale "xo-window-b" "stale: xo-window-b (idle 378s, possible wedge)" || fail "stale append failed"
  XO_STATE_OVERRIDE="$state" "$GRANT" publish branch-stale 2 || fail "second grant publication failed"
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" > "$dir/second.out" 2> "$dir/second.err" \
  ...
  grep "$(printf '\tstale\txo-window-b\t')" "$state/.wake-queue" >/dev/null \
    || fail "a stale branch acknowledgement consumed the granted wake"
  XO_STATE_OVERRIDE="$state" XO_SUPERVISION_ACTOR=branch "$DRAIN" --ack-through "${remedy%%$'\t'*}" --recovery-generation "${remedy##*$'\t'}" \
    || fail "the branch's printed remedy failed"
  [ ! -s "$state/.wake-queue" ] || fail "the branch's printed remedy left its wake queued"
  XO_STATE_OVERRIDE="$state" "$GRANT" deactivate "$$" branch-stale || fail "branch owner deactivation failed"
...
test_recovery_ack_failure_is_reported() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$dir/initial.out" 2> "$dir/initial.err" \
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${XO_TEST_ACK_MARKER:-}" ]; then
  exit 1
fi
exec "$XO_TEST_REAL_MV" "$@"
SH
  ...
  set +e
  PATH="$fakebin:$PATH" XO_TEST_REAL_MV="$real_mv" XO_TEST_ACK_MARKER="$state/.watcher-down" \
    XO_STATE_OVERRIDE="$state" "$DRAIN" --ack-through 0 --recovery-generation "$generation" \
...
test_interruption_before_and_after_raw_commit() {
  ...
  XO_STATE_OVERRIDE="$state" XO_WAKE_DRAIN_TEST_DELAY_BEFORE_COMMIT=5 "$DRAIN" > "$before_out" &
  pid=$!
  i=0
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$replay_out" 2> "$dir/replay.err" || fail "restored pre-commit wake did not drain"
  ...
  XO_STATE_OVERRIDE="$state" XO_WAKE_ENRICH_TEST_DELAY=5 "$DRAIN" > "$after_out" &
  pid=$!
  ...
  wait "$pid"
  set -e
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$empty_out" 2> "$dir/after-replay.err" \
...
# The guarded self-announced status append (xo_wake_status_append_self_announced)
...
test_self_announced_append_guards() {
  ...
  run_wake_lib xo_wake_status_append_self_announced "$state" "$status" \
...
test_self_held_lock_reclaims_instead_of_deadlocking() {
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    lock="$2/.fixture.lock"
    xo_lock_acquire_wait "$lock" || exit 10
    xo_lock_try_acquire "$lock" || exit 11
    xo_lock_release "$lock"
    [ ! -e "$lock" ] && [ ! -L "$lock" ] || exit 12
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$state" || rc=$?
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    ...
    xo_lock_acquire_wait "$lock" || exit 10
    ( xo_lock_try_acquire "$lock" && exit 13; exit 0 ) || exit 13
    xo_lock_release "$lock"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$state" || rc=$?
...
test_subshell_lock_ownership_without_bashpid() {
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    ...
    ( xo_lock_try_acquire "$lock" && exit 12; exit 0 ) || exit 12
    xo_lock_release "$lock"
    (
      xo_lock_acquire_wait "$lock" || exit 13
      [ "$(cat "$lock/pid")" != "$$" ] || exit 14
      xo_lock_try_acquire "$lock" || exit 15
      xo_lock_set_role "$lock" terminal-check || exit 16
      xo_lock_release "$lock"
...
#!/usr/bin/env bash
printf '%s\n' "$1" >> "$XO_HANDOFF_SLEEP_LOG"
exec "$XO_HANDOFF_REAL_SLEEP" "$@"
SH
  chmod +x "$dir/fakebin/sleep"

  XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    xo_lock_acquire_wait "$2" || exit 10
    printf "ready\n" > "$3"
    while [ ! -e "$4" ]; do sleep 0.05; done
    xo_lock_release "$2"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$lock" "$dir/holder.ready" "$dir/release-holder" &
  holder_pid=$!
  i=0
  ...
  PATH="$dir/fakebin:$PATH" XO_HANDOFF_SLEEP_LOG="$sleep_log" XO_HANDOFF_REAL_SLEEP="$real_sleep" \
    XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    xo_lock_acquire_wait_bounded "$2" 5 || exit 11
    current=${BASHPID:-$$}
    printf "%s\n" "$current" > "$3"
    while [ ! -e "$4" ]; do sleep 0.05; done
    [ "$(cat "$2/pid" 2>/dev/null || true)" = "$current" ] || exit 12
    xo_lock_release "$2"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$lock" "$dir/waiter.ready" "$dir/release-waiter" &
  waiter_pid=$!
  i=0
...
test_live_presentation_holder_is_deadlined_without_weakening_ack() {
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    xo_lock_acquire_wait "$2"
    printf "ready\n" > "$3"
    exec sleep 30
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$state/.wake-queue.lock" "$dir/queue.ready" &
  queue_holder=$!
  i=0
  ...
  start=$(date +%s)
  XO_STATE_OVERRIDE="$state" XO_STATUS_PRESENTATION_LOCK_TIMEOUT=1 \
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    ...
    printf "ready\n" > "$3"
    exec sleep 30
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$state/.status-presentation-lock" "$dir/presentation.ready" &
  presentation_holder=$!
  i=0
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    ...
    printf "ready\n" > "$3"
    exec sleep 30
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$state/.wake-queue.lock" "$dir/ack.ready" &
  ack_holder=$!
  i=0
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    ...
    shift
    xo_run_timed 1 "$@"
  ' _ "$ROOT/bin/xo-timeout-lib.sh" "$DRAIN" \
...
test_historical_annotation_skips_announced_status() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "drain failed"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "second drain failed"
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" 2> "$err" || fail "third drain failed"
