...
# harness limit, not a product one. `xo-herdr-lab.sh viewer start` now attaches
...
xo_live_gate default-on XO_HERDR_ATTACHED_VIEWER_LIVE_E2E herdr jq python3

[ -x "$LAB_HELPER" ] || { echo "skip: Herdr lab helper not executable at $LAB_HELPER"; exit 0; }

TMP_ROOT=$(xo_test_tmproot xo-herdr-attached-viewer)
FAKEBIN=$(xo_fakebin "$TMP_ROOT")
FOCUS_SWITCH_CONTROL="$TMP_ROOT/focus-switch"
ORIGINAL_PATH=$PATH
LAB_SESSION=$("$LAB_HELPER" name xo-herdr-attached-viewer)
...
OUT=$(drive xo_backend_herdr_projection_close_pane_focus_preserving "$LAB_SESSION" "$PANE_THREE_A")
STATUS=$?
...
OUT=$(drive xo_backend_herdr_projection_close_pane_focus_preserving "$LAB_SESSION" "$PANE_FOUR_A")
STATUS=$?
...
OUT=$(drive xo_backend_herdr_projection_close_pane_focus_preserving "$LAB_SESSION" "$PANE_FIVE_A")
STATUS=$?
...
FIXTURE=$(new_tab "$WS_SEVEN" xo-viewer-seeded-task) || fail "could not create the scenario 7 task tab"
...
OUT=$(drive xo_backend_herdr_workspace_prune_seeded_default_tab \
