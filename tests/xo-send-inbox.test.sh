#!/usr/bin/env bash
# tests/xo-send-inbox.test.sh - xo-send's inbox data plane.
#
# An ordinary text steer to a task recorded in this home no longer types its
# payload: xo-send appends a durable sequenced record to state/<id>.inbox/ and
# rings one constant self-describing doorbell line, best-effort. These tests
# drive the real xo-send executable over a stubbed tmux and pin:
...
TMP_ROOT=$(xo_test_tmproot xo-send-inbox)
TMP_ROOT=$(cd "$TMP_ROOT" && pwd)

# Stub tmux: logs literal typed text to XO_SEND_LOG and lets the submit and
# composer paths reach clean verdicts. XO_FAKE_TMUX_COMPOSER=pending renders a
# composer visibly holding text; XO_FAKE_TMUX_SEND_FAIL=1 fails send-keys.
...
setup_case() {  # <name> [harness] -> echoes case dir with home/state + t1 meta
  ...
  xo_write_meta "$dir/home/state/t1.meta" "window=sess:xo-t1" "kind=ship" "harness=$harness"
  printf '%s\n' "$dir"
}

run_send() {  # <case-dir> <err-file> [env...] -- <xo-send args...>
  local dir=$1 err=$2
  shift 2
  local envs=()
  while [ "$#" -gt 0 ] && [ "$1" != "--" ]; do
  ...
  env PATH="$dir/fakebin:$PATH" \
    XO_ROOT_OVERRIDE="$dir/home" XO_HOME="$dir/home" XO_SEND_LOG="$dir/send.log" \
    XO_SEND_SETTLE=0 ${envs[@]+"${envs[@]}"} \
    "$SEND" "$@" >/dev/null 2>"$err"
}

record_body() {  # <record>
  bash -c '. "$1"; xo_task_inbox_body "$2"' _ "$ROOT/bin/xo-task-inbox-lib.sh" "$2"
}
...
test_text_steer_rides_inbox() {
  ...
  assert_contains "$typed" "XO instruction waiting: list '$dir/home/state/t1.inbox'/*.msg" \
    "the doorbell should direct the worker to drain the inbox"
  case "$typed" in
    *"please rebase onto main"*) fail "the payload must never be typed:"$'\n'"$typed" ;;
  esac
  pass "xo-send inbox: the payload is recorded durably and only the doorbell is typed"
}
...
test_multiline_steer_is_legal() {
  ...
  pass "xo-send inbox: newlines are legal and the terminal can no longer truncate a steer"
}
...
test_resend_enqueues_new_sequence() {
  ...
  doorbells=$(grep -cF 'XO instruction waiting' "$dir/send.log" || true)
  ...
  pass "xo-send inbox: a re-send is a new durable record, never a retyped payload"
}

test_pending_composer_skips_ring_advisorily() {
  local dir err rc
  dir=$(setup_case pendingskip); err="$dir/send.err"
  run_send "$dir" "$err" XO_FAKE_TMUX_COMPOSER=pending -- t1 "steer past a stuck composer"; rc=$?
  ...
  pass "xo-send inbox: a visibly pending composer skips the ring, and the steer stays durably sent"
}

test_failed_ring_is_still_sent() {
  local dir err rc
  dir=$(setup_case ringfail); err="$dir/send.err"
  run_send "$dir" "$err" XO_FAKE_TMUX_SEND_FAIL=1 -- t1 "steer into a dead pane"; rc=$?
  expect_code 0 "$rc" "a failed doorbell must not fail the send"
  [ -f "$dir/home/state/t1.inbox/001.msg" ] || fail "the steer was not recorded"
  assert_contains "$(cat "$err")" "watcher will re-ring" \
    "the failed-ring notice should point at the re-ring"
  pass "xo-send inbox: a failed doorbell is still a durably sent steer"
}
...
test_harness_invocations_stay_typed() {
  ...
  pass "xo-send planes: slash and codex \$skill invocations stay typed; plain \$-text rides the inbox"
}
...
test_explicit_target_stays_typed() {
  ...
  pass "xo-send planes: an explicit backend target keeps the typed plane"
}
...
test_key_path_never_touches_inbox() {
  ...
  pass "xo-send planes: the --key lifecycle path never touches the inbox"
}
...
test_secondmate_marker_and_enqueue_delivery() {
  ...
  xo_write_secondmate_meta "$dir/home/state/domain.meta" "$dir/home" "sess:xo-domain"
  run_send "$dir" "$err" -- xo-domain "please summarize fleet health" \
  ...
  pass "xo-send inbox: a secondmate steer records marker+corr in the body and is delivered at enqueue"
}
...
source_arg=${@: -2:1}
target_arg=${@: -1}
if [ "${XO_FAIL_DELIVERY_CONFIRM:-0}" = 1 ] \
...
SH
  ...
  run_send "$dir" "$err" XO_FAIL_DELIVERY_CONFIRM=1 -- domain "durable once"; rc=$?
  # The durable record IS the delivery: even with the commit AND its recovery
  # marker both lost, the steer was delivered, so xo-send must not signal a
  ...
  pass "xo-send inbox: lost reply bookkeeping never invites a resend, and the delivered steer is never duplicated"
}
...
test_meta_lock_contention_fails_bounded() {
  ...
  bash -c '
    ...
    touch "$3"
    sleep 30
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$lock" "$marker" &
  holder=$!
  i=0
  ...
  run_send "$dir" "$err" XO_TASK_INBOX_LOCK_WAIT_SECS=0 -- t1 "must not hang"; rc=$?
  ...
  pass "xo-send inbox: metadata lock contention fails bounded without enqueue"
}
...
test_unwritable_inbox_fails_loudly() {
  ...
  run_send "$dir" "$err" -- xo-domain "this cannot be recorded"; rc=$?
  ...
  pass "xo-send inbox: an unwritable record is a loud local failure that leaves no false expectation"
}
