...
# (tests/xo-omp-harness.test.sh) can only pin over a fake API:
...
#   3. xo_watch_arm_omp starts a real watcher, an actionable close spawns a
...
xo_live_gate opt-in XO_OMP_LIVE_E2E omp node jq
...
MODEL=${XO_OMP_LIVE_MODEL:-openai-codex/gpt-6-astra}
# The guard's beacon grace for this lab. The watcher beats every XO_POLL=1s, so
...
mv "$PROJECT/bin/xo-turnend-guard.sh" "$PROJECT/bin/xo-turnend-guard.real.sh"
cat > "$PROJECT/bin/xo-turnend-guard.sh" <<SH
#!/usr/bin/env bash
payload=\$(cat)
printf '%s' "\$payload" | "\$(dirname "\$0")/xo-turnend-guard.real.sh" "\$@"
rc=\$?
printf 'rc=%s payload=%s\n' "\$rc" "\$payload" >> '$GUARD_SPY_LOG'
exit "\$rc"
SH
chmod +x "$PROJECT/bin/xo-turnend-guard.sh"
[ -f "$PROJECT/.omp/extensions/xo-primary-omp-watch.ts" ] || fail "lab checkout is missing the omp watch extension"
[ -f "$PROJECT/.omp/extensions/xo-primary-turnend-guard.ts" ] || fail "lab checkout is missing the omp turn-end extension"
...
(
  cd "$PROJECT" &&
    env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u XO_PI_HARNESS -u CURSOR_AGENT -u CURSOR_INVOKED_AS \
      -u XO_HOME -u XO_ROOT_OVERRIDE -u XO_STATE_OVERRIDE -u XO_CONFIG_OVERRIDE -u XO_DATA_OVERRIDE \
      XO_OMP_HARNESS=omp OMP_SKIP_SETUP=1 XO_POLL=1 XO_SIGNAL_GRACE=0 XO_HEARTBEAT=600 \
      XO_GUARD_GRACE="$GUARD_GRACE" \
      omp --mode rpc --no-session --cwd "$PROJECT" --config "$PROJECT/.omp/xo-worker-overlay.yml" --auto-approve \
...
rpc_send '{"id":"p1","type":"prompt","message":"From the XO session-start digest already in your context, reply with the single line that begins with SESSION START - and nothing else. Do not run any tool."}'
...
rpc_send '{"id":"p2","type":"prompt","message":"Call the xo_watch_arm_omp tool exactly once now, then reply with its result text verbatim and nothing else. Never run bin/xo-watch-arm.sh through bash."}'
...
if [ -z "$watcher_pid" ] || ! kill -0 "$watcher_pid" 2>/dev/null; then
  fail "no live watcher holds the lab home lock after xo_watch_arm_omp"
fi
pass "omp $OMP_VERSION: xo_watch_arm_omp started a live watcher through the extension"
...
wait_for_log "XO WATCHER WAKE: signal:" 240 || fail "the actionable close was not delivered to main as a watcher follow-up"
wait_for_agent_ends 3 360 || fail "omp did not finish the wake turn"
arm_calls=$(tool_call_count xo_watch_arm_omp)
[ "$arm_calls" -eq 1 ] || fail "the model re-armed from memory instead of the extension (xo_watch_arm_omp call count $arm_calls)"
...
i=0
while [ "$i" -lt 360 ]; do
  [ "$(tool_call_count xo_watch_arm_omp)" -ge 2 ] && break
  sleep 0.5
  i=$((i + 1))
done
[ "$(tool_call_count xo_watch_arm_omp)" -ge 2 ] \
  || { thaw; fail "the model did not reach for xo_watch_arm_omp after the compelled continuation"; }
...
pass "omp $OMP_VERSION: session_stop compelled the guard continuation (guard rc=2, then a stop_hook_active stop) and the model reached for xo_watch_arm_omp"
...
# XO's supervision contract, and cleanup reaps the lab either way.
exec 3>&-
i=0
while [ "$i" -lt 60 ]; do
