...
RECON="$ROOT/bin/xo-inactive-reconcile.sh"
DRAIN="$ROOT/bin/xo-wake-drain.sh"
WATCH="$ROOT/bin/xo-watch.sh"
TMP_ROOT=$(xo_test_tmproot xo-inactive-reconcile)
...
make_tools() { # <world>
  ...
  fake="$world/fakebin"
  mkdir -p "$fake"
  cat > "$fake/xo-crew-state.sh" <<'SH'
#!/usr/bin/env bash
printf 'state: %s · source: fake\n' "${XO_FAKE_CREW_STATE:-unknown}"
SH
  cat > "$fake/tmux" <<'SH'
#!/usr/bin/env bash
case "${1:-}" in
...
write_child() { # <home> <id> <status> [spawn-gen]
  ...
  xo_write_meta "$home/state/$id.meta" \
    "window=xo:xo-$id" "worktree=$home/projects/$id" "project=alpha" \
...
write_mate_meta() {
  xo_write_secondmate_meta "$MAIN/state/mate.meta" "$MATE"
...
run_reconcile() { # <home> [--startup]
  ...
  PATH="$WORLD/fakebin:$PATH" XO_ROOT_OVERRIDE="$WORLD/root" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" XO_CONFIG_OVERRIDE="$home/config" \
    XO_INACTIVE_RECONCILE_SECS=60 XO_INACTIVE_CREW_STATE_BIN="$WORLD/fakebin/xo-crew-state.sh" \
    XO_FORGE_LOG="$WORLD/forge.log" "$RECON" scan ${option:+"$option"}
}
...
run_report() { # <home> <child>
  ...
  PATH="$WORLD/fakebin:$PATH" XO_ROOT_OVERRIDE="$WORLD/root" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" XO_CONFIG_OVERRIDE="$home/config" \
    XO_INACTIVE_CREW_STATE_BIN="$WORLD/fakebin/xo-crew-state.sh" \
    XO_FORGE_LOG="$WORLD/forge.log" "$RECON" report "$child"
}
...
test_main_direct_terminal_presentation_receipt() {
  ...
  XO_FAKE_CREW_STATE='done' run_reconcile "$MAIN" --startup
  ...
  err="$WORLD/drain.err"
  XO_HOME="$MAIN" XO_STATE_OVERRIDE="$MAIN/state" "$DRAIN" >/dev/null 2> "$err"
  ...
  XO_HOME="$MAIN" XO_STATE_OVERRIDE="$MAIN/state" "$DRAIN" --ack-through "$seq" --recovery-generation "$generation"
...
test_local_secondmate_delivers_terminal_ledger_line() {
  ...
  XO_FAKE_CREW_STATE='unknown' run_reconcile "$MATE"
  ...
  XO_FAKE_CREW_STATE='done' run_reconcile "$MATE" --startup
...
test_busy_child_does_not_starve_later_ledger_outcomes() {
  ...
  XO_HOME="$MATE" XO_STATE_OVERRIDE="$MATE/state" bash -c '
    . "$1/bin/xo-wake-lib.sh"
    lock=$(xo_meta_lock_path "$XO_STATE_OVERRIDE/a-busy.meta")
...
test_terminal_line_during_state_read_yields_to_ledger_delivery() {
  ...
  cat > "$WORLD/fakebin/xo-crew-state.sh" <<'SH'
#!/usr/bin/env bash
printf 'done: completed during state read\n' >> "$XO_STATE_OVERRIDE/$1.status"
printf 'state: done · source: fake\n'
SH
  chmod +x "$WORLD/fakebin/xo-crew-state.sh"
...
test_terminal_line_after_inactive_delivery_is_not_reported_twice() {
  ...
  XO_FAKE_CREW_STATE='done' run_reconcile "$MATE"
...
test_report_avoids_scan_meta_lock_inversion() {
  ...
  XO_HOME="$MATE" XO_STATE_OVERRIDE="$MATE/state" bash -c '
    . "$1/bin/xo-wake-lib.sh"
    lock=$(xo_meta_lock_path "$XO_STATE_OVERRIDE/child.meta")
    xo_lock_acquire_wait "$lock"
    : > "$2/meta-held"
  ...
  XO_FAKE_CREW_STATE='unknown' run_reconcile "$MATE" --startup &
  scan_pid=$!
  i=0
...
test_local_secondmate_rejects_relative_parent_home() {
  ...
  (cd "$WORLD" && XO_FAKE_CREW_STATE='failed' run_reconcile "$MATE" --startup)
...
test_invalid_secondmate_marker_blocks_routing() {
  ...
  for kind in malformed symlink; do
    ...
    if [ "$kind" = malformed ]; then
      ...
      printf 'mate\n' > "$target"
      ln -s "$target" "$MATE/.xo-secondmate-home"
    fi

    out=$(XO_FAKE_CREW_STATE='failed' run_reconcile "$MATE" --startup)
    printf '%s\n' "$out" | grep -Fq 'inactive terminal outcomes remain unreconciled: invalid .xo-secondmate-home marker' \
...
test_reused_task_id_reports_each_incarnation() {
  ...
  XO_FAKE_CREW_STATE='failed' run_reconcile "$MATE" --startup
...
test_relaunch_cannot_replace_metadata_during_state_snapshot() {
  ...
  cat > "$WORLD/fakebin/xo-crew-state.sh" <<'SH'
#!/usr/bin/env bash
: > "${XO_RACE_WORLD:?}/state-started"
while [ ! -e "$XO_RACE_WORLD/state-release" ]; do sleep 0.05; done
printf 'state: failed · source: fake\n'
SH
  chmod +x "$WORLD/fakebin/xo-crew-state.sh"

  XO_RACE_WORLD="$WORLD" run_reconcile "$MATE" --startup &
  recon_pid=$!
  i=0
  ...
  XO_HOME="$MATE" XO_STATE_OVERRIDE="$MATE/state" bash -c '
    . "$1/bin/xo-wake-lib.sh"
    meta="$XO_STATE_OVERRIDE/child.meta"
    lock=$(xo_meta_lock_path "$meta")
    xo_lock_acquire_wait "$lock"
    awk '\''{ sub(/^spawn_gen=.*/, "spawn_gen=spawn-new"); print }'\'' "$meta" > "$meta.tmp"
    mv "$meta.tmp" "$meta"
    printf "working: replacement active\n" > "$XO_STATE_OVERRIDE/child.status"
    : > "$2/meta-updated"
    xo_lock_release "$lock"
  ' _ "$ROOT" "$WORLD" &
  update_pid=$!
...
test_nonterminal_and_captain_held_states_do_not_report() {
  ...
  for state in working paused parked unknown; do
    ...
    XO_FAKE_CREW_STATE="$state" run_reconcile "$MAIN" --startup
...
test_watcher_hook_and_idle_secondmate_exemption() {
  ...
  out="$WORLD/watch.out"
  PATH="$WORLD/fakebin:$PATH" XO_HOME="$MAIN" XO_STATE_OVERRIDE="$MAIN/state" \
    XO_INACTIVE_RECONCILE_SECS=60 XO_INACTIVE_CREW_STATE_BIN="$WORLD/fakebin/xo-crew-state.sh" \
    XO_FORGE_LOG="$WORLD/forge.log" XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    XO_FAKE_CREW_STATE='done' "$WATCH" > "$out" 2>&1 &
  pid=$!
  i=0
  while [ "$i" -lt 40 ]; do
  ...
  PATH="$WORLD/fakebin:$PATH" XO_HOME="$MAIN" XO_STATE_OVERRIDE="$MAIN/state" XO_POLL=1 XO_SIGNAL_GRACE=1 \
    XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$WORLD/idle.out" 2>&1 &
...
test_watcher_poll_delivers_child_ledger_line_to_parent() {
  ...
  PATH="$WORLD/fakebin:$PATH" XO_HOME="$MATE" XO_STATE_OVERRIDE="$MATE/state" XO_DATA_OVERRIDE="$MATE/data" \
    XO_CONFIG_OVERRIDE="$MATE/config" XO_INACTIVE_RECONCILE_SECS=60 \
    XO_INACTIVE_CREW_STATE_BIN="$WORLD/fakebin/xo-crew-state.sh" XO_FORGE_LOG="$WORLD/forge.log" \
    XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 \
    XO_FAKE_CREW_STATE='unknown' "$WATCH" > "$WORLD/mate-watch.out" 2>&1 &
  pid=$!
  i=0
  while [ "$i" -lt 100 ]; do
    kill -0 "$pid" 2>/dev/null || break
...
SH
  ...
  started=$(date +%s)
  XO_INACTIVE_RECONCILE_BUDGET_SECS=1 run_reconcile "$MAIN" --startup
...
test_full_scan_budget_includes_wake_lock_wait() {
  ...
  XO_HOME="$MAIN" XO_STATE_OVERRIDE="$MAIN/state" bash -c '
    . "$1/bin/xo-wake-lib.sh"
  ...
  started=$(date +%s)
  XO_INACTIVE_RECONCILE_BUDGET_SECS=1 XO_FAKE_CREW_STATE='done' run_reconcile "$MAIN" --startup
  elapsed=$(( $(date +%s) - started ))
  reap "$holder"
...
test_missing_parent_binding_names_itself() {
  ...
  out=$(XO_FAKE_CREW_STATE='done' run_reconcile "$MATE" --startup)
  case "$out" in
    *"actionable: child outcome needs parent report: child=child"*".xo-secondmate-parent"*) ;;
    *) fail "a missing parent binding did not name itself for a ledger delivery: $out" ;;
  esac
  case "$out" in
    *"actionable: inactive terminal outcome needs parent report: child=quiet"*".xo-secondmate-parent"*) ;;
...
test_notice_recovery_does_not_duplicate_wake() {
  ...
  err="$WORLD/drain.err"
  XO_HOME="$MATE" XO_STATE_OVERRIDE="$MATE/state" "$DRAIN" >/dev/null 2> "$err"
  seq=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--ack-through \([0-9][0-9]*\) --recovery-generation .*/\1/p' "$err")
  generation=$(sed -n 's/^WAKE_ACK_REQUIRED:.*--recovery-generation \([A-Za-z0-9._-][A-Za-z0-9._-]*\)$/\1/p' "$err")
  XO_HOME="$MATE" XO_STATE_OVERRIDE="$MATE/state" "$DRAIN" --ack-through "$seq" --recovery-generation "$generation"
  XO_FAKE_CREW_STATE='failed' run_reconcile "$MATE" --startup
