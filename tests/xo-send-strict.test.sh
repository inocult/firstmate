#!/usr/bin/env bash
# xo-send strict target resolution and key delivery reporting.
#
...
# silent-fallback failures: missing XO_HOME, unresolved selectors, prefixless
# herdr pane ids, dead explicit endpoints, and the healthy exact/xo-id paths.
...
TMP_ROOT=$(xo_test_tmproot xo-send-strict)
...
case "${1:-}" in
  send-keys)
    ...
    printf 'send-keys target=%s literal=%s arg=%s\n' "$target" "$literal" "${1:-}" >> "$XO_TMUX_LOG"
    # XO_FAKE_TMUX_SEND_KEY_FAIL names one key whose delivery fails, so the
    # --key exit contract can be driven both ways from the same stub.
    if [ "$literal" = 0 ] && [ -n "${XO_FAKE_TMUX_SEND_KEY_FAIL:-}" ] \
      && [ "${1:-}" = "$XO_FAKE_TMUX_SEND_KEY_FAIL" ]; then
      exit 1
    fi
    exit 0 ;;
  display-message)
    ...
    if [ -n "${XO_FAKE_TMUX_DEAD_TARGET:-}" ] && [ "$target" = "$XO_FAKE_TMUX_DEAD_TARGET" ]; then
      exit 1
    fi
    [ "$cursor" = 1 ] && { printf '1\n'; exit 0; }
    printf '%%1\n'
...
test_exact_lane_id_send_still_works() {
  ...
  xo_write_meta "$home/state/mpf-lane-m8.meta" "window=sess:xo-mpf-lane-m8" "kind=ship"

  PATH="$fb:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$home" XO_TMUX_LOG="$log" XO_SEND_SETTLE=0 \
    "$SEND" mpf-lane-m8 "lost dispatch" >/dev/null 2>"$err"; rc=$?
  expect_code 0 "$rc" "exact task id send should succeed when metadata exists"
  got=$(cat "$log")
  assert_contains "$got" "target=sess:xo-mpf-lane-m8 literal=1 arg=: XO instruction waiting" \
    "exact id should ring the doorbell at the meta target"
  assert_contains "$got" "target=sess:xo-mpf-lane-m8 literal=0 arg=Enter" "exact id should submit the doorbell with Enter"
  grep -qF 'lost dispatch' "$home/state/mpf-lane-m8.inbox/001.msg" \
    || fail "exact id should record the steer in the task inbox"
  pass "xo-send strict: exact task/lane ids resolve through home metadata"
}

test_unset_xo_home_fails() {
  local dir fb err log rc
  dir="$TMP_ROOT/nohome"; mkdir -p "$dir"
  fb=$(make_stubs "$dir"); err="$dir/send.err"; log="$dir/tmux.log"; : > "$log"

  env -u XO_HOME PATH="$fb:$PATH" XO_ROOT_OVERRIDE="$dir" XO_TMUX_LOG="$log" XO_SEND_SETTLE=0 \
    "$SEND" sess:win "hello" >/dev/null 2>"$err"; rc=$?
  [ "$rc" -ne 0 ] || fail "unset XO_HOME should fail"
  assert_contains "$(cat "$err")" "XO_HOME is not set" "unset XO_HOME diagnostic should be explicit"
  [ ! -s "$log" ] || fail "unset XO_HOME still attempted a send"$'\n'"$(cat "$log")"
  pass "xo-send strict: unset XO_HOME fails before target resolution"
}
...
test_unresolvable_target_does_not_tmux_fallback() {
  ...
  PATH="$fb:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$home" XO_TMUX_LOG="$log" XO_FAKE_TMUX_WINDOW=lost-target XO_SEND_SETTLE=0 \
  ...
  pass "xo-send strict: unresolvable selectors do not fall back to tmux"
}
...
test_prefixless_herdr_pane_id_fails() {
  ...
  xo_write_meta "$home/state/nudge.meta" \
  ...
  pass "xo-send strict: prefixless herdr pane ids are rejected before tmux fallback"
}
...
test_unmatched_single_colon_target_must_exist() {
  ...
  PATH="$fb:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$home" XO_TMUX_LOG="$log" XO_FAKE_TMUX_DEAD_TARGET=sess:missing XO_SEND_SETTLE=0 \
  ...
  pass "xo-send strict: unmatched single-colon explicit targets must verify live before sending"
}

test_xo_prefixed_herdr_session_is_an_explicit_target() {
  local dir fb home err log herdr_log rc
  dir="$TMP_ROOT/xo-remote-explicit"; mkdir -p "$dir"
  fb=$(make_stubs "$dir"); home=$(setup_home fmremote); err="$dir/send.err"; log="$dir/tmux.log"; herdr_log="$dir/herdr.log"
  : > "$log"
  : > "$herdr_log"

  PATH="$fb:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$home" XO_TMUX_LOG="$log" XO_HERDR_LOG="$herdr_log" XO_SEND_SETTLE=0 \
    "$SEND" xo-remote:w1:p2 --key Enter >/dev/null 2>"$err"; rc=$?
  expect_code 0 "$rc" "an xo-prefixed Herdr session target should be accepted as explicit"
  assert_grep 'pane get w1:p2 --session xo-remote' "$herdr_log" "xo-prefixed Herdr target was not verified in its session"
  assert_grep 'pane send-keys w1:p2 enter --session xo-remote' "$herdr_log" "xo-prefixed Herdr target was not sent its key in its session"
  assert_no_grep '--session default' "$herdr_log" "xo-prefixed Herdr target fell back to the default session"
  pass "xo-send strict: xo-prefixed Herdr sessions remain explicit backend targets"
}

test_healthy_xo_id_send_still_works() {
  local dir fb home err log rc got
  dir="$TMP_ROOT/healthy"; mkdir -p "$dir"
  fb=$(make_stubs "$dir"); home=$(setup_home healthy); err="$dir/send.err"; log="$dir/tmux.log"; : > "$log"
  xo_write_meta "$home/state/lane-ok.meta" "window=sess:xo-lane-ok" "kind=ship" "harness=codex"

  PATH="$fb:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$home" XO_TMUX_LOG="$log" XO_SEND_SETTLE=0 \
    "$SEND" xo-lane-ok "hello captain" >/dev/null 2>"$err"; rc=$?
  expect_code 0 "$rc" "healthy xo-id send should succeed"
  got=$(cat "$log")
  assert_contains "$got" "target=sess:xo-lane-ok literal=1 arg=: XO instruction waiting" \
    "healthy send should ring the doorbell at the meta target"
  assert_contains "$got" "target=sess:xo-lane-ok literal=0 arg=Enter" "healthy send should submit the doorbell with Enter"
  grep -qF 'hello captain' "$home/state/lane-ok.inbox/001.msg" \
    || fail "healthy send should record the steer in the task inbox"
  assert_contains "$(cat "$err")" "requested message WILL still be sent" "xo-send guard banner should keep send-specific continuation wording"
  pass "xo-send strict: healthy xo-<id> sends record the steer and ring once"
}

# A --key send is how xo interrupts a worker, so its exit status is the
...
test_key_send_exit_status_follows_delivery() {
  ...
  xo_write_meta "$home/state/lane-key.meta" "window=sess:xo-lane-key" "kind=ship"

  PATH="$fb:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$home" XO_TMUX_LOG="$log" XO_SEND_SETTLE=0 \
    "$SEND" lane-key --key Escape >/dev/null 2>"$err"; rc=$?
  expect_code 0 "$rc" "a delivered --key interrupt should report success"
  assert_contains "$(cat "$log")" "target=sess:xo-lane-key literal=0 arg=Escape" "the delivered case should send the named key"

  : > "$log"
  PATH="$fb:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$home" XO_TMUX_LOG="$log" XO_SEND_SETTLE=0 \
    XO_FAKE_TMUX_SEND_KEY_FAIL=Escape \
    "$SEND" lane-key --key Escape >/dev/null 2>"$err"; rc=$?
  ...
  assert_contains "$(cat "$log")" "target=sess:xo-lane-key literal=0 arg=Escape" "the undelivered case should still have attempted the send"
  pass "xo-send --key: exit status follows delivery, and an undelivered key never reports success"
}
