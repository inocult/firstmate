...
# across bin/xo-spawn.sh, bin/xo-promote.sh, and bin/xo-project-mode.sh.
#
# A ship task's delivery mode and yolo posture are xo's decision at intake,
...
BRIEF="$ROOT/bin/xo-brief.sh"
PROMOTE="$ROOT/bin/xo-promote.sh"
PROJECT_MODE="$ROOT/bin/xo-project-mode.sh"
TMP_ROOT=$(xo_test_tmproot xo-task-delivery)
...
run_spawn() {  # <home> <fakebin> <spawn-args...>
  ...
  XO_ROOT_OVERRIDE='' XO_HOME="$home" \
    ...
    XO_PROJECTS_OVERRIDE="$TMP_ROOT/projects-unused" XO_CONFIG_OVERRIDE="$home/config" \
    XO_SPAWN_NO_GUARD=1 XO_BACKEND=tmux PATH="$fakebin:$PATH" \
    "$SPAWN" "$@" 2>&1
}
...
ROWS
  pass "xo-spawn: a ship spawn requires a valid explicit mode and yolo before anything is created"
}
...
EOF
  ...
  pass "xo-spawn: scout and secondmate spawns refuse ship delivery flags"
}
...
EOF
  ...
  pass "xo-spawn: the brief's recorded mode and the spawn's explicit mode must agree"
}
...
ROWS
  pass "xo-spawn: a rigor downgrade against the registered posture is announced, never blocked"
}
...
EOF
  ...
  pass "xo-spawn: a scout spawn resolves no delivery posture from the registry"
}
...
test_promote_requires_and_records_the_delivery_contract() {
  ...
  write_scout_meta() {
    printf 'window=xo-promote-d1\nkind=scout\nworktree=/tmp/wt\n' > "$meta"
  }

  write_scout_meta
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 --mode direct-PR 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 --mode no-mistakes-prod-only --yolo off 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$blocked_data" \
  ...
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    "$PROMOTE" promote-d1 --mode direct-PR --yolo on 2>&1)
  status=$?
  ...
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$PROMOTE" promote-d1 --mode direct-PR --yolo on 2>&1)
  status=$?
  ...
  pass "xo-promote: promotion requires the delivery contract and records it exactly once"
}
...
test_promote_refuses_a_symlinked_task_record() {
  ...
  printf '%s\n' 'window=xo-promote-sym' 'kind=scout' 'worktree=/tmp/wt' > "$target"
  cp "$target" "$original"
  ln -s "$target" "$meta"
  ...
  pass "xo-promote: a symlinked task record is refused and its target is left untouched"
}
...
# prints against a capturing xo-send.sh, and asserts on the message the worker would
...
test_promotion_delivers_the_real_definition_of_done() {
  ...
  cat > "$sendroot/bin/xo-send.sh" <<'STUB'
#!/usr/bin/env bash
# Capture the message a promoted worker would receive, instead of steering one.
printf '%s' "$2" > "$XO_TEST_CAPTURE"
STUB
  chmod +x "$sendroot/bin/xo-send.sh"

  for mode in no-mistakes direct-PR local-only; do
    id="promote-dod-$(printf '%s' "$mode" | tr '[:upper:]' '[:lower:]')"
    meta="$home/state/$id.meta"
    printf 'window=xo-%s\nkind=scout\nworktree=/tmp/wt\n' "$id" > "$meta"
    XO_HOME="$home" "$BRIEF" "$id" fixture-project --scout >/dev/null 2>&1 \
    ...
    out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$PROMOTE" "$id" --mode "$mode" --yolo off 2>&1) \
    ...
    ( cd "$sendroot" \
      && XO_TEST_CAPTURE="$payload" \
         eval "$(printf '%s\n' "$out" | sed -n 's/^next: //p' | grep 'xo-send\.sh')" ) \
    ...
    assert_grep "If either does not resolve to the worktree you were launched in, stop and escalate to xo" "$payload" \
      "$mode: promoted worker was not told to stop for any wrong worktree"
    assert_grep "git checkout -b xo/$id" "$payload" \
    ...
    assert_grep "## XO spec" "$payload" \
      "$mode: promoted worker did not receive the XO spec subsection"
    ...
    XO_HOME="$home" "$BRIEF" "$id" fixture-project --mode "$mode" >/dev/null 2>&1 \
  ...
  assert_grep "ask-user findings are never yours to answer: escalate to xo" "$payload" \
  ...
  pass "xo-promote: a promoted worker receives the same mode-specific delivery contract a briefed one does"
}
...
EOF
  ...
  XO_HOME="$home" "$BRIEF" "$id" proj --mode no-mistakes >/dev/null 2>&1 \
  ...
  assert_contains "$out" "still contains {TASK} or {XO_SPEC}" \
  ...
  XO_HOME="$home" "$BRIEF" "$id" proj --mode direct-PR >/dev/null 2>&1 \
    || fail "filled-ship brief should scaffold"
  fill_brief_subsections "$home/data/$id/brief.md" \
    "Fix replacement of \`{TASK}\` in Herdr briefs." \
    "Keep literal \`{XO_SPEC}\` examples intact."
  out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode direct-PR --yolo off)
  assert_not_contains "$out" "still contains {TASK} or {XO_SPEC}" \
...
Do not copy this XO-authored constraint into intent.
...
EOF
  ...
  assert_not_contains "$authorized" "XO-authored constraint" \
...
# Definition of done
Delivery contract: mode=no-mistakes
Pass the entire Task and every XO requirement as --intent.
EOF
  out=$(run_spawn "$home" "$fakebin" "$id" "$proj" claude --mode no-mistakes --yolo off)
  assert_present "$home/data/$id/launch-brief.md" \
  ...
  assert_not_contains "$authorized" "Preserve the existing compatibility path." \
    "migrated launch contract included XO spec in intent"
  assert_grep "supersedes every earlier brief instruction about constructing \`--intent\`" \
    "$home/data/$id/launch-brief.md" \
...
EOF
  ...
  XO_HOME="$home" "$BRIEF" "$id" proj --scout >/dev/null 2>&1 \
  ...
  assert_contains "$out" "must contain nonempty ## Captain's intent and ## XO spec" \
  ...
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$PROMOTE" "$id" --mode direct-PR --yolo on 2>&1)
  status=$?
  ...
  assert_contains "$out" "promotion generates a separate ship-time spec" \
    "unfilled promotion did not distinguish scout and ship XO specs"
  assert_grep 'kind=scout' "$meta" "unfilled promotion still changed the task record"

  id=promote-missing-brief
  meta="$home/state/$id.meta"
  printf 'window=xo-%s\nkind=scout\nworktree=/tmp/wt\n' "$id" > "$meta"
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$PROMOTE" "$id" --mode direct-PR --yolo off 2>&1)
  status=$?
...
Keep this XO constraint out of captain intent.
...
EOF
  ...
  out=$(XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$PROMOTE" "$id" --mode no-mistakes --yolo off 2>&1)
  status=$?
  ...
  assert_no_grep "Ship the identity-check fix without adding a classifier." "$brief" \
    "promotion reused the scout-time XO spec as ship instructions"
  spec_body=$(awk '$0 == "## XO spec" { emit=1; next } emit && /^# / { exit } emit { print }' "$brief")
  assert_contains "$spec_body" "Verify isolation before anything else" \
    "promotion did not place its ship-time instructions in XO spec"
  assert_no_grep "SCOUT task" "$brief" \
    "promotion copied the scout Setup/Rules contract into XO spec"
...
EOF
  ...
  assert_no_grep "This scout-only setup must not become the spec." "$brief" \
    "promotion copied the following top-level section into XO spec"
...
EOF
  ...
  assert_contains "$spec_body" "Verify isolation before anything else" \
    "legacy promotion did not place promotion ship instructions in XO spec"
  assert_not_contains "$spec_body" "This is a SCOUT task" \
    "legacy promotion copied the scout Setup section into XO spec"
  pass "xo-spawn/xo-promote: leftover Task placeholders are refused until both subsections are filled"
}
...
EOF
  ...
  for project_kind in xo unrelated; do
    if [ "$project_kind" = xo ]; then
    ...
    for kind in no-mistakes direct-PR local-only scout; do
      ...
      assert_grep 'When this task works on XO itself' "$brief" "$project_kind $kind made the exception unconditional"
      ...
      ! grep -q '^This section supersedes every earlier brief instruction about your role' "$brief" ||
        fail "$project_kind $kind revoked the brief's own role for a task that is not XO"
  ...
  pass "xo-spawn: every legacy worker receives scoped role instructions without changing project or primary instructions"
}
...
test_project_mode_maps_the_conditional_policy
test_spawn_and_promote_require_filled_task_subsections
echo "# all xo-task-delivery tests passed"
