...
# createAgentSession surface, the custom bash and xo_branch_report tool
...
xo_live_gate opt-in XO_PI_BRANCH_LIVE_E2E npm jq node
export NODE_NO_WARNINGS=1

PI_PACKAGE_DIR=${XO_PI_PACKAGE_DIR:-"$(npm root -g)/@earendil-works/pi-coding-agent"}
if [ ! -f "$PI_PACKAGE_DIR/package.json" ]; then
  fail "Pi package absent: the live branch guard needs @earendil-works/pi-coding-agent installed (XO_PI_PACKAGE_DIR to override)"
fi
PI_VERSION=$(jq -r '.version' "$PI_PACKAGE_DIR/package.json" 2>/dev/null || printf 'unknown')

TMP_ROOT=$(xo_test_tmproot xo-pi-branch-live)
repo="$TMP_ROOT/repo"
home="$TMP_ROOT/home"
...
cp "$ROOT/.pi/extensions/xo-branch-supervision.ts" "$repo/.pi/extensions/xo-branch-supervision.ts"
...
while :; do
  if [ -e "$XO_LIVE_WATCH_TRIGGER" ]; then
    reason=$(cat "$XO_LIVE_WATCH_TRIGGER")
    rm -f "$XO_LIVE_WATCH_TRIGGER"
    printf '%s\n' "$reason"
    exit 0
  fi
  sleep 0.02
...
SH
chmod +x "$repo/bin/xo-operational-input.sh" "$repo/bin/xo-watch-arm.sh"
...
BRANCH_PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" \
  WATCH_PLUGIN="$repo/.pi/extensions/xo-primary-pi-watch.ts" \
  XO_HOME="$home" XO_REAL_ROOT="$ROOT" XO_WATCH_ROOT="$repo" \
  XO_LIVE_WATCH_LOG="$TMP_ROOT/live-watch.log" XO_LIVE_WATCH_TRIGGER="$TMP_ROOT/live-watch.trigger" \
...
const home = resolve(process.env.XO_HOME);
...
const bus = {
  ...
  emit(channel, data) {
    ...
    if (channel === "xo-branch-supervision:dispatch") offers.push(data);
  },
};
...
const pi = {
  ...
  registerTool(tool) {
    if (tool.name === "xo_watch_arm_pi") watcherTool = tool;
  },
  registerCommand() {},
...
process.env.XO_ROOT_OVERRIDE = process.env.XO_REAL_ROOT;
const branchMod = await import(pathToFileURL(process.env.BRANCH_PLUGIN).href);
branchMod.default(pi);
process.env.XO_ROOT_OVERRIDE = process.env.XO_WATCH_ROOT;
...
const armCount = () => existsSync(process.env.XO_LIVE_WATCH_LOG)
  ? readFileSync(process.env.XO_LIVE_WATCH_LOG, "utf8").split(/\n/).filter((line) => line.startsWith("arm ")).length
  : 0;
...
writeFileSync(process.env.XO_LIVE_WATCH_TRIGGER, "signal: live-sdk probe\n");
...
if (!fallback.includes("XO WATCHER WAKE: signal: live-sdk probe")) {
...
writeFileSync(process.env.XO_LIVE_WATCH_TRIGGER, "signal: live pin probe\n");
...
if (!pinFallback.includes("XO WATCHER WAKE: signal: live pin probe")) {
  throw new Error(`pinned watcher-owned fallback lost the wake reason: ${pinFallback}`);
}
const confirmations = readFileSync(process.env.XO_LIVE_WATCH_LOG, "utf8")
  .split(/\n/)
...
cat > "$erroragentdir/models.json" <<'JSON'
{
  "providers": {
    "xo-live-error": {
      "baseUrl": "https://xo-provider-error.invalid/v1",
      "api": "openai-completions",
      "apiKey": "xo-live-placeholder",
      "models": [
        { "id": "xo-live-error-model", "name": "xo live error", "contextWindow": 8192, "maxTokens": 512 }
      ]
    }
  }
}
...
BRANCH_PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" \
  WATCH_PLUGIN="$repo/.pi/extensions/xo-primary-pi-watch.ts" \
  XO_HOME="$errorhome" XO_REAL_ROOT="$ROOT" XO_WATCH_ROOT="$repo" \
  XO_LIVE_WATCH_LOG="$TMP_ROOT/error-watch.log" XO_LIVE_WATCH_TRIGGER="$TMP_ROOT/error-watch.trigger" \
...
watchMod.default(pi);
sessionCtx = {
  model: { provider: "xo-live-error", id: "xo-live-error-model" },
  sessionManager: { getSessionFile: () => `${home}/main.jsonl`, getEntries: () => [] },
};
const waitFor = async (predicate, label) => {
  for (let i = 0; i < 600; i += 1) {
...
await waitFor(() => existsSync(process.env.XO_LIVE_WATCH_LOG), "provider-error watcher arm");
writeFileSync(process.env.XO_LIVE_WATCH_TRIGGER, "signal: c1 429 probe\n");
...
const fallback = mainUserMessages[0].content;
if (!fallback.includes("XO WATCHER WAKE: signal: c1 429 probe")) {
...
cat > "$modeldir/models.json" <<'JSON'
{
  "providers": {
    "xo-live-fake": {
      ...
      "apiKey": "xo-live-placeholder",
      "models": [
        { "id": "xo-live-a", "name": "xo live a", "contextWindow": 8192, "maxTokens": 512 },
        { "id": "xo-live-b", "name": "xo live b", "contextWindow": 8192, "maxTokens": 512 }
      ]
    }
  }
}
JSON
PI_PACKAGE_DIR="$PI_PACKAGE_DIR" PI_CODING_AGENT_DIR="$modeldir" XO_LIVE_SESSIONS="$TMP_ROOT/model-sessions" \
...
const first = registry.find("xo-live-fake", "xo-live-a");
const second = registry.find("xo-live-fake", "xo-live-b");
...
if (created.session.model?.id !== "xo-live-a") {
...
if (repinned.session.model?.id !== "xo-live-b") {
...
if (unpinned.session.model?.id !== "xo-live-a") {
  ...
  "providers": {
    "xo-live-fake": {
      ...
      "models": [
        ...
        { "id": "xo-live-shallow", "name": "xo live shallow", "contextWindow": 8192, "maxTokens": 512, "reasoning": true },
        { "id": "xo-live-plain", "name": "xo live plain", "contextWindow": 8192, "maxTokens": 512 }
      ]
    }
  }
}
JSON
PI_PACKAGE_DIR="$PI_PACKAGE_DIR" PI_CODING_AGENT_DIR="$effortdir" XO_LIVE_SESSIONS="$TMP_ROOT/effort-sessions" \
...
const deep = registry.find("xo-live-fake", "xo-live-deep");
const shallow = registry.find("xo-live-fake", "xo-live-shallow");
const plain = registry.find("xo-live-fake", "xo-live-plain");
...
if (clampThinkingLevel(shallow, "xo-not-a-level") !== "off") {
...
PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" DELIVERY_DIR="$TMP_ROOT/delivery-sessions" \
...
capturedApi.appendEntry("xo-branch-visible-outcome", record);
...
const entry = entries.find((candidate) => candidate.type === "custom" && candidate.customType === "xo-branch-visible-outcome");
...
cat > "$streamdir/models.json" <<'JSON'
{
  "providers": {
    "xo-live-stream": {
      "baseUrl": "https://xo-live-stream.invalid/v1",
      "api": "openai-completions",
      "apiKey": "xo-live-placeholder",
      "models": [
        { "id": "xo-live-stream-model", "name": "xo live stream", "contextWindow": 8192, "maxTokens": 512 }
      ]
    }
  }
}
JSON
WATCH_PLUGIN="$repo/.pi/extensions/xo-primary-pi-watch.ts" \
  XO_HOME="$streamhome" XO_ROOT_OVERRIDE="$repo" \
  XO_LIVE_WATCH_LOG="$TMP_ROOT/stream-watch.log" XO_LIVE_WATCH_TRIGGER="$TMP_ROOT/stream-watch.trigger" \
  XO_LIVE_SESSIONS="$TMP_ROOT/stream-sessions" \
...
const chunk = (delta, finish) => `data: ${JSON.stringify({
  ...
  created: 1,
  model: "xo-live-stream-model",
  choices: [{ index: 0, delta, finish_reason: finish }],
  ...(finish ? { usage: { prompt_tokens: 1, completion_tokens: 1, total_tokens: 2 } } : {}),
})}\n\n`;
globalThis.fetch = async (input) => {
  const url = typeof input === "string" ? input : input instanceof URL ? input.href : input.url;
  if (!url.startsWith("https://xo-live-stream.invalid/")) {
    throw new Error(`unexpected network request in provider-free guard: ${url}`);
  }
  completions += 1;
...
const registry = new ModelRegistry(runtime);
await registry.refresh();
const model = registry.find("xo-live-stream", "xo-live-stream-model");
if (!model) throw new Error("the real registry did not resolve the local streaming model");
const { session } = await createAgentSession({
  cwd: process.cwd(),
  sessionManager: SessionManager.create(process.cwd(), resolve(process.env.XO_LIVE_SESSIONS)),
  settingsManager: settings,
  resourceLoader: loader,
  modelRuntime: runtime,
  model,
...
const armLog = process.env.XO_LIVE_WATCH_LOG;
...
const armTool = session.getToolDefinition("xo_watch_arm_pi");
if (!armTool) throw new Error("the real tool registry did not expose xo_watch_arm_pi");
...
writeFileSync(process.env.XO_LIVE_WATCH_TRIGGER, "signal: live streaming probe\n");
await waitFor(
...
writeFileSync(process.env.XO_LIVE_WATCH_TRIGGER, "signal: live idle probe\n");
