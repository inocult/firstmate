...
xo_live_gate opt-in XO_HARNESS_ADAPTER_INSTRUCTION_EVAL curl jq

ROUTER="$ROOT/.agents/skills/harness-adapters/SKILL.md"
TMP_ROOT=$(xo_test_tmproot xo-harness-adapter-instructions)
...
MODEL=${XO_HARNESS_ADAPTER_LOCAL_MODEL:-}
[ -n "$MODEL" ] || fail "XO_HARNESS_ADAPTER_LOCAL_MODEL must name an explicit local evaluator"
...
curl -fsS --max-time "${XO_HARNESS_ADAPTER_EVAL_TIMEOUT_SECONDS:-120}" \
...
resolve_native_binary() {
  ...
  if [ "$harness" = cursor ]; then
    xo_cursor_resolve_binary 2>/dev/null
    return
  fi
