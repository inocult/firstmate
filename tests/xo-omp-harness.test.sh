#!/usr/bin/env bash
# tests/xo-omp-harness.test.sh - the portable regression for the omp (Oh My Pi)
...
# XO_OMP_LIVE_E2E=1 tests/xo-omp-primary-live-e2e.test.sh is the live guard that
...
#   2. XO_OMP_HARNESS=omp is a precedence override that needs a real omp
...
#   7. The watch extension arms through xo_watch_arm_omp and delivers an
...
. "$ROOT/bin/xo-composer-lib.sh"
# shellcheck source=bin/xo-control-lib.sh
. "$ROOT/bin/xo-control-lib.sh"
# shellcheck source=bin/xo-session-lock-lib.sh
. "$ROOT/bin/xo-session-lock-lib.sh"

HARNESS="$ROOT/bin/xo-harness.sh"
TMP_ROOT=$(xo_test_tmproot xo-omp-harness)
...
test_detection_anchored_name_and_marker_precedence() {
  ...
  out=$(env -u CLAUDECODE -u XO_OMP_HARNESS -u PI_CODING_AGENT -u CURSOR_AGENT -u CURSOR_INVOKED_AS \
  ...
  # shellcheck disable=SC2016 # the quoted body expands inside the named shell
  out=$(env -u PI_CODING_AGENT -u CURSOR_AGENT -u CURSOR_INVOKED_AS CLAUDECODE=1 XO_OMP_HARNESS=omp \
    "$bin/omp" -c '"$1"; :' _ "$HARNESS")
  [ "$out" = omp ] || fail "XO_OMP_HARNESS under an omp ancestor must outrank an inherited CLAUDECODE, got '$out'"
  # ...and is inert when it leaks into a worker with no omp ancestor.
  # shellcheck disable=SC2016 # the quoted body expands inside the named shell
  out=$(env -u PI_CODING_AGENT -u CURSOR_AGENT -u CURSOR_INVOKED_AS CLAUDECODE=1 XO_OMP_HARNESS=omp \
    bash -c '"$1"; :' _ "$HARNESS")
  [ "$out" = claude ] || fail "a leaked XO_OMP_HARNESS without an omp ancestor must not relabel a claude worker, got '$out'"
  pass "xo-harness: omp detects by its anchored name; the marker is a precedence override that needs real omp ancestry"
}

test_lock_identity_and_liveness_classification() {
  xo_harness_process_matches omp '' || fail "session-lock identity must accept the exact omp name"
  xo_harness_process_matches /usr/local/bin/omp 'omp --cwd /x' || fail "session-lock identity must accept an omp path"
  ! xo_harness_process_matches ompd '' || fail "session-lock identity must not accept ompd"
  ! xo_harness_process_matches comp '' || fail "session-lock identity must not accept comp"
  # shellcheck source=bin/xo-backend.sh
  . "$ROOT/bin/xo-backend.sh"
  xo_backend_source tmux || fail "xo_backend_source tmux failed"
  [ "$(xo_agent_process_classify_name omp)" = agent ] || fail "tmux liveness must classify omp as an agent"
  [ "$(xo_agent_process_classify_name /opt/omp/bin/omp)" = agent ] || fail "tmux liveness must classify an omp path as an agent"
  [ "$(xo_agent_process_classify_name ompd)" != agent ] || fail "tmux liveness must not classify ompd as an agent"
  [ "$(xo_agent_process_classify_name comp)" != agent ] || fail "tmux liveness must not classify comp as an agent"
...
make_spawn_case() {  # <name> <harness> <id>
  ...
  make_fake_omp "$fakebin"
  xo_test_spawn_home "$home" "$harness"
  xo_git_worktree "$proj" "$wt" "wt-$name"
  xo_test_spawn_brief "$home" "$id"
  : > "$case_dir/launch.log"
...
run_scout_spawn() {  # <home> <wt> <fakebin> <launch-log> <spawn-args...>
  ...
  shift 4
  XO_FAKE_LAUNCH_LOG="$launchlog" xo_test_run_spawn "$home" "$wt" "$fakebin" "$@" --scout
}
...
test_spawn_launch_line_and_worker_wiring() {
  ...
  assert_contains "$launch" "env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT -u XO_PI_HARNESS -u GEMINI_CLI -u CURSOR_AGENT -u CURSOR_INVOKED_AS XO_OMP_HARNESS=omp OMP_SKIP_SETUP=1 '$FAKEBIN_DIR/omp'" \
    "omp launch did not clear foreign markers and establish its own at the launch boundary"
  assert_contains "$launch" "--config '$ROOT/.omp/xo-worker-overlay.yml' --auto-approve --cwd '$WT_DIR'" \
  ...
  [ "$(xo_busy_classify tmux fake:w omp "$id" "$state")" = "busy xo-spawn" ] \
    || fail "omp spawn must seed the busy-state contract"
  pass "xo-spawn: the omp launch line clears markers, pins posture, and wires the state-resident extension"
}
...
test_spawn_model_validation_scoped_to_listed_providers() {
  ...
  pass "xo-spawn: omp model validation is scoped to providers the listing can prove"
}

test_secondmate_launch_relies_on_discovery() {
  # A seeded secondmate home, launched for real through xo-spawn on omp: the
  ...
  : > "$launchlog"
  # XO_BACKEND=tmux pins the fake tmux even where the developer shell carries a
  # live Herdr environment; without it auto-detection would spawn a real pane.
  out=$(PATH="$fakebin:$PATH" TMUX='fake,1,0' XO_BACKEND=tmux CLAUDECODE=1 \
    XO_ROOT_OVERRIDE='' XO_HOME="$world/home" \
    ...
    "$ROOT/bin/xo-spawn.sh" sm "$home" omp --secondmate 2>&1)
  status=$?
  ...
  assert_contains "$launch" "--config '$ROOT/.omp/xo-worker-overlay.yml' --auto-approve --cwd '$home'" "secondmate launch lost the posture overlay or the pinned home directory: $launch"
  assert_contains "$launch" "XO_OMP_HARNESS=omp OMP_SKIP_SETUP=1 '$fakebin/omp'" "secondmate launch lost the omp marker or executable"
  assert_contains "$launch" "XO_SUPERVISION_MODEL=extension" "an omp secondmate must run the extension supervision model"
  assert_absent "$world/home/state/sm.omp-ext.ts" "a secondmate must not receive a per-task worker extension"
  pass "xo-spawn: a real omp secondmate launch relies on auto-discovery while crewmates load one -e"
}
...
test_secondmate_config_pinned_model_is_validated() {
  ...
  out=$(PATH="$fakebin:$PATH" TMUX='fake,1,0' XO_BACKEND=tmux CLAUDECODE=1 \
    ...
    XO_PROJECTS_OVERRIDE="$world/home/projects" XO_CONFIG_OVERRIDE="$world/home/config" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_LAUNCH_LOG="$launchlog" \
    "$ROOT/bin/xo-spawn.sh" sm "$home" --secondmate 2>&1)
  status=$?
  ...
  pass "xo-spawn: the config/secondmate-harness model pin is validated against the omp catalog before launch"
}
...
test_busy_extension_lifecycle() {
  ...
  [ "$(xo_busy_classify tmux fake:w omp "$id" "$state")" = "busy xo-spawn" ] || fail "turn_end must stay a notification, not a state edge"

  out=$(drive_omp_ext "$ext" agent-start) || fail "agent_start drive failed: $out"
  [ "$(xo_busy_classify tmux fake:w omp "$id" "$state")" = "busy omp-ext" ] || fail "agent_start must classify 'busy omp-ext'"

  out=$(drive_omp_ext "$ext" end-continuing) || fail "continuing agent_end drive failed: $out"
  [ "$(xo_busy_classify tmux fake:w omp "$id" "$state")" = "busy omp-ext" ] || fail "agent_end with willContinue must stay busy (a session_stop continuation is coming)"

  out=$(drive_omp_ext "$ext" end-final) || fail "final agent_end drive failed: $out"
  [ "$(xo_busy_classify tmux fake:w omp "$id" "$state")" = "idle omp-ext" ] || fail "a plain agent_end must classify 'idle omp-ext'"

  # A record from another harness's writer is never trusted for omp.
  xo_busy_source_trusted omp pi-ext && fail "omp must not trust the Pi extension's records"
  xo_busy_source_trusted omp omp-ext || fail "omp must trust its own extension's records"
...
test_control_composer_and_model_tables() {
  [ "$(xo_control_exit_command omp)" = /quit ] || fail "omp exit command must be /quit"
  [ "$(xo_control_interrupt_key omp)" = Escape ] || fail "omp interrupt key must be Escape"
  [ "$(xo_control_interrupt_repeat omp)" = 1 ] || fail "omp interrupts on a single press"
  [ -z "$(xo_control_interrupt_clear_key omp)" ] || fail "omp leaves its composer empty and needs no clear key"
  [ "$(xo_control_harness_wiring_paths omp /wt /st id1)" = "/st/id1.omp-ext.ts" ] || fail "omp wiring path must be the state-resident extension"
  printf 'Working…\n' | xo_busy_lines_match omp || fail "omp busy regex must match the TUI ellipsis form"
  printf 'Working...\n' | xo_busy_lines_match omp && fail "omp busy regex must not match the three-dot form no supervised pane renders"
  printf ' ⠧ 11s  · gpt-6-astra\n' | xo_busy_lines_match omp || fail "omp busy regex must match the braille spinner plus elapsed cell"
  printf ' ⣾ 3s  · gpt-6-astra\n' | xo_busy_lines_match omp || fail "omp busy regex must match the status-set spinner frames, not only the activity set"
  printf ' 󰵗  · gpt-6-astra · 36.7%%/41K\n' | xo_busy_lines_match omp && fail "an idle omp status row must not read busy"
  printf 'esc to interrupt\n' | xo_busy_lines_match omp && fail "omp must not borrow Claude's footer"
  local bin out
  bin=$(make_named_shells "$TMP_ROOT/named-model")
  # shellcheck disable=SC2016 # the quoted body expands inside the named shell
  out=$(env -u CLAUDECODE -u XO_OMP_HARNESS -u PI_CODING_AGENT -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u XO_SUPERVISION_MODEL \
    "$bin/omp" -c '. "$1"; xo_supervision_model' _ "$ROOT/bin/xo-wake-lib.sh")
...
record_omp_session() {  # <root> <home> <session-pid> [omit] [drift]
  ...
  for pair in \
    "xo-primary-omp-watch.ts:.omp-watch-extension-loaded:watch" \
    "xo-primary-turnend-guard.ts:.omp-turnend-extension-loaded:turnend"; do
    source=${pair%%:*}
    ...
    if [ "$drift" = "${pair##*:}" ]; then
      ...
      version=$(bash -c '. "$1"; xo_pi_extension_version "$2"' _ "$ROOT/bin/xo-wake-lib.sh" "$root/.omp/extensions/$source") || return 1
    fi
...
owns() {  # <root> <home>
  bash -c '. "$1"; xo_omp_extension_owns_supervision "$2" "$3"' _ "$ROOT/bin/xo-wake-lib.sh" "$2/state" "$1"
}
...
test_ownership_proof_is_omp_keyed() {
  ...
  bash -c '. "$1"; xo_pi_extension_owns_supervision "$2" "$3"' _ "$ROOT/bin/xo-wake-lib.sh" "$home/state" "$root" \
    && fail "omp markers must never satisfy the Pi proof"
  bash -c '. "$1"; xo_extension_owns_supervision "$2" "$3"' _ "$ROOT/bin/xo-wake-lib.sh" "$home/state" "$root" \
  ...
  local verdict
  verdict=$(XO_SUPERVISION_MODEL=extension XO_HOME="$home" bash -c '
    . "$1"; xo_watcher_supervision_verdict "$2" "$3" 999 "$4" "$5"; printf "%s %s" "$XO_WATCHER_VERDICT_OK" "$XO_WATCHER_VERDICT_REASON"' \
    _ "$ROOT/bin/xo-wake-lib.sh" "$home/state" "$root/bin/xo-watch.sh" "$home" "$root")
  ...
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  pass "xo-wake-lib: the omp ownership proof is keyed on its own extensions and gates the hand-off tolerance"
}
...
install_omp_extension_fixture() {  # <repo>
  ...
  cp "$ROOT/.omp/extensions/xo-primary-turnend-guard.ts" "$ROOT/.omp/extensions/xo-primary-omp-watch.ts" "$repo/.omp/extensions/"
  cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" "$ROOT/.pi/extensions/lib/xo-sessionstart-supervisor.mjs" "$repo/.pi/extensions/lib/"
  cp "$ROOT/bin/xo-operational-input.sh" "$repo/bin/"
...
#!/usr/bin/env bash
payload=$(cat); printf '%s\n' "$payload" >> "${XO_GUARD_LOG:?}"
case "$payload" in *'"stop_hook_active":true'*) exit 0 ;; esac
printf 'guard says: repair with xo_watch_arm_omp\n' >&2; exit 2
SH
  cat > "$repo/bin/xo-arm-pretool-check.sh" <<'SH'
#!/usr/bin/env bash
case "$*" in *xo-watch-arm.sh*'&'*) printf 'xo watcher-arm seatbelt: blocked\n' >&2; exit 2 ;; esac; exit 0
SH
  printf '#!/usr/bin/env bash\nexit 0\n' > "$repo/bin/xo-cd-pretool-check.sh"
  # shellcheck disable=SC2016 # $2 expands in the generated script
  printf '#!/usr/bin/env bash\nprintf "OMP DIGEST source=%%s\\n" "$2"\n' > "$repo/bin/xo-sessionstart-run.sh"
  chmod +x "$repo/bin/"*.sh
  out=$(XO_GUARD_LOG="$TMP_ROOT/guard/guard.log" XO_HOME="$home" EXT="$repo/.omp/extensions/xo-primary-turnend-guard.ts" node --input-type=module 2>&1 <<'EOF'
import { pathToFileURL } from "node:url";
...
if (!first?.message?.content?.includes("XO_OP: v1 session-start: OMP DIGEST source=startup")) throw new Error(`first start did not deliver a startup digest: ${JSON.stringify(first)}`);
if (first.message.display !== false || first.message.customType !== "xo-sessionstart-nudge") throw new Error("digest message lost its persistent shape");
...
const blocked = await handlers.get("tool_call")({ type: "tool_call", toolName: "bash", input: { command: "bin/xo-watch-arm.sh &" } }, {});
...
if (!r1.additionalContext.startsWith("⁣XO_OP: v1 turn-end-guard: ")) throw new Error(`continuation context is not typed operational input: ${r1.additionalContext}`);
if (!r1.additionalContext.includes("TURN WOULD END BLIND") || !r1.additionalContext.includes("repair with xo_watch_arm_omp")) throw new Error("continuation dropped the guard text");
...
const payloads = readFileSync(process.env.XO_GUARD_LOG, "utf8").trim().split("\n");
if (payloads.join("|") !== '{"stop_hook_active":false}|{"stop_hook_active":true}') throw new Error(`guard payloads were ${payloads.join("|")}`);
if (!existsSync(`${process.env.XO_HOME}/state/.omp-turnend-extension-loaded`)) throw new Error("loaded marker was not written");
...
if [ ! -e "${XO_HOME:?}/state/.e2e-fired" ]; then
  : > "$XO_HOME/state/.e2e-fired"
  sleep 1
...
SH
  ...
  out=$(XO_HOME="$home" XO_ROOT_OVERRIDE="$repo" XO_OMP_ARM_READY_TIMEOUT_MS=3000 XO_WATCH_REARM_RETRY_LIMIT=1 XO_WATCH_REARM_RETRY_BASE_MS=5 XO_WATCH_REARM_RETRY_MAX_MS=10 \
    EXT="$repo/.omp/extensions/xo-primary-omp-watch.ts" node --input-type=module 2>&1 <<'EOF'
import { pathToFileURL } from "node:url";
...
const pi = {
  ...
  registerCommand(n, o) { if (n === "xo-watch-arm-omp") command = o.handler; },
...
const mod = await import(pathToFileURL(process.env.EXT).href);
mod.default(pi);
if (!tool || tool.name !== "xo_watch_arm_omp") throw new Error("xo_watch_arm_omp was not registered");
if (!command) throw new Error("/xo-watch-arm-omp was not registered");
...
const marker = readFileSync(`${process.env.XO_HOME}/state/.omp-watch-extension-loaded`, "utf8").split("\n");
...
if (!sent[0].m.startsWith("⁣XO_OP: v1 watcher: XO WATCHER WAKE: signal: omp-e2e done")) throw new Error(`unexpected wake text: ${sent[0].m}`);
...
await handlers.get("session_shutdown")({}, {});
if (existsSync(`${process.env.XO_HOME}/state/extensions/omp-primary-watch/session-replacement-actionable.json`)) throw new Error("a consumed wake must not ride the replacement handoff");
process.exit(0);
EOF
)
  status=$?
  ...
  pass ".omp watch extension: xo_watch_arm_omp arms once, repeats as a no-op, and delivers an actionable close as one follow-up"
}
