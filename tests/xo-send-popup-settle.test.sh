#!/usr/bin/env bash
# xo-send pre-submit popup-settle selection (the codex `$<skill>` fix).
#
...
# popup settles lets it swallow the Enter, so the line never submits. xo-send
# absorbs this by pausing `settle` seconds AFTER typing and BEFORE the (retried)
# Enter - the first sleep xo_tmux_submit_core makes. These tests pin the
...
#
# The popup-settle is the FIRST sleep recorded: xo_tmux_submit_core types the text,
# then `sleep "$settle"`, then the Enter-retry loop (sleep 0.4 each) and finally
# xo-send's own post-submit XO_SEND_SETTLE pause. So tail-vs-head matters: this
# suite asserts on the HEAD sleep, distinct from xo-send-settle.test.sh which pins
# the TAIL (post-submit) pause. The retried Enter in xo_tmux_submit_core remains the
...
TMP_ROOT=$(xo_test_tmproot xo-send-popup-settle)

# Same stub shape as xo-send-settle.test.sh: a fake tmux that drives the submit
# path to a clean "empty" verdict on the first Enter, and a fake sleep that records
# every requested duration (one per line) into XO_SLEEP_LOG instead of sleeping.
...
# (the popup-settle) equals <expected>. XO_SEND_SETTLE=0 strips the trailing
# post-submit pause so the log holds only the popup-settle plus the 0.4 Enter wait,
# keeping the head assertion crisp. XO_ROOT_OVERRIDE points at a non-repo dir so
# xo-guard's tangle check stays silent; its watcher-liveness note goes to stderr
# (discarded).
first_settle() {  # <expected> <label> <harness|--explicit> <message> [selector-form]
  ...
  if [ "$harness" = --explicit ]; then
    ...
    xo_write_meta "$home/state/$meta_id.meta" "window=sess:win" "harness=$harness"
  fi
  : > "$log"
  env XO_SEND_SETTLE=0 PATH="$fb:$PATH" \
  ...
  pass "xo-send popup-settle: $label -> ${expected}s"
}
...
rides_inbox() {  # <label> <harness> <message>
  ...
  dir="$TMP_ROOT/case-$RANDOM"; mkdir -p "$dir/state"
  fb=$(make_stubs "$dir"); log="$dir/sleep.log"; home="$dir"
  xo_write_meta "$home/state/popupcase.meta" "window=sess:win" "harness=$harness"
  : > "$log"
  env XO_SEND_SETTLE=0 PATH="$fb:$PATH" \
    XO_ROOT_OVERRIDE="$home" XO_HOME="$home" XO_SLEEP_LOG="$log" \
    "$SEND" xo-popupcase "$msg" 2>/dev/null; rc=$?
  expect_code 0 "$rc" "$label: send should succeed"
  ...
  pass "xo-send popup-settle: $label -> inbox plane, fast doorbell"
}
...
# task id, not only by the legacy `xo-<id>` window label.
