#!/usr/bin/env bash
# Regression tests for xo-spawn's pooled-worktree base refresh.
#
...
TMP_ROOT=$(xo_test_tmproot xo-spawn-pool-base-freshen)
...
make_case() {
  ...
  git -C "$project" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm initial
  ...
  git -C "$publisher" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm advance-main
...
run_spawn() {
  local id=$1
  shift
  xo_test_run_spawn "$HOME_DIR" "$POOL_DIR" "$FAKEBIN_DIR" \
    "$id" "$PROJECT_DIR" "$@"
}
...
test_remote_seeded_home_spawns_from_treehouse_pool() {
  ...
  cat > "$HOME_DIR/.xo-secondmate-parent" <<'REC'
...
REC
  ...
  lock=$(XO_HOME="$HOME_DIR" bash -c '. "$1"; xo_treehouse_project_lock_path "$2"' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$PROJECT_DIR") \
  ...
  if [ "${XO_TEST_EVIDENCE:-0}" = 1 ]; then
    ...
    printf '$ XO_HOME=%s bin/xo-spawn.sh %s %s --scout\n%s\nexit=%s\n' \
...
test_stale_pool_base_refreshes_before_branching() {
  ...
  git -C "$POOL_DIR" checkout --quiet -b "xo/$id"
...
make_submodule_case() {  # <name> <id>
  ...
  git -C "$sub" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm sub-one
  subpin1=$(git -C "$sub" rev-parse HEAD)
  printf 'pin two\n' > "$sub/lib.txt"
  git -C "$sub" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qam sub-two
  ...
  printf 'base\n' > "$project/README.md"
  git -C "$project" add README.md
  git -C "$project" -c protocol.file.allow=always -c user.name='XO Tests' -c user.email='tests@example.invalid' \
  ...
  git -C "$publisher" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qam advance-pin
...
test_unpushed_submodule_commit_is_still_uncommitted_work() {
  ...
  git -C "$POOL_DIR/ui" -c user.name='XO Tests' -c user.email='tests@example.invalid' \
...
# with the pool's state file beside the slot, which is the shape xo-spawn claims
...
lay_out_as_pool_slot() {
  ...
  SLOT_CLAIM="$slot_root/1/.xo-slot-owner"
}

# The spawn side of the slot-owner claim that bin/xo-teardown.sh later reads:
...
echo "# all xo-spawn-pool-base-freshen tests passed"
