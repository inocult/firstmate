...
#
# XO is a treehouse-pooled git repo of itself: linked worktrees and
# secondmate homes all sit at a detached HEAD on the default branch, while the
# PRIMARY checkout (XO_ROOT) is a normal checkout on a real branch. The "tangle"
...
#            xo-spawn refuses to launch unless the resolved worktree is isolated.
#   GUARD 2 (detection)  - xo-guard and xo-bootstrap alarm when the primary is on
#            a feature branch, and stay silent on the default branch or detached.
# These cases pin: the shared lib's branch classification, the xo-guard banner,
# the xo-bootstrap problem line, the brief assertion ordering, and the xo-spawn
...
. "$ROOT/bin/xo-tangle-lib.sh"

TMP_ROOT=$(xo_test_tmproot xo-tangle-guard)
...
make_repo() {
  ...
  xo_git_add_origin "$dir" "$dir.origin.git"
  printf '%s\n' "$dir"
}

# --- shared lib: branch classification --------------------------------------

# xo_primary_tangle_branch is the whole scoping decision: a NAMED non-default
...
test_lib_classification() {
  ...
  while IFS='|' read -r label state branch expect; do
    ...
    out=$(xo_primary_tangle_branch "$repo" || true)
    [ "$out" = "$expect" ] || fail "$label: expected tangle='$expect', got '$out'"
  done <<'ROWS'
on the default branch is healthy|default||
on a feature branch is the tangle|feature|xo/readme-restructure-d3|xo/readme-restructure-d3
...
ROWS
  ...
  out=$(xo_primary_tangle_branch "$TMP_ROOT" || true)
  [ -z "$out" ] || fail "non-git dir wrongly reported a tangle: '$out'"
  pass "xo_primary_tangle_branch: feature branch alarms; default/detached/non-git stay silent"
}

# --- GUARD 2a: xo-guard banner ----------------------------------------------

run_guard() {
  # Scope the guard to a temp repo as the primary checkout; state lives under it.
  XO_ROOT_OVERRIDE="$1" XO_HOME="$1" "$ROOT/bin/xo-guard.sh" 2>&1
}
...
test_guard_banner() {
  ...
  pass "xo-guard: bordered tangle banner fires only for a feature branch and suppresses repair commands in read-only mode"
}

# --- GUARD 2b: xo-bootstrap problem line ------------------------------------

run_bootstrap() {
  # No projects/ under the home keeps fleet sync inert; grep isolates the line.
  XO_ROOT_OVERRIDE="$1" XO_HOME="$1" "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null
}
...
test_bootstrap_line() {
  ...
  git -C "$repo" checkout -q -B xo/tangle-bb2
  out=$(run_bootstrap "$repo" | grep '^TANGLE:' || true)
  assert_contains "$out" "xo/tangle-bb2" "bootstrap did not report the tangled branch"
  assert_contains "$out" "checkout main" "bootstrap TANGLE line lacked the restore remediation"
  out=$(XO_ROOT_OVERRIDE="$repo" XO_HOME="$repo" XO_BOOTSTRAP_DETECT_ONLY=1 "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null | grep '^TANGLE:' || true)
  assert_contains "$out" "xo/tangle-bb2" "detect-only bootstrap did not report the tangled branch"
  ...
  pass "xo-bootstrap: TANGLE problem line fires only for a feature branch and suppresses repair commands in detect-only mode"
}
...
test_brief_assertion_precedes_branch() {
  ...
  mkdir -p "$home/data"
  XO_HOME="$home" "$ROOT/bin/xo-brief.sh" tangle-brief-cc3 alpha --mode no-mistakes >/dev/null 2>&1
  ...
  br=$(grep -n 'git checkout -b xo/' "$brief" | head -1 | cut -d: -f1)
  if [ -z "$iso" ] || [ -z "$br" ]; then
  ...
  pass "xo-brief: ship brief asserts worktree isolation before the branch step"
}

# --- GUARD 1b: xo-spawn isolation abort -------------------------------------
...
run_spawn() {
  ...
  xo_test_run_spawn "$home" "$pane" "$fakebin" \
...
test_spawn_isolation_abort() {
  ...
  pass "xo-spawn: aborts unless the resolved worktree is a genuine, isolated worktree"
}

# --- GUARD 1c: xo-spawn tmux window construction ----------------------------

# The prevention guard also depends on xo-spawn building robust tmux commands
...
#     are disabled so the xo-<id> name survives treehouse cd'ing into the worktree;
...
#     display-message fall back to the active client's window and misread xo's
...
test_spawn_tmux_window_construction() {
  ...
  assert_grep "new-window -dP -F #{window_id} -t xo: -n xo-rec-win-gg7" "$rec" \
    "new-window must append at the session (trailing colon) and capture the window id"
  assert_no_grep "new-window -dP -F #{window_id} -t xo -n" "$rec" \
  ...
  pass "xo-spawn: appends windows by session-colon, pins the name, and targets the window id"
}
