...
TMP_ROOT=$(xo_test_tmproot xo-teardown-endpoint-safety)
...
#!/usr/bin/env bash
printf 'tmux' >> "${XO_RUNTIME_LOG:?}"
printf ' <%s>' "$@" >> "${XO_RUNTIME_LOG:?}"
printf '\n' >> "${XO_RUNTIME_LOG:?}"
exit 0
SH
  cat > "$TMP_ROOT/$dir/fakebin/treehouse" <<'SH'
#!/usr/bin/env bash
printf 'treehouse' >> "${XO_RUNTIME_LOG:?}"
printf ' <%s>' "$@" >> "${XO_RUNTIME_LOG:?}"
printf '\n' >> "${XO_RUNTIME_LOG:?}"
exit 0
SH
...
claim_pool_slot() {  # <case> <task-id> [home]
  ...
  printf 'task=%s\nhome=%s\n' "$id" "$home" > "$dir/pool/1/.xo-slot-owner"
}

run_case() {  # <case> <id>
  local dir=$1 id=$2
  XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" \
  XO_RUNTIME_LOG="$dir/runtime.log" PATH="$dir/fakebin:$PATH" \
    "$TEARDOWN" "$id" --force
}
...
test_invalid_endpoint_records_refuse_before_mutation() {
  ...
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:xo-other-task" "endpoint_task_id=other-task" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  assert_refused_without_mutation "$dir" "$id" "task-mismatched endpoint"

  dir=$(make_case empty-binding)
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:xo-$id" "endpoint_task_id=" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  assert_refused_without_mutation "$dir" "$id" "empty task binding"

  dir=$(make_case duplicate-binding)
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:xo-$id" "endpoint_task_id=$id" "endpoint_task_id=$id" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  assert_refused_without_mutation "$dir" "$id" "duplicate task binding"

  pass "xo-teardown: missing, empty, malformed, ambiguous, and task-mismatched endpoints refuse before every mutation or runtime call"
}
...
test_control_lock_contention_refuses_before_mutation() {
  ...
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:xo-$id" "endpoint_task_id=$id" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  ...
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:xo-$id" "endpoint_task_id=other-task" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  ...
  kill "$holder" 2>/dev/null || true
  wait "$holder" 2>/dev/null || true
  pass "xo-teardown: a concurrent lifecycle action refuses before mutation"
}
...
test_non_pool_teardown_ignores_task_set_lock() {
  ...
  (
    ...
    . "$ROOT/bin/xo-wake-lib.sh"
    xo_lock_try_acquire "$lock" || exit 1
    trap 'xo_lock_release "$lock"' EXIT
    : > "$ready"
    sleep 30
  ) &
  holder=$!
  ...
  kill "$holder" 2>/dev/null || true
  wait "$holder" 2>/dev/null || true
  pass "xo-teardown: non-pool cleanup ignores unrelated task publication locks"
}
...
test_metadata_lock_serializes_destructive_cleanup() {
  ...
  pass "xo-teardown: destructive cleanup serializes with metadata writers"
}
...
test_supported_backend_endpoint_records_validate() {
  ...
  id=tmux-task
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=xo:xo-$id" "worktree=$dir/worktree" "project=$dir/project"
  xo_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid tmux endpoint refused"
  [ "$XO_BACKEND_VALIDATED_BACKEND:$XO_BACKEND_VALIDATED_TARGET" = "tmux:xo:xo-$id" ] || fail "tmux endpoint validation returned wrong identity"

  id=tmux-spaced-session
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=team work:xo-$id" "worktree=$dir/worktree" "project=$dir/project"
  xo_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid tmux endpoint with a spaced session name refused"
  [ "$XO_BACKEND_VALIDATED_TARGET" = "team work:xo-$id" ] || fail "tmux validation changed the spaced session identity"
  ...
  xo_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid Herdr endpoint refused"
  ...
  xo_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid Zellij endpoint refused"

  id=orca-task
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=xo-$id" "endpoint_task_id=$id" "terminal=term-7" \
    "worktree=$dir/worktree" "project=$dir/project" "backend=orca" "orca_worktree_id=worktree-9"
  xo_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid Orca endpoint refused"
  [ "$XO_BACKEND_VALIDATED_TARGET" = term-7 ] || fail "Orca validation did not select its terminal"
  ...
  xo_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid cmux endpoint refused"

  for backend in tmux herdr zellij orca cmux; do
    set +e
    xo_backend_kill "$backend" "" >/dev/null 2>&1
    target=$?
    set -e
...
test_tmux_empty_target_refuses_without_invocation() {
  ...
  set +e
  XO_RUNTIME_LOG="$dir/runtime.log" PATH="$dir/fakebin:$PATH" \
    bash -c '. "$1/bin/xo-backend.sh"; xo_backend_source tmux; xo_backend_tmux_kill ""' _ "$ROOT" \
...
test_isolated_tmux_invalid_and_valid_cleanup() {
  local dir socket socket_id session='endpoint safety' target_id=target control=control target=xo-target
  local prefix_target=xo-prefix prefix_survivor=xo-prefix2 rc
...
[ "\${XO_TEST_TMUX_SOCKET:-}" = '$socket_id' ] || exit 92
...
SH
  ...
  xo_write_meta "$dir/home/state/invalid.meta" \
    "window=" "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  set +e
  env -u TMUX -u TMUX_PANE XO_TEST_TMUX_SOCKET="$socket_id" \
    XO_HOME="$dir/home" XO_ROOT_OVERRIDE="$ROOT" XO_RUNTIME_LOG="$dir/runtime.log" \
  ...
  env -u TMUX -u TMUX_PANE XO_TEST_TMUX_SOCKET="$socket_id" XO_RUNTIME_LOG="$dir/runtime.log" \
    PATH="$dir/fakebin:$PATH" bash -c \
    '. "$1/bin/xo-backend.sh"; xo_backend_source tmux; xo_backend_tmux_kill ""' _ "$ROOT" \
  ...
  env -u TMUX -u TMUX_PANE XO_TEST_TMUX_SOCKET="$socket_id" XO_RUNTIME_LOG="$dir/runtime.log" \
    PATH="$dir/fakebin:$PATH" bash -c \
    '. "$1/bin/xo-backend.sh"; xo_backend_source tmux; xo_backend_tmux_kill "$2"' _ "$ROOT" "$session:$prefix_target"
  ...
  xo_write_meta "$dir/home/state/$target_id.meta" \
  ...
  pass "xo-teardown: exact tmux cleanup preserves invalid and prefix-matched neighbors while removing only the recorded target"
}
...
test_bare_relative_origin_shares_project_lock_with_clone() {
  ...
  primary_lock=$(XO_HOME="$dir/home" bash -c \
    '. "$1"; xo_treehouse_project_lock_path "$2"' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$dir/project") \
    || fail "could not resolve the primary project's bare-origin lock"
  clone_lock=$(XO_HOME="$dir/home" bash -c \
    '. "$1"; xo_treehouse_project_lock_path "$2"' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$second_project") \
...
test_reused_pool_slot_refuses_before_touching_the_other_task() {
  ...
  xo_write_meta "$dir/home/state/$id.meta" \
    ...
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  xo_write_meta "$dir/home/state/$other.meta" \
    "window=xo:xo-$other" "endpoint_task_id=$other" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  ...
  pass "xo-teardown: a pool slot named by a second task record is never returned, killed, or reset"
}
...
test_cross_home_pool_slot_collision_refuses() {
  ...
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=xo:xo-$id" "endpoint_task_id=$id" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  xo_write_meta "$second_home/state/$other.meta" \
    "window=xo:xo-$other" "endpoint_task_id=$other" \
  ...
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "teardown returned a pool slot held by another xo home"
  ...
  pass "xo-teardown: a pool slot held by another xo home is never returned"
}
...
test_sole_slot_record_still_tears_down() {
  ...
  xo_write_meta "$dir/home/state/neighbour.meta" \
    "window=xo:xo-neighbour" "endpoint_task_id=neighbour" \
  ...
  kill "$worker" 2>/dev/null || true
  wait "$worker" 2>/dev/null || true
  pass "xo-teardown: a task that solely holds its slot still returns it"
}
...
SH
  ...
  grep -Fq "tmux <kill-window> <-t> <=xo:=xo-$id>" "$dir/runtime.log" \
  ...
  pass "xo-teardown: an exact recorded endpoint still tears down after changing cwd outside its worktree"
}
...
write_local_parent_record() {  # <home> <parent-home>
  cat > "$1/.xo-secondmate-parent" <<REC
...
resolve_project_lock() {  # <home> <project>
  XO_HOME="$1" bash -c '. "$1"; xo_treehouse_project_lock_path "$2"' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$2"
}
...
test_remote_seeded_home_returns_its_uncontested_slot() {
  ...
  if [ "${XO_TEST_EVIDENCE:-0}" = 1 ]; then
    ...
    printf '$ XO_HOME=%s bin/xo-teardown.sh %s --force\n' "$dir/home" "$id"
    printf 'stdout:\n'; cat "$dir/stdout"
    printf 'stderr:\n'; cat "$dir/stderr"
  ...
  pass "xo-teardown: a remote-seeded secondmate home returns its own uncontested pool slot"
}
...
test_remote_seeded_home_still_refuses_a_slot_its_child_holds() {
  ...
  xo_write_meta "$dir/home/state/$id.meta" \
    "window=xo:xo-$id" "endpoint_task_id=$id" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=scout"
  xo_write_meta "$child_home/state/$other.meta" \
    "window=xo:xo-$other" "endpoint_task_id=$other" \
  ...
  pass "xo-teardown: slot ownership across a remote-seeded home and its local child still refuses"
}
...
test_remote_layout_homes_serialize_on_one_project_lock() {
  ...
  XO_HOME="$child_home" bash -c \
    '. "$1"; xo_lock_try_acquire "$2" || exit 1; : > "$3"; exec sleep 30' _ \
    "$ROOT/bin/xo-wake-lib.sh" "$lock" "$dir/lock-held" &
  holder=$!
...
assert_reassigned_slot_left_alone() {  # <case> <id> <other> <description>
  ...
  assert_present "$dir/pool/1/.xo-slot-owner" "$description: another task's slot claim was removed"
  assert_contains "$(cat "$dir/pool/1/.xo-slot-owner")" "task=$other" \
...
test_reassigned_pool_slot_finishes_own_cleanup_without_touching_the_slot() {
  ...
  assert_present "$dir/pool/1/.xo-slot-owner" "teardown removed an unreadable slot claim"
  ...
  assert_contains "$(cat "$dir/stderr")" "$dir/pool/1/.xo-slot-owner" \
    "unreadable-claim refusal should name the claim file to inspect"

  pass "xo-teardown: a pool slot claimed by another task is left alone while the task's own cleanup finishes"
}
...
test_own_and_absent_slot_claims_still_tear_down() {
  ...
  assert_absent "$dir/pool/1/.xo-slot-owner" "own-claim teardown left its spent slot claim behind"
  grep -Fq "treehouse <return>" "$dir/runtime.log" \
  ...
  pass "xo-teardown: a task's own slot claim, and an unclaimed slot, both still tear down"
}
