#!/usr/bin/env bash
# tests/xo-watcher-lock.test.sh - watcher singleton + lock-primitive races +
...
WATCH_ARM="$ROOT/bin/xo-watch-arm.sh"
DRAIN="$ROOT/bin/xo-wake-drain.sh"
LIB="$ROOT/bin/xo-wake-lib.sh"
...
# ARM_CONFIRM_DEFAULT in bin/xo-watch-arm.sh). This is a ceiling spent only when
# an arm genuinely fails to exit; a passing case returns as soon as it does.
ARM_FAIL_EXIT_POLLS=400

TMP_ROOT=$(xo_test_tmproot xo-watcher-lock-tests)
...
test_singleton_start() {
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out1" &
  pid1=$!
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out2" &
  pid2=$!
  i=0
  while [ "$i" -lt 50 ]; do
    live=0
...
test_stale_watch_lock_reclaimed() {
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  i=0
  live=0
  lock_pid=
...
test_live_stale_watch_lock_is_actionable() {
  ...
  status=0
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_GUARD_GRACE=1 XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" 2> "$err" || status=$?
...
test_guard_warnings() {
  ...
  # XO_ROOT_OVERRIDE points the worktree-tangle check at a non-git dir so it stays
  ...
  CLAUDECODE=1 PI_CODING_AGENT='' GROK_AGENT='' XO_ROOT_OVERRIDE="$dir" XO_STATE_OVERRIDE="$state" XO_GUARD_GRACE=1 "$ROOT/bin/xo-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  ...
  sleep 60 &
  pid=$!
  identity=$(XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$pid") || fail "could not identify fresh guard watcher"
  ...
  # Non-git XO_ROOT keeps the worktree-tangle check inert so "fresh watcher ->
  # total silence" stays a pure assertion about watcher state.
  XO_ROOT_OVERRIDE="$dir" XO_STATE_OVERRIDE="$state" XO_GUARD_GRACE=300 "$ROOT/bin/xo-guard.sh" 2> "$err" >/dev/null || fail "guard failed"
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
...
test_lock_single_winner_under_concurrency() {
  ...
  pass "concurrent xo_lock_try_acquire yields exactly one winner"
}
...
test_lock_steals_dead_pid_lock() {
  ...
  printf '%s\n' "$dead" > "$lockdir/pid"
  rc=0
  newpid=$(XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if xo_lock_try_acquire "$2"; then cat "$2/pid"; else exit 7; fi
  ' _ "$LIB" "$lockdir") || rc=$?
...
test_lock_live_steal_mutex_is_not_reclaimed() {
  ...
  XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    xo_lock_try_acquire "$2.steal" || exit 7
    printf "%s\n" "${BASHPID:-$$}" > "$3"
    sleep 2
    xo_lock_release "$2.steal"
  ...
  out=$(XO_LOCK_STALE_AFTER=0 XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if xo_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s lockpid=%s stealpid=%s\n" "$rc" "${XO_LOCK_HELD_PID:-}" "$(cat "$2/pid" 2>/dev/null || true)" "$(cat "$2.steal/pid" 2>/dev/null || true)"
  ' _ "$LIB" "$lockdir")
...
test_lock_does_not_steal_live_lock() {
  ...
  mkdir "$lockdir"
  printf '%s\n' "$live" > "$lockdir/pid"
  out=$(XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    if xo_lock_try_acquire "$2"; then rc=0; else rc=1; fi
    printf "rc=%s held=%s\n" "$rc" "${XO_LOCK_HELD_PID:-}"
  ' _ "$LIB" "$lockdir")
  ...
  case "$out" in
    *"held=$live"*) ;;
    *) fail "live holder pid not reported via XO_LOCK_HELD_PID: $out" ;;
  esac
...
test_lock_late_claim_loses_after_recreate() {
  ...
  out=$(XO_LOCK_STALE_AFTER=0 XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    owner1=$(xo_lock_owner_dir "$2") || exit 20
    ln -s "$owner1" "$2" || exit 21
    touch -h -t 200001010000 "$2" 2>/dev/null || sleep 2
    if ! xo_lock_try_acquire "$2"; then exit 22; fi
    before=$(cat "$2/pid" 2>/dev/null || true)
    if xo_lock_claim "$2" "$owner1"; then late=won; else late=lost; fi
...
test_lock_paused_mid_acquire_claim_fails_during_steal() {
  ...
  out=$(XO_LOCK_STALE_AFTER=0 XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    owner=$(xo_lock_owner_dir "$2") || exit 20
    ln -s "$owner" "$2" || exit 21
    xo_lock_try_acquire "$2.steal" || exit 22
    steal_owner=${XO_LOCK_OWNER_DIR:-}
    if xo_lock_claim "$2" "$owner"; then late=won; else late=lost; fi
    if xo_lock_try_create "$2" "$steal_owner"; then stealer=won; else stealer=lost; fi
...
test_watch_restart_rejects_reused_pid() {
  ...
  PATH="$fakebin:$PATH" XO_HOME="$dir" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH_ARM" --restart > "$out" &
  pid=$!
  i=0
...
test_watch_restart_attaches_to_healthy_peer() {
  ...
  identity=$(XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$peer") || fail "could not identify peer pid"
  mkdir "$state/.watch.lock"
  ...
  printf '%s\n' "$identity" > "$state/.watch.lock/pid-identity"
  touch "$state/.last-watcher-beat"
  PATH="$fakebin:$PATH" XO_HOME="$dir" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 XO_ARM_ATTACH_POLL=0.1 XO_ARM_CONFIRM_TIMEOUT=1 "$WATCH_ARM" --restart > "$out" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_watcher_self_evicts_on_lock_takeover() {
  ...
  fakebin="$dir/fakebin"
  out="$dir/watch.out"
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=0.2 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH" > "$out" &
  pid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_self_eviction_is_loud_without_successor() {
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=0.2 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_attaches_and_waits_for_live_fresh_watcher() {
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_ARM_ATTACH_POLL=0.1 XO_ARM_CONFIRM_TIMEOUT=1 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
...
test_arm_starts_and_self_heals() {
  ...
  for row in clean dead-pid; do
    ...
    PATH="$fakebin:$PATH" XO_HOME="$dir" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    armpid=$!
    i=0
    while [ "$i" -lt 80 ]; do
      if [ "$row" = dead-pid ]; then
...
test_arm_hup_cleans_child_and_temp_output() {
  ...
  fakebin="$dir/fakebin"
  armout="$dir/arm.out"
  PATH="$fakebin:$PATH" XO_HOME="$dir" XO_STATE_OVERRIDE="$state" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  armpid=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
SH
  ...
  # settles (issue #2844).
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_GUARD_GRACE=0 XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=0 XO_HEARTBEAT=999999 XO_ARM_CONFIRM_TIMEOUT=60 "$WATCH_ARM" > "$armout" || rc=$?
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after immediate arm wake failed"
...
test_arm_waits_for_peer_beacon_after_child_stands_down() {
  ...
  PATH="$fakebin:$PATH" XO_HOME="$dir" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 XO_ARM_ATTACH_POLL=0.1 "$WATCH_ARM" > "$armout" &
  armpid=$!
...
test_arm_fails_loud_when_no_fresh_watcher_confirmable() {
  ...
  printf '%s\n' "$live" > "$state/.watch.lock/pid"
  touch -t 200001010000 "$state/.last-watcher-beat"
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 XO_ARM_CONFIRM_TIMEOUT=3 "$WATCH_ARM" > "$armout" &
  armpid=$!
...
SH
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_GUARD_GRACE=0 XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=0 XO_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  first_arm=$!
  ...
  PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_WATCH_PREDECESSOR_ARM_PID="$first_arm" XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
  successor_arm=$!
  i=0
  while [ "$i" -lt 80 ]; do
    grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
  ...
  while [ "$iteration" -lt 6 ]; do
    ...
    PATH="$fakebin:$PATH" XO_STATE_OVERRIDE="$state" XO_WATCH_CYCLE_LOG_MAX_BYTES=1400 XO_WATCH_CYCLE_LOG_KEEP_LINES=2 XO_POLL=5 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 XO_HEARTBEAT=999999 "$WATCH_ARM" > "$armout" &
    successor_arm=$!
    i=0
    while [ "$i" -lt 80 ]; do
      grep -qF 'watcher: started pid=' "$armout" 2>/dev/null && break
...
test_stopped_watcher_is_live_but_stale_then_exit_is_classified() {
  ...
  touch -t 200001010000 "$state/.last-watcher-beat"
  XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_pid_alive "$2"' _ "$LIB" "$watcher_pid" \
    || fail "SIGSTOP watcher was not classified as a live pid"
  if XO_HOME="$dir" XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_watcher_healthy "$2" "$3" 300 "$4"' _ "$LIB" "$state" "$WATCH" "$dir"; then
...
test_pid_identity_is_locale_invariant() {
  ...
  # xo_pid_identity, so its output must be byte-identical regardless of the caller's
  ...
  # LC_ALL=C pin in xo_pid_identity therefore breaks the equality assertions below
...
SH
  chmod +x "$fakebin/ps"
  baseline=$(PATH="$fakebin:$PATH" FAKE_PS_LOCALE_LOG="$locale_log" XO_PROC_ROOT_OVERRIDE="$no_proc" LC_ALL=C bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  via_lc_all=$(PATH="$fakebin:$PATH" FAKE_PS_LOCALE_LOG="$locale_log" XO_PROC_ROOT_OVERRIDE="$no_proc" LC_ALL=ko_KR.UTF-8 bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  via_lc_time=$(PATH="$fakebin:$PATH" FAKE_PS_LOCALE_LOG="$locale_log" XO_PROC_ROOT_OVERRIDE="$no_proc" LC_TIME=ko_KR.UTF-8 bash -c 'unset LC_ALL; . "$1"; xo_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  ...
  if LC_ALL=C ps -p "$live" -o lstart= -o command= >/dev/null 2>&1; then
    real_first=$(XO_PROC_ROOT_OVERRIDE="$no_proc" LC_ALL=C bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
    real_second=$(XO_PROC_ROOT_OVERRIDE="$no_proc" LC_TIME=ko_KR.UTF-8 bash -c 'unset LC_ALL; . "$1"; xo_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  fi
  kill "$live" 2>/dev/null || true
  wait "$live" 2>/dev/null || true
  [ -n "$baseline" ] || fail "xo_pid_identity produced no baseline identity under LC_ALL=C"
  [ "$via_lc_all" = "$baseline" ] || fail "xo_pid_identity varied with exported LC_ALL (got '$via_lc_all', want '$baseline')"
  [ "$via_lc_time" = "$baseline" ] || fail "xo_pid_identity varied with exported LC_TIME (got '$via_lc_time', want '$baseline')"
  while read -r observed; do
    [ "$observed" = C ] || fail "xo_pid_identity invoked ps without pinning LC_ALL=C (saw '$observed')"
  done < "$locale_log"
  if [ -n "$real_first" ]; then
    ...
    pass "xo_pid_identity real ps fallback is locale-invariant"
  else
    pass "real ps fallback locale check skipped where ps -o lstart= is unsupported"
  fi
  pass "xo_pid_identity is locale-invariant across LC_ALL/LC_TIME"
}
...
write_fake_proc_identity() {
  ...
  printf 'bash\0/path with spaces/xo-watch.sh\0--flag\0' > "$proc_root/$pid/cmdline"
}
...
test_proc_pid_identity_ignores_wall_clock_and_detects_pid_reuse() {
  ...
  before=$(XO_PROC_ROOT_OVERRIDE="$proc_root" XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$pid") \
    || fail "could not read initial fake Linux process identity"
  printf 'btime 1784094016\n' > "$proc_root/stat"
  after_time_jump=$(XO_PROC_ROOT_OVERRIDE="$proc_root" XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$pid") \
  ...
  [ "$before" = "$identity_key=987654 cmdline-hex=62617368002f706174682077697468207370616365732f786f2d77617463682e7368002d2d666c616700" ] \
  ...
  after_pid_reuse=$(XO_PROC_ROOT_OVERRIDE="$proc_root" XO_STATE_OVERRIDE="$state" bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$pid") \
...
test_stale_watch_reclaim_publishes_before_clear() {
  ...
  token=$(XO_STATE_OVERRIDE="$state" bash -c '
    . "$1"
    xo_recovery_marker_read "$2" || exit 1
...
test_msys_pid_identity_uses_proc() {
  ...
  sleep 300 &
  live=$!
  identity=$(bash -c '. "$1"; xo_pid_identity "$2"' _ "$LIB" "$live" 2>/dev/null)
  kill "$live" 2>/dev/null || true
  wait "$live" 2>/dev/null || true
  case "$identity" in
