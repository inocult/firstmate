#!/usr/bin/env bash
# tests/xo-composer-matrix-live-e2e.test.sh - the live composer-matrix guard
# (live-harness-optin family; task xo-composer-thin-adapter-refactor-r1).
#
# The shared composer classifier's shape catalogue (bin/xo-composer-lib.sh) is
# built entirely from vendor-rendered signals, so per
# .agents/skills/xo-coding-guidelines it must be proven against the
...
# isolated tmux server and requires the real xo_tmux_composer_state to reach
...
#
# Run explicitly with XO_COMPOSER_MATRIX_LIVE=1. No prompt is ever submitted
...
xo_live_gate opt-in XO_COMPOSER_MATRIX_LIVE tmux

SOCKET="xo-cmx-live-$$"
SESSION="cmxlive"
ZELLIJ_SESSION="xo-cmx-live-zj-$$"
CHECKED=0
FAILED=0
...
SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/xo-cmx-live.XXXXXX")
...
check_harness_idle_empty() {  # <name> <launch-cmd...>
  local name=$1 win="hx-$1" verdict='' i=0 budget=${XO_COMPOSER_MATRIX_LIVE_POLLS:-45} version dismissed=0 startup_screen
  shift
  ...
  while [ "$i" -lt "$budget" ]; do
    verdict=$(xo_tmux_composer_state "$SESSION:$win")
    [ "$verdict" = empty ] && break
    i=$((i + 1))
...
sleep 1
verdict=$(xo_tmux_composer_state "$SESSION:strictblank")
if [ "$verdict" = unknown ]; then
  if xo_pane_input_pending "$SESSION:strictblank"; then
    CHECKED=$((CHECKED + 1))
...
if command -v zellij >/dev/null 2>&1; then
  ...
  export XO_ROOT_OVERRIDE="$ROOT"
  # shellcheck source=/dev/null
  . "$ROOT/bin/xo-backend.sh"
  xo_backend_source zellij 2>/dev/null \
  ...
  ZJ_BG=$!
  i=0
  while [ "$i" -lt 10 ] && ! xo_backend_zellij_session_exists "$ZELLIJ_SESSION"; do
    i=$((i + 1))
    sleep 0.5
  done
  xo_backend_zellij_session_exists "$ZELLIJ_SESSION" \
    || fail "zellij ($zj_version): probe session setup failed"
  panes=$(xo_backend_zellij_cli "$ZELLIJ_SESSION" action list-panes --json 2>/dev/null) \
  ...
  xo_backend_zellij_send_literal "$target" 'while sleep 1; do date; done' \
    || fail "zellij ($zj_version): clock probe setup write failed"
  xo_backend_zellij_send_key "$target" Enter \
    || fail "zellij ($zj_version): clock probe setup submit failed"
  sleep 2
  probe='# audit-probe-never-submitted'
  xo_backend_zellij_send_literal "$target" "$probe" \
    || fail "zellij ($zj_version): false-positive probe write failed"
  sleep 0.5
  probe_capture=$(xo_backend_zellij_capture "$target" 40 2>/dev/null) \
  ...
  verdict=$(xo_composer_submit_retry_core xo_backend_zellij_send_key xo_backend_zellij_composer_state \
