#!/usr/bin/env bash
# Behavior tests for tests/lib.sh's xo_live_gate, the single decision every
...
#
# The family sweep at the end runs every real live guard with XO_LIVE=0 and
...
TMP_ROOT=$(xo_test_tmproot xo-live-gate)
BIN="$TMP_ROOT/bin"
mkdir -p "$BIN"
...
test_default_on_runs_when_the_tool_is_installed() {
  local path result
  path=$(guard default-present default-on XO_FAKE_LIVE fmfakeharness)
...
test_default_on_skips_and_names_the_absent_tool() {
  local path result
  path=$(guard default-absent default-on XO_FAKE_LIVE fmmissingharness)
  result=$(run_guard "$path")
...
test_opt_in_stays_off_until_asked() {
  local path result
  path=$(guard optin-idle opt-in XO_FAKE_LIVE fmfakeharness)
  result=$(run_guard "$path")
  [ "$(printf '%s' "$result" | sed -n 1p)" = 0 ] || fail "an unrequested opt-in guard must skip cleanly: $result"
  assert_contains "$result" "skip: live: opt-in; set XO_FAKE_LIVE=1 to run" \
...
test_own_variable_turns_an_opt_in_guard_on() {
  local path result
  path=$(guard optin-on opt-in XO_FAKE_LIVE fmfakeharness)
  result=$(run_guard "$path" XO_FAKE_LIVE=1)
  [ "$(printf '%s' "$result" | sed -n 1p)" = 0 ] || fail "XO_FAKE_LIVE=1 must run the guard: $result"
...
test_requested_run_fails_rather_than_skipping_on_an_absent_tool() {
  local path result
  path=$(guard optin-strict opt-in XO_FAKE_LIVE fmmissingharness)
  result=$(run_guard "$path" XO_FAKE_LIVE=1)
  [ "$(printf '%s' "$result" | sed -n 1p)" = 1 ] || fail "a demanded run with no tool must fail, not skip: $result"
  assert_contains "$result" "XO_FAKE_LIVE was requested but fmmissingharness is not installed" \
...
test_xo_live_turns_the_whole_family_off_and_on() {
  local path result
  path=$(guard fmlive-off default-on XO_FAKE_LIVE fmfakeharness)
  result=$(run_guard "$path" XO_LIVE=0)
  [ "$(printf '%s' "$result" | sed -n 1p)" = 0 ] || fail "XO_LIVE=0 must skip cleanly: $result"
  assert_contains "$result" "skip: live: disabled by XO_LIVE=0" "XO_LIVE=0 must say why it skipped"

  path=$(guard fmlive-on opt-in XO_FAKE_LIVE fmfakeharness)
  result=$(run_guard "$path" XO_LIVE=1)
  assert_contains "$result" ran "XO_LIVE=1 must turn an opt-in guard on"

  path=$(guard fmlive-on-strict opt-in XO_FAKE_LIVE fmmissingharness)
  result=$(run_guard "$path" XO_LIVE=1)
  [ "$(printf '%s' "$result" | sed -n 1p)" = 1 ] || fail "XO_LIVE=1 must make an absent tool a failure: $result"
}

test_a_guards_own_variable_wins_over_xo_live() {
  local path result
  path=$(guard own-off default-on XO_FAKE_LIVE fmfakeharness)
  result=$(run_guard "$path" XO_LIVE=1 XO_FAKE_LIVE=0)
  [ "$(printf '%s' "$result" | sed -n 1p)" = 0 ] || fail "an explicit per-guard opt-out must skip cleanly: $result"
  assert_contains "$result" "skip: live: disabled by XO_FAKE_LIVE=0" \
    "a guard's own 0 must win over XO_LIVE=1 and say so"
...
test_any_of_several_entry_points_turns_a_guard_on() {
  local path result
  path=$(guard multi opt-in XO_FAKE_LIVE,XO_FAKE_ALT_LIVE fmfakeharness)
  result=$(run_guard "$path")
  assert_contains "$result" "set XO_FAKE_LIVE=1 to run" \
    "a multi-entry guard must point at its primary variable when idle"
  result=$(run_guard "$path" XO_FAKE_ALT_LIVE=1)
...
test_gate_lets_a_guard_drive_the_real_fleet_scripts_under_a_gate_marker() {
  ...
  # refused by bin/xo-gate-refuse-lib.sh whenever the pipeline ran them, because
  ...
  path="$TMP_ROOT/bypass.test.sh"
  {
    printf '#!/usr/bin/env bash\nset -u\n'
    printf '. "%s/tests/lib.sh"\n' "$ROOT"
    printf 'xo_live_gate default-on XO_FAKE_LIVE fmfakeharness\n'
    printf '. "%s/bin/xo-gate-refuse-lib.sh"\n' "$ROOT"
    printf 'if xo_is_gate_agent; then printf "refused\\n"; else printf "allowed\\n"; fi\n'
  } > "$path"
  chmod +x "$path"
  set +e
...
test_every_live_guard_is_wired_to_the_shared_gate() {
  ...
  listing=$("$ROOT/bin/xo-test-run.sh" --family live-harness-optin --list) \
    || fail "could not list the live-harness family"
  while IFS= read -r script; do
    [ -n "$script" ] || continue
    script="$ROOT/$script"
    # bin/xo-test-run.sh runs every script through bash, so a guard that is not
    # marked executable is still a real suite member here.
    out=$(clean_env XO_LIVE=0 bash "$script" 2>&1) || fail "$(basename "$script") must exit 0 when live guards are disabled"
    assert_contains "$out" "skip: live: disabled by XO_LIVE=0" \
...
EOF
  ...
  pass "all $checked live guards refuse together on XO_LIVE=0"
}
...
test_xo_live_turns_the_whole_family_off_and_on
pass "XO_LIVE switches the whole family"
test_a_guards_own_variable_wins_over_xo_live
pass "a guard's own setting wins over XO_LIVE"
test_any_of_several_entry_points_turns_a_guard_on
