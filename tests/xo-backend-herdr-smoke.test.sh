#!/usr/bin/env bash
# tests/xo-backend-herdr-smoke.test.sh - real herdr smoke test for the herdr
# session-provider adapter (bin/backends/herdr.sh), P2 of
# data/xo-backend-design-d7 (herdr-addendum.md), extended for the P3
# workspace-per-home pass (AGENTS.md task herdr-sm-spaces-k4). Mirrors
# tests/xo-backend-tmux-smoke.test.sh's structure: every other suite fakes the
...
xo_herdr_lab_prepare "$SESSION" || fail "could not prepare isolated Herdr lab session"

# shellcheck source=/dev/null
. "$ROOT/bin/xo-backend.sh"
xo_backend_source herdr || fail "xo_backend_source herdr failed"

# --- version gate + container ensure -----------------------------------------

xo_backend_herdr_version_check || fail "version_check failed against the real installed herdr"
pass "real herdr: version_check accepts the installed binary's protocol"

# xo_backend_herdr_container_ensure now echoes
...
# single tab character; only xo_backend_herdr_create_task is ever allowed to
# act on the seeded tab id, and only for the container that just created it.
CONTAINER_RAW=$(xo_backend_herdr_container_ensure /tmp) || fail "container_ensure failed"
...
pass "real herdr: container_ensure starts the isolated session's server, creates the xo workspace ($CONTAINER), and reports its seeded default tab id ($SEEDED_TAB_ID)"
...
# protocols.
CLIENT_STATUS=$(xo_backend_herdr_client_status "$(command -v herdr)" "$SESSION")
...
# a tab as prunable.
CONTAINER2_RAW=$(xo_backend_herdr_container_ensure /tmp) || fail "second container_ensure failed"
...
pass "real herdr: container_ensure is idempotent (reuses/adopts the existing xo workspace, reports no seeded default tab on adoption)"

# --- create_task + duplicate refusal + default-tab prune ---------------------

LABEL="xo-smoke1"
TASK_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "$LABEL" /tmp "$SEEDED_TAB_ID") || fail "create_task failed"
...
# exactly one clean xo-<id> task tab.
...
pass "real herdr: create_task prunes the freshly-created workspace's seeded default tab, leaving exactly one clean xo-<id> task tab"
...
# server restart, and a restored xo-<id> task tab comes back a HUSK: a dead
...
LIVE_DUP_LABEL="xo-smoke-livedup"
LIVE_DUP_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "$LIVE_DUP_LABEL" /tmp) || fail "could not create the live-duplicate scenario's tab"
...
herdr pane report-agent "$LIVE_DUP_PANE_ID" --source xo-smoke-test --agent xo-smoke-live-agent --state idle --session "$SESSION" >/dev/null 2>&1 \
  || fail "could not register a live agent on the live-duplicate scenario's pane"
if xo_backend_herdr_create_task "$CONTAINER" "$LIVE_DUP_LABEL" /tmp >/dev/null 2>&1; then
...
xo_backend_herdr_kill "$SESSION:$LIVE_DUP_PANE_ID"

# 2. A husk (no registered agent at all - the restored-plain-shell shape)
#    must be CLOSED AND REPLACED instead of refused.
HUSK_LABEL="xo-smoke-husk1"
HUSK_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "$HUSK_LABEL" /tmp) || fail "could not create the husk-simulation tab"
...
REPLACED_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "$HUSK_LABEL" /tmp) \
...
xo_backend_herdr_kill "$SESSION:$NEW_HUSK_PANE_ID"
...
SM_SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/xo-herdr-smoke-sm.XXXXXX")
SM_HOME="$SM_SCRATCH/secondmate-home"
mkdir -p "$SM_HOME"
printf 'smoketest-sm1\n' > "$SM_HOME/.xo-secondmate-home"

SM_CONTAINER_RAW=$(XO_HOME="$SM_HOME" xo_backend_herdr_container_ensure /tmp) || fail "secondmate-shaped container_ensure failed"
...
pass "real herdr: a secondmate-shaped home (.xo-secondmate-home) gets its OWN herdr workspace, distinct from the primary's, in the SAME session"
...
SM_TASK_LABEL="xo-smtask1"
SM_TASK_IDS=$(XO_HOME="$SM_HOME" xo_backend_herdr_create_task "$SM_CONTAINER" "$SM_TASK_LABEL" /tmp "$SM_SEEDED_TAB_ID") || fail "secondmate create_task failed"
...
xo_herdr_lab_stop "$SESSION" >/dev/null 2>&1 \
  || fail "could not stop the isolated session for the restart-stability check"
sleep 0.5
xo_backend_herdr_server_ensure "$SESSION" || fail "the isolated session's server did not come back up after the stop"

POST_LIST=$(herdr workspace list --session "$SESSION" 2>&1)
POST_PRIMARY_ID=$(printf '%s' "$POST_LIST" | jq -r '.result.workspaces[]? | select(.label == "xo") | .workspace_id')
...
xo_backend_herdr_kill "$SESSION:$SM_PANE_ID"

# --- send_text_line (atomic run) ---------------------------------------------

xo_backend_herdr_send_text_line "$TARGET" "echo captain-on-deck-line" \
  || fail "send_text_line failed"
sleep 0.5
out=$(xo_backend_herdr_capture "$TARGET" 20) || fail "capture failed after send_text_line"
case "$out" in
...
xo_backend_herdr_send_literal "$TARGET" 'echo literal-then-key-captain' \
  || fail "send_literal failed"
sleep 0.2
xo_backend_herdr_send_key "$TARGET" Enter || fail "send_key Enter failed"
sleep 0.5
out=$(xo_backend_herdr_capture "$TARGET" 20) || fail "capture failed after send_literal+send_key"
case "$out" in
...
xo_backend_herdr_send_text_line "$TARGET" "cd /tmp"
sleep 0.3
p=$(xo_backend_herdr_current_path "$TARGET") || fail "current_path failed"
case "$p" in
  */tmp) : ;;
...
if [ "${XO_HERDR_SMOKE_REAL_CLAUDE:-0}" = 1 ] && command -v claude >/dev/null 2>&1; then
  xo_backend_herdr_send_literal "$TARGET" "CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '{\"feedbackDrafts\":\"off\"}' --print 'say the word HERDRSMOKEOK and nothing else'"
  sleep 0.2
  xo_backend_herdr_send_key "$TARGET" Enter
  found_working=0
  for _ in $(seq 1 20); do
    bs=$(xo_backend_herdr_busy_state "$TARGET" 2>/dev/null)
  ...
  out=$(xo_backend_herdr_capture "$TARGET" 30)
  case "$out" in
...
elif [ "${XO_HERDR_SMOKE_REAL_CLAUDE:-0}" != 1 ]; then
  echo "note: XO_HERDR_SMOKE_REAL_CLAUDE=1 not set; skipping the real-agent busy_state check" >&2
else
...
xo_backend_herdr_kill "$TARGET" || fail "kill on an already-dead target must stay best-effort (never fail)"
pass "real herdr: kill removes the pane and is idempotent/best-effort"

# --- list_live (label-based recovery discovery) ------------------------------

# Real xo spawns always re-run container_ensure immediately before
# create_task (bin/xo-spawn.sh), never reusing a container reference from an
...
# workspace.
CONTAINER_RAW=$(xo_backend_herdr_container_ensure /tmp) || fail "container_ensure for the second task failed"
CONTAINER=${CONTAINER_RAW%%$'\t'*}
SEEDED_TAB_ID=${CONTAINER_RAW#*$'\t'}
...
LABEL2="xo-smoke2"
TASK_IDS2=$(xo_backend_herdr_create_task "$CONTAINER" "$LABEL2" /tmp "$SEEDED_TAB_ID") || fail "second create_task failed"
read -r _TAB_ID2 PANE_ID2 <<EOF
$TASK_IDS2
EOF
live=$(xo_backend_herdr_list_live "$SESSION")
...
pass "real herdr: list_live discovers a live task tab by xo-<id> label"

xo_backend_herdr_kill "$SESSION:$PANE_ID2"
