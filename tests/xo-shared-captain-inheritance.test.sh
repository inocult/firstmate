...
TMP_ROOT=$(xo_test_tmproot xo-shared-captain)
...
assert_shared_readonly() {
  local path=$1
  [ "$(file_mode "$path")" = "$XO_SHARED_CAPTAIN_MODE" ] \
    || fail "$path mode should be $XO_SHARED_CAPTAIN_MODE, got $(file_mode "$path")"
}
...
test_first_copy_readonly_and_local_files_preserved() {
  ...
  out=$(XO_CONFIG_INHERIT_REPORT="$report" propagate_secondmate_inheritance "$primary" "$second")
...
test_drift_quarantine_collision_and_repeated_convergence() {
  ...
  chmod "$XO_SHARED_CAPTAIN_MODE" "$second/data/captain-shared.md"
  hash=$(xo_inherit_sha256 "$second/data/captain-shared.md")
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" XO_CONFIG_INHERIT_REPORT="$report" \
...
add_bootstrap_compatible_tools() {
  local fakebin=$1
  xo_fake_exit0 "$fakebin" node chrome-devtools-axi gh treehouse
...
EOF
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$data_override" \
    XO_PROJECTS_OVERRIDE="$home/projects" XO_CONFIG_OVERRIDE="$home/config" \
    XO_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/xo-spawn.sh" sm "$sm" codex --secondmate >/dev/null 2>&1 || true

  cmp -s "$data_override/captain-shared.md" "$sm/data/captain-shared.md" \
    || fail "spawn convergence point did not copy shared captain preferences from XO_DATA_OVERRIDE"
  assert_shared_readonly "$sm/data/captain-shared.md"
  pass "spawn convergence point propagates data/captain-shared.md from XO_DATA_OVERRIDE"
}
...
EOF
  ...
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$root" \
    XO_DATA_OVERRIDE="$data_override" \
  ...
  cmp -s "$data_override/captain-shared.md" "$sm/data/captain-shared.md" \
    || fail "bootstrap convergence point did not copy shared captain preferences from XO_DATA_OVERRIDE"
  assert_shared_readonly "$sm/data/captain-shared.md"
  pass "bootstrap convergence point propagates data/captain-shared.md from XO_DATA_OVERRIDE"
}
...
EOF
  ...
  chmod "$XO_SHARED_CAPTAIN_MODE" "$sm/data/captain-shared.md"
  write_shared "$data_override/captain-shared.md" "changed override shared bytes"

  out=$(PATH="$BASE_PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$root" \
    XO_DATA_OVERRIDE="$data_override" \
    "$ROOT/bin/xo-config-push.sh" 2>/dev/null)
  ...
  cmp -s "$data_override/captain-shared.md" "$sm/data/captain-shared.md" \
    || fail "config-push convergence point did not update shared captain preferences from XO_DATA_OVERRIDE"
  assert_shared_readonly "$sm/data/captain-shared.md"
  pass "xo-config-push convergence point updates changed shared captain source bytes from XO_DATA_OVERRIDE"
}
...
EOF
  fakebin=$(make_fake_spawn_toolchain "$w")
  add_bootstrap_compatible_tools "$fakebin"
  xo_fake_exit0 "$fakebin" pgrep

  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$root" \
    "$ROOT/bin/xo-session-start.sh")
...
echo "# all xo-shared-captain-inheritance tests passed"
