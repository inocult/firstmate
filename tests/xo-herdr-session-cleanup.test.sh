#!/usr/bin/env bash
# Focused safety tests for bin/xo-herdr-session-cleanup.sh.
...
TMP_ROOT=$(xo_test_tmproot xo-herdr-session-cleanup)
XO_TEST_CLEANUP_DIRS+=("$TMP_ROOT")
trap xo_test_cleanup EXIT

export XO_HOME="$TMP_ROOT/home"
export XO_STATE_OVERRIDE="$XO_HOME/state"
export XO_CONFIG_OVERRIDE="$XO_HOME/config"
mkdir -p "$XO_STATE_OVERRIDE" "$XO_CONFIG_OVERRIDE"
touch "$XO_CONFIG_OVERRIDE/herdr-presentation-spaces"
printf '%s\n' herdr > "$XO_CONFIG_OVERRIDE/backend"
FAKEBIN=$(xo_fakebin "$TMP_ROOT")
xo_fake_exit0 "$FAKEBIN" herdr
export PATH="$FAKEBIN:$PATH"
export XO_HERDR_SESSION_CLEANUP_SOURCE_ONLY=1
# shellcheck source=/dev/null
. "$ROOT/bin/xo-herdr-session-cleanup.sh"
unset XO_HERDR_SESSION_CLEANUP_SOURCE_ONLY

# The idle-shell proof now lives in the backend as
# xo_backend_herdr_pane_idle_shell_pid; prove it still reads Linux argv
...
argv_pid=$(
  ...
  XO_HERDR_PS_BIN="$FAKE_PS" xo_backend_herdr_pane_idle_shell_pid test w2:p1
...
if (
  ...
  XO_HERDR_PS_BIN="$FAKE_PS" XO_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
    xo_backend_herdr_pane_idle_shell_pid test w2:p1
) >/dev/null 2>&1; then
...
xo_backend_name() { printf herdr; }
xo_backend_herdr_session() { printf test; }
xo_backend_herdr_presentation_session_lock_path() { printf '%s/presentation.lock' "$TMP_ROOT"; }
xo_lock_try_acquire() {
  printf '%s\n' "$1" >> "$LOCK_LOG"
  mkdir "$1" 2>/dev/null
}
xo_lock_release() { rm -rf -- "$1"; }
xo_backend_herdr_pane_idle_shell_pid() { [ ! -e "$FIXTURE_DIR/process-unsafe" ] && printf '67\n'; }
...
fixture_workspaces() {
  ...
  printf '[{"workspace_id":"w1","label":"xo","focused":%s,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},' \
...
fixture_tabs() {
  ...
  while [ "$i" -le "$count" ]; do
    [ "$i" -eq 1 ] || printf ','
    printf '{"tab_id":"%s:t%s","workspace_id":"%s","focused":false,"label":"xo-task"}' "$WS" "$i" "$WS"
    i=$((i + 1))
  done
  printf ']'
}
...
xo_backend_herdr_cli() {
  ...
  if [ -e "$FIXTURE_DIR/closed" ]; then
    case "$first $second" in
      ...
      "workspace list") printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"xo","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1}]}}'; return 0 ;;
    esac
  fi
  if [ -e "$FIXTURE_DIR/race" ] && [ -e "$FIXTURE_DIR/snapshotted" ] && [ "$first $second" = "workspace list" ]; then
    printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"xo","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},{"workspace_id":"w2","label":"renamed","focused":false,"active_tab_id":"w2:t1","tab_count":1,"pane_count":1}]}}'
    return 0
  fi
  title=$(cat "$FIXTURE_DIR/title")
  tabs=$(cat "$FIXTURE_DIR/tabs")
...
write_v1() { # <id> [token]
  ...
  } > "$XO_STATE_OVERRIDE/$id.herdr-presentation"
}
...
reset_fixture() {
  rm -rf "$FIXTURE_DIR" "$TMP_ROOT"/*.lock "${XO_STATE_OVERRIDE:?}/"*
  mkdir -p "$FIXTURE_DIR"
  : > "$LOCK_LOG"; : > "$CLOSE_LOG"
...
assert_preserved() { # <case>
  ...
  [ -f "$XO_STATE_OVERRIDE/$ID.herdr-presentation" ] && had_journal=1
  xo_herdr_session_cleanup >/dev/null 2>&1
  if [ "$had_journal" -eq 1 ]; then
    [ -f "$XO_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "$name retired the journal"
  fi
...
reset_fixture
xo_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$XO_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "positive cleanup kept the journal"
[ "$(wc -l < "$CLOSE_LOG" | tr -d ' ')" = 1 ] || fail "positive cleanup did not close exactly once"
[ "$(sed -n '1p' "$LOCK_LOG")" = "$XO_STATE_OVERRIDE/.spawn-$ID.lock" ] || fail "task lock was not acquired first"
...
reset_fixture; printf 'version=1\ntask_id=%s\nprojection_id=short\n' "$ID" > "$XO_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "malformed journal"
...
reset_fixture; rm -f "$XO_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "zero journal match"
reset_fixture; write_v1 xo-task; assert_preserved "multiple journal matches"
reset_fixture; rm -f "$XO_STATE_OVERRIDE/$ID.herdr-presentation"; write_cross_home_v2; assert_preserved "cross-home journal"
reset_fixture; write_v2 "$XO_HOME" w9 "$TAB" "$PANE"; assert_preserved "v2 workspace binding mismatch"
reset_fixture; write_v2 "$XO_HOME" "$WS" w9:t1 "$PANE"; assert_preserved "v2 tab binding mismatch"
reset_fixture; write_v2 "$XO_HOME" "$WS" "$TAB" w9:p1; assert_preserved "v2 pane binding mismatch"
reset_fixture; write_v2 "$XO_HOME" "$WS" "$TAB" "$PANE"
xo_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$XO_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "matching v2 cleanup kept the journal"
...
reset_fixture; : > "$XO_STATE_OVERRIDE/$ID.meta"; assert_preserved "current task metadata"
...
cat > "$INTEGRATION_ROOT/bin/xo-herdr-session-cleanup.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "${XO_HOME:?}" >> "${XO_HERDR_CLEANUP_TRACE:?}"
SH
chmod +x "$INTEGRATION_ROOT/bin/xo-herdr-session-cleanup.sh"
printf '%s\n' manual > "$INTEGRATION_ROOT/home/config/backlog-backend"
XO_HOME="$INTEGRATION_ROOT/home" XO_HERDR_CLEANUP_TRACE="$TRACE" XO_BOOTSTRAP_DETECT_ONLY=1 \
  "$INTEGRATION_ROOT/bin/xo-bootstrap.sh" >/dev/null 2>&1
[ ! -e "$TRACE" ] || fail "detect-only bootstrap ran stale projection cleanup"
XO_HOME="$INTEGRATION_ROOT/home" XO_HERDR_CLEANUP_TRACE="$TRACE" \
  "$INTEGRATION_ROOT/bin/xo-bootstrap.sh" >/dev/null 2>&1
...
cat > "$INTEGRATION_ROOT/bin/xo-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'lock acquired'
SH
chmod +x "$INTEGRATION_ROOT/bin/xo-lock.sh"
XO_HOME="$INTEGRATION_ROOT/home" XO_ROOT_OVERRIDE="$INTEGRATION_ROOT" \
  XO_HERDR_CLEANUP_TRACE="$TRACE" \
  "$INTEGRATION_ROOT/bin/xo-session-start.sh" >/dev/null 2>&1 \
...
cat > "$INTEGRATION_ROOT/bin/xo-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'error: another live xo session holds the lock' >&2
exit 1
SH
chmod +x "$INTEGRATION_ROOT/bin/xo-lock.sh"
XO_HOME="$INTEGRATION_ROOT/home" XO_ROOT_OVERRIDE="$INTEGRATION_ROOT" \
...
printf 'all xo-herdr-session-cleanup tests passed\n'
