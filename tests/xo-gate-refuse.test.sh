...
#
# A confused no-mistakes gate agent runs inside an xo checkout, adopts the
# captain identity from AGENTS.md, and reaches for xo-spawn/xo-send/xo-teardown.
# bin/xo-gate-refuse-lib.sh is the xo capability-removal half: sourced at
...
# A normal xo session (real primary, real crew worktree) has NEITHER
...
#
# Finally, assert xo's TRACKED .no-mistakes.yaml parses and sets
...
run_guard_lib() {
  ...
  (
    cd "$cwd" || exit 111
    unset NO_MISTAKES_GATE XO_GATE_REFUSE_BYPASS
    case "$marker" in
...
test_helper_env_marker_refuses() {
  ...
  pass "xo-gate-refuse-lib: refuses when NO_MISTAKES_GATE is set"
}
...
test_helper_empty_env_marker_refuses() {
  ...
  pass "xo-gate-refuse-lib: refuses when NO_MISTAKES_GATE is set empty"
}
...
test_helper_path_backstop_refuses() {
  ...
  pass "xo-gate-refuse-lib: refuses from a gate worktree via git-common-dir (marker unset)"
}
...
test_helper_normal_is_noop() {
  ...
  pass "xo-gate-refuse-lib: no-op for a normal session (neither signal, set -eu clean)"
}
...
run_spawn() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u XO_GATE_REFUSE_BYPASS \
      ...
      XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$pane" TMUX=fake,1,0 \
      PATH="$fakebin:$PATH" "$@" \
...
test_spawn_refuses_and_admits() {
  ...
  xo_git_add_origin "$proj" "$TMP/spawn-origin.git"
  ...
  pass "xo-spawn: refuses on marker and gate-worktree backstop; a normal crew spawn is unaffected"
}

# --- xo-send ----------------------------------------------------------------

# A fake tmux that logs send-keys to XO_TMUX_LOG and reports live endpoints
# (mirrors tests/xo-send-strict), so a successful send is observable and a
...
case "${1:-}" in
  ...
  display-message)
    ...
    printf '%%1\n'; exit 0 ;;
  capture-pane) printf '╭────╮\n│    │\n╰────╯\n'; exit 0 ;;
  list-windows) printf 'xo-lane-ok\n'; exit 0 ;;
esac
exit 0
SH
...
run_send() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u XO_GATE_REFUSE_BYPASS \
      "PATH=$fakebin:$PATH" "XO_HOME=$home" "XO_ROOT_OVERRIDE=$home" \
      "XO_TMUX_LOG=$log" "XO_SEND_SETTLE=0" "$@" \
...
test_send_refuses_and_admits() {
  ...
  # env-marker refuse.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" xo-lane-ok "hello captain" NO_MISTAKES_GATE=1); rc=$?
  ...
  : > "$log"
  out=$(run_send "$GATE_WT" "$home" "$fakebin" "$log" xo-lane-ok "hello captain"); rc=$?
  ...
  # no-regression.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" xo-lane-ok "hello captain"); rc=$?
  ...
  [ "$(bash -c '. "$1"; xo_task_inbox_body "$2"' _ "$ROOT/bin/xo-task-inbox-lib.sh" \
  ...
  assert_contains "$(cat "$log")" "target=sess:xo-lane-ok literal=1 arg=: XO instruction waiting" \
    "send: normal steer should ring the durable inbox doorbell"
  pass "xo-send: refuses on marker and gate-worktree backstop; a normal steer uses the inbox"
}
...
# refused one leaves the task untouched (mirrors tests/xo-teardown make_case).
make_teardown_case() {
...
SH
  ...
  xo_write_meta "$case_dir/state/task-x1.meta" \
    "window=xo:xo-task-x1" "endpoint_task_id=task-x1" \
...
run_teardown() {
  ...
  ( cd "$cwd" && env -u NO_MISTAKES_GATE -u XO_GATE_REFUSE_BYPASS \
      ...
      "XO_DATA_OVERRIDE=$case_dir/data" "XO_CONFIG_OVERRIDE=$case_dir/config" \
...
test_teardown_refuses_and_admits() {
  ...
  pass "xo-teardown: refuses on marker and gate-worktree backstop; a normal teardown is unaffected"
}
