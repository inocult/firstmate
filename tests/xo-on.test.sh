...
TMP_ROOT=$(xo_test_tmproot xo-on)
...
cleanup() {
  ...
  if [ -f "$TMP_ROOT/remote-jobs/worker.pid" ]; then
    ...
    XO_REMOTE_JOB_STATE="$TMP_ROOT/remote-jobs"
    xo_remote_job_stop_worker_tree "$pid" 2>/dev/null || true
  fi
  rm -rf -- "$TMP_ROOT"
}
trap cleanup EXIT
...
cp "$ROOT/bin/xo-remote-entrypoint.sh" "$ROOT/bin/xo-remote-job-lib.sh" \
  "$ROOT/bin/xo-remote-job-worker.sh" "$REMOTE_ROOT/bin/"

cat > "$REMOTE_ROOT/bin/xo-probe-one.sh" <<'SH'
#!/usr/bin/env bash
set -u
out=$1
rc=$2
...
exit "$rc"
SH
cat > "$REMOTE_ROOT/bin/xo-probe-two.sh" <<'SH'
#!/usr/bin/env bash
printf 'home=%s\nroot=%s\nworker=%s\n' "$XO_HOME" "$XO_ROOT_OVERRIDE" "${XO_REMOTE_JOB_ACTIVE:-}"
if [ -n "${TOP_SECRET:-}" ]; then printf 'secret=leaked\n'; else printf 'secret=absent\n'; fi
SH
cat > "$REMOTE_ROOT/bin/xo-probe-path.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$PATH"
SH
cat > "$REMOTE_ROOT/bin/tasks-axi" <<SH
#!/usr/bin/env bash
printf '%s\n' "\${XO_REMOTE_JOB_ACTIVE:-absent}" >> "$TOOL_PROBE_LOG"
case "\${1:-}:\${2:-}" in
...
SH
cp "$ROOT/bin/xo-remote-doctor.sh" "$ROOT/bin/xo-tasks-axi-lib.sh" \
  "$ROOT/bin/xo-remote-herdr-owner-lib.sh" "$ROOT/bin/xo-backend.sh" "$REMOTE_ROOT/bin/"
mkdir -p "$REMOTE_ROOT/bin/backends"
cp "$ROOT/bin/backends/herdr.sh" "$REMOTE_ROOT/bin/backends/herdr.sh"
cat > "$REMOTE_ROOT/bin/xo-mutate.sh" <<'SH'
#!/usr/bin/env bash
printf 'mutation\n' >> "$1"
SH
...
printf '%s\0' 'plain' 'two words' '$(touch /tmp/xo-on-injected)' '' $'line one\nline two' > "$ARGV_EXPECTED"
printf 'payload one\npayload two\n' > "$TMP_ROOT/stdin"
set +e
# shellcheck disable=SC2016 # Literal shell-looking argv is the injection probe.
xo_on --stdin ios xo-probe-one.sh "$ARGV_ACTUAL" 23 \
  'plain' 'two words' '$(touch /tmp/xo-on-injected)' '' $'line one\nline two' \
...
assert_absent /tmp/xo-on-injected "shell-looking argv was interpreted"
pass "xo-on --stdin preserves argv, stdin, stdout, stderr, and exit status without shell interpretation"
...
set +e
xo_on ios xo-probe-one.sh "$REMOTE_HOME/argv-default.bin" 0 'default-closed' \
...
pass "xo-on defaults the remote command's stdin to /dev/null"
...
# this on the real ssh argv the XO_SSH_BIN process seam captured, never on
# xo-on.sh source text.
...
pass "xo-on arms a bounded SSH dead-peer detection window by default (${DEFAULT_INTERVAL}s x $DEFAULT_COUNT = ${DEFAULT_WINDOW}s)"

: > "$SSH_LOG"
XO_SSH_ALIVE_INTERVAL=7 XO_SSH_ALIVE_COUNT_MAX=2 xo_on ios xo-probe-two.sh >/dev/null
OVERRIDE_ARGV=$(tail -n 1 "$SSH_LOG")
assert_contains "$OVERRIDE_ARGV" 'ServerAliveInterval=7' "XO_SSH_ALIVE_INTERVAL override was not honored on the ssh transport"
assert_contains "$OVERRIDE_ARGV" 'ServerAliveCountMax=2' "XO_SSH_ALIVE_COUNT_MAX override was not honored on the ssh transport"
pass "xo-on's dead-peer detection window is env-overridable"

SSH_CALLS_BEFORE_INVALID=$(cat "$SSH_COUNT")
set +e
INVALID_INTERVAL_OUT=$(XO_SSH_ALIVE_INTERVAL=0 xo_on ios xo-probe-two.sh 2>&1)
INVALID_INTERVAL_RC=$?
INVALID_COUNT_OUT=$(XO_SSH_ALIVE_COUNT_MAX=not-a-number xo_on ios xo-probe-two.sh 2>&1)
INVALID_COUNT_RC=$?
set -e
[ "$INVALID_INTERVAL_RC" -eq 1 ] || fail "a zero XO_SSH_ALIVE_INTERVAL was accepted (got exit $INVALID_INTERVAL_RC)"
[ "$INVALID_COUNT_RC" -eq 1 ] || fail "a non-integer XO_SSH_ALIVE_COUNT_MAX was accepted (got exit $INVALID_COUNT_RC)"
assert_contains "$INVALID_INTERVAL_OUT" 'XO_SSH_ALIVE_INTERVAL must be a positive integer' "invalid interval did not explain its constraint"
assert_contains "$INVALID_COUNT_OUT" 'XO_SSH_ALIVE_COUNT_MAX must be a positive integer' "invalid count did not explain its constraint"
[ "$(cat "$SSH_COUNT")" -eq "$SSH_CALLS_BEFORE_INVALID" ] || fail "invalid keepalive configuration launched ssh"
pass "xo-on rejects invalid dead-peer settings before launching ssh"

out=$(TOP_SECRET='must-not-cross' xo_on remote-mac xo-probe-two.sh)
assert_contains "$out" "home=$REMOTE_HOME" "remote XO_HOME was not explicit"
...
CHILD_PATH=$(xo_on ios xo-probe-path.sh)
NVM_CHILD_DIRS=()
...
set +e
out=$(xo_on ios xo-remote-doctor.sh 2>&1)
set -e
...
# tests/xo-remote-doctor.test.sh owns the verdict against controlled fixtures.
set +e
out=$(xo_on ios xo-remote-doctor.sh 2>/dev/null)
set -e
...
xo_on ios xo-probe-two.sh >/dev/null
: > "$TOOL_PROBE_LOG"
set +e
out=$(xo_on ios xo-remote-doctor.sh 2>&1)
set -e
...
set +e
out=$(HOME="$DOCTOR_HOME" PATH="$DOCTOR_BIN:/usr/bin:/bin:/usr/sbin:/sbin" "$ROOT/bin/xo-remote-doctor.sh" 2>&1)
rc=$?
set -e
...
xo_on ios xo-probe-one.sh "$ARGV_TWO" 0 'second command' >/dev/null 2>/dev/null
cmp -s "$TMP_ROOT/argv-two-expected.bin" "$ARGV_TWO" || fail "second dynamic command did not execute"
pass "multiple xo-*.sh executables work without a command table"

for bad in '../xo-probe-one.sh' 'xo-probe-one.sh/extra' 'sh' 'xo-../../bin/sh'; do
  if xo_on ios "$bad" >/dev/null 2>&1; then
    fail "unsafe command name was accepted: $bad"
  fi
done
ln -s xo-probe-one.sh "$REMOTE_ROOT/bin/xo-symlink.sh"
if xo_on ios xo-symlink.sh >/dev/null 2>&1; then
  fail "a symlinked command was accepted"
fi
cat > "$REMOTE_ROOT/bin/xo-untracked.sh" <<'SH'
#!/usr/bin/env bash
printf 'untracked command ran\n'
SH
chmod +x "$REMOTE_ROOT/bin/xo-untracked.sh"
...
XO_GIT_SHADOW_LOG="$GIT_SHADOW_LOG" "$REMOTE_ROOT/bin/git" -C "$REMOTE_ROOT" ls-files --error-unmatch bin/xo-untracked.sh \
...
untracked_argv_b64=$(printf '%s\0' xo-untracked.sh | base64 | tr -d '\n')
set +e
out=$(XO_GIT_SHADOW_LOG="$GIT_SHADOW_LOG" XO_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  XO_REMOTE_JOB_STATE_ROOT="$TMP_ROOT/remote-jobs" "$REMOTE_ROOT/bin/xo-remote-entrypoint.sh" \
  1 "$untracked_root_b64" "$untracked_home_b64" "$untracked_argv_b64" 2>&1)
rc=$?
set -e
if [ "$rc" -eq 0 ]; then
  fail "an untracked xo-*.sh executable was accepted"
fi
...
set +e
out=$(
  # shellcheck disable=SC2329 # Exported for indirect use by xo_on.
  command() {
    if [ "${1:-}" = -v ] && [ "${2:-}" = git ]; then return 1; fi
    builtin command "$@"
  }
  export -f command
  xo_on ios xo-remote-doctor.sh 2>&1
)
set -e
assert_contains "$out" 'mode=check' "the trusted doctor could not bootstrap while git was unavailable"
printf '\n' >> "$REMOTE_ROOT/bin/xo-remote-doctor.sh"
set +e
out=$(
  # shellcheck disable=SC2329 # Exported for indirect use by xo_on.
  command() {
...
cp "$ROOT/bin/xo-remote-doctor.sh" "$REMOTE_ROOT/bin/xo-remote-doctor.sh"
chmod +x "$REMOTE_ROOT/bin/xo-remote-doctor.sh"
pass "doctor bootstrap remains authenticated when git is unavailable"

if XO_HOME="$LOCAL_HOME" XO_ROOT_OVERRIDE="$REMOTE_ROOT" XO_SSH_BIN="$FAKEBIN/fake-ssh" \
  "$ROOT/bin/xo-on.sh" '-oProxyCommand=bad' xo-probe-two.sh >/dev/null 2>&1; then
...
EOF
if xo_on ios xo-probe-two.sh >/dev/null 2>&1; then
...
argv_b64=$(printf '%s\0' xo-probe-two.sh | base64 | tr -d '\n')
if "$REMOTE_ROOT/bin/xo-remote-entrypoint.sh" 2 "$root_b64" "$home_b64" "$argv_b64" >/dev/null 2>&1; then
  fail "an incompatible transport protocol was accepted"
fi
traversal_root_b64=$(printf '%s' "$REMOTE_ROOT/../remote-root" | base64 | tr -d '\n')
if "$REMOTE_ROOT/bin/xo-remote-entrypoint.sh" 1 "$traversal_root_b64" "$home_b64" "$argv_b64" >/dev/null 2>&1; then
...
EOF
if xo_on remote-mac xo-probe-two.sh >/dev/null 2>&1; then
...
: > "$SSH_COUNT"
set +e
XO_FAKE_SSH_MODE=unreachable xo_on ios xo-mutate.sh "$REMOTE_HOME/mutations" >/dev/null 2>&1
rc=$?
set -e
...
: > "$SSH_COUNT"
set +e
XO_FAKE_SSH_MODE=ambiguous xo_on ios xo-mutate.sh "$REMOTE_HOME/mutations" >/dev/null 2>&1
rc=$?
set -e
