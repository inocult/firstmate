#!/usr/bin/env bash
# Behavior tests for the bearings projection wrapper over xo-fleet-snapshot.sh.
...
# shellcheck disable=SC1091
. "$ROOT/bin/xo-secondmate-registry-lib.sh"

BEARINGS="$ROOT/bin/xo-bearings-snapshot.sh"
TASKS_AXI_BIN=$(command -v tasks-axi || true)
TMP_ROOT=$(xo_test_tmproot xo-bearings)
# Keep disposable homes outside the snapshot's fixture repo boundary even when
# TMPDIR is inside an isolated source worktree.
XO_ROOT_OVERRIDE="$TMP_ROOT/fixture-root"
mkdir -p "$XO_ROOT_OVERRIDE"
export XO_ROOT_OVERRIDE
...
case "${1:-}" in
  ...
  capture-pane)
    case "$*" in
      *xo-domain-alpha*) printf 'stale terminal summary: Phase 7 started\n> \n' ;;
      *) printf 'all quiet\n> \n' ;;
    esac
    ;;
esac
...
if [ "${FAKE_GH_MANY:-0}" = 1 ]; then
  cat <<'JSON'
[{"number":1,"title":"One","url":"https://github.com/acme/repo/pull/1","headRefName":"xo/one","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]},{"number":2,"title":"Two","url":"https://github.com/acme/repo/pull/2","headRefName":"xo/two","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]},{"number":3,"title":"Three","url":"https://github.com/acme/repo/pull/3","headRefName":"xo/three","reviewDecision":"","mergeable":"MERGEABLE","statusCheckRollup":[]}]
JSON
  exit 0
fi
cat <<'JSON'
[{"number":9,"title":"Ship the thing","url":"https://github.com/kunchenguid/firstmate/pull/9","headRefName":"xo/ship-task","reviewDecision":"APPROVED","mergeable":"MERGEABLE","statusCheckRollup":[{"conclusion":"SUCCESS","status":"COMPLETED"}]}]
JSON
SH
  cat > "$fb/gh-axi" <<'SH'
#!/usr/bin/env bash
...
record_claude_state() {  # <state-dir> <id> <busy|idle>
  ...
  "$ROOT/bin/xo-busy-event.sh" apply "$state" "$id" "$semantic_state" --gen "$gen" \
...
write_fixture() {  # <home>
  ...
  printf '# XO fixture\n' > "$mate/AGENTS.md"
  printf 'mate\n' > "$mate/.xo-secondmate-home"
  printf -- '- mate - fixture domain (home: %s; scope: fixture work; projects: xo; added 2026-07-11)\n' \
    "$mate" > "$home/data/secondmates.md"
  cat > "$home/data/backlog.md" <<EOF
## In flight
- [ ] ship-task - Ship the thing (repo: xo) (kind: ship) (since 2026-07-11)
- [ ] scout-x - Investigate the thing data/scout-x/report.md (repo: xo) (kind: scout) (since 2026-07-11)

## Queued
- [ ] live-gate - Real queued work blocked-by: ship-task (repo: xo) (kind: ship)
- [ ] dead-gate - Old conditional work (repo: xo) (kind: scout)
  NOT REQUIRED - superseded 2026-07-11; kept as reference only.

## Done
- [x] done-a - Landed thing https://github.com/kunchenguid/firstmate/pull/7 (repo: xo) (kind: ship) (merged 2026-07-10)
EOF
  printf '# Scout X\n' > "$home/data/scout-x/report.md"
  xo_write_meta "$home/state/ship-task.meta" \
    "window=xo:xo-ship-task" \
  ...
  xo_write_meta "$home/state/scout-x.meta" \
    "window=xo:xo-scout-x" \
    "worktree=$home/projects/ship-wt" \
    "project=xo" \
    "harness=claude" \
    "kind=scout" \
  ...
  xo_write_meta "$home/state/mate.meta" \
    "window=xo:xo-mate" \
    "worktree=$mate" \
    "project=$mate" \
    "harness=codex" \
  ...
  xo_write_meta "$home/state/external-wait.meta" \
    "window=xo:xo-external-wait" \
    "worktree=$home/projects/ship-wt" \
    "project=xo" \
    "harness=claude" \
    "kind=ship" \
...
## In flight
- [ ] mate - Decide subscription order (repo: xo) (kind: ship) (since 2026-07-11)

## Queued
- [ ] mate-decision-race - Choose subscription order (repo: xo) (kind: captain) (hold: captain choice pending) (hold-kind: captain)

## Done
- [x] mate-landed - Secondmate-managed fix https://github.com/kunchenguid/firstmate/pull/50 (repo: xo) (kind: ship) (merged 2026-07-11)
EOF
  mkdir -p "$mate/projects/mate"
  xo_write_meta "$mate/state/mate.meta" \
    "window=xo:xo-mate" "worktree=$mate/projects/mate" "project=xo" \
...
refresh_local_secondmate_ledgers() {  # <parent-home>
  ...
  while IFS= read -r line || [ -n "$line" ]; do
    ...
    [ -f "$mate/.xo-secondmate-home" ] && [ -f "$mate/AGENTS.md" ] \
      && [ -d "$mate/bin" ] && [ -d "$mate/data" ] && [ -d "$mate/state" ] || continue
    PATH="$refresh_path" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$mate" \
      XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z XO_SNAPSHOT_NOW_EPOCH=1783792800 \
      "$ROOT/bin/xo-home-summary-refresh.sh" >/dev/null 2>&1 || true
  done < "$registry"
}
...
run() {  # <home> <fakebin> <args...>
  ...
  case " $* " in
    *" --all-landed "*) PATH="$fakebin:$PATH" XO_SNAPSHOT_SECONDMATE_LANDED_PER_HOME=0 refresh_local_secondmate_ledgers "$home" ;;
    *) PATH="$fakebin:$PATH" refresh_local_secondmate_ledgers "$home" ;;
  esac
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_BEARINGS_NOW=2026-07-11T18:00:00Z NET_LOG="$home/net.log" "$BEARINGS" "$@"
}

run_captain() {  # <home> <fakebin> <command args...>
  local home=$1 fakebin=$2
  shift 2
  PATH="$fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" XO_ROOT_OVERRIDE="$ROOT" \
    XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/xo-captain-hold.sh" "$@"
}
...
write_remote_home_summary() {  # <remote-home> <generated-epoch>
  ...
  jq -n --arg home "$home" --argjson epoch "$epoch" '{
    schema:"xo-secondmate-home-summary.v1",
    hold_classifier_schema:"xo-captain-hold-buckets.v1",
    ...
    queued:[
      {id:"remote-parked",title:"Remote parked hold",blocked_by:null,blocked_by_ids:[],unresolved_blocker_ids:[],blocked_reason:null,hold_reason:"parked",hold_kind:"captain",hold_until:null,hold_bucket:"live",hold_age_days:null,captain_actionable:true,repo:"xo",kind:"captain"},
      {id:"remote-aged",title:"Remote aged hold",blocked_by:null,blocked_by_ids:[],unresolved_blocker_ids:[],blocked_reason:null,hold_reason:"choose a route",hold_kind:"captain",hold_until:null,hold_bucket:"aged",hold_age_days:40,captain_actionable:false,repo:"xo",kind:"captain"}
    ],landed:[],endpoints:[],
...
make_remote_ledger_fleet() {  # <parent-home> <count>
  ...
  while [ "$i" -le "$count" ]; do
    ...
    xo_write_meta "$parent/state/$id.meta" \
...
printf '%s\t%s\n' "$remote_home" "${args[0]:-}" >> "$XO_TEST_LEDGER_CALL_LOG"
if [ -f "$remote_home/state/slow-ledger-read" ]; then
  active_marker="$XO_TEST_LEDGER_ACTIVE_DIR/collector-$$"
  : > "$active_marker"
  trap 'rm -f "$active_marker"' EXIT
  while [ ! -f "$XO_TEST_LEDGER_ACTIVE_DIR/overlap-proved" ]; do
    set -- "$XO_TEST_LEDGER_ACTIVE_DIR"/collector-*
    if [ "$#" -ge 5 ] && [ -e "$1" ]; then
      : > "$XO_TEST_LEDGER_ACTIVE_DIR/overlap-proved"
      break
    fi
    sleep 0.05
  done
  sleep 30 &
  sleeper=$!
  printf '%s %s\n' "$$" "$sleeper" >> "$XO_TEST_LEDGER_PID_LOG"
  wait "$sleeper"
fi
case "${args[0]:-}" in
  xo-remote-file.sh)
...
run_remote_ledger_bearings() {  # <parent-home> <fakebin> <epoch>
  ...
  XO_HOME="$parent" XO_ROOT_OVERRIDE="$ROOT" XO_SSH_BIN="$fakebin/fake-ssh" \
    XO_TEST_LEDGER_CALL_LOG="$parent/ledger-calls.log" \
    XO_TEST_LEDGER_PID_LOG="$parent/ledger-pids.log" \
    XO_TEST_LEDGER_ACTIVE_DIR="$parent/ledger-active" \
    XO_SNAPSHOT_CACHE_DIR="$parent/state/summary-cache" \
    XO_SNAPSHOT_BUDGET=15 XO_SNAPSHOT_NOW_EPOCH="$epoch" \
    XO_BEARINGS_NOW=2026-09-01T22:00:00Z "$BEARINGS" --json
}
...
write_domain_alpha_fixture() {  # <parent-home> <secondmate-home>
  ...
  printf -- '- domain-alpha - sample rollout (home: %s; scope: sample rollout and legal release; projects: sample; added 2026-07-13)\n' \
    "$mate" > "$home/data/secondmates.md"
  xo_write_secondmate_meta "$home/state/domain-alpha.meta" "$mate" "xo:xo-domain-alpha" sample
...
test_domain_alpha_stale_parent_event_does_not_become_current_work() {
  ...
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_NOW_EPOCH=1783792800 XO_SNAPSHOT_TERMINAL_LINES=2 XO_SNAPSHOT_TERMINAL_BYTES=64 \
    NET_LOG="$home/net.log" FAKE_GH_FAIL=1 "$ROOT/bin/xo-fleet-snapshot.sh" --json)
...
SH
  ...
  canonical=$(PATH="$fakebin:$PATH" STAT_LOG="$stat_log" XO_HOME="$home" \
    XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z XO_SNAPSHOT_NOW_EPOCH=1783792800 \
    "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.records[] | select(.id == "domain-alpha")
    | .provenance.selected == "structured-home"
...
test_parent_activity_evidence_is_bounded_and_disclosed() {
  ...
  fakebin=$(make_fakebin "$home")
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_PARENT_ACTIVITY_LINES=4 XO_SNAPSHOT_PARENT_ACTIVITY_BYTES=4096 \
    XO_SNAPSHOT_PARENT_ACTIVITIES=2 "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.records[] | select(.id == "domain-alpha")
  ...
  json=$(XO_SNAPSHOT_PARENT_ACTIVITY_LINES=4 XO_SNAPSHOT_PARENT_ACTIVITY_BYTES=4096 \
    XO_SNAPSHOT_PARENT_ACTIVITIES=2 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
EOF
  xo_write_meta "$mate/state/phase8.meta" \
    "window=xo:xo-phase8" "worktree=$mate/projects/phase8" "project=sample" \
...
append_landed_row() {  # <secondmate-home> <id> <title> <date>
  printf -- '- [x] %s - %s (repo: xo) (kind: ship) (merged %s)\n' \
...
write_parent_secondmate_event() {  # <parent> <id> <home> <note>
  xo_write_secondmate_meta "$1/state/$2.meta" "$3" "xo:xo-$2" sample
...
test_bad_secondmate_homes_never_revive_parent_work() {
  ...
  xo_git_init_commit "$wt"
  git -C "$wt" checkout -q -b xo/slow
  printf '## In flight\n- [ ] slow - Slow child (repo: sample) (kind: ship) (since 2026-07-13)\n\n## Queued\n\n## Done\n' > "$unknown_child/data/backlog.md"
  xo_write_meta "$unknown_child/state/slow.meta" \
    "window=xo:xo-slow" "worktree=$wt" "project=sample" \
    "harness=codex" "kind=ship" "mode=no-mistakes"
...
test_oversized_secondmate_summary_stays_strict_unknown() {
  ...
  xo_write_secondmate_meta "$home/state/oversized.meta" "$mate" "xo:xo-oversized" sample
...
EOF
  ...
  fakebin=$(make_fakebin "$home")
  json=$(XO_SNAPSHOT_SECONDMATE_MAX_BYTES=512 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
test_secondmate_and_child_bounds_are_disclosed() {
  ...
  while [ "$i" -le 3 ]; do
    ...
    xo_write_meta "$mate/state/$child.meta" \
      "window=xo:xo-$child" "worktree=$mate/projects/$child" "project=sample" \
      "harness=claude" "kind=ship" "mode=no-mistakes"
  ...
  printf '\n## Queued\n\n## Done\n' >> "$mate/data/backlog.md"
  fakebin=$(make_fakebin "$home")
  PATH="$fakebin:$PATH" XO_SNAPSHOT_SECONDMATE_CHILDREN=2 refresh_local_secondmate_ledgers "$home"
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_SECONDMATES=2 XO_SNAPSHOT_SECONDMATE_CHILDREN=2 "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
  ...
  json=$(XO_SNAPSHOT_SECONDMATES=2 XO_SNAPSHOT_SECONDMATE_CHILDREN=2 XO_BEARINGS_SECONDMATES=1 \
  ...
  expanded=$(XO_SNAPSHOT_SECONDMATE_CHILDREN=2 XO_BEARINGS_SECONDMATES=1 \
...
test_parent_decision_is_untrusted_contradiction_only() {
  ...
  xo_write_secondmate_meta "$home/state/authority.meta" "$mate" "xo:xo-authority" sample
...
test_parent_evidence_reconciles_by_verb_and_key() {
  ...
  xo_write_secondmate_meta "$home/state/hold.meta" "$hold" "xo:xo-hold" sample
  xo_write_secondmate_meta "$home/state/blocked.meta" "$blocked" "xo:xo-blocked" sample
  xo_write_secondmate_meta "$home/state/decision.meta" "$decision" "xo:xo-decision" sample
...
## Done
EOF
  xo_write_meta "$decision/state/$child.meta" \
    "window=xo:xo-$child" "worktree=$decision/projects/$child" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
## Done
EOF
  xo_write_meta "$mate/state/parked.meta" \
    "window=xo:xo-parked" "worktree=$mate/projects/parked" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
## Done
EOF
  xo_write_meta "$mate/state/done.meta" \
    "window=xo:xo-done" "worktree=$mate/projects/done" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
  xo_write_meta "$mate/state/failed.meta" \
    "window=xo:xo-failed" "worktree=$mate/projects/failed" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
test_registry_unavailability_and_bounds_are_explicit() {
  ...
  xo_write_secondmate_meta "$home/state/hidden.meta" "$mate" "xo:xo-hidden" sample
  ...
  refresh_local_secondmate_ledgers "$home"
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_REGISTRY_RECORDS=2 "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
  ...
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_REGISTRY_LINES=2 "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.registry
  ...
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_REGISTRY_BYTES=100 "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.registry
  ...
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_REGISTRY_BYTES="$((boundary - 1))" "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.registry
  ...
  json=$(XO_SNAPSHOT_REGISTRY_RECORDS=2 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
  ...
  xo_write_secondmate_meta "$home/state/z-hidden.meta" "$mate" "xo:xo-z-hidden" sample
  refresh_local_secondmate_ledgers "$home"
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_REGISTRY_RECORDS=3 "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  printf '%s' "$canonical" | jq -e '
    .secondmate_current.registry.complete == false
...
test_default_is_bounded_and_local_only() {
  ...
  awk '{if ($0 ~ /^- \[ \] ship-task /) sub(/ \(repo: xo\)/, ""); print}' \
  ...
  local canon; canon=$(PATH="$fakebin:$PATH" XO_HOME="$home" "$ROOT/bin/xo-fleet-snapshot.sh" --json)
  ...
  printf '%s' "$json" | jq -e '
    .schema == "xo-bearings.v1"
      and (.in_flight | any(.id == "ship-task" and .repo == "xo"))
...
## Queued
- [ ] work-gate - Captain-gated ship work (repo: xo) (kind: ship) (hold: captain go needed) (hold-kind: captain)
- [ ] later-call - Deferred captain call (repo: xo) (kind: captain) (hold: revisit with the captain) (hold-kind: captain) (hold-until: 2026-08-01)
- [ ] due-call - Due captain call (repo: xo) (kind: captain) (hold: overdue captain choice) (hold-kind: captain) (hold-until: 2026-07-11)
- [ ] parked-call - Prose-parked captain call (repo: xo) (kind: ship) (hold: DEFERRED by captain revisit later) (hold-kind: captain)
- [ ] external-gate - Externally held work (repo: xo) (kind: ship) (hold: upstream release pending) (hold-kind: external)

## Done
- [x] answered-call - Answered captain question (repo: xo) (kind: captain) (done 2026-07-10) (hold: captain choice pending) (hold-kind: captain)
- [x] shipped-work - Ordinary landed work (repo: xo) (kind: ship) (merged 2026-07-10)
EOF
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
test_undated_hold_phrasing_and_aging_projection() {
  ...
  printf '# XO fixture\n' > "$mate/AGENTS.md"
  printf 'aging-mate\n' > "$mate/.xo-secondmate-home"
  printf -- '- aging-mate - hold aging fixture (home: %s; scope: captain holds; projects: xo; added 2026-07-11)\n' \
    "$mate" > "$home/data/secondmates.md"
  xo_write_meta "$home/state/aging-mate.meta" \
    "kind=secondmate" "mode=secondmate" "harness=pi" "home=$mate" "projects=xo"
  cat > "$mate/data/backlog.md" <<'EOF'
## In flight

## Queued
- [ ] mate-parked - Remote parked call (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain)
- [ ] mate-blocked-parked - Remote blocked parked call blocked-by: missing-remote-blocker (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain)
- [ ] mate-future-parked - Remote parked call for later (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain) (hold-until: 2026-08-01)
- [ ] mate-due-parked - Remote parked call now due (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain) (hold-until: 2026-07-11)
- [ ] mate-due-not-required - Remote moot call now due (repo: xo) (kind: captain) (hold: choose a remote route) (hold-kind: captain) (hold-until: 2026-07-11)
  NOT REQUIRED - the remote decision is moot.
- [ ] mate-aged - Remote aged call (repo: xo) (kind: captain) (since 2026-06-01) (hold: choose a remote route) (hold-kind: captain)
...
## Queued
- [ ] parked-hold - Parked style call (repo: xo) (kind: ship) (since 2026-07-10) (hold: not urgent) (hold-kind: captain)
- [ ] blocked-parked - Blocked parked call blocked-by: missing-blocker (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain)
- [ ] future-parked - Parked style call for later (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain) (hold-until: 2026-08-01)
- [ ] due-parked - Parked style call now due (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain) (hold-until: 2026-07-11)
- [ ] due-superseded - Superseded call now due (repo: xo) (kind: captain) (hold: SUPERSEDED) (hold-kind: captain) (hold-until: 2026-07-11)
- [ ] aged-call - Aged genuine call (repo: xo) (kind: captain) (since 2026-06-01) (hold: choose a sample route) (hold-kind: captain)
  Captain hold set: 2026-06-01T00:00:00Z
- [ ] recent-call - Recent genuine call (repo: xo) (kind: captain) (since 2026-07-10) (hold: choose a sample route) (hold-kind: captain)
  Captain hold set: 2026-07-10T00:00:00Z
- [ ] contextual-call - Context is not a deferral (repo: xo) (kind: captain) (since 2026-07-10) (hold: choose whether to pursue this queued opportunity) (hold-kind: captain)
  Captain hold set: 2026-07-10T00:00:00Z
  This is not urgent context, but the captain decision is current.
- [ ] contextual-not-urgent - Leading context is not a deferral (repo: xo) (kind: captain) (since 2026-07-10) (hold: not urgent but choose the route now) (hold-kind: captain)
  Captain hold set: 2026-07-10T00:00:00Z
- [ ] contextual-comma - Comma context is not a deferral (repo: xo) (kind: captain) (since 2026-07-10) (hold: not urgent, choose the launch route now) (hold-kind: captain)
  Captain hold set: 2026-07-10T00:00:00Z
- [ ] metadata-context - Metadata-like context is not a deferral (repo: xo) (kind: captain) (since 2026-07-10) (hold: not urgent, priority: decide P1 or P2) (hold-kind: captain)
  Captain hold set: 2026-07-10T00:00:00Z
- [ ] contextual-opportunity - Leading opportunity is not a deferral (repo: xo) (kind: captain) (since 2026-07-10) (hold: queued opportunity: choose whether to proceed) (hold-kind: captain)
  Captain hold set: 2026-07-10T00:00:00Z
- [ ] contextual-gated - Leading gate is not a deferral (repo: xo) (kind: captain) (since 2026-07-10) (hold: captain-gated decision needs current approval) (hold-kind: captain)
  Captain hold set: 2026-07-10T00:00:00Z
- [ ] reheld-current-call - Re-held genuine call (repo: xo) (kind: captain) (since 2026-06-01) (hold: choose a current sample route) (hold-kind: captain)
  Captain hold set: 2026-07-10T00:00:00Z
  Current decision prose lists route north or route south.
  Resolution recorded by xo-captain-hold.
  Decision digest: historical-sample
  Resolution mode: released
  Captain decision:
  Not urgent at the time of the previous call.
- [ ] legacy-old-hold - Legacy unstamped hold (repo: xo) (kind: ship) (since 2026-06-01) (hold: choose a sample route) (hold-kind: captain)
...
EOF
  ...
  json=$(XO_SNAPSHOT_UNDATED_HOLD_AGE_DAYS=50 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
## Queued
- [ ] only-blocked-parked - Blocked parked call blocked-by: missing-blocker (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain) (hold-until: 2026-07-11)
...
## Queued
- [ ] reveal-blocked - Blocked parked call blocked-by: missing-blocker-1234567890123456789012345678901234567890 (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain)
- [ ] reveal-future - 1234567890123456789012345678901234567890123456789012345678901234567890123456789012345678901234567890 (repo: xo) (kind: captain) (hold: parked) (hold-kind: captain) (hold-until: 2026-12-01)
- [ ] reveal-aged - Aged undated call (repo: xo) (kind: captain) (since 2026-06-01) (hold: choose a route) (hold-kind: captain)
  Captain hold set: 2026-06-01T00:00:00Z
- [ ] reveal-live - 123456789012345678901234567890123456789012345678901234567890 (repo: xo) (kind: captain) (hold: choose A) (hold-kind: captain)
...
test_perl_fallback_bounds_github_call() {
  ...
  started=$(date +%s)
  json=$(PATH="$fakebin:$toolbin" XO_HOME="$home" XO_BEARINGS_NOW=2026-07-11T18:00:00Z \
    XO_BEARINGS_PR_TIMEOUT=1 NET_LOG="$home/net.log" FAKE_GH_SLEEP=1 "$BEARINGS" --include-prs --json)
...
write_large_fixture() {  # <home> <count>
  ...
  while [ "$i" -le "$count" ]; do
    ...
    xo_write_meta "$home/state/$id.meta" \
      "window=xo:xo-$id" \
...
test_section_caps_and_expansion_flags() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(XO_BEARINGS_IN_FLIGHT=2 XO_BEARINGS_DECISIONS=2 XO_BEARINGS_GATES=2 \
    XO_BEARINGS_REPORTS=2 XO_BEARINGS_RECORDED_PRS=2 XO_BEARINGS_UNHEALTHY=2 \
    run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
  ...
  expanded=$(XO_BEARINGS_IN_FLIGHT=2 XO_BEARINGS_DECISIONS=2 XO_BEARINGS_GATES=2 \
    XO_BEARINGS_REPORTS=2 XO_BEARINGS_RECORDED_PRS=2 XO_BEARINGS_UNHEALTHY=2 \
...
test_pr_repository_cap_and_expansion() {
  ...
  fakebin=$(make_fakebin "$home"); : > "$home/net.log"
  json=$(XO_BEARINGS_PR_REPOS=2 run "$home" "$fakebin" --include-prs --json)
  ...
  : > "$home/net.log"
  expanded=$(XO_BEARINGS_PR_REPOS=2 run "$home" "$fakebin" --include-prs --all-pr-repos --json)
...
test_per_repository_pr_cap_is_disclosed() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(XO_BEARINGS_PR_LIMIT=2 FAKE_GH_MANY=1 run "$home" "$fakebin" --include-prs --json)
  toon=$(XO_BEARINGS_PR_LIMIT=2 FAKE_GH_MANY=1 run "$home" "$fakebin" --include-prs)
  printf '%s' "$json" | jq -e '
    (.candidate_prs | length) == 2
    and (.prs | test("2 shown, at least 3 open; capped in 1 repo"))
    and ([.omitted[] | select(.surface == "candidate_prs showing 2 of at least 3; capped in 1 repo(s)" and .reveal == "raise XO_BEARINGS_PR_LIMIT")] | length) == 1
...
test_completed_scout_report_not_pending() {
  ...
  xo_write_meta "$home/state/lavish-103.meta" \
    "window=xo:xo-lavish-103" \
...
test_landed_accepts_only_kind_owned_delivery_artifacts() {
  ...
  "$TASKS_AXI_BIN" add answered-question \
    ...
    --repo xo --file "$main_backlog" >/dev/null \
  ...
  "$TASKS_AXI_BIN" add keyword-scout "SCOUT parser keywords" --kind scout \
    --repo xo --start --file "$main_backlog" >/dev/null \
...
- [x] done-pr-nondelivery - Closed without merge https://github.com/o/r/pull/5 (repo: xo) (kind: ship) (done 2026-07-12)
- [x] scout-pr-no-report - Scout without report https://github.com/o/r/pull/6 (repo: xo) (kind: scout) (merged 2026-07-12)
- [x] ship-reported-path - Ship naming data/ship-reported-path/report.md (repo: xo) (kind: ship) (reported 2026-07-12)
- [x] reported-scout - Report with $report_pr context $report_path (repo: xo) (kind: scout) (reported 2026-07-12)
- [x] local-delivery - Local with https://github.com/o/r/pull/9 context local main (repo: xo) (kind: ship) (done 2026-07-12)
EOF

  : > "$home/net.log"
  fleet_json=$(PATH="$fakebin:$PATH" XO_HOME="$home" \
    XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z NET_LOG="$home/net.log" \
    "$ROOT/bin/xo-fleet-snapshot.sh" --json) \
...
EOF
  fleet_json=$(PATH="$fakebin:$PATH" XO_HOME="$home" NET_LOG="$home/net.log" \
    "$ROOT/bin/xo-fleet-snapshot.sh" --json) || fail "keyword fleet snapshot failed"
...
test_all_landed_keeps_complete_global_order() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(XO_BEARINGS_LANDED=1 run "$home" "$fakebin" --json --all-landed)
  actual=$(printf '%s' "$json" | jq -r '.landed[] | "\(.owner)/\(.id)"')
...
test_landed_bounded_and_disclosed() {
  ...
  i=1
  while [ "$i" -le 12 ]; do
    printf -- '- [x] mate-landed-%02d - Secondmate fix %02d (repo: xo) (kind: ship) (merged 2026-06-%02d)\n' \
      "$i" "$i" "$((13 - i))" >> "$mate/data/backlog.md"
    i=$((i + 1))
  done
  fakebin=$(make_fakebin "$home")
  json=$(XO_BEARINGS_LANDED=20 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
test_live_blocker_is_not_charted_queue_work() {
  ...
  printf 'blocked [key=synthetic-dependency]: xo can refresh the synthetic token\n' > "$home/state/ship-task.status"
...
test_captains_call_anti_leak() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(run "$home" "$fakebin" --json)
  canonical=$(PATH="$fakebin:$PATH" XO_HOME="$home" "$ROOT/bin/xo-fleet-snapshot.sh" --json)
...
test_main_orphan_in_flight_is_disclosed_not_invented() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] only-orphan - Structured in flight without meta (repo: xo) (kind: ship) (since 2026-07-11)
...
## In flight
this current row is not structured
- [ ] structured-ship - Visible structured sibling (repo: xo) (kind: ship) (since 2026-07-11)

## Queued
another free-form note without checkbox
- [ ] structured-queued - Structured queued (repo: xo) (kind: ship)

## Done
EOF
  xo_write_meta "$home/state/structured-ship.meta" \
    "window=xo:xo-structured-ship" \
...
test_main_orphan_counterfactual_meta_clears_inventory_warning() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] orphan-ship - Gains meta in counterfactual (repo: xo) (kind: ship) (since 2026-07-11)
- [ ] visible-ship - Already live (repo: xo) (kind: ship) (since 2026-07-11)

## Queued
- [ ] queued-ship - Ordinary queue (repo: xo) (kind: ship)

## Done
EOF
  xo_write_meta "$home/state/visible-ship.meta" \
    "window=xo:xo-visible-ship" \
  ...
  xo_write_meta "$home/state/orphan-ship.meta" \
    "window=xo:xo-orphan-ship" \
    "worktree=$home/projects/orphan-ship" \
    "project=xo" \
    "harness=codex" \
    "kind=ship" \
...
seed_working_child() {  # <mate-home> <id> <doing> [repo]
  ...
  xo_write_meta "$mate/state/$id.meta" \
    "window=xo:xo-$id" "worktree=$mate/projects/$id" "project=sample" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
EOF
    for id in working-live working-live-two working-blocked working-dated working-aged; do
      ...
      xo_write_meta "$home/state/$id.meta" \
        "window=xo:xo-$id" "worktree=$home/projects/$id" "project=sample" \
        "harness=claude" "kind=ship" "mode=no-mistakes"
  ...
  summary=$(PATH="$fakebin:$PATH" XO_HOME="$mate" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_SECONDMATE_DECISIONS=1 \
    "$ROOT/bin/xo-fleet-snapshot.sh" --secondmate-home-summary)
  ...
  json=$(XO_SNAPSHOT_SECONDMATE_DECISIONS=1 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
  ...
  expanded=$(XO_SNAPSHOT_SECONDMATE_DECISIONS=1 run "$home" "$fakebin" --json --all-decisions --all-queued)
  printf '%s' "$expanded" | jq -e '
    ([.in_flight[].id] | contains([
      "working-live", "working-live-two", "working-blocked", "working-dated", "working-aged",
      "working-mate/working-live", "working-mate/working-live-two", "working-mate/working-blocked",
...
## Done
EOF
  xo_write_meta "$hibit/state/hibit-worker.meta" \
    "window=xo:xo-hibit-worker" "worktree=$hibit/projects/worker" "project=hibit" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
## Done
EOF
  xo_write_meta "$wheel/state/wheel-worker.meta" \
    "window=xo:xo-wheel-worker" "worktree=$wheel/projects/worker" "project=wheelhouse" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
EOF
  xo_write_meta "$sshhip/state/unreadable-child.meta" \
    "window=xo:dead-sshhip-child" "worktree=$sshhip/projects/child" "project=sshhip" \
    "harness=codex" "kind=ship" "mode=no-mistakes"
...
## Done
EOF
  xo_write_meta "$ha/state/prep.meta" \
    "window=xo:xo-prep" "worktree=$ha/projects/prep" "project=home-assistant" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
- [ ] unreadable-child - Submit App Store build (repo: sshhip) (kind: ship)' \
  ...
  xo_write_meta "$wheel/state/production-observation.meta" \
    "window=xo:xo-production-observation" "worktree=$wheel/projects/worker" "project=wheelhouse" \
  ...
  xo_write_meta "$sshhip/state/unreadable-child.meta" \
    "window=xo:xo-unreadable-child" "worktree=$sshhip/projects/child" "project=sshhip" \
    "harness=claude" "kind=ship" "mode=no-mistakes"
...
test_main_captain_readiness_matches_secondmate_projection() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] observation - Held observation (repo: xo) (kind: scout) (hold: watch production) (hold-kind: external)
- [ ] prep - Prepare canary (repo: xo) (kind: ship)

## Queued
- [ ] review - Security review (repo: xo) (kind: ship)
- [ ] captain-run - Run captain canary blocked-by: prep blocked-by: review (repo: xo) (kind: captain) (hold: captain runs canary) (hold-kind: captain)

## Done
EOF
  xo_write_meta "$home/state/prep.meta" \
    "window=xo:xo-prep" "worktree=$home/projects/prep" "project=xo" \
    "harness=codex" "kind=ship" "mode=no-mistakes"
  printf 'working: preparing main canary\n' > "$home/state/prep.status"
  xo_write_meta "$home/state/observation.meta" \
    "window=xo:xo-observation" "worktree=$home/projects/observation" "project=xo" \
...
## Done
- [x] prep - Prepare canary (repo: xo) (kind: ship) (done 2026-07-22)
EOF
...
## Done
- [x] prep - Prepare canary (repo: xo) (kind: ship) (done 2026-07-22)
- [x] review - Security review (repo: xo) (kind: ship) (done 2026-07-22)
EOF
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
test_task_teardown_during_metadata_capture_does_not_abort_snapshot() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] a-hold - Stable local worker (repo: xo) (kind: ship)

## Queued

## Done
EOF
  xo_write_meta "$home/state/a-hold.meta" \
    "window=fixture:a-hold" "project=xo" "harness=claude" "kind=ship" "mode=no-mistakes"
  xo_write_meta "$home/state/z-gone.meta" \
    "window=fixture:z-gone" "project=xo" "harness=claude" "kind=ship" "mode=no-mistakes"
...
test_current_state_uses_captured_status_observation() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] captured-status - Captured status fixture (repo: xo) (kind: ship)

## Queued

## Done
EOF
  xo_write_meta "$home/state/captured-status.meta" \
    "window=fixture:captured-status" "worktree=$worktree" "project=xo" \
...
SH
  ...
  json=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_NOW_EPOCH=1783792800 NET_LOG="$home/net.log" REAL_CP="$real_cp" \
...
test_relaunched_task_does_not_inherit_reused_endpoint_state() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] generation-race - Generation identity fixture (repo: xo) (kind: ship)

## Queued

## Done
EOF
  xo_write_meta "$home/state/generation-race.meta" \
    "window=fixture:xo-generation-race" "worktree=$worktree" "project=xo" \
...
if [ "${1:-}" = display-message ]; then
  if mkdir "$RACE_ONCE" 2>/dev/null; then
    tmp="$RACE_META.tmp.$$"
    cat > "$tmp" <<EOF
window=fixture:xo-generation-race
...
EOF
    mv "$tmp" "$RACE_META"
    printf 'needs-decision[replacement]: replacement-only decision https://github.com/acme/xo/pull/999\n' > "$RACE_STATUS"
...
SH
  ...
  json=$(PATH="$fakebin:$PATH" XO_HOME="$home" XO_SNAPSHOT_NOW=2026-07-11T18:00:00Z \
    XO_SNAPSHOT_NOW_EPOCH=1783792800 NET_LOG="$home/net.log" \
...
test_large_local_snapshot_overlaps_local_reads_without_projection_drift() {
  ...
  xo_git_init_commit "$worktree"
  git -C "$worktree" checkout -qb xo/synthetic-large-local
  fakebin=$(make_fakebin "$home")
...
SH
...

  {
    ...
    i=1
    while [ "$i" -le 5 ]; do
      printf -- '- [ ] local-%s - Synthetic local worker %s (repo: xo) (kind: ship)\n' "$i" "$i"
      i=$((i + 1))
    done
    printf '\n## Queued\n\n## Done\n'
    i=1
    while [ "$i" -le 300 ]; do
      printf -- '- [x] history-%s - Historical completed item %s https://github.com/acme/xo/pull/%s (repo: xo) (kind: ship) (done 2026-01-01)\n' "$i" "$i" "$i"
      i=$((i + 1))
    done
  } > "$home/data/backlog.md"
  i=1
  while [ "$i" -le 5 ]; do
    xo_write_meta "$home/state/local-$i.meta" \
      "window=fixture:local-$i" "worktree=$worktree" "project=xo" \
      "harness=claude" "kind=ship" "mode=no-mistakes"
    printf 'working: synthetic fixture\n' > "$home/state/local-$i.status"
    i=$((i + 1))
  done

  serial=$(FAKE_NM_DELAY=0 XO_SNAPSHOT_LOCAL_READ_CONCURRENCY=1 run "$home" "$fakebin" --json)
  ...
  FAKE_NM_DELAY=1 XO_SNAPSHOT_LOCAL_READ_CONCURRENCY=1 \
  ...
  FAKE_NM_DELAY=1 FAKE_NM_SIGNAL="$home/nm-started" \
    XO_SNAPSHOT_LOCAL_READ_CONCURRENCY=8 \
  ...
  [ "$parallel" = "$serial" ] \
    || fail "concurrent local observation changed the xo-bearings.v1 projection"
...
## Done
- [x] remote-approved - Captain-approved delivery $approved_pr (repo: xo) (kind: ship) (hold-kind: captain) (merged 2026-09-03)
EOF
  write_remote_home_summary "$remote_home" 1000
  tmp="$cache_file.tmp"
...
test_a_remote_home_without_any_ledger_is_explicitly_unreadable_without_remote_compute() {
  ...
  [ "$(awk -F '\t' 'NR == 1 { print $2 }' "$parent/ledger-calls.log")" = "xo-remote-file.sh" ] \
