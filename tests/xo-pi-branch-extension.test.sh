...
# (.pi/extensions/xo-branch-supervision.ts): wake dispatch acceptance and
# gating, the two-stage noise filter's second stage (verdict-driven delivery
# into main), store-first durability through the real bin/xo-branch-outcome.sh,
...
// live guard (tests/xo-pi-branch-live-e2e.test.sh) is what proves the real
...
const home = process.env.XO_HOME;
const realRoot = process.env.XO_ROOT_OVERRIDE;
...
if (!process.env.XO_TEST_SKIP_LOCK) {
...
function dispatch(message, projects, heartbeat, eligible) {
  ...
  bus.emit("xo-branch-supervision:dispatch", offer);
  return offer;
}
...
function outcomeScript(args) {
  const result = spawnSync("bash", [`${realRoot}/bin/xo-branch-outcome.sh`, ...args], {
    encoding: "utf8",
    env: { ...process.env, XO_HOME: home, XO_STATE_OVERRIDE: `${home}/state` },
  });
  if (result.status !== 0) throw new Error(`xo-branch-outcome.sh ${args.join(" ")} failed: ${result.stderr}`);
...
test_branch_dispatch_two_stage_filter_and_prefix_contract() {
  ...
  PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
...
if (!wakePrompt.includes("XO SUPERVISION WAKE: signal: task-9 done")) {
...
if (JSON.stringify(session.options.tools) !== JSON.stringify(["read", "bash", "xo_branch_report"])) {
...
if (hooked.env.XO_SUPERVISION_ACTOR !== "branch") throw new Error("branch bash does not inject the branch actor");
if (hooked.env.XO_LEASE_HOLDER_PID !== String(process.ppid)) throw new Error("branch bash does not pin the verified session-lock holder pid");
...
if (!rewriteA.prompt_cache_key.startsWith("xo-branch-")) throw new Error(`unexpected cache key: ${rewriteA.prompt_cache_key}`);
...
const report = session.options.customTools.find((tool) => tool.name === "xo_branch_report");
...
if (sentToMain[0].message.customType !== "xo-branch-merge") throw new Error("merge note has the wrong custom type");
...
const processingRequests = sentToMain.filter((sent) => sent.message.customType === "xo-branch-process");
...
if (sentToMain.some((sent) => sent.options.triggerTurn && sent.message.customType !== "xo-branch-process")) {
...
const captainEntries = mainEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome");
...
// renderer.
const outcomesTool = mainTools.find((tool) => tool.name === "xo_branch_outcomes");
if (!outcomesTool) throw new Error("xo_branch_outcomes was not registered on main");
const renderTheme = {
...
if (calmOffCall.constructor.name !== "Box" || calmOffCall.paddingX !== 1 || calmOffCall.paddingY !== 1) {
  throw new Error("xo_branch_outcomes changed its ordinary shell rendering");
}
if (calmOffResult.constructor.name !== "Container" || calmOffCall.children[0]?.text !== "xo_branch_outcomes" || calmOffCall.children[1]?.text !== "OUTCOME_DUMP") {
  throw new Error("xo_branch_outcomes changed its ordinary call or result rendering");
}
...
pi.events.emit("xo:calm-presentation", { active: true, stockExportRendering: false });
...
if (calmOnCall.constructor.name !== "Container" || calmOnCall.render(100).length !== 0 || calmOnResult.constructor.name !== "Container" || calmOnResult.render(100).length !== 0) {
  throw new Error("xo_branch_outcomes remained visible while Calm was on");
}
...
if (outcomesTool.renderCall({}, renderTheme, renderContext).constructor.name !== "Box" || outcomesTool.renderResult(stockResult, { expanded: false, isPartial: false }, renderTheme, renderContext).constructor.name !== "Container") {
  throw new Error("xo_branch_outcomes did not restore ordinary rendering when Calm was turned off");
}
...
if (!exportCallFellBack || !exportResultFellBack) {
  throw new Error("xo_branch_outcomes replaced Pi stock export rendering");
}
...
if (listedText.split("\n").length !== 2 || !listedText.includes("checks green")) {
  throw new Error(`xo_branch_outcomes did not read the store: ${listedText}`);
}
if (!renderers.has("xo-branch-merge")) throw new Error("merge-note renderer missing");
if (!entryRenderers.has("xo-branch-visible-outcome")) throw new Error("visible captain-outcome renderer missing");
const assertRenderedNote = (note, glyph) => {
  const fgCalls = [];
  const rendered = renderers.get("xo-branch-merge")(
    { content: note },
    { expanded: false },
    {
      fg(color, text) {
...
const captainRendered = entryRenderers.get("xo-branch-visible-outcome")(
  captainEntries[0],
  { expanded: false },
  renderTheme,
);
...
EOF
  ...
  local kind body
  kind=$(./bin/xo-operational-input.sh kind < "$home/state/delivered-processing-request") \
  ...
  body=$(./bin/xo-operational-input.sh body < "$home/state/delivered-processing-request") \
  ...
  case "$body" in
    *"do not re-drain, re-run, or acknowledge the wake."*"call xo_branch_processed with through=3 exactly once."*"never counts as processing."*) ;;
    *) fail "the processing request body lost the event-ownership boundary or the sequence-bound acknowledgement duty: $body" ;;
  esac
  if ./bin/xo-operational-input.sh kind < "$home/state/delivered-routine-note" >/dev/null 2>&1; then
...
globalThis.__fmExecuteBranchBash = async (context) => {
  const actor = spawnSync(
    "bash",
    ["-c", '. "$1"; xo_lease_actor', "_", `${realRoot}/bin/xo-lease-lib.sh`],
...
async function runFleetCommand(session, args) {
  ...
  const command = ["bin/xo-wake-drain.sh", ...args].join(" ");
...
globalThis.__fmOnBranchPrompt = async ({ session }) => {
  const mirror = session.ops
    .filter((op) => op.kind === "custom" && op.message.customType === "xo-main-mirror")
...
const requestedPrompts = [...longRequests, "XO give me a fresh system-resource report."];
...
const operational = spawnSync(
  "bash",
  [`${realRoot}/bin/xo-operational-input.sh`, "encode", "watcher"],
...
const legacyOperational = "⁣XO_OP: give me a fresh system-resource report.";
...
const outcomes = mainTools.find((tool) => tool.name === "xo_branch_outcomes");
...
for (let index = 0; index < requestedPrompts.length; index += 1) {
  ...
  const visible = entries.filter((entry) => entry.customType === "xo-branch-visible-outcome");
...
if (mirroredCaptainText.some((text) =>
  text.includes("operational watcher injection") || text.includes("XO_OP: give me a fresh system-resource report")
)) {
...
let processingRequests = sentToMain.filter((sent) => sent.message.customType === "xo-branch-process");
...
processingRequests = sentToMain.filter((sent) => sent.message.customType === "xo-branch-process");
...
mainEntries.push({
  type: "custom",
  customType: "xo-branch-visible-outcome",
...
await fire("session_start", {}, defaultSessionCtx);
await fire("agent_end", {});
const visible = mainEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome");
...
if (sentToMain.some((sent) => sent.message.customType !== "xo-branch-process")) {
...
await fire("session_shutdown", {});
await fire("session_start", {}, defaultSessionCtx);
if (mainEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome").length !== 2) {
  throw new Error("a second reload duplicated a visible captain outcome");
}
const rendered = entryRenderers.get("xo-branch-visible-outcome")(
  visible[0],
  { expanded: false },
  { fg: (_color, text) => text },
);
...
if (mainEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome" && entry.data.seq === seq3).length !== 1) {
...
const requests = () => sentToMain.filter((sent) => sent.message.customType === "xo-branch-process");
...
mainEntries.push({ type: "custom", customType: "xo-branch-visible-outcome", data: { version: 1, seq: legacy, task: "legacy", verdict: "captain", summary: "delivered before processing existed", silent: false } });
await fire("session_start", {}, defaultSessionCtx);
...
if (mainEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome" && entry.data.seq === seq).length !== 1) {
...
bus.emit("xo:native-tools", {
  register: (tool) => nativeTools.set(tool.name, tool),
  allowMessageType: (type) => messageTypes.add(type),
});
if ([...nativeTools.keys()].sort().join(",") !== "xo_branch_outcomes,xo_branch_processed") throw new Error("native discovery exposed unrelated tools");
...
if ([...messageTypes].sort().join(",") !== "xo-branch-merge,xo-branch-process,xo-sessionstart-nudge") throw new Error("operational message allowlist changed");
const processed = nativeTools.get("xo_branch_processed");
...
// The session exists before its wake prompt opens; the report below must be
// made inside that prompt, so wait for the prompt itself, not the session.
await settle(() => (globalThis.__fmPrompts ?? []).length === 2, "replacement branch prompt");
const report2 = globalThis.__fmSessions[1].options.customTools.find((tool) => tool.name === "xo_branch_report");
...
test_branch_cache_key_is_per_home_stable() {
  ...
  install_pi_branch_extension_fixture "$repo"
  probe() {
    PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$1" XO_ROOT_OVERRIDE="$ROOT" \
...
test_branch_default_on_heartbeat_afk_and_fallback() {
  ...
  install_pi_branch_extension_fixture "$repo"
  cp "$ROOT/bin/xo-branch-outcome.sh" "$ROOT/bin/xo-classify-lib.sh" \
    "$ROOT/bin/xo-lease.sh" "$ROOT/bin/xo-lease-lib.sh" "$ROOT/bin/xo-timeout-lib.sh" \
    "$ROOT/bin/xo-wake-lib.sh" "$ROOT/bin/xo-wake-grant.sh" "$broken/bin/"
  cat > "$broken/bin/xo-branch-prompt.sh" <<'SH'
#!/usr/bin/env bash
echo "synthetic generator failure" >&2
exit 1
SH
  chmod +x "$broken/bin/xo-branch-prompt.sh"
  PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
...
const heartbeatReport = heartbeatSession.options.customTools.find((tool) => tool.name === "xo_branch_report");
...
if (!sentToMain.some((sent) => sent.message.customType === "xo-branch-process" && sent.message.content.includes("task-2 has been stuck for an hour"))) {
...
EOF
  ...
  PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$TMP_ROOT/gating-home-2" XO_ROOT_OVERRIDE="$broken" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
...
// metadata records.
const lib = await import(pathToFileURL(`${dirname(process.env.PLUGIN)}/lib/xo-branch-dispatch.ts`).href);
...
EOF
  ...
  pass "xo_branch_report refuses a task the wake did not name, fleet included, while a heartbeat is unscoped"
}
...
const drain = spawnSync("bash", [`${realRoot}/bin/xo-wake-drain.sh`], {
  encoding: "utf8",
  env: { ...process.env, XO_HOME: home, XO_STATE_OVERRIDE: `${home}/state`, XO_ROOT_OVERRIDE: realRoot },
});
...
pi.events.emit("xo-branch-supervision:dispatch", duringProbe);
...
if (mirrored.some((m) => m.customType !== "xo-main-mirror")) throw new Error("mirror used the wrong custom type");
...
EOF
  ...
  PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_TEST_PRIOR_POINTER="$first_pointer" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { fire, dispatch, settle, makeCtx, home }; })()`);
const { fire, dispatch, settle, makeCtx, home } = globalThis.__t;
...
const prior = process.env.XO_TEST_PRIOR_POINTER;
...
test_branch_model_picker_keeps_follow_main_first_under_ranking() {
  ...
  cp "$ROOT/.pi/extensions/lib/xo-branch-model-picker.ts" "$repo/.pi/extensions/lib/xo-branch-model-picker.ts"
  LIB="$repo/.pi/extensions/lib/xo-branch-model-picker.ts" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
import { pathToFileURL } from "node:url";
...
test_replacement_activation_cleans_leases_and_retries_failure() {
  ...
  cat > "$fakebin/bash" <<'SH'
#!/bin/sh
if [ "$1" = "$XO_TEST_LEASE_SCRIPT" ] && [ ! -e "$XO_TEST_FAIL_MARKER" ]; then
  : > "$XO_TEST_FAIL_MARKER"
  exit 7
fi
exec "$XO_TEST_REAL_BASH" "$@"
SH
  chmod +x "$fakebin/bash"
  PATH="$fakebin:$PATH" PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_TEST_REAL_BASH="$real_bash" XO_TEST_LEASE_SCRIPT="$ROOT/bin/xo-lease.sh" \
    XO_TEST_FAIL_MARKER="$home/state/release-failed-once" DRIVER_PRELUDE="$DRIVER_PRELUDE" \
...
test_cold_start_activates_after_lock_acquisition() {
  ...
  install_pi_branch_extension_fixture "$repo"
  PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_TEST_SKIP_LOCK=1 DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
...
await fire("session_start", {}, defaultSessionCtx);
if (mainEntries.some((entry) => entry.customType === "xo-branch-visible-outcome")) {
...
const oldReport = oldSession.options.customTools.find((tool) => tool.name === "xo_branch_report");
...
try {
  oldBash.__options.spawnHook({
    command: "bin/xo-lease.sh claim task-stale --actor branch",
    cwd: home,
    env: {},
  });
} catch {
...
test_secondary_session_stays_inert() {
  ...
  PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_TEST_SKIP_LOCK=1 XO_TEST_LOCK_PID=$foreign_pid DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { dispatch, home }; })()`);
const { dispatch, home } = globalThis.__t;
import { existsSync, writeFileSync } from "node:fs";
writeFileSync(`${home}/state/.lock`, `${process.env.XO_TEST_LOCK_PID}\n`);
...
replacementBus.emit("xo-branch-supervision:dispatch", offer);
...
# Direct unit coverage of xo-branch-dispatch.ts's classification, independent
...
test_branch_dispatch_classifies_main_only_rows_and_writes_the_eligible_snapshot() {
  ...
  LIB="$repo/.pi/extensions/lib/xo-branch-dispatch.ts" XO_HOME="$home" GRANT="$ROOT/bin/xo-wake-grant.sh" \
    node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
import { pathToFileURL } from "node:url";
...
const state = `${process.env.XO_HOME}/state`;
const project = `${process.env.XO_HOME}/projects/approved`;
...
writeFileSync(
  ...
  [
    ...
    "1\t2\tstale\txo-window\tstale: later routine reminder",
  ].join("\n"),
);
...
if (needsDecisionMixed.taskByWakeKey["task-a.status"] !== "task-a" ||
  needsDecisionMixed.taskByWakeKey["xo-window"] !== "task-a") {
...
writeFileSync(
  `${state}/.wake-queue`,
  [
    "1\t1\tstale\txo-window\tstale: xo-window (awaiting the captain)",
...
if (captainHeldMixed.needsDecisionKeys.join(",") !== "xo-window") {
...
writeFileSync(
  `${state}/.wake-queue`,
  [
    "1\t1\tstale\txo-window\tstale: xo-window (first reminder)",
    "1\t2\tstale\txo-window\tstale: xo-window (second reminder)",
...
if (!repeatedCaptainHeld.eligible || repeatedCaptainHeld.eligibleSeqs.join(",") !== "3" ||
  repeatedCaptainHeld.needsDecisionKeys.join(",") !== "xo-window,xo-window") {
...
if (countedStatusReads !== 1 || repeatedCaptainHeldNextScan.needsDecisionKeys.join(",") !== "xo-window,xo-window") {
...
process.env.XO_CLASSIFY_CAPTAIN_HELD_VERB = "awaiting-captain";
...
if (!customHeld.eligible || customHeld.eligibleSeqs.join(",") !== "2" ||
  customHeld.needsDecisionKeys.join(",") !== "xo-window") {
  throw new Error(`a custom captain-held verb was offered to the branch: ${JSON.stringify(customHeld)}`);
}
delete process.env.XO_CLASSIFY_RESOLVE_VERB;
delete process.env.XO_CLASSIFY_CAPTAIN_HELD_VERB;

process.env.XO_CLASSIFY_RESERVED_KEY_PREFIXES = "secret-";
writeFileSync(
  `${state}/task-a.status`,
...
writeFileSync(
  ...
  [
    "1\t1\tcheck\tx-inbox\tcheck: pending x mention",
    "1\t2\tsignal\ttask-a.status\tsignal: task-a.status",
    "1\t3\tstale\txo-window\tstale: xo-window",
  ].join("\n"),
);
...
writeFileSync(
  ...
  [
    "1\t1\theartbeat\theartbeat\theartbeat",
    "1\t2\tinvented\tsomething\tinvented: not a kind xo_wake_append emits",
  ].join("\n"),
);
...
test_real_pi_picker_primitives_stay_bounded_and_searchable() {
  ...
  package_dir=${XO_PI_PACKAGE_DIR:-"$(npm root -g 2>/dev/null)/@earendil-works/pi-coding-agent"}
  ...
  cp "$ROOT/.pi/extensions/lib/xo-branch-model-picker.ts" "$fixture/lib/xo-branch-model-picker.ts"
  ...
  LIB="$fixture/lib/xo-branch-model-picker.ts" PI_VERSION_FILE="$package_dir/package.json" \
...
test_outcomes_tool_uses_stock_execution_and_export_consumers() {
  ...
  cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" "$fixture/.pi/extensions/lib/xo-operational-input.ts"
  ln -s "$package_dir" "$fixture/node_modules/@earendil-works/pi-coding-agent"
  ln -s "$package_dir/node_modules/@earendil-works/pi-tui" "$fixture/node_modules/@earendil-works/pi-tui"
...
const actualDefinition = tools.find((tool) => tool.name === "xo_branch_outcomes");
if (!actualDefinition) throw new Error("xo_branch_outcomes was not registered");
...
const stockRow = new ToolExecutionComponent("xo_branch_outcomes", "stock", args, { showImages: false }, stockDefinition, ui, process.cwd());
const actualRow = new ToolExecutionComponent("xo_branch_outcomes", "actual", args, { showImages: false }, actualDefinition, ui, process.cwd());
...
const actualResult = actualHtml.renderResult("actual-html", "xo_branch_outcomes", result.content, result.details, false);
...
if (!offer.accepted) throw new Error("branch did not accept the wake offer");
await settle(() => (globalThis.__fmPrompts ?? []).length === 1, "branch wake prompt");
const report = globalThis.__fmSessions[0].options.customTools.find((tool) => tool.name === "xo_branch_report");
...
const deliveredRoutine = sentToMain
  .filter((sent) => sent.message.customType === "xo-branch-merge" && sent.message.content.includes("interleaved outcome "))
...
const deliveredCaptain = mainEntries
  .filter((entry) => entry.customType === "xo-branch-visible-outcome" && entry.data.summary.startsWith("interleaved outcome "))
...
test_session_replacement_during_delivery_neither_loses_nor_duplicates() {
  ...
  cat > "$fakebin/ps" <<'SH'
#!/bin/sh
if [ -f "$XO_TEST_PS_ARM" ]; then
  rm -f "$XO_TEST_PS_ARM"
  : > "$XO_TEST_PS_ENTERED"
  while [ ! -f "$XO_TEST_PS_RELEASE" ]; do sleep 0.01; done
fi
exec "$XO_TEST_REAL_PS" "$@"
SH
  chmod +x "$fakebin/ps"
  PATH="$fakebin:$PATH" PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_TEST_REAL_PS="$real_ps" XO_TEST_PS_ARM="$home/state/ps-arm" \
    XO_TEST_PS_ENTERED="$home/state/ps-entered" XO_TEST_PS_RELEASE="$home/state/ps-release" \
    DRIVER_PRELUDE="$DRIVER_PRELUDE" node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
...
writeFileSync(process.env.XO_TEST_PS_ARM, "");
...
await settle(() => existsSync(process.env.XO_TEST_PS_ENTERED), "in-flight ownership subprocess");
await fire("session_shutdown", {});
const replacementStart = fire("session_start", {}, secondCtx);
writeFileSync(process.env.XO_TEST_PS_RELEASE, "");
...
const recovered = thirdEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome");
...
if (thirdEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome").length !== 1) {
...
test_store_failure_during_delivery_neither_loses_nor_duplicates() {
  ...
  cat > "$fakebin/bash" <<'SH'
#!/bin/sh
armed=$(cat "$XO_TEST_FAIL_ARM" 2>/dev/null || printf '')
if [ -n "$armed" ] && [ "$1" = "$XO_TEST_OUTCOME_SCRIPT" ] && [ "$2" = "$armed" ]; then
  : > "$XO_TEST_FAIL_ARM"
...
SH
  chmod +x "$fakebin/bash"
  PATH="$fakebin:$PATH" PLUGIN="$repo/.pi/extensions/xo-branch-supervision.ts" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_TEST_REAL_BASH="$real_bash" XO_TEST_OUTCOME_SCRIPT="$ROOT/bin/xo-branch-outcome.sh" \
    XO_TEST_FAIL_ARM="$home/state/store-fail-arm" DRIVER_PRELUDE="$DRIVER_PRELUDE" \
    node --input-type=module > "$TMP_ROOT/node-output" 2>&1 <<'EOF'
const prelude = process.env.DRIVER_PRELUDE;
await eval(`(async () => { ${prelude}; globalThis.__t = { fire, dispatch, settle, outcomeScript, sentToMain, mainEntries, defaultSessionCtx }; })()`);
const { fire, dispatch, settle, outcomeScript, sentToMain, mainEntries, defaultSessionCtx } = globalThis.__t;
...
const failArm = process.env.XO_TEST_FAIL_ARM;
...
armStoreFailure("");
await fire("turn_end", {}, defaultSessionCtx);
const visible = mainEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome" && entry.data.seq === failedSeq);
if (visible.length !== 1) {
...
await fire("turn_end", {}, defaultSessionCtx);
if (mainEntries.filter((entry) => entry.customType === "xo-branch-visible-outcome" && entry.data.seq === failedSeq).length !== 1) {
...
# representation, tracked as xo-pi-routine-delivery-idempotency-followup-r1,
...
const routineCopies = (summary) => sentToMain.filter(
  (sent) => sent.message.customType === "xo-branch-merge" && sent.message.content.includes(summary),
).length;
const captainCopies = (seq) => mainEntries.filter(
  (entry) => entry.customType === "xo-branch-visible-outcome" && entry.data.seq === seq,
).length;
