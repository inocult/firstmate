#!/usr/bin/env bash
# Behavior tests for bin/xo-ensure-agents-md.sh.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(xo_test_tmproot xo-ensure-agents-md)
...
test_created_agents_md_includes_self_governance() {
  ...
  mkdir -p "$repo"
  "$ROOT/bin/xo-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 || fail "xo-ensure-agents-md.sh failed for empty project"
  ...
  pass "xo-ensure-agents-md.sh: created AGENTS.md includes self-governance section"
}
...
test_fresh_setup_writes_real_claude_pointer() {
  ...
  mkdir -p "$repo"
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed creating a fresh pointer"
  ...
  pass "xo-ensure-agents-md.sh: fresh setup writes a real @AGENTS.md pointer"
}
...
EOF
  "$ROOT/bin/xo-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 || fail "xo-ensure-agents-md.sh failed for CLAUDE.md promotion"
  agents="$repo/AGENTS.md"
  ...
  pass "xo-ensure-agents-md.sh: promoted CLAUDE.md includes self-governance section"
}
...
test_promoted_claude_md_without_trailing_newline_keeps_blank_separator() {
  ...
  "$ROOT/bin/xo-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 || fail "xo-ensure-agents-md.sh failed for newline-less CLAUDE.md promotion"
  agents="$repo/AGENTS.md"
  assert_grep "Run tests with make test." "$agents" \
  ...
  assert_claude_pointer "$repo/CLAUDE.md"
  pass "xo-ensure-agents-md.sh: newline-less promotion keeps a blank separator line"
}
...
test_existing_agents_md_with_symlink_gains_self_governance() {
  ...
  agents="$repo/AGENTS.md"
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed for existing AGENTS.md with symlink"
  ...
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed on idempotent re-run"
  ...
  pass "xo-ensure-agents-md.sh: existing symlinked AGENTS.md gains the section idempotently"
}
...
test_correct_symlink_migrates_to_pointer_without_clobbering_agents() {
  ...
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed migrating a correct CLAUDE.md symlink"
  ...
  cp "$repo/CLAUDE.md" "$repo/.claude-after-first"
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed on post-migration re-run"
  ...
  pass "xo-ensure-agents-md.sh: correct symlink migrates to pointer without clobbering AGENTS.md"
}
...
test_existing_agents_md_without_claude_gains_section_and_pointer() {
  ...
  agents="$repo/AGENTS.md"
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed for existing AGENTS.md without CLAUDE.md"
  ...
  count=$(grep -Fc "## Maintaining this file" "$agents")
  [ "$count" -eq 1 ] || fail "injection wrote $count self-governance sections"
  pass "xo-ensure-agents-md.sh: existing AGENTS.md without CLAUDE.md gains section and pointer"
}
...
test_existing_agents_md_with_section_reports_unchanged() {
  ...
  "$ROOT/bin/xo-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 \
    || fail "xo-ensure-agents-md.sh failed building the fully-formed fixture"
  agents="$repo/AGENTS.md"
  assert_claude_pointer "$repo/CLAUDE.md"
  cp "$agents" "$repo/.before"
  cp "$repo/CLAUDE.md" "$repo/.claude-before"
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed on already-formed project"
  ...
  pass "xo-ensure-agents-md.sh: AGENTS.md that already has the section stays unchanged"
}
...
test_marked_project_guidance_stays_unchanged() {
  ...
  for eol in $'\n' $'\r\n'; do
    for route in bare pointer symlink promotion; do
      ...
      printf '%s%s' '<!-- xo:maintained-by-project -->' "$eol" \
        '# Project memory' "$eol" \
  ...
  pass "xo-ensure-agents-md.sh: marked project guidance is preserved across ensure paths and line endings"
}

test_reworded_guidance_requires_first_line_marker() {
  local repo marker count eol line
  for marker in '' 'Use <!-- xo:maintained-by-project --> here.' \
    '<!-- xo:maintained-by-project-extra -->' \
    $'```html\n<!-- xo:maintained-by-project -->\n```' \
    $'~~~html\n<!-- xo:maintained-by-project -->\n~~~' \
    '<!-- xo:maintained-by-project -->'; do
    for eol in $'\n' $'\r\n'; do
  ...
  pass "xo-ensure-agents-md.sh: prose and marker examples do not replace a first-line mark"
}
...
test_existing_crlf_agents_md_with_section_stays_unchanged() {
  ...
  cp "$repo/CLAUDE.md" "$repo/.claude-before"
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed on CRLF AGENTS.md with the section"
  ...
  pass "xo-ensure-agents-md.sh: CRLF AGENTS.md with the section stays unchanged"
}
...
test_existing_crlf_agents_md_without_section_preserves_crlf() {
  ...
  agents="$repo/AGENTS.md"
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh failed injecting into CRLF AGENTS.md"
  ...
  cp "$repo/CLAUDE.md" "$repo/.claude-after-first"
  "$ROOT/bin/xo-ensure-agents-md.sh" "$repo" >/dev/null 2>&1 \
    || fail "xo-ensure-agents-md.sh failed on idempotent CRLF re-run"
  cmp -s "$repo/.after-first" "$agents" \
    || fail "idempotent CRLF re-run modified AGENTS.md"
  cmp -s "$repo/.claude-after-first" "$repo/CLAUDE.md" \
    || fail "idempotent CRLF re-run modified CLAUDE.md"
  pass "xo-ensure-agents-md.sh: CRLF injection preserves line endings idempotently"
}
...
test_canonical_pointer_is_accepted_when_both_are_real_files() {
  ...
  write_fixture_claude_pointer "$repo"
  out=$("$ROOT/bin/xo-ensure-agents-md.sh" "$repo" 2>&1) \
    || fail "xo-ensure-agents-md.sh refused a canonical real CLAUDE.md pointer"
  assert_contains "$out" "unchanged:" "canonical pointer plus AGENTS.md was not reported unchanged"
  assert_claude_pointer "$repo/CLAUDE.md"
  pass "xo-ensure-agents-md.sh: canonical real CLAUDE.md pointer is not a conflict"
}
...
test_distinct_real_files_are_refused() {
  ...
  pass "xo-ensure-agents-md.sh: refuses distinct real AGENTS.md and CLAUDE.md"
}
...
test_agents_md_symlink_is_refused() {
  ...
  pass "xo-ensure-agents-md.sh: refuses AGENTS.md when it is a symlink"
}
...
test_wrong_target_symlink_is_refused() {
  ...
  pass "xo-ensure-agents-md.sh: refuses a CLAUDE.md symlink that does not point to AGENTS.md"
}
...
test_non_regular_claude_md_is_refused() {
  ...
  pass "xo-ensure-agents-md.sh: refuses a non-regular CLAUDE.md"
}
...
test_lowercase_agents_md_refuses_case_fragile_pointer() {
  ...
  pass "xo-ensure-agents-md.sh: refuses a case-variant lowercase agents.md (issue #389)"
}
