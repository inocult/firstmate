...
#
# The run-wrapper cases drive the REAL bin/xo-session-start.sh against a
...
if [ "${XO_SESSIONSTART_TEST_HARNESS:-0}" != 1 ]; then
  HARNESS_FIXTURE=$(mktemp -d "${TMPDIR:-/tmp}/xo-sessionstart-harness.XXXXXX") || exit 1
  ln -s /bin/bash "$HARNESS_FIXTURE/codex" || exit 1
  # shellcheck disable=SC2016 # Expand in the fixture shell, not this parent.
  XO_SESSIONSTART_TEST_HARNESS=1 "$HARNESS_FIXTURE/codex" \
    -c '"$@"; rc=$?; :; exit "$rc"' _ "$0" "$@"
  HARNESS_STATUS=$?
...
TMP_ROOT=$(xo_test_tmproot xo-sessionstart-nudge)
NUDGE="$ROOT/bin/xo-sessionstart-nudge.sh"
RUN="$ROOT/bin/xo-sessionstart-run.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/xo-operational-input.sh"
NUDGE_TEXT="Run \`bin/xo-session-start.sh\` now, exactly once, before executing any other instructions."
xo_operational_input_encode session-start "$NUDGE_TEXT" NUDGE_LINE \
...
run_nudge() {
  local root=$1
  XO_GATE_REFUSE_BYPASS=0 XO_ROOT_OVERRIDE="$root" XO_HOME="$root" "$NUDGE"
}
...
test_genuine_primary_nudges() {
  ...
  pass "xo-sessionstart-nudge: a genuine primary gets one explicitly marked instruction line"
}

test_gate_env_is_silent() {
  local root="$TMP_ROOT/gate-env"
  make_primary "$root"
  expect_silent_zero "gate env nudge" env NO_MISTAKES_GATE=1 XO_GATE_REFUSE_BYPASS=0 \
    XO_ROOT_OVERRIDE="$root" XO_HOME="$root" "$NUDGE"
  pass "xo-sessionstart-nudge: NO_MISTAKES_GATE is silent"
}
...
test_linked_secondmate_primary_nudges() {
  ...
  pass "xo-sessionstart-nudge: a marked linked secondmate home is a primary"
}
...
test_missing_state_is_silent() {
  ...
  pass "xo-sessionstart-nudge: a checkout without state is silent"
}
...
test_owned_lock_is_silent() {
  ...
  pass "xo-sessionstart-nudge: a lock holder in process ancestry is already run"
}

test_opencode_plugin_delivers_exact_nudge_once() {
  local root="$TMP_ROOT/opencode-primary" out status=0
  make_primary "$root"
  cp "$ROOT/bin/xo-sessionstart-nudge.sh" "$ROOT/bin/xo-primary-scope-lib.sh" \
    "$ROOT/bin/xo-gate-refuse-lib.sh" "$ROOT/bin/xo-operational-input.sh" "$root/bin/"
  chmod +x "$root/bin/xo-sessionstart-nudge.sh"
  out=$(PLUGIN="$ROOT/.opencode/plugins/xo-primary-sessionstart-nudge.js" \
...
const hooks = await mod.XoPrimarySessionstartNudge({
  client,
...
# xo-session-start.sh can execute: a git repo on main so the tangle check
...
RUN_PATH=${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
...
run_hook() {  # <root> [args...]
  ...
  env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
    XO_GATE_REFUSE_BYPASS=0 XO_ROOT_OVERRIDE="$root" XO_HOME="$root" PATH="$RUN_PATH" "$RUN" "$@"
}
...
# Every run-tier assertion keys off the digest banner, which xo-session-start.sh
...
test_run_startup_runs_the_full_digest() {
  ...
  assert_not_contains "$out" "XO_OP" "a run-tier open also emitted the nudge instruction"
...
test_run_clear_and_compact_reemit() {
  ...
  for source in clear compact; do
    ...
    assert_not_contains "$out" "XO_OP" "a $source open also emitted the nudge instruction"
  done
...
test_pi_startup_classifies_cli_continuations() {
  ...
  cp "$ROOT/.pi/extensions/xo-primary-turnend-guard.ts" "$fixture/.pi/extensions/"
  cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" \
    "$ROOT/.pi/extensions/lib/xo-sessionstart-supervisor.mjs" "$fixture/.pi/extensions/lib/"
  cat > "$fixture/bin/xo-sessionstart-run.sh" <<'SH'
#!/usr/bin/env bash
source_name=
while [ $# -gt 0 ]; do
  case "$1" in
...
printf '%s\n' "$source_name" >> "${XO_HOME:?}/state/sources"
SH
  cat > "$fixture/bin/xo-turnend-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$fixture/bin/"*.sh

  out=$(EXT="$fixture/.pi/extensions/xo-primary-turnend-guard.ts" \
    XO_HOME="$fixture" XO_ROOT_OVERRIDE="$fixture" \
...
test_pi_sessionstart_generation_prerequisite() {
  ...
  cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" \
    "$ROOT/.pi/extensions/lib/xo-sessionstart-supervisor.mjs" "$fixture/.pi/extensions/lib/"
  cp "$ROOT/bin/xo-operational-input.sh" "$fixture/bin/"
  cat > "$fixture/bin/xo-turnend-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
...
printf 'launch:%s:%s:%s:%s:%s\n' \
  "$count" "$behavior" "$source_name" "$$" "${XO_SESSIONSTART_SUPERVISOR_PID:-}" >> "$state/launches"
printf 'launch:%s\n' "$count" >> "$state/events"
case "$behavior" in
  success)
...
const runner = `${process.env.XO_HOME}/bin/xo-sessionstart-run.sh`;
const handlers = new Map();
const sent = [];
const pi = {
  on(event, handler) { handlers.set(event, handler); },
...
assert(!provenResult.message.content.includes("Run `bin/xo-session-start.sh`"), "proven path used manual fallback");
...
assert(emptyResult?.message?.content.includes("Run `bin/xo-session-start.sh` now, exactly once, before executing any other instructions."),
...
assert(spawnErrorResult?.message?.content.includes("Run `bin/xo-session-start.sh` now, exactly once"),
...
assert(failedResult?.message?.content.includes("Run `bin/xo-session-start.sh` now, exactly once"),
...
assert(failedResumeResult?.message?.content.includes("Run `bin/xo-session-start.sh` now, exactly once"),
...
assert(failedForkResult?.message?.content.includes("Run `bin/xo-session-start.sh` now, exactly once"),
...
assert(!timedResult.message.content.includes("Run `bin/xo-session-start.sh`"), "bounded timeout incorrectly used manual fallback");
...
printf '%s:%s:%s\n' "$index" "$$" "${XO_SESSIONSTART_SUPERVISOR_PID:-}" >> "$state/launches"
(
  trap '' TERM INT HUP
  while :; do sleep 30; done
) </dev/null >/dev/null 2>&1 &
...
test_pi_large_sessionstart_digest_is_delivered_loudly() {
  ...
  cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" \
    "$ROOT/.pi/extensions/lib/xo-sessionstart-supervisor.mjs" "$fixture/.pi/extensions/lib/"
  cp "$ROOT/bin/xo-sessionstart-run.sh" "$ROOT/bin/xo-sessionstart-nudge.sh" \
    "$ROOT/bin/xo-primary-scope-lib.sh" "$ROOT/bin/xo-gate-refuse-lib.sh" \
    "$ROOT/bin/xo-hook-host-lib.sh" \
    "$ROOT/bin/xo-operational-input.sh" "$fixture/bin/"
  cat > "$fixture/bin/xo-session-start.sh" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  out=$(EXT="$fixture/.pi/extensions/xo-primary-turnend-guard.ts" \
    XO_HOME="$fixture" XO_ROOT_OVERRIDE="$fixture" XO_GATE_REFUSE_BYPASS=1 \
    node --input-type=module 2>&1 <<'JS'
import { pathToFileURL } from "node:url";
const handlers = new Map();
const pi = {
...
if (!content.includes("XO_OP: v1 session-start:")) throw new Error("operational provenance was lost");
JS
  ) || status=$?
...
test_run_reads_source_from_the_hook_payload() {
  ...
  assert_contains "$out" "XO_OP" "a resume hook payload did not delegate to the nudge"
...
test_run_gate_and_scope_are_silent() {
  ...
  local out status=0
  make_run_primary "$root"
  expect_silent_zero "gate env run" env NO_MISTAKES_GATE=1 XO_GATE_REFUSE_BYPASS=0 \
    XO_ROOT_OVERRIDE="$root" XO_HOME="$root" PATH="$RUN_PATH" "$RUN" --source startup
  assert_absent "$root/state/.lock" "a gate agent's session open still took the fleet lock"
  out=$(env NO_MISTAKES_GATE=1 XO_GATE_REFUSE_BYPASS=0 \
    XO_ROOT_OVERRIDE="$root" XO_HOME="$root" PATH="$RUN_PATH" \
  ...
  xo_git_worktree "$base" "$linked" xo/run-linked
