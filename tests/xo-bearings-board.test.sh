#!/usr/bin/env bash
# Behavior tests for bin/xo-bearings-board.sh: fail-closed payload validation,
...
BOARD="$ROOT/bin/xo-bearings-board.sh"
TMP_ROOT=$(xo_test_tmproot xo-bearings-board)
...
case "${1-}" in
  ...
  poll)
    ...
    limit=${XO_TEST_STUB_MAX_BLOCK_SECONDS:-120}
...
run_board() {  # <home> <args...>
  local home=$1
  shift
  PATH="$home/fakebin:$PATH" XO_HOME="$home" \
...
test_build_refuses_malformed_payloads_before_touching_the_board() {
  ...
  printf '{"schema":"xo-bearings-board.v2"}\n' > "$data"
  set +e; out=$(run_board "$home" build "$data" 2>&1); rc=$?; set -e
  [ "$rc" -ne 0 ] || fail "a wrong-schema payload was accepted"
  assert_contains "$out" "xo-bearings-board.v1" "the schema refusal did not name the contract: $out"
...
test_build_injects_binds_then_arms() {
  ...
  grep -qxF '__XO_BEARINGS_BOARD_DATA__' "$board" \
...
## Done
EOF
  xo_write_meta "$home/state/$origin.meta" "project=$home/projects/sample" "kind=scout"
  ...
  cat > "$runtime/bin/xo-procevent-lavish.sh" <<'SH'
#!/usr/bin/env bash
set -eu
if [ "${1:-}" = arm ]; then
  artifact=${2:-}
...
SH
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$runtime" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    XO_BEARINGS_BOARD_TEMPLATE="$ROOT/.agents/skills/bearings/assets/board-template.html" \
    REAL_LAVISH_ADAPTER="$ROOT/bin/xo-procevent-lavish.sh" \
    REAL_PROCEVENT="$ROOT/bin/xo-procevent.sh" ORDER_PROOF_HOLD="$hold" \
    "$runtime/bin/xo-bearings-board.sh" build "$data" >/dev/null \
...
run_lavish_source_id() {  # <home> <artifact>
  ...
  PATH="$home/fakebin:$PATH" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/xo-procevent-lavish.sh" source-id "$2"
}
...
test_build_refuses_a_template_without_exactly_one_slot() {
  ...
  set +e
  out=$(XO_BEARINGS_BOARD_TEMPLATE="$home/broken-template.html" run_board "$home" build "$data" 2>&1)
  rc=$?
  set -e
...
SH
  ...
  set +e
  out=$(XO_PROC_ROOT_OVERRIDE="$home/no-proc" run_board "$home" build "$data" 2>&1)
  rc=$?
  set -e
