#!/usr/bin/env bash
# tests/xo-startup-network.test.sh - behavior tests for bin/xo-startup-network.sh,
...
TMP_ROOT=$(xo_test_tmproot xo-startup-network-tests)
DRAIN="$ROOT/bin/xo-wake-drain.sh"
XO_TEST_CLEANUP_DIRS+=("$TMP_ROOT")
trap xo_test_cleanup EXIT

# new_world <name>: an XO_HOME plus a fake code root whose bin/ is a real
# xo bin/ except for xo-bootstrap.sh, which is replaced by a scriptable
# stand-in. The stage's contract is about WHEN and WHETHER the network half runs
# and how its result is published; bin/xo-bootstrap.sh's own behavior is owned by
# tests/xo-bootstrap.test.sh, so pinning it here would duplicate that owner and
...
new_world() {
  ...
  rm -f "$root/bin/xo-bootstrap.sh"
  cat > "$root/bin/xo-bootstrap.sh" <<'SH'
#!/usr/bin/env bash
# Scriptable stand-in: records how it was invoked, then behaves as the test asks.
set -u
printf 'network=%s detect_only=%s\n' \
  "${XO_BOOTSTRAP_NETWORK:-all}" "${XO_BOOTSTRAP_DETECT_ONLY:-0}" \
  >> "${XO_FAKE_BOOTSTRAP_LOG:?}"
# The real sweeps record their elapsed times through xo-timing-lib.sh, which
# reaches them as an exported XO_TIMING_LOG. Recording the same way here proves
...
# is owned by tests/xo-bootstrap.test.sh.
if [ -n "${XO_TIMING_LOG:-}" ]; then
  . "$(dirname "$0")/xo-timing-lib.sh"
  xo_timing_record phase "${XO_FAKE_TIMING_PHASE:-gh-auth}" \
    "$(( $(xo_timing_now_ms) - 1500 ))" "${XO_FAKE_TIMING_DETAIL:-}"
fi
[ -z "${XO_FAKE_BOOTSTRAP_SLEEP:-}" ] || sleep "$XO_FAKE_BOOTSTRAP_SLEEP"
[ -z "${XO_FAKE_BOOTSTRAP_OUT:-}" ] || printf '%s\n' "$XO_FAKE_BOOTSTRAP_OUT"
exit "${XO_FAKE_BOOTSTRAP_RC:-0}"
SH
  chmod +x "$root/bin/xo-bootstrap.sh"
  cat > "$root/bin/ps" <<'SH'
#!/usr/bin/env bash
pid=
previous=
...
if [ "$pid" = "${XO_FAKE_HARNESS_PID:-}" ]; then
  case "$*" in
...
EOF
  ...
  pass "xo-startup-network: wait fails when no deferred stage publishes before its deadline"
}

run_stage() {  # <home> <root> <args...>
  local home=$1 root=$2
  shift 2
  PATH="$root/bin:$PATH" XO_FAKE_HARNESS_PID="${XO_FAKE_HARNESS_PID_OVERRIDE:-$$}" \
    XO_HOME="$home" XO_ROOT_OVERRIDE="$root" "$root/bin/xo-startup-network.sh" "$@"
}
...
EOF
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_SLEEP=10 \
  ...
  assert_contains "$pending" "$root/bin/xo-startup-network.sh report" \
  ...
  pass "xo-startup-network: start returns immediately and never holds the caller's stdout open"
}
...
EOF
  ...
  sleep 30 &
  claimant=$!
  XO_SESSION_START_TIMEOUT=15 XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_OUT='acknowledged result' \
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_OUT='MISSING: some-tool (install: brew install some-tool)' \
  ...
  pass "xo-startup-network: exactly one of the digest and the wake reports each actionable result"
}
...
EOF
  ...
  XO_SESSION_START_TIMEOUT=4 XO_FAKE_BOOTSTRAP_LOG="$log" \
    XO_FAKE_BOOTSTRAP_OUT='MISSING: some-tool (install: brew install some-tool)' \
    run_stage "$home" "$root" start --locked 0 --harvest-pid "$claimant"
  ...
  pass "xo-startup-network: a claimant crash after publication still surfaces the result"
}
...
EOF
  ...
  XO_SESSION_START_TIMEOUT=4 XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_OUT='unpublishable result' \
    run_stage "$home" "$root" start --locked 0 --harvest-pid "$claimant"
  ...
  pass "xo-startup-network: a report-publication failure is failed, diagnosed, and still wakes"
}
...
EOF
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" \
    XO_FAKE_BOOTSTRAP_OUT='BOOTSTRAP_INFO: fixture completed benign work' \
    run_stage "$home" "$root" run --locked 0
  [ ! -s "$home/state/.wake-queue" ] \
  ...
  pass "xo-startup-network: silent and explicitly informational successes never queue a main-blocking wake"
}
...
EOF
  ...
  pass "xo-startup-network: an actionable state=done report still queues a wake"
}
...
EOF
    ...
    if [ "$kind" = malformed ]; then
      ...
      printf 'mate\n' > "$target"
      ln -s "$target" "$home/.xo-secondmate-home"
    fi

    XO_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" run --locked 1
    ...
    err="$home/drain.err"
    XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$DRAIN" >/dev/null 2> "$err"
    ...
    XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" "$DRAIN" \
  ...
  pass "xo-startup-network: deferred invalid secondmate markers produce durable wakes"
}
...
EOF
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" run --locked 1 --lock-pid 111111
  ...
  : > "$log"
  printf '%s\n' $$ > "$home/state/.lock"
  XO_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  ...
  pass "xo-startup-network: manual callers cannot forge mutation authority"
}
...
EOF
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_SLEEP=20 XO_STARTUP_NETWORK_TIMEOUT=2 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  run_stage "$home" "$root" harvest --pid $$ >/dev/null
  XO_STARTUP_NETWORK_TIMEOUT=2 run_stage "$home" "$root" wait 10 >/dev/null \
  ...
  assert_contains "$report" "xo-startup-network.sh run --locked 1" \
  ...
  pass "xo-startup-network: an aggregate bound turns a wedged sweep into an actionable line"
}
...
phases=probe,sweeps
EOF
  assert_contains "$(XO_STARTUP_NETWORK_TIMEOUT=10 run_stage "$home" "$root" report)" \
  ...
  pass "xo-startup-network: an abandoned run reports as needing a rerun, never as in progress forever"
}
...
EOF
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_SLEEP=6 \
  ...
  pass "xo-startup-network: locked requests supersede in-flight probe-only workers"
}
...
EOF
  ...
  pass "xo-startup-network: a second start never launches a competing worker"
}
...
EOF
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_SLEEP=5 \
    run_stage "$home" "$root" start --locked 0 --harvest-pid $$
  ...
  pass "xo-startup-network: start atomically reserves the generation harvest observes"
}
...
EOF
  ...
  XO_FAKE_HARNESS_PID_OVERRIDE="$next_owner" XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_SLEEP=1 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  ...
  pass "xo-startup-network: a new lock owner gets a distinct worker generation"
}
...
EOF
  ...
  started=$(date +%s)
  rc=0
  out=$(PATH="$root/bin:$PATH" XO_FAKE_HARNESS_PID="$next_owner" \
    XO_HOME="$home" XO_ROOT_OVERRIDE="$root" "$root/bin/xo-lock.sh" 2>&1) || rc=$?
  elapsed=$(( $(date +%s) - started ))
  ...
  out=$(PATH="$root/bin:$PATH" XO_FAKE_HARNESS_PID="$next_owner" \
    XO_HOME="$home" XO_ROOT_OVERRIDE="$root" "$root/bin/xo-lock.sh" 2>&1) \
  ...
  pass "xo-startup-network: fleet-lock takeover cannot overlap a mutating sweep"
}
...
test_records_share_one_origin_so_offsets_form_a_timeline() {
  ...
  (
    # shellcheck source=bin/xo-timing-lib.sh
    . "$ROOT/bin/xo-timing-lib.sh"
    unset XO_TIMING_EPOCH_MS
    XO_TIMING_LOG=$log
    export XO_TIMING_LOG
    base=$(xo_timing_now_ms)
    xo_timing_record phase first "$base"
    xo_timing_record phase second "$(( base + 5000 ))"
    xo_timing_record phase third "$(( base + 9000 ))"
  )
  ...
  pass "xo-startup-network: timing records share one origin so their offsets form a timeline"
}
...
EOF
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_OUT='sweep finding' \
    XO_FAKE_TIMING_PHASE=fleet-sync XO_FAKE_TIMING_DETAIL=dotfiles-private \
  ...
  assert_not_contains "$harvest_out" "slowest:" \
    "the timings leaked into the session-start digest section"
  pass "xo-startup-network: timings are durable and printed only on demand"
}
...
EOF
  ...
  XO_STARTUP_NETWORK_TIMEOUT=1 XO_SESSION_START_TIMEOUT=2 \
    XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_BOOTSTRAP_SLEEP=20 \
    XO_FAKE_TIMING_PHASE=secondmate-liveness XO_FAKE_TIMING_DETAIL='mate-a@host-one' \
    run_stage "$home" "$root" run --locked 1
  ...
  pass "xo-startup-network: a timed-out run still publishes the partial timings it recorded"
}
...
EOF
  ...
  XO_FAKE_BOOTSTRAP_LOG="$log" XO_FAKE_TIMING_PHASE=secondmate-sync \
    XO_FAKE_TIMING_DETAIL="ssh -i /key host	v1	forged	0	9999
GITHUB_TOKEN=ghp_supersecretvalue" \
  ...
  pass "xo-startup-network: the timing artifact cannot carry a command line or forge records"
}
...
test_a_bounded_run_still_publishes_the_timings_it_managed_to_record
test_the_timing_artifact_cannot_carry_a_command_line_or_forge_records
echo "# xo-startup-network.test.sh: all assertions passed"
