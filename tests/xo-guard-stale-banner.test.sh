#!/usr/bin/env bash
# Regression tests for xo-guard's watcher-down banner deduplication.
#
# The first stale command in one XO_HOME must print the full actionable watcher
# banner.
...
TMP_ROOT=$(xo_test_tmproot xo-guard-stale-banner)
...
make_guard_case() {
  ...
  xo_write_meta "$home/state/task.meta" "window=xo:xo-task" "kind=ship"
  printf '%s\n' "$dir"
}
...
record_live_watcher() {
  ...
  home=$(case_home "$dir")
  identity=$(XO_STATE_OVERRIDE="$home/state" bash -c '. "$1"; xo_pid_identity "$2"' _ "$ROOT/bin/xo-wake-lib.sh" "$pid") || return 1
...
run_guard_case() {
  local dir=$1
  XO_ROOT_OVERRIDE="$(case_root "$dir")" \
    XO_HOME="$(case_home "$dir")" \
    XO_GUARD_GRACE=999 \
    XO_SUPERVISION_MODEL=persistent \
...
# The Pi extension model: .pi/extensions/xo-primary-pi-watch.ts tears the watcher
...
run_guard_case_extension() {
  ...
  XO_ROOT_OVERRIDE="$(case_root "$dir")" \
    XO_HOME="$(case_home "$dir")" \
    XO_GUARD_GRACE=999 \
    XO_SUPERVISION_MODEL=extension \
    "$ROOT/bin/xo-guard.sh" 2>&1
}

# The same extension-model call from the supervision branch actor
# (XO_SUPERVISION_ACTOR=branch, as the Pi branch extension injects it).
...
record_pi_extension_session() {
  ...
  for pair in \
    "xo-primary-pi-watch.ts:.pi-watch-extension-loaded:watch" \
    "xo-primary-turnend-guard.ts:.pi-turnend-extension-loaded:turnend"; do
    source=${pair%%:*}
    ...
    if [ "$drift" = "${pair##*:}" ]; then
      ...
      version=$(XO_STATE_OVERRIDE="$home/state" bash -c '. "$1"; xo_pi_extension_version "$2"' \
        _ "$ROOT/bin/xo-wake-lib.sh" "$root/.pi/extensions/$source") || return 1
    fi
...
test_first_stale_call_prints_full_banner() {
  ...
  pass "xo-guard stale banner: first stale call prints the full actionable banner"
}
...
test_repeated_same_episode_prints_reminder_only() {
  ...
  pass "xo-guard stale banner: repeated same-episode calls print a concise reminder only"
}
...
test_fresh_beacon_without_live_watcher_stays_alarm() {
  ...
  pass "xo-guard stale banner: a fresh beacon without a live watcher remains unhealthy"
}
...
test_x_mode_without_live_watcher_stays_alarm() {
  ...
  pass "xo-guard stale banner: X-mode polling without a live watcher remains unhealthy"
}
...
test_healthy_recovery_rearms_next_stale_episode() {
  ...
  pass "xo-guard stale banner: healthy recovery rearms the next stale episode"
}
...
test_concurrent_same_episode_prints_one_full_banner() {
  ...
  pass "xo-guard stale banner: concurrent same-episode calls claim exactly one full banner"
}
...
test_home_isolation() {
  ...
  pass "xo-guard stale banner: deduplication is isolated per XO_HOME"
}
...
test_queued_wake_warning_stays_independent() {
  ...
  pass "xo-guard stale banner: queued-wake warning remains independent"
}
...
test_read_only_before_writable_does_not_consume_full_banner() {
  ...
  pass "xo-guard stale banner: read-only before writable does not consume full banner"
}
...
test_read_only_during_episode_observes_without_mutating_marker() {
  ...
  pass "xo-guard stale banner: read-only during episode observes without mutating marker"
}
...
test_healthy_read_only_does_not_clear_marker() {
  ...
  pass "xo-guard stale banner: healthy read-only does not clear marker"
}
...
test_read_only_never_mutates_stale_banner_state_files() {
  ...
  pass "xo-guard stale banner: read-only never mutates stale-banner state files"
}
...
test_autoarm_fresh_beacon_without_watcher_is_healthy() {
  ...
  pass "xo-guard stale banner: auto-arm fresh beacon without a live watcher is healthy"
}
...
test_autoarm_stale_beacon_alarms_with_correct_reason() {
  ...
  pass "xo-guard stale banner: auto-arm stale beacon alarms with the true reason"
}
...
test_autoarm_stale_episode_is_stable() {
  ...
  pass "xo-guard stale banner: auto-arm stale episode stays one episode across calls"
}
...
test_autoarm_long_handling_turn_stays_silent() {
  ...
  pass "xo-guard stale banner: auto-arm long handling turn with a healthy rewake stays silent"
}
...
test_autoarm_long_turn_requires_every_healthy_signal() {
  ...
  pass "xo-guard stale banner: every auto-arm long-turn healthy signal is load-bearing"
}
...
test_autoarm_open_claim_does_not_explain_stale_beacon() {
  ...
  sleep 60 &
  pid=$!
  identity=$(xo_test_pid_identity "$pid") \
  ...
  pass "xo-guard stale banner: an open auto-arm claim does not explain a stale beacon"
}
...
test_autoarm_long_turn_does_not_silence_other_models() {
  ...
  pass "xo-guard stale banner: auto-arm long-turn health does not leak to other models"
}
...
test_persistent_no_watcher_banner_names_missing_process() {
  ...
  pass "xo-guard stale banner: persistent no-watcher banner names the true reason"
}
...
test_persistent_no_watcher_episode_survives_beacon_touch() {
  ...
  pass "xo-guard stale banner: a no-watcher episode survives a beacon mtime change"
}
...
test_extension_handoff_with_live_session_is_healthy() {
  ...
  pass "xo-guard stale banner: extension-owned hand-off with a live session is healthy"
}
...
test_extension_handoff_with_empty_lock_is_healthy() {
  ...
  pass "xo-guard stale banner: extension-owned empty lock is genuinely unheld"
}
...
test_extension_held_unhealthy_locks_stay_alarm() {
      ...
      *)
        ...
        case "$case_name" in
          dead-pid)
            ...
            ;;
          wrong-home)
            printf '%s\n' "$home/other" > "$home/state/.watch.lock/xo-home"
            ;;
          wrong-path)
            printf '%s\n' "$home/bin/not-xo-watch.sh" > "$home/state/.watch.lock/watcher-path"
            ;;
          identity-mismatch)
  ...
  pass "xo-guard stale banner: held unhealthy extension locks stay loud"
}
...
test_extension_without_ownership_evidence_stays_alarm() {
  ...
  pass "xo-guard stale banner: extension model without ownership evidence stays loud"
}
...
test_extension_ownership_needs_every_signal() {
  ...
  pass "xo-guard stale banner: every extension-ownership signal is load-bearing"
}
...
test_extension_stale_beacon_alarms_despite_live_session() {
  ...
  pid=$!
  record_pi_extension_session "$dir" "$pid" || fail "could not record the Pi extension session"
  out=$(XO_ROOT_OVERRIDE="$(case_root "$dir")" \
    XO_HOME="$home" \
    XO_GUARD_GRACE=1 \
    XO_SUPERVISION_MODEL=extension \
    "$ROOT/bin/xo-guard.sh" 2>&1)
  ...
  pass "xo-guard stale banner: extension model still alarms on a genuinely stale beacon"
}
...
test_extension_handoff_keeps_queued_wake_warning() {
  ...
  pass "xo-guard stale banner: queued-wake warning survives the extension hand-off tolerance"
}
...
# The supervision branch runs guarded commands (xo-peek, xo-crew-state) while
...
test_branch_actor_is_not_told_to_drain_queued_wakes() {
  ...
  touch "$home/state/.last-watcher-beat"
  printf '%s\n' \
    "1700000000	7	stale	xo:xo-task	stale: xo:xo-task (idle 378s, possible wedge)" \
  ...
  pass "xo-guard: the branch actor is never told to drain queued wakes while main still is"
}
...
test_persistent_model_ignores_pi_extension_evidence() {
  ...
  pass "xo-guard stale banner: persistent primaries ignore Pi extension evidence"
}
...
test_extension_live_watcher_is_healthy_without_ownership_evidence() {
  ...
  pass "xo-guard stale banner: extension model stays silent for a live watcher"
}

# The cases above pin the model. This one takes the end-user path instead: no
# XO_SUPERVISION_MODEL at all, so bin/xo-harness.sh must route a Pi primary to the
# extension model on its own. Without that routing the tolerance would never reach
# a real Pi home. The foreign markers are cleared because xo-harness.sh tests them
...
test_pi_harness_routes_itself_to_the_extension_model() {
  ...
  for harness in pi pi-signed; do
    ...
    [ "$harness" = pi ] || pi_env+=(XO_PI_HARNESS=pi-signed)
    ...
    record_pi_extension_session "$dir" "$pid" || fail "could not record the Pi extension session"
    touch "$home/state/.last-watcher-beat"
    out=$(env -u CLAUDECODE -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GROK_AGENT -u XO_SUPERVISION_MODEL \
      "${pi_env[@]}" \
      XO_ROOT_OVERRIDE="$(case_root "$dir")" \
      XO_HOME="$home" \
      XO_GUARD_GRACE=999 \
  ...
  pass "xo-guard stale banner: Pi and pi-signed primaries route themselves to the extension model"
}
