...
# bin/xo-harness.sh checks verified ENV markers before ancestry. Muse is
...
TMP_ROOT=$(xo_test_tmproot xo-muse-harness)
...
case "${1:-}" in
  show-environment)
    [ "${XO_FAKE_WORKER_META_KEY:-}" = present ] || exit 1
  ...
  send-keys)
    ...
    for arg in "$@"; do
      if [ "$prev" = -l ]; then
        printf '%s\n' "$arg" >> "$XO_FAKE_LAUNCH_LOG"
        if [ "${XO_FAKE_EXECUTE_MUSE_LAUNCH:-}" = 1 ]; then
          case "$arg" in
            *"$XO_FAKE_MUSE_EXECUTABLE"*) (cd "$XO_FAKE_PANE_PATH" && bash -c "$arg") ;;
          esac
        fi
        break
      fi
...
#!/usr/bin/env bash
set -u
[ -n "${XO_FAKE_HARNESS_RESULT:-}" ] || exit 0
exec "$XO_FAKE_MUSE_VERSIONED" -c 'result=$($XO_FAKE_HARNESS_PROBE); printf "%s" "$result" > "$XO_FAKE_HARNESS_RESULT"'
SH
...
run_muse_spawn() {  # <home> <proj> <wt> <fakebin> <id> [extra args...]
  ...
  XO_ROOT_OVERRIDE='' XO_HOME="$home" \
    ...
    XO_FAKE_LAUNCH_LOG="$home/launch.log" \
    XO_FAKE_MUSE_EXECUTABLE="$fakebin/muse" \
    XO_FAKE_MUSE_VERSIONED="$fakebin/muse-bin-test-version" \
    XO_FAKE_HARNESS_PROBE="$HARNESS" \
    XO_FAKE_EXECUTE_MUSE_LAUNCH="${XO_FAKE_EXECUTE_MUSE_LAUNCH:-}" \
    XO_FAKE_HARNESS_RESULT="${XO_FAKE_HARNESS_RESULT:-}" \
    XO_FAKE_WORKER_META_KEY="${XO_TEST_MUSE_WORKER_KEY-present}" \
    META_API_KEY="${XO_TEST_MUSE_KEY-test-key}" \
    XDG_CONFIG_HOME="${XO_TEST_MUSE_CONFIG_HOME-$home/xdgconfig}" \
    XDG_DATA_HOME="${XO_TEST_MUSE_DATA_HOME-$home/xdgdata}" \
    PATH="$fakebin:$PATH" \
...
# xo-harness.sh from a child of it.
#
...
test_detects_versioned_process_ancestor() {
  ...
  for bin in muse-bin-0.1.0-R708.1 muse-bin-9.9.9-RZZZ.9 muse; do
    ...
    [ "$out" = muse ] || fail "xo-harness.sh under process '$bin' reported '$out', expected muse"
  done
...
test_detection_is_anchored() {
  ...
  for bin in musescore amuse notmuse-bin muse-binary muse-bind; do
    ...
    out=$(env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
      -u CURSOR_AGENT -u CURSOR_INVOKED_AS -u GEMINI_CLI \
      "$dir/$bin" -c "r=\$(\"$HARNESS\"); printf '%s' \"\$r\"")
    [ "$out" != muse ] || fail "xo-harness.sh misdetected unrelated process '$bin' as muse"
  done
...
EOF
  ...
  out=$(CLAUDECODE=1 PI_CODING_AGENT=true GROK_AGENT=1 XO_PI_HARNESS=pi-signed \
    CURSOR_AGENT=1 CURSOR_INVOKED_AS=cursor-agent \
    XO_FAKE_EXECUTE_MUSE_LAUNCH=1 XO_FAKE_HARNESS_RESULT="$result" \
...
EOF
  ...
  # The captain accepted muse's self-update risk, so xo must not pin it.
...
EOF
  ...
  out=$(XO_TEST_MUSE_KEY='' XO_TEST_MUSE_WORKER_KEY='' run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" \
...
$rec
EOF
  out=$(XO_TEST_MUSE_KEY='caller-only-secret' XO_TEST_MUSE_WORKER_KEY='' \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode no-mistakes --yolo off)
  status=$?
  [ "$status" -ne 0 ] || fail "muse spawn accepted a caller-only META_API_KEY"
  assert_contains "$out" "set for xo-spawn but cannot be proven present" \
...
EOF
  ...
  XO_TEST_MUSE_KEY='' XO_TEST_MUSE_WORKER_KEY='' \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" \
...
EOF
  ...
  out=$(cd "$caller" && XO_TEST_MUSE_KEY='' XO_TEST_MUSE_WORKER_KEY='' \
    XO_TEST_MUSE_CONFIG_HOME=cfg XO_TEST_MUSE_DATA_HOME=data \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode no-mistakes --yolo off)
  status=$?
...
# dialect rejects the model-reawakening handlers an xo primary needs, so a
...
test_spawn_refuses_secondmate() {
  ...
  out=$(cd "$case_dir" && XO_ROOT_OVERRIDE='' XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_PROJECTS_OVERRIDE="$home/projects" XO_CONFIG_OVERRIDE="$home/config" \
    XO_SPAWN_NO_GUARD=1 TMUX="fake,1,0" META_API_KEY=test-key \
    PATH="$fakebin:$PATH" \
...
EOF
  ...
  prior=$(printf '%s\n' "$prior" | sed 's://*:/:g')
  XO_TEST_MUSE_DATA_HOME="$case_dir/xdgdata" \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode no-mistakes --yolo off >/dev/null \
...
make_send_case() {  # <name> <harness>
  ...
  home="$case_dir/home"
  fakebin=$(xo_fakebin "$case_dir/fake")
  id="send-$name"
  mkdir -p "$home/state"
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
...
case "${1:-}" in
  ...
  has-session) exit 0 ;;
  list-panes|list-windows) printf 'xo-send:0\n'; exit 0 ;;
  send-keys)
    shift
    printf '%s\n' "$*" >> "$XO_FAKE_KEY_LOG"
    [ "${XO_FAKE_KEY_FAIL:-}" = "$*" ] && exit 1
    exit 0
    ;;
esac
exit 0
SH
  ...
  xo_write_meta "$home/state/$id.meta" \
    "window=xo-send:0" "endpoint_task_id=$id" "worktree=$case_dir" \
...
run_send_key() {  # <home> <fakebin> <id> <key> <keylog>
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$1" XO_STATE_OVERRIDE="$1/state" \
    XO_FAKE_KEY_LOG="$5" PATH="$2:$PATH" \
    "$ROOT/bin/xo-send.sh" "$3" --key "$4" 2>&1
}
...
EOF
  keylog="$case_dir/keys.log"
  : > "$keylog"
  out=$(XO_FAKE_KEY_FAIL='-t xo-send:0 C-u' run_send_key "$home" "$fakebin" "$id" Escape "$keylog")
  status=$?
...
classify_muse() {  # <state-dir> <id>
  (
    ...
    xo_busy_classify tmux fake:0 muse "$2" "$1"
  )
}
...
test_muse_trusts_no_record_sources() {
  ...
  out=$(
    # shellcheck source=bin/xo-busy-lib.sh
    . "$ROOT/bin/xo-busy-lib.sh"
    xo_busy_sources_for_harness muse
  )
