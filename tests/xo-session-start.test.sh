#!/usr/bin/env bash
# tests/xo-session-start.test.sh - behavior tests for bin/xo-session-start.sh,
...
#     watcher ownership
#   - status-tail bounding, default and XO_SESSION_START_STATUS_TAIL override
...
#     tmux and herdr both
#   - composition: the script invokes the real xo-lock.sh/xo-bootstrap.sh/
#     xo-wake-drain.sh (their real, distinctive output appears verbatim), it
...
SESSION_START="$ROOT/bin/xo-session-start.sh"
BASE_PATH=${XO_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
TMP_ROOT=$(xo_test_tmproot xo-session-start-tests)
SESSION_START_TEST_HARNESS_PID=$$
SESSION_START_SECOND_MATE_ID="fmtest-sm-${TMP_ROOT##*.}"
SESSION_START_SECOND_MATE_TMP="/tmp/xo-$SESSION_START_SECOND_MATE_ID"
SESSION_START_HERDR_SECOND_MATE_ID="fmtest-herdr-${TMP_ROOT##*.}"
SESSION_START_HERDR_SECOND_MATE_TMP="/tmp/xo-$SESSION_START_HERDR_SECOND_MATE_ID"
XO_TEST_CLEANUP_DIRS+=("$TMP_ROOT" "$SESSION_START_SECOND_MATE_TMP" "$SESSION_START_HERDR_SECOND_MATE_TMP")
...
# xo repo) to use as XO_ROOT_OVERRIDE, plus an empty XO_HOME with
...
# make_fake_toolchain <fakebin>: every tool xo-bootstrap.sh detects, present
# and compatible, so its own detect-only section stays quiet except where a
# test deliberately breaks one. Mirrors xo-bootstrap.test.sh's fixture.
make_fake_toolchain() {
  local fakebin=$1
  xo_fake_exit0 "$fakebin" tmux node chrome-devtools-axi
  xo_fake_version_tool "$fakebin" lavish-axi XO_FAKE_LAVISH_AXI_VERSION 0.1.46
...
# or done rows. XO_FAKE_TASKS_AXI_READY sizes the ready set so the queued bound
...
#!/usr/bin/env bash
set -u
log=${XO_FAKE_TASKS_AXI_LOG:-}
[ -n "$log" ] && printf '%s\n' "$*" >> "$log"
ready_count=${XO_FAKE_TASKS_AXI_READY:-2}
require_file() {
...
case "${1:-}" in
  ...
  ready)
    ...
    while [ "$i" -le "$ready_count" ]; do
      printf '  ready-%s,queued,ship,xo,Ready item %s\n' "$i" "$i"
      i=$((i + 1))
    done
  ...
  list)
    ...
    case "$*" in
      ...
      *'--state in_flight'*)
        task_header 1
        printf '%s\n' '  compact-startup,in_flight,ship,xo,Compact startup digest,none,captain,captain choice pending'
        ;;
      *'--state held'*)
        task_header 1
        printf '%s\n' '  held-queued,queued,ship,xo,Held queued work,none,captain,captain choice pending'
        ;;
      *'--state queued'*'--blocked'*)
        task_header 1
        printf '%s\n' '  blocked-followup,queued,scout,xo,Follow compact startup,compact-startup,"-","-"'
        ;;
      *)
...
# make_fake_ps_claude <fakebin>: harness_pid()/holder_alive() (xo-lock.sh) walk
...
#!/usr/bin/env bash
set -u
harness=${XO_FAKE_HARNESS:-claude}
pid=
previous=
for argument in "$@"; do
...
case "$*" in
  *"comm="*)
    if [ -z "${XO_FAKE_HARNESS_PID:-}" ] || [ "$pid" = "$XO_FAKE_HARNESS_PID" ] \
      || [ "$pid" = "${XO_FAKE_LIVE_HOLDER_PID:-}" ]; then
...
# xo_backend_target_exists uses for a tmux endpoint liveness read.
make_fake_tmux() {
...
# XO_FAKE_TMUX_MODE selects missing, ambiguous, unreadable, or shell; missing
...
#!/usr/bin/env bash
set -u
mode=${XO_FAKE_TMUX_MODE:?}
log=${XO_FAKE_TMUX_LOG:?}
spawned=${XO_FAKE_TMUX_SPAWNED:?}
killed=${spawned}.killed
mate_home=${XO_FAKE_SECOND_MATE_HOME:?}
mate_id=${XO_FAKE_SECOND_MATE_ID:?}
mate_window="xo-$mate_id"
case "${1:-}" in
  display-message)
    target=
    format=
...
case "${1:-} ${2:-}" in
  ...
  "tab list")
    if [ -e "$spawned" ]; then
      printf '{"result":{"tabs":[{"tab_id":"t-focus","workspace_id":"ws1","label":"captain","focused":true},{"tab_id":"t-new","workspace_id":"ws1","label":"xo-%s","focused":false}]}}\n' "$mate_id"
    elif [ -e "$killed" ]; then
      printf '%s\n' '{"result":{"tabs":[{"tab_id":"t-focus","workspace_id":"ws1","label":"captain","focused":true}]}}'
    else
      printf '{"result":{"tabs":[{"tab_id":"t-focus","workspace_id":"ws1","label":"captain","focused":true},{"tab_id":"t-old","workspace_id":"ws1","label":"xo-%s","focused":false}]}}\n' "$mate_id"
    fi
    ;;
  "tab create")
    : > "$spawned"
...
# for the given pane id - the exact primitive xo_backend_target_exists uses
# for a herdr endpoint liveness read. No version/server-start calls: a
# liveness check must never auto-start a server (xo-backend.sh's contract).
make_fake_herdr() {
  local fakebin=$1 live=$2
  cat > "$fakebin/herdr" <<SH
#!/usr/bin/env bash
...
# Drop every harness env marker from bin/xo-harness.sh detect_own so the
# surrounding interactive shell cannot leak past the suite's fake ps harness.
# Markers today: CLAUDECODE (claude), PI_CODING_AGENT plus XO_PI_HARNESS
...
run_session_start() {
  ...
  if [ -n "$pi_harness" ]; then
    env -u CLAUDECODE -u GROK_AGENT PI_CODING_AGENT=true XO_PI_HARNESS="$pi_harness" \
      XO_HOME="$home" XO_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  else
    env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
      XO_HOME="$home" XO_ROOT_OVERRIDE="$root" PATH="$path" \
      "$SESSION_START"
  fi
}

run_pi_session_start() {  # <home> <root> <path> [xo-session-start args...]
  local home=$1 root=$2 path=$3
  shift 3
  env -u CLAUDECODE -u GROK_AGENT PI_CODING_AGENT=true XO_PI_HARNESS=pi \
    XO_FAKE_HARNESS_PID="$SESSION_START_TEST_HARNESS_PID" \
    XO_HOME="$home" XO_ROOT_OVERRIDE="$root" PATH="$path" \
    "$SESSION_START" "$@"
}

run_named_harness_session_start() {  # <harness> <home> <root> <path> [xo-session-start args...]
  local harness=$1 home=$2 root=$3 path=$4
  shift 4
  env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
    XO_FAKE_HARNESS="$harness" XO_FAKE_HARNESS_PID="$SESSION_START_TEST_HARNESS_PID" \
    XO_HOME="$home" XO_ROOT_OVERRIDE="$root" PATH="$path" \
    "$SESSION_START" "$@"
}
...
run_session_start_secondmate() {
  ...
  TMUX='' XO_BACKEND=tmux XO_FAKE_TMUX_MODE="$mode" XO_FAKE_TMUX_LOG="$log" \
    XO_FAKE_TMUX_SPAWNED="$spawned" XO_FAKE_SECOND_MATE_HOME="$mate" \
    XO_FAKE_SECOND_MATE_ID="$SESSION_START_SECOND_MATE_ID" \
    XO_FAKE_HARNESS_PID=$$ \
...
run_session_start_herdr_secondmate() {
  ...
  XO_BACKEND=herdr XO_FAKE_HERDR_LOG="$log" XO_FAKE_HERDR_STATE="$state" \
    XO_FAKE_SECOND_MATE_ID="$SESSION_START_HERDR_SECOND_MATE_ID" \
    XO_FAKE_HARNESS_PID=$$ \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH"
}
...
wait_for_network_stage() {
  ...
  XO_HOME="$home" XO_ROOT_OVERRIDE="$root" \
    "$ROOT/bin/xo-startup-network.sh" wait "$limit"
}
...
network_stage_report() {
  local home=$1 root=$2
  XO_HOME="$home" XO_ROOT_OVERRIDE="$root" "$ROOT/bin/xo-startup-network.sh" report
}
...
install_pi_turnend_extension_fixture() {
  ...
  cp "$ROOT/.pi/extensions/xo-primary-turnend-guard.ts" "$root/.pi/extensions/xo-primary-turnend-guard.ts"
}

install_pi_watch_extension_fixture() {
  local root=$1
  mkdir -p "$root/.pi/extensions"
  cp "$ROOT/.pi/extensions/xo-primary-pi-watch.ts" "$root/.pi/extensions/xo-primary-pi-watch.ts"
}

write_pi_watch_loaded_marker() {
  local home=$1 root=$2 pid=$3 version
  version=$(hash_file_for_test "$root/.pi/extensions/xo-primary-pi-watch.ts")
  printf '%s\n%s\n' "$version" "$pid" > "$home/state/.pi-watch-extension-loaded"
}

write_pi_turnend_loaded_marker() {
  local home=$1 root=$2 pid=$3 version
  version=$(hash_file_for_test "$root/.pi/extensions/xo-primary-turnend-guard.ts")
...
install_omp_extension_fixtures() {
  ...
  cp "$ROOT/.omp/extensions/xo-primary-omp-watch.ts" "$root/.omp/extensions/xo-primary-omp-watch.ts"
  cp "$ROOT/.omp/extensions/xo-primary-turnend-guard.ts" "$root/.omp/extensions/xo-primary-turnend-guard.ts"
}

write_omp_loaded_markers() {
  local home=$1 root=$2 pid=$3 version
  version=$(hash_file_for_test "$root/.omp/extensions/xo-primary-omp-watch.ts")
  printf '%s\n%s\n' "$version" "$pid" > "$home/state/.omp-watch-extension-loaded"
  version=$(hash_file_for_test "$root/.omp/extensions/xo-primary-turnend-guard.ts")
...
EOF
  ...
  # XO_BOOTSTRAP_DETECT_ONLY=1 actually suppressed the mutating sweep.
  mkdir -p "$home/other-secondmate/state"
  xo_write_secondmate_meta "$home/state/sm-x.meta" "$home/other-secondmate" "xo:xo-sm-x" alpha
  append_wake "$home/state" signal sm-x "done: surfaced before refusal" || fail "seed wake failed"
  git -C "$root" checkout -q -B xo/read-only-tangle
  ...
  expect_code 0 "$status" "xo-session-start.sh must exit 0 even on a lock refusal"
  assert_contains "$out" "READ-ONLY SESSION" "read-only banner missing on lock refusal"
  assert_contains "$out" "another live xo session holds the lock" "read-only banner did not surface xo-lock.sh's own error text"
  ...
  assert_contains "$out" "TANGLE: primary checkout on feature branch 'xo/read-only-tangle'" "read-only bootstrap did not surface the tangle diagnostic"
  ...
  assert_not_contains "$out" "drain them with bin/xo-wake-drain.sh" "read-only guard printed a mutating drain instruction"
  assert_not_contains "$out" "After draining queued wakes" "read-only guard printed a drain-then-rearm instruction"
  assert_not_contains "$out" "run bin/xo-watch-arm.sh" "read-only guard printed a mutating watcher-arm instruction"
  ...
  # untouched (xo-ff-lib would have tried to fast-forward it otherwise).
...
EOF
  ...
  expect_code 0 "$status" "xo-session-start.sh must exit 0 when lock publication fails"
  ...
  assert_not_contains "$out" "ANOTHER LIVE XO SESSION HOLDS THE FLEET LOCK" "lock publication failure falsely claimed a live lock holder"
...
EOF
  ...
  XO_TRACE_CONTEXT=off run_session_start "$home" "$root" "$fakebin:$BASE_PATH" >/dev/null
  ...
  XO_TRACE_CONTEXT=on run_session_start "$home" "$root" "$fakebin:$BASE_PATH" >/dev/null
  ...
  holder_pid=$!
  printf '%s\n' "$holder_pid" > "$home/state/.lock"
  out=$(XO_TRACE_CONTEXT=off run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  kill "$holder_pid" 2>/dev/null || true
  wait "$holder_pid" 2>/dev/null || true
...
case "$*" in
  *"comm="*)
    if [ -f "$XO_FAKE_LOCK_STATE/harness-$pid" ]; then
...
SH
  ...
  while [ "$i" -le 40 ]; do
    (
      ...
      if XO_HOME="$home" XO_FAKE_LOCK_STATE="$home/state" \
        XO_FAKE_HARNESS_PID="$harness_pid" PATH="$fakebin:$BASE_PATH" \
        "$ROOT/bin/xo-lock.sh" >/dev/null 2>&1; then
        printf '%s\n' "$harness_pid" >> "$winners"
      fi
      : > "$completed/$i"
...
EOF
  ...
  printf 'window=xo-sess:w1\nkind=ship\n' > "$home/state/task-a.meta"
  printf 'Captain memory that may be truncated away safely.\n' > "$home/data/captain.md"

  out=$(run_session_start "$home" "$root" "$fakebin:$(xo_test_base_path_sans "$BASE_PATH" node)")
...
EOF
    ...
    make_fake_ps_claude "$fakebin"
    rm -f "$fakebin/tmux"
    xo_fake_exit0 "$fakebin" herdr jq
    printf '%s\n' manual > "$home/config/backlog-backend"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  make_fake_tmux "$fakebin" "xo-sess:live"

  printf 'window=xo-sess:live\nkind=ship\n' > "$home/state/task-a.meta"
  ...
  out=$(XO_SESSION_START_STATUS_TAIL=2 run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  assert_contains "$out" "working: step 7" "XO_SESSION_START_STATUS_TAIL=2 tail missing the most recent line"
  assert_not_contains "$out" "working: step 5" "XO_SESSION_START_STATUS_TAIL=2 did not bound the tail to 2 lines"
...
EOF
  ...
  assert_contains "$out" "endpoint: alive (backend=tmux window=xo:xo-$SESSION_START_SECOND_MATE_ID)" \
...
EOF
  ...
  assert_contains "$out" "endpoint: dead (backend=tmux window=xo:xo-$SESSION_START_SECOND_MATE_ID)" \
...
EOF
  ...
  assert_contains "$(cat "$log")" "kill-window -t =xo:=xo-$SESSION_START_SECOND_MATE_ID" \
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  make_fake_tmux "$fakebin" "xo-sess:live-window"

  printf 'window=xo-sess:live-window\nkind=ship\n' > "$home/state/task-live.meta"
  printf 'window=xo-sess:dead-window\nkind=ship\n' > "$home/state/task-dead.meta"

  out=$(run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  assert_contains "$out" "endpoint: alive (backend=tmux window=xo-sess:live-window)" "live tmux endpoint not reported alive"
  assert_contains "$out" "endpoint: dead (backend=tmux window=xo-sess:dead-window)" "dead tmux endpoint not reported dead"
...
EOF
  ...
  # xo-lock.sh's own exact success text.
  assert_contains "$out" "lock acquired: harness pid" "xo-lock.sh's real output did not appear (composition, not reimplementation)"
  # xo-bootstrap.sh's own exact MISSING-tool line format.
  assert_contains "$out" "MISSING: node (install:" "xo-bootstrap.sh's real detect line did not appear verbatim"
  # xo-wake-drain.sh's real drained record (raw tab-separated queue line).
  assert_contains "$out" "$(printf 'signal\ttask-z.status\tneeds-decision: pick a library')" "xo-wake-drain.sh's real drained record did not appear"
  assert_contains "$out" "wake annotation: latest wake-EVENT observed at drain, not current state: task-z.status: needs-decision: pick a library" "xo-session-start.sh did not preserve the drain's separate annotation line"

  pass "xo-session-start.sh composes the real xo-lock.sh, xo-bootstrap.sh, and xo-wake-drain.sh output verbatim"
}
...
EOF
  ...
  XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" append \
  ...
  XO_HOME="$home" XO_SUPERVISION_ACTOR=branch XO_LEASE_HOLDER_PID=$$ "$ROOT/bin/xo-lease.sh" claim task-live --actor branch \
  ...
  assert_contains "$(XO_HOME="$home" "$ROOT/bin/xo-branch-outcome.sh" unread)" \
...
if [ "${1:-} ${2:-}" = 'axi status' ]; then
  if [ "${XO_BOOTSTRAP_NETWORK:-}" = only ]; then
    printf '%s\n' 'deferred' >> "${XO_FAKE_NM_CALLS:?}"
  else
    printf '%s\n' 'blocking' >> "${XO_FAKE_NM_CALLS:?}"
  fi
  ...
  ticks=0
  while [ ! -e "${XO_FAKE_NM_RELEASE:?}" ] && [ "$ticks" -lt 300 ]; do
    sleep 0.1
    ticks=$((ticks + 1))
  done
  : > "${XO_FAKE_NM_READ_FINISHED:?}"
...
SH
  ...
  xo_write_meta "$home/state/slow-child.meta" \
    'window=xo:xo-slow-child' "worktree=$worktree" 'project=xo' \
  ...
  out=$(XO_BACKEND=tmux XO_FAKE_HARNESS_PID="$SESSION_START_TEST_HARNESS_PID" \
    XO_FAKE_NM_CALLS="$calls" XO_FAKE_NM_RELEASE="$release_gate" \
    XO_FAKE_NM_READ_FINISHED="$read_finished" XO_INACTIVE_RECONCILE_SECS=60 \
    XO_INACTIVE_RECONCILE_BUDGET_SECS=30 XO_INACTIVE_CREW_STATE_BIN="$crew_state" \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
...
# is asserted deterministically in tests/xo-startup-network.test.sh, where the
...
EOF
  ...
  XO_FAKE_TASKS_AXI_LOG="$log" run_session_start "$home" "$root" "$fakebin:$BASE_PATH" >/dev/null
...
## In flight
- [ ] compact-startup - Compact startup digest (repo: xo) (kind: ship) (since 2026-07-15) (hold: captain choice pending) (hold-kind: captain)
...
## Queued
- [ ] blocked-followup - Follow compact startup blocked-by: compact-startup - waits for implementation (repo: xo) (kind: scout) (since 2026-07-15)
  QUEUED-BODY-LINE this is another long multiline note.
- [ ] held-queued - Held queued work (repo: xo) (kind: ship) (hold: captain choice pending) (hold-kind: captain)
EOF
  while [ "$i" -le 25 ]; do
    printf -- '- [ ] plain-%s - Plain queued item %s (repo: xo) (kind: ship)\n' "$i" "$i" >> "$path"
    i=$((i + 1))
  done
  cat >> "$path" <<'EOF'

## Done
- [x] landed-earlier - DONE-ROW-LINE already landed and torn down (repo: xo) (kind: ship)
EOF
}
...
EOF
  ...
  mkdir -p "$home/projects/xo"
  printf 'window=xo-sess:compact\nworktree=%s\nproject=xo\nkind=ship\n' "$home/projects/xo" \
    > "$home/state/compact-startup.meta"
  log="$home/tasks-axi.log"

  out=$(XO_FAKE_TASKS_AXI_LOG="$log" XO_FAKE_TASKS_AXI_READY=3 \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  assert_contains "$out" "compact-startup,in_flight,ship,xo,Compact startup digest,none,captain,captain choice pending" \
    "tasks-axi compact listing omitted in-flight identity, state, or hold metadata"
  assert_contains "$out" "held-queued,queued,ship,xo,Held queued work,none,captain,captain choice pending" \
    "tasks-axi compact listing omitted a held row or its hold metadata"
  assert_contains "$out" 'blocked-followup,queued,scout,xo,Follow compact startup,compact-startup,"-","-"' \
    "tasks-axi compact listing omitted blocked-by metadata"
  assert_contains "$out" "ready-3,queued,ship,xo,Ready item 3" \
  ...
  assert_contains "$out" "worktree=$home/projects/xo" "in-flight recovery worktree identity disappeared from startup digest"
...
EOF
  ...
  out=$(XO_FAKE_TASKS_AXI_READY=7 XO_SESSION_START_QUEUED_LIMIT=3 \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
...
EOF
  ...
  out=$(XO_SESSION_START_QUEUED_LIMIT=4 run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  assert_contains "$out" "(21 more queued - raise XO_SESSION_START_QUEUED_LIMIT or read data/backlog.md for the rest)" \
...
EOF
  ...
  mechanism=$(XO_TIMEOUT_MECHANISM_OVERRIDE=bash bash -c '. "$1"; xo_timeout_mechanism' \
    _ "$ROOT/bin/xo-timeout-lib.sh")
  [ "$mechanism" = bash ] || fail "the forced pure-Bash timeout fixture selected '$mechanism'"

  out=$(XO_TIMEOUT_MECHANISM_OVERRIDE=bash XO_SESSION_START_TIMEOUT=3 XO_STARTUP_NETWORK_TIMEOUT=2 \
  ...
  XO_HOME="$home" XO_ROOT_OVERRIDE="$root" XO_STARTUP_NETWORK_TIMEOUT=2 \
    "$ROOT/bin/xo-startup-network.sh" wait 30 >/dev/null || true
  sleep 1
  stray=$(pgrep -f "$fakebin/git" 2>/dev/null | wc -l | tr -d ' ')
  [ "$stray" -eq 0 ] || fail "the runtime bound left $stray hung subprocess(es) behind"

  status=0
  XO_TIMEOUT_MECHANISM_OVERRIDE=bash bash -c \
    '. "$1"; xo_run_timed 2 bash -c "exit 137"' _ "$ROOT/bin/xo-timeout-lib.sh" || status=$?
...
SH
  ...
  perl -e '
    ...
    waitpid $pid, 0;
    exit($? >> 8);
  ' env PATH="$fakebin:$BASE_PATH" "$driver" "$ROOT/bin/xo-timeout-lib.sh" \
...
EOF
  ...
  assert_absent "${TMPDIR:-/tmp}/xo-session-start-stage" "the stage breadcrumb leaked a fixed-name file"
...
EOF
  ...
  # xo-session-lock-lib.sh walks a BOUNDED sixteen parents to find it, and the
...
case "$*" in
  *"comm="*)
    if [ "$pid" = "${XO_FAKE_HARNESS_PID:-}" ]; then printf '%s\n' /usr/local/bin/claude
    else printf '%s\n' /bin/bash; fi
    ;;
  *"args="*)
    if [ "$pid" = "${XO_FAKE_HARNESS_PID:-}" ]; then printf '%s\n' claude
...
SH
  ...
  out=$(env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
    XO_HOME="$home" XO_ROOT_OVERRIDE="$root" PATH="$fakebin:$BASE_PATH" \
    bash -c 'export XO_FAKE_HARNESS_PID=$$; exec "$1" 8 "$2"' _ "$nest" "$SESSION_START")
...
EOF
  ...
  make_fake_ps_claude "$fakebin"
  mkdir -p "$home/other-secondmate/state"
  xo_write_secondmate_meta "$home/state/sm-r.meta" "$home/other-secondmate" "xo:xo-sm-r" alpha
  ...
  XO_FAKE_HARNESS_PID=$$ run_session_start "$home" "$root" "$fakebin:$BASE_PATH" >/dev/null
  wait_for_network_stage "$home" "$root" \
  ...
  reemit=$(XO_HOME="$home" XO_ROOT_OVERRIDE="$root" XO_FAKE_HARNESS_PID=$$ PATH="$fakebin:$BASE_PATH" \
    env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
  ...
  XO_STATE_OVERRIDE="$home/state" "$ROOT/bin/xo-wake-drain.sh" --ack-through "$sequence" \
...
XO_TEST_INSTRUCTION=original
Keep this original instruction.
EOF

  startup=$(XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --source startup)
  ...
  compact_equal=$(XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --reemit --source compact)
  ...
  cat > "$root/AGENTS.md" <<'EOF'
XO_TEST_INSTRUCTION=updated
The complete updated instruction must survive every stale rebuild.
EOF
  resume_out=$(XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --source resume)
  ...
  compact_first=$(XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --reemit --source compact)
  ...
  assert_contains "$compact_first" "XO_TEST_INSTRUCTION=updated" \
  ...
  compact_second=$(XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --reemit --source compact)
  assert_contains "$compact_second" "XO_TEST_INSTRUCTION=updated" \
  ...
  clear_out=$(XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --reemit --source clear)
  ...
  reset_out=$(XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --source reset)
...
EOF
  ...
  XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --source startup >/dev/null
  ...
  out=$(XO_FAKE_HARNESS=pi XO_FAKE_LIVE_HOLDER_PID="$holder_pid" \
...
EOF
  ...
  compact_out=$(XO_FAKE_HARNESS=pi run_pi_session_start "$home" "$root" "$fakebin:$BASE_PATH" --reemit --source compact)
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  git -C "$root" checkout -q -B xo/reemit-tangle

  reemit=$(XO_HOME="$home" XO_ROOT_OVERRIDE="$root" PATH="$fakebin:$BASE_PATH" \
    env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
    "$SESSION_START" --reemit)
  ...
  holder_pid=$!
  printf '%s\n' "$holder_pid" > "$home/state/.lock"
  readonly_out=$(XO_HOME="$home" XO_ROOT_OVERRIDE="$root" PATH="$fakebin:$BASE_PATH" \
    env -u CLAUDECODE -u PI_CODING_AGENT -u XO_PI_HARNESS -u GROK_AGENT \
    "$SESSION_START" --reemit)
  kill "$holder_pid" 2>/dev/null || true
  wait "$holder_pid" 2>/dev/null || true
...
EOF
  ...
  assert_not_contains "$out" "  bin/xo-watch-arm.sh" "AFK next step still told the agent to arm the watcher directly"
...
EOF
  ...
  out=$(XO_FAKE_HARNESS=pi run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  assert_contains "$out" "restart plain pi so $root/.pi/extensions/xo-primary-turnend-guard.ts and $root/.pi/extensions/xo-primary-pi-watch.ts auto-load" "pi extension load diagnostic omits the turn-end guard extension"
...
EOF
  ...
  out=$(XO_FAKE_HARNESS=pi-signed run_session_start "$home" "$root" "$fakebin:$BASE_PATH" pi-signed)
  ...
  assert_contains "$out" "restart pi-signed so $root/.pi/extensions/xo-primary-turnend-guard.ts and $root/.pi/extensions/xo-primary-pi-watch.ts auto-load" \
...
EOF
  ...
  out=$(XO_FAKE_HARNESS=omp run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  assert_contains "$out" "so $root/.omp/extensions/xo-primary-turnend-guard.ts and $root/.omp/extensions/xo-primary-omp-watch.ts auto-load" "omp diagnostic omits the two tracked extension paths"
...
echo "# xo-session-start.test.sh: all assertions passed"
