...
# config/ (xo-gitignore-config-name-by-name): an unrecognized file there makes
...
test_scratchpad2_does_not_dirty_porcelain() {
  ...
  local repo status
  repo=$(mktemp -d "${TMPDIR:-/tmp}/xo-scratchpad-ignore.XXXXXX")
  git init -q "$repo"
  cp "$ROOT/.gitignore" "$repo/.gitignore"
  git -C "$repo" add .gitignore
  git -C "$repo" -c user.name='XO Tests' -c user.email='tests@example.invalid' \
