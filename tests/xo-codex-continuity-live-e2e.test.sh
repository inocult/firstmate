...
xo_live_gate opt-in XO_CODEX_LIVE_E2E codex
...
PROMPT='Run exactly `bin/xo-watch-checkpoint.sh --seconds 1` as one foreground shell call. Do not use a background task and do not run xo-watch-arm.sh. After the checkpoint returns, reply briefly.'

(
  cd "$PROJECT" || exit 1
  printf '%s\n' "$$" > "$HOME_DIR/state/.lock"
  XO_HOME="$HOME_DIR" XO_ROOT_OVERRIDE="$PROJECT" codex exec \
