...
# cleanup ever touching the machine's real "xo" session (the default
# session name bin/backends/zellij.sh uses for actual xo task tabs) or
...
# "xo" session name, or a caller reaching for the fleet-wide
...
# explicitly, that name is NOT "xo" (the real default), and it is
...
# is NOT the literal "xo" default session name, and IS currently
...
zellij_refuse_if_unsafe() {  # <name>
  ...
  if [ "$name" = xo ]; then
    echo "zellij safety guard: refusing - name is literally 'xo' (the real default session a live fleet may use)" >&2
    return 1
  fi
...
# never-"xo" session running rather than risk the wrong target.
