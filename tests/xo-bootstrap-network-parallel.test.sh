...
# suite drives bin/xo-bootstrap.sh's network-only phase through XO_SSH_BIN, so
...
TMP_ROOT=$(xo_test_tmproot xo-bootstrap-network-parallel)
...
command -v python3 >/dev/null 2>&1 \
  || fail "python3 is required to decode the xo-on.sh argv payload"
...
[ -z "${XO_TEST_EVIDENCE_FILE:-}" ] || : > "$XO_TEST_EVIDENCE_FILE"
...
case "$host" in
  "${XO_FAKE_SSH_UNREACHABLE_HOST:-host-bravo}")
    exit 255
    ;;
esac
case "$command_name" in
  ...
  xo-remote-secondmate-control.sh)
    case "$subcommand" in
      ...
      sync)
        case "$host" in
          "${XO_FAKE_SSH_FAIL_HOST:-host-alpha}")
            printf 'synthetic tracked-file sync refusal for %s\n' "$host"
            exit 1
            ;;
          "${XO_FAKE_SSH_DIRTY_HOST:-host-charlie}")
...
if [ "\$slow" -eq 1 ]; then
  ...
  waited=0
  while ! grep -q '^START host-.* xo-remote-doctor.sh ' '$log' && [ "\$waited" -lt 500 ]; do
    sleep 0.01
    waited=\$((waited + 1))
  done
  sleep "\${XO_FAKE_GIT_FETCH_SLEEP:-0.4}"
...
#!/usr/bin/env bash
case "\$*" in *xo-bootstrap-par.XXXXXX*) exit 1 ;; esac
exec '$REAL_MKTEMP' "\$@"
SH
  ...
  xo_write_secondmate_meta "$home/state/alpha.meta" "$alpha_home"
  printf 'remote_host=host-alpha\n' >> "$home/state/alpha.meta"
  xo_write_secondmate_meta "$home/state/bravo.meta" "$bravo_home"
  printf 'remote_host=host-bravo\n' >> "$home/state/bravo.meta"
  xo_write_secondmate_meta "$home/state/charlie.meta" "$charlie_home"
  printf 'remote_host=host-charlie\n' >> "$home/state/charlie.meta"

  xo_git_init_commit "$home/projects/alpha"
  xo_git_add_origin "$home/projects/alpha" "$dir/alpha.origin.git"

  out=$(
    PATH="$fakebin:$BASE_PATH" \
    XO_HOME="$home" \
    XO_ROOT_OVERRIDE="$primary" \
    XO_BOOTSTRAP_NETWORK=only \
    XO_SSH_BIN="$fakebin/fake-ssh" \
    XO_FAKE_SSH_LOG="$log" \
    XO_FAKE_SSH_SLEEP=0.4 \
    XO_FAKE_SSH_UNREACHABLE_HOST=host-bravo \
    XO_FAKE_SSH_FAIL_HOST=host-alpha \
    XO_FAKE_SSH_DIRTY_HOST=host-charlie \
    XO_FAKE_GIT_FETCH_SLEEP=0.4 \
    XO_INHERITABLE_CONFIG='' \
...
EOF
  ...
  doctor_overlap=$(starts_before_first_end "$log" 'xo-remote-doctor.sh')
  ...
  liveness_starts=$(grep -c '^START .* xo-remote-secondmate-control.sh state$' "$log" || true)
...
echo "# all xo-bootstrap-network-parallel tests passed"
