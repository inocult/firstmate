#!/usr/bin/env bash
# Behavior tests for Cursor Agent CLI as an xo PRIMARY
...
# path in bin/xo-session-lock-lib.sh is exercised rather than stubbed.
# tests/xo-cursor-primary-live-e2e.test.sh is the opt-in guard against a real
# cursor-agent. Neither replaces the other.
# shellcheck disable=SC2016 # single quotes are deliberate: $XO_HOME expands inside the fake harness child
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(xo_test_tmproot xo-cursor-primary)
xo_git_identity fmtest fmtest@example.invalid

FAKEBIN=$(xo_fakebin "$TMP_ROOT/fakebin")
...
install_scripts() {
  ...
  for f in xo-turnend-guard-cursor.sh xo-turnend-guard.sh xo-sessionstart-cursor.sh \
           xo-sessionstart-run.sh xo-sessionstart-nudge.sh xo-arm-pretool-check.sh \
           xo-cd-pretool-check.sh xo-claude-stop-autoarm.sh xo-hook-host-lib.sh \
           xo-primary-scope-lib.sh xo-supervision-lib.sh xo-wake-lib.sh \
           xo-session-lock-lib.sh xo-cursor-lib.sh xo-operational-input.sh \
           xo-supervision-instructions.sh xo-harness.sh xo-lock.sh \
           xo-gate-refuse-lib.sh; do
    cp "$ROOT/bin/$f" "$dir/bin/$f"
  done
  cp "$ROOT/bin/xo-arm-command-policy.mjs" "$dir/bin/xo-arm-command-policy.mjs"
  cp "$ROOT/bin/xo-cd-command-policy.mjs" "$dir/bin/xo-cd-command-policy.mjs"
...
# An arm fixture standing in for bin/xo-watch-arm.sh. Real process, real output.
...
#!/usr/bin/env bash
printf '%s\n' "$$" >> "$XO_HOME/state/arm-ran"
if [ -e "$XO_HOME/state/arm-fast" ]; then
...
PARK_CHILD='
  printf "%s\n" "$$" > "$XO_HOME/state/.lock"
  "$XO_HOME/bin/xo-turnend-guard-cursor.sh"
'
...
run_park() {  # <dir> [loop_count] [loop_ceiling]
  ...
  if [ -n "$ceiling" ]; then
    printf '%s' "$payload" | env -u PI_CODING_AGENT XO_HOME="$dir" XO_CURSOR_PARK_POLL=1 \
      XO_CURSOR_TURNEND_LOOP_CEILING="$ceiling" "$FAKE_CURSOR" -c "$PARK_CHILD" 2>/dev/null
  else
    printf '%s' "$payload" | env -u PI_CODING_AGENT XO_HOME="$dir" XO_CURSOR_PARK_POLL=1 \
...
run_session() {  # <dir> <event> <source> [session-id]
  ...
  printf '%s' "$payload" | XO_HOME="$dir" XO_SESSION_SOURCE="$source" "$FAKE_CURSOR" -c '
    printf "%s\n" "$$" > "$XO_HOME/state/.lock"
    "$XO_HOME/bin/xo-sessionstart-cursor.sh" --source "$XO_SESSION_SOURCE"
  ' 2>/dev/null
}
...
kind_of_followup() {  # <json> -> the operational kind
  ...
  body=$(followup_of "$1")
  [ -n "$body" ] || return 1
  printf '%s' "$body" | "$ROOT/bin/xo-operational-input.sh" kind
}
...
test_turnend_guard_stands_down_on_cursor_payload() {
  ...
  out=$(printf '%s' "$CURSOR_PAYLOAD" | bash "$dir/bin/xo-turnend-guard.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' "$CURSOR_PAYLOAD" | bash "$dir/bin/xo-turnend-guard.sh" --cursor 2>&1); status=$?
  ...
  pass "xo-turnend-guard: Cursor payload is inert without --cursor and blocks with it"
}
...
test_turnend_guard_still_blocks_for_claude_payload() {
  ...
  : > "$dir/state/task1.meta"
  printf '%s' "$CLAUDE_STOP_PAYLOAD" | bash "$dir/bin/xo-turnend-guard.sh" >/dev/null 2>&1
  status=$?
  expect_code 2 "$status" "the host guard must not disturb a genuine Claude Stop payload"
  pass "xo-turnend-guard: a non-Cursor payload keeps blocking"
}
...
test_autoarm_stands_down_on_cursor_payload() {
  ...
  printf '%s' "$CURSOR_PAYLOAD" | XO_HOME="$dir" "$FAKE_CURSOR" -c '
      printf "%s\n" "$$" > "$XO_HOME/state/.lock"
      exec "$XO_HOME/bin/xo-claude-stop-autoarm.sh"
    ' >/dev/null 2>&1
  status=$?
  ...
  pass "xo-claude-stop-autoarm: inert on a Cursor-delivered payload"
}

test_sessionstart_run_stands_down_on_cursor_payload() {
  local dir out
  dir=$(make_primary_dir "$TMP_ROOT/host-sessionstart")
  cat > "$dir/bin/xo-session-start.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$$" >> "$XO_HOME/state/digest-ran"
printf 'DIGEST BODY\n'
SH
  chmod +x "$dir/bin/xo-session-start.sh"
  out=$(printf '%s' "$CURSOR_PAYLOAD" | XO_HOME="$dir" bash "$dir/bin/xo-sessionstart-run.sh" 2>&1)
  ...
  out=$(printf '{"source":"startup","session_id":"s"}' | XO_HOME="$dir" bash "$dir/bin/xo-sessionstart-run.sh" 2>&1)
  case "$out" in *'DIGEST BODY'*) ;; *) fail "a Claude-shaped payload must still run the digest, got: $out" ;; esac
  pass "xo-sessionstart-run: inert on a Cursor payload, unchanged otherwise"
}
...
test_pretool_guards_deduplicate_and_render_cursor_deny() {
  ...
  payload='{"tool_name":"Shell","tool_input":{"command":"bin/xo-watch-arm.sh &"},"cursor_version":"2026.08.11-e8db854"}'
  out=$(printf '%s' "$payload" | bash "$dir/bin/xo-arm-pretool-check.sh" 2>&1); status=$?
  ...
  out=$(printf '%s' "$payload" | bash "$dir/bin/xo-arm-pretool-check.sh" --cursor 2>/dev/null); status=$?
  ...
  pass "xo-arm-pretool-check: Cursor duplicate allows, --cursor denies in Cursor's own shape"
}
...
test_cd_guard_renders_cursor_deny() {
  ...
  out=$(printf '%s' "$payload" | XO_HOME="$dir" bash "$dir/bin/xo-cd-pretool-check.sh" --cursor 2>/dev/null)
  decision=$(printf '%s' "$out" | jq -r '.permission // empty' 2>/dev/null)
  [ "$decision" = deny ] || fail "expected a Cursor deny object from the cd guard, got: $out"
  out=$(printf '%s' "$payload" | XO_HOME="$dir" bash "$dir/bin/xo-cd-pretool-check.sh" 2>&1)
  [ -z "$out" ] || fail "the cd guard's Claude-settings duplicate produced output under Cursor: $out"
  pass "xo-cd-pretool-check: Cursor duplicate allows, --cursor denies in Cursor's own shape"
}
...
test_park_delivers_actionable_wake_as_followup() {
  ...
  case "$body" in *'xo-wake-drain.sh'*) ;; *) fail "the follow-up must tell the session to drain first: $body" ;; esac
...
test_park_serializes_supersession_with_followup_commit() {
  ...
  write_arm_fixture "$dir" actionable
  cat >> "$dir/bin/xo-operational-input.sh" <<'SH'
xo_operational_input_encode() {
  local kind=${1-} body=${2-} result_var=${3-}
  [ -n "$result_var" ] && xo_operational_kind_is_current "$kind" && [ -n "$body" ] || return 2
  if ( set -C; : > "$XO_HOME/state/commit-entered" ) 2>/dev/null; then
    while [ ! -e "$XO_HOME/state/commit-release" ]; do sleep 0.05; done
  fi
...
test_superseded_park_does_not_consume_nag_budget() {
  ...
  : > "$dir/state/task1.meta"
  write_arm_fixture "$dir" failed
  cat > "$dir/bin/xo-turnend-guard.sh" <<'SH'
#!/usr/bin/env bash
if ( set -C; : > "$XO_HOME/state/first-guard-entered" ) 2>/dev/null; then
  while [ ! -e "$XO_HOME/state/first-guard-release" ]; do sleep 0.05; done
fi
...
test_park_inert_under_pi_coding_agent() {
  ...
  out=$(printf '%s' "$payload" | env -u CURSOR_AGENT -u CURSOR_INVOKED_AS \
    XO_HOME="$dir" PI_CODING_AGENT=true XO_CURSOR_PARK_POLL=1 \
    "$FAKE_CURSOR" -c "$PARK_CHILD" 2>/dev/null)
...
test_park_still_parks_with_pi_leak_and_cursor_identity() {
  ...
  out=$(printf '%s' "$payload" | env -u CURSOR_INVOKED_AS \
    XO_HOME="$dir" PI_CODING_AGENT=true CURSOR_AGENT=1 XO_CURSOR_PARK_POLL=1 \
    "$FAKE_CURSOR" -c "$PARK_CHILD" 2>/dev/null)
  ...
  out=$(printf '%s' "$payload" | env -u CURSOR_AGENT \
    XO_HOME="$dir" PI_CODING_AGENT=true CURSOR_INVOKED_AS=cursor-agent XO_CURSOR_PARK_POLL=1 \
    "$FAKE_CURSOR" -c "$PARK_CHILD" 2>/dev/null)
...
xo_operational_input_encode() {
  local kind=${1-} body=${2-} result_var=${3-}
  [ -n "$result_var" ] && xo_operational_kind_is_current "$kind" && [ -n "$body" ] || return 2
  : > "$XO_HOME/state/afk-commit-entered"
  while [ ! -e "$XO_HOME/state/afk-commit-release" ]; do sleep 0.05; done
  printf -v "$result_var" '%s%s: %s' "$XO_OPERATIONAL_HEADER_PREFIX" "$kind" "$body"
}
SH
...
test_park_inert_without_session_lock() {
  ...
  : > "$dir/state/task1.meta"
  write_arm_fixture "$dir" actionable
  out=$(printf '%s' "$CURSOR_PAYLOAD" | env -u PI_CODING_AGENT XO_HOME="$dir" bash "$dir/bin/xo-turnend-guard-cursor.sh" 2>/dev/null)
...
test_park_inert_in_child_worktree() {
  ...
  xo_git_worktree "$base" "$child" xo/cursor-park-child
  mkdir -p "$child/state"
  : > "$child/AGENTS.md"
...
test_park_ignores_malformed_payload() {
  ...
  : > "$dir/state/task1.meta"
  write_arm_fixture "$dir" actionable
  out=$(printf 'not json at all' | env -u PI_CODING_AGENT XO_HOME="$dir" bash "$dir/bin/xo-turnend-guard-cursor.sh" 2>/dev/null)
  [ -z "$out" ] || fail "a malformed payload must fail open, got: $out"
  out=$(printf '{"loop_count":"three","cursor_version":"x"}' | env -u PI_CODING_AGENT XO_HOME="$dir" bash "$dir/bin/xo-turnend-guard-cursor.sh" 2>/dev/null)
...
install_digest_fixture() {  # <dir>
  cat > "$1/bin/xo-session-start.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$XO_HOME/state/digest-args"
printf 'XO DIGEST "quoted" line\nsecond line\n'
SH
  chmod +x "$1/bin/xo-session-start.sh"
}
...
test_sessionstart_emits_additional_context() {
  ...
  case "$ctx" in *'XO DIGEST "quoted" line'*) ;; *) fail "the digest must reach model context verbatim, got: $out" ;; esac
  ...
  pass "xo-sessionstart-cursor: sessionStart injects context"
}
...
test_sessionstart_silent_in_child_worktree() {
  ...
  xo_git_worktree "$base" "$child" xo/cursor-session-child
  mkdir -p "$child/state"
  : > "$child/AGENTS.md"
  install_scripts "$child"
  install_digest_fixture "$child"
  out=$(printf '{"hook_event_name":"sessionStart","cursor_version":"x"}' \
    | XO_HOME="$child" bash "$child/bin/xo-sessionstart-cursor.sh" --source startup 2>/dev/null)
  [ -z "$out" ] || fail "a child worktree must never take the helm: $out"
  pass "xo-sessionstart-cursor: silent inside a child crewmate worktree"
}
...
test_tracked_registration_covers_the_primary_events() {
  ...
  [ -f "$reg" ] || fail "xo must ship a tracked project-scope .cursor/hooks.json"
  ...
  jq -e '[.hooks.sessionStart[]] | all(.timeout > 120)' "$reg" >/dev/null 2>&1 \
    || fail "the session-open timeout must sit above bin/xo-session-start.sh's own 120s budget"
...
# adapter at Cursor's own registered limit with its DEFAULT ceiling: xo's
...
test_default_ceiling_bites_before_the_registered_loop_limit() {
  ...
  pass "cursor bounds nest: xo's default ceiling stops the loop before Cursor's loop_limit does"
}
