#!/usr/bin/env bash
# Behavior tests for bin/xo-claude-trust.sh and the claude spawn that calls it.
#
...
TMP_ROOT=$(xo_test_tmproot xo-claude-trust)

TRUST="$ROOT/bin/xo-claude-trust.sh"
...
test_fresh_worktree_is_trusted() {
  ...
  [ -z "$(find "$CONFIG" -maxdepth 1 -name '.claude.json.xo-trust.*' -print -quit)" ] \
    || fail "a temporary store file was left behind in the config directory"
  pass "xo-claude-trust.sh: a fresh task worktree is trusted"
}
...
test_registration_is_idempotent() {
  ...
  pass "xo-claude-trust.sh: repeat registration is idempotent"
}
...
test_primary_checkout_is_refused() {
  ...
  pass "xo-claude-trust.sh: refuses the primary checkout"
}
...
test_cdpath_cannot_defeat_the_primary_checkout_refusal() {
  ...
  pass "xo-claude-trust.sh: an exported CDPATH cannot defeat the scope refusal"
}
...
test_git_env_overrides_cannot_defeat_the_primary_checkout_refusal() {
  ...
  pass "xo-claude-trust.sh: inherited git environment overrides cannot defeat the scope refusal"
}
...
test_home_directory_is_refused_even_when_it_is_a_worktree() {
  ...
  pass "xo-claude-trust.sh: refuses a home directory the git checks would accept"
}

# xo-spawn forwards CLAUDE_CONFIG_DIR onto the worker verbatim and the worker's
...
test_relative_config_dir_is_refused() {
  ...
  pass "xo-claude-trust.sh: refuses a relative CLAUDE_CONFIG_DIR"
}
...
test_config_directory_is_refused() {
  ...
  pass "xo-claude-trust.sh: refuses the Claude config directory"
}
...
test_non_git_directory_is_refused() {
  ...
  pass "xo-claude-trust.sh: refuses a directory that is not a git worktree"
}
...
test_missing_directory_is_refused() {
  ...
  pass "xo-claude-trust.sh: refuses a path that does not exist"
}
...
test_foreign_project_worktree_is_refused() {
  ...
  xo_git_worktree "$other" "$other_wt" wt-other
  ...
  pass "xo-claude-trust.sh: refuses a worktree belonging to another project"
}
...
test_worktree_subdirectory_is_refused() {
  ...
  pass "xo-claude-trust.sh: refuses a subdirectory of the worktree"
}
...
JSON
  ...
  pass "xo-claude-trust.sh: preserves unrelated store content"
}
...
test_symlinked_store_to_a_foreign_owned_target_is_refused() {
  ...
  if [ "$(id -u)" = 0 ]; then
    pass "xo-claude-trust.sh: refuses a store symlinked to another user's file (skipped as root)"
    return 0
  fi
  ...
  pass "xo-claude-trust.sh: refuses a store symlinked to another user's file"
}
...
test_symlinked_store_to_an_owned_target_is_accepted() {
  ...
  [ -z "$(find "$CASE_DIR/dotfiles" -maxdepth 1 -name '.claude.json.xo-trust.*' -print -quit)" ] \
    || fail "a temporary store file was left beside the resolved target"
  pass "xo-claude-trust.sh: follows a store symlink to this user's own file and leaves the link intact"
}
...
test_missing_node_is_refused() {
  ...
  pass "xo-claude-trust.sh: a missing node is refused rather than degraded"
}
...
test_scope_refusal_stays_fail_closed_without_node() {
  ...
  assert_contains "$out" "primary checkout" "the refusal did not name the primary checkout"
  pass "xo-claude-trust.sh: a scope refusal stays fail-closed without node"
}
...
test_corrupt_store_fails_closed() {
  ...
  pass "xo-claude-trust.sh: refuses an unparseable store and leaves it untouched"
}
...
# temp root /tmp/xo-<id> is the other resource created on the way to the arm, and
# nothing removes it either: xo-teardown finds it through tasktmp= in the task's
...
test_refused_spawn_leaves_no_task_state() {
  ...
  if [ "$(id -u)" = 0 ]; then
    pass "xo-spawn.sh: a trust-refused claude spawn leaves no task state (skipped as root)"
    return 0
  fi
  mkdir -p "$config"
  ln -s /etc/passwd "$config/.claude.json"
  ...
  xo_git_worktree "$proj" "$wt" wt-refused
  xo_test_spawn_brief "$home" "$id"
  out=$(XO_TEST_CLAUDE_CONFIG_DIR="$config" \
    xo_test_run_spawn "$home" "$wt" "$fakebin" "$id" "$proj" claude \
  ...
  [ ! -e "/tmp/xo-$id" ] \
    || { rm -rf "/tmp/xo-$id"; fail "a refused spawn stranded a temp root no teardown can find"; }
  pass "xo-spawn.sh: a trust-refused claude spawn leaves no task state behind"
}

# The spawn half: a real xo-spawn of a claude worker must pre-register the
...
test_claude_spawn_pretrusts_its_worktree_and_reaches_the_brief() {
  ...
  fakebin=$(make_spawn_fakebin "$case_dir/fake" claude)
  xo_test_spawn_home "$home" claude
  xo_git_worktree "$proj" "$wt" wt-spawn
  xo_test_spawn_brief "$home" trustspawn
  out=$(XO_TEST_CLAUDE_CONFIG_DIR="$config" XO_FAKE_LAUNCH_LOG="$launch_log" \
    xo_test_run_spawn "$home" "$wt" "$fakebin" trustspawn "$proj" claude \
    --mode no-mistakes --yolo off)
  ...
  pass "xo-spawn.sh: a claude spawn pre-trusts its worktree and launches with the brief"
}
