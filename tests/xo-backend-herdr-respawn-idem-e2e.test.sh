#!/usr/bin/env bash
# tests/xo-backend-herdr-respawn-idem-e2e.test.sh - isolated real-herdr
# regression test for xo-restart idempotency against herdr's
...
# restored xo-<id> task tab came back a husk - a dead pane, or a plain
# agent-less shell in the saved cwd - and bin/backends/herdr.sh's
# xo_backend_herdr_create_task refused to spawn into it unconditionally,
...
# xo_backend_herdr_create_task now closes-and-replaces the resulting husk
...
# before. Adapter-level (xo_backend_herdr_container_ensure/create_task), not
# through the full bin/xo-spawn.sh + treehouse pipeline - mirrors
# tests/xo-backend-herdr-prune-safety-e2e.test.sh's own style, and avoids any
...
SESSION="xo-lab-respawn-idem-e2e-$$"
export HERDR_SESSION="$SESSION"
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/xo-herdr-respawn-idem.XXXXXX")
cleanup_all() {
...
# xo_backend_herdr_create_task is the ONE function both bin/xo-spawn.sh's
...
# only in XO_HOME-shadowing (tests/xo-backend-herdr-workspace-per-home-e2e.test.sh),
# never in this duplicate-guard logic, which has no home-specific branching.

PROJ_CWD="$SCRATCH/proj"
mkdir -p "$PROJ_CWD"

RAW=$(xo_backend_herdr_container_ensure "$PROJ_CWD") || fail "container_ensure failed"
CONTAINER=${RAW%%$'\t'*}
SEEDED_TAB_ID=${RAW#*$'\t'}
WSID=${CONTAINER#*:}

CREW_LABEL="xo-respawn-crew1"
CREW_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD" "$SEEDED_TAB_ID") \
...
SM_LABEL="xo-respawn-sm1"
SM_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "$SM_LABEL" "$PROJ_CWD") \
...
pass "repro setup: two real xo-<id> task tabs exist (crewmate-shaped and secondmate-shaped), neither with a registered agent"
...
sleep 0.5
xo_backend_herdr_server_ensure "$SESSION" || fail "the isolated session's server did not come back up after the restart"
...
RESPAWN_CREW_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD") \
...
RESPAWN_SM_IDS=$(xo_backend_herdr_create_task "$CONTAINER" "$SM_LABEL" "$PROJ_CWD") \
...
herdr pane report-agent "$NEW_CREW_PANE_ID" --source xo-respawn-e2e --agent xo-respawn-live-agent --state idle --session "$SESSION" >/dev/null 2>&1 \
  || fail "could not register a live agent on the respawned crewmate-shaped pane"

if xo_backend_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD" >/dev/null 2>&1; then
...
xo_backend_herdr_kill "$SESSION:$NEW_CREW_PANE_ID"
xo_backend_herdr_kill "$SESSION:$NEW_SM_PANE_ID"
