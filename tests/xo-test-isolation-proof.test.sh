...
PROOF="$ROOT/bin/xo-test-isolation-proof.sh"
RUNNER="$ROOT/bin/xo-test-run.sh"

assert_present "$PROOF" "bin/xo-test-isolation-proof.sh is missing"
[ -x "$PROOF" ] || fail "bin/xo-test-isolation-proof.sh must be executable"

test_unknown_pool_is_refused() {
  local tmp rc
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-isolation-proof-pool.XXXXXX")
  set +e
...
test_family_pool_json_identifies_admission() {
  ...
  tmp=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-isolation-proof-json.XXXXXX")
  repo="$tmp/repo"
  proof="$repo/bin/xo-test-isolation-proof.sh"
  json="$tmp/proof.json"
  ...
  cp "$PROOF" "$proof"
  cat >"$repo/bin/xo-test-run.sh" <<'SH'
#!/usr/bin/env bash
if { [ "$1" = --list ] || [ "$1" = --list-scheduled ]; } && [ "$2" = --family ]; then
  case "$3" in
    fixture-family)
      printf '%s\n' tests/xo-proof-fixture-a.test.sh tests/xo-proof-fixture-b.test.sh
      exit 0
      ;;
    admitted-family)
      printf '%s\n' tests/xo-proof-slow.test.sh tests/xo-proof-fast.test.sh tests/xo-proof-replacement.test.sh
      exit 0
      ;;
    skipped-family)
      printf '%s\n' tests/xo-proof-skipped.test.sh
      exit 0
      ;;
  esac
fi
...
exit 2
SH
  for fixture in xo-proof-fixture-a.test.sh xo-proof-fixture-b.test.sh; do
...
SH
  ...
  cat >"$repo/tests/xo-proof-slow.test.sh" <<'SH'
#!/usr/bin/env bash
waited=0
...
SH
  cat >"$repo/tests/xo-proof-fast.test.sh" <<'SH'
#!/usr/bin/env bash
echo "ok - fast proof fixture"
SH
  cat >"$repo/tests/xo-proof-replacement.test.sh" <<'SH'
#!/usr/bin/env bash
...
SH
  cat >"$repo/tests/xo-proof-skipped.test.sh" <<'SH'
#!/usr/bin/env bash
echo
echo "skip: herdr not found"
SH
  chmod +x "$proof" "$repo/bin/xo-test-run.sh" "$repo/tests/xo-proof-"*.test.sh
  set +e
...
assert artifact["xo_test_run_jobs_enabled"] is False
...
assert artifact["xo_test_run_jobs_enabled"] is True
...
' "$capped_json" || fail "over-cap family pool artifact metadata is incorrect"
  ...
  grep -Fq 'tests/xo-proof-skipped.test.sh: skip: herdr not found' "$tmp/skipped.err" \
...
test_candidates_exclude_serial_classes() {
  ...
  listed=$("$PROOF" --list)
  for banned in \
    tests/xo-test-isolation-proof.test.sh \
    ...
    tests/xo-afk-inject-e2e.test.sh \
    tests/xo-pi-primary-live-e2e.test.sh \
    tests/xo-pr-check-security.test.sh \
    tests/xo-backend-cmux-smoke.test.sh; do
...
test_list_exclusions_documents_reasons() {
  ...
  printf '%s\n' "$out" | grep -Fq 'xo-watcher-lock.test.sh' \
    || fail "exclusions must document watcher-lock serial reason"
  printf '%s\n' "$out" | grep -Fq 'xo-backend-herdr-smoke.test.sh' \
...
test_family_map_labels_this_contract() {
  ...
  printf '%s\n' "$fam" | grep -Fq 'tests/xo-test-isolation-proof.test.sh' \
    || fail "xo-test-isolation-proof.test.sh must map to pure-contract-unit"
  ...
  [ "$scheduled_first" = tests/xo-watch-triage.test.sh ] \
...
test_fixture_repo_branch_is_pinned() {
  ...
  root=$(xo_test_tmproot xo-fixture-branch-pin) || fail "could not create a fixture root"
  dir="$root/repo"
  config="$root/gitconfig"
  printf '[init]\n\tdefaultBranch = master\n' > "$config"
  GIT_CONFIG_GLOBAL="$config" xo_git_init_commit "$dir"
