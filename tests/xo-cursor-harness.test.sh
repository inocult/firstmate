#!/usr/bin/env bash
# tests/xo-cursor-harness.test.sh - the portable regression for the Cursor
...
# tests/xo-harness-liveness-drift-live-e2e.test.sh is what catches vendor drift
...
TMP_ROOT=$(xo_test_tmproot xo-cursor-harness)
...
test_identity_accepts_cursor_shapes_rejects_lookalikes() {
  ...
  xo_cursor_process_matches node '' "$bin/cursor-agent" \
    || fail "tmux's node + cursor-agent argv[0] must identify as cursor"
  xo_cursor_process_matches "$bin/cursor-agent" '' '' \
    || fail "ps's cursor-agent install path must identify as cursor"
  xo_cursor_process_matches cursor-agent '' '' \
  ...
  xo_cursor_process_matches agent '' "$bin/agent" \
  ...
  if command -v node >/dev/null 2>&1; then
    ...
    if [ -n "$out" ]; then
      ! xo_cursor_process_matches "$out" '' "$out" \
  ...
  ! xo_cursor_process_matches node '' /opt/agent/bin/runner \
    || fail "an 'agent/' directory component must not identify as cursor"
  ! xo_cursor_process_matches node '' /tmp/cursor-agent/bin/runner \
    || fail "a cursor-agent directory outside the versioned install tree must not identify"
  ! xo_cursor_process_matches MainThread '' '' \
    || fail "a bare MainThread with no cursor evidence must not identify"
  ! xo_cursor_process_matches node '' '' \
    || fail "a node with no argv[0] evidence must not identify"
  pass "xo_cursor_process_matches: cursor's real shapes identify; real node/agent lookalikes do not"
}
...
test_identity_signals_diverge() {
  ...
  xo_cursor_process_matches "$odd/cursor-agent" '' '' \
  ...
  case "/$(xo_cursor_canonical_path "$bin/agent")/" in
    */cursor-agent/*) : ;;
    *) fail "path-signal fixture must canonicalize into a cursor-agent tree" ;;
  esac
  pass "xo_cursor_process_matches: name and install-tree signals each carry a verdict alone"
}
...
test_verify_executable_refuses_unrelated_agent() {
  ...
  xo_cursor_verify_executable "$bin/agent" \
    || fail "the alias inside cursor's install tree must verify"
  ! xo_cursor_verify_executable "$odd/agent" \
    || fail "an unrelated executable named agent must NOT verify as cursor"
  pass "xo_cursor_verify_executable: the legacy alias is accepted only with cursor evidence"
}
...
test_resolve_binary_prefers_stable_path() {
  ...
  out=$(PATH="$bin:$PATH" xo_cursor_resolve_binary) \
  ...
  pass "xo_cursor_resolve_binary: prints the stable launcher, not the versioned target"
}
...
test_tmux_classifies_cursor_pane_without_inferring_dead() {
  ...
  ( XO_BACKEND_LIB_DIR="$ROOT/bin"; . "$ROOT/bin/backends/tmux.sh"
    [ "$(xo_agent_process_classify_name node "$bin/cursor-agent")" = agent ] \
      || fail "a cursor pane reported as node must classify agent"
    [ "$(xo_agent_process_classify_name '' "$bin/cursor-agent")" = agent ] \
    ...
    [ "$(xo_agent_process_classify_name node /usr/bin/node)" = other ] \
      || fail "an unrelated node must stay 'other', never agent"
    [ "$(xo_agent_process_classify_name agent /usr/local/bin/agent)" = other ] \
      || fail "an unrelated agent must stay 'other', never agent"
    # Neighbours must not regress.
    [ "$(xo_agent_process_classify_name claude '')" = agent ] || fail "claude regressed"
    [ "$(xo_agent_process_classify_name zsh '')" = shell ] || fail "zsh regressed"
  ) || exit 1
...
test_cursor_marker_outranks_inherited_claudecode() {
  ...
  pass "xo-harness.sh: cursor's marker outranks an inherited CLAUDECODE"
}
...
JS
  ...
  pass "xo-harness.sh: cursor-like node script names do not establish ancestry identity"
}

# --- 4. The transcript busy fold --------------------------------------------

# Build a bound cursor workspace: a project dir keyed by .workspace-trusted, a
# conversation transcript, and the per-task sidecar xo-spawn writes.
...
{"role":"assistant"}
')
  out=$(xo_busy_classify tmux none cursor task "$state")
...
')
  log=$(xo_busy_cursor_transcript "$state" task) \
    || fail "the malformed-close transcript fixture must resolve"
  out=$(xo_busy_cursor_turn_state "$log")
  [ "$out" = busy ] \
  ...
  out=$(PATH="$no_jq_bin" xo_busy_cursor_turn_state "$log")
  [ "$out" = busy ] \
...
')
  ...
  out=$(xo_busy_classify tmux none cursor task "$empty_state")
  ...
  out=$(xo_busy_classify tmux none cursor task "$TMP_ROOT/unclaimed/state")
