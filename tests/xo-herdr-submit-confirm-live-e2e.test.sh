...
# and requires xo_backend_herdr_send_text_submit to report empty for a landed
# idle steer. It fails naming the harness and version rather than degrading
# quietly.
#
# Run explicitly with XO_HERDR_SUBMIT_CONFIRM_LIVE=1 after a Herdr or Claude
# upgrade, and before trusting a refreshed docs/verification/runtime-backends.md
# "Herdr submit confirmation" entry.
# Every Herdr call, including adapter calls, is routed through bin/xo-herdr-lab.sh.
set -u
...
xo_live_gate opt-in XO_HERDR_SUBMIT_CONFIRM_LIVE herdr jq claude

[ -x "$LAB_HELPER" ] || fail "XO_HERDR_SUBMIT_CONFIRM_LIVE=1 but the Herdr lab helper is not executable at $LAB_HELPER"
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/xo-herdr-submit-confirm-live.XXXXXX")
...
WS_JSON=$(lab workspace create --cwd "$ROOT" --label xo-submitlive --no-focus) \
...
verdict=$(xo_backend_herdr_send_text_submit "$TARGET" "Reply with exactly $TOKEN and nothing else." 3 0.4 0.4) \
...
[ "$CHECKED" -gt 0 ] || fail "XO_HERDR_SUBMIT_CONFIRM_LIVE=1 checked no harness"
