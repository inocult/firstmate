#!/usr/bin/env bash
# tests/xo-classify-corr-token.test.sh - a status line may carry the correlation
# token bin/xo-pending-reply-lib.sh embeds in a marked request and a secondmate
# echoes back (bin/xo-brief.sh), between the verb and the rest of the line. Every
...
# hard as they defend the fix: only the exact token an xo library writes is
...
# through the REAL bin/xo-wake-drain.sh, the classifier predicates through the
# library's own sourced entry points (as tests/xo-watch-triage.test.sh does), and
...
TMP_ROOT=$(xo_test_tmproot xo-classify-corr-token-tests)
...
drain_open() {  # <state> <out>
  local state=$1 out=$2
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed over $state"
...
test_token_is_read_through_in_every_position_it_is_written_in() {
  ...
  # bin/xo-secondmate-report.sh helper writes. Every one is a real observed shape.
...
test_prose_and_malformed_tokens_never_become_transitions() {
  ...
  # well-formed key past _xo_decision_key_transition_allowed.
  local -a impostors=(
...
test_daemon_and_crew_state_case_arms_read_through_the_token() {
  ...
  # cannot be proven through status_is_*: bin/xo-supervise-daemon.sh matches
  # working|resolved|captain-held to take the transient-stale path instead of
  # the terminal one, and bin/xo-crew-state.sh's map_log_state maps each verb to
...
test_pending_reply_escalation_matching_is_unaffected() {
  # bin/xo-pending-reply-lib.sh filters a status file by verb when looking for
...
test_incremental_and_whole_file_folds_agree_over_correlated_lines() {
  ...
  while [ "$round" -lt 6 ]; do
    ...
    whole=$(XO_STATE_OVERRIDE="$state" bash -c '. "$1"; status_open_decisions "$2"' _ \
      "$ROOT/bin/xo-classify-lib.sh" "$status")
    inc=$(XO_STATE_OVERRIDE="$state" bash -c '. "$1"; status_open_decisions_incremental "$2"' _ \
      "$ROOT/bin/xo-classify-lib.sh" "$status")
    [ "$whole" = "$inc" ] \
...
test_a_cursor_written_before_this_change_is_rebuilt() {
  ...
  cursor="$state/.task-stale.open-decisions-cursor"
  {
    ...
    printf 'ident=%s\n' "$(bash -c '. "$1"; _xo_open_decisions_file_ident "$2"' _ \
      "$ROOT/bin/xo-classify-lib.sh" "$status")"
  } > "$cursor"
...
test_the_real_writers_produce_tokens_this_library_reads() {
  ...
  local i=0 corr
  while [ "$i" -lt 5 ]; do
    corr=$(bash -c '. "$1"; xo_pending_reply_new_id' _ "$ROOT/bin/xo-pending-reply-lib.sh")
    [ -n "$corr" ] || fail "the correlation library produced an empty id"
    token=$(bash -c '. "$1"; xo_pending_reply_corr_token "$2"' _ \
      "$ROOT/bin/xo-pending-reply-lib.sh" "$corr")
...
parent_home=$parent
EOF
  XO_HOME="$mate" "$REPORT" "done" "$corr" "audit clean" \
  ...
  XO_HOME="$mate" "$REPORT" --doc needs-decision "$corr" data/x/report.md "see the report" \
