...
TMP_ROOT=$(xo_test_tmproot xo-watch-checkpoint)
...
test_quiet_checkpoint_exits_124_cleanly() {
  ...
  err="$home/err.txt"
  status=0
  XO_HOME="$home" XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 "$CHECKPOINT" --seconds 1 >"$out" 2>"$err" || status=$?
...
test_signal_passes_through_and_exits_zero() {
  ...
  status=0
  XO_HOME="$home" XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=999999 "$CHECKPOINT" --seconds 8 >"$out" 2>"$err" || status=$?
  expect_code 0 "$status" "signal checkpoint exit"
  assert_contains "$(cat "$out")" "signal:" "signal wake was not passed through"
  drained=$(XO_HOME="$home" "$ROOT/bin/xo-wake-drain.sh")
...
#!/usr/bin/env bash
printf 'env check fired with XO_CHECK_INTERVAL=%s\n' "${XO_CHECK_INTERVAL:-missing}"
SH
  chmod 0700 "$home/state/env-check.check.sh"
  XO_HOME="$home" "$ROOT/bin/xo-check-register.sh" env-check >/dev/null \
    || fail "could not register checkpoint custom check"
  status=0
  XO_HOME="$home" XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=1 "$CHECKPOINT" --seconds 5 >"$out" 2>"$err" || status=$?
  expect_code 0 "$status" "check checkpoint exit"
  assert_contains "$(cat "$out")" "check:" "check wake was not passed through"
  assert_contains "$(cat "$out")" "XO_CHECK_INTERVAL=1" "watcher environment was not preserved"
...
test_existing_singleton_watcher_is_not_success() {
  ...
  status=0
  XO_HOME="$home" XO_GUARD_GRACE=300 "$CHECKPOINT" --seconds 5 >"$out" 2>"$err" || status=$?
