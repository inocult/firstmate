#!/usr/bin/env bash
# tests/xo-remote-herdr-guard.test.sh - the xo-remote launch agent's guard.
#
# Drives the real bin/xo-remote-herdr-guard.sh (and the owner library it
...
TMP_ROOT=$(xo_test_tmproot xo-remote-herdr-guard)
mkdir -p "$TMP_ROOT"
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
HOLDER_PIDS=()
HOLDER_FD=5
trap 'if [ "${#HOLDER_PIDS[@]}" -gt 0 ]; then kill "${HOLDER_PIDS[@]}" 2>/dev/null || true; fi; xo_test_cleanup || true' EXIT
...
[ -f "$XO_FAKE_SOCKET_OWNER" ] || exit 0
while IFS= read -r pid; do
  [ -n "$pid" ] || continue
  printf 'p%s\n' "$pid"
  printf 'n%s\n' "$XO_FAKE_HERDR_SOCKET"
done < "$XO_FAKE_SOCKET_OWNER"
SH
...
label=${domain#*/}
label=${label#*/}
state="$XO_FAKE_STATE/launchctl-$scope-$label"
[ -f "$state" ] || exit 113
cat "$state"
SH
cat > "$FAKE/herdr" <<'SH'
...
case "$*" in
  "status --json --session "*)
    if [ -f "$XO_FAKE_STATE/release-after" ]; then
      left=$(cat "$XO_FAKE_STATE/release-after")
      if [ "$left" -gt 0 ]; then
        printf '%s\n' "$((left - 1))" > "$XO_FAKE_STATE/release-after"
      else
        rm -f "$XO_FAKE_STATE/release-after"
    ...
    ;;
  "server stop --session "*)
    if [ -f "$XO_FAKE_STATE/stop-ignored" ]; then
      exit 0
    elif [ -f "$XO_FAKE_STATE/stop-releases-after" ]; then
      cp "$XO_FAKE_STATE/stop-releases-after" "$XO_FAKE_STATE/release-after"
    else
      printf 'false\n' > "$XO_FAKE_HERDR_RUNNING"
    fi
    ;;
  "server --session "*)
    printf 'pid=%s session=%s\n' "$$" "${3:-}" > "$XO_FAKE_STATE/started"
    ;;
esac
exit 0
SH
...
hold_under() {
  ...
  mkfifo "$fifo"
  eval "exec ${HOLDER_FD}<>\"\$fifo\""
  ( export XO_HOLDER_JQ="$JQ" XO_HOLDER_FIFO="$fifo" XO_HOLDER_PIDFILE="$pidfile"
    exec -a "$argv0" bash -c 'env -i XO_HOLDER=1 "$XO_HOLDER_JQ" . "$XO_HOLDER_FIFO" & printf "%s\n" "$!" > "$XO_HOLDER_PIDFILE"; wait' "$@" ) &
  HOLDER_PIDS+=("$!")
  HOLDER_FD=$((HOLDER_FD + 1))
  local i=0
...
guard() { # [extra env assignments...]
  ...
  GUARD_OUT=$(
    env -i PATH="$CASE_PATH" HOME="$TMP_ROOT" \
      XO_FAKE_STATE="$CASE_STATE" XO_FAKE_HERDR_LOG="$CASE_LOG" XO_FAKE_HERDR_RUNNING="$CASE_RUNNING" \
      XO_FAKE_SOCKET_OWNER="$CASE_OWNER" XO_FAKE_HERDR_SOCKET="$CASE_SOCKET" \
      XO_HOLDER_JQ="$JQ" \
      XO_REMOTE_HERDR_GUARD_STOP_WAIT_TENTHS=8 \
...
hold XO_PROBE_MARKER=1
PROBE_PID=$HOLDER_PID
sleep 0.2
# shellcheck source=bin/xo-remote-herdr-owner-lib.sh
. "$ROOT/bin/xo-remote-herdr-owner-lib.sh"
probe_env=$(xo_remote_herdr_process_env "$PROBE_PID")
case "$probe_env" in
  *XO_PROBE_MARKER=1*) ;;
...
load_job gui dev.xo.herdr.xo-remote "$LAUNCHD_PID"
guard
...
load_job gui dev.xo.remote-job
guard
...
load_job gui dev.xo.herdr.xo-remote
load_job user dev.xo.herdr.xo-remote
guard
