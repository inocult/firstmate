#!/usr/bin/env bash
# tests/xo-wake-daemon-lifecycle-e2e.test.sh - the watcher + supervise-daemon
...
# xo-watch.sh runs plus the daemon's own functions. The captain-relevant
# status-phrase matrix and the lock-primitive races stay as focused units
# (xo-daemon.test.sh, xo-watcher-lock.test.sh) - an e2e cannot deterministically
...
TMP_ROOT=$(xo_test_tmproot xo-wake-daemon-e2e)
...
# always-on standalone triage is covered by xo-watch-triage.test.sh. fakebin
...
test_routine_then_terminal_after_restart() {
  ...
  XO_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" 2> "$drain_err" \
    || fail "drain after routine signal failed"
  grep "$(printf '\tsignal\t')" "$drain_out" | grep -F "$status_file" >/dev/null \
    || fail "routine signal was not queued"
  XO_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_PANE_ALIVE=1 XO_FAKE_TMUX_SENT="$sent" \
    XO_FAKE_TMUX_CAPTURE="$dir/pane.txt" XO_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
...
test_stale_pane_transient_persistent_resume() {
  ...
  PATH="$fakebin:$PATH" XO_FAKE_TMUX_WINDOW="$win" XO_FAKE_TMUX_CAPTURE="$dir/pane.txt" \
  ...
  # own semantic busy-state record (bin/xo-busy-lib.sh), not rendered pane text.
  stale_marker_record "$win" "$state"
  echo $(( $(date +%s) - 500 )) > "$state/.subsuper-stale-$key"
  printf 'Working...\n' > "$dir/pane.txt"
  xo_write_meta "$state/stale-w2.meta" "window=$win" "worktree=$dir/wt" "kind=ship" "harness=pi"
  resumed_gen=$("$ROOT/bin/xo-busy-event.sh" arm "$state" stale-w2)
  "$ROOT/bin/xo-busy-event.sh" apply "$state" stale-w2 busy --gen "$resumed_gen" \
