#!/usr/bin/env bash
# tests/xo-trace-context-spawn.test.sh - spawn-path integration regressions for
...
TMP_ROOT=$(xo_test_tmproot xo-trace-context-spawn)
...
case "${1:-}" in
  ...
  list-windows)
    [ -z "${XO_FAKE_DUPLICATE_WINDOW:-}" ] || printf '%s\n' "$XO_FAKE_DUPLICATE_WINDOW"
    exit 0
    ;;
  has-session|new-session|new-window|kill-window) exit 0 ;;
  send-keys)
    if [ "${XO_FAKE_TRACEPARENT_SEND_FAIL:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
    ...
    if [ "${XO_FAKE_TRACEPARENT_SEND_UNSAFE:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
    ...
    if [ "${XO_FAKE_TRACE_METADATA_APPEND_FAIL:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
          "export TRACEPARENT="*)
            chmod a-w "$XO_FAKE_META_PATH"
            ;;
        esac
      done
    fi
...
SH
  ...
  xo_fake_exit0 "$fakebin" treehouse
  printf '%s\n' "$fakebin"
}
...
# Hermetic against an ambient XO_TRACE_CONTEXT: `env -u` unsets it so enablement
...
run_spawn() {
  ...
  env -u XO_TRACE_CONTEXT \
    XO_ROOT_OVERRIDE='' XO_HOME="$home" HOME="$home/user-home" CLAUDE_CONFIG_DIR='' \
    ...
    XO_FAKE_TRACEPARENT_SEND_FAIL="${XO_FAKE_TRACEPARENT_SEND_FAIL:-0}" \
    XO_FAKE_TRACEPARENT_SEND_UNSAFE="${XO_FAKE_TRACEPARENT_SEND_UNSAFE:-0}" \
    XO_FAKE_TRACE_METADATA_APPEND_FAIL="${XO_FAKE_TRACE_METADATA_APPEND_FAIL:-0}" \
    XO_FAKE_META_PATH="$home/state/$1.meta" \
    XO_FAKE_LAUNCH_LOG="$launchlog" PATH="$fakebin:$PATH" \
    "$SPAWN" "$@" --mode no-mistakes --yolo off 2>&1
}

# Same, but with an explicit XO_TRACE_CONTEXT override, to prove the env decides.
run_spawn_tc() {
...
start_trace_session() {
  ...
  printf '%s\n' "$$" > "$home/state/.lock"
  if [ -n "$tc" ]; then
    XO_TRACE_CONTEXT="$tc" xo_trace_context_session_start \
      "$home/config" "$home/state/.trace-context-effective"
  else
    (
      unset XO_TRACE_CONTEXT
      xo_trace_context_session_start \
        "$home/config" "$home/state/.trace-context-effective"
    )
  fi
}
...
# Two-level primary -> secondmate -> worker regression for the XO_TRACE_CONTEXT
# effective override. Drives bin/xo-spawn.sh TWICE against real homes and a real
...
run_two_level() {
  ...
  # Seed the secondmate home so validate_xo_home_for_spawn accepts it.
  mkdir -p "$sm/bin" "$sm/data"
  printf '# XO\n' > "$sm/AGENTS.md"
  printf 'sm-%s\n' "$name" > "$sm/.xo-secondmate-home"
  printf 'charter\n' > "$sm/data/charter.md"
  ...
  : > "$smlog"
  env XO_TRACE_CONTEXT="$penv" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$prim" \
    XO_STATE_OVERRIDE="$prim/state" XO_DATA_OVERRIDE="$prim/data" \
    XO_PROJECTS_OVERRIDE="$prim/projects" XO_CONFIG_OVERRIDE="$prim/config" \
    XO_SPAWN_NO_GUARD=1 CLAUDECODE=1 TMUX="fake,1,0" \
    XO_FAKE_LAUNCH_LOG="$smlog" PATH="$smfake:$PATH" \
    "$SPAWN" "$sm_id" "$sm" --secondmate >/dev/null 2>&1 || true

  # Extract the EXACT env the primary put on the secondmate: the normalized
  # XO_TRACE_CONTEXT in the launch prefix, and the TRACEPARENT carrier (if any).
  TL_ENV_TC=$(grep -o 'XO_TRACE_CONTEXT=[a-z]*' "$smlog" | head -1 | cut -d= -f2)
  ...
  wproj="$base/wproj"
  wwt="$base/wwt"
  xo_git_worktree "$wproj" "$wwt" "wt-$name"
  ...
  env XO_TRACE_CONTEXT="$TL_ENV_TC" TRACEPARENT="$TL_CARRIER" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$sm" HOME="$sm/user-home" CLAUDE_CONFIG_DIR='' \
    XO_STATE_OVERRIDE="$sm/state" XO_DATA_OVERRIDE="$sm/data" \
    XO_PROJECTS_OVERRIDE="$sm/projects" XO_CONFIG_OVERRIDE="$sm/config" \
    XO_SPAWN_NO_GUARD=1 XO_FAKE_PANE_PATH="$wwt" TMUX="fake,1,0" \
    XO_FAKE_LAUNCH_LOG="$wlog" PATH="$wfake:$PATH" \
...
test_enabled_records_and_injects_identical_carrier_before_launch() {
  ...
  xo_trace_context_valid "$mtp" || fail "enabled spawn must record a valid traceparent= in meta (got '$mtp')"
  itp=$(injected_traceparent "$LAUNCH_LOG")
  xo_trace_context_valid "$itp" || fail "enabled spawn must inject a valid TRACEPARENT export (got '$itp')"
...
test_disabled_writes_and_injects_neither() {
  ...
  read_case_record "$rec"
  # No config/trace-context and no XO_TRACE_CONTEXT: default-off.
...
test_failed_delivery_omits_metadata_and_still_launches() {
  ...
  out=$(XO_FAKE_TRACEPARENT_SEND_FAIL=1 \
...
test_unsafe_delivery_refuses_to_append_launch() {
  ...
  out=$(XO_FAKE_TRACEPARENT_SEND_UNSAFE=1 \
    run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$CASE_ID" "$PROJ_DIR")
  status=$?
...
test_failed_metadata_append_unsets_carrier_and_still_launches() {
  ...
  out=$(XO_FAKE_TRACE_METADATA_APPEND_FAIL=1 \
    run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$CASE_ID" "$PROJ_DIR")
  status=$?
...
test_duplicate_secondmate_spawn_does_not_converge_trace_context() {
  ...
  mkdir -p "$sm/bin" "$sm/data"
  printf '# XO\n' > "$sm/AGENTS.md"
  printf '%s\n' "$id" > "$sm/.xo-secondmate-home"
  printf 'charter\n' > "$sm/data/charter.md"
  fake=$(make_spawn_fakebin "$base/fake")

  out=$(env -u XO_TRACE_CONTEXT \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$prim" \
    XO_STATE_OVERRIDE="$prim/state" XO_DATA_OVERRIDE="$prim/data" \
    XO_PROJECTS_OVERRIDE="$prim/projects" XO_CONFIG_OVERRIDE="$prim/config" \
    XO_SPAWN_NO_GUARD=1 CLAUDECODE=1 TMUX="fake,1,0" \
    XO_FAKE_DUPLICATE_WINDOW="xo-$id" XO_FAKE_LAUNCH_LOG="$log" \
...
test_relaunch_reuses_recorded_carrier() {
  ...
  xo_trace_context_valid "$first" || fail "first spawn must record a valid carrier (got '$first')"
...
test_session_start_freezes_env_override_and_ignores_later_edits() {
  ...
  meta="$HOME_DIR/state/$CASE_ID.meta"
  ! grep -q '^traceparent=' "$meta" || fail "session-frozen off must ignore a later XO_TRACE_CONTEXT=on"
  ...
  meta="$HOME_DIR/state/$CASE_ID.meta"
  xo_trace_context_valid "$(meta_traceparent "$meta")" \
    || fail "session-frozen on must ignore a later XO_TRACE_CONTEXT=off"
...
test_secondmate_env_on_file_absent_keeps_nested_worker_enabled() {
  ...
  [ "$TL_ENV_TC" = on ] || fail "the primary must deliver XO_TRACE_CONTEXT=on to the secondmate (got '$TL_ENV_TC')"
  xo_trace_context_valid "$TL_CARRIER" || fail "an enabled primary must mint a carrier for the secondmate (got '$TL_CARRIER')"
  xo_trace_context_valid "$TL_WORKER_TP" \
...
# XO_TRACE_CONTEXT=off is not a real kill switch. Before the fix the copied file
...
test_secondmate_env_off_file_present_keeps_nested_worker_disabled() {
  ...
  [ "$TL_ENV_TC" = off ] || fail "the primary must deliver XO_TRACE_CONTEXT=off to the secondmate (got '$TL_ENV_TC')"
...
test_two_routed_tasks_through_one_secondmate_root_distinct_traces() {
  ...
  xo_git_worktree "$proj_a" "$wt_a" wt-routed-a
  xo_git_worktree "$proj_b" "$wt_b" wt-routed-b
  ...
  xo_trace_context_valid "$tp_a" || fail "routed task A must record a valid carrier (got '$tp_a')"
  xo_trace_context_valid "$tp_b" || fail "routed task B must record a valid carrier (got '$tp_b')"
...
# carrier and the delivered XO_TRACE_CONTEXT snapshot are always derived from ONE
...
# (XO_TRACE_CONTEXT unset), which is exactly where the two-read correction matters
...
test_secondmate_carrier_and_snapshot_share_one_decision() {
  ...
  [ "$TL_ENV_TC" = on ] || fail "a file-enabled secondmate must snapshot XO_TRACE_CONTEXT=on (got '$TL_ENV_TC')"
  xo_trace_context_valid "$TL_CARRIER" \
  ...
  [ "$TL_ENV_TC" = off ] || fail "a file-disabled secondmate must snapshot XO_TRACE_CONTEXT=off (got '$TL_ENV_TC')"
  [ -z "$TL_CARRIER" ] \
    || fail "a file-disabled secondmate must inject no carrier, consistent with the off snapshot (got '$TL_CARRIER')"
  pass "secondmate carrier and XO_TRACE_CONTEXT snapshot always agree, both derived from one frozen decision (file-decided path)"
}
...
echo "# all xo-trace-context-spawn tests passed"
