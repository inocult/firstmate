#!/usr/bin/env bash
# Behavior tests for xo-bootstrap.sh reporting and session-start clone refresh bounds.
#
...
# all is well. xo consumes the exact 'MISSING: treehouse (install: ...)',
...
# scan.
# Dedicated network-phase cases pin XO_BOOTSTRAP_NETWORK as a true partition of
...
TMP_ROOT=$(xo_test_tmproot xo-bootstrap-tests)
export XO_BACKEND_CMUX_BUNDLE_BIN="$TMP_ROOT/no-bundled-cmux"
...
# xo_backend_name and flip a default-backend case onto a non-tmux backend. Unset
...
# treehouse's `get --help` advertises --lease only when XO_FAKE_TREEHOUSE_LEASE_HELP=1.
make_fake_toolchain() {
  local dir=$1 fakebin
...
if [ "${1:-}" = get ] && [ "${2:-}" = --help ]; then
  if [ "${XO_FAKE_TREEHOUSE_LEASE_HELP:-}" = 1 ]; then
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${XO_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.46.0 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${XO_FAKE_QUOTA_AXI_VERSION:-0.1.29}"
  exit 0
fi
exit 0
SH
...
make_fake_fleet_sync_root() {
  ...
  cat > "$fake_root/bin/xo-fleet-sync.sh" <<'SH'
#!/usr/bin/env bash
[ -z "${XO_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ] || : > "$XO_FAKE_FLEET_SYNC_STARTED_MARKER"
printf '%s\n' 'alpha: synced'
printf '%s\n' 'beta: skipped: no origin remote'
exec perl -e 'sleep 300'
SH
  chmod +x "$fake_root/bin/xo-fleet-sync.sh"
  printf '%s\n' "$fake_root"
}
...
run_bootstrap_timeout_case() {
  ...
  (
    ...
    git() {
      local tries
      if [ "${XO_FAKE_GIT_WAIT_FOR_FLEET_START:-}" = 1 ] && [ -n "${XO_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ]; then
        tries=0
        while [ "$tries" -lt 5 ] && [ ! -e "$XO_FAKE_FLEET_SYNC_STARTED_MARKER" ]; do
          command sleep 0.01
          tries=$((tries + 1))
        done
      fi
      if [ -n "${XO_FAKE_GIT_SYNC_STARTED_RECORD:-}" ] && [ -n "${XO_FAKE_FLEET_SYNC_STARTED_MARKER:-}" ] && [ -e "$XO_FAKE_FLEET_SYNC_STARTED_MARKER" ]; then
        printf '%s\n' "$*" >> "$XO_FAKE_GIT_SYNC_STARTED_RECORD"
      fi
      command git "$@"
    }
    export -f sleep
    ...
    if [ "$override" = __unset__ ]; then
      PATH="$fakebin:$BASE_PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$fake_root" \
        XO_FAKE_FLEET_SYNC_STARTED_MARKER="$started_marker" \
        XO_FAKE_GIT_SYNC_STARTED_RECORD="$git_record" \
        XO_FAKE_GIT_WAIT_FOR_FLEET_START="$wait_for_marker" \
        XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null
    else
      PATH="$fakebin:$BASE_PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$fake_root" \
        XO_FLEET_SYNC_BOOTSTRAP_TIMEOUT="$override" \
        XO_FAKE_FLEET_SYNC_STARTED_MARKER="$started_marker" \
        XO_FAKE_GIT_SYNC_STARTED_RECORD="$git_record" \
        XO_FAKE_GIT_WAIT_FOR_FLEET_START="$wait_for_marker" \
        XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh" 2>/dev/null
...
test_bootstrap_reporting() {
  ...
  while IFS='^' read -r label lease tasks quota backend mode expect notcontains; do
    ...
    # XO_ROOT_OVERRIDE points the worktree-tangle check at the non-git home dir so
    ...
    out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
      XO_FAKE_TREEHOUSE_LEASE_HELP="$lease" "$ROOT/bin/xo-bootstrap.sh")
    case "$mode" in
      empty)
...
test_no_mistakes_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
      XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_FAKE_NO_MISTAKES_VERSION="$version" "$ROOT/bin/xo-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
test_gh_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
      XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_FAKE_GH_AXI_VERSION="$version" "$ROOT/bin/xo-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
test_lavish_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
      XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_FAKE_LAVISH_AXI_VERSION="$version" "$ROOT/bin/xo-bootstrap.sh") \
...
test_tasks_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
      XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
test_quota_axi_min_version() {
  ...
  while IFS='^' read -r label version mode; do
    ...
    fakebin=$(make_fake_toolchain "$case_dir")
    out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
      XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_FAKE_QUOTA_AXI_VERSION="$version" "$ROOT/bin/xo-bootstrap.sh")
    case "$mode" in
      empty)
        [ -z "$out" ] || fail "$label: expected silence, got: $out" ;;
      missing)
...
SH
  ...
  out=$(PATH="$fakebin:$BASE_PATH" BASH_ENV="$bash_env" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh")
...
test_orca_backend_gates_orca_tool_only_when_selected() {
  ...
  fakebin=$(make_fake_toolchain "$case_dir")
  out=$(PATH="$fakebin:$(xo_test_base_path_sans "$BASE_PATH" orca)" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh")
...
test_herdr_install_requires_manual_action() {
  local out status
  out=$("$ROOT/bin/xo-bootstrap.sh" install herdr 2>&1)
  status=$?
...
test_cmux_bundled_cli_satisfies_dependency() {
  ...
  fakebin=$(make_fake_toolchain_no_tmux "$case_dir")
  xo_fake_exit0 "$case_dir/bundle" cmux
  bundle="$case_dir/bundle/cmux"
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_BACKEND_CMUX_BUNDLE_BIN="$bundle" XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh")
...
test_treehouse_lease_check_follows_resolved_backend() {
  ...
  fakebin=$(make_fake_toolchain "$case_dir")
  rm -f "$fakebin/tmux"
  xo_fake_exit0 "$fakebin" orca
  # XO_FAKE_TREEHOUSE_LEASE_HELP unset: the fake treehouse advertises NO --lease.
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    "$ROOT/bin/xo-bootstrap.sh")
...
test_fleet_sync_timeout_explicit_override_wins() {
  ...
  pass "bootstrap preserves XO_FLEET_SYNC_BOOTSTRAP_TIMEOUT as an explicit override"
}
...
run_routine_bootstrap_fixture() {
  ...
  home=${fixture%%|*}
  fakebin=${fixture#*|}
  PATH="$fakebin:$BASE_PATH" XO_BACKEND=tmux XO_HOME="$home" XO_ROOT_OVERRIDE="$root" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 \
    "$shell" "$ROOT/bin/xo-bootstrap.sh"
}
...
# XO_BOOTSTRAP_NETWORK splits one bootstrap run into its local and network
...
SH
  ...
  all_out=$(PATH="$fakebin:$(xo_test_base_path_sans "$BASE_PATH" node)" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh")
  ...
  skip_out=$(PATH="$fakebin:$(xo_test_base_path_sans "$BASE_PATH" node)" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_BOOTSTRAP_NETWORK=skip "$ROOT/bin/xo-bootstrap.sh")
  ...
  only_out=$(PATH="$fakebin:$(xo_test_base_path_sans "$BASE_PATH" node)" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_BOOTSTRAP_NETWORK=only "$ROOT/bin/xo-bootstrap.sh")
  ...
  [ "$(PATH="$fakebin:$(xo_test_base_path_sans "$BASE_PATH" node)" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_BOOTSTRAP_NETWORK=sikp "$ROOT/bin/xo-bootstrap.sh")" = "$all_out" ] \
    || fail "an unrecognized XO_BOOTSTRAP_NETWORK value did not fall back to the complete run"
  pass "bootstrap: XO_BOOTSTRAP_NETWORK partitions one run into local and network halves"
}
...
test_network_sweeps_recheck_lock_ownership() {
  ...
  cat > "$fake_root/bin/xo-fleet-sync.sh" <<'SH'
#!/usr/bin/env bash
: > "${XO_FAKE_FLEET_SYNC_STARTED_MARKER:?}"
SH
  chmod +x "$fake_root/bin/xo-fleet-sync.sh"

  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$fake_root" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_BOOTSTRAP_NETWORK=only \
    XO_BOOTSTRAP_NETWORK_LOCK_PID=111111 XO_FAKE_FLEET_SYNC_STARTED_MARKER="$marker" \
    "$ROOT/bin/xo-bootstrap.sh")
...
# assert_timing_record <log> <scope> <name> <detail> <msg>: one bin/xo-timing-lib.sh
...
# bin/xo-timing-lib.sh stays inert unless XO_TIMING_LOG names a file, so an
...
test_network_phases_record_per_step_elapsed_times() {
  ...
  printf '%s\n' $$ > "$case_dir/home/state/.lock"
  fakebin=$(make_fake_toolchain "$case_dir")
  # A real clone with a real origin, so xo-fleet-sync.sh genuinely iterates it.
  xo_git_init_commit "$case_dir/home/projects/alpha"
  xo_git_add_origin "$case_dir/home/projects/alpha" "$case_dir/alpha-origin"
  ...
  xo_write_secondmate_meta "$case_dir/home/state/mate-a.meta" "$case_dir/home"

  log="$case_dir/timings.tsv"
  PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_FAKE_TREEHOUSE_LEASE_HELP=1 XO_BOOTSTRAP_NETWORK=only \
    XO_BOOTSTRAP_NETWORK_LOCK_PID=$$ XO_TIMING_LOG="$log" XO_TIMING_EPOCH_MS=0 \
...
SH
  ...
  : > "$log"
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TASKS_AXI_LOG="$log" XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh")
  ...
  : > "$log"
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TASKS_AXI_LOG="$log" XO_FAKE_TREEHOUSE_LEASE_HELP=1 \
    XO_TASKS_AXI_COMPATIBLE=1 "$ROOT/bin/xo-bootstrap.sh")
  ...
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_FAKE_TASKS_AXI_LOG="$log" XO_FAKE_TREEHOUSE_LEASE_HELP=1 \
    XO_TASKS_AXI_COMPATIBLE=yes "$ROOT/bin/xo-bootstrap.sh")
  ...
  out=$(XO_TASKS_AXI_COMPATIBLE=1 bash -c '. "$1"; printf "%s\n" "${XO_TASKS_AXI_COMPATIBLE-unset}"' \
    _ "$ROOT/bin/xo-tasks-axi-lib.sh")
...
test_crew_dispatch_active_rules_are_verbose_bootstrap_info() {
  ...
  out=$(PATH="$fakebin:$BASE_PATH" XO_HOME="$case_dir/home" XO_ROOT_OVERRIDE="$case_dir/home" \
    XO_BOOTSTRAP_VERBOSE_FACTS=1 XO_FAKE_TREEHOUSE_LEASE_HELP=1 "$ROOT/bin/xo-bootstrap.sh")
