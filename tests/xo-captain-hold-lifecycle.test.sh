...
TMP_ROOT=$(xo_test_tmproot xo-captain-hold)
...
## Done
EOF
  fakebin=$(xo_fakebin "$home")
  xo_fake_exit0 "$fakebin" tmux treehouse no-mistakes gh gh-axi
  printf '%s\n' "$home"
}
...
run_lavish() {  # <home> <command args...>
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/xo-procevent-lavish.sh" "$@"
}
...
run_procevent() {  # <home> <command args...>
  ...
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" \
    XO_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/xo-procevent.sh" "$@"
}

run_bearings() {  # <home> [extra args]
  local home=$1
  shift
  PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_BEARINGS_NOW=2026-07-14T12:00:00Z \
    "$BEARINGS" --json "$@"
}
...
run_teardown() {  # <home> <id>
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" "$id"
}
...
run_pr_merge() {  # <home> <id> <url>
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" XO_TEST_GH_LOG="$home/gh.log" \
    XO_TEST_GH_AXI_LOG="$home/gh-axi.log" "$ROOT/bin/xo-pr-merge.sh" "$@"
}
...
#!/usr/bin/env bash
if [ "${XO_TEST_REUSE_TEARDOWN:-}" = 1 ] \
    && [ ! -e "${XO_TEST_REUSE_TEARDOWN_ONCE:-}" ]; then
  : > "$XO_TEST_REUSE_TEARDOWN_ONCE"
  : > "$XO_TEST_REUSE_TEARDOWN_READY"
  while [ ! -e "$XO_TEST_REUSE_TEARDOWN_RELEASE" ]; do
    "$XO_TEST_REAL_SLEEP" 0.01
  done
fi
exec "$XO_TEST_REAL_PERL" "$@"
SH
  cat > "$home/fakebin/sleep" <<'SH'
#!/usr/bin/env bash
if [ "${XO_TEST_REUSE_MERGE:-}" = 1 ] && [ "${1:-}" = 0.1 ] \
    && [ ! -e "${XO_TEST_REUSE_MERGE_ONCE:-}" ]; then
  : > "$XO_TEST_REUSE_MERGE_ONCE"
  : > "$XO_TEST_REUSE_MERGE_READY"
  while [ ! -e "$XO_TEST_REUSE_MERGE_RELEASE" ]; do
    "$XO_TEST_REAL_SLEEP" 0.01
  done
fi
exec "$XO_TEST_REAL_SLEEP" "$@"
SH
...
run_shim() {  # <home> <command args...>
  ...
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/xo-decision-hold.sh" "$@"
}
...
# report attested: xo-hold-migration rehomed each held row under a prefixed xo-
...
# markdown-only installs, mirroring tests/xo-control-relaunch.test.sh.

# Build a fixture home whose configured backend is a scratch Beads graph.
# Echoes "<home>|<graph-beads-dir>". The graph repo dir is named "xo" because
...
EOF
  fb=$(xo_fakebin "$home")
  xo_fake_exit0 "$fb" tmux treehouse no-mistakes gh gh-axi
  printf '%s\n' "$home|$graph/.beads"
}
...
probe_tasks_axi_beads() {
  local probe_home="$TMP_ROOT/.probe" probe_graph="$TMP_ROOT/.probe-xo"
...
write_scout_with_attested_inventory() {  # <home> <scout-id> <keys>
  ...
  xo_write_meta "$home/state/$scout.meta" \
    "window=xo:xo-$scout" \
...
test_verify_resolves_a_hold_migrated_under_the_configured_prefix() {
  ...
  (cd "$home" && BEADS_ACTOR=fixture tasks-axi add xo-other-legacy-row \
    "Second migrated hold" --repo sample) >/dev/null 2>&1 \
    || fail "could not create the prefix-migrated fixture row"
  (cd "$home" && BEADS_ACTOR=fixture tasks-axi hold xo-other-legacy-row \
  ...
  write_known_rows_stub "$(xo_fakebin "$home")" xo-other-legacy-row
  ...
  assert_contains "$out" "other-legacy-row=xo-other-legacy-row" \
...
test_marker_noted_row_wins_over_a_prefix_namesake() {
  ...
  for row in xo-dual-row xo-marked-dual-row xo-solo-row; do
  ...
  bdrow "$beads" note xo-marked-dual-row \
    "migrated from data/backlog.md id dual-row on 2026-09-04" \
    >/dev/null 2>&1 || fail "could not record the migration marker note"
  write_known_rows_stub "$(xo_fakebin "$home")" \
    xo-dual-row xo-marked-dual-row xo-solo-row
  ...
  assert_not_contains "$out" "dual-row=xo-dual-row" \
    "the bare prefix namesake shadowed the row carrying the migration marker"
  assert_contains "$out" "solo-row=xo-solo-row" \
    "completion did not name the row the prefix guess attested against"
  run_captain "$home" verify "$scout" >/dev/null \
...
SH
  ...
  PATH="$fb:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$home/data" XO_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/xo-captain-hold.sh" hold "$id" --reason "captain must decide" >/dev/null \
  ...
  PATH="$fb:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$home/data" XO_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/xo-captain-hold.sh" answer "$id" --decision-file "$decision" >/dev/null \
...
EOF
  ...
  XO_STATE_OVERRIDE="$home/state" bash -c '
    . "$1"
    xo_wake_status_mark_current "$2" "$3"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$home/state" "$home/state/$id.status" \
  ...
  XO_STATE_OVERRIDE="$home/state" bash -c '
    . "$1"; xo_wake_signal_seen_current "$2" "$3"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$home/state" "$home/state/$id.status" \
  ...
  open=$(bash -c '. "$1"; status_open_decisions "$2"' _ \
    "$ROOT/bin/xo-classify-lib.sh" "$home/state/$id.status")
...
test_answer_records_and_closes() {
  ...
  assert_contains "$show" "Resolution recorded by xo-captain-hold" "the answered task lost the decision record"
...
EOF
  ...
  XO_CAPTAIN_HOLD_NOW=2026-06-01T12:00:00Z run_captain "$home" hold sample-widget \
  ...
  XO_CAPTAIN_HOLD_NOW=2026-07-14T12:00:00Z run_captain "$home" hold sample-widget \
    --reason "captain pricing call needed" >/dev/null \
    || fail "could not re-hold the released work item"
  snap=$(PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$home/data" XO_CONFIG_OVERRIDE="$home/config" \
    XO_PROJECTS_OVERRIDE="$home/projects" XO_SNAPSHOT_NOW=2026-07-14T12:00:00Z \
    "$ROOT/bin/xo-fleet-snapshot.sh" --json) || fail "fleet snapshot failed after re-hold"
  printf '%s' "$snap" | jq -e '
...
if [ "${1:-}" = hold ] && [ "${2:-}" = sample-old-call ]; then
  ...
  : > "$XO_HOME/hold-observed-after-stamp"
fi
exec "$REAL_TASKS_AXI" "$@"
EOF
  chmod +x "$home/fakebin/tasks-axi"

  XO_CAPTAIN_HOLD_NOW=2026-07-14T12:00:00Z run_captain "$home" hold sample-old-call \
  ...
  if XO_CAPTAIN_HOLD_NOW=2026-07-14T12:00:00Z run_captain "$home" hold sample-stamp-failure \
...
## Done
EOF
  XO_CAPTAIN_HOLD_NOW=2026-07-14T12:00:00Z run_captain "$home" hold sample-interrupted-call \
    --reason "captain route choice pending" >/dev/null \
...
if [ "${1:-}" = done ] && [ "${2:-}" = sample-interrupted-call ] \
  && [ ! -e "$XO_HOME/close-failed-once" ]; then
  cp "$XO_HOME/data/backlog.md" "$XO_HOME/at-close/backlog.md" || exit 93
  : > "$XO_HOME/close-failed-once"
  exit 92
fi
if [ "${1:-}" = update ] && [ "${2:-}" = sample-interrupted-call ] \
  && [ ! -e "$XO_HOME/normalize-failed-once" ]; then
  state=$("$REAL_TASKS_AXI" show "$2" --full | sed -n 's/^  state: //p' | head -1)
  if [ "$state" = done ]; then
    : > "$XO_HOME/normalize-failed-once"
    exit 94
  fi
fi
exec "$REAL_TASKS_AXI" "$@"
EOF
  ...
  snap=$(PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$home/at-close" XO_CONFIG_OVERRIDE="$home/config" \
    XO_PROJECTS_OVERRIDE="$home/projects" XO_SNAPSHOT_NOW=2026-07-14T12:00:00Z \
    "$ROOT/bin/xo-fleet-snapshot.sh" --json) || fail "fleet snapshot failed at interrupted close boundary"
  printf '%s' "$snap" | jq -e '
  ...
  show=$(tasks_in "$home" show sample-interrupted-call --full)
  assert_contains "$show" 'body: "Resolution recorded by xo-captain-hold.' \
...
## Done
EOF
  XO_CAPTAIN_HOLD_NOW=2026-07-14T12:00:00Z run_captain "$home" hold sample-existing-call \
    --reason "captain choice on existing work" >/dev/null \
    || fail "could not hold the existing task"
  XO_CAPTAIN_HOLD_NOW=2026-07-20T12:00:00Z run_captain "$home" hold sample-existing-call \
    --reason "captain choice on existing work" >/dev/null \
  ...
  snap=$(PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$home/data" XO_CONFIG_OVERRIDE="$home/config" \
    XO_PROJECTS_OVERRIDE="$home/projects" XO_SNAPSHOT_NOW=2026-07-14T12:00:00Z \
    "$ROOT/bin/xo-fleet-snapshot.sh" --json) || fail "fleet snapshot failed"
  printf '%s' "$snap" | jq -e '
  ...
  snap=$(PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$home/data" XO_CONFIG_OVERRIDE="$home/config" \
    XO_PROJECTS_OVERRIDE="$home/projects" XO_SNAPSHOT_NOW=2026-08-01T12:00:00Z \
    "$ROOT/bin/xo-fleet-snapshot.sh" --json) || fail "fleet snapshot failed at the due date"
  printf '%s' "$snap" | jq -e '
    ([.backlog.records[] | select(.id == "sample-later-call")][0]) as $later
    | ([.backlog.records[] | select(.id == "sample-existing-call")][0]) as $existing
...
EOF
  ...
  xo_write_secondmate_meta "$parent/state/sample-mate.meta" "$mate" \
    "xo:xo-sample-mate" sample
...
EOF
  ...
  tasks_in "$mate" add quoted-record-call "Choose quoted record handling" --kind ship --repo sample \
    --body 'Documentation quote: Resolution recorded by xo-captain-hold.' >/dev/null \
...
test_bound_channel_answers_close_at_answer_time() {
  ...
  xo_fake_exit0 "$home/fakebin" lavish-axi
...
EOF
  ...
  cat > "$home/adapter-root/bin/xo-procevent-fixturechan.sh" <<SH
#!/usr/bin/env bash
# Fixture channel: reports keyed captain answers and nothing else.
case "\${1-}" in
  answers) exec "$ROOT/bin/xo-procevent-lavish.sh" answers "\${2-}" ;;
  reconciles) exec "$ROOT/bin/xo-procevent-lavish.sh" reconciles "\${2-}" ;;
esac
exit 2
SH
  chmod +x "$home/adapter-root/bin/xo-procevent-fixturechan.sh"
  run_captain "$home" bind fixture-src >/dev/null \
    || fail "could not bind the fixture channel"
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$home/adapter-root" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/xo-procevent.sh" register fixturechan fixture-src -- cat "$result" >/dev/null \
    || fail "could not register the fixture channel source"
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$home/adapter-root" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_PROCEVENT_CLAIM_ROOT="$home/procevent-claims" \
    "$ROOT/bin/xo-procevent.sh" start fixture-src >/dev/null 2>&1
  assert_absent "$home/state/procevent-inbox/fixture-src.1.handled" \
    "feeding a captain answer retired the notification xo still needs"
...
test_reconcile_closes_with_evidence_or_keeps_the_call_open() {
  ...
  printf 'sample-active-call\n' \
    | XO_CAPTAIN_HOLD_NOW=2026-09-07T06:00:00Z run_captain "$home" reconcile-requests \
        --source-id board-src --source "captured board result sequence 2" >/dev/null \
    || fail "could not create the second reconcile request"
  XO_CAPTAIN_HOLD_NOW=2026-09-07T06:01:00Z run_captain "$home" reconcile note sample-active-call \
...
if [ "${1:-}" = done ] && [ "${2:-}" = sample-reconcile-close-retry ] \
  && [ ! -e "$XO_HOME/reconcile-close-failed" ]; then
  : > "$XO_HOME/reconcile-close-failed"
  exit 92
fi
if [ "${1:-}" = update ] && [ "${2:-}" = sample-reconcile-note-retry ]; then
  "$REAL_TASKS_AXI" "$@" || exit $?
  : > "$XO_HOME/reconcile-note-updated"
  exit 0
fi
if [ "${1:-}" = show ] && [ "${2:-}" = sample-reconcile-note-retry ] \
  && [ -e "$XO_HOME/reconcile-note-updated" ] && [ ! -e "$XO_HOME/reconcile-note-show-failed" ]; then
  : > "$XO_HOME/reconcile-note-show-failed"
  exit 93
fi
exec "$REAL_TASKS_AXI" "$@"
SH
...
# xo to look, and the recorded answer - because each of those was intact
...
test_board_answer_reaches_the_keyed_answer_intake() {
  ...
  xo_test_track_procevent_home "$home" "$home/procevent-claims"
...
prompts[1]{tag,text,prompt}:
  "choice","Take the north route","Context data: {\"schema\":\"xo-bearings-answer.v1\",\"question\":\"sample-board-call\",\"selection\":\"north\",\"note\":\"\"}"
OUT
SH
  chmod +x "$stub"
  ...
  pass "a board answer reaches the keyed-answer intake and wakes xo"
}
...
SH
  ...
  : > "$home/send.log"
  env PATH="$fb:$PATH" XO_ROOT_OVERRIDE="$home" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_SEND_LOG="$home/send.log" XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-send.sh" "$id" --resolve-key chat-choice "go with option A" >/dev/null 2>&1 \
    || fail "an answer to a transferred legacy decision was refused by the chat channel"
  # The answer rides xo-send's durable inbox plane: the record carries the
  ...
  env PATH="$fb:$PATH" XO_ROOT_OVERRIDE="$home" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_SEND_LOG="$home/send.log" XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-send.sh" "$id" --resolve-key sample-chat-followup "take the second option" >/dev/null 2>&1 \
  ...
  env PATH="$fb:$PATH" XO_ROOT_OVERRIDE="$home" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_SEND_LOG="$home/send.log" XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-send.sh" "$id" --resolve-key sample-chat-reconcile reconcile >/dev/null 2>&1
  set -e
  ...
  if env PATH="$fb:$PATH" XO_ROOT_OVERRIDE="$home" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_SEND_LOG="$home/send.log" XO_SEND_SETTLE=0 \
    "$ROOT/bin/xo-send.sh" "$id" --resolve-key sample-chat-followup "again" \
...
run_drain() {  # <home>
  ...
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    ...
    XO_DATA_OVERRIDE="$home/data" XO_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/xo-wake-drain.sh" 2>/dev/null
}
...
EOF
  ...
  printf '%s\n' "$drain" | grep -F 'xo-captain-hold.sh answer' >/dev/null \
...
test_teardown_never_closes_a_captain_held_task() {
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" "$forced" --force \
...
test_retained_row_artifacts_survive_captain_answers() {
  ...
  xo_git_worktree "$repo" "$wt" xo/approved-merge
  ...
  xo_write_meta "$home/state/$approved_id.meta" \
    "window=xo:xo-$approved_id" "endpoint_task_id=$approved_id" "worktree=$wt" \
  ...
  xo_git_worktree "$local_repo" "$local_wt" "xo/$local_id"
  printf 'landed locally\n' > "$local_wt/local.txt"
  git -C "$local_wt" add local.txt
  git -C "$local_wt" -c user.name='XO Tests' -c user.email='tests@example.invalid' \
  ...
  xo_write_meta "$home/state/$local_id.meta" \
    "window=xo:xo-$local_id" "endpoint_task_id=$local_id" "worktree=$local_wt" \
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" "$ROOT/bin/xo-merge-local.sh" "$local_id" \
...
test_interrupted_cleanup_keeps_the_captain_call_recoverable() {
  ...
  xo_write_meta "$home/state/$id.meta" \
    "window=xo:xo-$id" "worktree=$wt" "project=$home/projects/sample" \
...
SH
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" "$id" --force \
  ...
  xo_fake_exit0 "$home/fakebin" treehouse
  bootstrap=$(PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" XO_BOOTSTRAP_NETWORK=skip \
    "$ROOT/bin/xo-bootstrap.sh" 2>&1) \
...
EOF
  ...
  PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$data" XO_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/xo-captain-hold.sh" hold "$id" \
  ...
  PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$data" XO_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/xo-captain-hold.sh" complete "$id" "$id" >/dev/null \
...
SH
  ...
  set +e
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" "$id" --force \
    > "$home/teardown.out" 2> "$home/teardown.err"
  rc=$?
  set -e
  ...
  PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    XO_DATA_OVERRIDE="$data" XO_CONFIG_OVERRIDE="$home/config" \
    "$ROOT/bin/xo-captain-hold.sh" answer "$id" --decision-file "$home/answer.txt" \
  ...
  json=$(PATH="$home/fakebin:$PATH" XO_HOME="$home" XO_DATA_OVERRIDE="$data" \
    XO_BEARINGS_NOW=2026-07-14T12:00:00Z "$BEARINGS" --json) \
...
# the retention must address it the way teardown does, not XO_HOME/data.
...
test_merge_approval_releases_before_zero_done_retention() {
  ...
  xo_git_worktree "$repo" "$wt" xo/zero-retention-merge
  ...
  xo_write_meta "$home/state/$id.meta" \
    "window=xo:xo-$id" "endpoint_task_id=$id" "worktree=$wt" \
    "project=$repo" "harness=codex" "kind=ship" "mode=no-mistakes" \
...
test_pr_merge_entrypoint_refuses_a_captain_held_task() {
  ...
  xo_git_worktree "$repo" "$wt" "xo/$pr_id"
  ...
  xo_write_meta "$home/state/$pr_id.meta" \
    "window=xo:xo-$pr_id" "endpoint_task_id=$pr_id" "worktree=$wt" \
    "project=$repo" "harness=codex" "kind=ship" "mode=no-mistakes" \
...
test_local_merge_entrypoint_refuses_a_captain_held_task() {
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/xo-merge-local.sh" "$local_id" \
...
test_local_merge_entrypoint_separates_an_unreadable_record_from_an_absent_one() {
  ...
  wt="$home/projects/$id"
  xo_git_worktree "$repo" "$wt" "xo/$id"
  printf 'untracked local delivery\n' > "$wt/local.txt"
  git -C "$wt" add local.txt
  git -C "$wt" -c user.name='XO Tests' -c user.email='tests@example.invalid' \
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$ROOT/bin/xo-merge-local.sh" "$id" \
...
test_merge_entrypoints_validate_identity_and_state_before_locking() {
  ...
  set +e
  xo_run_timed 2 env PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" \
    XO_HOME="$home" XO_STATE_OVERRIDE="$pr_state" \
    "$ROOT/bin/xo-pr-merge.sh" sample-missing-pr-state \
  ...
  set +e
  xo_run_timed 2 env PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" \
    XO_HOME="$home" XO_STATE_OVERRIDE="$local_state" \
    "$ROOT/bin/xo-merge-local.sh" sample-missing-local-state \
  ...
  xo_run_timed 2 env PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" \
    XO_HOME="$home" XO_STATE_OVERRIDE="$home/state" \
    "$ROOT/bin/xo-merge-local.sh" "$bad_id" \
...
test_merge_entrypoints_refuse_a_reused_task_incarnation() {
  ...
  xo_git_worktree "$old_repo" "$old_wt" "xo/$id"
  ...
  xo_write_meta "$home/state/$id.meta" \
    "window=xo:xo-$id" "endpoint_task_id=$id" "worktree=$old_wt" \
  ...
  PATH="$home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" XO_TEST_REUSE_TEARDOWN=1 \
    XO_TEST_REUSE_TEARDOWN_ONCE="$home/reuse-teardown-once" \
    XO_TEST_REUSE_TEARDOWN_READY="$teardown_ready" \
    XO_TEST_REUSE_TEARDOWN_RELEASE="$teardown_release" \
    XO_TEST_REAL_PERL="$real_perl" XO_TEST_REAL_SLEEP="$real_sleep" \
  ...
  XO_TEST_REUSE_MERGE=1 XO_TEST_REUSE_MERGE_ONCE="$home/reuse-merge-once" \
    XO_TEST_REUSE_MERGE_READY="$merge_ready" XO_TEST_REUSE_MERGE_RELEASE="$merge_release" \
    XO_TEST_REAL_PERL="$real_perl" XO_TEST_REAL_SLEEP="$real_sleep" \
  ...
  xo_git_worktree "$new_repo" "$new_wt" "xo/$id"
  tasks_in "$home" reopen "$id" >/dev/null || fail "could not reopen the reused PR task"
  xo_write_meta "$home/state/$id.meta" \
    "window=xo:xo-$id" "endpoint_task_id=$id" "worktree=$new_wt" \
  ...
  xo_git_worktree "$local_old_repo" "$local_old_wt" "xo/$local_id"
  printf 'original local delivery\n' > "$local_old_wt/original.txt"
  git -C "$local_old_wt" add original.txt
  git -C "$local_old_wt" -c user.name='XO Tests' \
  ...
  xo_write_meta "$local_home/state/$local_id.meta" \
    "window=xo:xo-$local_id" "endpoint_task_id=$local_id" \
  ...
  PATH="$local_home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$local_home" \
    XO_STATE_OVERRIDE="$local_home/state" XO_DATA_OVERRIDE="$local_home/data" \
    XO_CONFIG_OVERRIDE="$local_home/config" XO_TEST_REUSE_TEARDOWN=1 \
    XO_TEST_REUSE_TEARDOWN_ONCE="$local_home/reuse-teardown-once" \
    XO_TEST_REUSE_TEARDOWN_READY="$local_teardown_ready" \
    XO_TEST_REUSE_TEARDOWN_RELEASE="$local_teardown_release" \
    XO_TEST_REAL_PERL="$real_perl" XO_TEST_REAL_SLEEP="$real_sleep" \
  ...
  PATH="$local_home/fakebin:$PATH" XO_TEST_REUSE_MERGE=1 \
    XO_TEST_REUSE_MERGE_ONCE="$local_home/reuse-merge-once" \
    XO_TEST_REUSE_MERGE_READY="$local_merge_ready" \
    XO_TEST_REUSE_MERGE_RELEASE="$local_merge_release" \
    XO_TEST_REAL_PERL="$real_perl" XO_TEST_REAL_SLEEP="$real_sleep" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$local_home" \
    XO_STATE_OVERRIDE="$local_home/state" XO_DATA_OVERRIDE="$local_home/data" \
    XO_CONFIG_OVERRIDE="$local_home/config" \
    "$ROOT/bin/xo-merge-local.sh" "$local_id" > "$local_home/reuse-merge.out" \
...
#!/usr/bin/env bash
if [ "${1:-}" = -qxF ] && [ "${2:-}" = "pr=${XO_TEST_RACE_PR_URL:-}" ] \
    && [ "${3:-}" = "${XO_TEST_RACE_PR_META:-}" ]; then
  "$XO_TEST_REAL_GREP" "$@" || exit $?
  : > "$XO_TEST_RACE_READY"
  while [ ! -e "$XO_TEST_RACE_RELEASE" ]; do sleep 0.01; done
  exit 0
fi
exec "$XO_TEST_REAL_GREP" "$@"
SH
  chmod +x "$home/fakebin/grep"
  XO_TEST_REAL_GREP="$real_grep" XO_TEST_RACE_PR_URL="$pr" \
    XO_TEST_RACE_PR_META="$home/state/$id.meta" XO_TEST_RACE_READY="$ready" \
    XO_TEST_RACE_RELEASE="$release" run_pr_merge "$home" "$id" "$pr" \
...
#!/usr/bin/env bash
if [ "$*" = "-C ${XO_TEST_RACE_REPO:-} rev-parse --short main" ]; then
  output=$("$XO_TEST_REAL_GIT" "$@") || exit $?
  : > "$XO_TEST_RACE_READY"
  while [ ! -e "$XO_TEST_RACE_RELEASE" ]; do sleep 0.01; done
  printf '%s\n' "$output"
  exit 0
fi
exec "$XO_TEST_REAL_GIT" "$@"
SH
  chmod +x "$local_home/fakebin/git"
  before=$(git -C "$local_repo" rev-parse main)
  PATH="$local_home/fakebin:$PATH" XO_TEST_REAL_GIT="$real_git" \
    XO_TEST_RACE_REPO="$local_repo" XO_TEST_RACE_READY="$local_ready" \
    XO_TEST_RACE_RELEASE="$local_release" XO_ROOT_OVERRIDE="$ROOT" \
    XO_HOME="$local_home" XO_STATE_OVERRIDE="$local_home/state" \
    XO_DATA_OVERRIDE="$local_home/data" XO_CONFIG_OVERRIDE="$local_home/config" \
    "$ROOT/bin/xo-merge-local.sh" "$local_id" \
  ...
  PATH="$local_home/fakebin:$PATH" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$local_home" \
    XO_STATE_OVERRIDE="$local_home/state" XO_DATA_OVERRIDE="$local_home/data" \
    XO_CONFIG_OVERRIDE="$local_home/config" "$TEARDOWN" "$local_id" --force \
...
test_teardown_refuses_a_ship_when_the_captain_hold_cannot_be_read() {
  ...
  xo_write_meta "$home/state/$id.meta" \
    "window=xo:xo-$id" "worktree=$home/projects/missing-$id" \
...
SH
  ...
  set +e
  PATH="$home/fakebin:$PATH" REAL_TASKS_AXI="$TASKS_AXI_BIN" \
    TASKS_AXI_FAIL_SHOW_ID="$id" XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" \
    XO_STATE_OVERRIDE="$home/state" XO_DATA_OVERRIDE="$home/data" \
    XO_CONFIG_OVERRIDE="$home/config" "$TEARDOWN" "$id" --force \
    > "$home/teardown.out" 2> "$home/teardown.err"
  rc=$?
