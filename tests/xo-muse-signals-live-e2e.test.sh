...
xo_live_gate opt-in XO_MUSE_SIGNALS_LIVE muse tmux node

LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-muse-signals.XXXXXX") || fail "could not create the isolated Muse lab"
trap cleanup EXIT
...
"$REAL_TMUX" -L "$SOCKET" new-window -d -t "$SESSION:" -n muse -c "$WORKSPACE" -- \
  ...
  "$MUSE_BIN" --provider echo --yolo "xo Muse signal drift guard" \
  || fail "could not launch Muse with the echo provider"

SESSION_LOG=
for _ in $(seq 1 150); do
  SESSION_LOG=$(xo_busy_muse_matching_logs "$LAB/data/muse/sessions" "$WORKSPACE" 2>/dev/null | head -1)
  [ -z "$SESSION_LOG" ] || break
  sleep 0.2
done
...
RUN_STATE=
for _ in $(seq 1 150); do
  RUN_STATE=$(xo_busy_muse_run_state "$SESSION_LOG" 2>/dev/null || true)
  [ "$RUN_STATE" = busy ] && break
  sleep 0.2
done
[ "$RUN_STATE" = busy ] || fail "xo_busy_muse_run_state never observed the real echo turn in flight"
...
[ "$RUN_STATE" = settled ] || fail "xo_busy_muse_run_state did not settle the real echo turn"
...
for _ in $(seq 1 100); do
  COMPOSER_STATE=$(xo_tmux_composer_state "$TARGET")
