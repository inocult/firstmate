#!/usr/bin/env bash
# tests/xo-secondmate-liveness.test.sh - the session-start secondmate liveness
# guarantee owned by bin/xo-backend.sh's detailed xo_backend_agent_state and
# bin/xo-bootstrap.sh's secondmate_liveness_sweep that acts on it.
#
...
# shell. xo_backend_target_exists only checks pane PRESENCE, so it reports
...
#   - xo_backend_agent_state is the detailed owner that distinguishes alive,
...
#   - xo_backend_agent_alive preserves the older three-state compatibility view.
#   - bin/xo-bootstrap.sh's secondmate_liveness_sweep recovers only dead or
...
#     already did).
#   - The sweep is skipped entirely under XO_BOOTSTRAP_DETECT_ONLY=1 (the
...
TMP_ROOT=$(xo_test_tmproot xo-secondmate-liveness)

# --- unit level: xo_backend_tmux_agent_state --------------------------------
...
test_tmux_agent_state_classifies() {
  ...
  for harness in claude codex opencode grok kimi pi pi-signed pi-launcher Pi; do
    ...
    out=$(PATH="$fb:$BASE_PATH" bash -c '. "$0/bin/xo-backend.sh"; xo_backend_agent_state tmux sess:win' "$ROOT")
  ...
  [ "$(PATH="$fb:$BASE_PATH" bash -c '. "$0/bin/xo-backend.sh"; xo_backend_agent_alive tmux sess:win' "$ROOT")" = unknown ] \
  ...
  out=$(PATH="$fb:$BASE_PATH" bash -c '. "$0/bin/xo-backend.sh"; xo_backend_agent_state tmux sess:xo-sm1' "$ROOT")
  [ "$out" = missing ] || fail "a readable inventory omitting the target should classify as missing, got '$out'"
  [ "$(PATH="$fb:$BASE_PATH" bash -c '. "$0/bin/xo-backend.sh"; xo_backend_agent_alive tmux sess:xo-sm1' "$ROOT")" = dead ] \
  ...
  pass "xo_backend_tmux_agent_state: separates live, dead, missing, ambiguous, and unreadable"
}

test_tmux_agent_state_rejects_malformed_targets_before_probe() {
  local fakebin marker target out
  fakebin=$(xo_fakebin "$TMP_ROOT/tmux-malformed")
...
SH
  ...
  for target in sess sess: :win sess:win:extra; do
    out=$(PATH="$fakebin:$BASE_PATH" XO_TEST_TMUX_MARKER="$marker" \
      bash -c '. "$0/bin/xo-backend.sh"; xo_backend_agent_state tmux "$1"' "$ROOT" "$target")
  ...
  pass "xo_backend_tmux_agent_state: rejects malformed targets before probing tmux"
}

# --- unit level: xo_backend_herdr_agent_state -------------------------------
...
test_herdr_agent_state_preserves_husk_classifier() {
  ...
  for row in 'dead missing' 'no-agent dead' 'live alive' 'unknown unreadable'; do
    pane_state=${row%% *}
    expected=${row#* }
    out=$(XO_TEST_PANE_STATE="$pane_state" bash -c '. "$0/bin/backends/herdr.sh"; xo_backend_herdr_pane_agent_state() { printf "%s" "$XO_TEST_PANE_STATE"; }; xo_backend_herdr_agent_state "sess:p1"' "$ROOT")
    [ "$out" = "$expected" ] || fail "Herdr pane state $pane_state should map to $expected, got '$out'"
  done

  out=$(bash -c '. "$0/bin/backends/herdr.sh"; xo_backend_herdr_agent_state "no-colon-target"' "$ROOT")
  [ "$out" = unreadable ] || fail "an unparseable Herdr target should classify as unreadable, got '$out'"

  out=$(bash -c '. "$0/bin/backends/herdr.sh"; xo_backend_herdr_pane_agent_state() { printf "no-agent"; }; xo_backend_herdr_agent_alive "sess:p1"' "$ROOT")
  [ "$out" = dead ] || fail "the Herdr compatibility view should keep a no-agent husk dead, got '$out'"

  pass "xo_backend_herdr_agent_state: preserves missing/no-agent/live/unknown husk behavior"
}
...
test_agent_state_dispatcher_and_compatibility() {
  ...
  out=$(bash -c '. "$0/bin/xo-backend.sh"; xo_backend_source herdr; xo_backend_herdr_pane_agent_state() { printf "live"; }; xo_backend_agent_state herdr sess:p1' "$ROOT")
  [ "$out" = alive ] || fail "detailed dispatcher should route Herdr, got '$out'"

  out=$(bash -c '. "$0/bin/xo-backend.sh"; xo_backend_agent_state zellij sess:7' "$ROOT")
  [ "$out" = unverified ] || fail "Zellij should remain unverified, got '$out'"
  out=$(bash -c '. "$0/bin/xo-backend.sh"; xo_backend_agent_alive zellij sess:7' "$ROOT")
  [ "$out" = unknown ] || fail "the compatibility dispatcher should map unverified to unknown, got '$out'"

  pass "xo_backend_agent_state: routes tmux/Herdr and keeps Zellij unverified"
}

# --- sweep level: bin/xo-bootstrap.sh's secondmate_liveness_sweep -----------

# make_toolchain <dir>: the fixed set of stubs bin/xo-bootstrap.sh's read-only
# diagnostics need to stay quiet (mirrors tests/xo-secondmate-sync.test.sh's
# make_fake_toolchain), MINUS tmux - callers add their own controllable tmux.
make_toolchain() {
  local dir=$1 fakebin
  fakebin=$(xo_fakebin "$dir")
  xo_fake_exit0 "$fakebin" node chrome-devtools-axi pi-signed
...
# make_liveness_tmux <dir>: a controllable tmux stub. XO_TEST_PANE_CMD may be
...
case "${1:-}" in
  ...
  list-windows)
    case "$mode" in
      missing) printf '%s\n' main; exit 0 ;;
      unreadable) exit 1 ;;
      *) [ -e "${XO_TMUX_CALL_LOG:?}.killed" ] || printf '%s\n' xo-sm1; exit 0 ;;
    esac
    ;;
  new-window|kill-window)
    printf '%s\n' "$*" >> "${XO_TMUX_CALL_LOG:?}"
    [ "${1:-}" = kill-window ] && : > "${XO_TMUX_CALL_LOG}.killed"
    [ "${XO_TEST_FAIL_NEW_WINDOW:-0}" = 1 ] && [ "${1:-}" = new-window ] && exit 1
    [ "${1:-}" = new-window ] && rm -f "${XO_TMUX_CALL_LOG}.killed"
    exit 0
    ;;
  has-session) exit 0 ;;
esac
...
# new_world <name>: a scratch xo HOME (state/, watcher beacon, pinned
# harness) with no kind=secondmate meta yet. XO_ROOT is left to resolve
# naturally to the real checkout under test ($ROOT), exactly as production
# always has it - this sweep's own xo-spawn.sh invocation resolves the
# secondmate harness through $XO_ROOT/bin/xo-harness.sh, which only exists in
...
# which has no xo-spawn.sh launch template.
new_world() {
  local name=$1 w
  w="$TMP_ROOT/$name"
...
run_bootstrap() {  # <fakebin> <home> <pane-cmd> <call-log> [extra env...] -> stdout
  ...
  PATH="$fb:$BASE_PATH" TMUX='' XO_BACKEND=tmux XO_HOME="$home" \
    XO_TEST_PANE_CMD="$cmd" XO_TMUX_CALL_LOG="$log" \
    env "$@" "$ROOT/bin/xo-bootstrap.sh" 2>&1
}

test_sweep_respawns_confirmed_dead_secondmate() {
  local w fb tmuxfb log out
  w=$(new_world sweep-dead)
  add_sm_home "$w" sm1 xo:xo-sm1
  ...
  assert_contains "$(cat "$log")" "kill-window -t =xo:=xo-sm1" \
...
test_sweep_leaves_alive_secondmate_untouched() {
  ...
  out=$(run_bootstrap "$tmuxfb:$fb" "$w/home" claude "$log" XO_BOOTSTRAP_VERBOSE_FACTS=1)
...
test_sweep_respawns_authoritatively_missing_pi_secondmate() {
  ...
  add_sm_home "$w" sm1 xo:xo-sm1 pi
  fb=$(make_toolchain "$w"); tmuxfb=$(make_liveness_tmux "$w")
  log="$w/calls.log"; : > "$log"
...
test_sweep_respawns_authoritatively_missing_pi_signed_secondmate() {
  ...
  add_sm_home "$w" sm1 xo:xo-sm1 pi-signed
  fb=$(make_toolchain "$w"); tmuxfb=$(make_liveness_tmux "$w")
  log="$w/calls.log"; : > "$log"
...
test_sweep_reports_missing_endpoint_relaunch_failure() {
  ...
  out=$(run_bootstrap "$tmuxfb:$fb" "$w/home" missing "$log" XO_TEST_FAIL_NEW_WINDOW=1)
...
test_sweep_never_acts_on_unverified_harness_dead_reading() {
  ...
  add_sm_home "$w" sm1 xo:xo-sm1 custom-agent
  fb=$(make_toolchain "$w"); tmuxfb=$(make_liveness_tmux "$w")
  log="$w/calls.log"; : > "$log"
...
test_sweep_skipped_under_detect_only() {
  ...
  out=$(run_bootstrap "$tmuxfb:$fb" "$w/home" zsh "$log" XO_BOOTSTRAP_DETECT_ONLY=1)
  ...
  pass "sweep: skipped entirely under XO_BOOTSTRAP_DETECT_ONLY=1, exactly like the other mutating sweeps"
}
...
echo "# all xo-secondmate-liveness tests passed"
