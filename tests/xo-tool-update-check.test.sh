#!/usr/bin/env bash
# Tests for xo-tool-update-check.sh, the watched tooling update report.
#
...
CHECK="$ROOT/bin/xo-tool-update-check.sh"
CHECKPOINT="$ROOT/bin/xo-watch-checkpoint.sh"
TMP_ROOT=$(xo_test_tmproot xo-tool-update-check)
...
run_check() {
  ...
  shift 3
  local status=0
  env XO_CHECK_TIMEOUT=30 "$@" XO_HOME="$home" PATH="$path" XO_TOOL_UPDATE_INTERVAL=0 "$CHECK" >"$out" 2>&1 || status=$?
...
test_unusable_announce_pattern_is_reported_not_read_as_silence() {
  ...
  status=0
  XO_HOME="$home" "$CHECK" arm >/dev/null 2>&1 || status=$?
...
SH
  ...
  run_check "$home" "$(fixture_path "$dir")" "$out" XO_TOOL_UPDATE_BUDGET_SECS=2
  report=$(cat "$out")
...
SH
  ...
  write_config "$home" '{"tools":[{"name":"no-mistakes","command":"no-mistakes-fixture","version_args":["--version"],"announce_args":["--help"],"announce_pattern":"A new version of no-mistakes is available: [^ ]+ -> [^ ]+"}]}'
  out="$home/out.txt"
  run_check "$home" "$(fixture_path "$dir")" "$out" XO_TOOL_UPDATE_PROBE_SECS=1
  report=$(cat "$out")
...
test_commits_behind_origin_are_reported() {
  ...
  write_config "$home" "{\"tools\":[{\"name\":\"xo\",\"git\":{\"repo\":\"$work\",\"remote\":\"origin\",\"branch\":\"main\"}}]}"
  out="$home/out.txt"
  run_check "$home" "$PATH" "$out"
  assert_contains "$(cat "$out")" "xo update available: local main is 2 commits behind origin/main" "commits behind the origin branch were not reported"
...
test_default_branch_is_detected_when_branch_is_omitted() {
  ...
  write_config "$home" "{\"tools\":[{\"name\":\"xo\",\"git\":{\"repo\":\"$work\"}}]}"
  out="$home/out.txt"
  run_check "$home" "$PATH" "$out"
  assert_contains "$(cat "$out")" "xo update available: local main is 1 commit behind origin/main" "the default branch was not detected from the remote"
...
test_default_branch_is_asked_of_the_remote_when_the_clone_has_no_record() {
  ...
  out="$home/out.txt"
  run_check "$home" "$PATH" "$out"
  assert_contains "$(cat "$out")" "xo update available: local main is 2 commits behind origin/main" "the default branch was not asked of the remote"
...
test_current_and_ahead_repositories_are_silent() {
  ...
  write_config "$home" "{\"tools\":[{\"name\":\"xo\",\"git\":{\"repo\":\"$work\",\"branch\":\"main\"}}]}"
  out="$home/out.txt"
  run_check "$home" "$PATH" "$out"
...
test_unusable_git_source_is_reported() {
  ...
  write_config "$home" "{\"tools\":[{\"name\":\"xo\",\"git\":{\"repo\":\"$TMP_ROOT/git-broken/not-a-repo\",\"branch\":\"main\"}}]}"
  out="$home/out.txt"
  run_check "$home" "$PATH" "$out"
  assert_contains "$(cat "$out")" "xo check failed" "an unusable git source was not reported"
...
test_unreadable_remote_is_not_reported_as_a_missing_branch() {
  ...
  # wakes xo with a diagnosis that is simply wrong, so the report must
  ...
  run_check "$home" "$PATH" "$out"
  report=$(cat "$out")
  assert_contains "$report" "xo check failed" "a remote that could not be read was not reported"
...
test_missing_branch_on_a_readable_remote_is_still_reported() {
  ...
  write_config "$home" "{\"tools\":[{\"name\":\"xo\",\"git\":{\"repo\":\"$work\",\"remote\":\"origin\",\"branch\":\"release\"}}]}"
  out="$home/out.txt"
  run_check "$home" "$PATH" "$out"
  assert_contains "$(cat "$out")" "xo check failed: origin has no branch release" "a branch the remote does not have was not reported as missing"
...
test_git_probes_stop_when_the_sweep_budget_is_gone() {
  ...
  write_config "$home" "{\"tools\":[{\"name\":\"xo\",\"command\":\"$TOOL\",\"git\":{\"repo\":\"$work\",\"remote\":\"origin\",\"branch\":\"main\"}}]}"
  out="$home/out.txt"
  run_check "$home" "$(fixture_path "$slow")" "$out" XO_TOOL_UPDATE_BUDGET_SECS=1
  report=$(cat "$out")
  assert_contains "$report" "check incomplete: the time budget ran out before xo" "a sweep with no budget left did not say which tool it did not finish"
...
SH
  ...
  # wrong reason.
  run_check "$home" "$(fixture_path "$dir")" "$out" XO_TOOL_UPDATE_PROBE_SECS=3
  report=$(cat "$out")
  assert_not_contains "$report" "update available" "a probe that never answered was reported as an available update"
  assert_contains "$report" "xo check failed: $work did not answer whether it already has" "the stalled object query was not the reported failure"
  [ "$(git -C "$work" rev-parse HEAD)" = "$head_before" ] || fail "the check moved the watched repository's HEAD"
...
SH
  ...
  run_check "$home" "$(fixture_path "$dir")" "$out" XO_TOOL_UPDATE_PROBE_SECS=1
  report=$(cat "$out")
  assert_contains "$report" "xo check failed: $work did not answer whether it is a git repository" "a repository probe that never answered was not reported as such"
...
test_an_overlong_report_says_it_was_cut() {
  ...
  for i in $(seq 1 30); do
    ...
    tools_json="$tools_json{\"name\":\"absent-tool-$i\",\"command\":\"xo-absent-fixture-$i\"}"
  done
...
test_probes_are_skipped_between_intervals() {
  ...
  status=0
  XO_HOME="$home" PATH="$(fixture_path "$dir")" XO_CHECK_TIMEOUT=30 XO_TOOL_UPDATE_INTERVAL=900 XO_TOOL_UPDATE_NOW="$now" \
    "$CHECK" >"$out" 2>&1 || status=$?
  expect_code 0 "$status" "first cadence run exit"
  assert_grep 'xo-tool-updates-v1' "$home/state/.tool-updates" "the first run did not record its sweep"

  # A finding appears, but the interval has not elapsed, so no probe runs.
  make_copy "$dir" "$TOOL" 'no version here'
  status=0
  XO_HOME="$home" PATH="$(fixture_path "$dir")" XO_CHECK_TIMEOUT=30 XO_TOOL_UPDATE_INTERVAL=900 XO_TOOL_UPDATE_NOW="$((now + 300))" \
    "$CHECK" >"$out" 2>&1 || status=$?
  expect_code 0 "$status" "gated cadence run exit"
  [ ! -s "$out" ] || fail "a run inside the interval probed and spoke: $(cat "$out")"

  status=0
  XO_HOME="$home" PATH="$(fixture_path "$dir")" XO_CHECK_TIMEOUT=30 XO_TOOL_UPDATE_INTERVAL=900 XO_TOOL_UPDATE_NOW="$((now + 901))" \
    "$CHECK" >"$out" 2>&1 || status=$?
...
test_an_oversized_budget_is_cut_to_fit_and_reported() {
  ...
  out="$home/out.txt"
  status=0
  env XO_HOME="$home" PATH="$(fixture_path "$stale:$fresh")" XO_TOOL_UPDATE_INTERVAL=0 \
    XO_TOOL_UPDATE_BUDGET_SECS=60 XO_CHECK_TIMEOUT=30 "$CHECK" >"$out" 2>&1 || status=$?
  ...
  status=0
  env XO_HOME="$home" PATH="$(fixture_path "$stale:$fresh")" XO_TOOL_UPDATE_INTERVAL=0 \
    XO_CHECK_TIMEOUT=30 "$CHECK" >"$out" 2>&1 || status=$?
  ...
  status=0
  env XO_HOME="$home" PATH="$(fixture_path "$stale:$fresh")" XO_TOOL_UPDATE_INTERVAL=0 \
    XO_TOOL_UPDATE_BUDGET_SECS=60 XO_CHECK_TIMEOUT=120 "$CHECK" >"$out" 2>&1 || status=$?
...
test_invalid_environment_and_action_refuse() {
  ...
  status=0
  XO_HOME="$home" XO_TOOL_UPDATE_INTERVAL=5 "$CHECK" >/dev/null 2>&1 || status=$?
  expect_code 2 "$status" "too-small interval exit"
  status=0
  XO_HOME="$home" XO_TOOL_UPDATE_PROBE_SECS=0 "$CHECK" >/dev/null 2>&1 || status=$?
  expect_code 2 "$status" "zero probe bound exit"
  status=0
  XO_HOME="$home" XO_TOOL_UPDATE_BUDGET_SECS=999 "$CHECK" >/dev/null 2>&1 || status=$?
  expect_code 2 "$status" "oversized budget exit"
  status=0
  XO_HOME="$home" "$CHECK" sweep >/dev/null 2>&1 || status=$?
  expect_code 2 "$status" "unknown action exit"
  status=0
  XO_HOME="$home" "$CHECK" --help >/dev/null 2>&1 || status=$?
...
test_arm_registers_the_check_and_disarm_removes_it() {
  ...
  write_config "$home" "{\"tools\":[{\"name\":\"herdr\",\"command\":\"$TOOL\"}]}"
  status=0
  XO_HOME="$home" "$CHECK" arm >/dev/null || status=$?
  ...
  assert_grep 'xo-custom-check-v1' "$home/state/tool-updates.check-trust" "the trust binding has the wrong schema"

  # Arming twice must stay valid rather than invalidating its own binding.
  XO_HOME="$home" "$CHECK" arm >/dev/null || fail "arming twice failed"
  assert_grep 'xo-custom-check-v1' "$home/state/tool-updates.check-trust" "re-arming lost the trust binding"

  XO_HOME="$home" "$CHECK" disarm >/dev/null || fail "disarm failed"
...
test_a_failed_registration_leaves_no_unregistered_shim() {
  ...
  # cycle and wakes xo about unauthenticated state checks until someone
...
test_a_failed_rearm_leaves_no_shim_the_trust_binding_lost() {
  ...
  make_copy "$dir" "$TOOL" 'herdr 0.8.2'
  write_config "$home" "{\"tools\":[{\"name\":\"herdr\",\"command\":\"$TOOL\"}]}"
  XO_HOME="$home" "$CHECK" arm >/dev/null || fail "the first arm failed"
  ...
  status=0
  env XO_HOME="$home" PATH="$(fixture_path "$fake")" "$CHECK" arm >/dev/null 2>&1 || status=$?
...
test_arm_resolves_a_relative_home_into_the_shim() {
  ...
  status=0
  (cd "$TMP_ROOT" && XO_HOME=arm-relative "$CHECK" arm >/dev/null 2>&1) || status=$?
  expect_code 0 "$status" "arm with a relative home exit"

  out="$home/out.txt"
  status=0
  (cd / && env -u XO_HOME PATH="$(fixture_path "$stale:$fresh")" XO_CHECK_TIMEOUT=30 XO_TOOL_UPDATE_INTERVAL=0 \
...
test_armed_check_wakes_the_watcher_with_the_skew_report() {
  ...
  make_copy "$fresh" "$TOOL" 'herdr 0.8.2'
  write_config "$home" "{\"tools\":[{\"name\":\"herdr\",\"command\":\"$TOOL\"}]}"
  XO_HOME="$home" "$CHECK" arm >/dev/null || fail "could not arm the watched tool check"

  out="$home/out.txt"
  err="$home/err.txt"
  status=0
  env XO_HOME="$home" PATH="$(fixture_path "$stale:$fresh")" XO_CHECK_TIMEOUT=30 XO_TOOL_UPDATE_INTERVAL=0 \
    XO_POLL=1 XO_SIGNAL_GRACE=1 XO_CHECK_INTERVAL=1 \
