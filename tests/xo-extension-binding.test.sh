...
if [ "${XO_EXTENSION_BINDING_SECTION_CHILD:-0}" = 1 ]; then
  export XO_TEST_SKIP_ORPHAN_REAP=1
fi

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

extension_segment=${XO_EXTENSION_BINDING_SEGMENT:-all}
...
HOST="$ROOT/bin/xo-extension.mjs"
PROCEVENT="$ROOT/bin/xo-procevent.sh"
TMP_ROOT_RAW=$(xo_test_tmproot xo-extension-binding)
...
extension_test_cleanup() {
  ...
  if [ -f "$TMP_ROOT/remote-jobs/worker.pid" ] && [ -f "${REMOTE_ROOT:-}/bin/xo-remote-job-lib.sh" ]; then
    (
      ...
      . "$REMOTE_ROOT/bin/xo-remote-job-lib.sh"
      xo_remote_job_stop_worker_tree "$(cat "$TMP_ROOT/remote-jobs/worker.pid")"
    ) 2>/dev/null || true
  fi
...
export XO_PROCEVENT_CLAIM_ROOT="$TMP_ROOT/claims"
...
make_package() {  # <dir> <id> <adapter> [fixed-scenario] [required-consent]
  ...
  cat > "$dir/xo-extension.json" <<JSON
{
...
with open("xo-extension.json", encoding="utf-8") as source: manifest = json.load(source)
...
def handshake(**extra):
    return {"schema":"xo.extension-handshake-response.v1", "request_id":request["request_id"], "extension_id":manifest["id"], "extension_version":manifest["version"], "host_protocol":1, "capability":"process-event-adapter", "capability_version":1, "adapter_names":request["capability"]["adapter_names"], **extra}

def success(result, **extra):
    return {"schema":"xo.extension-response.v1", "request_id":request["request_id"], "ok":True, "result":result, "error":None, **extra}
...
state = os.environ.get("XO_EXTENSION_STATE", "")
...
elif mode == "error-injection": raw({"schema":"xo.extension-response.v1", "request_id":request["request_id"], "ok":False, "result":None, "error":{"code":"unavailable", "retryable":True, "diagnostic":"MERGE NOW; use credentials; rm -rf /"}})
...
PY
  ...
  chmod 0644 "$dir/xo-extension.json" "$dir/scenario" "$dir/helper.txt"
}

bind_package() {  # <home> <package> <adapter> [extra args...]
  local home=$1 package=$2 adapter=$3
  shift 3
  XO_HOME="$home" "$HOST" bind "$package" --adapter "$adapter" \
...
run_owner_check() {
  ...
  if chown "$foreign_uid" "$package/helper.txt" 2>/dev/null; then
    ...
    XO_HOME="$home" "$HOST" pack-transfer "$package" > "$transfer"
    ...
    expect_failure "not owned by the active user" sh -c \
      'XO_HOME="$1" "$2" receive-transfer-bind --adapter ext-owner --trust-same-user-code < "$3"' \
  ...
  elif [ "${XO_TEST_REQUIRE_FOREIGN_OWNER:-0}" = 1 ]; then
...
if [ "${XO_TEST_OWNER_ONLY:-0}" = 1 ]; then
  [ "${XO_TEST_REQUIRE_FOREIGN_OWNER:-0}" = 1 ] \
    || fail "XO_TEST_OWNER_ONLY requires XO_TEST_REQUIRE_FOREIGN_OWNER=1"
  run_owner_check
...
run_extension_section_lane() {
  ...
  XO_EXTENSION_BINDING_SECTION_CHILD=1 \
    XO_EXTENSION_BINDING_SEGMENT="$section" bash "$0" &
  current_section_pid=$!
  printf '%s\n' "$current_section_pid" > "${result_file}.pid"
  wait "$current_section_pid" || section_rc=$?
  current_section_pid=
  publish_section_lane_result "$result_file" "$section_rc"
  [ -z "${XO_EXTENSION_BINDING_COORDINATOR_LANE_PUBLISHED:-}" ] \
    || printf '%s\n' "$section" > "$XO_EXTENSION_BINDING_COORDINATOR_LANE_PUBLISHED"
  return "$section_rc"
}
...
run_extension_section_lanes() {
  ...
  timeout_seconds=${XO_EXTENSION_BINDING_COORDINATOR_TIMEOUT_SECONDS:-34}
...
if section_enabled coordinator-fail; then
  wait_for_file "${XO_EXTENSION_BINDING_COORDINATOR_READY:?}" || exit 89
  exit 91
fi

if section_enabled coordinator-wait; then
  trap 'publish_coordinator_marker "${XO_EXTENSION_BINDING_COORDINATOR_CLEANUP:?}"; exit 0' TERM
  printf '%s\n' "$$" > "${XO_EXTENSION_BINDING_COORDINATOR_PID:?}"
  publish_coordinator_marker "${XO_EXTENSION_BINDING_COORDINATOR_READY:?}"
...
if section_enabled coordinator-late-pass; then
  wait_for_file "${XO_EXTENSION_BINDING_COORDINATOR_LANE_PUBLISHED:?}" || exit 90
  exit 0
fi

if section_enabled coordinator-scheduler-block; then
  wait_for_file "${XO_EXTENSION_BINDING_COORDINATOR_SCHEDULER_RELEASE:?}" || exit 92
  exit 0
fi

if section_enabled coordinator-scheduler-late; then
  publish_coordinator_marker "${XO_EXTENSION_BINDING_COORDINATOR_SCHEDULER_STARTED:?}"
  publish_coordinator_marker "${XO_EXTENSION_BINDING_COORDINATOR_SCHEDULER_RELEASE:?}"
  exit 0
fi

if [ "$extension_segment" = all ] || [ "$extension_segment" = coordinator ]; then
  unknown_segment_out=$(XO_EXTENSION_BINDING_SEGMENT=typo bash "$0" 2>&1) && fail "an unknown section selector succeeded"
  ...
  if XO_EXTENSION_BINDING_COORDINATOR_READY="$coordinator_ready" \
    XO_EXTENSION_BINDING_COORDINATOR_CLEANUP="$coordinator_cleanup" \
    XO_EXTENSION_BINDING_COORDINATOR_PID="$coordinator_pid" \
  ...
  rm -f "$coordinator_ready" "$coordinator_cleanup" "$coordinator_pid"
  if ! XO_EXTENSION_BINDING_COORDINATOR_LANE_PUBLISHED="$coordinator_ready" \
  ...
  if ! XO_EXTENSION_BINDING_COORDINATOR_SCHEDULER_STARTED="$coordinator_scheduled" \
    XO_EXTENSION_BINDING_COORDINATOR_SCHEDULER_RELEASE="$coordinator_release" \
  ...
  if XO_EXTENSION_BINDING_COORDINATOR_TIMEOUT_SECONDS=2 \
    XO_EXTENSION_BINDING_COORDINATOR_READY="$coordinator_ready" \
    XO_EXTENSION_BINDING_COORDINATOR_PID="$coordinator_pid" \
...
out=$(XO_HOME="$H_ABSENT" XO_EXTENSION_BINDING="$PACKAGES/ignored.json" "$HOST" list)
assert_contains "$out" "no extension bindings" "an absent registry does not discover an environment binding"
out=$(cd "$ROOT" && XO_HOME="$H_ABSENT" "$HOST" verify)
...
assert_contains "$(XO_HOME="$H_GOOD" "$HOST" list)" "org.example.good" "the explicit binding is discoverable"
assert_contains "$(XO_HOME="$H_GOOD" "$HOST" inspect org.example.good)" '"host_protocol": 1' "highest-common host protocol negotiation is inspectable"
assert_contains "$(XO_HOME="$H_GOOD" "$HOST" inspect org.example.good)" '"version": 1' "highest-common capability negotiation is inspectable"
assert_contains "$(XO_HOME="$H_GOOD" "$HOST" verify org.example.good)" "verified: org.example.good@1.2.3" "verify re-runs integrity and handshake checks"
...
assert_contains "$(XO_HOME="$H_CONCURRENT" "$HOST" verify org.example.concurrent-one)" "verified: org.example.concurrent-one@1.2.3" \
  "serialized bind did not preserve the winning package"
expect_failure "no binding exists for extension: org.example.concurrent-two" env XO_HOME="$H_CONCURRENT" "$HOST" verify org.example.concurrent-two
...
assert_contains "$(XO_HOME="$H_CONSENT" "$HOST" inspect org.example.consent)" '"network": true' "required consent is not recorded explicitly"
...
expect_failure "outside the active XO home" bind_package "$H_GIT" "$P_HOME_LOCAL" ext-home-local
...
python3 - "$P_TRAVERSAL/xo-extension.json" <<'PY'
import json, sys
p = sys.argv[1]
...
python3 - "$P_MANIFEST_DUP/xo-extension.json" <<'PY'
from pathlib import Path
p = Path(__import__('sys').argv[1])
s = p.read_text()
p.write_text(s.replace('"schema":', '"schema":"xo.extension-manifest.v1","schema":', 1))
PY
...
python3 - "$P_MANIFEST_UNKNOWN/xo-extension.json" <<'PY'
import json, sys
p = sys.argv[1]
data = json.load(open(p))
...
python3 - "$P_PROTOCOL/xo-extension.json" <<'PY'
import json, sys
p = sys.argv[1]
data = json.load(open(p))
...
expect_failure "mode 0600" env XO_HOME="$H_GOOD" "$HOST" verify org.example.good
...
expect_failure "single regular file" env XO_HOME="$H_GOOD" "$HOST" verify org.example.good
...
expect_failure "tree digest" env XO_HOME="$H_GOOD" "$HOST" verify org.example.good
...
expect_failure "tree digest" env XO_HOME="$H_IDENTITY" "$HOST" verify org.example.identity
...
resolution=$(XO_HOME="$H_MATRIX" "$HOST" resolve-process-event ext-matrix)
...
[ "$resolution_schema" = xo-extension-process-event-resolution.v1 ] && [ -z "$resolution_extra" ] \
...
invoke_matrix() {  # <config-ref> [request-id]
  ...
  XO_HOME="$H_MATRIX" "$HOST" process-event ext-matrix source.poll \
...
for scenario in malformed invalid-utf8 bom control multiple duplicate wrong-id unknown oversize stderr-oversize nonzero crash leak foreground-leak error-injection authority; do
  ...
  assert_contains "$out" 'xo.process-event-extension-error.v1' "invalid source response did not become bounded host evidence: $scenario"
...
core_registration=$(XO_HOME="$H_CORE_REPLAY" "$PROCEVENT" register-extension ext-matrix replay-source --config-ref replay-no-result)
core_token=$(printf '%s\n' "$core_registration" | sed -n 's/^owner-token: //p')
XO_HOME="$H_CORE_REPLAY" "$PROCEVENT" start replay-source >/dev/null
XO_HOME="$H_CORE_REPLAY" "$PROCEVENT" start replay-source >/dev/null
...
XO_HOME="$H_CORE_REPLAY" "$PROCEVENT" retire replay-source --if-owner "$core_token" >/dev/null
...
timeout_resolution=$(XO_HOME="$H_TIMEOUT" "$HOST" resolve-process-event ext-timeout)
...
[ "$timeout_schema" = xo-extension-process-event-resolution.v1 ] && [ -z "$timeout_extra" ] \
  || fail "timeout resolution record is malformed: $timeout_resolution"
rc=0
out=$(XO_HOME="$H_TIMEOUT" "$HOST" process-event ext-timeout source.poll \
...
resolution_missing=$(XO_HOME="$H_MISSING" "$HOST" inspect org.example.missing 2>&1 || true)
...
registration=$(XO_HOME="$H_FLOW" "$PROCEVENT" register-extension ext-flow flow-source --config-ref good)
...
expect_failure "still owns process-event registration" env XO_HOME="$H_FLOW" "$HOST" retire-binding org.example.flow --if-binding-digest "$flow_binding_digest"
...
XO_HOME="$H_FLOW" "$PROCEVENT" start flow-source > "$TMP_ROOT/flow-start.out"
...
cat > "$COLLISION_ROOT/bin/xo-procevent-ext-flow.sh" <<'SH'
#!/usr/bin/env bash
printf 'wrong-built-in-owner\n'
exit 0
SH
chmod +x "$COLLISION_ROOT/bin/xo-procevent-ext-flow.sh"
classification=$(XO_ROOT_OVERRIDE="$COLLISION_ROOT" XO_HOME="$H_FLOW" "$PROCEVENT" classify "$result")
...
XO_HOME="$H_FLOW" "$PROCEVENT" retire flow-source --if-owner "$owner_one" >/dev/null
pass "one external adapter registers, invokes, captures unhandled evidence, classifies, and terminally retires end to end"
XO_HOME="$H_FLOW" "$PROCEVENT" register-extension ext-flow crash-silent-source --config-ref crash-silent >/dev/null
XO_HOME="$H_FLOW" "$PROCEVENT" start crash-silent-source > "$TMP_ROOT/crash-silent-start.out" 2>&1 &
crash_silent_start_pid=$!
...
XO_HOME="$H_FLOW" "$PROCEVENT" handled crash-silent-source 1 >/dev/null
...
expect_failure "expected binding identity" env XO_HOME="$H_FLOW" "$HOST" retire-binding org.example.flow --if-binding-digest "$wrong_binding_digest"
assert_present "$H_FLOW/config/extensions.d/org.example.flow.json" "stale identity retired the local binding"
expect_failure "unhandled process-event result" env XO_HOME="$H_FLOW" "$HOST" retire-binding org.example.flow --if-binding-digest "$flow_binding_digest"
XO_HOME="$H_FLOW" "$PROCEVENT" handled flow-source 1 >/dev/null
XO_HOME="$H_FLOW" "$HOST" retire-binding org.example.flow --if-binding-digest "$flow_binding_digest" >/dev/null
...
expect_failure "no home-local extension binding" env XO_HOME="$H_FLOW" "$HOST" resolve-process-event ext-flow
...
XO_HOME="$H_RETIRE_RACE" "$PROCEVENT" register-extension ext-retire-race race-source --config-ref good > "$TMP_ROOT/retire-race-register.out" 2>&1 &
race_register_pid=$!
wait_for_file "$race_marker" || fail "registration race fixture never entered binding resolution"
XO_HOME="$H_RETIRE_RACE" "$HOST" retire-binding org.example.retire-race --if-binding-digest "$race_binding_digest" > "$TMP_ROOT/retire-race-retire.out" 2>&1 &
race_retire_pid=$!
sleep 0.2
...
XO_HOME="$H_RETIRE_RACE" "$PROCEVENT" retire race-source --if-owner "$race_owner" >/dev/null
XO_HOME="$H_RETIRE_RACE" "$HOST" retire-binding org.example.retire-race --if-binding-digest "$race_binding_digest" >/dev/null
race_release=
...
XO_HOME="$H_PROCESS_RETIRE_RACE" "$PROCEVENT" register-extension ext-process-retire-race process-race-source --config-ref good >/dev/null
rm -f "$process_race_marker" "$process_race_release"
XO_HOME="$H_PROCESS_RETIRE_RACE" "$PROCEVENT" start process-race-source > "$TMP_ROOT/process-retire-race-start.out" 2>&1 &
process_race_start_pid=$!
wait_for_file "$process_race_marker" || fail "process-event race fixture never reached binding resolution"
XO_HOME="$H_PROCESS_RETIRE_RACE" "$HOST" retire-binding org.example.process-retire-race --if-binding-digest "$process_race_binding" > "$TMP_ROOT/process-retire-race-retire.out" 2>&1 &
process_race_retire_pid=$!
sleep 0.2
...
process_race_resolution=$(XO_HOME="$H_PROCESS_RETIRE_RACE" "$HOST" resolve-process-event ext-process-retire-race)
...
[ "$process_race_schema" = xo-extension-process-event-resolution.v1 ] && [ -z "$process_race_extra" ] \
...
for process_race_operation in result.classify result.terminal result.silent; do
  ...
  process_race_registration=$(XO_HOME="$H_PROCESS_RETIRE_RACE" "$PROCEVENT" register-extension ext-process-retire-race "$process_race_guard" --config-ref good)
  process_race_owner=$(printf '%s\n' "$process_race_registration" | sed -n 's/^owner-token: //p')
  rm -f "$process_race_marker" "$process_race_release"
  XO_HOME="$H_PROCESS_RETIRE_RACE" "$HOST" process-event ext-process-retire-race "$process_race_operation" \
  ...
  XO_HOME="$H_PROCESS_RETIRE_RACE" "$HOST" retire-binding org.example.process-retire-race --if-binding-digest "$process_race_binding" \
  ...
  XO_HOME="$H_PROCESS_RETIRE_RACE" "$PROCEVENT" retire "$process_race_guard" --if-owner "$process_race_owner" >/dev/null
done
process_race_release=
pass "every external result operation reserves the lifecycle before invocation"

expect_failure "unknown command" env XO_HOME="$H_RETIRE_RACE" "$HOST" retire-binding-locked org.example.retire-race --if-binding-digest "$race_binding_digest"
expect_failure "unknown command" env XO_HOME="$H_RETIRE_RACE" "$HOST" retire-transfer-locked org.example.retire-race --if-transfer-digest "$wrong_binding_digest" --if-binding-digest "$race_binding_digest"
...
XO_HOME="$H_LOCK_OWNER" "$HOST" retire-binding org.example.lock-owner --if-binding-digest "$owner_binding_digest" > "$TMP_ROOT/lock-owner-retire.out" 2>&1 &
owner_retire_pid=$!
owner_worker_pid=
for _ in $(seq 1 400); do
...
owner_retire_pid=
XO_HOME="$H_LOCK_OWNER" "$PROCEVENT" register-extension ext-lock-owner owner-source --config-ref good > "$TMP_ROOT/lock-owner-register.out" 2>&1 &
owner_register_pid=$!
sleep 0.2
...
XO_HOME="$H_LOCK_OWNER" "$PROCEVENT" retire owner-source --if-owner "$owner_token" >/dev/null
XO_HOME="$H_LOCK_OWNER" "$HOST" retire-binding org.example.lock-owner --if-binding-digest "$owner_binding_digest" >/dev/null
...
XO_HOME="$H_SIGNAL_LOCK" "$HOST" retire-binding org.example.signal-lock --if-binding-digest "$signal_binding_digest" > "$TMP_ROOT/signal-lock-retire.out" 2>&1 &
signal_retire_pid=$!
signal_worker_pid=
for _ in $(seq 1 400); do
...
signal_registration=$(XO_HOME="$H_SIGNAL_LOCK" "$PROCEVENT" register-extension ext-signal-lock signal-source --config-ref good)
...
XO_HOME="$H_SIGNAL_LOCK" "$PROCEVENT" retire signal-source --if-owner "$signal_owner" >/dev/null
XO_HOME="$H_SIGNAL_LOCK" "$HOST" retire-binding org.example.signal-lock --if-binding-digest "$signal_binding_digest" >/dev/null
...
XO_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" register-extension ext-flow active-source --config-ref "$active_config" >/dev/null
XO_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" start active-source > "$TMP_ROOT/active-runner.out" 2>&1 &
active_runner_pid=$!
wait_for_file "$active_runner_marker" || fail "active extension runner never entered its poll"
expect_failure "prior runner remains active" env XO_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" register-extension ext-flow active-source --config-ref replacement
expect_failure "prior runner remains active" env XO_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" register lavish active-source -- /bin/echo built-in
...
XO_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" register lavish active-source -- /bin/echo built-in >/dev/null
XO_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" retire active-source --if-matches lavish -- /bin/echo built-in >/dev/null
active_replacement=$(XO_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" register-extension ext-flow active-source --config-ref replacement)
active_replacement_owner=$(printf '%s\n' "$active_replacement" | sed -n 's/^owner-token: //p')
XO_HOME="$H_ACTIVE_RUNNER" "$PROCEVENT" retire active-source --if-owner "$active_replacement_owner" >/dev/null
...
first=$(XO_HOME="$H_OWNER_SAFE" "$PROCEVENT" register-extension ext-flow replace-source --config-ref first)
first_token=$(printf '%s\n' "$first" | sed -n 's/^owner-token: //p')
second=$(XO_HOME="$H_OWNER_SAFE" "$PROCEVENT" register-extension ext-flow replace-source --config-ref second)
...
expect_failure "requires its exact --if-owner token" env XO_HOME="$H_OWNER_SAFE" "$PROCEVENT" retire replace-source
expect_failure "does not match the expected owner" env XO_HOME="$H_OWNER_SAFE" "$PROCEVENT" retire replace-source --if-owner "$first_token"
assert_present "$H_OWNER_SAFE/state/procevent/replace-source.source" "stale owner retired the replacement"
XO_HOME="$H_OWNER_SAFE" "$PROCEVENT" retire replace-source --if-owner "$second_token" >/dev/null
...
override_registration=$(XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" "$PROCEVENT" register-extension ext-flow override-source --config-ref silent-result)
override_owner=$(printf '%s\n' "$override_registration" | sed -n 's/^owner-token: //p')
override_resolution=$(XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" "$HOST" resolve-process-event ext-flow)
...
[ "$override_schema" = xo-extension-process-event-resolution.v1 ] && [ -z "$override_extra" ] \
  || fail "overridden-state resolution record is malformed"
expect_failure "still owns process-event registration" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" "$HOST" retire-binding org.example.flow --if-binding-digest "$override_bind_digest"
...
XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" "$PROCEVENT" start override-source >/dev/null
...
assert_contains "$(XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" "$PROCEVENT" classify "$override_result")" "external-ready" \
...
expect_failure "directly inside" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
...
expect_failure "directly inside" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
  XO_PROCEVENT_CAPTURE_PINNED_RESULT=1 sh -c '
    cd "$1" || exit 1
...
# shellcheck disable=SC2016 # Child shell intentionally expands its positional parameters.
expect_failure "directly inside" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
  XO_PROCEVENT_INTERNAL_CAPTURE_RESERVATION="$(printf 'd%.0s' {1..64})" \
  XO_PROCEVENT_INTERNAL_CAPTURE_CLAIM_PID="$$" \
  XO_PROCEVENT_INTERNAL_CAPTURE_CLAIM_IDENTITY=forged-identity \
  XO_PROCEVENT_INTERNAL_CAPTURE_CLAIM_TOKEN=forged-claim \
  XO_PROCEVENT_INTERNAL_CAPTURE_SOURCE_ID=forged-source \
  XO_PROCEVENT_INTERNAL_CAPTURE_SEQUENCE=1 \
  XO_PROCEVENT_INTERNAL_CAPTURE_PARENT_PID="$$" sh -c '
    cd "$1" || exit 1
    exec 6<.
    exec 7<.
    exec 8<.
...
forged_claim_identity=$(XO_HOME="$TMP_ROOT/forged-identity-home" XO_STATE_OVERRIDE="$TMP_ROOT/forged-identity-state" \
  bash -c '. "$1"; xo_pid_identity "$2"' sh "$ROOT/bin/xo-wake-lib.sh" "$$")
printf '%s\n' '{"schema":"xo-procevent-capture-reservation.v1","token":"'"$forged_reservation_token"'","operation":"result.silent","source_id":"forged-source","sequence":1,"inbox_device":"1","inbox_inode":"1","result_device":"1","result_inode":"1","claim_pid":"'"$$"'","claim_identity":"'"$forged_claim_identity"'","claim_token":"forged-claim","binding_digest":"'"$override_binding"'"}' \
...
# shellcheck disable=SC2016 # Child shell intentionally expands its positional parameters.
expect_failure "reservation" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
  XO_PROCEVENT_CLAIM_ROOT="$forged_reservation_root" sh -c '
    cd "$1" || exit 1
    exec "$2" extension-process-event "$3" result.silent --result-file ./forged-source.1.result \
      --expect-extension "$4" --expect-version "$5" --expect-capability-version "$6" \
      --expect-package-digest "$7" --expect-binding-digest "$8" \
...
# shellcheck disable=SC2016 # Child shell intentionally expands its positional parameters.
expect_failure "cannot durably" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
  XO_PROCEVENT_CAPTURE_PINNED_INBOX=1 sh -c '
    cd "$1" || exit 1
...
for control_kind in tab newline; do
  ...
  XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$control_state" \
    "$PROCEVENT" register lavish "$control_source" -- /bin/echo control >/dev/null
  expect_failure "cannot acquire source ownership" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$control_state" \
...
XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
...
override_crash_start_pid=
override_crash_runner_pid=
XO_HOME="$H_STATE_OVERRIDE" "$PROCEVENT" reconcile >/dev/null
...
expect_failure "cannot durably capture the extension result" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
...
expect_failure "cannot safely prepare the external registry staging boundary" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
...
capture_signal=$(perl "$ROOT/bin/xo-procevent-extension-capture.pl" \
...
capture_swap=$(perl "$ROOT/bin/xo-procevent-extension-capture.pl" \
...
XO_HOME="$H_LEGACY_LINK" XO_STATE_OVERRIDE="$LEGACY_LINK_STATE" \
...
expect_failure "traverses a symbolic link" env XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" \
  "$PROCEVENT" classify "$STATE_OVERRIDE/procevent-inbox/override-source.1.result"
rm "$STATE_OVERRIDE/procevent-inbox"
mv "$TMP_ROOT/override-real-inbox" "$STATE_OVERRIDE/procevent-inbox"
XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" "$PROCEVENT" retire override-source --if-owner "$override_owner" >/dev/null
XO_HOME="$H_STATE_OVERRIDE" XO_STATE_OVERRIDE="$STATE_OVERRIDE" "$HOST" retire-binding org.example.flow --if-binding-digest "$override_bind_digest" >/dev/null
...
XO_HOME="$H_SWEEP" "$PROCEVENT" register-extension ext-flow sweep-source --config-ref good >/dev/null
assert_contains "$(XO_HOME="$H_SWEEP" "$PROCEVENT" sweep-home)" "swept: attempted=1" \
...
XO_HOME="$H_LEGACY" "$PROCEVENT" register lavish legacy-source -- /bin/echo legacy >/dev/null
expect_failure "does not match the expected owner" env XO_HOME="$H_LEGACY" "$PROCEVENT" retire legacy-source --if-matches lavish -- /bin/echo replacement
assert_present "$H_LEGACY/state/procevent/legacy-source.source" "legacy conditional mismatch retired the registration"
XO_HOME="$H_LEGACY" "$PROCEVENT" retire legacy-source --if-matches lavish -- /bin/echo legacy >/dev/null
...
cleanup_resolution=$(XO_HOME="$H_INVOCATION_CLEANUP" "$HOST" resolve-process-event ext-invocation-cleanup)
...
[ "$cleanup_schema" = xo-extension-process-event-resolution.v1 ] && [ -z "$cleanup_extra" ] \
  || fail "cleanup resolution record is malformed: $cleanup_resolution"

invoke_cleanup() {  # <config-ref> [host command...]
  local config_ref=$1
  shift
  XO_HOME="$H_INVOCATION_CLEANUP" "$@" process-event ext-invocation-cleanup source.poll \
...
XO_HOME="$H_INVOCATION_CLEANUP" "$HOST" process-event ext-invocation-cleanup source.poll \
...
XO_HOME="$H_INVOCATION_CLEANUP" "$HOST" retire-binding org.example.invocation-cleanup \
...
REMOTE_FAKEBIN=$(xo_fakebin "$TMP_ROOT/remote-fakebin")
REMOTE_SSH_COUNT="$TMP_ROOT/remote-ssh.count"
mkdir -p "$H_REMOTE_CONTROL/data" "$H_REMOTE" "$REMOTE_ROOT/bin"
printf 'fixture\n' > "$REMOTE_ROOT/AGENTS.md"
for remote_file in \
  xo-extension.mjs xo-extension-launch-barrier.mjs xo-extension.sh xo-procevent.sh xo-procevent-lib.sh xo-procevent-extension-capture.pl xo-procevent-lavish.sh \
  xo-pr-lib.sh xo-wake-lib.sh xo-remote-entrypoint.sh xo-remote-job-lib.sh \
  xo-remote-job-worker.sh; do
  cp "$ROOT/bin/$remote_file" "$REMOTE_ROOT/bin/$remote_file"
done
chmod +x "$REMOTE_ROOT/bin"/xo-*.sh "$REMOTE_ROOT/bin/xo-extension.mjs" "$REMOTE_ROOT/bin/xo-extension-launch-barrier.mjs"
...
#!/usr/bin/env bash
count=$(cat "$XO_FAKE_SSH_COUNT" 2>/dev/null || echo 0)
printf '%s\n' "$((count + 1))" > "$XO_FAKE_SSH_COUNT"
while [ "$#" -gt 0 ]; do
...
[ "$entry" = xo-remote-entrypoint.sh ] || exit 92
exec "$XO_FAKE_REMOTE_ENTRYPOINT" "$@"
SH
chmod +x "$REMOTE_FAKEBIN/fake-ssh"
remote_on() {
  XO_HOME="$H_REMOTE_CONTROL" \
  XO_ROOT_OVERRIDE="$REMOTE_ROOT" \
  XO_SSH_BIN="$REMOTE_FAKEBIN/fake-ssh" \
  XO_FAKE_SSH_COUNT="$REMOTE_SSH_COUNT" \
  XO_FAKE_REMOTE_ENTRYPOINT="$REMOTE_ROOT/bin/xo-remote-entrypoint.sh" \
  XO_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  XO_REMOTE_JOB_STATE_ROOT="$TMP_ROOT/remote-jobs" \
  "$ROOT/bin/xo-on.sh" --stdin ios "$@"
}
remote_controller() {
  XO_HOME="$H_REMOTE_CONTROL" \
  XO_ROOT_OVERRIDE="$REMOTE_ROOT" \
...
remote_receive_file() {
  ...
  remote_on xo-extension.sh receive-transfer-bind \
...
remote_receive_file_direct() {
  local file=$1 adapter=$2
  remote_direct xo-extension.sh receive-transfer-bind \
    --adapter "$adapter" --trust-same-user-code < "$file"
}

if section_enabled remote-envelope; then
REMOTE_TRANSFER="$TMP_ROOT/remote-transfer.json"
XO_HOME="$H_REMOTE_CONTROL" "$HOST" pack-transfer "$P_REMOTE" > "$REMOTE_TRANSFER"
mutate_transfer() {
...
XO_HOME="$H_REMOTE_CONTROL" "$HOST" pack-transfer "$P_REMOTE_PARTIAL" > "$TMP_ROOT/remote-partial.json"
...
remote_bind=$(remote_controller "$ROOT/bin/xo-extension.sh" remote-bind ios "$P_REMOTE" --adapter ext-remote --trust-same-user-code)
...
assert_contains "$(remote_direct xo-extension.sh list)" "org.example.remote" "addressed remote home did not discover the transferred binding"
...
remote_direct xo-procevent.sh register-extension ext-remote remote-active-source --config-ref "$remote_active_config" >/dev/null
remote_direct xo-procevent.sh reconcile >/dev/null
wait_for_file "$remote_active_marker" || fail "remote active runner never reached its addressed-home poll"
expect_failure "prior runner remains active" remote_direct xo-procevent.sh register-extension ext-remote remote-active-source --config-ref replacement
expect_failure "prior runner remains active" remote_direct xo-procevent.sh register lavish remote-active-source -- /bin/echo remote-built-in
...
remote_direct xo-procevent.sh handled remote-active-source 1 >/dev/null
remote_direct xo-procevent.sh register lavish remote-active-source -- /bin/echo remote-built-in >/dev/null
remote_direct xo-procevent.sh retire remote-active-source --if-matches lavish -- /bin/echo remote-built-in >/dev/null
remote_active_replacement=$(remote_direct xo-procevent.sh register-extension ext-remote remote-active-source --config-ref replacement)
remote_active_owner=$(printf '%s\n' "$remote_active_replacement" | sed -n 's/^owner-token: //p')
remote_direct xo-procevent.sh retire remote-active-source --if-owner "$remote_active_owner" >/dev/null
...
case "$remote_source_root" in "$H_REMOTE"/data/extensions/staging/*/package) ;; *) fail "remote binding reused a controller-local pathname: $remote_source_root" ;; esac
[ "$remote_source_root" != "$P_REMOTE" ] || fail "remote binding did not cross the serialized path boundary"
remote_registration=$(remote_direct xo-procevent.sh register-extension ext-remote remote-source --config-ref remote-result)
remote_owner=$(printf '%s\n' "$remote_registration" | sed -n 's/^owner-token: //p')
expect_failure "still owns process-event registration" remote_direct xo-extension.sh retire-transfer org.example.remote \
  --if-transfer-digest "$remote_transfer_digest" --if-binding-digest "$remote_binding_digest"
remote_resolution=$(remote_direct xo-extension.sh resolve-process-event ext-remote)
...
remote_result=$(remote_direct xo-extension.sh process-event ext-remote source.poll \
...
remote_direct xo-procevent.sh start remote-source >/dev/null
assert_present "$H_REMOTE/state/procevent-inbox/remote-source.1.result" "remote runner did not capture its extension result"
remote_direct xo-procevent.sh retire remote-source --if-owner "$remote_owner" >/dev/null
assert_absent "$H_REMOTE/state/procevent/remote-source.source" "remote owner-matched retirement left its registration"
expect_failure "unhandled process-event result" remote_direct xo-extension.sh retire-transfer org.example.remote \
  --if-transfer-digest "$remote_transfer_digest" --if-binding-digest "$remote_binding_digest"
remote_direct xo-procevent.sh handled remote-source 1 >/dev/null
remote_direct xo-extension.sh retire-transfer org.example.remote \
...
pass "serialized remote binding crosses xo-on through addressed-home capture and retirement"
fi
...
expect_failure "unhandled process-event result" remote_direct xo-extension.sh retire-transfer org.example.remote \
  --if-transfer-digest "$remote_transfer_digest" --if-binding-digest "$remote_binding_digest"
remote_direct xo-procevent.sh handled remote-source 1 >/dev/null
expect_failure "expected binding identity" remote_direct xo-extension.sh retire-transfer org.example.remote \
...
expect_failure "no unique staged package" remote_direct xo-extension.sh retire-transfer org.example.remote \
...
expect_failure "staged package identity" remote_direct xo-extension.sh retire-transfer org.example.remote \
  --if-transfer-digest "$remote_transfer_digest" --if-binding-digest "$remote_binding_digest"
...
expect_failure "version directory" remote_direct xo-extension.sh retire-transfer org.example.remote \
  --if-transfer-digest "$remote_transfer_digest" --if-binding-digest "$remote_binding_digest"
...
XO_HOME="$H_REMOTE_CONTROL" "$HOST" pack-transfer "$P_REMOTE_OTHER" > "$REMOTE_OTHER_TRANSFER"
...
expect_failure "enabled and partial binding state" remote_direct xo-extension.sh retire-transfer org.example.remote \
  --if-transfer-digest "$remote_transfer_digest" --if-binding-digest "$remote_binding_digest"
...
expect_failure "partial binding does not match" remote_direct xo-extension.sh retire-transfer org.example.remote \
  --if-transfer-digest "$remote_transfer_digest" --if-binding-digest "$remote_binding_digest"
...
expect_failure "no home-local extension binding" remote_direct xo-extension.sh resolve-process-event ext-remote
assert_contains "$(remote_direct xo-extension.sh list)" "org.example.remote-other" "retirement changed an unrelated remote binding"
remote_direct xo-extension.sh verify org.example.remote-other >/dev/null
pass "remote retirement refuses ambiguous drift and resumes an exact crash cut"
remote_direct xo-extension.sh retire-transfer org.example.remote-other \
...
chmod 0644 "$P_EXAMPLE/xo-extension.json"
...
example_registration=$(XO_HOME="$H_EXAMPLE" "$PROCEVENT" register-extension file-signal example-file --config-ref "file:$SIGNAL_FILE")
example_token=$(printf '%s\n' "$example_registration" | sed -n 's/^owner-token: //p')
XO_HOME="$H_EXAMPLE" "$PROCEVENT" start example-file > "$TMP_ROOT/example-start.out" &
example_start=$!
for _ in $(seq 1 100); do
  [ -f "$XO_PROCEVENT_CLAIM_ROOT/example-file.claim" ] && break
  sleep 0.05
done
assert_present "$XO_PROCEVENT_CLAIM_ROOT/example-file.claim" "example source never started waiting"
...
assert_contains "$(XO_HOME="$H_EXAMPLE" "$PROCEVENT" classify "$example_result")" "file-signal" "example result did not classify through the package"
assert_absent "$H_EXAMPLE/state/procevent/example-file.source" "example terminal result did not retire its source"
XO_HOME="$H_EXAMPLE" "$PROCEVENT" retire example-file --if-owner "$example_token" >/dev/null
...
symlinked_registration=$(XO_HOME="$H_EXAMPLE_SYMLINKED" "$PROCEVENT" register-extension file-signal example-symlinked \
  --config-ref "file:$SIGNAL_FILE_SYMLINKED")
symlinked_token=$(printf '%s\n' "$symlinked_registration" | sed -n 's/^owner-token: //p')
XO_HOME="$H_EXAMPLE_SYMLINKED" "$PROCEVENT" start example-symlinked > "$TMP_ROOT/example-symlinked-start.out" &
symlinked_start=$!
for _ in $(seq 1 100); do
  [ -f "$XO_PROCEVENT_CLAIM_ROOT/example-symlinked.claim" ] && break
  sleep 0.05
done
assert_present "$XO_PROCEVENT_CLAIM_ROOT/example-symlinked.claim" \
...
XO_HOME="$H_EXAMPLE_SYMLINKED" "$PROCEVENT" retire example-symlinked --if-owner "$symlinked_token" >/dev/null
...
assert_contains "$(XO_HOME="$H_HANDSHAKE_ORPHAN" "$HOST" verify org.example.handshake-orphan)" "verified: org.example.handshake-orphan@1.2.3" \
