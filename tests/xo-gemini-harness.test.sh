...
# shellcheck source=bin/xo-gemini-lib.sh
. "$ROOT/bin/xo-gemini-lib.sh"

HARNESS="$ROOT/bin/xo-harness.sh"
TMP_ROOT=$(xo_test_tmproot xo-gemini-harness)
...
test_gemini_marker_outranks_inherited_claudecode() {
  ...
  pass "xo-harness.sh: gemini's marker outranks an inherited CLAUDECODE"
}
...
test_gemini_does_not_claim_inherited_ai_agent() {
  ...
  pass "xo-harness.sh: an inherited AI_AGENT never claims the gemini identity"
}
...
test_gemini_ancestry_matches_only_a_native_command_name() {
  ...
  fakebin=$(xo_fakebin "$TMP_ROOT/anc-native")
  cat > "$fakebin/ps" <<'SH'
#!/usr/bin/env bash
case "$*" in
...
SH
  ...
  pass "xo-harness.sh: ancestry detects a natively-named gemini command"
}

test_gemini_ancestry_rejects_unrelated_mentions() {
  local fakebin out
  fakebin=$(xo_fakebin "$TMP_ROOT/anc-negatives")
  cat > "$fakebin/ps" <<'SH'
#!/usr/bin/env bash
case "$*" in
  *"comm="*) printf '%s\n' "${FAKE_PS_COMM:?}"; exit 0 ;;
...
SH
  ...
  pass "xo-harness.sh: ancestry rejects unrelated gemini mentions"
}
...
const r = spawnSync(process.env.XO_HARNESS_BIN, { env, encoding: 'utf8' });
process.stdout.write(r.stdout || '');
JS
    out=$(XO_HARNESS_BIN="$HARNESS" node "$dir/gemini" 2>/dev/null | tr -d '\n')
    [ "$out" = gemini ] \
      || fail "where node reports comm=node, a gemini script path must detect gemini, got '$out'"
    pass "xo-harness.sh: this platform's node reports comm=node and ancestry reaches gemini"
    return 0
  fi
...
JS
  ...
  out=$(XO_HARNESS_BIN="$HARNESS" GEMINI_CLI=1 node -e '
const { spawnSync } = require("child_process");
const r = spawnSync(process.env.XO_HARNESS_BIN, { encoding: "utf8" });
process.stdout.write(r.stdout || "");' 2>/dev/null | tr -d '\n')
  [ "$out" = gemini ] \
    || fail "GEMINI_CLI must identify a node-bundle gemini worker, got '$out'"
  pass "xo-harness.sh: the node bundle is marker-detected, never ancestry-detected"
}
...
test_gemini_process_identity_reads_the_script_argument() {
  ...
  xo_gemini_args_are_gemini 'node /home/u/.local/bin/gemini -y' \
    || fail "the installed launcher shape must be recognized as gemini"
  xo_gemini_args_are_gemini '/home/u/.local/node/bin/node --max-old-space-size=10000 /home/u/.local/bin/gemini --skip-trust -y' \
    || fail "node option flags before the script must be skipped"
  xo_gemini_args_are_gemini '/opt/node/lib/node_modules/@google/gemini-cli/bundle/gemini.js' \
    || fail "the published package tree must be recognized as gemini"
  xo_gemini_args_are_gemini 'gemini -y' \
  ...
  ! xo_gemini_args_are_gemini '/home/u/.local/node/bin/node' \
    || fail "a bare node process must not be claimed as gemini"
  ! xo_gemini_args_are_gemini 'node /home/u/app/server.js' \
  ...
  ! xo_gemini_args_are_gemini 'node /home/u/app/server.js --model gemini' \
    || fail "a later flag value naming gemini must not claim the identity"
  ! xo_gemini_args_are_gemini 'tail -f /var/log/gemini.log' \
    || fail "an unrelated command reading a gemini-named file must not match"
  # A directory component alone is not evidence.
  ! xo_gemini_args_are_gemini 'node /home/u/gemini/other.js' \
    || fail "a gemini directory component alone must not claim the identity"
  pass "xo-gemini-lib.sh: identity comes from the script argument, never a bare interpreter"
}
...
JS
  "$dir/node" "$dir/gemini" &
  pid=$!
  while ! xo_gemini_pid_is_gemini "$pid"; do
    attempts=$((attempts + 1))
  ...
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  pass "xo-gemini-lib.sh: process argv preserves whitespace in interpreter and Gemini script paths"
}

test_gemini_control_mechanics_are_the_verified_ones() {
  local out
  xo_control_harness_supported gemini || fail "gemini must be a supported control harness"
  out=$(xo_control_harness_family gemini-0.58.0)
  [ "$out" = gemini ] || fail "a recorded gemini* harness must resolve to gemini, got '$out'"
  out=$(xo_control_interrupt_key gemini)
  [ "$out" = Escape ] || fail "gemini interrupts on Escape, got '$out'"
  out=$(xo_control_interrupt_repeat gemini)
  [ "$out" = 1 ] || fail "gemini interrupts on a single press, got '$out'"
  out=$(xo_control_interrupt_clear_key gemini)
  [ -z "$out" ] || fail "gemini needs no composer clear key, got '$out'"
  out=$(xo_control_exit_command gemini)
  [ "$out" = /quit ] || fail "gemini exits with /quit, got '$out'"
  pass "xo-control-lib.sh: gemini carries its verified interrupt and exit mechanics"
}

test_gemini_is_crewmate_and_scout_only() {
  xo_control_harness_supports_kind gemini ship \
    || fail "gemini must be verified for ship work"
  xo_control_harness_supports_kind gemini scout \
    || fail "gemini must be verified for scout work"
  ! xo_control_harness_supports_kind gemini secondmate \
    || fail "gemini has no primary supervision protocol and must be refused for secondmates"
  pass "xo-control-lib.sh: gemini is a crewmate/scout adapter only"
}

test_gemini_wiring_stays_outside_the_worktree() {
  local out
  out=$(xo_control_harness_wiring_paths gemini /wt /state task-1)
  [ "$out" = "/state/task-1.gemini-settings.json" ] \
    || fail "gemini's per-task wiring is its xo-owned settings file, got '$out'"
  case "$out" in
    /wt/*) fail "gemini must never claim a path inside the worktree: '$out'" ;;
  esac
  pass "xo-control-lib.sh: gemini's wiring stays outside the project worktree"
}
