#!/usr/bin/env bash
# tests/xo-backend-autodetect-smoke.test.sh - real herdr smoke test for runtime
# backend AUTO-DETECTION (bin/xo-backend.sh's xo_backend_detect, wired into
# xo_backend_name between config/backend and the tmux default).
#
# Unlike tests/xo-backend-herdr.test.sh (fake herdr CLI) and
# tests/xo-backend-herdr-smoke.test.sh (real herdr, adapter primitives called
# directly), this suite drives the REAL bin/xo-spawn.sh and bin/xo-teardown.sh
# end to end, because auto-detection is an xo-spawn-TIME decision, not an
# adapter primitive - it has to be proven where xo_backend_name is actually
# called. The real spawn runs in a helper-provisioned, per-run named Herdr lab
# session, with a scratch XO_HOME and scratch local-only project. Concurrent
...
# is covered as a fast, deterministic fake-tmux xo-spawn.sh test in
# tests/xo-backend.test.sh (test_spawn_autodetect_nesting_resolves_tmux_silently).
...
# by xo_backend_detect's own unit coverage plus that fake-tmux xo-spawn test.
#
# Safety (2026-07-02 incident): every test-owned Herdr operation goes through
# bin/xo-herdr-lab.sh, which appends the named session flag and verifies the
...
# The old xo-spawn bug that originally motivated this fixture shape was fixed in
# xo-spawn-symlink-guard-s8: xo-spawn.sh now normalizes PROJ_ABS and observed
# backend cwd reads before the worktree-discovery comparison.
# The dedicated regression is
# tests/xo-backend.test.sh:test_spawn_symlinked_project_prefix_avoids_false_refusal.
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/xo-backend-autodetect-smoke.XXXXXX")
HERDR_LAB_HELPER="$ROOT/bin/xo-herdr-lab.sh"
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name xo-autodetect-smoke-concurrency-h3) || {
  rm -rf "$TMP_ROOT"
...
# --- scratch world: XO_HOME with NO backend config, one throwaway project ---
...
git -C "$PROJ" -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm initial
...
env -u TMUX -u XO_BACKEND PATH="$PATH" HERDR_ENV=1 \
  XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$STATE" XO_DATA_OVERRIDE="$DATA" \
  XO_CONFIG_OVERRIDE="$CONFIG" XO_PROJECTS_OVERRIDE="$TMP_ROOT/unused-projects" \
  XO_SPAWN_NO_GUARD=1 \
  "$ROOT/bin/xo-spawn.sh" "$ID" "$PROJ" "sh -c 'echo autodetect-smoke-ok'" --mode no-mistakes --yolo off \
  >"$OUT_FILE" 2>"$ERR_FILE"
status=$?
[ "$status" -eq 0 ] || fail "xo-spawn.sh did not succeed auto-detecting herdr"$'\n'"--- stdout ---"$'\n'"$(cat "$OUT_FILE")"$'\n'"--- stderr ---"$'\n'"$(cat "$ERR_FILE")"

assert_contains_local "$(cat "$ERR_FILE")" "NOTICE" \
  "xo-spawn.sh did not print the auto-detect notice to stderr when selecting herdr"
assert_contains_local "$(cat "$ERR_FILE")" "EXPERIMENTAL herdr backend" \
  "xo-spawn.sh's auto-detect notice did not flag herdr as experimental"
pass "real herdr: xo-spawn.sh auto-detects herdr from HERDR_ENV=1 (no explicit config) and prints the loud notice"

META="$STATE/$ID.meta"
[ -f "$META" ] || fail "xo-spawn.sh did not write a meta file for $ID"
...
XO_ROOT_OVERRIDE="$ROOT" XO_STATE_OVERRIDE="$STATE" XO_DATA_OVERRIDE="$DATA" \
  ...
  "$ROOT/bin/xo-teardown.sh" "$ID" >"$TEARDOWN_OUT" 2>&1
status=$?
[ "$status" -eq 0 ] || fail "xo-teardown.sh failed for the auto-detected herdr task"$'\n'"$(cat "$TEARDOWN_OUT")"
[ -f "$META" ] && fail "xo-teardown.sh did not remove $META"
if "$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" pane get "$PANE" >/dev/null 2>&1; then
  fail "xo-teardown.sh did not close the auto-detected herdr pane"
fi
WT=
