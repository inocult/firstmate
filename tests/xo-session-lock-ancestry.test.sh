#!/usr/bin/env bash
# tests/xo-session-lock-ancestry.test.sh - session-lock harness identity
# (bin/xo-session-lock-lib.sh).
#
...
# shellcheck disable=SC2016 # single quotes are deliberate: $XO_HOME and $$ expand inside the fixture child
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(xo_test_tmproot xo-session-lock-ancestry)
xo_git_identity fmtest fmtest@example.invalid

LIB="$ROOT/bin/xo-session-lock-lib.sh"
...
FAKEBIN=$(xo_fakebin "$TMP_ROOT/harness-bin")
...
case "$pid:$field:${XO_TEST_CLAUDE_SHAPE:-linux}" in
  ...
  700:ppid=:*) printf '%s\n' 1 ;;
  *:comm=:*) printf '%s\n' bash ;;
  *:args=:*) printf '%s\n' 'bash /repo/bin/xo-claude-stop-autoarm.sh' ;;
  *:ppid=:*) printf '%s\n' 700 ;;
esac
SH
  chmod +x "$fakebin/ps"
  ...
  for shape in linux macos; do
    got=$(XO_TEST_CLAUDE_SHAPE="$shape" lib_eval "$fakebin" 'xo_harness_ancestry_pid') \
    ...
    XO_TEST_CLAUDE_SHAPE="$shape" lib_eval "$fakebin" 'xo_harness_pid_alive 700' \
      || fail "$shape: a live version-named session was not recognized as a harness"
    XO_TEST_CLAUDE_SHAPE="$shape" lib_eval "$fakebin" "xo_session_lock_owned_by_self '$dir/state'" \
...
case "$pid:$field:${XO_TEST_PATH_SHAPE:-hookdir}" in
...
SH
  ...
  for shape in hookdir piprefix; do
    if XO_TEST_PATH_SHAPE="$shape" lib_eval "$fakebin" 'xo_harness_ancestry_pid'; then
      fail "$shape: an ordinary script path was treated as a harness process"
    fi
    if XO_TEST_PATH_SHAPE="$shape" lib_eval "$fakebin" 'xo_harness_pid_alive 810'; then
      fail "$shape: an ordinary script path passed the harness-liveness predicate"
    fi
    if XO_TEST_PATH_SHAPE="$shape" lib_eval "$fakebin" "xo_session_lock_owned_by_self '$dir/state'"; then
...
SH
  ...
  got=$(lib_eval "$fakebin" 'xo_harness_ancestry_pid') || fail "the contiguous harness run was not resolved"
  [ "$got" = 900 ] || fail "ancestry crossed a non-harness gap, resolved '$got' instead of 900"
  printf '920\n' > "$dir/state/.lock"
  if lib_eval "$fakebin" "xo_session_lock_owned_by_self '$dir/state'"; then
    fail "an unrelated harness beyond a non-harness gap was accepted as this session's lock owner"
  fi
  printf '900\n' > "$dir/state/.lock"
  lib_eval "$fakebin" "xo_session_lock_owned_by_self '$dir/state'" \
...
SH
  ...
  lib_eval "$fakebin" 'xo_harness_pid_alive 600' \
...
#!/usr/bin/env bash
if [ "${XO_FIXTURE_ORPHAN_HERE:-0}" = 1 ]; then
  i=0
...
printf '%s\n' "$$" > "$XO_HOME/state/session-pid"
printf '%s\n' "$$" > "$XO_HOME/state/.lock"
"$XO_HOME/bin/xo-claude-stop-autoarm.sh" </dev/null > "$XO_HOME/state/hook.out" 2>&1
printf '%s\n' "$?" > "$XO_HOME/state/hook.rc"
SH
...
printf '%s\n' "$$" > "$XO_HOME/state/daemon-pid"
"$XO_SESSION_BIN" "$XO_HOME/session.sh"
exit 0
SH
...
run_fixture_tree() {  # <dir> <session-bin> [<daemon-bin>]
  ...
  if [ -n "$daemon_bin" ]; then
    XO_HOME="$dir" XO_SESSION_BIN="$session_bin" XO_FIXTURE_ORPHAN_HERE=0 \
      bash -c '"$0" "$1" &' "$daemon_bin" "$dir/daemon.sh"
  else
    XO_HOME="$dir" XO_FIXTURE_ORPHAN_HERE=1 \
