#!/usr/bin/env bash
# xo-send answerer-closes (--resolve-key) behavior.
#
...
# xo-send's --resolve-key removes that writer-dependency at its source: the
# ANSWERING xo closes the decision in this home's own ledger at answer
# time - for a local target that is ENQUEUE time, because the durable inbox
# write is delivery to the task's record. These tests drive the real xo-send
# executable over stubbed transports and assert closure through the real
# consumer (xo-wake-drain.sh's OPEN DECISIONS section), never through source
# text:
...
TMP_ROOT=$(xo_test_tmproot xo-send-resolve-key)

# Stub tmux: logs literal typed text to XO_SEND_LOG and lets the submit path
# reach a clean "empty" verdict (numeric cursor_y, empty bordered composer).
# XO_FAKE_TMUX_SEND_FAIL=1 makes send-keys fail so the delivery-failure leg can
...
case "${1:-}" in
  send-keys)
    [ "${XO_FAKE_TMUX_SEND_FAIL:-0}" = 1 ] && exit 1
    shift
    literal=0
    while [ $# -gt 0 ]; do
      case "$1" in
  ...
  capture-pane) printf '╭────╮\n│    │\n╰────╯\n'; exit 0 ;;
  list-windows)
    printf '%s\n' xo-t1 xo-t2 xo-t3 xo-t4 xo-t5 xo-t6 xo-t7 xo-t8 xo-t9 xo-mate
    exit 0 ;;
esac
exit 0
SH
...
SH
  chmod +x "$fb/sleep"
  # Stub ssh transport for the remote-secondmate legs, selected via XO_SSH_BIN.
  # Records the full remote invocation and exits XO_FAKE_SSH_RC (default 0).
  cat > "$fb/fake-ssh" <<'SH'
#!/usr/bin/env bash
cat > /dev/null
printf '%s\n' "$*" >> "$XO_SSH_LOG"
...
# run_send <fakebin> <home> <send-log> <xo-send args...>: run the real xo-send
...
test_answer_send_closes_open_decision() {
  ...
  xo_write_meta "$home/state/t1.meta" "window=sess:xo-t1" "kind=ship"
  ...
  assert_contains "$(cat "$log")" "XO instruction waiting" "the doorbell should be rung for the answer"
  grep -F 'resolved [key=api-shape]: answered: go with REST' "$home/state/t1.status" >/dev/null \
    || fail "xo-send did not append the closing resolved line:"$'\n'"$(cat "$home/state/t1.status")"

  out=$(drain_out "$home")
  if printf '%s' "$out" | grep -F 'OPEN DECISIONS' >/dev/null; then
    fail "the answered decision still lists as open: $out"
  fi
  pass "xo-send --resolve-key: the answer send itself closes the open decision"
}
...
# gate the watcher's signal scan consumes (bin/xo-wake-lib.sh).
test_answer_close_is_self_announced() {
  ...
  xo_write_meta "$home/state/t9.meta" "window=sess:xo-t9" "kind=ship"
  ...
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$home/state" "$home/state/t9.status" \
  ...
  if XO_STATE_OVERRIDE="$home/state" bash -c '
    . "$1"; xo_wake_signal_seen_current "$2" "$3"
  ' _ "$ROOT/bin/xo-wake-lib.sh" "$home/state" "$home/state/t9.status"; then
    fail "a later worker line after the self-announced close was swallowed"
  fi
  pass "xo-send --resolve-key: the close never re-wakes its own home, later lines still do"
}
...
test_colon_first_key_position_is_answerable() {
  ...
  xo_write_meta "$home/state/t8.meta" "window=sess:xo-t8" "kind=ship"
  ...
  pass "xo-send --resolve-key: a colon-first stated key is open under that key and answerable"
}
...
test_answer_starts_work_never_orphans() {
  ...
  pass "xo-send --resolve-key: an answer that starts a workstream leaves no orphaned decision"
}
...
test_routine_steer_never_closes() {
  ...
  xo_write_meta "$home/state/t3.meta" "window=sess:xo-t3" "kind=ship"
  ...
  pass "xo-send: a send without --resolve-key never closes a decision, and working/done still cannot"
}
...
test_not_open_key_refuses_before_send() {
  ...
  xo_write_meta "$home/state/t4.meta" "window=sess:xo-t4" "kind=ship"
  ...
  pass "xo-send --resolve-key: a key that is not open refuses loudly before anything is sent"
}
...
test_failed_ring_still_closes_at_enqueue() {
  ...
  xo_write_meta "$home/state/t5.meta" "window=sess:xo-t5" "kind=ship"
  printf 'blocked [key=creds]: need the deploy token\n' > "$home/state/t5.status"

  : > "$log"
  env PATH="$fb:$PATH" XO_FAKE_TMUX_SEND_FAIL=1 \
    XO_ROOT_OVERRIDE="$home" XO_HOME="$home" XO_SEND_LOG="$log" XO_SEND_SETTLE=0 \
  ...
  pass "xo-send --resolve-key: the close happens at enqueue, surviving a failed doorbell"
}
...
test_failed_enqueue_does_not_close() {
  ...
  pass "xo-send --resolve-key: a failed enqueue never closes the decision"
}
...
test_multiple_keys_close_together() {
  ...
  xo_write_meta "$home/state/t6.meta" "window=sess:xo-t6" "kind=ship"
  {
  ...
  pass "xo-send --resolve-key: one answer closes each named key and only those"
}
...
test_local_secondmate_answer_marked_and_closed() {
  ...
  run_send "$fb" "$home" "$log" xo-domain --resolve-key fleet-split "shard by team"; rc=$?
  ...
  pass "xo-send --resolve-key: a marked local-secondmate answer closes with the plain answer text"
}

# Remote secondmate: the answer crosses the (stubbed) ssh transport through the
# real xo-on.sh + registry route, while the close is the SAME local ledger
...
test_remote_secondmate_answer_closes_locally() {
  ...
  : > "$log"
  env PATH="$fb:$PATH" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" XO_SEND_LOG="$log" XO_SEND_SETTLE=0 \
    XO_SSH_BIN="$fb/fake-ssh" XO_SSH_LOG="$ssh_log" XO_FAKE_SSH_RC=0 \
  ...
  assert_grep 'xo-remote-entrypoint.sh' "$ssh_log" \
  ...
  pass "xo-send --resolve-key: a remote-secondmate answer closes the same local ledger, transport-only difference"
}
...
# though the key was right there on the line. This drives the real xo-send
...
test_remote_reply_corr_tag_does_not_block_resolve_key() {
  ...
  pass "xo-send --resolve-key: a remote reply's leading [corr=...] tag no longer blocks closing its stated key"
}
...
test_remote_transport_failure_does_not_close() {
  ...
  env PATH="$fb:$PATH" \
    XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" XO_SEND_LOG="$log" XO_SEND_SETTLE=0 \
    XO_SSH_BIN="$fb/fake-ssh" XO_SSH_LOG="$ssh_log" XO_FAKE_SSH_RC=1 \
  ...
  pass "xo-send --resolve-key: a failed remote transport never closes the decision"
}
...
test_flag_misuse_refuses() {
  ...
  xo_write_meta "$home/state/t7.meta" "window=sess:xo-t7" "kind=ship"
  printf 'needs-decision [key=k]: choose\n' > "$home/state/t7.status"

  # --resolve-key with --key (both orders) is refused: an answer is text.
  : > "$log"
  env PATH="$fb:$PATH" XO_ROOT_OVERRIDE="$home" XO_HOME="$home" XO_SEND_LOG="$log" XO_SEND_SETTLE=0 \
  ...
  pass "xo-send --resolve-key: --key, empty message, explicit targets, and malformed keys refuse loudly"
}

# The reported silent no-op: xo-send --resolve-key on a reserved pending-reply-*
...
test_reserved_pending_reply_key_closes_through_resolve_key() {
  ...
  xo_write_meta "$home/state/mate.meta" "window=sess:xo-mate" "kind=ship"
  ...
  pass "xo-send --resolve-key: a reserved pending-reply key actually closes through the operator path"
}
...
test_unrelated_writer_cannot_close_or_hijack_reserved_key() {
  ...
  pass "xo-send --resolve-key: an unrelated writer cannot close or hijack a reserved key, and the operator close still can"
}
...
test_unclosable_reserved_key_refuses_before_send() {
  ...
  env PATH="$fb:$PATH" \
    XO_ROOT_OVERRIDE="$home" XO_HOME="$home" XO_SEND_LOG="$log" XO_SEND_SETTLE=0 \
    XO_CLASSIFY_RESERVED_KEY_PREFIXES='pending-reply- secret-' \
  ...
  pass "xo-send --resolve-key: a reserved key this send cannot close refuses loudly before anything is sent"
}
...
test_long_decision_key_refuses_before_send() {
  ...
  pass "xo-send --resolve-key: an overlong decision key refuses before sending"
}
...
test_failed_close_recovery_command_is_shell_safe() {
  ...
  pass "xo-send --resolve-key: failed-close recovery commands safely quote operator text and paths"
}
...
test_remote_reserved_pending_reply_key_closes_locally() {
  ...
  pass "xo-send --resolve-key: a remote secondmate reserved-key close is the same local ledger append"
}
