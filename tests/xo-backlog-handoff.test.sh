#!/usr/bin/env bash
# tests/xo-backlog-handoff.test.sh - full item-block handoff (header + indented body).
#
...
TMP_ROOT=$(xo_test_tmproot xo-backlog-handoff)
HANDOFF_FAKEBIN=$(make_fake_tmux "$TMP_ROOT/default-fake")
export PATH="$HANDOFF_FAKEBIN:$PATH"
export XO_FAKE_TMUX_WINDOW='xo:xo-design'
export XO_FAKE_TMUX_LOG="$TMP_ROOT/default-tmux.log"
export XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/default-fake/pane.txt"
export XO_SEND_SETTLE=0 XO_SEND_SLEEP=0 XO_SEND_RETRIES=1
...
inbox_body_stream() { # <state-dir> <task-id>
  ...
  for rec in "$1/$2.inbox"/*.msg; do
    [ -f "$rec" ] || continue
    bash -c '. "$1"; xo_task_inbox_body "$2"' _ \
      "$ROOT/bin/xo-task-inbox-lib.sh" "$rec"
  done
}
...
doorbell_count() { # <backend-log>
  grep -cF 'XO instruction waiting:' "$1" 2>/dev/null || true
}
...
EOF
  ...
  XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" PATH="$fakebin:$PATH" \
    XO_FAKE_TMUX_WINDOW='xo:xo-design' \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/live-wake-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/live-wake-fake/pane.txt" \
    XO_SEND_SETTLE=0 XO_SEND_SLEEP=0 XO_SEND_RETRIES=1 \
    "$ROOT/bin/xo-backlog-handoff.sh" design wake-item > "$out" 2>&1 \
  ...
  XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" PATH="$fakebin:$PATH" \
    ...
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/live-wake-fake/pane.txt" \
    XO_SEND_SETTLE=0 XO_SEND_SLEEP=0 XO_SEND_RETRIES=1 \
    "$ROOT/bin/xo-backlog-handoff.sh" design wake-item > "$TMP_ROOT/live-wake-rerun.out" 2>&1 \
...
EOF
  ...
  out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design retry-item 2>&1) || rc=$?
...
EOF
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design retry-item > "$TMP_ROOT/retry-wake.out" 2>&1 \
...
SH
  ...
  out=$(PATH="$rejectbin:$basebin:$PATH" XO_BASE_TMUX="$basebin/tmux" \
    XO_HOME="$home" XO_FAKE_TMUX_WINDOW='xo:xo-design' \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/known-fail-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/known-fail-fake/pane.txt" \
    "$ROOT/bin/xo-backlog-handoff.sh" design known-fail 2>&1) || rc=$?
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design known-fail \
...
#!/usr/bin/env bash
if [ "${1:-}" = send-keys ]; then
  touch "$XO_RECONCILE_RACE_ENTERED"
  while [ ! -f "$XO_RECONCILE_RACE_RELEASE" ]; do sleep 0.02; done
  exit 1
fi
exec "$XO_BASE_TMUX" "$@"
SH
  ...
  PATH="$blockbin:$basebin:$PATH" XO_BASE_TMUX="$basebin/tmux" XO_HOME="$home" \
    XO_RECONCILE_RACE_ENTERED="$TMP_ROOT/reconcile-race.entered" \
    XO_RECONCILE_RACE_RELEASE="$TMP_ROOT/reconcile-race.release" \
    XO_FAKE_TMUX_WINDOW='xo:xo-design' \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/reconcile-race-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/reconcile-race-fake/pane.txt" \
    "$ROOT/bin/xo-backlog-handoff.sh" design reconcile-race \
  ...
  corr=$(cut -d: -f2- "$home/state/.backlog-handoff-design.wake-pending")
  XO_PENDING_REPLY_NOW=9999999999 bash -c '
    . "$1"
    xo_pending_reply_reconcile_delivery "$2" "$3"
  ' _ "$ROOT/bin/xo-pending-reply-lib.sh" "$home/state" "$corr" \
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design reconcile-race \
...
EOF
  ...
  xo_fake_crash_injector "$fakebin"
...
case " $* " in
  *" --file "*" --to "*)
    if [ "$rc" -eq 0 ] && [ "${1:-}" = mv ]; then
      ...
      xo-crash-inject "$handoff_pid" || exit 1
    fi
    ;;
esac
exit "$rc"
SH
  ...
  set +e
  XO_REAL_TASKS_AXI="$real_tasks" PATH="$fakebin:$PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-backlog-handoff.sh" design crash-item > "$TMP_ROOT/move-crash.out" 2>&1
  rc=$?
  set +e
...
EOF
  rc=0
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design unrelated-move \
  ...
  : > "$TMP_ROOT/default-tmux.log"
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design crash-item \
...
SH
  ...
  set +e
  XO_REAL_TASKS_AXI="$real_tasks" PATH="$fakebin:$PATH" XO_HOME="$home" \
    "$ROOT/bin/xo-backlog-handoff.sh" design pre-move-crash > "$TMP_ROOT/pre-move-crash.out" 2>&1
  rc=$?
  set -e
...
EOF
  rc=0
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design unrelated-ready \
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design pre-move-crash \
...
SH
  ...
  set +e
  PATH="$fakebin:$PATH" XO_REAL_RM="$real_rm" \
    XO_CONFIRM_CRASH_ONCE="$TMP_ROOT/confirm-crash.once" \
    XO_CONFIRM_WAKE_MARKER="$home/state/.backlog-handoff-design.wake-pending" \
    XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design confirm-crash \
...
## Done
EOF
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design after-crash \
...
SH
  ...
  set +e
  PATH="$fakebin:$PATH" XO_REAL_MV="$real_mv" XO_HOME="$home" \
    "$ROOT/bin/xo-backlog-handoff.sh" design attempt-crash \
  ...
  rc=0
  out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design attempt-crash 2>&1) || rc=$?
...
#!/usr/bin/env bash
case "$*" in
  *"XO instruction waiting:"*)
    if mkdir "$XO_BLOCK_WAKE_ONCE" 2>/dev/null; then
      touch "$XO_BLOCK_WAKE_ENTERED"
      while [ ! -f "$XO_BLOCK_WAKE_RELEASE" ]; do sleep 0.02; done
    fi
    ;;
esac
exec "$XO_BASE_TMUX" "$@"
SH
  ...
  PATH="$blockbin:$basebin:$PATH" XO_HOME="$home" XO_BASE_TMUX="$basebin/tmux" \
    XO_BLOCK_WAKE_ONCE="$TMP_ROOT/concurrent.once" \
    XO_BLOCK_WAKE_ENTERED="$TMP_ROOT/concurrent.entered" \
    XO_BLOCK_WAKE_RELEASE="$TMP_ROOT/concurrent.release" \
    XO_FAKE_TMUX_WINDOW='xo:xo-design' \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/concurrent-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/concurrent-fake/pane.txt" \
    "$ROOT/bin/xo-backlog-handoff.sh" design concurrent-a > "$TMP_ROOT/concurrent-a.out" 2>&1 &
  first=$!
  i=0
...
EOF
  PATH="$blockbin:$basebin:$PATH" XO_HOME="$home" XO_BASE_TMUX="$basebin/tmux" \
    ...
    XO_FAKE_TMUX_LOG="$TMP_ROOT/concurrent-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/concurrent-fake/pane.txt" \
    "$ROOT/bin/xo-backlog-handoff.sh" design concurrent-b > "$TMP_ROOT/concurrent-b.out" 2>&1 &
  second=$!
  sleep 0.2
...
SH
  chmod +x "$blockbin/tmux"
  PATH="$blockbin:$basebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_BASE_TMUX="$basebin/tmux" XO_BLOCK_WAKE_ENTERED="$TMP_ROOT/teardown-race.entered" \
    XO_BLOCK_WAKE_RELEASE="$TMP_ROOT/teardown-race.release" \
    XO_FAKE_TMUX_WINDOW='xo:xo-design' \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/teardown-race-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-race-fake/pane.txt" \
    "$ROOT/bin/xo-backlog-handoff.sh" design teardown-race > "$TMP_ROOT/teardown-race-handoff.out" 2>&1 &
  handoff=$!
  i=0
  ...
  PATH="$basebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_FAKE_TMUX_WINDOW='xo:xo-design' \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/teardown-race-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-race-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" design --force > "$TMP_ROOT/teardown-race-teardown.out" 2>&1 &
  teardown=$!
  sleep 0.3
...
SH
  ...
  set +e
  PATH="$rm_bin:$fakebin:$PATH" XO_REAL_RM="$real_rm" XO_FAIL_HOME="$fail_home" \
    XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_FAKE_TMUX_WINDOW='xo:xo-design' \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/teardown-home-fail-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-home-fail-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" design --force > "$TMP_ROOT/teardown-home-fail.out" 2>&1
  rc=$?
  set -e
  ...
  PATH="$fakebin:$PATH" XO_HOME="$home" XO_ROOT_OVERRIDE="$ROOT" \
    XO_FAKE_TMUX_WINDOW='xo:xo-design' \
    XO_FAKE_TMUX_LOG="$TMP_ROOT/teardown-home-fail-tmux.log" \
    XO_FAKE_TMUX_CAPTURE="$TMP_ROOT/teardown-home-fail-fake/pane.txt" \
    "$ROOT/bin/xo-teardown.sh" design --force > "$TMP_ROOT/teardown-home-retry.out" 2>&1 \
...
seed_public_commitment() {
  ...
  XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$home" "$ROOT/bin/xo-public-followup.sh" register \
...
EOF
  ...
  local out rc=0
  out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design promised-item plain-item 2>&1) || rc=$?
...
EOF
  ...
  local out rc=0
  out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design quiet-item 2>&1) || rc=$?
...
EOF
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design body-item >/dev/null \
...
EOF
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design section-tail >/dev/null \
...
test_body_moves_when_last_lines_of_file() {
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design eof-item >/dev/null \
...
test_eof_body_before_seeded_destination_section_keeps_boundary() {
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design seeded-eof-item >/dev/null \
...
test_untouched_eof_line_preserves_terminator() {
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design move-item >/dev/null \
...
EOF
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design idem-item >/dev/null \
  ...
  local out
  out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design idem-item 2>&1) \
...
EOF
  ...
  if out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design malformed-body 2>&1); then
...
test_indented_heading_is_not_section_boundary() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## Queued
- [ ] ha-codex-fast-default-4e - harness default work (repo: xo)
...
- [ ] next-item - after the trap (repo: xo)
EOF

  local expected_block
  expected_block=$(extract_item_block "$home/data/backlog.md" ha-codex-fast-default-4e)

  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design ha-codex-fast-default-4e >/dev/null \
...
EOF
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design multi-para >/dev/null \
  ...
  dest_after=$(cat "$sub/data/backlog.md")
  local out
  out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" design multi-para 2>&1) \
...
EOF
  ...
  XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" "$id" paren-item >/dev/null \
...
EOF
  ...
  local out rc=0
  out=$(XO_HOME="$home" "$ROOT/bin/xo-backlog-handoff.sh" "$id" orphan-item 2>&1) || rc=$?
