#!/usr/bin/env bash
# Behavior tests for bin/xo-mail-check.sh, the standing received-mail check.
#
...
#
#   * the `check` action itself, which runs the real xo-mail.sh poll against a
...
CHECK="$ROOT/bin/xo-mail-check.sh"
TMP_ROOT=$(xo_test_tmproot xo-mail-check)
...
# explicit argument so the missing-mail-plane case runs a copy without xo-mail.sh.
run_check() {
  local home=$1 out=$2 check=$3
  shift 3
  local status=0
  env -u XO_MAIL_USER -u XO_MAIL_PASS -u XO_IMAP_HOST -u XO_SMTP_HOST \
    -u XO_MAIL_CHECK_BUDGET \
    XO_CHECK_TIMEOUT=30 \
    "$@" XO_HOME="$home" PATH="$FAKEBIN:$PATH" \
...
# write_env <home>: the four XO_MAIL_* values the poll requires.
write_env() {
  local home=$1
  printf '%s\n' \
    'XO_MAIL_USER=test@example.invalid' \
    'XO_MAIL_PASS=test-pass' \
    'XO_IMAP_HOST=imap.test.invalid' \
    'XO_SMTP_HOST=smtp.test.invalid' > "$home/.env"
}
...
enter_mailbox() {
  ...
  [ -e "$home/bin/xo-wake-lib.sh" ] || ln -s "$ROOT/bin/xo-wake-lib.sh" "$home/bin/xo-wake-lib.sh"
...
test_help_and_usage() {
  ...
  pass "xo-mail-check: help and usage plumbing"
}
...
test_arm_writes_and_binds_the_check_and_disarm_removes_it() {
  ...
  assert_contains "$(cat "$home/state/mail.check.sh")" "xo-mail-check.sh check" "shim dispatches the check action"
  assert_contains "$(cat "$home/state/mail.check.sh")" "XO_HOME=$home" "shim pins the absolute home"
  ...
  pass "xo-mail-check: arm writes and binds, re-arm is idempotent, disarm removes"
}
...
test_arm_resolves_a_relative_home_into_the_shim() {
  ...
  write_env "$home"
  rel="$(basename "$home")"
  out=$(cd "$TMP_ROOT" && env XO_HOME="$rel" "$CHECK" arm 2>&1) || fail "arm with a relative XO_HOME must succeed: $out"
  assert_contains "$(cat "$home/state/mail.check.sh")" "export XO_HOME=$home" "the shim pins the resolved absolute home, not the relative spelling"
  pass "xo-mail-check: arm resolves a relative home into the shim"
}
...
test_arm_refuses_a_symlink_at_the_shim_path() {
  ...
  pass "xo-mail-check: arm refuses a symlink at the shim path"
}

test_arm_refuses_without_the_mail_plane() {
  local tmpbin home out rc=0
  # A copy of the check tool with no xo-mail.sh beside it is a home whose mail
  ...
  cp "$ROOT/bin/xo-mail-check.sh" "$tmpbin/"
  for lib in xo-timeout-lib.sh xo-pr-lib.sh xo-line-cap-lib.sh xo-check-lib.sh; do
    [ -e "$tmpbin/$lib" ] || ln -s "$ROOT/bin/$lib" "$tmpbin/$lib"
  done
  out=$(XO_HOME="$home" "$tmpbin/xo-mail-check.sh" arm 2>&1) || rc=$?
  ...
  pass "xo-mail-check: arm refuses without the mail plane"
}
...
printf "42\\t2026-09-05T00:00:00Z\\talice@example.com\\tHello\\n"'
  ...
  assert_contains "$(cat "$home/state/.mail-check")" "xo-mail-check-v1" "the record carries its schema"
  ...
  pass "xo-mail-check: a successful poll that surfaces new mail emits one wake line"
}
...
test_failure_is_reported_once_until_it_changes() {
  ...
  write_env "$home"
  enter_mailbox "$home" \
    'printf "xo-mail poll error: connection refused\\n" >&2
exit 1'

  out="$home/out.txt"
  run_check "$home" "$out" "$CHECK"
  assert_contains "$(cat "$out")" "mail: xo-mail poll error: connection refused" "a failing poll reports its cause in one line"
  ...
  assert_contains "$(cat "$home/state/.mail-check")" "reported=xo-mail poll error: connection refused" "the record carries the reported finding"
...
exit 1'
  out="$home/out4.txt"
  run_check "$home" "$out" "$CHECK"
  assert_contains "$(cat "$out")" "mail: xo-mail poll error: connection refused" "a failure after a healthy poll is news again"
  pass "xo-mail-check: a poll failure is reported once and re-reported after recovery"
}
...
test_unconfigured_home_is_reported_once() {
  ...
  assert_contains "$(cat "$out")" "XO_MAIL_USER" "the report names the missing variable"
  out="$home/out2.txt"
  run_check "$home" "$out" "$CHECK"
  [ ! -s "$out" ] || fail "the unconfigured state must not repeat: $(cat "$out")"
  pass "xo-mail-check: an unconfigured home is reported once, not every poll"
}
...
test_slow_poll_times_out_and_is_reported() {
  ...
  enter_mailbox "$home" \
    'sleep 6'
  out="$home/out.txt"
  run_check "$home" "$out" "$CHECK" XO_MAIL_CHECK_BUDGET=5
  ...
  pass "xo-mail-check: a slow poll times out into a one-line report"
}
...
test_repeated_failure_that_queued_new_mail_still_wakes() {
  ...
  # and unacted; the check must print again so the watcher wakes xo.
  local tmpbin home out check_bin
  tmpbin="$TMP_ROOT/repeat-wake/bin"
  home="$TMP_ROOT/repeat-wake/home"
  mkdir -p "$tmpbin" "$home/state"
  check_bin="$tmpbin/xo-mail-check.sh"
  cp "$ROOT/bin/xo-mail-check.sh" "$tmpbin/"
  for lib in xo-timeout-lib.sh xo-pr-lib.sh xo-line-cap-lib.sh xo-check-lib.sh; do
    [ -e "$tmpbin/$lib" ] || ln -s "$ROOT/bin/$lib" "$tmpbin/$lib"
  done
  printf '%s\n' '#!/usr/bin/env bash' 'echo "xo-mail: woke for 42"' 'echo "xo-mail: connection refused" >&2' 'exit 1' > "$tmpbin/xo-mail.sh"
  chmod +x "$tmpbin/xo-mail.sh"
  ...
  pass "xo-mail-check: a repeated failure that queued new mail still wakes"
}
...
test_repeated_timeout_still_wakes() {
  ...
  printf '%s\n' '#!/usr/bin/env bash' 'exit 124' > "$tmpbin/xo-mail.sh"
  chmod +x "$tmpbin/xo-mail.sh"
  ...
  pass "xo-mail-check: a repeated timeout still wakes"
}
...
printf "77\\t2026-09-05T00:00:00Z\\tfrom@x\\tHello\\tok\\n"'
  ...
  pass "xo-mail-check: a fail-closed poll after a wake reports the failure, not the wake"
}
...
printf "77\\t2026-09-05T00:00:00Z\\tfrom@x\\tHello\\tretry\\n"'
  ...
  chmod 0600 "$home/state/.mail-retry"
  pass "xo-mail-check: a repeated status-4 fail-closed poll still wakes"
}
...
test_repeated_status2_stays_queued_still_wakes() {
  ...
  cat > "$tmpbin/xo-mail.sh" <<EOF
#!/usr/bin/env bash
printf '1\t1\tcheck\tmail:9\tcheck: mail 9 - stays queued\\n' >> "\$XO_HOME/state/.wake-queue"
echo "xo-mail: wake for 9 could not be rolled back or durably recorded; the wake stays queued and the next poll heals it - a possible duplicate, never a lost mail" >&2
exit 1
EOF
  chmod +x "$tmpbin/xo-mail.sh"
  ...
  [ "$(wc -l < "$out" | tr -d '[:space:]')" = 1 ] || fail "the repeat report is exactly one line: $(cat "$out")"
  pass "xo-mail-check: a repeated status-2 stays-queued poll still wakes"
}
...
test_repeated_heal_failure_stays_silent() {
  ...
  pass "xo-mail-check: a repeated pre-wake heal failure stays silent"
}
...
test_missing_mail_plane_is_reported() {
  ...
  out="$home/out.txt"
  run_check "$home" "$out" "$check_bin"
  assert_contains "$(cat "$out")" "mail: xo-mail.sh is missing next to this check" "a home lacking the mail plane reports it"
  pass "xo-mail-check: a missing mail plane is reported, not assumed"
}
