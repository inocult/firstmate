...
# xo-wake-drain.sh now calls xo-guard.sh to assert watcher liveness on every
# drain. xo-guard.sh's first check warns when the xo PRIMARY checkout
# (XO_ROOT) sits on a feature branch; with no override XO_ROOT resolves to the
...
# direct xo-guard.sh tests use. A per-call XO_ROOT_OVERRIDE still wins where a
# suite sets its own (e.g. the watcher-lock guard-banner cases).
if [ -z "${XO_ROOT_OVERRIDE:-}" ]; then
  XO_ROOT_OVERRIDE="$(xo_test_tmproot xo-wake-tangle-root)"
  export XO_ROOT_OVERRIDE
fi

# Wedge-alarm notifier recorder (safety seam). The away-mode wedge alarm fires a
# real OS-level desktop notification by default. Point its XO_WEDGE_ALARM_EXEC
...
# $XO_WEDGE_ALARM_LOG, which a test sets to its own file to assert on; unset means
# /dev/null. XO_WEDGE_ALARM_FAIL=<channel> makes the recorder exit non-zero for
...
_xo_wedge_rec_dir=$(xo_test_tmproot xo-wedge-rec)
cat > "$_xo_wedge_rec_dir/rec" <<'REC'
#!/usr/bin/env bash
printf '%s\t%s\n' "${1:-}" "${2:-}" >> "${XO_WEDGE_ALARM_LOG:-/dev/null}"
case " ${XO_WEDGE_ALARM_FAIL:-} " in *" ${1:-} "*) exit 1 ;; esac
exit 0
REC
chmod +x "$_xo_wedge_rec_dir/rec"
export XO_WEDGE_ALARM_EXEC="$_xo_wedge_rec_dir/rec"
...
append_wake() {
  local state=$1 kind=$2 key=$3 payload=$4 lib="$ROOT/bin/xo-wake-lib.sh"
  XO_STATE_OVERRIDE="$state" bash -c '
    # shellcheck disable=SC1090,SC1091
    . "$1"
    xo_wake_append "$2" "$3" "$4"
...
if [ "${1:-}" = "list-windows" ]; then
  if [ -n "${XO_FAKE_TMUX_WINDOWS:-}" ]; then
    printf '%s\n' "$XO_FAKE_TMUX_WINDOWS"
  elif [ -n "${XO_FAKE_TMUX_WINDOW:-}" ]; then
    printf '%s\n' "${XO_FAKE_TMUX_WINDOW#*:}"
  fi
  exit 0
fi
if [ "${1:-}" = "capture-pane" ]; then
  if [ -n "${XO_FAKE_TMUX_CAPTURE_COUNT_FILE:-}" ]; then
    _capture_count=$(cat "$XO_FAKE_TMUX_CAPTURE_COUNT_FILE" 2>/dev/null || echo 0)
    printf '%s\n' "$((_capture_count + 1))" > "$XO_FAKE_TMUX_CAPTURE_COUNT_FILE"
    if [ -n "${XO_FAKE_TMUX_CAPTURE_FAIL_AFTER:-}" ] \
      && [ "$_capture_count" -ge "$XO_FAKE_TMUX_CAPTURE_FAIL_AFTER" ]; then
      exit 1
    fi
  fi
  if [ -n "${XO_FAKE_TMUX_FORBIDDEN_TARGET:-}" ]; then
    _prev=
    for _arg in "$@"; do
      if [ "$_prev" = -t ] && [ "$_arg" = "$XO_FAKE_TMUX_FORBIDDEN_TARGET" ]; then
        exit 1
      fi
      _prev=$_arg
    done
  fi
  if [ -n "${XO_FAKE_TMUX_CAPTURE:-}" ]; then
...
# Install a hermetic fake xo-crew-state.sh into <fakebin> and echo its path. The
# watcher's absorb-only-when-provably-working triage calls this (via
# XO_CREW_STATE_BIN) to read a crew's current state on no-verb signal and stale
...
# A per-id override XO_FAKE_CREW_STATE_<sanitized-id> wins; otherwise the shared
# XO_FAKE_CREW_STATE; otherwise an unknown verdict (NOT provably working), the
# safe default so a test that forgets to set one surfaces rather than absorbs.
make_fake_crew_state() {  # <fakebin>
  local fakebin=$1
  cat > "$fakebin/xo-crew-state.sh" <<'SH'
#!/usr/bin/env bash
set -u
id=${1:-}
key=$(printf '%s' "$id" | tr -c 'A-Za-z0-9' '_')
var="XO_FAKE_CREW_STATE_$key"
val=${!var:-${XO_FAKE_CREW_STATE:-}}
printf '%s\n' "${val:-state: unknown · source: none · fake default}"
exit 0
SH
  chmod +x "$fakebin/xo-crew-state.sh"
  printf '%s\n' "$fakebin/xo-crew-state.sh"
}

# Prime <file>'s .seen-* marker to its CURRENT signature through the production
# signature owner (bin/xo-wake-lib.sh), so a test can declare "everything in
...
ack_drain_err() {  # <state> <stderr-file>
  ...
  XO_STATE_OVERRIDE="$state" "$ROOT/bin/xo-wake-drain.sh" \
...
case "${1:-}" in
  ...
  capture-pane)
    ...
    # structural full-pane scan or xo_pane_is_busy's "-S -40" tail) return the whole capture. -e is accepted and
    ...
    [ -n "${XO_FAKE_TMUX_CAPTURE:-}" ] || exit 0
    if [ -n "$_S" ] && [ -n "$_E" ]; then
      case "$_S$_E" in
        *[!0-9]*) cat "$XO_FAKE_TMUX_CAPTURE" 2>/dev/null ;;
        *) sed -n "$((_S + 1)),$((_E + 1))p" "$XO_FAKE_TMUX_CAPTURE" 2>/dev/null ;;
      esac
    else
      cat "$XO_FAKE_TMUX_CAPTURE" 2>/dev/null
    fi
    exit 0 ;;
  send-keys)
    while [ "$#" -gt 0 ]; do
      case "$1" in
        -l) shift; [ "$#" -gt 0 ] && {
          printf '%s\n' "$1" >> "${XO_FAKE_TMUX_SENT:-/dev/null}"
          # Reflect sent text into capture so pane_input_pending sees it as
          # pending input (text in the composer).
          [ -n "${XO_FAKE_TMUX_CAPTURE:-}" ] && printf '%s\n' "$1" >> "$XO_FAKE_TMUX_CAPTURE"
        } ;;
        Enter)
          # Optionally swallow Enter (file-based flag) to test the retry path.
          if [ -n "${XO_FAKE_TMUX_SWALLOW_FILE:-}" ] && [ -f "$XO_FAKE_TMUX_SWALLOW_FILE" ]; then
            rm -f "$XO_FAKE_TMUX_SWALLOW_FILE"
          else
            printf '[ENTER]\n' >> "${XO_FAKE_TMUX_SENT:-/dev/null}"
            # Enter submits: clear the last line (the typed text) from the
            # capture, simulating the composer being cleared on submit.
            if [ -n "${XO_FAKE_TMUX_CAPTURE:-}" ] && [ -s "$XO_FAKE_TMUX_CAPTURE" ]; then
              _tmp=$(mktemp 2>/dev/null) || _tmp="${XO_FAKE_TMUX_CAPTURE}.tmp"
              sed '$d' "$XO_FAKE_TMUX_CAPTURE" > "$_tmp" 2>/dev/null && mv -f "$_tmp" "$XO_FAKE_TMUX_CAPTURE"
...
#!/usr/bin/env bash
set -u
COMPOSER="${XO_FAKE_COMPOSER:?XO_FAKE_COMPOSER unset}"
write_composer() {
  text=$1
  width=$((${#text} + 4))
  border=
...
case "${1:-}" in
  ...
  send-keys)
    ...
    if [ "$is_enter" = 1 ]; then
      if [ -n "${XO_FAKE_SWALLOW:-}" ] && [ -f "$XO_FAKE_SWALLOW" ]; then
        [ "${XO_FAKE_PERSIST_SWALLOW:-0}" = 1 ] || rm -f "$XO_FAKE_SWALLOW"
      else
        [ -n "${XO_FAKE_SENT:-}" ] && printf '[ENTER]\n' >> "$XO_FAKE_SENT"
        write_composer ""
      fi
    elif [ "$lit" = 1 ]; then
      [ "${XO_FAKE_SEND_FAIL:-0}" = 1 ] && exit 1
      [ -n "${XO_FAKE_SENT:-}" ] && printf '%s\n' "$text" >> "$XO_FAKE_SENT"
      write_composer "$text"
    fi
    exit 0 ;;
esac
