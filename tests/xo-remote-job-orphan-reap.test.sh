...
#
# bin/xo-remote-job-reap-orphans.sh is a machine-wide sweep by design, so these
...
TMP_ROOT=$(xo_test_tmproot xo-remote-job-orphan-reap)
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
REAPER="$ROOT/bin/xo-remote-job-reap-orphans.sh"
...
# --- a real worker fixture, launched exactly the way xo-on's Linux start does -

# build_remote_root <dir>: a minimal but genuine XO code root carrying
# the real worker and job library.
build_remote_root() {
  local root=$1
  mkdir -p "$root/bin"
  cp "$ROOT/bin/xo-remote-job-lib.sh" "$ROOT/bin/xo-remote-job-worker.sh" "$root/bin/"
  chmod +x "$root/bin"/*.sh
  printf 'fixture\n' > "$root/AGENTS.md"
...
start_worker() {
  ...
  pid=$(
    ...
    export XO_REMOTE_JOB_ORPHAN_GRACE_SECONDS=1
    # shellcheck source=bin/xo-remote-job-lib.sh
    . "$ROOT/bin/xo-remote-job-lib.sh"
    xo_remote_job_start_linux_worker "$root" "$account_home" >&2 || exit 1
    deadline=$(( $(date +%s) + 10 ))
    while [ "$(date +%s)" -lt "$deadline" ]; do
      pid=$(pgrep -f "^/bin/bash $root/bin/xo-remote-job-worker.sh\$" | head -n 1)
...
cat > "$CASE2/remote-root/bin/xo-remote-job-worker.sh" <<'SH'
#!/bin/bash
...
SH
chmod +x "$CASE2/remote-root/bin/xo-remote-job-worker.sh"
printf 'fixture\n' > "$CASE2/remote-root/AGENTS.md"

set -m
"$CASE2/remote-root/bin/xo-remote-job-worker.sh" >/dev/null 2>&1 &
STALE=$!
set +m
track "$STALE"
