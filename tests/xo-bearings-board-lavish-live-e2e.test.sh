#!/usr/bin/env bash
# tests/xo-bearings-board-lavish-live-e2e.test.sh - live drift guard proving
# the real lavish-axi still behaves the way bin/xo-bearings-board.sh's session
...
# portable counterpart in tests/xo-bearings-board.test.sh pins the build's logic
...
xo_live_gate default-on XO_BEARINGS_LAVISH_LIVE lavish-axi jq curl
...
LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-bearings-lavish-live.XXXXXX") || fail "cannot create the guard lab"
LAB=$(cd -P -- "$LAB" && pwd -P)
...
run_board() {
  XO_HOME="$LAB" XO_STATE_OVERRIDE="$LAB/state" XO_DATA_OVERRIDE="$LAB/data" \
    XO_PROCEVENT_CLAIM_ROOT="$LAB/procevent-claims" \
    "$ROOT/bin/xo-bearings-board.sh" "$@"
}
