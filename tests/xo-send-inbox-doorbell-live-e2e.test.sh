#!/usr/bin/env bash
# tests/xo-send-inbox-doorbell-live-e2e.test.sh - the live doorbell guard
...
# written into the stub, so per .agents/skills/xo-coding-guidelines
# this is proven against every INSTALLED verified harness: each is launched
# idle in an isolated tmux server, steered through the REAL xo-send (durable
...
#
# Run explicitly with XO_SEND_INBOX_LIVE_E2E=1. This test spends a small
...
# XO_SEND_INBOX_LIVE_HARNESSES="claude codex ..." when needed, and tune the
# per-harness wait with XO_SEND_INBOX_LIVE_TIMEOUT (seconds, default 240).
...
xo_live_gate opt-in XO_SEND_INBOX_LIVE_E2E tmux

unset NO_MISTAKES_GATE

SOCKET="xo-inbox-live-$$"
SESSION="inboxlive"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/xo-inbox-live.XXXXXX")
LAB=$(cd "$LAB" && pwd)
TIMEOUT=${XO_SEND_INBOX_LIVE_TIMEOUT:-240}
CHECKED=0
FAILED=0
...
# xo-send and the composer readiness read both reach tmux through bare `tmux`
...
# bin/xo-spawn.sh uses), so the doorbell-triggered shell actions need no
...
check_harness_doorbell() {  # <name>
  ...
  if ! XO_HOME="$home" XO_ROOT_OVERRIDE="$home" "$ROOT/bin/xo-send.sh" "$task" \
    "XO live check: run exactly this shell command now: touch $acted - then follow the mv instruction you were given for this message. Reply with one short line." \
    >/dev/null 2>&1; then
    FAILED=1
    printf 'not ok - %s (%s): xo-send refused the live steer\n' "$name" "$version" >&2
    tmux -L "$SOCKET" kill-window -t "$SESSION:$win" 2>/dev/null || true
    return 0
  fi
  ...
  [ -f "$rec" ] || {
    FAILED=1
    printf 'not ok - %s (%s): xo-send left no durable inbox record\n' "$name" "$version" >&2
    tmux -L "$SOCKET" kill-window -t "$SESSION:$win" 2>/dev/null || true
    return 0
  }
  i=0
  while [ "$i" -lt "$TIMEOUT" ]; do
    ...
    if [ "$i" -eq $((TIMEOUT / 2)) ] && [ -f "$rec" ]; then
      xo_task_inbox_ring tmux "$SESSION:$win" "$rec" || true
...
HARNESSES=${XO_SEND_INBOX_LIVE_HARNESSES:-'claude codex opencode pi grok kimi muse'}
