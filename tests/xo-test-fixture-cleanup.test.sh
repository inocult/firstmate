...
# (xo_test_tmproot / xo_test_cleanup / xo_test_reap_orphans).
#
# The near-universal call pattern across this suite is
# `TMP_ROOT=$(xo_test_tmproot prefix)`, which forks a subshell to capture the
...
test_fixture_root_gone_after_normal_exit() {
  ...
  child_out=$(bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(xo_test_tmproot xo-test-cleanup-exit)
    printf "%s\n" "$d"
  ...
  assert_absent "$child_dir" \
    "xo_test_tmproot's fixture root survived its owning process's normal exit"
  pass "xo_test_tmproot cleans up its fixture root on normal exit"
}

test_fixture_root_gone_after_sigterm() {
  local harness dirfile child_dir pid tries
  harness=$(xo_test_tmproot xo-test-cleanup-sigterm-harness)
  dirfile="$harness/child-dir"
  bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(xo_test_tmproot xo-test-cleanup-term)
    printf "%s\n" "$d" > "'"$dirfile"'"
  ...
  wait "$pid" 2>/dev/null
  assert_absent "$child_dir" \
    "xo_test_tmproot's fixture root survived SIGTERM to its owning process"
  pass "xo_test_tmproot cleans up its fixture root on SIGTERM"
}

test_cleanup_registry_resists_precreation() {
  local harness shared_tmp victim
  harness=$(xo_test_tmproot xo-test-cleanup-registry-harness)
  shared_tmp="$harness/shared-tmp"
  victim="$harness/victim"
  mkdir -p "$shared_tmp" "$victim"

  TMPDIR="$shared_tmp" bash -c '
    printf "%s\n" "$1" > "$TMPDIR/.xo-test-cleanup.$$"
    . "$2"
  ' _ "$victim" "$LIB"
...
test_fixture_registration_failure_rolls_back_root() {
  ...
  harness=$(xo_test_tmproot xo-test-cleanup-registration-harness)
  failure_tmp="$harness/tmp"
  registry_dir="$harness/registry-dir"
  mkdir -p "$failure_tmp" "$registry_dir"

  if output=$(TMPDIR="$failure_tmp" XO_TEST_CLEANUP_REGISTRY="$registry_dir" \
    xo_test_tmproot xo-test-cleanup-registration-failure 2>/dev/null); then
    fail "xo_test_tmproot succeeded after its cleanup registry rejected registration"
  fi
  [ -z "$output" ] || fail "xo_test_tmproot published an unregistered fixture root"
  for leaked_root in "$failure_tmp"/xo-test-cleanup-registration-failure.*; do
    [ ! -e "$leaked_root" ] || fail "xo_test_tmproot leaked a root after registration failed"
  done
...
test_orphan_sweep_respects_fixture_ownership() {
  ...
  harness=$(xo_test_tmproot xo-test-cleanup-orphan-harness)
  dirfile="$harness/active-dir"
  bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(xo_test_tmproot xo-test-cleanup-active)
    printf "%s\n" "$d" > "'"$dirfile"'"
    while :; do sleep 0.1; done
  ' &
  pid=$!
  ...
  touch -t 202001010000 "$active_dir/.xo-test-fixture"

  stale_dir=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-cleanup-stale.XXXXXX")
  printf '%s\n%s\n' "$$" reused-process-identity > "$stale_dir/.xo-test-fixture"
  touch -t 202001010000 "$stale_dir/.xo-test-fixture"
  fresh_dir=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-cleanup-fresh.XXXXXX")
  : > "$fresh_dir/.xo-test-fixture"
...
test_orphan_sweep_reaps_read_only_package_tree() {
  ...
  stale_dir=$(mktemp -d "${TMPDIR:-/tmp}/xo-test-cleanup-read-only.XXXXXX")
