...
xo_live_gate opt-in XO_PI_LIVE_E2E pi tmux
...
ASCII_ONLY='XO_OP: v1 watcher: captain-authored text'
...
run_native_ahoy_regressions() {
  ...
  cp "$ROOT/.pi/extensions/xo-primary-turnend-guard.ts" "$AHOY_PROJECT/.pi/extensions/"
  cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" "$AHOY_PROJECT/.pi/extensions/lib/"
  cp \
  ...
  chmod +x "$AHOY_PROJECT/bin/xo-sessionstart-nudge.sh"
  # shellcheck disable=SC2016 # Variables expand in the generated script, not this test shell.
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    'set -u' \
    'file="${XO_HOME:?}/state/session-start-count"' \
    'count=0' \
    '[ ! -f "$file" ] || count=$(sed -n "1p" "$file")' \
    'count=$((count + 1))' \
    'printf "%s\n" "$count" > "$file"' \
    'printf "SESSION_START_DONE count=%s\n" "$count"' \
    > "$AHOY_PROJECT/bin/xo-session-start.sh"
  chmod +x "$AHOY_PROJECT/bin/xo-session-start.sh"
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  printf '%s\n' \
    '---' \
    'name: bearings' \
  ...
  first_out=$(
    cd "$AHOY_PROJECT" &&
      XO_HOME="$first_home" pi --print --approve --no-session --no-context-files --no-extensions \
        -e .pi/extensions/xo-primary-turnend-guard.ts \
  ...
  later_out=$(
    cd "$AHOY_PROJECT" &&
      XO_HOME="$later_home" pi --print --approve --no-session --no-context-files --no-extensions \
        -e .pi/extensions/xo-primary-turnend-guard.ts \
        --no-skills --skill .agents/skills \
        --model openai-codex/gpt-5.6-sol --thinking low \
...
cp "$ROOT/.pi/extensions/xo-calm.ts" "$PROJECT/.pi/extensions/xo-calm.ts"
cp "$ROOT/.pi/extensions/xo-primary-pi-watch.ts" "$PROJECT/.pi/extensions/xo-primary-pi-watch.ts"
cp "$ROOT/.pi/extensions/lib/xo-calm-assistant-layout.ts" "$PROJECT/.pi/extensions/lib/xo-calm-assistant-layout.ts"
cp "$ROOT/.pi/extensions/lib/xo-calm-operational-user-layout.ts" "$PROJECT/.pi/extensions/lib/xo-calm-operational-user-layout.ts"
cp "$ROOT/.pi/extensions/lib/xo-calm-visibility.ts" "$PROJECT/.pi/extensions/lib/xo-calm-visibility.ts"
...
cp "$ROOT/bin/xo-watch-arm.sh" "$PROJECT/bin/xo-watch-arm.sh"
cp "$ROOT/bin/xo-operational-input.sh" "$PROJECT/bin/xo-operational-input.sh"
cp "$ROOT/bin/xo-supervision-instructions.sh" "$PROJECT/bin/xo-supervision-instructions.sh"
chmod +x "$PROJECT/bin/xo-operational-input.sh"
mkdir -p "$HOME_DIR/state" "$HOME_DIR/config"

"$TMUX" -L "$SOCKET" new-session -d -s "$SESSION" -c "$PROJECT" \
  "env XO_HOME='$HOME_DIR' XO_ROOT_OVERRIDE='$PROJECT' XO_POLL=1 XO_SIGNAL_GRACE=0 XO_HEARTBEAT=600 bash -lc 'printf \"%s\\n\" \"\$\$\" > \"\$XO_HOME/state/.lock\"; pi --approve --no-session --no-context-files --no-extensions -e .pi/extensions/xo-calm.ts -e .pi/extensions/xo-primary-turnend-guard.ts -e .pi/extensions/xo-primary-pi-watch.ts --model openai-codex/gpt-5.6-sol --thinking low; rc=\$?; printf \"PI_EXIT=%s\\n\" \"\$rc\"; sleep 300'"
...
send_prompt "Start supervision with xo_watch_arm_pi and never use bash to arm supervision. After the watcher wake arrives, run bin/xo-wake-drain.sh and reply exactly HANDLED."
...
foreground_arm='$ bin/xo-watch-arm.sh'
