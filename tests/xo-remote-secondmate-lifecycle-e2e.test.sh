...
TMP_ROOT=$(xo_test_tmproot xo-remote-secondmate-e2e)
mkdir -p "$TMP_ROOT"
...
FAKEBIN=$(xo_fakebin "$TMP_ROOT/fake")
...
cleanup() {
  ...
  XO_HOME="$PARENT" XO_PROCEVENT_CLAIM_ROOT="$CLAIMS" \
    "$ROOT/bin/xo-procevent.sh" sweep-home >/dev/null 2>&1 || true
...
case "\${1:-}" in
  ...
  list-windows)
    [ -f "\$state" ] || exit 0
    name=\$(cut -d'|' -f1 "\$state")
    case "\$*" in *'#{session_name}:#{window_name}'*) printf 'xo:%s\n' "\$name" ;; *) printf '%s\n' "\$name" ;; esac
    exit 0
    ;;
  new-window)
    name=; cwd=
...
REMOTE_ORIGIN="$TMP_ROOT/xo-origin.git"
...
$command_fields
EOF
case "${XO_FAKE_SSH_MODE:-normal}:$command_name:$command_rel" in
  inherit-partial:xo-remote-inherit.sh:config/crew-harness) exit 255 ;;
  inherit-block:xo-remote-inherit.sh:data/captain-shared.md)
    cat > "$XO_FAKE_INHERIT_PAYLOAD"
    touch "$XO_FAKE_INHERIT_ENTERED"
    while [ ! -f "$XO_FAKE_INHERIT_RELEASE" ]; do sleep 0.02; done
    "$XO_FAKE_REMOTE_ENTRYPOINT" "$@" < "$XO_FAKE_INHERIT_PAYLOAD"
    exit $?
    ;;
esac
# The readiness gate is answered here rather than by the real doctor, which
# would inspect and repair the RUNNER's own account. tests/xo-remote-doctor.test.sh
...
if [ "$command_name" = xo-remote-doctor.sh ]; then
  printf '%s %s\n' "${XO_FAKE_SSH_MODE:-normal}" "${_command_action:--}" >> "$XO_FAKE_DOCTOR_LOG"
  case "${XO_FAKE_SSH_MODE:-normal}" in
    ...
    doctor-fix-unknown)
      ...
      printf 'check launchagent=fixable: no XO herdr launch agent\n'
...
if [ "${XO_FAKE_SSH_MODE:-normal}" = doctor-fixable ] \
  && [ "$command_name" = xo-remote-secondmate-control.sh ] \
  && [ "$_command_action" = state ] \
  && [ ! -f "$XO_FAKE_DOCTOR_REPAIRED" ]; then
  printf 'unreadable\n'
  exit 0
fi
case "${XO_FAKE_SSH_MODE:-normal}:$command_name:$command_rel" in
  launch-nonherdr-route:xo-remote-secondmate-control.sh:*)
    [ "$_command_action" = launch ] || exit 93
    printf 'schema=xo-remote-secondmate-control.v1\n'
    printf 'backend=tmux\n'
    printf 'target=xo:xo-ios\n'
    printf 'harness=codex\n'
    exit 0
    ;;
  launch-default-session-route:xo-remote-secondmate-control.sh:*)
    [ "$_command_action" = launch ] || exit 93
    printf 'schema=xo-remote-secondmate-control.v1\n'
    ...
    exit 0
    ;;
  provision-block-fail:xo-remote-home-provision.sh:*)
    touch "$XO_FAKE_SEED_ENTERED"
    while [ ! -f "$XO_FAKE_SEED_RELEASE" ]; do sleep 0.02; done
    exit 1
    ;;
  launch-block:xo-remote-secondmate-control.sh:*)
    [ "$_command_action" = launch ] || exit 93
    touch "$XO_FAKE_LAUNCH_ENTERED"
    while [ ! -f "$XO_FAKE_LAUNCH_RELEASE" ]; do sleep 0.02; done
    ;;
esac
case "${XO_FAKE_SSH_MODE:-normal}" in
  unreachable) exit 255 ;;
  ambiguous) "$XO_FAKE_REMOTE_ENTRYPOINT" "$@"; exit 255 ;;
...
publish_healthy_watcher_identity() { # <state> <home> <watch-script>
  ...
  identity=$(XO_HOME="$PARENT" XO_STATE_OVERRIDE="$PARENT/state" /bin/bash -c \
    '. "$1"; xo_pid_identity "$2"' _ "$ROOT/bin/xo-wake-lib.sh" "$$") \
    || fail "could not derive fixture watcher identity"
  mkdir -p "$state/.watch.lock"
  printf '%s\n' "$$" > "$state/.watch.lock/pid"
  printf '%s\n' "$identity" > "$state/.watch.lock/pid-identity"
  printf '%s\n' "$home" > "$state/.watch.lock/xo-home"
...
remote_env() {
  ...
  XO_PROCEVENT_CLAIM_ROOT="$CLAIMS" \
  XO_SSH_BIN="$FAKEBIN/fake-ssh" \
  XO_FAKE_SSH_COUNT="$SSH_COUNT" \
  ...
  XO_FAKE_REMOTE_CWD="$TMP_ROOT" \
  XO_FAKE_SEED_ENTERED="$TMP_ROOT/seed.entered" \
  XO_FAKE_SEED_RELEASE="$TMP_ROOT/seed.release" \
  XO_FAKE_DOCTOR_LOG="$DOCTOR_LOG" \
  XO_FAKE_DOCTOR_REPAIRED="$TMP_ROOT/doctor.repaired" \
  XO_FAKE_INHERIT_ENTERED="$TMP_ROOT/inherit.entered" \
  XO_FAKE_INHERIT_RELEASE="$TMP_ROOT/inherit.release" \
  XO_FAKE_INHERIT_PAYLOAD="$TMP_ROOT/inherit.payload" \
  XO_FAKE_LAUNCH_ENTERED="$TMP_ROOT/launch.entered" \
  XO_FAKE_LAUNCH_RELEASE="$TMP_ROOT/launch.release" \
  XO_SEND_SETTLE=0 XO_SEND_SLEEP=0 XO_REMOTE_REPLY_WAIT_SECONDS=10 \
  "$@"
}
...
SH
chmod +x "$FAKEBIN/git"
printf 'schema=xo-remote-home-provision.v1\nid_b64=%s\ncharter_b64=%s\nproject_count=0\n' \
  "$(printf ios | base64 | tr -d '\n')" \
  "$(printf 'Concurrent provisioning charter.\n' | base64 | tr -d '\n')" \
  > "$TMP_ROOT/provision.manifest"
PATH="$FAKEBIN:$PATH" XO_HOME="$TMP_ROOT/concurrent-home" XO_ROOT_OVERRIDE="$REMOTE_ROOT" \
  "$REMOTE_ROOT/bin/xo-remote-home-provision.sh" < "$TMP_ROOT/provision.manifest" \
...
[ "$(cat "$TMP_ROOT/concurrent-home/.xo-secondmate-home")" = ios ] \
...
if [ "${XO_TEST_PROVISION_ONLY:-0}" = 1 ]; then
  echo "ALL TESTS PASSED"
  exit 0
fi

mkdir -p "$TMP_ROOT/seed-parent/data" "$TMP_ROOT/seed-parent/state"
XO_SECONDMATE_CHARTER='Failing seed charter.' XO_SECONDMATE_SCOPE='failed seed' \
  XO_FAKE_SSH_MODE=provision-block-fail seed_env "$ROOT/bin/xo-remote-home-seed.sh" \
...
XO_SECONDMATE_CHARTER='Successful seed charter.' XO_SECONDMATE_SCOPE='successful seed' \
  seed_env "$ROOT/bin/xo-remote-home-seed.sh" seed-keep remote-mac "$REMOTE_ROOT" \
...
assert_present "$TMP_ROOT/seed-keep-home/.xo-secondmate-home" "serialized seed lost its published remote home"
pass "remote seed rollback preserves serialized competing routes"

: > "$DOCTOR_LOG"
if XO_SECONDMATE_CHARTER='Unknown readiness charter.' XO_SECONDMATE_SCOPE='unknown readiness' \
  XO_FAKE_SSH_MODE=doctor-fix-unknown seed_env "$ROOT/bin/xo-remote-home-seed.sh" \
...
: > "$DOCTOR_LOG"
if XO_SECONDMATE_CHARTER='Unready host charter.' XO_SECONDMATE_SCOPE='unready host' \
  XO_FAKE_SSH_MODE=doctor-human seed_env "$ROOT/bin/xo-remote-home-seed.sh" \
...
out=$(XO_SECONDMATE_CHARTER='Repairable host charter.' XO_SECONDMATE_SCOPE='repairable host' \
  XO_FAKE_SSH_MODE=doctor-fixable seed_env "$ROOT/bin/xo-remote-home-seed.sh" \
...
assert_present "$TMP_ROOT/seed-repair-home/.xo-secondmate-home" "the repaired host was never provisioned"
...
# Seeding must not need a copy of the project in this home: xo names the
...
xo_git_init_commit "$TMP_ROOT/seed-parent/projects/resident"
# Pin the bare origin's initial branch to the one xo_git_init_commit creates.
...
xo_git_init_commit "$TMP_ROOT/beta-src"
...
if XO_SECONDMATE_CHARTER='Unsupplied origin charter.' XO_SECONDMATE_SCOPE='unsupplied origin' \
  seed_env "$ROOT/bin/xo-remote-home-seed.sh" seed-noorigin remote-mac "$REMOTE_ROOT" \
...
if XO_SECONDMATE_CHARTER='Unsafe origin charter.' XO_SECONDMATE_SCOPE='unsafe origin' \
  seed_env "$ROOT/bin/xo-remote-home-seed.sh" seed-unsafe remote-mac "$REMOTE_ROOT" \
...
if XO_SECONDMATE_CHARTER='Local-only charter.' XO_SECONDMATE_SCOPE='local only' \
  seed_env "$ROOT/bin/xo-remote-home-seed.sh" seed-localonly remote-mac "$REMOTE_ROOT" \
...
if XO_SECONDMATE_CHARTER='Unregistered charter.' XO_SECONDMATE_SCOPE='unregistered' \
  seed_env "$ROOT/bin/xo-remote-home-seed.sh" seed-unregistered remote-mac "$REMOTE_ROOT" \
...
out=$(XO_SECONDMATE_CHARTER='Own beta delivery on the build Mac.' \
  XO_SECONDMATE_SCOPE='beta delivery and validation' \
  seed_env "$ROOT/bin/xo-remote-home-seed.sh" seed-noclone remote-mac "$REMOTE_ROOT" \
...
if XO_HOME="$TMP_ROOT/unsafe-origin-home" XO_ROOT_OVERRIDE="$REMOTE_ROOT" \
  "$REMOTE_ROOT/bin/xo-remote-home-provision.sh" < "$TMP_ROOT/unsafe-origin.manifest" \
...
# XO is a shared template, so seeding must carry a project origin from any
...
forge_project() { # <project> <origin-url>
  ...
  tab=$(printf '\t')
  xo_git_init_commit "$TMP_ROOT/forge-src-$project"
  ...
  git -C "$TMP_ROOT/forge-src-$project" \
    -c user.name='XO Tests' -c user.email='tests@example.invalid' commit -qm origin
...
out=$(XO_SECONDMATE_CHARTER='Own delivery for projects hosted anywhere.' \
  XO_SECONDMATE_SCOPE='multi-forge delivery' \
  seed_env "$ROOT/bin/xo-remote-home-seed.sh" seed-forge remote-mac "$REMOTE_ROOT" \
  "$FORGE_HOME" \
...
out=$(XO_SECONDMATE_CHARTER='Own iOS delivery on the build Mac.' \
  XO_SECONDMATE_SCOPE='iOS implementation and Xcode validation' \
  remote_env "$ROOT/bin/xo-remote-home-seed.sh" ios remote-mac "$REMOTE_ROOT" "$REMOTE_HOME" alpha)
...
assert_present "$REMOTE_HOME/.xo-secondmate-home" "remote provisioning did not publish the identity marker"
...
if XO_SECONDMATE_CHARTER='Own iOS delivery on the build Mac.' \
  XO_SECONDMATE_SCOPE='iOS implementation and Xcode validation' \
  remote_env "$ROOT/bin/xo-remote-home-seed.sh" ios remote-mac "$REMOTE_ROOT" "$TMP_ROOT/other-home" alpha \
  >/dev/null 2>&1; then
...
if printf 'complete' | XO_HOME="$PROTOCOL_HOME" "$REMOTE_ROOT/bin/xo-remote-inherit.sh" \
...
XO_HOME="$PROTOCOL_HOME" "$REMOTE_ROOT/bin/xo-remote-inherit.sh" \
...
if XO_HOME="$PROTOCOL_HOME" "$REMOTE_ROOT/bin/xo-remote-inherit.sh" \
...
EOF
remote_env "$ROOT/bin/xo-home-seed.sh" validate >/dev/null || fail "mixed local and remote registry validation failed"
...
if XO_FAKE_SSH_MODE=inherit-partial remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate \
...
out=$(remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate)
...
assert_grep 'remote_herdr_session=xo-remote' "$PARENT/state/ios.meta" "parent metadata omitted the pinned remote Herdr session"
assert_grep 'remote_target=xo-remote:' "$PARENT/state/ios.meta" "parent metadata did not record an xo-remote endpoint"
assert_grep 'herdr_session=xo-remote' "$REMOTE_HOME/state/parent-route/ios.meta" "remote metadata did not record the pinned Herdr session"
assert_grep '--session xo-remote' "$HERDR_LOG" "remote launch did not target the xo-remote session"
...
publish_healthy_watcher_identity "$PARENT/state" "$PARENT" "$ROOT/bin/xo-watch.sh"
[ "$(remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh state ios)" = alive ] \
...
[ "$(remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh observe ios)" = idle ] \
...
[ "$(remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh state ios 2>/dev/null)" = unverified ] \
  || fail "legacy default-session metadata was not classified unverified"
if remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh route ios >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh send ios probe >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh key ios Enter >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh capture ios >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh observe ios >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh retire ios --force >/dev/null 2>&1 \
  || remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh launch ios codex - - herdr >/dev/null 2>&1; then
...
' "$TMP_ROOT/remote-ios-before-default-session.meta" > "$remote_route_meta"
[ "$(remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh state ios 2>/dev/null)" = unverified ] \
  || fail "mismatched xo-remote target was not classified unverified"
cmp -s "$TMP_ROOT/herdr-before-default-session.log" "$HERDR_LOG" \
  || fail "mismatched xo-remote target caused a Herdr operation"
...
set +e
XO_FAKE_SSH_MODE=launch-nonherdr-route remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate \
...
set +e
XO_FAKE_SSH_MODE=launch-default-session-route remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate \
...
assert_grep "remote launch returned Herdr session 'default', expected 'xo-remote'" "$TMP_ROOT/spawn-default-session-route.out" \
...
set +e
remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh launch ios codex - - herdr \
...
This file is main-authoritative and maintained by the main xo.
...
EOF
XO_FAKE_SSH_MODE=inherit-block remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate \
...
EOF
remote_env "$ROOT/bin/xo-config-push.sh" > "$TMP_ROOT/spawn-concurrent-push.out" 2>&1 &
spawn_config_push=$!
sleep 0.2
...
set +e
XO_FAKE_SSH_MODE=ambiguous remote_env "$ROOT/bin/xo-send.sh" xo-ios \
...
assert_grep 'XO instruction waiting' "$HERDR_LOG" "the remote doorbell never rang"
CORR=$(newest_remote_inbox_corr)
[ -n "$CORR" ] || fail "remote send did not carry a correlation token"
assert_grep "XO_PENDING_REPLY_EXISTING_CORR=$CORR" "$TMP_ROOT/send.err" "ambiguous remote send did not print its correlation-reusing command"
...
remote_env "$ROOT/bin/xo-procevent.sh" start "$SID" >/dev/null \
  || fail "remote reply source did not capture the correlated answer"
RESULT="$PARENT/state/procevent-inbox/$SID.1.result"
remote_env "$ROOT/bin/xo-procevent-remote-reply.sh" handle ios 1 "$RESULT" >/dev/null \
...
printf 'grok\n' > "$PARENT/config/crew-harness"
set +e
XO_FAKE_SSH_MODE=inherit-partial remote_env "$ROOT/bin/xo-config-push.sh" \
...
publish_healthy_watcher_identity "$PARENT/state" "$PARENT" "$REMOTE_ROOT/bin/xo-watch.sh"
remote_env "$ROOT/bin/xo-bootstrap.sh" > "$TMP_ROOT/config-partial-retry.out" \
...
remote_env "$ROOT/bin/xo-procevent-remote-reply.sh" handle ios 2 "$PARTIAL_CONFIG_RESULT" >/dev/null \
...
EOF
XO_FAKE_SSH_MODE=inherit-block remote_env "$ROOT/bin/xo-config-push.sh" \
...
EOF
remote_env "$ROOT/bin/xo-bootstrap.sh" > "$TMP_ROOT/config-concurrent-second.out" 2>&1 &
config_second=$!
sleep 0.2
...
if remote_env "$ROOT/bin/xo-config-push.sh" > "$TMP_ROOT/config-push-fail.out" 2>&1; then
...
chmod 755 "$REMOTE_HOME/state/parent-route/ios.inbox"
remote_env "$ROOT/bin/xo-config-push.sh" > "$TMP_ROOT/config-push-retry.out" \
...
remote_env "$ROOT/bin/xo-procevent-remote-reply.sh" handle ios 3 "$CONFIG_RESULT" >/dev/null \
...
resolve_ios_pending() {
  ...
  for pending_record in "$PARENT/state/pending-replies"/*; do
    ...
    remote_env "$ROOT/bin/xo-procevent-remote-reply.sh" handle ios "$pending_seq" "$pending_result" >/dev/null \
...
XO_ROOT_OVERRIDE="$ROOT" XO_HOME="$LOCAL_HOME" \
  "$ROOT/bin/xo-home-summary-refresh.sh" >/dev/null \
  || fail "local fixture did not publish its home ledger"
remote_env "$ROOT/bin/xo-on.sh" ios xo-home-summary-refresh.sh >/dev/null \
  || fail "remote fixture did not publish its home ledger"
SNAPSHOT=$(remote_env "$ROOT/bin/xo-fleet-snapshot.sh" --json)
...
UPDATE_OUT=$(remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh update ios)
...
RELAUNCH_UNVERIFIED=$(remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh \
...
RELAUNCH_CHECKPOINT=$(remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh \
...
rm -f "$TMP_ROOT/doctor.repaired"
: > "$DOCTOR_LOG"
[ "$(XO_FAKE_SSH_MODE=doctor-fixable remote_env "$ROOT/bin/xo-on.sh" ios xo-remote-secondmate-control.sh state ios)" = unreadable ] \
...
BOOT_REPAIRED=$(XO_FAKE_SSH_MODE=doctor-fixable remote_env "$ROOT/bin/xo-bootstrap.sh")
[ "$(cat "$DOCTOR_LOG")" = 'doctor-fixable -
doctor-fixable --fix
...
export XO_HERDR_PAIR_DIR="$TMP_ROOT/client-pair"
SHADOWED_STATE=$(XO_HOME="$REMOTE_HOME" XO_ROOT_OVERRIDE="$REMOTE_ROOT" \
  PATH="$TMP_ROOT/client-pair/stale:$REMOTE_ROOT/bin:$TMP_ROOT/client-pair/tools:/usr/bin:/bin" \
  "$REMOTE_ROOT/bin/xo-remote-secondmate-control.sh" state ios 2>"$TMP_ROOT/shadowed-state.err")
...
BOOT_LEGACY=$(remote_env "$ROOT/bin/xo-bootstrap.sh")
...
BOOT_UNAVAILABLE=$(XO_FAKE_SSH_MODE=unreachable remote_env "$ROOT/bin/xo-bootstrap.sh")
...
UNAVAILABLE=$(XO_FAKE_SSH_MODE=unreachable remote_env "$ROOT/bin/xo-fleet-snapshot.sh" --json)
...
# A sibling remote secondmate workspace shares xo-remote and must survive every
# refusal and the eventual successful retirement of ios.
# This fixture overrides XO_ROOT for transport, so teardown's root-owned guard
# sees the fixture root rather than the source script path used by xo-send.
publish_healthy_watcher_identity "$PARENT/state" "$PARENT" "$REMOTE_ROOT/bin/xo-watch.sh"
resolve_ios_pending
SIBLING_CREATE=$("$REMOTE_ROOT/bin/herdr" workspace create --cwd "$REMOTE_ROOT" \
  --label 2ndmate-macos --no-focus --session xo-remote)
...
if remote_env "$ROOT/bin/xo-teardown.sh" ios >/dev/null 2>&1; then
...
remote_env "$ROOT/bin/xo-bootstrap.sh" >/dev/null \
...
retired_wake_corr=$(XO_HOME="$PARENT" bash -c '
  . "$1"
  xo_pending_reply_create "$2" "$2/state" ios "New routed work is in your backlog."
' _ "$ROOT/bin/xo-pending-reply-lib.sh" "$PARENT") \
  || fail "could not seed remote receiver wake retirement state"
retired_wake_rec="$PARENT/state/pending-replies/$retired_wake_corr"
XO_HOME="$PARENT" bash -c '
  . "$1"
  xo_pending_reply_set "$2" phase resolved
  xo_pending_reply_set "$2" delivered_epoch 1
' _ "$ROOT/bin/xo-pending-reply-lib.sh" "$retired_wake_rec" \
...
XO_HOME="$PARENT" /bin/bash -c '
  . "$1"
  xo_lock_acquire_wait "$2"
  touch "$3"
  while [ ! -f "$4" ]; do sleep 0.02; done
  xo_lock_release "$2"
' _ "$ROOT/bin/xo-wake-lib.sh" "$handoff_lock" "$TMP_ROOT/handoff.entered" \
...
XO_FAKE_SSH_MODE=launch-block remote_env "$ROOT/bin/xo-spawn.sh" ios --secondmate \
...
remote_env "$ROOT/bin/xo-teardown.sh" ios > "$TMP_ROOT/teardown-serialized.out" 2>&1 &
teardown_pid=$!
sleep 0.2
...
' "$HERDR_STATE" >/dev/null \
  || fail "remote retirement removed the sibling secondmate workspace or pane from xo-remote"
assert_no_grep 'session stop' "$HERDR_LOG" "remote retirement stopped the shared xo-remote session"
assert_no_grep 'server stop' "$HERDR_LOG" "remote retirement stopped the shared xo-remote server"
