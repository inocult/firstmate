...
# tests/xo-cursor-primary-live-e2e.test.sh.
#
# Three facts in this area come from the vendor, not from XO, so a stub
...
#       (bin/xo-startup-network.sh), so a harness that reaps the hook's process
...
# tests/xo-sessionstart-nudge.test.sh pins that routing portably with real
# processes and no harness. This guard covers only what CI cannot see.
#
# It swaps a RECORDER in for bin/xo-sessionstart-run.sh inside a throwaway lab
...
xo_live_gate opt-in XO_SESSIONSTART_HOOK_LIVE_E2E,XO_PI_SESSIONSTART_RACE_LIVE_E2E tmux
...
LAB="${TMPDIR:-/tmp}/xo-sessionstart-hook-live-e2e.$$"
SOCKET="xo-ss-hook-$$"
CHECKED=0
ABSENT=
...
# --- lab ---------------------------------------------------------------------
#
# An XO-shaped checkout carrying the harness's own TRACKED registration,
...
make_lab() {  # <harness> -> echoes lab dir
  ...
  for stub in xo-turnend-guard.sh xo-claude-stop-autoarm.sh xo-arm-pretool-check.sh \
    xo-cd-pretool-check.sh xo-subagent-pretool-check.sh; do
  ...
  # tests/xo-bootstrap.test.sh already owns.
  ln -sf "$ROOT/bin/xo-startup-network.sh" "$lab/bin/xo-startup-network.sh"
  ln -sf "$ROOT/bin/xo-timeout-lib.sh" "$lab/bin/xo-timeout-lib.sh"
  ln -sf "$ROOT/bin/xo-wake-lib.sh" "$lab/bin/xo-wake-lib.sh"
  ln -sf "$ROOT/bin/xo-session-lock-lib.sh" "$lab/bin/xo-session-lock-lib.sh"
  cat > "$lab/bin/xo-bootstrap.sh" <<'SH'
#!/usr/bin/env bash
...
set -u
sleep 6
printf 'detached worker survived the hook\n' > "${XO_LIVE_DETACH_MARKER:?}"
exit 0
SH
  chmod +x "$lab/bin/xo-bootstrap.sh"

  cat > "$lab/bin/xo-sessionstart-run.sh" <<'SH'
#!/usr/bin/env bash
...
printf '%s\n' "$source" >> "$record"
# Exactly what bin/xo-session-start.sh does after taking the lock.
if [ -n "${XO_LIVE_DETACH_MARKER:-}" ]; then
  "$(dirname "$0")/xo-startup-network.sh" start --locked 0 --harvest-pid $$ >/dev/null 2>&1 || true
fi
printf 'FMHOOKTOKEN-%s-%s-%s\n' "$source" "$(grep -c . "$record" | tr -d ' ')" "${XO_LIVE_NONCE:?}"
exit 0
SH
  chmod +x "$lab/bin/xo-sessionstart-run.sh"
  ...
  case "$harness" in
    ...
    pi)
      ...
      cp "$ROOT/.pi/extensions/xo-primary-turnend-guard.ts" "$lab/.pi/extensions/"
      cp "$ROOT/.pi/extensions/lib/xo-operational-input.ts" \
        "$ROOT/.pi/extensions/lib/xo-sessionstart-supervisor.mjs" "$lab/.pi/extensions/lib/"
      cp "$ROOT/bin/xo-operational-input.sh" "$lab/bin/"
...
probe_process_opens() {  # <harness> <version> <lab> <expect-resume> <cold-argv...> -- <resume-argv...>
  ...
  out=$( cd "$lab" && XO_LIVE_RECORD="$record" XO_LIVE_NONCE="$LIVE_NONCE" XO_ROOT_OVERRIDE="$lab" XO_HOME="$lab" \
    XO_LIVE_DETACH_MARKER="$marker" \
  ...
  : > "$record"
  ( cd "$lab" && XO_LIVE_RECORD="$record" XO_LIVE_NONCE="$LIVE_NONCE" XO_ROOT_OVERRIDE="$lab" XO_HOME="$lab" \
...
probe_context_reset() {  # <harness> <version> <lab> <clear-command> <launch-argv...>
  ...
  tmux -L "$SOCKET" new-session -d -s "$session" -c "$lab" -x 200 -y 50 \
    -e XO_LIVE_RECORD="$record" -e XO_ROOT_OVERRIDE="$lab" -e XO_HOME="$lab" \
    -e XO_LIVE_NONCE="$LIVE_NONCE" \
    "$*" \
...
probe_pi_sessionstart_prerequisite() {
  ...
  cp "$ROOT/bin/xo-operational-input.sh" "$project/bin/"
...
export default function (pi: ExtensionAPI): void {
  pi.registerProvider("race-local", {
    ...
    streamSimple(model, context) {
      ...
      if (prompt !== "other") {
        appendFileSync(
          `${process.env.XO_HOME}/state/provider-calls`,
      ...
      queueMicrotask(() => {
        ...
        if (prompt !== "other" && nativeCount === 0 && !manual) {
          ...
          const toolCall = {
            ...
            id: `manual-${Date.now()}`,
            name: "bash",
            arguments: { command: "bin/xo-session-start.sh" },
          };
...
TS
  ...
  tmux -L "$SOCKET" new-session -d -s "$session" -c "$project" -x 180 -y 50 \
    "env XO_HOME='$home' XO_ROOT_OVERRIDE='$project' PI_CODING_AGENT_DIR='$config' PI_OFFLINE=1 pi --approve --session-dir '$sessions' --no-context-files --no-skills --no-prompt-templates --tools bash --model race-local/deterministic; rc=\$?; printf '\nPI_EXIT=%s\n' \"\$rc\"; sleep 60" \
...
if [ "${XO_PI_SESSIONSTART_RACE_LIVE_E2E:-0}" = 1 ]; then
  probe_pi_sessionstart_prerequisite
  if [ "${XO_SESSIONSTART_HOOK_LIVE_E2E:-0}" != 1 ]; then
    echo "# xo-sessionstart-hook-live-e2e.test.sh: offline Pi /new race assertions passed"
    exit 0
  fi
fi
  ...
  case "$harness" in
    ...
    pi)
      probe_process_opens pi "$version" "$lab" resume \
        pi -p -e "$lab/.pi/extensions/xo-primary-turnend-guard.ts" --no-context-files --no-tools \
        -- pi -p -c -e "$lab/.pi/extensions/xo-primary-turnend-guard.ts" --no-context-files --no-tools
      probe_context_reset pi "$version" "$lab" /new \
        pi -e "$lab/.pi/extensions/xo-primary-turnend-guard.ts" --no-context-files
      ;;
  esac
  CHECKED=$((CHECKED + 1))
done
...
echo "# xo-sessionstart-hook-live-e2e.test.sh: all live assertions passed"
