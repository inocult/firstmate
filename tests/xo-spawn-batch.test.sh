#!/usr/bin/env bash
# Behavior tests for xo-spawn.sh batch dispatch (`id=repo` pairs).
#
...
# the tests create no windows or worktrees. XO_SPAWN_NO_GUARD=1 keeps them off the
...
TMP_ROOT=$(xo_test_tmproot xo-spawn-batch)
export XO_BACKEND=tmux

# Clear ambient xo overrides so the behavior test owns its environment.
run_spawn() {
  XO_ROOT_OVERRIDE='' \
    XO_HOME='' \
    XO_STATE_OVERRIDE='' \
    XO_DATA_OVERRIDE='' \
    XO_PROJECTS_OVERRIDE='' \
    XO_CONFIG_OVERRIDE='' \
    XO_SPAWN_NO_GUARD=1 \
    "$SPAWN" "$@" 2>&1
}
...
# A projects/ path is resolved through the xo home, never the caller cwd,
...
test_projects_path_scoping() {
  ...
  while IFS='|' read -r label use_override id; do
    ...
    if [ "$use_override" = yes ]; then
      out=$(XO_ROOT_OVERRIDE='' XO_STATE_OVERRIDE='' XO_DATA_OVERRIDE='' XO_CONFIG_OVERRIDE='' \
        XO_HOME="$home" XO_PROJECTS_OVERRIDE="$projects" XO_SPAWN_NO_GUARD=1 \
      ...
      out=$(XO_ROOT_OVERRIDE='' XO_STATE_OVERRIDE='' XO_DATA_OVERRIDE='' XO_PROJECTS_OVERRIDE='' XO_CONFIG_OVERRIDE='' \
        XO_HOME="$home" XO_SPAWN_NO_GUARD=1 \
        "$SPAWN" "$id" projects/alpha codex --mode no-mistakes --yolo off 2>&1)
    fi
    status=$?
...
XO_HOME scopes projects/|no|nope-home-z7
XO_PROJECTS_OVERRIDE scopes projects/|yes|nope-override-z8
ROWS
  pass "projects/ paths are scoped through the xo home for single-task spawn"
}
