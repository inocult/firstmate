...
import { encodeXoOperationalInput } from "./lib/xo-operational-input.js";

const COORDINATOR_KEY = "__xoOpenCodeWatchArm";
...
const ARM_READY_TIMEOUT_MS = positiveInteger("XO_OPENCODE_ARM_READY_TIMEOUT_MS", ARM_READY_TIMEOUT_DEFAULT_MS);
const ARM_RETIRE_TIMEOUT_MS = positiveInteger("XO_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const REARM_RETRY_BASE_MS = positiveInteger("XO_WATCH_REARM_RETRY_BASE_MS", 250);
const REARM_RETRY_MAX_MS = positiveInteger("XO_WATCH_REARM_RETRY_MAX_MS", 4000);
const REARM_RETRY_LIMIT = positiveInteger("XO_WATCH_REARM_RETRY_LIMIT", 5);
...
function effectivePaths(root) {
  ...
  const fmHome = process.env.XO_HOME || process.env.XO_ROOT_OVERRIDE || fmRoot;
...
async function isPrimaryRoot(root, home) {
  ...
  if (existsSync(`${root}/.xo-secondmate-home`)) return false;
  if (home && home !== root && existsSync(`${home}/.xo-secondmate-home`)) return false;
...
function classifyArmClose(stdout, stderr, code, signal) {
  ...
  if (code && code !== 0) {
    return {
      kind: "failure",
      message: `watcher: FAILED - xo-watch-arm.sh exited ${code}${combined.trim() ? `\n${combined.trim()}` : ""}`,
    };
  }
  return {
    kind: "failure",
...
async function sendPrompt(paths, client, sessionID, text) {
  const encoded = await encodeXoOperationalInput(paths.root, "watcher", text);
...
function confirmHandlingDelivery(paths, recovery) {
  try {
    const result = spawnSync(
      "bash",
      [`${paths.root}/bin/xo-watch-arm.sh`, "--handling-delivered", recovery.generation, "--watcher-pid", recovery.watcherPid],
      {
        cwd: paths.root,
        encoding: "utf8",
        env: { ...process.env, XO_HOME: paths.home, XO_STATE_OVERRIDE: paths.state, XO_ROOT_OVERRIDE: paths.root },
      },
    );
...
function wakePrompt(reason) {
  return `WATCHER FIRED - drain queued wakes with bin/xo-wake-drain.sh and handle the reported wake. Watcher continuity is plugin-owned.\n\n${reason}`;
}
...
function spawnArm(paths, sessionID, client, predecessorArmPid = "") {
  ...
  const env = {
    ...process.env,
    XO_HOME: paths.home,
    XO_ROOT_OVERRIDE: paths.root,
    XO_CONFIG_OVERRIDE: paths.config,
    XO_WATCH_PREDECESSOR_ARM_PID: predecessorArmPid,
  };
  const armChild = spawn("bash", ["-lc", 'config_dir="${XO_CONFIG_OVERRIDE:-$XO_HOME/config}"; [ -f "$config_dir/x-mode.env" ] && . "$config_dir/x-mode.env"; exec "$XO_ROOT_OVERRIDE/bin/xo-watch-arm.sh" --restart'], {
    cwd: paths.root,
    env,
    stdio: ["ignore", "pipe", "pipe"],
  });
...
export const XoPrimaryWatchArm = async ({ client, directory, worktree }) => {
