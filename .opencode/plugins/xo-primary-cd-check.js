...
// the primary xo checkout before the agent's bash tool relocates the
// shell out of the home (see bin/xo-cd-pretool-check.sh and docs/cd-guard.md).
// This mirrors xo-primary-pretool-check.js, calling the cd-guard owner instead
...
export const XoPrimaryCdCheck = async ({ directory, worktree }) => {
  const root = worktree ? (() => {
    try {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const result = await runProcess(`${root}/bin/xo-cd-pretool-check.sh`, ["--command", command]);
