---
name: breach
description: Implement a selected Plane ticket through a claimed Firstmate operative, adapting Matt Pocock's implement workflow to isolated worktrees and supervised PR delivery.
metadata:
  internal: true
---
...
Resolve a human-readable ticket identifier to the actual UUID and verify the configured Plane project and implementation repository before claiming.
Read the ticket and its acceptance criteria; the adapter must verify readiness and dependencies.
...
Provide the immutable reviewed head SHA, base/merge-base SHA, exact Plane ticket/spec and repo standards; keep Standards and Spec findings separate.
...
Only verified merge and accepted criteria permit the adapter's completion transition to Done.
