...
name: xo-codexapp
description: >-
  Agent-only playbook for coordinating visible Codex Desktop threads alongside XO without pretending they are a selectable shell backend.
  Use before creating, reading, steering, archiving, debugging, or reviewing a Codex App visible thread for XO work, and before responding to requests to make Codex App native to XO.
...
Use this playbook when XO work needs a visible Codex Desktop thread.
The current supported shape is Desktop host-tool choreography plus an explicit status-file return-channel check, not a `codex-app` value in `XO_BACKEND`.

## Boundary

Codex Desktop visible threads are companion host-tool workflows, not a selectable XO backend.
...
If local helper scripts exist for Codex App work, use only helpers explicitly provided by the operator or maintained by XO.
...
3. Do not create projectless threads for repo work.
   If the project is absent, stop and ask for the project to be added or use a normal XO backend instead.
4. Decide whether this is a real XO-managed task or a visible companion thread.
...
A Desktop-owned Codex thread can append to XO status files only when the prompt gives an absolute path and the Desktop permission context can write that checkout.
That makes status writes a verified return-channel requirement, not a fact to assume.

For an XO-managed task, include an explicit status instruction:

```text
Append supervisor-visible status lines to <absolute-xo-home>/state/<task-id>.status.
Use only these prefixes for status changes: working:, needs-decision:, blocked:, paused:, done:, failed:.
Use paused: only for a deliberate known external wait that should be rechecked later, never for a blocker that needs xo to act.
...
Do not claim it is a complete XO backend.
...
For XO reconciliation, prefer concrete evidence:
...
Avoid repeating long transcripts into XO docs or PR bodies.
...
If there is a real XO task record, leave teardown decisions to the normal XO task flow instead of this skill.
