...
- **Work that spawns a real, longer-running job** (dispatching a crewmate, a scout investigation, a ship task) cannot report an outcome yet, so it follows **acknowledge first -> act -> follow up on completion**:
  ...
  3. **Bind the follow-up to wherever the work actually lives, before clearing the inbox.**
     **The decision rule: work that stays in this home takes the lightweight link; work routed to a second mate takes a promised-final commitment bound to that second mate's home.**
     ...
     - **Local task (this home spawned it):** `bin/xo-x-link.sh <task-id> <request_id>` (records the request id, a timestamp, a follow-up counter, and reply platform/budget context).
       Do this right after the task is spawned, and always **before** removing the inbox file (step 2f).
       Linking before cleanup lets `bin/xo-x-link.sh` copy the context directly from the inbox, while the durable per-request context recorded by the poll preserves it independently for delayed and concurrent follow-ups.
     ...
     - **Second-mate-routed work (the request's project or domain belongs to a registered second mate, so the work is or will be routed there):** the link cannot be used at all.
       It writes into this home's own `state/<task-id>.meta`, and a routed task's record lives in the second mate's home, so `bin/xo-x-link.sh` refuses and points you back here.
       Register a **typed promised-final commitment bound to that home** up front instead - see "Promised final replies" below for the exact commands - and put its `bin/xo-public-followup.sh brief <obligation-id>` output into the routed worker's instructions so the terminal result comes back as typed data.
...
- **Pure acknowledgment** ("thanks", a reaction, a loop-closing nicety with nothing to add) - skip: post nothing, but first **dismiss it at the relay** (`bin/xo-x-dismiss.sh <request_id>`) so the relay drops the request and stops re-offering it, then clear the inbox file.
...
Inbound media arrives as URLs in the payload, and you fetch and view it with your own tools; XO never downloads it for you.
...
Compose the reply as one piece of prose; if it is genuinely too long for one message, `bin/xo-x-reply.sh` automatically splits it into a platform-aware numbered thread on fenced-code, paragraph, line, and word boundaries.
...
2. **Drain every pending mention.** For each `state/x-inbox/*.json` file:
   ...
   c. **Act on an actionable request through the normal lifecycle.** Treat it exactly as a captain prompt typed in session: run ordinary intake (resolve the project), then file the backlog item, dispatch a crewmate, start a scout, or ship through the gate - whatever the request calls for.
      ...
      **If the request spawned a real, longer-running task in THIS home** (you ran `bin/xo-spawn.sh` here), link that task to this mention so milestone and completion follow-ups can be posted: `bin/xo-x-link.sh <task-id> <request_id>`.
      **Link here, in step 2c, before the step 2f inbox cleanup** - `bin/xo-x-link.sh` can copy both the mention's reply platform and explicit budget from the still-present inbox payload without a relay lookup.
   ...
   e. **Submit it without ever inlining the reply into a shell command.**
      ...
      ```sh
      bin/xo-x-reply.sh <request_id> --text-file <path-to-reply-file>
      ```

      (`bin/xo-x-reply.sh <request_id> -`, reading the reply on stdin, is equally fine.) It echoes the `request_id` and exits 0 on success; non-zero on a failed live post or failed dry-run record.
   ...
   g. **On failure** (a non-zero exit from `bin/xo-x-reply.sh` or `bin/xo-x-dismiss.sh`), leave that inbox file in place, move on to the next, and do not retry blindly.
...
- XO has **up to three** follow-ups per mention, within a 7-day window, chained in the same thread - it spends them only on genuine milestones the captain would want surfaced (e.g. investigation done and a build started, work shipped or ready, or the task failing), never on routine internal churn.
- If a linked task is replaced by a successor for the same relay request, carry the prior `x_followups=`, `x_request_ts=`, `x_platform=`, and `x_reply_max_chars=` values with `bin/xo-x-link.sh <new-task-id> <request_id> --carry-count <n> --carry-ts <epoch> --carry-platform <x|discord> --carry-max <n>` so recovery preserves the consumed budget, original window, and reply split budget after the inbox file is gone.
- On each such milestone, XO checks whether a follow-up is still due with `bin/xo-x-followup.sh --check <task-id>` (prints the `request_id` when the link exists, the count is under the cap, and the window has not lapsed; silent otherwise, pruning an exhausted or expired link).
- If due, it composes a short, public-safe update and posts it with `bin/xo-x-followup.sh <task-id> --text-file <path>` (or stdin), which posts via the relay's follow-up endpoint; a successful non-final post increments the counter and keeps the link so a later milestone can still post against it.
  When the update carries one real visual artifact, add `--image <path>`; the helper forwards it to `bin/xo-x-reply.sh --followup` so the same image contract used for ordinary replies applies here too.
- On a terminal wake (PR merged / scout report / local merge / failed), XO posts the task's **final** outcome ("done, here's the result"; for a failure, an honest "this one didn't pan out") with `bin/xo-x-followup.sh <task-id> --final --text-file <path>` only when no promised-final public commitment is registered for that work. When the promised-final procedure above applies, `bin/xo-public-followup.sh consume` and `deliver` own the terminal reply and clear the legacy link at the validated receipt boundary, so do not call `xo-x-followup.sh --final` for the same outcome. If delivery reports that link cleanup needs reconciliation, do not post anything else; `bin/xo-x-followup.sh --clear <task-id>` is the clear-only recovery command in the bound work home.
...
`tasks-axi public-followup --help` owns the typed obligation, its states, and its file contracts; `bin/xo-public-followup.sh --help` owns XO's flags; do not restate either here.
...
2. Register it with `bin/xo-public-followup.sh register <obligation-id> --relation <relation-id> --work-home <main|secondmate:<id>> --work-id <task-id> --generation <n>`.
   This is what makes the commitment reconcilable without you.
3. Put `bin/xo-public-followup.sh brief <obligation-id>` output straight into the worker's brief.
...
1. Run `bin/xo-public-followup.sh consume`.
...
2. For each ready commitment, run `bin/xo-public-followup.sh deliver <obligation-id>`.
...
4. After a successful deliver (or when the digest lists an `open-loop` line), decide the disposition in that same turn:
   ...
   - The public loop is finished: `bin/xo-public-followup.sh retire <id> --reason "<why the loop is done>"`.
...
- Never edit `bin/xo-x-poll.sh`, `bin/xo-x-reply.sh`, or the watcher to "answer faster"; the cadence is handled by the locked session-start bootstrap step.
