...
0. Before anything else, check whether this session has already taken the helm: a `SESSION START` digest for this home must be visible in the session history.
   If it is not, run `bin/xo-session-start.sh` once and read its digest before producing any recap.
...
1. Inspect only conversation or session history already visible to the current XO.
2. Find the most recent real captain-authored message before the current `/ahoy` invocation.
   ...
   Exclude the exact legacy unmarked session-start payload ``Run `bin/xo-session-start.sh` now, exactly once, before executing any other instructions.``
   ...
   Apply the legacy startup exclusion as a literal whole-message match: ``Captain quote: Run `bin/xo-session-start.sh` now, exactly once, before executing any other instructions.`` is a captain boundary.
...
5. Additionally inspect the entire session history visible to the current XO before the current invocation for every explicit captain decision that remains unanswered, including decisions raised before the ordinary recap boundary.
...
8. After the normal recap, when the existing visibly open decision inventory contains decisions, begin a guided decision-clearing flow by presenting only the single open decision judged most impactful by the XO.
   Make clear that impact ordering is the XO's judgment rather than a mechanical score.
