...
Conversation and harness-native skill invocation use `../../../bin/xo-send.sh`.
`../../../docs/agent-control.md` owns the data-plane split, and `../../../bin/xo-control-lib.sh` owns executable capabilities.
...
Select only its documented trust choice from the active XO home, binding `XO_HOME` unless already correct, then inspect again under the router-owned completion postcondition.
...
A Claude secondmate is deliberately not pre-registered, because `../../../bin/xo-spawn.sh` runs its per-harness pre-launch setup only for non-secondmate kinds, so the registration is never invoked for one.
...
The consequence is that a claude secondmate whose home Claude has never trusted meets the workspace-trust dialog itself, and XO cannot answer it any more than it can for a crewmate.
...
Popup, queued-input, and readiness handling belongs to `../../../bin/xo-composer-lib.sh` and the selected backend.
