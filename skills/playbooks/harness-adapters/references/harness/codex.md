...
| Fact | Value |
|---|---|
| Busy state | Unknown until a semantic source is live-verified: the app-server turn lifecycle is unreachable for a pane worker, and project lifecycle hooks did not fire for an XO-launched worker. |
...
`../../../bin/xo-send.sh` gives a leading `$` a 1.2-second settle before the first Enter only when the exact task metadata records `harness=codex`, with the target backend's submit retry as the safety net.
...
The XO primary's `.codex/hooks.json` registers a Stop hook that pipes Codex's payload to `../../../bin/xo-turnend-guard.sh`.
...
The tracked hook anchors to `pwd -P`, verifies that root is XO-shaped and hook-bearing, and then invokes the guard with the original payload.

Codex's primary watcher protocol is `../../../bin/xo-watch-checkpoint.sh --seconds "${XO_CODEX_WATCH_CHECKPOINT:-180}"`, not `../../../bin/xo-watch-arm.sh`.
