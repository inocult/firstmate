...
| Fact | Value |
|---|---|
| Busy state | The XO-owned plugin's semantic `session.status`: `busy` and `retry` are active, `idle` is inactive, latched to the worker's own session. |
| Exit command | `/exit`. |
| Interrupt | Double Escape; it is known to be flaky while a long shell command runs, so use `../../../bin/xo-control.sh <task-id> relaunch` for a wedged pane. |
...
| Effort flag | None for XO's interactive `opencode --prompt` launch verified on 1.17.6; `opencode run` has `--variant`, but that is not this path. |
...
Tmux and Herdr delegate this exception to the one `xo_composer_queued_enter_verdict` policy in `../../../bin/xo-composer-lib.sh`.
Backend-specific signals are documented in `../../../docs/tmux-backend.md` and `../../../docs/herdr-backend.md`.
Regression coverage is `../../../tests/xo-tmux-submit-busy.test.sh`, `../../../tests/xo-composer-lib.test.sh`, and `../../../tests/xo-backend-herdr.test.sh`.
The live Herdr guard is `XO_HERDR_SUBMIT_CONFIRM_LIVE=1 ../../../tests/xo-herdr-submit-confirm-live-e2e.test.sh`.

## Primary integration

The primary integration was verified on 2026-07-08 with OpenCode 1.17.6.
`.opencode/plugins/xo-primary-turnend-guard.js` listens for `session.idle`.
Throwing from `session.idle` does not block `opencode run`, so the primary adapter treats the event as passive and uses `client.session.promptAsync` to force one follow-up turn when `../../../bin/xo-turnend-guard.sh` returns 2.
...
The companion `.opencode/plugins/xo-primary-watch-arm.js` owns normal TUI watcher supervision, wakes it with `client.session.promptAsync`, and coordinates with the guard before a blind-turn follow-up.
