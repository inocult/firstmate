...
Verified as a CREWMATE and SCOUT adapter only; `../../../../../bin/xo-spawn.sh` refuses a secondmate launch on it because `../../../../../docs/supervision-protocols/` carries no gemini wake protocol.
...
An XO-repo task needs exactly those workspace skills, so the spawn uses the environment variable and `--skip-trust` must not be substituted for it.
XO's OWN busy hooks do not depend on this, because they ride the system settings layer described below.
...
A Gemini worker needs a credential it can use without a dialog, and XO does not manage one.
...
`GEMINI_CLI=1` is load-bearing rather than a fast path, so `../../../../../bin/xo-harness.sh` checks it BEFORE `CLAUDECODE`.
...
`../../../../../tests/xo-gemini-harness.test.sh` pins both the marker precedence and this ancestry boundary.
...
A live gemini pane's foreground group reads `comm=MainThread` and `argv0=<node path>`, so neither of `bin/backends/tmux.sh`'s existing name sources can see it, and `bin/xo-control.sh` refused every lifecycle verb with `endpoint reads 'ambiguous'` until this was closed.
`../../../../../bin/xo-gemini-lib.sh` owns the narrow structural rule that fixes it: identity comes from argv[1], the script argument, accepted only when it is named `gemini` or lives under `@google/gemini-cli/`.
...
`../../../../../bin/xo-spawn.sh` writes an XO-owned per-task settings file at `state/<id>.gemini-settings.json` with three hooks bound to the minted busy generation, and the launch reaches it through `GEMINI_CLI_SYSTEM_SETTINGS_PATH`.
...
Hook arrays MERGE across Gemini's settings layers rather than overriding, so a project's own hooks still run alongside XO's; both were observed firing for one turn.
`../../../../../bin/xo-teardown.sh` removes the file, so nothing survives into a pooled worktree.
...
`~/.agents/skills/no-mistakes` is therefore discovered as a user skill and loads even in an untrusted folder, which is what keeps XO's delivery path available.
Workspace skills need the workspace trust the launch already grants, which is what makes an XO-repo task's own `.agents/skills` reachable.
