...
`../../../bin/xo-claude-trust.sh` records `hasTrustDialogAccepted` for that worktree path in `${CLAUDE_CONFIG_DIR:-$HOME}/.claude.json`, and `../../../bin/xo-spawn.sh` refuses the spawn when the write fails rather than launching a worker that would wedge.

Never try to answer the trust dialog with a key.
XO's key plane carries only Enter, Escape, and C-c with no arrow navigation, so it cannot move a dialog's selection at all, and the observed rendering starts on `No, exit`, which means a sent Enter ends the session instead of accepting.
...
XO cannot move a selection with Enter, Escape, and C-c alone, so it cannot accept this dialog at all, and an operator accepts it once per machine instead.
...
As defense in depth, `xo_composer_strip_ghost` in `../../../bin/xo-composer-lib.sh` removes SGR-2 runs before pending classification on styled tmux, Herdr, and Zellij readers.
...
Styled capture stays internal to the boolean detector; `xo-peek` and model-facing captures remain plain, without escapes.
...
The controls are scoped to the launched process and never modify the captain's global Claude settings; `launch_template()` in `../../../../../bin/xo-spawn.sh` owns their exact mechanics and defense-in-depth rationale.
...
Primary `.claude/settings.json` registers `../../../bin/xo-turnend-guard.sh --claude` and `../../../bin/xo-claude-stop-autoarm.sh` with `asyncRewake: true` and `timeout: 28800`.
...
Project `.claude/settings.json` loads only when the exact project root is the session root; Claude does not search parents, so XO starts at repository root.
...
The Stop-owned watcher hook runs every Stop, foregrounds `../../../bin/xo-watch-arm.sh` only when eligible, and uses exit-2 async reawakening as notification.
...
`../../../bin/xo-subagent-pretool-check.sh` denies delegation-shaped tool names.
...
`../../../docs/subagent-guard.md` owns the contract, recommendation, `XO_ALLOW_SUBAGENT=1`, and applicability review.
