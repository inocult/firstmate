...
| Fact | Value |
|---|---|
| Busy state | The XO-owned extension's `agent_start` marks busy and `agent_settled`, confirmed by `ctx.isIdle()`, marks idle; this covers retries, compaction, tool loops, and queued continuations. |
| Exit command | `/quit`. |
...
Native Codex sessions may request `ultra` through the native extension flag described by `../../../bin/xo-spawn.sh`; it is separate from Pi's thinking levels.
...
Fullscreen can bury steering messages by rewriting scrollback, so XO avoids it when the installed CLI supports the override.
`../../../bin/xo-spawn.sh --help` owns the executable-pinning and version-safe launch mechanics.

Pi-signed is the signed wrapper identity verified on version 0.82.0.
XO records `pi-signed` without normalization and refuses rather than falling back to `pi` when that wrapper is unavailable.
...
`../../../bin/xo-spawn.sh` keeps the worker turn-end extension in `state/`, outside the worktree, because project-local extension files worsen the trust gate and pollute the project.
...
Native-harness progress uses the separate generation-bound marker owned by `../../../bin/xo-busy-event.sh`; it never fabricates Pi turn completion.
...
`.pi/extensions/xo-primary-turnend-guard.ts` listens for logical-run `agent_settled`, not per-tool-loop `turn_end`, and uses `pi.sendUserMessage(..., { deliverAs: "followUp" })` to force one guarded follow-up when `../../../bin/xo-turnend-guard.sh` returns 2.
...
The primary watcher protocol also requires `.pi/extensions/xo-primary-pi-watch.ts`.
The Pi engine auto-discovers both tracked project-local extensions once the project is trusted.
The model arms through the `xo_watch_arm_pi` tool, never through a foreground shell arm.
Native-harness adapters can discover the same guarded XO tools and operational message allowlist through the public Pi event-bus contract in `.pi/extensions/lib/xo-native-contract.ts`; no Pi built-in tools cross that contract.
The tool result and clean-exit fallback are owned by `../../../docs/supervision-protocols/pi.md`.
`../../../bin/xo-session-start.sh` reports when the live Pi-family session has not loaded both extensions and points at the selected executable after project trust as the fix, with `-e` as a trust-free fallback.

When a secondmate is launched on Pi or Pi-signed, `../../../bin/xo-spawn.sh --secondmate` launches the selected executable with both `-e .pi/extensions/xo-primary-turnend-guard.ts` and `-e .pi/extensions/xo-primary-pi-watch.ts`.
