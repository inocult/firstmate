// XO primary watcher bridge for omp (Oh My Pi).
//
// A port of .pi/extensions/xo-primary-pi-watch.ts for the omp fork. The arm,
...
//     hooks exist.
//   - The arming tool is xo_watch_arm_omp and its human fallback
//     /xo-watch-arm-omp; the loaded-build marker is state/.omp-watch-extension-loaded.
//
...
// resolves bin/xo-operational-input.sh relative to its own location, which is
...
const armScript = `${fmRoot}/bin/xo-watch-arm.sh`;
...
const retryBaseMs = positiveInteger("XO_WATCH_REARM_RETRY_BASE_MS", 250);
const retryMaxMs = positiveInteger("XO_WATCH_REARM_RETRY_MAX_MS", 4000);
const retryLimit = positiveInteger("XO_WATCH_REARM_RETRY_LIMIT", 5);
// 35s on Windows so the budget stays above arm's MSYS confirm default (30s in
// bin/xo-watch-arm.sh): a slow but successful Git Bash cold start must not be
// SIGTERMed mid-confirmation. Conditioned on win32 so other platforms keep 12s.
const armReadyTimeoutMs = positiveInteger(
  "XO_OMP_ARM_READY_TIMEOUT_MS",
  process.platform === "win32" ? 35000 : 12000,
);
const armRetireTimeoutMs = positiveInteger("XO_WATCH_ARM_RETIRE_TIMEOUT_MS", 1000);
const repairOnlyHint = "call xo_watch_arm_omp again only after a later notification says the cycle is missing, failed, or unhealthy";
...
type ReplacementCoordinatorGlobal = typeof globalThis & {
  __xoOmpWatchReplacements?: Map<string, ReplacementCoordinator>;
};
const replacementCoordinatorGlobal = globalThis as ReplacementCoordinatorGlobal;
const replacementCoordinators = replacementCoordinatorGlobal.__xoOmpWatchReplacements ??= new Map<string, ReplacementCoordinator>();
...
function classifyClose(stdout: string, stderr: string, code: number | null, signal: NodeJS.Signals | null): CloseClassification {
  ...
  if (code && code !== 0) {
    return {
      kind: "failure",
      message: `watcher: FAILED - xo-watch-arm.sh exited ${code}${combined ? `\n${combined}` : ""}`,
    };
  }
  return {
    kind: "failure",
...
export default function (pi: ExtensionAPI) {
    ...
    try {
      const result = spawnSync(
        ...
        [armScript, "--handling-delivered", recovery.generation, "--watcher-pid", recovery.watcherPid],
        {
          cwd: fmRoot,
          encoding: "utf8",
          env: { ...process.env, XO_HOME: fmHome, XO_STATE_OVERRIDE: state, XO_ROOT_OVERRIDE: fmRoot },
        },
      );
  ...
  async function processPendingActionables(owner: SessionGeneration): Promise<void> {
    ...
    try {
      ...
      if (generationIsLive(owner)) {
        ...
        // bounded retries, which hands repair to main through xo_watch_arm_omp
  ...
  function startArm(owner: SessionGeneration, predecessorArmPid = ""): ArmResult {
    ...
    const ownership = lockOwnership();
    if (ownership === "other") return { ok: false, message: "watcher: read-only - session lock is held by another xo session" };
    if (ownership === "missing") {
      return {
        ok: false,
        message: "watcher: not armed - no live session holds the lock; run bin/xo-session-start.sh to reclaim it, then call xo_watch_arm_omp to re-arm",
      };
    }
    markLoaded();
    if (owner.child) {
    ...
    const env = {
      ...
      XO_WATCH_ARM_SCRIPT: armScript,
      XO_WATCH_PREDECESSOR_ARM_PID: predecessorArmPid,
    };
    const armChild = spawn("bash", ["-lc", "config_dir=\"${XO_CONFIG_OVERRIDE:-$XO_HOME/config}\"; [ -f \"$config_dir/x-mode.env\" ] && . \"$config_dir/x-mode.env\"; exec \"$XO_WATCH_ARM_SCRIPT\" --restart"], {
      cwd: fmRoot,
      env,
  ...
  pi.registerCommand?.("xo-watch-arm-omp", {
    description: "Arm xo watcher supervision through the omp extension instead of foreground bash.",
  ...
  pi.registerTool?.({
    name: "xo_watch_arm_omp",
    label: "Arm xo watcher",
    description: "Start the first required omp watcher cycle, or repair one only after a notification says the cycle is missing, failed, or unhealthy. Do not call after ordinary work or ordinary notifications; the omp extension re-arms automatically. Never run bin/xo-watch-arm.sh through bash.",
    ...
    promptGuidelines: [
      "Call xo_watch_arm_omp only for the first required cycle or after a notification says the cycle is missing, failed, or unhealthy. Do not call it after ordinary work, turn completion, or ordinary signal, stale, check, or heartbeat handling because the omp extension owns re-arming. Never run bin/xo-watch-arm.sh through bash.",
    ],
