// XO turn-end guard, pre-tool seatbelts, and native session-start
// delivery for the omp (Oh My Pi) primary.
//
// A port of .pi/extensions/xo-primary-turnend-guard.ts with the turn-end
...
// bin/xo-turnend-guard.sh reads exactly as it reads Claude's payload, bounding
...
// supervisor-initiated interrupt is deliberately unguarded (bin/xo-control.sh
...
// Shared with the Pi extensions; the owner resolves bin/xo-operational-input.sh
// relative to its own location, which is this same repository root.
import {
  classifyXoCurrentOperationalText,
  encodeXoOperationalInput,
} from "../../.pi/extensions/lib/xo-operational-input.ts";
...
type SessionstartMessage = {
  customType: "xo-sessionstart-nudge";
  content: string;
  display: false;
...
function runSessionstartHook(generation: SessionstartGeneration): Promise<SessionstartResult> {
  return new Promise((resolveResult) => {
    ...
    const runner = `${root}/bin/xo-sessionstart-run.sh`;
    ...
    try {
      child = spawn(
        ...
        supervised
          ? [
              `${root}/.pi/extensions/lib/xo-sessionstart-supervisor.mjs`,
              runner,
              "--source",
              generation.source,
              "--pi-prerequisite",
...
function sessionstartMessage(
  ...
  try {
    ...
    const content = classifyXoCurrentOperationalText(raw)
      ? raw
      : encodeXoOperationalInput("session-start", raw);
    return {
      customType: "xo-sessionstart-nudge",
      content,
      display: false,
...
function runGuard(stopHookActive: boolean): Promise<{ code: number; stderr: string }> {
  return new Promise((resolveResult) => {
    const child = spawn(`${root}/bin/xo-turnend-guard.sh`, {
...
// PreToolUse seatbelts (bin/xo-arm-pretool-check.sh, docs/arm-pretool-check.md;
// bin/xo-cd-pretool-check.sh, docs/cd-guard.md). Both piggyback on this same
...
function runPretoolCheck(command: string): Promise<{ code: number; stderr: string }> {
  return runChecker("xo-arm-pretool-check.sh", command);
}

function runCdCheck(command: string): Promise<{ code: number; stderr: string }> {
  return runChecker("xo-cd-pretool-check.sh", command);
}
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on?.("session_stop", async (event) => {
    ...
    let content: string;
    try {
      content = encodeXoOperationalInput(
        "turn-end-guard",
